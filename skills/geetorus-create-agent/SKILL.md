...
description: >
  Create new agents in Geetorus with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Geetorus Create Agent Skill
...
```sh
curl -sS "$GEETORUS_API_URL/api/agents/me" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```

### 2. Discover adapter configuration for this Geetorus instance
...
`skills/geetorus-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/geetorus-create-agent/references/baseline-role-guide.md`
...
- for coding or execution agents, include the Geetorus execution contract: start actionable work in the same heartbeat; do not stop at a plan unless planning was requested; leave durable progress with a clear next action; use child issues for long or parallel delegated work instead of polling; mark blocked work with owner/action; respect budget, pause/cancel, approval gates, and company boundaries
...
`skills/geetorus-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `GEETORUS_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$GEETORUS_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"

curl -sS -X POST "$GEETORUS_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$GEETORUS_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$GEETORUS_API_URL/api/approvals/$GEETORUS_APPROVAL_ID" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"

curl -sS "$GEETORUS_API_URL/api/approvals/$GEETORUS_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
- Template index and how to apply a template: `skills/geetorus-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/geetorus-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/geetorus-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/geetorus-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/geetorus-create-agent/references/api-reference.md`
