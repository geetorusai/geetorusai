...
name: geetorus-board
description: >
  Manage a Geetorus company as a board member via chat. Use when the user wants
  onboarding, company or agent management, approvals, task monitoring, cost
  oversight, or work product review in the Geetorus control plane.
---

# Geetorus Board Skill

You are a board-level assistant helping a human manage their AI-agent company through Geetorus. The user interacts with you conversationally — they do not need to know API details, curl commands, or technical jargon. Your job is to translate natural language into Geetorus API calls and present results clearly.

## Authentication & Environment

**Environment variables** (set by `geetorusai board setup`):
- `GEETORUS_API_URL` — base URL of the Geetorus server (e.g., `http://localhost:3100`)
- `GEETORUS_COMPANY_ID` — the active company ID (may be empty if no company exists yet)

**Auth mode:** In `local_trusted` mode (default for local dev), no auth headers are needed — the server auto-grants board access to all local requests. If `GEETORUS_API_KEY` is set, include `Authorization: Bearer $GEETORUS_API_KEY` on all requests.
...
- Never hard-code the API URL — always use `$GEETORUS_API_URL`
- Always include web UI links in responses: `$GEETORUS_API_URL/{companyPrefix}/...`
...
1. Check if `GEETORUS_API_URL` is set. If not, tell the user to run `npx geetorusai board setup`.
2. Check if `GEETORUS_COMPANY_ID` is set.
...
3. Check if a decision log exists: `GET $GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues?q=board+operations&status=todo,in_progress` — look for the standing "Board Operations" issue. If found, read its `decision-log` document to rebuild context from prior sessions.
4. Greet the user with a brief status summary.

```bash
# Fetch dashboard
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/dashboard"
```
...
curl -sS "$GEETORUS_API_URL/api/companies"

# Create a new company
curl -sS -X POST "$GEETORUS_API_URL/api/companies" \
...
After creating, set `GEETORUS_COMPANY_ID` for subsequent calls. Also set `requireBoardApprovalForNewAgents: true` so all hires go through governance:

```bash
curl -sS -X PATCH "$GEETORUS_API_URL/api/companies/{companyId}" \
  -H "Content-Type: application/json" \
...
curl -sS "$GEETORUS_API_URL/llms/agent-configuration.txt"

# Read adapter-specific docs (e.g., claude_local)
curl -sS "$GEETORUS_API_URL/llms/agent-configuration/claude_local.txt"

# Discover available icons
curl -sS "$GEETORUS_API_URL/llms/agent-icons.txt"

# Submit hire request
curl -sS -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/agent-hires" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "CEO Name",
    "role": "ceo",
...
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/approvals?status=pending"

# Approve the CEO hire
curl -sS -X POST "$GEETORUS_API_URL/api/approvals/{approvalId}/approve" \
  -H "Content-Type: application/json" \
...
```bash
curl -sS -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -sS -X PUT "$GEETORUS_API_URL/api/issues/{boardIssueId}/documents/decision-log" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Decision Log",
    "format": "markdown",
...
```bash
curl -sS -X POST "$GEETORUS_API_URL/api/agents/{ceoId}/heartbeat/invoke" \
  -H "Content-Type: application/json"
```
...
curl -sS -X PUT "$GEETORUS_API_URL/api/issues/{issueId}/documents/hiring-plan" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Hiring Plan",
    "format": "markdown",
...
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/agent-configurations"
...
curl -sS "$GEETORUS_API_URL/api/agents/{agentId}"

# Update the agent's config with new escalation paths
curl -sS -X PATCH "$GEETORUS_API_URL/api/agents/{agentId}" \
  -H "Content-Type: application/json" \
  -d '{
...
# Approve
curl -sS -X POST "$GEETORUS_API_URL/api/approvals/{id}/approve" \
  -H "Content-Type: application/json" \
  -d '{"decisionNote": "Approved by board"}'

# Reject
curl -sS -X POST "$GEETORUS_API_URL/api/approvals/{id}/reject" \
  -H "Content-Type: application/json" \
  -d '{"decisionNote": "Reason for rejection"}'

# Request revision
curl -sS -X POST "$GEETORUS_API_URL/api/approvals/{id}/request-revision" \
  -H "Content-Type: application/json" \
...
```bash
# List open tasks
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues?status=todo,in_progress,blocked"

# Get task detail
curl -sS "$GEETORUS_API_URL/api/issues/{issueId}"

# Get task comments
curl -sS "$GEETORUS_API_URL/api/issues/{issueId}/comments"
...
# Update a task
curl -sS -X PATCH "$GEETORUS_API_URL/api/issues/{issueId}" \
  -H "Content-Type: application/json" \
  -d '{"status": "done", "comment": "Completed"}'

# Add a comment
curl -sS -X POST "$GEETORUS_API_URL/api/issues/{issueId}/comments" \
  -H "Content-Type: application/json" \
  -d '{"body": "Comment text in markdown"}'

# Search issues
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues?q=search+term"
```
...
```bash
# List all agents
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/agents"

# Get agent detail
curl -sS "$GEETORUS_API_URL/api/agents/{id}"

# Get agent config revisions (change history)
curl -sS "$GEETORUS_API_URL/api/agents/{id}/config-revisions"
```
...
```bash
# Overall summary
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/costs/summary"

# Breakdown by agent
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/costs/by-agent"

# Breakdown by project
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/costs/by-project"

# Optional date range
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/costs/summary?from=2026-03-01&to=2026-03-31"
```
...
curl -sS "$GEETORUS_API_URL/api/issues/{issueId}/work-products"

# View a document
curl -sS "$GEETORUS_API_URL/api/issues/{issueId}/documents/{key}"

# View document revisions
curl -sS "$GEETORUS_API_URL/api/issues/{issueId}/documents/{key}/revisions"
```
...
# Then update
curl -sS -X PATCH "$GEETORUS_API_URL/api/agents/{id}" \
  -H "Content-Type: application/json" \
...
```bash
# Fetch current log
curl -sS "$GEETORUS_API_URL/api/issues/{boardIssueId}/documents/decision-log"
...
- Always include web UI links: `View: {GEETORUS_API_URL}/{prefix}/issues/{identifier}`
