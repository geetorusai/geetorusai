...
usage() {
  cat <<'EOF'
Usage:
  geetorus-upload-artifact.sh FILE [options]

Uploads a generated file from the current workspace to the current Geetorus
issue, then creates an attachment-backed artifact work product by default.

Required environment for live uploads:
  GEETORUS_API_URL, GEETORUS_API_KEY, GEETORUS_COMPANY_ID, GEETORUS_TASK_ID, GEETORUS_RUN_ID

Options:
  --issue-id ID          Issue id to attach to (default: GEETORUS_TASK_ID)
  --company-id ID        Company id (default: GEETORUS_COMPANY_ID)
...
Examples:
  bash scripts/geetorus-upload-artifact.sh dist/demo.mp4 \
    --title "Demo video render" \
    --summary "MP4 render for board review"

  bash scripts/geetorus-upload-artifact.sh out/walkthrough.webm \
    --title "Walkthrough video" \
    --content-type video/webm

  bash scripts/geetorus-upload-artifact.sh out/result.png \
...
acquire_operation_lock() {
  ...
  umask 077
  operation_state_root="${GEETORUS_HELPER_STATE_DIR:-${TMPDIR:-/tmp}/geetorus-upload-artifact}"
...
file_path=""
issue_id="${GEETORUS_TASK_ID:-}"
company_id="${GEETORUS_COMPANY_ID:-}"
title=""
summary=""
content_type=""
...
if [[ -z "${GEETORUS_API_URL:-}" || -z "${GEETORUS_API_KEY:-}" || -z "${GEETORUS_RUN_ID:-}" ]]; then
  printf 'Missing GEETORUS_API_URL, GEETORUS_API_KEY, or GEETORUS_RUN_ID.\n' >&2
  exit 1
fi

if [[ -z "$issue_id" || -z "$company_id" ]]; then
  printf 'Missing issue or company id. Pass --issue-id/--company-id or set GEETORUS_TASK_ID/GEETORUS_COMPANY_ID.\n' >&2
  exit 1
fi

api_root="${GEETORUS_API_URL%/}"
case "$api_root" in
...
operation_key="$(
  sha256_text "$api_base|$company_id|$issue_id|$GEETORUS_RUN_ID|$original_filename|$file_sha256|$content_type"
)"
...
if [[ -z "$attachment" && -f "$unknown_upload_marker" && "$retry_unknown_upload" != "1" ]]; then
  printf '%s\n' 'A previous matching upload ended without a definitive response, and Geetorus has not exposed its durable attachment yet.' >&2
...
if [[ -n "$chat_comment" ]]; then
  printf -- '- Geetorus comment binding: saved. External publication requires an authorized active chat origin; this helper does not confirm provider delivery.\n'
fi
