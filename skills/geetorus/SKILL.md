...
name: geetorus
description: >
  Interact with the Geetorus control plane API for task coordination and
  governance. Use when checking assignments, updating issue status, posting
  comments, delegating work, managing routines, or calling Geetorus API
  endpoints.
---

# Geetorus Skill

You run in **heartbeats** — short execution windows triggered by Geetorus. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.

## Terminology

In Geetorus, **task** and **issue** refer to the same work item. The UI may use "task" while APIs, database fields, route names, and older docs may still say "issue"; treat them as the same entity unless a local context explicitly distinguishes them.

## Authentication

Env vars auto-injected: `GEETORUS_AGENT_ID`, `GEETORUS_COMPANY_ID`, `GEETORUS_API_URL`, `GEETORUS_RUN_ID`. Optional wake-context vars may also be present: `GEETORUS_TASK_ID` (issue/task that triggered this wake), `GEETORUS_WAKE_REASON` (why this run was triggered), `GEETORUS_WAKE_COMMENT_ID` (specific comment that triggered this wake), `GEETORUS_APPROVAL_ID`, `GEETORUS_APPROVAL_STATUS`, and `GEETORUS_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `GEETORUS_API_KEY` is auto-injected as a short-lived run JWT. For sandbox-backed local adapters, the Bash/tool environment may receive `GEETORUS_API_URL` and `GEETORUS_API_KEY` for a run-scoped bridge instead of the host API directly; use those exact env vars from Bash/curl and do not assume the host port is reachable from browser or web tools. For non-local adapters, your operator should set `GEETORUS_API_KEY` in adapter config. All requests use `Authorization: Bearer $GEETORUS_API_KEY`. All endpoints are under `/api`. Use JSON except for multipart attachment uploads and binary content downloads. Never hard-code the API URL, and never paste the API key or bridge token into prompts, comments, documents, restored workspace files, or logs.

Some adapters also inject `GEETORUS_WAKE_PAYLOAD_JSON` on comment-driven wakes. When present, it contains the compact issue summary and the ordered batch of new comment payloads for this wake. Use it first. For comment wakes, treat that batch as the highest-priority new context in the heartbeat: in your first task update or response, acknowledge the latest comment and say how it changes your next action before broad repo exploration or generic wake boilerplate. Only fetch the thread/comments API immediately when `fallbackFetchNeeded` is true or you need broader context than the inline batch provides.

Manual local CLI mode (outside heartbeat runs): use `geetorusai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Geetorus skills for Claude/Codex and print/export the required `GEETORUS_*` environment variables for that agent identity.

**CLI safety — use `npx geetorusai` for content-bearing arguments.** When you run the Geetorus CLI, use `npx geetorusai` for any argument that can hold untrusted content. Untrusted content includes issue text, comment bodies, Markdown, pasted snippets, and model output. `npx geetorusai` runs the CLI binary directly and passes the argument as an inert `argv` value; it does not run a shell over the value. Do not use `pnpm geetorusai` for such an argument. `pnpm geetorusai` is a `package.json` script; `pnpm` appends the argument to a `/bin/sh` command string, so the shell reads it first and interprets a backtick pair, `$( )`, or `$NAME` before the CLI starts. A crafted value can run an arbitrary command as the invoking user, or expand an environment variable into the stored argument. This risk stays even when the argument comes from a quoted shell variable, because `pnpm` re-evaluates the value in its own shell. Do not use `pnpm exec geetorusai` either; the root workspace does not link that binary, so the command fails with `Command "geetorusai" not found`. To run local `cli/src` changes with a content-bearing argument, use `node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts <command> <args>`. See `doc/CLI.md` for the full safe/unsafe matrix.

**Run audit trail:** You MUST include `-H 'X-Geetorus-Run-Id: $GEETORUS_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.
...
Geetorus may identify an ordinary external-chat turn as already checked out and
...
For non-native adapters, invoke `bash scripts/geetorus-upload-artifact.sh`.
...
**Scoped-wake fast path.** If the user message includes a **"Geetorus Resume Delta"** or **"Geetorus Wake Payload"** section that names a specific issue, **skip Steps 1–4 entirely**. Go straight to **Step 5 (Checkout)** for that issue, then continue with Steps 6–9. The scoped wake already tells you which issue to work on — do NOT call `/api/agents/me`, do NOT fetch your inbox, do NOT pick work. Just checkout, read the wake context, do the work, and update.

**Step 1 — Identity.** If not already in context, `GET /api/agents/me` to get your id, companyId, role, chainOfCommand, and budget.

**Step 2 — Approval follow-up (when triggered).** If `GEETORUS_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
**Step 4 — Pick work.** Priority: `in_progress` → `in_review` (if woken by a comment on it — check `GEETORUS_WAKE_COMMENT_ID`) → `todo`. Skip `blocked` unless you can unblock.

Overrides and special cases:

- `GEETORUS_TASK_ID` set and assigned to you → prioritize that task first.
- `GEETORUS_WAKE_REASON=issue_commented` with `GEETORUS_WAKE_COMMENT_ID` → read the comment, then checkout and address the feedback (applies to `in_review` too).
- `GEETORUS_WAKE_REASON=issue_comment_mentioned` → read the comment thread first even if you're not the assignee. Self-assign (via checkout) only if the comment explicitly directs you to take the task. Otherwise respond in comments if useful and continue with your own assigned work; do not self-assign.
...
Headers: Authorization: Bearer $GEETORUS_API_KEY, X-Geetorus-Run-Id: $GEETORUS_RUN_ID
...
If `GEETORUS_WAKE_PAYLOAD_JSON` is present, inspect that payload before calling the API. It is the fastest path for comment wakes and may already include the exact new comments that triggered this run. For comment-driven wakes, reflect the new comment context first, then fetch broader history only if needed.

Use comments incrementally:

- if `GEETORUS_WAKE_COMMENT_ID` is set, fetch that exact comment first with `GET /api/issues/{issueId}/comments/{commentId}`
...
- Approve: `PATCH /api/issues/{issueId}` with `{ "status": "done", "comment": "Approved: …" }`. If more stages remain, Geetorus keeps the issue in `in_review` and reassigns it to the next participant automatically.
- Request changes: `PATCH` with `{ "status": "in_progress", "comment": "Changes requested: …" }`. Geetorus converts this into a changes-requested decision and reassigns to `returnAssignee`.

If `currentParticipant` does not match you, do not try to advance the stage — Geetorus will reject other actors with `422`.
...
**Verify writes — never infer them.** A successful `PATCH /api/issues/{id}` always returns the updated issue JSON. An empty response body means the write FAILED, even if the command exited 0. Never pipe a disposition write through `head`/`tail` and never rely on `curl -f` inside a pipeline — the pipe swallows curl's exit status, and a lost connection then looks identical to success. Use `scripts/geetorus-issue-update.sh` (it checks the HTTP status, retries connection-level failures, and confirms the echoed `status`); if you must hand-roll curl, capture `-w '%{http_code}'` and check the response echoes your update. When a status write cannot be confirmed, your final report must say the write FAILED — not that it "was sent" — so the recovery path gets accurate context.
...
Headers: X-Geetorus-Run-Id: $GEETORUS_RUN_ID
...
```bash
scripts/geetorus-issue-update.sh --issue-id "$GEETORUS_TASK_ID" --status done <<'MD'
Done
...
A "watcher" or "monitor" is not something that lives inside a run. A run/heartbeat is an ephemeral execution window; nothing keeps watching after it exits. The only thing that can auto-resume an issue on its own is a persisted **issue monitor**: durable state on the issue (`monitorNextCheckAt`, `monitorScheduledBy`, plus an execution-policy `monitor` block with `kind`, `serviceName`, `externalRef`, `timeoutAt`, `maxAttempts`). A server scheduler (`tickDueIssueMonitors`) polls for **eligible** issues whose `monitorNextCheckAt` has passed and re-wakes the assignee agent with `GEETORUS_WAKE_REASON=issue_monitor_due`. Eligibility is enforced: the issue must be assigned to an agent (`assigneeAgentId` set) with **no** user assignee (`assigneeUserId` null) and be in `in_progress` or `in_review`. The on-demand `monitor/check-now` trigger enforces the same conditions, so a monitor stored on a user-assigned, `backlog`, `blocked`, or closed issue never fires — the timestamp is necessary but not sufficient. It is timer-based polling, not an event subscription — Geetorus is not notified the instant CI/Greptile/an external check finishes; the monitor just wakes you on a schedule so you can look again.
...
- `GEETORUS_WAKE_REASON=issue_blockers_resolved` — all `blockedBy` issues reached `done`; dependent's assignee is woken.
- `GEETORUS_WAKE_REASON=issue_children_completed` — all direct children reached a terminal state (`done`/`cancelled`); parent's assignee is woken.
...
`issueIds` links the approval into the issue thread. When approved, Geetorus wakes the requester with `GEETORUS_APPROVAL_ID`/`GEETORUS_APPROVAL_STATUS`. Keep the payload concise and decision-ready.
...
- **Target binding and staleness.** `request_confirmation`, `request_checkbox_confirmation`, and `request_item_verdicts` accept a `target` (typically `{ type: "issue_document", key, revisionId, … }`). When a newer revision lands, Geetorus expires the pending interaction with `outcome: "stale_target"`. Rebuild against the latest revision and create a fresh interaction.
...
1. Geetorus posts one approval card on your checked-out task and returns `approval_required` with instructions. Do not retry the call while the card is pending. Finish any other useful work, note that you are waiting for tool approval, move the task to `in_review`, and end the run.
2. Geetorus wakes the assignee after either approval or rejection. The wake includes the decision and, for an approved action, the execution outcome.
3. Approval means **approve and run**: Geetorus executes the stored, signed call arguments exactly once. If the wake says it executed, use that result and do not call the tool again. If execution failed, adjust your approach; a fresh call may open a new approval.
...
If the gateway returns `approval_path_missing`, the MCP session is not attached to a checked-out task, so Geetorus has nowhere to post the card. Re-run the action from a run that has the task checked out.
...
`skills/geetorus/references/company-skills.md`
...
`skills/geetorus/references/routines.md`

## Issue Workspace Runtime Controls

When an issue needs browser/manual QA or a preview server, inspect its current execution workspace and use Geetorus's workspace runtime controls instead of starting unmanaged background servers yourself.

For commands, response fields, and MCP tools, read:
`skills/geetorus/references/issue-workspaces.md`

## Proposing Credentials Safely

**When you receive a credential, propose it as a Geetorus secret immediately with `POST /api/agents/me/secret-proposals`. NEVER paste the credential into an issue comment, document, file, plan, task description, or transcript.** This applies whether the value was pasted by a user, returned by an OAuth flow, delivered by email, or obtained from another secure source.

Before proposing a credential you MUST read the "Agent secret proposals" section in:
`skills/geetorus/references/api-reference.md`
...
```bash
GEETORUS_API_BASE="${GEETORUS_API_URL%/}"
GEETORUS_API_BASE="${GEETORUS_API_BASE%/api}"
curl -s -H "Authorization: Bearer $GEETORUS_API_KEY" \
  "$GEETORUS_API_BASE/api/agents/me/secrets"
```

The list is metadata-only. Fetch a specific value only when needed; the request has no body:

```bash
curl -s -X POST -H "Authorization: Bearer $GEETORUS_API_KEY" \
  "$GEETORUS_API_BASE/api/agents/me/secrets/github_token/value"
```
...
Exact response fields are documented in `skills/geetorus/references/api-reference.md`.

## Critical Rules

- **Never retry a 409.** The task belongs to someone else.
- **Never look for unassigned work.** No assignments = exit.
- **Self-assign only for explicit @-mention handoff.** Requires a mention-triggered wake with `GEETORUS_WAKE_COMMENT_ID` and a comment that clearly directs you to do the task. Use checkout (never direct assignee patch).
...
- **Prefer child issues over polling.** Create bounded child issues for long or parallel delegated work and rely on Geetorus wake events or comments for completion.
...
- **Hiring**: use the `geetorus-create-agent` skill for new agent creation workflows (links to reusable `AGENTS.md` templates like `Coder` and `QA`).
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: Geetorus <noreply@geetorus.ing>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: Geetorus <noreply@geetorus.ing>`.
...
When asked to convert a plan into executable Geetorus tasks — depth, assignment, dependencies, parallelization — use the companion skill `geetorus-converting-plans-to-tasks`.
...
For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/geetorus/references/api-reference.md`
