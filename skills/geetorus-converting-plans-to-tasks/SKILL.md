...
description: >
  Convert Geetorus plans into executable issue graphs. Use when asked to plan,
  scope, or break down Geetorus company work into assigned tasks with specialty
  fit, dependencies, blockers, and parallelization.
---

# Geetorus — Converting Plans to Tasks

A companion skill for turning a plan into executable Geetorus work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Geetorus works for you.

For the **mechanics** of recording a plan (issue document with key `plan`, comment links, approval gating, who to reassign back to), follow the _Planning_ section of the `geetorus` skill. This skill covers planning method, not the API surface.
...
- Not a replacement for the `geetorus` skill's planning mechanics. Use both.
