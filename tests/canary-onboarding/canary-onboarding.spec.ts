...
const expectedVersion = process.env.GEETORUSAI_VERSION!;
...
test("the exact published canary installs and reaches Connect a model", async ({
  ...
  // Browser storage is scoped to the Geetorus origin, not to a data directory.
  ...
  await page.addInitScript(() => {
    localStorage.setItem("geetorus-onboarding-state", JSON.stringify({
      step: 1,
      companyName: "",
