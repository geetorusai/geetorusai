...
```sh
pnpm exec tsx cli/src/index.ts test-drive --data-dir /tmp/geetorus-hiring-qa --no-browser
```
...
`GEETORUS_RUNNER_API_TOOLS_ENABLED=true` in the disposable data directory's
`instances/default/.env`, then restart test-drive. Test-drive clears inherited
`GEETORUS_*` variables before loading that file. The existing opt-in managed API
...
managed API hiring, native `create_task` delegation, and `geetorus_finish`
...
HIRING_AI_LIVE=1 HIRING_AI_ENVIRONMENT=daytona \
  ...
  HIRING_AI_DAYTONA_IMAGE=ghcr.io/geetorusai/geetorus-daytona-runner@sha256:YOUR_DIGEST \
...
docker cp "$hiring_image_container:/opt/geetorus-runner/provider-pack" /tmp/hiring-native-artifacts/provider-pack
docker cp "$hiring_image_container:/usr/local/bin/geetorus-runnerd" /tmp/hiring-native-artifacts/geetorus-runnerd
...
```dotenv
GEETORUS_RUNNER_REMOTE_PROVIDER_PACK_PATH=/tmp/hiring-native-artifacts/provider-pack
GEETORUS_RUNNER_REMOTE_BINARY_PATH=/tmp/hiring-native-artifacts/geetorus-runnerd
```
...
pnpm --filter @geetorusai/ui exec storybook dev -p 6010 -c storybook/.storybook --ci --no-open
