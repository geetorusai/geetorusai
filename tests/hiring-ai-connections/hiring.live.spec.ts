...
const coordination = native
  ? `Use the native Geetorus tools. First get_task_context to verify the current task and your identity. For hiring use call_api with operationId POST /api/companies/{companyId}/agent-hires and the requested JSON body. Use create_task for subtasks. Do not use shell commands or raw HTTP. Never read or print credentials. For completion call geetorus_finish exactly once with reportedWorkDisposition done, summary describing the completed work, completionClaim {contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]}, evidence [], and verification []. Wait for that tool to succeed before your final response.`
  : `Use the injected GEETORUS_API_URL and GEETORUS_API_KEY for every API request. Include Authorization: Bearer $GEETORUS_API_KEY and X-Geetorus-Run-Id: $GEETORUS_RUN_ID. Never print credentials or the environment. Verify GET /api/agents/me identifies you before writes; stop if it does not. You must act as the agent, not the local board. Use standard Geetorus APIs or the managed API tool, and do not install a CLI. Do not read or modify unrelated files. For completion use PATCH /api/issues/$GEETORUS_TASK_ID with status done and a comment.`;
...
for (const source of ["anthropic", "openai"] as const) {
  test(`${environment}: ${runner}: ${providers[source].label} hires both providers, delegates, and repairs missing auth in the task`, async ({ page, request }, testInfo) => {
    ...
    if (environment === "daytona") {
      ...
      const installed = (await api("/plugins")).find((plugin: any) => plugin.packageName === "@geetorusai/plugin-daytona");
    ...
    const adapterType = (provider: Provider) => native ? "geetorus_runner" : providers[provider].adapterType;
    ...
    const simplePrompt = (kind: string) => `${coordination}\nCalculate 31*37 and use exactly ${proof(kind)} as your ${native ? "geetorus_finish summary and final response" : "completion comment"}. Mark this task done. No subtasks or files.`;
    ...
    const child = native
      ...
      : `Create a subtask with title ${JSON.stringify(selfTitle)}, parentId equal to GEETORUS_TASK_ID, assigneeAgentId ${manager.id}, status todo, and description ${JSON.stringify(simplePrompt("SELF"))}.`;
