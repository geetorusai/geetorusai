...
export interface RunnerTaskFixture {
  ...
  /** Restart the isolated Geetorus server after the waiting turn settles. */
...
export interface RunnerE2ERuntimeUsage {
  ...
  /** Sum of the selected Geetorus heartbeat-run spans. */
...
export interface RunnerE2EResult {
  schema: "geetorus.runner-e2e.result/v1" | "geetorus.runner-e2e.result/v2";
  executionId: string;
  suiteId?: string;
  suiteDefinitionHash?: string;
  source?: {
...
export interface RunnerE2EHistoryIndex {
  schema: "geetorus.runner-e2e.history/v1";
  updatedAt: string;
