...
import { deriveGeetorusViteHmrPort } from "../../packages/shared/src/runtime-exposure/ports.js";
...
export function runnerE2EServerPortConflictsWithDatabase(
  ...
  return (
    ...
    deriveGeetorusViteHmrPort(serverPort) === databasePort
  );
}
...
export async function reserveRunnerE2EServerPort(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      const hmrPort = deriveGeetorusViteHmrPort(serverReservation.port);
      try {
  ...
  throw new Error(
    `Failed to reserve a conflict-free Geetorus/Vite port pair after ${maxAttempts} attempts`,
  );
}
