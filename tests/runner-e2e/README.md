...
see the [Geetorus evaluation guide](../../doc/evals.md). This README is the
authoritative runbook for Product E2E runner cells; the separate Runner Evals
protocol guide lives at
`packages/geetorus-runner/docs/runner-protocol-live-evals.md`.

This is the billable browser acceptance campaign system for Geetorus runner
profiles. It is deliberately separate from `tests/e2e`: every independently
scheduled execution gets
a fresh Geetorus home, embedded Postgres database, instance configuration,
...
The launcher always sets `GEETORUS_ANNOUNCEMENTS_ENABLED=false` for its isolated
...
- `OPENROUTER_API_KEY`
- `DAYTONA_API_KEY`
- `GEETORUS_E2E_DAYTONA_IMAGE` (Daytona only)
...
Geetorus receives secret references in agent/environment payloads. Provider
keys, Daytona keys, `DATABASE_URL`, and `DATABASE_MIGRATION_URL` are removed
from the Geetorus child process.
...
List cells without loading credentials or starting Geetorus:
...
Geetorus server while the interaction is waiting, reloads that state, and
...
```bash
pnpm --filter @geetorusai/ui build
...
Geetorus starts. `--max-parallel <n>` controls the number of isolated
profile/environment/case harnesses that can overlap (default 1, also configurable
with `GEETORUS_E2E_MAX_PARALLEL`). Headed/UI/debug runs are forced to one worker.
...
multiple Geetorus/Postgres/Chromium stacks and can consume substantial CPU and
memory.
...
image="ghcr.io/geetorusai/geetorus-daytona-runner:e2e-content-${content_id}"
if ! docker buildx imagetools inspect "$image" >/dev/null 2>&1; then
  docker buildx build \
    --platform linux/amd64 \
    --build-arg "GEETORUS_RUNNER_CONTENT_ID=${content_id}" \
    --build-arg "GEETORUS_RUNNER_SOURCE_REVISION=${source_revision}" \
...
`geetorus-eval-kernel`, and `geetorus-runner`. Changes elsewhere in the
...
Resolve the manifest digest and set `GEETORUS_E2E_DAYTONA_IMAGE` to
`ghcr.io/geetorusai/geetorus-daytona-runner@sha256:...`. The repository
...
diagnostics, failure screenshot, and sanitized Geetorus/run logs when
...
time order, using Geetorus chat styling: user bubbles on the right, agent replies
...
```bash
gh run download <run-id> --repo geetorusai/geetorus --name github-pages --dir /tmp/runner-e2e-pages
...
Geetorus, invoke an agent, create a Daytona lease, or consume provider tokens.
...
3. scans the closed Geetorus home/database and workspace as streams;
...
The temporary Geetorus home, embedded database, raw workspace, master key,
...
any branch in `geetorusai/geetorus`. The authorization job resolves that
...
`runs-on/fleet=geetorus-public-pr-x64/env=public-ci`. Any other value uses the
...
`repo:geetorusai/geetorus:environment:runner-e2e-history`, and permit only
...
SVG/active content, databases, Geetorus homes, workspaces, raw/unallowlisted
logs, or credentials.

See [FIXTURES.md](./FIXTURES.md) before adding or changing a profile,
environment, task, matcher, or future Geetorus object fixture.
...
production switch removes the legacy Geetorus operational skill because Runner
...
suite checks persisted Geetorus effects; it does not claim to prove the absence
...
`GEETORUS_RUNNER_E2E_REPORT_ROOT` at that campaign,
`GEETORUS_RUNNER_E2E_REPORT_OUT` at a fresh output directory, and
`GEETORUS_RUNNER_E2E_EXPECTED_IDS` at the JSON array of selected execution IDs.
...
matched. Set distinct `GEETORUS_E2E_CAMPAIGN_ID` values such as
...
GEETORUS_PLAYWRIGHT_CHANNEL=chrome pnpm test:e2e:runner:browser-support
```
