...
describe("runner E2E report aggregation", () => {
  it("keeps interrupted journeys incomplete unless their evidence is invalid", async () => {
    ...
    for (const [caseId, validEvidence] of [["task-card-accept", true], ["task-reply-accept", false]] as const) {
      ...
      await writeFile(path.join(directory, "result.json"), JSON.stringify({
        schema: "geetorus.runner-e2e.result/v2", executionId, suiteId: "first-task",
    ...
    await expect(execFileAsync(process.execPath, [path.join(repositoryRoot, "cli/node_modules/tsx/dist/cli.mjs"), path.join(repositoryRoot, "tests/runner-e2e/report.ts")], {
      cwd: repositoryRoot, env: { ...process.env, GEETORUS_RUNNER_E2E_REPORT_ROOT: root, GEETORUS_RUNNER_E2E_REPORT_OUT: output, GEETORUS_RUNNER_E2E_EXPECTED_IDS: JSON.stringify(ids) },
    })).rejects.toBeDefined();
  ...
  it("selects the latest retry and enforces cleanup and pass evidence", async () => {
    ...
    const base: RunnerE2EResult = {
      schema: "geetorus.runner-e2e.result/v1",
      executionId,
      attempt: 2,
      status: "passed",
    ...
    await execFileAsync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          GEETORUS_RUNNER_E2E_REPORT_ROOT: root,
          GEETORUS_RUNNER_E2E_REPORT_OUT: output,
          GEETORUS_RUNNER_E2E_EXPECTED_IDS: JSON.stringify([executionId]),
          GEETORUS_RUNNER_E2E_SOURCE_SHA:
            "0123456789abcdef0123456789abcdef01234567",
          GEETORUS_RUNNER_E2E_SOURCE_REF:
          ...
          GEETORUS_RUNNER_E2E_HISTORY_PUBLIC_BASE_URL:
            "https://reports.example.test/",
          GEETORUS_RUNNER_E2E_HISTORY_PREFIX: "/runner-e2e/",
        },
      },
    );
    ...
    expect(normalized).toMatchObject({
      schema: "geetorus.runner-e2e.campaign/v2",
      selected: 1,
      executed: 1,
      passed: 1,
      failed: 0,
      ...
      source: {
        ...
        workflowRunUrl:
          "https://github.com/geetorusai/geetorus/actions/runs/123456",
      },
    });
    ...
    expect(summary).toContain(
      "[Open the workflow run and per-cell job logs](https://github.com/geetorusai/geetorus/actions/runs/123456)",
    );
    expect(summary).toContain(
      "[Download the merged report and per-cell evidence](https://github.com/geetorusai/geetorus/actions/runs/123456#artifacts)",
    );
    expect(summary).toContain(
