...
export async function setupFirstTaskFixtures(input: {
  ...
  if (execution.profile.generation === "native") {
    ...
    expect(migrated.adapterType).toBe("geetorus_runner");
    ...
    expect(migrated.adapterConfig?.geetorusSkillSync?.desiredSkills).toEqual(
      (
        runtimePatch.adapterConfig.geetorusSkillSync as {
...
export async function runFirstTaskFlow(input: {
  ...
  try {
    ...
    const desired =
      agent.adapterConfig?.geetorusSkillSync?.desiredSkills ?? [];
    expect(
    ...
    if (scenario.opening === "ordinary") {
      ...
      await page
        // Geetorus includes the option description in the accessible name.
