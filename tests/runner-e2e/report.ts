...
async function main() {
  const root = path.resolve(
    process.env.GEETORUS_RUNNER_E2E_REPORT_ROOT ?? "tests/runner-e2e/results",
  );
  const output = path.resolve(
    process.env.GEETORUS_RUNNER_E2E_REPORT_OUT ?? path.join(root, "merged"),
  );
  const expectedInput = JSON.parse(
    process.env.GEETORUS_RUNNER_E2E_EXPECTED_IDS ?? "[]",
  ) as string[];
  if (
    ...
    throw new Error(
      "GEETORUS_RUNNER_E2E_EXPECTED_IDS must be a JSON string array",
    );
  }
  const expected = expectedInput.map(canonicalExecutionId);
  if (expected.length === 0 || new Set(expected).size !== expected.length) {
    throw new Error(
      "GEETORUS_RUNNER_E2E_EXPECTED_IDS must contain unique selected executions",
    );
  }
  ...
  for (const resultFile of resultFiles) {
    ...
    if (
      parsed.schema !== "geetorus.runner-e2e.result/v1" &&
      parsed.schema !== "geetorus.runner-e2e.result/v2"
    )
      continue;
  ...
  const summaryLinks = runnerE2ESummaryLinks({
    ...
    historyPublicBaseUrl:
      process.env.GEETORUS_RUNNER_E2E_HISTORY_PUBLIC_BASE_URL,
    historyPrefix: process.env.GEETORUS_RUNNER_E2E_HISTORY_PREFIX,
  });
