  ...
  try {
    ...
    if (
      ...
      if (caseId === "continuity-restart") {
        ...
        expect(
          String(fresh.contextSnapshot?.geetorusTaskMarkdown ?? ""),
      ...
      if (caseId === "clarify-reuse") {
        ...
        const clarification = `It is the garden club; use the existing Garden ${nonce} project. Make one assigned task for yourself to write a two-sentence welcome note. Include ${marker} in that note, save it as a Geetorus document attached to that execution task, and finish that execution task. Please get it started now.`;
        ...
        await page
          ...
          .first()
          .fill(
            `Revise the plan: the finished welcome note itself must contain the exact phrase ${marker} instead of ${draftMarker}. Include that requirement in the revised plan. The execution task should save that welcome note as a Geetorus document attached to that task. Present this revised plan for approval; wait for that approval before handing it off as agreed.`,
          );
        await reviseButton.click();
        await idle(2);
