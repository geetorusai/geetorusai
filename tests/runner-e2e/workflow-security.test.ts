...
describe("public repository paid workflow security", () => {
  it("uses the reviewed master branch for the first-party trusted PR workflow", async () => {
    ...
    const trustedWorkflowCalls = [
      ...ordinaryPrWorkflow.matchAll(
        /^\s+uses:\s+(geetorusai\/geetorus\/\.github\/workflows\/pr-trusted\.yml)@([^\s#]+)$/gmu,
      ),
    ];

    expect(trustedWorkflowCalls).toHaveLength(1);
    expect(trustedWorkflowCalls[0]?.[1]).toBe(
      "geetorusai/geetorus/.github/workflows/pr-trusted.yml",
    );
  ...
  it("gates every provider-secret job with stable actor IDs", async () => {
    ...
    expect(authorizeJob).toContain(
      "aws_runner='runs-on/fleet=geetorus-public-pr-x64/env=public-ci'",
    );
    ...
    expect(paidJob).toContain(
      "GEETORUS_PLAYWRIGHT_CHANNEL: ${{ needs.authorize.outputs.playwright_channel }}",
    );
    ...
    expect(preparedBeforeProviderAccess).toContain(
      '"@geetorusai/plugin-daytona"',
    );
    expect(preparedBeforeProviderAccess).toContain('"@geetorusai/plugin-sdk"');
    expect(preparedBeforeProviderAccess).toContain(
      'realpath "$daytona_root/node_modules/@geetorusai/plugin-sdk"',
    );
    expect(preparedBeforeProviderAccess).toContain(
    ...
    expect(fullStack).toContain(
      "GEETORUS_RUNNER_SOURCE_REVISION=${TARGET_SHA}",
    );
    ...
    for (const targetProvenanceJob of [paidJob, reportJob]) {
      expect(targetProvenanceJob).toContain(
        "GEETORUS_RUNNER_E2E_SOURCE_SHA: ${{ needs.authorize.outputs.target_sha }}",
      );
      expect(targetProvenanceJob).toContain(
        "GEETORUS_RUNNER_E2E_SOURCE_REF: ${{ needs.authorize.outputs.target_ref }}",
      );
    }
  ...
  it("builds runner outputs once without provider credentials and verifies them in every paid cell", async () => {
    ...
    expect(buildJob).toContain(
      "pnpm --filter @geetorusai/geetorus-runner build:typescript",
    );
    expect(buildJob).toContain(
      "pnpm --filter @geetorusai/geetorus-runner build:runner-binaries",
    );
    expect(buildJob).toContain(
      "node packages/geetorus-runner/scripts/build-provider-pack.mjs",
    );
    expect(buildJob).toContain(
      "node packages/geetorus-runner/scripts/materialize-opencode-binary.mjs",
    );
    ...
    expect(testJob).toContain(
      "test -x packages/geetorus-runner/runner/target/debug/geetorus-runnerd",
    );
  ...
  it("uses the reviewed AWS Chrome channel without weakening the local executable override", async () => {
    ...
    expect(config).toContain(
      "process.env.GEETORUS_PLAYWRIGHT_CHANNEL?.trim()",
    );
  ...
  it("binds rerun evidence and Pages artifacts to the exact workflow attempt", async () => {
    ...
    expect(report).toContain(
      "GEETORUS_RUNNER_E2E_REPORT_ROOT: ${{ github.workspace }}/selected-runner-e2e",
    );
    expect(report).toContain(
      "GEETORUS_RUNNER_E2E_HISTORY_PUBLIC_BASE_URL: ${{ vars.RUNNER_E2E_HISTORY_PUBLIC_BASE_URL }}",
    );
    expect(report).toContain(
      "GEETORUS_RUNNER_E2E_HISTORY_PREFIX: ${{ vars.RUNNER_E2E_HISTORY_PREFIX || 'runner-e2e' }}",
    );
    expect(
