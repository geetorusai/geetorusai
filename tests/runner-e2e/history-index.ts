...
function sourceCell(campaign: RunnerE2EHistoryCampaign) {
  ...
  const shaLabel = /^[0-9a-f]{40}$/i.test(sha ?? "")
    ? `<a href="https://github.com/geetorusai/geetorus/commit/${html(sha)}">${html(shortSha)}</a>`
    : html(shortSha);
...
<head>
  ...
  <title>Runner E2E Campaigns · Geetorus</title>
  <style>
    @font-face { font-family: "Geetorus Inter"; src: url("assets/InterVariable.woff2") format("woff2"); font-style: normal; font-weight: 100 900; font-display: swap; }
    :root { color-scheme: light dark; --background:#fff; --foreground:#0a0a0a; --muted:#60666a; --border:#e5e5e5; --raised:#fafafa; --pass:#188a3c; --pass-bg:#dcfce7; --fail:#991b1b; --fail-bg:#fee2e2; --font:"Geetorus Inter",Inter,ui-sans-serif,system-ui,-apple-system,sans-serif; --mono:ui-monospace,SFMono-Regular,Menlo,monospace; }
...
<body>
  <div class="brand-bar">
    <a class="brand-lockup" href="https://geetorus.ing" aria-label="Geetorus home">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="7.5" stroke="currentColor" stroke-width="3.4" fill="none" /></svg>
      <span>Geetorus</span>
    </a>
  ...
  <main>
    <header>
      <div>
        ...
        <p class="lede">Each row is one workflow campaign against a Geetorus revision. Open a report for its configuration matrices, matchers, per-test billing, declared screenshots, and sanitized structured evidence. Additional diagnostic evidence remains in access-controlled workflow artifacts.</p>
      </div>
