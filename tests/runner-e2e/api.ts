...
export class RunnerApi {
  ...
  constructor(readonly request: APIRequestContext) {
    const port = process.env.GEETORUS_RUNNER_E2E_PORT?.trim();
    if (!port) throw new Error("GEETORUS_RUNNER_E2E_PORT is required");
   ...
   * never serialized into trace/blob evidence before Geetorus encrypts it.
   */
