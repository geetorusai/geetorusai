...
import { QUALIFIED_ACPX_PROFILES } from "../../packages/geetorus-runner/src/drivers/acpx/qualified-profiles.js";
import { QUALIFIED_OPENCODE_MODEL } from "../../packages/geetorus-runner/src/drivers/opencode/opencode-server-driver.js";
...
function commonAgent(
  ...
  return {
    ...
    instructionsBundle: {
      ...
      files: {
        "AGENTS.md": [
          "You are running a paid Geetorus end-to-end acceptance fixture.",
          "Follow the assigned task and its Geetorus work mode literally.",
          ...
          "Legacy adapters must use the public Geetorus API and the injected GEETORUS_API_URL, GEETORUS_API_KEY, GEETORUS_TASK_ID, and GEETORUS_RUN_ID values for comments, documents, interactions, and status changes.",
          ...(adapterType === "geetorus_runner"
            ? []
            : [
                'For a planning task, do not inspect the OpenAPI schema. PUT /api/issues/$GEETORUS_TASK_ID/documents/plan with {title:"Plan",format:"markdown",body,changeSummary}; read latestRevisionId and latestRevisionNumber from that response. Then POST /api/issues/$GEETORUS_TASK_ID/interactions with {kind:"request_confirmation",continuationPolicy:"wake_assignee",payload:{version:1,prompt,acceptLabel:"Approve",rejectLabel:"Reject",rejectRequiresReason:true,target:{type:"issue_document",key:"plan",revisionId,revisionNumber}}}, and PATCH the issue to {status:"in_review"}. Include Authorization and X-Geetorus-Run-Id on every write.',
              ]),
...
function nativeProfile(input: {
  ...
  return {
    ...
    buildAgent(buildInput) {
      ...
      return commonAgent(buildInput, input.id, "geetorus_runner", {
        provider: input.provider,
        model: input.model,
  ...
  },
  {
    ...
    buildEnvironment(input) {
      if (!isImmutableDaytonaImage(input.daytonaImage)) {
        throw new Error(
          "GEETORUS_E2E_DAYTONA_IMAGE must be an immutable image digest",
        );
      }
      return {
...
export const runnerTasks: readonly RunnerTaskFixture[] = [
  {
    ...
    buildTitle: (nonce) => `Runner E2E GEETORUS_E2E_OK_${nonce}`,
    buildVisibleMarker: (nonce) => `GEETORUS_E2E_OK_${nonce}`,
    buildPrompt: (nonce) =>
      [
        "Complete this task in a single run.",
        `The exact marker also appears unescaped in the task title: GEETORUS_E2E_OK_${nonce}`,
        `Your final visible task-thread response must be exactly this marker: GEETORUS_E2E_OK_${nonce}`,
        `In a native runner, call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"GEETORUS_E2E_OK_${nonce}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Wait for that tool call to succeed, then emit exactly GEETORUS_E2E_OK_${nonce} once as the complete user-facing final response. Do not write a user-facing final response before geetorus_finish succeeds, and do not call another tool.`,
        `In a legacy runner, make exactly one public-API write containing the marker: PATCH /api/issues/$GEETORUS_TASK_ID with {"status":"done","comment":"GEETORUS_E2E_OK_${nonce}"}. Do not POST to /comments, and do not include the marker in any other write.`,
    ...
    buildMatchers(nonce, execution) {
      return [
        { kind: "message_exact", expected: `GEETORUS_E2E_OK_${nonce}` },
        {
          kind: "message_occurrences",
          expected: `GEETORUS_E2E_OK_${nonce}`,
          count: 1,
        },
        {
          kind: "issue_status",
  ...
  },
  {
    ...
    buildVisibleMarker: (nonce) => `GEETORUS_E2E_PLAN_DONE_${nonce}`,
    buildPlanMarkers: (nonce) => ({
      draft: `GEETORUS_E2E_PLAN_DRAFT_${nonce}`,
      revised: `GEETORUS_E2E_PLAN_REVISED_${nonce}`,
    }),
    buildRevisionRequest: (nonce) =>
      [
        "Revise this same plan; do not implement it yet.",
        `Remove GEETORUS_E2E_PLAN_DRAFT_${nonce} and include GEETORUS_E2E_PLAN_REVISED_${nonce}.`,
        ...
        "In a native runner, call write_document for key `plan`, then call request_human_input exactly once with interactionKind `confirmation`, targetRevisionId set to the returned latest Plan revision, and continuationPolicy `wake_assignee`; do not call geetorus_finish while waiting. After write_document succeeds, request_human_input must be your immediate next action using that call's returned latestRevisionId; do not emit assistant text, end the response or heartbeat, or stop after write_document alone.",
        "In a legacy runner, first GET the current `plan` issue document, then PUT the revised Plan with `baseRevisionId` set to that response's `latestRevisionId`; after the update succeeds, create the equivalent request_confirmation targeting the newly returned `latestRevisionId` through the public Geetorus API.",
      ].join(" "),
    buildPrompt: (nonce) =>
      [
        ...
        `The initial Plan body must contain GEETORUS_E2E_PLAN_DRAFT_${nonce}.`,
        ...
        `Before the revised Plan is accepted, do not spell, quote, repeat, announce, or include GEETORUS_E2E_PLAN_DONE_${nonce} in any visible response, comment, or tool payload; refer to it only as “the terminal marker.”`,
        "Only after the revised plan is accepted, implement it by posting one final visible task-thread response containing exactly " +
          `GEETORUS_E2E_PLAN_DONE_${nonce}` +
          " and mark the task Done.",
        `For a native runner, remain in the requested planning collaboration mode. Call write_document for key \`plan\`, then call request_human_input exactly once with interactionKind \`confirmation\`, targetRevisionId set to the returned latest Plan revision, and continuationPolicy \`wake_assignee\`. For both the initial Plan and the revised Plan, those two tool calls form one indivisible response sequence: immediately after write_document succeeds, request_human_input must be your next action using that call's returned latestRevisionId. Do not emit assistant text, end the response or heartbeat, or stop after write_document alone before the matching confirmation request succeeds. Do not call geetorus_finish while waiting for either Plan confirmation. When an acceptance wake arrives, first call get_task_context. Treat the wake as valid only when that control-plane result is for the current task and identifies the exact revised Plan revision used as the confirmation target as accepted; otherwise do not finish and continue waiting for the matching revision-bound confirmation. After that verification succeeds, your immediate next action must be the geetorus_finish tool call. Do not call list_documents or any other tool, and do not emit any assistant text, acknowledgement, progress note, or preamble between verification and geetorus_finish. Call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"GEETORUS_E2E_PLAN_DONE_${nonce}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Wait for that tool call to succeed, then emit only GEETORUS_E2E_PLAN_DONE_${nonce} as the complete final response. Do not write a user-facing final response before geetorus_finish succeeds, and do not call another tool.`,
        `For a legacy runner, use the public Geetorus API. The first PUT of the \`plan\` issue document creates it. For every later PUT, first GET the current document and set \`baseRevisionId\` to its \`latestRevisionId\`; a 409 means you must GET again and retry with the new latest revision. Create a \`request_confirmation\` targeting the successful PUT response's \`latestRevisionId\` with \`continuationPolicy: wake_assignee\`, and move the issue to \`in_review\` while waiting. After the revised Plan is accepted, write GEETORUS_E2E_PLAN_DONE_${nonce} exactly once through one atomic issue PATCH with status \`done\` and that exact comment; do not POST a separate comment or perform a second write.`,
    ...
    buildMatchers(nonce, execution) {
      return [
        {
          kind: "message_exact",
          expected: `GEETORUS_E2E_PLAN_DONE_${nonce}`,
        },
        {
          kind: "message_occurrences",
          expected: `GEETORUS_E2E_PLAN_DONE_${nonce}`,
  ...
  },
  {
    ...
    buildPrompt: (nonce) =>
      [
        ...
        `In a native runner, call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"E2E_ASK_12_${nonce}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. For the entire run, geetorus_finish must be your only tool call: never call report_progress or any other tool before or after it. Wait for that tool call to succeed, then emit exactly E2E_ASK_12_${nonce} as the complete final response. Do not write a user-facing final response before geetorus_finish succeeds.`,
        `In a legacy runner, make exactly one public-API write containing the marker: PATCH /api/issues/$GEETORUS_TASK_ID with {"status":"done","comment":"E2E_ASK_12_${nonce}"}. Do not POST to /comments, do not PATCH the status separately, and do not include the marker in any other API write.`,
      ].join("\n"),
    buildMatchers(nonce, execution) {
      return [
        {
...
const structuredQuestionResumeTask = {
  ...
  buildVisibleMarker: (nonce) => `GEETORUS_E2E_QUESTION_DONE_${nonce}`,
  buildQuestionAnswer: (nonce) => ({
    optionLabel: "Cobalt",
    expectedMarker: `GEETORUS_E2E_QUESTION_DONE_${nonce}`,
  }),
  buildPrompt: (nonce) =>
    [
      ...
      `Before the answer arrives, do not spell, quote, repeat, announce, or include GEETORUS_E2E_QUESTION_DONE_${nonce} in any visible response, comment, or tool payload; refer to it only as “the terminal marker.”`,
      ...
      'In a legacy runner, derive `API_ORIGIN` exactly once with `API_ORIGIN="${GEETORUS_API_URL%/}"; API_ORIGIN="${API_ORIGIN%/api}"`. Build every endpoint as `$API_ORIGIN/api/...`; never append `/api` to a base that already ends in `/api`.',
      `In a legacy runner, create exactly one question interaction: POST $API_ORIGIN/api/issues/$GEETORUS_TASK_ID/interactions once with {"kind":"ask_user_questions","idempotencyKey":"question-${nonce}","continuationPolicy":"wake_assignee","payload":{"version":1,"questions":[{"id":"verification-word","prompt":"Choose the verification word.","selectionMode":"single","required":true,"options":[{"id":"cobalt","label":"Cobalt"},{"id":"amber","label":"Amber"}]}]}} using Authorization and X-Geetorus-Run-Id. Do not create a replacement interaction if a later write fails.`,
      'In a legacy runner, after that POST returns 2xx, PATCH $API_ORIGIN/api/issues/$GEETORUS_TASK_ID with exactly {"status":"in_review"}. Do not include `reviewInteractionId`: it only designates confirmation interactions, not `ask_user_questions`. If the PATCH fails, retry only that PATCH and never POST the interaction again.',
      ...
      `After the answer arrives, if it is Cobalt, publish exactly GEETORUS_E2E_QUESTION_DONE_${nonce} once as the complete visible response and mark the task Done.`,
      `In a native runner, after the answer arrives, call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"GEETORUS_E2E_QUESTION_DONE_${nonce}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Wait for that tool call to succeed, then emit exactly GEETORUS_E2E_QUESTION_DONE_${nonce} as the complete final response. Do not write a user-facing final response before geetorus_finish succeeds, and do not call another tool.`,
      `In a legacy runner, make exactly one completion write: PATCH $API_ORIGIN/api/issues/$GEETORUS_TASK_ID with {"status":"done","comment":"GEETORUS_E2E_QUESTION_DONE_${nonce}"}. Do not POST a separate comment or perform a second write containing the marker.`,
  ...
  buildMatchers(nonce, execution) {
    return [
      {
        kind: "message_exact",
        expected: `GEETORUS_E2E_QUESTION_DONE_${nonce}`,
      },
      {
        kind: "message_occurrences",
        expected: `GEETORUS_E2E_QUESTION_DONE_${nonce}`,
...
export const openRouterBreadthTasks: readonly RunnerTaskFixture[] = [
  {
    ...
    buildPrompt: (nonce) =>
      [
        ...
        "Your first response action must be the geetorus_finish tool call. Do not emit any assistant text, acknowledgement, or preamble before calling it.",
        `Call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"${breadthMarker("H", nonce)}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Wait for that tool call to succeed, then emit exactly ${breadthMarker("H", nonce)} as the complete user-facing final response. Do not write a user-facing final response before geetorus_finish succeeds, and do not call another tool.`,
  ...
  },
  {
    ...
    buildPrompt: (nonce) =>
      [
        ...
        "Do not call geetorus_finish while the question is pending.",
        `After the answer arrives, if it is Cobalt, call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"${breadthMarker("Q_C", nonce)}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Wait for that tool call to succeed, then emit exactly ${breadthMarker("Q_C", nonce)} as the complete user-facing final response. Do not write a user-facing final response before geetorus_finish succeeds, and do not call another tool.`,
  ...
  },
  {
    ...
    buildPrompt: (nonce) =>
      [
        ...
        "Do not call geetorus_finish while confirmation is pending.",
        `After that exact Plan revision is accepted, call geetorus_finish exactly once with {reportedWorkDisposition:"done",summary:"${breadthMarker("P_OK", nonce)}",completionClaim:{contractRevision:"1",objectiveSatisfied:true,criteria:[{criterionId:"objective",status:"satisfied",evidenceRefs:[]}],remainingWork:[]},evidence:[],verification:[]}. Wait for that tool call to succeed, then emit exactly ${breadthMarker("P_OK", nonce)} as the complete user-facing final response. Do not write a user-facing final response before geetorus_finish succeeds, and do not call another tool.`,
...
function warmTurnMarker(turn: 1 | 2 | 3, nonce: string) {
  return `GEETORUS_E2E_WARM_T${turn}_${nonce}`;
}
...
export const connectionReviewSuite: RunnerSuiteFixture = {
  ...
  tasks: (["approve", "decline", "always", "restart"] as const).map(decision => ({
    ...
    buildVisibleMarker: nonce => `GEETORUS_E2E_REVIEW_DONE_${nonce}`,
    buildPrompt: nonce => [
      ...
      `After consuming the real result (or the human decline), finish the task and publish exactly GEETORUS_E2E_REVIEW_DONE_${nonce} once as your final task response.`,
      'For native runners use geetorus_finish. Legacy runners PATCH the task with status done and the final comment. Do not create unrelated work.',
    ].join("\n"),
    buildMatchers: (nonce, execution) => terminalMatchers(`GEETORUS_E2E_REVIEW_DONE_${nonce}`, execution),
  })),
};
...
export const runnerSuites: readonly RunnerSuiteFixture[] = [
  ...
  {
    id: "everyday-workflows", label: "Everyday Geetorus Work", manualOnly: true,
  ...
  ...(process.env.GEETORUS_RUNNER_E2E_CONNECTION_REVIEWS === "1" ? [connectionReviewSuite] : []),
  {
...
export function validateRunnerCatalog(): MatrixExecution[] {
  ...
  for (const environment of [...runnerEnvironments, daytonaWarmEnvironment]) {
    const payload = environment.buildEnvironment({
      ...
      daytonaImage:
        "ghcr.io/geetorusai/geetorus-daytona-runner@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  ...
  for (const profile of [...allProfiles, ...everydayProfiles]) {
    ...
    const payload = profile.buildAgent({
      ...
      workspacePath: "/tmp/geetorus-runner-e2e-schema",
      secretRefs: sampleRefs,
      executionId: "schema-validation",
    });
