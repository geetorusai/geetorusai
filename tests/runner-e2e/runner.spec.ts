...
async function restartIsolatedGeetorusServer(input: {
  api: RunnerApi;
  requestId: string;
  deadlineAt: number;
}): Promise<void> {
...
const executionIds = (() => {
  const encoded = process.env.GEETORUS_RUNNER_E2E_EXECUTION_IDS;
  if (encoded) {
    ...
    if (
      ...
      throw new Error(
        "GEETORUS_RUNNER_E2E_EXECUTION_IDS must be a non-empty JSON string array",
      );
    }
    return parsed;
  }
  const single = process.env.GEETORUS_RUNNER_E2E_EXECUTION_ID;
  if (!single)
    throw new Error("GEETORUS_RUNNER_E2E_EXECUTION_IDS is required");
  return [single];
})();
const executions = executionIds.map(runnerExecutionById);
const attempt = Number(process.env.GEETORUS_RUNNER_E2E_ATTEMPT ?? "1");
const temporaryRoot = process.env.GEETORUS_RUNNER_E2E_TEMP_ROOT;
const privateRoot = process.env.GEETORUS_RUNNER_E2E_PRIVATE_DIR;
const workspacePath = process.env.GEETORUS_RUNNER_E2E_WORKSPACE;
...
function nativeRunEventIntegrityFailures(
  ...
  for (const event of events) {
    ...
    if (
      envelope.schema !== "geetorus.prp.event.v1" ||
  ...
  test(`${execution.id} completes through the browser and public APIs`, async ({
    ...
    try {
      ...
      fixtures = execution.task.flow === "first_task"
        ...
        workspacePath,
        credentials,
        daytonaImage: process.env.GEETORUS_E2E_DAYTONA_IMAGE,
      });
      ...
      if (execution.task.flow === "continuation") {
        const continuation = await runContinuationFlow({
          ...
          restart: () => restartIsolatedGeetorusServer({ api, requestId: `continuation-${nonce}`, deadlineAt: startedAtMs + deadlineMs }),
        ...
        const story = await runEverydayFlow({
          ...
          restart: () => restartIsolatedGeetorusServer({ api, requestId: `story-${nonce}`, deadlineAt: startedAtMs + deadlineMs }),
        ...
        const chat = await runChatFlow({
          ...
          restart: () => restartIsolatedGeetorusServer({ api, requestId: `chat-${nonce}`, deadlineAt: startedAtMs + deadlineMs }),
      ...
      if (execution.task.flow === "governed_tool_review") {
        ...
        if (execution.task.toolReviewDecision === "restart") {
          await restartIsolatedGeetorusServer({ api, requestId: `tool-review-${nonce}`, deadlineAt });
          await page.reload();
        }
        ...
        if (execution.task.restartServerBeforeQuestionAnswer) {
          ...
          await restartIsolatedGeetorusServer({
            api,
            requestId: restartRequestId,
            deadlineAt,
          });
          const documentSentinel = `__geetorus_runner_restart_${nonce.replaceAll("-", "_")}`;
          await page.evaluate(
      ...
      if (execution.task.flow === "warm_three_turn") {
        turnTimings = selectedRuns.map((candidate, index) => {
          ...
          const acquisition = record(
            record(candidate.contextSnapshot).geetorusEnvironment,
      ...
      const context = record(run.contextSnapshot);
      const environmentContext = record(context.geetorusEnvironment);
      const workspaceContext = record(context.geetorusWorkspace);
      ...
      const observedRuntimeMode =
        run.runtimeMode ??
        (persistedAgent.adapterType === "geetorus_runner"
          ? "native"
          : "legacy");
      ...
      if (execution.task.flow === "warm_three_turn") {
        const runEnvironmentContexts = selectedRuns.map((candidate) =>
          record(record(candidate.contextSnapshot).geetorusEnvironment),
        );
        ...
        const projectWorkspaceIds = selectedRuns.map(
          (candidate) =>
            record(record(candidate.contextSnapshot).geetorusWorkspace)
              .workspaceId,
        );
