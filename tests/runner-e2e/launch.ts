...
async function terminateProcessGroup(pid: number) {
  ...
  return processGroupIsAlive(pid)
    ? `Geetorus/Playwright process group ${pid} survived SIGKILL`
    : null;
}
...
async function prepareProviderPath(
  ...
  const runnerRequire = createRequire(
    path.join(repositoryRoot, "packages/geetorus-runner/package.json"),
  );
  ...
  const packageBin = path.join(
    repositoryRoot,
    "packages/geetorus-runner/node_modules/.bin",
  );
...
function syntheticResult(
  ...
  return {
    schema: "geetorus.runner-e2e.result/v2",
...
async function runAttempt(input: {
  ...
  try {
    ...
    await Promise.all([
      mkdir(geetorusHome, { recursive: true }),
    ...
    const childEnv: NodeJS.ProcessEnv = {
      ...
      PATH: providerPath,
      GEETORUS_RUNNER_E2E_EXECUTION_IDS: JSON.stringify(
        executions.map((candidate) => candidate.id),
      ),
      GEETORUS_RUNNER_E2E_ATTEMPT: String(attempt),
      GEETORUS_RUNNER_E2E_PORT: String(port),
      GEETORUS_RUNNER_E2E_TEMP_ROOT: temporaryRoot,
      GEETORUS_RUNNER_E2E_PRIVATE_DIR: privateDir,
      GEETORUS_RUNNER_E2E_WORKSPACE: workspace,
      GEETORUS_RUNNER_E2E_SERVER_LOG: path.join(privateDir, "server.log"),
      GEETORUS_RUNNER_BINARY: runnerBinary,
      GEETORUS_RUNNER_REMOTE_BINARY_PATH:
        resolveGeetorusRemoteRunnerBinaryForHarness(executions, runnerBinary),
      ...
      GEETORUS_VITE_CACHE_DIR: path.join(temporaryRoot, "vite-cache"),
      GEETORUS_RUNNER_E2E_TEST_TIMEOUT_MS: String(
        Math.max(
          ...executions.map(
            (candidate) =>
      ...
      GEETORUS_AGENT_JWT_SECRET: agentJwtSecret,
      GEETORUS_DECISION_SIGNING_SECRET: decisionSigningSecret,
      GEETORUS_TOOL_ACTION_SIGNING_SECRET: toolActionSigningSecret,
          ...
          while (true) {
            ...
            const leak = await findSecretLeakInDirectory(directory, credentials, {
              ...
              allowDisappearedFile: (file) =>
                label === "Geetorus home" &&
                isEphemeralPostgresScanFile(geetorusHome, file),
            });
            if (!leak) break;
            const isManagedCodexRuntimeAuth =
              label === "Geetorus home" &&
              isEphemeralCodexRuntimeAuthFile(geetorusHome, leak.file);
    ...
    if (
      temporaryRoot.startsWith(`${os.tmpdir()}${path.sep}geetorus-runner-e2e-`)
    ) {
...
async function main() {
  ...
  if (
    ...
    !isImmutableDaytonaImage(process.env.GEETORUS_E2E_DAYTONA_IMAGE)
  ) {
    throw new Error(
      "GEETORUS_E2E_DAYTONA_IMAGE must be an immutable image@sha256 digest for Daytona cells",
    );
  }

  const campaignId = cleanId(
    process.env.GEETORUS_E2E_CAMPAIGN_ID ??
