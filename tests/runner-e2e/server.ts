...
const logPath = required("GEETORUS_RUNNER_E2E_SERVER_LOG");
...
const port = required("GEETORUS_RUNNER_E2E_PORT");
const repositoryRoot = path.resolve(import.meta.dirname, "../..");
const tsxCli = path.join(repositoryRoot, "cli/node_modules/tsx/dist/cli.mjs");
const geetorusCli = path.join(repositoryRoot, "cli/src/index.ts");
const {
  controlDirectory,
  restartRequestPath,
...
const serverEnvironment = buildGeetorusServerEnvironment(process.env, {
  NODE_ENV: "test",
  PORT: port,
  ...
  GEETORUS_INSTANCE_ID: required("GEETORUS_INSTANCE_ID"),
  GEETORUS_AGENT_JWT_SECRET: required("GEETORUS_AGENT_JWT_SECRET"),
  GEETORUS_DECISION_SIGNING_SECRET: required(
    "GEETORUS_DECISION_SIGNING_SECRET",
  ),
  GEETORUS_TOOL_ACTION_SIGNING_SECRET: required(
    "GEETORUS_TOOL_ACTION_SIGNING_SECRET",
  ),
  BETTER_AUTH_SECRET: required("BETTER_AUTH_SECRET"),
  GEETORUS_BIND: "loopback",
  GEETORUS_BIND_HOST: "127.0.0.1",
  GEETORUS_DEPLOYMENT_MODE: "local_trusted",
  GEETORUS_DEPLOYMENT_EXPOSURE: "private",
  SERVE_UI: "true",
  GEETORUS_STORAGE_PROVIDER: "local_disk",
  GEETORUS_STORAGE_LOCAL_DIR: path.join(temporaryRoot, "storage"),
  GEETORUS_SECRETS_PROVIDER: "local_encrypted",
  GEETORUS_SECRETS_STRICT_MODE: "true",
  GEETORUS_DB_BACKUP_ENABLED: "false",
  GEETORUS_DB_BACKUP_DIR: path.join(temporaryRoot, "backups"),
...
function startServer() {
  if (shutdownRequested()) {
    throw new Error("Refusing to start Geetorus after wrapper shutdown");
  }
  const candidate = spawn(
    process.execPath,
    [tsxCli, geetorusCli, "onboard", "--yes", "--run"],
    {
      cwd: repositoryRoot,
      env: definedServerEnvironment,
      stdio: ["ignore", "pipe", "pipe"],
      // Stay in the launcher-created process group. That lets the launcher stop
      // Playwright, this wrapper, Geetorus, embedded Postgres, and runner children
  ...
  candidate.once("error", (error) => {
    ...
    if (!expectedStops.has(candidate) && !shutdownRequested()) {
      unexpectedChildFailure = new Error(
        `Geetorus server spawn failed: ${error.message}`,
      );
    }
  });
  candidate.once("exit", () => {
    ...
    if (!expectedStops.has(candidate) && !shutdownRequested()) {
      unexpectedChildFailure = new Error(
        `Geetorus server stopped unexpectedly: ${describeChildExit(candidate)}`,
      );
    }
  });
...
async function stopServer(
  ...
  try {
    ...
    if (childExited(candidate) || childErrors.has(candidate)) return;
    throw new Error("Could not signal the Geetorus server to stop");
  }
  if (await waitForExit(candidate, gracefulStopTimeoutMs)) return;

  appendLog(
    `\nGeetorus did not stop within ${gracefulStopTimeoutMs}ms; sending SIGKILL\n`,
  );
  try {
    candidate.kill("SIGKILL");
  } catch {
    if (childExited(candidate) || childErrors.has(candidate)) return;
    throw new Error("Could not force the Geetorus server to stop");
  }
  if (!(await waitForExit(candidate, 5_000))) {
    throw new Error("Geetorus server did not exit after SIGKILL");
  }
}
...
async function waitForHealth(candidate: ChildProcess) {
  ...
  while (Date.now() < deadline) {
    if (shutdownRequested()) {
      throw new Error("Wrapper shutdown interrupted the Geetorus restart");
    }
    if (childErrors.has(candidate) || childExited(candidate)) {
      throw new Error(
        `Replacement Geetorus server could not start: ${describeChildExit(candidate)}`,
      );
    }
    try {
  ...
  throw new Error(
    `Replacement Geetorus server did not become healthy within ${restartTimeoutMs}ms`,
  );
}
...
async function waitForHealthToStop() {
  ...
  throw new Error(
    "The old Geetorus server remained healthy after its launcher exited",
  );
}
...
async function restartServer(requestId: string) {
  ...
  appendLog(`\nRestart request ${requestId}: stopping Geetorus\n`);
  const previous = child;
  if (!previous) throw new Error("No Geetorus server is available to restart");
  ...
  appendLog(`Restart request ${requestId}: starting Geetorus\n`);
  ...
  appendLog(`Restart request ${requestId}: Geetorus is healthy\n`);
...
for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"] as const) {
  process.on(signal, () => {
    ...
    try {
      ...
      // The Geetorus process may already have exited.
    }
  });
}
...
try {
  ...
  appendLog(`\nGeetorus E2E server supervisor failed: ${message}\n`);
  ...
  if (running) {
    try {
      ...
      appendLog(
        `Failed to stop Geetorus after supervisor failure: ${stopError instanceof Error ? stopError.message : String(stopError)}\n`,
      );
    }
  }
}
