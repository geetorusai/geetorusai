...
describe("runner E2E campaign history", () => {
  ...
  it("records the resolved paid target instead of the trusted workflow checkout", () => {
    vi.stubEnv("GEETORUS_RUNNER_E2E_SOURCE_SHA", "target-sha");
    vi.stubEnv("GEETORUS_RUNNER_E2E_SOURCE_REF", "refs/heads/target");
...
describe("historical publication security", () => {
  ...
  it("publishes declared screenshots while keeping active evidence private", async () => {
    ...
    ).toBe("geetorus.runner-e2e.campaign/v2");
    await expect(
  ...
  it("requires a private-origin-compatible destination shape", () => {
    expect(
      validateHistoryDestination({
        bucket: "geetorus-runner-e2e-history",
        prefix: "/runner-e2e/",
        publicBaseUrl: "https://history.geetorus.ai/",
      }),
    ).toEqual({
      prefix: "runner-e2e",
      publicBaseUrl: "https://history.geetorus.ai",
    ...
    expect(() =>
      validateHistoryDestination({
        bucket: "geetorus-runner-e2e-history",
        prefix: "runner-e2e",
        publicBaseUrl: "http://history.geetorus.ai/",
      }),
  ...
  it("rejects non-allowlisted files and fingerprints an immutable bundle", async () => {
    ...
    expect(isHistoricalBundlePathAllowed("geetorus-home/database")).toBe(
      false,
    );
