...
import {
  ...
  buildGeetorusServerEnvironment,
  buildRunnerE2EProcessEnvironment,
  resolveGeetorusRemoteRunnerBinaryForHarness,
  resolveGeetorusRunnerBinaryForHarness,
...
describe("runner E2E local binary resolution", () => {
  ...
  it("uses the debug runner binary built by the E2E workflow", () => {
    expect(
      ...
      path.join(
        "/repository",
        "packages/geetorus-runner/runner/target/debug/geetorus-runnerd",
      ),
    );
  });
  ...
  it("preserves an explicit runner binary override", () => {
    expect(
      resolveGeetorusRunnerBinaryForHarness(
        [localNativeExecution],
        "/repository",
        "/custom/geetorus-runnerd",
        "linux",
      ),
    ).toBe("/custom/geetorus-runnerd");
  });

  it("uses and stages the same build-once binary for remote native cells", () => {
    const runnerBinary = resolveGeetorusRunnerBinaryForHarness(
    ...
    expect(
      resolveGeetorusRemoteRunnerBinaryForHarness(
        [remoteNativeExecution],
        runnerBinary,
        "/cross-compiled/geetorus-runnerd",
        "darwin",
      ),
    ).toBe("/cross-compiled/geetorus-runnerd");
  });
});
...
describe("runner E2E provider environment", () => {
  ...
  it("allows the pinned model only for isolated legacy OpenCode harnesses", () => {
    for (const execution of [legacyLocal, legacyDaytona]) {
      ...
      ).toEqual({ KEEP_ME: "yes", OPENCODE_ALLOW_ALL_MODELS: "true", GEETORUS_ANNOUNCEMENTS_ENABLED: "false" });
    }
    ...
    for (const execution of [nativeOpenCode, breadthOpenCode]) {
      ...
      ).toEqual({ KEEP_ME: "yes", GEETORUS_ANNOUNCEMENTS_ENABLED: "false" });
    }
  });

  it("disables announcements through the server boundary for every runner cell", () => {
    for (const execution of runnerMatrix) {
      const source = { GEETORUS_ANNOUNCEMENTS_ENABLED: "true" };
      const env = buildRunnerE2EProcessEnvironment(source, [execution]);
      expect(buildGeetorusServerEnvironment(env).GEETORUS_ANNOUNCEMENTS_ENABLED).toBe("false");
      expect(source.GEETORUS_ANNOUNCEMENTS_ENABLED).toBe("true");
    }
  });
});
...
describe("hiring capability opt-in", () => {
  it("enables API tools only when the manual hiring story is selected", () => {
    ...
    expect(buildRunnerE2EProcessEnvironment({}, [hire]).GEETORUS_RUNNER_API_TOOLS_ENABLED).toBe("true");
    expect(buildRunnerE2EProcessEnvironment({}, [delegate]).GEETORUS_RUNNER_API_TOOLS_ENABLED).toBeUndefined();
    expect(buildRunnerE2EProcessEnvironment({}, []).GEETORUS_RUNNER_API_TOOLS_ENABLED).toBeUndefined();
  });
});
...
describe("runner E2E sensitive API boundary", () => {
  it("keeps secret request bodies out of Playwright API tracing", async () => {
    vi.stubEnv("GEETORUS_RUNNER_E2E_PORT", "43123");
...
describe("runner E2E matchers", () => {
  it("normalizes message text and evaluates state invariants", async () => {
    const results = await evaluateMatchers(
      [
        { kind: "message_contains", expected: "GEETORUS_E2E_OK_nonce" },
        {
          kind: "message_occurrences",
          expected: "GEETORUS_E2E_OK_nonce",
          count: 1,
        },
      ...
      {
        message: "  complete   GEETORUS\\_E2E\\_OK\\_nonce  ",
        issueStatus: "done",
        runtimeMode: "native",
      },
    );
...
describe("runner E2E run observations", () => {
  ...
  it("retries only the zero-marker DeepSeek hello terminal emission variance", () => {
    ...
    const observation = {
      ...
      expectedMarker,
      finalRunMessage:
        "I'll complete this deterministic hello task by calling geetorus_finish once.",
      allAgentMessages:
        "I'll complete this deterministic hello task by calling geetorus_finish once.",
...
describe("runner E2E failure policy", () => {
  ...
  it("retries only transient infrastructure failures", () => {
    ...
    expect(
      classifyFailure(
        new Error(
          "runner_ingress_unavailable: geetorus-runnerd: cumulative ACK cannot move beyond the produced source cursor",
        ),
      ),
    ).toBe("transient_infrastructure");
    expect(
...
describe("runner E2E server isolation", () => {
  ...
  it("strips database and paid-provider credentials from the Geetorus process", () => {
    const env = buildGeetorusServerEnvironment(
      {
        PATH: "/bin",
        ...
        GEETORUS_API_KEY: "ambient-board-key",
        GEETORUS_AGENT_API_KEY: "ambient-agent-key",
        GEETORUS_TASK_BRIDGE_TOKEN: "ambient-task-token",
        GEETORUS_SETUP_TOKEN: "ambient-setup-token",
        GEETORUS_SECRETS_MASTER_KEY: "ambient-master-key",
        GEETORUS_SECRETS_MASTER_KEY_FILE: "/outside/master.key",
        GEETORUS_STORAGE_S3_BUCKET: "production-bucket",
      },
      {
        GEETORUS_HOME: "/tmp/cell/geetorus-home",
        GEETORUS_CONFIG: "/tmp/cell/geetorus-home/instances/e2e/config.json",
        XDG_CACHE_HOME: "/tmp/cell/xdg-cache",
        GEETORUS_AGENT_JWT_SECRET: "generated-agent-jwt",
        GEETORUS_DECISION_SIGNING_SECRET: "generated-decision-key",
        GEETORUS_TOOL_ACTION_SIGNING_SECRET: "generated-tool-key",
    ...
    expect(env.GEETORUS_AGENT_API_KEY).toBeUndefined();
    expect(env.XDG_CACHE_HOME).toBe("/tmp/cell/xdg-cache");
    expect(env.GEETORUS_AGENT_JWT_SECRET).toBe("generated-agent-jwt");
    expect(env.GEETORUS_TASK_BRIDGE_TOKEN).toBeUndefined();
    expect(env.GEETORUS_SETUP_TOKEN).toBeUndefined();
    expect(env.GEETORUS_SECRETS_MASTER_KEY).toBeUndefined();
    expect(env.GEETORUS_SECRETS_MASTER_KEY_FILE).toBeUndefined();
    expect(env.GEETORUS_STORAGE_S3_BUCKET).toBeUndefined();
    expect(() =>
      assertIsolatedServerEnvironment(env, {
        temporaryRoot: "/tmp/cell",
        geetorusHome: "/tmp/cell/geetorus-home",
        configPath: "/tmp/cell/geetorus-home/instances/e2e/config.json",
      }),
    ).not.toThrow();
  });

  it("uses absolute repository paths for the Playwright web server", () => {
    const command = runnerE2EWebServerCommand("/workspace/geetorus");
    expect(command).toContain(
      "'/workspace/geetorus/cli/node_modules/tsx/dist/cli.mjs'",
    );
    expect(command).toContain(
      "'/workspace/geetorus/tests/runner-e2e/server.ts'",
    );
  });
  ...
  it("accepts only embedded state paths beneath the temporary root", async () => {
    ...
    const database = path.join(root, "geetorus-home", "db");
    ...
    const configPath = path.join(root, "geetorus-home", "config.json");
...
describe("runner E2E evidence redaction", () => {
  ...
  it("redacts exact and shaped credentials recursively", () => {
    ...
    expect(sanitizeJson("geetorus.runner-e2e.evidence/v1", [secret])).toBe(
      "geetorus.runner-e2e.evidence/v1",
    );
  });
  ...
  it("recognizes managed and durable Codex runtime auth files", () => {
    const root = path.join(os.tmpdir(), "geetorus-home");
    expect(
      isEphemeralCodexRuntimeAuthFile(
        root,
        path.join(
    ...
    expect(
      isEphemeralCodexRuntimeAuthFile(
        ...
        path.join(
          root,
          "instances/instance-1/runtime/geetorus-runner/acpx/acpx/session-1/codex-home/auth.json",
        ),
      ),
    ).toBe(true);
    expect(
      isEphemeralCodexRuntimeAuthFile(
        ...
        path.join(
          root,
          "instances/instance-1/runtime/geetorus-runner/durable-sessions/session-1/codex-home/auth.json",
        ),
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      isEphemeralCodexRuntimeAuthFile(
        ...
        path.join(
          root,
          "instances/instance-1/runtime/geetorus-runner/durable-sessions/session-1/codex-home/config.toml",
        ),
      ),
    ).toBe(false);
  });
