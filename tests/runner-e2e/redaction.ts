...
export function isEphemeralCodexRuntimeAuthFile(
  ...
  const relative = path.relative(geetorusHome, file).split(path.sep).join("/");
  return (
    /^instances\/[^/]+\/companies\/[^/]+\/agents\/[^/]+\/codex-home\/auth\.json$/.test(
      relative,
    ) ||
    /^instances\/[^/]+\/runtime\/geetorus-runner\/durable-sessions\/[^/]+\/codex-home\/auth\.json$/.test(
      relative,
    ) ||
    /^instances\/[^/]+\/runtime\/geetorus-runner\/acpx\/acpx\/[^/]+\/codex-home\/auth\.json$/.test(
      relative,
    )
  );
}
...
export function isEphemeralPostgresPidFile(geetorusHome: string, file: string): boolean {
  const relative = path.relative(geetorusHome, file).split(path.sep).join("/");
  return /^instances\/[^/]+\/db\/postmaster\.pid$/.test(relative);
}

export function isEphemeralPostgresScanFile(geetorusHome: string, file: string): boolean {
  const relative = path.relative(geetorusHome, file).split(path.sep).join("/");
  ...
  return isEphemeralPostgresPidFile(geetorusHome, file) ||
