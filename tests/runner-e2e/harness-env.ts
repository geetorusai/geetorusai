...
const AMBIENT_GEETORUS_CREDENTIAL_KEYS = [
  "GEETORUS_API_KEY",
  "GEETORUS_AGENT_API_KEY",
  "GEETORUS_TASK_BRIDGE_TOKEN",
  "GEETORUS_SETUP_TOKEN",
  "GEETORUS_SECRETS_MASTER_KEY",
 ...
 */
export function resolveGeetorusRunnerBinaryForHarness(
  ...
  return path.join(
    ...
    "target",
    "debug",
    platform === "win32" ? "geetorus-runnerd.exe" : "geetorus-runnerd",
  );
}
 ...
 */
export function resolveGeetorusRemoteRunnerBinaryForHarness(
  executions: readonly MatrixExecution[],
  runnerBinary: string | undefined,
  configuredPath = process.env.GEETORUS_RUNNER_REMOTE_BINARY_PATH,
  platform: NodeJS.Platform = process.platform,
): string | undefined {
  if (configuredPath?.trim()) return configuredPath;
...
export function buildRunnerE2EProcessEnvironment(
  ...
  result.GEETORUS_ANNOUNCEMENTS_ENABLED = "false";
  ...
  if (executions.some((e) => e.suite.id === "everyday-workflows" && e.task.id === "hire-reuse")) {
    result.GEETORUS_RUNNER_API_TOOLS_ENABLED = "true";
  }
  if (
    executions.length > 0 &&
    executions.every(
...
/**
 * Build the environment inherited by the Geetorus server. Paid credentials
 ...
 */
export function buildGeetorusServerEnvironment(
  source: NodeJS.ProcessEnv,
  ...
  for (const key of [
    ...CREDENTIAL_NAMES,
    ...DATABASE_KEYS,
    ...AMBIENT_GEETORUS_CREDENTIAL_KEYS,
...
export function assertIsolatedServerEnvironment(
  ...
  const home = env.GEETORUS_HOME;
  const config = env.GEETORUS_CONFIG;
  if (home !== expected.geetorusHome || config !== expected.configPath) {
    throw new Error(
      "Geetorus server environment does not use the allocated home/config paths",
    );
  }
  if (
    ...
    throw new Error(
      "Geetorus server paths escape the isolated temporary root",
    );
  }
  if (env.XDG_CACHE_HOME !== path.join(expected.temporaryRoot, "xdg-cache")) {
    throw new Error(
      "Geetorus server cache does not use the allocated temporary root",
    );
  }
  for (const key of [
    ...CREDENTIAL_NAMES,
    ...
    if (env[key])
      throw new Error(
        `Geetorus server environment unexpectedly contains ${key}`,
      );
  }
  for (const key of GENERATED_SERVER_SECRET_KEYS) {
    if (!env[key])
      throw new Error(`Geetorus server environment is missing ${key}`);
  }
}
