...
describe("runner E2E Daytona image contract", () => {
  it("builds runnerd and the provider pack and verifies every required transport", async () => {
    ...
    expect(dockerfile).toContain("--bin geetorus-runnerd");
    expect(dockerfile).toContain("build-provider-pack.mjs /provider-pack");
    expect(normalizedDockerfile).not.toContain(
      "COPY packages/geetorus-eval-kernel ./packages/geetorus-eval-kernel",
    );
    expect(normalizedDockerfile).not.toContain(
      "COPY packages/geetorus-runner ./packages/geetorus-runner",
    );
    expect(dockerfile).toContain(
      "COPY packages ./packages",
    );
    expect(dockerfile).toContain(
      "/opt/geetorus-runner/provider-pack/provider-pack.json",
    );
    expect(dockerfile).toContain(
      "${GEETORUS_RUNNER_PROVIDER_PACK_ROOT}/node_modules/.bin",
    );
    ...
    expect(dockerfile).toContain(
      'metadata="$(geetorus-runnerd --build-metadata)"',
    );
    expect(dockerfile).toContain("provider-pack.json");
    expect(dockerfile).toContain("io.geetorus.runner.content-id");
    ...
    expect(dockerignore).toContain("packages/geetorus-runner/dist");
    expect(dockerignore).toContain("packages/geetorus-runner/runner/target");
    for (const developmentOnlyInput of [
      ...
      "packages/geetorus-runner/**/*.md",
      "packages/geetorus-runner/**/*.test.ts",
    ...
    expect(workflow).toContain(
      '--build-arg "GEETORUS_RUNNER_CONTENT_ID=${IMAGE_CONTENT_ID}"',
    );
    expect(workflow).toContain(
      "IMAGE_CACHE: ghcr.io/geetorusai/geetorus-daytona-runner:e2e-buildcache-amd64",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("GEETORUS_RUNNER_PROVIDER_PACK_ROOT=");
    expect(workflow).toContain(
      "node packages/geetorus-runner/scripts/build-provider-pack.mjs packages/geetorus-runner/provider-pack",
    );
    expect(workflow).toContain(
      "GEETORUS_RUNNER_REMOTE_PROVIDER_PACK_PATH: ${{ github.workspace }}/packages/geetorus-runner/provider-pack",
    );
    expect(workflow).toContain(
      "GEETORUS_RUNNER_SOURCE_REVISION: ${{ needs.daytona_image.outputs.source_revision }}",
    );
  ...
  it("hashes the audited image dependency closure rather than the repository revision", async () => {
    ...
    expect(DAYTONA_IMAGE_INPUT_PATHS).not.toContain(
      "packages/geetorus-eval-kernel",
    );
    expect(DAYTONA_IMAGE_INPUT_PATHS).not.toContain(
      "packages/geetorus-runner",
    );
  ...
  it("changes for runtime source, package, lockfile, Dockerfile, frontend, base, or platform inputs", async () => {
    const root = await mkdtemp(
      path.join(tmpdir(), "geetorus-daytona-image-id-"),
    );
    const inputPaths = [
      "docker/daytona-runner/Dockerfile",
      "package.json",
    ...
    try {
      ...
      await mkdir(path.join(root, "packages/geetorus-runner/src"), {
        recursive: true,
      });
      await mkdir(
        path.join(
          root,
          "packages/geetorus-runner/runner/crates/runner-core/src",
        ),
        { recursive: true },
      );
      await writeFile(
      ...
      await writeFile(
        path.join(root, "packages/geetorus-runner/package.json"),
        '{"name":"@geetorusai/geetorus-runner"}\n',
      );
      await writeFile(
        path.join(root, "packages/geetorus-runner/src/runner.ts"),
        "version one\n",
      );
      await writeFile(
        path.join(
          root,
          "packages/geetorus-runner/runner/crates/runner-core/src/lib.rs",
        ),
      ...
      for (const relativePath of [
        ...
        "pnpm-lock.yaml",
        "packages/geetorus-runner/package.json",
        "packages/geetorus-runner/src/runner.ts",
        "packages/geetorus-runner/runner/crates/runner-core/src/lib.rs",
      ]) {
  ...
  it("reuses the image for runner-only tests and documentation", async () => {
    const root = await mkdtemp(
      path.join(tmpdir(), "geetorus-daytona-runner-development-inputs-"),
    );
    const options = {
      repositoryRoot: root,
      inputPaths: ["packages/geetorus-runner"],
      baseImages: [`example.test/base:1@sha256:${"a".repeat(64)}`],
      frontendDigest: `sha256:${"c".repeat(64)}`,
    } as const;
    try {
      const runnerRoot = path.join(root, "packages/geetorus-runner");
