...
describe("Codex CI sandbox trust boundary", () => {
  ...
  it("keeps privileged policy changes out of target-controlled tests", async () => {
    ...
    expect(source).toMatch(/"sandbox",\s*"--permission-profile",\s*"geetorus-e2e-probe"/);
    expect(source).toContain(
      "permissions.geetorus-e2e-probe.network.enabled=false",
    );
    expect(source).toContain(
      'permissions.geetorus-e2e-probe.filesystem={":root"="read"}',
    );
