...
describe("runner E2E catalog", () => {
  ...
  it("defines the warm Daytona continuity fixture as exactly two Codex cells", () => {
    ...
    expect(followups[1]).toContain(
      '{"status":"done","comment":"GEETORUS_E2E_WARM_T3_nonce"}',
    );
  ...
  it("derives the qualified local native OpenCode profiles from the ranked snapshot", () => {
    ...
    expect(
      openRouterBreadthProfiles.every(
        (profile) =>
          profile.adapterType === "geetorus_runner" &&
  ...
  it("defines deterministic two-run question and plan state machines", () => {
    ...
    expect(localQuestion?.buildPrompt("nonce")).toContain(
      "do not spell, quote, repeat, announce, or include GEETORUS_E2E_QUESTION_DONE_nonce",
    );
    expect(localQuestion?.buildPrompt("nonce")).toContain(
      "refer to it only as “the terminal marker.”",
    );
    expect(localQuestion?.buildPrompt("nonce")).toContain(
      'API_ORIGIN="${GEETORUS_API_URL%/}"; API_ORIGIN="${API_ORIGIN%/api}"',
    );
    expect(localQuestion?.buildPrompt("nonce")).toContain(
      '"idempotencyKey":"question-nonce"',
    );
    expect(localQuestion?.buildPrompt("nonce")).toContain(
      'PATCH $API_ORIGIN/api/issues/$GEETORUS_TASK_ID with exactly {"status":"in_review"}',
    );
    expect(localQuestion?.buildPrompt("nonce")).toContain(
  ...
  it("emits native terminal text after the terminal tool succeeds", () => {
    ...
    for (const prompt of [
      ...
      expect(prompt!.indexOf("geetorus_finish exactly once")).toBeLessThan(
    ...
    expect(breadthHello).toContain(
      "Your first response action must be the geetorus_finish tool call",
    );
    expect(breadthHello).toContain(
    ...
    expect(nativeAsk).toContain("geetorus_finish must be your only tool call");
  ...
  it("gives legacy planning agents a direct bounded API recipe", () => {
    ...
    expect(task!.buildPrompt("nonce")).toContain(
      "do not spell, quote, repeat, announce, or include GEETORUS_E2E_PLAN_DONE_nonce",
    );
    expect(task!.buildPrompt("nonce")).toContain(
      'summary:"GEETORUS_E2E_PLAN_DONE_nonce"',
    );
    ...
    expect(task!.buildPrompt("nonce")).toContain(
      "After that verification succeeds, your immediate next action must be the geetorus_finish tool call",
    );
  ...
  it("requires one atomic legacy Ask completion write", () => {
    ...
    expect(prompt).toContain(
      'PATCH /api/issues/$GEETORUS_TASK_ID with {"status":"done","comment":"E2E_ASK_12_nonce"}',
    );
  ...
  it("accepts only complete immutable Daytona digests", () => {
    expect(
      isImmutableDaytonaImage(
        `ghcr.io/geetorusai/geetorus-daytona-runner@sha256:${"a".repeat(64)}`,
      ),
    ).toBe(true);
    expect(
      isImmutableDaytonaImage(
        "ghcr.io/geetorusai/geetorus-daytona-runner@sha256:REPLACE_ME",
      ),
    ).toBe(false);
    expect(
      isImmutableDaytonaImage(
        "ghcr.io/geetorusai/geetorus-daytona-runner:e2e-latest",
      ),
    ).toBe(false);
  });
});
