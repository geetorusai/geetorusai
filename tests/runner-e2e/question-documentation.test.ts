...
function recording() {
  ...
  const runs = [1, 2, 3].map(id => ({ id: String(id), status: "succeeded", runtimeMode: "native", runnerProfileJson: { nativeExecutionInput: { task: { prompt: "Use Geetorus's request_human_input for durable task questions." } } } }));
