  ...
  const checks = [
    ...
    },
    {
      ...
      passed: inputs.length >= 3 && inputs.every(prompt => typeof prompt === "string"
        && prompt.includes("Use Geetorus's request_human_input for durable task questions.")
        && !prompt.includes("## Questions that need a user response")
        && !prompt.includes("payload.questionSet")
        && !prompt.includes("Create a durable human question or approval card on the current Geetorus task bound to this run")),
