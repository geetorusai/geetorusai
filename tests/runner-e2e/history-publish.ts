...
interface BundleManifest {
  schema: "geetorus.runner-e2e.bundle/v1";
  campaignId: string;
  bundleDigest: string;
...
export function buildHistoryPointers(history: RunnerE2EHistoryIndex) {
  ...
  return {
    latest: {
      schema: "geetorus.runner-e2e.pointer/v1",
...
async function main() {
  const reportRoot = path.resolve(
    process.env.GEETORUS_RUNNER_E2E_REPORT_DIR ??
  ...
  if (campaign.schema !== "geetorus.runner-e2e.campaign/v2") {
