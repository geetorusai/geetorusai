...
async function main() {
  ...
  const expected = JSON.parse(
    required("GEETORUS_RUNNER_E2E_EXPECTED_IDS"),
  ) as unknown;
  if (
  ...
  const jobs = JSON.parse(
    await readFile(required("GEETORUS_RUNNER_E2E_JOBS_JSON"), "utf8"),
  ...
  const selections = await selectRerunArtifacts({
    artifactRoot: required("GEETORUS_RUNNER_E2E_ARTIFACT_ROOT"),
    selectedRoot: required("GEETORUS_RUNNER_E2E_SELECTED_ROOT"),
    jobs,
    expectedExecutionIds: expected,
    workflowRunId: runId,
    workflowRunAttempt: Number(required("GITHUB_RUN_ATTEMPT")),
    sourceSha: required("GEETORUS_RUNNER_E2E_SOURCE_SHA"),
    sourceRef: required("GEETORUS_RUNNER_E2E_SOURCE_REF"),
