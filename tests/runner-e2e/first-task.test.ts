...
function recording(caseId = "task-reply-accept"): FirstTaskEvidence {
  ...
  const opening: FirstTaskCheckpoint = {
    ...
    agents: [{ id: "agent" }],
    comments: [
      { id: "greeting", authorAgentId: "agent", body: "Welcome to Geetorus" },
    ],
    interactions: [
      {
        id: "opening",
...
Accept the card above and I write it. This task stays in review until then.`;
    ...
    (id) => {
      ...
      const original = {
        adapterConfig: {
          ...
          geetorusSkillSync: { desiredSkills: ["first-task"] },
          model: null,
        },
      ...
      expect(patch.adapterType).toBe("geetorus_runner");
      expect(patch.adapterConfig).toMatchObject({
        instructionsFilePath: "/managed/AGENTS.md",
        geetorusSkillSync: original.adapterConfig.geetorusSkillSync,
        ...
        adapterConfig: {
          geetorusSkillSync: {
            desiredSkills: [
              ...
              "geetorusai/geetorus/first-task",
            ],
          },
        },
      });
      expect(
        (
          withOperational.adapterConfig.geetorusSkillSync as {
            desiredSkills: string[];
          }
        ).desiredSkills,
      ).toEqual(["geetorusai/geetorus/first-task"]);
  ...
  it("keeps source and display hashes distinct when instruction examples are redacted", () => {
    ...
    const snapshot = snapshotInstruction("geetorus/SKILL.md", content);
