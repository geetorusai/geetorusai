...
async function running(
  request: APIRequestContext,
  issueId: string,
  adapter: "process" | "geetorus_runner",
) {
  let run:
...
for (const adapter of ["process", "geetorus_runner"] as const) {
  test(`${adapter}: queue, composer Stop, subtree pause/cancel, and resume`, async ({
    page,
    request,
  }, testInfo) => {
    test.skip(
      adapter === "geetorus_runner" && !process.env.GEETORUS_STOP_FAKE_CODEX,
      "Set GEETORUS_STOP_FAKE_CODEX and GEETORUS_RUNNER_BINARY for real runnerd with the deterministic provider.",
    );
    ...
    try {
          ...
          .poll(
            async () => {
              const calls = await readFile(
                process.env.GEETORUS_STOP_CODEX_LOG!,
                "utf8",
              ).catch(() => "");
      ...
      if (adapter === "process") {
        ...
        expect(
          await readFile(process.env.GEETORUS_STOP_CODEX_LOG!, "utf8"),
      ...
      if (adapter === "geetorus_runner") {
        await expect.poll(async () => {
          const calls = await readFile(process.env.GEETORUS_STOP_CODEX_LOG!, "utf8");
          return calls.split("turn/start").length - 1;
