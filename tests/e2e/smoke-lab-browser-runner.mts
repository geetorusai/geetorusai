...
const DEMO_EMAIL = "smoke@geetorus.test";
  ...
  for (const scenario of scenarios) {
    ...
    await doStep(scenario, "connect", async () => {
      if (scenario.authMode === "oauth") {
        ...
        // commit (chrome-error page) before navigating into the Geetorus UI.
    ...
    await doStep(scenario, "revoke", async () => {
      if (scenario.transport === "gateway_session") {
        ...
        const listRes = await fetch(`${BASE}${new URL(session.toolsUrl, BASE).pathname}`, { headers: { "x-geetorus-tool-gateway-token": session.token } });
        ...
        const after = await fetch(`${BASE}${new URL(session.toolsUrl, BASE).pathname}`, { headers: { "x-geetorus-tool-gateway-token": session.token } });
