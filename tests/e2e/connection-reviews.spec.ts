...
function reviewAgentScript(connectionId: string, query: string) {
  return `
const base = process.env.GEETORUS_API_URL + "/api";

const headers = { authorization: "Bearer " + process.env.GEETORUS_API_KEY, "content-type": "application/json", "x-geetorus-run-id": process.env.GEETORUS_RUN_ID };
...
const issueId = process.env.GEETORUS_TASK_ID ?? (await api("/heartbeat-runs/" + process.env.GEETORUS_RUN_ID)).contextSnapshot.issueId;
...
const session = await api("/tool-gateway/sessions", "POST", { runId: process.env.GEETORUS_RUN_ID, ttlMs: 60000 });
const gatewayHeaders = { "x-geetorus-tool-gateway-token": session.token, "content-type": "application/json" };
...
for (const journey of [
  ...
  test(`connection review: ${journey}, synchronized task history and actual continuation`, async ({
    ...
    test.skip(
      journey === "restart" && !process.env.GEETORUS_REVIEW_RESTART_FILE,
    ...
    try {
      ...
      if (journey === "restart") {
        const control = process.env.GEETORUS_REVIEW_RESTART_FILE!;
