...
for (const journey of [
  ...
  test(`fresh execution recovery and current-request journey: ${journey}`, async ({
    ...
    try {
      ...
      const env = {
        ...process.env,
        GEETORUS_RECOVERY_CEO_LINEAGE: journey === "ceo_lineage" ? "1" : "0",
      ...
      await expect
        .poll(
          () =>
            logs.match(
              /Geetorus is ready at (http:\/\/127\.0\.0\.1:\d+)/,
            )?.[1],
          { timeout: 100_000 },
        )
        .toBeTruthy();
      ...
      const nativeRunnerToggle = page.getByRole("switch", {
        name: "Toggle Geetorus Runner experimental setting",
      });
      ...
      if (journey === "safe_restart") {
        ...
        base = logs
          .slice(startOffset)
          .match(/Geetorus is ready at (http:\/\/127\.0\.0\.1:\d+)/)![1]!;
