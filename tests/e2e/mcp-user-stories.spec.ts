...
function buildGatewayCallScript(connectionId: string, toolName: string, parameters: Json = {}) {
  return `
const required = ["GEETORUS_API_URL", "GEETORUS_API_KEY", "GEETORUS_RUN_ID"];
for (const key of required) {
  if (!process.env[key]) throw new Error(\`Missing \${key}\`);
}
const headers = {
  "authorization": \`Bearer \${process.env.GEETORUS_API_KEY}\`,
  "content-type": "application/json"
};
const sessionRes = await fetch(\`\${process.env.GEETORUS_API_URL}/api/tool-gateway/sessions\`, {
  method: "POST",
  headers,
...
const toolsRes = await fetch(\`\${process.env.GEETORUS_API_URL}/api/tool-gateway/tools\`, {
...
const callRes = await fetch(\`\${process.env.GEETORUS_API_URL}/api/tool-gateway/tools/call\`, {
  method: "POST",
  headers: {
    "content-type": "application/json",
    "x-geetorus-tool-gateway-token": session.token
  },
  body: JSON.stringify({
    tool: tool.name,
