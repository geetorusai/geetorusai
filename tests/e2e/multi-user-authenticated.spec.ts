...
const BASE = process.env.GEETORUS_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.GEETORUS_E2E_DATA_DIR ?? process.env.GEETORUS_HOME;
const CONFIG_PATH = process.env.GEETORUS_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".geetorus/config.json");
const BOOTSTRAP_SCRIPT_PATH = path.resolve(process.cwd(), "packages/db/scripts/create-auth-bootstrap-invite.ts");
const OWNER_PASSWORD = "geetorus-owner-password";
const INVITED_PASSWORD = "geetorus-invited-password";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@geetorus.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@geetorus.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("GEETORUS_E2E_DATA_DIR or GEETORUS_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Geetorus" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your Geetorus account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your Geetorus instance" })).toBeVisible();
  ...
  try {
    ...
    expect(joined.ok).toBe(true);
      ...
      try {
        ...
        const ownerRecent = await page.evaluate(() => Object.keys(localStorage).filter(key => key.startsWith("geetorus.recentAgentChats:")));
        const memberRecent = await invited.page.evaluate(() => Object.keys(localStorage).filter(key => key.startsWith("geetorus.recentAgentChats:")));
