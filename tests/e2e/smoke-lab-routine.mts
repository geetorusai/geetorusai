 ...
 * Geetorus company — provided in every routine run's env):
 *   GEETORUS_API_URL, GEETORUS_API_KEY, GEETORUS_COMPANY_ID, GEETORUS_RUN_ID
 *   ROUTINE_ISSUE_ID=<uuid>            issue to record against (default GEETORUS_TASK_ID)
...
const CP_BASE = (() => {
  const b = (process.env.GEETORUS_API_URL ?? "").replace(/\/$/, "");
  return b.replace(/\/api$/, "");
})();
const CP_KEY = process.env.GEETORUS_API_KEY ?? "";
const CP_COMPANY = process.env.GEETORUS_COMPANY_ID ?? "";
const CP_RUN = process.env.GEETORUS_RUN_ID ?? "";
const ROUTINE_ISSUE_ID = process.env.ROUTINE_ISSUE_ID ?? process.env.GEETORUS_TASK_ID ?? "";
...
async function cpUploadAsset(file: string): Promise<string | null> {
  ...
  try {
    ...
    const res = await fetch(`${CP_BASE}/api/companies/${CP_COMPANY}/assets/images`, {
      method: "POST",
      headers: { authorization: `Bearer ${CP_KEY}`, "x-geetorus-run-id": CP_RUN },
      body: form as any,
    });
    if (!res.ok) {
