  ...
  for (const provider of PROVIDERS) {
    test(`${provider.name}: catalog, setup, and connection management tabs`, async ({
      ...
      if (provider.provider === "slack") {
        ...
        await expect(
          page.getByText(
            `https://geetorus.example.test/api/chat-webhooks/public-slack/slack`,
            { exact: true },
          ),
        ).toBeVisible();
        await expect(
        ...
        await expect(
          page.getByText(
            /Slack's bare \/status command is not a Geetorus control/,
          ),
        ).toBeVisible();
        ...
        await expect(saveChangesStep).toHaveText(
          "Return to App Manifest in Slack and click Save Changes. The copied manifest already contains the event, interaction, and slash-command URLs. Slack verifies the Events URL when you save; Geetorus records Interactivity and slash command health only after each signed callback is observed.",
        );
      ...
      await expect(
        page.getByText(
          /Their tasks run only with an isolated workspace and sandbox environment; otherwise Geetorus safely refuses the request/,
        ),
      ).toBeVisible();
      ...
      const confirmationUrl = `https://geetorus.example.test/${seed.prefix}/chat-identity/confirm?token=e2e-redacted`;
      await expect(
  ...
  for (const theme of ["light", "dark"] as const) {
    test(`discovers dedicated lines and completes the channel wizard (${theme})`, async ({
      ...
      await page.addInitScript(
        (value) => localStorage.setItem("geetorus.theme", value),
        theme,
      );
      await page.goto(`/${seed.prefix}/apps`);
