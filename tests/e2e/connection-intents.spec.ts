...
function connectionAwareScript(connectionId: string) {
  return `
const post = async (url, body, token = process.env.GEETORUS_RUNTIME_TOOLS_TOKEN) => {
...
const search = await post(process.env.GEETORUS_RUNTIME_TOOLS_CONNECTIONS_SEARCH_URL, { query: "notion" });
...
if (notion.state !== "ready") {
  const requested = await post(process.env.GEETORUS_RUNTIME_TOOLS_CONNECTION_REQUEST_URL, { service: notion.service });
...
const apiHeaders = { authorization: \`Bearer \${process.env.GEETORUS_API_KEY}\`, "content-type": "application/json" };
const sessionResponse = await fetch(\`\${process.env.GEETORUS_API_URL}/api/tool-gateway/sessions\`, {
  method: "POST",
  headers: apiHeaders,
  body: JSON.stringify({ runId: process.env.GEETORUS_RUN_ID, ttlMs: 60000 })
});
if (!sessionResponse.ok) throw new Error(await sessionResponse.text());
const session = await sessionResponse.json();
const toolsResponse = await fetch(\`\${process.env.GEETORUS_API_URL}/api/tool-gateway/tools\`, {
  headers: { "x-geetorus-tool-gateway-token": session.token }
});
...
const call = await fetch(\`\${process.env.GEETORUS_API_URL}/api/tool-gateway/tools/call\`, {
  method: "POST",
  headers: { "x-geetorus-tool-gateway-token": session.token, "content-type": "application/json" },
