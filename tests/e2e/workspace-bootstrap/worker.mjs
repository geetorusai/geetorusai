...
const headers = { Authorization: `Bearer ${env.GEETORUS_API_KEY}`, "X-Geetorus-Run-Id": env.GEETORUS_RUN_ID, "Content-Type": "application/json" };
const api = async (route, method = "GET", body) => {
  const response = await fetch(`${env.GEETORUS_API_URL}/api${route}`, { method, headers, body: body ? JSON.stringify(body) : undefined });
  assert(response.ok, `${response.status}: ${await response.clone().text()}`);
  return response.json();
};
const run = await api(`/heartbeat-runs/${env.GEETORUS_RUN_ID}`);
const issueId = run.contextSnapshot.issueId;
const repositories = await readdir(path.join(process.cwd(), ".geetorus-repositories"));
const repo = path.join(process.cwd(), ".geetorus-repositories", repositories.find((name) => !name.includes(".clone-")));
