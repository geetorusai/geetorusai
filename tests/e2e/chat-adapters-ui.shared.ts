 ...
 *
 * Provider APIs are deliberately not contacted here. The shared Geetorus
...
export const PROVIDERS: ProviderCase[] = [
  ...
  },
  {
    ...
    slug: "github",
    name: "GitHub",
    accountLabel: "geetorus-ai",
    botLabel: "Maya",
    // GitHub's API returns the actor login with this suffix, while people
    // invoke the App with the bare slug.
    botUsername: "maya-geetorus[bot]",
    resourceLabel: "geetorus-ai/geetorus",
    secondaryResourceLabel: "geetorus-ai/chat-e2e",
    resourceType: "repository",
    externalUrl: "https://github.com/geetorus-ai/geetorus/issues/123",
  ...
  },
  {
    ...
    slug: "telegram",
    name: "Telegram",
    accountLabel: "@maya_geetorus_bot",
    botLabel: "Maya",
    botUsername: "maya_geetorus_bot",
    resourceLabel: "Maya test chat",
    secondaryResourceLabel: "Maya group chat",
    resourceType: "direct_message",
    externalUrl: "https://t.me/maya_geetorus_bot",
...
> = {
  ...
  telegram: {
    ...
    remove:
      "queues durable removal of its Telegram webhook and command menu. After Telegram confirms that cleanup, Geetorus retires the saved token",
  },
};
...
export function endpointFixture(provider: ProviderCase, seed: Seed) {
  ...
  return {
    ...
    setup: {
      ...
      webhookUrl: `https://geetorus.example.test/api/chat-webhooks/public-${provider.provider}/${provider.provider}`,
      messagingEndpoint: `https://geetorus.example.test/api/chat-webhooks/public-${provider.provider}/microsoft-teams`,
...
export async function installChatControlPlaneMock(
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (
      ...
      await fulfill(route, {
        confirmationUrl: `https://geetorus.example.test/${seed.prefix}/chat-identity/confirm?token=e2e-redacted`,
      });
      return;
    }
...
export async function fillProviderSetup(page: Page, provider: ProviderCase) {
  if (provider.provider === "slack") {
    ...
    await expect(page.locator("body")).not.toContainText("geetorus-test.pem");
...
export async function expectMinimumProviderSetup(page: Page, provider: ProviderCase) {
  const webhookUrl = `https://geetorus.example.test/api/chat-webhooks/public-${provider.provider}/${provider.provider}`;
  if (provider.provider === "slack") {
  ...
  if (provider.provider === "microsoft-teams") {
    const messagingEndpoint =
      "https://geetorus.example.test/api/chat-webhooks/public-microsoft-teams/microsoft-teams";
    await expect(
    ...
    expect(manifest).not.toContain("api://geetorus-chat/");
