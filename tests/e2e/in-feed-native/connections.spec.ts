...
for (const journey of ['connect', 'decline', 'restart'] as const) test(`fresh native runner connection journey: ${journey}`, async ({ page }, info) => {
  ...
  try {
    ...
    const env = { ...process.env, IN_FEED_FIXTURE_KEY: 'not-a-real-model-key', NODE_ENV: 'test', GEETORUS_TEST_CONNECTION_DELIVERY_HOLD: journey === 'restart' ? '1' : '0', PATH: `${root}/tests/e2e/fixtures/in-feed-bin:${process.env.PATH}` };
    ...
    await expect.poll(() => logs.match(/Geetorus is ready at (http:\/\/127\.0\.0\.1:\d+)/)?.[1], { timeout: 100_000 }).toBeTruthy();
    let base = logs.match(/Geetorus is ready at (http:\/\/127\.0\.0\.1:\d+)/)![1]!;
    ...
    await page.getByRole('switch', { name: 'Toggle Geetorus Runner experimental setting' }).click();
    ...
    await page.getByRole('button', { name: /Geetorus Runner/ }).click();
    await page.getByRole('button', { name: 'Save', exact: true }).first().click();
    await expect.poll(async () => (await api(`/agents/${agent.id}`)).adapterType).toBe('geetorus_runner');
    ...
    if (journey === 'restart') {
      ...
      logs = '';
      processHandle = spawn(process.execPath, ['cli/node_modules/tsx/dist/cli.mjs', 'cli/src/index.ts', 'test-drive', '--harness', 'codex', '--api-key-env', 'IN_FEED_FIXTURE_KEY', '--data-dir', dataDir, '--no-browser'], { cwd: root, env: { ...env, GEETORUS_TEST_CONNECTION_DELIVERY_HOLD: '0' }, detached: true, stdio: ['ignore', 'pipe', 'pipe'] });
      processHandle.stdout!.on('data', (chunk) => { logs += chunk.toString(); });
      processHandle.stderr!.on('data', (chunk) => { logs += chunk.toString(); });
      await expect.poll(() => logs.match(/Geetorus is ready at (http:\/\/127\.0\.0\.1:\d+)/)?.[1], { timeout: 100_000 }).toBeTruthy();
      base = logs.match(/Geetorus is ready at (http:\/\/127\.0\.0\.1:\d+)/)![1]!;
