...
const RESULT_SCHEMA = "geetorus.runner-acceptance.result/v1" as const;
...
function missingResult(cell: RunnerAcceptanceCell, generatedAt: string): AggregatedAcceptanceResult {
  return {
    schema: "geetorus.runner-acceptance.result/v1",
    cellId: cell.id,
    attempt: 0,
    status: "failed",
...
export function buildRunnerAcceptanceReport(input: {
  ...
  return {
    schema: "geetorus.runner-acceptance.report/v1",
    generatedAt,
