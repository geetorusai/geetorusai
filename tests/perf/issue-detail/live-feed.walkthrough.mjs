...
const target = process.env.GEETORUS_LAYOUT_LIVE_URL;
if (!target || !["localhost", "127.0.0.1", "[::1]"].includes(new URL(target).hostname)) {
  throw new Error("Set GEETORUS_LAYOUT_LIVE_URL to a disposable localhost task with a native Codex assignee.");
}
...
try {
  ...
  assert.equal(agent.adapterType, "geetorus_runner");
  ...
  assert.equal(run.adapterType, "geetorus_runner");
