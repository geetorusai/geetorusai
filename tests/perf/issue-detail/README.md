...
The rig starts an isolated seeded Geetorus instance, runs five samples for each scenario/profile, and writes median-ready raw data plus a Markdown table to `test-results/issue-detail-perf/`.
...
```sh
GEETORUS_ISSUE_PERF_RUNS=7 GEETORUS_ISSUE_PERF_PORT=3210 pnpm exec playwright test --config tests/perf/issue-detail/playwright.config.ts
```
...
```sh
GEETORUS_ISSUE_PERF_BASE_URL=http://127.0.0.1:3102 \
...
configured native Geetorus runner and Codex authentication; deterministic
browser fixtures do not substitute for watching an actual provider run.

For the live walkthrough, first assign a disposable task to a native Geetorus
...
```sh
GEETORUS_LAYOUT_LIVE_URL=http://127.0.0.1:3102/LAY/issues/LAY-8 \
