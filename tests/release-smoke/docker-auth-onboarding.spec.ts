...
const ADMIN_EMAIL =
  process.env.GEETORUS_RELEASE_SMOKE_EMAIL ??
  process.env.SMOKE_ADMIN_EMAIL ??
  "smoke-admin@geetorus.local";
const ADMIN_PASSWORD =
  process.env.GEETORUS_RELEASE_SMOKE_PASSWORD ??
  process.env.SMOKE_ADMIN_PASSWORD ??
  "geetorus-smoke-password";
...
// GEETORUS_RELEASE_SMOKE_ANTHROPIC_API_KEY and fail up front without it.
...
const BASE_URL =
  process.env.GEETORUS_RELEASE_SMOKE_BASE_URL ?? "http://127.0.0.1:3232";
const TARGET_IS_LOOPBACK = /^https?:\/\/(localhost|127\.0\.0\.1)([:/]|$)/i.test(
  BASE_URL
);
const ANTHROPIC_API_KEY =
  process.env.GEETORUS_RELEASE_SMOKE_ANTHROPIC_API_KEY ??
...
const FIRST_TASK_TITLE = "Geetorus onboarding";
...
const ONBOARDING_DRAFT_STORAGE_KEY = "geetorus-onboarding-state";
...
test.describe("Docker authenticated onboarding smoke", () => {
  test("logs in, completes onboarding, and hires the lead agent", async ({
    ...
    expect(
      ANTHROPIC_API_KEY,
      "This target reaches the real provider — set GEETORUS_RELEASE_SMOKE_ANTHROPIC_API_KEY to a key it accepts"
    ).toBeTruthy();
    ...
    // half-finished) …
    await expect(
      page.getByText("Welcome to Geetorus!").first()
    ).toBeVisible({ timeout: 20_000 });
