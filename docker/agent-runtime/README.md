...
Container images for running coding-agent harnesses in sandboxed environments (for example the kubernetes sandbox provider, stage 1 of the k8s contribution). Images are named `agent-runtime-{harness}:{version}` and published to `ghcr.io/geetorusai/` by the `agent-runtime-images` workflow. The registry is overridable: every reference flows through the `REGISTRY` bake variable.
...
- Non-root user `geetorus` (uid/gid 1000)
...
**Geetorus Binaries:**
- `/usr/local/bin/geetorus-agent-shim`: Go binary compiled from `tools/agent-shim/`. Reads `/run/geetorus/runtime-command.json` and `syscall.Exec`s the harness CLI.

**Defaults:**
- `USER`: 1000:1000 (geetorus, non-root)
- `WORKDIR`: `/workspace` (mount workspace volumes here)
- `ENTRYPOINT`: `/usr/bin/tini --` (PID-1 reaper, forwards signals)
- `CMD`: `/usr/local/bin/geetorus-agent-shim`
...
docker run --rm ghcr.io/geetorusai/agent-runtime-claude:dev claude-code --version
```

## Agent Container (geetorus-agent-shim)

The main agent process runs as the shim (PID 1 under tini). The shim:

1. Reads `/run/geetorus/runtime-command.json` (path overridable via `-spec`), a JSON file mounted by whatever schedules the run
