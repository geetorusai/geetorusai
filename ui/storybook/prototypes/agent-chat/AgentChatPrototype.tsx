...
const logItems = [
  ...
  },
  {
    ...
    item: {
      id: "save-plan",
      type: "command_execution",
      command: "geetorus documents update PAP-241 plan",
    },
  },
  {
    type: "item.completed",
...
export function AgentChatPrototype({
  ...
  useLayoutEffect(() => {
    ...
    const recentKey = `geetorus.recentAgentChats:${issue.companyId}:user-board`;
      ...
      if (path.endsWith("/activity") && scenario.startsWith("project-") && scenario !== "project-failed" && scenario !== "project-reused") return Response.json([{
        ...
        createdAt: "2026-09-10T15:42:30Z", details: {
          ...
          repositories: scenario === "project-no-repo" ? [] : [
            { id: "1", name: "geetorusai/geetorus", url: "https://github.com/geetorusai/geetorus" },
            ...(scenario === "project-multi-repo" ? [{ id: "2", name: "geetorusai/onboarding", url: "https://github.com/geetorusai/onboarding" }] : []),
          ],
        },
      }]);
    ...
    return () => {
      ...
      if (entryScenario) {
        ...
        window.dispatchEvent(new Event("geetorus:recent-agent-chats"));
      }
      queryClient.clear();
    };
