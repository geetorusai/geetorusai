...
import type { AdapterEnvironmentTestResult, Agent } from "@geetorusai/shared";
import { storybookAgents, storybookHiredAgent } from "../fixtures/geetorusData";
...
import { models as claudeModels } from "@geetorusai/adapter-claude-local";
import { models as codexModels } from "@geetorusai/adapter-codex-local";
import { models as openCodeModels } from "@geetorusai/adapter-opencode-local";
import { models as cursorModels } from "@geetorusai/adapter-cursor-local";
import { models as geminiModels } from "@geetorusai/adapter-gemini-local";
import { models as grokModels } from "@geetorusai/adapter-grok-local";
import { models as kimiModels } from "@geetorusai/adapter-kimi-local";
...
export const NEW_AGENT_ADAPTERS = [
  ...
  "grok_local", "kimi_local", "opencode_local", "pi_local", "hermes_local", "geetorus_runner",
] as const;
...
export function NewAgentWizard({ initialScreen = "name", initialAdapter = null, initialName = "", initialOpen = true, cloud = false, nativeRunnerEnabled = true,
  ...
  const [connections, setConnections] = useState<Record<string, ConnectionMethod | undefined>>(() => {
    ...
    const key = initialAdapter === "geetorus_runner" ? `${initialAdapter}/${initialRunnerProvider}` : initialAdapter ?? "claude_local";
  ...
  const isRunner = selected === "geetorus_runner";
  ...
  async function runTest() {
    ...
    try {
      const result = await agentsApi.testEnvironment(PREVIEW_COMPANY_ID, selected, {
        adapterConfig,
        environmentId: environment === "Geetorus Computer" ? "environment-storybook-sandbox"
      ...
      {modal ? <>
                ...
                <form className="flex min-h-0 flex-col" onSubmit={event => {
                  ...
                  <div className="flex min-h-0 flex-col gap-7 overflow-y-auto px-6 pb-8 sm:px-10">
                    ...
                    {screen === "name" ? <div className="flex flex-col gap-2">
                      ...
                      {adapter === "geetorus_runner" && <SelectField label="Runner" value={runnerProvider}
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex min-w-0 flex-col gap-2">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><AdapterMark adapter={brandAdapter} /><span>{getAdapterDisplay(brandAdapter).label}</span>{isRunner && <Badge variant="outline">{runnerProvider === "Codex (app server)" ? "Native app server runner" : runnerProvider === "Claude (ACPX)" ? "ACPX runner" : "Geetorus Runner"}</Badge>}</div>
            </div>
          </div>
        </div>
        ...
        {screen === "connect" && connectionProvider ? <OnboardingCard className="mx-auto">
          ...
          {selected !== "cursor_cloud" && <Section title="Environment">
            <SelectField label="Environment" hideLabel value={environment} onChange={value => { setEnvironments(previous => ({ ...previous, [selected]: value })); resetTest(); }} options={["Organization default", "Geetorus Computer", "Local machine"]} />
          </Section>}
          </fieldset>
