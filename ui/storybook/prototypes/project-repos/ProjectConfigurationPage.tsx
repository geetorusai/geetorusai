...
import type { Project } from "@geetorusai/shared";
...
import { storybookProjects } from "../../fixtures/geetorusData";
  ...
  return (
    <aside className="flex h-full w-60 shrink-0 flex-col bg-sidebar text-sidebar-foreground" aria-label="Bull navigation">
      ...
      <nav className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto p-4">
        <a className="flex items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-sidebar-accent" href="https://bull.staging.geetorus.app/BUL/issues" target="_blank" rel="noreferrer"><SquarePen className="size-4" />New Task</a>
        {navigation.map(({ label, icon: Icon, path, heading }) => <div key={label}>
          ...
          <a href={`https://bull.staging.geetorus.app/BUL/${path}`} target="_blank" rel="noreferrer" aria-current={path === "projects" ? "page" : undefined} className={cn("flex items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-sidebar-accent", path === "projects" && "bg-sidebar-accent")}><Icon className="size-4" />{label}</a>
        </div>)}
      </nav>
