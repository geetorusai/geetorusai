...
import type { Agent, AgentSkillSnapshot, CompanySkillListItem } from "@geetorusai/shared";
...
const library: CompanySkillListItem[] = [
  libSkill("geetorus", "Geetorus", "Coordination skill: heartbeats, checkout, comments, and routine API patterns."),
  libSkill("design-guide", "Design guide", "Geetorus UI design system reference: tokens, typography, status colors."),
];
...
function staleSnapshot(): AgentSkillSnapshot {
  return {
    ...
    supported: true,
    mode: "ephemeral",
    desiredSkills: ["geetorus", "legacy-retired-skill"],
    warnings: [],
    entries: [
      {
        key: "geetorus",
  ...
  return {
    ...
    entries: [
      ...
      },
      {
        ...
        sourcePath: null,
        targetPath: null,
        detail: "Detected in the adapter's skills directory; managed outside Geetorus.",
      },
...
export const StaleWarning: StoryObj = {
  ...
  render: () => (
    ...
    >
      <EdgeState agent={buildAgent("agent-stale", ["geetorus", "legacy-retired-skill"])} snapshot={staleSnapshot()} />
    </StoryFrame>
  ),
};
...
export const DetectedAdapter: StoryObj = {
  ...
  render: () => (
    <StoryFrame
      ...
      subtitle="A user-installed skill outside Geetorus's management shows in a collapsed, read-only section with a lock icon."
    >
      <EdgeState agent={buildAgent("agent-detected", ["geetorus"])} snapshot={detectedSnapshot()} />
    </StoryFrame>
  ),
};
