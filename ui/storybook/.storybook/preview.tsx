...
function installStorybookApiFixtures() {
  ...
  const currentWindow = window as typeof window & {
    __geetorusStorybookFetchInstalled?: boolean;
  };
  if (currentWindow.__geetorusStorybookFetchInstalled) return;

  const originalFetch = window.fetch.bind(window);
  currentWindow.__geetorusStorybookFetchInstalled = true;
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (/^\/api\/companies\/[^/]+\/ai-connections\/local\/attempts$/.test(url.pathname)) {
      ...
      return Response.json({ sessionId: "local-storybook", expiresAt: new Date(Date.now() + 1_800_000).toISOString(), command: body.provider === "anthropic"
        ? "CLAUDE_CONFIG_DIR='/geetorus/login' claude auth login"
        : "CODEX_HOME='/geetorus/login' codex login --device-auth" });
    }
    ...
    if (adapterSchemaMatch) {
      ...
      const schemas = (
        window as typeof window & {
          __geetorusStorybookAdapterSchemas?: Record<string, unknown>;
        }
      ).__geetorusStorybookAdapterSchemas;
...
const preview: Preview = {
  ...
  globalTypes: {
    theme: {
      description: "Geetorus color mode",
      defaultValue: "dark",
      toolbar: {
        title: "Theme",
        icon: "mirror",
