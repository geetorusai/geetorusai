...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Geetorus", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Geetorus",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Geetorus");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Geetorus");
    expect(buildDocumentTitle([], null)).toBe("Geetorus");
  });
});
