...
import type { Company } from "@geetorusai/shared";
...
describe("CompanyProvider", () => {
  ...
  it("keeps the stored company when the company request fails", async () => {
    ...
    localStorage.setItem("geetorus.selectedCompanyId", "company-a");
    ...
    expect(localStorage.getItem("geetorus.selectedCompanyId")).toBe("company-a");
  });

  it("does not expose a stale stored company id before companies load", async () => {
    localStorage.setItem("geetorus.selectedCompanyId", "stale-company");
  ...
  it("replaces a stale stored company id with the first loaded company", async () => {
    ...
    expect(localStorage.getItem("geetorus.selectedCompanyId")).toBe("company-1");
  });
  ...
  describe("when the account changes in this tab", () => {
    ...
    it("drops the previous account's selection and re-reads the list", async () => {
      ...
      expect(localStorage.getItem("geetorus.selectedCompanyId")).toBe("company-2");
    });
    ...
    it("does not treat a never-answered session as a signed-out session", async () => {
      ...
      localStorage.setItem("geetorus.selectedCompanyId", "company-1");
