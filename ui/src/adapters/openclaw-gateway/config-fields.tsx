...
export function OpenClawGatewayConfigFields({
  ...
  return configFieldsForSection(section, (
    <>
        ...
        <Field label="Session key">
          <DraftInput
            value={
              isCreate
                ? values!.sessionKey ?? ""
                : eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "geetorus"))
            }
            onCommit={(v) =>
              isCreate
                ? set!({ sessionKey: v })
      ...
      <Field label="Geetorus API URL override">
        <DraftInput
          value={
            isCreate
              ? values!.geetorusApiUrl ?? ""
              : eff("adapterConfig", "geetorusApiUrl", String(config.geetorusApiUrl ?? ""))
          }
          onCommit={(v) =>
            isCreate
              ? set!({ geetorusApiUrl: v })
              : mark("adapterConfig", "geetorusApiUrl", v || undefined)
          }
          immediate
          className={inputClass}
          placeholder="https://geetorus.example"
        />
      </Field>
      ...
      {!isCreate && (
        <Field label="Claimed API key path">
          <DraftInput
            ...
            immediate
            className={inputClass}
            placeholder="~/.openclaw/workspace/geetorus-claimed-api-key.json"
          />
        </Field>
      )}
      ...
      <Field label="Device auth">
        <div className="text-xs text-muted-foreground leading-relaxed">
          When enabled, Geetorus persists a device key during onboarding so pairing approvals
