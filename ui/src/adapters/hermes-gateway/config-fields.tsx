...
export function HermesGatewayConfigFields({
  ...
  const geetorusApiUrl = String(readValue("geetorusApiUrl", "") ?? "");
  ...
  return configFieldsForSection(section, (
    <>
      <Field
        label="API base URL"
        hint="Hermes API server base URL that Geetorus can reach, such as http://127.0.0.1:8642 or a private HTTPS URL. Default dashboard root/chat URLs such as http://127.0.0.1:9119/chat are accepted and map to /api."
      >
        <DraftInput
          value={apiBaseUrl}
      ...
      <SecretField
        ...
        placeholder="Hermes API_SERVER_KEY, not GEETORUS_API_KEY"
        stored={!isCreate && hasStoredApiKey && !editApiKeyValue}
      />

      <Field
        label="Geetorus API URL"
        hint="Optional Geetorus API URL reachable by the Hermes host. This is not a credential."
      >
        <DraftInput
          value={geetorusApiUrl}
          onCommit={(v) => writeValue("geetorusApiUrl", v || undefined)}
          immediate
          className={inputClass}
