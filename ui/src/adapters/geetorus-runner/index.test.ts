...
import { geetorusRunnerUIAdapter } from "./index";

describe("geetorus runner transcript projection", () => {
  it("renders committed PRP semantic tool items with the existing chat parts", () => {
    const started = geetorusRunnerUIAdapter.parseStdoutLine(JSON.stringify({
      type: "geetorus.prp.event",
      event: {
        eventType: "item.started",
        payload: { item: { type: "tool_use", id: "call-1", name: "get_task_context", input: {} } },
      },
    }), "2026-08-21T12:00:00.000Z");
    const completed = geetorusRunnerUIAdapter.parseStdoutLine(JSON.stringify({
      type: "geetorus.prp.event",
      event: {
  ...
  it("maps native Codex deltas, camel-case tools, and usage into the shared chat transcript", () => {
    const parse = geetorusRunnerUIAdapter.createStdoutParser!().parseLine;
    ...
    expect(event("item.completed", {
      ...
      expect.objectContaining({ kind: "result", subtype: "geetorus.usage", inputTokens: 120, outputTokens: 30 }),
    ]);
  });
  ...
  it("never exposes the structured task result envelope as final-response prose", () => {
    ...
    expect(event("item.delta", { kind: "agentMessage", channel: "final", text: "\"geetorus.run_result.v1\"}" })).toEqual([]);
  ...
  it("projects every canonical provider family as structured activity instead of JSON prose", () => {
    ...
    for (const [eventType, family, payload] of cases) {
      const entries = geetorusRunnerUIAdapter.parseStdoutLine(JSON.stringify({
        type: "geetorus.prp.event",
        event: { eventType, payload },
      }), "2026-08-21T12:00:00.000Z");
    ...
    const modelRoute = geetorusRunnerUIAdapter.parseStdoutLine(JSON.stringify({
      type: "geetorus.prp.event",
  ...
  it("coalesces runtime request lifecycle data and emits terminal state", () => {
    ...
    expect(event("runtime_request.created", { request: { requestId: "request-2", requestKind: "runtime", type: "input", status: "pending", prompt: "Choose", input: { schema: "geetorus.question_set.v1", questions: [{ id: "environment", prompt: "Where?", required: true, answerMode: "single_select", options: [{ id: "staging", label: "Staging" }] }] } } }))
    ...
    expect(event("runtime_request.created", { request: { requestId: "request-redacted", requestKind: "runtime", type: "input", status: "pending", prompt: "Choose", input: { schema: "***REDACTED***", questions: [{ id: "environment", prompt: "Where?", required: true, answerMode: "single_select", options: [{ id: "staging", label: "Staging" }] }] } } }))
      .toEqual([expect.objectContaining({ kind: "runtime_request", requestId: "request-redacted", questionSet: expect.objectContaining({ schema: "geetorus.question_set.v1" }) })]);
    ...
    expect(event("runtime_request.created", { request: { requestId: "request-cancel", requestKind: "runtime", type: "input", status: "pending", prompt: "Cancel me", input: { schema: "geetorus.question_set.v1", questions: [{ id: "reason", prompt: "Why?", required: false, answerMode: "text" }] } } }))
  ...
  it("normalizes MCP semantic tools, canonical usage, and actionable failures", () => {
    ...
    expect(event("mcp_app.tool_input", {
      semantic_tool: { callId: "mcp-1", operationId: "geetorus.tasks.create", content: { references: [{ kind: "issue", id: "PAP-2" }] } },
    })).toEqual([expect.objectContaining({ kind: "tool_call", toolUseId: "mcp-1", name: "geetorus.tasks.create" })]);
    expect(event("mcp_app.tool_result", {
      semantic_tool: { callId: "mcp-1", operationId: "geetorus.tasks.create", outcome: "denied", code: "audience_denied" },
