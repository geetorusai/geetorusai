import type { GeetorusQuestion, GeetorusQuestionResponse, GeetorusQuestionSet, TranscriptEntry } from "@geetorusai/adapter-utils";
import type { UIAdapterModule } from "../types";
import { parseCodexStdoutLine, buildGeetorusRunnerConfig } from "@geetorusai/adapter-codex-local/ui";
import { CodexLocalConfigFields } from "../codex-local/config-fields";

type JsonRecord = Record<string, unknown>;

interface GeetorusRunnerParserState {
...
function structuredResultSummary(value: string): string | null {
  ...
  try {
    ...
    return text(parsed.schema) === "geetorus.run_result.v1" && text(parsed.summary)
      ? text(parsed.summary)
      : null;
  } catch {
    return null;
...
function itemId(event: JsonRecord, item: JsonRecord): string {
  return text(event.itemId, text(item.id, "geetorus-runner-item"));
}
...
function dynamicToolEntries(
  ...
  const id = itemId(event, item);
  const name = text(item.tool, text(item.name, "Geetorus tool"));
  if (phase === "started") {
...
function parseItemEvent(
  ...
  phase: "started" | "completed",
  ts: string,
  state: GeetorusRunnerParserState,
): TranscriptEntry[] {
...
function parseQuestionResponse(value: unknown): GeetorusQuestionResponse | null {
  const response = record(value);
  if (response.schema !== "geetorus.question_response.v1") return null;
  const rawAnswers = record(response.answers);
  const answers: GeetorusQuestionResponse["answers"] = {};
  ...
  return { schema: "geetorus.question_response.v1", answers };
}

function parseQuestionSet(value: unknown): GeetorusQuestionSet | null {
  ...
  if (input.schema !== undefined
    && input.schema !== "geetorus.question_set.v1"
    && input.schema !== "***REDACTED***") return null;
  const questions = input.questions.map(record).slice(0, 64).map((question, questionIndex): GeetorusQuestion => {
    const answerMode: GeetorusQuestion["answerMode"] = question.answerMode === "single_select" || question.answerMode === "multi_select" ? question.answerMode : "text";
    ...
    const inputType: NonNullable<GeetorusQuestion["textValidation"]>["inputType"] =
      validation.inputType === "number" || validation.inputType === "integer" || validation.inputType === "text"
        ? validation.inputType
        : undefined;
    const parsedQuestion: GeetorusQuestion = {
...
function semanticToolEntries(eventType: string, payload: JsonRecord, ts: string): TranscriptEntry[] {
  ...
  const operationId = text(semantic.operationId, "Geetorus operation");
...
function parsePrpEvent(
  ...
  if (eventType === "session.started" || eventType === "session.resumed") {
    ...
    const model = text(context.model, text(record(payload.model).name, "Geetorus runner"));
    const sessionId = text(payload.providerSessionId, text(payload.driverSessionId, text(event.normalizedSessionId)));
    return [{ kind: "system", ts, text: `Geetorus session ${eventType === "session.resumed" ? "resumed" : "started"} · ${model}${sessionId ? ` · ${sessionId}` : ""}` }];
  }
...
function createParserState(): GeetorusRunnerParserState {
  return {
...
function parseGeetorusRunnerLine(line: string, ts: string, state: GeetorusRunnerParserState): TranscriptEntry[] {
  let parsed: unknown;
  try {
  ...
  if (envelope.type !== "geetorus.prp.event") return parseCodexStdoutLine(line, ts);
  const event = record(envelope.event);
  return Object.keys(event).length > 0 ? parsePrpEvent(event, ts, state) : [];
}

export function parseGeetorusRunnerStdoutLine(line: string, ts: string): TranscriptEntry[] {
  return parseGeetorusRunnerLine(line, ts, createParserState());
}

export const geetorusRunnerUIAdapter: UIAdapterModule = {
  type: "geetorus_runner",
  label: "Geetorus Runner",
  parseStdoutLine: parseGeetorusRunnerStdoutLine,
  createStdoutParser: () => {
    let state = createParserState();
    return {
      parseLine: (line, ts) => parseGeetorusRunnerLine(line, ts, state),
      reset: () => { state = createParserState(); },
    };
  },
  ConfigFields: CodexLocalConfigFields,
  buildAdapterConfig: buildGeetorusRunnerConfig,
};
