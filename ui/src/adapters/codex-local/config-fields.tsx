...
import {
  ...
  type GeetorusRunnerPermissionMode,
...
export function CodexLocalConfigFields({
  ...
  const runnerManaged = adapterType === "geetorus_runner";
  ...
  // `runnerManaged` already does for the Geetorus Runner.
  ...
  const configuredRunnerProvider = runnerManaged
    ...
    : "codex";
  const runnerProvider: GeetorusRunnerProvider = isGeetorusRunnerProvider(
    configuredRunnerProvider,
  )
    ? configuredRunnerProvider
    : "codex";
  const runnerPermissionCapability =
    GEETORUS_RUNNER_PERMISSION_CAPABILITIES[runnerProvider];
  ...
  const runnerLifecycleMode = runnerManaged
    ? isCreate
      ? (values!.geetorusRunnerLifecycleMode ?? "per_turn")
      : eff(
          "adapterConfig",
          "lifecycleMode",
  ...
  const runnerIdleTimeoutMs = runnerManaged
    ? resolveGeetorusRunnerIdleTimeoutMs(
        isCreate
          ? values!.geetorusRunnerIdleTimeoutMs
  ...
  const fastModeMessage = fastModeManualModel
    ...
    : fastModeSupported
      ...
      : `Fast mode currently only works on ${supportedModelsLabel} or manual model IDs. Geetorus will ignore this toggle until the model is switched.`;
    ...
    <>
      ...
      {runnerManaged && (
        ...
        >
          <select
            ...
            onChange={(event) => {
              const provider = isGeetorusRunnerProvider(event.target.value)
                ? event.target.value
                : "codex";
              const model =
                provider === "opencode"
      ...
      {runnerManaged && runnerPermissionCapability.configurable && (runnerPermissionCapability.options.length > 1 || runnerPermissionModeUnsupported) && (
        <Field
          ...
          hint={`${runnerPermissionCapability.description} The selected mode does not widen Geetorus's workspace, network, credential, or planning boundaries.`}
        >
          <Select
            value={
            ...
            onValueChange={(selectedMode) => {
              const value = resolveGeetorusRunnerPermissionMode(
                runnerProvider,
                selectedMode,
              ) as GeetorusRunnerPermissionMode;
              if (isCreate) {
                set!({
                  adapterSchemaValues: {
                    ...values!.adapterSchemaValues,
          ...
          {runnerPermissionModeUnsupported && runnerProvider === "codex" && (
            <p className="mt-1 text-xs text-destructive" role="alert">
              This saved Codex mode cannot start or recover a Geetorus Runner
        ...
        >
          <select
            ...
            onChange={(event) => {
              ...
              isCreate
                ? set!({ geetorusRunnerLifecycleMode: value })
        ...
        >
          {isCreate ? (
            <input
              type="number"
              min={1}
              max={GEETORUS_RUNNER_IDLE_TIMEOUT_MAX_MS}
              className={inputClass}
              value={runnerIdleTimeoutMs}
              onChange={(event) =>
                set!({
                  geetorusRunnerIdleTimeoutMs:
                    resolveGeetorusRunnerIdleTimeoutMs(
            ...
            <DraftNumberInput
              ...
              onCommit={(value) =>
                mark(
                  "adapterConfig",
                  "idleTimeoutMs",
                  resolveGeetorusRunnerIdleTimeoutMs(value),
                )
              }
              immediate
              className={inputClass}
        ...
        <>
          ...
          {!managedSandboxOnly && (
            <Field
              ...
              hint="Optional ACP session state directory. Defaults to Geetorus-managed organization/agent scoped storage."
            >
