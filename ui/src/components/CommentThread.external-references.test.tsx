...
import type { Approval } from "@geetorusai/shared";
...
describe("CommentThread external object decoration (integration)", () => {
  ...
  it("decorates a resolved URL and leaves an unknown URL unchanged when rendered through the real MarkdownBody", () => {
    ...
    expect(resolvedLink?.classList.contains("geetorus-markdown-external-ref")).toBe(true);
    ...
    expect(unknownLink?.classList.contains("geetorus-markdown-external-ref")).toBe(false);
