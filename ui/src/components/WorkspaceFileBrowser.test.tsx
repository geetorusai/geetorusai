...
import type { Project, ProjectWorkspace, WorkspaceFileListDirectoryItem, WorkspaceFileListFileItem, WorkspaceFileListItem, WorkspaceFileListResponse } from "@geetorusai/shared";
...
function createWorkspace(overrides: Partial<ProjectWorkspace> = {}): ProjectWorkspace {
  return {
    ...
    name: "Geetorus Content",
    sourceType: "local_path",
    cwd: "/srv/geetorus/home/geetorusai/geetorus-content",
    repoUrl: null,
    repoRef: null,
    defaultRef: null,
    visibility: "default",
...
describe("WorkspaceFileBrowser", () => {
  ...
  it("groups nested paths into collapsible folder rows instead of repeating full paths", () => {
    useQueryMock.mockReturnValue(
      ok(availableResponse([
        createItem({
          title: "tweet.md",
          relativePath: "videos/90-days-geetorus/tweet.md",
          displayPath: "videos/90-days-geetorus/tweet.md",
        }),
        createItem({
          title: "90-days-geetorus-1x1.mp4",
          relativePath: "videos/90-days-geetorus/out/90-days-geetorus-1x1.mp4",
          displayPath: "videos/90-days-geetorus/out/90-days-geetorus-1x1.mp4",
          previewKind: "video",
        }),
      ])),
    );
    ...
    expect(container.textContent).toContain("90-days-geetorus");
    expect(container.textContent).toContain("tweet.md");
    expect(container.textContent).toContain("90-days-geetorus-1x1.mp4");
    expect(container.textContent).not.toContain("videos/90-days-geetorus/tweet.md");
  ...
  it("opens a result from a selected other project workspace", () => {
    const contentItem = createItem({
      ...
      displayPath: "Geetorus Content / content-os/cases/active/2026-06-06-pap-10199-bundled-skills/README.md",
      workspaceLabel: "Geetorus Content",
      workspaceKind: "project_workspace",
      workspaceId: "workspace-content",
      projectId: "project-content",
      projectName: "Geetorus Content",
    });
    useQueryMock.mockImplementation((options: { queryKey: readonly unknown[] }) => {
    ...
    expect(container.textContent).toContain("Geetorus Content / Geetorus Content");
    const listCall = useQueryMock.mock.calls.find(([options]) => options.queryKey?.[3] === "list");
    expect(listCall?.[0].queryKey[4]).toMatchObject({
      workspace: "project",
      projectId: "project-content",
  ...
  it("focuses an initial folder in a selected other project workspace", () => {
    ...
    const contentItem = createItem({
      ...
      displayPath: `Geetorus Content / ${folderPath}README.md`,
      workspaceLabel: "Geetorus Content",
      workspaceKind: "project_workspace",
      workspaceId: "workspace-content",
      projectId: "project-content",
