...
describe("TaskChatThread runtime transcript selection", () => {
  ...
  it("uses runner-only controls only for an actual native Geetorus Runner run", () => {
      ...
      onRetryFailedRun={onRetryFailedRun} linkedRuns={[
        {
          ...
          agentId: "agent-1", agentName: "Runner", adapterType: "geetorus_runner",
  ...
  it.each(["legacy", "native"] as const)("groups repeated %s pre-start holds without hiding executed work", (runtimeMode) => {
    const heldRun = (id: string, recoveryActionId: string, startedAt: string | null = null) => ({
      ...
      agentId: "agent-1", adapterType: runtimeMode === "native" ? "geetorus_runner" : "claude_local",
  ...
  it("keeps workspace contention out of the conversation's cancellation markers", () => {
    render(<TaskChatThread comments={[]} onAdd={async () => {}} linkedRuns={[{
      ...
      agentId: "agent-1", agentName: "Runner", adapterType: "geetorus_runner", startedAt: null,
        ...
        linkedRuns={[
          {
            ...
            resultJson: {
              presentationDecision: {
                schema: "geetorus.run_presentation_decision.v1",
                chosenSource: "none",
                commentId: null,
              },
            },
  ...
  it.each([
    ...
    { adapterType: "geetorus_runner", status: "failed", runtimeMode: "native" as const,
...
describe("TaskChatThread composer alignment", () => {
  ...
  it("uses master's production composer and gutters when Streamlined UI is off", () => {
    ...
    const composer = container.querySelector(".geetorus-task-chat-composer");
...
describe("TaskChatThread no-live-execution-path recovery", () => {
  const noLivePathComment = {
    ...
    authorAgentId: null,
    authorUserId: null,
    body: "Geetorus retried continuation, but it still has no live execution path.",
    presentation: {
...
describe("TaskChatThread Geetorus Runner queue", () => {
  const queuedComment = {
    id: "queued-prp-1",
    companyId: "company-1",
    issueId: "issue-1",
    ...
    render(
      <TaskChatThread
        ...
        activeRun={{
          ...
          // a Geetorus request while the linked-run adapter classification is
          // still stale.
          adapterType: "codex_local",
        }}
      />,
...
describe("TaskChatThread composer execution controls", () => {
  it.each(["process", "geetorus_runner"])("passes the task's stop action through for %s execution", async (adapterType) => {
