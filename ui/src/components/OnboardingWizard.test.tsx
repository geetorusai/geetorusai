...
const mockAgentsApi = vi.hoisted(() => ({
  ...
  testEnvironment: vi.fn(
    async (): Promise<import("@geetorusai/shared").AdapterEnvironmentTestResult> => ({
  ...
  getAdapterAuthSignal: vi.fn(
    async (): Promise<import("@geetorusai/shared").AdapterAuthSignalResponse> => ({
      status: "present",
    }),
  ),
...
const mockEnvironmentsApi = vi.hoisted(() => ({
  ...
  capabilities: vi.fn(
    async (): Promise<import("@geetorusai/shared").EnvironmentCapabilities> =>
      (await import("@geetorusai/shared")).getEnvironmentCapabilities([]),
  ),
}));
...
import { ADAPTER_AUTH_MISSING_CHECK_CODE, getEnvironmentCapabilities } from "@geetorusai/shared";
...
describe("OnboardingWizard restore-gate (stale localStorage across accounts)", () => {
    ...
    it("hires when a stored login exists, even though a probe without the binding would warn adapter_auth_missing", async () => {
      ...
      mockAgentsApi.testEnvironment.mockImplementation(
        (async (...args: unknown[]) => {
          ...
          import("@geetorusai/shared").AdapterEnvironmentTestResult
        >,
      );
      const { root } = await openConnectStep();
      // Selecting the provider automatically verifies the saved subscription.
  ...
  describe("the adapter step login panel (cheap auth signal, no adapter test)", () => {
        ...
        checks: [
          {
            ...
            level: "info",
            message: 'Environment test identity for "Geetorus Computer".',
            detail: "geetorusLeaseId=ff9e58e9; provider=daytona",
          },
        ],
        testedAt: new Date().toISOString(),
      });
    ...
    it("shows local Claude instructions and saves its connection before hiring", async () => {
      ...
      expect(document.body.textContent).toContain("machine running Geetorus");
