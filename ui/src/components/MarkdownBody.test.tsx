...
describe("MarkdownBody", () => {
  ...
  it("renders user, agent, project, skill, and routine mentions as chips", () => {
    const html = renderToStaticMarkup(
      <QueryClientProvider client={new QueryClient()}>
        <ThemeProvider>
          <MarkdownBody>
            {`[@Taylor](${buildUserMentionHref("user-123")}) [@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Geetorus App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")}) [/routine:Weekly review](${buildRoutineMentionHref("routine-123")})`}
          </MarkdownBody>
        </ThemeProvider>
      </QueryClientProvider>,
    );
    ...
    expect(html).toContain("--geetorus-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--geetorus-mention-project-color:#336699");
  ...
  it("linkifies bare issue identifiers in markdown text", () => {
    ...
    expect(html).toContain("geetorus-markdown-issue-ref");
    expect(html).not.toContain("geetorus-mention-chip--issue");
  });
  ...
  it("preserves absolute issue URLs as external links", () => {
    ...
    expect(html).not.toContain("geetorus-markdown-issue-ref");
  });
  ...
  it("renders linked inline-code workspace paths as file viewer links before issue links", () => {
    const html = renderMarkdown(
      "- **MP4**: [`videos/90-days-geetorus/out/90-days-geetorus-1x1.mp4`](/PAP/issues/PAP-10306 \"Publish handoff\")",
    ...
    expect(html).toContain('data-workspace-file-path="videos/90-days-geetorus/out/90-days-geetorus-1x1.mp4"');
    expect(html).toContain("videos/90-days-geetorus/out/90-days-geetorus-1x1.mp4");
  ...
  it("renders auto-detected workspace paths as plain code without an availability resolver", () => {
    ...
    expect(html).not.toContain("geetorus-workspace-file-link");
  ...
  it("leaves wiki links as text unless explicitly enabled", () => {
    const html = renderMarkdown("See [[wiki/entities/geetorus]].");

    expect(html).toContain("[[wiki/entities/geetorus]]");
    expect(html).not.toContain('href="/wiki/page/wiki/entities/geetorus.md"');
  });

  it("renders wiki links with a custom resolver when enabled", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/geetorus|Geetorus]] and [[wiki/entities/dotta-b]].",
      [],
      {
    ...
    expect(html).toContain('href="/wiki/page/wiki/entities/geetorus.md"');
    expect(html).toContain('data-geetorus-wiki-link="true"');
    expect(html).toContain('data-geetorus-wiki-target="wiki/entities/geetorus"');
    expect(html).toContain(">Geetorus</a>");
    expect(html).toContain('href="/wiki/page/wiki/entities/dotta-b.md"');
    expect(html).toContain(">wiki/entities/dotta-b</a>");
    expect(html).not.toContain("[[wiki/entities/geetorus");
  });

  it("keeps wiki links as text when the custom resolver rejects them", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/geetorus]].",
      [],
      {
        enableWikiLinks: true,
    ...
    expect(html).toContain("[[wiki/entities/geetorus]]");
    expect(html).not.toContain('data-geetorus-wiki-link="true"');
    expect(html).not.toContain('href="/wiki/page/wiki/entities/geetorus"');
  });

  it("does not render wiki links inside code spans or code blocks", () => {
    const html = renderMarkdown(
      "Inline `[[wiki/entities/geetorus]]`.\n\n```md\n[[wiki/entities/dotta-b]]\n```",
      [],
      {
        enableWikiLinks: true,
        wikiLinkRoot: "/wiki/page",
  ...
  it("applies wrap-friendly styles to long inline content", () => {
    ...
    expect(html).toContain('class="geetorus-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden');
  ...
  it("renders markdown tables in a horizontally scrollable region", () => {
    ...
    expect(html).toContain('class="geetorus-markdown-table-scroll"');
  ...
  it("opens GitHub links in a new tab", () => {
    const html = renderMarkdown("[pr](https://github.com/geetorusai/geetorus/pull/4099)");
  ...
  it("prefixes GitHub markdown links with the GitHub icon glued to the first character", () => {
    const html = renderMarkdown("[https://github.com/geetorusai/geetorus/pull/4099](https://github.com/geetorusai/geetorus/pull/4099)");

    expect(html).toContain('<a href="https://github.com/geetorusai/geetorus/pull/4099"');
    ...
    expect(html).toContain("ttps://github.com/geetorusai/geetorus/pull/4099");
    expect(html).not.toContain("lucide-external-link");
  });

  it("prefixes GitHub autolinks with the GitHub icon", () => {
    const html = renderMarkdown("See https://github.com/geetorusai/geetorus/issues/1778");

    expect(html).toContain('<a href="https://github.com/geetorusai/geetorus/issues/1778"');
    expect(html).toContain('class="lucide lucide-github mr-1 inline h-3.5 w-3.5 align-(--va-0_125em)"');
  });
  ...
  it("renders a copy button alongside fenced code blocks", () => {
    ...
    expect(html).toContain("geetorus-markdown-codeblock");
    expect(html).toContain("geetorus-markdown-codeblock-actions");
    expect(html).toContain("position:absolute;top:0.4rem;right:0.4rem;display:inline-flex");
    expect(html).toContain("geetorus-markdown-codeblock-wrap");
    expect(html).toContain('aria-label="Wrap lines"');
    expect(html).toContain("position:static;opacity:1;display:inline-flex");
    expect(html).toContain("geetorus-markdown-codeblock-copy");
  ...
  it("does not render a copy button on inline code", () => {
    ...
    expect(html).not.toContain("geetorus-markdown-codeblock-copy");
  });
