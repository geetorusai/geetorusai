...
export function LocalProviderLoginInstructions({ adapterType, login }: {
  ...
  return <div className="min-w-0 max-w-full space-y-3 text-sm text-muted-foreground">
    ...
    </> : <p>{isolated ? `Sign in to ${provider} for this connection on the machine running Geetorus. Your existing terminal login stays separate.` : `Connect uses your local ${provider} account on the machine running Geetorus.`}</p>}
