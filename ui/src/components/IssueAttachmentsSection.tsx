...
import { Download, ExternalLink, FileText, Maximize2, Geetorus, Trash2 } from "lucide-react";
    ...
    <div id={`attachment-${attachment.id}`} className="scroll-mt-20 rounded-lg border border-border p-3">
      ...
      <div className="mt-3 rounded-md hover:bg-accent/10">
        {isLoading ? (
          ...
          <FoldCurtain>
            <MarkdownBody className="geetorus-edit-in-place-content min-h-(--sz-220px) text-sm leading-7" softBreaks={false}>
              {data ?? ""}
            </MarkdownBody>
          </FoldCurtain>
        )}
