...
vi.mock("@/context/CompanyContext", () => ({
  useCompany: () => ({
    ...
    selectedCompany: { id: "company-1", name: "Geetorus", issuePrefix: "PAP" },
  }),
}));
...
describe("InvitesSection", () => {
  ...
  const inviteHistory = Array.from({ length: 25 }, (_, index) => {
    ...
    return {
      ...
      invitedByUser: {
        ...
        email: `board${inviteNumber}@geetorus.local`,
        image: null,
      },
  ...
  beforeEach(() => {
    ...
    createCompanyInviteMock.mockImplementation(() => {
      return Promise.resolve({
        token: "new-token",
        inviteUrl: "https://geetorus.local/invite/new-token",
  ...
  it("renders a human-only invite flow and keeps invite history in a table", async () => {
    ...
    expect(clipboardWriteTextMock).toHaveBeenCalledWith("https://geetorus.local/invite/new-token");
    expect(container.textContent).toContain("Latest invite link");
    expect(container.textContent).toContain("This URL includes the current Geetorus domain returned by the server.");
  ...
  it("falls back to selectable text when browser clipboard access is unavailable", async () => {
    ...
    expect(inviteInput?.value).toBe("https://geetorus.local/invite/new-token");
