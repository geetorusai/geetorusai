...
describe("SidebarCompanyMenu", () => {
  ...
  it("hides the production-shell invite shortcut when the operator hides the invites surface", async () => {
    ...
    // GEETORUS_HIDDEN_SETTINGS like the streamlined menu — this is the knob
    // Geetorus Cloud uses to drop the shortcut on its managed stacks.
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
