...
import { isUuidLike, type EnvSecretRefBinding } from "@geetorusai/shared";
 ...
 */
const ENUM_UNSET_VALUE = "__geetorus_unset__";
...
const SecretField = React.memo(({
  ...
  return (
    <FieldWrapper
      ...
      description={
        description ||
        "Pick an existing organization secret, or paste a raw value (Geetorus will store it as a secret on save)."
      }
      required={isRequired}
      error={error}
      disabled={disabled}
...
export function JsonSchemaForm({
  ...
  const { essentials, advancedGroups, advancedKeys } = useMemo(() => {
    ...
    for (const entry of Object.entries(properties)) {
      ...
      if (propSchema["x-geetorus-advanced"] === true) {
        advancedKeys.add(key);
        const rawGroup = propSchema["x-geetorus-group"];
