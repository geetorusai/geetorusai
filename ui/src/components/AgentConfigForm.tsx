...
import { aiConnectionBindingSchema } from "@geetorusai/shared";
...
import { AGENT_DEFAULT_MAX_CONCURRENT_RUNS, supportedEnvironmentDriversForAdapter, isValidBrowserCode, ADAPTER_AUTH_MISSING_CHECK_CODE } from "@geetorusai/shared";
...
import { DEFAULT_CLAUDE_LOCAL_MODEL } from "@geetorusai/adapter-claude-local";
...
// Canonical type lives in @geetorusai/adapter-utils; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@geetorusai/adapter-utils";
import {
...
export function supportsAdapterModelRefresh(adapterType: string): boolean {
  return adapterType === "claude_local" || adapterType === "codex_local" || adapterType === "geetorus_runner" || adapterType === "opencode_local";
}

export function resolveGeetorusRunnerTransitionModel(
  previousAdapterType: string,
  previousModel: unknown,
): string {
  return geetorusRunnerTransitionConfig(previousAdapterType, previousModel).model as string;
}
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const runnerProvider = adapterType === "geetorus_runner"
  ...
  const {
    ...
    enabled: Boolean(selectedCompanyId && isLocal && adapterType !== "opencode_local" && adapterType !== "geetorus_runner"),
  });
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      ...
      if (props.compactTestFeedback) {
        const providerAdapter = adapterType === "geetorus_runner"
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-900 dark:text-amber-100">
                  Prompt template is replayed on every pulse. Keep it compact and dynamic to avoid recurring token cost and cache churn.
                </div>
              </>
            )}
          </div>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
                      ...
                      adapterConfig: {
                        model:
                          t === "gemini_local"
                            ...
                            : t === "geetorus_runner"
                              ? resolveGeetorusRunnerTransitionModel(adapterType, config.model)
                              : "",
                        effort: "",
                        modelReasoningEffort: "",
                        variant: "",
                        ...
                        ...(t === "codex_local"
                          ...
                          : t === "geetorus_runner"
                            ? {
                                ...geetorusRunnerTransitionConfig(adapterType, eff("adapterConfig", "model", config.model)),
                              }
                          : {}),
                      },
                    }));
          ...
          {isLocal && (<>
              <ModelDropdown
                ...
                allowDefault={adapterType !== "opencode_local" && adapterType !== "pi_local" && adapterType !== "geetorus_runner"}
                ...
                onDetectModel={adapterType === "opencode_local" || adapterType === "geetorus_runner"
                  ? undefined
                  : async () => {
          ...
          >
            ...
            {eff<unknown>("debug", "providerTrace", debug.providerTrace) === "raw" ? (
              <div className="mt-3 flex items-start gap-2 rounded-md border border-border bg-background/60 px-3 py-2 text-xs text-foreground">
                ...
                <span>
                  Raw tracing is on for future runs. Geetorus keeps at most 64 MiB per run and automatically deletes it after 24 hours.
                </span>
              </div>
            ) : null}
          </div>
...
export type AdapterLoginPanelProps = AdapterLoginDescriptor & {
  aiConnection?: import("@geetorusai/shared").AiConnectionLoginIntent;
