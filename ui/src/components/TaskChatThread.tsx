import type { ActivityEvent } from "@geetorusai/shared";
...
import { requiresExecutionReconciliation } from "@geetorusai/shared";
...
import {
  ...
  geetorusRunnerFinalResponse,
  geetorusRunnerAcceptedResponseWake,
  geetorusRunnerTimelineItems,
  prependIssueBrief,
  settledRunChildren,
...
function isNativeGeetorusRunnerRun(
  run:
    | {
  ...
  return (
    run?.runtimeMode === "native" && run.adapterType === "geetorus_runner"
  );
}
...
function acceptedSemanticResult(
  ...
  for (const candidate of candidates) {
    ...
    if (result.schema !== "geetorus.run_result.v1") continue;
    return result;
  }
  return null;
}
...
function presentationDecisionCommentId(
  ...
  if (record.schema !== "geetorus.run_presentation_decision.v1")
    return undefined;
...
export function TaskChatThread(props: TaskChatThreadProps) {
  ...
  const heldGeetorusRunnerRunId =
    liveRun && isNativeGeetorusRunnerRun(liveRun)
      ? liveRun.id
      : settlingRun &&
          isNativeGeetorusRunnerRun(
            runs.find((run) => run.id === settlingRun.id),
          )
        ? settlingRun.id
        : null;
  const heldGeetorusRunnerStartedAtMs =
    liveRun && heldGeetorusRunnerRunId === liveRun.id
      ? ((liveRun.startedAt ? toMs(liveRun.startedAt) : null) ??
        toMs(liveRun.createdAt))
      : (settlingRun?.startedAtMs ?? null);
  const heldGeetorusRunnerFinalText = useMemo(() => {
    if (!heldGeetorusRunnerRunId) return null;
    const entries = transcriptByRun.get(heldGeetorusRunnerRunId) ?? [];
    ...
    return null;
  }, [heldGeetorusRunnerRunId, transcriptByRun]);
  ...
  const lastCommentIdByRun = useMemo(() => {
    ...
    // A settled Geetorus turn normally attaches to its durable final reply.
  ...
  const planDocumentSourceRunId = useMemo(() => {
    ...
    const linkedReview = (interactions ?? []).find((interaction) => {
      ...
      return (
        isNativeGeetorusRunnerRun(sourceRun) &&
        interactionTargetsPlanRevision(interaction, planDocument)
      );
    });
    ...
    const matchingRuns = [...candidates.values()].filter((run) => {
      if (!isNativeGeetorusRunnerRun(run)) return false;
      if (
  ...
  const orderedEntries = useMemo(() => {
    ...
    visibleComments.forEach((comment, index) => {
      ...
      if (
        heldGeetorusRunnerRunId &&
        isRunnerResponseComment({
          comment,
          runId: heldGeetorusRunnerRunId,
          runStartedAtMs: heldGeetorusRunnerStartedAtMs,
          finalText: heldGeetorusRunnerFinalText,
        })
      )
        return;
    ...
    heldGeetorusRunnerRunId,
    heldGeetorusRunnerStartedAtMs,
    heldGeetorusRunnerFinalText,
    queuedCommentIds,
  ]);
  ...
  const legacyTimelineAnchorsByRun = useMemo(() => {
    ...
    for (const [runId, window] of windows) {
      if (
        isNativeGeetorusRunnerRun(window) ||
  ...
  const { items, settledRunIds, settledReplyRunIds } = useMemo<{
    ...
    for (const source of runs) {
      ...
      const sourceAcceptedResponseWake =
        ...
        geetorusRunnerAcceptedResponseWake(parsedSource, source.id);
      const sourceYielded =
      ...
      const sourceIsGeetorusRunner = isNativeGeetorusRunnerRun(source);
      ...
      if (entries.length === 0) {
        if (sourceIsGeetorusRunner && sourceYielded) {
          settledRunIds.add(source.id);
          continue;
        }
        if (
          isNativeGeetorusRunnerRun(source) &&
          acceptedSummary &&
          !sourceHasPresentationComment
        ) {
          ...
          settledTurns.push({
            turn: {
              ...
              items: [],
              finalResponse: geetorusRunnerFinalResponse([], {
                runId: source.id,
                agentName: meta?.agentName,
      ...
      const startSlotMs = startSlotRaw
        ? toMs(startSlotRaw)
        : Number.POSITIVE_INFINITY;
      const timelineAnchors = sourceIsGeetorusRunner
      ...
      const projectedSegments = segments.map((segment) => {
        ...
        return {
          segment,
          parsed,
          timelineItems: sourceIsGeetorusRunner
            ? geetorusRunnerTimelineItems(parsed)
            : parsed,
        };
      });
      const sourceResponseText =
        sourceIsGeetorusRunner && !sourceYielded
          ? (sourcePresentationText ??
            geetorusRunnerFinalResponse(parsedSource, {
              runId: source.id,
              agentName: meta?.agentName,
              fallbackSummary: acceptedSummary,
            })?.text)
          : undefined;
      const timelineItemsBySegment = sourceIsGeetorusRunner
        ? omitProgressRepeatedByResponseAcrossSegments(
      ...
      for (const [segmentIndex, projected] of projectedSegments.entries()) {
          ...
          !sourceHasPresentationComment
            ? sourceAcceptedResponseWake
              ? // A steering anchor may split acceptance from its terminal.
                ...
                : undefined
              : geetorusRunnerFinalResponse(parsed, {
                  runId: source.id,
                  agentName: meta?.agentName,
                  fallbackSummary:
                    segmentIndex === lastPopulatedSegmentIndex
        ...
        if (children.length === 0 && !finalResponse && !sourceIsGeetorusRunner)
          continue;
        settledRunIds.add(source.id);
        ...
        settledTurns.push({
          turn: {
            ...
            agentIcon: meta?.agentId
              ? agentMap?.get(meta.agentId)?.icon
              : undefined,
            standaloneHeader: sourceIsGeetorusRunner,
            continuedAfterSteering: sourceIsGeetorusRunner && segmentIndex > 0,
            animateFold: liveSeenRef.current.has(source.id),
            items: children,
            finalResponse,
    ...
    if (liveRun) {
      const timelineAnchors = isNativeGeetorusRunnerRun(liveRun)
        ...
        for (const [segmentIndex, segment] of segments.slice(0, -1).entries()) {
          ...
          const parsed =
            liveRun.id === planDocumentSourceRunId && planTurnItem
              ...
              : parsedTranscript;
          const children = settledRunChildren(
            isNativeGeetorusRunnerRun(liveRun)
              ? geetorusRunnerTimelineItems(parsed)
              : parsed,
          );
          ...
          settledTurns.push({
            turn: {
              ...
              agentIcon: liveRun.agentId
                ...
                : undefined,
              standaloneHeader: isNativeGeetorusRunnerRun(liveRun),
              continuedAfterSteering:
                isNativeGeetorusRunnerRun(liveRun) && segmentIndex > 0,
              items: children,
              summary: buildTurnSummary(segment.entries, {
                durationMs: segmentDurationMs,
              }),
  ...
  const settlingIsGeetorusRunner =
    settlingRun != null &&
    isNativeGeetorusRunnerRun(runs.find((run) => run.id === settlingRun.id));
  const settlingHasComment =
    settlingRun != null &&
    comments.some((comment) =>
      settlingIsGeetorusRunner
        ? isRunnerResponseComment({
            comment,
  ...
  const tailRunSource = tailRunId
    ...
    : undefined;
  const geetorusRunnerTail = isNativeGeetorusRunnerRun(tailRunSource);
  const suppressGeetorusRunnerTailFinal = Boolean(
    geetorusRunnerTail &&
  ...
  const interactionDraftKey = selectedPendingInput
    ? `geetorus:task-input:${issueId ?? "unknown"}:${selectedPendingInput.key}`
    : undefined;
  ...
  const assigneeUsesGeetorusRunner = Boolean(
    issueAssigneeAgentId &&
    agentMap?.get(issueAssigneeAgentId)?.adapterType === "geetorus_runner",
  );
  ...
  const handleThreadAdd = useCallback(
    async (...args: Parameters<typeof onAdd>) => {
      if (assigneeUsesGeetorusRunner) setRunnerSubmissionPending(true);
      try {
        await onAdd(...args);
      } catch (error) {
        setRunnerSubmissionPending(false);
    ...
    [assigneeUsesGeetorusRunner, onAdd],
  );
  const optimisticRunnerStartup =
    assigneeUsesGeetorusRunner &&
    !tailRunId &&
                            ...
                            <div data-testid="task-chat-live-transcript">
                              ...
                              optimisticRunnerStartup ? (
                                <TaskChatRunnerTurn
                                  ...
                                  suppressFinal={
                                    suppressGeetorusRunnerTailFinal
                                  }
                                  continuedAfterSteering={
                                    geetorusRunnerTail &&
                                    tailTimelineAnchors.length > 0
