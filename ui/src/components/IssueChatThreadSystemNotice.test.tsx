...
import type { Agent, SuccessfulRunHandoffState } from "@geetorusai/shared";
...
vi.mock("../hooks/useGeetorusIssueRuntime", () => ({
  useGeetorusIssueRuntime: () => ({}),
}));
...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    ...
    expect(container.textContent).toContain("Geetorus needs a disposition");
  ...
  it("labels system notice source as Geetorus when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("Geetorus");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to Geetorus in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("Geetorus");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("Geetorus needs a disposition before this issue can continue.");
  ...
  it("keeps the required disposition warning loud when no live continuation exists", () => {
    ...
    expect(container.textContent).toContain("Geetorus needs a disposition before this issue can continue.");
  });
