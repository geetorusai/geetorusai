...
describe("AgentsUsingSkillDialog", () => {
  ...
  it("removes via GET-then-sync and preserves the agent's other skills", async () => {
    mockAgentsApi.skills.mockResolvedValue({
      ...
      supported: true,
      mode: "managed",
      desiredSkills: ["geetorus/demo", "geetorus/other"],
      desiredSkillEntries: [
        { key: "geetorus/demo", versionId: null },
        { key: "geetorus/other", versionId: null },
    ...
    expect(sentEntries).toEqual([{ key: "geetorus/other", versionId: null }]);
  });
  ...
  it("pins a version by sending the full set with the target repinned", async () => {
    ...
    expect(mockAgentsApi.syncSkills).toHaveBeenCalledTimes(1);
    const [, sentEntries] = mockAgentsApi.syncSkills.mock.calls[0];
    expect(sentEntries).toContainEqual({ key: "geetorus/other", versionId: null });
    expect(sentEntries).toContainEqual({ key: "geetorus/demo", versionId: "ver-2" });
  });
