...
import {
  ...
  type GeetorusIssueRuntimeReassignment,
} from "../hooks/useGeetorusIssueRuntime";
...
import { buildAgentMentionHref } from "@geetorusai/shared";
...
function parseReassignment(
  target: string,
): GeetorusIssueRuntimeReassignment | null {
...
const IssueChatTextPart = memo(function IssueChatTextPart({
  ...
  return (
    <WorkspaceFileMarkdownBody
      className={cn(
        "text-sm leading-6",
        onAccent && "geetorus-markdown-on-accent",
      )}
...
> = {
  geetorus_provider_activity: ClipboardList,
};
...
function IssueChatToolPart({
  ...
  if (toolName === "geetorus_provider_activity") {
    return (
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose whether voted AI outputs can be shared with Geetorus Labs.
...
function SystemNoticeCommentContent({
  ...
  const source = (() => {
    ...
    if (authorType === "system") {
      const label = runAgentName ?? "Geetorus";
      if (runAgentId && runId)
    ...
    if (runAgentId && runId) {
      return {
        label: authorName ?? runAgentName ?? "Geetorus",
...
export function IssueChatThread({
  ...
  const runtime = useGeetorusIssueRuntime({
    messages,
    isRunning,
