...
describe("SidebarRecentTasks", () => {
  ...
  it("preserves pending restart wake retries from before the snapshot storage migration", async () => {
    ...
    window.localStorage.setItem("geetorus.recentTasks:company-1:user-1", JSON.stringify([{ ...issue, recordedAt: 1 }]));
    window.localStorage.setItem("geetorus.recentTasks:company-1:user-1:restart-wake-retry", JSON.stringify([issue.id]));
    mockIssuesApi.get.mockResolvedValue(issue);
    ...
    expect(window.localStorage.getItem("geetorus.recentTasks:company-1:user-1:restart-wake-retry")).toBeNull();
  });
