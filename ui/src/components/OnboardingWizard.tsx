...
import type { AiConnectionBinding } from "@geetorusai/shared";
...
import { AGENT_ROLES, AGENT_ROLE_LABELS, ADAPTER_AUTH_MISSING_CHECK_CODE } from "@geetorusai/shared";
...
import { DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX } from "@geetorusai/adapter-codex-local";
...
// instance administrator has opted into Geetorus Runner elsewhere. The
...
function restoreOnboardingAdapterType(savedAdapterType: unknown): AdapterType {
  return typeof savedAdapterType === "string" && savedAdapterType !== "geetorus_runner"
    ? savedAdapterType
    : "claude_local";
}
...
export const ONBOARDING_STORAGE_KEY = "geetorus-onboarding-state";
const DEFAULT_TASK_TITLE = "Geetorus onboarding";
/**
...
function OnboardingWizardInner({
  ...
  const savedNativeRunnerDraft = saved?.adapterType === "geetorus_runner";
  ...
  const {
    ...
    // resolve against the local Geetorus host (environmentId = null).
  ...
  */
  ...
  async function handleGiveHeartbeat() {
    ...
    if (adapterType === "geetorus_runner") {
      setAdapterType("claude_local");
      setModel("");
      setError("Geetorus Runner is not available during onboarding. Choose a legacy adapter.");
      return;
    }
    if (createdAgentId) {
      setStep(5);
                    ...
                    <OnboardingHeading
                      ...
                      lede={
                        step === 3 ? undefined : step === 4 ? (
                          <>Geetorus works with your subscription or API keys.</>
                        ) : (
              ...
              {step === 1 && (
                <div className="mx-auto w-full space-y-9">
                  <OnboardingHeading
                    ...
                    lede="Welcome to Geetorus — let's set up your organization."
                  />
