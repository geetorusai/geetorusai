...
import type { CompanySecret, UserSecretDefinition } from "@geetorusai/shared";
import {
...
describe("validateName", () => {
  ...
  it("flags reserved prefixes as warnings", () => {
    const issue = validateName("GEETORUS_HOME", new Set(), reserved);
    expect(issue?.level).toBe("warn");
    expect(issue?.message).toMatch(/Reserved prefix/);
  });

  it("charset error takes precedence over reserved prefix", () => {
    expect(validateName("GEETORUS-X", new Set(), reserved)?.level).toBe("error");
  });
});
