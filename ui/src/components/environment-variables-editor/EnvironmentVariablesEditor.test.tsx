...
import type { CompanySecret, EnvBinding } from "@geetorusai/shared";
...
describe("EnvironmentVariablesEditor", () => {
  ...
  it("keeps long secret names clear of the latest version control", () => {
    const longSecret = makeSecret("long", {
      name: "/geetorus-cloud/prod/provider/resend/api-key-with-a-very-long-name",
      latestVersion: 4,
    });
  ...
  it("renders name warnings as a row spanning the name and value columns", () => {
    render(
      <EnvironmentVariablesEditor
        value={{ GEETORUS_PAGE_BASE_URL: { type: "plain", value: "https://pages.geetorus.ing" } }}
        secrets={secrets}
        onChange={() => {}}
        onCreateSecret={async () => secrets[0]}
      />,
  ...
  it("lets slash-delimited secrets be browsed by folder and exposes full paths on hover", async () => {
    const cloudProviderKey = "/geetorus-cloud/prod/provider/aws-access-key-id";
    const cloudMigrationKey = "/geetorus-cloud/prod/migration/postgres-url";
    ...
    render(
      <SecretPicker
        ...
        secrets={[
          ...
          makeSecret("plain-key", { name: "geetorus-page-aws-access-key-id", key: "geetorus-page-aws-access-key-id" }),
        ]}
        onSelect={onSelect}
        onCreateNew={() => {}}
        disablePortal
    ...
    expect(rowTitle(itemByText("geetorus-cloud"))).toBe("/geetorus-cloud");
    expect(rowTitle(itemByText("geetorus-page-aws-access-key-id"))).toBe(
      "geetorus-page-aws-access-key-id",
    );

    itemByText("geetorus-cloud")!.click();
    await flush();
    expect(rowTitle(itemByText("prod"))).toBe("/geetorus-cloud/prod");

    itemByText("prod")!.click();
    await flush();
    expect(rowTitle(itemByText("provider"))).toBe("/geetorus-cloud/prod/provider");
    expect(rowTitle(itemByText("migration"))).toBe("/geetorus-cloud/prod/migration");
  ...
  it("keeps secret search global while browsing starts at slash folders", async () => {
      ...
      <SecretPicker
        ...
        secrets={[
          makeSecret("provider-key", {
            name: "/geetorus-cloud/prod/provider/aws-access-key-id",
            key: "/geetorus-cloud/prod/provider/aws-access-key-id",
          }),
          makeSecret("migration-key", { name: cloudMigrationKey, key: cloudMigrationKey }),
        ]}
        onSelect={() => {}}
    ...
    combobox.focus();
    await flush();
    expect(document.body.textContent).toContain("geetorus-cloud");
