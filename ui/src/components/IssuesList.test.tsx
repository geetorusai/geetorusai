...
import type { Issue, Project } from "@geetorusai/shared";
...
describe("IssuesList", () => {
  ...
  it("uses the master list and legacy persistence when Streamlined UI is off", async () => {
    ...
    localStorage.setItem(
      taskCollectionPreferencesStorageKey({
        companyId: "company-1",
        collectionKey: "geetorus:test-issues",
      }),
      JSON.stringify({
        version: 1,
        companyId: "company-1",
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        ...
        agents={[]}
        projects={[]}
        viewStateKey="geetorus:test-issues"
  ...
  it("filters issue rows by external-object status summaries", async () => {
    ...
    localStorage.setItem("geetorus:test-issues:company-1", JSON.stringify({ externalObjectStatuses: ["failed"] }));
    mockExternalObjectsApi.getIssueSummaries.mockResolvedValue({
      summaries: {
        "issue-failed": {
          total: 1,
  ...
  it("uses workspace group defaults when creating an issue from a grouped section", async () => {
    localStorage.setItem(
      "geetorus:test-issues:company-1",
  ...
  it("lets board users choose the per-column page size", async () => {
    ...
    expect(localStorage.getItem("geetorus:test-issues:company-1")).toContain("\"boardColumnPageSize\":25");
  ...
  it("uses context-scoped persisted column visibility", async () => {
    localStorage.setItem("geetorus:test-issues:company-1:issue-columns", JSON.stringify(["id", "assignee"]));
  ...
  it("filters the list to a single workspace when a workspace name is clicked", async () => {
    localStorage.setItem("geetorus:test-issues:company-1:issue-columns", JSON.stringify(["id", "workspace"]));
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: true });
    mockExecutionWorkspacesApi.listSummaries.mockResolvedValue([
      {
        id: "workspace-alpha",
  ...
  it("can hide date group separators from the persisted Columns option", async () => {
    const collectionKey = "geetorus:test-issues";
    localStorage.setItem(
      taskCollectionPreferencesStorageKey({
        companyId: "company-1",
        collectionKey,
