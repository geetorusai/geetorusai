...
import type { Agent, IssueRecoveryAction } from "@geetorusai/shared";
...
function buildWorkspaceValidationAction(
  ...
  const provenance = {
    ...
    plainLanguageReason:
      'The recorded branch "PAP-522-recorded" is not an ancestor of the checked-out branch "nleach/PAP-1405-live", so Geetorus cannot prove a forward-only reconciliation.',
...
describe("IssueRecoveryActionCard repair workspace (quarantine_restore)", () => {
  ...
  it("confirm popover restates the dirty count, live branch, rescue branch and recorded branch, then fires the handler", () => {
    ...
    ).toContain("geetorus/rescue/PAP-1405/");
...
describe("IssueRecoveryActionCard owner-sticky retry lineage", () => {
  ...
  it("stays quiet and names the retry lane while the original owner is being retried", () => {
    ...
    expect(node.textContent).toContain("Geetorus is retrying the original owner");
  });
  ...
  it("warns strongly only once the automatic path is exhausted", () => {
    ...
    expect(node.textContent).not.toContain("Geetorus is retrying the original owner");
