...
describe("TaskChatSystemNotice (PAP-443)", () => {
  ...
  it("shows workspace-ready comments as a compact row with expandable workspace metadata", () => {
    ...
    expect(details?.querySelector(".geetorus-markdown")).toBeNull();
  });
  ...
  it("shows Try again while folded and invokes it without expanding the notice", async () => {
    ...
    renderNotice(
      {
        text: "Geetorus retried continuation, but it still has no live execution path.",
        presentation: {
          kind: "system_notice",
          tone: "danger",
  ...
  it("keeps workspace-ready events as a compact expandable notice", () => {
    renderNotice({
      text: "Workspace ready. The isolated worktree is available at `/tmp/geetorus/worktrees/PAP-91`.",
      metadata: null,
    });
    ...
    expect(container.textContent).not.toContain("/tmp/geetorus/worktrees/PAP-91");

    flushSync(() => button.click());
    expect(container.textContent).toContain("/tmp/geetorus/worktrees/PAP-91");
  });
