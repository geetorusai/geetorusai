...
describe("tool activity vocabulary", () => {
  ...
  it("normalizes camel, Pascal, snake, kebab, dotted, and both MCP formats", () => {
    ...
    const mcp = toolActivityPresentation({ name: "mcp.geetorus.search_tasks" });
    expect(mcp).toMatchObject({
      family: "mcp",
      icon: McpIcon,
      runningLabel: "Searching tasks",
      completedLabel: "Searched tasks",
      sourceLabel: "Geetorus",
      confidence: "exact",
    });
  });
  ...
  it("gives anticipated provider tools and Geetorus operations purpose-specific labels", () => {
    const anticipated = [
    ...
    const geetorus = [
      ...
      "register_deliverable", "finish_task", "geetorus_finish", "block_task", "geetorus_block",
    ...
    for (const name of [...anticipated, ...geetorus]) {
      const presentation = toolActivityPresentation({ name, namespace: geetorus.includes(name) ? "geetorus" : undefined });
    ...
    expect(toolActivityPresentation({ name: "report_progress", namespace: "geetorus" }).runningLabel).toBe("Reporting progress");
    expect(toolActivityPresentation({ name: "geetorus_block", namespace: "geetorus" }).completedLabel).toBe("Reported a blocker");
    expect(toolActivityPresentation({ name: "schedule_wake", namespace: "geetorus" }).runningLabel).toBe("Scheduling a wake-up");
  });
