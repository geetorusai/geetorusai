...
import type { HeartbeatRunEvent } from "@geetorusai/shared";
import {
  assembleThreadItems,
  attachSettledTurns,
  buildActivityPhases,
  ...
  geetorusRunnerActivityItems,
  geetorusRunnerFinalResponse,
  geetorusRunnerHistoryItems,
...
describe("accepted native response-wake answers", () => {
  ...
  it.each(["accepted", "provider_final"])(
    ...
    (mode) => {
      expect(
        geetorusRunnerFinalResponse(fixture(mode), { runId }),
      ).toMatchObject({
        text: summary,
        channel: "final",
      });
  ...
  it("does not grant the exception to another run or a live fallback", () => {
    expect(
      geetorusRunnerFinalResponse(fixture(), { runId: "other-run" }),
    ).toBeUndefined();
    expect(
      geetorusRunnerFinalResponse(fixture(), { runId, allowFallback: false }),
    ).toBeUndefined();
  });
});
  ...
  it("keeps completion events inspectable but hides both tool representations from live and settled feed activity", () => {
    for (const running of [true, false]) {
      const entries: TranscriptEntry[] = [
        { kind: "tool_call", ts: TS, toolUseId: "legacy-finish", name: "geetorus_finish", input: {} },
        ...["geetorus_finish", "search_tasks"].map((name) => ({
      ...
      const activity = geetorusRunnerActivityItems(parsed);
      expect(activity).toHaveLength(2);
      expect(JSON.stringify(activity)).not.toContain("geetorus_finish");
      expect(JSON.stringify(activity)).toContain("search_tasks");
      expect(JSON.stringify(activity)).toContain("Repository is not trusted");
      expect(geetorusRunnerTimelineItems(parsed)).toEqual(activity);
    }
  });
});
  ...
  it("preserves named ACPX tool identity and progress when terminal updates omit them", () => {
    ...
    expect(item.details).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ label: "Namespace", value: "geetorus" }),
    ...
    expect(providerActivityPresentation(item)).toMatchObject({
      ...
      detail: "Searching the task index · Geetorus · search_tasks",
    });
  });
...
describe("buildActivityPhases provider summaries", () => {
  ...
  it("summarizes the sanitized DOT-212 mix with at most three semantic groups", () => {
    ...
    const items: TaskChatItem[] = [
      ...
      providerTool("context", "get_task_context", "read", "geetorus"),
      providerTool("documents", "list_documents", "list", "geetorus"),
      providerTool("history", "get_task_history", "read", "geetorus"),
      providerTool("tasks", "search_tasks", "search", "geetorus"),
      providerTool("progress", "report_progress", "edit", "geetorus"),
      providerTool("block", "geetorus_block", "edit", "geetorus"),
    ];
    expect(buildActivityPhases(items, false)[0]?.summary).toBe(
      "Searched available tools 6 times, read from Geetorus 3 times, used Geetorus 3 times",
    );
  });
});
...
describe("buildTurnTimelineRows (DOT-217)", () => {
  ...
  it("keeps runtime requests in the Geetorus Runner timeline input", () => {
  ...
  it("keeps aggregate workspace changes at the turn boundary outside activity", () => {
    const rows = buildTurnTimelineRows(
      geetorusRunnerTimelineItems([tool("before"), workspace, tool("after")]),
      false,
    );
  ...
  it("suppresses file-change fragments only when an aggregate event exists", () => {
    ...
    expect(geetorusRunnerActivityItems([fileChange])).toEqual([fileChange]);
    expect(geetorusRunnerActivityItems([fileChange, workspace])).toEqual([
      workspace,
    ]);
  });
  ...
  it("keeps a saved plan at its write_document boundary", () => {
    ...
    const rows = buildTurnTimelineRows(
      geetorusRunnerTimelineItems(embedded),
      false,
    );
    expect(
      rows.map((row) =>
      ...
      [
        {
          ...
          cachedTokens: 0,
          costUsd: 0,
          subtype: "geetorus_runner_session_usage",
          isError: false,
          errors: [],
        },
      ],
...
describe("geetorus runner semantic channels", () => {
  ...
  it("filters lifecycle and usage noise from only the runner history surface", () => {
    ...
    expect(geetorusRunnerHistoryItems(items).map((item) => item.id)).toEqual([
      "tool",
      "interrupt",
      "runner",
    ]);
  ...
  it("projects one semantic activity row per useful logical item", () => {
    ...
    expect(geetorusRunnerActivityItems(items).map((item) => item.id)).toEqual([
      "progress",
      "thinking",
      "tool",
      "interrupt",
