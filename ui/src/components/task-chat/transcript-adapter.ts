 ...
 * interstitial into its status line; the Geetorus Runner task surface instead
...
export function transcriptToTaskChatItems(
  ...
  for (const [i, entry] of entries.entries()) {
    switch (entry.kind) {
      ...
      case "result": {
        if (
          entry.subtype !== "geetorus_runner_usage" &&
          entry.subtype !== "geetorus_runner_session_usage"
        )
          break;
...
/**
 * Keep the geetorus runner's expanded activity history focused on work the
 ...
 */
export function geetorusRunnerHistoryItems(
  parsed: readonly TaskChatItem[],
): TaskChatItem[] {
...
/**
 * Semantic rows for the Geetorus runner's Codex-style activity disclosure.
 *
 ...
 */
export function geetorusRunnerActivityItems(
  parsed: readonly TaskChatItem[],
): TaskChatItem[] {
    ...
    switch (item.kind) {
      ...
      case "tool": {
        ...
        if (
          ...
          return false;
        return normalizedName !== "geetorus_finish";
      }
      case "marker":
      ...
      case "protocol":
        ...
        if (
          ...
          providerItemDetail(item, "Name") === "geetorus_finish"
        ) return false;
        if (
          hasAggregateWorkspaceChange &&
          item.surface === "provider_activity" &&
...
/**
 * Ordered input for the Geetorus Runner task-turn timeline. This keeps the
 ...
 */
export function geetorusRunnerTimelineItems(
  parsed: readonly TaskChatItem[],
): TaskChatItem[] {
  const activityIds = new Set(
    geetorusRunnerActivityItems(parsed).map((item) => item.id),
  );
 ...
 */
export function geetorusRunnerAcceptedResponseWake(
  parsed: readonly TaskChatItem[],
...
export function geetorusRunnerFinalResponse(
  parsed: readonly TaskChatItem[],
  options?: {
    runId?: string;
    agentName?: string;
  ...
  if (yielded) {
    const accepted =
      options?.allowFallback === false
        ? undefined
        : geetorusRunnerAcceptedResponseWake(parsed, options?.runId);
      ...
      switch (key) {
        ...
        case "geetorus_read":
          text =
            count === 1
              ? "Read from Geetorus"
              : `Read from Geetorus ${count} times`;
          break;
        case "task_operation":
          text =
            count === 1 ? "Used Geetorus" : `Used Geetorus ${count} times`;
          break;
        default:
