...
const EXACT_ACTIONS: Record<string, ExactAction> = {
  ...
  geetorus_finish: { action: "finish", running: "Reporting completion", completed: "Reported completion" },
  block_task: { action: "block", running: "Reporting a blocker", completed: "Reported a blocker" },
  geetorus_block: { action: "block", running: "Reporting a blocker", completed: "Reported a blocker" },
  ...
  generic_api_request: { action: "request", running: "Calling the Geetorus API", completed: "Called the Geetorus API" },
};
...
function geetorusSummaryGroup(action: Action): ToolSummaryGroup {
  if (action === "read" || action === "list") return group("geetorus_read", "Geetorus read", "Geetorus reads");
...
export function toolActivityPresentation(input: ToolActivityPresentationInput): ToolActivityPresentation {
  ...
  const sourceLabel = namespace
    ...
    : transport === "mcp"
      ? "MCP"
      : undefined;
  const summaryGroup = namespace.toLowerCase() === "geetorus"
    ? geetorusSummaryGroup(action)
