...
describe("DocumentAnnotationLayer", () => {
  ...
  it("uses borderless token-driven overlay boxes for annotation highlight states", async () => {
    ...
    const highlights = Array.from(container.querySelectorAll(".geetorus-doc-annotation-highlight"));
    expect(highlights).toHaveLength(4);

    for (const highlight of highlights) {
      expect(highlight.className).toBe("geetorus-doc-annotation-highlight absolute");
  ...
  it("does not render highlights for text clipped by folded document content", async () => {
    ...
    expect(container.querySelector(".geetorus-doc-annotation-highlight")).toBeNull();
    expect(container.querySelector(".geetorus-doc-annotation-hit-target")).toBeNull();
  });
  ...
  it("keeps native CSS highlight ranges synchronized while using the overlay for visual paint", async () => {
    ...
    expect(container.querySelector(".geetorus-doc-annotation-highlight")).not.toBeNull();
    expect(container.querySelector(".geetorus-doc-annotation-hit-target")).not.toBeNull();
    const openHighlightCall = setHighlight.mock.calls.find(([name]) => name === "geetorus-doc-annotation-open");
    expect(openHighlightCall).toBeTruthy();
    expect((openHighlightCall?.[1] as MockHighlight).ranges).toHaveLength(1);

    await act(async () => root?.unmount());
    root = null;
    expect(deleteHighlight).toHaveBeenCalledWith("geetorus-doc-annotation-open");
