...
import type { Agent, ResourceMemberships } from "@geetorusai/shared";
...
describe("SidebarAgents", () => {
  ...
  it("keeps top mode in stored org-aware order", async () => {
    localStorage.setItem("geetorus.agentOrder:company-1:user-1", JSON.stringify(["agent-b", "agent-a", "agent-c"]));
    mockAgentsApi.list.mockResolvedValue([
      makeAgent({ id: "agent-a", name: "Alpha", urlKey: "alpha" }),
      makeAgent({ id: "agent-b", name: "Bravo", urlKey: "bravo" }),
  ...
  it("sorts alphabetically and persists the selected mode per company and user", async () => {
    ...
    expect(localStorage.getItem("geetorus.agentSortMode:company-1:user-1")).toBe("alphabetical");
  });
