...
import { buildIssueReferenceHref, buildProjectMentionHref, buildRoutineMentionHref, buildSkillMentionHref } from "@geetorusai/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Geetorus App", "Ping @Geetorus App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Geetorus App",
      atPos: 5,
      endPos: "Ping @Geetorus App".length,
    });
  });
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Geetorus App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
  ...
  it("marks the autocomplete portal as floating UI for modal pointer handling", async () => {
    ...
    const menu = option.closest("[data-geetorus-floating-ui]");
    expect(menu).toBeTruthy();
