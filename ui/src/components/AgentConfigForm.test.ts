...
import type { Environment } from "@geetorusai/shared";
import {
  resolveGeetorusRunnerTransitionModel,
...
describe("supportsAdapterModelRefresh", () => {
  ...
  it("keeps the refresh action hidden for adapters without a live refresh hook", () => {
    ...
    expect(supportsAdapterModelRefresh("geetorus_runner")).toBe(true);
    expect(supportsAdapterModelRefresh("process")).toBe(false);
  });
});

describe("resolveGeetorusRunnerTransitionModel", () => {
  it("preserves Claude custom model IDs", () => {
    expect(resolveGeetorusRunnerTransitionModel("claude_local", "custom-claude-model")).toBe("custom-claude-model");
  });
  it("preserves an explicit model from codex_local", () => {
    expect(resolveGeetorusRunnerTransitionModel("codex_local", "gpt-5.5"))
      .toBe("gpt-5.5");
  });

  it("uses the current Codex default when the source model is blank", () => {
    expect(resolveGeetorusRunnerTransitionModel("codex_local", ""))
      .toBe("gpt-5.6-sol");
  });
});
