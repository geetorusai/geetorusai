...
import type { DocumentAnnotationAnchorSelector } from "@geetorusai/shared";
...
const PENDING_HIGHLIGHT_THREAD_ID = "__geetorus-pending-annotation__";
...
const NATIVE_HIGHLIGHT_NAMES: Record<NativeHighlightKind, string> = {
  open: "geetorus-doc-annotation-open",
  focused: "geetorus-doc-annotation-focused",
  stale: "geetorus-doc-annotation-stale",
  resolved: "geetorus-doc-annotation-resolved",
};
...
export function DocumentAnnotationLayer({
      ...
      ? new window.MutationObserver((mutations) => {
        if (selectionDebugEnabled) {
          const markdownMutations = mutations.filter((mutation) =>
            Boolean(elementFromNode(mutation.target)?.closest(".geetorus-markdown")),
          );
        ...
        const onlyLayerMutations = mutations.every((mutation) => {
          ...
          return !!target?.closest(".geetorus-doc-annotation-layer, .geetorus-doc-annotation-visual-layer");
        });
  ...
  const content = (
    <>
      <div className="geetorus-doc-annotation-visual-layer pointer-events-none absolute inset-0 z-0" aria-hidden="true">
          ...
          {highlightRects.map((rect, index) => {
            ...
            return (
              <span
                ...
                className="geetorus-doc-annotation-highlight absolute"
                style={{
                  top: rect.top,
                  left: rect.left,
                  width: rect.width,
      ...
      <div
        className="geetorus-doc-annotation-layer pointer-events-none absolute inset-0 z-(--z-2)"
        aria-hidden="true"
      >
        <div ref={overlayRef} className="relative h-full w-full">
          {highlightRects.map((rect, index) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "geetorus-doc-annotation-hit-target pointer-events-auto absolute cursor-pointer rounded-none bg-transparent transition-colors",
          ...
          {highlightRects.map((rect, index) =>
            rect.isTail && rect.anchorState === "stale" ? (
              <span
                ...
                aria-hidden="true"
                data-thread-id={rect.threadId}
                className="geetorus-doc-annotation-tail pointer-events-none absolute inline-flex items-center justify-center rounded-sm bg-amber-500/95 text-amber-50 shadow-sm dark:bg-amber-500/90 dark:text-amber-50"
                style={{
          ...
          {pendingAnchor && toolbarPosition ? (
            <div
              ...
              className="geetorus-doc-annotation-selection-toolbar pointer-events-auto absolute z-10 flex items-center gap-1 rounded-md border border-border bg-popover px-1 py-1 shadow-md"
