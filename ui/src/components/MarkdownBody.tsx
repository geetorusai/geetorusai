...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className={cn("geetorus-markdown-issue-ref", "font-normal underline")}
      title={title}
...
function MarkdownCaseLink({
  ...
  return (
    <Link
      ...
      className={cn("geetorus-markdown-case-ref", "font-normal underline")}
...
function MarkdownExternalLink({
  ...
  return (
    <a
      ...
      className="geetorus-markdown-external-ref"
    >
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-geetorus-wiki-link": "true",
        "data-geetorus-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="geetorus-markdown-codeblock" data-wrap-lines={wrapLines || undefined}>
      <pre
        {...preProps}
        ref={preRef}
        style={{
      ...
      <div
        className="geetorus-markdown-codeblock-actions"
      ...
      >
        <button
          ...
          title={wrapLabel}
          className="geetorus-markdown-codeblock-action geetorus-markdown-codeblock-wrap"
          style={wrapLines
            ? {
        ...
        <button
          ...
          title={copyLabel}
          className="geetorus-markdown-codeblock-action geetorus-markdown-codeblock-copy"
        ...
        >
          ...
          <span className="geetorus-markdown-codeblock-action-label">{copyLabel}</span>
        </button>
      </div>
    </div>
  );
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`geetorus-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="geetorus-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("geetorus-mermaid-status", error && "geetorus-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="geetorus-mermaid-source">
...
function MarkdownBodyImpl({
  ...
  const components = useMemo<Components>(() => {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="geetorus-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-geetorus-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            href={targetHref}
            className={cn(
              "geetorus-mention-chip",
              `geetorus-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "geetorus-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "geetorus-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
