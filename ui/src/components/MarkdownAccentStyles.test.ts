...
describe("accent markdown styles", () => {
  it("inherits the current message bubble foreground for prose text, links, and list counters", () => {
    const block = cssBlock(".geetorus-markdown.geetorus-markdown-on-accent");
  ...
  it("keeps ordered-list markers and rendered link variants readable on accent bubbles", () => {
    expect(cssBlock(".geetorus-markdown.geetorus-markdown-on-accent li::marker")).toContain(
      "color: currentColor",
    );
    expect(cssBlock(".geetorus-markdown.geetorus-markdown-on-accent :where(a, a:visited)")).toContain(
      "color: currentColor",
    );
    expect(cssBlock(".geetorus-markdown.geetorus-markdown-on-accent .geetorus-workspace-file-link")).toContain(
      "color: currentColor",
    );
    expect(
      cssBlock(
        ".geetorus-markdown.geetorus-markdown-on-accent :where(a.geetorus-mention-chip, a.geetorus-project-mention-chip)",
      ),
