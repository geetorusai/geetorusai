...
import type { ActivityEvent, Issue, Agent, ProviderTraceMetadata } from "@geetorusai/shared";
import {
...
const RETRY_PENDING_LIVENESS_COPY: LivenessCopy = {
  ...
  description: "Geetorus queued an automatic retry that has not started yet.",
};
...
function watchdogDecisionErrorMessage(error: unknown) {
  ...
  return error instanceof Error && error.message.trim().length > 0
    ? error.message
    : "Geetorus could not record the watchdog decision.";
}
      ...
      pushToast({
        ...
        body:
          error instanceof Error
            ? error.message
            : "Geetorus could not start the trace re-run.",
        tone: "error",
    ...
    <section className="space-y-3" aria-label="Task run ledger">
      ...
      {latestSilentRun?.outputSilence ? (
        ...
        >
          ...
          <p className="mt-1">
            {latestSilentRun.outputSilence.evaluationIssueIdentifier
              ? "This signal is informational. Geetorus did not create new delegated recovery work."
              : "This signal is informational. Geetorus did not create or assign a recovery task."}
          </p>
