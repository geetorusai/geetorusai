...
import type { Issue, IssueDocument } from "@geetorusai/shared";
...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace> = {}): ExecutionWorkspace {
  return {
    ...
    cwd: "/tmp/geetorus/PAP-1",
    repoUrl: null,
    baseRef: "master",
    branchName: "pap-1-workspace",
    providerType: "git_worktree",
    providerRef: "/tmp/geetorus/PAP-1",
...
function createProject(overrides: Partial<Project> = {}): Project {
  ...
  return {
    ...
    codebase: {
      ...
      defaultRef: "master",
      repoName: null,
      localFolder: "/tmp/geetorus",
      managedFolder: "/tmp/geetorus",
      effectiveLocalFolder: "/tmp/geetorus",
      origin: "local_folder",
    },
...
describe("IssueProperties", () => {
  ...
  it("copies branch and folder values with visible feedback", async () => {
    ...
    const folderCopyButton = container.querySelector<HTMLButtonElement>(
      'button[aria-label="Copy /tmp/geetorus/PAP-1 to clipboard"]',
    );
    expect(branchCopyButton).not.toBeNull();
    expect(folderCopyButton?.querySelector('[data-middle-truncate="true"]')?.textContent)
      .toBe("/tmp/geetorus/PAP-1");
    expect(folderCopyButton?.title).toBe("/tmp/geetorus/PAP-1");
  ...
  it("renders each external object as its own properties row using display metadata", async () => {
    ...
    expect(pullRequestLink?.className).not.toContain("geetorus-mention-chip");
