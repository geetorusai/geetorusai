...
describe("CloudAccessGate", () => {
  ...
  it("shows browser sign-in setup for signed-out private bootstrap-pending instances", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "Finish setting up this Geetorus");

    expect(container.textContent).toContain("Finish setting up this Geetorus");
    expect(container.textContent).toContain("Sign in / Create account");
    expect(container.textContent).toContain("npx geetorusai auth bootstrap-ceo");
  ...
  it("keeps public bootstrap-pending instances invite-only", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "This Geetorus is waiting on its first admin");

    expect(container.textContent).toContain("This Geetorus is waiting on its first admin");
