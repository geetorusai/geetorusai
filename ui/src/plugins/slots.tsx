 ...
 *
 * Plugin bundles are built with `external: ["@geetorusai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__geetorusPluginBridge__`)
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        const R = globalThis.__geetorusPluginBridge__?.react;
        if (!R) {
          throw new Error("Geetorus plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react/jsx-runtime":
      ...
      source = `
        const BRIDGE = globalThis.__geetorusPluginBridge__;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__geetorusPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__geetorusPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingGeetorusSdkUiComponent() {
            throw new Error('Geetorus plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@geetorusai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@geetorusai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@geetorusai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@geetorusai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@geetorusai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__geetorusPluginBridge__) {
 ...
 * self-register via `window.geetorusPlugins.registerReactComponent()`.
 ...
 * of `@geetorusai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
