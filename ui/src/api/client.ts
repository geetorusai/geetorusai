...
function applyObservabilityHeaders(headers: Headers) {
  if (headers.has("X-Geetorus-Tab-Visible")) return; // caller override wins
  const visibility = getPageVisibility();
  headers.set("X-Geetorus-Tab-Visible", getVisibilityHeaderValue(visibility));
  if (typeof window !== "undefined" && window.location) {
    headers.set("X-Geetorus-Route", window.location.pathname);
  }
}
