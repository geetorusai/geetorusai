...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Geetorus API key");
    ...
    expect(prompt).toContain("agentDefaultsPayload.geetorusApiUrl");
    ...
    expect(prompt).toContain("Geetorus maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Geetorus host");
    expect(prompt).toContain("Hermes-originated Geetorus API calls");
    expect(prompt).toContain("Do not rotate or invent a Geetorus key manually");
