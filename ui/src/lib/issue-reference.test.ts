...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Geetorus issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/geetorusai/geetorus/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/geetorusai/geetorus/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Geetorus issue URLs so origin, port, and hash are not lost", () => {
