import type { ActivityEvent, Issue, SuccessfulRunHandoffState } from "@geetorusai/shared";
...
export function isSuccessfulRunHandoffEscalationComment(text: string) {
  ...
  return trimmed === SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY
    || /^Geetorus exhausted the bounded successful-run handoff correction\b/i.test(trimmed);
}
