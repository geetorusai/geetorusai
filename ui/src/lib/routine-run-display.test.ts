...
import type { RoutineVariable } from "@geetorusai/shared";
...
describe("runRowSubtitle", () => {
    ...
    const subtitle = runRowSubtitle(
      {
        status: "succeeded",
        failureReason: null,
        triggerPayload: { customer: "Acme", retries: 3, GEETORUS_RUN_ID: "ignored" },
      },
      variables,
    );
  ...
  it("only includes declared routine variables, not builtin payload keys", () => {
    const subtitle = runRowSubtitle(
      { status: "succeeded", failureReason: null, triggerPayload: { GEETORUS_RUN_ID: "x" } },
      variables,
    );
