...
const HANDOFF_REASON_COPY: Record<string, string> = {
  ...
  runtime_url_unusable: "The runtime row is publishing a URL Geetorus cannot open.",
...
export function resolveWorkspaceAccessState(input: {
    ...
    if (failure.reason === "workspace_not_ready" || failure.reason === "runtime_url_unusable") {
      ...
      return {
        ...
        description: [
          ...
          validating ? "Geetorus is still confirming the clone." : "One bounded repair replaces the isolated database.",
        ].join(" "),
        action: validating
  ...
  if (startingService) {
    return {
      ...
      description: "Geetorus is starting the workspace runtime and waiting for its board URL.",
  ...
  if (unhealthyService) {
    return {
      ...
      description: cause
        ?? "The runtime is up but did not report a usable database, so Geetorus will not publish it as ready.",
      action: { kind: "repair", label: "Repair workspace" },
      handoffAvailable,
    };
  }
