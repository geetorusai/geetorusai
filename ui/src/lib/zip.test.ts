...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("geetorus-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("geetorus-demo/COMPANY.md");
  ...
  it("reads a Geetorus zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
  ...
  it("reads standard DEFLATE zip archives created outside Geetorus", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("geetorus-demo");
...
describe("estimateZipArchiveSize", () => {
  it("matches the real archive byte length for mixed text and binary entries", () => {
    ...
    expect(estimateZipArchiveSize(files, "geetorus-demo")).toBe(
      createZipArchive(files, "geetorus-demo").byteLength,
    );
  });
  ...
  it("returns the 22-byte end-of-central-directory record for an empty map", () => {
    expect(estimateZipArchiveSize({}, "geetorus-demo")).toBe(22);
    expect(estimateZipArchiveSize({}, "geetorus-demo")).toBe(
      createZipArchive({}, "geetorus-demo").byteLength,
    );
  });
});
