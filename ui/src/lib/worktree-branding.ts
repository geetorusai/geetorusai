 ...
 * (`GEETORUS_IN_WORKTREE=true` on the server). Unlike {@link getWorktreeUiBranding}
...
export function isWorktreeRuntime(): boolean {
  return readMetaContent("geetorus-worktree-enabled") === "true";
}

/**
 * Runtime instance id of the worktree preview serving this UI, injected by the
 * server as a `<meta name="geetorus-instance-id">` tag. Returns null outside a
...
export function getWorktreeInstanceId(): string | null {
  return readMetaContent("geetorus-instance-id");
}

export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("geetorus-worktree-enabled") !== "true") return null;

  const name = readMetaContent("geetorus-worktree-name");
  const color = normalizeHexColor(readMetaContent("geetorus-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("geetorus-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
