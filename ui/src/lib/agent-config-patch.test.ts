...
describe("buildAgentUpdatePatch", () => {
  ...
  it("preserves geetorus skill-sync selections when changing adapter types", () => {
    // Desired skills are adapter-agnostic (company-level selections) but are
    // persisted inside the per-adapter config under `geetorusSkillSync`. A
    ...
    expect((patch.adapterConfig as Record<string, unknown>).geetorusSkillSync).toEqual({
