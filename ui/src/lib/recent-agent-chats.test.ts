...
describe("agent chat navigation and session markers", () => {
  ...
  it("keeps visits personal and company scoped and moves only the visited agent", () => {
    ...
    expect(
      JSON.parse(localStorage.getItem("geetorus.recentAgentChats:a:user1")!),
    ).toEqual(["agent1", "agent2"]);
    expect(
      JSON.parse(localStorage.getItem("geetorus.recentAgentChats:b:user1")!),
    ).toEqual(["agent3"]);
    expect(
      JSON.parse(localStorage.getItem("geetorus.recentAgentChats:a:user2")!),
    ).toEqual(["agent4"]);
  });
  it("removes legacy plaintext retry records", () => {
    const scope = "company:user:agent";
    const key = `geetorus:agent-chat-pending:${scope}`;
