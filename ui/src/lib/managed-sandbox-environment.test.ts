...
describe("managed sandbox environment helpers", () => {
  it("identifies platform-managed rows by the provisioner marker", () => {
    expect(isPlatformManagedEnvironment({ metadata: { managedByGeetorus: true } })).toBe(true);
    expect(isPlatformManagedEnvironment({ metadata: { managedByGeetorus: false } })).toBe(false);
