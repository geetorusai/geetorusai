...
declare global {
  interface Window {
    __GEETORUS_ISSUE_DETAIL_NAVIGATE_START__?: number;
  }
}
...
export function beginIssueDetailNavigation(): void {
  ...
  const externallyCapturedStart = window.__GEETORUS_ISSUE_DETAIL_NAVIGATE_START__;
  delete window.__GEETORUS_ISSUE_DETAIL_NAVIGATE_START__;
