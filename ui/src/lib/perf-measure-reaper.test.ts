...
describe("startPerfMeasureReaper", () => {
  ...
  beforeEach(() => {
    ...
    delete (globalThis as { __geetorusKeepPerfMeasures?: boolean }).__geetorusKeepPerfMeasures;
  });
  ...
  it("skips clearing while the opt-out flag is set (e.g. profiling)", () => {
    const stop = startPerfMeasureReaper(10_000);
    (globalThis as { __geetorusKeepPerfMeasures?: boolean }).__geetorusKeepPerfMeasures = true;
    vi.advanceTimersByTime(30_000);
    expect(clearMeasures).not.toHaveBeenCalled();
    (globalThis as { __geetorusKeepPerfMeasures?: boolean }).__geetorusKeepPerfMeasures = false;
    vi.advanceTimersByTime(10_000);
    expect(clearMeasures).toHaveBeenCalledTimes(1);
    stop();
  });
