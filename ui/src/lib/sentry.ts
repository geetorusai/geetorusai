...
export function initBrowserErrorMonitoring(dsn: string): Promise<void> {
  return enqueue(async () => {
    ...
    try {
      ...
      console.error("[geetorus] Sentry browser bootstrap failed", err);
    }
  });
}
...
export function teardownBrowserErrorMonitoring(): Promise<void> {
  return enqueue(async () => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[geetorus] Sentry client detach failed", err);
    }
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[geetorus] Sentry teardownBrowserErrorMonitoring failed", err);
    }
  });
}
...
export function captureBrowserException(error: unknown): void {
  void enqueue(async () => {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[geetorus] Sentry captureBrowserException failed", err);
    }
  });
}
