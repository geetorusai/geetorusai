...
import type { Agent, IssueComment } from "@geetorusai/shared";
...
function authorNameForComment(
  ...
  if (authorAgentId) {
    return agentMap?.get(authorAgentId)?.name ?? (options?.isSystemNotice ? "Geetorus" : authorAgentId.slice(0, 8));
  }
  const authorUserId = comment.authorUserId ?? null;
  if (!authorUserId) return options?.isSystemNotice ? "Geetorus" : "You";
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (entry.kind === "provider_activity") {
      ...
      orderedParts.push({
        type: "tool-call",
        toolCallId,
        toolName: "geetorus_provider_activity",
        args,
        argsText: "",
