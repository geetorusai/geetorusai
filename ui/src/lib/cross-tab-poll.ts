 ...
 *
 * Goal: when a user restores many Geetorus tabs at once, we do not want every
...
const TAB_ID_STORAGE_KEY = "geetorus:shared-poll:tab-id";
...
export function sharedPollingLeaseKey(companyId: string): string {
  return `geetorus:shared-poll:${sanitizeCompanyId(companyId)}:leader`;
}

export function sharedPollingChannelName(companyId: string): string {
  return `geetorus:shared-poll:${sanitizeCompanyId(companyId)}`;
}
...
function createBrowserLeaseStore(companyId: string): LeaseStore | null {
  ...
  try {
    const probeKey = `geetorus:shared-poll:${sanitizeCompanyId(companyId)}:probe`;
