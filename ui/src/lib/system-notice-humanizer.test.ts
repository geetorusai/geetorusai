...
describe("humanizeSystemNotice", () => {
  it("uses presentation title and tone when present", () => {
    const result = humanizeSystemNotice({
      body: "Geetorus automatically retried dispatch but it still has no live execution path.",
      presentation: {
        kind: "system_notice",
        tone: "danger",
  ...
  it("classifies claude_auth_required retry failures", () => {
    const result = humanizeSystemNotice({
      body:
        "Geetorus automatically retried continuation for this assigned `in_progress` issue during terminal run " +
  ...
  it("still humanizes no-live-execution-path notices without an owner link", () => {
    const result = humanizeSystemNotice({
      body:
        "Geetorus automatically retried dispatch, but it still has no live execution path. " +
