...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Geetorus will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Geetorus handles this" mechanics bullets, composed from the
