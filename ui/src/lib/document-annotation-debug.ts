export const SELECTION_DEBUG_STORAGE_KEY = "geetorusDebugSelection";
...
declare global {
  interface Window {
    __geetorusSelectionDebug?: SelectionDebugState;
  }
}
...
export function initializeSelectionDebug(): SelectionDebugState {
  if (!window.__geetorusSelectionDebug) {
    window.__geetorusSelectionDebug = createState();
    console.info(`[geetorus selection debug] enabled; set localStorage.${SELECTION_DEBUG_STORAGE_KEY} = "0" and reload to disable`);
  }
  return window.__geetorusSelectionDebug;
}
...
function record(type: string, details: SelectionDebugEvent["details"]): void {
  ...
  console.debug("[geetorus selection debug]", event);
}
