...
const deepResearch = skill("11111111-1111-4111-8111-111111111111", "deep-research", "geetorus/deep-research");
const scopedDeepResearch = skill("22222222-2222-4222-8222-222222222222", "deep-research", "community/deep-research");
const browser = skill("33333333-3333-4333-8333-333333333333", "browser", "geetorus/browser");
...
describe("company skill routes", () => {
  ...
  it("uses a unique key to disambiguate colliding slugs", () => {
    ...
    expect(canonicalSkillRouteToken(deepResearch, skills)).toBe("geetorus/deep-research");
    expect(canonicalSkillRouteToken(scopedDeepResearch, skills)).toBe("community/deep-research");
    expect(skillRoute(deepResearch, skills)).toBe("/skills/geetorus/deep-research");
  });

  it("falls back to a slug plus short id when the key is not route-safe", () => {
    const unsafe = skill("44444444-4444-4444-8444-444444444444", "deep-research", "geetorus/files/deep-research");
  ...
  it("parses token paths and encoded file paths", () => {
    expect(parseSkillRoute("geetorus/deep-research/files/references/setup%20guide.md")).toEqual({
      skillToken: "geetorus/deep-research",
