...
const context = {
  ...
  projects: [
    { id: "11111111-1111-4111-8111-111111111111", name: "Geetorus App", urlKey: "geetorus-app" },
  ],
  labels: [
...
describe("parseSearchQuery", () => {
  ...
  it("parses project names", () => {
    expect(parseSearchQuery("project:geetorus-app", context).filters).toEqual({
  ...
  it("parses mixed free text and multiple operators", () => {
    expect(parseSearchQuery("auth status:in_progress priority:critical project:geetorus-app", context)).toMatchObject({
      query: "auth",
      filters: {
        status: ["in_progress"],
        priority: ["critical"],
