 ...
 * managed-config environment provisioner stamps `metadata.managedByGeetorus`).
...
export function isPlatformManagedEnvironment(
  ...
  return environment?.metadata?.managedByGeetorus === true;
}
 ...
 * default Geetorus environment, and the raw driver key ("sandbox") is
