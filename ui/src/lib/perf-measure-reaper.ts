 ...
 * does not even report. On a busy Geetorus tab this was measured at ~340
...
export function startPerfMeasureReaper(intervalMs: number = DEFAULT_INTERVAL_MS): () => void {
  ...
  const reap = () => {
    if ((globalThis as PerfMeasureReaperGlobal).__geetorusKeepPerfMeasures) return;
