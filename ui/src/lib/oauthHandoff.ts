import type { ToolOAuthStartResult } from "@geetorusai/shared";
...
const PENDING_HANDOFF_KEY = "geetorus.cloud-oauth-handoff.v1";
...
function parseHandoff(value: unknown): { kind: "geetorus_cloud"; session: string } | null {
  if (value === undefined) return null;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new OAuthHandoffError("Geetorus Cloud returned an invalid sign-in handoff.", "invalid_handoff");
  }
  const handoff = value as Record<string, unknown>;
  if (
    handoff.kind !== "geetorus_cloud"
  ...
  return { kind: "geetorus_cloud", session: handoff.session };
}
...
function handoffFailure(status: number, code: unknown): OAuthHandoffError {
  ...
  if (status === 401 || status === 403) {
    return new OAuthHandoffError("Geetorus Cloud could not authorize this connection for your account.", "forbidden");
  }
  return new OAuthHandoffError("Geetorus Cloud couldn’t prepare secure sign-in. Try again.", "unavailable");
}
...
async function postCloudHandoff(
  ...
  throw new OAuthHandoffError(
    lastError instanceof Error ? lastError.message : "Geetorus Cloud couldn’t prepare secure sign-in. Try again.",
    "unavailable",
  );
}
...
export async function prepareOAuthNavigation(
  ...
  if (!authorization.ok) {
    throw new OAuthHandoffError("Geetorus Cloud returned an invalid provider sign-in address.", "invalid_handoff");
  }
...
export function savePendingCloudHandoff(
  ...
  const handoff = parseHandoff({ kind: "geetorus_cloud", session });
  if (!handoff) throw new OAuthHandoffError("Geetorus Cloud returned an invalid sign-in handoff.", "invalid_handoff");
...
export function readPendingCloudHandoff(
...
): { kind: "geetorus_cloud"; session: string } | null {
  ...
  try {
    ...
    return parseHandoff({ kind: "geetorus_cloud", session: pending.session });
  } catch {
