...
function enrollmentAccessStorageKey(appKey: string): string {
  return `geetorus.connector-enrollment-access:${appKey}`;
}
...
function oauthCallbackErrorMessage(outcome: string | null, code: string | null): string {
  ...
  if (code === "github_installation_required") {
    return "GitHub access is required. Install Geetorus and grant at least one repository, then try again.";
  }
    ...
    entry
      ? entry.methods.filter(
          (candidate) =>
            ...
            (candidate.oauthStrategy === "geetorus_cloud_connector" ||
              candidate.oauthStrategy === "geetorus_id_connector"),
        )
      : [],
  );
}
...
export function readConnectionIntentOAuthOutcome(
  ...
  if (
    message.type !== "geetorus.connection-intent.oauth"
...
export interface ConnectionSetupFlowProps {
  aiConnection?: import("@geetorusai/shared").AiConnectionBinding;
...
export function ConnectionSetupFlow({
  ...
  const directOAuthSource = credentialSource === "geetorus_vault"
  ...
  // simple path: Geetorus probes the endpoint and branches on what it finds.
  ...
  const openAuthorization = useCallback((url: string) => {
    ...
    if (!popup || popup.closed) {
      setOAuthPhase("error");
      setOAuthError("Geetorus couldn’t open the sign-in window. Open sign-in in a new tab to continue.");
    ...
    try {
      ...
      if (target.kind === "reauthentication") {
        ...
        if (!destination || destination.closed || !start.handoff) {
          throw new Error("Geetorus couldn’t preserve this sign-in while refreshing your account.");
        }
      ...
      setOAuthPhase("error");
      setOAuthError(error instanceof Error ? error.message : "Geetorus couldn’t start secure sign-in. Try again.");
      onPhaseChange?.("needs_retry");
    } finally {
  ...
  const useMatchedGalleryEntry = (picked: AppDefinition) => {
    ...
    if (credentialSource === "geetorus_vault" && canUseAutomaticOAuthFastPath(picked)) {
  ...
  const startConnectorEnrollment = useMutation({
    ...
    onSuccess: (status) => {
      if (!status.verificationUrl) {
        closeEnrollmentPopup();
        setConnectorEnrollmentError("Geetorus Cloud did not return an enrollment link. Try again.");
        return;
      }
    ...
    onError: (error) => {
      ...
      setConnectorEnrollmentError(
        error instanceof Error ? error.message : "Geetorus couldn’t reach Geetorus Cloud. Try again.",
      );
    },
  });
  ...
  const automaticOAuthEntry = credentialSource === "geetorus_vault" && entryAutomaticOAuthMethod ? entry : null;
  const directOAuthEntry = credentialSource === "geetorus_vault" && canUseAutomaticOAuthFastPath(entry) ? entry : null;
      ...
      setOAuthError(
        details?.code === "invalid_grant"
          ...
          : error instanceof Error
            ? error.message
            : "Geetorus couldn’t start secure sign-in. Try again.",
      );
    },
  });
    ...
    mutationFn: async (entryOverride?: AppDefinition) => {
      ...
      if (connectEntry) {
        ...
        result = await toolsApi.connectApp(selectedCompanyId!, {
          ...
          name: connectionName,
          credentialSource,
          ...(credentialSource === "geetorus_vault" ? { credentialValues: credentials } : {}),
          ...(credentialSource === "vercel_connect"
      ...
      // before the browser leaves Geetorus.
  ...
  useEffect(() => {
    ...
    const method = methods.length === 1 ? methods[0]! : null;
    const automaticOAuth = credentialSource === "geetorus_vault" && Boolean(automaticOAuthMethod(requestedEntry));
    ...
    if (automaticOAuth && (applicationsQuery.isError || connectionsQuery.isError)) {
      setOAuthPhase("error");
      setOAuthError("Geetorus couldn’t check for an existing connection. Try again.");
      setStep("key");
      return;
    }
  }, [
    ...
    return (
      <div className="mx-auto max-w-xl rounded-xl border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Geetorus couldn’t check the retained connection. The retained connection was not changed.
        </p>
  ...
  if ((resumeConnectionId || reconnectConnectionId) && galleryQuery.isError) {
    return (
      <div className="mx-auto max-w-xl rounded-xl border border-border bg-card p-6">
        <h2 className="text-lg font-semibold text-foreground">Couldn’t load connection setup</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Geetorus couldn’t load the provider details needed to restore this connection. The retained connection was not changed.
        </p>
        <div className="mt-5 flex flex-wrap gap-2">
  ...
  if (reconnectConnectionId && unavailableReconnectId === reconnectConnectionId) {
    return (
      <div className="mx-auto max-w-xl rounded-xl border border-border bg-card p-6">
        <h2 className="text-lg font-semibold text-foreground">This connection can’t be reconnected</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Geetorus no longer has a supported setup method for this retained connection. The retained connection was not changed.
        </p>
        <Button type="button" variant="outline" className="mt-5" onClick={() => navigate("/apps")}>
          Back to apps
        </Button>
    ...
    <div className="max-w-5xl">
      ...
      {managedConnectorUnavailable && entry ? (
        <div className="mx-auto max-w-xl rounded-xl border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            This instance is connected to Geetorus, but {entry.name} sign-in is not currently available. Try again shortly or contact your instance administrator.
          </p>
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-foreground">
                  Connect with Geetorus
                </h2>
                <p className="mt-2 text-sm text-muted-foreground">
                  You must connect this instance to Geetorus to connect to {entry.name} (you only need to do this once).
                </p>
              </div>
            </div>
            ...
            {connectorEnrollmentQuery.isError || connectorEnrollmentError ? (
              <InlineBanner tone="danger" className="mt-4">
                {connectorEnrollmentError ?? "Geetorus couldn’t check Cloud registration. Try again."}
              </InlineBanner>
            ) : null}
  ...
  const status = phase === "entry"
    ? {
        title: resuming
          ...
          : `Connect ${serverName} to Geetorus`,
        body: resuming
          ? `Your connection is saved. Continue in ${serverName} to approve access; its identity and agent access will stay the same.`
          : `Geetorus will open ${serverName} so you can choose a workspace and approve access.`,
      }
    : phase === "starting"
      ? {
          title: "Preparing secure sign-in",
          body: `Geetorus is creating a secure ${serverName} connection.`,
        }
      : phase === "redirecting"
        ? {
        ...
        : {
            ...
            body: error ?? "Geetorus couldn’t start secure sign-in. Try again.",
          };
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                Create and manage the provider connector in Vercel. Geetorus stores its reference and applies agent access, policy, approval, and audit controls here.
              </p>
            </div>
            {vercelConnectAvailability ? (
 ...
 * Geetorus will use to satisfy it.
 *
    ...
    <div className="mx-auto max-w-xl">
      ...
      {matchedEntry && onUseMatchedEntry ? (
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-muted/40 px-3 py-2">
          <div className="flex min-w-0 items-center gap-2 text-sm">
            <AppLogo name={matchedEntry.name} logoUrl={matchedEntry.branding.logoUrl} darkLogoUrl={matchedEntry.branding.darkLogoUrl} size={24} />
            <span className="truncate">Geetorus has a guided setup for {matchedEntry.name}.</span>
          </div>
        ...
        <Collapsible open={advancedOpen} onOpenChange={onAdvancedOpenChange}>
          ...
          <CollapsibleContent className="space-y-5 pt-4">
            ...
            {authMode === "oauth" ? (
              <div className="space-y-4">
                <p className="text-xs text-muted-foreground">
                  Geetorus sets sign-in up on its own whenever the server allows it. Only fill these in when the
                  server's docs tell you to register Geetorus yourself first.
                </p>
                <div>
 ...
 * terms. Geetorus decides internally whether that means a preconfigured client,
...
const GENERIC_AUTH_MODE_OPTIONS: Array<{ mode: GenericMcpAuthMode; label: string; hint: string }> = [
  {
    mode: "auto",
    label: "Let Geetorus check",
    hint: "Geetorus asks the server what it needs and walks you through it. Start here.",
  },
  {
    mode: "none",
  ...
  },
  {
    mode: "bearer",
    label: "Key or token",
    hint: "Geetorus sends your key as an Authorization header.",
  },
  {
    mode: "custom_headers",
    label: "Custom headers",
    hint: "For servers that name their own headers. Values are stored as Geetorus secrets and can\u2019t be read back.",
  },
  {
    mode: "oauth",
    label: "Browser sign-in",
    hint: "You\u2019ll sign in at the provider. Add a client ID and secret only if the provider requires you to register Geetorus first.",
  },
];
...
function KeyStep({
  ...
  const capabilitySelection = capabilityGroups.length > 1 ? (
    <div>
      <label className="text-sm font-medium text-foreground">What should Geetorus be able to do?</label>
      <RadioCardGroup
  ...
  const authenticationSelection = managedGoogleMethod && customerGoogleMethod && capabilityMethods.length === 2 ? (
    ...
    >
      {usingCustomGoogleOAuth ? "Use Geetorus instead" : "Use your own Google OAuth app"}
    </Button>
        ...
        {usingVercel && vercelReview && vercelConnectAvailability ? (
          <div className="space-y-4 rounded-lg border border-border p-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Geetorus does not copy Vercel’s setup forms. Finish connector setup there, then paste its UID below.
              </p>
              <a
            ...
            <div>
              ...
              />
              <p className="mt-2 text-xs text-muted-foreground">
                Geetorus validates the connector and stores only its reference and redacted verification metadata.
              </p>
            </div>
          </div>
        ) : null}
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border p-4">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Register Geetorus's callback URI in {entry.name}, then enter the customer-owned client details.
        </p>
      ...
      {callbackUrl ? (
        <div>
          <label className="text-sm font-medium text-foreground">Geetorus callback URL</label>
          <div className="mt-2 flex min-w-0 flex-col gap-2 sm:flex-row">
            <div
              title={callbackUrl}
              className="min-h-11 min-w-0 flex-1 rounded-md border border-input bg-muted/40 px-3 py-2.5 font-mono text-xs leading-tight text-foreground break-all"
