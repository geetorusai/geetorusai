...
import type { ToolConnectionCredentialSource } from "@geetorusai/shared";
...
import { GeetorusCloudOAuthHandoffPage } from "./pages/apps/GeetorusCloudOAuthHandoff";
...
function ProductionSurface({ children }: { children: ReactNode }) {
  return <Suspense fallback={<GeetorusLoading />}>{children}</Suspense>;
}
    ...
    <>
      ...
      <Route element={<HiddenSettingsPageGate pageKey="company.export" />}>
        <Route
          path="company/export/*"
          element={(
            <Suspense fallback={<GeetorusLoading />}>
              <CompanyExport />
            </Suspense>
          )}
        />
          ...
          {!matchedCompany && cloudInstance && !createStackUrl ? (
            <p className="text-sm text-muted-foreground">
              {t("app.cloudCreateUnavailable", {
                defaultValue:
                  "Organizations are created in Geetorus Cloud. This instance can't reach it right now — try again from your Cloud portfolio.",
              })}
            </p>
          ) : (
            <Button
...
export function App() {
  ...
  return (
    <>
      <Routes>
        <Route path="oauth-handoff" element={<GeetorusCloudOAuthHandoffPage />} />
        <Route path="auth" element={<AuthPage />} />
        <Route path="board-claim/:token" element={<BoardClaimPage />} />
        <Route path="cli-auth/:id" element={<CliAuthPage />} />
        <Route path="invite/:token" element={<InviteLandingPage />} />
        <Route element={streamlinedUiLoaded ? <CloudAccessGate /> : <GeetorusLoading />}>
