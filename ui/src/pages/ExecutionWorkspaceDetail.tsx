...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, RoutineListItem, WorkspaceOperation } from "@geetorusai/shared";
...
function LegacyWorkspaceTabRedirect({ workspaceId }: { workspaceId: string }) {
  useEffect(() => {
    try {
      localStorage.removeItem(`geetorus:execution-workspace-tab:${workspaceId}`);
    } catch {}
  }, [workspaceId]);
...
function ExecutionWorkspaceIssuesList({
  ...
  return (
    <IssuesList
      ...
      viewStateKey="geetorus:execution-workspace-issues-view"
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        ...
        body: "Geetorus created a run using this execution workspace.",
        tone: "success",
      });
    },
    onSettled: () => {
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                ...
                <CardDescription>
                  Edit the concrete path, repo, branch, provisioning, teardown, and runtime overrides attached to this execution workspace. Saved changes affect future runs; Geetorus may refresh or replace a reused workspace when config changes.
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                ...
                {!hideHostPaths && (
                  <>
                    <div className="space-y-4">
                      ...
                      <Field label="Provision command" hint="Runs when Geetorus prepares this execution workspace">
                        <Textarea
                ...
                <div className="space-y-4">
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Geetorus checks fixed ports again when a service starts and rejects cross-workspace conflicts.
                      </p>
                    </div>
                  ) : null}
                </div>
