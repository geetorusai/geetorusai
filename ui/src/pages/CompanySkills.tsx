...
import type { FolderListItem, FolderListResult } from "@geetorusai/shared";
...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "geetorus":
      return { icon: Paperclip, label: sourceLabel ?? "Geetorus", managedLabel: "Geetorus managed" };
    default:
...
function classifySource(skill: {
  ...
  if (skill.sourceBadge === "geetorus") return "company";
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      ...
      tooltip: "Geetorus could not validate this skill as Agent Skills markdown. Install at your own risk.",
...
export function buildDiscoveryCards(
  ...
  for (const [identity, entry] of catalogByKey) {
    ...
    cards.push({
      ...
      name: entry.name,
      slug: entry.slug,
      author: entry.packageName ?? "Geetorus",
...
export function CompanySkills() {
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: `${skill.name} is now editable in the Geetorus workspace.`,
      });
    },
    onError: (error) => {
  ...
  const studioDescription = studioForkFromId
    ...
    : "Create an editable organization skill in the Geetorus workspace.";
  return (
    <>
      {policyDenial.denial ? (
      ...
      {isStudioNew ? (
        ...
        <DiscoveryGrid
          ...
          folderNudgeStorageKey={showInstalledFolders ? `geetorus:skills-folder-nudge:${selectedCompanyId ?? "none"}` : undefined}
