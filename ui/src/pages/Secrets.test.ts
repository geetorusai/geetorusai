...
import type { CompanySecretProviderConfig, SecretProviderDescriptor } from "@geetorusai/shared";
import {
...
describe("Secrets page provider helpers", () => {
  it("previews the derived AWS managed path from provider health details", () => {
    ...
    ).toBe("geetorus/prod-us-1/company-123/anthropic-api-key");
  });
  ...
  it("uses provider health copy when an unconfigured provider reports missing bootstrap inputs", () => {
      ...
      providers: [
        {
          ...
          status: "warn",
          message:
            "AWS Secrets Manager provider is not ready: missing GEETORUS_SECRETS_AWS_DEPLOYMENT_ID.",
        },
      ],
    };
    ...
    expect(
      ...
      "Deployment default AWS Secrets Manager is not configured. Select a ready provider vault or configure the deployment default. AWS Secrets Manager provider is not ready: missing GEETORUS_SECRETS_AWS_DEPLOYMENT_ID.",
    );
  });
