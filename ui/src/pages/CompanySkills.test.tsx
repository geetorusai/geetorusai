...
import type { CatalogSkill, CompanySkillDetail, CompanySkillListItem, CompanySkillVersion, FolderListResult } from "@geetorusai/shared";
...
describe("DiscoveryGrid IA presentation", () => {
  ...
  it("distinguishes installation from agent enablement on cards", async () => {
    ...
    expect(node.textContent).toContain("Geetorus catalog");
  });
...
describe("SkillDetailPage settings", () => {
  ...
  it("renders long source paths in full so they can wrap inside the sidebar", async () => {
    const v1 = makeVersion(1, "# Demo Skill");
    const longSourcePath = "/srv/geetorus/home/geetorusai/geetorus/.agents/skills/prepare-pr/SKILL.md";
    const node = await renderSkillDetail([v1], {
      activeTab: "agents",
      detail: makeDetail(v1, {
...
describe("install-time agent enablement", () => {
  ...
  function makeCatalogSkill(): CatalogSkill {
    return {
      id: "catalog-1",
      key: "geetorusai/bundled/product/wireframe",
      kind: "bundled",
      category: "product",
      slug: "wireframe",
      name: "wireframe",
