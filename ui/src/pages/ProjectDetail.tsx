...
import { PROJECT_COLORS, PROJECT_ICON_NAMES, isUuidLike, type BudgetPolicySummary } from "@geetorusai/shared";
...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey="geetorus:project-issues-view"
...
function ProjectPluginOperationsList({
  ...
  return (
    <IssuesList
      ...
      liveIssueIds={liveIssueIds}
      projectId={projectId}
      viewStateKey={`geetorus:project-plugin-operations-view:${pluginKey}`}
      onUpdateIssue={(id, data) => updateIssue.mutate({ id, data })}
    />
  );
}
...
export function ProjectDetail() {
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`geetorus:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`geetorus:project-tab:${project.id}`, tab); } catch {}
    }
