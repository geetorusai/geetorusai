...
import type { RoutineListItem, RoutineVariable } from "@geetorusai/shared";
import type { FolderListItem } from "@geetorusai/shared";
import {
  AllUnfiledBanner,
  BulkBar,
  DeleteFolderDialog,
...
export function Routines() {
  ...
  const routineViewStateKey = selectedCompanyId
    ? `geetorus:routines-view:${selectedCompanyId}`
    : "geetorus:routines-view";
  ...
  const createFolder = useMutation({
      ...
      if (moveAfterCreateIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            ...
            body: moveError instanceof Error ? moveError.message : "Geetorus could not move the selected routines.",
            tone: "error",
          });
          return;
        }
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Geetorus could not save the folder.",
        tone: "error",
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Geetorus could not update the folder.",
        tone: "error",
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Geetorus could not delete the folder.",
        tone: "error",
      });
    },
  });
  const moveRoutineToFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: "Move failed",
        body: mutationError instanceof Error ? mutationError.message : "Geetorus could not move the routine.",
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Geetorus could not update the routine.",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Geetorus could not start the routine run.",
        tone: "error",
      });
    },
  });
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="runs">
          <IssuesList
            ...
            viewStateKey="geetorus:routine-recent-runs-view"
        ...
        >
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              After creation, Geetorus takes you straight to trigger setup. Draft routines stay paused until you add a default agent.
            </div>
      ...
      {activeTab === "routines" ? (
        <div className={cn(showFolderRail && "flex gap-4")}>
          ...
          {routineViewState.groupBy === "folder" && !hasRoutineFolders && !foldersLoading && visibleRoutines.length > 0 ? (
            <AllUnfiledBanner
              storageKey={`geetorus:routines-folder-nudge:${selectedCompanyId ?? "none"}`}
              itemLabelPlural="routines"
