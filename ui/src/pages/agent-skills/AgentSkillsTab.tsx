...
import type { Agent, AgentDesiredSkillEntry } from "@geetorusai/shared";
...
const MATERIALIZATION_NOTE =
  "Enabled skills are materialized into the stable Geetorus-managed prompt bundle on the agent's next run.";

/** Company skill key of the Geetorus core skill that carries beta releases. */
const GEETORUS_CORE_SKILL_KEY = "geetorusai/geetorus/geetorus";
...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
  ...
  const geetorusCoreSkill = useMemo(
    () => (companySkills ?? []).find((skill) => skill.key === GEETORUS_CORE_SKILL_KEY) ?? null,
    [companySkills, skillSnapshot],
  );

  // Seeded releases (release_id IS NOT NULL) for the geetorus core skill. Only
  // fetched when the flag is on and the skill is present in the library.
  const { data: geetorusVersions } = useQuery({
    queryKey: queryKeys.companySkills.versions(companyId ?? "", geetorusCoreSkill?.id ?? ""),
    queryFn: () => companySkillsApi.versions(companyId!, geetorusCoreSkill!.id),
    enabled: Boolean(companyId && betaSkillsEnabled && geetorusCoreSkill?.id),
  });
  const geetorusReleases = useMemo(
    () => (geetorusVersions ?? []).filter((version) => version.releaseId != null),
    [geetorusVersions],
  );
  ...
  const unsupportedMessage = useMemo(() => {
    ...
    if (
      ...
      return "Geetorus cannot manage skills for custom ACP commands yet.";
    }
    if (agent.adapterType === "openclaw_gateway") {
      return "Geetorus cannot manage OpenClaw skills here. Visit your OpenClaw instance to manage this agent's skills.";
    }
    return "Geetorus cannot manage skills for this adapter yet. Manage them in the adapter directly.";
  ...
  // The release picker only applies to the enabled geetorus core skill while the
  // beta-skills flag is on and seeded releases exist.
  const releasePickerActive = betaSkillsEnabled && geetorusReleases.length > 0;
  ...
  const renderRow = (row: AgentSkillRowData, variant: "enabled" | "available") => {
    ...
    const legacyGeetorusBlocked = agent.adapterType === "geetorus_runner"
      && variant === "available"
      && row.key === GEETORUS_CORE_SKILL_KEY;
    const rowDisabled = unsupported || legacyGeetorusBlocked;
    const rowDisabledReason = legacyGeetorusBlocked
      ? "Geetorus Runner uses native semantic coordination and cannot attach the legacy Geetorus operational skill."
      : unsupportedMessage;
    const showReleasePicker =
      releasePickerActive && variant === "enabled" && row.key === GEETORUS_CORE_SKILL_KEY;
    const pinnedVersionId = versionPins[row.key] ?? null;
    const pinnedRelease = pinnedVersionId
      ? geetorusReleases.find((release) => release.id === pinnedVersionId) ?? null
      : null;
