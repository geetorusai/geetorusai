...
describe("buildAgentSkillSourceMeta", () => {
  ...
  it("does not surface long filesystem labels for local skills", () => {
    expect(source({
      sourceBadge: "local",
      sourceLabel: "/Users/dev/work/geetorus/skills/private-review",
      sourceLocator: null,
  ...
  it("keeps human-readable project scan labels for local skills", () => {
    expect(source({
      sourceBadge: "local",
      sourceLabel: "Geetorus App / Engineering workspace",
      sourceLocator: null,
      sourceType: "local_path",
    })).toBe("Geetorus App / Engineering workspace");
  });

  it("does not surface long filesystem labels for catalog skills", () => {
    expect(source({
      sourceBadge: "catalog",
      sourceLabel: "/srv/geetorus/home/.geetorus/instances/default/skills/company-id/__catalog__/briefs-discover-cards--68f7e3ad47",
      sourceLocator: null,
      sourceType: "catalog",
    })).toBe("Catalog");
  });
