...
const GEETORUS_DEVELOPER_MODE_TOGGLE_SELECTOR =
  'button[aria-label="Toggle Geetorus developer mode experimental setting"]';
...
const GEETORUS_RUNNER_TOGGLE_SELECTOR =
  'button[aria-label="Toggle Geetorus Runner experimental setting"]';
...
function defaultExperimentalSettings(): InstanceExperimentalSettingsPayload {
  return {
    ...
    enableGeetorusDeveloperMode: false,
...
function setWorktreeRuntimeMeta(enabled: boolean) {
  const name = "geetorus-worktree-enabled";
...
function setWorktreeInstanceIdMeta(instanceId: string | null) {
  const name = "geetorus-instance-id";
  let meta = document.querySelector<HTMLMetaElement>(`meta[name="${name}"]`);
  if (instanceId) {
    if (!meta) {
      meta = document.createElement("meta");
...
describe("InstanceExperimentalSettings — Conference Room Chat card (PAP-11233)", () => {
  ...
  it("keeps Geetorus Runner default-off and exposes an explicit opt-in", async () => {
    await renderPage();

    expect(container.textContent).toContain("Geetorus Runner");
    expect(container.textContent).toContain("Onboarding continues to use legacy adapters");
    const toggle = container.querySelector<HTMLButtonElement>(
      GEETORUS_RUNNER_TOGGLE_SELECTOR,
    );
    expect(toggle?.getAttribute("aria-checked")).toBe("false");
  ...
  it("renders and patches the Built-in Agents experimental toggle", async () => {
    ...
    expect(container.textContent).toContain("Show Geetorus-managed built-in agent surfaces");
  ...
  it("renders and patches the Beta skills experimental toggle", async () => {
    ...
    expect(container.textContent).toContain("pin beta releases of the Geetorus core skill");
  ...
  it("renders and patches Geetorus Developer Mode", async () => {
    await renderPage();

    expect(container.textContent).toContain("Geetorus Developer Mode");
    expect(container.textContent).toContain("including Honeycomb trace queries on run pages");

    const toggle = container.querySelector<HTMLButtonElement>(
      GEETORUS_DEVELOPER_MODE_TOGGLE_SELECTOR,
    );
    expect(toggle?.getAttribute("aria-checked")).toBe("false");
    ...
    expect(mockInstanceSettingsApi.updateExperimental).toHaveBeenCalledWith({
      enableGeetorusDeveloperMode: true,
    });
    expect(toggle?.getAttribute("aria-checked")).toBe("true");
  });
...
describe("InstanceExperimentalSettings — cloud-managed keys", () => {
  const MANAGED_BADGE_TEXT = "Managed by Geetorus Cloud";
  ...
  it("renders a managed key locked with the badge while unmanaged keys stay editable", async () => {
    await renderPage({
      ...
      enableBuiltInAgents: true,
      managedKeys: {
        enableBuiltInAgents: { managed: true, managedBy: "geetorus-cloud" },
      },
    });
  ...
  it("keeps a managed isolated-workspaces-by-default setting locked", async () => {
    ...
    await renderPage({
      ...
      enableIsolatedWorkspacesByDefault: true,
      managedKeys: {
        enableIsolatedWorkspacesByDefault: { managed: true, managedBy: "geetorus-cloud" },
      },
    });
  ...
  it("keeps a managed chat connectors setting locked", async () => {
    await renderPage({
      ...defaultExperimentalSettings(),
      managedKeys: { enableChatConnectors: { managed: true, managedBy: "geetorus-cloud" } },
    });
  ...
  it("locks Status Cards when managed Summaries is disabled", async () => {
    await renderPage({
      ...defaultExperimentalSettings(),
      managedKeys: {
        enableSummaries: { managed: true, managedBy: "geetorus-cloud" },
      },
    });
  ...
  it("locks Summaries on when managed Status Cards is enabled", async () => {
    await renderPage({
      ...
      enableStatusCards: true,
      managedKeys: {
        enableStatusCards: { managed: true, managedBy: "geetorus-cloud" },
      },
    });
