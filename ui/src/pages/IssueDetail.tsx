...
const IssueDetailChatTab = memo(function IssueDetailChatTab({
  ...
  // the protocol: native Geetorus turns can steer in place, while legacy
    ...
    const responseComments = classicTaskInterfaceEnabled
      ...
      : interactions.flatMap((interaction): IssueDetailComment[] => {
          ...
          const queuedTargetRunId =
            ...
            interruptibleIssueRun.adapterType !== "geetorus_runner"
              ? interaction.sourceRunId
              : null;
          const body =
            interaction.kind === "ask_user_questions"
  ...
  const effectiveQueuedCommentQueue = useMemo(() => {
    ...
    const fallbackProtocol =
      ...
      liveRuntimeRun.adapterType === "geetorus_runner"
        ? "geetorus_runner_v1"
        : "legacy";
      ...
      try {
        ...
        if (code === "queued_comment_already_dispatching") {
          pushToast({
            ...
            body: "The continuation started before the discard was confirmed, so Geetorus could not unsend it.",
            tone: "error",
            ttlMs: 15_000,
    ...
    () =>
      ...
      showRichSubIssuesSection ? (
        <IssuesList
          ...
          viewStateKey={`geetorus:issue-detail:${issue.id}:subissues-view`}
            ...
            >
              ...
              {resolvedDetailTab === "chat" ? (
                <IssueDetailChatTab
                  ...
                  userLabelMap={userLabelMap}
                  userProfileMap={userProfileMap}
                  draftKey={conversationAgent ? `geetorus:agent-chat-draft:${issue.companyId}:${currentUserId}:${conversationAgent.id}` : `geetorus:issue-comment-draft:${issue.id}`}
