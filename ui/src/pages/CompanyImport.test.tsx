...
import type { CompanyPortabilityImportResult, CompanyPortabilityPreviewResult } from "@geetorusai/shared";
...
describe("CompanyImport", () => {
  ...
  it("resumes watching a stored import job on mount", async () => {
    ...
    sessionStorage.setItem(
      "geetorus:company-import-job:company-1:acme/starter",
    ...
    expect(sessionStorage.getItem("geetorus:company-import-job:company-1:acme/starter")).toBeNull();
  });
  ...
  it("hides Geetorus Runner import configuration while its experimental flag is off", async () => {
    mockAdaptersApi.list.mockResolvedValue([
      { type: "claude_local", disabled: false },
      { type: "codex_local", disabled: false },
      { type: "geetorus_runner", disabled: true },
    ]);
    await previewMixedAdapterPackage();
  ...
  it("offers Geetorus Runner import configuration after its experimental flag is enabled", async () => {
    mockAdaptersApi.list.mockResolvedValue([
      { type: "claude_local", disabled: false },
      { type: "codex_local", disabled: false },
      { type: "geetorus_runner", disabled: false },
    ]);
    await previewMixedAdapterPackage();

    for (const select of findAdapterSelects()) {
      expect(Array.from(select.options).map((option) => option.value))
        .toContain("geetorus_runner");
    }
  });
