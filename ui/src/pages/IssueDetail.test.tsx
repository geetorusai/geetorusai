...
import { ONBOARDING_FIRST_TASK_ORIGIN_KIND } from "@geetorusai/shared";
import type {
...
describe("IssueDetail", () => {
  ...
  it("projects a native follow-up into the steering well before the post resolves", async () => {
    ...
    mockAgentsApi.list.mockResolvedValue([
      createAgent({ adapterType: "geetorus_runner" }),
    ]);
    mockHeartbeatsApi.activeRunForIssue.mockResolvedValue(activeRun);
    mockHeartbeatsApi.liveRunsForIssue.mockResolvedValue([activeRun]);
    mockIssuesApi.getQueuedComments.mockResolvedValue(
  ...
  it("forwards composer work mode changes to the issues API", async () => {
    ...
    localStorage.setItem(
      "geetorus:issue-comment-draft:issue-1",
    ...
    expect(localStorage.getItem("geetorus:issue-comment-draft:issue-1")).toBe(
      "Draft follow-up message",
    );
    localStorage.removeItem("geetorus:issue-comment-draft:issue-1");
  });
  ...
  it("keeps the authoritative Geetorus queue mounted after handoff promotion", async () => {
    mockIssuesApi.get.mockResolvedValue(
      createIssue({
        status: "in_progress",
        assigneeAgentId: "agent-1",
  ...
  it("keeps the last queued message mounted until steering is acknowledged so rejection stays visible", async () => {
    ...
    mockAgentsApi.list.mockResolvedValue([createAgent({ adapterType: "geetorus_runner" })]);
    ...
    mockHeartbeatsApi.activeRunForIssue.mockResolvedValue({
      ...
      agentId: "agent-1", agentName: "Runner", adapterType: "geetorus_runner", issueId: "issue-1",
    });
