      ...
      <div className="flex max-w-lg items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {failed
              ? error ?? "Geetorus couldn’t prepare the provider sign-in. Try again."
              : phase === "reauthenticating"
                ? "Your Geetorus sign-in is being refreshed."
                : "Geetorus is opening the provider securely."}
          </p>
          {failed ? (
            <div className="mt-6 flex items-center gap-2">
              <Button type="button" onClick={onRetry}>Try again</Button>
              <Button type="button" variant="ghost" onClick={onCancel}>Return to Geetorus</Button>
            </div>
          ) : null}
        </div>
      </div>
...
/** Fixed tenant landing used only after Geetorus Cloud refreshes login. */
export function GeetorusCloudOAuthHandoffPage() {
  ...
  const resume = useCallback(async () => {
    ...
    if (!handoff) {
      setPhase("error");
      setError("This sign-in expired. Return to Geetorus and start the connection again.");
      return;
    }
    setPhase("loading");
    setError(null);
    try {
      ...
      if (target.kind === "reauthentication") {
        setPhase("error");
        setError("Geetorus couldn’t refresh this sign-in. Try again to continue.");
        return;
      }
      clearPendingCloudHandoff();
      ...
      setPhase("error");
      setError(caught instanceof Error ? caught.message : "Geetorus couldn’t prepare secure sign-in.");
    }
  }, []);
