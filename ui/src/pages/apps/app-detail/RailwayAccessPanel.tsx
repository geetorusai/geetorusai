...
import type { ConfigureRailwaySsh, ConnectionGrantsResponse, RailwaySshSetup, ToolConnection } from "@geetorusai/shared";
...
export function RailwayAccessPanel({ connection, grants }: { connection: ToolConnection; grants?: ConnectionGrantsResponse }) {
  ...
  return <section className="space-y-4" aria-labelledby={`${id}-title`}>
    ...
    <div className="space-y-2">
      ...
      <p className="text-sm text-muted-foreground">To allow Geetorus direct SSH access to Railway containers, you can optionally generate an SSH key pair. <a className="underline" href="https://docs.railway.com/cli/ssh" target="_blank" rel="noreferrer">Railway SSH documentation</a></p>
    </div>
    ...
    {(canConfigure || canRemove) && grantId && <>
      ...
      {setup && <>
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Register this public key in the Railway account used by this authorization. The private key stays in Geetorus’s vault. <a className="underline" href="https://docs.railway.com/cli/ssh#manage-ssh-keys" target="_blank" rel="noreferrer">Railway key setup</a></p>
        </div>
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Paste the ssh.railway.com line from a known_hosts entry you have independently verified. Geetorus refuses untrusted or changed host keys.</p>
        </div>
        ...
        <p className="text-sm text-muted-foreground">{setup.enabled ? "Container access is enabled for this authorization." : "Register the public key and verify the host key before enabling access."} Removing the key stops new Geetorus connections. Also remove its public key from Railway.</p>
      </>}
    </>}
