import { checkMcpRemoteHeaderName, checkMcpRemoteHeaderValue, mcpRemoteHeaderRejectionMessage } from "@geetorusai/shared";
import type { GenericMcpAuthMode } from "@geetorusai/shared";
...
export function genericConnectGuidance(
  ...
  const fallback = message?.trim() || "Geetorus couldn't connect to that address. Check it and try again.";
  switch (code) {
    ...
    case "remote_http_private_endpoint":
      return {
        ...
        body: "This Geetorus is reachable from the internet, so it won't call addresses on your local network. Use the server's public address instead.",
        focus: "url",
      };
    ...
    case "mcp_header_rejected":
      return {
        title: "Geetorus can't send that header",
        body: fallback,
        focus: "credentials",
      };
    case "tool_access_name_conflict":
      return {
        title: "Geetorus couldn’t name this connection",
    ...
    case "oauth_challenge":
      return {
        ...
        body: "It asked us to authenticate but didn't offer a sign-in Geetorus can complete on its own. Add the key or headers its docs list under Advanced authentication.",
        focus: "credentials",
      };
    ...
    case "oauth_manual_client_rebinding_required":
      return {
        ...
        body: "Register Geetorus in the provider's settings, then add the client ID and secret it gives you under Advanced authentication.",
        focus: "credentials",
      };
    case "oauth_redirect_origin_unsupported":
    case "oauth_redirect_uri_invalid":
      return {
        title: "This Geetorus needs a public HTTPS address first",
        body: "Sign-in sends the operator back to Geetorus, so this instance has to be reachable over HTTPS. Ask your Geetorus admin to configure it.",
        focus: "deployment",
      };
    case "runtime_error":
      return {
    ...
    default:
      return {
        title: "Geetorus couldn't connect",
        body: fallback,
        focus: "none",
      };
  }
 ...
 * into Geetorus secrets — nothing here ever puts a value in the config.
 */
