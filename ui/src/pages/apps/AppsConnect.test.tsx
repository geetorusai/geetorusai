...
import { CONNECTABLE_APP_DEFINITIONS, GOOGLE_WORKSPACE_CONNECTOR_PROFILES, getAppStoreDefinition } from "@geetorusai/shared";
...
const mockCompany = vi.hoisted(() => ({
  value: {
    ...
    selectedCompany: { id: "company-1", name: "Geetorus" } as { id: string; name: string } | null,
  },
}));
...
describe("AppsConnect — Connect with a link (M4 frame)", () => {
  ...
  beforeEach(() => {
    ...
    mockCompany.value = {
      ...
      selectedCompany: { id: "company-1", name: "Geetorus" },
    };
    mockSearch.value = "";
    ...
    getCloudConnectorEnrollmentMock.mockResolvedValue({
      ...
      environment: "staging",
      origins: ["https://geetorus.example.test"],
    });
    startCloudConnectorEnrollmentMock.mockResolvedValue({
      ...
      environment: "staging",
      origins: [],
      verificationUrl: "https://my-staging.geetorus.app/connections/enroll?id=enroll-test",
    });
  ...
  it.each(["config-url", "transport-url", "transport-serverUrl"] as const)("reloads a generic task reconnect endpoint from %s with selection-only card metadata", async (source) => {
    ...
    listConnectionsMock.mockResolvedValue({ connections: [{
      ...choice, companyId: "company-1", transport: "mcp_remote", authKind: "none", credentialPolicy: "shared", credentialSource: "geetorus_vault",
  ...
  it("preserves an edited task reconnect endpoint after refreshing connection data", async () => {
    ...
    listApplicationsMock.mockResolvedValue({ applications: [{ id: choice.applicationId, name: "Archive", applicationKey: "archive", type: "mcp_http" }] });
    listConnectionsMock.mockResolvedValue({ connections: [{ ...choice, companyId: "company-1", transport: "mcp_remote", authKind: "none", credentialPolicy: "shared", credentialSource: "geetorus_vault", config: { url: "https://archive.example.test/mcp" } }] });
    ...
    listConnectionsMock.mockResolvedValue({ connections: [{ ...choice, companyId: "company-1", transport: "mcp_remote", authKind: "none", credentialPolicy: "shared", credentialSource: "geetorus_vault", config: { url: "https://refreshed.example.test/mcp" } }] });
  ...
  it("shows Gmail's instance configuration notice instead of a dead connect path", async () => {
    const reason = "Configure Geetorus ID before connecting Gmail.";
    listGalleryMock.mockResolvedValue({
  ...
  it("collects customer-owned OAuth client details for a curated manual OAuth app", async () => {
    ...
    expect(container.textContent).toContain("Geetorus callback URL");
  ...
  it("prompts for one-time Cloud registration inside Gmail when managed methods are not yet advertised", async () => {
    ...
    expect(container.textContent).toContain("Connect with Geetorus");
    expect(container.textContent).toContain(
      "You must connect this instance to Geetorus to connect to Gmail (you only need to do this once).",
    );
    expect(buttonByText("Connect with Geetorus")?.closest(".rounded-xl")?.classList.contains("border-border")).toBe(true);
    ...
    await act(async () => {
      buttonByText("Connect with Geetorus")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
    await flushReact();
    ...
    expect(navigateTopLevelMock).toHaveBeenCalledWith(
      "https://my-staging.geetorus.app/connections/enroll?id=enroll-test",
    );
  });
  ...
  it.each([false, true])("retains task access and interaction through enrollment (popup blocked: %s)", async (popupBlocked) => {
    ...
    await passAccessStep();
    await act(async () => buttonByText("Connect with Geetorus")?.click());
    await flushReact();
    expect(open).toHaveBeenCalled();
    if (popupBlocked) {
      expect(popup.location.assign).not.toHaveBeenCalled();
    } else {
      expect(popup.location.assign).toHaveBeenCalledWith("https://my-staging.geetorus.app/connections/enroll?id=enroll-test");
    }
    ...
    expect(fallback?.href).toBe("https://my-staging.geetorus.app/connections/enroll?id=enroll-test");
    expect(navigateTopLevelMock).not.toHaveBeenCalled();
    expect(startCloudConnectorEnrollmentMock).toHaveBeenCalledWith("company-1", "Geetorus", "/apps/connect?source=gmail&stage=setup&intent=intent-1&enrollment_host=dialog");
    ...
    expect(container.textContent).toContain("What should Geetorus be able to do?");
  ...
  it.each(["2020-01-01T00:00:00.000Z", "2099-01-01T00:00:00.000Z"])(
    ...
    async (expiresAt) => {
      ...
      getCloudConnectorEnrollmentMock.mockResolvedValue({
        ...
        environment: "staging",
        origins: [],
        verificationUrl: "https://my-staging.geetorus.app/connections/enroll?id=cached",
        expiresAt,
      });
      ...
      expect(startCloudConnectorEnrollmentMock).toHaveBeenCalledWith(
        "company-1", "Geetorus", "/apps/connect?source=github&stage=setup",
      );
      expect(navigateTopLevelMock).toHaveBeenCalledWith(
        "https://my-staging.geetorus.app/connections/enroll?id=enroll-test",
      );
      expect(navigateTopLevelMock).not.toHaveBeenCalledWith(
        "https://my-staging.geetorus.app/connections/enroll?id=cached",
      );
    },
  );
  ...
  it("restores the setup step after the one-time enrollment callback", async () => {
    ...
    expect(container.textContent).not.toContain("Connect with Geetorus");
  });
  ...
  it("keeps GitHub sign-in intent when enrollment recovery reveals an unavailable profile", async () => {
    ...
    expect(container.textContent).toContain("Geetorus couldn’t check Cloud registration. Try again.");
    expect(container.textContent).not.toContain("Your GitHub key");
  ...
  it("preserves a dedicated agent identity across the full-page enrollment callback", async () => {
    ...
    expect(JSON.parse(window.sessionStorage.getItem(
      "geetorus.connector-enrollment-access:github",
    ) ?? "null")).toEqual({
      companyId: "company-1",
      grantKind: "agent",
    ...
    expect(window.sessionStorage.getItem("geetorus.connector-enrollment-access:github")).toBeNull();
    mockCompany.value = {
      selectedCompanyId: "company-1",
      selectedCompany: { id: "company-1", name: "Geetorus" },
    };
  ...
  it.each([...new Set(Object.values(GOOGLE_WORKSPACE_CONNECTOR_PROFILES).map((profile) => profile.appSlug))]
    ...
    async ({ slug, enrollmentReturn }) => {
      ...
      const readMethod = definition.methods.find((method) => method.key === "geetorus-read")!;
      mockSearch.value = enrollmentReturn
        ? `source=${slug}&stage=setup&cloud_connector=enrolled`
        : `source=${slug}`;
      if (enrollmentReturn) {
        window.sessionStorage.setItem(`geetorus.connector-enrollment-access:${slug}`, JSON.stringify({
      ...
      expect(buttonByText("Continue to sign in")?.disabled).toBe(true);
      const managedAuth = buttonByText("Use Geetorus instead");
      ...
      await flushReact();
      expect(connectAppMock).toHaveBeenCalledWith("company-1", expect.objectContaining({
        galleryKey: slug, connectionMethodKey: "geetorus-read", grantKind: "user",
      }));
  });
  ...
  it("never renders self-host enrollment when the connector identity is already active", async () => {
    ...
    expect(container.textContent).not.toContain("You must connect this instance to Geetorus");
    expect(container.textContent).not.toContain("Connect with Geetorus");
  });
  ...
  it("keeps Google Drive prerequisites off access and defaults to its write-capable method", async () => {
    ...
    expect(container.textContent).not.toContain("does not enable unrelated Geetorus customers");
  ...
  it("renders Google Calendar as one minimal, unboxed setup screen", async () => {
    ...
    const capabilityQuestion = Array.from(container.querySelectorAll("label")).find(
      (label) => label.textContent === "What should Geetorus be able to do?",
    );
  ...
  it("connects PostHog without a project ID and keeps optional controls advanced", async () => {
    ...
    expect(connectAppMock).toHaveBeenCalledWith("company-1", {
      ...
      credentialSource: "geetorus_vault",
  ...
  it("hands reviewed PostHog credentials to Vercel and submits only the connector reference", async () => {
    ...
    await act(async () => {
      setInputValue(connectorInput!, "posthog/geetorus");
    });
    await flushReact();
  ...
  it("shows an in-flight state while Geetorus prepares Notion sign-in", async () => {
    mockSearch.value = "source=notion";
    listGalleryMock.mockResolvedValueOnce({ apps: [NOTION] });
  ...
  it("shows installation recovery for GitHub even when an advanced PAT method is available", async () => {
    ...
    mockSearch.value = `source=github&resume=${connectionId}&oauth=failed&code=github_installation_required&installation_url=https%3A%2F%2Fgithub.com%2Fapps%2Fgeetorus-for-github%2Finstallations%2Fnew`;
    listGalleryMock.mockResolvedValue({ apps: [GITHUB_MANAGED] });
    ...
    await render();
    await flushReact();
    expect(container.textContent).toContain("Install Geetorus and grant at least one repository");
    expect(container.querySelector('a[href="https://github.com/apps/geetorus-for-github/installations/new"]')?.textContent).toBe("Install Geetorus on GitHub");
    expect(container.textContent).not.toContain("Your GitHub key");
    await act(async () => buttonByText("Try again")!.click());
    await flushReact();
  ...
  it.each(["connection", "application"] as const)(
    ...
    async (failedLookup) => {
      ...
      expect(container.textContent).not.toContain("Couldn’t load connection setup");
      expect(container.textContent).toContain("Connect Notion to Geetorus");
    },
  );
  ...
  it("shows the Google Sheets robot email and keeps empty sheet links from continuing", async () => {
    listGalleryMock.mockResolvedValueOnce({
      apps: [
        { ...GOOGLE_SHEETS, availability: { available: true, robotEmail: "robot@geetorus.iam.gserviceaccount.com" } },
      ],
    });
    await render();
    ...
    expect(container.textContent).toContain("robot@geetorus.iam.gserviceaccount.com");
    expect(container.textContent).toContain(
...
describe("AppsConnect — guided generic MCP flow (PAP-17087)", () => {
  ...
  it("offers a curated setup as a convenience without leaving the generic flow", async () => {
    ...
    expect(container.textContent).toContain("Geetorus has a guided setup for Zapier.");
    expect(container.textContent).toContain("Connect your own MCP server");
  ...
  it("uses deployment guidance without rendering the server env-var message", async () => {
    connectAppMock.mockRejectedValue(apiError(
      ...
      "OAuth connections require GEETORUS_PUBLIC_URL or an auth public base URL",
    ));
    await render();
    await gotoLinkFrame(container, "https://mcp.example.test/mcp");
    await act(async () => {
    ...
    expect(container.textContent).toContain("This Geetorus needs a public HTTPS address first");
    expect(container.textContent).not.toContain("GEETORUS_PUBLIC_URL");
  });
  ...
  it("does not ask the operator to resolve an internal name conflict", async () => {
    ...
    expect(container.textContent).toContain("Geetorus couldn’t name this connection");
  ...
  it("exchanges a managed connect response in the tenant without opening Cloud confirmation", async () => {
    ...
    connectAppMock.mockResolvedValue({
      ...
      auth: {
        kind: "oauth",
        startUrl: "https://my.geetorus.app/connections/confirm?session=legacy",
        handoff: { kind: "geetorus_cloud", session },
      },
    });
    await render();
    await gotoLinkFrame(container, "https://mcp.example.test/mcp");
  ...
  it("blocks a header Geetorus refuses to send before making a request", async () => {
    await render();
    await gotoLinkFrame(container, "https://mcp.example.test/mcp");
    await openAdvanced();
    await act(async () => {
    ...
    expect(container.textContent).toContain('Geetorus manages the "Host" header');
