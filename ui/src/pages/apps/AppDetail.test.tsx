...
import { getAppStoreDefinition } from "@geetorusai/shared";
  ...
  return organizationGrant({
    ...
    providerTenant: {
      github: {
        ...
        installationIds: ["456"],
        installationOwnerLogins: ["geetorusai"],
        repositories: [{ id: "789", fullName: "geetorusai/test-repo", installationId: "456" }],
        installationUrl: "https://github.com/apps/geetorus-test/installations/new",
...
describe("AppDetail", () => {
  ...
  it("keeps personal managed authorization in the tenant until the provider is ready", async () => {
    ...
    startPersonalAuthorizationMock.mockResolvedValue({
      url: "https://my.geetorus.app/connections/confirm?session=legacy",
  ...
  it("shows the personal GitHub username and every accessible repository", async () => {
    ...
    listConnectionGrantsMock.mockResolvedValue({
      ...
      grants: [dedicatedGitHubGrant({ kind: "user", subjectAgentId: null, subjectUserId: "user-1" }, {
        repositoryCount: 2,
        repositories: [
          { id: "1", fullName: "geetorusai/first", installationId: "456", private: true },
          { id: "2", fullName: "geetorusai/second", installationId: "456", private: false },
        ],
      })],
    ...
    expect(container.textContent).toContain("geetorusai/first");
    expect(container.textContent).toContain("geetorusai/second");
    expect(container.querySelector('a[href="https://github.com/geetorusai/first"] [aria-label="Private repository"]')).toBeTruthy();
    expect(container.querySelector('a[href="https://github.com/geetorusai/second"] [aria-label="Private repository"]')).toBeNull();
    const configureHint = [...container.querySelectorAll("p a")].find((link) => link.textContent === "Configure access on GitHub");
    expect(configureHint?.getAttribute("href")).toBe("https://github.com/apps/geetorus-test/installations/new");
  });
  ...
  it.each([undefined, "https://github.com/settings/installations"])("loads missing GitHub app configuration instead of linking legacy settings (%s)", async (installationUrl) => {
    ...
    listConnectionGrantsMock.mockResolvedValue({
      connection: { id: "conn-1", uid: "conn-1" },
      grants: [dedicatedGitHubGrant({ kind: "user", subjectAgentId: null, subjectUserId: "user-1" }, {
        appSlug: "geetorus-staging", installationUrl: "https://github.com/apps/geetorus-staging/installations/new",
      })],
      capabilities: fullCapabilities(), currentUserId: "user-1", members: [],
    });
    ...
    expect(container.querySelector('a[href="https://github.com/apps/geetorus-staging/installations/new"]')?.textContent).toBe("Add More Repos on GitHub");
  ...
  it.each([false, true])("shows repositories across accounts without filter controls (empty: %s)", async (empty) => {
    ...
    listConnectionGrantsMock.mockResolvedValue({
      ...
      grants: [dedicatedGitHubGrant({ kind: "user", subjectAgentId: null, subjectUserId: "user-1" }, {
        ...
        installationOwnerLogins: ["geetorusai", "dottabot", "empty-org"],
        repositories: empty ? [] : [
          { id: "1", fullName: "geetorusai/first", installationId: "456" },
          { id: "2", fullName: "geetorusai/second", installationId: "456" },
    ...
    expect(repositoryNames()).toEqual(empty ? [] : ["geetorusai/first", "geetorusai/second", "dottabot/first"]);
    if (empty) {
    ...
    expect(container.querySelector('a[href="https://github.com/apps/geetorus-test/installations/new"]')?.textContent).toBe("Add More Repos on GitHub");
  ...
  it("shows dedicated GitHub access as compact action rows and links to the agent", async () => {
    ...
    expect(container.querySelector('a[href="https://github.com/geetorusai/test-repo"]')?.textContent).toBe("geetorusai/test-repo");
    expect(container.querySelector(
      'a[href="https://github.com/apps/geetorus-test/installations/new"]',
