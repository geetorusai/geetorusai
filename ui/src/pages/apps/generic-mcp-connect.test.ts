...
describe("oauthCallbackUrlForBrowser", () => {
  ...
  it("preserves public HTTPS origins", () => {
    expect(oauthCallbackUrlForBrowser("https://geetorus.example.test")).toBe(
      "https://geetorus.example.test/api/tools/oauth/callback",
    );
  });
});
...
describe("genericConnectGuidance", () => {
  ...
  it("points at the deployment when Geetorus itself has no public HTTPS address", () => {
  ...
  it("does not ask the operator to resolve an internal name conflict", () => {
    ...
    expect(guidance).toMatchObject({ title: "Geetorus couldn’t name this connection", focus: "none" });
  ...
  it("passes a rejected header's own message through", () => {
    const guidance = genericConnectGuidance(
      "mcp_header_rejected",
      'Geetorus manages the "Host" header and cannot send a custom value for it.',
    );
...
describe("customHeaderError", () => {
  ...
  it("rejects headers Geetorus refuses to send", () => {
    expect(customHeaderError([{ id: "a", name: "Host", value: "evil.example" }]))
      .toContain('Geetorus manages the "Host" header');
  });
