...
import { GeetorusCloudOAuthHandoffPage } from "./GeetorusCloudOAuthHandoff";
...
describe("GeetorusCloudOAuthHandoffPage", () => {
  it("keeps a tenant loading state visible until the provider URL is ready", async () => {
    ...
    await act(async () => root.render(<GeetorusCloudOAuthHandoffPage />));
  ...
  it("keeps terminal handoff failures in Geetorus instead of opening confirmation", async () => {
    savePendingCloudHandoff(SESSION);
  ...
  it("does not loop when recent-login recovery remains stale", async () => {
    ...
    expect(container.textContent).toContain("Geetorus couldn’t refresh this sign-in. Try again to continue.");
    expect(navigateTopLevel).not.toHaveBeenCalled();
  });
});
