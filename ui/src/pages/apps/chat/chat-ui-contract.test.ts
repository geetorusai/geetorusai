...
describe("chat connector UI contract", () => {
  it("describes the close command as a conversation control rather than a task status change", () => {
    ...
    expect(setup).not.toContain("Close the active Geetorus task");
  });
  ...
  it("keeps provider capabilities automatic and settings focused on plausible reach", () => {
    ...
    expect(detail).toContain("otherwise Geetorus safely refuses the request");
    expect(setup).toContain("Link the account you’re testing");
    expect(setup).toContain("Geetorus does not replay the refused request");
  ...
  it("states the provider boundary for reconnect and removal", () => {
    ...
    for (const reconnectCopy of [
      ...
      "automatically refreshes its Geetorus webhook and command menu",
    ]) {
    ...
    for (const removalCopy of [
      ...
      "After Telegram confirms that cleanup, Geetorus retires the saved token",
  ...
  it("uses only executable provider credential flows", () => {
    ...
    expect(setup).toContain('resource: "https://geetorus.ing"');
    ...
    expect(setup).not.toContain("api://geetorus-chat/");
    ...
    expect(setup).toContain("Geetorus records Interactivity");
    ...
    expect(setup).not.toContain("@geetorusai/teams-connect");
