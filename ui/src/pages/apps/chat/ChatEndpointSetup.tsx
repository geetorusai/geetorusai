...
import { isAgentStatusInvokable } from "@geetorusai/shared";
...
function slackBotNameForAgent(agentName: string): string {
  const safeName = agentName
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 24);
  return safeName || "geetorus-agent";
}
...
function ProviderConnectStep({
  ...
  const loadPrivateKeyFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPrivateKeyFileError(
        error instanceof Error
          ? error.message
          : "Geetorus couldn't read that file. Choose the .pem file again or paste the private key.",
      );
    } finally {
  ...
  const slackAppName = `${slackBotName.slice(0, 25)}-geetorus`;
  const slackWebhookUrl =
    endpoint.setup?.webhookUrl ?? "<geetorus-webhook-url>";
...
features:
  ...
  agent_view:
    agent_description: "Work with a Geetorus agent in a task-backed conversation."
  bot_user:
          ...
          commandLists: [
            {
              ...
              commands: [
                {
                  title: "/status",
                  description: "Show the active Geetorus task status",
                },
                {
                  title: "/new",
                  description: "Start a new Geetorus task in this chat",
                },
                {
                  title: "/close",
    ...
    return (
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {repairing
              ...
              : "Create one dedicated Discord application and bot for this Geetorus agent."}
          </p>
        </div>
      <ol className="list-decimal space-y-2 pl-5 text-sm">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          Reactions, Embed Links, and Attach Files. Geetorus still requires
    ...
    return (
      <div className="space-y-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {repairing
              ? "Reconnect verifies this same BotFather bot and automatically refreshes its Geetorus webhook and command menu. It does not recreate the bot or change its chat memberships. Leave the token blank to reuse the saved credential."
        ...
        <p className="rounded-md border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
          Geetorus works with Telegram&apos;s default bot privacy mode and
        ...
        {!endpoint.setup?.webhookUrl && (
          <p className="text-sm text-destructive">
            Configure a public HTTPS URL for this Geetorus instance before
            connecting Telegram.
          </p>
        )}
        <Button
      ...
      <div className="space-y-5">
        ...
        <ol className="list-decimal space-y-2 pl-5 text-sm">
          ...
          <li>
            ...
            Application ID, set its messaging endpoint to the Geetorus URL
        ...
        <section className="space-y-3 rounded-lg border border-border p-4">
          ...
          <ol className="list-decimal space-y-3 pl-5 text-sm">
            ...
            <li>
              ...
              <strong>Settings · Configuration</strong> and paste the Geetorus{" "}
        ...
        <p className="text-sm text-muted-foreground">
          ...
          Geetorus-specific fields to verify in Developer Portal or merge into
        ...
        <p className="text-sm text-muted-foreground">
          Geetorus does not use Teams single sign-on in this release. The
        ...
        <p className="text-sm text-muted-foreground">
          ...
          Geetorus retains and acts only on messages admitted by your Geetorus
  ...
  if (provider === "github")
    return (
      <div className="space-y-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {repairing
              ...
              : "Configure its webhook and permissions, then verify the App with Geetorus."}
          </p>
        </div>
        {!repairing && (
          <ol className="list-decimal space-y-2 pl-5 text-sm">
            <li>
              ...
              Geetorus homepage URL below, and leave user authorization off.
            </li>
            <li>
              Keep <strong>Webhooks · Active</strong> on. Enter the Geetorus
              webhook URL and the Geetorus-generated webhook secret below, and
        ...
        {endpointValue(
          "Geetorus homepage URL",
          publicOrigin(endpoint.setup?.webhookUrl),
        )}
        {endpointValue("Geetorus webhook URL", endpoint.setup?.webhookUrl)}
        <Button
          variant="outline"
          onClick={() =>
            window.open(
        ...
        <div className="grid gap-2">
          ...
          {generatedWebhookSecret ? (
            <>
              ...
              <p className="text-sm text-muted-foreground">
                Copy this value now. Geetorus will not show it again.
              </p>
            </>
          ) : (
            <p className="text-sm text-muted-foreground">
              {endpoint.setup?.webhookSecretConfigured
                ...
                : "Generate the secret in Geetorus, then paste it into the GitHub App."}
            </p>
          )}
          <div>
            <Button
    ...
    return (
      <div className="space-y-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Point the Slack app at Geetorus now that its signing secret is
            connected.
          </p>
        </div>
        {endpointValue("Geetorus webhook URL", endpoint.setup?.webhookUrl)}
        ...
        <ol className="list-decimal space-y-2 pl-5 text-sm">
          <li>
            ...
            Events URL when you save; Geetorus records Interactivity and slash
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {repairing
            ...
            : "Bring your own Slack app. The manifest requests the scopes Geetorus needs; credentials remain write-only."}
        </p>
      </div>
      <ol className="list-decimal space-y-2 pl-5 text-sm">
        <li>
...
function TryStep({
        ...
        : unlinkedIdentities.length > 0
          ? guestIsolationState === "disabled"
            ? {
                tone: "warning" as const,
                title: "Link the account you’re testing",
                body: `An observed external account is unlinked, and isolated guest work is off, so it cannot safely start ${agentName}. Link the account in Access, then ${freshConversationInstruction}; Geetorus does not replay the refused request.`,
              }
            : {
                tone: "info" as const,
  ...
  const instructions =
    provider === "imessage-photon" ? [
      ...
      "Link the discovered sender to a Geetorus person in Access, then send a fresh request.",
