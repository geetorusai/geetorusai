      ...
      <main className="mx-auto max-w-lg space-y-5 px-6 py-12">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            Geetorus permissions in {identity.companyName}.
          </p>
        </div>
        <Button asChild>
          <Link
  ...
  return (
    <main className="mx-auto max-w-lg space-y-6 px-6 py-12">
      ...
      <dl className="divide-y divide-border border-y border-border">
        ...
        <div className="flex items-center justify-between gap-4 py-3">
          <dt className="text-sm text-muted-foreground">Geetorus account</dt>
          <dd className="text-right text-sm font-medium">{geetorusAccount}</dd>
        </div>
        <div className="flex items-center justify-between gap-4 py-3">
          <dt className="text-sm text-muted-foreground">Agent</dt>
          <dd className="text-sm font-medium">
            {identity.botLabel ?? "Geetorus agent"}
          </dd>
        </div>
      </dl>
      <p className="text-sm text-muted-foreground">
        Confirm only if this is your {providerNames[identity.provider]}{" "}
        identity. Geetorus will check your current organization membership on
        every action.
      </p>
      {confirm.isError && (
