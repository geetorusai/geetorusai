...
import type { ConnectToolAppResult, McpJsonImportPreview } from "@geetorusai/shared";
...
describe("PasteConfigTab — activation handoff (PAP-11092)", () => {
        ...
        drafts: [
          {
            ...
            warnings: ["Header Authorization will be stored as a Geetorus secret before activation."],
          },
        ],
      },
  ...
  it("uses the background Cloud handoff returned with an imported OAuth connection", async () => {
    ...
    await pasteAndCheck(NOTION_PREVIEW, NOTION_CONFIG);
    const result = oauthConnectResult("https://my.geetorus.app/connections/confirm?session=legacy");
    result.auth = {
      ...result.auth!,
        ...
        drafts: [
          {
            ...
            credentialFields: [],
            warnings: ["Imported stdio commands stay draft-only unless mapped to an approved Geetorus template."],
          },
        ],
      },
