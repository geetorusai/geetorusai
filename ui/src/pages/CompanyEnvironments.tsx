...
function environmentDeleteBlockMessage(impact: EnvironmentDeleteBlastRadius): string | null {
  ...
  if (impact.reusableSandboxLeaseCount > 0) {
    return "Cannot delete this environment while it has a reusable sandbox lease. Remove the associated execution workspace or issue so Geetorus can destroy the sandbox, then retry.";
  }
  return null;
}
          ...
          <div className="min-w-0 space-y-1">
            ...
            <div className="text-xs text-muted-foreground">
              ...
              <span
                ...
                title={templateRef
                  ? `Provider ${activeTemplate.templateKind} ref ${templateRef} (Geetorus template ${activeTemplate.id})`
                  : activeTemplate.id}
              >
                    ...
                    {environment.driver === "ssh" ? (
                      ...
                      <div className="text-xs text-muted-foreground">
                        {(() => {
                          ...
                          // Geetorus"; repeating provider vocabulary like
                      ...
                      <div className="text-xs text-muted-foreground">Runs on this Geetorus host.</div>
                    )}
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
        ...
        <div data-testid="managed-environment-form-page">
          <div className="pb-4">
            ...
            <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
              {editingEnvironment.description ?? "Your agent runs on a computer managed by Geetorus."}
            </p>
            <p className="mt-1 max-w-3xl text-xs text-muted-foreground">
              This environment is provisioned and maintained for you. You can add environment
              variables for your agents; its name and configuration are managed by Geetorus.
            </p>
          </div>
          <div className="py-4">
            <Field
      ...
      (mode === "create" || (editingEnvironment && !isPlatformManagedEnvironment(editingEnvironment))) ? (
        ...
        <div data-testid="environment-form-page">
          <div className="pb-4">
            ...
            <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
              Configure a reusable execution target for your agents. Saved changes affect future runs; Geetorus may start fresh sessions or sandbox leases after environment config changes.
            </p>
          </div>
              ...
              {environmentForm.driver === "ssh" ? (
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  {/*
                    ...
                    Geetorus execution host, so it stays visible under the
                  ...
                  */}
                  <Field label="Remote workspace path" hint="Absolute path that Geetorus will verify during SSH connection tests.">
                    <input
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                      type="text"
                      placeholder="/Users/geetorus/workspace"
                  ...
                  <div className="space-y-1.5" data-testid="environment-delete-lease-holders">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {reusableLeaseOnlyBlock
                        ...
                        : "Close these workspaces to let Geetorus destroy their sandboxes, then retry the delete."}
                    </div>
                  </div>
                ) : null}
