...
describe("CompanyEnvironments — test provider button", () => {
  ...
  it("explains that successful sandbox provider tests use a temporary sandbox", async () => {
    ...
    mockEnvironmentsApi.probe.mockResolvedValue({
      ok: true,
      driver: "sandbox",
      summary: "Connected to Daytona sandbox geetorus-probe.",
      details: {
        provider: "daytona",
        diagnostics: [],
        metadata: {
    ...
    expect(mockEnvironmentsApi.probe).toHaveBeenCalledExactlyOnceWith("env-1", "company-1");
    expect(container.textContent).toContain("Connected to Daytona sandbox geetorus-probe.");
    ...
    await waitForAssertion(() => {
      ...
      expect(
        dialog?.querySelector(
          "[title='Provider snapshot ref redacted-template-ref (Geetorus template 12345678-90ab-cdef-1234-567890abcdef)']",
        ),
      ).toBeTruthy();
