...
/**
 * Filter .geetorus.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterGeetorusYaml(yaml: string, checkedFiles: Set<string>): string {
 ...
 * content (regenerated README.md, filtered .geetorus.yaml) when present.
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://geetorus.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("npx geetorusai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Geetorus](https://geetorus.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Geetorus](https://geetorus.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .geetorus.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .geetorus.yaml
    const yamlPath = exportData.geetorusExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterGeetorusYaml(exportData.files[yamlPath], checkedFiles);
    }
