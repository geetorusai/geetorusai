...
import { redactCommandText as redactCommandSecretText } from "@geetorusai/adapter-utils";
...
import { redactHomePathUserSegments, redactHomePathUserSegmentsInValue } from "@geetorusai/adapter-utils";
...
const COMMAND_ENV_KEY_RE = /(^command$|^cmd$|command[-_]?line|resolved[-_]?command|GEETORUS_RESOLVED_COMMAND)/i;
  ...
  return (
    <div className={cn("agent-settings-content mx-auto max-w-5xl space-y-8", `agent-settings-${activeView}`)}>
      ...
      {builtInState && (
        ...
        >
          Ships with Geetorus and powers <strong>{builtInFeatureLabel}</strong>. Configure it like
...
export function ConfigurationTab({
  ...
  const catalogProvider = agent.adapterType === "geetorus_runner" ? String(agent.adapterConfig.provider ?? "codex") : undefined;
                ...
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="right" sideOffset={4}>
                      Managed: Geetorus stores and serves the instructions bundle. External: you provide a path on disk where the instructions live.
                    </TooltipContent>
                  </Tooltip>
                </span>
                <div className="flex gap-2">
              ...
              <label className="space-y-1.5 min-w-0">
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="right" sideOffset={4}>
                      The absolute directory on disk where the instructions bundle lives. In managed mode this is set by Geetorus automatically.
                    </TooltipContent>
                  </Tooltip>
                </span>
                {currentMode === "managed" ? (
...
function RunDetail({ run: initialRun, agentRouteId, adapterType, adapterConfig }: { run: HeartbeatRun; agentRouteId: string; adapterType: string; adapterConfig: Record<string, unknown> }) {
  ...
  const geetorusDeveloperMode =
    experimentalSettings?.enableGeetorusDeveloperMode === true;
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 p-4 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <HoneycombRunLink
                runId={run.id}
                enabled={geetorusDeveloperMode && canUseProviderTrace}
              />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          API keys allow this agent to authenticate calls to the Geetorus server.
        </p>
        <div className="flex items-center gap-2">
          <Input
