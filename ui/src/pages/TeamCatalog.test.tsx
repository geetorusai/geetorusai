...
const mockAdapterAvailability = vi.hoisted(() => ({
  disabled: new Set<string>(["geetorus_runner"]),
  loaded: true,
}));
...
describe("TeamCatalog routes", () => {
  it("round-trips file paths containing literal tildes", () => {
    const route = teamRoute("geetorusai/bundled/test/team", "agents/a~b/AGENTS.md");

    expect(route).toBe("/teams-catalog/geetorusai%2Fbundled%2Ftest%2Fteam/files/agents%7Ea~b%7EAGENTS.md");
    expect(parseTeamRoute(route.replace("/teams-catalog/", ""))).toEqual({
      catalogRef: "geetorusai/bundled/test/team",
      filePath: "agents/a~b/AGENTS.md",
    });
  });

  it("fails closed for Geetorus Runner until adapter availability is loaded and enabled", () => {
    expect(listTeamInstallAdapterTypes(new Set(), false)).not.toContain("geetorus_runner");
    ...
    expect(listTeamInstallAdapterTypes(new Set(["geetorus_runner"]), true))
      .not.toContain("geetorus_runner");
    expect(listTeamInstallAdapterTypes(new Set(), true)).toContain("geetorus_runner");
    expect(resolveTeamInstallAdapterType("geetorus_runner", ["claude_local", "codex_local"]))
      .toBe("claude_local");
    expect(resolveTeamInstallAdapterType("geetorus_runner", ["codex_local"]))
      .toBe("codex_local");
    expect(resolveTeamInstallAdapterType("geetorus_runner", []))
      .toBeNull();
  });
});
...
function makeTeam(overrides: Partial<CatalogTeam> = {}): CatalogTeam {
  return {
    id: "team-no-deps",
    key: "geetorusai/bundled/company-defaults/team-no-deps",
    kind: "bundled",
...
describe("TeamCatalog install preview path", () => {
  ...
  beforeEach(() => {
    mockAdapterAvailability.disabled = new Set(["geetorus_runner"]);
    ...
    mockTeamCatalogApi.installed.mockResolvedValue([
      {
        ...
        catalogKey: "geetorusai/bundled/company-defaults/team-no-deps",
        present: true,
