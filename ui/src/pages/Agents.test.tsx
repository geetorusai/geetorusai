...
import type { Agent, Environment, EnvironmentCapabilities } from "@geetorusai/shared";
...
import type { AgentOrgChainHealth } from "@geetorusai/shared";
...
function makeBuiltInAgentState(overrides: Partial<BuiltInAgentState> = {}): BuiltInAgentState {
  return {
    definition: {
      ...
      defaultInstructions: "You are Geetorus's built-in Briefs agent.",
...
describe("Agents", () => {
  ...
  it("gives mobile agent names the full row width after the leading status indicator", async () => {
    ...
    mockAgentsApi.list.mockResolvedValue([
      makeAgent({
        id: "agent-mobile",
        name: "Geetorus Engineer With A Much Longer Display Name",
        title: "Software Engineer With A Much Longer Specialty Title",
        urlKey: "geetorus-engineer-long",
      }),
    ]);
    ...
    const row = findAgentRow(container, "Geetorus Engineer With A Much Longer Display Name");
    expect(row).not.toBeNull();
    ...
    const name = row?.querySelector("span[title='Geetorus Engineer With A Much Longer Display Name']");
    ...
    const actions = row?.querySelector('button[aria-label="Open actions for Geetorus Engineer With A Much Longer Display Name"]');
  ...
  it("uses configured names for local-driver environments", async () => {
    ...
    expect(container.textContent).toContain("Geetorus host");
  });
  ...
  it("shows and filters built-in agents when the experimental flag is enabled", async () => {
    ...
    expect(container.textContent).toContain("Briefs Agent");
    expect(container.textContent).not.toContain("Regular Agent");
    expect(container.querySelector('[title="Ships with Geetorus"]')).toBeNull();
