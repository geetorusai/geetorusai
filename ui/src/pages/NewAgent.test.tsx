...
beforeEach(() => {
  ...
  state.adapters = [
    ...
    "opencode_local",
    "pi_local",
    "geetorus_runner", "cursor_cloud", "cursor", "gemini_local", "kimi_local", "grok_local", "hermes_local", "hermes_gateway",
...
describe("New agent setup", () => {
  ...
  it("sends Cursor Cloud repo/ref and transient API key, then saves an organization secret", async () => {
    ...
    await fill("GitHub repository", "https://github.com/geetorusai/geetorus");
    ...
    expect(api.testEnvironment.mock.calls[0][2]).toMatchObject({
      adapterConfig: { repoUrl: "https://github.com/geetorusai/geetorus", repoStartingRef: "master" },
    ...
    expect(config).toMatchObject({ repoUrl: "https://github.com/geetorusai/geetorus", repoStartingRef: "master", env: {
  ...
  it.each([
    ...
    ["geetorus_runner", "claude", "Claude", "ANTHROPIC_API_KEY"],
    ["geetorus_runner", "codex", "OpenAI", "OPENAI_API_KEY"],
  ...
  it.each(["codex", "claude", "opencode"])(
    ...
    async (runner) => {
      await render("geetorus_runner", runner);
      if (runner !== "opencode")
  ...
  it("blocks a disabled runner even when opened through a URL", async () => {
    state.adapters = [
      { type: "geetorus_runner", loaded: true, disabled: true },
    ];
    await render("geetorus_runner");
