...
import type { Project, ProjectWorkspace } from "@geetorusai/shared";
...
vi.mock("@/lib/router", () => ({
  ...
  useLocation: () => ({
    pathname: "/PAP/projects/geetorus-app/workspaces/workspace-1",
    search: mockRouteSearch.value,
    hash: "",
    state: null,
  }),
  useNavigate: () => mockNavigate,
  useParams: () => ({ companyPrefix: "PAP", projectId: "geetorus-app", workspaceId: "workspace-1" }),
}));
...
function project(overrides: Partial<Project> = {}): Project {
  ...
  return {
    ...
    codebase: {
      ...
      managedFolder: workspace.cwd ?? "/tmp/geetorus",
      effectiveLocalFolder: workspace.cwd ?? "/tmp/geetorus",
      origin: "local_folder",
    },
...
describe("ProjectWorkspaceDetail plugin tabs", () => {
  ...
  it("renders an arbitrary project_workspace plugin detail tab from the generic URL value", async () => {
    ...
    mockRouteSearch.value = "?tab=plugin%3Ageetorus.quality%3Aquality-tab&diffView=head&baseRef=origin%2Fmaster";

    await render();

    expect(container.querySelector('[data-tab-value="configuration"]')?.textContent).toBe("Configuration");
    expect(container.querySelector('[data-tab-value="plugin:geetorus.quality:quality-tab"]')?.textContent).toBe("Quality");
    ...
    expect(mockPluginSlotMount).toHaveBeenCalledWith(
      expect.objectContaining({
        slot: expect.objectContaining({ pluginKey: "geetorus.quality", id: "quality-tab" }),
  ...
  it("keeps the project workspace heading visible on plugin tabs", async () => {
    ...
    mockRouteSearch.value = "?tab=plugin%3Ageetorus.quality%3Aquality-tab";
  ...
  it("navigates plugin tabs with only the generic plugin tab parameter", async () => {
    ...
    await act(async () => {
      (container.querySelector('[data-tab-value="plugin:geetorus.quality:quality-tab"]') as HTMLButtonElement).click();
    });

    expect(mockNavigate).toHaveBeenCalledWith(
      "/projects/geetorus-app/workspaces/workspace-1?tab=plugin%3Ageetorus.quality%3Aquality-tab",
    );
  ...
  it("shows a missing plugin placeholder instead of configuration for stale plugin tab URLs", async () => {
    ...
    expect(container.querySelector('a[href="/projects/geetorus-app/workspaces/workspace-1?tab=configuration"]')?.textContent).toBe(
...
describe("ProjectWorkspaceDetail local path under the managed-sandbox-only policy", () => {
  ...
  it("shows the local path field and fact row when the policy is off", async () => {
    ...
    expect(container.textContent).toContain("/tmp/geetorus");
  });
  ...
  it("hides the local path field and fact row when the policy is on", async () => {
    ...
    expect(container.textContent).not.toContain("/tmp/geetorus");
