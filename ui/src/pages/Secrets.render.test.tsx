    ...
    candidates: [
      {
        ...
        config: {
          ...
          kmsKeyId: "alias/geetorus-secrets",
          ownerTag: "platform",
...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Geetorus config", async () => {
    ...
    expect(document.body.textContent).toContain("from Geetorus only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Geetorus"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
  ...
  it("writes a new value through the provider for external reference secrets", async () => {
    const externalSecret = makeCompanySecret({
      ...
      name: "geetorus-cloud/prod/provider/neon/admin-api-key",
      provider: "aws_secrets_manager",
      managedMode: "external_reference",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus-cloud/prod/provider/neon/admin-api-key",
      providerConfigId: "vault-aws",
    });
  ...
  it("explains AWS managed secret creation failures with actionable safe details", async () => {
    const rawProviderMessage =
      "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/Geetorus is not authorized";
    mockSecretsApi.create.mockRejectedValueOnce(
      new ApiError("AWS Secrets Manager denied the request. Check IAM permissions for this provider vault.", 403, {
        details: {
          ...
          safeAlternative:
            "If the secret already exists in AWS, link it as an external reference instead of creating a Geetorus-managed value.",
        },
      }),
    );
    const root = createRoot(container);
  ...
  it("discovers AWS provider vault candidates and applies selected values as prefill", async () => {
    ...
    await act(async () => {
      ...
      setInputValue(prefixInput!, "geetorus");
    });
    await flushReact();
    ...
    expect((document.getElementById("provider-vault-secret-name-prefix") as HTMLInputElement).value).toBe("geetorus");
    expect((document.getElementById("provider-vault-kms-key-id") as HTMLInputElement).value).toBe("alias/geetorus-secrets");
  ...
  it("shows AWS discovery errors without replacing manual vault form values", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Geetorus is not authorized";
    ...
    expect(errorBanner?.textContent).toContain("Geetorus server runtime/provider credential path");
