...
2. **No issue exists** — describe the problem directly in your PR body, following one of our [issue templates](.github/ISSUE_TEMPLATE/) so a reviewer has the same fields they'd get from a filed issue:
   - **Bug fix:** what happened, expected behavior, steps to reproduce, Geetorus version/commit, deployment mode. See [`bug_report.yml`](.github/ISSUE_TEMPLATE/bug_report.yml).
...
Many contributors run their own Geetorus instance to manage their work. Issue ids and links from *your* instance are private — reviewers and other contributors cannot open them, so they show up as clutter or broken links.

In your PR title, description, commits, and comments, **only reference public GitHub issues and PRs** — `#123`, `Fixes #123` / `Closes #123` / `Refs #123`, or full `https://github.com/geetorusai/geetorus/...` URLs.

Do **not** include references to internal/instance-local Geetorus work, such as:
...
Tooling (including Geetorus) often names a working branch after an internal issue and task — e.g. `PAPA-42-why-did-this-break`. That name leaks instance-local context, isn't meaningful to reviewers, and ends up as the public branch on your PR.
...
- Do **not** include internal Geetorus ticket ids (`PAPA-123`, `PAP-224`), instance task slugs, or other instance-derived details in the branch name.
...
### Geetorus Gates Must Pass

All Geetorus CI gates (lint, typecheck, tests, build, and any other required checks) must be satisfied before a PR can be merged. Don't ask for a merge while gates are red — fix them first.
...
We actively manage the core Geetorus feature roadmap.
...
> - Geetorus is the open source app people use to manage AI agents for work
...
> - This pull request adds per-adapter configuration of compaction, either auto or geetorus managed
> - That way we can get optimal performance from any adapter/provider in Geetorus
