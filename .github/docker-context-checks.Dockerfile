...
# `packages/geetorus-runner/**/*.md` ignore rule stripped the committed
...
RUN test -f packages/geetorus-runner/generated/capability/semantic-tool-contracts.json \
 && test -f packages/geetorus-runner/generated/semantic-action-catalog.json \
 && test -f packages/geetorus-runner/spec/evals/stress-workflow-traceability.json \
 && test -d packages/geetorus-runner/protocol/fixtures/replay
...
RUN node -e ' \
  const manifest = require("/context/packages/geetorus-runner/spec/evals/stress-workflow-traceability.json"); \
  const { accessSync } = require("node:fs"); \
  const { resolve } = require("node:path"); \
  let count = 0; \
  for (const finding of manifest.findings) \
    for (const path of finding.regressionTests) { \
      accessSync(resolve("/context/packages/geetorus-runner", path)); \
      count += 1; \
    } \
...
RUN AJV_RANGE="$(node -p "require('/context/packages/geetorus-runner/package.json').dependencies.ajv")" \
 && mkdir /probe-deps && cd /probe-deps && npm init -y >/dev/null \
 && npm install --ignore-scripts --no-audit --no-fund "ajv@${AJV_RANGE}" \
 && ln -s /probe-deps/node_modules /context/packages/geetorus-runner/node_modules \
 && cd /context/packages/geetorus-runner \
