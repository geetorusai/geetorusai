...
test('passes with full github.com URL', () => {
  assert.equal(
    checkLinkedIssue('See https://github.com/geetorusai/geetorus/issues/202', 'fix: bug').passed,
    true
  );
});
...
test('fails when the Geetorus issue URL is embedded inside another host', () => {
  const result = checkLinkedIssue(
    'See https://evil.example/https://github.com/geetorusai/geetorus/issues/123',
    'fix: bug'
  );
  assert.equal(result.passed, false);
});

test('fails when the Geetorus issue URL continues into another host', () => {
  const result = checkLinkedIssue(
    'See https://github.com/geetorusai/geetorus/issues/123.evil.example',
    'fix: bug'
  );
  assert.equal(result.passed, false);
});
