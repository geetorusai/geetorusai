...
const producer = { id: 123, head_sha: sha, head_branch: "master", path: ".github/workflows/cloud-migrator-artifacts.yml",
  head_repository: { id: 1170821064, full_name: "geetorusai/geetorus" }, event: "push", status: "completed", conclusion: "success" };
function bundle() {
  const packages = {}; const files = new Map();
  const entries = { "": { dependencies: { "@geetorusai/db": version } } };
  for (const name of ["db", "shared"]) {
    const metadata = previewManifest({ name: `@geetorusai/${name}`, dependencies: {} }, sha);
    ...
    entries[`node_modules/@geetorusai/${name}`] = { version, resolved: pin.url, integrity: pin.integrity, dependencies: metadata.dependencies };
  }
...
for (const fixture of [{ failure: 403 }, { failure: 503 }, { wrongImage: true },
  ...
  ...[{ head_sha: "b".repeat(40) }, { head_branch: "feature" }, { path: ".github/workflows/evil.yml" },
    { head_repository: { id: 123, full_name: "someone/geetorus" } }, { event: "pull_request" }].map((wrong) => ({ run: { ...producer, ...wrong } }))]) {
...
test("CLI verifies the exact bytes, source, master workflow and hosted runner and cleans up on failure", () => {
  ...
  assert.throws(() => verifyManifestProvenance(Buffer.from("exact manifest\n"), sha, { exec: (cmd, args) => {
    ...
    for (const [flag, value] of [["--repo", "geetorusai/geetorus"], ["--source-digest", sha], ["--source-ref", "refs/heads/master"],
      ["--cert-identity", "https://github.com/geetorusai/geetorus/.github/workflows/cloud-migrator-artifacts.yml@refs/heads/master"]]) assert.equal(args[args.indexOf(flag) + 1], value);
...
test("versioned readiness retains every source gate and removes duplicate automatic npm publication", () => {
  ...
  assert.match(ready, /if: github.repository == 'geetorusai\/geetorus' && github.ref == 'refs\/heads\/master'/);
