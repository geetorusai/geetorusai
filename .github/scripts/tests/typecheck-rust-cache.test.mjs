...
const github = { repository: "geetorusai/geetorus", event_name: "push", ref: "refs/heads/master", sha };
for (const [name, overrides, ref, allowed] of [
  ...
  ["fork", { repository: "someone/geetorus" }, sha, false],
...
test("cache excludes workspace code and executable installs, and preserves full checks", () => {
  ...
  assert.match(cache, /workspaces: packages\/geetorus-runner\/runner -> target/);
