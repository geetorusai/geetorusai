...
test('resolveBaseRef: returns the explicit base ref without making API calls', async () => {
  ...
  }, 'token', 'geetorusai/geetorus', 6469, 'release/1.2');
...
test('resolveBaseRef: falls back to the PR base ref', async () => {
  ...
  }, 'token', 'geetorusai/geetorus', 6469);

  assert.equal(baseRef, 'main');
  assert.deepEqual(seenPaths, ['/repos/geetorusai/geetorus/pulls/6469']);
});
...
test('checkDependencies: compares package files against the resolved base ref instead of master', async () => {
  ...
  assert.deepEqual(seenPaths, [
    '/repos/geetorusai/geetorus/contents/packages/foo/package.json?ref=release%2F1.2',
    '/repos/geetorusai/geetorus/contents/packages/foo/package.json?ref=refs%2Fpull%2F6469%2Fhead',
  ]);
});
