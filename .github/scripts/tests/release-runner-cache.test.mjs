...
const runner = workflow.split("  verify_geetorus_runner:")[1].split("  build:")[0];
...
test("Runner dependency caching selects the package's pinned compiler before computing its key", () => {
  ...
  assert.match(setup, /working-directory: packages\/geetorus-runner/);
  ...
  assert.match(runner, /workspaces: packages\/geetorus-runner\/runner -> target/);
...
test("the shared cache excludes workspace artifacts and only restores or saves the exact master-push source", () => {
  ...
  assert.equal(saveIf, "${{ matrix.lane == 'rust' && github.repository == 'geetorusai/geetorus' && github.event_name == 'push' && github.ref == 'refs/heads/master' && inputs.ref == github.sha }}");
...
test("parallel lanes cover check:all exactly once and never bypass verification", () => {
  const scripts = JSON.parse(readFileSync(new URL("../../../packages/geetorus-runner/package.json", import.meta.url))).scripts;
  ...
  const verify = runner.split("      - name: Verify Geetorus Runner\n")[1].split("      - name: Warm debug")[0];
  assert.match(verify, /RUNNER_CHECKS: \$\{\{ matrix.checks \}\}/);
  assert.match(verify, /set -euo pipefail/);
  assert.match(verify, /for check in \$RUNNER_CHECKS; do\s+pnpm --filter @geetorusai\/geetorus-runner "\$check"\s+done/);
...
test("only the trusted Rust lane writes, and warms both build profiles before saving", () => {
  ...
  assert.match(warm, /run: pnpm --filter @geetorusai\/geetorus-runner build:rust/);
  const sha = "a".repeat(40);
  const base = { repository: "geetorusai/geetorus", event_name: "push", ref: "refs/heads/master", sha };
  for (const lane of ["protocol", "rust"]) {
    for (const [overrides, ref, trusted] of [
      ...
      [{ repository: "someone/geetorus" }, sha, false],
