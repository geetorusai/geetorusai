...
const pr = prWorkflow.split("  verify_geetorus_runner:")[1].split("  build:")[0];
const release = releaseWorkflow.split("  verify_geetorus_runner:")[1].split("  build:")[0];
...
const keyInputs = [
  ...
  /workspaces: (packages\/geetorus-runner\/runner -> target)/,
...
test("the PR lane pins the compiler before the key is computed", () => {
  ...
  const verify = pr.indexOf("      - name: Verify Geetorus Runner\n");
...
test("a pull request never writes to or evicts the master cache entry", () => {
  const step = pr.split("      - name: Restore Runner Rust dependencies (read only)")[1]
    .split("      - name: Verify Geetorus Runner\n")[0];
