...
test('fetchAllPullRequestFiles: returns a single short page', async () => {
  ...
  }, 'geetorusai/geetorus', 6469, 'token');

  assert.deepEqual(seenPaths, [
    '/repos/geetorusai/geetorus/pulls/6469/files?per_page=100&page=1',
  ]);
...
test('fetchAllPullRequestFiles: keeps fetching when a page is full', async () => {
  ...
  assert.deepEqual(seenPaths, [
    '/repos/geetorusai/geetorus/pulls/6469/files?per_page=100&page=1',
    '/repos/geetorusai/geetorus/pulls/6469/files?per_page=100&page=2',
  ]);
