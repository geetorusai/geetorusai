...
const fleet = "runs-on/fleet=geetorus-post-merge-x64/env=public-ci";
const sha = "a".repeat(40);
const base = {
  repository: "geetorusai/geetorus", repository_id: "1170821064",
  ref: "refs/heads/master", event_name: "push", sha,
};
const expectedJobs = {
  "cloud-readiness.yml": [],
  "cloud-migrator-artifacts.yml": [],
  "release-verify.yml": ["typecheck", "general_tests", "serialized_tests", "runner_workflow_evals", "verify_geetorus_runner", "build"],
...
for (const [file, expectedNames] of Object.entries(expectedJobs)) {
  ...
  for (const [, job, body] of routed) {
    ...
    const cases = [
      ...
      { name: "fork", github: { repository: "someone/geetorus", repository_id: "123" } },
...
test("Cloud readiness bookkeeping never waits for the AWS verification fleet", () => {
  ...
  for (const [name, needs] of [
    ...
    assert.match(body, /^    if: github.repository == 'geetorusai\/geetorus' && github.ref == 'refs\/heads\/master'$/m);
