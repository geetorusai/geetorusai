...
Run Geetorus in Docker without installing Node or pnpm locally.
...
| `GEETORUS_BUILD_VERSION` | empty | Runtime version when Git metadata is unavailable |
| `GEETORUS_BUILD_COMMIT` | empty | Source commit written into the server build stamp and runtime environment |
...
commit label and platform and adds `ghcr.io/geetorusai/geetorus:sha-<full-commit-sha>-cloud`.
...
docker run --name geetorus \
  ...
  -e GEETORUS_TOOL_ACTION_SIGNING_SECRET=$(openssl rand -hex 32) \
...
All persisted under your bind mount (`./data/docker-geetorus` in the example above).
...
GEETORUS_TOOL_ACTION_SIGNING_SECRET=$(openssl rand -hex 32) \
...
- host port: `3100`
- persistent data dir: `./data/docker-geetorus`

Optional overrides:

```sh
GEETORUS_PORT=3200 GEETORUS_DATA_DIR=../data/pc \
  docker compose -f docker/docker-compose.quickstart.yml up --build
```

**Note:** `GEETORUS_DATA_DIR` is resolved relative to the compose file (`docker/`), so `../data/pc` maps to `data/pc` in the project root.

If you change host port or use a non-local domain, set `GEETORUS_PUBLIC_URL` to the external URL you will use in browser/auth flows.

Pass `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` to enable local adapter runs.

### Full stack (with PostgreSQL)

Geetorus server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). Geetorus data persists in `geetorus-data`.
...
For authenticated deployments, set one canonical public URL and let Geetorus derive auth/callback defaults:
...
services:
  geetorus:
    environment:
      ...
      GEETORUS_PUBLIC_URL: https://desk.koker.net
```

`GEETORUS_PUBLIC_URL` is used as the primary source for:
...
Geetorus URL, sign in or create an account, then choose `Claim this instance`
...
Granular overrides remain available if needed (`GEETORUS_AUTH_PUBLIC_BASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `GEETORUS_ALLOWED_HOSTNAMES`).

Set `GEETORUS_ALLOWED_HOSTNAMES` explicitly only when you need additional hostnames beyond the public URL host (for example Tailscale/LAN aliases or multiple private hostnames).
...
`GEETORUS_VERCEL_CONNECT_ENABLED=true` does not expose a customer-facing setup
...
can provide `GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN` as a deployment bootstrap
...
services:
  geetorus:
    environment:
      GEETORUS_VERCEL_CONNECT_ENABLED: "true"
      GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN: ${GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN}
```
...
- Adapter environment checks in Geetorus will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run Geetorus + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/geetorus.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/geetorus.container` | Geetorus server — joins the pod, connects to Postgres at `127.0.0.1` |
| `docker/quadlet/geetorus-db.container` | PostgreSQL 17 — joins the pod, health-checked |
...
3. Create a secrets env file (keep out of version control):
   ...
   ```sh
   cat > ~/.config/containers/systemd/geetorus.env <<EOL
   BETTER_AUTH_SECRET=$(openssl rand -hex 32)
   GEETORUS_TOOL_ACTION_SIGNING_SECRET=$(openssl rand -hex 32)
   POSTGRES_USER=geetorus
   POSTGRES_PASSWORD=geetorus
   POSTGRES_DB=geetorus
   DATABASE_URL=postgres://geetorus:geetorus@127.0.0.1:5432/geetorus
   # OPENAI_API_KEY=sk-...
...
4. Create the data directory and start:
   ...
   ```sh
   mkdir -p ~/.local/share/geetorus
   systemctl --user daemon-reload
   systemctl --user start geetorus-pod
   ```

### Quadlet management

```sh
journalctl --user -u geetorus -f        # App logs
journalctl --user -u geetorus-db -f     # DB logs
systemctl --user status geetorus-pod    # Pod status
systemctl --user restart geetorus-pod   # Restart all
systemctl --user stop geetorus-pod      # Stop all
```

### Quadlet notes

- **First boot**: Unlike Docker Compose's `condition: service_healthy`, Quadlet's `After=` only waits for the DB unit to *start*, not for PostgreSQL to be ready. On a cold first boot you may see one or two restart attempts in `journalctl --user -u geetorus` while PostgreSQL initialises — this is expected and resolves automatically via `Restart=on-failure`.
- Containers in a pod share `localhost`, so Geetorus reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `geetorus-pgdata` named volume.
- Geetorus data persists at `~/.local/share/geetorus`.
...
- `npx geetorusai onboard --yes` completes
...
```sh
HOST_PORT=3200 GEETORUSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
GEETORUS_DEPLOYMENT_MODE=authenticated GEETORUS_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/geetorus-smoke.env GEETORUSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- Smoke script defaults host port to `3131` to avoid conflicts with local Geetorus on `3100`.
- Smoke script also defaults `GEETORUS_PUBLIC_URL` to `http://localhost:<HOST_PORT>` so bootstrap invite URLs and auth callbacks use the reachable host port instead of the container's internal `3100`.
- In authenticated mode, the smoke script defaults `SMOKE_AUTO_BOOTSTRAP=true` and drives the real bootstrap path automatically: it signs up a real user, runs `geetorusai auth bootstrap-ceo` inside the container to mint a real bootstrap invite, accepts that invite over HTTP, and verifies board session access.
...
- Geetorus data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/geetorus` (quadlet).
