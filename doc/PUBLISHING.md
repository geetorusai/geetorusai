...
Low-level reference for how Geetorus packages are prepared and published to npm.
...
Geetorus no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `geetorusai`, imports code from workspace packages such as:
...
### Manual first publish for `@geetorusai/ui`
...
npm view @geetorusai/ui version
...
- If `npm view @geetorusai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@geetorusai/ui`, the `geetorusai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Geetorus uses calendar versions:
...
- `geetorusai@2026.318.1-canary.2`
...
- every published internal `@geetorusai/*` dependency referenced by that manifest exists on npm
...
Geetorus does not auto-publish every non-private workspace package anymore.
...
pnpm run release:bootstrap-package -- @geetorusai/new-package
...
pnpm run release:bootstrap-package -- @geetorusai/new-package --publish
```

The helper script:

- refuses names outside the `@geetorusai/` scope
...
1. open `https://www.npmjs.com/package/@geetorusai/new-package`
2. go to `Settings` → `Trusted publishing`
3. add repository `geetorusai/geetorus`
