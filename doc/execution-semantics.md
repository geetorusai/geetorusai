...
This document explains how Geetorus interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

Geetorus separates four concepts that are easy to blur together:
...
This is a hard invariant. Geetorus is single-assignee by design.

## 3. Status Semantics

Geetorus issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, Geetorus may still need a wake path to ensure the assignee actually sees it
...
- waiting on an external dependency or system when Geetorus does not own a scheduled re-check
...
- Geetorus can wake the assignee
- Geetorus can track runs linked to the issue
- Geetorus can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- Geetorus can track the ownership and status
- Geetorus cannot rely on heartbeat/run semantics to keep them moving
...
Geetorus already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.
...
An unresolved workspace base ref is another configuration-incomplete condition. A `git_worktree` workspace bases a fresh worktree on a configured base ref. Geetorus first fetches a remote-only ref before dispatch: it maps an unqualified name (for example `fix/foo`) or a remote-tracking name (for example `origin/fix/foo`) to `origin/<branch>`, runs the authenticated fetch, and re-checks the commit. A ref that resolves lets work continue on the resolved commit. A ref that is still unresolvable after the fetch produces a configuration-incomplete blocker that names the requested ref, rather than a dispatched-then-failed run. Because the adapter never started, Geetorus queues no missing-comment retry. The recovery action dedupes by the canonical remote ref (`origin/<branch>`), not the operator spelling. Two equivalent spellings of one remote branch, for example `fix/foo` and `origin/fix/foo`, share one recovery identity, so a repeated failure reuses the active action and does not reset the attempt count or post a second notice. A different remote branch is a distinct blocker. Geetorus resolves the prior recovery action, creates a new action for the new ref, and notifies the operator with the new ref instead of overwriting the active action of the prior ref.

## 6. Parent/Sub-Issue vs Blockers

Geetorus uses two different relationships for different jobs.
...
Blocked issues should stay idle while blockers remain unresolved. Geetorus should not create a queued heartbeat run for that issue until the final blocker is done and the `issue_blockers_resolved` wake can start real work.

`cancelled` is terminal for the blocker issue itself, but it does not satisfy the dependency. A cancelled blocker edge remains unresolved until the edge is removed or replaced, and Geetorus must surface blocker attention on the dependent regardless of whether that dependent is currently displayed as `blocked`, `todo`, `backlog`, or another non-terminal agent-owned status.
...
The default is to implement on the source issue. The run-scoped `geetorus-converting-plans-to-tasks` guidance decides whether a minimum child graph is justified by an ownership, parallelism, dependency, review, or lifecycle boundary. A child must not be created merely because a plan was accepted, and the source must not be blocked merely because children exist.
...
The accepted-plan decomposition API and records remain as an optional compatibility surface. When that API is explicitly used, Geetorus treats it as an exact-once control-plane primitive, not as the runner's ordinary child-creation binding.
...
Geetorus does not need to mandate a specific storage shape in this document. The record may live in a dedicated table, source-issue execution state, interaction metadata, or another durable product surface. What matters is the contract:
...
If the live run disappears, Geetorus must repair, resume, or visibly block the existing claim. It must not leave the source issue in a state where a second run can interpret the same acceptance as fresh permission to create sibling issues again.
...
For agent-owned, non-terminal issues, Geetorus should never leave work in a state where nobody is responsible for the next move and nothing will wake or surface it.

This is a visibility contract, not an auto-completion contract. If Geetorus cannot safely infer the next action, it should surface the ambiguity with a blocked state, a visible notice, or an explicit recovery action. It must not silently mark work done from prose comments or guess that a dependency is complete.
...
An external wait counts as a live or waiting path only when the next move survives the current heartbeat and is represented in Geetorus's durable control-plane state. Valid external-wait shapes are:
...
A one-shot issue monitor consumes its persisted `nextCheckAt` when it dispatches the assignee wake. If that monitor-consuming run is lost before it records a new disposition or future monitor, Geetorus restores exactly one bounded continuation using the existing process-loss retry limit; if that continuation is also lost, the normal recovery-action escalation owns the next step instead of creating another monitor loop.

An unmanaged local process is not a durable action path. Shell jobs started with `&`, `nohup`, local polling loops, detached PTY sessions, adapter child processes, or similar background watchers do not keep an issue live unless Geetorus persists them as a run or pairs a managed runtime service with a monitor, scheduled wake, blocker, or delegated issue that owns the next check. A PID, session id, log file, comment, or promise to check later is evidence only. The process may be killed when the adapter invocation or heartbeat exits and cannot be assumed observable or recoverable by another worker.

Before a heartbeat finalizes, its issue disposition must therefore be evaluated from durable Geetorus state, not from processes still visible only to that heartbeat. An agent-owned issue may remain `in_progress` after the heartbeat only when another valid action-path primitive already exists. If the only claimed continuation is a local/background watcher, finalization treats the issue as having no live path even when the process has not yet been observed exiting.
...
A board comment can be an interrupt, an ownership change, both, or neither. Geetorus must keep those concepts separate in the product contract.
...
For adapter-backed execution, an active run or queued wake counts as a live path only when Geetorus can also prove that the selected workspace is coherent for that adapter invocation. A wake that cannot start in the intended workspace is only a failed delivery attempt, not a healthy liveness path.
...
- the adapter will receive the same effective workspace/cwd that Geetorus resolved for the run, including the same workspace ids and `GEETORUS_WORKSPACE_*` environment values
...
Assigning an issue normally implies executable intent. When create APIs receive an assignee and no explicit status, Geetorus defaults the issue to `todo` so the assignee has a wake path instead of silently inheriting the unassigned `backlog` default.

An explicit assigned `backlog` issue remains valid when the creator is deliberately parking the work. It must not wake the assignee just because it has an assignee. Geetorus should make that choice visible in activity and UI so operators can distinguish intentional parking from a missed handoff.
...
An `in_review` issue is stalled when it has no typed participant, no pending interaction or approval, no user owner, no active monitor, no active run, no queued wake, and no explicit recovery action. Geetorus should surface that state as recovery work rather than silently completing the issue or leaving blocker chains parked indefinitely.

When an execution-policy review stage has a pending agent participant, the participant's run is part of the review path only while it is live or queued. If that participant run reaches a terminal state while `executionState.status` remains `pending`, no decision has been recorded. After a successful run with no review decision, Geetorus should queue one bounded normal-model recovery wake for the same participant when the agent is invokable and no other review path exists. A failed participant instead follows the provider-continuity rules below: local conversational adapters can start a bounded continuation turn, while native sessions use validated resume/replacement. Other adapters retain their action-recovery gates. The original assignee stays unchanged. If that recovery run also finishes while the stage remains pending, or the participant cannot be invoked, Geetorus must move the source issue to an explicit blocked/recovery path instead of leaving `in_review` to drift silently.
...
- `nextCheckAt`: when Geetorus should wake the assignee
- `notes`: non-secret instructions for what the assignee should check
- `serviceName`: optional non-secret external-service context
- `externalRef`: optional external-service reference input; Geetorus treats it as secret-adjacent, redacts it before persistence/visibility, and omits it from activity and wake payloads
- `timeoutAt`, `maxAttempts`, and `recoveryPolicy`: optional recovery hints for bounded waits

Monitors are not recurring intervals. When a monitor fires, Geetorus clears the scheduled monitor and queues an `issue_monitor_due` wake for the assignee. If the external service is still pending, the assignee must explicitly re-arm the monitor with a new `nextCheckAt`. If the issue moves to `done`, `cancelled`, an invalid status, or a human/unassigned owner, the monitor is cleared.
...
Monitor bounds are enforced. Geetorus rejects attempts to re-arm a monitor whose `timeoutAt` or `maxAttempts` is already exhausted. When a scheduled monitor reaches an exhausted bound at trigger time, Geetorus clears it and follows `recoveryPolicy`: `wake_owner` queues a bounded recovery wake for the assignee, `create_recovery_issue` opens visible issue-backed recovery work, and `escalate_to_board` records a board-visible escalation comment/activity.

Use `blocked` instead of a monitor when no Geetorus assignee owns a responsible polling path. In that case, name the external owner/action or create first-class recovery/blocker work.
...
Geetorus now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, Geetorus queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, Geetorus moves the issue to `blocked` and opens or updates a board-owned recovery action without changing the source assignee or waking a substitute agent; the visible comment is evidence, not the recovery path by itself
...
- Geetorus queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, Geetorus moves the issue to `blocked` and opens or updates a board-owned recovery action without changing the source assignee or waking a substitute agent; the visible comment is evidence, not the recovery path by itself
...
The same bounded rule applies when the previous heartbeat reported waiting on a local/background watcher and that watcher was killed, disappeared, or was never represented by a durable Geetorus primitive. Geetorus queues at most one continuation for the same recovery fingerprint. If the continuation also leaves only local watcher evidence, Geetorus must surface a real blocker or explicit recovery action instead of repeating continuation recovery. A new monitor, scheduled wake, healthy delegated blocker issue, or other durable source mutation resolves that recovery fingerprint normally.
...
- if the issue has a real waiting target — open (non-terminal) sub-tasks or existing unresolved blockers — Geetorus converts the deliberate wait into a first-class dependency wait: it sets the issue `blocked` by those issues, keeps the original assignee, and posts a plain-language comment explaining that the task will resume automatically when its dependencies finish. The issue then self-resumes through the normal `issue_blockers_resolved` path; no recovery action or escalation owner is involved
- if the issue has no current typed waiting target and the original owner is invokable, Geetorus classifies it as `deliberate_wait_without_target` and gives that owner five normal-model disposition-repair attempts: immediate, then after 60, 120, 240, and 480 seconds, with up to 10 percent jitter on delayed attempts
- before every attempt, Geetorus revalidates unresolved blockers and children, interactions, linked approvals, monitors, execution stages, queued wakes, active runs, work products, owner invokability, and budget or governance gates. Any real live or waiting path suppresses the retry
...
- after five attempts with the same fingerprint, Geetorus opens one board-owned source-scoped recovery action. The source assignee remains unchanged, no manager/creator/executive substitute is woken, and the board chooses whether to repair, retry the original owner, explicitly reassign, or resolve
...
On startup and on the periodic recovery loop, Geetorus performs the following recovery passes:
...
If no included issue has a live path, Geetorus computes a stop fingerprint from durable subtree state, including at least:
...
If the watchdog moved work forward, Geetorus should not mark the old fingerprint as permanently acceptable just because the watchdog issue completed. The next scan should observe the changed subtree state and either suppress because work is live or compute a new stopped fingerprint later.
...
An active run can still be unhealthy even when its process is `running`. Geetorus treats prolonged output silence as a watchdog signal, not as proof that the run is failed.
...
A completion tool such as `geetorus_finish` reports task disposition; it does not
end the provider turn. Geetorus continues persisting and displaying provider
...
Geetorus trusts the server-selected startup execution root in the isolated
...
Codex retains the model conversation. Geetorus resumes with `excludeTurns: true`,
...
Geetorus opens an explicit recovery action when the system can identify a problem but cannot safely complete the work itself.
...
An upgrade may encounter an already-active agent-owned recovery action. Geetorus keeps that record readable and resolvable for compatibility, but periodic reconciliation does not enqueue another takeover wake from it.
...
In these cases Geetorus should leave a visible issue/comment trail instead of silently retrying.

## 14. What This Does Not Mean

These semantics do not change V1 into an auto-reassignment system.

Geetorus still does not:
...
That is the execution contract Geetorus should present to operators.
...
Geetorus submits saved input through normal task admission, once, with the
...
Native ACPX sessions register the assigned Geetorus MCP gateway alongside the
