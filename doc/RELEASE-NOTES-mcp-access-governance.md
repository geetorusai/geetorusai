...
Geetorus now governs every MCP tool call an agent makes. Operators install managed connections, define profiles and policies, approve high-risk actions, and read an append-only audit log. Default-deny on unknown tools; quarantine on schema drift; approval required by default for writes; trust rules to lift human-in-the-loop on safe repeats.

## Why this exists

Agents that can call arbitrary MCP tools can also leak data, modify accounts, or run destructive operations the operator never approved. Previous Geetorus releases relied on the agent's runtime trusting whatever MCP server it was pointed at. That is the wrong default for production. MCP Access Governance moves the trust boundary to Geetorus itself: every call is selected, evaluated, audited, and (when needed) gated on a human.
...
- Agents speak MCP only to the Geetorus gateway. The gateway returns the agent's effective tool list, validates each call against profile + policies, and records the result.
...
- **Local stdio in `authenticated/public`**: fails closed unless `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` is set on a designated trusted worker.
...
1. Apply DB migrations as usual (`pnpm geetorusai migrate`).
...
5. If you run `authenticated/public`, decide whether you want a trusted runtime worker for local stdio. If yes, set `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` on that worker and only that worker. If no, leave it unset — `remote_http` connections continue to work.
...
- Endpoint mode (Geetorus's own `/mcp` surface) is not subject to the profile/policy stack.
...
curl -fsS -H "Authorization: Bearer $BOARD_API_KEY" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/runtime-health" | jq '{status, alerts}'
