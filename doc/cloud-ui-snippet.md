...
Cloud operators can set `GEETORUS_CLOUD_UI_SNIPPET` to an HTML snippet.
...
`GEETORUS_CLOUD_UI_SNIPPET_B64` carries the same snippet through them as
standard base64 of the UTF-8 HTML:

```sh
GEETORUS_CLOUD_UI_SNIPPET_B64="$(base64 < snippet.html)"
```
...
`base64 -d`. A present `GEETORUS_CLOUD_UI_SNIPPET` always wins, blank
...
```sh
GEETORUS_CLOUD_UI_SNIPPET_B64="$(base64 < snippet.body.js)"
```
...
No signing secret or Plain API key is required. No Geetorus customer identity
or organization data is passed. Plain manages the anonymous browser session;
there is no Geetorus account-switch integration. Ask users for identifying
