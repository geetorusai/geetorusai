...
Geetorus Telemetry events, and they are not OpenTelemetry exports. A run-log
...
the literal `\u0000`, and the reserved `$geetorusRunEventJsonV1` field contains
...
Geetorus Telemetry export, and legacy adapters do not use this writer.

## Native Restart Recovery Run-Log Event

Geetorus writes a `native.recovery.transition` event for every native restart
...
Geetorus Telemetry or OpenTelemetry.
...
Geetorus writes one `run.startup.step` event to the run log for each bring-up
...
Geetorus writes one `run.phase.timing` event to the run log for each
