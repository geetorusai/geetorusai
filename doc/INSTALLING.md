...
Geetorus supports a managed installation, an ephemeral `npx` tryout, a
...
3. delegates installation to `geetorusai install`;
...
The `geetorus.ing` checksum detects transfer or publishing mistakes, but it is
...
```sh
raw_base=https://raw.githubusercontent.com/geetorusai/geetorus
...
separate delivery path from `geetorus.ing`; do not treat a checksum served by
the same origin as the artifact as an independent trust anchor.

Each installer flag also has a `GEETORUS_INSTALL_*` environment-variable
...
`env.GEETORUS_CODEX_ACP_NETWORK_ACCESS="false"`. Execution-target network denial
...
Managed installs pin the validated Node executable in the `geetorusai` shim
...
`npx geetorusai@latest install --yes`. Do not use the old managed shim to
...
~/.local/bin/geetorusai
```

The `geetorusai` shim remains stable while `current` switches atomically
between complete payloads. Geetorus keeps the two previous managed payloads
for rollback. Configuration, databases, uploads, logs, secrets, and workspaces
remain under `~/.geetorus/instances/` and are not stored inside CLI payloads.
...
```sh
npx --registry https://registry.npmjs.org geetorusai install
```

Install canary or pin an exact published version:

```sh
npx --registry https://registry.npmjs.org geetorusai install --canary
npx --registry https://registry.npmjs.org geetorusai install --version 2026.720.0
```

Install a branch, tag, or commit from GitHub:

```sh
npx --registry https://registry.npmjs.org geetorusai install --ref master
npx --registry https://registry.npmjs.org geetorusai install --ref v2026.720.0
npx --registry https://registry.npmjs.org geetorusai install --ref <commit-sha>
```
...
Interactive onboarding asks whether Geetorus should run as a background
...
```sh
geetorusai onboard --yes                    # configure only; no service install
geetorusai onboard --yes --install-service  # explicit automation opt-in
...
```sh
geetorusai service install
geetorusai service status
geetorusai service start
geetorusai service stop
geetorusai service restart
geetorusai service logs -f
geetorusai service uninstall
```

Geetorus uses a systemd user service on Linux and WSL2 systems with user
systemd, and a LaunchAgent on macOS. Containers, WSL1, and systems without a
supported user service manager receive foreground `geetorusai run` guidance
...
Use one server process per instance. `geetorusai run` refuses to start when
...
```sh
geetorusai update --latest
geetorusai update --canary
geetorusai update --version 2026.720.0
```
...
If the service is stopped, start it with `geetorusai service start` before
updating so Geetorus can take the safety backup. Use
`geetorusai update --no-backup` only when you intentionally accept updating
...
```sh
npm install --global --registry https://registry.npmjs.org geetorusai
geetorusai onboard
```
...
The managed `geetorusai update` command can update managed and global npm
...
`geetorusai uninstall` removes the managed shim, manifest, and CLI payloads.
It deliberately preserves `~/.geetorus/instances/`, including configuration,
databases, uploads, logs, secrets, backups, and workspaces. Back up and remove
that data separately only when you intend to delete the Geetorus instance.
