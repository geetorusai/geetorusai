...
Geetorus ships with **opt-in** OpenTelemetry auto-instrumentation for the
server process. When activated it produces **traces only** — no metrics and no
logs are exported by this integration.

`@opentelemetry/api` is a normal dependency of `@geetorusai/server`. Every
...
decision. `@opentelemetry/api` is the one OpenTelemetry package Geetorus
...
export OTEL_SERVICE_NAME="geetorus"
...
1. **The build stamp.** The server `build` script writes the commit SHA into
   ...
   build excludes `.git`, so the build script reads the `GEETORUS_BUILD_COMMIT`
...
3. **The `OTEL_SERVICE_VERSION` environment variable.** This is the fallback
   ...
   build. `OTEL_SERVICE_VERSION` is a Geetorus-specific variable, not an
   OpenTelemetry SDK variable, so Geetorus controls this precedence.
...
If `OTEL_EXPORTER_OTLP_PROTOCOL` is set to an unrecognized value, Geetorus
...
Geetorus Runner task runs emit a single foldable OpenTelemetry trace. This is
...
`geetorus.native.span.` prefix and a closed key allowlist; values are limited
...
Geetorus ships with **opt-in** Sentry error monitoring for the server
...
- `tracesSampleRate` is `0`, on both runtimes. Geetorus sends no
...
option so it never fights Geetorus's separate, independently opt-in
...
Geetorus opens OpenTelemetry spans on the sandbox start path. These spans are
an Observability surface. They are not Geetorus Telemetry events. The
generated telemetry contract does not cover them, so this section is their
canonical contract.

The spans are opt-in. Geetorus exports them only when an OTLP endpoint is
configured. With no endpoint the whole span path is a no-op. Geetorus opens the
spans only for a run that targets a remote sandbox. A local run and an SSH run
stay out of these spans.

Every span attribute uses the closed `geetorus.sandbox.startup.` prefix and
...
Each numeric attribute is finite. Geetorus omits an attribute when its value is
...
| `bridge.geetorus`                    | Geetorus bridge start step.                                                                                                                               | `sandbox.startup`               |
...
| `sandbox.callbackBridge.relayRequest` | One Geetorus-API callback request — read the request, write the response, remove it.                                                                      | the active run span             |
...
The `geetorus.sandbox.startup.outcome` attribute uses a closed value set:
...
| ---------------------------------------- | ------- | -------- | ---------------------------------------------------------- |
| `geetorus.sandbox.startup.root.wall_ms` | number  | no       | The root-span wall time of the whole bring-up.             |
| `geetorus.sandbox.startup.root.work_ms` | number  | no       | The sum of the step wall times.                            |
| `geetorus.sandbox.startup.root.diff_ms` | number  | no       | `work_ms − wall_ms`; the overlap the parallel steps saved. |
| `geetorus.sandbox.startup.provider`     | string  | yes      | The normalized provider family.                            |
| `geetorus.sandbox.startup.cold_start`   | boolean | yes      | Whether the bring-up is a cold start.                      |
| `geetorus.sandbox.startup.region`       | string  | yes      | The clamped region label.                                  |
| `geetorus.sandbox.startup.image_id`     | string  | yes      | The hashed image id.                                       |
| `geetorus.sandbox.startup.sandbox_id`   | string  | yes      | The hashed sandbox id.                                     |
| `geetorus.sandbox.startup.lease_id`     | string  | yes      | The hashed lease id.                                       |
...
| Attribute                                                    | Type   | Optional | Meaning                                                  |
| ------------------------------------------------------------ | ------ | -------- | -------------------------------------------------------- |
| `geetorus.sandbox.startup.step.wall_ms`                     | number | no       | The wall time of the step.                               |
| `geetorus.sandbox.startup.outcome`                          | string | no       | The step outcome (`ok`, `skipped`, or `failed`).         |
| `geetorus.sandbox.startup.provider`                         | string | yes      | The normalized provider family.                          |
| `geetorus.sandbox.startup.batch`                            | string | yes      | A shared tag that marks two parallel steps as one batch. |
| `geetorus.sandbox.startup.handshake.create_runtime.wall_ms` | number | yes      | The create-runtime sub-time of the `acp.handshake` step. |
| `geetorus.sandbox.startup.handshake.ensure_session.wall_ms` | number | yes      | The ensure-session sub-time of the `acp.handshake` step. |
...
The `sandbox.exec` span uses this closed attribute allowlist. Geetorus omits a
numeric attribute when the provider does not report the value.

| Attribute                                       | Type    | Optional | Meaning                                                                    |
| ----------------------------------------------- | ------- | -------- | -------------------------------------------------------------------------- |
| `geetorus.sandbox.startup.provider`            | string  | no       | The normalized provider family.                                            |
| `geetorus.sandbox.startup.exec.command`        | string  | no       | The clamped `argv[0]` command label.                                       |
| `geetorus.sandbox.startup.exec.exit_code`      | number  | yes      | The numeric process exit code.                                             |
| `geetorus.sandbox.startup.exec.wall_ms`        | number  | no       | The host-measured wall time of the execution.                              |
| `geetorus.sandbox.startup.exec.wait_before_ms` | number  | yes      | The provider handle-fetch wait before the execution ran.                   |
| `geetorus.sandbox.startup.exec.sandbox_ms`     | number  | yes      | The in-sandbox run time of the execution.                                  |
| `geetorus.sandbox.startup.exec.network_ms`     | number  | yes      | The transport time the host adds; `wall_ms − wait_before_ms − sandbox_ms`. |
| `geetorus.sandbox.startup.exec.critical_path`  | boolean | no       | Whether the execution sits on the startup critical path.                   |
| `geetorus.sandbox.startup.exec.cache_hit`      | boolean | yes      | Whether the provider served the sandbox handle from its warm cache.        |
| `geetorus.sandbox.startup.outcome`             | string  | no       | The execution outcome (`ok` or `failed`).                                  |

The plugin decides the cache hit at the sandbox-handle lookup. The span no
longer infers a cache hit from `wait_before_ms == 0`. Geetorus omits the
...
`geetorus.sandbox.startup.` attribute prefix and obey the same opt-in and
...
| `sandbox.daytona.transfer`           | The transfer step: an upload to the sandbox (inbound) or a download from the sandbox (outbound). The `geetorus.sandbox.startup.transfer.direction` attribute records the direction. | the active sync task span (`stage.*` inbound, `restore.*` under `sandbox.syncBack` outbound) |
...
| ------------------------------------------------ | ------ | -------- | --------------------------------------------------------------------------------------------------------- |
| `geetorus.sandbox.startup.provider`             | string | no       | The normalized provider family.                                                                           |
| `geetorus.sandbox.startup.outcome`              | string | yes      | The step outcome (`ok`, `skipped`, or `failed`).                                                          |
| `geetorus.sandbox.startup.pack.wall_ms`         | number | yes      | The host-local wall time of the pack step. It rides the `sandbox.daytona.pack` span.                      |
| `geetorus.sandbox.startup.transfer.wall_ms`     | number | yes      | The wall time of the transfer step. It rides the `sandbox.daytona.transfer` span.                         |
| `geetorus.sandbox.startup.transfer.guard.count` | number | yes      | The number of serial guard round trips before one transfer. It rides the `sandbox.daytona.transfer` span. |
| `geetorus.sandbox.startup.transfer.direction`   | string | yes      | The transfer direction (`inbound` or `outbound`). It rides the `sandbox.daytona.transfer` span.           |
...
Geetorus opens a fixed observability surface for the sandbox duplex transport.
This instrumentation is separate from Geetorus Telemetry events and from the
...
Run `pnpm --filter @geetorusai/server exec tsx ../scripts/benchmark-skill-preparation.ts` for an isolated embedded
