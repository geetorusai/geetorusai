...
Operator guide for Geetorus's MCP tool access surface. Audience: board users and CloudOps engineers who install connections, write policies, approve action requests, and respond to runtime alerts. For runtime alert response, pair this with [MCP-RUNTIME-OPERATIONS.md](./MCP-RUNTIME-OPERATIONS.md).
...
- [Geetorus as MCP endpoint vs MCP gateway](#geetorus-as-mcp-endpoint-vs-mcp-gateway)
...
Geetorus governs MCP tool access by separating four concerns:
...
curl -fsS -X POST \
  ...
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/examples/safe-read-only-todo-kv/install" \
  -d '{}' | jq .
...
curl -fsS -X POST \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/examples/safe-read-only-todo-kv/smoke" \
...
## Geetorus as MCP endpoint vs MCP gateway

Geetorus plays two roles in the MCP graph, and confusing them is the most common operator error.

```
              Geetorus as MCP endpoint
              (clients call Geetorus)
                       ┌──────────────┐
   Claude Code  ─────▶ │   Geetorus  │
   IDE / CLI    ─────▶ │ /mcp surface │
                       └──────────────┘
                          (task ops, agent ops)


              Geetorus as MCP gateway / proxy
              (agents call upstream MCP through Geetorus)

   ┌─────┐       ┌────────────┐       ┌──────────────┐
   │Agent│──────▶│ Geetorus  │──────▶│ Upstream MCP │
...
**Endpoint mode** — Geetorus exposes its own MCP surface so external clients (Claude Code, IDEs, scripts) can manipulate Geetorus tasks and agents. This is what `doc/TASKS-mcp.md` covers. Access control here is the standard Geetorus auth model ([DEPLOYMENT-MODES.md](./DEPLOYMENT-MODES.md)): bearer keys, sessions, board API key.

**Gateway mode** — Geetorus proxies tool calls from a Geetorus agent to an upstream MCP server (GitHub, Linear, a local stdio fixture, etc.). Every call goes through profile selection, policy evaluation, optional human approval, rate limiting, redaction, and audit. This is what the rest of this document covers.

Operators usually mean *gateway* when they say "MCP access governance". For Geetorus-managed local adapter runs, Geetorus writes adapter MCP config that points at named gateway endpoints with short-lived scoped bearer tokens. Policies, approvals, and the audit log only exist for calls that enter gateway mode.

V1 does not claim host-wide MCP enforcement. If an unmanaged external client, hand-edited adapter config, or process outside the Geetorus-controlled workspace calls an upstream MCP server directly, Geetorus can warn about known overlapping config entries but cannot prevent or audit that bypass. Treat managed MCP config as a control-plane containment feature for Geetorus-launched agents, not as an endpoint firewall for the operator's whole machine.
...
| --- | --- | --- |
| `remote_http` | Hosted SaaS MCP servers (GitHub, Linear, custom remote MCP). Default for cloud. | Geetorus authenticates with stored credential refs and proxies calls. Process supervision is upstream's problem. |
...
Operators do not paste arbitrary `command` / `args` for stdio. Allowed stdio entries are limited to the approved template catalog (e.g. `geetorus.echo-calculator-time`, `geetorus.synthetic-todo-kv`). To add a new template, ship a code change.
...
curl -fsS -X POST \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/connections" \
  -d '{
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/tool-connections/$CONNECTION_ID/health-check" -d '{}' \
...
Each tool discovered on a connection becomes a **catalog entry** with a risk level Geetorus infers from MCP annotations:
...
When a catalog refresh discovers a new write/destructive tool that did not exist on the prior schema, Geetorus sets `status: quarantined` and records the reason in `quarantineReason`. Quarantined entries are never returned to the agent's tool list until an operator reviews and re-enables them. This is the **changed-tool quarantine** rule and the primary defense against an upstream server silently adding a destructive verb.
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/profiles" \
  -d '{
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/profiles/$PROFILE_ID/bind" \
...
curl -fsS -H "Authorization: Bearer $BOARD_API_KEY" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/profiles/effective/agents/$AGENT_ID" \
...
```sh
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/policy/test" \
  -d '{
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/tool-gateway/action-requests/$ACTION_REQUEST_ID/approve" \
...
curl -fsS -X POST -H "X-Geetorus-Tool-Gateway-Token: $GATEWAY_TOKEN" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/tool-gateway/tools/call" \
  -d '{
    "tool": "create_item",
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/action-requests/$ACTION_REQUEST_ID/trust-rule" \
  -d '{
    "approvalThreshold": 2,
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/trust-rules/$POLICY_ID/revoke" \
...
Cloud reminder: in `authenticated/public`, local stdio slots fail closed unless `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` is set on a worker explicitly designated to supervise local processes. Set it on one worker, not on the API edge.
...
```sh
curl -fsS -H "Authorization: Bearer $BOARD_API_KEY" \
  "$GEETORUS_URL/api/tool-gateway/audit?companyId=$COMPANY_ID&limit=100" \
...
Local stdio MCP connections introduce a different trust model from remote_http: Geetorus is running a child process under its own credentials. Two questions decide whether this is safe in your deployment:
...
2. **Who controls the template?** Only approved templates baked into the Geetorus build can run. Agents cannot supply arbitrary `command` / `args`.
...
| `authenticated/public` (internet-facing) | Fail closed | Only when one worker is designated trusted by setting `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` and is isolated from the public-facing edge. |

In all modes:

- `remote_http` is the preferred path. If you can replace a local stdio fixture with a remote_http endpoint, do.
- Never set `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` on the same worker that serves public HTTP traffic.
...
- **No CLI surface for tool access yet.** Connections, profiles, policies, approvals, and trust rules are managed via the UI and the REST API only. There is no `geetorusai tool ...` subcommand.
...
- **Endpoint mode (Geetorus as MCP server) is not policy-governed.** Tool access governance applies only to *gateway mode* — Geetorus's own MCP endpoint surface (`/mcp`) uses standard Geetorus auth and is not subject to the profile/policy stack.
...
| Gateway calls | `POST /api/tool-gateway/tools/call` | `X-Geetorus-Tool-Gateway-Token` header; body uses `tool` + `parameters`. Approval-required calls respond `409` with `reasonCode: approval_required` and an `actionRequestId`; the agent retries with `approvedActionRequestId`. |
