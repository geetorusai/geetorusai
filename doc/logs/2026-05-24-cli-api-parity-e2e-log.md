...
Full Geetorus CLI/API parity smoke pass against a disposable local source-tree instance.

## Isolation Contract

- Repo: `/Users/aronprins/Documents/GeetorusAI/geetorus`
- Scratch root: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity`
- `GEETORUS_HOME`: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home`
- `GEETORUS_INSTANCE_ID`: `cli-api-parity`
- `GEETORUS_CONFIG`: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
- `GEETORUS_CONTEXT`: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/context.json`
- `GEETORUS_AUTH_STORE`: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/auth.json`
- `GEETORUS_API_URL`: `http://127.0.0.1:3197`
- `GEETORUS_SERVER_PORT`: `3197`
- `PORT`: `3197`
- `CODEX_HOME`: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/codex-home`
- `CLAUDE_HOME`: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/claude-home`
- `DATABASE_URL`: unset
...
- Command: `sed -n ... geetorus-localdev-runbook.md`, `doc/DEVELOPING.md`, `doc/CLI.md`, `doc/DATABASE.md`, `doc/GOAL.md`, `doc/PRODUCT.md`, `doc/SPEC-implementation.md`, `doc/DEPLOYMENT-MODES.md`, `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`
...
- Actual result: Runbook requires explicit scratch paths, port `3197`, unset database env vars, `pnpm geetorusai onboard --yes --run --bind loopback`, and pre-test isolation checks.
- Status: PASS.
...
- Prerequisites/IDs used: none.
- Expected result: All Geetorus/Codex/Claude paths point under `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` are unset; port `3197` has no listener.
...
- Status: PASS.
- Output summary: No references to `~/.geetorus`, `~/.codex`, `~/.claude`, or `localhost:3100`.
- Follow-up: Start Geetorus with the runbook command.

### 2026-05-24T11:06:45+02:00 - Start isolated instance

- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm geetorusai onboard --yes --run --bind loopback`
- Purpose: Create and start the disposable source-tree Geetorus instance.
...
- Actual result: Server started in `local_trusted/private` mode on `127.0.0.1:3197`, with embedded PostgreSQL data at `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/db`.
- Status: PASS.
...
- Command: `pnpm geetorusai env`; `pnpm geetorusai context show --json`; `curl -sS http://127.0.0.1:3197/api/health`; `find tmp/cli-api-parity/home -maxdepth 4 -type d`
...
- Actual result: Config path, context path, storage path, secrets key path, and DB directory all resolve under `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity`; health returned `status: ok`, version `0.3.1`, `deploymentMode: local_trusted`, `companyDeletionEnabled: true`.
- Status: PASS.
- Output summary: `geetorusai env` redacted here because it prints the generated `GEETORUS_AGENT_JWT_SECRET`. Context existed at the scratch path with an empty `default` profile.
- Follow-up: Set API context and begin CLI parity checks.

### 2026-05-24T11:08:20+02:00 - Basic context/auth/connectivity

- Command: `pnpm geetorusai context set --api-base http://127.0.0.1:3197 --use --json`; `pnpm geetorusai whoami --json`; `pnpm geetorusai company list --json`; `pnpm geetorusai access whoami --json`
...
- Command: `pnpm geetorusai company create --payload-json '{"name":"CLI API Parity Test","description":"Disposable company for CLI API parity testing","goal":"Exercise the CLI API surface end to end"}' --json`; `pnpm geetorusai context set --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm geetorusai company get 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm geetorusai company update 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"description":"Updated by CLI API parity test","budgetMonthlyCents":12345}' --json`
...
- Command: edited `cli/src/commands/client/context.ts`, `cli/src/client/context.ts`, and `cli/src/__tests__/context.test.ts`; `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai context set --api-base http://127.0.0.1:3197 --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm geetorusai context show --json`
...
- Output summary: The assigned `process` adapter agent ran automatically and generated heartbeat runs. The issue later moved to `blocked` via recovery handling because the smoke process printed output without a concrete Geetorus disposition.
...
- Command: `token board create --company-id ... --name cli-parity-board --never-expires --json`; `token board list --json`; `whoami --api-key <board-token> --json`; `token agent create --company-id ... --agent ... --name cli-parity-agent --json`; `token agent list --company-id ... --agent ... --json`; `context set --profile cli-agent --persona agent ... --api-key-env-var-name GEETORUS_API_KEY --json`; `agent me --profile cli-agent --json`; `agent inbox --profile cli-agent --json`; `issue list --profile cli-agent --company-id ... --json`; `company list --profile cli-agent --json`; `token agent revoke ...`; `token board revoke ...`
...
- Command: `kill <geetorus pid on 3197>`; `pnpm geetorusai onboard --yes --run --bind loopback`; `curl http://127.0.0.1:3197/api/health`; `issue tree-hold:get <issue-id> null --json`
...
- Status: PASS.
- Output summary: No real `~/.geetorus`, `~/.codex`, or `~/.claude` paths were used. The server session is currently running under the isolated environment.
...
- Command: `curl -sf http://127.0.0.1:3197/api/health`; `pnpm geetorusai health --json`; `pnpm geetorusai adapter list/get/config-schema/ui-parser/models/model-profiles/detect-model/test-environment ... --json`; `pnpm geetorusai environment list/capabilities/probe-config/create ... --json`; `pnpm geetorusai project-workspace create/list/update/delete ... --json`; `pnpm geetorusai workspace list --company-id ... --json`; `pnpm geetorusai plugin list/examples/ui-contributions/tools/init ... --json`
...
- Command: `pnpm geetorusai asset image:upload --company-id <company-id> --file doc/assets/avatars/zinc.png --namespace cli-parity --alt ... --title ... --json`; `pnpm geetorusai asset content <asset-id> --out tmp/cli-api-parity/artifacts/asset-download.png --json`; `pnpm geetorusai asset logo:upload --company-id <company-id> --file ui/public/favicon-32x32.png --json`; `pnpm geetorusai skill list/create/get/file/file:update/update-status/install-update/delete/scan-projects ... --json`
...
- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm geetorusai environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/GeetorusAI/geetorus"}}' --json`
- Purpose: Verify `BUG-004` against the restarted isolated source-tree server.
- Prerequisites/IDs used: Same scratch env; server restarted with `pnpm geetorusai onboard --yes --run --bind loopback`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `pnpm geetorusai environment create/get/leases/probe/update/delete ... --json`; `pnpm geetorusai plugin install/list/inspect/health/config/jobs/local-folders/ui-contributions/disable/enable/uninstall ... --json`; `pnpm geetorusai secrets list/create/link/declarations/migrate-inline-env ... --json`
- Purpose: Add positive non-local environment coverage, plugin lifecycle coverage, and deeper secrets coverage.
- Prerequisites/IDs used: Company `12e9db4b-f66c-459b-959e-d645002240fb`; bundled plugin path `/Users/aronprins/Documents/GeetorusAI/geetorus/packages/plugins/plugin-workspace-diff`; temporary SSH environment `cc5ae311-13f5-42b8-8044-11065b4e1af0`; temporary plugin install `e8421ed5-c103-4950-afb7-1463a0fbb9c5`; temporary secret `20c74546-7bec-4766-80cd-0b6c57545f7d`.
...
- Command: `pnpm geetorusai secrets update 20c74546-7bec-4766-80cd-0b6c57545f7d --payload-json ... --json`; `pnpm geetorusai secrets rotate 20c74546-7bec-4766-80cd-0b6c57545f7d --value ... --json`; `pnpm geetorusai secrets usage 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm geetorusai secrets access-events 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm geetorusai secrets delete 20c74546-7bec-4766-80cd-0b6c57545f7d --yes --confirm 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm geetorusai secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`
...
- Expected result: `geetorusai health` exists; `geetorusai access whoami` works; `invite test-resolution` has a URL option; `join list --status pending` maps to `pending_approval`; issue help text no longer overstates valid cancel/recovery inputs.
...
- Command: `pnpm geetorusai health --json`; `pnpm geetorusai access whoami --json`; `pnpm geetorusai join list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --status pending --request-type agent --json`; `pnpm geetorusai invite create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{}' --json`; `pnpm geetorusai invite test-resolution <token> --url https://example.com/invite/<token> --json`; `pnpm geetorusai invite revoke <invite-id> --json`; `pnpm geetorusai issue recovery:resolve --help`; `pnpm geetorusai issue interaction:cancel --help`
...
- Prerequisites/IDs used: Isolated `CLAUDE_HOME=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/claude-home`.
...
- Status: PASS.
- Output summary: Mounted `llmRoutes` under `/api` in addition to the existing root mount. Updated available-skill discovery to read `CLAUDE_HOME/skills` when configured, include built-in Geetorus repo skills in `available-skill list`, and allow `available-skill get` for safe listed/built-in skill names.
...
- Command: `pnpm geetorusai llm agent-configuration --json`; `pnpm geetorusai llm agent-icons --json`; `pnpm geetorusai llm agent-configuration:adapter process --json`; `pnpm geetorusai available-skill list --json`; `pnpm geetorusai available-skill get geetorus --json`; `pnpm geetorusai available-skill get cmux --json`; `pnpm geetorusai openapi --json`
...
- Expected result: LLM docs commands pass; built-in Geetorus skills are listed and fetchable; real-user `~/.claude` skills are not listed; `openapi` still documents the unresolved gap if no route exists.
- Actual result: LLM docs commands passed. `available-skill list` returned Geetorus repo skills such as `diagnose-why-work-stopped`; `available-skill get geetorus` returned markdown. `available-skill get cmux` now returns 404 because `cmux` is no longer listed from the real Claude home. `openapi` still returns `404: API route not found`.
- Status: MIXED.
...
- Command: environment echo; `pnpm geetorusai health --json`; `pnpm geetorusai token board list --json`; `pnpm geetorusai token board revoke <redacted-board-token-id> --json`; `pnpm geetorusai token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`; `pnpm geetorusai plugin list --json`; `pnpm geetorusai secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm geetorusai environment list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm geetorusai project-workspace list d32032ce-d95e-4c4e-a942-dd98498025fb --json`; `pnpm geetorusai openapi --json`
...
- Command: `pnpm geetorusai doctor --config <scratch-config>`; `pnpm geetorusai doctor --config <scratch-config> --repair --yes`; `pnpm geetorusai env --config <scratch-config>`; `pnpm geetorusai db:backup --config <scratch-config> --dir tmp/cli-api-parity/artifacts/root-setup/backups --retention-days 1 --filename-prefix cli-parity --json`; `pnpm geetorusai allowed-hostname cli-parity.test --config <scratch-config>`; `pnpm geetorusai auth bootstrap-ceo --config <scratch-config> --force --base-url http://127.0.0.1:3197`; `pnpm geetorusai auth whoami --json`; `pnpm geetorusai routines disable-all --config <scratch-config> --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm geetorusai env-lab doctor --instance cli-api-parity --json`; `pnpm geetorusai env-lab status --instance cli-api-parity --json`
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `GEETORUS_WORKTREES_DIR=tmp/cli-api-parity/worktrees-home pnpm geetorusai worktree:list --json`; `pnpm geetorusai worktree env --config <scratch-config> --json`; `pnpm geetorusai worktree:merge-history --from current --to current --company CLI --dry`; `pnpm geetorusai cloud push --company 12e9db4b-f66c-459b-959e-d645002240fb --dry-run --json`
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; scratch worktree root `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/worktrees-home`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Actual result: `worktree:list` passed and showed the current repo branch `improvement/cli-api-parity` with no Geetorus worktree config. `worktree env --config <scratch-config> --json` passed and printed the scratch `GEETORUS_CONFIG` plus generated env values; the generated JWT secret is intentionally not copied into this log. `worktree:merge-history --from current --to current --company CLI --dry` failed as expected with `Source and target Geetorus configs are the same. Choose different --from/--to worktrees.` `cloud push --dry-run` failed as expected with `Cloud sync is disabled. Enable the cloud sync experimental setting before running geetorusai cloud push.`
...
- Command: `HOME=tmp/cli-api-parity/shell-home GEETORUS_WORKTREES_DIR=tmp/cli-api-parity/worktree-instances pnpm geetorusai worktree:make cli-parity-wt --home <scratch-worktree-home> --from-config <scratch-config> --server-port 3198 --db-port 54331 --seed-mode minimal`; `pkill` only for the runaway scratch install attempt; edited `cli/src/commands/worktree.ts` and `cli/src/__tests__/worktree.test.ts`; `pnpm exec vitest run cli/src/__tests__/worktree.test.ts`; `pnpm --dir cli typecheck`; `geetorusai worktree:cleanup cli-parity-wt --home <scratch-worktree-home> --force`; reran `geetorusai worktree:make ...`; `geetorusai worktree:list --json`; `geetorusai worktree env --config <scratch-worktree-config> --json`; `geetorusai worktree:merge-history --from geetorus-cli-parity-wt --to current --company CLI --dry`
...
- Purpose: Exercise cloud connect/push without a real external Geetorus Cloud stack, finish run subcommands that were not covered by earlier run list/get/events/log checks, and verify `connect` behavior in the non-interactive test runner.
...
- Expected result: Cloud sync can be enabled only in the scratch instance; `cloud connect` stores a fake upstream connection under scratch `GEETORUS_HOME`; `cloud push --dry-run` exports local company data and posts a preview bundle to the fake upstream; run read/control commands return structured results or controlled 404 for a nonexistent workspace operation; `connect` refuses non-interactive execution with guidance.
...
- Expected result: Geetorus remains running on `127.0.0.1:3197`; fake cloud port `3199` is stopped; no active board or agent tokens from the test remain; plugin list is empty; disposable routine is archived; `openapi` remains the one documented unresolved API route gap.
...
- Expected result: `/api/openapi.json` and `geetorusai openapi --json` return a valid OpenAPI 3.0 document with the reference route inventory, including representative CLI/API parity paths such as `/api/companies/{companyId}/agents` and `/api/agents/{id}/keys`.
- Actual result: Focused test and `server` typecheck passed. After restart, direct curl returned `{"openapi":"3.0.0","pathCount":247}`. The CLI command returned `openapi: "3.0.0"`, title `Geetorus API`, `247` paths, `/api/openapi.json` summary `Get the generated OpenAPI document`, and `/api/agents/{id}/keys` POST summary `Create an agent API key`.
...
- Command: Edited `cli/src/commands/client/agent.ts`; ran `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm --silent geetorusai agent instructions-path:update --help`.
...
- Actual result: Install/list/inspect/health/logs/config/jobs/job:runs/job:trigger/webhook/dashboard/bridge:data/data/action/upgrade/disable/enable/uninstall passed. Config commands initially failed until payloads were corrected to `{"configJson":{...}}`; bridge action initially failed until payload used `key` instead of `action`. `bridge:stream` returned expected `Plugin stream bridge is not enabled`. Local-folder calls returned expected validation because the kitchen-sink manifest declares no local folders. `plugin tools` listed `geetorus-kitchen-sink-example:echo`, but `plugin tool:execute` returned `502: worker for plugin "geetorus-kitchen-sink-example" is not running` even though bridge calls to the same plugin worker succeeded.
...
- Command: Edited `server/src/services/plugin-tool-dispatcher.ts`, `server/src/services/plugin-loader.ts`, and `server/src/__tests__/plugin-database.test.ts`; ran `pnpm exec vitest run server/src/__tests__/plugin-database.test.ts`; `pnpm --dir server typecheck`; restarted the isolated server; installed kitchen-sink plugin; `plugin tools`; `plugin tool:execute --payload-json '{"tool":"geetorus-kitchen-sink-example:echo",...}'`; cleanup `plugin uninstall --force`.
...
- Prerequisites/IDs used: Same scratch server and kitchen-sink plugin; tool `geetorus-kitchen-sink-example:echo`.
- Expected result: The listed echo tool dispatches to the running kitchen-sink worker and returns the echo result.
- Actual result: Focused test and server typecheck passed. After restart, `plugin tools` listed `geetorus-kitchen-sink-example:echo`; `plugin tool:execute` returned `content: "CLI parity tool after fix"` and the expected run context. The plugin was uninstalled and `plugin list` returned `[]`.
...
- Expected result: Deleting a webhook routine trigger removes the generated geetorus-managed secret and binding. No active secrets remain from parity cleanup.
...
- Actual result: Final handoff stated that the full isolated CLI/API parity pass completed and was committed, the scratch Geetorus server was still running on `http://127.0.0.1:3197`, and goal usage was 2,625,855 tokens over about 2h 23m.
- Status: PASS.
- Output summary:
...
- Final paths:
  - Scratch home: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home`
  - Config: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
  - Context: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/context.json`
  - Auth store: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/auth.json`
  - Embedded DB: `/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/db`, active on pg port `54330`
...
```sh
export GEETORUS_HOME=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home
export GEETORUS_INSTANCE_ID=cli-api-parity
export GEETORUS_CONFIG=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json
export GEETORUS_CONTEXT=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/context.json
export GEETORUS_AUTH_STORE=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/auth.json
export GEETORUS_API_URL=http://127.0.0.1:3197
export GEETORUS_SERVER_PORT=3197
export PORT=3197
export CODEX_HOME=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/codex-home
export CLAUDE_HOME=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/claude-home
unset DATABASE_URL DATABASE_MIGRATION_URL
pnpm geetorusai health --json
```
...
- Command: `pnpm add @asteasolutions/zod-to-openapi@7.3.4 --filter @geetorusai/server`; replaced `server/src/routes/openapi.ts` inventory stub with the schema-backed generator from `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`; added route wrapper exports; tightened `server/src/__tests__/openapi-routes.test.ts`; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; `pnpm --dir server typecheck`; restarted isolated server with the scratch env; `pnpm geetorusai openapi --json`.
...
- Prerequisites/IDs used: Isolated scratch server restarted on `127.0.0.1:3197`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` unset; `GEETORUS_HOME`, `GEETORUS_CONFIG`, `GEETORUS_CONTEXT`, `GEETORUS_AUTH_STORE`, `CODEX_HOME`, and `CLAUDE_HOME` all under `tmp/cli-api-parity`.
- Expected result: `/api/openapi.json` and `geetorusai openapi --json` return OpenAPI 3.0 with schema-backed request bodies, security schemes, public-operation security overrides, and create-operation `201` responses.
...
- Command: `pnpm geetorusai health --json`; `pnpm geetorusai openapi --json`; `lsof -nP -iTCP:3197 -sTCP:LISTEN`; `git status --short --branch`, all with the scratch `GEETORUS_*`, `CODEX_HOME`, and `CLAUDE_HOME` environment and with `DATABASE_URL`/`DATABASE_MIGRATION_URL` unset.
...
- Command: Stopped the foreground scratch server process; started the same runbook command in detached screen session `geetorus-cli-parity`; verified `pnpm geetorusai health --json`; checked `lsof -nP -iTCP:3197 -sTCP:LISTEN`; checked `screen -ls`.
- Purpose: Leave the disposable instance running without tying it to the active tool session.
- Prerequisites/IDs used: Same scratch env and unset database variables; detached screen session `91568.geetorus-cli-parity`.
- Expected result: Server continues running on non-default port `3197` with the same isolated home/config/context/auth paths.
- Actual result: Health returned `status:"ok"` with deployment mode `local_trusted`; `node` PID `91583` is listening on `127.0.0.1:3197`; `screen -ls` shows detached session `91568.geetorus-cli-parity`.
- Status: PASS.
- Output summary: Detached server log is `tmp/cli-api-parity/artifacts/caveat-followup/server-screen.log`.
- Follow-up: Manual continuation can use the same env block and `screen -r geetorus-cli-parity` to inspect the server session.
...
- Expected result: The webhook trigger's generated geetorus-managed secret and binding are removed when the trigger is deleted.
...
- Reproduction command: Install `packages/plugins/examples/plugin-kitchen-sink-example`, then run `pnpm geetorusai plugin tool:execute --payload-json '{"tool":"geetorus-kitchen-sink-example:echo","parameters":{"message":"CLI parity tool"},"runContext":{"companyId":"<company-id>","projectId":"<project-id>","agentId":"<agent-id>","runId":"<run-id>"}}' --json`.
- Expected result: The listed tool dispatches to the running kitchen-sink worker and returns a `ToolResult`.
- Actual result: `plugin tools` listed `geetorus-kitchen-sink-example:echo`, and bridge data/action calls to the same plugin worker succeeded, but `tool:execute` returned `502: Cannot execute tool ... worker for plugin "geetorus-kitchen-sink-example" is not running`.
...
- Reproduction command: `pnpm geetorusai token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`.
...
- Reproduction command: `HOME=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/shell-home pnpm geetorusai worktree:make cli-parity-wt --home /Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/worktree-instances --from-config /Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --server-port 3198 --db-port 54331 --seed-mode minimal`.
...
- Suspected cause: The CLI did not reuse the pnpm executable that launched the current Geetorus command, so dependency installation was subject to PATH/shim behavior under an overridden `HOME`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/worktree.test.ts`; `pnpm --dir cli typecheck`; live isolated `worktree:cleanup --force` for the partial worktree; live isolated `worktree:make ... --seed-mode minimal`; `worktree:list --json`; `worktree env --config <scratch-worktree-config> --json`; `worktree:merge-history --from geetorus-cli-parity-wt --to current --company CLI --dry`.
- Remaining risk: Low. If Geetorus is launched outside pnpm, dependency installation still falls back to PATH lookup as before.

### BUG-008 - `configure --section <invalid>` printed an error but exited 0

- Status: Fixed and live-verified.
- Severity: Low command UX/scripting bug.
- Reproduction command: `pnpm geetorusai configure --config /Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --section invalid-section`.
...
- Status: Fixed.
- Severity: High for isolated CLI testing; a non-default `apiBase` can be silently removed and later commands may fall back to `http://localhost:3100` if `GEETORUS_API_URL` is absent.
- Reproduction command: `pnpm geetorusai context set --api-base http://127.0.0.1:3197 --use --json`; then `pnpm geetorusai context set --company-id <company-id> --use --json`; then `pnpm geetorusai context show --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai context show --json`.
...
- Reproduction command: `pnpm geetorusai access whoami --json`.
...
- Fix summary: Added `geetorusai access whoami` as an alias for the existing top-level `whoami` command.
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai access whoami --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm geetorusai issue interaction:accept <issue-id> <request-confirmation-interaction-id> --json`.
...
- Reproduction command: `pnpm geetorusai issue tree-hold:get <issue-id> null --json` or `pnpm geetorusai issue tree-hold:release <issue-id> null --json`.
...
- Reproduction command: `pnpm geetorusai issue interaction:cancel <issue-id> <request-confirmation-interaction-id> --reason "..." --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai issue interaction:cancel --help`.
...
- Status: Fixed help text.
- Severity: Low command UX drift.
- Reproduction command: `pnpm geetorusai issue recovery:resolve <issue-id> --action-id <action-id> --outcome restored --source-issue-status blocked --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai issue recovery:resolve --help`.
...
- Severity: Low command UX drift.
- Reproduction command: `pnpm geetorusai agent instructions-path:update <process-agent-id> --payload-json '{"path":"docs/cli-parity.md"}' --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai agent instructions-path:update --help`.
...
- Reproduction command: `pnpm geetorusai invite test-resolution <invite-token> --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai invite test-resolution <token> --url https://example.com/invite/<token> --json`.
...
- Status: Fixed and live-verified.
- Severity: Low command UX drift.
- Reproduction command: `pnpm geetorusai join list --company-id <company-id> --status pending --request-type agent --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai join list --company-id <company-id> --status pending --request-type agent --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm geetorusai openapi --json`; `pnpm geetorusai available-skill get cmux --json`; `pnpm geetorusai llm agent-configuration --json`; `pnpm geetorusai llm agent-icons --json`; `pnpm geetorusai llm agent-configuration:adapter process --json`.
...
- Fix summary: Mounted LLM docs routes under `/api`; made available-skill discovery honor `CLAUDE_HOME`, include built-in Geetorus repo skills, and fetch safe skill markdown consistently; added `/api/openapi.json`, then upgraded it from the initial path inventory to the schema-backed `OpenAPIRegistry`/`OpenApiGeneratorV3` implementation from the parity reference.
- Verification command: `pnpm exec vitest run server/src/__tests__/llms-routes.test.ts cli/src/__tests__/access-parity.test.ts`; `pnpm --dir server typecheck`; `pnpm --dir cli typecheck`; live `llm` and `available-skill` commands after restart; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; live `curl http://127.0.0.1:3197/api/openapi.json`; live `pnpm geetorusai openapi --json`; follow-up live schema-backed `geetorusai openapi --json`.
...
- Reproduction command: `CLAUDE_HOME=/Users/aronprins/Documents/GeetorusAI/geetorus/tmp/cli-api-parity/claude-home pnpm geetorusai available-skill list --json`.
- Expected result: Skill discovery uses the isolated Claude home or built-in repo skills only.
- Actual result: Before the fix, the list included `cmux` from the real user Claude skills home, and `available-skill get cmux` failed because only a hardcoded Geetorus subset was fetchable.
- Suspected cause: Server code read `HOME/.claude/skills` directly and did not add built-in Geetorus skills unless they were present in Claude's skills directory.
- Files changed: `server/src/routes/access.ts`, `doc/bugs/2026-05-24-cli-api-parity-e2e-log.md`.
- Fix summary: Use `CLAUDE_HOME/skills` when `CLAUDE_HOME` is set, include built-in Geetorus skills in catalog output, and resolve safe skill markdown from both Claude and Geetorus skills directories.
- Verification command: live `available-skill list`, `available-skill get geetorus`, and `available-skill get cmux` after restarting the isolated server.
- Remaining risk: Low; this is runtime environment-sensitive and covered by live isolated verification.

### MISMATCH-008 - `geetorusai health` is not registered

- Status: Fixed and live-verified.
- Severity: Low command/API parity gap.
- Reproduction command: `pnpm geetorusai health --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm geetorusai health --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm geetorusai environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/GeetorusAI/geetorus"}}' --json`.
...
- Status: Fixed and live-verified.
- Severity: Medium CLI/API parity gap.
- Reproduction command: `pnpm geetorusai secrets --help` did not expose commands for `PATCH /api/secrets/:id`, `POST /api/secrets/:id/rotate`, `GET /api/secrets/:id/usage`, `GET /api/secrets/:id/access-events`, or `DELETE /api/secrets/:id`.
