# Geetorus Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Geetorus's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Geetorus app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Geetorus HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@geetorusai/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Geetorus plugin design is based on the following assumptions:

1. Geetorus is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Geetorus business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Geetorus core.
...
2. Let plugins add major capabilities without editing Geetorus core.
...
4. Depend on project-local plugin folders such as `.geetorus/plugins`.
...
The single Geetorus deployment an operator installs and controls.

### 5.2 Company

A first-class Geetorus business object inside the instance.
...
A trusted in-process extension loaded directly by Geetorus core.
...
An installable instance-wide extension package loaded through the Geetorus plugin runtime.
...
Geetorus has two extension classes.
...
Geetorus already has a concrete workspace model:
...
Plugins live under the Geetorus instance directory.

Suggested layout:

- `~/.geetorus/instances/default/plugins/package.json`
- `~/.geetorus/instances/default/plugins/node_modules/`
- `~/.geetorus/instances/default/plugins/.cache/`
- `~/.geetorus/instances/default/data/plugins/<plugin-id>/`
...
Geetorus should add CLI commands:

- `pnpm geetorusai plugin list`
- `npx geetorusai plugin install <package[@version]>`
- `npx geetorusai plugin uninstall <plugin-id>`
- `npx geetorusai plugin upgrade <plugin-id> [version]`
- `npx geetorusai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@geetorus/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface GeetorusPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
- `minimumHostVersion` is preferred, with `minimumGeetorusVersion` retained for
...
Plugins may contribute tools that Geetorus agents can use during runs.
...
- Geetorus server starts one worker process per installed plugin
...
Receives one typed Geetorus domain event.
...
export function definePlugin(definition: PluginDefinition): GeetorusPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: GeetorusPluginManifestV1;
  config: {
...
6. For recurring business workflows that should create visible Geetorus work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@geetorusai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@geetorusai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@geetorusai/plugin-sdk/ui`)
...
Geetorus-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Geetorus navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@geetorusai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@geetorusai/plugin-sdk/ui`
...
The `@geetorusai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that stores the shared `{ type: "secret_ref", secretId, version? }` object shape and resolves through the Geetorus secret provider system rather than a plain text input
...
1. Core Geetorus data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Geetorus objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Geetorus product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Geetorus later decides git state is core product data, it should become a first-party table too
...
3. Secret refs resolve through the existing Geetorus secret provider system using explicit `companyId`.
...
6. The operator may force-purge immediately via CLI: `npx geetorusai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Geetorus server. This is a normative requirement, not optional.
...
### 27.1 `@geetorusai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@geetorusai/plugin-test-harness";
...
For developing a plugin against a running Geetorus instance:

- The operator installs the plugin from a local path: `npx geetorusai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-geetorus-plugin`) that scaffolds:

- `package.json` with correct `geetorusPlugin` keys
...
- `@geetorus/plugin-workspace-files`
- `@geetorus/plugin-terminal`
- `@geetorus/plugin-git`
- `@geetorus/plugin-linear`
- `@geetorus/plugin-github-issues`
- `@geetorus/plugin-grafana`
- `@geetorus/plugin-runtime-processes`
- `@geetorus/plugin-stripe`
...
- `@geetorusai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@geetorusai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@geetorusai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@geetorusai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@geetorusai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@geetorusai/plugin-sdk/ui`
...
- `@geetorusai/plugin-test-harness`
- `create-geetorus-plugin` starter template
...
Geetorus should not implement a generic in-process hook bag modeled directly after local coding tools.

Geetorus should implement:
...
That is the complete target design for the Geetorus plugin system.
