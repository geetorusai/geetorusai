...
This guide describes the current, implemented way to create a Geetorus plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Geetorus app.
...
- The host provides a small shared React component kit through
  `@geetorusai/plugin-sdk/ui`; use it for common Geetorus controls before
...
Geetorus owns inline markdown rendering; plugins must not return React, HTML,
...
```bash
geetorusai plugin init @yourscope/plugin-name --output /absolute/path/to/plugin-repos
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@geetorusai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@geetorusai/plugin-sdk` from the local Geetorus checkout into a `.geetorus-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/geetorus/packages/plugins/sdk` if you have more than one Geetorus checkout.

## Local development workflow

See the short [Local Plugin Development guide](./LOCAL_PLUGIN_DEVELOPMENT.md) for the full happy path (`pnpm dev` → `geetorusai plugin install <absolute-path>` → `geetorusai plugin list`) and reload semantics.
...
The in-repo [`@geetorusai/plugin-createos`](../../packages/plugins/sandbox-providers/createos/README.md)
...
## Managed Geetorus resources

Plugins that provide durable Geetorus business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Geetorus agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Geetorus issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Geetorus managed agents.
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Geetorus heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
```ts
import { definePlugin } from "@geetorusai/plugin-sdk";
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Geetorus issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Geetorus records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@geetorusai/plugin-sdk/ui`
...
Use shared components from `@geetorusai/plugin-sdk/ui` when the plugin needs a
Geetorus-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Geetorus UX patterns to reduce drift and
...
Use `FileTree` from `@geetorusai/plugin-sdk/ui` whenever the plugin only needs
