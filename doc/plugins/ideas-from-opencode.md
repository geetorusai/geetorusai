...
Geetorus V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Geetorus-shaped design.

Assumption for this document: Geetorus is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Geetorus objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Geetorus.

The main conclusion is:

- Geetorus should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Geetorus should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Geetorus should use multiple extension classes instead of one generic plugin bag:
...
If Geetorus does this well, the examples you listed become straightforward:
...
Relevant Geetorus files reviewed for current extension seams:
...
It is too dangerous for Geetorus core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Geetorus — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Geetorus should absolutely do this.
...
Geetorus should copy this discipline.
...
Geetorus should adopt this style for plugin actions, automations, and UI schemas.
...
Geetorus can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Geetorus too.

## What Geetorus Should Not Copy Directly
...
Geetorus is a control plane for an operator-managed instance with company objects.
...
Geetorus is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Geetorus should not auto-load arbitrary code from a workspace repo like `.geetorus/plugins` or project directories.
...
For Geetorus, equivalent hooks into:
...
Geetorus should not allow plugins to silently replace:
...
For Geetorus it would be risky because it combines:
...
Geetorus should require an explicit operator install step.

## Why Geetorus Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Geetorus |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Geetorus instance |
...
That means Geetorus should borrow the good ideas from `opencode` but use a stricter architecture.

## Geetorus Already Has Useful Pre-Plugin Seams

Geetorus has several extension-like seams already:
...
This is good news.
Geetorus does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Geetorus Plugin Model
...
Geetorus already has this pattern implicitly:
...
- `plugin` means integration code that talks to Geetorus through a typed plugin protocol and capability model
...
- invoke explicit Geetorus actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@geetorusai/plugin-sdk/ui`) with design tokens and common components.
...
Geetorus should treat plugin installation as a global instance-level action.

Examples:

- install `@geetorus/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Geetorus objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Geetorus already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Geetorus too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@geetorus/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@geetorusai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Geetorus instance.

A `create-geetorus-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Geetorus achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Geetorus server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Geetorus's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@geetorusai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@geetorusai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Geetorus

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@geetorusai/plugin-sdk";

export default definePlugin({
  id: "@geetorus/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Geetorus entities
    });
    ...
    ctx.events.on("plugin.@geetorus/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@geetorusai/plugin-sdk/ui";
...
Any Geetorus plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Geetorus objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@geetorus/plugin-linear`)
...
- reuse core tables when the data is clearly part of Geetorus itself
...
If a concept is becoming part of Geetorus's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Geetorus itself
...
### 3. Scope plugin data to Geetorus objects before adding custom schemas

A lot of plugin data naturally hangs off existing Geetorus objects:
...
If Geetorus eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@geetorus/plugin-workspace-files`
...
Package idea: `@geetorus/plugin-terminal`
...
Package idea: `@geetorus/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Geetorus.
- Board reviews the diff after a run without leaving Geetorus.
...
- optional `events.emit` (e.g. `plugin.@geetorus/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@geetorus/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@geetorus/plugin-linear`

This plugin syncs Geetorus work with Linear. It is useful for:

- importing backlog from Linear
- linking Geetorus issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Geetorus and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Geetorus.
- Geetorus issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@geetorus/plugin-github-issues`

This plugin syncs Geetorus issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Geetorus issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Geetorus task governance
...
- Plugin settings:
  ...
  - whether new Geetorus issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Geetorus issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Geetorus.
- GitHub webhooks update status/comment state in Geetorus.
- A PR is linked back to the Geetorus issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@geetorus/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@geetorus/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Geetorus. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Geetorus issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Geetorus plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Geetorus dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Geetorus issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Geetorus core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@geetorus/plugin-runtime-processes`
...
Package idea: `@geetorus/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Geetorus. It is useful for:
...
- Failed payment spikes or churn events can generate Geetorus issues for follow-up.
...
- Stripe data should stay additive to Geetorus core
...
- plugin UI bundle loading, host bridge, `@geetorusai/plugin-sdk/ui`
...
- `@geetorusai/plugin-test-harness` and `create-geetorus-plugin` starter template
...
Geetorus should not implement "an OpenCode-style generic in-process hook system."
Geetorus should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Geetorus
...
8. Ship `@geetorusai/plugin-test-harness` and `create-geetorus-plugin` starter template.
