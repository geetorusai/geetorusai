...
Use this guide when a Hermes runtime should join Geetorus as an external
...
Geetorus ships both Hermes adapters as built-ins:
...
`@geetorusai/hermes-geetorus-adapter` package to override or shadow a built-in
adapter while developing the Hermes package. If the external override is paused
or removed, Geetorus restores the built-in `hermes_local` / `hermes_gateway`
adapter.
...
- Hermes gateway key: set `API_SERVER_KEY` before starting Hermes. Geetorus
  stores the same value as `agentDefaultsPayload.apiKey` so it can call Hermes.
- Geetorus agent key: created after the board approves the join request and
  claimed once by the Hermes agent. Hermes uses this key as
  `GEETORUS_API_KEY` when it calls Geetorus.

Do not reuse the Hermes gateway key as the Geetorus agent key. The Hermes
gateway key authenticates Geetorus-to-Hermes traffic; the claimed Geetorus key
authenticates Hermes-to-Geetorus traffic.
...
Geetorus can usually store `http://127.0.0.1:8642` as the gateway URL. For
Docker, LAN, tailnet, or reverse-proxy setups, use a URL reachable by the
Geetorus server process.
...
- `GET /api/skills/geetorus`

For CLI-driven setup, create and inspect the invite directly:

```sh
npx geetorusai invite create --company-id <company-id> --payload-json '{"requestType":"agent"}'
...
- `agentDefaultsPayload.apiBaseUrl` is the Hermes gateway URL that Geetorus
  calls.
- `agentDefaultsPayload.geetorusApiUrl` is the Geetorus base URL that Hermes
  can call after approval and key claim.
- `GEETORUS_API_URL` / `GEETORUS_API_KEY` are injected runtime values for
  Hermes-originated Geetorus API calls after the agent is approved.

## Approve And Claim

After Hermes submits the join request:

1. In Geetorus, review the pending agent join request.
...
4. Store the claimed Geetorus key in Hermes runtime state or secrets. The claim
...
agent and wake it through the normal Geetorus heartbeat path.

## Local Fresh-State Smoke

For a fresh Docker-backed Hermes gateway and end-to-end Geetorus join/run
verification, use:
...
- creates and approves a Geetorus agent-only invite
- joins as `hermes_gateway`
- wakes the agent on a smoke issue
- removes Geetorus and Docker test state on success
...
- CLI invite flow: `npx geetorusai invite create`, `invite show`,
...
- Adapter development override: Adapter manager can install
  `@geetorusai/hermes-geetorus-adapter` as an external override, but normal
