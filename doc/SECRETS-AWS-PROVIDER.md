...
Operational contract for the hosted `aws_secrets_manager` secret provider used by Geetorus Cloud.

## Scope

- Hosted provider for Geetorus-managed company and user-scoped secrets when Geetorus Cloud runs on AWS.
- Source of truth for secret values is AWS Secrets Manager, not Postgres.
- Geetorus stores only metadata needed for ownership, bindings, version selection, audit, and runtime resolution.
- AWS provider bootstrap credentials are deployment/runtime credentials, not Geetorus-managed company secrets.
- Remote import for existing AWS secrets is metadata-only. Preview/import uses
  AWS inventory metadata and creates Geetorus external references; it does not
  copy plaintext into Geetorus.
...
The AWS provider has a chicken-and-egg boundary: Geetorus cannot use
`company_secrets` to unlock the AWS provider that stores those secrets. The
initial AWS trust must exist before the Geetorus server starts.

Allowed bootstrap locations:

- Infrastructure IAM or workload identity attached to the Geetorus server
  runtime.
- Process environment or orchestrator secret store used to start the Geetorus
  server.
...
keys into the Geetorus board UI. Do not store those bootstrap keys in
`company_secrets`.

## Geetorus Cloud Bootstrap

Geetorus Cloud must provision the AWS backing resources before any board user
can create AWS-backed company secrets:

1. Create or select the deployment KMS key.
2. Create the Geetorus server runtime role for the deployment.
...
4. Configure the server runtime with the non-secret provider environment
   variables below.
5. Run `geetorusai doctor` or the provider health endpoint from the deployed
...
Once this is in place, the board UI can create Geetorus-managed AWS secrets and
Geetorus will write them under the deployment/company namespace.
...
```sh
aws sso login --profile geetorus-dev
AWS_PROFILE=geetorus-dev \
GEETORUS_SECRETS_PROVIDER=aws_secrets_manager \
GEETORUS_SECRETS_AWS_REGION=us-east-1 \
GEETORUS_SECRETS_AWS_DEPLOYMENT_ID=dev-local \
GEETORUS_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-... \
pnpm dev
```
...
should not be written to Geetorus config, committed to `.env` files, stored in
`company_secrets`, or used as the default Geetorus Cloud bootstrap path.
...
GEETORUS_SECRETS_PROVIDER=aws_secrets_manager
GEETORUS_SECRETS_AWS_REGION=us-east-1
GEETORUS_SECRETS_AWS_DEPLOYMENT_ID=prod-us-1
GEETORUS_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-...
```
...
```sh
GEETORUS_SECRETS_AWS_PREFIX=geetorus
GEETORUS_SECRETS_AWS_ENVIRONMENT=production
GEETORUS_SECRETS_AWS_PROVIDER_OWNER=geetorus
GEETORUS_SECRETS_AWS_ENDPOINT=
GEETORUS_SECRETS_AWS_DELETE_RECOVERY_DAYS=30
```

Naming convention for Geetorus-managed secrets:

```text
geetorus/{deploymentId}/{companyId}/{secretKey}
```

Tag set for Geetorus-managed secrets:

- `geetorus:managed-by=geetorus`
- `geetorus:provider-owner=<owner tag>`
- `geetorus:deployment-id=<deployment id>`
- `geetorus:company-id=<company id>`
- `geetorus:secret-key=<secret key>`
- `geetorus:environment=<environment tag>`

When the user-secret service creates Geetorus-managed AWS values, keep them
...
```text
geetorus-ext/<environment>/<company-id>/user-secrets/<definition-key>/<opaque-owner-id>
```

Geetorus stores the external ref and provider version as metadata. Treat those
...
- One Geetorus app role per deployment.
- One deployment-scoped KMS key per deployment at launch.
- Future per-company KMS keys remain compatible because Geetorus stores provider refs and version metadata separately from values.
...
```text
arn:aws:secretsmanager:<region>:<account-id>:secret:geetorus/<deployment-id>/*
```
...
- Prefer workload identity / role-based auth. Do not store AWS credentials inline in Geetorus config.
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:<region>:<account-id>:secret:geetorus/<deployment-id>/*"
    },
    {
      "Sid": "GeetorusDeploymentKms",
      "Effect": "Allow",
      "Action": [
        "kms:Encrypt",
        "kms:Decrypt",
...
- Geetorus-managed secrets may be deleted only by Geetorus or an operator with equivalent break-glass access.
- External references may resolve through Geetorus runtime, but Geetorus should not delete the external secret resource.
...
```json
{
  "Sid": "GeetorusRemoteSecretInventory",
  "Effect": "Allow",
...
operator-approved external prefixes that Geetorus is allowed to consume:

```json
{
  "Sid": "GeetorusResolveImportedExternalReferences",
  "Effect": "Allow",
...
permissions scoped to `geetorus/<deployment-id>/*`; do not broaden them for
remote import.

The same rule applies to user-scoped external refs. Geetorus derives the
...
role can read a broad external prefix, that access is broader than Geetorus's
...
- Prefer one Geetorus runtime role per environment/account.
...
Geetorus also blocks importing refs under its own managed namespace as
external references. Use the Geetorus-managed flow for
`geetorus/{deploymentId}/{companyId}/{secretKey}` resources.

## Existing AWS Secrets

V1 keeps existing AWS Secrets Manager entries as **linked external references**, not adopted
Geetorus-managed resources.

Use the Geetorus-managed flow when Geetorus should create and rotate the value. The AWS
...
```text
/geetorus-bench/anthropic_api_key
```

In that mode Geetorus stores only the path or ARN, resolves it at runtime, and records
redacted access events. Operators rotate the actual value in AWS. Update the Geetorus
reference only when the AWS path, ARN, or pinned provider version changes.

Geetorus does not currently offer an "adopt existing AWS secret" flow that takes over future
`PutSecretValue` writes for an arbitrary existing secret. Adding that later requires explicit
confirmation UX, scope validation, expected Geetorus tags, and security/cloud-ops review.

## Data Custody

- Geetorus stores `externalRef`, `providerVersionRef`, provider id, fingerprint hash, status, and binding metadata.
- Geetorus does not store AWS secret plaintext in `company_secret_versions.material`.
- Runtime resolution fetches the value from AWS only when a bound consumer needs it.

## Rotation Runbook

Manual Geetorus-managed rotation:

1. Write the new value through the Geetorus secret rotate flow.
2. Geetorus creates a new AWS secret version with `PutSecretValue`.
3. Geetorus records the new `providerVersionRef` in `company_secret_versions`.
4. Re-run or restart affected workloads that consume `latest`, or pin consumers to a specific Geetorus version before rollout when you need staged release safety.

Guidance:

- Prefer pinned Geetorus secret versions for risky rollouts.
...
- Geetorus database metadata for secret ownership, bindings, status, and provider version refs.
...
1. Restore Geetorus database metadata.
...
4. Confirm the Geetorus runtime role can call `GetSecretValue` on the restored
   managed prefix and approved external prefixes.
5. Confirm the role still has decrypt access to the CMKs referenced by
   `GEETORUS_SECRETS_AWS_KMS_KEY_ID` and by any external user-secret values.
...
1. Stop or pause affected Geetorus runs.
2. Audit recent Geetorus secret access events for impacted secret ids and consumers.
...
4. Rotate impacted secrets in AWS through Geetorus-managed versioning.
...
- `GEETORUS_SECRETS_PROVIDER=aws_secrets_manager`
- The required `GEETORUS_SECRETS_AWS_*` environment variables set.

Suggested smoke:

1. Create a test secret through the Geetorus board or API under a throwaway company.
2. Confirm the resulting AWS secret name matches `geetorus/{deploymentId}/{companyId}/{secretKey}`.
3. Rotate the secret once and confirm a new `providerVersionRef` appears in Geetorus metadata.
