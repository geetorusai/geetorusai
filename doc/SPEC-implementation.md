# Geetorus V1 Implementation Spec
...
Geetorus V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.geetorus/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.geetorus/instances/default/data/storage` (`local_disk`)
...
- `runtime_config` jsonb not null default `{}`; contains Geetorus runtime policy such as heartbeat scheduling and debug settings
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Geetorus runtime-owned keys are injected

Routine execution issues add a routine-scoped env overlay after project env and before Geetorus runtime-owned keys. Routine env uses the same secret-aware binding format, is stored on `routines.env`, is snapshotted in routine revisions, and resolves secret refs against the routine binding target so routine-owned secrets do not require direct bindings on the executing agent.
...
- V1 attachment serving contract:
  ...
  - Script-capable content such as HTML is always served as an attachment with `X-Content-Type-Options: nosniff` and a sandboxed, deny-by-default CSP; it is never rendered inline on the Geetorus origin.
...
- Attachment-backed artifact work products use `type: "artifact"`, `provider: "geetorus"`, and metadata with `attachmentId`, `contentType`, `byteSize`, `contentPath`, `openPath`, `downloadPath`, and optional `originalFilename`.
...
- heartbeat finalization evaluates liveness from persisted Geetorus state; an issue cannot remain healthy `in_progress` solely because the exiting heartbeat started a local/background watcher
...
- when Geetorus cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
Geetorus V1 keeps a company-scoped visibility model as the default because centralized authorization and scoped work-object controls are not yet a core V1 control surface.
...
| Company skills | Open to authenticated company agents; core enforces invariants and any stored restriction policy | Geetorus EE policy editor, protected-skill controls, presets, simulation, and policy audit UX |
...
Core Skill Studio and Geetorus EE MUST treat those codes as hard platform failures, not as prompts to loosen policy.
...
- Export MUST include explicit skill policy configuration and retained legacy grants in `.geetorus.yaml`, never secret values or environment-specific paths. An unconfigured company exports no synthetic restriction.
- If Geetorus EE is unavailable or removed, core continues to enforce stored policies and expose the policy API. Normal skill work remains available under the open default; explicit denials use core remediation text rather than a broken EE-only link.
...
On a Geetorus Cloud-managed instance, `POST /companies` returns `403` with
...
Geetorus coordination without disabling its sandbox; explicit operator
...
autonomous turns continue. The Geetorus runner's persistent ACP backend opts
...
- if unset, auto-use embedded PostgreSQL under `~/.geetorus/instances/default/db`
...
- public template marketplace integration (GeeTorus)
...
- `.geetorus.yaml` sidecar for Geetorus-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.geetorus.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Geetorus routine fidelity in `.geetorus.yaml`
- Geetorus imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.geetorus.yaml`
- export never includes secret values; env inputs are reported as portable declarations instead
- export preserves explicit company skill policy and retained legacy skill grants in `.geetorus.yaml`; absence of policy remains the open default
