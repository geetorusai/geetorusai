...
Geetorus ships core trust preset names so containment decisions are enforced in
...
scope for risky access, Geetorus fails closed.
...
agent can read or mutate through the Geetorus API and prevents raw untrusted
...
Managed `low_trust_review` runs fail closed unless Geetorus can enforce the
runtime boundary:
...
local review, but Geetorus-managed low-trust execution requires a sandboxed
