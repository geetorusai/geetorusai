...
Geetorus supports two runtime modes:
...
Geetorus now treats **bind** as a separate concern from auth:
...
- Better Auth request rate limiting is off by default for private mode to keep local/LAN repair loops from locking out the operator; set `GEETORUS_AUTH_RATE_LIMIT_ENABLED=true` to opt in
...
- Better Auth request rate limiting is on by default; set `GEETORUS_AUTH_RATE_LIMIT_ENABLED=false` only when an explicit front-door limiter covers the deployment
- recommended bind is `loopback` behind a reverse proxy; direct `lan/custom` is advanced
- local stdio MCP runtime slots fail closed by default; set `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` only when a trusted worker/runtime host is configured to supervise those processes. Remote HTTP MCP remains the preferred public-hosted path.

### Geetorus Cloud warm-pool identity

A Cloud-managed warm-pool process initially boots under a `pool-*` origin. It
receives only Cloud's public verification set in
`GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS`. Before Cloud activates a claimed stack,
the existing server-to-server health request carries a short-lived Ed25519 JWS
that binds the immutable `GEETORUS_CLOUD_STACK_ID`, pool claim, previous
origin, canonical HTTPS origin, and slug. Geetorus verifies and persists that
...
The Harness signing private key is never present in Geetorus, browsers, or
...
`GEETORUS_PUBLIC_URL` and do not participate in this protocol.
...
npx geetorusai run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Geetorus emits a startup warning with a one-time high-entropy claim URL.
...
For `authenticated/private`, Geetorus supports a browser-first setup path:

1. open the Geetorus URL from the private network or appliance UI
2. sign in or create a Geetorus account
