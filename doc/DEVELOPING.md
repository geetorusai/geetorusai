...
The PR caller uses `geetorusai/geetorus/.github/workflows/pr-trusted.yml@master`.
The AWS runner group `geetorus-public-pr` must allow
`geetorusai/geetorus/.github/workflows/pr-trusted.yml@refs/heads/master`.
...
`pnpm dev` and `pnpm dev:once` are now idempotent for the current repo and instance: if the matching Geetorus dev runner is already alive, Geetorus reports the existing process instead of starting a duplicate.

To run against a separate local state root, pass `--data-dir`. The dev runner
translates it to an isolated `GEETORUS_HOME` before migration checks or server
startup, so embedded PostgreSQL and other default instance state live under that
directory:

```sh
pnpm dev --data-dir ./tmp/geetorus-dev
```
...
```sh
pnpm dev:list --data-dir ./tmp/geetorus-dev
pnpm dev:stop --data-dir ./tmp/geetorus-dev
```
...
These run the `@geetorusai/ui` Storybook on port `6006` and build the static output to `ui/storybook-static/`.
...
or inject external font CSS to make Geetorus text render correctly.

Font assets live in Vite's public directory so `pnpm --filter @geetorusai/ui build`
...
Primary-instance rebuilds that restart `geetorus.service` can request one-shot live-run adoption instead of using the normal graceful shutdown drain. Before restarting the service, write the marker from the newly staged app with the current service PID:

```sh
old_main_pid="$(systemctl show geetorus.service -p MainPID --value)"
pnpm --filter @geetorusai/server exec tsx ../scripts/request-hot-restart.ts --server-pid "$old_main_pid"
systemctl restart geetorus.service
```
...
the process now using its PID. Geetorus refuses to create a new request without
...
```sh
old_main_pid="$(systemctl show geetorus.service -p MainPID --value)"
pnpm --filter @geetorusai/server exec tsx ../scripts/request-hot-restart.ts \
...
```sh
GEETORUS_API_BASE="${GEETORUS_API_URL:-http://127.0.0.1:3100}"
GEETORUS_API_BASE="${GEETORUS_API_BASE%/api}"
curl -fsS "$GEETORUS_API_BASE/api/heartbeat-runs/$RUN_ID" \
...
jq -e --arg run "$RUN_ID" \
  ...
  "$GEETORUS_HOME/hot-restart-report.json"
```
...
```sh
pnpm geetorusai auth bootstrap-ceo
```
...
>
> The first-run experience often starts with `npx geetorusai onboard --yes` (before you have a repo checkout). If your global `~/.npmrc` sets `registry` to a private registry (for example GitHub Packages), `npx` may try to resolve `geetorusai` from that private registry and fail with `E404`.
>
> Diagnostic:
>
> ```sh
...
>
> ```sh
> npx --registry https://registry.npmjs.org geetorusai onboard --yes
> ```

`geetorusai run` does:

1. auto-onboard if config is missing
2. `geetorusai doctor` with repair enabled
...
exclusive with `--api-key-env`; Geetorus redacts it from its own output, but
...
Build and run Geetorus in Docker:

```sh
docker build -t geetorus-local .
docker run --name geetorus \
  -p 3100:3100 \
  -e HOST=0.0.0.0 \
  -e GEETORUS_HOME=/geetorus \
  -v "$(pwd)/data/docker-geetorus:/geetorus" \
  geetorus-local
```
...
```text
~/.geetorus/instances/default/                  # instance root
...
`GEETORUS_HOME` and `GEETORUS_INSTANCE_ID` override the home root and instance id respectively. `geetorusai onboard` echoes the resolved values in its banner (`Local home: <home> | instance: <id> | config: <path>`) so you can confirm where state will land before continuing.
...
- `~/.geetorus/instances/default/data/storage`
...
Use the helper bundled with the Geetorus skill from the repo root:

```sh
skills/geetorus/scripts/geetorus-upload-artifact.sh dist/demo.mp4 \
...
```sh
skills/geetorus/scripts/geetorus-upload-artifact.sh out/walkthrough.webm \
...
When a local agent run has no resolved project/session workspace, Geetorus falls back to an agent home workspace under the instance root:

- `~/.geetorus/instances/default/workspaces/<agent-id>`

This path honors `GEETORUS_HOME` and `GEETORUS_INSTANCE_ID` in non-default setups.

For `codex_local`, Geetorus assigns new and updated agents an isolated Codex home under the instance root and blocks shared host/company Codex homes:

- `~/.geetorus/instances/default/companies/<company-id>/agents/<agent-id>/codex-home`

Geetorus also persists an empty `OPENAI_API_KEY` override for those agents so a host-level `OPENAI_API_KEY` cannot leak into Codex runs through process inheritance. If an operator explicitly configures `adapterConfig.env.CODEX_HOME`, it must not point at the shared company `codex-home`, `$CODEX_HOME`, or `~/.codex`.

If the `codex` CLI is not installed or not on `PATH`, `codex_local` agent runs fail at execution time with a clear adapter error. Quota polling uses a short-lived `codex app-server` subprocess: when `codex` cannot be spawned, that provider reports `ok: false` in aggregated quota results and the API server keeps running (it must not exit on a missing binary).

Local adapters require their corresponding CLI/session setup on the machine running Geetorus. External adapters are installed through the adapter/plugin flow and should not require hardcoded imports in `server/` or `ui/`.

## Project Repository Checkouts

Tasks use every distinct repository attached to their project, including repository-only sources with no local folder. Geetorus creates a managed checkout when no local folder is configured. The selected repository remains at the task workspace root. Other project repositories have editable, independent Git checkouts under `.geetorus-repositories/<name>-<key>`. Workspace hints expose each checkout path to the agent.

When an additional repository has a configured local checkout, Geetorus seeds the task copy from its current commit and uncommitted files. Git-ignored files stay out of that copy. Subsequent task edits stay in the task copy. They do not overwrite the configured source folder. Existing task copies retain their work across runs.
...
If a repository is detached or its source configuration changes, its previous task copy is retained under `.geetorus-runtime/detached-repositories/` and excluded from future sandbox transfers. Referenced projects continue to use the separate read-only multi-project workspace behavior.
...
- `GEETORUS_WORKSPACE_GIT_SCAN_CONCURRENCY` (default `2`, range `1`–`16`)
- `GEETORUS_WORKSPACE_GIT_SCAN_QUEUE_CAPACITY` (default `32`, range `0`–`1024`)
- `GEETORUS_WORKSPACE_GIT_SCAN_TIMEOUT_MS` (default `8000`, range `100`–`120000`)
- `GEETORUS_WORKSPACE_GIT_SCAN_CACHE_TTL_MS` (default `10000`, range `0`–`60000`)
...
When developing from multiple git worktrees, do not point two Geetorus servers at the same embedded PostgreSQL data directory.

Instead, create a repo-local Geetorus config plus an isolated instance for the worktree:

```sh
geetorusai worktree init
# or create the git worktree and initialize it in one step:
npx geetorusai worktree:make geetorus-pr-432
```

This command:

- writes repo-local files at `.geetorus/config.json` and `.geetorus/.env`
- creates an isolated instance under `~/.geetorus-worktrees/instances/<worktree-id>/`
- when run inside a linked git worktree, mirrors the effective git hooks into that worktree's private git dir
- picks a free app port and embedded PostgreSQL port
- disables automatic database backups for the isolated instance
- by default seeds the isolated DB in `minimal` mode from the current effective Geetorus instance/config (repo-local worktree config when present, otherwise the default instance) via a logical SQL snapshot
...
Seeded worktree instances quarantine copied live execution by default for both `minimal` and `full` seeds. During restore, Geetorus disables copied agent timer heartbeats, resets copied `running` agents to `idle`, blocks and unassigns copied agent-owned `in_progress` issues, and unassigns copied agent-owned `todo`/`in_review` issues. This keeps a freshly booted worktree from starting agents for work already owned by the source instance. Pass `--preserve-live-work` only when you intentionally want the isolated worktree to resume copied assignments.

The same quarantine stops copied project/execution-workspace runtime desired states and clears copied runtime process claims. Without this reset, booting the cloned Geetorus database could restart a source workspace's dev service from the isolated instance, creating duplicate runners, port reassignment, and stale public URLs.

After `worktree init`, both the server and the CLI auto-load the repo-local `.geetorus/.env` when run inside that worktree, so normal commands like `pnpm dev`, `geetorusai doctor`, and `geetorusai db:backup` stay scoped to the worktree instance.

`pnpm dev` now fails fast in a linked git worktree when `.geetorus/.env` is missing, instead of silently booting against the default instance/port. If that happens, run `geetorusai worktree init` in the worktree first.
...
- `pnpm geetorusai worktree ensure-seeded` performs the deferred seed **exactly once**. It is lock-guarded and idempotent: only a complete `verified` manifest short-circuits it, so it is safe to call repeatedly and from concurrent processes. Managed workspaces derive the source from the control-plane-provided base project workspace when it carries its own `.geetorus/config.json`, and otherwise from the control plane's own registered instance config; either way the workspace's manifest never selects it. Manual worktrees must pass `--from-config`.
- `geetorusai run` calls `ensureWorktreeSeeded` automatically before doctor/boot. Managed runs transparently seed a lean worktree from their registered base workspace; an unmanaged lean worktree must first run `worktree ensure-seeded --from-config <source-config>`.
- Managed Geetorus git worktrees default to the repository's `scripts/provision-worktree.sh` when the strategy omits `provisionCommand`, so the isolated config and pending manifest cannot be silently skipped. Runtime startup also runs `scripts/provision-worktree-runtime.sh` automatically when no explicit runtime provision command is configured and the manifest is not verified. Explicitly configured provision commands still take precedence.
- The built-in deferred seed is recorded as its own terminal `workspace_seed` operation. A zero exit code is not enough for success: the operation succeeds only when `.geetorus/seed-manifest.json` contains complete verified evidence; failed, missing, or malformed manifests produce a failed operation with the seed phase in metadata.
- Worktrees created before lazy seeding shipped may have neither marker. Geetorus adopts them only after their configured database proves a compatible migration journal and the core Geetorus schema; otherwise managed startup creates a pending manifest and performs the normal verified seed. Manual markerless worktrees must provide `--from-config` so the source remains explicit.

Both `minimal` and `full` modes use the same terminal data-validation contract. Source validation accepts a migration journal that is a prefix of the checkout's journal and records the source revision in seed diagnostics; it rejects a source that is ahead of the checkout because that would require a downgrade. After restore, Geetorus applies pending migrations and requires the target journal to be current. Both validations also read an auth user with an instance administrator role, active company membership, and representative cloned company/issue pair. Authenticated instances additionally require that administrator to have a non-empty credential account. `local_trusted` instances accept the implicit local Board user without an account row because that mode intentionally has no human login flow. Restore, migrations, execution quarantine, routine pausing, workspace rebinding, and post-restore validation all run under the seed lock. An interruption leaves the exact active phase in terminal `failed` state; it cannot produce readiness evidence.
...
- `GEETORUS_DB_BACKUP_ENABLED=false`
- `GEETORUS_WORKTREE_NAME=<worktree-name>`
- `GEETORUS_WORKTREE_COLOR=<hex-color>`

The server/UI use those values for worktree-specific branding such as the top banner and dynamically colored favicon.
Authenticated worktree servers also use the `GEETORUS_INSTANCE_ID` value to scope Better Auth cookie names.
...
When Geetorus closes a server-managed git worktree, it also reclaims the isolated instance referenced by that worktree's repo-local `.geetorus/.env`. New server-managed worktrees use a collision-resistant instance id derived from the resolved absolute worktree path, and Geetorus persists the resulting instance root as execution-workspace ownership metadata. Cleanup requires the env pointer to match that persisted root, stops a running embedded PostgreSQL process, and then removes the instance directory. The deletion guard only accepts canonical instance paths below `GEETORUS_WORKTREES_DIR/instances/`; legacy or mismatched ownership, pointers to the default/live Geetorus home, and all other locations are logged and left untouched.

Print shell exports explicitly when needed:

```sh
geetorusai worktree env
# or:
...
Key material is derived, never shared. The control plane keeps a root secret (`GEETORUS_WORKSPACE_HANDOFF_SECRET`, or a domain-separated derivation from the instance's existing signing secret when that is unset) and injects only per-workspace values into the guest process:

| Variable | Purpose |
| --- | --- |
| `GEETORUS_WORKSPACE_HANDOFF_KEY` | Per-workspace ticket verification key. A guest cannot mint a ticket for a sibling workspace. |
| `GEETORUS_WORKSPACE_READINESS_TOKEN` | Bearer token the control plane presents to read this workspace's protected readiness. |
| `GEETORUS_EXECUTION_WORKSPACE_ID` | Execution workspace the guest was provisioned for, used for identity checks. |
| `GEETORUS_EXECUTION_WORKSPACE_COMPANY_ID` | Company whose board the guest represents. Scopes both the membership check and the readiness probes, so "some company in the clone is fine" cannot pass for the one being opened. |
...
**`npx geetorusai worktree init [options]`** — Create repo-local config/env and an isolated instance for the current worktree.

| Option | Description |
|---|---|
| `--name <name>` | Display name used to derive the instance id |
| `--instance <id>` | Explicit isolated instance id |
| `--home <path>` | Home root for worktree instances (default: `~/.geetorus-worktrees`) |
...
```sh
geetorusai worktree init --no-seed
geetorusai worktree init --seed-mode full
geetorusai worktree init --from-instance default
geetorusai worktree init --from-data-dir ~/.geetorus
geetorusai worktree init --force
```
...
```sh
cd /path/to/geetorus/.geetorus/worktrees/PAP-884-ai-commits-component
npx geetorusai worktree init --force --seed-mode minimal \
  --name PAP-884-ai-commits-component \
  --from-config ~/.geetorus/instances/default/config.json
```

That rewrites the worktree-local `.geetorus/config.json` + `.geetorus/.env`, recreates the isolated instance under `~/.geetorus-worktrees/instances/<worktree-id>/`, and preserves the git worktree contents themselves.
...
**`npx geetorusai worktree repair [options]`** — Repair the current linked worktree by default, or create/repair a named linked worktree under `.geetorus/worktrees/` when `--branch` is provided. The command never targets the primary checkout unless you explicitly pass `--branch`.
...
```sh
# From inside a linked worktree, rebuild missing .geetorus metadata and reseed it from the default instance.
cd /path/to/geetorus/.geetorus/worktrees/PAP-1132-assistant-ui-pap-1131-make-issues-comments-be-like-a-chat
pnpm geetorusai worktree repair

# From the primary checkout, create or repair a linked worktree for a branch under .geetorus/worktrees/.
...
For an already-created worktree where you want to keep the existing repo-local config/env and only overwrite the isolated database, use `worktree reseed` instead. Stop the target worktree's Geetorus server first so the command can replace the DB safely.

**`npx geetorusai worktree reseed [options]`** — Re-seed an existing worktree-local instance from another Geetorus instance or worktree while preserving the target worktree's current config, ports, and instance identity.
...
cd /path/to/geetorus
npx geetorusai worktree reseed \
  --from current \
...
**`npx geetorusai worktree:make <name> [options]`** — Create `~/NAME` as a git worktree, then initialize an isolated Geetorus instance inside it. This combines `git worktree add` with `worktree init` in a single step.
...
```sh
npx geetorusai worktree:make geetorus-pr-432
npx geetorusai worktree:make my-feature --start-point origin/main
npx geetorusai worktree:make experiment --no-seed
```

**`npx geetorusai worktree env [options]`** — Print shell exports for the current worktree-local Geetorus instance.
...
```sh
pnpm geetorusai worktree env
pnpm geetorusai worktree env --json
eval "$(npx geetorusai worktree env)"
```

For project execution worktrees, Geetorus can also run a project-defined provision command after it creates or reuses an isolated git worktree. Configure this on the project's execution workspace policy (`workspaceStrategy.provisionCommand`). The command runs inside the derived worktree and receives `GEETORUS_WORKSPACE_*`, `GEETORUS_PROJECT_ID`, `GEETORUS_AGENT_ID`, and `GEETORUS_ISSUE_*` environment variables so each repo can bootstrap itself however it wants.
...
Heavier setup that is only needed by a managed runtime service can use `workspaceStrategy.runtimeProvisionCommand`. Geetorus runs this command lazily before spawning the first service in a start batch, serializes concurrent provisioning for the same workspace, and records the attempt as `workspace_runtime_provision`. The command receives the same workspace environment as `provisionCommand` and should be idempotent because later service-start batches invoke it again.
...
Managed `geetorus-dev` worktree services enable `GEETORUS_UI_DEV_MIDDLEWARE=true` by default, so newly started worktrees hot-reload UI source changes. Managed HTTPS services use this default only when they publish the Geetorus Vite HMR companion listener, which is the default exposure configuration. A service or adapter can explicitly set the variable to `false` when it intentionally needs to exercise the built UI bundle.

In Vite middleware mode, Geetorus gives HMR a dedicated HTTP server bound to the managed runtime's loopback host. The browser still derives the HMR hostname from the public HTTPS page, and exposed runtimes use secure WebSockets, so listener containment does not break remote hot reload.

When a workspace service runs Geetorus for browser OAuth QA, configure its `expose.urlTemplate` with the canonical URL the browser can reach. Geetorus preserves explicit `GEETORUS_PUBLIC_URL` or `BETTER_AUTH_URL` settings; otherwise it uses a valid exposed HTTPS origin (or loopback HTTP) as the managed runtime fallback for Better Auth and `/api/tools/oauth/callback`. Internal service names such as `http://geetorus-dev:<port>` are rejected unless that hostname is genuinely the browser route. Use a unique origin per isolated worktree. See [Execution Workspaces And Runtime Services](../docs/guides/board-operator/execution-workspaces-and-runtime-services.md#browser-reachable-origins-for-oauth-qa) for configuration and verification.

## Geetorus Runner Adapter Conversion

The experimental Geetorus Runner offers native Codex, OpenCode, and **ACPX
...
runs; immutable run descriptors remain readable. The **Geetorus Runner**
...
For native Codex runs, Geetorus passes the resolved execution workspace as
`GEETORUS_WORKSPACE_CWD` and uses it as the provider containment boundary. A
...
For fast sandbox startup, bake `geetorus-runnerd` and the latest stable agent
...
Native discovery checks `/opt/geetorus-runner/bin`, then `$HOME/.local/bin`,
...
Geetorus Runner keeps its heartbeat run, native session, logical runner, and
...
- A verified live runner re-registers its existing PRP authority and reconnects
  with the same operating-system PID. Geetorus does not spawn a competing
  runner.
...
- A runner that died before its first authenticated connection can restart on
  ...
  checkpoint exists. Geetorus quarantines the incomplete root first.
- A live but mismatched or unverifiable process fails closed. Geetorus does not
...
- On the next run, a completed local Codex session whose warm controller died
  ...
  checkpoint quarantined by older controllers. Geetorus requires matching
...
```sh
pnpm --filter @geetorusai/geetorus-runner build:runner-binaries
pnpm exec vitest run server/src/services/native-runtime/native-runner-restart-recovery.integration.test.ts
pnpm --filter @geetorusai/geetorus-runner exec vitest run src/live/runnerd-codex-transport.test.ts -t 'adopts a live runner'
```

The suite uses isolated PostgreSQL state, isolated `GEETORUS_HOME` roots, real
...
The Geetorus app ships a curated catalog of company skills out of the box. The
...
paths at request time. Root `skills/` remains reserved for Geetorus runtime
skills and is not part of the catalog.

Skill-capable legacy local adapters always select the bundled
`geetorusai/geetorus/geetorus` operational skill when it is present in the
...
`geetorusSkillSync.desiredSkills`. The native `geetorus_runner` does not use
...
```sh
pnpm --filter @geetorusai/skills-catalog validate
```
...
```sh
pnpm --filter @geetorusai/skills-catalog build:manifest
```

The package's `build` script runs `build:manifest` and then `tsc`; tests live
under `pnpm --filter @geetorusai/skills-catalog test`. Validation fails when:
...
CI runs `pnpm --filter @geetorusai/skills-catalog validate` and the package's
...
```sh
pnpm --filter @geetorusai/teams-catalog validate
```

Regenerate `generated/catalog.json` after editing catalog team files:

```sh
pnpm --filter @geetorusai/teams-catalog build:manifest
```
...
Geetorus can run automatic logical database backups on a timer. These backups cover
...
- every 60 minutes
- retain 30 days
- backup dir: `~/.geetorus/instances/default/data/backups`

Automatic backups are disabled for isolated worktree instances created with
`geetorusai worktree init` or `geetorusai worktree:make`. Existing worktree
...
```sh
pnpm geetorusai configure --section database
```

Run a one-off backup manually:

```sh
pnpm geetorusai db:backup
# or:
pnpm db:backup
```
...
- `GEETORUS_DB_BACKUP_ENABLED=true|false`
- `GEETORUS_DB_BACKUP_INTERVAL_MINUTES=<minutes>`
- `GEETORUS_DB_BACKUP_RETENTION_DAYS=<days>`
- `GEETORUS_DB_BACKUP_DIR=/absolute/or/~/path`
- `GEETORUS_DB_BACKUP_MAX_AGE_HOURS=<hours>` controls the `/api/health`
  stale-backup warning threshold
- `GEETORUS_DB_BACKUP_ALERT_FILE=/path/to/failure-marker` lets external cron
  wrappers surface the last failed backup in `/api/health`
- `GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS=<days>` sets how long the
...
Without `GEETORUS_DB_BACKUP_ALERT_FILE`, health checks look for
...
- Default local key path: `~/.geetorus/instances/default/secrets/master.key`
- Override key material directly: `GEETORUS_SECRETS_MASTER_KEY`
- Override key file path: `GEETORUS_SECRETS_MASTER_KEY_FILE`
- Back up the key file and database together; either one alone is not enough to restore local encrypted secrets.

Strict mode (recommended outside local trusted machines):

```sh
GEETORUS_SECRETS_STRICT_MODE=true
```
...
- `pnpm geetorusai onboard` writes a default `secrets` config section (`local_encrypted`, strict mode off, key file path set) and creates a local key file when needed.
- `pnpm geetorusai configure --section secrets` lets you update provider/strict mode/key path and creates the local key file when needed.
- `pnpm geetorusai doctor` validates secrets adapter configuration, can create a missing local key file with `--repair`, and reports missing AWS Secrets Manager bootstrap env when that provider is selected.
...
```sh
GEETORUS_TOKEN_BROKER_ALLOWED_HOSTS=broker.internal.example,10.0.0.42
```

Entries are comma- or whitespace-separated exact hostnames (no wildcards). The
host configured by `GEETORUS_PAGES_API_URL` is included automatically. Every
...
```sh
GEETORUS_HTTP_ADAPTER_PRIVATE_ENDPOINT_ALLOWLIST=http://hooks.internal.example:8080,https://10.0.0.42
```
...
```sh
GEETORUS_ENABLE_COMPANY_DELETION=false
```
...
Geetorus CLI now includes client-side control-plane commands in addition to setup commands.

Quick examples:

```sh
npx geetorusai issue list --company-id <company-id>
npx geetorusai issue create --company-id <company-id> --title "Investigate checkout conflict"
npx geetorusai issue update <issue-id> --status in_progress --comment "Started triage"
```
...
```sh
pnpm geetorusai issue list
pnpm geetorusai dashboard get
```
...
- `GET /api/skills/geetorus` returns the Geetorus heartbeat skill markdown.
...
- In authenticated mode, run with board auth via `GEETORUS_AUTH_HEADER` or `GEETORUS_COOKIE`.

Optional auth flags (for authenticated mode):

- `GEETORUS_AUTH_HEADER` (for example `Bearer ...`)
- `GEETORUS_COOKIE` (session cookie header value)

## PostHog MCP Live Smoke Test

The PostHog smoke targets an already-running authenticated Geetorus instance.
...
```sh
INTEGRATIONS_POSTHOG_GEETORUS_E2E_EMAIL=operator@example.test \
INTEGRATIONS_POSTHOG_GEETORUS_DEV_LOGIN_PASSWORD='<environment-delivered>' \
INTEGRATIONS_POSTHOG_POSTHOG_PROJECT_ID=483530 \
pnpm smoke:posthog-live https://geetorus.example.test
```

The command fails before browser launch unless all three integration bindings
are present, and never prints their values. A Geetorus heartbeat derives the
target origin from its injected `GEETORUS_API_URL`; the positional URL (or
`--base-url <url>`) selects the running instance for a manual invocation. It
creates no trace, video, or HAR, begins
screenshots only after OAuth returns to Geetorus, enables read actions only,
...
Sanitized JSON and PNG evidence defaults to `GEETORUS_RUN_SCRATCH_DIR` when the
command runs in a heartbeat; set `POSTHOG_EVIDENCE_DIR` for a different output
directory.

Run the focused harness checks without contacting Geetorus or PostHog:
...
- defaults to isolated config dir `~/.openclaw-geetorus-smoke`
- resets smoke agent state each run by default (`OPENCLAW_RESET_STATE=1`) to avoid stale provider/auth drift

Networking behavior for this smoke script:

- auto-detects and prints a Geetorus host URL reachable from inside OpenClaw Docker
- default container-side host alias is `host.docker.internal` (override with `GEETORUS_HOST_FROM_CONTAINER` / `GEETORUS_HOST_PORT`)
- if Geetorus rejects container hostnames in authenticated/private mode, allow `host.docker.internal` via `npx geetorusai allowed-hostname host.docker.internal` and restart Geetorus
...
`geetorusSecretRedactions` from the run context. They do not load the full
...
limit. Legacy `geetorus_plugin` placeholder connections are excluded: their
