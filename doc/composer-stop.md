...
```sh
pnpm --filter @geetorusai/ui exec storybook dev -p 6016 -c storybook/.storybook --no-open
```
...
```sh
pnpm exec vitest run --project @geetorusai/ui ui/src/components/task-chat/TaskChatComposer.test.tsx ui/src/components/TaskChatThread.test.tsx ui/src/pages/IssueDetail.test.tsx ui/src/lib/wait-for-stopped-runs.test.ts
pnpm exec vitest run --project @geetorusai/server server/src/__tests__/issue-tree-control-routes.test.ts
...
```sh
cargo build --manifest-path packages/geetorus-runner/runner/Cargo.toml --bin geetorus-runnerd --bin fake-codex-app-server
GEETORUS_STOP_FAKE_CODEX="$PWD/packages/geetorus-runner/runner/target/debug/fake-codex-app-server" \
GEETORUS_RUNNER_BINARY="$PWD/packages/geetorus-runner/runner/target/debug/geetorus-runnerd" \
pnpm exec playwright test --config tests/e2e/playwright-composer-stop.config.ts
```

The suite boots a disposable local-trusted instance on port 3199 (override with
`GEETORUS_E2E_PORT`). It never attaches to an existing server. Native coverage
