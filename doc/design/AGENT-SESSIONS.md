...
> "In the Geetorus repo: make X look like Y. Show me before/after screenshots from the visual suite before you re-baseline anything."
...
> "Create a worktree off master, apply shadcn preset `<CODE>` as token values only (values-only — review the CLI's diff, revert scaffolding), reconcile the Geetorus status/agent color tiers, then build me a before/after gallery of the key surfaces."
...
- **Live test drive**: for big changes, ask for a running instance from the worktree — `npx geetorusai worktree init` once, then `PORT=3300 pnpm dev:once` gives an isolated Geetorus (own database, own config; your real instance is untouched). Click around; real use surfaces what screenshots can't.
