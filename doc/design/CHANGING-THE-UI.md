# Changing Geetorus's UI — a field guide
...
Geetorus's look lives in **three layers**, and you almost always work in the first one:
...
Then **review the git diff and keep only the CSS-variable value changes** — the CLI also tries to rewrite `components.json`, `lib/utils.ts`, and add dependencies; revert those (it once deleted 240 lines of our utils). Never use `shadcn apply --preset` (it overwrites component files). After the token diff is clean: Recipe 1 steps 3–4, plus a sanity pass on the Geetorus-specific tiers (agent gradients, WCAG-tuned status hues) for clashes.
