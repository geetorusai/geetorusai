...
```bash
cd ~/Projects/DEV/geetorus
git fetch origin && git worktree add ../geetorus-run3 -b design/component-convergence origin/master
cd ../geetorus-run3 && pnpm install
```
...
```bash
cd ~/Projects/DEV/geetorus-design-simplify
...
```
Converge Geetorus's duplicated hand-rolled UI onto the shared
...
4. pnpm check:token-gates 3/3 CLEAN; pnpm typecheck green;
   pnpm --filter @geetorusai/ui build exit 0.
