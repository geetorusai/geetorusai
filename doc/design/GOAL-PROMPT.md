...
```
Refactor Geetorus's UI so every visual value flows through the single
...
5. Merge to master when satisfied (rebase first if master moved). Scrap path: `git worktree remove ../geetorus-design-simplify --force`.
