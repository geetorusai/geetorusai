# ADR: Geetorus Runner architecture

- Status: Proposed
- Date: 2026-08-24
- Owners: Geetorus control plane and runner maintainers
- Related: [Geetorus Runner compatibility and rollout](geetorus-runner-compatibility.md)

## Context

Geetorus is a control plane. It owns companies, agents, issues, budgets,
approvals, and durable workflow state. Agent providers remain execution
services. Existing adapters invoke those services directly from the Geetorus
server.

Geetorus Runner introduces a separate execution process for provider sessions.
This process needs durable delivery, restart recovery, and governed access to
Geetorus actions. It must not become a second control plane. It must also land
...
Add a standalone package named `@geetorusai/geetorus-runner`. The package owns
the language-neutral Geetorus Runner Protocol (PRP), the Rust runner process,
provider drivers, deterministic replay, and semantic action dispatch contracts.

Add one explicit adapter named `geetorus_runner`. The adapter is available only
...
Do not route existing adapters through Geetorus Runner. A direct adapter keeps
its current invocation, transcript, interaction, cancellation, and finalization
paths.

## Goals

- Keep runner process ownership outside the Geetorus server process.
- Preserve Geetorus as the authority for identity, policy, and workflow state.
...
- Give runnerd a broad Geetorus API credential.
...
Geetorus APIs and task-thread models.

## Dependency direction

The runner package must build and test without importing Geetorus server, UI,
...
Geetorus server ----implements----> runner public ports
```

The dependency points from an implementation to a contract. The Geetorus
server may implement a public runner port. The runner package must not import
the server implementation.

The initial public package surfaces are:

- `@geetorusai/geetorus-runner` for runtime contracts and clients.
- `@geetorusai/geetorus-runner/testing` for deterministic fakes and
  conformance helpers.
...
The Geetorus server is authoritative for:
...
stored. Runnerd never receives a broad Geetorus API key.
...
Questions use `geetorus.question_set.v1`. Responses use the matching canonical
...
2. The agent explicitly selects `geetorus_runner`.
...
[Geetorus Runner compatibility and rollout](geetorus-runner-compatibility.md).
...
Geetorus governance into the runner.
