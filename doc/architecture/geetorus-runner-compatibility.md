# Geetorus Runner compatibility and rollout

- Status: Proposed
- Date: 2026-08-24
- Parent decision: [Geetorus Runner architecture](geetorus-runner.md)

## Purpose

This document defines compatibility rules for introducing the experimental
Geetorus Runner. These rules are acceptance criteria for each implementation
...
| none | `geetorus_runner` with any qualified provider | off | Reject the fresh start with a stable rollout-disabled error. |
| none | `geetorus_runner` with a qualified provider | on | Use PRP v1 and the provider's persisted runnerd backend. |
| none | `geetorus_runner` with an incomplete or unqualified profile | on | Reject the profile before runnerd starts. |
...
The server must not fall back from a selected `geetorus_runner` start to
...
Adding Geetorus Runner must not add runner imports or runner branches inside a
...
- creation UI does not offer `geetorus_runner`;
- edit UI does not offer switching to `geetorus_runner`;
- server creation and import reject a new `geetorus_runner` selection;
...
- creation, import, and edit accept `geetorus_runner` only with a qualified
...
New structured questions use `geetorus.question_set.v1` and the matching
