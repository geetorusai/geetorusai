...
report that work is done, blocked, ready for review, or yielded, but Geetorus's
...
The runner returns a `geetorus.run_result.v1` result and a matching terminal
...
`classifyNativeEvidence()` compares model claims with durable Geetorus
...
Ordinary issue completion changes Geetorus workflow state, but it does not by
...
Geetorus no longer creates a generic "Native completion review" because a
...
when it calls `geetorus_finish`. An empty `needs_review` report without an
