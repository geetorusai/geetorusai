...
`.github/workflows/cloud-readiness.yml` run in `geetorusai/geetorus`, with
...
`geetorus-post-merge` runner group. The separate Fleet label is
`runs-on/fleet=geetorus-post-merge-x64/env=public-ci`. Its 36 reserved slots use
...
`geetorus-cloud-build-x64` RunsOn Fleet for canonical `geetorusai/geetorus`
...
The cloud Fleet uses a separate runner group, `geetorus-cloud-build`, restricted
...
```sh
gh api repos/geetorusai/geetorus/actions/cache/storage-limit
```
...
```sh
gh api --paginate 'repos/geetorusai/geetorus/actions/caches?ref=refs/heads/master&key=node-cache-Linux-x64-pnpm-&per_page=100' \
...
entries="$(gh api --paginate --slurp 'repos/geetorusai/geetorus/actions/caches?ref=refs/heads/master&per_page=100')"
...
' >/dev/null; then
  gh api --method DELETE "repos/geetorusai/geetorus/actions/caches/$bad_cache_id"
fi
```
