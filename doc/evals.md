# Geetorus evaluation guide

Geetorus has two live eval families with different questions, owners, and
...
- **Runner Evals:** real Runner/provider behavior against a seeded mock control
  plane. Definitions live in `geetorus-evals/evals/geetorus-runner`; see the
  [direct live protocol evals](../packages/geetorus-runner/docs/runner-protocol-live-evals.md).
- **Product E2E Evals:** real browser, Geetorus server, database, Runner,
...
Geetorus surfaces and whether the resulting artifact and state are usable.
...
out an exact `geetorus-evals` revision, builds the Runner and viewer, runs a
...
Geetorus instance per cell and uses public APIs and the production browser
...
environment, and contract match. The evaluated Geetorus revision may
...
`geetorusai/geetorus-evals`; Runner integration, viewer, aggregation, and
publication code live under `packages/geetorus-runner` and the
...
[`runner-workflow-evals.md`](../packages/geetorus-runner/docs/runner-workflow-evals.md).
...
[direct live protocol guide](../packages/geetorus-runner/docs/runner-protocol-live-evals.md).
...
- **Product failure:** evidence shows Geetorus or Runner behavior violates the
...
- **Infrastructure failure:** the evidence points to provider/profile
  ...
  implicates Geetorus or Runner; classify from the observed failure and
...
source of truth. Keep the campaign ID, Geetorus commit, `geetorus-evals`
...
[pages.geetorus.ing/evals](https://pages.geetorus.ing/evals/).

For a repeatable workflow, use the matching skill: [geetorus-evals](../.agents/skills/geetorus-evals/SKILL.md),
...
`~/geetorusai/.agents/skills` (not `~/geetorusai/skills`). From the Geetorus
checkout, run:

```sh
for skill in geetorus-evals add-runner-eval add-product-e2e-eval; do
  install -d "$HOME/geetorusai/.agents/skills/$skill"
  install -m 644 ".agents/skills/$skill/SKILL.md" \
    "$HOME/geetorusai/.agents/skills/$skill/SKILL.md"
done
```
...
python3 scripts/evals-hub/build.py --output .geetorus/evals-hub
```
...
Publish with the [Geetorus page helper](../.agents/skills/geetorus-page/SKILL.md)
...
```sh
export GEETORUS_PAGE_BUCKET=pages.geetorus.ing
export GEETORUS_PAGE_BASE_URL=https://pages.geetorus.ing
export AWS_REGION=us-east-1
bash .agents/skills/geetorus-page/scripts/publish.sh .geetorus/evals-hub --slug evals --dry-run
bash .agents/skills/geetorus-page/scripts/publish.sh .geetorus/evals-hub --slug evals
```

For later refreshes, rebuild in the same output directory and publish with
`--update`. Keep its ignored `.geetorus-page/state.json` ownership record;
