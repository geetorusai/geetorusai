...
Geetorus ships on four channels. Pick the one that matches your appetite for
freshness versus stability — switching is just a matter of which version you
install.

| Channel | What it is | Updates | npm | Docker |
| --- | --- | --- | --- | --- |
| `stable` | The recommended release | every week or two | `geetorusai@latest` | `ghcr.io/geetorusai/geetorus:latest` |
| `beta` | Release candidates soaking before stable | when promoted | `geetorusai@beta` | `ghcr.io/geetorusai/geetorus:beta` |
| `nightly` | Yesterday's merges, smoke-tested as a unit | once a night | `geetorusai@nightly` | `ghcr.io/geetorusai/geetorus:nightly` |
| `canary` | Every merge to `master`, as it happens | many times a day | `geetorusai@canary` | `ghcr.io/geetorusai/geetorus:canary` |
...
```bash
npx geetorusai@latest onboard    # stable
...
```bash
docker pull ghcr.io/geetorusai/geetorus:latest    # stable
docker pull ghcr.io/geetorusai/geetorus:beta
docker pull ghcr.io/geetorusai/geetorus:nightly
docker pull ghcr.io/geetorusai/geetorus:canary
```
