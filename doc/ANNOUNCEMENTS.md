...
Geetorus displays one optional announcement card in the board UI. Its feed is
`https://pages.geetorus.ing/announcements/v1/current.json`. The instance fetches
JSON on demand and renders it with native components.

## Operator configuration

- `GEETORUS_ANNOUNCEMENTS_ENABLED=false` disables fetching and display.
- `GEETORUS_ANNOUNCEMENTS_FEED_URL` overrides the public HTTPS manifest URL.
...
timestamp), and `minimumGeetorusVersion` (stable `major.minor.patch`). Internal
...
Set `GEETORUS_PAGE_BUCKET`, optionally `GEETORUS_PAGE_BASE_URL`, and the page
uploader's namespaced `GEETORUS_PAGE_AWS_ACCESS_KEY_ID` and
`GEETORUS_PAGE_AWS_SECRET_ACCESS_KEY` (optional `GEETORUS_PAGE_AWS_SESSION_TOKEN`),
or `GEETORUS_PAGE_AWS_PROFILE`. Ambient AWS credentials also work.
For a host serving a subdirectory, `GEETORUS_PAGE_DEFAULT_PREFIX` prepends a
...
Geetorus controls. Add an `animation` alongside the required static `image`:
...
Geetorus verifies the digest, validates the HTML, and renders the result in an
...
Geetorus does not request or play the animation. Also include a
...
Older Geetorus builds that do not recognize `animation` treat that feed as
...
```sh
cp -R announcements/examples/animated .geetorus/announcement-animation-preview
# Edit HTML; recompute its digest and rename it; update current.json.
node cli/node_modules/tsx/dist/cli.mjs scripts/publish-announcements.ts .geetorus/announcement-animation-preview --staging animated-preview --dry-run
node cli/node_modules/tsx/dist/cli.mjs scripts/publish-announcements.ts .geetorus/announcement-animation-preview --staging animated-preview --publish
```
...
For Geetorus's existing preview host, use the branch preview area that
CloudFront already has permission to read:

```sh
aws sso login --profile geetorus-dev
export GEETORUS_PAGE_AWS_PROFILE=geetorus-dev
export GEETORUS_PAGE_BUCKET=geetorusai-runner-e2e-history-078455283791-us-east-1
export GEETORUS_PAGE_BASE_URL=https://d1p6rlowie26tp.cloudfront.net
export GEETORUS_PAGE_DEFAULT_PREFIX=storybook/branches/codex-announcements

# Copy the public fixture into an ignored directory and edit current.json there.
mkdir -p .geetorus
cp -R announcements/examples/staging .geetorus/announcement-preview
node cli/node_modules/tsx/dist/cli.mjs scripts/publish-announcements.ts .geetorus/announcement-preview --staging my-preview --dry-run
node cli/node_modules/tsx/dist/cli.mjs scripts/publish-announcements.ts .geetorus/announcement-preview --staging my-preview --publish
```
...
deliberately clears inherited `GEETORUS_*` variables.

```sh
mkdir -p .geetorus/announcement-test-drive/instances/default
# On a new test directory, create this file. On reuse, update these entries
# while preserving the file's existing keys.
cat > .geetorus/announcement-test-drive/instances/default/.env <<'EOF'
GEETORUS_ANNOUNCEMENTS_FEED_URL=https://d1p6rlowie26tp.cloudfront.net/storybook/branches/codex-announcements/announcements/staging/my-preview/v1/current.json
GEETORUS_ANNOUNCEMENTS_ENABLED=true
...
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts test-drive --data-dir .geetorus/announcement-test-drive --no-browser
```
...
Show after three seconds when opening or returning to Geetorus, after company
