...
This spec defines how Geetorus actually runs agents while staying runtime-agnostic.
...
1. Geetorus is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Geetorus manages lifecycle, and on exit Geetorus parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `geetorus.skill` (shared Geetorus skill text block)
...
2. Preferred transport is env vars (`GEETORUS_*`) injected at runtime.
