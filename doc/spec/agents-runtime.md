...
For `claude_local` and `codex_local`, Geetorus assumes the CLI is already installed and authenticated on the host machine.
...
Geetorus stores resumable session state per `(agent, taskKey, adapterType)`.
