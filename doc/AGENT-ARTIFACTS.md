...
must be attached to the Geetorus issue before the agent chooses a final
...
confirms storage and response binding in Geetorus; it does not confirm delivery
...
Use Bash to run the helper bundled with the Geetorus skill from the repo root; installed skill files may not retain executable permissions:

```sh
bash skills/geetorus/scripts/geetorus-upload-artifact.sh path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The helper uses the authenticated Geetorus API from the current heartbeat
environment:
...
`artifact` and `provider` to `geetorus`, with metadata canonicalized from the
...
Workspace file links resolve only inside registered Geetorus workspaces. The
...
when it carries both that `projectId` and `workspaceId`. Geetorus does not
...
2. Upload it with `skills/geetorus/scripts/geetorus-upload-artifact.sh`.
...
also pass each intended file with `geetorusai issue comment --attachment-id
<id>`. Geetorus binds only those exact uploaded files to that comment; other
...
```sh
bash skills/geetorus/scripts/geetorus-upload-artifact.sh dist/demo.mp4 \
...
```sh
bash skills/geetorus/scripts/geetorus-upload-artifact.sh out/walkthrough.webm \
...
```sh
bash skills/geetorus/scripts/geetorus-upload-artifact.sh render.bin \
  --title "Demo video render" \
...
```sh
curl -sS -X POST \
  "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues/$GEETORUS_TASK_ID/attachments" \
...
```sh
curl -sS -X POST \
  "$GEETORUS_API_URL/api/issues/$GEETORUS_TASK_ID/work-products" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
...
Use `type: "artifact"`, `provider: "geetorus"`, and metadata containing the
...
```sh
GEETORUS_FILE_DELIVERY_DAYTONA=1 pnpm exec vitest run server/src/__tests__/file-delivery-bridges.test.ts
```

The live fixture deletes each sandbox before checking that its attachments
remain downloadable from Geetorus. It does not run unless explicitly enabled.
