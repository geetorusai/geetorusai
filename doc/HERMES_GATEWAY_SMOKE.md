...
`pnpm test:hermes-gateway-smoke` does not start Docker, Hermes, or Geetorus. It
...
- Board/operator auth is required through `GEETORUS_AUTH_HEADER`,
  `GEETORUS_COOKIE`, or a board-capable `GEETORUS_API_KEY`.
- Diagnostic files are redacted before they are written, except the join output
  file intentionally contains the claimed Geetorus agent key and is written
  `chmod 600`.
...
- `GEETORUS_API_URL`: Geetorus URL used by the operator shell.
- `GEETORUS_API_URL_FOR_HERMES`: Geetorus URL used from inside the Hermes
  container or remote Hermes host.
- `HERMES_GATEWAY_API_BASE_URL`: Hermes gateway URL stored on the Geetorus
  adapter, reachable by the Geetorus server.
...
## Docker Desktop or Linux host Geetorus

Use this when Geetorus runs on the host at `127.0.0.1:3100` and Docker can
...
## Same Docker network as Geetorus

Use this when Geetorus is a container on a Docker network and the Hermes smoke
...
GEETORUS_API_URL=http://127.0.0.1:3100 \
GEETORUS_AUTH_HEADER='Bearer <board-token>' \
HERMES_CONTAINER_NAME=geetorus-hermes-gateway-smoke \
HERMES_SMOKE_NETWORK=geetorus_default \
HERMES_DOCKER_ADD_HOST=0 \
HERMES_GATEWAY_API_BASE_URL=http://geetorus-hermes-gateway-smoke:8642 \
HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642 \
GEETORUS_API_URL_FOR_HERMES=http://geetorus:3100 \
HERMES_GATEWAY_ALLOW_INSECURE_HTTP=1 \
pnpm smoke:hermes-gateway-e2e
```

Change `geetorus_default` and `geetorus` to your Compose network and service
name. The unsafe HTTP flag is required because Geetorus stores a non-loopback
`http://` gateway URL for private Docker DNS.

## LAN or private-network Geetorus

Use this when Geetorus is exposed on a private IP or tailnet address and the
Hermes container can reach that address.

```sh
GEETORUS_API_URL=http://192.168.1.20:3100 \
GEETORUS_AUTH_HEADER='Bearer <board-token>' \
GEETORUS_API_URL_FOR_HERMES=http://192.168.1.20:3100 \
...
Use this when Geetorus should talk to Hermes through a TLS hostname. The smoke
container still publishes a local port, and your reverse proxy forwards the TLS
hostname to that port.

```sh
GEETORUS_API_URL=https://geetorus.example.com \
GEETORUS_AUTH_HEADER='Bearer <board-token>' \
GEETORUS_API_URL_FOR_HERMES=https://geetorus.example.com \
...
`HERMES_GATEWAY_API_BASE_URL` to the URL Geetorus should store on the adapter.
