...
Use it when Geetorus acts on an external system through a governed connection: a stored credential, a capability catalog, access profiles and policy rules, and audit. Inbound integrations, such as an external client acting on Geetorus, use gateway or webhook guidance instead.
...
or **Paste a config** with no Geetorus code change at all — including servers
...
Write a catalog entry when Geetorus should *promote* a vendor: branding, tailored
...
resource credential governed by the Geetorus instance, never a sign-in
...
Geetorus resolves short-lived tokens at invocation time. Before writing a
...
| --- | --- | --- |
| Transport | `mcp_remote`, `local_stdio`, `rest_api` | How does Geetorus reach actions? |
...
| Credential source | `geetorus_vault`, reviewed `vercel_connect` | Where does durable provider credential material live? |
...
- Remote MCP + DCR OAuth + Geetorus vault + organization identity: Jira.
- Remote MCP + customer OAuth app + Geetorus vault: Asana.
...
- Remote MCP + Geetorus-managed OAuth client + per-user grant: Google
  Workspace MCP previews.
- Remote MCP + Geetorus-managed OAuth client + personal or dedicated-agent
...
| Pattern | Definition shape | What the user sees | What Geetorus stores |
| --- | --- | --- | --- |
...
| Geetorus-managed OAuth | `oauthStrategy: "geetorus_cloud_connector"`, `connectorProfile`, `platform_shared` | Browser sign-in through Geetorus Cloud | Provider tokens still land in the instance vault on a user grant. Cloud handles the fixed provider callback but does not persist plaintext credentials; Geetorus ID remains identity-only. |
...
For standard OAuth methods, Geetorus resolves a client in this order:
...
client secrets, and API keys live in the Geetorus instance vault.
...
Geetorus provider secret refs. Never make a method accept both sources in the
...
Geetorus gateway capability. At invocation time the gateway rechecks company,
...
key under `~/.geetorus/instances/<instance>/secrets/master.key`. A usable
...
- A generated URL containing credentials is split. Geetorus stores a safe URL
...
- Whether Geetorus itself needs provider approval. Customer-admin approval is
  self-serve; provider approval of Geetorus is not.
...
registration. Geetorus's catalog preflight is intentionally non-registering:
...
provider requires Geetorus approval or redirect allowlisting that a customer
...
- DCR versus CIMD. Geetorus chooses automatically.
...
instructions out of the universal Geetorus skill and provider-specific tools
...
AgentMail allowlists, rather than rebuilding it in Geetorus.

**Keep ongoing interactions in Geetorus tasks.** Connections are where users
...
`../../geetorus-content/research/connections/vercel/templates`. Override it
when necessary:

```sh
GEETORUS_CONTENT_TEMPLATES=/absolute/path/to/templates \
  pnpm connections:ingest-app-definitions
```
...
```json
{
  ...
  "guidanceMd": "Register Geetorus's callback URI in the provider console.",
  "consoleLinks": {
...
protocol metadata that Geetorus owns, not to force a user's business input.
...
```sh
pnpm --filter @geetorusai/shared typecheck
pnpm --filter @geetorusai/server typecheck
pnpm --filter @geetorusai/ui typecheck
```
...
Geetorus redirect policies permit it. Browser-started setup on an authenticated
...
hostname and board-mutation guards. An explicit `GEETORUS_PUBLIC_URL` remains
...
Deterministic fixtures prove Geetorus logic. A store-ready provider also needs
...
8. **Revoke/remove** — revoke at the provider or remove in Geetorus. Confirm
...
| `oauthStrategy` | Managed broker strategy. New definitions use `geetorus_cloud_connector`; `geetorus_id_connector` is recognized only to require migration when an old grant expires. The protocols and provider clients are not interchangeable. Only valid for OAuth. |
...
| Provider rejects redirect URI | Deployment/provider rule | actual browser origin, `GEETORUS_PUBLIC_URL`, `redirectConstraints`, provider app registration. |
...
| --- | --- | --- | --- |
| MCP-direct | The vendor exposes an official or stable MCP server whose tools map cleanly to Geetorus grants. | `mcp_remote`; `local_stdio` only for approved trusted templates. | Linear, Notion, Sentry, Vercel, Exa, Apify, Context7. |
| OpenAPI-shim | The vendor has a documented REST/OpenAPI surface but no stable MCP server, and a generated/thin shim can expose safe actions. | Shim service or approved template that presents an MCP-compatible catalog to Geetorus. | Datadog, Apollo, QuickBooks, Ramp/Brex, Zendesk. |
...
- OAuth: delegated user or workspace authorization. The OAuth client may come
  ...
  or a reviewed Geetorus Cloud connector profile. Do not assume Geetorus owns a
...
Do not add durable vendor credentials to agent env, project env, runtime env, adapter config, issue comments, screenshots, logs, fixture JSON, or plugin config. Agents receive a run-scoped gateway token; Geetorus resolves the vendor credential server-side and audits the call.
...
Do not infer Geetorus ownership from Vercel's “Managed” label. A Vercel-backed
connection remains `customer`/`dcr` according to the existing Geetorus model.
The resulting connection has an external connector ref and zero Geetorus
...
List each initial action before implementation. Do not rely on vendor tool names alone; Geetorus needs normalized metadata for review, policy, and audit.
...
without a Geetorus release. This is runtime setup behavior, not currently an
...
Geetorus's resource boundaries, risk classification, tier defaults, optional
...
- Add an explicit block only for a tool Geetorus must never expose, and prove
...
- Registration request: `client_name` `Geetorus (<instance host>)`,
...
- Env-registered clients always win: when
  `GEETORUS_TOOL_OAUTH_<PROVIDER>_CLIENT_ID/_SECRET` are configured, the
...
HTTPS `GEETORUS_PUBLIC_URL`), then DCR, then client credentials the operator
...
when a binding moves, a Geetorus-minted client re-registers and an
...
For a curated entry, `ownershipModes` still decides whether Geetorus may
...
**DCR needs neither Geetorus ID nor Geetorus Connect.** DCR is always
...
the redirect URI. `id.geetorus.ing` authenticates operators only and never
holds resource tokens; `connect.geetorus.ing` is a fallback only for
...
1. **Service involvement statement.** Say explicitly whether Geetorus ID or
   Geetorus Connect participates in the flow. For RFC 7591 DCR providers the
...
2. **Sequence diagram + exact endpoints.** A sequence diagram of how the
   ...
   token, registration (if DCR), and the Geetorus callback. Keep mermaid
...
4. **Provider consent interaction guards.** During browser smoke tests, do not
   ...
   Geetorus callback, token-exchange, and MCP health failures.
...
- Auth endpoints (exact paths):
  ...
  - Geetorus callback: `/api/tools/oauth/callback` (or n/a)
- Redirect constraints (probed): none / https-or-loopback-http / requires-public-redirect
- Geetorus ID / Geetorus Connect involvement: <"none — DCR is instance-local; cloud and self-hosted use the same path" for RFC 7591 providers; otherwise name the role>
  ...
  "methods": [
    {
      ...
      "guidanceMd": "Register a Linear OAuth app and add Geetorus's redirect URI before connecting.",
      "riskTier": "S2",
...
3. Geetorus stores OAuth material in `company_secrets` and shows redacted workspace/account metadata.
4. Operator selects workspace/team/project filters and reviews the S2 Allowed
   action defaults.
5. Geetorus runs health check and catalog refresh.
...
- First-30 classification: MCP-direct. Notion ships an official hosted MCP
  server; its ~20 `notion-*` tools map directly to Geetorus grants.
...
- Ownership modes: `dcr` (default, zero setup) and `customer`
  ...
  `GEETORUS_TOOL_OAUTH_NOTION_CLIENT_ID/_SECRET`, which always wins when set).
...
Geetorus ID / Geetorus Connect involvement: **none — DCR is instance-local**
...
| Geetorus connect (wizard) | `POST /api/companies/:companyId/tools/apps/connect` |
| Geetorus OAuth start | `POST /api/tools/oauth/:connectionId/start` |
| Geetorus callback | `GET /api/tools/oauth/callback` |
...
sequenceDiagram
    ...
    participant UI as Geetorus UI<br/>/PAP/apps/connect?source=notion
    participant S as Geetorus instance server<br/>(cloud or self-hosted — same path)
...
| --- | --- |
| `http://geetorus-dev:3100/api/tools/oauth/callback` | 400 `invalid_redirect_uri` — "Redirect URI must use HTTPS unless it is a loopback HTTP URI" |
| `https://geetorus-dev:3100/api/tools/oauth/callback` | Accepted — private host is fine over HTTPS |
...
- What the admin must register: **nothing**. Notion's authorization server
  ...
  registration, no Geetorus ID or Geetorus Connect involvement.
- Optional escape hatch: to use a pre-registered classic Notion integration
  instead, set `GEETORUS_TOOL_OAUTH_NOTION_CLIENT_ID` and
  `GEETORUS_TOOL_OAUTH_NOTION_CLIENT_SECRET`; the env client always takes
...
- Instance prerequisites: the instance base URL must be HTTPS on any host or
  ...
  geetorus-dev did). Apps is a standard product surface and `/apps/*` routes
...
3. Notion redirects to the instance's own `GET /api/tools/oauth/callback`;
   Geetorus exchanges the code, stores token material in `company_secrets`,
...
4. Operator confirms resource filters and reviews the S3 Allowed action
   defaults.
5. Geetorus runs health check and catalog refresh; `notion-*` tools appear
   on the actions step.
...
- Zero-setup OAuth connect succeeds on
  `https://geetorus-dev.tail29c1aa.ts.net/PAP/apps/connect?source=notion`
