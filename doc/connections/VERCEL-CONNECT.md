...
Audience: Geetorus operators and engineers enabling Vercel Connect for outbound
remote MCP apps.

Vercel Connect is an optional credential authority inside Apps v2. Operators
create, attach, authorize, and inspect connectors in Vercel. Geetorus records
...
not enter Geetorus secrets, connection config, agent config, prompts, API
...
> **Preview UI paused:** Geetorus currently withholds Vercel Connect from Apps
...
Geetorus pins `@vercel/connect` to `0.6.1`.

| Setting | Meaning |
| --- | --- |
| `GEETORUS_VERCEL_CONNECT_ENABLED=true` | Enables the backend capability for controlled testing and existing connections. It does not currently expose a customer-facing setup entry. |
| `VERCEL_OIDC_TOKEN` | Workload identity injected by Vercel and preferred when present. |
| `GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN` | BYO instance bootstrap authority for deployments without Vercel workload OIDC. |
| `GEETORUS_INSTANCE_ID` | Included in derived pseudonymous user subjects when configured. |
...
Treat `GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN` like any other deployment
bootstrap secret: inject it through the host/container secret facility, rotate
it outside Geetorus, and never put it in `company_secrets`, app config, task
...
rollout. Geetorus prefers workload OIDC whenever both authorities are present,
...
normal SDK cache path rather than persisted by Geetorus.
...
1. Open [Vercel Connect](https://vercel.com/connect), create the provider
   ...
   Geetorus. Vercel remains the inventory and management UI.
2. In a future reviewed build, Geetorus will expose one isolated Vercel Connect
...
3. Choose the reviewed app, open the Vercel handoff if needed, and paste only
   the connector UID (for example `notion/geetorus`) or connector ID (`scl_…`).
4. Geetorus calls `getConnectorMetadata` and rejects missing, unattached, or
...
5. For a user OAuth connector, continue through Vercel authorization. The
   ...
   app/API-key connectors, Geetorus verifies setup with a token request. If an
   installation is required, complete it in Vercel and retry; Geetorus does not
   use the experimental installation API.
6. Review the discovered catalog and complete the existing Geetorus access,
   resource-filter, risk, and agent-install steps.

V1 requires one dedicated app-subject connector per Geetorus connection. Do
not reuse an app-subject connector across Geetorus companies. Multi-installation
routing and shared app-subject connectors are intentionally deferred.

## Runtime and subject binding

Geetorus selects the effective connection grant before requesting a token. It
derives user-subject IDs from the Geetorus instance, company, connection, grant
kind, and responsible user. Organization OAuth uses a company-scoped
pseudonymous user subject; personal OAuth adds the Geetorus user. App/API-key
methods use Vercel's `app` subject. Neither browsers nor agents may supply these
subject identifiers.

Token resolution uses the SDK cache plus Geetorus single-flight deduplication.
...
`none` and PKCE `S256`; Geetorus sends the canonical resource plus reviewed
...
placement. Geetorus injects the bearer only after the connection grant,
...
Stable Geetorus reason codes include:

| Code | Operator action |
| --- | --- |
| `vercel_connect_unavailable` | Restore workload OIDC or the BYO access token. |
| `vercel_connect_auth_failed` | Repair or refresh Geetorus's Vercel authority, and verify that the configured token type is accepted by Connect. |
...
Geetorus creates the existing authorization interaction. Reconnect UI sends
...
external cleanup. Geetorus revokes subjects it generated for user-mode
...
synchronization, Geetorus-hosted relay services, and Vercel's experimental
...
instead of copying rates into Geetorus documentation.
