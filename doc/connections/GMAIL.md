...
Geetorus connects to Google's hosted Gmail MCP server at
`https://gmailmcp.googleapis.com/mcp/v1`. Gmail authorization is separate from
Google sign-in:

- Google sign-in identifies a Geetorus ID user and requests only
...
Do not add Gmail scopes to the Google sign-in client. The existing Geetorus
Cloud application at `my.geetorus.app` hosts the public Gmail OAuth callback;
Geetorus ID remains identity-only. The originating Geetorus instance remains
...
| --- | --- | --- | --- |
| Development | `geetorus-gmail-dev` | `Geetorus Gmail Connection Dev` | Local Geetorus Cloud origin + `/v1/connector/oauth/google/callback` |
| Staging | `geetorus-gmail-staging` | `Geetorus Gmail Connection Staging` | `https://my-staging.geetorus.app/v1/connector/oauth/google/callback` |
| Production | `geetorus-gmail-prod` | `Geetorus Gmail Connection Production` | `https://my.geetorus.app/v1/connector/oauth/google/callback` |

Replace the development port if the local Geetorus Cloud application uses another
port. Do not register Tailscale, customer, or other self-hosted Geetorus
instance URLs with Google. The browser always returns to Geetorus Cloud first;
...
Keeping projects separate is a Geetorus release policy. It prevents a
...
until Google verification and Geetorus Security review are complete.

### 1. Create the project

1. Open [Google Cloud project creation](https://console.cloud.google.com/projectcreate).
2. Select the Geetorus Cloud organization and billing account.
...
- Logo: the approved Geetorus logo
- Homepage: the public Geetorus product page
- Privacy policy: the public policy that describes Gmail data handling
- Terms of service: the public Geetorus terms
- Authorized domain: `geetorus.app`
...
originating Geetorus instance stores Gmail credentials and that Geetorus Cloud
...
### 7. Configure the Geetorus Cloud broker deployment

Set these on the existing Geetorus Cloud application that owns the redirect URI above. This is
the broker half of the configuration; the originating Geetorus instance is
configured separately under [Configure each originating Geetorus
instance](#configure-each-originating-geetorus-instance).
...
| Fixed callback | Local Cloud origin + `/v1/connector/oauth/google/callback` | `https://my-staging.geetorus.app/v1/connector/oauth/google/callback` | `https://my.geetorus.app/v1/connector/oauth/google/callback` |
...
used. The callback is derived from Geetorus Cloud's configured customer origin
...
the instance's `GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT`.

Geetorus Cloud derives a safe development, staging, or production fallback
...
- `/v1/connector/oauth/google/callback` on Geetorus Cloud;
- `response_type=code`;
...
token fragment may appear in a browser URL. The browser return from Geetorus
...
sequenceDiagram
    ...
    participant P as Originating Geetorus instance
    participant C as Geetorus Cloud connector
...
1. The instance generates an Ed25519 signing key and a separate X25519 seal
   ...
   X25519 lets Geetorus Cloud encrypt token responses that only the instance can
   open.
2. An instance administrator signs in to Geetorus Cloud through its existing
   Geetorus ID OIDC login and enrolls the instance. Enrollment is
...
3. Geetorus Cloud binds the account, opaque instance id, both public keys,
...
Geetorus Cloud may retain instance-encrypted initial-token ciphertext for at most
...
Removing one managed Google profile revokes only the local Geetorus grant.
Geetorus does not call Google's token revocation endpoint for that action.
...
### Configure each originating Geetorus instance

Generate the two long-lived instance keys once. PEM-encoded PKCS#8 keys work
directly with Geetorus:

```sh
openssl genpkey -algorithm ED25519 -out geetorus-cloud-signing.pem
openssl genpkey -algorithm X25519 -out geetorus-cloud-sealing.pem
openssl pkey -in geetorus-cloud-signing.pem -pubout -out geetorus-cloud-signing.pub.pem
openssl pkey -in geetorus-cloud-sealing.pem -pubout -out geetorus-cloud-sealing.pub.pem
```

Keep both private files in the instance secret manager. Enroll only the public
files with Geetorus Cloud, together with the instance id, the matching environment,
and every exact browser return origin. Then configure the originating Geetorus
deployment:

| Variable | Development | Staging | Production |
| --- | --- | --- | --- |
| `GEETORUS_CLOUD_CONNECTOR_BASE_URL` | Local Geetorus Cloud URL | `https://my-staging.geetorus.app` | `https://my.geetorus.app` |
| `GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT` | `development` | `staging` | `production` |
| `GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID` | Enrolled development instance id | Enrolled staging instance id | Enrolled production instance id |
| `GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY` | Development Ed25519 private key | Staging Ed25519 private key | Production Ed25519 private key |
| `GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY` | Development X25519 private key | Staging X25519 private key | Production X25519 private key |
...
HTTP is accepted only for a loopback Geetorus Cloud URL; staging and production
must use HTTPS.
...
`GEETORUS_ID_CONNECTOR_*` values are not aliases for this protocol. Geetorus
...
`CONNECTOR_MIGRATION_REQUIRED`. Enroll it with Geetorus Cloud and reconnect
...
2. Confirm the broker's `CLOUD_HARNESS_CONNECTOR_ENVIRONMENT` and the instance's
   `GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT` both read `development`. A mismatch
...
1. Complete Developer Preview enrollment, restricted-scope verification, any
   required security assessment, and Geetorus Security review.
...
| Local or Tailscale return is rejected | Enroll the exact origin on Geetorus Cloud. Only loopback HTTP is allowed; Tailscale must use HTTPS. |
| Every signed request fails on environment | `CLOUD_HARNESS_CONNECTOR_ENVIRONMENT`, the enrollment record, and `GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT` must agree. |
