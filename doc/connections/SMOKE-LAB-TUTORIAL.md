...
`http://geetorus-dev:45439`), and a `pnpm dev` server built with
...
1. **Connect via the fake OAuth provider.**
   ...
   - Click it. The fake provider's **real consent page** opens: a brown banner
     *"SMOKE TEST - not a real provider"*, headed *"Geetorus Smoke OAuth login +
     consent"*.
   - The **email is pre-filled** (`smoke@geetorus.test`). Type the password
...
A recurring Geetorus routine — **"Daily Smoke Lab integration smoke (P1-P7)"** —
