...
Geetorus person can send a DM, exchange files, answer questions, and respond to
ordinary confirmations. Each conversation remains attached to a Geetorus task.
...
2. In the [Photon dashboard](https://app.photon.codes/), obtain a project ID
   and project secret. Geetorus checks the project's actual allocation. Pro
   ...
   This enrollment does not authorize them in Geetorus. See Photon's
...
4. Send a fresh message to the displayed dedicated number, or to the sender's
   ...
   identity through Geetorus's identity confirmation flow. Send another fresh
...
5. With a dedicated line, to use a group, add the number in Apple Messages and send a message to discover
   it. Enable the group in Geetorus's Settings page, then send a fresh request.
...
grant Geetorus authority. Revoked links and inactive/viewer memberships cannot
...
Geetorus's canonical validators check required answers and selection/numerical
...
kinds that need the full review surface remain in Geetorus. The channel supplies
a task link and instructions. No individual-iMessage web permalinks are fabricated.

## Photos and files

Text, JPEG/PNG/WebP/GIF, allowed documents, audio, and video use Geetorus's existing
...
and known Photon receipts, then use Geetorus's operator resolution/retry controls.
...
frames. Geetorus's small authenticated gRPC recovery transport retains their
...
Geetorus retains delivery-unknown state if no stored receipt exists. Inspect the
