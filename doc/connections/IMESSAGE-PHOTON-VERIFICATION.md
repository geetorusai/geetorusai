...
[PR #13299](https://github.com/geetorusai/geetorus/pull/13299) carries the current
...
| Generated forward migration | Generated through `pnpm db:generate`; `@geetorusai/db check:migrations` passed. Disposable database migrations exercised by integration tests. |
| Native HEIF platform packages | macOS arm64 executed; other published platforms not executed. |

### Local test prerequisites

The standard `pnpm test:run` launcher isolates `GEETORUS_CONFIG`, `GEETORUS_HOME`,
...
```sh
cargo build --manifest-path packages/geetorus-runner/runner/Cargo.toml --bin fake-codex-app-server
```
...
- After the shared recovery fix and an isolated server restart, reconnect replayed
  the original message at 13:26 UTC. Geetorus discovered the exact sender but
...
- At 13:49–13:50 UTC, a synthetic HEIC was sent through Apple Messages. Geetorus
...
- At 18:13–18:14 UTC, an operator-supplied iPhone camera HEIC passed the same
  ...
  1,132,602-byte HEIC before ingestion. Geetorus retained those received bytes
