...
1. **Every connection has exactly one durable credential authority.** The
   ...
   then Vercel stores the durable provider credential and Geetorus stores only
...
2. **Connection operations are company-scoped and brokered by Geetorus.**
...
- Geetorus objects mutated by integrations: issues, comments, documents,
...
- Route webhook payloads only to the owning company/connection. Ignore request
  ...
  connection.
- Apply resource filters before storing external mappings or mutating Geetorus.
- For revoked/disabled connections, acknowledge where provider semantics require
  it, log the filtered/drop outcome, and do not mutate Geetorus state.
...
- Provider OAuth/app-installation scopes may be broader than Geetorus resource
  filters. Geetorus must enforce the narrower internal filter.
