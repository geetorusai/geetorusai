...
Post-read action: classify a new integration request, pick the right Geetorus
...
- **D2: one credential authority per connection, brokered projections.** The
  default is the Geetorus instance vault: durable third-party credentials live
  ...
  durable credential authority and Geetorus stores only the connector reference
...
- **D5: inbound stays thin.** External clients that call Geetorus use scoped
  Geetorus tokens and existing profiles/rules. They do not get a separate
  permission model.
...
Geetorus run/session token; the server or broker resolves the connection,
...
| --- | --- | --- | --- |
| **P1. Sign-in methods** | *Who are you?* | `geetorus-id` (id.geetorus.ing → Account) | Minimal-scope provider tokens (`openid email profile`), used once to authenticate, encrypted at rest, never exported |
| **P2. Connections (Apps)** | *What may your agents touch?* | Geetorus App instances (`tool_connections`), acquired via the **connect broker** for hosted + self-hosted | Rich-scope, long-lived resource tokens in the **instance's** encrypted vault; per-agent grants; risk-tier policy defaults |
| **P3. Login with Geetorus** | *Who may authenticate against us?* | `geetorus-id` OIDC provider + DB-backed client registry | Our ES256 ID/access tokens issued *by* us to registered RPs (instances, the broker, future third parties) |
...
> Sign-in tokens are never reused as resource tokens; id.geetorus.ing never
> stores resource tokens; no connections hub on the ID service.

P2 tokens flow broker → instance vault as pass-through only; the id.geetorus.ing
...
- **Blast radius.** id.geetorus.ing holding every customer's Vercel/Slack/GitHub
...
connection. `id.geetorus.ing` is not involved, and neither sign-in tokens nor
...
| --- | --- | --- |
| Geetorus App instances | P2 | **"Connections"** |
| id.geetorus.ing Account | P1 | **"Ways to sign in"** |
| id.geetorus.ing admin | P3 | **"OIDC clients"** (until the app store productizes it) |
...
- [Connecting any remote MCP server](./GENERIC-REMOTE-MCP.md) is the baseline:
  ...
  Geetorus code change, and how sign-in resolves a client.
