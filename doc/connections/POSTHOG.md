...
Geetorus connects to PostHog's hosted MCP service at
...
- a PostHog personal API key stored as a Geetorus secret and sent as an
...
PostHog OAuth or an API key managed directly by Geetorus.

Geetorus does not silently fall back from OAuth to an API key. The selected
...
through browser sign-in — with no Geetorus-specific code involved. See
[Connecting any remote MCP server](./GENERIC-REMOTE-MCP.md).

## Service involvement

PostHog hosts both the MCP resource and OAuth authorization service. Geetorus
...
`/api/tools/oauth/callback`. No Geetorus-operated vendor relay is involved.

```mermaid
sequenceDiagram
    actor A as Administrator
    participant P as Geetorus
...
| Geetorus callback | `/api/tools/oauth/callback` |

Redirect-URI constraints and token lifetimes remain provider-controlled and
must be rechecked during credentialed QA; Geetorus does not encode guessed
values for either.
...
5. The default setup requests all feature groups and tools. Geetorus fixes
...
6. For OAuth, continue through browser consent. For API-key setup, create a
   ...
   Geetorus. Never put the key in connection configuration or a URL.
...
When configured, Geetorus sends the optional project pin as the
...
capability. Geetorus sends configured `readonly`, `features`, `tools`, and
