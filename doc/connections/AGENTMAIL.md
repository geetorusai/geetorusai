...
the current allowed list adds that agent when setup completes. Every provider thread in that inbox has one Geetorus task. Subjects are
...
can have only one non-archived Geetorus endpoint across the instance.

Verified custom domains are selectable after checking the API key. Complete DNS
setup in [AgentMail](https://docs.agentmail.to/custom-domains). Geetorus does not
register domains or manage DNS.

The setup and Permissions page warn that an unrestricted inbox can receive mail
from anyone. Configure sender allowlists in AgentMail; Geetorus does not manage
or verify them. AgentMail controls new-message and reply lists separately. The
wizard recommends Geetorus’s existing **Low-trust review** preset and lets the
...
registers a Geetorus-owned webhook. The raw request body is verified using Svix
...
is recorded in the email envelope; an email address never grants Geetorus
...
Remote email images are not rendered. Attachments use Geetorus's content-type,
size, company, and task bounds.

An agent must own the inbox, be assigned the source task, and supply the running
source task's `X-Geetorus-Run-Id` at acceptance. Board actions require company
...
```sh
geetorusai email inboxes
geetorusai email thread "$GEETORUS_TASK_ID"
geetorusai email send --file email-request.json
geetorusai email reply --file email-reply.json
geetorusai email delivery '<publication-uuid>'
```
...
provider message ID and Geetorus publication header, or by an operator confirming
...
Geetorus. It never deletes the provider inbox or task history. If a revoked key
prevents provider cleanup, local disconnection still completes and reports that
Geetorus's provider registrations need cleanup in AgentMail.
...
AgentMail runs in the Geetorus control plane using its vaulted credentials. It
