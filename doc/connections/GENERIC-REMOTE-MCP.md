...
Geetorus can connect a standards-compliant remote HTTP MCP server without a
Geetorus code change. A curated `AppDefinition` is a **convenience layer** —
...
convenience layer on top for a vendor Geetorus should promote.
...
| --- | --- | --- |
| Guided URL | **Apps → Connect an app → Connect your own MCP server** | You have the server's address. Geetorus probes it and walks you through whatever it needs. |
| Paste a config | **Advanced → Paste a config** | A README gave you an `mcpServers` snippet, or the server needs headers with names Geetorus could not guess. |
...
on how to obtain each credential. Paste only the JSON block back into Geetorus;
Geetorus reads the header names from it and asks you for the values, which it
stores as Geetorus secrets.

## What the guided URL flow does

After you paste an address and press **Check link**, Geetorus probes the
...
before a personal authorization exists. If the server requires OAuth, Geetorus
creates the personal grant only after sign-in succeeds. If the server is public,
Geetorus creates a personal grant with no credentials after the probe succeeds.
...
Every value you enter becomes a Geetorus secret. Values are write-only: they
...
Geetorus refuses to send header names it manages or that belong to the
...
You never choose this; Geetorus resolves it and the wizard shows none of it.
Recorded here for security review and diagnostics. In preference order:

1. **Deployment-preconfigured client.** `GEETORUS_TOOL_OAUTH_<PROVIDER>_CLIENT_ID`
   / `_SECRET`, or the unsuffixed `GEETORUS_TOOL_OAUTH_CLIENT_ID` / `_SECRET`.
   Always wins when set.
2. **Client ID Metadata Document (CIMD).** When the authorization server
   advertises `client_id_metadata_document_supported`, Geetorus presents the URL
   of its own published metadata document as the `client_id`. Nothing is
   registered. **Requires a public HTTPS base URL** (`GEETORUS_PUBLIC_URL`):
   ...
   methods Geetorus uses — no company, connection or secret data.
3. **Dynamic client registration (RFC 7591).** When the authorization server
   advertises a `registration_endpoint`. Geetorus registers a public client
...
- a Geetorus-minted client (CIMD or DCR) is **re-registered**;
- a client you supplied yourself is **not** — Geetorus stops and asks you to
...
(`checkOAuthEndpointUrl` in `@geetorusai/shared`) and must be:
...
- **Free of embedded credentials.** `https://accounts.google.com@evil.test/…`
  reads as the wrong site to a human, so Geetorus refuses it.
...
already-running, browser-reachable HTTPS Geetorus instance with these bindings
provided by the execution environment:

- `GEETORUS_E2E_BASE_URL`, `GEETORUS_E2E_EMAIL`, and
  `GEETORUS_DEV_LOGIN_PASSWORD` for the target instance;
- `GEETORUS_API_URL`, `GEETORUS_API_KEY`, `GEETORUS_RUN_ID`, and
  `GEETORUS_TASK_ID` for the control plane;
...
Geetorus login succeed. It disables trace, video, and HAR capture, takes only
...
default to `GEETORUS_RUN_SCRATCH_DIR`, or to `NOTION_EVIDENCE_DIR` when set.
