...
Geetorus presents Google Workspace as nine independent Apps entries, not as
...
Google OAuth client or the same Geetorus Cloud broker deployment.
...
permission. Consequently, Geetorus-managed Google OAuth is limited to
...
available, it uses Geetorus by default. A small **Use your own Google OAuth app**
link reveals the custom client fields; **Use Geetorus instead** returns to the
managed method. The available authentication methods are:

- **Connect with Geetorus** uses the Geetorus Cloud broker when that exact
  profile is returned for this enrolled instance by the signed
  `POST https://my.geetorus.app/v1/connector/instance-status` request. The
...
- **Use the Geetorus robot account** remains an additional Google Sheets-only
...
The Geetorus-managed method signs every broker request with one explicit
...
same user and managed client, so Geetorus does not call it while removing one
Workspace profile.
...
All Geetorus-managed Google methods use the existing enrolled-instance keys:

```dotenv
GEETORUS_CLOUD_CONNECTOR_BASE_URL=https://my.geetorus.app
GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT=production
GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID=inst_example
GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY=...
GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY=...
```

No per-app client secret is stored on the Geetorus instance for the managed
...
`config.json` or set `GEETORUS_PUBLIC_URL`; the enrolled origin becomes the
durable callback binding. The former `GEETORUS_ID_CONNECTOR_*` values use an incompatible
Geetorus ID protocol and are not read aliases. Enroll with Geetorus Cloud and
...
Geetorus-managed method is omitted unless its exact profile is enabled at the
