...
| Subject | The app/workspace or Geetorus user on whose behalf a credential is requested. | The calling agent. |
...
| Trigger | Provider-origin event definition that starts governed Geetorus work. | An unauthenticated webhook handler. |
| Connector service | Geetorus-operated relay for managed OAuth callbacks, credential custody, and webhook intake at `connect.geetorus.ing`. | Geetorus ID or the per-company broker. |
...
| Skill | Agent instruction package that calls governed connections through Geetorus. |
...
- Inbound clients use scoped Geetorus auth; outbound vendor calls use the Apps
