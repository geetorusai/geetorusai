...
```sh
GEETORUS_E2E_PORT=3222 pnpm exec playwright test -c tests/e2e/connection-reviews.config.ts
```
...
```sh
GEETORUS_RUNNER_E2E_CONNECTION_REVIEWS=1 pnpm test:e2e:runner -- --suite connection-reviews
```
...
For a real Notion test, start `geetorusai test-drive` from this checkout using
...
```sh
pnpm --filter @geetorusai/ui exec storybook dev -p 6018 -c storybook/.storybook --no-open --ci
```
