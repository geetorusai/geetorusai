...
**The shared dev worktree service works as-is now.** The Geetorus-managed dev
service (`pnpm dev`, `GEETORUS_DEPLOYMENT_MODE=authenticated` + `NODE_ENV=production`,
e.g. `http://geetorus-dev:45439`) is private, so it can run the Smoke Lab directly —
...
export NODE_ENV=test PORT=3211 \
  GEETORUS_HOME=/tmp/pap-smoke-home \
  GEETORUS_INSTANCE_ID=pap-smoke \
  GEETORUS_CONFIG=/tmp/pap-smoke-home/instances/pap-smoke/config.json \
  GEETORUS_BIND=loopback \
  GEETORUS_DEPLOYMENT_MODE=local_trusted \
  GEETORUS_DEPLOYMENT_EXPOSURE=private
pnpm geetorusai onboard --yes --run    # serves http://127.0.0.1:3211, own embedded PG
...
(`.geetorus/browser-runtime/chromium-arm64/bin/chromium-agent-browser`) — see
