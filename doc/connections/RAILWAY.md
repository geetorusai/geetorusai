...
Railway appears in Apps and uses Geetorus's shared remote-MCP OAuth connection,
vault, catalog, grants, policies, gateway, and audit trail. It is a resource
connection, not Geetorus sign-in. No plugin or database migration is required.
...
4. Refresh actions to check API access. Geetorus uses the hosted `list-workspaces`
...
Use a public HTTPS Geetorus origin, or a loopback HTTP origin such as
...
| `geetorus-railway-list-projects`, `list-services`, `list-environments` | Explicit workspace ID for projects, project ID for services/environments; 1–100 results per page, cursor ≤512 characters | Read |
...
Direct tool names have the `geetorus-railway-` prefix. Railway may not shadow
...
access controls and explicit Geetorus action policies to constrain authorization.
...
Source deployment (`geetorus-railway-deploy-revision`) is also blocked. The
...
configuration change. Geetorus therefore offers 11 direct actions and no source
