...
Geetorus uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.geetorus/instances/default/db/` directory for storage
2. Ensures the `geetorus` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.geetorus/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Geetorus config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/geetorus` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://geetorus:geetorus@localhost:5432/geetorus
```
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. Geetorus uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
DATABASE_APPLICATION_NAME=geetorus  # application_name in pg_stat_activity; default: geetorus
```
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.geetorus/instances/default/db/`) |
...
- Create every migration with `pnpm --filter @geetorusai/db generate`. Do not hand-write a snapshot.
...
Geetorus stores current-user sidebar membership state in:
...
but no actor or Geetorus generation. IDs therefore must not be recycled when
...
Geetorus supports automatic and manual logical database backups. These dumps include
...
`geetorusai db:backup` / `pnpm db:backup` commands and backup retention
configuration.
...
Geetorus stores secret metadata and versions in:
...
- Default key file: `~/.geetorus/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.geetorus/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
...
- `GEETORUS_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
