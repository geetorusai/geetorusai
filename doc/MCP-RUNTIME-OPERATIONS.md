...
This runbook covers Geetorus Tools & Access runtime slots for MCP connections. It is written for board and CloudOps operators responding to stuck local stdio slots, degraded remote HTTP connections, capacity deferrals, restart storms, and secret-resolution failures.
...
Tool action approvals require `GEETORUS_TOOL_ACTION_SIGNING_SECRET` to be set independently from auth/JWT secrets. `geetorusai onboard` generates it for local instances, and worktree setup propagates or generates an independent value in the worktree `.env`; operator-managed deployments must set it explicitly. Rotate it deliberately: changing it invalidates outstanding signed tool-action approvals, so drain or reject pending approvals before rotation.

## Support Matrix

| Transport | Local trusted | Hosted cloud / public authenticated | Notes |
| --- | --- | --- | --- |
| `remote_http` | Supported | Supported | Preferred production path. Geetorus proxies calls through the gateway with policy, audit, timeout, and redaction controls. |
| `local_stdio` | Supported through approved templates and supervised runtime slots | Supported only when an explicitly trusted MCP runtime worker/host is configured | Set `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` or `GEETORUS_TOOL_RUNTIME_TRUSTED_HOST` only for a worker that is allowed to supervise local processes. Do not enable arbitrary agent-supplied commands. |
...
curl -fsS \
  ...
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/runtime-health" | jq .
```
...
1. Read the health summary and note firing alert names:
   ...
   curl -fsS \
     -H "Authorization: Bearer $BOARD_API_KEY" \
     "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/runtime-health" | jq '{status, metrics, alerts}'
   ```
...
2. List durable runtime slots:
   ...
   curl -fsS \
     -H "Authorization: Bearer $BOARD_API_KEY" \
     "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/runtime-slots" | jq .
   ```
...
curl -fsS -X POST \
  ...
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/runtime-slots/$SLOT_ID/stop" \
  -d '{}' | jq .
```
...
curl -fsS -X POST \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/runtime-slots/$SLOT_ID/restart" \
  -d '{}' | jq .
```
...
1. Run the connection health check:
   ...
   curl -fsS -X POST \
     -H "Authorization: Bearer $BOARD_API_KEY" \
     -H "Content-Type: application/json" \
     "$GEETORUS_URL/api/tool-connections/$CONNECTION_ID/health-check" \
...
2. Refresh the catalog after a remote endpoint or stdio template recovers:
   ...
   curl -fsS -X POST \
     -H "Authorization: Bearer $BOARD_API_KEY" \
     -H "Content-Type: application/json" \
     "$GEETORUS_URL/api/tool-connections/$CONNECTION_ID/catalog/refresh" \
