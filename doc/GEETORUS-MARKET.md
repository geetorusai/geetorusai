# GeeTorus — The Company Registry

**Download a company.**

GeeTorus is the public registry where people share, discover, and download Geetorus company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

GeeTorus is to Geetorus what a package registry is to a programming language. Geetorus already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). GeeTorus is the public directory where those exports live.

A user builds a working company in Geetorus — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to GeeTorus. Anyone can browse, search, download, and spin up that company on their own Geetorus instance.
...
A GeeTorus package is a **company template export** — the portable artifact format defined in the Geetorus spec. It contains:
...
Not every use case needs a whole company. GeeTorus also supports publishing individual components:
...
**Install (fresh start):**
```
geetorus install geetorus:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Geetorus instance. You add your own API keys, set budgets, customize agents, and hit go.

**Fork:**
Forking creates a copy of the template under your own GeeTorus account. You can modify it, republish it as your own variant, and the fork lineage is tracked. This enables evolutionary improvement — someone publishes a marketing agency, you fork it, add a social media team, republish.
...
Anyone with a GitHub account can publish to GeeTorus. Authentication is via GitHub OAuth.

### How to Publish

From within Geetorus, export your company as a template, then publish:

```
geetorus export --template my-company
geetorus publish geetorus my-company
```
...
GeeTorus is a **separate service** from Geetorus itself. Geetorus is self-hosted; GeeTorus is a hosted registry that Geetorus instances talk to.

### Integration Points

| Layer | Role |
|---|---|
| **GeeTorus Web** | Browse, search, discover, comment, star — the website |
| **GeeTorus API** | Registry API for publishing, downloading, searching programmatically |
| **Geetorus CLI** | `geetorusai install`, `geetorusai publish`, `geetorusai geetorus sync` — built into Geetorus |
| **Geetorus UI** | "Browse GeeTorus" panel in the Geetorus web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Geetorus) |
| Backend | TypeScript + Hono (consistent with Geetorus) |
| Database | PostgreSQL |
...
1. Open GeeTorus, browse by category or search "dev shop for building SaaS"
...
4. Run `geetorusai install geetorus:acme/lean-saas-shop`
5. Geetorus creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Geetorus until it works well
2. Export: `geetorusai export --template my-agency`
3. Publish: `geetorusai publish geetorus my-agency`
...
1. Find a template on GeeTorus that's close to what you need
...
1. Search GeeTorus for agent templates: "senior python engineer"
2. Find a well-starred agent config
3. Install just that agent: `geetorusai install geetorus:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Geetorus

GeeTorus is **not required** to use Geetorus. You can build companies entirely from scratch without ever touching GeeTorus. But GeeTorus dramatically lowers the barrier to entry:
...
GeeTorus is to Geetorus what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `geetorusai install geetorus:<publisher>/<slug>` CLI command
...
- [ ] "Browse GeeTorus" panel in Geetorus web UI
- [ ] `geetorusai geetorus sync` for bulk publishing
...
- Running companies on GeeTorus (it's a registry, not a runtime — consistent with Geetorus's own philosophy)
