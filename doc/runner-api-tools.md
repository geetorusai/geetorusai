...
`GEETORUS_RUNNER_API_TOOLS_ENABLED=true` on the server to enable it. For an
initial company rollout, also set `GEETORUS_RUNNER_API_TOOLS_COMPANY_IDS` to a
...
The companion `geetorus-evals` worktree contains `evals/runner-api-tools`.
