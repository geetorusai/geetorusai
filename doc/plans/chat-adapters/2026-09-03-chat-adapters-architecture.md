# Geetorus Chat Adapters Architecture

**Status:** decision-complete implementation plan
**Date:** 2026-09-03
**Geetorus base:** `origin/master` at `8430bd897f01dd4b91e0970efffb71b97e5a2685`
...
Geetorus will let a company expose any of its agents through external chat systems without turning those systems into a second control plane. External chat is transport and presentation. Geetorus remains authoritative for agents, tasks, runs, permissions, approvals, budgets, artifacts, liveness, and audit history.
...
The decisive identity choice is **one native bot identity per Geetorus agent endpoint**. A Slack workspace may contain several Geetorus agents, but each is installed as a distinct Slack app/bot and addressed through its native mention. Geetorus will not hide several agents behind one dispatcher bot in v1.

Thread-capable providers use a Hermes-style activation model. A person mentions the bot in the channel's root timeline; the bot creates or opens a native thread rooted at that message, creates exactly one Geetorus issue for its endpoint, and moves the conversation into that thread. Slack and Discord can continue eligible replies in the bound thread without another mention. Teams uses the same post/reply boundary and its required app manifest requests the resource-specific consent needed to deliver unmentioned channel-thread replies; setup cannot complete until the live root-and-reply test proves that grant is effective. GitHub binds an existing issue, pull-request conversation, or inline review-comment thread rather than manufacturing a second GitHub thread.

## 2. Product invariants

1. A channel endpoint belongs to one company, one Apps connection, one adapter, and exactly one Geetorus agent. The endpoint's assigned agent is immutable after creation. Connecting a different agent requires a new connection/endpoint; the setup and connector-detail UI never offers **Change agent**.
2. On a provider with `create_thread_from_root`, a new root-level native mention is an activation envelope: verify it, create or open the provider thread, create the Geetorus issue, reply in the thread, and leave the root timeline quiet except for the provider's normal thread indicator.
3. One bot-owned external thread maps to exactly one Geetorus issue for that endpoint. The binding is idempotent by endpoint plus activation root/thread id. The exceptional case where another Geetorus bot joins through an explicit route still creates a separate related issue because Geetorus preserves single assignment; it may not silently share or steal the first endpoint's issue.
4. Once bound, every eligible human message delivered from that provider thread continues the same Geetorus issue. Slack, Discord, and a correctly installed Teams app need no repeated mention. Geetorus requires and live-verifies Teams RSC rather than exposing a weaker mention-per-turn mode. Telegram privacy-on groups require a reply to the bot or another mention. A fresh unmentioned root message is ignored. A mention inside a pre-existing provider thread may activate that thread when policy allows, but it still yields only one issue binding.
5. Providers without creatable native threads use a declared fallback: use the existing provider conversation/comment thread, or combine the stable conversation with an explicit Geetorus session generation. GitHub uses the existing issue/PR or inline review-comment thread. Telegram forum topics use `message_thread_id`; ordinary Telegram DMs/groups maintain one active issue until `/new`, **New task**, or `/close` advances/closes the binding.
...
7. The issue remains a normal Geetorus task. Its title, description, status, project, goal, priority, documents, and artifacts remain editable. Its assigned agent is locked to the endpoint agent for the lifetime of the external task. Connecting a different agent requires a new connection and a new external task; there is no normal detach-and-reassign flow.
8. Incoming messages are attributed to an external principal. Linked principals act as their mapped Geetorus user. When enabled in Access, unlinked principals act only through the fixed restricted external profile; the internal sponsoring principal is not a selectable end-user role.
9. Agent execution always uses the assigned Geetorus agent's existing adapter, runtime, permissions, budgets, checkout rules, and approval gates. A channel message never creates a new execution authority.
10. Only a safe, explicitly external publication projection leaves Geetorus. Raw chain-of-thought, internal comments, tool traces, run logs, secrets, hidden activities, and internal identifiers do not.
11. Agent output is eligible for automatic publication. Board comments are Geetorus-only unless their author explicitly chooses **Send to channel**.
...
14. Every active endpoint uses the maximum safe capability set available to its adapter, provider installation, current conversation type, and current Geetorus permission check. Reactions, streaming, rich messages/cards, buttons, modals, commands, files, edits, DMs, and private-response fallbacks are implementation behavior, not per-endpoint on/off settings. Capability negotiation selects the best legal path and degrades unsupported behavior to safe text plus a Geetorus URL; it never bypasses Geetorus authorization.

## 3. Ownership boundary

### 3.1 Native Geetorus chat-adapters subsystem

Chat adapters are part of Geetorus itself, not a bundled or separately installed plugin. The subsystem owns:
...
Use Chat SDK for platform-specific normalization and presentation, not as Geetorus's authority. Chat SDK supplies:
...
Geetorus supplies a database-backed Chat SDK state adapter. In-memory and standalone Redis state may be used in adapter unit tests, but never as the production source of truth for endpoint subscriptions, locks, queues, history, or task bindings.
...
The connection-purpose choice is conditional, not a permanent extra wizard step. Show it for every selected provider whose registry entry exposes both chat and tool connection surfaces, not through a provider-name exception. A chat-only provider skips directly to **Which agent do you want to chat with?** using Geetorus's existing single-agent selector. Selection is final for that endpoint.
...
- Geetorus generates and stores GitHub's webhook secret, reveals it once for copying to GitHub, and requests only the App ID and private-key PEM;
...
Provider installation is an availability ceiling, not Geetorus authorization. Slack/Discord/Teams channel membership, Telegram chat membership, and the repositories selected in a GitHub App installation determine the resources whose events the provider can deliver. Geetorus independently enables a subset of those resources. Effective reach is the intersection of provider availability, Geetorus enablement, active endpoint state, and current actor authorization.
...
- **Access** controls who external people represent. Linked identities use current Geetorus user permissions; allowed unlinked identities use the fixed restricted external profile. The endpoint's sponsoring principal remains an internal authority ceiling and audit field, not ordinary UI configuration.
- **Conversations** is a read-only cross-link list: external conversation, Geetorus task, current state, **Open provider**, and **Open task**. It has no binding controls, detach action, or boundary explainer.
...
- a root mention in Slack or Teams creates a provider-native thread and one Geetorus task; later replies in that bound thread continue the same task without another mention when the provider delivers them;
...
- linked users use current Geetorus permissions, allowed unlinked users receive the fixed restricted external profile, overlapping turns queue, only safe milestones and final output publish, and agent-to-agent routes remain off.

Geetorus always uses the maximum safe provider capability set. Activity owns health, delivery diagnostics, and conditional repair actions. Relay and provider-specific developer transports live under instance administration, not endpoint onboarding.
...
All records carry `company_id`, timestamps, and appropriate foreign keys. These are first-party Geetorus tables in the normal database schema and migration lifecycle.
...
Explicit mapping from an external principal to one Geetorus user, with creator, confirmation time, revocation, and last authorization check. Links are company-scoped and never inferred from matching email alone.

Unique: company plus external principal. A principal has at most one active Geetorus-user mapping in a company.

### `chat_conversations`

Maps endpoint plus normalized external conversation/thread identity to one Geetorus issue. Store conversation kind, thread activation mode, activation root message/event id, provider thread id, provisioning state, subscription state, activation source, issue id, lifecycle (`active | completed | unavailable | endpoint_removed`), latest inbound/outbound ids, and timestamps. A provisional row keyed by the root activation survives a crash between Geetorus issue creation and provider-thread creation and lets reconciliation finish without duplicating either side. Lifecycle changes never unlock agent reassignment or erase the historical link.
...
Maps an inbound comment/action or outbound publication to provider message ids. Store direction, message/thread ids, revision, deletion state, and the Geetorus comment/publication/action reference.
...
One-time identity links are short-lived, single-use, bound to company/endpoint/principal, and completed only after Geetorus authentication. The confirmation page displays both identities and the target company before mutation.
...
7. Enforce endpoint status, provider availability, Geetorus resource enablement, rate limits, route policy, and principal authorization. A valid event from an available but disabled resource is recorded as ignored with only the minimum safe metadata; it creates no task, wakes no agent, and sends no response.
...
9. Transactionally create the assigned Geetorus issue and a provisional conversation row before any non-idempotent provider call. The issue includes source metadata and a backlink, but no provider secret.
...
13. Use Geetorus's normal wakeup path. Existing checkout, active-run, budget, pause, and liveness rules decide whether work queues, steers, or waits.
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `create_thread_from_root` | Slack, Discord, thread-capable Teams channels                                                                                           | A root `@bot` mention creates/opens a native thread and exactly one endpoint-owned Geetorus issue. All output stays in that thread; follow-ups continue there when delivered under the provider's mention/subscription/RSC rules. |
| `use_existing_thread`     | GitHub issues, pull-request conversations, inline review-comment threads; providers where the mention is already inside a native thread | The addressed existing thread becomes the external boundary and maps once to one endpoint-owned Geetorus issue.                                                                                                                   |
...
6. On success, link the provider message to the Geetorus source. On terminal failure, retain a visible diagnostic and Geetorus retry control without mutating the task result.

### 7.4 Interactive callback

1. Verify and durably record the action exactly like other ingress.
2. Resolve the external principal and its current Geetorus mapping.
3. Re-read the target task/interaction and its current resolver audience or approval policy.
4. Authorize as the linked Geetorus user. An allowed unlinked principal may answer only non-governed interactions allowed by the restricted profile; it cannot approve, hire, spend, change permissions, change budgets, or reassign agents.
5. Apply the Geetorus mutation transactionally and exactly once. Resolution never implies authorization for its downstream effect.
6. Return an ephemeral/card update where supported or a text result with a Geetorus URL.

## 8. Identity and permission model

### Linked principals

A linked principal becomes a Geetorus user actor only after explicit confirmation. Every action is reauthorized using current membership and permissions; a stale link conveys no cached authority. Activity includes provider/tenant/principal, Geetorus user, endpoint, delivery/action id, and authorization result.

### Restricted external principals

Every endpoint has an internal sponsoring principal and a versioned restricted external profile. The sponsoring principal is derived from the connection owner or instance policy; it is an audit and authority ceiling, not an Access-tab choice. Effective unlinked authority is the intersection of that ceiling, the enabled endpoint resource, restricted-profile operations, and target-state constraints. The initial allowlist is limited to starting/continuing the endpoint's bound task, uploading allowed attachments, and answering explicitly guest-resolvable non-governed questions. Unlinked people cannot use Geetorus as a general API principal and cannot govern the company. The Access tab exposes only whether unlinked participation is allowed and the explicit linked-identity list.

### Agent messages

Messages from another Geetorus bot resolve as external bot principals. They are ignored unless a matching directed route is active. Routed events carry an immutable origin endpoint, publication id, route id, visited endpoint set, and hop count. The receiving endpoint creates/continues its own task. Exceeding the hop limit, revisiting an endpoint, repeating a causal fingerprint, or targeting the source endpoint suppresses the event and writes audit evidence.

## 9. Chat SDK feature policy

The table below is an implementation contract, not a menu of endpoint toggles. For each publication or callback, Geetorus intersects adapter capabilities, provider installation/permission health, conversation type, safe-publication rules, and the current actor's Geetorus authority. It then uses the most capable legal rendering or interaction path automatically.

| Feature                      | Geetorus behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
Geetorus exposes `/api/chat/webhooks/:publicEndpointId` at a stable HTTPS origin. This is selected automatically for cloud and publicly reachable authenticated/self-hosted instances. Provider credentials and signing material are secret references. Health checks confirm reachability, credential validity, subscription state, and a real test event. The endpoint wizard never asks the user to choose “direct webhook.”

### Relay mode

A private instance opens an outbound authenticated WebSocket to a lightweight relay. Providers send to the relay; the relay verifies its outer endpoint binding and forwards an encrypted, bounded envelope. Geetorus still performs provider signature verification before processing. The relay retains only retry metadata and encrypted payloads for a short configured TTL, has no Geetorus user credential, and cannot invoke arbitrary APIs. Fenced endpoint ownership prevents two connected instances from consuming one delivery. Instance administration selects/configures relay once; individual endpoint wizards inherit it automatically.
...
1. **Core contracts and mock adapter:** shared types, first-party schema and migrations, Geetorus state adapter, public ingress, delivery/outbox workers, safe projection, thread-provisioning state machine, and exhaustive mock-provider tests.
...
- Concurrency tests for all five overlap modes with Geetorus task mutation serialized correctly.
...
- UI tests for setup, provider-available versus Geetorus-enabled reach, permissions, identity linking, immutable task assignment, explicit publication, conversation cross-links, diagnostics, empty/error/revoked states, and responsive layouts.
...
The first stable release is complete when an operator can connect any active Geetorus agent to Slack, Teams, Discord, Telegram, or GitHub; explicitly enable a subset of provider-available resources; an addressed native thread/object or explicit linear-chat session in that subset creates exactly one bound Geetorus issue for that endpoint; eligible follow-ups continue it using the provider's documented reply/mention rule; the existing Geetorus agent runs it under normal governance; safe output and artifacts return to the same conversation; failures are diagnosable and retryable; and every state transition is auditable. The provider-specific setup, permission, boundary, and fallback contract is maintained in `2026-09-04-chat-adapters-platform-surfaces.md`; the current navigation and UI inventory is maintained in `2026-09-04-chat-adapters-ui-surfaces-v8.md` and `index.html`.
