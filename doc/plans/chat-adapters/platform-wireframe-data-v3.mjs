...
const interactionAnnotations = {
  "15": [
    ...
    "Geetorus creates exactly one assigned issue and shows its Slack source, external participant, and publication state.",
    "Ari continues by replying in the same thread without another mention; files and actions remain in that context.",
    "Maya's safe progress and final answer publish in the thread; failures offer retry or a Geetorus link."
  ],
  "18": [
    ...
    "Geetorus binds that exact GitHub object or review thread to one assigned issue; PR conversation and inline review stay distinct.",
    "Later comments continue the same issue, while bot-authored comments and duplicate deliveries are ignored.",
    "Progress edits the existing comment; files and governed actions use authenticated Geetorus links."
  ],
  "21": [
    ...
    "Geetorus creates one assigned issue and records tenant, team/channel, thread, and external participant attribution.",
    "Replies, files, and Adaptive Card or task-module actions continue only when current Teams delivery and Geetorus permissions allow.",
  ...
  "24": [
    ...
    "Geetorus shows the active issue and makes the linear-chat boundary explicit instead of implying a Slack-style native thread.",
    "Maya uses throttled post/edit and inline buttons; unsupported or governed actions return text or DM with a Geetorus link."
  ]
};
...
const interactionSubtitles = {
  "15": "What Ari sees in Slack and the single Geetorus issue created behind the thread.",
  "18": "What Ari sees in GitHub and how the existing object becomes one Geetorus issue.",
...
export const providerScreens = v2Screens.map((screen) => {
  ...
  return {
    ...
    rationale: "This is a product-behavior walkthrough: the external conversation people see beside the Geetorus issue it creates."
  };
});
