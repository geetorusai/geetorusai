# Geetorus Chat Adapters UI Surfaces — v8
...
- **Geetorus enablement:** Geetorus responds only in provider resources that a Geetorus administrator has enabled for this connection. Invitation or installation alone is not permission to create a task.
- **Effective reach:** A message is eligible only when the provider delivers it, its resource is enabled in Geetorus, the connection is active, and the sender has authority for the requested action.
...
- **Access:** list Discord text channels available to the installed bot, with Geetorus enablement as an independent narrower allowlist, plus linked numeric Discord-user identities and the unlinked-participation policy.
- **Conversations:** show the Discord thread or DM generation, Geetorus task, current state, **Open Discord**, and **Open task**. There are no detach or rebinding controls.
...
| 11 | Geetorus | Task | Externally connected task | 1280×800 | 375×812 |
...
Purpose: Bring your own Slack app using Geetorus's prepared manifest.
...
2. Each toggle is Geetorus's independent allow or deny decision.
3. The provider action changes availability; newly discovered destinations remain disabled.
4. Private-conversation reach is an explicit Geetorus choice.

Rationale: Provider membership is the ceiling; Geetorus enablement is the narrower enforcement boundary.
...
2. The restricted profile permits task conversation but never Geetorus governance.
3. Linked accounts map a stable Slack workspace ID + user ID to a Geetorus user and can be revoked.
...
Purpose: Bring your own dedicated GitHub App and verify it with Geetorus.

1. The customer-owned App path is the complete shipped setup; no managed App Manifest exchange is required.
2. Geetorus generates the webhook secret and never returns it from normal endpoint reads.
...
4. GitHub installation scope and Geetorus repository enablement remain independent reach controls.
...
Rationale: Bring-your-own App credentials are sufficient to ship and preserve one provider bot identity per Geetorus agent.
...
3. One external conversation maps to one Geetorus task.

Actions:

- **Open GitHub:** Opens an installed repository while Geetorus waits for the first signed mention and follow-up to complete setup.
...
1. The only guest-policy choice is whether unlinked people may participate.
2. The restricted profile permits task conversation but never Geetorus governance.
3. Linked accounts map a stable GitHub host + numeric user ID to a Geetorus user and can be revoked.
...
1. Geetorus provides the exact public messaging endpoint.
...
2. Geetorus does not generate a complete Teams package or promise an install link.
...
- **Open Microsoft Teams:** Opens Teams while Geetorus waits for the authenticated mention and reply.
...
1. The endpoint is the one Geetorus-specific value required by Microsoft.
2. Every instruction is a provider portal operation.
3. The three identity fields are the minimum credentials Geetorus needs.
4. Connect does not generate a Teams package or install link.

Actions:

- **Copy Geetorus endpoint:** Copies the public messaging endpoint for the Azure Bot resource.
...
1. The only guest-policy choice is whether unlinked people may participate.
2. The restricted profile permits task conversation but never Geetorus governance.
3. Linked accounts map a stable Microsoft tenant ID + Entra object ID to a Geetorus user and can be revoked.
...
1. The only guest-policy choice is whether unlinked people may participate.
2. The restricted profile permits task conversation but never Geetorus governance.
3. Linked accounts map a stable Telegram bot ID + numeric user ID to a Geetorus user and can be revoked.
...
Purpose: A normal Geetorus task connected to its provider conversation.
