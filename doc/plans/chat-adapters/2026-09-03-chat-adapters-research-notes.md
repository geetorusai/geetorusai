...
**Date:** 2026-09-03
**Geetorus implementation base:** `8430bd897f01dd4b91e0970efffb71b97e5a2685`
...
How should Geetorus place its existing agents inside Slack and other external communication systems while preserving Geetorus tasks, runs, permissions, and governance as the source of truth?

## Geetorus baseline

Geetorus is already task/comment-centric rather than a generic chatbot. It has:
...
That means a channel integration should not create another agent runtime or conversational database. Its job is to translate external events into governed Geetorus task operations and translate safe Geetorus output back into the provider.
...
The catalog should seed Geetorus setup metadata, but it is not a compatibility guarantee. Geetorus must maintain its own reviewed registry with pinned package/version, maturity, deployment compatibility, and feature-test results.

### What Geetorus must not delegate

Chat SDK's subscription, queue, lock, and history abstractions are bot-building conveniences. Geetorus needs stronger durable delivery, task binding, actor authorization, audit, and outbox semantics. A Geetorus state adapter should implement the SDK contract on Geetorus-owned records while leaving Geetorus's delivery ledger authoritative.

### Thread topology and GitHub

Chat SDK normalizes provider threads, but Geetorus must choose what a thread means. The selected model is Hermes-style for channel products: a root `@bot` mention on Slack, Discord, or a compatible Teams channel creates/opens a native provider thread; that thread owns exactly one Geetorus issue for the endpoint; all later conversation stays inside it without repeated mentions. This keeps the channel timeline readable and gives Geetorus a stable task boundary.

GitHub joins the first supported group through its official Chat SDK adapter. Its issue, pull-request, or discussion already is the native conversation thread, so an addressed comment binds that existing thread to one Geetorus issue instead of creating a second GitHub thread. Telegram uses a stable chat/topic boundary where nested threads are unavailable. These differences belong in adapter capabilities, not provider-name conditionals in orchestration code.
...
OpenTag binds one visible persona (`AGENT_DISPLAY_NAME`) to one AG-UI agent URL. It does not solve Geetorus's company, multi-agent, task, permissions, budget, or audit model. Its managed Intelligence service owns provider credentials, delivery, state, and concurrency; Geetorus must own those controls itself or through an optional relay that does not become the business authority.
...
That is appropriate for a single product persona. It does not match Geetorus's core identity model, where a company has many independently configured agents with separate roles, runtimes, permissions, managers, and budgets. Geetorus should therefore expose each selected agent as its own provider bot identity. The shared unit is the channel, not a merged Geetorus agent.
...
This validates a future managed provisioning path, but Geetorus cannot depend on it initially:

- it is Slack-specific while the architecture must cover many providers;
- it simplifies installation, not Geetorus task/run/permission semantics;
- provider workspace permission inheritance does not replace Geetorus authorization;
- self-hosted Geetorus still needs BYO credentials and private-network relay options.

## One bot per agent versus shared bot

| Model                                    | Strength                                                                              | Failure in Geetorus                                                                             | Decision                                            |
| ---------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------- |
...
| One bot identity per Geetorus agent     | Native addressing, visible role, clean task ownership, separate permissions and audit | More provider installations and credential lifecycle                                             | Adopt                                               |
| One fixed product persona                | Simple, Claude Tag/OpenTag-like experience                                            | Does not expose the Geetorus company roster                                                     | Allow only as one ordinary Geetorus agent endpoint |

Within a shared Slack channel, `@Researcher` and `@Engineer` are separate apps. A root mention creates the native thread and one issue owned by the addressed endpoint; human replies in that thread continue it without another mention. If a second Geetorus bot participates through an explicit route, Geetorus records a separate related single-assignee issue and guarded route provenance rather than stealing or sharing the first issue.
...
## Feature-to-Geetorus mapping

| Chat SDK feature           | Geetorus source/target               | Required guard                                       |
| -------------------------- | ------------------------------------- | ---------------------------------------------------- |
