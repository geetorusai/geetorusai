...
**Date:** 2026-09-03
**Geetorus base:** `origin/master` at `8430bd897f01dd4b91e0970efffb71b97e5a2685` (refreshed from earlier planning references through `d593463ab6394cd356bf27448ea28bad8cccf4ec`)
...
**Archived wireframes:** [v1 SVG snapshot](https://github.com/geetorusai/geetorus/tree/1c4a45f0ef7d627aa98e4f3ae3116d4507386d1a/doc/plans/chat-adapters/wireframes) ([archive and regeneration notes](./wireframes-archive.md))

## 1. Information architecture

Channel integrations extend existing Geetorus surfaces rather than adding a new global product area.

- **Apps / Connectors** remains discovery and connection management.
- Chat adapters are a native Geetorus subsystem surfaced through Apps; they are not installed or managed as a plugin.
...
- Identity linking uses a minimal public Geetorus route reached from an ephemeral provider message or DM.
- Private self-host relay configuration lives inside the channel endpoint; it is not a global infrastructure page.
- Slack, Teams, Discord, Telegram, and GitHub are the initial supported set. Adapter capabilities determine whether Geetorus creates a native thread, binds an existing thread, or uses the stable conversation as the issue boundary.
...
- Always name the selected Geetorus agent and provider bot identity together.
- Always distinguish tool access from chat presence.
- State who can trigger the agent, where, and as which Geetorus principal before activation.
- Describe effective permissions; never imply that a provider membership grants Geetorus authority.
- Put safe defaults first: root mention creates/opens a provider thread and one Geetorus issue, threaded replies continue without mentions, queued overlap, public milestones only, linked-user permissions, sponsored restricted guests, agent routes off.
...
- Desktop uses the existing Geetorus primary and contextual sidebars. Mobile uses the existing drawer/header pattern with one full-width content column and 48px actions.
...
3. Channel method explains people message one selected Geetorus agent.
...
Purpose: establish the endpoint's permanent Geetorus owner.
...
3. On Slack, Discord, and compatible Teams channels, a root mention creates/opens a native thread and exactly one endpoint-owned Geetorus issue; later thread replies need no mention.
...
Purpose: establish external-to-Geetorus authority before activation.

1. Endpoint sponsor selection explains why a sponsor is required.
2. Linked-user path maps a provider principal to one Geetorus user after confirmation.
...
3. Test message instructions verify root mention, provider-thread creation, one Geetorus issue, unmentioned threaded follow-up, and fresh-root silence.
...
2. Queue/overlap policy names the Geetorus run consequence.
...
1. Rows show provider resource/thread, exactly one endpoint-owned Geetorus issue, participant count, subscription, and activity.
2. Filters cover active, waiting, failed, detached, and DM conversations.
3. Selection opens a detail panel with provider and Geetorus backlinks.
...
Purpose: see everywhere a particular Geetorus agent can be reached.
...
5. Empty state explains that the agent still works normally inside Geetorus.
...
2. External participant comments use provider attribution without impersonating a Geetorus user.
...
Purpose: safely map one provider principal to the currently authenticated Geetorus user.
...
4. Success explains that future actions use current Geetorus permissions.
...
5. Offline/degraded states distinguish provider acceptance from Geetorus processing.
...
- Use **channel connection** for the Geetorus configuration and **bot identity** for the provider-visible account.
...
- Default activation on thread-capable channels: “Mention this agent in the channel. It opens a thread and one Geetorus issue; continue in that thread without mentioning it again.”
- Existing-thread activation: “Mention this agent in a GitHub issue, pull request, discussion, or another supported existing thread. That thread binds to one Geetorus issue.”
- Conversation fallback: “This provider has no nested threads; this chat or topic is the Geetorus issue boundary.”
...
- Guest governance denial: “Link your Geetorus account and use an authorized user, or open this action in Geetorus.”
...
- Desktop wires use the current Geetorus global/contextual sidebar structure and preserve scanning density.
...
No external provider client is wireframed: those products own their UI. The package specifies the Geetorus surfaces and describes provider-visible behavior in annotations and examples.
