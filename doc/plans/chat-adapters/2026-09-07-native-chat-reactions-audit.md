...
E2E (`geetorus_runner` → `codex` → `gpt-5.6-luna`). The running backend was
...
The follow-up adapter revision `geetorus-discord-v5` preserves the Gateway
...
reaction, but Geetorus recorded neither event. That provider message
...
The Maya agent was rechecked as `geetorus_runner` → `codex` → `gpt-5.6-luna`.
