# Native Geetorus runner chat qualification — 2026-09-07

## Scope and runtime

The live Maya E2E fixture was switched from legacy ACP/Sol to
`adapterType: geetorus_runner`, Codex provider, model `gpt-5.6-luna`.
...
browser send timestamp to Geetorus's provider publication acknowledgement,
...
The exact `maya-e2e-geetorus` app, `A0C03GA5FPU`, still had a verified Events
...
Native activity is durably recorded in Geetorus's run-event path and consumed
...
The live Geetorus task UI was inspected: native turns show worked duration,
...
as `geetorus_runner` / `codex` / `gpt-5.6-luna`, and the four text-run records
...
Geetorus API key. Consequently, the previous shell-helper file instructions
