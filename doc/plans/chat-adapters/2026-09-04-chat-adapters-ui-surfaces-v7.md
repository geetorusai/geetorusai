# Geetorus Chat Adapters UI Surfaces — v7
...
Archived wireframes: [v7 SVG snapshot](https://github.com/geetorusai/geetorus/tree/1c4a45f0ef7d627aa98e4f3ae3116d4507386d1a/doc/plans/chat-adapters/wireframes-v7) ([archive and regeneration notes](./wireframes-archive.md))
...
- **Channel activation:** A root mention creates a provider-native thread and one Geetorus task on Slack and Teams. Replies in that thread continue the same task without another mention.
- **Existing provider thread:** The first mention inside an unbound Slack or Teams thread binds that existing thread to one new Geetorus task. Earlier messages are not imported automatically.
...
- **GitHub conversations:** A mention binds the addressed issue, pull-request conversation, or inline review thread to one Geetorus task.
- **Telegram conversations:** DMs and ordinary groups use one active task. A forum topic has one stable topic-to-task binding.
- **Delivery:** Geetorus chooses direct verified webhooks when reachable and the instance relay when private. This is deployment configuration, not an endpoint preference.
...
| 11  | Geetorus       | Task                     | Externally bound task                  | 1280×800  | 375×812  |
| 12  | Geetorus       | Agent                    | Agent Channels                         | 1280×800  | 375×812  |
...
Rationale: This is the only shared Geetorus-specific setup decision.
...
Purpose: Geetorus prepared a Slack App Manifest for Maya.
...
- **Open Slack app setup:** Opens Slack's official app-from-manifest URL with Geetorus's generated manifest encoded in the link.
- **Continue after installing:** Advances to the two credential fields after the operator has installed the new app in Slack.

Rationale: The custom path gives exact provider instructions without exposing Geetorus's automatic configuration.
...
Rationale: A customer-owned Slack App cannot return these values to Geetorus, so both fields are necessary.
...
2. The instructions teach the root-mention-to-thread Geetorus task boundary.
3. There is one action: open Slack and perform the test.

Actions:

- **Open Slack:** Opens the installed workspace while Geetorus waits for the root mention and thread reply to complete setup.
...
2. Linked provider identities act as their current Geetorus users and retain ordinary permission checks.
...
Purpose: Native conversation-to-Geetorus task bindings.

1. Each row names the provider-native conversation boundary and its single Geetorus issue.
...
3. Geetorus creates exactly one assigned issue and shows its Slack source, external participant, and publication state.
4. Ari continues by replying in the same thread without another mention; files and actions remain in that context.
5. Maya's safe progress and final answer publish in the thread; failures offer retry or a Geetorus link.

Rationale: The walkthrough explains automatic behavior without turning it into configuration.

### 16 · Create Maya in GitHub

Purpose: Create a dedicated GitHub App from Geetorus's prepared manifest.
...
- **Create in GitHub:** Posts Geetorus's App Manifest to GitHub. GitHub returns to Geetorus after creation, and Geetorus stores the returned App credentials.
...
- **Install in GitHub:** Opens GitHub's App installation page and returns the installation and selected repository IDs to Geetorus.

Rationale: There is no Geetorus form to duplicate GitHub's repository picker.
...
- **Open GitHub:** Opens an installed repository while Geetorus waits for the first signed mention to complete setup.
...
3. Only App ID and private key return to Geetorus; the generated webhook secret is already stored.
4. Verification happens as part of Connect rather than on another screen.

Actions:

- **Copy Geetorus webhook settings:** Copies the endpoint URL and generated webhook secret needed in the existing GitHub App settings.
...
3. Geetorus binds that exact GitHub object or review thread to one assigned issue; PR conversation and inline review stay distinct.
4. Later comments continue the same issue, while bot-authored comments and duplicate deliveries are ignored.
5. Progress edits the existing comment; files and governed actions use authenticated Geetorus links.
...
- **Copy setup command:** Copies a one-time Geetorus command that invokes Microsoft's Teams Developer CLI, signs the operator in, creates the Teams App and bot registration, and sends the resulting identity to this setup draft.
...
3. Tenant approval is handled by Microsoft's install experience, not another Geetorus choice.
...
- **Open Microsoft Teams:** Opens Teams while Geetorus waits for the first authenticated mention and reply to complete setup.
...
1. The copy control provides the one Geetorus value required by Microsoft.
2. Every instruction is a portal operation the tenant administrator must perform.
3. The three fields are the minimum identity values Geetorus needs to send as the bot.
4. Connect verifies the identity and produces the same install step as the default flow.

Actions:

- **Copy Geetorus endpoint:** Copies the public messaging endpoint that must be entered on the Azure Bot resource.
...
3. Geetorus creates one assigned issue and records tenant, team/channel, thread, and external participant attribution.
4. Replies, files, and Adaptive Card or task-module actions continue only when current Teams delivery and Geetorus permissions allow.
...
- **Open Maya in Telegram:** Opens the bot's t.me link while Geetorus waits for the first verified private message to complete setup.

Rationale: A private chat is Telegram's shortest path from BotFather token to a working Geetorus conversation.
...
4. Geetorus shows the active issue and makes the linear-chat boundary explicit instead of implying a Slack-style native thread.
5. Maya uses throttled post/edit and inline buttons; unsupported or governed actions return text or DM with a Geetorus link.

Rationale: The walkthrough explains automatic behavior without turning it into configuration.

### 11 · Externally bound task

Purpose: A normal Geetorus task with explicit publication and detach controls.
