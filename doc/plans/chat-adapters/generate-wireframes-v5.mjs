...
function globalSidebar(height) {
  ...
  return `<g data-region="global-sidebar">${tx(24, 40, "Geetorus", 20, "#000", 'font-weight="700"')}${items.map((item, index) => {
    const y = 80 + index * 48;
...
const sharedScreens = [
  ...
  { id:"03", slug:"choose-agent", title:"Which agent do you want to chat with?", subtitle:"Choose the one Geetorus agent represented by this connection.", group:"Start", rationale:"Agent choice happens once; every provider setup screen then shows it as immutable.", annotations:oldAnnotations.get("03") },
  { id:"11", slug:"bound-task", title:"Externally bound task", subtitle:"A normal Geetorus task with explicit publication and detach controls.", group:"Geetorus", rationale:"External work remains governed by the ordinary task experience.", annotations:oldAnnotations.get("11") },
  { id:"12", slug:"agent-channels", title:"Agent Channels", subtitle:"See every provider identity representing this agent.", group:"Geetorus", rationale:"Agent detail summarizes endpoints; Connectors continues to manage them.", annotations:oldAnnotations.get("12") }
];
