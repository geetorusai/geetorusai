...
- `packages/geetorus-runner/src/live/runnerd-final-output-burst.benchmark.test.ts`
- `packages/geetorus-runner/test/fixtures/fake-final-burst-codex-app-server.mjs`
- Production change: `packages/geetorus-runner/runner/crates/runner-core/src/durable/runner.rs`, `poll_executor_events`.
...
`/tmp/geetorus-final-burst-cargo.YoIBsw/baseline-geetorus-runnerd`; the candidate
is `/tmp/geetorus-final-burst-cargo.YoIBsw/release/geetorus-runnerd`.
...
`.geetorus-runtime/chat-adapters-live/runner-output-burst-benchmark-20260908/`:
...
Run from `packages/geetorus-runner`:

```sh
GEETORUS_FINAL_BURST_BENCHMARK=1 GEETORUS_FINAL_BURST_REPETITIONS=3 GEETORUS_FINAL_BURST_BINARY=/tmp/geetorus-final-burst-cargo.YoIBsw/baseline-geetorus-runnerd pnpm exec vitest run src/live/runnerd-final-output-burst.benchmark.test.ts
GEETORUS_FINAL_BURST_BENCHMARK=1 GEETORUS_FINAL_BURST_REPETITIONS=3 GEETORUS_FINAL_BURST_BINARY=/tmp/geetorus-final-burst-cargo.YoIBsw/release/geetorus-runnerd pnpm exec vitest run src/live/runnerd-final-output-burst.benchmark.test.ts
```

Without `GEETORUS_FINAL_BURST_BINARY`, the test selects the existing staged
...
```sh
cargo test --manifest-path runner/Cargo.toml --locked --offline --target-dir /tmp/geetorus-final-burst-cargo.YoIBsw -j 2 -p geetorus-runner-core --lib durable::runner::tests
cargo test --manifest-path runner/Cargo.toml --locked --offline --target-dir /tmp/geetorus-final-burst-cargo.YoIBsw -j 2 -p geetorus-runner-core --lib -- --test-threads=2
