# Geetorus Chat Adapters — Setup Audit v5
...
Date: 2026-09-04
Geetorus base: `7b094724e65c04949706df638d497afb02c84b62`
Historical review viewer: [Git archive](./wireframes-archive.md).
Archived setup wireframes: [v5 SVG snapshot](https://github.com/geetorusai/geetorus/tree/1c4a45f0ef7d627aa98e4f3ae3116d4507386d1a/doc/plans/chat-adapters/wireframes-v5) ([archive and regeneration notes](./wireframes-archive.md))

## Decision

Connector setup asks only for decisions or values that Geetorus cannot safely infer, provision, receive from a provider callback, or inherit from the instance deployment.
...
- Every provider uses a persistent step rail with completed, current, and remaining phases. A provider redirect may leave Geetorus, but the draft and current phase remain resumable.
...
- Geetorus fixes required events, permissions, callback URLs, command declarations, and maximum safe interaction capabilities. They are not setup options.
- Geetorus selects delivery from instance reachability. Direct callback, relay, Socket Mode, and polling do not appear as endpoint preferences.
- Credentials obtained by an authenticated provider handoff go directly to Geetorus's secret store. They are not displayed or copied through the UI.
...
| Bot name/avatar configuration                                                                     | Show a read-only preview or provider-owned result          | Provider naming and uniqueness rules belong in the provider handoff. Geetorus may propose the agent name and avatar.                                                |
| Direct webhook                                                                                    | Remove as a choice                                         | It is the automatic path when the instance has a verified public callback.                                                                                           |
| Private Geetorus / relay                                                                         | Remove as a choice                                         | A private instance uses its configured outbound relay automatically. Relay enrollment and keys belong to instance administration, not to each endpoint.              |
...
| Telegram polling                                                                                  | Remove from endpoint setup                                 | Polling and webhook delivery are mutually exclusive. Geetorus may use polling for a local developer instance, never as a normal endpoint preference.                |
| Feature switches for reactions, streaming, cards, actions, modals, commands, files, edits, or DMs | Remove                                                     | Geetorus always uses the maximum safe feature supported by the adapter, installation, conversation, and current Geetorus authorization.                            |
...
| Credentials returned by OAuth or manifest callback                                                | Hide completely                                            | Geetorus can store them directly without asking the operator to handle a secret.                                                                                    |
| Customer-owned credentials with no callback                                                       | Keep only the irreducible values; submit write-only        | Geetorus cannot authenticate without them. The connector shows secret references and rotation state after setup, never the stored values.                           |
| Provider resource choice                                                                          | Keep in the provider handoff                               | Workspace, organization, repository, tenant, team, channel, group, or chat membership is governed by provider policy. Geetorus may narrow the returned scope later. |
...
| Deployment condition                                         | What Geetorus does                                                                                                                            | What the endpoint wizard shows                                                                          |
| ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Geetorus Cloud or publicly reachable self-hosted instance   | Registers the endpoint's unguessable verified HTTPS callback directly with the provider.                                                       | **Automatic** during setup; read-only delivery health after setup.                                      |
| Private self-hosted instance with Geetorus relay configured | The instance maintains an authenticated outbound relay connection; the relay accepts the provider callback and forwards the verified envelope. | **Automatic** during setup; relay health at instance administration and read-only endpoint diagnostics. |
...
| ------------------------------------ | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Slack — customer-owned app           | Bot token and signing secret                                                   | Slack's app-from-manifest handoff preconfigures the App but does not return these two customer-owned values to Geetorus. No webhook URL, app token, or delivery choice is requested.                                      |
| GitHub — customer-owned App          | App ID and private-key PEM; GitHub Enterprise Server host only when applicable | Geetorus generates, stores, and reveals the webhook secret once for copying to GitHub. It then authenticates and verifies the App callback, events, and permissions without asking the operator to paste the secret back. |
...
All secrets are write-only inputs to Geetorus's existing secret store. Setup and connector detail retain only secret references, redacted suffixes, health, and rotation actions.
...
| 41     | Try Maya         | **Open Slack**                 | Opens the installed workspace while Geetorus waits for a signed root mention. A valid mention creates the Slack thread and its one Geetorus task. |
...
An Add to Slack flow can be introduced when Geetorus participates in Slack's managed agent-deployment program. It is a convenience only and is not a first-release dependency.
...
| Screen | Phase               | Retained action             | What happens                                                                                                                          |
| ------ | ------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 16     | Create GitHub App   | **Generate webhook secret** | Geetorus generates and stores a 32-byte secret and reveals it once for copying into the GitHub App.                                  |
...
| 46     | Try Maya            | **Open GitHub**             | Opens an installed repository while Geetorus waits for a signed mention in an issue, PR conversation, or inline review thread.       |
...
The required path asks only for the App ID and private-key PEM after Geetorus has generated the webhook secret. Contents, Actions, and Administration permissions are absent because this is a chat connection; a GitHub tool connection is separate.
...
| Screen | Phase              | Retained action             | What happens                                                                                                                          |
| ------ | ------------------ | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 19     | Register Teams bot | **Open Microsoft setup**    | Guides the operator through a customer-owned single-tenant Entra App and Azure Bot registration using Geetorus's messaging endpoint. |
...
| 50     | Try Maya           | **Open Microsoft Teams**    | Opens Teams while Geetorus waits for the first authenticated activity from an installed scope.                                       |
...
| 52     | Try Maya            | **Open Telegram**          | Opens Telegram while Geetorus waits for a real update from the intended DM, group, or forum topic.                                                             |
...
- **Use this connection as an agent tool** enters Geetorus's existing connection credential and human/agent-access flow.
