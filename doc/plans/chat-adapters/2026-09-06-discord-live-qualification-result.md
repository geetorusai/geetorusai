...
> **Status: core Discord transport, ordered follow-up bursts, receipt cleanup, and keep-open idle recovery have live proof, but the full DC1–DC7 matrix remains unqualified.** Geetorus has verified the dedicated bot identity, Message Content intent, Clawd membership, and a permission-complete text channel against Discord. The later clean-source checkpoint supersedes the intermediate unsolicited-recovery blocker.

## Resumed live setup — 2026-09-07 UTC

The operator entered the existing bot token directly into Geetorus's masked
...
because Geetorus called the numeric Get Guild Member route with the literal
...
working tree based on `f5f31d2e1`. The Geetorus endpoint reached its real
...
Geetorus task (`CHA-3`, issue `1976d84b-0bdf-4342-8afa-1a3e5d9be57c`), and one
...
Geetorus board; no one-time link or credential is recorded here. A fresh root
...
principal created native thread `1546513811672932372`, exactly one Geetorus
task (`CHA-4`, issue `c65f32f8-a612-4f85-97c5-61bed2de58e2`), and one bound
conversation. Only `#general` was enabled in Geetorus; the other ten discovered
...
1. create a dedicated Discord application and bot for the immutable Geetorus agent;
2. enable Message Content Intent and enter only Application ID, Server ID, and the write-only bot token in Geetorus;
...
4. install the bot in Clawd, connect it in Geetorus, enable only the intended test channel, and execute DC1–DC7 from the browser runbook.
...
- immutable application identity, including a database uniqueness constraint that prevents one Discord Application ID from backing multiple active Geetorus agent endpoints even across different servers;
- server and effective-channel-permission verification, channel discovery, a Geetorus allowlist, and a separate direct-message reach switch;
- one root mention to one Discord public thread and one Geetorus task, with thread replies serialized onto that task and DMs isolated into linear task generations;
- endpoint, resource, principal, and root-message preflight before provider-thread creation; denied roots retain only a payload-redacted filtered audit and create no provider thread or Geetorus work;
