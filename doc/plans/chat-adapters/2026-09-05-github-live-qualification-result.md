...
On `95cbbd08e`, the user-authorized PEM import connected **Geetorus Maya E2E
...
tunnel was stopped after a real signed issue comment reached Geetorus.

- [Issue 1](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/1)
...
- [Issue 2](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/2)
...
- [Disabled-repository issue 1](https://github.com/cryppadotta/geetorus-chat-e2e-disabled/issues/1#issuecomment-5571234021)
  produced a GitHub webhook response **200 / ignored**, with no Geetorus
...
[bot comment 5571558895](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/2#issuecomment-5571558895)
...
[bot comment 5571729974](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/2#issuecomment-5571729974)
...
[PR 3](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/pull/3),
...
[bot reply `5572100025`](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/pull/3#issuecomment-5572100025)
containing exactly `GH-PR-LEVEL-0907-OK`. Geetorus bound provider thread
`github:cryppadotta/geetorus-chat-e2e-enabled:3` to conversation
...
[bot reply `3950666803`](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/pull/3/changes#r3950666803)
containing exactly `GH-PR-REVIEW-0907-OK`. Geetorus bound the distinct provider
thread `github:cryppadotta/geetorus-chat-e2e-enabled:3:rc:3950666444` to
...
thread on the same PR bind to different Geetorus conversations and tasks, and
...
433 ms), not Geetorus queueing or provider transport; the result was correct,
...
Markdown link to `https://github.com/user-attachments/files/…`. Geetorus
...
text rather than being republished as a Geetorus-owned attachment.
...
Pull requests permissions. Geetorus therefore must not claim that a checked
...
Geetorus task link. A private/local Board produces a private-task notice with
no unusable localhost or webhook-ingress URL.

The task banner presents this provider-specific boundary before send: checked
files remain on the Geetorus task, while GitHub receives the authenticated
...
[bot reply `5572302077`](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/2#issuecomment-5572302077)
...
- Four agent runs in that historical task failed closed because the principal was unlinked and the instance had no low-trust isolation environment. That is a Geetorus governance boundary, not a GitHub transport failure, and it must not be presented as successful agent execution.
- The current draft endpoint whose id begins `a31` contains only a Geetorus-generated webhook secret. It has no verified GitHub App identity, private key, installation, repository, signed ping, conversation, or task.
...
The live setup attempt exposed a control-plane defect before GitHub credentials existed. Regenerating Geetorus's webhook secret was treated as rotation of a configured App, which moved the endpoint to `attention` and asked the operator to reconnect credentials that had never been supplied. That was a false degraded state, not a provider failure.

The committed fix distinguishes first-time setup from live credential rotation:

1. Geetorus generates a random 32-byte webhook secret server-side, vaults it through endpoint-owned secret references, returns the plaintext once from the board-authenticated setup-secret route, and marks the response `Cache-Control: no-store`.
...
5. **Runtime singleflight:** concurrent webhooks that arrive while a configured GitHub runtime is cold share one initialization promise. Geetorus installs one runtime and both requests proceed through it instead of racing duplicate adapter instances.
...
9. **Stable repository identity:** repository rename or transfer is reconciled through GitHub's immutable numeric repository ID. Geetorus preserves the resource, conversation, task, allowlist choice, and follow-up route while updating mutable owner/name coordinates and provider URLs; a conflicting dual-coordinate binding fails closed.
10. **Cold-start response budget:** the provider ingress deadline begins before runtime initialization. A signed webhook that cannot finish cold adapter startup inside the provider budget returns promptly and proceeds only through bounded durable retry instead of consuming GitHub's delivery timeout before Geetorus begins accounting for it.
11. **Provider-global App ownership:** the immutable numeric App registration
    id has one live Geetorus endpoint even if GitHub transfers the App to a
...
- Geetorus base used for the live run: `5da649986016e4010da8156f83f5bfc9c0128be4`
...
- Geetorus endpoint: `4e87c64e-7d0b-497d-85d2-6eb8820340fc` (archived during cleanup)
- External conversation: `github:cryppadotta/geetorus-chat-e2e-enabled:issue:1`
- Geetorus task: `9ad34556-30b5-47a1-b207-ba666d8d897e`
...
1. Geetorus generated and stored the webhook secret without exposing it through normal endpoint reads.
...
3. The App was installed on one selected private repository. Geetorus discovered that repository disabled by default.
4. A mention sent before Geetorus access was enabled was durably filtered with `Destination is not enabled in Geetorus`.
5. After enabling the repository, a root GitHub issue comment mentioning the immutable App bot created exactly one Geetorus conversation and one task.
6. A non-mention follow-up in the same GitHub issue remained in the subscribed conversation.
7. An explicit Geetorus board publication produced a GitHub bot reply and reached `published` state.
...
- Archived the Geetorus chat endpoint, which retired its endpoint-owned secrets but did not change any GitHub App registration, installation, repository grant, or webhook setting.
- Separately deleted all four disposable GitHub Apps in GitHub after qualifying the provider form and manifest paths.
- Deleted the explicitly disposable private repository `geetorus-chat-e2e-enabled`.
- Stopped the temporary registration server, public relay, and isolated Geetorus process.
