...
`geetorus_runner`, provider `codex`, model `gpt-5.6-luna`. The server's startup
...
and inspected Geetorus Activity and the provider. Returning to Settings proved
...
| -------- | ---------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Slack    | `#pc-chat-live-0905b`                    | 17:56:10.018    | Activity: filtered, “Destination is not enabled in Geetorus”; no reaction/reply                                                          |
| GitHub   | `cryppadotta/geetorus-chat-e2e-enabled` | 18:01:08.615    | Saved comment persisted after reload; signed webhook acknowledged at 18:01:10; content rejected before durable ingress; no reaction/reply |
...
[issuecomment-5574211696](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/2#issuecomment-5574211696).
...
- Slack's original private test channel, Discord `#general`, and GitHub's
  `geetorus-chat-e2e-enabled` repository were restored enabled.
- GitHub's separate `geetorus-chat-e2e-disabled` repository stayed disabled;
...
`geetorus-chat-e2e-enabled`, then sent `GITHUB-REACH-RECEIPT-0907-1325` at
...
[issuecomment-5574403287](https://github.com/cryppadotta/geetorus-chat-e2e-enabled/issues/2#issuecomment-5574403287).

Geetorus Activity showed “message ignored”, “Destination is not enabled in
Geetorus”, and **Sep 7, 2026, 1:25:02 PM** (local time). The rendered row was
...
`geetorus_runner` → `codex` → `gpt-5.6-luna`.
