...
const setupData = {
  Slack: {
    ...
    delivery: "Direct verified webhook", deliveryNote: "Advanced: Geetorus relay or Slack Socket Mode",
  ...
  GitHub: {
    bot: "Maya → maya-geetorus[bot]", identity: "Chat purpose · GitHub App recommended",
...
const settingsData = {
  Slack: {
    ...
    boundary: ["Root @maya → Slack thread", "One thread ↔ one Geetorus issue", "Bound replies need no mention"],
  ...
  GitHub: {
    ...
    capabilities: ["Mention activation · On", "Receipt reaction · On", "One edited GFM progress comment", "Files → Geetorus links", "Labels/trusted authors · Advanced"],
    ...
    fallback: "No stream/buttons/modals/DM → GFM text + Geetorus URL"
  },
  "Microsoft Teams": {
...
const interactionData = {
  ...
  "Microsoft Teams": [
    ...
    ["Ingress", "Verify bot activity + tenant/member", "Persist, scope-check, resolve Geetorus actor"],
...
function setupDesktop(screen) {
  ...
  return baseSvg(1280, 800, `${globalSidebar()}${topbar(`CONNECTORS  ›  Connect ${screen.provider}`)}${setupContext(screen.provider)}${heading(screen, "Provider handoff")}
    ...
    ${rc(504, 264, 344, 132)}${tx(528, 294, "IN GEETORUS", 12, "#666", 'font-weight="600"')}${tx(528, 324, d.delivery, 14, "#000", 'font-weight="700"')}${tx(528, 350, d.deliveryNote, 12, "#666")}${tx(528, 378, "Public endpoint copied · deployment detected", 12, "#666")}
...
function interactionsDesktop(screen) {
  ...
  return baseSvg(1280, 800, `${globalSidebar()}${topbar(`CONNECTORS  ›  Maya on ${screen.provider}  ›  Interaction model`)}${detailContext(screen.provider, "Conversations")}${heading(screen)}${tx(504, 188, "NATIVE EVENT", 12, "#666", 'font-weight="600"')}${tx(650, 188, "PROVIDER + GEETORUS RESULT", 12, "#666", 'font-weight="600"')}${rendered}${tx(504, 732, "All paths use durable delivery, current authorization, one task binding, and safe outbound projection.", 12, "#666")}${annotations(rows.map((_, index) => ({x:496,y:210+index*98,w:736,h:98})) )}`);
}
...
function setupMobile(screen) {
  ...
  return baseSvg(375, 812, `${mobileHeader("Connectors")}${mobileTitle(screen, "Setup")}
    ...
    ${rc(16, 254, 343, 92)}${tx(36, 282, "IN GEETORUS", 12, "#666", 'font-weight="600"')}${tx(36, 310, d.delivery, 14, "#000", 'font-weight="700"')}${tx(36, 332, d.deliveryNote.slice(0, 48), 12, "#666")}
