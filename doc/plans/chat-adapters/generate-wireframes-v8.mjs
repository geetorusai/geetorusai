...
function globalSidebar(height) {
  ...
  return `<g data-region="global-sidebar">${tx(24, 40, "Geetorus", 20, "#000", 'font-weight="700"')}${items
    .map((item, index) => {
      const y = 80 + index * 48;
...
function accessDesktop(data, provider) {
  ...
  const body = [
    ...
    tx(
      528,
      402,
      "Can message enabled conversations and attach safe files. Cannot approve or administer Geetorus.",
      12,
      "#666",
    ),
    ...
    tx(
      512,
      516,
      "Linked people use their current Geetorus permissions.",
      12,
      "#666",
    ),
    ln(512, 544, 1224, 544),
  ];
  data.linked.forEach(([external, geetorus, action], index) => {
    const y = 560 + index * 80;
    if (index) body.push(ln(528, y, 1208, y, 'stroke="#e6e6e6"'));
    body.push(tx(528, y + 28, external, 14, "#000", 'font-weight="600"'));
    body.push(tx(528, y + 52, geetorus, 12, "#666"));
  ...
  const body = [
    ...
    multiline(
      ...
      [
        ...
        "Cannot approve or administer Geetorus.",
      ],
      12,
      "#666",
      18,
    ...
    tx(16, 588, "Uses current Geetorus permissions.", 12, "#666"),
    ln(16, 616, 359, 616),
  ];
  data.linked.forEach(([external, geetorus, action], index) => {
    ...
    body.push(tx(24, y + 52, geetorus, 12, "#666"));
...
function conversationsDesktop(data, provider) {
  ...
  const body = [
    ...
    tx(744, 216, "GEETORUS TASK", 12, "#666", 'font-weight="700"'),
...
const sharedDefinitions = [
  ...
  },
  {
    ...
    subtitle: "A normal Geetorus task connected to its provider conversation.",
    group: "Geetorus",
    tab: "Task",
    annotations: [
...
for (const provider of providers) {
  ...
  const settingsAnnotations = [
    ...
    "Each toggle is Geetorus's independent allow or deny decision.",
    "The provider action changes availability; newly discovered destinations remain disabled.",
    ...(data.conversationToggles.length
      ? ["Private-conversation reach is an explicit Geetorus choice."]
      : []),
  ];
  detailScreens.push({
    id: data.id,
    ...
    rationale:
      "Provider membership is the ceiling; Geetorus enablement is the narrower enforcement boundary.",
  ...
  detailScreens.push({
    ...
    annotations: [
      ...
      "The restricted profile permits task conversation but never Geetorus governance.",
      `Linked accounts map a stable ${data.identityHint} to a Geetorus user and can be revoked.`,
    ],
    rationale:
...
function flowSvg() {
  ...
  return baseSvg(
    1280,
    880,
    `${tx(48, 48, "Chat connector product flow", 28, "#000", 'font-weight="700"')}${tx(48, 80, "The provider grants availability; Geetorus grants permission to act.", 14, "#666")}
...
function uiDocument() {
  ...
  const permissions = permissionModel
    ...
    .join("\n");
  return `# Geetorus Chat Adapters UI Surfaces — v8\n\nDate: 2026-09-04  \nOriginal planning base: \`${baseSha}\`; release qualification records the exact tested revision separately.  \nReview viewer: [\`index.html\`](./index.html)  \nWireframes: [\`wireframes-v8/\`](./wireframes-v8/)\n\n## Permission model\n\n${permissions}\n\n## Access tab\n\n**Settings answers where the bot may work. Access answers who an external sender represents and what Geetorus authority applies.** A linked external identity acts as its mapped Geetorus user and is checked against current permissions on every action. An unlinked identity may be allowed under the fixed restricted profile: it can converse within enabled resources and attach safe files, but it cannot approve, change budgets, hire, manage permissions or connections, or reassign agents. The connection owner remains an internal audit and authority ceiling; it is not ordinary UI configuration.\n\n## Conversations tab\n\nEach provider has one plain list. Every row contains the external conversation, Geetorus task, current state, an Open-provider link, and Open task. There is no separate binding-management section or conversation-boundary explainer. If provider access disappears, the row becomes unavailable while its history remains inspectable.\n\nThe former \"How conversations work\" screens are removed. Provider-native activation and reply behavior remains implementation documentation, not a standalone product page.\n\n## Screen inventory\n\n| ID | Group | Surface | Title | Desktop | Mobile |\n|---|---|---|---|---|---|\n${inventory}\n\n## Annotation and action notes\n\n${details}\n`;
}
...
function viewerHtml() {
  ...
  const sections = groups
    .map(
      ([label, screens]) =>
        `<div class="provider-break"><div class="lede">${esc(label)}</div><h2>${label === "Start" ? "Shared connection start" : label === "Geetorus" ? "Shared Geetorus surfaces" : `${esc(label)} connector`}</h2></div>${screens
          .map((screen) => {
