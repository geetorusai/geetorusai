# Geetorus Chat Adapters — Live Browser E2E Runbook

**Status:** executable implementation and release acceptance contract

**Date:** 2026-09-04

**Geetorus source:** `codex/chat-adapters`; every execution records the exact tested SHA and contemporaneous `origin/master` revision in its qualification result. The runbook itself is revision-independent and must not be read as proof for whichever commit happens to be current.
...
This is the runbook I will use to qualify each real chat adapter through its actual provider UI and the Geetorus UI. It is not a mock-only Playwright plan and it does not assume database access as proof. The browser journey must demonstrate that a provider event becomes exactly one Geetorus task, that the assigned Geetorus agent runs under normal governance, and that only safe output returns to the same provider conversation.

This document is the browser acceptance contract during implementation and the stable release gate afterward. A scenario is not complete until its visible provider state, visible Geetorus state, and durable Activity/Conversation records all agree.
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| Slack           | Customer-owned Slack app created from Geetorus's manifest; Bot User OAuth Token and Signing Secret entered once         | Managed **Add to Slack** OAuth installation    |
| GitHub          | Customer-owned GitHub App; Geetorus-generated webhook secret copied to GitHub, then App ID and private key entered once | GitHub App Manifest create-and-return exchange |
...
Direct verified webhooks are the required transport for Slack, GitHub, Teams, and Telegram. Discord uses a direct outbound Gateway connection and therefore does not require a public Geetorus URL. A private-instance relay, Slack Socket Mode, and Telegram polling are separate conditional deployment tests; none is a choice in the endpoint wizard.

A webhook provider is not deployment-qualified merely because it passed through a temporary tunnel. Live development may use an ephemeral HTTPS tunnel to find product defects, but stable release evidence requires a durable public ingress origin whose callback URLs survive process restarts and whose Geetorus secrets master key is preserved with the instance.
...
Maya E2E uses native Geetorus Runner with Codex `gpt-5.6-luna`; actual native
...
- **Telegram:** broad live evidence covers private chat, group/topic isolation, FIFO/bursts, commands, edits/reactions, documents, native confirmation continuation, and exact once-only final presentation. After rotating the expired test tunnel on the merged build, a fresh task returned exact `TELEGRAM-MERGED-A-0906` once and edited its working placeholder in place. A later code audit found that URL-changing reconnects could ask Telegram to drop queued updates; the working-tree fix preserves provider backlog on every reconnect and drops stale pre-Geetorus updates only during initial setup. That repair has focused deterministic coverage but has not yet been proved with a real queued-update outage. It is not a complete TG1–TG6 pass; identity governance, media boundaries, rate-limit recovery, token rotation, and cleanup remain incomplete.
...
2. **Reach is enforced:** provider installation or invitation only makes a resource available; Geetorus independently decides whether it is enabled.
3. **Identity and governance hold:** linked people use current Geetorus permissions, while allowed unlinked people remain inside the restricted external profile.
...
- the Geetorus Connectors catalog, setup wizard, Settings, Access, Conversations, Activity, agent, and task screens;
...
- identity-link confirmation as the mapped Geetorus user;
- screenshots and visible-state assertions at each evidence checkpoint.

The required v1 journeys stay in the browser. If Geetorus later ships a product-displayed one-time helper command, I may execute that command exactly as shown and return to the browser; I do not replace UI steps with private APIs.
...
- Use a separate authenticated browser profile/context for the installer, linked participant, and unlinked participant. Never switch identities in a way that leaves an ambiguous provider or Geetorus session.
- Never read secrets back from Geetorus, browser storage, cookies, or password managers. Secret entry is write-only and screenshots must show only masked values.
...
| ------------------------------ | ---------------------------------------------------------------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Deterministic provider fixture |                                                     Every pull request | Browser coverage of Geetorus setup, Settings, Access, Conversations, Activity, task attribution, durable delivery, deduplication, and publication without external provider flakiness. |
...
Before opening a real provider, I run the deterministic Geetorus-side browser case for that provider:
...
2. Perform the provider or Geetorus action through the visible browser UI.
...
5. Open the paired Geetorus and provider records from their own links; never infer the pairing from similar text alone.
...
The normal visibility budgets are 15 seconds for provider acknowledgement or durable inbound Activity, 30 seconds for conversation/task creation, 120 seconds for the deterministic agent result, and 30 seconds for publication after the Geetorus comment is committed. Exceeding a budget triggers triage; it does not justify clicking twice or creating a second root message.
...
| -------------------- | ------------------ | ---------------------------------------------------------------- |
| Installer            | Dana E2E           | Geetorus, provider app administration, provider conversation    |
| Linked participant   | Ari E2E            | Provider conversation, Geetorus identity-link confirmation      |
...
### 3.1 Required Geetorus fixture

Use an authorized Geetorus staging instance with real HTTPS webhook callbacks.
...
qualification it must use the new Geetorus Runner with Codex, initially
`gpt-5.6-luna`. Verify actual admitted run records show `adapterType:
geetorus_runner`, `runtimeMode: native`, `driverKind: codex_app_server`, and
...
| `GOVERN <run-id>`    | Create a governed Geetorus approval and publish only the provider-safe approval status/link.                          |
...
when the native runner makes them available to Geetorus.

### 3.2 Required people

| Role                 | Geetorus identity            | Provider identity              | Purpose                                                        |
| -------------------- | ----------------------------- | ------------------------------ | -------------------------------------------------------------- |
| Installer            | Dana E2E · company admin      | Provider sandbox administrator | Creates the connection and changes Settings/Access.            |
| Linked participant   | Ari E2E · ordinary member     | Separate provider member       | Confirms identity linking and current Geetorus authorization. |
| Unlinked participant | Jules E2E · no Geetorus link | Separate provider member       | Exercises restricted external access and link-required denial. |

The provider bot identity is dedicated to `Maya E2E`. It must not share a native bot identity with another Geetorus agent endpoint.

### 3.3 Resource naming and isolation

All resources must be disposable or explicitly designated for Geetorus testing:

| Provider | Available/enabled fixture                | Available/disabled fixture          |
| -------- | ---------------------------------------- | ----------------------------------- |
| Slack    | `#pc-e2e-enabled`                        | `#pc-e2e-disabled`                  |
| GitHub   | `geetorus-chat-e2e-enabled`             | `geetorus-chat-e2e-disabled`       |
| Discord  | `#pc-e2e-enabled`                        | `#pc-e2e-disabled`                  |
| Teams    | `Geetorus Chat E2E / Enabled`           | `Geetorus Chat E2E / Disabled`     |
| Telegram | `Geetorus Chat E2E Enabled` group/forum | `Geetorus Chat E2E Disabled` group |
...
| Telegram        | The BotFather bot and chat memberships; only Geetorus's webhook and command menu are removed automatically |

### 3.5 Shared preflight

Before starting a provider run:

1. Confirm Geetorus health and sign in as Dana E2E.
2. In instance **Experimental** settings, confirm **Chat connectors** is enabled
   ...
   Geetorus task through that runner before sending a provider test message.
...
5. Confirm the verified webhook ingress is publicly reachable when qualifying Slack, GitHub, Teams, or Telegram. For a private board, set `GEETORUS_CHAT_WEBHOOK_PUBLIC_URL` to the HTTPS webhook-only origin and keep `GEETORUS_PUBLIC_URL` at the real board origin. Public `POST /api/chat-webhooks/*` may be forwarded; the private board/API must not be. Verify external task links use the board, never the webhook-only host; local/private links should be omitted with neutral instructions. For Discord, confirm outbound HTTPS/WebSocket access to Discord instead. Relay qualification is a separate deployment run described in section 10.
6. For first-time setup, use a new dedicated bot identity and disposable endpoint.
   ...
   history. Geetorus removal does not uninstall provider resources, except
   that Telegram webhook/menu cleanup is automatic.
7. Start browser recording/screenshots before `/apps`; record the Geetorus SHA and current time.
...
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Slack           | Sign in to the sandbox workspace, approve installation, or paste the customer-owned bot token/signing secret into Geetorus's masked fields. | Resume at the Slack consent result, verify the requested scopes, invite the bot, and execute S1–S7. |
...
| Discord         | Sign in to the developer portal, complete CAPTCHA, approve the server install, or paste the bot token into Geetorus's masked field.         | Verify the bot identity, Message Content intent, exact install permissions, and execute DC1–DC7.    |
| Microsoft Teams | Sign in to the test tenant, satisfy tenant-admin consent, or enter the client secret in Geetorus's masked field.                            | Verify the created bot/app identity and install target, then execute T1–T7.                         |
| Telegram        | Sign in to Telegram Web or copy the BotFather token into Geetorus's masked field.                                                           | Verify the bot identity with the provider, then execute TG1–TG6.                                    |
...
### C2 — Provider availability versus Geetorus enablement
...
5. Wait beyond the normal event-to-acknowledgement window, then inspect Geetorus Conversations, Tasks, and Activity.

**Pass:** the provider may deliver the event, but Geetorus creates no task, wakes no agent, and publishes no response. Activity records an ignored delivery with safe metadata. After Dana enables the destination and repeats with a new marker, exactly one task is created.
...
4. Verify the provider shows a private or concise safe denial/link and Geetorus records a denied authorization. The approval remains unresolved.
5. In **Access**, create an identity link for Ari. Open the one-time link in Ari's Geetorus browser session, verify both identities/company, and confirm.
6. Have Ari create or continue work. Verify the task attributes Ari as the linked Geetorus user.
...
2. In the Geetorus task, add board comment `INTERNAL-<run-id>` without **Send to channel**.
3. Confirm it never appears at the provider.
4. Add `PUBLIC-<run-id>` with **Send to channel** selected.
5. Confirm it appears once in the bound provider conversation with delivered status in Geetorus.
...
**Pass:** only safe milestones/final output and explicit board publication leave Geetorus. No internal material is exposed.
...
2. Verify Geetorus stores a bounded normal attachment, Maya reads the marker, and the result file is reachable through a provider-supported upload or an authenticated Geetorus task link. Never expose the private Board or an asset through the public webhook origin.
...
7. Verify Geetorus appends a correction/tombstone rather than rewriting audit history.
...
- Permission to install a Geetorus Slack app and invite it to the two test channels.
...
1. In Geetorus, perform C1 and select Slack.
2. On **Connect a Slack app**, copy the generated manifest and open Slack app settings.
3. In Slack, choose **Create New App** → **From an app manifest**, select only the sandbox workspace, paste the manifest, and inspect its bot scopes before clicking **Create**. Abort if Slack shows scopes beyond the versioned Geetorus manifest.
...
4. Open **OAuth & Permissions**, click **Install to Workspace**, review the consent page, approve it, and copy the **Bot User OAuth Token** into Geetorus's masked field.
5. Open **Basic Information**, reveal the **Signing Secret**, and paste it directly into Geetorus's masked field. Do not capture either secret.
6. Click **Connect Slack app**. Geetorus verifies the token and advances to **Finish Slack setup**.
...
8. Return to Geetorus and click **Start Slack message test**.
...
11. Return to Geetorus, click **I've sent the test message** once, and verify Settings opens with `#pc-e2e-enabled` enabled.
...
2. Select only the sandbox workspace, inspect the requested scopes, approve installation, and return to Geetorus.
...
2. Refresh Slack Settings in Geetorus if discovery is not pushed immediately.
3. Verify the row says invited/available but is off.
4. Run C2.
5. Remove Maya from that Slack channel and refresh.

**Pass:** Slack membership is the provider ceiling. Geetorus's toggle is the independent allowlist. Removal marks the row unavailable and blocks new work without erasing the previous task row.
...
- files ingest and publish without exposing Geetorus credentials;
...
- as a linked non-viewer, click native **Stop** during a long response. Verify the exact task/run is stopped, the visible confirmation says it stopped at your request, and working status clears. Repeat the same event, deliver it late after a new turn, and try as an unlinked or revoked identity: no later/unrelated run may be cancelled. The Geetorus Activity tab records the result;
...
3. Complete that task in Geetorus and send `ECHO <run-id>-DM2`; confirm a new task.
...
6. Run the registered slash command with a task body. Verify Slack gets exactly one `Starting a task…` root, the Conversations row uses that message as its native thread boundary, and every Geetorus response remains under that root. In a DM, verify the next `status` and `close` commands target that same task rather than creating or controlling an unrelated base-DM task.
7. During deterministic fault injection, simulate a connection loss after Slack may have accepted the starter root. Verify Activity labels the task start unconfirmed and does not replay it automatically. After checking Slack, exercise both explicit paths: **Retry anyway** warns that it can duplicate both the starter and task, while **Cancel task start** leaves no Geetorus task. Verify a concurrent double-click produces at most one retry and each operator decision has an activity-log audit row.
...
Capture all shared evidence plus the Slack OAuth scope screen, thread, DM lifecycle, modal, file, disabled-channel result, Conversations row, and deduplicated delivery. Remove the disposable custom App, delete test messages/channels only when the sandbox cleanup policy allows it, revoke identity links, and remove the Geetorus test connections through the UI. Preserve Geetorus tasks/activity for audit unless the whole E2E company is designated disposable.
...
1. In Geetorus, perform C1 and select GitHub → **Chat with an agent**.
2. On **Create or connect a GitHub App**, copy the Geetorus webhook URL and click **Open new GitHub App form**.
3. In the sandbox organization, open **Settings** → **Developer settings** → **GitHub Apps** → **New GitHub App**.
4. In Geetorus, click **Generate webhook secret**. Copy the one-time value immediately, then enter the Geetorus webhook URL and generated secret in GitHub with the webhook active and SSL verification enabled. Geetorus must show only the configured state plus **Waiting for GitHub to deliver its signed webhook ping…** after refresh. If **Regenerate webhook secret** is used, update GitHub before expecting another webhook to verify.
...
7. Return to Geetorus and wait for **GitHub has verified this webhook.** A correctly signed GitHub `ping` must set this state; an unsigned or incorrectly signed `ping` must not. **Connect and verify** remains disabled until this proof arrives.
8. Copy the numeric **App ID** into Geetorus. Under **Private keys**, generate and retain the downloaded PEM, then use **Choose .pem file** in Geetorus (or paste the multiline key into its masked field). Confirm the loaded state without revealing, recording, or screenshotting the key. Empty, unreadable, or over-64-KiB files should show a safe inline error; imports must preserve line breaks and never upload a file separately from the credential setup request.
...
10. Return to Geetorus and click **Connect and verify**. Geetorus must verify the App identity without displaying the secret or private key again.
11. In `geetorus-chat-e2e-enabled`, open a new issue titled `<run-id> setup`, comment `@<verified-bot-login> ECHO <run-id>-SETUP`, then add an unmentioned follow-up comment.
12. Return to Geetorus, run the setup test once, and verify Settings opens with the tested repository enabled and the second installation repository disabled.

**Pass:** Geetorus generates and stores the webhook secret, returns it only once for copying to GitHub, requires a correctly signed setup ping before credential verification, and asks the operator to enter only App ID and private key; least-privilege repository permissions are visible in GitHub; the real issue conversation completes setup; no PAT is used.

### G2 — Conditional GitHub App Manifest setup

Run only after Geetorus ships a **Create in GitHub** manifest exchange:
...
**Pass:** Geetorus exchanges the one-time code server-to-server and never asks the operator to paste App ID, private key, or webhook secret. Until this control exists, record G2 as **NOT SHIPPED — NON-BLOCKING**, not failed.

### G3 — Repository reach

1. Run C2 with the disabled repository.
2. In GitHub App installation settings, remove the disabled repository while Geetorus has it enabled.
3. Return to Settings and refresh.

**Pass:** GitHub installation selection is the provider ceiling; Geetorus is the narrower enablement layer. Removed repository access becomes unavailable and no new work occurs there.

### G4 — GitHub conversation boundaries

1. In an enabled repository issue, mention Maya and run C4 using ordinary issue comments.
2. In the seeded pull request's main conversation, mention Maya and record the Geetorus task.
...
- rich actions/forms fall back to explanatory text plus an authenticated Geetorus link;
- public inbound GitHub uploads referenced in the exact admitted comment are ingested from canonical `github.com/user-attachments/assets/…` or `files/…` URLs. Test a PNG and text file in issue, PR, and inline-review comments, then verify exact bytes on the Geetorus task and native agent inspection. Arbitrary external links are not fetched;
...
- Team `Geetorus Chat E2E` with the Enabled and Disabled channels.
...
1. In Geetorus, perform C1 and select Microsoft Teams. Keep **Connect Maya to Microsoft Teams** open and copy the displayed Geetorus messaging endpoint. Follow the on-screen **Microsoft portal field map** and use **Copy manifest settings** as the Geetorus-specific manifest reference.
...
3. In Azure, create an **Azure Bot** using that existing Application ID and the single-tenant identity type. Set its messaging endpoint to Geetorus's displayed URL and enable its Microsoft Teams channel.
4. In Teams Developer Portal, select **Apps > New app**. Under **Configure > App features > Bot**, add the existing bot using the same Application ID and enable Personal, Team, and Group chat scopes plus file support. Under **Configure > Permissions**, add the RSC application permissions in the next step. Complete the required app metadata and icons; Geetorus's copied block is a field reference, not a complete app package.
5. Add the resource-specific application permissions required by the shipped manifest: `ChannelMessage.Read.Group` and `ChatMessage.Read.Chat`. These RSC grants let the installed app receive every message in that team or group chat without an `@mention`; explain that provider access in the app description shown to installers. Geetorus still retains and acts only on messages admitted by its reach/access rules. Do not grant tenant-wide directory/history permissions.
   Keep the copied `webApplicationInfo.id` equal to the same Entra Application ID so Teams can bind those RSC permissions to the app, and keep its nonempty RSC-only `resource` value. Geetorus does not use Teams single sign-on in this release; the resource is only an RSC placeholder, so this connection does not require registering an Entra Application ID URI or adding delegated Microsoft Graph permissions. One team install covers that team's standard channels. Private and shared channels require a separate installation and are not supported by this release.
...
7. Return to Geetorus. Enter only Application/Client ID, Directory/Tenant ID, and the client-secret value, then click **Verify Microsoft credentials**. Confirm the secret remains masked and is not shown again.
8. In Teams, open the app installation surface, click **Add**, and install it into `Geetorus Chat E2E` and personal scope when prompted.
...
10. Return to Geetorus, run the setup test once, and verify Settings opens with the tested channel enabled.
...
2. Deliver at least one installation or message lifecycle event from each test channel before expecting it in Geetorus. For an untouched channel, send a benign unmentioned discovery message; Geetorus does not fetch a Teams channel inventory proactively.
3. In Geetorus Settings, confirm both discovered team channels are available but only Enabled is on.
...
**Pass:** Teams app installation is the provider ceiling; Geetorus independently enables individual channels.

### T4 — Channel threads and delivery grant

1. In Enabled, start a new channel post with `@Maya ECHO <run-id>-ROOT`.
2. Confirm Maya replies beneath that post and one Geetorus task is created.
...
**Pass:** provider installation makes each surface available; Geetorus's DM/group settings control eligibility.
...
- personal-chat Bot Framework file-download attachments are ingested only when
  ...
  passes Geetorus's allowed-content policy and configured size ceiling (10 MB
...
- denials use targeted activity when supported, otherwise DM or concise text plus a Geetorus link;
...
- edit a source message, soft-delete it, restore it, then edit it again; verify
  Geetorus records exactly one correction, tombstone, restoration, and later
...
5. Remove the recipient's Geetorus authority between the card and acceptance.
...
1. Send a small PNG from Teams. Verify its exact bytes belong to the current
   Geetorus input, and Maya can inspect that picture rather than an earlier
...
Capture Microsoft consent/install scope, Enabled/Disabled behavior, channel thread, reply-permission mode, DM/group behavior, Adaptive Card/task module, Conversations rows, and Activity repair state. Remove the disposable app from extra teams and chats, delete the test app registration/Azure Bot only when created for this run, revoke identity links, and remove the Geetorus connection. Do not delete the shared developer tenant or baseline team.
...
1. In Geetorus, perform C1 and select Telegram.
2. Click **Open BotFather**.
3. In Telegram Web, send `/newbot`, enter `Maya E2E <run suffix>`, and choose a unique username ending in `bot`.
4. Copy the returned bot token directly into Geetorus's masked write-only field. Do not screenshot or record it.
5. Click **Connect**.
6. Open the bot's private chat, click **Start**, and send `ECHO <run-id>-SETUP`.
7. Return to Geetorus and verify setup completes with direct messages enabled.

For routine nightly smoke, reuse a dedicated pre-provisioned bot but create a fresh Geetorus connection. Never attach the same bot token to two active endpoints.
...
2. If a membership callback has not discovered a group yet, send `/task@MayaBot DISCOVER <run-id>` in it so Telegram delivers a provider-native command to Geetorus.
3. Open Geetorus Settings. Verify both groups are available and Disabled remains off.
...
If Telegram upgrades an enabled basic group to a supergroup while Topics are enabled, verify Geetorus marks the old chat unavailable and disabled, carries the explicit Geetorus enablement to the replacement supergroup, preserves its human title, and keeps any existing topic-to-task bindings on the same topic IDs.

**Pass:** Telegram membership/discovery is the provider ceiling. Geetorus enables the narrower set of groups/topics.
...
- `FORM` uses inline keyboard buttons; fields that require a modal fall back to a Geetorus link or sequential prompts;
- `/task <request>`, `/new`, `/status`, and `/close` are parsed as the documented small command vocabulary, and Geetorus registers that menu automatically;
...
2. Open the linked Geetorus task. The saved answer and task/run history remain
...
1. In Geetorus, perform C1 and select Discord.
2. Open Discord Developer Portal and create one application dedicated to `Maya E2E`. Copy its Application ID.
3. On the Bot page, enable the **Message Content Intent**, reset the token if necessary, and paste the token only into Geetorus's masked field.
4. Enter the sandbox Server ID in Geetorus. Once the Application ID and Server ID are present, inspect Geetorus's generated **Install bot in this server** link before opening it.
...
7. Click **Connect Discord bot**. Geetorus must verify that the token belongs to the Application ID, Message Content is enabled, the bot is installed in the stated server, and at least one text channel grants the complete feature set.
8. Enable only `#pc-e2e-enabled` in Access. In that channel, send a new root message containing `@Maya E2E ECHO <run-id>-SETUP`, then reply once inside the thread Geetorus creates.
9. Return to Geetorus, complete the message test, and verify Settings opens for the same immutable agent and Discord server.
...
1. Confirm the bot can see both sandbox text channels, but only Enabled is on in Geetorus.
...
5. Remove the bot from the server only during the disposable recovery case; verify Geetorus cannot treat its own allowlist as a substitute for provider membership.

**Pass:** Discord membership and effective channel permissions are the provider ceiling. Geetorus's per-channel allowlist is an independent, narrower gate.

### DC3 — Thread boundary, ordering, and duplicate safety

1. In Enabled, send one root mention with `ECHO <run-id>-ROOT`.
2. Verify Geetorus creates exactly one Discord public thread named from the request text with the bot mention removed and creates exactly one Geetorus task.
...
5. Mention Maya in a second root message. Verify a second Discord thread and second Geetorus task are created, with no cross-publication.
...
1. On the existing dedicated bot, verify `/geetorus status`, `/geetorus new`
   and `/geetorus close` appear after automatic registration. Existing unrelated
...
2. Verify the DM uses its own Geetorus task generation and cannot reuse a guild-channel thread.
...
4. Revoke the Geetorus identity link during a queued follow-up and verify authorization is rechecked before the agent wakes or publishes.

**Pass:** the DM toggle is enforced, guild and DM scopes cannot cross, and mutable Discord names never confer Geetorus identity.
...
6. Exercise one bounded REST failure and one provider `retry_after` longer than 60 seconds. Verify Geetorus stops a hung request at 25 seconds, preserves Discord's structured status/`retry_after`, does not retry at the old 60-second boundary, and schedules only the allowed durable retry at the provider's full backoff window.
...
Capture the Developer Portal intent screen, exact OAuth permission request, server install, enabled/disabled channel states, two root threads, queued replies, reaction/edit/delete behavior, DM result, Conversations rows, and Activity recovery state. Remove the Geetorus connection and verify its Gateway runtime stops, credential references and stored secret rows are cleared, the endpoint is archived, and retained Conversations/task history is marked `endpoint_removed` rather than erased. Remove only the disposable bot/application or test messages authorized for provider cleanup, and revoke identity links. Never capture or record the bot token.
...
1. Start a private Geetorus instance with no inbound public route.
...
**Pass:** the provider journey is unchanged; relay is instance transport, not endpoint configuration. The relay cannot act as a Geetorus user or invoke arbitrary APIs.
...
| Rich cards               | Block Kit                     | GFM + Geetorus link                      | Discord embed                            | Adaptive Card                                           | Formatted text/inline keyboard           |
...
| Commands                 | Registered slash command      | Text mention vocabulary only              | Registered `/geetorus status/new/close` | Card/message vocabulary                                 | `/new`, `/status`, `/close`              |
...
2. visible Geetorus state and current URL;
...
Never “fix” a failing run by manually editing a task, changing the assigned agent, enabling a broader provider permission, deleting the duplicate evidence, or bypassing Geetorus's Settings/Access enforcement.
...
- [ ] Actual live runs use the new Geetorus Runner with the recorded Codex model and binary SHA; deterministic model fixtures are not counted as live proof.
...
- [ ] Provider-available versus Geetorus-enabled reach passed, including the disabled-resource negative case.
...
The final result is **PASS** only when all blocking checks pass on the same Geetorus SHA and adapter version. A conditional provider fallback is acceptable only when the UI advertised that exact fallback before the user depended on the unavailable native behavior.

## 14. Provider operator references

These official references are the browser runner's drift checks when a provider renames or moves a setup control. The permissions and events displayed by the versioned Geetorus setup remain the test's exact least-privilege contract; a changed provider UI is not permission to grant more access.
