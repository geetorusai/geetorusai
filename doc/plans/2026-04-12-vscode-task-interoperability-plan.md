...
Geetorus should not replace its workspace runtime service model with VS Code tasks.
...
- Geetorus owns long-running workspace services and their desired state
- Geetorus shows operators exactly which named thing they are starting or stopping
- Geetorus distinguishes long-running services from one-shot jobs
...
This is directionally correct for Geetorus because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Geetorus does not yet leverage existing workspace metadata.

## Recommendation

Keep Geetorus runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Geetorus.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Geetorus service or job semantics should become runnable in Geetorus.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Geetorus runtime services.
...
That creates a bad fit if Geetorus tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Geetorus's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Geetorus should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Geetorus can resolve
...
- selected variable substitution when Geetorus can resolve it safely from workspace context
- mapping task metadata into Geetorus readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Geetorus cannot execute directly
...
Geetorus needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Geetorus metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Geetorus
...
For a workspace with `cwd`, Geetorus should look for `.vscode/tasks.json`.
...
Geetorus should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Geetorus metadata marks it as a service, or
...
Geetorus-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Geetorus would import and why.
...
Goal:
run supported imported service commands through the current Geetorus supervisor.
...
Success condition:
imported service commands behave like native Geetorus services once adopted.
...
- removal of Geetorus's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Geetorus to VS Code and making the model worse for non-VS-Code repos
...
Geetorus should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Geetorus workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
