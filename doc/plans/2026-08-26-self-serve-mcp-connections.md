...
Geetorus treats a connection method as the capability boundary. A curated MCP method may declare automatic OAuth registration (`dcr`, including CIMD), a customer-owned OAuth client (`customer`), an API key, or a provider-generated MCP URL. Provider tokens and client secrets remain in the instance's encrypted vault. Geetorus ID remains the future broker for `platform_shared` registrations; the self-serve catalog does not depend on it.
...
- [x] Keep Geetorus ID limited to explicitly brokered `platform_shared` methods such as Gmail.
...
|---|---:|:---:|:---:|---|
| Jira | 1 | [x] | [ ] | Reference DCR/CIMD flow; `https://mcp.atlassian.com/v1/mcp/authv2`. On 2026-08-27, browser authorization, 26-tool discovery, tenant lookup, a safe project read, JQL search, and reconnect passed against `geetorusteam.atlassian.net`; revocation remains pending because the working connection was retained. Atlassian's `/authv2` rollout requires the reviewed protected-resource scope set to be sent explicitly before consent. |
| Airtable | 1 | [x] | [ ] | Enterprise client allowlisting may apply. On 2026-08-27, browser authorization was limited to the single `Untitled Base`, tool discovery succeeded, and `List Airtable bases` returned that base through Geetorus; reconnect/revoke and the final log audit remain pending. |
...
| Make | 1 | [x] | [ ] | DCR. |
| Manufact | 1 | [x] | [ ] | DCR. |
| Miro | 1 | [x] | [ ] | Enterprise client restrictions may apply. On 2026-08-27, the first live exchange found that Geetorus overrode Miro's advertised DCR client-auth order and selected `client_secret_basic`; preserving the provider's `client_secret_post` preference fixed the exchange. Reauthorization, 60-tool discovery, and `Who Am I` then succeeded; reconnect/revoke and the final log audit remain pending. |
...
| Vercel | Blocked | [x] | n/a | Reconsider when reviewed-client approval is removed or Geetorus is approved. |
...
- [ ] Gmail — intentionally unavailable on this instance because its Geetorus ID connector is not configured; Browse shows the instance-provided configuration notice instead of starting OAuth.
...
| # | App | Method / declared ownership | Credential-free preflight | Callback / registration source | Browser, callback/token, and catalog outcome | Prerequisite / Geetorus error code | Conclusion |
|---:|---|---|---|---|---|---|---|
...
| 15 | ClickHouse | Recommended hosted OAuth / `dcr` | Reachable; OAuth metadata yes; DCR yes; CIMD no | Exact callback / no registration attempted | Geetorus required a ClickHouse Cloud service ID before OAuth; no callback, token, or catalog | Service selection required; error code none | `resource_selection_needed` |
...
| 19 | Supabase | Recommended hosted OAuth / `dcr` | Reachable; OAuth metadata yes; DCR yes; CIMD no | Exact callback / no registration attempted | Geetorus required a project reference before OAuth; no callback, token, or catalog | Development project selection required; error code none | `resource_selection_needed` |
...
## Geetorus Cloud managed OAuth broker — 2026-08-31

The managed-callback P2 is part of the existing Geetorus Cloud application at
`my.geetorus.app`. It does not add a service, hostname, repository, login
system, or provider route to Geetorus ID. Geetorus ID authenticates the user
...
code exchange, refresh, and revocation. The originating Geetorus instance is
the only durable provider-token vault and continues to execute provider tools
directly.

The production Google callback is fixed at
`https://my.geetorus.app/v1/connector/oauth/google/callback`; the reserved Box
path remains dark until Geetorus owns a distributable, provider-approved Box
...
sequenceDiagram
    ...
    participant P as Self-hosted Geetorus
    participant C as Geetorus Cloud
    participant I as Geetorus ID

    U->>P: Enable Geetorus-managed connections
    ...
    P-->>U: Open my.geetorus.app/connections/enroll
...
sequenceDiagram
    ...
    participant C as my.geetorus.app
    participant I as Geetorus ID
...
```mermaid
sequenceDiagram
    participant P as Geetorus instance
    participant C as Geetorus Cloud
    participant O as Provider OAuth
...
- “Self-serve” allows normal accounts, subscriptions, tenant-admin policies, and OAuth consent, but excludes a Geetorus/provider partnership.
...
- This program covers hosted remote MCP connections and credential custody. Generic REST execution and Geetorus-ID-managed shared OAuth registrations remain separate follow-up programs.
