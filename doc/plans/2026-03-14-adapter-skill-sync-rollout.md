...
This document defines the rollout plan for adapter-wide skill support in Geetorus.
...
- Geetorus stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Geetorus currently has these adapters:
...
1. Desired skills live in Geetorus for every adapter.
...
These adapters have a stable local skills directory that Geetorus can read and manage.
...
These adapters do not have a meaningful Geetorus-owned persistent install state.
...
- desired skills are stored in Geetorus
...
- desired skills stored in Geetorus
...
- Geetorus reconciles desired skills into `~/.cursor/skills`
...
- Geetorus reconciles desired skills into `~/.gemini/skills`
...
- Geetorus reconciles desired skills into `~/.pi/agent/skills`
...
- Geetorus can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Geetorus skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Geetorus may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Geetorus skills that are clearly marked as Geetorus-managed
...
- Geetorus stores desired skills only
...
- avoid destructive wording unless Geetorus can prove a skill is Geetorus-managed
