...
This document defines the V1 design for a Geetorus adapter that integrates with
...
4. synthesized stdout events for Geetorus UI/CLI
...
3. **Agent callback auth to Geetorus** uses a bootstrap exchange flow (no long-lived Geetorus key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Geetorus endpoints, not full SKILL.md prompt injection.
...
- `geetorusPublicUrl` (optional override; else `GEETORUS_PUBLIC_URL` env)
...
## Geetorus Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `GEETORUS_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.geetorusPublicUrl`
2. `process.env.GEETORUS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Geetorus credentials in prompt text.

Flow:

1. Before launch/follow-up, Geetorus mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Geetorus validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Geetorus API calls.
...
1. Prompt includes a compact instruction to fetch skills from Geetorus.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/geetorus`
   - `GET /api/skills/geetorus-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <geetorusPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Geetorus run context.
...
5. `geetorusPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Geetorus skill endpoints |
| Geetorus auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Geetorus context.
3. Add richer PR metadata surfacing in Geetorus UI.
