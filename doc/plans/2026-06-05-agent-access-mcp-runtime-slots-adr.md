...
- A Geetorus MCP server package that exposes Geetorus REST operations outward as MCP tools.
...
1. MVP clients are Geetorus agents only. External human AI clients are out of scope.
...
4. Geetorus is the enforcement gateway. Agents connect to Geetorus, not directly to upstream MCP credentials or ungoverned upstream configs.
5. The gateway governs Geetorus plugin tools, Geetorus self-MCP tools, and upstream MCP tools through one policy/audit/invocation model.
...
9. First examples are synthetic fixtures, then Geetorus self-read, GitHub triage, and approval-gated outbox/social examples.
...
A company-visible tool source. MVP types are `mcp_http`, `mcp_stdio`, `geetorus_plugin`, and `geetorus_self`. Future types such as `a2a` can be added behind the same concepts.
...
Server-evaluated rule that can allow, block, require approval, redact, rate-limit, or defer because runtime capacity is unavailable. Policies are evaluated from authoritative Geetorus state, not model-provided context.

`Gateway Session`

A short-lived per-run session minted by Geetorus. It exposes only allowed tools for that run and prevents upstream credentials from reaching the agent process.
...
At heartbeat invocation time, Geetorus resolves the authoritative agent, issue, project, routine, workspace, adapter, and run records. It then creates a gateway session tied to that run.

The adapter receives only Geetorus gateway configuration:

- gateway base URL;
- short-lived session token;
- optional generated MCP config pointing at Geetorus;
...
- Remote HTTP MCP is preferred. It uses no local process. Geetorus proxies requests with timeouts, retries where safe, auth, and audit.
...
- Geetorus self-read: list issues, get issue context, read plan document, list recent runs.
- Geetorus self-governed write: add comment draft, create child issue draft, update plan draft.
...
- mint/read run-bound gateway session where invoked by Geetorus runtime;
...
- `packages/mcp-server/README.md` for the current Geetorus MCP server boundary.
