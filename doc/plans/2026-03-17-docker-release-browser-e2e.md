...
Today release smoke testing for published Geetorus packages is manual and shell-driven:
...
- Docker
- `npx geetorusai@canary`
- `npx geetorusai@latest`
...
- runs `geetorusai@${GEETORUSAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@geetorus.local`
- password: `geetorus-smoke-password`
...
- the suite passes locally against both:
  - `GEETORUSAI_VERSION=canary`
  - `GEETORUSAI_VERSION=latest`
...
1. A published `geetorusai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `geetorusai@latest` Docker install can be smoke-tested by Playwright in CI.
