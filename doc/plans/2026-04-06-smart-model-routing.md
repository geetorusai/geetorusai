...
This document defines a V1 plan for "smart model routing" in Geetorus.
...
- preserve Geetorus's existing task, session, and audit invariants
...
That separation is a better fit for Geetorus than copying Hermes' exact keyword heuristic.

## 3. Current Geetorus State

Geetorus already has the right execution shape for adapter-specific routing, but it currently assumes one model per heartbeat run.

Current implementation facts:

- `server/src/services/heartbeat.ts` builds rich run context, including `geetorusWake`, workspace metadata, and session handoff context
...
Geetorus should implement smart model routing as an adapter-local, opt-in execution pattern.

V1 decision:

1. Do not add a global server-side router that tries to understand every adapter.
2. Do not copy Hermes' prompt-keyword classifier as Geetorus's default routing policy.
...
- Geetorus heartbeats are structured, issue-scoped, and already include wake metadata
...
- the adapter already injects Geetorus skills and workspace metadata cleanly
...
This matters because Geetorus's board UI is supposed to make cost and behavior legible.

## 9. Why Not Copy Hermes Exactly

Hermes' cheap-route heuristic is useful precedent, but Geetorus should not start there.

Reasons:

- Hermes is optimizing free-form conversational turns
- Geetorus agents run structured, issue-scoped heartbeats with explicit task and workspace context
- Geetorus already knows whether a run is fresh vs resumed, issue-scoped vs approval follow-up, and what workspace/session exists
- those execution facts are stronger routing signals than prompt keyword matching

If Geetorus later wants a cheap-only completion path for trivial runs, that can be a second-stage feature built on observed run data, not the first implementation.
...
Geetorus should ship smart model routing as:
