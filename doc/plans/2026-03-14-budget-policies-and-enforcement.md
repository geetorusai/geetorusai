...
Geetorus already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Geetorus should implement next.

## Product Goals

Geetorus should let operators:
...
For the next implementation pass, Geetorus should enforce these defaults:
...
Geetorus should separate subscription-included usage from billed spend:
...
Geetorus should have two threshold classes:
...
Geetorus must know that a project is budget-paused and enforce it.
...
Geetorus must also block execution before new work starts.
