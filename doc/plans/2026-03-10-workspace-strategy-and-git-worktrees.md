...
`PAP-447` asks how Geetorus should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Geetorus instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Geetorus:

- some operators use Geetorus to manage Geetorus and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Geetorus
- Claude and Codex expose different built-in affordances, so Geetorus should not overfit to one tool

## Core Product Decision

Geetorus should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Geetorus
...
- Geetorus should not assume that every adapter can see or use a host filesystem path directly
...
If Geetorus makes worktrees agent-first, it will blur:
...
- Geetorus core resolves and records execution workspace state
...
That gives Geetorus room to support:
...
- Geetorus handles workspace runtime services under the hood using project/workspace policy
...
Once Geetorus is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Geetorus should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Geetorus cannot infer
...
- when issue moves to done, should Geetorus auto-commit?
- should it auto-push?
...
Geetorus already has the right foundation for a project-first model.
...
For server-spawned local adapters, Geetorus already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Geetorus should mirror directly

Implication:

- for `codex_local`, Geetorus should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Geetorus should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Geetorus should separate:
...
Geetorus should be able to express intentions such as:
...
These are not specific to Geetorus. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".geetorus/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Geetorus-specific `devServer` flag.
...
Geetorus should distinguish between:
...
Geetorus should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Geetorus should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Geetorus
...
For runtime services specifically, Geetorus should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Geetorus-only dev-server flags
...
3. Inject resolved execution workspace details into `context.geetorusWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Geetorus instance can be reused through env/config injection when appropriate
...
- Geetorus can decide whether to reuse or start a fresh service deterministically
...
3. Test against Geetorus-on-Geetorus workflows first.
...
7. Workspace runtime services are modeled generically, not as Geetorus-specific dev-server toggles.
