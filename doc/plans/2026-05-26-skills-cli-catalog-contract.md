...
Date: 2026-05-26
Source plan: approved Geetorus skills CLI and catalog plan

This document freezes the first implementation contract for the `geetorusai skills`
command group and the app-shipped skills catalog. It is intentionally a build
contract, not a full product spec.

## Decisions

- `geetorusai skills` manages Geetorus company skills. It does not manage
...
- Root `skills/` remains reserved for Geetorus runtime and operational skills.
...
- Agent desired skills continue to live in
  `adapterConfig.geetorusSkillSync.desiredSkills` for the first release. Do not
...
- Catalog skill: an app-shipped skill entry in `@geetorusai/skills-catalog`.
...
- Company-scoped commands also accept `-C, --company-id <id>` and otherwise use
  `GEETORUS_COMPANY_ID` or the active context profile.
...
| `skills agent sync <agent-ref> --skill <skill-ref>...` | Replaces the agent's non-required desired skill set with the supplied refs and triggers adapter sync. Required Geetorus skills remain enforced by the server. | `AgentSkillSnapshot` |
...
Catalog commands are for the app-shipped Geetorus catalog only. External GitHub,
...
Package name: `@geetorusai/skills-catalog`.
...
- `Company ID is required. Pass --company-id, set GEETORUS_COMPANY_ID, or set a context profile.`
...
- `geetorusai skills --help` exposes the Phase B command group.
