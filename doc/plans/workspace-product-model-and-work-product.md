...
Geetorus needs to support two very different but equally valid ways of working:
...
Today, Geetorus already has the beginnings of this model:
...
- making Geetorus feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Geetorus users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Geetorus deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Geetorus into a full code review product
...
This object must be recorded explicitly so that Geetorus can:
...
Geetorus should treat PRs as a type of work product linked back to:
...
Geetorus already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Geetorus must be a first-class deployment mode

Geetorus cannot assume that it is running on the same machine as the code.

In cloud deployments, Geetorus may:
...
- `Runtime service`: a process or service Geetorus owns or tracks for a workspace
...
It also lets cloud-hosted Geetorus deployments point at codebases and remotes without pretending the Geetorus host has direct filesystem access.
...
Without an explicit `execution workspace` record, Geetorus has nowhere stable to attach:
...
Geetorus needs a single place to show:
...
- Geetorus-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Geetorus owning the execution host
...
## 1. Geetorus host and execution host must be decoupled
...
without the Geetorus host itself running local git or local preview processes.
...
Geetorus should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Geetorus.
...
Geetorus should keep workspace automation centered on a small set of hooks:
...
Geetorus should provide the equivalent visibility in the project `Code` surface:
...
Geetorus should treat execution workspaces as the natural home for:
...
Geetorus should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Geetorus should continue treating cleanup policy as part of the core workspace model:
...
Geetorus should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Geetorus host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Geetorus should borrow Worktrunk's good execution patterns while keeping the broader Geetorus model:
...
That keeps Geetorus focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Geetorus deployments remain first-class

That is a better fit for Geetorus than either extreme:
