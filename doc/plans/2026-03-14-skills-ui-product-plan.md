...
This document defines the product and UI plan for skill management in Geetorus.
...
- Geetorus company import/export can include skills as package content
...
3. The UI must always tell the truth about what Geetorus knows:
   - desired state in Geetorus
...
Geetorus should treat skills at two scopes:
...
- created locally inside Geetorus later
...
7. “Understand what Geetorus knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Geetorus-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Geetorus did not install.

These should be clearly marked:

- external
- not managed by Geetorus
...
- `desired_only`: Geetorus wants it, adapter does not show it yet
- `external`: adapter has it but Geetorus does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Geetorus only tracks desired state
...
- Geetorus can still track desired skills
...
- Geetorus can see adapter skills
...
- `.geetorus.yaml` may add Geetorus-specific fidelity, but should not replace the base shortname association model
...
- `.geetorus.yaml` may add Geetorus-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Geetorus should have a company-scoped skill model or managed package model representing:
...
Geetorus should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Geetorus-specific.
...
That gives Geetorus one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
