...
1. Trust the execution root that Geetorus selects at startup. Resolve it on
...
2. Codex retains model conversation context. Geetorus reads provider state to
...
so startup cannot load the Geetorus server checkout by accident. Persist the
