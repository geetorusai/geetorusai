...
- New config/env settings:
  ...
  - `GEETORUS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `GEETORUS_AGENT_JWT_ISSUER` (default: `geetorus`)
  - `GEETORUS_AGENT_JWT_AUDIENCE` (default: `geetorus-api`)
...
1. In:
   ...
   inject `GEETORUS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `GEETORUS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `GEETORUS_AGENT_ID`
  - `GEETORUS_COMPANY_ID`
  - `GEETORUS_API_URL`
...
- Local adapters authenticate without manual `GEETORUS_API_KEY` config.
