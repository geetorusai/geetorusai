...
Add a single storage subsystem for Geetorus that supports:
...
- API serves attachment bytes through authenticated Geetorus endpoints.
...
- Default local path is under instance root: `~/.geetorus/instances/<instanceId>/data/storage`.
...
- `geetorusai onboard` writes a valid `storage` config block by default.
- `geetorusai configure --section storage` can switch between local and s3 modes.
...
- `geetorusai doctor` reports actionable storage status.
