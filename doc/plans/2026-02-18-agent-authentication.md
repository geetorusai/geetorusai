...
Agents need API keys to authenticate with Geetorus. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Geetorus
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Geetorus generates a token and passes it directly to the agent
...
`GEETORUS_API_URL`, `GEETORUS_AGENT_ID`, `GEETORUS_COMPANY_ID`. We need to
add a `GEETORUS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Geetorus CLI
...
```
geetorus auth login
# Opens browser -> user confirms -> token stored at ~/.geetorus/credentials
```
...
managed by the Geetorus server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Geetorus.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Geetorus SKILL.md (or a link to it)
   - What information Geetorus needs from the agent (e.g. webhook URL, adapter
...
5. Geetorus stores the pending registration.
...
7. On approval, Geetorus provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Geetorus issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.geetorus.ing/skills/geetorus/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Geetorus should send you heartbeats",
...
1. Generate a Geetorus invite link for the company.
...
4. A Geetorus company member approves the new agent.
5. Geetorus begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `GEETORUS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `geetorusai auth login` for developer-managed remote agents.                                      |
