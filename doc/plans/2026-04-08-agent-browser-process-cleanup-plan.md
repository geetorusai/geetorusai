...
Yes, there is a likely root cause in Geetorus's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Geetorus can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Geetorus already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Geetorus may think it cancelled the run because the parent process is gone
...
This work is successful when Geetorus can:
...
- require every skill author to add bespoke browser teardown logic before Geetorus can clean up correctly
...
- make the leak measurable from Geetorus's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Geetorus
...
Treat this as a Geetorus executor ownership bug, not an `agent-browser` bug.
