...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/geetorus-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `GEETORUS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `GEETORUS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Geetorus may create host-local worktrees, processes, and paths
- `cloud`
  - Geetorus should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `geetorus | github | vercel | s3 | custom`
...
- use `GEETORUS_EXECUTION_TOPOLOGY` as a defaulting guardrail
