...
Geetorus's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Geetorus's API, UI, and data model | Yes |
...
modules/
  observability/
    geetorus.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@geetorusai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@geetorusai/core";
...
```
1. Scan modules/ directory for geetorus.module.json manifests
...
```jsonc
// geetorus.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@geetorusai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@geetorusai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:geetorus/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.geetorus.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm geetorusai store list                    # browse available modules and templates
pnpm geetorusai store install <module-id>     # install a module
pnpm geetorusai store import <template-id>    # import a company template
pnpm geetorusai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Geetorus instance | `server:started` |
...
4. **Module config** — `geetorus.config.json` with per-module config, env var interpolation
...
10. **`@geetorusai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
