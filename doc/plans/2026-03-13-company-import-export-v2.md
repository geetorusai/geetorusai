...
- `docs/specs/geetorus-plan.md` assumptions about blueprint bundle shape where they conflict with the markdown-first package model

## 1. Purpose

This document defines the next-stage plan for Geetorus company import/export.

The core shift is:

- move from a Geetorus-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Geetorus.
...
Geetorus already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Geetorus
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `geetorus.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.geetorus.yaml` sidecar for high-fidelity Geetorus-specific details
...
11. adapter-aware skill sync surfaces so Geetorus can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Geetorus at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Geetorus sidecar.
...
Geetorus must not redefine `SKILL.md`.
...
- Geetorus-specific fidelity lives in `.geetorus.yaml`
- Geetorus may resolve and install `SKILL.md` packages, but it must not require a Geetorus-only skill format
...
### 5.4 Base Package Vs Geetorus Extension
...
- base package:
  ...
  - no Geetorus-only runtime fields by default
- Geetorus extension:
  - `.geetorus.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Geetorus tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`geetorus.manifest.json` is not part of the future package direction.
...
Geetorus import/export should support these entity kinds:
...
- Geetorus can treat “this set of agents came from team package X” as the imported-team model
...
Geetorus should support source references in package metadata with:
...
Geetorus should define a new adapter surface area around skills:
...
Planned Geetorus behavior:

- if an adapter supports read, Geetorus should show current skills in the UI
- if an adapter supports write, Geetorus should let the user enable/disable imported skills
- if an adapter supports sync, Geetorus should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.geetorus.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.geetorus.yaml` alongside the base package
...
## 10. Storage Model Inside Geetorus
...
Geetorus should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `geetorusai company export <company-id> --out <path>`
- `geetorusai company import <path-or-url> --dry-run`
- `geetorusai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Geetorus storage, or only referenced at import time?
...
3. Should Geetorus support direct local folder selection in the web UI, or keep that CLI-only initially?
...
3. build phase 2 markdown-first package reader before expanding GeeTorus or `companies.sh`
4. treat the old manifest-based format as deprecated and not part of the future surface

This keeps Geetorus aligned with:
