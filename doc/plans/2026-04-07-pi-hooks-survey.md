...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Geetorus.
...
- current Geetorus plugin and adapter surfaces in this repo
...
## What Geetorus already has

Geetorus already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Geetorus

These ideas from `pi-mono` fit Geetorus with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Geetorus should continue exposing run and transcript events to plugins, for example:
...
This matches Geetorus's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Geetorus already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Geetorus already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Geetorus, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Geetorus core:

### 1. Arbitrary mutation hooks on control-plane decisions

Geetorus should not let general plugins rewrite:
...
Geetorus should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Geetorus is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Geetorus's board-and-issues model.

## Recommended Geetorus direction
...
If the question is "what works for Geetorus too?":
