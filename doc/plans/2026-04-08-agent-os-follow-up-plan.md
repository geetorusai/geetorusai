...
Turn the `agent-os` research into a low-risk Geetorus execution plan that preserves Geetorus's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Geetorus should not absorb `agent-os` as a product model or orchestration layer.

Geetorus should evaluate `agent-os` in three narrow areas:
...
This work is successful when Geetorus has:
...
- a proposed runtime capability model that fits current Geetorus adapters
...
- replace Geetorus heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Geetorus core

## Existing Geetorus integration points
...
- prove that Geetorus can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Geetorus's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Geetorus-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Geetorus workloads
...
- identify where Geetorus should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Geetorus
...
- runtime experimentation could blur the boundary between Geetorus as control plane and Geetorus as execution platform
