...
The most important requirement is credential ergonomics. External integrations need a reliable "way in" to Geetorus:
...
- `agent local-cli` creates an agent API key through board access, installs local skills, and prints `GEETORUS_API_URL`, `GEETORUS_COMPANY_ID`, `GEETORUS_AGENT_ID`, and `GEETORUS_API_KEY`.
...
The CLI must always know which Geetorus API it is operating against. This is especially important for fork/local development, where Geetorus may run on `3101+` rather than the upstream default `3100`.
...
- `geetorusai connect` must show the resolved API base before any auth or mutation and allow the user to override it.
...
```json
{
  ...
  "profiles": {
    "default": {
      ...
      "persona": "agent",
      "agentId": "agent-id",
      "apiKeyEnvVarName": "GEETORUS_API_KEY"
    }
  }
}
```
...
```sh
geetorusai token board create --company-id <company-id> --name "external-admin"
geetorusai token board list
geetorusai token board revoke <key-id>
```
...
```sh
geetorusai token agent create --company-id <company-id> --agent <agent-id-or-name> --name "external-worker"
geetorusai token agent list --company-id <company-id> --agent <agent-id-or-name>
geetorusai token agent revoke --agent <agent-id-or-name> <key-id>
```
...
```sh
geetorusai agent-prompt <agent-name-or-id> <agent-api-key> "Prompt here"
```

Recommended safer variants:

```sh
geetorusai agent prompt --agent <agent-name-or-id> --api-key-env GEETORUS_API_KEY "Prompt here"
geetorusai agent prompt --profile my-agent "Prompt here"
geetorusai board prompt --agent <agent-name-or-id> "Prompt here"
```
...
- Default prompt target should be `issue create + assign + wake`, because Geetorus's communication model is tasks/comments, not chat.
...
```sh
geetorusai agent wake <agent>
geetorusai run list --company-id <company-id>
geetorusai run get <run-id>
geetorusai run log <run-id>
geetorusai run cancel <run-id>
geetorusai issue runs <issue-id>
```
...
```sh
geetorusai project list|get|create|update|delete
geetorusai goal list|get|create|update|delete
```
...
```sh
geetorusai issue child create <issue-id>
geetorusai issue document list|get|put|delete|lock|unlock|revisions|restore
geetorusai issue work-product list|create|update|delete
geetorusai issue interaction list|create|accept|reject|respond|cancel
geetorusai issue attachment list|upload|download|delete
geetorusai issue force-release <issue-id>
geetorusai issue label list|create|delete
geetorusai issue read|unread|archive|unarchive
```
...
```sh
geetorusai agent create|update|pause|resume|approve|terminate|delete
geetorusai agent org
geetorusai agent config get|revisions|rollback
geetorusai agent instructions get|set|file
geetorusai adapter list|models|profiles|detect|test|install|enable|disable|reload
```
...
```sh
geetorusai cost summary|by-agent|by-project|by-provider|issue
geetorusai cost event create
geetorusai finance event create|list|summary
geetorusai budget overview|set-company|set-agent|policy-create|incident-resolve
```
...
```sh
geetorusai invite create|list|revoke|show|onboarding
geetorusai join list|approve|reject|claim-key
geetorusai member list|update|archive|permissions
geetorusai admin user list|promote|demote|company-access
```
...
```sh
geetorusai routine list|create|get|update|run|runs|trigger|revision
geetorusai environment list|create|get|update|delete|probe|leases
geetorusai workspace list|get|update|operations|runtime
geetorusai project workspace list|create|update|delete|runtime
```
...
```text
geetorusai connect
geetorusai token board|agent create|list|revoke
geetorusai whoami
geetorusai prompt ...
geetorusai board ...
geetorusai agent ...
...
geetorusai budget ...
geetorusai routine ...
geetorusai environment ...
geetorusai workspace ...
geetorusai invite ...
geetorusai member ...
geetorusai plugin ...
geetorusai instance ...
...
- Add aliases only for high-frequency flows, for example `geetorusai ask` as an alias for `geetorusai prompt`.
...
- Destructive tests must require an explicit test opt-in such as an env var or a dedicated test command; normal unit tests must not mutate a real running Geetorus instance.
...
- A new user can run `geetorusai connect`, confirm or override the API base, select board or agent, and get a saved working profile tied to that API base.
...
```sh
geetorusai agent-prompt AgentName "$AGENT_API_KEY" "Prompt here"
```

- The one-liner creates or updates Geetorus work, does not require a browser, and fails with a clear company/agent mismatch error when the token does not belong to the requested agent.
