# LLM Wiki Geetorus Asset And Work-Product Security Gate

Status: accepted Phase 5 policy
Date: 2026-05-06
Owner: Security engineering
Scope: Geetorus-derived ingestion into the LLM Wiki before any asset or work-product content indexing ships

## Decision

Phase 5 remains **fail-closed** for Geetorus assets and work products.

- Geetorus-derived **text extraction is allowed only** for issue titles/descriptions, issue comments, and issue documents.
- Geetorus **assets/attachments** and **issue work products** are **metadata-only** in Phase 5.
...
These source kinds may contribute body text to Geetorus-derived source bundles:

| Source kind | Allowed body fields | Reason |
| --- | --- | --- |
| Issue | `title`, `description`, identifier/status metadata | First-party Geetorus text under company ACL |
| Comment | `body` | First-party Geetorus text under company ACL |
| Document | `body`, `title`, `key`, revision metadata | First-party Geetorus text under company ACL |
...
- metadata-only references built from allowlisted structured fields already stored in Geetorus
...
- Every asset MIME type is treated as opaque for Geetorus-derived indexing.
...
- non-default-space routing for Geetorus-derived asset/work-product references

## Security Rationale

This gate exists because the current host surfaces have different trust properties:

- issue/comment/document text is first-party Geetorus content already exposed through company-scoped issue/document APIs
