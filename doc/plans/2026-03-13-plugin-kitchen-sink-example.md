...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Geetorus plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Geetorus.
...
- package: `@geetorusai/plugin-kitchen-sink-example`
- plugin id: `geetorus.kitchen-sink-example` or `geetorus-kitchen-sink-example`

Recommendation: use `geetorus-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Geetorus event such as `issue.created`
...
Recommendation: default to a Geetorus-local endpoint or a stable public echo endpoint to avoid flaky docs.
