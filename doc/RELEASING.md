...
Maintainer runbook for shipping Geetorus across npm, GitHub, and the website-facing changelog surface.
...
Geetorus uses calendar versions that still fit semver syntax:
...
2. **npm** — `geetorusai` and public workspace packages are published
...
- publishes the user-facing `geetorusai` package last, so `geetorusai@canary` does not advance before the full package set exists
- verifies that `canary` resolves to the just-published version and that published internal dependencies exist on npm
- installs `geetorusai@canary` into a clean temporary prefix as the final npm gate
...
```bash
npx geetorusai@canary onboard
# or
npx geetorusai@canary onboard --data-dir "$(mktemp -d /tmp/geetorus-canary.XXXXXX)"
```
...
```bash
npx geetorusai@nightly onboard
```
...
[Run the action here](https://github.com/geetorusai/geetorus/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Geetorus. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
GEETORUSAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
GEETORUSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary GEETORUSAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable GEETORUSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f geetorus_version=canary
gh workflow run release-smoke.yml -f geetorus_version=nightly
gh workflow run release-smoke.yml -f geetorus_version=beta
gh workflow run release-smoke.yml -f geetorus_version=latest
```
...
- `npx geetorusai@canary onboard` installs
