...
This is the end-to-end demo for the MCP Access Governance launch. It walks the three required cases — **read**, **approval-gated write**, **denied/destructive** — against the real [`@geetorusai/kv-demo-mcp-server`](../packages/kv-demo-mcp-server/README.md) package. The server is a standalone Node process that exposes four key/value MCP tools and a tiny web UI over the same in-memory store, so you can call a tool from an agent and watch the value appear in a browser tab in real time.
...
- Geetorus running in `local_trusted` or `authenticated/private` mode. Public mode is fine as long as the Geetorus process can reach `http://127.0.0.1:8848` (we connect over `remote_http`, so no trusted runtime worker is required).
...
- The KV demo server package built (`pnpm --filter @geetorusai/kv-demo-mcp-server build`).
- Board API key (`$BOARD_API_KEY`) exported. Company ID (`$COMPANY_ID`) exported. Agent ID (`$AGENT_ID`) for the caller exported.
- Geetorus URL (`$GEETORUS_URL`) exported.
...
All API requests use `Authorization: Bearer $BOARD_API_KEY` for board calls. Gateway calls use a dedicated session token via the `X-Geetorus-Tool-Gateway-Token` header — they do not use `Authorization`. See Step 5 for how the token is minted.

## Step 0 — Frame the demo

Spoken intro:

> "Geetorus ships an MCP gateway that sits between every agent and every upstream tool. Three things happen on every call: we pick the tool against a profile, we evaluate policies, and we record an audit event. I'm going to connect a tiny key/value MCP server I'm running on this laptop, then run a read, a write that needs approval, and a destructive call that gets denied. The KV server has a web UI on the same port that shows its values — so when the agent's write lands, you'll see it appear in the browser. The data lives in the server; the policy decisions and the audit log live in Geetorus."
...
```sh
pnpm --filter @geetorusai/kv-demo-mcp-server start
```
...
CONNECT=$(curl -fsS -X POST \
  ...
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/apps/connect" \
  -d '{
...
curl -fsS -X POST \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/apps/$CONNECTION_ID/finish" \
  -d '{
...
export RUN_ID=$(curl -fsS \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/heartbeat-runs?agentId=$AGENT_ID&limit=20" \
...
SESSION=$(curl -fsS -X POST \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/tool-gateway/sessions" \
  -d '{
...
Gateway calls use the session token via `X-Geetorus-Tool-Gateway-Token`. The body uses `tool` (string) and `parameters` (object).

```sh
curl -fsS -X POST \
  -H "X-Geetorus-Tool-Gateway-Token: $GATEWAY_TOKEN" \
  -H "Content-Type: application/json" \
...
> "The agent doesn't know whether the tool was denied by the profile, by a policy, or by quarantine. It just knows the call failed and the reason code. The operator sees the full decision in the audit row. The KV server was never touched — the gateway short-circuited before the request left Geetorus."
...
curl -fsS \
  -H "Authorization: Bearer $BOARD_API_KEY" \
  "$GEETORUS_URL/api/tool-gateway/audit?companyId=$COMPANY_ID&limit=20" \
...
The next `kv_list` call returns an empty `entries` array. The Values UI shows the empty table again. Geetorus's audit history is untouched — it still records that the calls happened, just against a server that has since reset.
...
### Geetorus-side cleanup (for a fully clean state)
...
curl -fsS -X POST -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/companies/$COMPANY_ID/tools/trust-rules/$TRUST_RULE_POLICY_ID/revoke" \
...
curl -fsS -X PATCH -H "Authorization: Bearer $BOARD_API_KEY" -H "Content-Type: application/json" \
  "$GEETORUS_URL/api/tool-applications/$APPLICATION_ID" \
...
The demo is also the clearest way to show the data boundary between Geetorus and the upstream MCP server.

| Concern | Stored in the KV demo server | Stored in Geetorus |
| --- | --- | --- |
