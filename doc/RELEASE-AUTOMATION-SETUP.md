...
This document covers the GitHub and npm setup required for the current Geetorus release model:
...
Do this for every public package that Geetorus publishes.
...
3. add a trusted publisher for the GitHub repository `geetorusai/geetorus`
...
2. open that package on npm and add the `geetorusai/geetorus` trusted publisher for `.github/workflows/release.yml`
...
2. revoke any personal automation token that used to publish Geetorus
...
```bash
npm install --prefix "$(mktemp -d)" geetorusai@canary --no-audit --no-fund
```

The release script runs this clean-prefix install after publishing every workspace
package dependency-first and publishing `geetorusai` last. A package that is not
...
`release-verify.yml` runs `Verify Geetorus Runner` on two independent runners.
...
When AWS routing is enabled, Cloud image builds use `geetorus-cloud-build-x64`
and source verification uses `geetorus-post-merge-x64`. The artifact wait and
