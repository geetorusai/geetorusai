...
describe("forceLoopbackBindInCommand", () => {
  ...
  it("keeps the legacy lan aliases so dev-service detection still matches", () => {
    // `isGeetorusDevRuntimeService` matches `--tailscale-auth` as a substring;
...
describe("isGeetorusDevRunnerCommand", () => {
  it("matches the real managed dev commands", () => {
    expect(isGeetorusDevRunnerCommand("pnpm dev --bind lan")).toBe(true);
    expect(isGeetorusDevRunnerCommand("pnpm dev")).toBe(true);
    expect(isGeetorusDevRunnerCommand("pnpm dev:once --tailscale-auth")).toBe(true);
    expect(isGeetorusDevRunnerCommand("pnpm dev:watch")).toBe(true);
    expect(isGeetorusDevRunnerCommand("npm run dev --bind lan")).toBe(true);
    expect(isGeetorusDevRunnerCommand("yarn dev")).toBe(true);
    expect(isGeetorusDevRunnerCommand("node /tmp/x/dev-runner.mjs --bind lan")).toBe(true);
    expect(isGeetorusDevRunnerCommand("tsx ../scripts/dev-runner.ts watch")).toBe(true);
  });

  it("does not match unrelated commands that merely use --bind", () => {
    expect(isGeetorusDevRunnerCommand('python3 -m http.server "$PORT" --bind 127.0.0.1')).toBe(false);
    expect(isGeetorusDevRunnerCommand("node -e 'listen()'")).toBe(false);
    expect(isGeetorusDevRunnerCommand("pnpm build")).toBe(false);
    expect(isGeetorusDevRunnerCommand("pnpm develop")).toBe(false);
    expect(isGeetorusDevRunnerCommand("./my-dev-runnerish --bind lan")).toBe(false);
  });
});

describe("rewriteUrlHostToLoopback", () => {
  it("redirects a MagicDNS probe target to loopback, keeping port and path", () => {
    expect(rewriteUrlHostToLoopback("http://geetorus-dev:42003/api/health")).toBe(
      "http://127.0.0.1:42003/api/health",
    );
    expect(rewriteUrlHostToLoopback("http://geetorus-dev:42003")).toBe("http://127.0.0.1:42003/");
  });
