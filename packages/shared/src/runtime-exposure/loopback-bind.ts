 ...
 * so an exposed Geetorus dev runtime MUST bind `127.0.0.1`. The server used to
 * request that with env vars alone (`GEETORUS_BIND` / `GEETORUS_BIND_HOST`),
 ...
 * exposure overwrites `GEETORUS_BIND` from its own `--bind` argv and deletes
 * `GEETORUS_BIND_HOST` outright. A branch pinned at such a commit therefore
...
/**
 * A Geetorus dev-runner invocation — the only shape that understands
 * `--bind` / `--bind-host`.
 *
 ...
 */
const GEETORUS_DEV_RUNNER_COMMAND =
  /(?:^|[\s;&|])(?:(?:pnpm|npm|yarn|bun)(?:\s+run)?\s+dev(?::once|:watch|:server)?(?=\s|$)|[^\s]*dev-runner(?:\.[cm]?[jt]s)?(?=\s|$))/;

export function isGeetorusDevRunnerCommand(command: string): boolean {
  return GEETORUS_DEV_RUNNER_COMMAND.test(command);
}

/**
 * Rewrite a Geetorus dev-runner command so it explicitly requests the loopback
 * bind, replacing whatever bind selection it carried.
 *
 * A command that is not a dev-runner invocation is returned untouched — see
 * {@link isGeetorusDevRunnerCommand} for why that guard is not optional.
 *
 ...
 * deployment mode an exposed lane wants, and `isGeetorusDevRuntimeService`
...
export function forceLoopbackBindInCommand(command: string): string {
  if (!isGeetorusDevRunnerCommand(command)) return command;
 ...
 * MagicDNS name like `http://geetorus-dev:42003` — which only ever answered
