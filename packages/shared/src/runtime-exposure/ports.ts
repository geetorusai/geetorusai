 ...
 * and tests all agree on the dedicated Geetorus runtime port range and the
...
/**
 * Dedicated, allowlisted port range for managed Geetorus runtime APP
...
/**
 * Fixed offset between an app port and its Geetorus Vite HMR companion port.
 * Matches Geetorus dev mode's `server port + offset` HMR convention and keeps
...
export function deriveViteHmrPort(appPort: number): number {
  ...
  return deriveGeetorusViteHmrPort(appPort);
}

/**
 * Derive Geetorus's Vite HMR companion port for any valid application port.
 *
 * Normal Geetorus instances can use ports outside the dedicated exposure
 ...
 */
export function deriveGeetorusViteHmrPort(serverPort: number): number {
