...
describe("runtime exposure port policy", () => {
  ...
  it("shares the generic Geetorus HMR derivation with high-port overflow fallback", () => {
    expect(deriveGeetorusViteHmrPort(3_100)).toBe(13_100);
    expect(deriveGeetorusViteHmrPort(55_535)).toBe(65_535);
    expect(deriveGeetorusViteHmrPort(55_536)).toBe(45_536);
    expect(() => deriveGeetorusViteHmrPort(0)).toThrow(/valid TCP port/);
  });

  it("builds https URLs on the non-standard port", () => {
    expect(buildRuntimeExposureUrl("geetorus-dev.tail29c1aa.ts.net", 42010)).toBe(
      "https://geetorus-dev.tail29c1aa.ts.net:42010",
    );
    expect(buildRuntimeExposureHealthUrl("geetorus-dev.tail29c1aa.ts.net", 42010)).toBe(
      "https://geetorus-dev.tail29c1aa.ts.net:42010/api/health",
    );
  });
