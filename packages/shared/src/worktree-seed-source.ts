...
import { resolveGeetorusInstanceId } from "./home-paths.js";
...
function readInstanceId(configPath: string, label: "source" | "target"): string {
  ...
  if (!existsSync(envPath)) {
    ...
    if (path.basename(path.dirname(configDir)) === "instances") {
      return resolveGeetorusInstanceId(path.basename(configDir));
    }
    throw new Error(`Registered ${label} Geetorus config is missing its adjacent .env instance pointer.`);
  }
  const contents = readFileSync(envPath, "utf8");
  for (const rawLine of contents.split(/\r?\n/)) {
    const match = rawLine.match(
      /^\s*(?:export\s+)?GEETORUS_INSTANCE_ID\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s#]+))/,
    );
    const value = (match?.[1] ?? match?.[2] ?? match?.[3] ?? "").trim();
    if (value) return value;
  }
  throw new Error(`Registered ${label} Geetorus config has no GEETORUS_INSTANCE_ID binding.`);
}
...
function inspectDeclaredEntry(entryPath: string, configPath: string, detail?: string): Stats | null {
  try {
    ...
    throw new Error(
      `Registered base project workspace Geetorus config at ${configPath} cannot be inspected (${errorCode(error)}${detail ?? ""}).`,
    );
  }
}
 ...
 * at the config itself or at the `.geetorus` directory holding it, still counts as a
...
export function baseWorkspaceDeclaresInstanceConfig(baseWorkspaceCwd: string): boolean {
  const configDir = path.join(baseWorkspaceCwd, ".geetorus");
  const configPath = path.join(configDir, "config.json");
  if (inspectDeclaredEntry(configPath, configPath)) return true;

  // The probe above resolves `.geetorus` before it reaches the config, so a broken link
  // there also reports ENOENT. Only an absent or traversable `.geetorus` lets the caller
  // name another source; a link that hides whatever it points at is malformed, not empty.
  const configDirEntry = inspectDeclaredEntry(configDir, configPath, " on its .geetorus entry");
  if (configDirEntry?.isSymbolicLink()) {
    try {
      statSync(configDir);
    } catch (error) {
      throw new Error(
        `Registered base project workspace Geetorus config at ${configPath} cannot be inspected (${errorCode(error)} on its .geetorus symlink target).`,
      );
    }
  }
  return false;
...
export function resolveRegisteredWorktreeSeedSource(
  ...
  if (registeredCwd) {
    ...
    registeredConfigPath = baseWorkspaceDeclaresInstanceConfig(canonicalBaseCwd)
      ? path.join(canonicalBaseCwd, ".geetorus", "config.json")
      : null;
  }
  ...
  if (!selectedPath) {
    throw new Error(
      "Registered base project workspace has no Geetorus config of its own and no explicit source was provided.",
    );
  }
  const canonicalSourceConfigPath = canonicalRegularFile(selectedPath, "Registered source Geetorus config");
  if (registeredConfigPath && canonicalSourceConfigPath !== registeredConfigPath) {
    throw new Error("Registered source Geetorus config escapes the base project workspace or uses a symlink alias.");
  }

  if (explicitSource) {
    const canonicalExplicitSource = canonicalRegularFile(explicitSource, "Explicit source Geetorus config");
    if (canonicalExplicitSource !== canonicalSourceConfigPath) {
      throw new Error("Explicit source Geetorus config does not match the registered base project workspace.");
    }
  }

  const canonicalTargetConfigPath = canonicalRegularFile(
    input.targetConfigPath,
    "Target worktree Geetorus config",
  );
  if (canonicalSourceConfigPath === canonicalTargetConfigPath) {
    throw new Error("Source and target Geetorus configs are the same canonical file.");
  }
  ...
  if (targetInstanceId !== input.expectedTargetInstanceId) {
    throw new Error("Target Geetorus instance does not match the registered worktree instance.");
  }
  if (sourceInstanceId === targetInstanceId) {
    throw new Error("Source and target Geetorus configs name the same instance.");
  }
 ...
 * carries its own `.geetorus/config.json` stays authoritative, so an explicit path
