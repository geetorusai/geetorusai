 ...
 * uses, and its value is stored as a Geetorus secret like every other one.
 */
...
/**
 * Headers Geetorus refuses to project from a user-supplied config.
 *
 ...
 * ambient browser-style credentials that Geetorus cannot scope or rotate.
...
/**
 * Is `name` a header Geetorus is willing to send on a user-configured remote
...
export function mcpRemoteHeaderRejectionMessage(
  ...
  switch (reason) {
    ...
    case "forbidden":
      return `Geetorus manages the "${headerName}" header and cannot send a custom value for it.`;
    case "value_too_long":
