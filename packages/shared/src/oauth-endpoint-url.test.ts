...
describe("checkOAuthEndpointUrl", () => {
  ...
  it("allows plaintext http for Geetorus's own origin only", () => {
    const options = { allowInsecureOrigins: ["http://geetorus.test"] };
    expect(checkOAuthEndpointUrl("http://geetorus.test/api/smoke-lab/oauth/authorize", options).ok).toBe(true);
    // Port and scheme are part of the origin, so a neighbour is not exempt.
    expect(checkOAuthEndpointUrl("http://geetorus.test:8080/authorize", options)).toEqual({
      ok: false,
      reason: "insecure_transport",
    });
    ...
    expect(checkOAuthEndpointUrl("http://geetorus.test/authorize", { allowInsecureOrigins: ["nonsense"] })).toEqual({
      ok: false,
      reason: "insecure_transport",
    });
  });
