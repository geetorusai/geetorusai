...
describe("buildAlreadyImportedMessage", () => {
  it("names the landed company with its prefix", () => {
    expect(
      buildAlreadyImportedMessage({ id: "company-2", name: "Geetorus", issuePrefix: "PAPA" }),
    ).toBe(
      'This exact package was already imported by a completed transfer. The earlier import landed in the company "Geetorus" (PAPA) — open it from the company switcher. Re-export the package to import it again.',
    );
  });
