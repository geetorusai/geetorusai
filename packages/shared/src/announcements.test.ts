...
describe("announcement contract", () => {
  ...
  it("rejects unknown fields throughout the remotely authored manifest", () => {
    ...
    const external = { kind: "external", label: "Learn more", url: "https://geetorus.ing" };
    for (const value of [
  ...
  it("checks expiration and minimum versions numerically, including prereleases", () => {
    const item = announcementSchema.parse({ ...announcement, expiresAt: "2027-01-01T00:00:00Z", minimumGeetorusVersion: "2026.913.0" });
