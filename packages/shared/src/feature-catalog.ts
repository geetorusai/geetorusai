...
export const INSTANCE_FEATURE_CATALOG: Record<InstanceFeatureKey, FeatureCatalogEntry> = {
  ...
  enableNativeRunner: {
    ...
    description:
      "Allow explicitly configured local Codex, OpenCode, and qualified ACPX agents to use the experimental Rust Geetorus Runner, including authenticated sandbox ingress when required. Onboarding remains on legacy adapters.",
    tier: "managed",
    cloudDefault: false,
    // On by default for self-hosted instances. Requires a Rust toolchain (or
    // GEETORUS_RUNNER_BINARY) for `pnpm dev`, which builds runnerd whenever
    // this is on.
  ...
  enableBuiltInAgents: {
    ...
    description:
      "Show Geetorus-managed built-in agent surfaces, including roster badges, the Built-in agents tab, and setup controls.",
    tier: "managed",
    cloudDefault: false,
    selfHostedDefault: false,
  },
  enableBetaSkills: {
    title: "Beta skills",
    description: "Allow agents to pin beta releases of the Geetorus core skill.",
    tier: "preference",
    cloudDefault: false,
    selfHostedDefault: false,
  },
  ...
  enableGeetorusDeveloperMode: {
    title: "Geetorus Developer Mode",
    description:
      "Show internal Geetorus maintainer tools and observability links, including Honeycomb trace queries on run pages.",
    tier: "preference",
    cloudDefault: false,
    selfHostedDefault: false,
  },
  ...
  enableRunnerPreviewIngress: {
    ...
    description:
      "Compatibility-only key retained for older managed configs. Runner ingress follows the Geetorus Runner setting.",
    tier: "managed",
    cloudDefault: false,
    selfHostedDefault: false,
  },
