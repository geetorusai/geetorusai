...
/** `Plugin: geetorusai.plugin-briefs` → `Briefs`; null when not a plugin label. */
function pluginPackageLabel(raw: string): string | null {
  const match = /^plugin:\s*(.+)$/i.exec(raw);
  if (!match) return null;
  let leaf = match[1].trim();
  // Keep the package leaf only: `geetorusai.plugin-briefs` → `plugin-briefs`.
 ...
 *   `Plugin: geetorusai.plugin-briefs` → `Briefs`
