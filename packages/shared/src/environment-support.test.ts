...
describe("isSandboxProviderSupportedForAdapter", () => {
  it("treats Geetorus Runner as a remote-managed adapter", () => {
    expect(adapterSupportsRemoteManagedEnvironments("geetorus_runner")).toBe(true);
    expect(supportedEnvironmentDriversForAdapter("geetorus_runner")).toEqual([
      "local",
      "ssh",
      "sandbox",
    ]);
