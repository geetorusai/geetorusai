 ...
 * whatever chat surface they like, so anything folded in here leaves Geetorus.
 ...
 * operator has to paste, because that is the only part Geetorus's config
 * preview parser reads. Everything else is prose for the human.
 */
export const MCP_CONFIG_HELP_PROMPT = `I want to connect an MCP (Model Context Protocol) server to Geetorus, and I need a paste-ready config.
...
export const MCP_CONFIG_HELP_INSTRUCTIONS = [
  ...
  "Geetorus reads the header names from that JSON and asks you for the values, then stores them as Geetorus secrets — so the config you paste should contain placeholders, not live credentials.",
] as const;
