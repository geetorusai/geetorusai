...
export const toolConnectionCredentialSourceSchema = z.enum(["geetorus_vault", "vercel_connect"]);
...
export const connectionGrantSchema = z.object({
  ...
  if (grant.externalCredential && grant.credentialSecretRefs.length > 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["credentialSecretRefs"], message: "External grants cannot also contain Geetorus secret references" });
  }
});
 ...
 *
 * `auto` is the default and the only value the simple path sends: Geetorus
 ...
 * it becomes a Geetorus secret ref and is never read back.
 */
...
/**
 * Reject `headers.*` credential paths whose header name Geetorus refuses to
...
export const connectToolAppSchema = z.object({
  ...
  credentialSource: z.enum(["geetorus_vault", "vercel_connect"]).optional(),
  ...
  const source = value.credentialSource ?? "geetorus_vault";
...
export const toolPolicyConditionsSchema = z.object({
  ...
  trustBoundary: z.object({
    ...
    geetorusSelfOnly: z.boolean().optional(),
  }).strict().optional(),
