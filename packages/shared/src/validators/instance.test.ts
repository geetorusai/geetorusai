...
describe("instance experimental settings validators", () => {
  ...
  it("defaults Geetorus developer mode off and accepts explicit patches", () => {
    expect(instanceExperimentalSettingsSchema.parse({}).enableGeetorusDeveloperMode).toBe(false);
    expect(
      patchInstanceExperimentalSettingsSchema.parse({ enableGeetorusDeveloperMode: true }),
    ).toEqual({ enableGeetorusDeveloperMode: true });
  });
