...
describe("readRuntimeExposureIntent", () => {
  it("treats a legacy expose block with no exposure fields as unset", () => {
    // The pre-feature Geetorus App template shape: an `expose` block that only
    // describes the backend URL. This must be defaultable, not opted out.
    expect(readRuntimeExposureIntent({ urlTemplate: "http://geetorus-dev:{{port}}" })).toBe("unset");
...
describe("resolveDeclaredRuntimeExposureConfig", () => {
  it("returns null unless the block explicitly opts in", () => {
    ...
    expect(resolveDeclaredRuntimeExposureConfig({ urlTemplate: "http://geetorus-dev:{{port}}" })).toBeNull();
  ...
  it("honors an explicit sub-field override", () => {
    expect(
      resolveDeclaredRuntimeExposureConfig({ type: "tailscale_https", includeGeetorusViteHmr: false }),
    ).toEqual({ ...DEFAULT_TAILSCALE_HTTPS_EXPOSURE, includeGeetorusViteHmr: false });
  });
  ...
  it("accepts a well-formed ready status", () => {
    expect(() =>
      runtimeExposureStatusSchema.parse({
        ...
        state: "ready",
        publicUrl: "https://geetorus-dev.tail29c1aa.ts.net:42010",
        hostname: "geetorus-dev.tail29c1aa.ts.net",
        listeners: [
