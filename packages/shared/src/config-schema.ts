...
export const databaseBackupConfigSchema = z.object({
  ...
  dir: z.string().default("~/.geetorus/instances/default/data/backups"),
}).passthrough();

export const databaseConfigSchema = z.object({
  mode: z.enum(["embedded-postgres", "postgres"]).default("embedded-postgres"),
  connectionString: z.string().optional(),
  embeddedPostgresDataDir: z.string().default("~/.geetorus/instances/default/db"),
  embeddedPostgresPort: z.number().int().min(1).max(65535).default(54329),
  backup: databaseBackupConfigSchema.default({
    enabled: true,
    intervalMinutes: 60,
    retentionDays: 7,
    dir: "~/.geetorus/instances/default/data/backups",
  }),
}).passthrough();

export const loggingConfigSchema = z.object({
  mode: z.enum(["file", "cloud"]),
  logDir: z.string().default("~/.geetorus/instances/default/logs"),
}).passthrough();
...
export const storageLocalDiskConfigSchema = z.object({
  baseDir: z.string().default("~/.geetorus/instances/default/data/storage"),
}).passthrough();

export const storageS3ConfigSchema = z.object({
  bucket: z.string().min(1).default("geetorus"),
...
export const storageConfigSchema = z.object({
  ...
  localDisk: storageLocalDiskConfigSchema.default({
    baseDir: "~/.geetorus/instances/default/data/storage",
  }),
...
export const secretsLocalEncryptedConfigSchema = z.object({
  keyFilePath: z.string().default("~/.geetorus/instances/default/secrets/master.key"),
}).passthrough();
...
export const secretsConfigSchema = z.object({
  ...
  localEncrypted: secretsLocalEncryptedConfigSchema.default({
    keyFilePath: "~/.geetorus/instances/default/secrets/master.key",
  }),
}).passthrough();
...
export const geetorusConfigSchema = z
  .object({
...
export type GeetorusConfig = z.infer<typeof geetorusConfigSchema>;
 ...
 */
export function mergeGeetorusConfig(
  source: GeetorusConfig,
  update: GeetorusConfig,
): GeetorusConfig {
...
export function findGeetorusConfigKeyWarnings(config: unknown): ConfigKeyWarning[] {
  if (!isRecord(config)) return [];
  const warnings: ConfigKeyWarning[] = [];
  collectConfigKeyWarnings(config, geetorusConfigSchema, "", warnings);
  return warnings;
}
