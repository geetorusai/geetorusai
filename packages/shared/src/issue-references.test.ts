...
describe("issue references", () => {
  ...
  it("parses relative and absolute issue hrefs", () => {
    ...
    expect(parseIssueReferenceHref("https://geetorus.ing/PAP/issues/pap-789#comment-1")).toEqual({
      identifier: "PAP-789",
    });
    expect(parseIssueReferenceHref("https://geetorus.ing/projects/PAP-789")).toBeNull();
  });
