...
export type IssueQueuedCommentProtocol = "geetorus_runner_v1" | "legacy";
 ...
 * mirrors `geetorus.question_set.v1` without making the shared package depend
 * on a particular runner implementation.
 */
export interface GeetorusQuestionSetOption {
  id: string;
  label: string;
  description?: string;
  recommended?: boolean;
...
export interface GeetorusQuestionSetQuestion {
  id: string;
  header?: string;
  prompt: string;
  helpText?: string;
  ...
  options?: GeetorusQuestionSetOption[];
  customAnswer?: {
    enabled: true;
    label?: string;
    placeholder?: string;
...
export interface GeetorusQuestionSetPayload {
  schema: "geetorus.question_set.v1";
  title?: string;
  description?: string;
  submitLabel?: string;
  questions: GeetorusQuestionSetQuestion[];
}
