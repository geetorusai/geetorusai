 ...
 *
 * The Geetorus extension keywords below are recognised by the Geetorus UI
 * but are otherwise ignored by standard JSON Schema validators.
 */
export type JsonSchema = {
  /**
   * When true, the Geetorus config UI hides this property behind an
   * "Advanced options" disclosure. Defaults to false (always visible).
   */
  "x-geetorus-advanced"?: boolean;
  /**
   ...
   * `x-geetorus-advanced` is not true.
   */
  "x-geetorus-group"?: string;
  [key: string]: unknown;
};
...
// ---------------------------------------------------------------------------
// Manifest sub-types — nested declarations within GeetorusPluginManifestV1
// ---------------------------------------------------------------------------
...
export interface PluginEnvironmentDriverDeclaration {
  ...
  /**
   * How Geetorus should apply a captured template ref back into this provider's
...
/**
 * Declares a normal Geetorus agent that a plugin can provision and later
...
export interface PluginManagedAgentDeclaration {
  ...
  /** Suggested Geetorus runtime configuration. */
...
/**
 * Declares a normal Geetorus project that a plugin can provision and later
 * resolve by stable key within each company.
 */
...
/**
 * Lower-bound semver requirement for the Geetorus host.
 *
...
export interface GeetorusPluginManifestV1 {
   ...
   */
  minimumGeetorusVersion?: PluginMinimumHostVersion;
