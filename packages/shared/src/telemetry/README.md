...
This document explains how contributors should use Geetorus's public telemetry
...
`packages/shared/src/telemetry/generated/geetorus-telemetry.ts`.
...
| --------------------------------------------- | ---------------------------------------------------------------------------------------- |
| First-party event names                       | `GeetorusEventName` in `generated/geetorus-telemetry.ts`                               |
| Per-event dimensions and optionality          | `EventDimensionsMap` in `generated/geetorus-telemetry.ts`                               |
| Enum descriptions for telemetry dimensions    | `GEETORUS_ENUM_DESCRIPTIONS` in `generated/geetorus-telemetry.ts`                      |
| Schema version and event envelope helpers     | `SCHEMA_VERSION`, `makeEvent()`, and `makeBatch()` in `generated/geetorus-telemetry.ts` |
...
Geetorus telemetry uses named events with explicit dimension fields. Treat
...
This document covers Geetorus Telemetry only. The generated Telemetry
...
1. Start from `generated/geetorus-telemetry.ts`. The generated types are what
