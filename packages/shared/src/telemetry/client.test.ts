...
describe("TelemetryClient runtime event gate", () => {
  ...
  it("uses own-property membership so prototype event names are swallowed", async () => {
    ...
    // @ts-expect-error constructor is grammar-valid but not a registered Geetorus event.
    client.track("constructor", {});
    // @ts-expect-error toString is grammar-valid but not a registered Geetorus event.
