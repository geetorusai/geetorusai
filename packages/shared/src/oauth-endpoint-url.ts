/**
 * Scheme/transport safety for OAuth endpoints Geetorus learned from a remote
 * server (PAP-17099).
 *
 ...
 * `authorization_endpoint` it names is not just fetched server-side — Geetorus
 ...
 *   reads as the wrong origin to a human and Geetorus must not help;
...
export interface OAuthEndpointUrlOptions {
   ...
   * in practice just Geetorus's own deployment origin. A deployment served over
   * plaintext HTTP on a LAN address can still run its own authorization
   * endpoints (the smoke-lab fixture does), and opening Geetorus from Geetorus
...
/**
 * Is `value` an OAuth endpoint Geetorus is willing to use — and, for the
...
export function oauthEndpointUrlRejectionMessage(
  ...
  switch (reason) {
    ...
    case "malformed":
      return `This server's ${label} address is not a valid URL, so Geetorus stopped.`;
    case "unsupported_scheme":
      return `This server's ${label} address does not use https, so Geetorus stopped.`;
    case "insecure_transport":
      return `This server's ${label} address is not secure (https), so Geetorus stopped.`;
    case "embedded_credentials":
      return `This server's ${label} address hides a different site behind a username, so Geetorus stopped.`;
    case "fragment":
      return `This server's ${label} address is malformed for sign-in, so Geetorus stopped.`;
  }
}
