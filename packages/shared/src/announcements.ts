...
export const DEFAULT_ANNOUNCEMENT_FEED_URL = "https://pages.geetorus.ing/announcements/v1/current.json";
...
export const announcementSchema = z.object({
  ...
  minimumGeetorusVersion: z.string().regex(/^\d+\.\d+\.\d+$/).optional(),
...
export function isAnnouncementEligible(announcement: Announcement, version: string, now = Date.now()): boolean {
  ...
  if (!announcement.minimumGeetorusVersion) return true;
  const installed = /^(\d+)\.(\d+)\.(\d+)(-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.exec(version);
  if (!installed) return false;
  const minimum = announcement.minimumGeetorusVersion.split(".").map(Number);
  for (let i = 0; i < 3; i++) {
