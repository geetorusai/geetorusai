 ...
 *     --config /path/to/geetorus/config.json \
 *     [--claude-config-dir ~/.claude] \
 *     [--dry-run] [--json]
 *
 * Or in a Geetorus checkout shell:
 *   pnpm --filter @geetorusai/db exec tsx scripts/clean-poisoned-claude-sessions.ts --dry-run
 *
...
Flags:
  --config <path>            Path to geetorus config.json (defaults to
                             $GEETORUS_HOME/instances/default/config.json or
...
function defaultConfigPath(): string | null {
  ...
  if (process.env.GEETORUS_HOME) {
    candidates.push(path.join(process.env.GEETORUS_HOME, "config.json"));
  }
  candidates.push(path.join(home, ".geetorus", "instances", "default", "config.json"));
...
async function main(argv: string[]): Promise<void> {
  ...
  if (!databaseUrl) {
    throw new Error(
      "Unable to resolve database URL. Pass --database-url or --config <geetorus config.json>.",
    );
  }
