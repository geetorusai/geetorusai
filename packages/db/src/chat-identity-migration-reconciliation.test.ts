...
(support.supported ? describe : describe.skip)(
  ...
  () => {
    ...
    it(
      ...
      async () => {
        const database = await startEmbeddedPostgresTestDatabase(
          "geetorus-chat-provenance-fresh-",
        );
        const sql = postgres(database.connectionString, {
          max: 1,
          onnotice: () => {},
...
(support.supported ? describe : describe.skip)(
  ...
  () => {
    it(
      ...
      async () => {
        const database = await startEmbeddedPostgresTestDatabase(
          "geetorus-chat-upstream-upgrade-",
        );
        ...
        const directory = await mkdtemp(
          join(tmpdir(), "geetorus-chat-old-migrations-"),
        );
    ...
    it(
      ...
      async () => {
        const database = await startEmbeddedPostgresTestDatabase(
          "geetorus-chat-identity-migration-",
        );
        const sql = postgres(database.connectionString, {
          max: 1,
          onnotice: () => {},
