...
(support.supported ? describe : describe.skip)("session goal migration", () => {
  it("preserves preview goals, tombstone revisions, and pending actions on replay", async () => {
    ...
    try {
      ...
      await sql`INSERT INTO agents (id, company_id, name, role, adapter_type) VALUES (${agentId}, ${companyId}, 'Goal agent', 'engineer', 'geetorus_runner')`;
      ...
      await sql`INSERT INTO agent_task_sessions (id, company_id, agent_id, adapter_type, task_key, goal_json, goal_status, goal_revision)
        VALUES (${sessionId}, ${companyId}, ${agentId}, 'geetorus_runner', 'active', ${sql.json(goal)}, 'paused', 7),
               (${clearedId}, ${companyId}, ${agentId}, 'geetorus_runner', 'cleared', NULL, NULL, 9)`;
