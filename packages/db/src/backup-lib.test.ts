...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("geetorus-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("geetorus-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const backupDir = createTempDir("geetorus-db-backup-retention-");
      const realDateNow = Date.now;
      Date.now = () => Date.UTC(2026, 2, 31, 12, 0, 0);

      const janNewest = path.join(backupDir, "geetorus-test-2026-01-28T12-00-00.sql.gz");
      const janOlder = path.join(backupDir, "geetorus-test-2026-01-10T12-00-00.sql.gz");
      const decOld = path.join(backupDir, "geetorus-test-2025-12-15T12-00-00.sql.gz");
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "geetorus_restore_target",
      );
      const backupDir = createTempDir("geetorus-db-backup-output-");
      ...
      try {
        ...
        expect(result.backupFile).toMatch(/geetorus-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "geetorus_full_logical_restore_target",
      );
      const backupDir = createTempDir("geetorus-db-full-logical-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        await sourceSql.unsafe(`
          ...
          VALUES ('geetorus-migration-history', 1770000000000);
        `);
        await sourceSql.unsafe(`
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "geetorus-full-logical-test",
          backupEngine: "javascript",
        ...
        const migrationRows = await restoreSql.unsafe<{ hash: string }[]>(`
          ...
          WHERE "hash" = 'geetorus-migration-history'
        `);
        expect(migrationRows).toEqual([{ hash: "geetorus-migration-history" }]);
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "geetorus_composite_fk_restore_target",
      );
      const backupDir = createTempDir("geetorus-db-composite-fk-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "geetorus-composite-fk-test",
          backupEngine: "javascript",
        });
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "geetorus_copy_fk_restore_target",
      );
      const backupDir = createTempDir("geetorus-db-copy-fk-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const originalPgDumpPath = process.env.GEETORUS_PG_DUMP_PATH;
      const originalPsqlPath = process.env.GEETORUS_PSQL_PATH;
      process.env.GEETORUS_PG_DUMP_PATH = "/bin/false";
      process.env.GEETORUS_PSQL_PATH = "/bin/false";
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "geetorus-copy-fk-test",
          backupEngine: "auto",
        });
        ...
        if (originalPgDumpPath === undefined) {
          delete process.env.GEETORUS_PG_DUMP_PATH;
        } else {
          process.env.GEETORUS_PG_DUMP_PATH = originalPgDumpPath;
        }
        if (originalPsqlPath === undefined) {
          delete process.env.GEETORUS_PSQL_PATH;
        } else {
          process.env.GEETORUS_PSQL_PATH = originalPsqlPath;
        }
        await sourceSql.end();
        await restoreSql.end();
      }
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("geetorus-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Geetorus database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- geetorus statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
