...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "geetorus");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://geetorus:geetorus@127.0.0.1:${preferredPort}/geetorus`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "geetorus");
    return {
      connectionString: `postgres://geetorus:geetorus@127.0.0.1:${port}/geetorus`,
  ...
  const adminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "geetorus");

  return {
    connectionString: `postgres://geetorus:geetorus@127.0.0.1:${selectedPort}/geetorus`,
