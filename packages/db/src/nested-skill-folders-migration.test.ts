...
describeEmbeddedPostgres("nested skill folders migration", () => {
  ...
  it("backfills existing folders, bundled skills, and project scan skills", async () => {
    const database = await startEmbeddedPostgresTestDatabase("geetorus-nested-folders-migration-");
    ...
    await sql`
      ...
      VALUES
        (${bundledSkillId}, ${companyId}, 'geetorusai/bundled/software-development/review', 'review', 'Review', '# Review', '{"sourceKind":"geetorus_bundled"}'::jsonb),
