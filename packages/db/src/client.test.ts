...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("geetorus-db-client-");
...
describeEmbeddedPostgres("createDb pool defaults", () => {
  it("names its backends and closes them once idle", async () => {
    ...
    const shortLived = createDb(url, { applicationName: "geetorus-idle-test", idleTimeoutSeconds: 1 });
    cleanups.push(async () => {
      await shortLived.$client.end({ timeout: 1 });
    });
    await shortLived.$client`select 1`;
    expect(await backendsNamed("geetorus-idle-test")).toBe(1);
    ...
    let remaining = await backendsNamed("geetorus-idle-test");
    while (remaining > 0 && Date.now() < deadline) {
      await new Promise((resolve) => setTimeout(resolve, 250));
      remaining = await backendsNamed("geetorus-idle-test");
    }
...
describeEmbeddedPostgres("applyPendingMigrations", () => {
  ...
  it(
    ...
    async () => {
      ...
      const directory = await fs.promises.mkdtemp(join(tmpdir(), "geetorus-native-prior-migrations-"));
      ...
      try {
        ...
        await expect(verifySql`
          INSERT INTO completion_contracts (
            ...
            ${companyId}, ${otherIssueId}, 1, 'geetorus.completion-contract.v1',
        ...
        await verifySql`
          INSERT INTO completion_contracts (
            ...
            ${contractId}, ${companyId}, ${issueId}, 1, 'geetorus.completion-contract.v1',
            'policy-v1', 'low', 'server', 'review', ${JSON.stringify({ criteria: [] })}::jsonb,
        ...
        await verifySql`
          INSERT INTO completion_contracts (
            ...
            'geetorus.completion-contract.v1', 'policy-v1', 'low', 'server', 'review',
        ...
        const replayed = await replaySql.unsafe<{
          ...
          SELECT
            ...
            (
              SELECT count(*)::int
              FROM pg_trigger
              WHERE tgname = 'geetorus_issue_status_version_trigger'
