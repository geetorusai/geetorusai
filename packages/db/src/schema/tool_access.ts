...
export const toolConnections = pgTable(
  "tool_connections",
  {
    ...
    credentialSource: text("credential_source").$type<ToolConnectionCredentialSource>().notNull().default("geetorus_vault"),
  ...
  (table) => [
    ...
    check("tool_connections_credential_source_check", sql`${table.credentialSource} in ('geetorus_vault', 'vercel_connect')`),
    check("tool_connections_credential_source_one_of_check", sql`(
      (${table.credentialSource} = 'geetorus_vault' and ${table.externalCredential} is null)
      or
