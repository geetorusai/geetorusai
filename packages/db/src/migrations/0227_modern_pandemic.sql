...
CREATE OR REPLACE FUNCTION geetorus_bump_issue_status_version()
RETURNS trigger AS $$
BEGIN
...
DROP TRIGGER IF EXISTS geetorus_issue_status_version_trigger ON "issues";--> statement-breakpoint
CREATE TRIGGER geetorus_issue_status_version_trigger
BEFORE UPDATE OF "status" ON "issues"
FOR EACH ROW EXECUTE FUNCTION geetorus_bump_issue_status_version();--> statement-breakpoint
DO $$ BEGIN
...
-- geetorus:migration-safety-ignore large-create-index-not-concurrently: nullable native source IDs mean historical rows need no backfill and the invariant must commit atomically with the new columns.
...
-- geetorus:migration-safety-ignore large-create-index-not-concurrently: nullable native source sequence fields mean historical rows need no backfill and the invariant must commit atomically with the new columns.
...
--> statement-breakpoint
-- geetorus:migration-safety-ignore large-create-index-not-concurrently: the primary key already makes this ownership tuple unique; this supporting index only enables composite foreign keys.
