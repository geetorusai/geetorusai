...
-- Keeps the oldest row per (company_id) where driver='sandbox' AND metadata.managedByGeetorus is true.
-- Scoped to Geetorus-managed rows only: tenant-created sandbox envs (without the marker) are untouched.
DO $$
DECLARE
...
BEGIN
  WITH ranked AS (
    ...
    WHERE driver = 'sandbox'
      AND (metadata ->> 'managedByGeetorus')::boolean = true
  ),
  deleted AS (
