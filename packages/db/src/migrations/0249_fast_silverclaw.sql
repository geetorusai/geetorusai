...
-- geetorus:migration-safety-ignore large-create-index-not-concurrently: Transactional migrations require the unique delivery claim before writers start. This new key namespace has no rows, but index construction can briefly block wakeup writes.
