  ...
  WITH ranked AS (
    SELECT
      ...
      CASE
        ...
        WHEN e.driver = 'local' THEN '__geetorus_builtin_local__'
        WHEN e.driver = 'sandbox' AND (e.metadata ->> 'managedByGeetorus')::boolean = true
          THEN '__geetorus_managed_sandbox__'
        ELSE e.id::text
      END AS group_key,
      row_number() OVER (
        PARTITION BY CASE
...
SELECT
  ...
  '{}'::jsonb,
  '{}'::jsonb,
  '{"managedByGeetorus": true, "defaultForInstance": true}'::jsonb,
  now(),
  now()
WHERE NOT EXISTS (
  SELECT 1
...
UPDATE "environments"
SET
  "metadata" = COALESCE("metadata", '{}'::jsonb) || '{"managedByGeetorus": true, "defaultForInstance": true}'::jsonb,
  "updated_at" = now()
WHERE "driver" = 'local';
--> statement-breakpoint
...
CREATE UNIQUE INDEX "environments_managed_sandbox_idx"
  ON "environments" USING btree ("driver")
  WHERE "driver" = 'sandbox' AND ("metadata" ->> 'managedByGeetorus')::boolean = true;
--> statement-breakpoint
