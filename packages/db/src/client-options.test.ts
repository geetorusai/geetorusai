...
describe("databaseClientOptionsFromEnv", () => {
  ...
  it("parses pool and timeout settings", () => {
    expect(
      databaseClientOptionsFromEnv({
        ...
        DATABASE_APPLICATION_NAME: " geetorus-web ",
      }),
    ).toEqual({
      maxConnections: 25,
  ...
  it("maps to postgres.js option names", () => {
    expect(
      ...
      connect_timeout: 10,
      max_lifetime: 1800,
      connection: { application_name: "geetorus-web" },
    });
  });
});
  ...
  it("keeps every explicit value, including an idle timeout of 0", () => {
    expect(
      resolveDatabaseClientOptions({
        ...
        applicationName: "geetorus-cli",
      }),
    ).toEqual({ maxConnections: 3, idleTimeoutSeconds: 0, applicationName: "geetorus-cli" });
