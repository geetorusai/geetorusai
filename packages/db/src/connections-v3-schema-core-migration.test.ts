...
describeEmbeddedPostgres("connections v3 schema core migration", () => {
  ...
  it("backfills a workspace grant and rolls back without losing the connection", async () => {
    const database = await startEmbeddedPostgresTestDatabase("geetorus-connections-v3-migration-");
    ...
    await sql`INSERT INTO "companies" ("id", "name", "issue_prefix") VALUES (${companyId}, 'Geetorus', 'PAP')`;
