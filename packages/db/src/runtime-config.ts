...
import {
  ...
  resolveGeetorusEnvPathForConfig,
} from "@geetorusai/shared/home-paths";
...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "geetorus-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".geetorus", CONFIG_BASENAME);
...
function resolveGeetorusConfigPath(): string {
  if (process.env.GEETORUS_CONFIG?.trim()) {
    return path.resolve(process.env.GEETORUS_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveGeetorusConfigPathForInstance();
}

function resolveGeetorusEnvPath(configPath: string): string {
  return resolveGeetorusEnvPathForConfig(configPath);
}
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveGeetorusEnvPath(configPath);
