...
const DEFAULT_GEETORUS_EMBEDDED_POSTGRES_PORT = 54329;

function getReservedTestPorts(): Set<number> {
  const configuredPorts = [
    DEFAULT_GEETORUS_EMBEDDED_POSTGRES_PORT,
    Number.parseInt(process.env.GEETORUS_EMBEDDED_POSTGRES_PORT ?? "", 10),
    ...String(process.env.GEETORUS_TEST_POSTGRES_RESERVED_PORTS ?? "")
      .split(",")
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Geetorus ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  ...
  try {
    started = await startEmbeddedPostgresWithRetry("geetorus-embedded-postgres-probe-");
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://geetorus:geetorus@127.0.0.1:${port}/geetorus`;
