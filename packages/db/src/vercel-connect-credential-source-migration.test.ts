...
describeEmbeddedPostgres("Vercel Connect credential-source migration", () => {
  it("defaults legacy rows to the vault and rejects mixed credential custody", async () => {
    const database = await startEmbeddedPostgresTestDatabase("geetorus-vercel-connect-migration-");
    ...
    `;
    expect(legacy).toEqual({ credential_source: "geetorus_vault", external_credential: null });
    ...
    await sql`
      UPDATE "tool_connections" SET
        ...
        "external_credential" = ${sql.json({ provider: "vercel_connect", connectorUid: "posthog-geetorus" })},
