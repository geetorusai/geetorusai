...
export const GEETORUS_RUNNER_INGRESS_PORT = 43_127;
export const GEETORUS_RUNNER_CONNECT_PATH_PREFIX = "/api/runner/v1/connect";
...
export type GeetorusRunnerTransport =
  | {
...
export class GeetorusRunnerTransportError extends Error {
  readonly code:
  ...
  constructor(
    code: GeetorusRunnerTransportError["code"],
    message: string,
    options?: ErrorOptions,
  ) {
    super(`${code}: ${message}`, options);
    this.name = "GeetorusRunnerTransportError";
    this.code = code;
  }
}
...
function connectPath(runId: string): string {
  if (!runId || runId.includes("/") || runId.includes("?") || runId.includes("#")) {
    throw new GeetorusRunnerTransportError(
      "runner_transport_ineligible",
      "Runner run id is not safe for a WebSocket route.",
    );
  }
  return `${GEETORUS_RUNNER_CONNECT_PATH_PREFIX}/${encodeURIComponent(runId)}`;
}
...
export async function resolveGeetorusRunnerTransport(input: {
...
} & RunnerIngressAuthorization): Promise<GeetorusRunnerTransport> {
  ...
  if (
    ...
    if (!ingressAuthorized) {
      throw new GeetorusRunnerTransportError(
        "runner_ingress_unavailable",
        "Runner ingress is not authorized for this Geetorus Runner run.",
      );
    }
    ...
    const ingress = await getRunnerIngressEndpoint({
      ...
      port: GEETORUS_RUNNER_INGRESS_PORT,
      path,
    });
    return {
      mode: "provider_ingress",
      listenAddress: "0.0.0.0",
      listenPort: GEETORUS_RUNNER_INGRESS_PORT,
      listenPath: path,
      ingress,
    };
  }
