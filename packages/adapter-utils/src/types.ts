...
import type { ExecutionContinuationEnvelope, NativeFinalizationResult } from "@geetorusai/shared";
...
export interface AdapterEnvironmentTestContext {
   ...
   *
   * If omitted (or `kind === "local"`), the adapter tests on the Geetorus
...
export interface AdapterRuntimeCommandSpec {
  /**
   * The command Geetorus should execute for this adapter in the current config.
   */
  command: string;
  /**
...
export interface ServerAdapterModule {
  ...
  /** How this adapter receives Geetorus's run-scoped control tools. */
   ...
   * rather than reading config.geetorusRuntimeSkills.
   */
...
export type TranscriptEntry =
  ...
  | { kind: "runtime_request"; ts: string; requestId: string; requestKind: "runtime" | "command_approval" | "file_approval" | "permission_approval" | "user_input" | "elicitation" | null; turnId: string | null; requestType: "permission" | "input"; status: "pending" | "resolved" | "expired" | "cancelled"; prompt: string; choices: Array<{ key: string; label: string }>; fields: Array<{ name: string; label: string; placeholder: string | null }>; questionSet?: GeetorusQuestionSet | null; resolvedAction?: string | null; response?: GeetorusQuestionResponse | null }
...
export interface CreateConfigValues {
  ...
  geetorusRunnerLifecycleMode?: "per_turn" | "warm";
  geetorusRunnerIdleTimeoutMs?: number;
  cwd: string;
