...
describe("stdin file race (parent PAP-4037)", () => {
  ...
  async function startPollerWrapper(options?: { maxRetries?: number }) {
    const sessionDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-stdin-poll-"));
    ...
    const env: Record<string, string> = {
      ...
      GEETORUS_PROCESS_SESSION_COMMAND_B64: commandPayload,
    };
    if (options?.maxRetries != null) {
      env.GEETORUS_PROCESS_SESSION_STDIN_MAX_RETRIES = String(options.maxRetries);
    }
  ...
  it("serializes host stdin writes so a slow earlier write still lands first", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-stdin-host-order-"));
  ...
  it("holds stdinEnd on stop until an earlier pending stdin write lands first", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-stdin-stop-order-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "echo-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', (c) => process.stdout.write(c));\n", "utf8");
  ...
  it("finalizes the command-managed host write with an atomic rename onto the .json path", async () => {
    const remoteRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-stdin-host-cmd-"));
    ...
    expect(finalizeScript).toContain(`> '${jsonPath}.geetorus-upload.decoded'`);
  });

  it("never exposes a partial .json file under a concurrent reader (command-managed host write)", async () => {
    const remoteRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-stdin-host-race-"));
    cleanupDirs.push(remoteRoot);
    const stdinDir = path.join(remoteRoot, "stdin");
    await mkdir(stdinDir, { recursive: true });
...
describe("deterministic remote process-session wrapper shutdown (PAP-5316)", () => {
  ...
  async function getFakeBirthtimePreloadPath(): Promise<string> {
    if (!fakeBirthtimePreloadPath) {
      fakeBirthtimePreloadPath = (async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "geetorus-birthtime-preload-"));
        ...
        await writeFile(
          ...
          [
            ...
            `const target = process.env.GEETORUS_TEST_FAKE_BIRTHTIME_TARGET;`,
            `const mode = process.env.GEETORUS_TEST_FAKE_BIRTHTIME_MODE;`,
  ...
  async function getProbeSwapPreloadPath(): Promise<string> {
    if (!probeSwapPreloadPath) {
      probeSwapPreloadPath = (async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-swap-preload-"));
        ...
        await writeFile(
          ...
          [
            `const fs = require("fs");`,
            `const path = require("path");`,
            `const mode = process.env.GEETORUS_TEST_PROBE_SWAP_MODE;`,
            `const seq = process.env.GEETORUS_TEST_PROBE_SWAP_SEQ;`,
            `const symlinkTarget = process.env.GEETORUS_TEST_PROBE_SWAP_SYMLINK_TARGET;`,
            `if (mode && seq) {`,
            `  const expectedName = ".geetorus-birthtime-probe-" + process.pid + "-" + seq;`,
            `  let swapped = false;`,
            `  const originalLstat = fs.promises.lstat.bind(fs.promises);`,
            `  fs.promises.lstat = async (candidatePath, opts) => {`,
  ...
  async function getFstatFailurePreloadPath(): Promise<string> {
    if (!fstatFailurePreloadPath) {
      fstatFailurePreloadPath = (async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "geetorus-fstat-failure-preload-"));
        ...
        await writeFile(
          ...
          [
            `const fs = require("fs");`,
            `const path = require("path");`,
            `const seq = process.env.GEETORUS_TEST_FSTAT_FAILURE_SEQ;`,
            `if (seq) {`,
            `  const expectedName = ".geetorus-birthtime-probe-" + process.pid + "-" + seq;`,
  ...
  async function startWrapperProcess(options?: {
    ...
    const sessionDir = await mkdtemp(path.join(options?.parentDir ?? os.tmpdir(), "geetorus-wrapper-lifecycle-"));
    cleanupDirs.push(sessionDir);
    const stdinDir = path.join(sessionDir, "stdin");
    ...
    if (options?.maxRetries != null) env.GEETORUS_PROCESS_SESSION_STDIN_MAX_RETRIES = String(options.maxRetries);
    if (options?.terminateGraceMs != null) env.GEETORUS_PROCESS_SESSION_TERMINATE_GRACE_MS = String(options.terminateGraceMs);

    const execArgv: string[] = [];
    if (options?.fakeBirthtime) {
      env.GEETORUS_TEST_FAKE_BIRTHTIME_TARGET = options.fakeBirthtime.target;
      env.GEETORUS_TEST_FAKE_BIRTHTIME_MODE = options.fakeBirthtime.mode;
      execArgv.push("--require", await getFakeBirthtimePreloadPath());
    }
    if (options?.probeSwap) {
      env.GEETORUS_TEST_PROBE_SWAP_SEQ = String(options.probeSwap.seq);
      env.GEETORUS_TEST_PROBE_SWAP_MODE = options.probeSwap.mode;
      if (options.probeSwap.symlinkTarget) env.GEETORUS_TEST_PROBE_SWAP_SYMLINK_TARGET = options.probeSwap.symlinkTarget;
      execArgv.push("--require", await getProbeSwapPreloadPath());
    }
    if (options?.fstatFailure) {
      env.GEETORUS_TEST_FSTAT_FAILURE_SEQ = String(options.fstatFailure.seq);
  ...
  async function startTrackedBridgeSession(input: {
    ...
    const bridge = await startAdapterExecutionTargetProcessSessionBridge({
      runId: input.runId,
      target,
      runtimeRootDir: path.posix.join(input.rootDir, ".geetorus-runtime", "acpx"),
      adapterKey: "acpx",
      command: process.execPath,
      args: [childPath],
      cwd: input.rootDir,
  ...
  it("T2 leaves neither the wrapper nor a stubborn child alive after stop()", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-stubborn-child-"));
    cleanupDirs.push(rootDir);
    ...
    const wrapperScriptSubstring = path.posix.join(rootDir, ".geetorus-runtime", "acpx", "process-sessions");
    try {
  ...
  it("T4 stopping one session leaves a sibling session's wrapper and child alive", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-session-isolation-"));
    cleanupDirs.push(rootDir);
    const runner = createLocalSandboxRunner();
  ...
  it("T5 still writes both control messages, finishes fast, and warns never after a forged exit event the live poll reads early", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-forged-exit-"));
    cleanupDirs.push(rootDir);
    ...
    const finalizeWrites = scripts.filter((script) => script.includes("base64 -d") && script.includes(".geetorus-upload.decoded"));
  ...
  it("T14 a forged exit event alone does not shorten the wait when the wrapper never truly runs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-forged-only-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "quiet-child.mjs");
    await writeFile(childPath, "process.stdin.resume();\n", "utf8");
  ...
  it("T6 issues no operating-system signal from the host during stop()", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-no-signal-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "quiet-child.mjs");
    await writeFile(childPath, "process.stdin.resume();\n", "utf8");
  ...
  it("T9 exits with an error event when sessionDir is a symbolic link", async () => {
    const targetDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-symlink-target-"));
    ...
    const child = spawn(process.execPath, [wrapperPath], {
      ...
      env: {
        ...process.env,
        GEETORUS_PROCESS_SESSION_DIR: linkDir,
        GEETORUS_PROCESS_SESSION_COMMAND_B64: commandPayload,
      },
      stdio: ["ignore", "pipe", "pipe"],
    });
  ...
  it("T10 the emitted wrapper strips its own session env vars from the child", async () => {
    const wrapper = await startWrapperProcess({
      ...
      args: [
        "-e",
        "process.stdout.write(JSON.stringify(Object.keys(process.env).filter((k) => k.startsWith('GEETORUS_PROCESS_SESSION'))));process.exit(0)",
      ],
    });
  ...
  it("T12 stop() finishes well inside the shutdown budget and logs no warning after a normal child exit", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-normal-exit-"));
    cleanupDirs.push(rootDir);
  ...
  it("T13 warns and still removes sessionDir when the wrapper never acknowledges and never exits", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-never-acks-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "quiet-child.mjs");
    await writeFile(childPath, "process.stdin.resume();\n", "utf8");
  ...
  it("T15 latches on a lost session identity: a recreated control directory cannot keep the wrapper or its child alive", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-lost-identity-"));
    cleanupDirs.push(rootDir);
  ...
  it("T16 accepts a zero creation time when the filesystem does not report birth time", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-birthtime-zero-"));
    cleanupDirs.push(rootDir);
  ...
  it("T17 fails closed at capture when the reported creation time follows the change time, so a change-time copy never passes as a real creation time", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-birthtime-followctime-"));
    cleanupDirs.push(rootDir);
  ...
  it("T18 latches on an EACCES lstat failure on sessionDir during verification, not only on a removed directory", async () => {
    ...
    const parentDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-eacces-sessiondir-"));
  ...
  it("T19 latches on an EACCES lstat failure on stdinDir during verification, even though sessionDir itself still stats cleanly", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-eacces-stdindir-"));
    cleanupDirs.push(rootDir);
  ...
  it("T20 refuses to write through a probe path a sandbox peer pre-created as a symbolic link, and leaves that link and its target untouched", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-symlink-race-"));
    cleanupDirs.push(rootDir);
    const pidFile = path.join(rootDir, "t20-child.pid");
    const childPath = path.join(rootDir, "t20-child.mjs");
    await writeFile(childPath, trackedChildSource(pidFile), "utf8");

    const sessionDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-symlink-session-"));
    cleanupDirs.push(sessionDir);
    const stdinDir = path.join(sessionDir, "stdin");
    await mkdir(stdinDir, { recursive: true });
    ...
    // ".geetorus-birthtime-probe-<pid>-1" on the wrapper's first probe call,
    ...
    const probePath = path.join(sessionDir, `.geetorus-birthtime-probe-${child.pid}-1`);
  ...
  it("T21 still removes its own probe file normally when no peer ever replaces it", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-no-swap-"));
    cleanupDirs.push(rootDir);
    ...
    const probePath = path.join(wrapper.sessionDir, `.geetorus-birthtime-probe-${wrapper.pid}-1`);
  ...
  it("T22 leaves a peer's replacement file untouched instead of deleting it", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-swap-file-"));
    cleanupDirs.push(rootDir);
  ...
  it("T23 leaves a peer's replacement directory untouched instead of deleting it", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-swap-dir-"));
    cleanupDirs.push(rootDir);
  ...
  it("T24 leaves a peer's replacement symbolic link and its target untouched instead of deleting or following it", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-swap-symlink-"));
    cleanupDirs.push(rootDir);
  ...
  it("T25 fails closed at capture when its own probe file's identity cannot be read, so no orphan wrapper or child ever starts polling", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-probe-fstat-failure-"));
    cleanupDirs.push(rootDir);
