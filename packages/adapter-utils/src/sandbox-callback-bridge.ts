...
// (GEETORUS_BRIDGE_RESPONSE_TIMEOUT_MS), so the host loop fails fast and writes
...
export const SANDBOX_CALLBACK_BRIDGE_ENTRYPOINT = "geetorus-bridge-server.mjs";
const SANDBOX_EXEC_CHANNEL_ENV = "GEETORUS_SANDBOX_EXEC_CHANNEL";
...
// `GEETORUS_API_BRIDGE_MODE`. The generated gateway rejects every other
...
/** Span name that wraps one Geetorus-API callback request — read the request,
...
// reverse bridge. Keep this in sync with the Geetorus skill in
// `skills/geetorus/SKILL.md` and `references/api-reference.md`.
export const DEFAULT_SANDBOX_CALLBACK_BRIDGE_ROUTE_ALLOWLIST: readonly SandboxCallbackBridgeRouteRule[] = [
  ...
  // Hiring (geetorus-create-agent skill): adapter/icon discovery, comparing
...
export function buildSandboxCallbackBridgeEnv(input: {
  ...
  return {
    GEETORUS_API_BRIDGE_MODE: SANDBOX_CALLBACK_BRIDGE_FILE_MODE,
    GEETORUS_BRIDGE_QUEUE_DIR: input.queueDir,
    GEETORUS_BRIDGE_TOKEN: input.bridgeToken,
    GEETORUS_BRIDGE_HOST: input.host?.trim() || "127.0.0.1",
    GEETORUS_BRIDGE_PORT: String(input.port && input.port > 0 ? Math.trunc(input.port) : 0),
    GEETORUS_BRIDGE_POLL_INTERVAL_MS: String(
      normalizeTimeoutMs(input.pollIntervalMs, DEFAULT_BRIDGE_POLL_INTERVAL_MS),
    ),
    GEETORUS_BRIDGE_RESPONSE_TIMEOUT_MS: String(
      normalizeTimeoutMs(input.responseTimeoutMs, DEFAULT_BRIDGE_RESPONSE_TIMEOUT_MS),
    ),
    GEETORUS_BRIDGE_MAX_QUEUE_DEPTH: String(
      normalizeTimeoutMs(input.maxQueueDepth, DEFAULT_BRIDGE_MAX_QUEUE_DEPTH),
    ),
    GEETORUS_BRIDGE_MAX_BODY_BYTES: String(
...
export async function createSandboxCallbackBridgeAsset(): Promise<SandboxCallbackBridgeAsset> {
  const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-asset-"));
...
export function createFileSystemSandboxCallbackBridgeQueueClient(): SandboxCallbackBridgeQueueClient {
  return {
    ...
    writeTextFile: async (remotePath, body) => {
      ...
      const tempPath = `${remotePath}.geetorus-upload.decoded`;
    ...
    writeResponseFile: async (responsePath, body, options = {}) => {
      ...
      const lockDir = `${responsePath}.geetorus-write.lock`;
...
export function createCommandManagedSandboxCallbackBridgeQueueClient(input: {
  ...
  return {
    ...
    writeTextFile: async (remotePath, body) => {
      ...
      const tempPath = `${remotePath}.geetorus-upload.b64`;
      const decodedPath = `${remotePath}.geetorus-upload.decoded`;
      await runChecked(
...
export async function startSandboxCallbackBridgeWorker(input: {
  ...
  // Wrap each Geetorus-API callback request in a
  ...
  const processRequestFile = async (fileName: string) => {
    ...
    const finalize = async (response: SandboxCallbackBridgeResponse) => {
      ...
      for (let attempt = 1; attempt <= MAX_BACKSTOP_WRITE_ATTEMPTS; attempt += 1) {
        try {
          ...
          console.warn(
            `[geetorus] sandbox callback bridge failed to write response for ${response.id} (attempt ${attempt}/${MAX_BACKSTOP_WRITE_ATTEMPTS}): ${lastWriteError}`,
          );
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[geetorus] sandbox callback bridge handler failed for ${request.id}: ${error instanceof Error ? error.message : String(error)}`,
        );
  ...
  const writeAbortedHandlerBackstop = async (
    ...
    for (let attempt = 1; attempt <= MAX_BACKSTOP_WRITE_ATTEMPTS; attempt += 1) {
      try {
        ...
        console.warn(
          `[geetorus] sandbox callback bridge failed to write 504 backstop for ${requestId} (attempt ${attempt}/${MAX_BACKSTOP_WRITE_ATTEMPTS}): ${error instanceof Error ? error.message : String(error)}`,
        );
        if (attempt < MAX_BACKSTOP_WRITE_ATTEMPTS) {
          await new Promise((resolve) => setTimeout(resolve, BACKSTOP_WRITE_RETRY_MS));
        }
  ...
  const failPendingRequests = async (
    ...
    for (const fileName of fileNames) {
      ...
      try {
        ...
        console.warn(
          `[geetorus] sandbox callback bridge could not read pending request ${requestId}: ${error instanceof Error ? error.message : String(error)}`,
        );
        continue;
      }
      ...
      for (let attempt = 1; attempt <= MAX_BACKSTOP_WRITE_ATTEMPTS; attempt += 1) {
        try {
          ...
          lastWriteError = error instanceof Error ? error.message : String(error);
          console.warn(
            `[geetorus] sandbox callback bridge failed to write recovery 503 for ${requestId} (attempt ${attempt}/${MAX_BACKSTOP_WRITE_ATTEMPTS}): ${lastWriteError}`,
          );
          if (attempt < MAX_BACKSTOP_WRITE_ATTEMPTS) {
            await new Promise((resolve) => setTimeout(resolve, BACKSTOP_WRITE_RETRY_MS));
          }
      ...
      if (wrote503) {
        ...
        console.warn(
          `[geetorus] sandbox callback bridge kept queued request ${requestId} after every recovery 503 write failed: ${lastWriteError}`,
        );
      }
    }
  };
  ...
  const surfaceRunError = async (error: Error) => {
    ...
    console.warn(`[geetorus] ${error.message}`);
  };
  ...
  const handleWatchdogTrip = async (idleMs: number) => {
    ...
    try {
      ...
      console.warn(
        `[geetorus] sandbox callback bridge watchdog failed to abort queued requests: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  // `bridge.geetorus` step. The loop runs run-time execs for the whole run,
  ...
  const loop = runWithoutActiveStep(() => (async () => {
    ...
    try {
        ...
        for (const fileName of actionableFileNames) {
          ...
          try {
            ...
            try {
              ...
              console.warn(
                `[geetorus] sandbox callback bridge failed to abort queued requests after a request failure: ${failPendingError instanceof Error ? failPendingError.message : String(failPendingError)}`,
              );
            }
          } finally {
            inFlight -= 1;
      ...
      try {
        ...
        console.warn(
          `[geetorus] sandbox callback bridge failed to abort queued requests after worker failure: ${failPendingError instanceof Error ? failPendingError.message : String(failPendingError)}`,
        );
      }
    } finally {
...
/**
 * Content-hash-skip write of a Geetorus-authored text file into the sandbox, in
...
export async function syncRemoteTextFileWithHashSkip(input: {
  ...
  const remoteUploadPath = `${input.remotePath}.geetorus-upload.b64`;
  ...
  const syncResult = await runShell(
    ...
    [
      ...
      "    sha256sum \"$1\" | grep -oE '[0-9a-fA-F]{64}' | head -n 1",
      "    return 0",
      "  fi",
      "  if command -v shasum >/dev/null 2>&1; then",
      "    shasum -a 256 \"$1\" | grep -oE '[0-9a-fA-F]{64}' | head -n 1",
      "    return 0",
      "  fi",
      "  return 127",
      "}",
...
export async function syncSandboxCallbackBridgeEntrypoint(input: {
  ...
  const { uploaded, sha256 } = await syncRemoteTextFileWithHashSkip({
    ...
    lockDir: path.posix.join(input.assetRemoteDir, ".geetorus-bridge-upload.lock"),
...
const bridgeMode = process.env.GEETORUS_API_BRIDGE_MODE || "${SANDBOX_CALLBACK_BRIDGE_FILE_MODE}";
const queueDir = process.env.GEETORUS_BRIDGE_QUEUE_DIR;
const bridgeToken = process.env.GEETORUS_BRIDGE_TOKEN;
const host = process.env.GEETORUS_BRIDGE_HOST || "127.0.0.1";
const port = Number(process.env.GEETORUS_BRIDGE_PORT || "0");
...
const bridgeNonce = process.env.GEETORUS_BRIDGE_NONCE || "";
const pollIntervalMs = Number(process.env.GEETORUS_BRIDGE_POLL_INTERVAL_MS || "100");
const responseTimeoutMs = Number(
  process.env.GEETORUS_BRIDGE_RESPONSE_TIMEOUT_MS || "${DEFAULT_BRIDGE_RESPONSE_TIMEOUT_MS}",
);
const maxQueueDepth = Number(process.env.GEETORUS_BRIDGE_MAX_QUEUE_DEPTH || "${DEFAULT_BRIDGE_MAX_QUEUE_DEPTH}");
const maxBodyBytes = Number(process.env.GEETORUS_BRIDGE_MAX_BODY_BYTES || "${DEFAULT_BRIDGE_MAX_BODY_BYTES}");
...
if (!bridgeToken) {
  throw new Error("GEETORUS_BRIDGE_TOKEN is required.");
}
...
if (
  ...
  throw new Error("Unsupported GEETORUS_API_BRIDGE_MODE: " + bridgeMode);
}
if (bridgeMode !== "${SANDBOX_CALLBACK_BRIDGE_HTTP2_MODE}" && !queueDir) {
  throw new Error("GEETORUS_BRIDGE_QUEUE_DIR and GEETORUS_BRIDGE_TOKEN are required.");
}
...
process.on("uncaughtException", (error) => {
  process.stderr.write(
    "[geetorus-bridge] uncaught exception: " + (error && error.stack ? error.stack : String(error)) + "\\n",
  );
  if (!gatewayReady) {
    process.exit(1);
  }
...
process.on("unhandledRejection", (reason) => {
  ...
  process.stderr.write("[geetorus-bridge] unhandled rejection: " + detail + "\\n");
  if (!gatewayReady) {
    process.exit(1);
  }
});
...
async function runFileGateway() {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      const bridgeOutcome = responseHeaders["x-geetorus-bridge-outcome"];
      ...
      if (uncertainWrite) res.setHeader("x-geetorus-bridge-outcome", "indeterminate");
  ...
  server.once("error", (error) => {
    ...
    process.stderr.write(
      "[geetorus-bridge] server error: " + (error && error.stack ? error.stack : String(error)) + "\\n",
    );
    if (!gatewayReady) {
      process.exit(1);
    }
...
function runHttp2Gateway() {
  function diag(message) {
    ...
    process.stderr.write("[geetorus-bridge] " + message + "\\n");
  }
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      // standard retry policy does not repeat the request. The outcome header and
      // body stay, so a caller that reads them still sees the indeterminate result.
      const bridgeOutcome = (response.headers || {})["x-geetorus-bridge-outcome"];
      if (bridgeOutcome === "indeterminate") {
        res.statusCode = 409;
      } else {
        res.statusCode = typeof response.status === "number" ? response.status : 200;
  ...
  if (!Number.isInteger(port) || port <= 0) {
    diag("http2 gateway requires a positive assigned GEETORUS_BRIDGE_PORT; got " + String(port));
    process.exit(1);
  }
