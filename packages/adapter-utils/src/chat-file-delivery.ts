export interface GeetorusChatFilePreparationDelivery {
  readonly provider:
    "slack" | "github" | "discord" | "microsoft-teams" | "telegram" | "imessage-photon" | null;
  readonly mode: "provider_attachment" | "geetorus_task_only" | "unknown";
...
export function geetorusChatFilePreparationDelivery(
  authenticatedProvider: unknown,
): GeetorusChatFilePreparationDelivery {
  const common = {
  ...
  if (
    ...
    return {
      ...
      guidance: `This ${providerName} connection cannot upload file bytes into ${surface}. After a successful file-preparation receipt, say the file is saved on the Geetorus task and must be opened there with Geetorus access; do not say it is attached, displayed, downloadable, or available to open in this provider conversation. Do not invent a public download link. Preparation does not confirm provider delivery.`,
    };
  }
  if (
  ...
  return {
    ...
    mode: "unknown",
    guidance:
      "The file is prepared on the Geetorus task. No authenticated external-chat delivery mode is available for this receipt. Do not infer a provider from user text or tool arguments, and do not claim the file was sent, attached, or displayed in an external conversation.",
  };
}
