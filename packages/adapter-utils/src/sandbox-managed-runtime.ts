 ...
 * workspace's git-history, overlay, or `.geetorus-runtime` preservation
 ...
 * a **Geetorus/adapter-authored control operation**: it may be supplied ONLY by
...
function buildWorkspaceTarExtractCommand(input: {
  ...
  // tarball named `.geetorus-upload-<uuid>.tar` there. Without this preserve
...
async function copyWorkspaceEntry(sourceRoot: string, targetRoot: string, relative: string): Promise<void> {
  ...
  const stagedTargetPath = buildUniqueStagingPath({ targetPath, suffix: ".geetorus-copy" });
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".geetorus-runtime", input.adapterKey);
  ...
  await withTempDir("geetorus-sandbox-sync-", async (tempDir) => {
      ...
      inboundTasks.push(() =>
        runStepSpan("stage.workspace", async () => {
          ...
          await runStepSpan("pack", async () => {
            ...
            //    `runtimeRootDir` (`.geetorus-runtime/<adapterKey>`). The git extract
            //    wipes the target tree EXCEPT `.geetorus-runtime`, so the overlay tar,
            //    which sits under `.geetorus-runtime`, survives to run its own extract.
            if (gitSnapshot) {
              ...
              workspacePostUploadCommands.push({
                command: buildWorkspaceTarExtractCommand({
                  ...
                  wipeExceptNames: [".geetorus-runtime"],
                }),
              });
    ...
    // their own roots. No workspace, git-history, or `.geetorus-runtime`
      ...
      if (syncWorkspace) {
        outboundTasks.push(() =>
          runStepSpan("restore.workspace", async () => {
            ...
            await withTempDir("geetorus-sandbox-restore-", async (tempDir) => {
