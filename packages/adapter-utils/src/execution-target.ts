...
export interface AdapterSandboxExecutionTarget extends AdapterExecutionTargetWorkspaceMetadata {
  ...
  /** Host-owned lifecycle override for geetorus_runner in this environment. */
...
export interface AdapterExecutionTargetProcessOptions {
  ...
  /**
   * Sandbox-only: factory from the Geetorus bridge handle that streams the
  ...
  /**
   * Sandbox-only: the atomic run-disposition settle from the Geetorus bridge
...
export interface AdapterExecutionTargetGeetorusBridgeHandle {
  env: Record<string, string>;
  /**
...
function resolveDefaultGeetorusApiUrl(): string {
  const runtimeHost = resolveHostForUrl(
    process.env.GEETORUS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  // 3100 matches the default Geetorus dev server port when the runtime does not provide one.
  const runtimePort = process.env.GEETORUS_LISTEN_PORT ?? process.env.PORT ?? "3100";
  return `http://${runtimeHost}:${runtimePort}`;
}

function isBridgeDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.GEETORUS_BRIDGE_DEBUG?.trim().toLowerCase();
...
export function adapterExecutionTargetUsesGeetorusBridge(
  target: AdapterExecutionTarget | null | undefined,
): boolean {
  return target?.kind === "remote";
}
 ...
 * source. Callers prefix with `[geetorus] ` and append a newline.
 */
...
function applyRunDispositionSeam(
  ...
  const note = `[geetorus] The sandbox duplex control channel was lost (${lossReason}) before the run completed.\n`;
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[geetorus] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the Geetorus host.
 *
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".geetorus-runtime", key);
}
...
function githubOperationLauncherDirectory(input: GitHubLauncherLocation): string {
  ...
  return input.target?.kind === "remote"
    ? path.posix.join(input.target.remoteCwd, ".geetorus-runtime", "github", input.runId)
    : path.join(os.tmpdir(), "geetorus-github-runtime", input.runId);
}
...
const env = {};
env.GEETORUS_RUNNER_NETWORK_ROOTS = JSON.stringify(['/etc/resolv.conf','/etc/hosts','/etc/nsswitch.conf','/etc/ssl/certs','/etc/ssl/cert.pem'].flatMap(p => { try { return [fs.realpathSync(p)]; } catch { return []; } }));
if (process.argv[1] === 'host') {
  for (const [key, value] of Object.entries(process.env)) {
    if (/^(GH_TOKEN|GITHUB_TOKEN|GH_ENTERPRISE_TOKEN|GITHUB_ENTERPRISE_TOKEN|GEETORUS_GIT_TOKEN|GH_CONFIG_DIR|GIT_CONFIG_(GLOBAL|SYSTEM|NOSYSTEM|COUNT|KEY_\d+|VALUE_\d+)|GIT_(AUTHOR|COMMITTER)_(NAME|EMAIL)|GIT_ASKPASS|SSH_ASKPASS|SSH_AUTH_SOCK|GIT_SSH_COMMAND|GIT_SSH)$/.test(key)) env[key] = value;
  }
  env.GEETORUS_GITHUB_HOST_HOME = process.env.HOME || '';
...
try {
  ...
  if (fs.realpathSync(top) === fs.realpathSync(process.cwd())) {
    env.GEETORUS_GIT_METADATA_ROOTS = JSON.stringify(cp.execFileSync('git', ['rev-parse','--path-format=absolute','--git-common-dir','--git-dir'], {encoding:'utf8',stdio:['ignore','pipe','ignore']}).trim().split('\n').map(p => fs.realpathSync(p)));
  }
} catch {}
...
if [ "$1" = host ]; then
  ...
  printf 'GEETORUS_GITHUB_HOST_HOME\0%s\0' "$HOME"
...
for file in /etc/resolv.conf /etc/hosts /etc/nsswitch.conf /etc/ssl/certs /etc/ssl/cert.pem; do
  ...
  if [ -e "$file" ]; then
    ...
    printf 'GEETORUS_RUNNER_NETWORK_ROOT\0%s\0' "$parent/$(basename "$file")"
  fi
done
cwd=$(pwd -P)
...
if [ -n "$top" ] && [ "$(cd "$top" && pwd -P)" = "$cwd" ]; then
  for kind in --git-common-dir --git-dir; do
    ...
    printf 'GEETORUS_GIT_METADATA_ROOT\0%s\0' "$root"
  done
fi
printf '\0GEETORUS_GIT_CONTEXT_END\0'
`;
    const result = await adapterExecutionTargetCommandRunner(remote).execute({
      command: "sh", args: ["-c", probe, "geetorus-git-context", input.hostCredentials ? "host" : "managed"],
    ...
    const payload = result.stdout.split("\0GEETORUS_GIT_CONTEXT_V1\0")[1]?.split("\0GEETORUS_GIT_CONTEXT_END\0")[0];
    ...
    for (let index = 0; index + 1 < records.length; index += 2) {
      ...
      if (key === "GEETORUS_GIT_METADATA_ROOT") roots.push(value);
      else if (key === "GEETORUS_RUNNER_NETWORK_ROOT") networkRoots.push(value);
      else discovered[key] = value;
    }
    discovered.GEETORUS_GIT_METADATA_ROOTS = JSON.stringify([...new Set(roots)]);
    discovered.GEETORUS_RUNNER_NETWORK_ROOTS = JSON.stringify([...new Set(networkRoots)]);
  } else {
  ...
  return { ...discovered, ...input.env,
    ...(input.hostCredentials ? { GEETORUS_GITHUB_HOST_HOME: discovered.GEETORUS_GITHUB_HOST_HOME } : {}),
    GEETORUS_GIT_METADATA_ROOTS: discovered.GEETORUS_GIT_METADATA_ROOTS ?? "[]",
    GEETORUS_RUNNER_NETWORK_ROOTS: discovered.GEETORUS_RUNNER_NETWORK_ROOTS ?? "[]",
    GEETORUS_GITHUB_AUTH_MODE: input.hostCredentials ? "host" : "managed",
    GEETORUS_RUNNER_NETWORK_ACCESS: input.networkAccess ? "enabled" : "disabled",
  };
}
...
export async function prepareGitHubOperationLaunchers(input: {
  ...
  return { ...input.env, PATH: managedPath, ZDOTDIR: directory, BASH_ENV: `${directory}/.bashrc`,
    GH_CONFIG_DIR: configDirectory, GEETORUS_GITHUB_LAUNCHER_DIR: directory };
}

function buildBridgeResponseHeaders(response: Response): Record<string, string> {
  const out: Record<string, string> = {};
  // Keep `x-geetorus-bridge-outcome` in this list. The host marks a
  ...
  for (const key of ["content-type", "etag", "last-modified", "x-geetorus-bridge-outcome"]) {
...
const PROCESS_SESSION_PROXY_SCRIPT = "geetorus-process-session-proxy.mjs";
const PROCESS_SESSION_REMOTE_SCRIPT = "geetorus-process-session-remote.mjs";
...
const PROCESS_SESSION_REMOTE_STREAM_SCRIPT = "geetorus-process-session-remote-stream.mjs";
...
// Geetorus-authored `.mjs` that only changes when the build changes, so on a
...
async function syncProcessSessionRemoteScript(input: {
  ...
  const { uploaded } = await syncRemoteTextFileWithHashSkip({
    ...
    lockDir: path.posix.join(input.remoteScriptDir, ".geetorus-process-session-script.lock"),
...
export async function startAdapterExecutionTargetProcessSessionBridge(input: {
  ...
  // with other work — e.g. starting the geetorus callback bridge — and hand the
  ...
  const bridgeRuntimeDir = path.posix.join(
    input.runtimeRootDir?.trim() || path.posix.join(target.remoteCwd, ".geetorus-runtime", input.adapterKey),
    "process-sessions",
  );
  ...
  // can defer it until an upstream dependency (e.g. the geetorus bridge's env)
  ...
  if (!streamOutput) {
    await onLog("stdout", `[geetorus] Starting ACP process session bridge in sandbox (${target.providerKey ?? "provider"}).\n`);
    const startResult = await runner.execute({
      ...
      args: shellCommandArgs(
        [
          ...
          `GEETORUS_PROCESS_SESSION_DIR=${shellQuote(sessionDir)} ` +
            `GEETORUS_PROCESS_SESSION_COMMAND_B64=${shellQuote(commandPayload)} ` +
            `nohup node ${shellQuote(remoteScriptPath)} >/dev/null 2>&1 < /dev/null &`,
        ].join("\n"),
      ),
      cwd: target.remoteCwd,
      env: {
        GEETORUS_SANDBOX_EXEC_CHANNEL: "bridge",
      },
      timeoutMs,
  ...
  const proxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-proxy-"));
  ...
  const poll = async () => {
    ...
    try {
      ...
      await onLog("stderr", `[geetorus] ACP process session bridge poll failed: ${message}\n`);
  ...
  if (streamOutput) {
    ...
    await onLog(
      "stdout",
      `[geetorus] Starting streamed ACP process session bridge in sandbox (${target.providerKey ?? "provider"}).\n`,
    );
      ...
      const commandSettled = (async () => {
        try {
          const result = await runner.execute({
            ...
            cwd: target.remoteCwd,
            env: {
              GEETORUS_PROCESS_SESSION_DIR: sessionDir,
              GEETORUS_PROCESS_SESSION_COMMAND_B64: streamCommandPayload,
              GEETORUS_SANDBOX_EXEC_CHANNEL: "bridge",
            },
            timeoutMs,
            useSession: true,
  ...
  return {
    ...
    stop: async () => {
      ...
      if (!acknowledgedInTime) {
        await onLog(
          "stderr",
          `[geetorus] ACP process session wrapper did not acknowledge shutdown within ${DEFAULT_PROCESS_SESSION_SHUTDOWN_WAIT_MS}ms; removing the session directory anyway.\n`,
        ).catch(() => undefined);
      }
...
const stdinMaxParseRetries = (() => {
  const raw = Number.parseInt(process.env.GEETORUS_PROCESS_SESSION_STDIN_MAX_RETRIES || "", 10);
...
const terminateGraceMs = (() => {
  const raw = Number.parseInt(process.env.GEETORUS_PROCESS_SESSION_TERMINATE_GRACE_MS || "", 10);
...
function nextProbeFileName() {
  probeSeq += 1;
  return ".geetorus-birthtime-probe-" + process.pid + "-" + probeSeq;
}
...
const sessionDir = process.env.GEETORUS_PROCESS_SESSION_DIR;
const commandPayload = process.env.GEETORUS_PROCESS_SESSION_COMMAND_B64;
...
delete childEnv.GEETORUS_PROCESS_SESSION_DIR;
delete childEnv.GEETORUS_PROCESS_SESSION_COMMAND_B64;
...
export async function startAdapterExecutionTargetGeetorusBridge(input: {
  runId: string;
  target: AdapterExecutionTarget | null | undefined;
  runtimeRootDir: string | null | undefined;
  adapterKey: string;
  ...
  // a real recorder.
  duplexObservabilityRecorder?: DuplexObservabilityRecorder | null;
}): Promise<AdapterExecutionTargetGeetorusBridgeHandle | null> {
  if (!adapterExecutionTargetUsesGeetorusBridge(input.target)) {
    return null;
  }
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side Geetorus API token.");
  }
  ...
  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ...
      : path.posix.join(target.remoteCwd, ".geetorus-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "geetorus-bridge");
  ...
  const configuredAttachmentBytes = Number(process.env.GEETORUS_ATTACHMENT_MAX_BYTES);
  ...
  const maxBodyBytes =
    typeof input.maxBodyBytes === "number" && Number.isFinite(input.maxBodyBytes) && input.maxBodyBytes > 0
      ...
      : defaultBodyBytes;
  // The bridge worker runs inside the same process that serves the Geetorus
  // API, so forwarded sandbox calls must target the LOCAL listen origin. The
  // GEETORUS_RUNTIME_API_URL / GEETORUS_API_URL exports now prefer a
  ...
  // exports GEETORUS_LISTEN_HOST / GEETORUS_LISTEN_PORT before any run
  // executes, and resolveDefaultGeetorusApiUrl() maps wildcard listen hosts
  ...
  const hostApiUrl = input.hostApiUrl?.trim() || resolveDefaultGeetorusApiUrl();
  ...
  await onLog(
    "stdout",
    `[geetorus] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  // GEETORUS_BRIDGE_DEBUG opts into verbose stdout logs of every bridge proxy
  ...
  // One forward of a relayed sandbox request onto the existing Geetorus API
  ...
  const forwardBridgeRequest = async (
    ...
    if (emitDebugLog) {
      await onLog(
        "stdout",
        `[geetorus] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
      );
    }
    ...
    headers.set("x-geetorus-run-id", input.runId);
    ...
    if (emitDebugLog) {
      await onLog(
        "stdout",
        `[geetorus] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
      );
    }
  ...
  if (duplexRequested && capabilityGranted && typeof openDuplexChannel === "function") {
    ...
    try {
      ...
      const gatewayEnv: Record<string, string> = {
        GEETORUS_API_BRIDGE_MODE: SANDBOX_CALLBACK_BRIDGE_HTTP2_MODE,
        GEETORUS_BRIDGE_TOKEN: bridgeToken,
        GEETORUS_BRIDGE_HOST: "127.0.0.1",
        GEETORUS_BRIDGE_PORT: String(assignedPort),
        GEETORUS_BRIDGE_NONCE: nonce,
        GEETORUS_BRIDGE_MAX_BODY_BYTES: String(maxBodyBytes),
      };
      ...
      await onLog(
        "stderr",
        `[geetorus] Could not open the sandbox duplex channel (${reason}). Using the file bridge.\n`,
      );
      channel = null;
    }
    ...
    if (channel) {
      ...
      if (!readiness.ok) {
        ...
        await onLog(
          "stderr",
          `[geetorus] Sandbox duplex readiness failed (${readiness.reason}). Using the file bridge.\n`,
        );
      } else {
        ...
        if (prefaceResult === "missing") {
          ...
          await onLog(
            "stderr",
            "[geetorus] Sandbox HTTP/2 client preface did not appear inside the bounded readiness buffer (preface_missing). Using the file bridge.\n",
          );
        } else {
          ...
          const recordHttp2Loss = (event: Http2TelemetryEventName): void => {
            ...
            void onLog("stderr", `[geetorus] Sandbox HTTP/2 channel lost (${reason}). The run fails.\n`);
          };
          ...
          const http2Server = createHttp2BridgeServer({
            ...
            // sandbox-side gateway (`GEETORUS_BRIDGE_MAX_BODY_BYTES`,
          ...
          await onLog(
            "stdout",
            "[geetorus] Sandbox HTTP/2 transport ready; serving the host-assigned origin.\n",
          );
          ...
          if (target.transport === "sandbox" && target.streamRunLogs !== false) {
            ...
            await onLog("stdout", "[geetorus] Sandbox run log streaming enabled for this run.\n");
          }
          return {
            env: {
              GEETORUS_API_URL: sandboxOrigin,
              GEETORUS_API_KEY: bridgeToken,
              GEETORUS_API_BRIDGE_MODE: SANDBOX_CALLBACK_BRIDGE_HTTP2_MODE,
            },
  ...
  try {
    ...
    // setup on the active `bridge.geetorus` step, and runs each request under
  ...
  return {
    env: {
      GEETORUS_API_URL: server.baseUrl,
      GEETORUS_API_KEY: bridgeToken,
      GEETORUS_API_BRIDGE_MODE: "queue_v1",
      GEETORUS_BRIDGE_QUEUE_DIR: queueDir,
    },
    runLogTail,
    stop: async () => {
      await Promise.allSettled([
