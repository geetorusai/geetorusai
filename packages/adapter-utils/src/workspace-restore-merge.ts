...
async function isLockStale(lockDir: string): Promise<boolean> {
  try {
    ...
    // that window. Mirror the materializeGeetorusSkillCopy lock pattern: fall
...
async function acquireDirectoryMergeLock(lockDir: string): Promise<() => Promise<void>> {
  ...
  while (true) {
    try {
      ...
      // the materializeGeetorusSkillCopy lock pattern — remove and retry.
 ...
 * target directory must resolve to the same `GEETORUS_HOME` and
 * `GEETORUS_INSTANCE_ID`. That shared resolution is what keeps mutual
 ...
 * lock acquisition.
 *
 * The root reads `GEETORUS_HOME` and `GEETORUS_INSTANCE_ID` from `env`, so an
