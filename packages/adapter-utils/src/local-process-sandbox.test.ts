...
describe("local process sandbox", () => {
  ...
  it.runIf(process.platform === "linux")("describes every valid allowlist input when no proxy rules remain", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-network-rules-"));
  ...
  it.runIf(process.platform === "linux")("builds a fresh-root bubblewrap command with workspace access", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-fs-sandbox-"));
    cleanup.push(root);
  ...
  it.runIf(process.platform === "linux")("binds a confined absolute alias to the synchronized workspace", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-fs-alias-"));
    cleanup.push(root);
    const workspace = path.join(root, "workspace");
    await fs.mkdir(workspace);
  ...
  it.runIf(process.platform === "linux")("rejects writable out-of-tree paths without an outbound restore mapping", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-fs-outbound-"));
    cleanup.push(root);
    const workspace = path.join(root, "workspace");
  ...
  it.runIf(process.platform === "linux")("builds a network-only namespace without changing filesystem visibility", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-network-sandbox-"));
    cleanup.push(workspace);
    const target = await buildLocalProcessSandboxSpawnTarget({
      executable: process.execPath,
      args: ["-e", "console.log('ok')"],
  ...
  it.runIf(process.platform === "linux")("forwards allowed proxy targets with a deep TMPDIR and rejects other hosts", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-network-proxy-"));
    cleanup.push(workspace);
    ...
    expect(Buffer.byteLength(path.join(deepTmpDir, "geetorus-network-sandbox-XXXXXX", "proxy.sock"))).toBeGreaterThan(107);
    expect(Buffer.byteLength(socketPath)).toBeLessThanOrEqual(107);
    expect(socketPath).toMatch(/^\/tmp\/geetorus-network-sandbox-/);
    ...
    try {
      ...
      await expect(request("http://example.com/")).resolves.toEqual({
        ...
        body: '{"error":{"code":"network_target_denied","message":"Network target denied by Geetorus sandbox policy."}}\n',
      });
      ...
      expect(connectResponse).toContain(
        '{"error":{"code":"network_target_denied","message":"Network target denied by Geetorus sandbox policy."}}\n',
      );
    } finally {
  ...
  it.runIf(process.platform === "linux")("always permits trusted Geetorus control-plane URLs", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-network-trusted-"));
    cleanup.push(workspace);
  ...
  it("fails clearly when Bubblewrap is unavailable", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-fs-sandbox-missing-"));
    cleanup.push(workspace);
    await expect(
  ...
  it.runIf(Boolean(process.env.GEETORUS_TEST_BWRAP))(
    "prevents reads outside the workspace while allowing workspace writes",
    async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-fs-sandbox-integration-"));
      cleanup.push(root);
      const workspace = path.join(root, "workspace");
      ...
      const result = await runChildProcess("filesystem-sandbox-test", process.execPath, ["-e", script], {
        ...
        localProcessSandbox: {
          ...
          command: process.env.GEETORUS_TEST_BWRAP,
        },
      });
  ...
  it.runIf(Boolean(process.env.GEETORUS_TEST_BWRAP && process.env.GEETORUS_TEST_SANDBOX_BUILD))(
  ...
  it.runIf(Boolean(process.env.GEETORUS_TEST_BWRAP))(
    ...
    async () => {
      const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-network-deny-"));
      cleanup.push(workspace);
  ...
  it.runIf(Boolean(process.env.GEETORUS_TEST_BWRAP))(
    ...
    async () => {
      const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-network-allowlist-"));
      cleanup.push(workspace);
      const server = http.createServer((_request, response) => response.end("allowed-response"));
      await new Promise<void>((resolve) => server.listen(0, "127.0.0.1", resolve));
      const address = server.address();
