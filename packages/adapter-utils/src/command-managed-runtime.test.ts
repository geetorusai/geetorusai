...
describe("command managed runtime", () => {
  ...
  it("reports a missing sandbox file as ENOENT without masking command failures", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "geetorus-remote-missing-"));
    try {
  ...
  it("keeps the runtime overlay out of sandbox workspace sync by default", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-runtime-"));
    ...
    await writeFile(path.join(localWorkspaceDir, ".geetorus-runtime", "state.json"), "{\"keep\":true}\n", "utf8");
    ...
    await expect(readFile(path.join(remoteWorkspaceDir, ".geetorus-runtime", "state.json"), "utf8")).rejects
    ...
    await writeFile(path.join(remoteWorkspaceDir, ".geetorus-runtime", "remote-state.json"), "{\"remote\":true}\n", "utf8");
    await prepared.restoreWorkspace();

    await expect(readFile(path.join(localWorkspaceDir, "README.md"), "utf8")).resolves.toBe("remote workspace\n");
    await expect(readFile(path.join(localWorkspaceDir, ".geetorus-runtime", "state.json"), "utf8")).resolves
      .toBe("{\"keep\":true}\n");
    await expect(readFile(path.join(localWorkspaceDir, ".geetorus-runtime", "remote-state.json"), "utf8")).rejects
      .toMatchObject({ code: "ENOENT" });
  ...
  it("stages runtime assets without replacing or restoring an in-place workspace", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-runtime-assets-only-"));
    cleanupDirs.push(rootDir);
    ...
    expect(prepared.assetDirs.home).toBe(path.join(remoteWorkspaceDir, ".geetorus-runtime", "codex", "home"));
  ...
  it("stages each additional project into an isolated dir on the base64/tar transport, one failure skipped", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-runtime-additional-"));
    cleanupDirs.push(rootDir);
    ...
    const runtimeRootDir = path.posix.join(remoteWorkspaceDir, ".geetorus-runtime", "claude");
  ...
  it("keeps adapter detection on the profile-backed shell path", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-runtime-detect-"));
    cleanupDirs.push(rootDir);
  ...
  it("runs setup commands from a stable root cwd when staging into a nested remote workspace dir", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-runtime-nested-"));
    cleanupDirs.push(rootDir);

    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteBaseDir = path.join(rootDir, "remote-base");
    const remoteWorkspaceDir = path.join(remoteBaseDir, ".geetorus-runtime", "runs", "test", "workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("uploads a multi-MB payload in a single process and preserves exact bytes", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-write-"));
    cleanupDirs.push(rootDir);
  ...
  it("stages a single-file write to <path>.geetorus-upload then atomically renames it (single-stream path)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-atomic-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
    ...
    // stage-then-atomic-rename shape (temp .geetorus-upload + `mv -f`).
    const script = (calls[0].args ?? []).join(" ");
    expect(script).toContain(`${remotePath}.geetorus-upload`);
    expect(script).toContain(`trap cleanup EXIT`);
    expect(script).toContain(`mv -f`);
    expect(script.indexOf(".geetorus-upload")).toBeLessThan(script.indexOf("mv -f"));
    expect(await readFile(remotePath, "utf8")).toBe("hello atomic\n");
  });

  it("cleans up a staged upload when rename fails", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-upload-cleanup-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
    ...
    runner.execute = async (input) => {
      ...
      if (script.includes("mv -f") && script.includes(".geetorus-upload.")) {
    ...
    const uploadCall = calls.find((call) => (call.args ?? []).join(" ").includes(".geetorus-upload."));
    expect(uploadCall).toBeDefined();
    const stagedPath = (uploadCall?.args ?? []).join(" ").match(/([/A-Za-z0-9_.-]+\.geetorus-upload\.[A-Za-z0-9-]+)/)?.[1];
  ...
  it("stages a single-file write to a temp then renames it on the chunked fallback path too", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-atomic-fallback-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
    ...
    expect(scripts.some((script) => script.includes(`${remotePath}.geetorus-upload`))).toBe(true);
  ...
  it("fallback syncIn tarballs+uploads a directory then runs post-upload commands in order", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-syncin-fallback-"));
    cleanupDirs.push(rootDir);
    ...
    const scripts = calls.map((call) => (call.args ?? []).join("\n"));
    const uploadIdx = scripts.findIndex((s) => s.includes(".geetorus-syncin.tar") && s.includes("base64 -d"));
  ...
  it("fallback syncIn writes a mode-constrained file directly to its target and then applies the mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-syncin-mode-"));
    cleanupDirs.push(rootDir);
    ...
    const scripts = calls.map((call) => (call.args ?? []).join(" "));
    expect(scripts.some((script) => script.includes(".geetorus-syncin."))).toBe(false);
    expect(scripts.some((script) => script.includes("mv -f") && script.includes(".geetorus-syncin."))).toBe(
      false,
    );
  ...
  it("test_single_stream_writeFile_collapses_roundtrips_under_96MiB", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-single-stream-collapse-"));
    cleanupDirs.push(rootDir);
  ...
  it("falls back to chunked upload progress when the runner cannot report mid-stream stdin progress", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-write-fallback-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
  ...
  it("falls back to bounded chunks when the runner does not explicitly opt in", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-write-fallback-no-progress-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
  ...
  it("downloads in bounded stdout chunks and reports monotonic byte progress to the total", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-command-read-"));
    cleanupDirs.push(rootDir);
