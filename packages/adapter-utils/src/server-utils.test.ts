...
import {
  ...
  isGeetorusExternalChatContractTurn,
  isGeetorusExternalChatQuestionResponseTurn,
  ...
  resolveGeetorusDesiredSkillNames,
...
describe("runtime connection tool delivery", () => {
  const access = {
    ...
    mcpEndpoint: "https://geetorus.test/mcp/runtime-tools",
    rest: {
      connectionsSearch:
        "https://geetorus.test/runtime-tools/connections/search",
      connectionRequest:
        "https://geetorus.test/runtime-tools/connections/request",
    },
  ...
  it("delivers the complete environment contract and canonical guidance", () => {
    expect(buildRuntimeToolsEnv(access)).toEqual({
      ...
      GEETORUS_RUNTIME_TOOLS_AVAILABLE:
        "connections_search,connection_request",
      GEETORUS_RUNTIME_TOOLS_GUIDANCE: CONNECTION_INTENT_AGENT_GUIDANCE,
    });
  });

  it("does not leak descriptor identity through guidance", () => {
    const env = buildRuntimeToolsEnv(access);
    expect(env.GEETORUS_RUNTIME_TOOLS_GUIDANCE).not.toContain(
      access.bearerToken,
    );
    expect(env.GEETORUS_RUNTIME_TOOLS_GUIDANCE).not.toContain(
      access.mcpEndpoint,
  ...
  it("uses the exact same guidance in the default heartbeat prompt", () => {
    expect(DEFAULT_GEETORUS_AGENT_PROMPT_TEMPLATE).toContain(
      CONNECTION_INTENT_AGENT_GUIDANCE,
    );
    expect(DEFAULT_GEETORUS_CONVERSATION_PROMPT_TEMPLATE).toContain(CONNECTION_INTENT_AGENT_GUIDANCE);
    expect(DEFAULT_GEETORUS_CONVERSATION_PROMPT_TEMPLATE).not.toContain("Execution contract:");
    expect(DEFAULT_GEETORUS_CONVERSATION_PROMPT_TEMPLATE).not.toContain("child issues");
  });
});
...
describe("legacy adapter skill selection", () => {
  ...
  it("keeps the operational skill selected after an explicit empty replacement", () => {
    expect(
      resolveLegacyGeetorusDesiredSkillNames(
        { geetorusSkillSync: { desiredSkills: [] } },
  ...
  it("does not force optional skills or synthesize a missing operational entry", () => {
    const config = {
      geetorusSkillSync: { desiredSkills: [optionalEntry.key] },
    };
    expect(
      resolveLegacyGeetorusDesiredSkillNames(config, [
        operationalEntry,
        optionalEntry,
      ]),
    ).toEqual([GEETORUS_OPERATIONAL_SKILL_KEY, optionalEntry.key]);
    expect(
      resolveLegacyGeetorusDesiredSkillNames(config, [optionalEntry]),
    ).toEqual([optionalEntry.key]);
  });

  it("leaves the configurable resolver available for native runners", () => {
    expect(resolveGeetorusDesiredSkillNames({}, [operationalEntry])).toEqual(
      [],
    );
  });
});
...
describe("buildInvocationEnvForLogs", () => {
  it("redacts inline secrets from resolved command metadata", () => {
    const loggedEnv = buildInvocationEnvForLogs(
      ...
      {
        resolvedCommand:
          "env OPENAI_API_KEY=sk-live-example GEETORUS_API_KEY='geetorus-quoted-secret' custom-acp --geetorus-api-key=geetorus-flag-secret --token ghp_example_secret",
      },
    );

    expect(loggedEnv.SAFE_VALUE).toBe("visible");
    expect(loggedEnv.GEETORUS_RESOLVED_COMMAND).toBe(
      "env OPENAI_API_KEY=***REDACTED*** GEETORUS_API_KEY='***REDACTED***' custom-acp --geetorus-api-key=***REDACTED*** --token ***REDACTED***",
    );
  });
});
...
describe("materializeGeetorusSkillCopy", () => {
  it("refuses to materialize into an ancestor of the source", async () => {
    const root = await fs.mkdtemp(
      path.join(os.tmpdir(), "geetorus-skill-copy-"),
    );
    try {
      ...
      await expect(
        materializeGeetorusSkillCopy(source, path.join(root, "parent")),
  ...
  it("does not delete and recopy an unchanged materialized skill target", async () => {
    ...
    try {
      ...
      const first = await materializeGeetorusSkillCopy(source, target);
      ...
      const second = await materializeGeetorusSkillCopy(source, target);
  ...
  it("breaks stale materialization locks left by dead processes", async () => {
    ...
    try {
      ...
      await expect(
        materializeGeetorusSkillCopy(source, target),
...
describe("adapter skill snapshots", () => {
  const requiredEntry = {
    key: "geetorusai/geetorus/geetorus",
    runtimeName: "geetorus",
    source: "/runtime/geetorus",
  };
  const optionalEntry = {
      ...
      availableEntries: [
        {
          ...
          source:
            "/geetorus/skills/example/__runtime__/reflection-coach--abc123",
  ...
  it("can surface read-only external skills for runtime-mounted adapters", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      externalDetail:
        "Installed outside Geetorus management in the Claude skills home.",
    });
  ...
  it("reports persistent adapter installed, stale, external, and missing states", () => {
    const snapshot = buildPersistentSkillSnapshot({
      ...
      desiredSkills: [requiredEntry.key, "missing-skill"],
      installed: new Map([
        ["geetorus", { targetPath: "/runtime/geetorus", kind: "symlink" }],
        [
          "ascii-heart",
    ...
    expect(snapshot.entries).toContainEqual(
      expect.objectContaining({
        ...
        state: "external",
        managed: false,
        detail: "Installed outside Geetorus management.",
      }),
    );
    expect(snapshot.entries).toContainEqual(
      expect.objectContaining({
  ...
  it("reports stale managed persistent skills when Geetorus owns an undesired available skill", () => {
    const snapshot = buildPersistentSkillSnapshot({
      adapterType: "cursor",
...
describe("renderGeetorusWakePrompt", () => {
  it("leaves conversation disposition and accepted-plan handoff to the injected chat policy", () => {
    ...
    const ordinary = renderGeetorusWakePrompt(payload, { resumedSession: true });
    ...
    for (const resumedSession of [false, true]) {
      const chat = renderGeetorusWakePrompt(payload, {
  ...
  it("recognizes only normalized, harness-checked-out ordinary external-chat wakes", () => {
    expect(isGeetorusExternalChatTurn(ordinaryExternalChatWake)).toBe(true);
    const normalized = JSON.parse(
      stringifyGeetorusWakePayload(ordinaryExternalChatWake) ?? "{}",
    );
    ...
    expect(isGeetorusExternalChatTurn(normalized)).toBe(true);
    ...
    expect(isGeetorusExternalChatTurn(incomplete)).toBe(false);
    ...
    for (const payload of excluded) {
      expect(isGeetorusExternalChatTurn(payload)).toBe(false);
    }
  ...
  it("describes authenticated reviewed chat as an execution binding, not a checkout or approval", () => {
    ...
    expect(isGeetorusExternalChatContractTurn(reviewed)).toBe(true);
    const normalized = JSON.parse(
      stringifyGeetorusWakePayload(reviewed) ?? "{}",
    );
    expect(normalized).toMatchObject({
      checkedOutByHarness: false,
      externalChatExecutionBound: true,
    });
    const prompt = renderGeetorusWakePrompt(reviewed);
    ...
    expect(
      isGeetorusExternalChatContractTurn({ ...reviewed, issue: null }),
    ).toBe(false);
  });

  it("renders one authoritative direct-response contract for fresh and resumed external-chat turns", () => {
    for (const prompt of [
      renderGeetorusWakePrompt(ordinaryExternalChatWake),
      renderGeetorusWakePrompt(ordinaryExternalChatWake, {
        resumedSession: true,
      }),
    ]) {
      ...
      expect(prompt).toContain("server-authenticated github chat turn");
      expect(prompt).toContain("Make zero Geetorus API calls");
      ...
      expect(prompt).not.toContain(
        "POST /api/issues/$GEETORUS_TASK_ID/checkout",
      );
    }

    const freshPrompt = renderGeetorusWakePrompt(ordinaryExternalChatWake);
    ...
    const incompleteResumePrompt = renderGeetorusWakePrompt(
      {
        ...ordinaryExternalChatWake,
        continuationSummary: {
          ...ordinaryExternalChatWake.continuationSummary,
  ...
  it("uses only the run-scoped reader when an authenticated chat wake does not fit inline", () => {
    ...
    expect(isGeetorusExternalChatTurn(readerWake)).toBe(false);
    expect(isGeetorusExternalChatContractTurn(readerWake)).toBe(true);

    const genericPrompt = renderGeetorusWakePrompt(readerWake);
    ...
    for (const prompt of [
      renderGeetorusWakePrompt(readerWake, {
      ...
      expect(prompt).toContain("Make zero other Geetorus API calls");
  ...
  it("preserves and renders the issue description in structured wake payloads", () => {
    ...
    expect(
      JSON.parse(stringifyGeetorusWakePayload(payload) ?? "{}"),
    ).toMatchObject({
      issue: {
        description:
          "Update launch-card.svg and change the CTA to Try Team free.",
    ...
    expect(renderGeetorusWakePrompt(payload)).toContain(
      "Issue description:\n" +
  ...
  it("renders the simplified-english interaction directive only when the payload enables it", () => {
    ...
    expect(renderGeetorusWakePrompt(payload)).not.toContain("ASD-STE100");

    const enabled = { ...payload, simplifiedEnglishInteractions: true };
    const fresh = renderGeetorusWakePrompt(enabled);
    ...
    expect(
      renderGeetorusWakePrompt(enabled, { resumedSession: true }),
    ).toContain("ASD-STE100 Simplified Technical English");
    expect(
      JSON.parse(stringifyGeetorusWakePayload(enabled) ?? "{}"),
    ).toMatchObject({
  ...
  it("suppresses the issue description when the prompt already carries the task-context markdown", () => {
    ...
    const prompt = renderGeetorusWakePrompt(payload, {
    ...
    const promptJson = stringifyGeetorusWakePayload(payload, {
  ...
  it("omits the issue description from non-assignment resume deltas and leaves a fetch breadcrumb", () => {
    ...
    const commentResume = renderGeetorusWakePrompt(
    ...
    const assignedResume = renderGeetorusWakePrompt(
    ...
    const freshComment = renderGeetorusWakePrompt({
      ...basePayload,
      reason: "issue_commented",
    });
  ...
  it("omits whitespace-only issue descriptions from structured wake prompts", () => {
    ...
    expect(renderGeetorusWakePrompt(payload)).not.toContain(
      "Issue description:",
    );
  });
  ...
  it("keeps the default local-agent prompt action-oriented", () => {
    ...
    expect(DEFAULT_GEETORUS_AGENT_PROMPT_TEMPLATE).toContain(
      "POST /api/issues/$GEETORUS_TASK_ID/interactions",
    );
    ...
    expect(DEFAULT_GEETORUS_AGENT_PROMPT_TEMPLATE).not.toContain(
      "/api/issues/{issueId}",
    );
    expect(DEFAULT_GEETORUS_AGENT_PROMPT_TEMPLATE).not.toContain(
      "/api/issues/{id}",
  ...
  it("leaves the execution contract to the heartbeat template on fresh scoped wake prompts", () => {
    const prompt = renderGeetorusWakePrompt({
      reason: "issue_assigned",
      issue: {
        id: "issue-1",
        identifier: "PAP-1580",
    ...
    expect(prompt).toContain("## Geetorus Wake Payload");
  ...
  it("adds the execution contract to resume delta prompts and opted-in fresh prompts", () => {
    ...
    for (const prompt of [
      renderGeetorusWakePrompt(payload, { resumedSession: true }),
      renderGeetorusWakePrompt(payload, { includeExecutionContract: true }),
    ]) {
      expect(prompt).toContain(
      ...
      expect(prompt).toContain(
        "Immediately before returning, verify that Geetorus records one of those dispositions",
      );
      expect(prompt).toContain(
        "a successful process exit or final response is not sufficient",
      );
  ...
  it.each(["answered", "accepted"])(
    ...
    (interactionStatus) => {
      ...
      expect(prompt).toContain(
        "Use internal Geetorus tools to satisfy the task lifecycle, including marking the task done when its requested work is complete.",
      );
      expect(prompt).toContain(
        "Exact-output constraints apply to provider-visible prose, not necessary internal tool calls",
      );
      expect(prompt).toContain(
        "Do not narrate answer receipt, interaction IDs, Geetorus workflow, delegation, task status, or closure",
      );
      expect(prompt).toContain(
  ...
  it.each([false, true])(
    ...
    (resumedSession) => {
      for (const checkedOutByHarness of [false, true]) {
        ...
        const prompt = renderGeetorusWakePrompt(wake, { resumedSession });
        expect(isGeetorusExternalChatTurn(wake)).toBe(false);
        expect(isGeetorusExternalChatQuestionResponseTurn(wake)).toBe(true);
  ...
  it("keeps newer comments outside the resolved-question background", () => {
    ...
    const prompt = renderGeetorusWakePrompt(wake);
    expect(isGeetorusExternalChatQuestionResponseTurn(wake)).toBe(true);
    expect(prompt).toContain(
      "Original request for context (only the answered questions listed above are resolved):",
    );
  ...
  it("does not grant the native chat answer prompt to malformed or unbound answer metadata", () => {
    ...
    for (const override of invalid) {
      ...
      const prompt = renderGeetorusWakePrompt(wake);
      expect(isGeetorusExternalChatQuestionResponseTurn(wake)).toBe(false);
      expect(prompt).not.toContain(
  ...
  it("keeps exactly one execution contract in a composed fresh heartbeat prompt", () => {
    const wakePrompt = renderGeetorusWakePrompt({
      reason: "issue_assigned",
      issue: {
        id: "issue-1",
        identifier: "PAP-1580",
    ...
    const composed = [wakePrompt, DEFAULT_GEETORUS_AGENT_PROMPT_TEMPLATE].join(
      "\n\n",
    );
  ...
  it("trims comment-batch boilerplate on fresh wakes with zero pending comments", () => {
    ...
    const zeroCommentPrompt = renderGeetorusWakePrompt(base);
    ...
    const commentPrompt = renderGeetorusWakePrompt({
      ...base,
      reason: "issue_commented",
      commentWindow: { requestedCount: 1, includedCount: 1, missingCount: 0 },
    ...
    const fallbackPrompt = renderGeetorusWakePrompt({
      ...base,
      fallbackFetchNeeded: true,
    });
  ...
  it("renders the execution workspace branch guard only on non-resumed sessions", () => {
    ...
    const firstPrompt = renderGeetorusWakePrompt(payload);
    ...
    const resumedPrompt = renderGeetorusWakePrompt(payload, {
      resumedSession: true,
    });
    expect(resumedPrompt).toContain("## Geetorus Resume Delta");
  ...
  it("renders a plugin session message as the user turn without granting it system authority", () => {
    ...
    expect(prompt).toContain("not a Geetorus system or board instruction");
  ...
  it("preserves Chinese, Japanese, and Hindi issue and comment text in scoped wake prompts", () => {
    ...
    const serialized = stringifyGeetorusWakePayload(payload);
  ...
  it("renders planning-mode directives for assignment and comment wakes", () => {
    const assignmentPrompt = renderGeetorusWakePrompt({
      reason: "issue_assigned",
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
...
describe("selectGeetorusTaskMarkdown", () => {
  const fullMarkdown =
    'Geetorus task context:\n- Issue: "PAP-1"\n\nIssue description:\n```text\nThe brief.\n```';
  const compactMarkdown = 'Geetorus task context:\n- Issue: "PAP-1"';
  ...
  it("returns the full markdown for fresh sessions and assignment-shaped resumes", () => {
    const context = {
      geetorusTaskMarkdown: fullMarkdown,
      geetorusTaskMarkdownCompact: compactMarkdown,
      geetorusWake: wake("issue_commented"),
    };
    expect(selectGeetorusTaskMarkdown(context)).toBe(fullMarkdown);
    expect(
      selectGeetorusTaskMarkdown(
        { ...context, geetorusWake: wake("issue_assigned") },
        { resumedSession: true },
      ),
    ).toBe(fullMarkdown);
  });
...
describe("renderGeetorusWakePrompt - task watchdog", () => {
  ...
  it("survives a JSON round-trip through stringifyGeetorusWakePayload", () => {
    const payload = {
      ...baseWatchdogPayload,
      taskWatchdog: {
        watchedIssueId: "watched-issue-1",
    ...
    const prompt = renderGeetorusWakePrompt(parsed);
    expect(prompt).toContain("## Task Watchdog Mandate");
  ...
  it("truncates oversized custom instructions and caps terminal leaf summaries", () => {
    ...
    const serialized = stringifyGeetorusWakePayload({
      ...baseWatchdogPayload,
      taskWatchdog: {
        watchedIssueId: "watched-issue-1",
        watchedIssueIdentifier: "PAP-8000",
...
describe("applyGeetorusWorkspaceEnv", () => {
  it("adds shared workspace env vars including AGENT_HOME", () => {
    const env = applyGeetorusWorkspaceEnv(
      {},
      {
        ...
        workspaceRepoUrl: "https://github.com/geetorusai/geetorus.git",
    ...
    expect(env).toEqual({
      GEETORUS_WORKSPACE_CWD: "/tmp/workspace",
      GEETORUS_WORKSPACE_SOURCE: "project_primary",
      GEETORUS_WORKSPACE_STRATEGY: "git_worktree",
      GEETORUS_WORKSPACE_ID: "workspace-1",
      GEETORUS_WORKSPACE_REPO_URL:
        "https://github.com/geetorusai/geetorus.git",
      GEETORUS_WORKSPACE_REPO_REF: "main",
      GEETORUS_WORKSPACE_BRANCH: "feature/test",
      GEETORUS_WORKSPACE_WORKTREE_PATH: "/tmp/worktree",
...
describe("shapeGeetorusWorkspaceEnvForExecution", () => {
  it("maps editable project repositories inside the remote workspace", () => {
    const result = shapeGeetorusWorkspaceEnvForExecution({
      workspaceCwd: "/host/task", executionCwd: "/sandbox/task", executionTargetIsRemote: true,
      workspaceHints: [{ workspaceId: "backend", cwd: "/host/task/.geetorus-repositories/backend" }],
    });
    expect(result.workspaceHints).toEqual([{ workspaceId: "backend", cwd: "/sandbox/task/.geetorus-repositories/backend" }]);
  });
  it("rewrites workspace env paths for remote execution", () => {
    const shaped = shapeGeetorusWorkspaceEnvForExecution({
      workspaceCwd: "/tmp/workspace",
      workspaceWorktreePath: "/tmp/worktree",
      workspaceHints: [
        {
          workspaceId: "workspace-1",
          cwd: "/tmp/workspace",
          repoUrl: "https://github.com/geetorusai/geetorus.git",
        },
        {
...
describe("refreshGeetorusWorkspaceEnvForExecution", () => {
  it("rewrites Geetorus workspace env to the prepared remote runtime cwd", () => {
    const env: Record<string, string> = {
      GEETORUS_WORKSPACE_CWD: "/remote/workspace",
      GEETORUS_WORKSPACE_WORKTREE_PATH: "/host/worktree",
      GEETORUS_WORKSPACES_JSON: JSON.stringify([
        { workspaceId: "workspace-1", cwd: "/remote/workspace" },
    ...
    const shaped = refreshGeetorusWorkspaceEnvForExecution({
      env,
      envConfig: {
        QA_PROJECT_WORKSPACE_CWD: "/host/workspace",
      },
      ...
      executionTargetIsRemote: true,
      executionCwd: "/remote/workspace/.geetorus-runtime/runs/run-1/workspace",
    });

    expect(shaped).toEqual({
      workspaceCwd: "/remote/workspace/.geetorus-runtime/runs/run-1/workspace",
      workspaceWorktreePath: null,
      workspaceHints: [
        {
          workspaceId: "workspace-1",
          cwd: "/remote/workspace/.geetorus-runtime/runs/run-1/workspace",
        },
        {
          workspaceId: "workspace-2",
        },
    ...
    expect(env.GEETORUS_WORKSPACE_CWD).toBe(
      "/remote/workspace/.geetorus-runtime/runs/run-1/workspace",
    );
    expect(env.GEETORUS_WORKSPACE_WORKTREE_PATH).toBeUndefined();
    expect(env.QA_PROJECT_WORKSPACE_CWD).toBe(
      "/remote/workspace/.geetorus-runtime/runs/run-1/workspace",
    );
    expect(JSON.parse(env.GEETORUS_WORKSPACES_JSON ?? "[]")).toEqual([
      {
        workspaceId: "workspace-1",
        cwd: "/remote/workspace/.geetorus-runtime/runs/run-1/workspace",
      },
  ...
  it("forwards resolved adapter env but never overrides Geetorus runtime env", () => {
    const env: Record<string, string> = {
      GEETORUS_RUN_ID: "run-1",
      GEETORUS_TASK_ID: "issue-1",
      GEETORUS_API_URL: "http://runtime:3100",
    };

    refreshGeetorusWorkspaceEnvForExecution({
      env,
      envConfig: {
        // Plain non-GEETORUS key.
        ...
        GEETORUS_TASK_ID: "attacker-issue",
        GEETORUS_API_URL: "http://evil:9999",
      },
      workspaceCwd: null,
    });
    ...
    expect(env.GEETORUS_TASK_ID).toBe("issue-1");
    expect(env.GEETORUS_API_URL).toBe("http://runtime:3100");
  });

  it("applies a configured GEETORUS_* key only when Geetorus has not set it", () => {
    const env: Record<string, string> = {};

    refreshGeetorusWorkspaceEnvForExecution({
      env,
      envConfig: {
        GEETORUS_CLOUD_PROVIDER_TOKEN: "cloud-token",
      },
      workspaceCwd: null,
    });

    // Geetorus did not assign this GEETORUS_*-named key for the run, so the
    // configured value flows through to the spawned process.
    expect(env.GEETORUS_CLOUD_PROVIDER_TOKEN).toBe("cloud-token");
  });

  it("never accepts GEETORUS_API_KEY from config env", () => {
    const env: Record<string, string> = {};

    refreshGeetorusWorkspaceEnvForExecution({
      env,
      envConfig: {
        GEETORUS_API_KEY: "explicit-key",
      },
      workspaceCwd: null,
    });

    // The harness-minted run token is the only GEETORUS_API_KEY source;
    // a configured value is dropped even when Geetorus has not set one.
    expect(env.GEETORUS_API_KEY).toBeUndefined();
  });
});
...
describe("buildGeetorusEnv", () => {
  ...
  it("prefers an explicit GEETORUS_API_URL override over the derived runtime URL", () => {
    withEnv(
      {
        GEETORUS_API_URL: "http://localhost:3100",
        GEETORUS_RUNTIME_API_URL: "http://203.0.113.7:3100",
      },
      () => {
        const env = buildGeetorusEnv({
          id: "agent-1",
          companyId: "company-1",
        });
        expect(env.GEETORUS_API_URL).toBe("http://localhost:3100");
        expect(env.GEETORUS_AGENT_ID).toBe("agent-1");
        expect(env.GEETORUS_COMPANY_ID).toBe("company-1");
      },
    );
  });

  it("falls back to the derived runtime URL when no explicit override is set", () => {
    withEnv({ GEETORUS_RUNTIME_API_URL: "http://203.0.113.7:3100" }, () => {
      const env = buildGeetorusEnv({ id: "agent-1", companyId: "company-1" });
      expect(env.GEETORUS_API_URL).toBe("http://203.0.113.7:3100");
    });
  });

  it("derives a listen-host URL when neither override is set", () => {
    withEnv(
      { GEETORUS_LISTEN_HOST: "0.0.0.0", GEETORUS_LISTEN_PORT: "3200" },
      () => {
        const env = buildGeetorusEnv({
          id: "agent-1",
          companyId: "company-1",
        });
        expect(env.GEETORUS_API_URL).toBe("http://localhost:3200");
      },
    );
  });
});
...
describe("runtime skill assignment boundaries", () => {
  it("preserves an explicitly empty assignment instead of discovering bundled connector skills", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skills-empty-"));
    try {
      await fs.mkdir(path.join(root, "agentmail"));
      await fs.writeFile(path.join(root, "agentmail", "SKILL.md"), "---\nname: agentmail\ndescription: Email connector\n---\n");
      const discovered = await readGeetorusRuntimeSkillEntries({}, root, [root]);
      expect(discovered.some((entry) => entry.runtimeName === "agentmail")).toBe(true);
      expect(await readGeetorusRuntimeSkillEntries({ geetorusRuntimeSkills: [] }, root, [root])).toEqual([]);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
