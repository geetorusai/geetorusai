...
import type { GeetorusSkillEntry } from "./server-utils.js";
import { buildSkillLibraryManifestMarkdown } from "./skill-library-manifest.js";

function entry(overrides: Partial<GeetorusSkillEntry> & Pick<GeetorusSkillEntry, "key">): GeetorusSkillEntry {
  return {
...
describe("buildSkillLibraryManifestMarkdown", () => {
  ...
  it("renders enabled, not-enabled, and broken states deterministically and key-sorted", () => {
    const entries = [
      ...
      entry({ key: "geetorusai/geetorus/geetorus" }),
      entry({
    ...
    const desiredSkillKeys = new Set(["geetorusai/geetorus/geetorus", "acme/tools/broken"]);
    ...
    expect(manifest).toContain("- geetorusai/geetorus/geetorus — enabled");
    ...
    const coreIndex = manifest!.indexOf("geetorusai/geetorus/geetorus —");
