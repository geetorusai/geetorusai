...
const NETWORK_PROXY_TEMP_PREFIX = "geetorus-network-sandbox-";
...
function assertUnixSocketPathLength(socketPath: string): void {
  ...
  if (pathBytes > UNIX_SOCKET_PATH_MAX_BYTES) {
    throw new Error(
      `Geetorus sandbox proxy socket path is ${pathBytes} bytes, exceeding the Linux limit of ${UNIX_SOCKET_PATH_MAX_BYTES}: ${socketPath}`,
    );
  }
}
...
async function createNetworkProxyTempDir(): Promise<string> {
  ...
  throw new Error("Unable to create a Linux-safe Geetorus sandbox proxy socket directory.", { cause: lastError });
}
...
async function startNetworkAllowlistProxy(
  ...
  const server = http.createServer((request, response) => {
    ...
    try {
      ...
      writeProxyError(response, 400, "invalid_request_url", "Geetorus sandbox proxy requires an absolute request URL.");
      return;
    }
    const port = target.port || (target.protocol === "https:" ? "443" : "80");
    if (target.protocol !== "http:") {
      writeProxyError(response, 400, "https_requires_connect", "HTTPS targets must use CONNECT through the Geetorus sandbox proxy.");
      return;
    }
    if (!isNetworkTargetAllowed(target.hostname, port, rules)) {
      writeProxyError(response, 403, "network_target_denied", "Network target denied by Geetorus sandbox policy.");
      return;
    }
  ...
  server.on("connect", (request, clientSocket, head) => {
    ...
    if (!hostname || !/^\d+$/.test(port) || !isNetworkTargetAllowed(hostname, port, rules)) {
      clientSocket.end(connectProxyError(
        ...
        "Network target denied by Geetorus sandbox policy.",
      ));
      return;
    }
