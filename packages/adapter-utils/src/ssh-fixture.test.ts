...
async function createFixtureRootDir(): Promise<string> {
  const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ssh-fixture-"));
...
describe("ssh env-lab fixture", () => {
  ...
  it("builds a remote script that sources login profiles but no nvm", async () => {
    const target = await buildSshSpawnTarget({
      spec: {
        ...
        port: 22,
        username: "ssh-user",
        remoteCwd: "/srv/geetorus/workspace",
    ...
    expect(remoteScript).toContain("/srv/geetorus/workspace");
  ...
  it("syncs a local directory into the remote fixture workspace", async () => {
    ...
    await writeFile(path.join(localDir, "message.txt"), "hello from geetorus\n", "utf8");
    ...
    expect(result.stdout).toContain("hello from geetorus");
  ...
  it("reports exact git-history import percentage from the known bundle size", async () => {
    ...
    await git(localRepo, ["config", "user.name", "Geetorus Test"]);
    await git(localRepo, ["config", "user.email", "test@geetorus.dev"]);
  ...
  it("round-trips a git workspace through the SSH fixture", async () => {
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(started.workspaceDir)} && git config user.name "Geetorus SSH" && git config user.email "ssh@geetorus.dev" && git add tracked.txt untracked.txt && git commit -m "remote update" >/dev/null && printf "remote dirty\\n" > tracked.txt && printf "remote extra\\n" > remote-only.txt`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
  ...
  it("round-trips remote git commits through the managed runtime restore path", async () => {
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(prepared.workspaceRemoteDir)} && git config user.name "Geetorus SSH" && git config user.email "ssh@geetorus.dev" && printf "committed\\n" > tracked.txt && git add tracked.txt && git commit -m "remote update" >/dev/null && printf "dirty remote\\n" > tracked.txt`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
  ...
  it("propagates remote commits to the local worktree with no git remote configured (no-remote-git contract)", async () => {
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(prepared.workspaceRemoteDir)} && git config user.name "Geetorus SSH" && git config user.email "ssh@geetorus.dev" && printf "deliverable\\n" > tracked.txt && git add tracked.txt && git commit -m "remote-only commit" >/dev/null`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
  ...
  it("merges concurrent remote commits through the managed runtime restore path", async () => {
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(preparedA.workspaceRemoteDir)} && git config user.name "Geetorus SSH" && git config user.email "ssh@geetorus.dev" && printf "from run a\\n" > run-a.txt && git add run-a.txt && git commit -m "remote update a" >/dev/null`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
    await runSshCommand(
      config,
      `cd ${JSON.stringify(preparedB.workspaceRemoteDir)} && git config user.name "Geetorus SSH" && git config user.email "ssh@geetorus.dev" && printf "from run b\\n" > run-b.txt && git add run-b.txt && git commit -m "remote update b" >/dev/null`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
    ...
    await expect(readFile(path.join(localRepo, "run-a.txt"), "utf8")).resolves.toBe("from run a\n");
    await expect(readFile(path.join(localRepo, "run-b.txt"), "utf8")).resolves.toBe("from run b\n");
    expect(await git(localRepo, ["log", "-1", "--pretty=%s"])).toContain("Geetorus SSH sync merge");
