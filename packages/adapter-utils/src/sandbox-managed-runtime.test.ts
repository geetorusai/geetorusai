...
async function initGitRepo(repoDir: string): Promise<void> {
  ...
  await git(repoDir, ["config", "user.name", "Geetorus Test"]);
  await git(repoDir, ["config", "user.email", "test@geetorus.dev"]);
...
describe("sandbox managed runtime", () => {
  ...
  it.each(["host_current", "adopt_remote", "durable_seed"] as const)("stages and restores both project repositories with independent Git histories (%s)", async (mode) => {
    const root = await mkdtemp(path.join(os.tmpdir(), "geetorus-multi-repo-"));
    cleanupDirs.push(root);
    const local = path.join(root, "local");
    const remote = path.join(root, "remote");
    const secondPath = ".geetorus-repositories/backend";
    ...
    await writeFile(path.join(local, ".git/info/exclude"), ".geetorus-repositories/\n");
  ...
  it("adopts a warm remote workspace without inbound overwrite and still merges outbound changes", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "geetorus-sandbox-adopt-"),
    );
  ...
  it("reconstructs a replacement workspace from the exact durable pre-turn seed", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "geetorus-sandbox-durable-seed-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
  ...
  it("preserves excluded local workspace artifacts during restore mirroring", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-restore-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    await mkdir(path.join(sourceDir, "src"), { recursive: true });
    await mkdir(path.join(targetDir, ".claude"), { recursive: true });
    await mkdir(path.join(targetDir, ".geetorus-runtime"), { recursive: true });
    ...
    await writeFile(path.join(targetDir, ".geetorus-runtime", "state.json"), "{}\n", "utf8");

    await mirrorDirectory(sourceDir, targetDir, {
      preserveAbsent: [".geetorus-runtime", ".claude", ".claude.json"],
    });
    ...
    await expect(readFile(path.join(targetDir, ".geetorus-runtime", "state.json"), "utf8")).resolves.toBe("{}\n");
  ...
  it("applies file mode on a staged sibling before renaming into place", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-copy-mode-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    ...
    expect(chmodMock.mock.calls[0]?.[0]).toContain(".geetorus-copy.");
  ...
  it("cleans up a staged sibling when chmod fails before rename", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-copy-cleanup-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    const relativePath = path.join("nested", "script.sh");
    ...
    expect(stagedPath).toContain(".geetorus-copy.");
  ...
  it("syncs workspace and assets through a provider-neutral sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-managed-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    await mkdir(path.join(localWorkspaceDir, ".geetorus-runtime"), { recursive: true });
    await writeFile(path.join(localWorkspaceDir, ".geetorus-runtime", "state.json"), "{}\n", "utf8");
    ...
    await expect(readFile(path.join(localWorkspaceDir, ".geetorus-runtime", "state.json"), "utf8")).resolves.toBe("{}\n");
  ...
  it("falls back to the host-known byte count when the provider reports 0 for inbound workspace sync", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-zero-report-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it.each(["workspace", "git-workspace"])(
    ...
    async (reservedKey) => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-asset-key-"));
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
      const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("syncs git-backed workspaces through a shallow standalone clone and keeps .git out of archives", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-git-"));
    cleanupDirs.push(rootDir);
    ...
    await git(sourceRepoDir, ["config", "user.name", "Geetorus Test"]);
    await git(sourceRepoDir, ["config", "user.email", "test@geetorus.dev"]);
    ...
    await git(remoteWorkspaceDir, ["config", "user.name", "Geetorus Sandbox"]);
    await git(remoteWorkspaceDir, ["config", "user.email", "sandbox@geetorus.dev"]);
  ...
  it("repairs stale host index deletions when the sandbox restores a clean git worktree", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-clean-restore-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("does not fail clean restore checks when local working tree changes survive", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-preserved-local-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        "[geetorus] Workspace restore preserved local working tree changes after clean sandbox restore.",
      );
    } finally {
      warnSpy.mockRestore();
    }
  ...
  it("excludes unignored dependency trees from git-backed workspace overlay archives", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-unignored-deps-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("excludes an anchor-workspace ignored file whose name has leading and trailing whitespace from the staged tree", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-ignored-whitespace-"));
    cleanupDirs.push(rootDir);
  ...
  it("builds workspace/asset tarballs without a './' self-entry (so untar does not chmod/utime an unowned target dir)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-tarself-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("excludes transient symlinked home dirs from the asset tar while keeping required content", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-home-tmp-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("emits throttled, labeled upload and restore progress with direction and percentages", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-progress-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("creates valid empty workspace tarballs when the workspace is empty", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-empty-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("provisions a contribution-less asset via a plain tar extract and restores it as a no-op", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-default-asset-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("round-trips a non-codex asset through generic provision + restore contributions", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-seam-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("rejects a provision stageFile.name that is not a simple basename", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-traversal-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("routes a custom-provisioned asset through a single syncIn operation with its post-upload command (native runner → 0 direct writeFile/run)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-native-asset-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("stages git and workspace via syncIn preserving .geetorus-runtime (native runner → 0 direct writeFile/run)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-native-git-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    await git(sourceRepoDir, ["commit", "-m", "base"]);
    await git(sourceRepoDir, ["worktree", "add", "-b", "work", localWorkspaceDir, "HEAD"]);
    // Pre-seed the sandbox with a `.geetorus-runtime` dir that MUST survive.
    await mkdir(path.join(remoteWorkspaceDir, ".geetorus-runtime"), { recursive: true });
    await writeFile(path.join(remoteWorkspaceDir, ".geetorus-runtime", "keep.txt"), "keep\n", "utf8");
    ...
    // `.geetorus-runtime` while replacing the rest of the tree (wipe-except-preserved).
    const gitCommand = op.postUploadCommands![0].command;
    expect(gitCommand).toContain(".geetorus-runtime");
    expect(gitCommand).toContain("tar -xf");

    // The pre-seeded runtime dir survived the git+workspace staging.
    await expect(
      readFile(path.join(remoteWorkspaceDir, ".geetorus-runtime", "keep.txt"), "utf8"),
    ).resolves.toBe("keep\n");
  ...
  it("the workspace wipe command preserves in-flight sync scratch tarballs (.geetorus-upload-*)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-scratch-shape-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    expect(wipeCommand).toContain("! -name '.geetorus-upload-*'");
  });

  it("the workspace wipe keeps an in-flight scratch tarball at the root but removes a stale sibling", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-scratch-race-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    await git(sourceRepoDir, ["commit", "-m", "base"]);
    await git(sourceRepoDir, ["worktree", "add", "-b", "work", localWorkspaceDir, "HEAD"]);
    // Pre-seed the sandbox root. `.geetorus-upload-test.tar` simulates a
    // concurrent referenced-project scratch tarball in flight; `stale-junk.txt`
    // is an unrelated child that the wipe must remove.
    await mkdir(remoteWorkspaceDir, { recursive: true });
    await writeFile(path.join(remoteWorkspaceDir, ".geetorus-upload-test.tar"), "scratch\n", "utf8");
    ...
    await expect(
      readFile(path.join(remoteWorkspaceDir, ".geetorus-upload-test.tar"), "utf8"),
  ...
  it("issues one merged syncIn operation for a git-backed workspace stage-sync with two ordered extract commands", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-merged-git-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    // Both tar targets live under `.geetorus-runtime`, so the git extract's wipe
    // (which preserves `.geetorus-runtime`) cannot delete the overlay tar before
    // the overlay extract runs.
    for (const mapping of op.files) {
      expect(mapping.targetPath).toContain("/.geetorus-runtime/");
    }
    ...
    expect(commands[0].command).toContain(".geetorus-runtime");
  ...
  it("the merged workspace confine guard covers both tar mappings (escape in either trips it)", () => {
    const runtimeRoot = "/home/daytona/geetorus-workspace/.geetorus-runtime/test-adapter";
    const tempRoot = "/tmp/geetorus-sandbox-sync-abc";
    const gitMapping = {
  ...
  it("collapses a representative codex_local start to two syncIn round-trips: one merged workspace op plus the asset op", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-sandbox-codex-roundtrip-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("authors the advisory access intent rw on workspace, git, and asset inbound mappings", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-access-rw-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    // asset tar.
    const remoteAssetDir = path.posix.join(remoteWorkspaceDir, ".geetorus-runtime", "test-adapter", "home");
  ...
  it("authors the advisory access intent ro on referenced-project inbound mappings", async () => {
    const flagKey = "GEETORUS_MULTI_PROJECT_WORKSPACE_SYNC";
    const priorFlag = process.env[flagKey];
    process.env[flagKey] = "1";
    try {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-access-ro-"));
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("reports the real transferred bytes for a referenced project's inbound staging", async () => {
    ...
    process.env[flagKey] = "1";
    try {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-project-bytes-"));
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
      const referencedDir = path.join(rootDir, "referenced-project");
  ...
  // `GEETORUS_MULTI_PROJECT_WORKSPACE_SYNC` gates whether run prep resolves any
  ...
  it("stages multiple referenced projects into isolated sandbox dirs end-to-end, skipping a failing source", async () => {
    ...
    process.env[flagKey] = "1";
    try {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-e2e-additional-"));
      cleanupDirs.push(rootDir);
      ...
      const runtimeRootDir = path.posix.join(remoteWorkspaceDir, ".geetorus-runtime", "test-adapter");
  ...
  describe("resolveReferencedSourceIgnore", () => {
    it("re-relativizes root-relative ignored paths to a nested localPath", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-nested-"));
      cleanupDirs.push(rootDir);
    ...
    it("keeps today's fixed excludes for a non-Git source", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-nongit-"));
      cleanupDirs.push(rootDir);
    ...
    it("fails closed on a real Git error instead of returning an unfiltered result", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-fail-"));
      cleanupDirs.push(rootDir);
      const repo = path.join(rootDir, "repo");
      await initGitRepo(repo);
    ...
    for (const sensitivePath of SENSITIVE_PATH_PROBES) {
      it(`redacts a caught Git error embedding ${sensitivePath} to the fixed category`, async () => {
        const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-redact-caught-"));
        cleanupDirs.push(rootDir);
        const repo = path.join(rootDir, "repo");
        await initGitRepo(repo);
        try {
      ...
      it(`redacts a non-descendant toplevel embedding ${sensitivePath} to the fixed category`, async () => {
        const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-redact-nondescendant-"));
        cleanupDirs.push(rootDir);
    ...
    it("fails closed with a fixed category when the parsed ignored-entry count exceeds the bound", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-bound-count-"));
      cleanupDirs.push(rootDir);
      const repo = path.join(rootDir, "repo");
      await initGitRepo(repo);
    ...
    it("fails closed with a fixed category when the total UTF-8 byte size of ignored paths exceeds the bound", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-bound-bytes-"));
      cleanupDirs.push(rootDir);
      const repo = path.join(rootDir, "repo");
      await initGitRepo(repo);
    ...
    it("stages no bytes for either a count-breach or a byte-breach project, and stages a healthy sibling", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-bound-staging-"));
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("escapes glob metacharacters so a literal ignored path never over-excludes a sibling", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-glob-"));
    cleanupDirs.push(rootDir);
    const referencedDir = path.join(rootDir, "referenced-project");
  ...
  it("never ships a Git-ignored secret in a referenced project's staged tree", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-secret-"));
    cleanupDirs.push(rootDir);
    const referencedDir = path.join(rootDir, "referenced-project");
    await initGitRepo(referencedDir);
  ...
  it("stages no bytes for a project whose ignore resolution failed, and stages the rest", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-ignore-failed-skip-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const healthyDir = path.join(rootDir, "referenced-healthy");
  ...
  it("builds the workspace tarball inside one host pack span for a usual workspace sync", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-pack-span-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("nests the host pack span under the stage.workspace task span", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-pack-nest-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
...
describe("sandbox managed runtime inbound coordinator", () => {
  ...
  async function makeInboundDirs(names: string[]): Promise<{
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-inbound-coordinator-"));
    cleanupDirs.push(rootDir);
...
describe("sandbox managed runtime outbound coordinator", () => {
  ...
  async function makeOutboundDirs(names: string[]): Promise<{
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-outbound-coordinator-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(workspaceDir, { recursive: true });
  ...
  it("with concurrency permitted, two concurrent restore tasks use separate temporary state", async () => {
    ...
    expect(tempA!).toContain("geetorus-sandbox-restore-");
    expect(tempB!).toContain("geetorus-sandbox-restore-");
...
describe("sandbox git-bundle export transport", () => {
  ...
  it("moves the bundle through one native syncOut file mapping, never through readFile", async () => {
    ...
    const { localWorkspaceDir, remoteWorkspaceDir } = await setupGitBackedWorkspace("geetorus-bundle-native-");
  ...
  it("reads the bundle through readFile when the client has no native syncOut", async () => {
    const capture: TransportCapture = { syncOutOperations: [], readFilePaths: [] };
    const { localWorkspaceDir, remoteWorkspaceDir } = await setupGitBackedWorkspace("geetorus-bundle-fallback-");
  ...
  it("retries the full bundle through the native branch when the delta misses its prerequisite", async () => {
    const capture: TransportCapture = { syncOutOperations: [], readFilePaths: [] };
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bundle-retry-"));
    cleanupDirs.push(rootDir);
    ...
    await git(localWorkspaceDir, ["config", "user.name", "Geetorus Test"]);
    await git(localWorkspaceDir, ["config", "user.email", "test@geetorus.dev"]);
  ...
  it("reports the real transferred bytes for the native git-history export and workspace restore", async () => {
    const capture: TransportCapture = { syncOutOperations: [], readFilePaths: [] };
    const { localWorkspaceDir, remoteWorkspaceDir } = await setupGitBackedWorkspace("geetorus-restore-bytes-");
    const client = makeTransportClient(true, capture);
    const prepared = await prepareSandboxManagedRuntime({
      spec: gitSpec(remoteWorkspaceDir),
      adapterKey: "test-adapter",
