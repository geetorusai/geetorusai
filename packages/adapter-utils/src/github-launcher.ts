...
if (!['git', 'gh'].includes(program) || !executable) {
  process.stderr.write('Geetorus: requested GitHub command is not installed.\n');
  process.exit(127);
}
async function main() {
  let env = { ...process.env };
  const diagnostic = (code) => process.stderr.write('Geetorus: GitHub ' + code + '; continuing without managed credentials.\n');
  ...
  try {
    ...
    configDirectory = fs.mkdtempSync(path.join(configRoot, 'geetorus-github-operation-'));
  ...
  } catch { diagnostic('configuration_directory_unavailable'); }
  {
    for (const key of Object.keys(env)) {
      if (/^(GH_TOKEN|GITHUB_TOKEN|GH_ENTERPRISE_TOKEN|GITHUB_ENTERPRISE_TOKEN|GEETORUS_GIT_TOKEN|GIT_AUTHOR_.*|GIT_COMMITTER_.*|GIT_CONFIG_.*|GIT_ASKPASS|SSH_ASKPASS|SSH_AUTH_SOCK|GIT_SSH.*)$/.test(key)) delete env[key];
    }
    Object.assign(env, {
    ...
    const base = env.GEETORUS_GITHUB_BROKER_URL || env.GEETORUS_API_URL;
    try {
    let response;
    if (base && env.GEETORUS_GITHUB_BROKER_TOKEN) {
      const url = base.replace(/\/+$/, '').replace(/\/api$/, '') + '/runtime-tools/github/credentials';
      for (let attempt = 0; attempt < 30; attempt++) {
        response = await fetch(url, {
          method: 'POST', redirect: 'error', signal: AbortSignal.timeout(10000),
          headers: { authorization: 'Bearer ' + (env.GEETORUS_GITHUB_BRIDGE_TOKEN || env.GEETORUS_API_KEY || env.GEETORUS_GITHUB_BROKER_TOKEN),
            'x-geetorus-github-capability': env.GEETORUS_GITHUB_BROKER_TOKEN, 'content-type': 'application/json' },
          body: '{}',
        });
      ...
      if (result.status === 'unavailable') {
        const reason = typeof result.reason === 'string'
          ...
          : 'Check the GitHub connection in Geetorus';
        process.stderr.write('Geetorus: GitHub access unavailable: ' + reason + '. Continuing without GitHub credentials.\n');
      }
      if (result.status === 'available' && configReady) {
        for (const [key, value] of Object.entries(result.env || {})) {
          if (/^(GH_TOKEN|GITHUB_TOKEN|GEETORUS_GIT_TOKEN|GIT_TERMINAL_PROMPT|GIT_AUTHOR_(NAME|EMAIL)|GIT_COMMITTER_(NAME|EMAIL)|GIT_CONFIG_COUNT|GIT_CONFIG_(KEY|VALUE)_\d+)$/.test(key) && typeof value === 'string') env[key] = value;
        }
      }
      }
  ...
  child.once('error', () => { process.stderr.write('Geetorus: GitHub command could not start.\n'); process.exitCode = 1; });
  child.once('exit', (code, signal) => { process.exitCode = code === null ? 128 : code; });
}
main().catch(() => { process.stderr.write('Geetorus: GitHub launcher_setup_failed.\n'); process.exitCode = 1; });
`;
}
...
export function githubBrokerEnvironment(input: Record<string, unknown>, broker: { url: string; token: string }): Record<string, string> {
  ...
  for (const key of ["GH_TOKEN", "GITHUB_TOKEN", "GH_ENTERPRISE_TOKEN", "GITHUB_ENTERPRISE_TOKEN", "GEETORUS_GIT_TOKEN", "GIT_AUTHOR_NAME", "GIT_AUTHOR_EMAIL", "GIT_COMMITTER_NAME", "GIT_COMMITTER_EMAIL", "GIT_CONFIG_COUNT", "GEETORUS_GITHUB_OPERATION_ACTIVE"]) env[key] = "";
  for (const key of Object.keys(env)) {
  ...
  env.GIT_SSH_COMMAND = "ssh -F /dev/null -o IdentityAgent=none -o IdentitiesOnly=yes -o IdentityFile=none -o BatchMode=yes";
  env.SSH_AUTH_SOCK = "";
  env.GEETORUS_GITHUB_BROKER_URL = broker.url;
  env.GEETORUS_GITHUB_BROKER_TOKEN = broker.token;
  return env;
}
