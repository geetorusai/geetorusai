...
async function sandbox(layout: string) {
  const root = await mkdtemp(path.join(os.tmpdir(), "geetorus-launcher-env-"));
  roots.push(root);
...
describe("managed GitHub launcher environment", () => {
  it.each([false, true])("probes the remote workspace when the controller cwd is absent (host credentials: %s)", async (hostCredentials) => {
    ...
    expect(env.GEETORUS_RUNNER_NETWORK_ACCESS).toBe("enabled");
    expect(env.GEETORUS_GIT_METADATA_ROOTS).toBe("[]");
    expect(JSON.parse(env.GEETORUS_RUNNER_NETWORK_ROOTS!)).not.toHaveLength(0);
  ...
  it("reads Git metadata from the SSH workspace instead of an existing controller directory", async () => {
    ...
    expect(JSON.parse(env.GEETORUS_GIT_METADATA_ROOTS!)).toEqual([await realpath(path.join(fixture.root, ".git"))]);
  });
  ...
  it("uses target Git configuration without importing controller credentials", async () => {
    ...
    const env = await prepareGitHubExecutionEnvironment({
      target: fixture.target, cwd: fixture.root, env: {
        GEETORUS_GIT_METADATA_ROOTS: '["/injected"]',
        GEETORUS_RUNNER_NETWORK_ROOTS: '["/injected"]',
        GEETORUS_GITHUB_HOST_HOME: "/injected",
        GEETORUS_GITHUB_AUTH_MODE: "managed",
        GEETORUS_RUNNER_NETWORK_ACCESS: "disabled",
      }, hostCredentials: true, networkAccess: true,
    });
    expect(env.GEETORUS_GIT_METADATA_ROOTS).not.toContain("/injected");
    expect(env.GEETORUS_RUNNER_NETWORK_ROOTS).not.toContain("/injected");
    expect(env.GEETORUS_GITHUB_AUTH_MODE).toBe("host");
    expect(env.GEETORUS_RUNNER_NETWORK_ACCESS).toBe("enabled");
    expect(env.GEETORUS_GITHUB_HOST_HOME).toBe(fixture.root);
    expect(env.GH_CONFIG_DIR).toBe(path.join(fixture.root, ".config/gh"));
    expect(env.GH_TOKEN).toBeUndefined();
    expect(env.GEETORUS_GITHUB_LAUNCHER_DIR).toBeUndefined();
  });

  it("preserves local host credential helpers and validates worktree metadata", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "geetorus-host-git-")); roots.push(root);
    ...
    expect(env.GEETORUS_GIT_METADATA_ROOTS).toContain("/repo/.git");
    ...
    expect(isolated.GEETORUS_RUNNER_NETWORK_ACCESS).toBe("disabled");
    expect(isolated.GEETORUS_GITHUB_HOST_HOME).toBeUndefined();
  });
  ...
  it.each(["nvm/current/bin", "usr/local/bin", "tools with 'quotes'/bin"])(
    ...
    async (layout) => {
      ...
      expect(env.PATH).toBe(`${env.GEETORUS_GITHUB_LAUNCHER_DIR}:${fixture.remotePath}`);
      for (const cli of ["claude", "codex"]) {
        ...
        expect(result.stdout.trim().split("\n")).toEqual([
          `${env.GEETORUS_GITHUB_LAUNCHER_DIR}/git`,
          `${env.GEETORUS_GITHUB_LAUNCHER_DIR}/gh`,
          `${cli} started`,
        ]);
      }
      for (const profile of [".profile", ".bash_profile", ".bashrc", ".zshenv", ".zprofile", ".zshrc"]) {
        const script = await readFile(path.join(env.GEETORUS_GITHUB_LAUNCHER_DIR, profile), "utf8");
  ...
  it("keeps an explicit empty remote PATH empty apart from the managed wrappers", async () => {
    ...
    expect(env.PATH).toBe(env.GEETORUS_GITHUB_LAUNCHER_DIR);
    expect(fixture.runner.execute.mock.calls.every(([input]) => !input.args?.join(" ").includes("$PATH"))).toBe(true);
