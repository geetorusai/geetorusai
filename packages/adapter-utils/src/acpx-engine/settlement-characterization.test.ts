...
function stubBridges() {
  ...
  return { geetorusStops, processStops, anyStopped };
}
...
describe("ACP settlement — Layer A: engine teardown orchestration", () => {
  ...
  it("test_completed_non_persistent_local_run_closes_runtime_once", async () => {
    ...
    expect((closeSpy.mock.calls[0]! as unknown[])[0]).toMatchObject({
      reason: "geetorus completed turn cleanup",
  ...
  it("test_terminal_close_reason_and_discard_persistent_state_per_outcome", async () => {
    ...
    const cases: Array<{
      ...
      { status: "completed", reason: "geetorus completed turn cleanup", discard: false, exitCode: 0 },
      { status: "failed", reason: "geetorus turn failed", discard: false, exitCode: 1 },
      { status: "cancelled", reason: "geetorus turn cancelled", discard: true, exitCode: 1 },
    ];
