...
it("spawns a real Node ACP agent with per-session env on this platform", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "geetorus-acpx-spawn-smoke-"),
  );
  tempRoots.push(root);
  ...
  const result = await execute({
    ...
    config: {
      ...
      stateDir,
      cwd: repoRoot,
      env: { GEETORUS_ACPX_SPAWN_SMOKE: "spawn-ok" },
    },
    context: {},
  ...
  expect(stderr).toContain("geetorus-acp-echo-agent started");
});

it("fails closed on a typed ACP session failure without exposing its provider text", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "geetorus-acpx-typed-failure-"),
  );
  tempRoots.push(root);
  const logs: string[] = [];
  ...
  const result = await execute({
    ...
    config: {
      ...
      cwd: repoRoot,
      env: { GEETORUS_ACPX_TYPED_FAILURE_CANARY: providerText },
    },
    context: {},
    onLog: async (_stream: string, text: string) => logs.push(text),
    onMeta: async () => {},
...
it("fails closed on a typed ACP session failure in persistent mode", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "geetorus-acpx-persistent-typed-failure-"),
  );
  tempRoots.push(root);
...
it("preserves ordinary assistant text even when it resembles a provider error", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "geetorus-acpx-error-shaped-answer-"),
  );
  tempRoots.push(root);
  const answer =
  ...
  const result = await execute({
    ...
    config: {
      ...
      stateDir: path.join(root, "state"),
      cwd: repoRoot,
      env: { GEETORUS_ACPX_SPAWN_SMOKE: answer },
    },
    context: {},
    onLog: async () => {},
    onMeta: async () => {},
...
it("keeps a typed retry warning nonfatal when the turn produces an answer", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "geetorus-acpx-typed-warning-"),
  );
  tempRoots.push(root);
  ...
  const result = await execute({
    ...
    config: {
      ...
      cwd: repoRoot,
      env: {
        GEETORUS_ACPX_TYPED_WARNING_CANARY: warningCanary,
        GEETORUS_ACPX_SPAWN_SMOKE: answer,
      },
    },
    context: {},
    onLog: async (_stream: string, text: string) => logs.push(text),
...
it("captures the Node error shape for a host-invalid spawn cwd", async () => {
  ...
  const missingCwd = path.join(
    os.tmpdir(),
    "geetorus-acpx-missing-spawn-cwd",
    "nested",
    "does-not-exist",
  );
