...
import {
  ...
  type AdapterExecutionTargetGeetorusBridgeHandle,
...
import {
  ...
  isGeetorusExternalChatTurn,
  isGeetorusRuntimeEnvKey,
  joinPromptSections,
  materializeGeetorusSkillCopy,
  parseObject,
  ...
  renderTemplate,
  resolveGeetorusInstanceRootForAdapter,
  selectGeetorusTaskMarkdown,
  resolveLegacyGeetorusDesiredSkillNames,
  removeMaintainerOnlySkillSymlinks,
  rewriteWorkspaceCwdEnvVarsForExecution,
  shapeGeetorusWorkspaceEnvForExecution,
  stringifyGeetorusWakePayload,
  type GeetorusSkillEntry,
} from "@geetorusai/adapter-utils/server-utils";
import { shellQuote } from "@geetorusai/adapter-utils/ssh";
import {
  createAcpRuntime,
  createAgentRegistry,
  createRuntimeStore,
...
const GEETORUS_MANAGED_CODEX_SKILLS_MANIFEST = ".geetorus-managed-skills.json";
...
type GeetorusAcpRuntimeOptions = AcpRuntimeOptions & {
...
type AcpxRuntimeFactory = (options: GeetorusAcpRuntimeOptions) => AcpRuntime;
 ...
 * `sessionKey` (`geetorus:companyId:agentId:taskKey:fingerprint`) — the SAME
 ...
 * `@geetorusai/adapter-utils`, a dependency of those packages — cannot import
...
interface AcpxPreparedRuntime {
  ...
  geetorusBridge: AdapterExecutionTargetGeetorusBridgeHandle | null;
  ...
  geetorusClaudeSettings: GeetorusClaudeSettingsResult | null;
 ...
 * form is `geetorus:companyId:agentId:taskKey:fingerprint`.
 */
export function buildSessionKey(identity: SessionKeyIdentity, fingerprint: string): string {
  return `geetorus:${identity.companyId}:${identity.agentId}:${identity.taskKey}:${fingerprint}`;
}

// ACPX runs inside the long-lived Geetorus server process. A local child needs
...
function defaultGeetorusInstanceDir(): string {
  const home = process.env.GEETORUS_HOME?.trim() || path.join(os.homedir(), ".geetorus");
  const instanceId = process.env.GEETORUS_INSTANCE_ID?.trim() || "default";
  return resolveGeetorusInstanceRootForAdapter({
    homeDir: home,
    instanceId,
  });
}
...
function defaultStateDir(companyId: string, agentId: string): string {
  return path.join(defaultGeetorusInstanceDir(), "companies", companyId, "acp-engine", "agents", agentId);
}

function resolveManagedCodexHomeDir(companyId: string): string {
  return path.join(defaultGeetorusInstanceDir(), "companies", companyId, "codex-home");
}
...
function resolveManagedGrokHomeDir(companyId: string): string {
  return path.join(defaultGeetorusInstanceDir(), "companies", companyId, "grok-home");
}
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[geetorus] Using Geetorus-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  skills: GeetorusSkillEntry[];
  label: string;
}): Promise<string> {
  const hash = createHash("sha256");
  hash.update(`geetorus-acpx-${input.label}-skills:v1\n`);
...
async function resolveSelectedRuntimeSkills(
  config: Record<string, unknown>,
  moduleDir: string,
): Promise<{ allSkills: GeetorusSkillEntry[]; selectedSkills: GeetorusSkillEntry[]; desiredSkillNames: string[] }> {
  const allSkills = await readGeetorusRuntimeSkillEntries(config, moduleDir);
  const desiredSkillNames = resolveLegacyGeetorusDesiredSkillNames(config, allSkills);
  ...
  return {
    ...
    selectedSkills: allSkills.filter(
      (entry) => desiredSet.has(entry.key) && !isGeetorusSkillSourceMissing(entry),
    ),
    desiredSkillNames,
  };
}
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      const result = await materializeGeetorusSkillCopy(entry.source, target);
      const skillMdStat = await fs.stat(path.join(target, "SKILL.md")).catch(() => null);
      if (!skillMdStat?.isFile()) {
        await fs.rm(target, { recursive: true, force: true });
        await input.onLog(
          "stderr",
          `[geetorus] Skipped ACPX Claude skill "${entry.key}": the staged copy at ${target} has no usable SKILL.md.\n`,
        );
        continue;
      }
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[geetorus] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[geetorus] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedNames.length > 0
    ? [
        "Geetorus has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedNames.length > 0
      ? [`Materialized ${selectedNames.length} Geetorus skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
...
async function readManagedCodexSkillsManifest(skillsHome: string): Promise<Set<string>> {
  const manifestPath = path.join(skillsHome, GEETORUS_MANAGED_CODEX_SKILLS_MANIFEST);
  try {
...
async function writeManagedCodexSkillsManifest(skillsHome: string, skillNames: Iterable<string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(skillsHome, GEETORUS_MANAGED_CODEX_SKILLS_MANIFEST),
...
async function reconcileManagedCodexSkills(input: {
  skillsHome: string;
  allSkills: GeetorusSkillEntry[];
  selectedSkills: GeetorusSkillEntry[];
  onLog: AdapterExecutionContext["onLog"];
}): Promise<void> {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[geetorus] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[geetorus] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[geetorus] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[geetorus] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[geetorus] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function prepareGeminiSkillRuntime(input: {
  ...
  for (const skillName of removedSkills) {
    await input.onLog("stdout", `[geetorus] Removed maintainer-only ACPX Gemini skill "${skillName}" from ${skillsHome}\n`);
  }
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result === "created" || result === "repaired") {
        await input.onLog(
          "stdout",
          `[geetorus] ${result === "repaired" ? "Repaired" : "Linked"} ACPX Gemini skill "${entry.runtimeName}" into ${skillsHome}\n`,
        );
      }
    } catch (err) {
      if (isErrnoException(err, "EPERM")) {
        ...
        await input.onLog(
          "stdout",
          `[geetorus] Copied ACPX Gemini skill "${entry.runtimeName}" into ${skillsHome} because symlinks are unavailable.${result.skippedSymlinks.length > 0 ? ` Skipped ${result.skippedSymlinks.length} nested symlink(s).` : ""}\n`,
        );
        continue;
      }
      await input.onLog(
        "stderr",
        `[geetorus] Failed to link ACPX Gemini skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
interface GeetorusClaudeSettingsResult {
  filePath: string;
  allow: string[];
...
// widen the SDK's Read sandbox to include the Geetorus state dirs the agent
// needs to talk to its own control plane.
async function writeGeetorusClaudeSettings(input: {
  cwd: string;
  stateDir: string;
  agentHome: string;
  companyId: string;
}): Promise<GeetorusClaudeSettingsResult> {
  const filePath = path.join(input.cwd, ".claude", "settings.local.json");
  const instanceRoot = defaultGeetorusInstanceDir();
  const companyRoot = path.join(instanceRoot, "companies", input.companyId);
  const geetorusAdditionalDirectories = uniqueSorted([
    input.stateDir,
    input.agentHome,
    companyRoot,
  ]);
  const geetorusAllow = uniqueSorted([
    "Bash(curl:*)",
    "Bash(env:*)",
    "Bash(env)",
    `Bash(${input.cwd}/scripts/geetorus-issue-update.sh:*)`,
    `Bash(${input.cwd}/scripts/geetorus:*)`,
  ]);
  ...
  const existingAdditionalDirectories = Array.isArray(existingPerms.additionalDirectories)
    ...
    : [];
  const mergedAllow = uniqueSorted([...existingAllow, ...geetorusAllow]);
  const mergedAdditionalDirectories = uniqueSorted([
    ...existingAdditionalDirectories,
    ...geetorusAdditionalDirectories,
  ]);
...
async function stageAcpRemoteRuntime(input: {
  ...
  await input.onLog(
    "stdout",
    `[geetorus] Syncing workspace to ${describeAdapterExecutionTarget(input.target)}.\n`,
  );
...
async function disposeFreshStagedRuntime(input: {
  ...
  try {
    ...
    await input.onLog(
      "stderr",
      `[geetorus] Failed to dispose the fresh staged runtime after a managed-home seam error: ${
...
async function buildRuntime(input: {
  ...
  const secretsContext = parseObject(context.geetorusSecrets);
  ...
  // Referenced-project workspace hints exposed to the agent through GEETORUS_WORKSPACES_JSON. The
  ...
  const envConfig = parseObject(config.env);
  const env: Record<string, string> = { ...buildGeetorusEnv(agent), GEETORUS_RUN_ID: runId };
  const wakeTaskId =
  ...
  // user/adapter-configured env flows through this loop; per-wake GEETORUS_*
  // runtime vars (GEETORUS_RUN_ID, wake/approval ids, ...) were assigned to
  ...
  const scratch = parseObject(context.geetorusScratch);
  const scratchKeys = scratch.type === "heartbeat_run" && typeof scratch.dir === "string"
    ? new Set(["GEETORUS_RUN_SCRATCH_DIR", "GEETORUS_TASK_SCRATCH_DIR", "GEETORUS_SCRATCH_DIR", "GEETORUS_TMPDIR",
  ...
  for (const [key, value] of Object.entries(shapedEnvConfig)) {
    if (typeof value !== "string") continue;
    // Runtime GEETORUS_* always wins over config: skip a GEETORUS_* key that
    // Geetorus has already assigned this run. GEETORUS_API_KEY is never
    // accepted from config — the harness-minted run token is the only source.
    // A GEETORUS_* key Geetorus did NOT set is stable per-run config, so it
    // applies and feeds the fingerprint hash below.
    if (isForbiddenConfigEnvKey(key)) continue;
    if (isGeetorusRuntimeEnvKey(key) && key in env) continue;
    env[key] = value;
  ...
  if (authToken) env.GEETORUS_API_KEY = authToken;
  ...
  if (acpxAgent === "codex") {
    ...
    if (codexStartupConfig.invalidExistingConfig) {
      await input.ctx.onLog(
        "stderr",
        "[geetorus] Ignoring invalid user CODEX_CONFIG while applying runtime Codex settings; expected a JSON object.\n",
      );
    }
  ...
  let geetorusClaudeSettings: GeetorusClaudeSettingsResult | null = null;
  if (acpxAgent === "claude") {
    ...
    geetorusClaudeSettings = await writeGeetorusClaudeSettings({
      cwd,
      stateDir,
      agentHome,
    ...
    skillCommandNotes.push(
      `Wrote Geetorus-managed Claude settings to ${geetorusClaudeSettings.filePath} (defaultMode=${geetorusClaudeSettings.defaultMode}${
        geetorusClaudeSettings.overrodeDontAsk ? "; overrode user dontAsk" : ""
      }, +${geetorusClaudeSettings.additionalDirectories.length} read root(s), +${geetorusClaudeSettings.allow.length} allow rule(s)).`,
    );
    ...
    const desired = resolveLegacyGeetorusDesiredSkillNames(
      config,
      await readGeetorusRuntimeSkillEntries(config, input.engine.moduleDir),
    );
    skillsIdentity = { mode: "custom_unsupported", desiredSkillNames: desired };
    if (desired.length > 0) {
      skillCommandNotes.push("Selected Geetorus skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
  ...
  const fingerprintIdentity: SessionFingerprintIdentity = {
    ...
    geetorusClaudeSettings: geetorusClaudeSettings
      ? {
          allow: geetorusClaudeSettings.allow,
          additionalDirectories: geetorusClaudeSettings.additionalDirectories,
          defaultMode: geetorusClaudeSettings.defaultMode,
        }
      : null,
    ...
    // plain, secret_ref, and stable GEETORUS_* config such as an explicit
    // GEETORUS_API_KEY) into the fingerprint so a change to any forwarded value
  ...
  if (useRemoteProcessSession && executionTarget?.kind === "remote") {
    ...
    sandboxSite = createSandboxRunSite({
      ...
      publishStagedProjectHints: (stagedProjectDirs) => {
        const shapedHints = shapeGeetorusWorkspaceEnvForExecution({
          workspaceCwd: effectiveWorkspaceCwd,
          workspaceWorktreePath,
          workspaceHints,
          executionTargetIsRemote,
        ...
        if (shapedHints.length > 0) {
          env.GEETORUS_WORKSPACES_JSON = JSON.stringify(shapedHints);
        }
      },
      onReuseLog: () =>
        input.ctx.onLog(
          "stdout",
          "[geetorus] Reusing the staged in-sandbox runtime for this resumed session (no workspace re-ship / managed-home re-seed).\n",
        ),
      startGeetorusBridge: (runtimeRootDir) =>
        startAdapterExecutionTargetGeetorusBridge({
          runId,
          ...
          adapterKey: input.engine.adapterType,
          timeoutSec,
          hostApiToken: env.GEETORUS_API_KEY,
      ...
      onGeetorusBridgeLog: () =>
        input.ctx.onLog("stdout", "[geetorus] Sandbox ACP API callback bridge enabled for this run.\n"),
    ...
    const inSandboxSkillsRoot =
      ...
      path.posix.join(
        stagedRuntime.runtimeRootDir ??
          path.posix.join(stagedRuntime.workspaceRemoteDir ?? cwd, ".geetorus-runtime", acpxAgent),
        "skills",
      );
  ...
  // geetorus callback bridge — fires the same abandon-path cleanup. The
  // geetorus bridge starts after the workspace + managed home were already
  ...
  let geetorusBridge: AdapterExecutionTargetGeetorusBridgeHandle | null = null;
  ...
  try {
    if (useRemoteProcessSession && sandboxSite) {
      ...
      // the one geetorus-env → process-session-launch dependency at a single
      ...
      geetorusBridge = transport.controlBridge;
    ...
    const startedControl = sandboxSite?.controlBridge ?? geetorusBridge;
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[geetorus] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[geetorus] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
function renderGeetorusEnvNote(env: Record<string, string>): string {
  const geetorusKeys = Object.keys(env)
    .filter((key) => key.startsWith("GEETORUS_"))
    .sort();
  if (geetorusKeys.length === 0) return "";
  return [
    "Geetorus runtime note:",
    `The following GEETORUS_* environment variables are available in this run: ${geetorusKeys.join(", ")}`,
    "Do not assume these variables are missing without checking your shell environment.",
  ].join("\n");
}

function renderApiAccessNote(env: Record<string, string>): string {
  if (!env.GEETORUS_API_URL || !env.GEETORUS_API_KEY) return "";
  const lines = [
    "Geetorus API access note:",
    "Use terminal commands with curl to make Geetorus API requests.",
    "Normalize the base URL before adding API paths:",
    `  GEETORUS_API_BASE="\${GEETORUS_API_URL%/}"; GEETORUS_API_BASE="\${GEETORUS_API_BASE%/api}"`,
    "GET example:",
    `  curl -s -H "Authorization: Bearer $GEETORUS_API_KEY" "$GEETORUS_API_BASE/api/agents/me"`,
  ];
  if (env.GEETORUS_TASK_ID) {
    lines.push(
      "Scoped issue comment example:",
      `  curl -s -X POST -H "Authorization: Bearer $GEETORUS_API_KEY" -H "Content-Type: application/json" -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" -d '{"body":"Status update from agent."}' "$GEETORUS_API_BASE/api/issues/$GEETORUS_TASK_ID/comments"`,
    );
  } else {
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean, env: Record<string, string>): Promise<{
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[geetorus] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !resumedSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const taskContextNote = selectGeetorusTaskMarkdown(context, { resumedSession });
  const externalChatTurn = isGeetorusExternalChatTurn(context.geetorusWake);
  ...
  const geetorusEnvNote = externalChatTurn ? "" : renderGeetorusEnvNote(env);
  ...
  return {
    ...
    promptMetrics: {
      ...
      runtimeNoteChars: geetorusEnvNote.length + apiAccessNote.length,
...
/**
 * Build the short run summary that Geetorus may auto-post as an issue comment
...
async function emitAcpxFailure(input: {
  ...
  if (childStderrTail) {
    await ctx.onLog(
      "stderr",
      `[geetorus] ACPX child stderr tail (${phase}):\n${childStderrTail}\n`,
    );
  }
  await emitAcpxLog(ctx, {
    type: "acpx.error",
 ...
 * prefix. */
const RUN_ROOT_SPAN_ATTR_PREFIX = "geetorus.task.run.";
...
const TURN_SPAN_ATTR_PREFIX = "geetorus.agent.turn.";
  ...
  return async function executeAcpxEngine(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    const hostSite = createHostRunSite({
      ...
      closeWarmEntry: async (entry) => {
        await entry.runtime
          .close({ handle: entry.handle, reason: "geetorus idle cleanup", discardPersistentState: false })
          .catch(() => {});
    ...
    try {
        ...
        const reason =
          step === SYNC_BACK_SETTLEMENT_STEP
            ...
            : teardownErr instanceof Error
              ...
              : String(teardownErr);
        await ctx
          .onLog("stderr", `[geetorus] ACPX teardown step "${step}" failed: ${reason}\n`)
          .catch(() => {});
      };
      ...
      const startup = async (): Promise<StartupResult> => {
       try {
        ...
        for (const failure of referencedProjectStagingFailures) {
          await ctx.onLog(
            "stderr",
            `[geetorus] Referenced project ${failure.projectId} failed to stage; the run continues without it: ${failure.error}\n`,
          );
        }
        ...
        await ctx.onLog(
          "stderr",
          `[geetorus] ${formatAdapterExecutionTimeoutStartLogLine(prepared.timeoutResolution)}\n`,
        );
        ...
        const runtimeOptions: GeetorusAcpRuntimeOptions = {
          cwd: prepared.cwd,
          ...
          // must not merge the Geetorus server's ambient environment back in
        ...
        if (!canResume && asString(previousParams.runtimeSessionName, "")) {
          await ctx.onLog(
            "stdout",
            `[geetorus] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
          );
        }
        ...
        const isHandshakeTransportLost = (): boolean =>
          prepared.geetorusBridge?.readRunDisposition?.().failed ?? false;
        ...
        const closeLateHandshakeHandle = (lateHandle: AcpRuntimeHandle): void => {
          ...
          void runtime
            .close({
              handle: lateHandle,
              reason: "geetorus late handshake cleanup",
            ...
            .catch(() =>
              ctx
                .onLog(
                  "stderr",
                  "[geetorus] ACPX handshake late close failed: acpx_handshake_late_close_failed\n",
                )
                .catch(() => {}),
            );
        };
        ...
        const recordLateHandshakeRejection = (): void => {
          void ctx
            .onLog("stderr", "[geetorus] ACPX handshake late rejection: acpx_handshake_late_rejection\n")
            .catch(() => {});
        };
        ...
        try {
          if (!handle) {
            try {
              ...
              await ctx.onLog(
                "stdout",
                `[geetorus] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
              );
          ...
          runtimeSettlement = {
            ...
            reason: "geetorus handshake cleanup",
            discardPersistentState: false,
            dropWarmEntry: true,
        ...
        if (!handle) {
          ...
          runtimeSettlement = {
            ...
            reason: "geetorus missing-handle cleanup",
            discardPersistentState: false,
            dropWarmEntry: false,
            recordCloseError: true,
            cancelTurnReason: null,
      ...
      try {
        ...
        runtimeSettlement = {
          mode: "direct",
          handle: sessionHandle,
          reason: "geetorus config cleanup",
          discardPersistentState: false,
          dropWarmEntry: true,
          recordCloseError: true,
          cancelTurnReason: null,
      ...
      const stepPromptBuild = async (_signal: AbortSignal): Promise<void> => {
        ...
        if (ctx.onMeta) {
          await ctx.onMeta({
            ...
            cwd: prepared.cwd,
            commandNotes: [
              `ACPX runtime embedded in Geetorus with ${prepared.mode} session mode.`,
      ...
      const stepTurnStart = (signal: AbortSignal, startTimeoutMs: number | undefined): StartedTurn => {
        ...
        const bridge = prepared.geetorusBridge;
        if (bridge?.onLoss) {
          const cancelForLoss = (reason: DuplexLossReason) => {
            void turn.cancel({ reason: `geetorus sandbox duplex channel lost (${reason})` }).catch(() => {});
      ...
      const LOSS_DEADLINE_TERMINAL: AcpRuntimeTurnResult = {
        status: "cancelled",
        stopReason: "geetorus_duplex_loss_deadline",
      };
      const stepEventRelay = async (): Promise<AcpRuntimeTurnResult> => {
        ...
        if (!eventsEnded) {
          ...
          await turn.closeStream({ reason: "geetorus duplex loss cancel deadline" }).catch(() => {});
      ...
      const stepTurnFinalize = async (
        ...
        const disposition = prepared.geetorusBridge?.settleRunDisposition?.() ?? null;
        ...
        runtimeSettlement = {
          ...
          handle: sessionHandle,
          reason: timedOut
            ? "geetorus timeout cleanup"
            : channelLost
              ? "geetorus duplex channel lost cleanup"
              : failedTurn
                ? `geetorus turn ${terminal.status}`
                : "geetorus completed turn cleanup",
        ...
        runtimeSettlement = {
          mode: "direct",
          handle: sessionHandle,
          reason: timedOut ? "geetorus timeout cleanup" : "geetorus error cleanup",
      ...
      const LIVE_RUN_SCOPED_API_KEY = "geetorus-run-scoped-api-key";
      ...
      const settlementSteps: SettlementSteps = {
        ...
        endSession: (slots, decision) => timedPhase("end_session", async () => {
          ...
          const remoteChannelLost =
            settlement.skipRemoteClose || (prepared.geetorusBridge?.readRunDisposition?.().failed ?? false);
