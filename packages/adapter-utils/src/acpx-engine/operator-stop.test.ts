...
async function setup(tool?: string) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'geetorus-acp-stop-'));
  roots.push(root);
  ...
  const ctx = {
    ...
    config: { agent: 'custom', agentCommand: `${JSON.stringify(process.execPath)} ${JSON.stringify(fixture)}`, mode: 'persistent',
      ...
      env: { GEETORUS_STOP_FIXTURE_ROOT: root, ...(tool ? { GEETORUS_STOP_FIXTURE_TOOL: tool } : {}) } },
...
it('keeps the Stop deadline active after cancellation returns until provider exit', async () => {
  ...
  ctx.config.env = { ...(ctx.config.env as object), GEETORUS_STOP_FIXTURE_HANG_ON_CLOSE: '1' };
