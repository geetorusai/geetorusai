...
async function ensureRealAcpSession(input: {
  ...
  const stateRoot = await makeTempDir("geetorus-acpx-remote-spawn-state-");
...
it("reproduces host-spawn ENOENT when the advertised session cwd is host-nonexistent", async () => {
  ...
  const sandboxParent = await makeTempDir("geetorus-acpx-remote-spawn-sandbox-");
...
it("spawnCwd redirects the host spawn to a host-valid dir while the advertised session cwd stays remoteCwd", async () => {
  ...
  const hostSpawnCwd = await makeTempDir("geetorus-acpx-remote-spawn-host-");
...
it("kills the ACP agent when process identity persistence rejects", async () => {
  const hostCwd = await makeTempDir("geetorus-acpx-spawn-persistence-");
