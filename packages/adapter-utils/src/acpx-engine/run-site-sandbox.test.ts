...
import type { PreparedAdapterExecutionTargetRuntime } from "@geetorusai/adapter-utils/execution-target";

function makeStagedRuntime(id: string): PreparedAdapterExecutionTargetRuntime {
  return {
    runtimeRootDir: `/remote/${id}/.geetorus-runtime/acpx`,
...
function makeSite(overrides: Partial<SandboxRunSiteOptions> = {}) {
  ...
  const options: SandboxRunSiteOptions = {
    ...
    onReuseLog: async () => {},
    startGeetorusBridge: async () => {
      bridgeCalls.push("geetorus:start");
      return { env: { GEETORUS_API_KEY: "run-token" }, stop: async () => {} } as never;
    },
...
describe("sandbox run site", () => {
  it("test_sandbox_site_registers_staging_bridges_and_lease_in_ledger", async () => {
    ...
    await site.placeWorkspace(makeContext("geetorus:c:a:t:fp"));
    await site.startTransport(makeContext("geetorus:c:a:t:fp"));
  ...
  it("test_sandbox_site_place_workspace_carries_referenced_project_failure_reason", async () => {
    ...
    const failedStaged = {
      runtimeRootDir: "/remote/fail/.geetorus-runtime/acpx",
      additionalSourceDirs: {},
  ...
  it("test_sandbox_site_preserves_bridge_overlap_and_callback_sequencing", async () => {
    const events: string[] = [];
    let releaseGeetorus!: () => void;
    const geetorusGate = new Promise<void>((resolve) => {
      releaseGeetorus = resolve;
    });
    let processLaunchEnv: Record<string, string> | null = null;
    const { site } = makeSite({
      startGeetorusBridge: async () => {
        events.push("geetorus:start");
        await geetorusGate;
        events.push("geetorus:env-ready");
        return { env: { GEETORUS_API_KEY: "run-token" }, stop: async () => {} } as never;
      },
      startProcessSessionBridge: async ({ launchEnv }) => {
    ...
    // Both bridges start before the geetorus env resolves: their setup overlaps.
    await Promise.resolve();
    await Promise.resolve();
    expect(events).toContain("geetorus:start");
    expect(events).toContain("process-session:start");
    expect(events).not.toContain("process-session:launch");

    // Release the geetorus env; the process-session launch now observes the
    // merged run-scoped env (the single sequencing point).
    releaseGeetorus();
    const transport = await transportPromise;
    expect(events.indexOf("geetorus:env-ready")).toBeLessThan(events.indexOf("process-session:launch"));
    expect(processLaunchEnv).toEqual({ BASE: "1", CODEX_HOME: "/remote/home", GEETORUS_API_KEY: "run-token" });
  ...
  it("test_sandbox_site_borrow_yields_files_and_runtime_is_still_created", async () => {
    ...
    const borrowed = site.reuse().borrow("geetorus:c:a:t:fp");
    expect(borrowed?.stagedRuntime).toBe(cachedRuntime);
    expect(stagedRuntimes.has("geetorus:c:a:t:fp")).toBe(true);
