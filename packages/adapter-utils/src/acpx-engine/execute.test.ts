...
import type { AdapterExecutionContext, AdapterRuntimeMcpAccess } from "@geetorusai/adapter-utils";
import {
...
vi.mock("@geetorusai/adapter-utils/execution-target", async (importActual) => {
  const actual = await importActual<typeof import("@geetorusai/adapter-utils/execution-target")>();
  return {
    ...actual,
    prepareAdapterExecutionTargetRuntime: vi.fn(actual.prepareAdapterExecutionTargetRuntime),
    startAdapterExecutionTargetGeetorusBridge: vi.fn(actual.startAdapterExecutionTargetGeetorusBridge),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-acpx-skills-"));
  tempRoots.push(root);
  return root;
}
...
// `geetorus.sandbox.startup.` prefix from the attribute contract.
...
const ALLOWED_RUN_SPAN_ATTRIBUTE_KEYS = new Set<string>([
  "geetorus.task.run.run_id",
  "geetorus.task.run.wall_ms",
]);
...
describe("shared ACPX engine runtime behavior", () => {
  ...
  it("warns when runtime settings replace malformed user CODEX_CONFIG", async () => {
    ...
    expect(logs).toContainEqual({
      stream: "stderr",
      text: "[geetorus] Ignoring invalid user CODEX_CONFIG while applying runtime Codex settings; expected a JSON object.\n",
    });
  });
  ...
  it("includes Geetorus env and API access notes in the ACPX prompt without leaking the token", async () => {
    const { meta } = await runExecutor(
    ...
    expect(prompt).toContain("Geetorus runtime note:");
    expect(prompt).toContain("GEETORUS_AGENT_ID");
    expect(prompt).toContain("GEETORUS_API_KEY");
    expect(prompt).toContain("GEETORUS_WAKE_PAYLOAD_JSON");
    expect(prompt).toContain("Geetorus API access note:");
    expect(prompt).toContain('GEETORUS_API_BASE="${GEETORUS_API_URL%/}"; GEETORUS_API_BASE="${GEETORUS_API_BASE%/api}"');
    expect(prompt).toContain("$GEETORUS_API_BASE/api/agents/me");
    expect(prompt).toContain("$GEETORUS_API_BASE/api/issues/$GEETORUS_TASK_ID");
    expect(prompt).toContain("X-Geetorus-Run-Id");
    expect(prompt).not.toContain("$GEETORUS_API_URL/api/");
  ...
  it.each([
    ...
    const context = {
      ...
      taskId: "chat-1",
      geetorusTaskMarkdown: chatDirective,
      geetorusTaskMarkdownCompact: chatDirective,
      geetorusWake: {
    ...
    expect(String(fresh.meta[0]?.prompt)).toContain(custom ? "Custom agent instructions." : "Continue your Geetorus conversation");
    expect(String(reset.meta[0]?.prompt)).toContain(custom ? "Custom agent instructions." : "Continue your Geetorus conversation");
  ...
  it("uses only the guarded external-chat contract for a default ACPX prompt", async () => {
    const { meta } = await runExecutor(
      { agent: "custom", agentCommand: "node ./fake-acp.js" },
      {
        ...
        context: {
          taskId: "issue-chat-1",
          geetorusTaskMarkdown: "# CHAT-1 — Answer the provider message",
          geetorusWake: {
    ...
    expect(prompt).not.toContain("Geetorus API access note:");
    expect(prompt).not.toContain("Geetorus runtime note:");
    ...
    const { meta } = await runExecutor(
      { agent: "custom", agentCommand: "node ./fake-acp.js" },
      {
        ...
        context: {
          ...
          geetorusTaskMarkdown: "# CHAT-2 — Answer every queued message",
          geetorusWake: {
            reason: "External chat message received",
  ...
  it("does not show a scoped issue API command when the task id is unavailable", async () => {
    ...
    expect(prompt).not.toContain("$GEETORUS_API_BASE/api/issues/$GEETORUS_TASK_ID");
  });
  ...
  it.skipIf(process.platform === "win32")("materializes ACPX Claude skills without symlinked descendants", async () => {
    ...
    const { meta } = await runExecutor({
      agent: "claude",
      stateDir,
      geetorusRuntimeSkills: [skill],
      geetorusSkillSync: { desiredSkills: [skill.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("revokes removed ACPX Codex skills and skips symlinked descendants", async () => {
    ...
    const baseConfig = {
      ...
      geetorusRuntimeSkills: [keep, remove],
    };

    await runExecutor({
      ...baseConfig,
      geetorusSkillSync: { desiredSkills: [keep.key, remove.key] },
    });
    expect(await pathExists(path.join(codexHome, "skills", remove.runtimeName, "SKILL.md"))).toBe(true);

    await runExecutor({
      ...baseConfig,
      geetorusSkillSync: { desiredSkills: [keep.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("keeps the operational skill in an ACPX Codex home after an empty replacement", async () => {
    ...
    const codexHome = path.join(root, "codex-home");
    const operational = {
      ...await createSkill(skillRoot, "geetorus"),
    ...
    await runExecutor({
      ...
      stateDir: path.join(root, "state"),
      env: { CODEX_HOME: codexHome },
      geetorusRuntimeSkills: [operational],
  ...
  it.skipIf(process.platform === "win32")("replaces stale managed Codex auth files with source symlinks", async () => {
    ...
    const geetorusHome = path.join(root, "geetorus-home");
    const geetorusInstanceId = "test-instance";
    ...
    try {
      ...
      process.env.GEETORUS_INSTANCE_ID = geetorusInstanceId;
      await runExecutor({
        agent: "codex",
        stateDir: path.join(root, "state"),
  ...
  it("forwards resolved adapter env through session options without overriding runtime vars", async () => {
    ...
    const { sessionInputs } = await runExecutor(
      {
        ...
        env: {
          ...
          // GEETORUS_API_KEY is never accepted from config.
          GEETORUS_API_KEY: "config-key",
          // A GEETORUS_*-named key the harness does not assign flows through.
    ...
    expect(env.GEETORUS_TASK_ID).toBe("issue-real");
    expect(env.GEETORUS_API_KEY).toBe("runtime-secret-token");
  ...
  it("busts the session fingerprint when resolved adapter env changes but not across wakes", async () => {
    ...
    // per-wake GEETORUS_* churn does not needlessly reset the session.
  ...
  it("keeps rotated run scratch paths out of session identity while retaining user temp overrides", async () => {
    ...
    async function withScratch(dir: string, userTemp: string) {
      return runExecutor({ ...config, env: {
        GEETORUS_RUN_SCRATCH_DIR: dir, GEETORUS_TASK_SCRATCH_DIR: dir,
        GEETORUS_SCRATCH_DIR: dir, GEETORUS_TMPDIR: dir, TEMP: dir, TMP: dir, TMPDIR: userTemp,
      } }, { context: { taskId: "issue-1", geetorusScratch: { type: "heartbeat_run", dir, tempKeysApplied: ["TEMP", "TMP"] } } });
    }
  ...
  it("busts the session fingerprint when a stable configured GEETORUS_* value rotates", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const baseConfig = { agentCommand: "node ./fake-acp.js", stateDir };

    // A configured GEETORUS_*-named value the harness does not assign (e.g. a
    ...
    const withKey = await runExecutor(
      { ...baseConfig, env: { GEETORUS_CLOUD_PROVIDER_TOKEN: "explicit-key-1" } },
      { context },
    );
    const rotatedKey = await runExecutor(
      { ...baseConfig, env: { GEETORUS_CLOUD_PROVIDER_TOKEN: "explicit-key-2" } },
      { context },
    );
  ...
  it("busts the session fingerprint when the referenced-project set or pinned checkout changes", async () => {
    ...
    const withReferenced = (additional: unknown[]) => ({
      context: {
        taskId: "issue-1",
        wakeReason: "issue_assigned",
        geetorusWorkspace: { cwd, realization: { additional } },
      },
    });
    const projectA = {
      path: "/host/project-a",
  ...
  it("busts the session fingerprint when referenced-project files change at the same host path", async () => {
    ...
    const withProject = () => ({
      context: {
        taskId: "issue-1",
        wakeReason: "issue_assigned",
        geetorusWorkspace: { cwd, realization: { additional: [referencedProject] } },
      },
    });
    const fp = (r: { result: { sessionParams?: unknown } }) =>
      (r.result.sessionParams as { configFingerprint?: string } | undefined)?.configFingerprint;
  ...
  it("shapes ACPX session env for remote execution identities", async () => {
    ...
    const { sessionInputs, runtimeOptions } = await runExecutor(
      ...
      { context: { geetorusWorkspace: { cwd: localCwd, workspaceWorktreePath: localCwd } }, executionTarget: { kind: "remote", transport: "ssh", remoteCwd } },
    );
    const env = (sessionInputs[0]!.sessionOptions as { env: Record<string, string> }).env;
    expect(env.GEETORUS_WORKSPACE_CWD).toBe(localCwd);
  ...
  it("starts sandbox ACP process sessions in the remote execution cwd", async () => {
    ...
    const runner = createLocalSandboxRunner(
      (input: { args?: string[]; env?: Record<string, string> }) => {
        if (input.env?.GEETORUS_SANDBOX_EXEC_CHANNEL === "bridge") {
          const script = input.args?.[1] ?? "";
          const match = script.match(/GEETORUS_PROCESS_SESSION_COMMAND_B64='([^']+)'/);
          if (match) {
    ...
    expect(proxyCommand).toContain("geetorus-process-session-proxy.mjs");
    ...
    expect(payloadEnv).toMatchObject({
      GEETORUS_API_BRIDGE_MODE: "queue_v1",
    });
    expect(String(payloadEnv.GEETORUS_API_URL ?? "")).toMatch(
      /^http:\/\/127\.0\.0\.1:\d+$/,
    );
    expect(payloadEnv.GEETORUS_API_KEY).toBeTruthy();
    expect(payloadEnv.GEETORUS_API_KEY).not.toBe("real-run-jwt");
  });
  ...
  it("passes Geetorus env through ACPX session options instead of process.env", async () => {
    ...
    const previousApiKey = process.env.GEETORUS_API_KEY;
    try {
      delete process.env.GEETORUS_API_KEY;
      const result = await execute({
        runId: "run-1",
        agent: { id: "agent-1", companyId: "company-1" },
        runtime: {},
      ...
      expect(result.exitCode).toBe(0);
      expect(observedSessionEnv?.GEETORUS_API_KEY).toBe("runtime-key");
      expect(process.env.GEETORUS_API_KEY).toBeUndefined();
    } finally {
      if (previousApiKey === undefined) delete process.env.GEETORUS_API_KEY;
      else process.env.GEETORUS_API_KEY = previousApiKey;
    }
  });

  it("writes a Geetorus-managed .claude/settings.local.json for the claude agent so it can reach the Geetorus API", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    ...
    const { meta } = await runExecutor(
      ...
      { context: { geetorusWorkspace: { cwd, agentHome: path.join(root, "agent-home") } } },
    );
    ...
    expect(allow).toContain(`Bash(${cwd}/scripts/geetorus-issue-update.sh:*)`);
    ...
    const note = (meta[0]?.commandNotes as string[] | undefined)?.find((entry) =>
      entry.includes("Geetorus-managed Claude settings"),
    );
    expect(note).toBeTruthy();
  });

  it("merges Geetorus allowlist into an existing .claude/settings.local.json without losing user entries", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const cwd = path.join(root, "worktree");
  ...
  it("injects runtime MCP servers and fingerprints their identity without persisting bearer tokens", async () => {
    ...
    const server = {
      name: "github",
      url: "https://geetorus.example/api/tool-gateway/gateways/github/mcp",
...
describe("findAncestorBin", () => {
  ...
  it("finds the binary in the start directory's own node_modules/.bin", async () => {
    const root = await makeTempRoot();
    const packageDir = path.join(root, "node_modules", "@geetorusai", "adapter-utils");
...
describe("gemini ACP flag selection", () => {
  ...
  it("applies the 4h sandbox backstop when timeoutSec is unset on a sandbox execution target", async () => {
    ...
    expect(startLine!.text).toContain(
      `[geetorus] Adapter execution timeout: timeoutSec=${DEFAULT_REMOTE_SANDBOX_ADAPTER_TIMEOUT_SEC} ` +
  ...
  it("reports a self-describing timeout error when the wall-clock timer kills a turn", async () => {
    ...
    const execute = createAcpxEngineExecutor({
      createRuntime: () => ({
        ...
        startTurn: () => ({
          // Never yields on its own: only the Geetorus wall-clock timer's
...
describe("ACPX engine remote sandbox staging seam (PR 1: workspace + cwd)", () => {
  ...
  it("test_remote_buildRuntime_crosses_staging_seam", async () => {
    ...
    const geetorusArgs = vi.mocked(startAdapterExecutionTargetGeetorusBridge).mock.calls[0]![0];
    const processArgs = vi.mocked(startAdapterExecutionTargetProcessSessionBridge).mock.calls[0]![0];
    expect(geetorusArgs.runtimeRootDir).toBeTruthy();
    expect(processArgs.runtimeRootDir).toBeTruthy();
    expect(String(geetorusArgs.runtimeRootDir)).toContain(".geetorus-runtime");
    expect(processArgs.runtimeRootDir).toBe(geetorusArgs.runtimeRootDir);
  ...
  it("hands the merged geetorus env to the process-session launch when the setups overlap", async () => {
    const { stateDir, localCwd, remoteCwd, executionTarget } = await setupRemoteSandbox();
    ...
    // the seam that lets its env-independent setup overlap the geetorus bridge
    // start instead of running strictly after it.
    const processArgs = vi.mocked(startAdapterExecutionTargetProcessSessionBridge).mock.calls[0]![0];
    expect(typeof processArgs.env).toBe("function");

    // ...and despite the overlap the launch still observes the MERGED geetorus
    // env: the geetorus-`env` → process-session-launch hand-off stays sequenced
    ...
    const payloadEnv = ((launchPayload as Record<string, unknown> | null)?.env ?? {}) as Record<
      ...
      unknown
    >;
    expect(payloadEnv).toMatchObject({ GEETORUS_API_BRIDGE_MODE: "queue_v1" });
    expect(String(payloadEnv.GEETORUS_API_URL ?? "")).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
    expect(payloadEnv.GEETORUS_API_KEY).toBeTruthy();
    expect(payloadEnv.GEETORUS_API_KEY).not.toBe("real-run-jwt");
  });
  ...
  it("publishes referenced-project workspace hints repointed at their staged sandbox directories", async () => {
    ...
    const payloadEnv = ((launchPayload as Record<string, unknown> | null)?.env ?? {}) as Record<
      ...
      unknown
    >;
    const workspacesJson = payloadEnv.GEETORUS_WORKSPACES_JSON;
  ...
  it("stops the process-session bridge when the geetorus bridge fails under concurrency", async () => {
    const { stateDir, localCwd, executionTarget } = await setupRemoteSandbox();
    // The geetorus bridge fails; the process-session bridge — started CONCURRENTLY
    ...
    vi.mocked(startAdapterExecutionTargetGeetorusBridge).mockImplementationOnce(async () => {
      throw new Error("geetorus bridge boom");
    });
    ...
    ).rejects.toThrow("geetorus bridge boom");
  ...
  it("test_local_foundation_unchanged", async () => {
    ...
    expect(vi.mocked(startAdapterExecutionTargetGeetorusBridge)).not.toHaveBeenCalled();
...
describe("ACPX engine remote managed-home seam (PR 2: per-adapter home seed)", () => {
  ...
  it("test_remote_seam_stages_assets_and_env_remap_reaches_process", async () => {
    ...
    expect(remoteAssetDir).toContain(".geetorus-runtime");
  ...
  it("test_remote_seam_surfaces_referenced_project_staging_failure_reason", async () => {
    ...
    expect(failureLine?.text).toBe(
      "[geetorus] Referenced project proj-x failed to stage; the run continues without it: extract failed: boom\n",
    );
  });
});
...
describe("ACPX engine Claude skill bundle staging (remote ACP lane)", () => {
  ...
  it("drops a skill that fails to materialize from the prompt, the identity, and the staged bundle", async () => {
    ...
    // A skill whose source is a symlink. `materializeGeetorusSkillCopy`
    ...
    const { meta, result } = await runExecutor(
      {
        ...
        stateDir,
        cwd: localCwd,
        geetorusRuntimeSkills: [review, broken],
        geetorusSkillSync: { desiredSkills: [review.key, broken.key] },
      },
      { authToken: "real-run-jwt", executionTarget, prepareRemoteManagedHome: stagingClaudeSeam() },
    );
    ...
    const { meta } = await runExecutor(
      {
        ...
        stateDir,
        cwd: localCwd,
        geetorusRuntimeSkills: [broken],
        geetorusSkillSync: { desiredSkills: [broken.key] },
      },
      { authToken: "real-run-jwt", executionTarget, prepareRemoteManagedHome: stagingClaudeSeam() },
    );
  ...
  it("drops a skill whose staged copy has no usable SKILL.md from the prompt, the identity, and the staged bundle", async () => {
    ...
    // throw), but whose `SKILL.md` is a symlink. `materializeGeetorusSkillCopy`
    ...
    const symlinkedSkillMd = {
      key: "geetorusai/test/symlinked-skill-md",
    ...
    const noSkillMd = {
      key: "geetorusai/test/no-skill-md",
    ...
    const { meta, result } = await runExecutor(
      {
        ...
        stateDir,
        cwd: localCwd,
        geetorusRuntimeSkills: [review, symlinkedSkillMd, noSkillMd],
        geetorusSkillSync: { desiredSkills: [review.key, symlinkedSkillMd.key, noSkillMd.key] },
      },
      { authToken: "real-run-jwt", executionTarget, prepareRemoteManagedHome: stagingClaudeSeam() },
    );
...
describe("ACPX engine sandbox-start spans (opt-in root + child parenting)", () => {
  ...
  it("test_task_run_span_opens_and_ends_once_for_remote", async () => {
    ...
    // the span.
    expect(runSpan.attributes["geetorus.task.run.run_id"]).toMatch(/^[0-9a-f]{12}$/);
    expect(String(runSpan.attributes["geetorus.task.run.run_id"])).not.toContain("run-");
  });
...
describe("ACPX engine per-step startup timing (run.startup.step events)", () => {
  ...
  it("does not emit startup-step events on a local (non-sandbox) run except workspace.resolve", async () => {
    ...
    expect(emitted.has("bridge.geetorus")).toBe(false);
...
describe("ACPX engine run lifecycle corrections (F1: settle every failure after buildRuntime)", () => {
  ...
  it("test_runtime_create_failure_stops_bridges_and_releases_staging_lease", async () => {
    const { stateDir, localCwd, executionTarget } = await setupRemoteSandbox();
    const geetorusStop = vi.fn(async () => {});
    const processStop = vi.fn(async () => {});
    vi.mocked(startAdapterExecutionTargetGeetorusBridge).mockImplementationOnce(
      async () => ({ env: {}, stop: geetorusStop }) as never,
    );
    vi.mocked(startAdapterExecutionTargetProcessSessionBridge).mockImplementationOnce(
    ...
    expect(geetorusStop).toHaveBeenCalledTimes(1);
  ...
  it("test_prompt_build_failure_returns_error_result_with_phase_prepare_turn", async () => {
    ...
    Object.defineProperty(context, "geetorusSessionHandoffMarkdown", {
      enumerable: false,
      get() {
...
describe("ACPX engine run lifecycle corrections (F3: one teardown error policy)", () => {
  ...
  function stubBridges() {
    const geetorusStops: Array<ReturnType<typeof vi.fn>> = [];
    const processStops: Array<ReturnType<typeof vi.fn>> = [];
    vi.mocked(startAdapterExecutionTargetGeetorusBridge).mockImplementation(async () => {
      const stop = vi.fn(async () => {});
      geetorusStops.push(stop);
    ...
    return { geetorusStops, processStops, anyStopped, stoppedCount };
  }
  ...
  it("test_teardown_continues_after_one_teardown_step_fails", async () => {
    const { stateDir, localCwd, executionTarget } = await setupRemoteSandbox();
    const { geetorusStops, processStops, anyStopped } = stubBridges();
    const stagingLocks = new Map<string, Promise<unknown>>();
    const logs: Array<{ stream: string; text: string }> = [];
    const execute = createAcpxEngineExecutor({
      stagingLocks,
    ...
    expect(anyStopped(geetorusStops)).toBe(true);
  ...
  it("test_result_mapping_throw_after_close_does_not_rerun_teardown", async () => {
    const { stateDir, localCwd, executionTarget } = await setupRemoteSandbox();
    const { geetorusStops, processStops, stoppedCount } = stubBridges();
    let closeCount = 0;
    const execute = createAcpxEngineExecutor({
      warmHandles: new Map(),
      stagedRuntimes: new Map(),
    ...
    expect(stoppedCount(geetorusStops)).toBe(1);
  ...
  it("test_flush_child_stderr_runs_on_every_exit_path", async () => {
    ...
    try {
      ...
      for (const scenario of scenarios) {
        const root = await makeTempRoot();
        const marker = `geetorus-flush-probe-${scenario.name}`;
        const execute = createAcpxEngineExecutor({
          createRuntime: (options) => {
...
describe("ACPX engine sandbox bridge run-disposition seam (fail-closed)", () => {
  ...
  function createFakeBridgeHandle() {
    ...
    const handle = {
      env: {
        GEETORUS_API_URL: "http://127.0.0.1:1",
        GEETORUS_API_KEY: "bridge-token",
        GEETORUS_API_BRIDGE_MODE: "http2_v1",
      },
