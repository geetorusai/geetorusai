...
describe("acpx identity split and launch environment", () => {
  ...
  it("inherits only safe host context and the selected provider's credentials", () => {
    const inherited = {
      ...
      GEETORUS_NATIVE_MCP_TOKEN: "native-mcp-host-secret",
      GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: "managed-auth-host-secret",
  ...
  it("does not project any ambient host environment across a remote boundary", () => {
    const inherited = {
      ...
      GEETORUS_NATIVE_MCP_TOKEN: "ambient-native-mcp-secret",
      GEETORUS_RUNNER_BOOTSTRAP_TICKET: "ambient-bootstrap-secret",
    };
  ...
  it("keeps explicit remote adapter and run contributions while rejecting ambient authority", () => {
    const launchEnvironment = finalizeLaunchEnvironment(
      {
        ...
        GEETORUS_RUNTIME_API_URL: "http://geetorus.internal/api",
      },
      [
        {
          scope: "session",
          env: { GEETORUS_NATIVE_MCP_TOKEN: "explicit-run-contribution" },
        },
      ],
      {
        acpxAgent: "codex",
    ...
    expect(launchEnvironment.env).toMatchObject({
      ...
      EXPLICIT_ADAPTER_SECRET: "adapter-secret",
      GEETORUS_RUNTIME_API_URL: "http://geetorus.internal/api",
      GEETORUS_NATIVE_MCP_TOKEN: "explicit-run-contribution",
    });
    expect(launchEnvironment.env).not.toHaveProperty("PATH");
    expect(launchEnvironment.env).not.toHaveProperty("GEETORUS_RUNNER_BOOTSTRAP_TICKET");
  });
