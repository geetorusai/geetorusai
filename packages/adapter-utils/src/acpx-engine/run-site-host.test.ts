...
describe("host run site", () => {
  ...
  it("test_host_site_warm_hit_and_warm_save_match_baseline_sets", async () => {
    ...
    const entry = makeEntry("geetorus:c:a:t:fp", Date.now());
    store.save(entry.handle.sessionKey, entry);
    expect(new Set(warmHandles.keys())).toEqual(new Set(["geetorus:c:a:t:fp"]));
    expect(warmHandles.get("geetorus:c:a:t:fp")).toBe(entry);
    ...
    expect(store.borrow("geetorus:c:a:t:fp")).toBe(entry);
    ...
    await store.discard("geetorus:c:a:t:fp");
