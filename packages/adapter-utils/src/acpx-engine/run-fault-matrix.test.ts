...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-acpx-fault-"));
  tempRoots.push(root);
  return root;
}
...
describe("composed ACPX run fault matrix", () => {
  ...
  it("case_14c_eacces_teardown_reaches_result_as_restore_permission_denied", async () => {
    ...
    const eaccesError: NodeJS.ErrnoException = new Error(
      `EACCES: permission denied, mkdir '/srv/telemetry-backend.geetorus-restore.lock' (pid ${process.pid})`,
    );
