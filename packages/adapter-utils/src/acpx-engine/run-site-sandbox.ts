...
import type {
  ...
  AdapterExecutionTargetGeetorusBridgeHandle,
...
export interface SandboxRunSiteOptions {
  ...
  /** Start the host-side geetorus callback bridge. */
  readonly startGeetorusBridge: (
  ...
  readonly measureBridgeStep: <T>(step: "bridge.geetorus" | "bridge.process-session", run: () => Promise<T>) => Promise<T>;
  /**
  ...
  readonly onGeetorusBridgeLog: () => Promise<void>;

  /** Stop both bridges and run the managed-home copy-back on teardown. */
  readonly stopBridges: (input: {
    controlBridge: AdapterExecutionTargetGeetorusBridgeHandle | null;
...
export interface SandboxRunSite {
  ...
  /**
   * The geetorus callback bridge the run started, or null before
   ...
   */
  readonly controlBridge: AdapterExecutionTargetGeetorusBridgeHandle | null;
...
export function createSandboxRunSite(options: SandboxRunSiteOptions): SandboxRunSite {
  ...
  let controlBridge: AdapterExecutionTargetGeetorusBridgeHandle | null = null;
  ...
  return {
    ...
    async startTransport(): Promise<SandboxRunSiteTransport> {
      ...
      // dependency — the geetorus bridge's returned env must reach the
      ...
      const geetorusStart = options.measureBridgeStep("bridge.geetorus", () =>
        options.startGeetorusBridge(stagedRootDir),
      );
      // The single sequencing point (geetorus env → process-session launch),
      ...
      const finalizeLaunchEnv = (): Promise<Record<string, string>> =>
        (launchEnvPromise ??= (async () => {
          const geetorus = await geetorusStart;
          // The geetorus bridge token is run-scoped: it lives for this run only and
          ...
          if (geetorus) {
            contributions.push({ scope: "run", env: geetorus.env } as unknown as RunScopedContribution);
            await options.onGeetorusBridgeLog();
          }
      ...
      const [geetorus, processSession] = await Promise.allSettled([geetorusStart, processSessionStart]);
      controlBridge = geetorus.status === "fulfilled" ? geetorus.value : null;
      agentBridge = processSession.status === "fulfilled" ? processSession.value : null;
      const failure =
        geetorus.status === "rejected"
          ? geetorus.reason
    ...
    get controlBridge(): AdapterExecutionTargetGeetorusBridgeHandle | null {
