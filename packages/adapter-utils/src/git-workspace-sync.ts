...
export const PROJECT_REPOSITORIES_DIR = ".geetorus-repositories";
...
export const GIT_SYNC_COMMIT_IDENTITY_ARGS = [
  ...
  "user.name=Geetorus",
  "-c",
  "user.email=noreply@geetorus.ing",
] as const;
...
export async function withShallowGitWorkspaceClone<T>(
  ...
  const cloneDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-git-workspace-"));
  const tempRef = `refs/geetorus/git-sync/import/${randomUUID()}`;
  try {
...
export function createImportedGitRef(scope = "remote"): string {
  return `refs/geetorus/git-sync/imported/${scope}/${randomUUID()}`;
}

export function createRemoteGitExportRef(scope = "remote"): string {
  return `refs/geetorus/git-sync/export/${scope}/${randomUUID()}`;
}
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (noCommonAncestor) {
      ...
      const graftCommit = await createUnrelatedHistoryGraftCommit({
        ...
        syncLabel: "Geetorus remote git sync",
      });
      try {
    ...
    const mergeCommit = await runLocalGit(
      ...
      [
        ...
        input.importedHead,
        "-m",
        `Geetorus remote git sync merge ${input.importedHead.slice(0, 12)}`,
      ],
      {
        timeout: 60_000,
        maxBuffer: 64 * 1024,
