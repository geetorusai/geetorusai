...
import { resolveGeetorusInstanceRootForAdapter } from "./server-utils.js";
import {
...
describe("workspace restore merge", () => {
  ...
  it("round-trips a deterministic durable snapshot and rejects traversal", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "geetorus-snapshot-"),
    );
  ...
  it("preserves sibling files when sequential stale-baseline restores create the same nested directory tree", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
    cleanupDirs.push(rootDir);
  ...
  describe("describeWorkspaceRestoreFailure", () => {
    ...
    it("never reflects a sentinel host path or process id, however the caught error is classified", () => {
      ...
      const error: NodeJS.ErrnoException = new Error(
        `EACCES: permission denied, mkdir '${sentinelPath}.geetorus-restore.lock' (pid ${sentinelPid})`,
      );
      error.code = "EACCES";
  ...
  describe("instance-scoped directory merge lock", () => {
    // Points GEETORUS_HOME (and, where noted, GEETORUS_INSTANCE_ID) at a
    // temporary directory so the lock root never touches the real Geetorus
    ...
    function useTempGeetorusHome(homeDir: string, instanceId: string): void {
      previousHome = process.env.GEETORUS_HOME;
      previousInstanceId = process.env.GEETORUS_INSTANCE_ID;
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
        cleanupDirs.push(rootDir);
        useTempGeetorusHome(path.join(rootDir, "geetorus-home"), "test-instance");

        // The old lock sat beside the target, so it needed mkdir rights in the
        // target's parent. The new lock root lives under GEETORUS_HOME instead,
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
        cleanupDirs.push(rootDir);
        const geetorusHome = path.join(rootDir, "geetorus-home");
        useTempGeetorusHome(geetorusHome, "test-instance");
        ...
        const lockRootDir = path.join(geetorusHome, "instances", "test-instance", "locks", "directory-merge");
    ...
    it("rejects a lock root that already exists as a symlink", async () => {
      ...
      const locksDir = path.join(geetorusHome, "instances", "test-instance", "locks");
    ...
    it("closes the create/validate TOCTOU window: rejects a lock root a racing writer swapped for a symlink during creation", async () => {
      ...
      await mkdir(path.join(geetorusHome, "instances", "test-instance", "locks"), { recursive: true });
  ...
  describe("caller-provided env for the lock root", () => {
    ...
    it("two callers that pass the same env with a temporary GEETORUS_HOME take the same lock under that home", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
      cleanupDirs.push(rootDir);
      const explicitHome = path.join(rootDir, "explicit-home");
      const env: NodeJS.ProcessEnv = { GEETORUS_HOME: explicitHome, GEETORUS_INSTANCE_ID: "test-instance" };
    ...
    it("does not write a lock entry under process.env.GEETORUS_HOME when the caller passes its own env", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
      cleanupDirs.push(rootDir);
      const explicitHome = path.join(rootDir, "explicit-home");
      const env: NodeJS.ProcessEnv = { GEETORUS_HOME: explicitHome, GEETORUS_INSTANCE_ID: "test-instance" };
      ...
      const realInstanceRoot = resolveGeetorusInstanceRootForAdapter();
    ...
    it("resolves the lock root under the default instance id when the caller env sets GEETORUS_HOME but not GEETORUS_INSTANCE_ID, ignoring process.env.GEETORUS_INSTANCE_ID", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
      cleanupDirs.push(rootDir);
      const explicitHome = path.join(rootDir, "explicit-home");
      const env: NodeJS.ProcessEnv = { GEETORUS_HOME: explicitHome };

      const previousInstanceId = process.env.GEETORUS_INSTANCE_ID;
      process.env.GEETORUS_INSTANCE_ID = "wrong-instance";
      try {
        const targetDir = path.join(rootDir, "target");
        await mkdir(targetDir, { recursive: true });

        // The independent, no-caller-env resolution of "GEETORUS_HOME set,
        // GEETORUS_INSTANCE_ID unset" — the expected default instance id.
        const expectedInstanceRoot = resolveGeetorusInstanceRootForAdapter({ homeDir: explicitHome, env: {} });
    ...
    it("does not read process.env.GEETORUS_HOME when the caller env sets neither variable", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-restore-merge-"));
      cleanupDirs.push(rootDir);
      ...
      process.env.GEETORUS_HOME = fakeProcessHome;
      // Stand in for the real host home directory, so the "no env at all"
      // fallback lands under a temp dir instead of the real ~/.geetorus.
      ...
      try {
        ...
        const expectedInstanceRoot = resolveGeetorusInstanceRootForAdapter({ env: {} });
        const expectedLockRootDir = path.join(expectedInstanceRoot, "locks", "directory-merge");
