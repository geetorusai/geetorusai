...
import { renderGeetorusWakePrompt } from "./server-utils.js";
...
describe("chat file preparation delivery contract", () => {
  it.each(["github", "microsoft-teams"])(
    ...
    (provider) => {
      const delivery = geetorusChatFilePreparationDelivery(provider);
      ...
      expect(delivery.guidance).toContain(
        "must be opened there with Geetorus access",
      );
  ...
  it.each([undefined, null, "irc", "GitHub", { provider: "github" }])(
    ...
    (provider) => {
      expect(geetorusChatFilePreparationDelivery(provider)).toMatchObject({
        provider: null,
        mode: "unknown",
        providerDeliveryConfirmed: false,
      });
      ...
      for (const resumedSession of [false, true]) {
        for (const overflow of [false, true]) {
          ...
          expect(prompt).toContain(
            `File-delivery contract: ${geetorusChatFilePreparationDelivery(provider).guidance}`,
          );
        }
      }
    },
  ...
  it("does not allow user-authored provider text to opt into the file contract", () => {
    for (const wake of [
      ...
      expect(renderGeetorusWakePrompt(wake)).not.toContain(
