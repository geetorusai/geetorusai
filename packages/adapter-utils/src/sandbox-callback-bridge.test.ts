...
describe("sandbox callback bridge", () => {
    ...
    return {
      execute: async (input: {
        ...
        if (
          ...
          env.GEETORUS_TEST_STDIN = input.stdin;
          args[1] = `printf '%s' \"$GEETORUS_TEST_STDIN\" | (${args[1]})`;
        }
        try {
  ...
  it("round-trips localhost bridge requests over the sandbox queue without forwarding the bridge token", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-runtime-"));
    ...
    const queueDir = path.posix.join(prepared.runtimeRootDir, "geetorus-bridge");
    ...
    const okResponse = await fetch(`${bridge.baseUrl}/api/agents/me?view=compact`, {
      headers: {
        ...
        "x-geetorus-run-id": "run-bridge-1",
    ...
    expect(seenRequests[0]?.headers["x-geetorus-run-id"]).toBeUndefined();
    ...
    const githubResponse = await fetch(`${bridge.baseUrl}/runtime-tools/github/credentials`, {
      ...
      headers: {
        authorization: `Bearer ${bridgeToken}`,
        "content-type": "application/json",
        "x-geetorus-github-capability": "test-run-scoped-capability",
      },
      body: "{}",
    });
    ...
    expect(seenRequests[1]).toMatchObject({
      ...
      headers: { "x-geetorus-github-capability": "test-run-scoped-capability" },
    });
  ...
  it("serves schema discovery over the queue and denies schema mutations and lookalikes", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-schema-"));
    cleanupDirs.push(rootDir);
  ...
  it("denies non-allowlisted requests by default", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-default-policy-"));
    cleanupDirs.push(rootDir);
  ...
  it("drains already-queued requests on stop", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-drain-"));
    cleanupDirs.push(rootDir);
  ...
  it("writes fast 503 responses for queued requests that miss the drain deadline", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-drain-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("handles SSH queue polling failures without emitting an unhandled rejection", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-ssh-failure-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      const worker = await startSandboxCallbackBridgeWorker({
        client: {
          ...
          listJsonFiles: async () => {
            throw new Error(
              "list /remote/.geetorus-runtime/gemini/geetorus-bridge/queue/requests failed with exit code 255: kex_exchange_identification: read: Connection reset by peer",
            );
          },
          readTextFile: async () => {
  ...
  it("recovers from transient queue polling failures and keeps relaying requests", async () => {
    ...
    // failure window.
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-transient-"));
    cleanupDirs.push(rootDir);
  ...
  it("keeps the queue-directory setup on the startup step but resets the poll loop store", async () => {
    // The worker starts inside the measured `bridge.geetorus` step. Its awaited
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-step-store-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("test_geetorus_loop_exec_parents_to_run_context", async () => {
    // The worker starts inside the measured `bridge.geetorus` step. Its awaited
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-run-parent-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("wraps each request in a sandbox.callbackBridge.relayRequest span", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-relay-span-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("test_geetorus_loop_exec_stays_unparented_without_getter", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-no-getter-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("serializes remote response writes so stop does not recreate a late orphaned response", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-response-lock-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(
      readdir(directories.responsesDir).then((entries) =>
        entries.filter((entry) => entry.endsWith(".tmp") || entry.includes(".geetorus-write.lock")),
      ),
    ).resolves.toEqual([]);
  });

  it("rejects non-JSON request bodies and full queues at the bridge server", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-server-guards-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 when the host response times out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 for malformed host response files", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-malformed-response-"));
    cleanupDirs.push(rootDir);
  ...
  it("reuses an already-uploaded bridge entrypoint when the remote file hash matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-sync-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "geetorus-bridge-server.mjs"), "utf8")).resolves.toBe(expandedSource);
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
            entry.endsWith(".geetorus-upload.b64") ||
            entry.endsWith(".partial") ||
            entry === ".geetorus-bridge-upload.lock",
        ),
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("rejects a corrupted bridge entrypoint upload without committing a torn remote file", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-sync-corrupt-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "geetorus-bridge-server.mjs"), "utf8")).rejects.toThrow();
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
  ...
  // The process-session remote script is a static, Geetorus-authored `.mjs`
  ...
  it("test_process_session_script_skipped_when_remote_hash_matches: warm start with a matching remote hash writes 0 execs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-hashskip-warm-"));
    cleanupDirs.push(rootDir);
    const remoteDir = path.join(rootDir, "runtime", "codex", "process-sessions");
    const remotePath = path.posix.join(remoteDir, "geetorus-process-session-remote.mjs");
    const lockDir = path.posix.join(remoteDir, ".geetorus-process-session-script.lock");
      ...
      readdir(remoteDir).then((entries) =>
        entries.filter(
          (entry) =>
            entry.endsWith(".geetorus-upload.b64") ||
            entry.endsWith(".partial") ||
            entry === ".geetorus-process-session-script.lock",
        ),
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("test_process_session_script_rewritten_on_hash_mismatch: a mismatched remote hash still rewrites the script", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-hashskip-cold-"));
    cleanupDirs.push(rootDir);
    const remoteDir = path.join(rootDir, "runtime", "codex", "process-sessions");
    const remotePath = path.posix.join(remoteDir, "geetorus-process-session-remote.mjs");
    const lockDir = path.posix.join(remoteDir, ".geetorus-process-session-script.lock");
  ...
  it("fails loud when the hash-skip sync exec errors instead of silently re-uploading", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-hashskip-fail-"));
    cleanupDirs.push(rootDir);
    const remoteDir = path.join(rootDir, "runtime", "codex", "process-sessions");
    const remotePath = path.posix.join(remoteDir, "geetorus-process-session-remote.mjs");
    const lockDir = path.posix.join(remoteDir, ".geetorus-process-session-script.lock");
  ...
  it("permits the documented heartbeat surface and denies unrelated routes", () => {
    const allowed: Array<{ method: string; path: string }> = [
      ...
      // Hire skill (geetorus-create-agent): discovery + submit + issue linking
  ...
  it("marks command-managed bridge operations with the bridge execution channel", async () => {
    ...
    await client.makeDir("/workspace/.geetorus-runtime/codex/geetorus-bridge/queue");
  ...
  it("creates the bridge queue directories in one directory-creation exec", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-makedirs-"));
    cleanupDirs.push(rootDir);
  ...
  it("falls back to sequential makeDir when the queue client omits makeDirs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-makedir-fallback-"));
    cleanupDirs.push(rootDir);
  ...
  it("times out a stalled poll, surfaces a run-level error, and recovers to deliver the request", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-hang-"));
    cleanupDirs.push(rootDir);
  ...
  it("finalizes an aborted handler that ignores the signal and never settles with a non-retryable 504 backstop", async () => {
    ...
    expect(parsed.headers?.["x-geetorus-bridge-outcome"]).toBe("indeterminate");
    expect(responseWrites.some((write) => write.status === 503)).toBe(false);
    expect(responseWrites.some((write) => write.status === 502)).toBe(false);
  ...
  it("maps an indeterminate host outcome to a non-retryable 409 in the in-sandbox bridge server", () => {
    ...
    expect(source).toContain("x-geetorus-bridge-outcome");
  ...
  it("trips the watchdog on a stalled poll, writes a 503, and surfaces a run-level error", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-watchdog-"));
    cleanupDirs.push(rootDir);
  ...
  it("processes a fast request with no false-positive timeout and no run-level error", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-fast-"));
    cleanupDirs.push(rootDir);
  ...
  it("cleans up a timed-out request file and keeps serving after the host recovers", async () => {
    const fixture = await prepareGatewayFixture("geetorus-bridge-timeout-clean-");
  ...
  it("sweeps stale request files before rejecting at the queue-depth cap", async () => {
    const fixture = await prepareGatewayFixture("geetorus-bridge-stale-sweep-");
  ...
  it("exits fast when the gateway cannot bind its port instead of lingering un-ready", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-bind-fail-"));
    cleanupDirs.push(rootDir);
    const entrypoint = path.join(rootDir, "geetorus-bridge-server.mjs");
    ...
    const child = spawn(process.execPath, [entrypoint], {
      env: {
        ...
        GEETORUS_BRIDGE_TOKEN: "test-token",
        GEETORUS_BRIDGE_PORT: String(blockedPort),
      },
    ...
    expect(stderr).toContain("[geetorus-bridge] server error");
  ...
  it("exits nonzero for the retired duplex_v1 mode instead of starting the queue gateway", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-mode-duplex-"));
    cleanupDirs.push(rootDir);
    const entrypoint = path.join(rootDir, "geetorus-bridge-server.mjs");
    await writeFile(entrypoint, getSandboxCallbackBridgeServerSource(), "utf8");
    const queueDir = path.join(rootDir, "queue");
    ...
    const child = spawn(process.execPath, [entrypoint], {
      env: {
        ...process.env,
        GEETORUS_API_BRIDGE_MODE: "duplex_v1",
        GEETORUS_BRIDGE_QUEUE_DIR: queueDir,
        GEETORUS_BRIDGE_TOKEN: "test-token",
        GEETORUS_BRIDGE_PORT: "0",
      },
      stdio: ["ignore", "ignore", "pipe"],
    });
    let stderr = "";
    ...
    expect(stderr).toContain("Unsupported GEETORUS_API_BRIDGE_MODE: duplex_v1");
  }, 15_000);
  ...
  it("exits nonzero for an unknown bridge mode instead of starting the queue gateway", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-mode-unknown-"));
    cleanupDirs.push(rootDir);
    const entrypoint = path.join(rootDir, "geetorus-bridge-server.mjs");
    await writeFile(entrypoint, getSandboxCallbackBridgeServerSource(), "utf8");
    const queueDir = path.join(rootDir, "queue");
    ...
    const child = spawn(process.execPath, [entrypoint], {
      env: {
        ...process.env,
        GEETORUS_API_BRIDGE_MODE: "totally_unknown_mode",
        GEETORUS_BRIDGE_QUEUE_DIR: queueDir,
        GEETORUS_BRIDGE_TOKEN: "test-token",
        GEETORUS_BRIDGE_PORT: "0",
      },
    ...
    expect(exitCode).not.toBe(0);
    expect(stderr).toContain("Unsupported GEETORUS_API_BRIDGE_MODE: totally_unknown_mode");
  }, 15_000);
  ...
  it("test_http2_gateway_writes_no_frame_between_ready_and_the_preface", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-http2-gateway-"));
    cleanupDirs.push(rootDir);
    const entrypoint = path.join(rootDir, "geetorus-bridge-server.mjs");
    await writeFile(entrypoint, getSandboxCallbackBridgeServerSource(), "utf8");
  ...
  async function startHttp2GatewayForTest(options: {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-http2-test-"));
    cleanupDirs.push(rootDir);
    const entrypoint = path.join(rootDir, "geetorus-bridge-server.mjs");
    await writeFile(entrypoint, getSandboxCallbackBridgeServerSource(), "utf8");
    ...
    const child = spawn(process.execPath, [entrypoint], {
      env: {
        ...process.env,
        GEETORUS_API_BRIDGE_MODE: "http2_v1",
        GEETORUS_BRIDGE_TOKEN: options.bridgeToken,
        GEETORUS_BRIDGE_PORT: String(assignedPort),
        GEETORUS_BRIDGE_NONCE: "test-nonce",
        ...(options.maxBodyBytes != null
          ? { GEETORUS_BRIDGE_MAX_BODY_BYTES: String(options.maxBodyBytes) }
          : {}),
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
  ...
  it("maps an indeterminate host outcome to a non-retryable 409 on the HTTP/2 gateway for an unsafe method", async () => {
    ...
    const gateway = await startHttp2GatewayForTest({
      ...
      forwardRequest: async () => ({
        status: 504,
        headers: { "content-type": "application/json", "x-geetorus-bridge-outcome": "indeterminate" },
  ...
  async function startQueueGatewayForFileTest(options: {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), "geetorus-file-codec-"));
  ...
  it("rejects a request body over maxBodyBytes on the queue path before it writes the queue file", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-queue-maxbody-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("keeps the queue request payload's body field as a plain JSON string", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-bridge-queue-body-shape-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
