...
describe("sandbox adapter execution targets", () => {
  ...
  function encodeTailTick(stdout: Buffer, stderr: Buffer): string {
    return [
      "__GEETORUS_RUN_LOG_STDOUT__",
      stdout.toString("base64"),
      "__GEETORUS_RUN_LOG_STDERR__",
      stderr.toString("base64"),
      "__GEETORUS_RUN_LOG_END__",
      "",
    ].join("\n");
  }
  ...
  it("preserves stdin when wrapping sandbox adapter commands for run-log streaming", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-run-log-stdin-"));
    ...
    const logsDir = path.posix.join(rootDir, ".geetorus-runtime", "bridge", "logs");
  ...
  it("creates the process session directories only in the launch exec, not in upfront makeDir execs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-makedir-"));
    cleanupDirs.push(rootDir);
    ...
    const bridge = await startAdapterExecutionTargetProcessSessionBridge({
      ...
      target,
      runtimeRootDir: path.posix.join(rootDir, ".geetorus-runtime", "acpx"),
      adapterKey: "acpx",
  ...
  it.each([
    ...
    async ({ outputMode, streamOutputViaSession }) => {
      const rootDir = await mkdtemp(
        path.join(os.tmpdir(), `geetorus-process-session-${outputMode}-path-`),
      );
      cleanupDirs.push(rootDir);
  ...
  it("test_process_session_poll_exec_parents_to_run_context", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-poll-parent-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("test_process_session_poll_exec_stays_unparented_without_getter", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-poll-nogetter-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("test_process_session_stdin_exec_reads_send_time_run_parent", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stdin-parent-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
    ...
    const runner = {
      execute: async (input: Parameters<typeof delegate.execute>[0]) => {
        ...
        // The `.geetorus-upload` temp path under the `stdin` directory is unique
        // to the stdin-write path; the poll loop reads the `events` directory.
        const script = (input.args ?? []).join("\n");
        if (stdinWriteStep === "unset" && /\/stdin\/[^\s']*geetorus-upload/.test(script)) {
  ...
  it("wraps a stdin write in a sandbox.agentSession.sendInput span", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-sendinput-span-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("wraps each poll tick in a sandbox.agentSession.pollOutput span", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-poll-span-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("bridges bidirectional sandbox process sessions through a local ACPX-spawnable proxy", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      ...
      const report = await describeProxyRun(result, path.posix.join(rootDir, ".geetorus-runtime", "acpx"));
  ...
  it("buffers sandbox process session output until the local proxy connects", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-buffer-"));
    cleanupDirs.push(rootDir);
  ...
  it("delivers full output when the sandbox child exits immediately after writing", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-fast-exit-"));
    cleanupDirs.push(rootDir);
  ...
  it("ignores unauthenticated connections to the process session bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-auth-"));
    cleanupDirs.push(rootDir);
  ...
  it("streams sandbox process session output before the remote child exits", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-"));
    cleanupDirs.push(rootDir);
  ...
  describe("streamed output (streamOutputViaSession)", () => {
    it("bridges bidirectional sessions when the wrapper streams output to stdout", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-echo-"));
      cleanupDirs.push(rootDir);
    ...
    it("wraps the long-lived streamed launch in a sandbox.agentProcess span", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-span-"));
      cleanupDirs.push(rootDir);
      const childPath = path.join(rootDir, "echo-acp-child.mjs");
      await writeFile(
        childPath,
    ...
    it("parents the sandbox.agentProcess span to the live run root, not the bring-up step", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-parent-"));
      cleanupDirs.push(rootDir);
      const childPath = path.join(rootDir, "noop-acp-child.mjs");
      await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
      const target: AdapterSandboxExecutionTarget = {
    ...
    it("ends the sandbox.agentProcess span at stop() even when the process lingers", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-linger-"));
      cleanupDirs.push(rootDir);
    ...
    it("buffers streamed output until the local proxy connects", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-buffer-"));
      cleanupDirs.push(rootDir);
    ...
    it("delivers full streamed output when the sandbox child exits immediately", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-fast-exit-"));
      cleanupDirs.push(rootDir);
    ...
    it("streams live output before the child exits and never writes output event files", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-live-"));
      cleanupDirs.push(rootDir);
      ...
      try {
        ...
        const hasEventsDir = await readdir(
          path.posix.join(rootDir, ".geetorus-runtime", "acpx", "process-sessions"),
    ...
    it("keeps the agent command on the persistent session and forces bridge control execs off it", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-process-session-stream-isolation-"));
      cleanupDirs.push(rootDir);
      const childPath = path.join(rootDir, "echo-acp-child.mjs");
      await writeFile(
        childPath,
      ...
      try {
        ...
        expect(
          result.stdout,
          await describeProxyRun(result, path.posix.join(rootDir, ".geetorus-runtime", "acpx")),
        ).toBe("out:hello\n");
  ...
  it("treats SSH targets as bridge-only", () => {
    ...
    expect(adapterExecutionTargetUsesGeetorusBridge(target)).toBe(true);
    expect(adapterExecutionTargetSessionIdentity(target)).toEqual({
      transport: "ssh",
      host: "ssh.example.test",
      port: 22,
  ...
  it("starts a localhost Geetorus bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-geetorus-run-id"] === "string" ? req.headers["x-geetorus-run-id"] : null,
      });
    ...
    const bridge = await startAdapterExecutionTargetGeetorusBridge({
      runId: "run-bridge",
      target,
      runtimeRootDir,
      adapterKey: "codex",
    ...
    try {
      expect(bridge).not.toBeNull();
      expect(bridge?.env.GEETORUS_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.GEETORUS_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.GEETORUS_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.GEETORUS_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.GEETORUS_API_KEY}`,
  ...
  it("creates a sandbox run log tail factory when bridge streaming is enabled", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-stream-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("defaults sandbox run log streaming on and honors the explicit opt-out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-stream-default-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    const defaultBridge = await startAdapterExecutionTargetGeetorusBridge({
    ...
    const optOutBridge = await startAdapterExecutionTargetGeetorusBridge({
  ...
  it("tails sandbox run log chunks with byte offsets and dedupes the final batch", async () => {
    ...
    const tail = createSandboxRunLogTailFactory({
      runner,
      remoteCwd: "/workspace",
      logsDir: "/workspace/.geetorus-runtime/codex/geetorus-bridge/queue/logs",
      pollIntervalMs: 1,
    ...
    expect(runner.execute).toHaveBeenCalledWith(expect.objectContaining({
      command: "sh",
      cwd: "/workspace",
      env: { GEETORUS_SANDBOX_EXEC_CHANNEL: "bridge" },
      timeoutMs: 50,
    }));
  });
  ...
  it("delivers the final batch and a warning when run log polling degrades", async () => {
    ...
    expect(combinedStream(events, "stderr")).toBe(
      "final err\n[geetorus] Run log streaming degraded during the run; remaining output was delivered at completion.\n",
    );
  });

  it("exposes the Geetorus bridge to the sandbox shell surface", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-shell-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    try {
      expect(bridge).not.toBeNull();
      const shellProbe = [
        "const url = `${process.env.GEETORUS_API_URL}/api/agents/me`;",
        "fetch(url, { headers: { authorization: `Bearer ${process.env.GEETORUS_API_KEY}`, accept: 'application/json' } })",
        "  .then(async (response) => {",
        "    const body = await response.json();",
        "    process.stdout.write(JSON.stringify({",
        "      status: response.status,",
        "      body,",
        "      bridgeMode: process.env.GEETORUS_API_BRIDGE_MODE,",
        "    }));",
        "  })",
        "  .catch((error) => {",
      ...
      expect(`${result.stdout}\n${result.stderr}`).not.toContain(bridge!.env.GEETORUS_API_KEY);
      ...
      expect(runnerCommandText).not.toContain(bridge!.env.GEETORUS_API_KEY);
      const runtimeFiles = (await readRuntimeTextFiles(runtimeRootDir)).join("\n");
      expect(runtimeFiles).not.toContain("real-run-jwt");
      expect(runtimeFiles).not.toContain(bridge!.env.GEETORUS_API_KEY);
      expect(requests).toEqual([{
        method: "GET",
        url: "/api/agents/me",
        auth: "Bearer real-run-jwt",
  ...
  it("uses the effective adapter timeout when starting the sandbox callback bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-timeout-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("fails an oversized host response with a non-retryable 409 so a committed mutation never repeats", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    try {
      const response = await fetch(`${bridge!.env.GEETORUS_API_URL}/api/issues/issue-1/comments`, {
        method: "POST",
        headers: {
          authorization: `Bearer ${bridge!.env.GEETORUS_API_KEY}`,
          "content-type": "application/json",
      ...
      expect(response.headers.get("x-geetorus-bridge-outcome")).toBe("indeterminate");
  ...
  it("keeps an oversized host response for a safe method retryable so the read failure does not turn terminal", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-safe-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    try {
      const response = await fetch(`${bridge!.env.GEETORUS_API_URL}/api/issues/issue-1`, {
        method: "GET",
        headers: {
          authorization: `Bearer ${bridge!.env.GEETORUS_API_KEY}`,
        },
      ...
      expect(response.headers.get("x-geetorus-bridge-outcome")).toBeNull();
      await expect(response.json()).resolves.toEqual({
        error: "Bridge response body exceeded the configured size limit of 512 bytes.",
      });
      expect(requests).toEqual([{
  ...
  it("forwards the host indeterminate-outcome header so the sandbox server maps the 504 to a non-retryable 409", async () => {
    ...
    // `x-geetorus-bridge-outcome: indeterminate` header. The forward must keep
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-outcome-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    const apiServer = createServer((_req, res) => {
      res.writeHead(504, {
        "content-type": "application/json",
        "x-geetorus-bridge-outcome": "indeterminate",
      });
  ...
  it("forwards bridge traffic to the local listen origin even when public API URLs are configured", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-local-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    // exports the public origin via GEETORUS_RUNTIME_API_URL / GEETORUS_API_URL
    // and the local listen host/port via GEETORUS_LISTEN_HOST / GEETORUS_LISTEN_PORT.
    ...
    vi.stubEnv("GEETORUS_RUNTIME_API_URL", "https://public.example.invalid");
    vi.stubEnv("GEETORUS_API_URL", "https://public.example.invalid");
    vi.stubEnv("GEETORUS_LISTEN_HOST", "0.0.0.0");
    vi.stubEnv("GEETORUS_LISTEN_PORT", String(address.port));
  ...
  it("lets an explicit hostApiUrl input override the bridge forward target", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-execution-target-bridge-override-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    vi.stubEnv("GEETORUS_RUNTIME_API_URL", "https://public.example.invalid");
    vi.stubEnv("GEETORUS_API_URL", "https://public.example.invalid");
    vi.stubEnv("GEETORUS_LISTEN_HOST", "203.0.113.1");
    vi.stubEnv("GEETORUS_LISTEN_PORT", "9");
  ...
  function makeDuplexSelectionRunner(onOpen?: (ctx: DuplexOpenContext) => void): {
    ...
    const openDuplexChannel = async (openInput: {
      ...
      const nonce = /GEETORUS_BRIDGE_NONCE='([^']*)'/.exec(joined)?.[1] ?? "";
      const port = /GEETORUS_BRIDGE_PORT='([^']*)'/.exec(joined)?.[1] ?? "";
  ...
  function makeHttp2SelectionRunner(onOpen?: (ctx: Http2OpenContext) => void): {
    ...
    const openDuplexChannel = async (openInput: {
      ...
      const nonce = /GEETORUS_BRIDGE_NONCE='([^']*)'/.exec(joined)?.[1] ?? "";
      const port = /GEETORUS_BRIDGE_PORT='([^']*)'/.exec(joined)?.[1] ?? "";
      const bridgeToken = /GEETORUS_BRIDGE_TOKEN='([^']*)'/.exec(joined)?.[1] ?? "";
  ...
  function buildMultipartAttachmentUpload(fileBytes: Buffer): { body: Buffer; contentType: string } {
    const boundary = "geetorus-test-boundary";
  ...
  it("test_startup_failure_falls_back_to_queue_v1", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-startup-fail-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    ...
    const bridge = await startAdapterExecutionTargetGeetorusBridge({
      ...
      target,
      runtimeRootDir: path.join(remoteCwd, ".geetorus-runtime", "codex"),
      adapterKey: "codex",
      hostApiToken: "real-run-jwt",
    ...
    try {
      ...
      const uploadResponse = await fetch(
        `${bridge!.env.GEETORUS_API_URL}/api/companies/co-1/issues/issue-1/attachments`,
        {
          method: "POST",
          headers: {
            authorization: `Bearer ${bridge!.env.GEETORUS_API_KEY}`,
  ...
  it("test_daytona_selects_http2_v1", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-select-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    try {
      ...
      expect(bridge?.env.GEETORUS_API_BRIDGE_MODE).toBe("http2_v1");
  ...
  it("streams run logs on the http2 path under the same gate and log line as the file path", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-runlog-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("returns no run-log tail on the http2 path when streaming is opted out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-runlog-off-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("routes http2 channel-open and fallback records to a recorder attached on the server seam", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-recorder-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    const openBridge = await startAdapterExecutionTargetGeetorusBridge({
    ...
    try {
      expect(openBridge?.env.GEETORUS_API_BRIDGE_MODE).toBe("http2_v1");
    ...
    const fallbackBridge = await startAdapterExecutionTargetGeetorusBridge({
    ...
    try {
      expect(fallbackBridge?.env.GEETORUS_API_BRIDGE_MODE).toBe("queue_v1");
  ...
  it.each([
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-gate-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it.each([
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-fail-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it.each([
    ...
    async ({ buildReady }) => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-addr-"));
      cleanupDirs.push(rootDir);
      const remoteCwd = path.join(rootDir, "workspace");
      await mkdir(remoteCwd, { recursive: true });
      ...
      try {
        ...
        expect(bridge?.env.GEETORUS_API_BRIDGE_MODE).toBe("queue_v1");
        expect(bridge?.env.GEETORUS_API_URL).not.toContain(String(attackerPort));
        expect(control.closeCount + control.stopCount).toBeGreaterThanOrEqual(1);
  ...
  it("test_route_allowlist_header_cleanup_and_token_replacement_hold_on_http2", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-403-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_http2_forward_preserves_request_and_response_bytes", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-raw-bytes-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const sessionRef: { current: http2.ClientHttp2Session | null } = { current: null };
  ...
  it("records an http2 request span with latency and the fixed dimension keys", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-obs-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("increments the fallback counter with an approved reason when the capability is absent", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-fb-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it.each([
    ...
    async ({ error, expectedReason }) => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-stage-"));
      cleanupDirs.push(rootDir);
      const remoteCwd = path.join(rootDir, "workspace");
      await mkdir(remoteCwd, { recursive: true });
      const api = await startRecordingApiServer();
  ...
  it.each([
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-loss-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("keeps serving the request path when the telemetry recorder throws", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-guard-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("keeps sentinel route, query, body, and tokens off the http2 telemetry and logs", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-redact-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    const previousDebug = process.env.GEETORUS_BRIDGE_DEBUG;
    process.env.GEETORUS_BRIDGE_DEBUG = "1";
    let bridge: Awaited<ReturnType<typeof startAdapterExecutionTargetGeetorusBridge>> = null;
    try {
      bridge = await startAdapterExecutionTargetGeetorusBridge({
        runId: "run-redact",
        target,
        runtimeRootDir: path.join(remoteCwd, ".geetorus-runtime", "codex"),
        adapterKey: "codex",
      ...
      if (previousDebug === undefined) delete process.env.GEETORUS_BRIDGE_DEBUG;
      else process.env.GEETORUS_BRIDGE_DEBUG = previousDebug;
      sessionRef.current?.close();
      await bridge?.stop();
      await api.close();
    }
  ...
  it("maps a sentinel provider key to the constant other across every sink", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-prov-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("caps the pre-READY readiness buffer and falls back with a contaminated reason", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-cap-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("caps the pre-READY buffer under many small newline-less chunks", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-cap-small-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("bounds the pre-READY newline-scan work by the bytes received", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-scan-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("bounds the pre-READY buffer growth-copy work by the bytes received", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-growth-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("bounds the pre-READY skip scan work by the bytes received", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-blank-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_a_prologue_of_any_length_before_the_ready_line_is_discarded", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-noise-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    const { runner, control } = makeHttp2SelectionRunner((ctx) => {
      ctx.emitRaw("sh -c exec env GEETORUS_BRIDGE_NONCE=... node gateway.mjs\n");
  ...
  it("settles a wrong-nonce READY frame as a nonce mismatch, even after a noise line", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-noise-nonce-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("enforces the buffer cap on an over-cap blank prefix before it accepts a valid READY frame", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-capbypass-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("records the channel-open span with the fallback_reason dimension on the fallback path", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-duplex-openspan-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("drops a header outside the allowlist on the host http2 forward path", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-hdr-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("selects the http2 transport for a large forward budget", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-budget-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_a_missing_preface_aborts_the_open_and_falls_back_to_queue_v1", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-no-preface-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_disabled_flag_selects_queue_v1", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-disabled-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_fallback_to_queue_v1_happens_at_most_once_per_run", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-one-way-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_the_host_writes_no_byte_before_the_ready_line_is_accepted", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-no-early-write-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_invalid_ready_line_bytes_do_not_reach_a_log_line", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-invalid-ready-bytes-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_close_after_orderly_completion_keeps_the_run_result", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-orderly-close-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_safe_request_during_channel_loss_stays_retryable", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-safe-retry-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    try {
      ...
      expect(response.status).toBe(502);
      expect(response.headers["x-geetorus-bridge-outcome"]).toBeUndefined();
    } finally {
      sessionRef.current?.close();
      await bridge?.stop();
      await api.close();
  ...
  it("test_unsafe_request_during_channel_loss_is_indeterminate", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-unsafe-indeterminate-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    try {
      ...
      expect(response.headers["x-geetorus-bridge-outcome"]).toBe("indeterminate");
    } finally {
      sessionRef.current?.close();
      await bridge?.stop();
      await api.close();
  ...
  it("aborts the host forward and its response-body read when the sandbox stream closes", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-abort-forward-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const sessionRef: { current: http2.ClientHttp2Session | null } = { current: null };
  ...
  it("test_a_denied_response_chunk_cancels_the_reader_before_it_copies_the_chunk", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-denied-response-chunk-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const sessionRef: { current: http2.ClientHttp2Session | null } = { current: null };
  ...
  it("test_a_denied_concatenated_response_body_never_allocates_the_copy", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-denied-response-concat-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const sessionRef: { current: http2.ClientHttp2Session | null } = { current: null };
  ...
  it("test_a_denied_response_chunk_on_a_mutating_method_is_indeterminate_not_retryable", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-denied-response-mutation-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const sessionRef: { current: http2.ClientHttp2Session | null } = { current: null };
  ...
  it("test_concurrent_request_and_response_bodies_never_pass_the_process_ceiling", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-ceiling-aggregate-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const sessionRef: { current: http2.ClientHttp2Session | null } = { current: null };
  ...
  it("test_the_host_denies_a_body_over_the_resolved_limit_not_only_the_gateway", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-host-body-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const target: AdapterSandboxExecutionTarget = {
  ...
  it("test_the_queue_transport_still_forwards_with_no_reservation_owner", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-queue-no-reservation-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".geetorus-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    try {
      expect(bridge?.env.GEETORUS_API_BRIDGE_MODE).toBe("queue_v1");
      const response = await fetch(`${bridge!.env.GEETORUS_API_URL}/api/issues/abc/comments`, {
        method: "POST",
        headers: {
          authorization: `Bearer ${bridge!.env.GEETORUS_API_KEY}`,
          "content-type": "application/json",
  ...
  async function runReadinessReplay(emit: (ctx: Http2OpenContext) => void) {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-pty-replay-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
    ...
    const mode = bridge?.env.GEETORUS_API_BRIDGE_MODE;
    await bridge?.stop();
    await api.close();
    return { mode, control };
  }
  ...
  it("PTY replay: accepts READY after an echoed prompt and wrapper line", async () => {
    const { mode } = await runReadinessReplay((ctx) => {
      ctx.emitRaw(
        "daytona@212487a7f3c9:~$ exec 2>'/tmp/geetorus-duplex-x.log'; stty raw -echo; " +
          "exec 'bash' '-c' 'exec env GEETORUS_BRIDGE_NONCE=" + ctx.nonce + " node gateway.mjs'\r\n",
      );
  ...
  it("test_the_session_starts_at_the_client_preface_after_the_ready_line", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-preface-offset-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
  ...
  it("test_a_preface_pattern_before_the_ready_line_does_not_start_a_session", async () => {
    ...
    // that follows READY.
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-http2-preface-lookalike-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    await mkdir(remoteCwd, { recursive: true });
    const api = await startRecordingApiServer();
