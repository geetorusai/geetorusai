...
const catalogSkills = [
  ...
  {
    id: "geetorusai:bundled:geetorus-operations:task-planning",
...
describe("teams catalog manifest", () => {
  ...
  it("builds stable manifest entries from catalog team directories", async () => {
    ...
    expect(result.manifest.teams[0]!.requiredSkills).toEqual([
      expect.objectContaining({
        ...
        resolved: true,
        catalogSkillKey: "geetorusai/bundled/software-development/github-pr-workflow",
        agentSlugs: ["cto"],
      }),
    ]);
  ...
  it("reports frontmatter, directory, uniqueness, reference, and skill errors together", async () => {
    ...
    await writeTeam(packageDir, "bundled", "Bad_Category", "duplicate", {
      frontmatter: [
        "name: Duplicate",
        "schema: agentcompanies/v1",
        "key: geetorusai/bundled/software-development/other",
    ...
    expect(result.errors).toEqual(
      expect.arrayContaining([
        ...
        expect.stringContaining("key must be geetorusai/bundled/Bad_Category/duplicate"),
  ...
  it("detects stale generated manifests", async () => {
    ...
    await fs.writeFile(
      ...
      formatCatalogManifest({
        schemaVersion: 1,
        packageName: "@geetorusai/teams-catalog",
    ...
    expect(result.errors).toContain(
      "generated/catalog.json is stale. Run pnpm --filter @geetorusai/teams-catalog build:manifest.",
    );
  });
});
