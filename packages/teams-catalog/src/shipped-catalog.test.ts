...
const EXPECTED_BUNDLED_KEYS = [
  "geetorusai/bundled/company-defaults/core-exec-team",
  "geetorusai/bundled/product/product-design",
  "geetorusai/bundled/software-development/product-engineering",
];

const EXPECTED_OPTIONAL_KEYS = [
  "geetorusai/optional/content/content-machine",
];
...
describe("shipped teams catalog", () => {
  ...
  it("uses canonical geetorusai keys derived from kind/category/slug", () => {
    ...
    for (const team of catalogTeams) {
      const expectedKey = `geetorusai/${team.kind}/${team.category}/${team.slug}`;
      const expectedId = `geetorusai:${team.kind}:${team.category}:${team.slug}`;
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@geetorusai/teams-catalog");
  ...
  it("resolves shipped teams by id, key, and unique slug", () => {
    const sample = catalogTeams.find((team) => team.key === "geetorusai/bundled/company-defaults/core-exec-team");
