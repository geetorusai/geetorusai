...
const CATALOG_PACKAGE_NAME = "@geetorusai/teams-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @geetorusai/teams-catalog build:manifest.");
    }
  }
...
async function loadCatalogSkills(packageDir: string, errors: string[]): Promise<CatalogSkillSummary[]> {
  try {
    const catalogPackageName = "@geetorusai/skills-catalog";
    ...
    try {
      ...
      errors.push(`Could not load @geetorusai/skills-catalog for skill requirement validation: ${errorMessage(error)}`);
      return [];
    }
  }
}
...
async function buildCatalogTeam(
  ...
  const id = `geetorusai:${candidate.kind}:${candidate.category}:${candidate.slug}`;
  const key = `geetorusai/${candidate.kind}/${candidate.category}/${candidate.slug}`;
...
function resolveSkillRequirement(
  ...
  errors.push(`${prefix} skill reference "${ref}" does not resolve to a local team skill or @geetorusai/skills-catalog skill.`);
  return {
    type: "catalog",
    ref,
    agentSlugs: agentSlugs.sort(),
...
function classifyCatalogFile(relativePath: string): CatalogTeamFileKind {
  ...
  if (relativePath === ".geetorus.yaml") return "extension";
...
function collectCandidateUniquenessErrors(candidates: TeamCandidate[], errors: string[]) {
  const projected = candidates.map((candidate) => ({
    id: `geetorusai:${candidate.kind}:${candidate.category}:${candidate.slug}`,
    key: `geetorusai/${candidate.kind}/${candidate.category}/${candidate.slug}`,
    slug: candidate.slug,
