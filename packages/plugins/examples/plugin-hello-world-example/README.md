# @geetorusai/plugin-hello-world-example
...
- a minimal React widget rendered in the Geetorus dashboard
...
```bash
pnpm --filter @geetorusai/plugin-hello-world-example build
npx geetorusai plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `npx geetorusai plugin uninstall geetorus.hello-world-example --force` then  
  `npx geetorusai plugin install ./packages/plugins/examples/plugin-hello-world-example`.
