...
type GeetorusSprite = (typeof GEETORUS_SPRITES)[number];
...
function spriteSize(sprite: GeetorusSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = GEETORUS_SPRITES[(baseCol + baseRow) % GEETORUS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = GEETORUS_SPRITES[Math.floor(Math.random() * GEETORUS_SPRITES.length)]!;
