...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/absolute/path/to/geetorus/packages/plugins/examples/plugin-orchestration-smoke-example","isLocalPath":true}'
```
...
```bash
curl -X POST http://127.0.0.1:3100/api/plugins/geetorusai.plugin-orchestration-smoke-example/api/issues/<issue-id>/smoke \
  -H "Content-Type: application/json" \
