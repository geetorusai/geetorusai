...
Example Geetorus plugin that demonstrates:
...
```bash
pnpm --filter @geetorusai/plugin-file-browser-example build
npx geetorusai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
npx geetorusai plugin uninstall geetorus-file-browser-example --force
```
...
- Optional: use `geetorus-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
