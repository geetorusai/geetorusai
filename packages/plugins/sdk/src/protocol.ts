 ...
 * channel.
 *
 * The Geetorus plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@geetorusai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Geetorus messages.
 */
...
> {
   ...
   */
  readonly geetorusInvocation?: PluginInvocationContext;
  /** Opaque top-level invocation id echoed by worker→host requests. */
  readonly geetorusInvocationId?: string;
}
...
> {
   ...
   * Worker→host notifications echo only `geetorusInvocationId`.
   */
  readonly geetorusInvocation?: PluginInvocationContext;
...
/**
 * Geetorus plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Geetorus instance. */
    instanceId: string;
    /** Semver version of the running Geetorus host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
...
export interface PluginPerformActionActorContext {
  /** Authenticated principal type resolved by the Geetorus host. */
...
export interface PluginLoginPtyOpenParams {
   ...
   * `/tmp/geetorus-adapter-login/<uuid>`. The worker revalidates the shape
...
export interface WorkerToHostNotifications {
   ...
   * the envelope field `geetorusInvocationId`, which the worker RPC host stamps
