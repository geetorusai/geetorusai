/**
 * Core types for the Geetorus plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@geetorusai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @geetorusai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Geetorus object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Geetorus UI. */
  title?: string;
...
export interface PluginLocalFoldersClient {
  ...
  declarations(): import("@geetorusai/shared").PluginLocalFolderDeclaration[];
...
/**
 * `ctx.events` — subscribe to and emit Geetorus domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Geetorus domain event or a plugin-namespaced event.
   *
 ...
 * Geetorus secret provider system at execution time.
 *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed Geetorus routines.
 *
 ...
 * ```
 *
 * @see usePluginStream in `@geetorusai/plugin-sdk/ui`
 */
