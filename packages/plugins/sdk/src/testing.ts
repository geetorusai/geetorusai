...
function requireCapability(manifest: GeetorusPluginManifestV1, allowed: Set<PluginCapability>, capability: PluginCapability) {
 ...
 * can validate plugin behavior without spinning up the Geetorus server runtime.
 */
export function createTestHarness(options: TestHarnessOptions): TestHarness {
  ...
  function isManagedAgent(agent: Agent, agentKey: string) {
    const marker = agent.metadata?.geetorusManagedResource;
    return Boolean(
      marker
