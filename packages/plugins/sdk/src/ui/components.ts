 ...
 *
 * These components are exported from `@geetorusai/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@geetorusai/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function GeetorusSdkUiComponent(props: TProps) {
...
/**
 * Renders Geetorus's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders Geetorus's native issue list component for company-scoped plugin
...
/**
 * Renders Geetorus's native managed routines list for plugin settings pages.
 */
