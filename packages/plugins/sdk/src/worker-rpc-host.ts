...
import type { GeetorusPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@geetorusai/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: GeetorusPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@geetorusai/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
    ...
    return new Promise<WorkerToHostMethods[M][1]>((resolve, reject) => {
      ...
      try {
        ...
        const request = {
          ...
          ...(activeInvocation ? { geetorusInvocationId: activeInvocation.id } : {}),
        };
        sendMessage(request);
      } catch (err) {
  ...
  async function handleHostRequest(request: JsonRpcRequest): Promise<void> {
    ...
    try {
      ...
      const result = request.geetorusInvocation
        ? await invocationContextStorage.run(request.geetorusInvocation, invoke)
        : await invoke();
  ...
  function handleLine(line: string): void {
    ...
    if (isJsonRpcResponse(message)) {
      ...
      const runNotification = (fn: () => void | Promise<void>) => {
        if (notif.geetorusInvocation) {
          return invocationContextStorage.run(notif.geetorusInvocation, fn);
        }
        return fn();
      };
