# `@geetorusai/plugin-sdk`

Official TypeScript SDK for Geetorus plugin authors.

- **Worker SDK:** `@geetorusai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@geetorusai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@geetorusai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@geetorusai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@geetorusai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@geetorusai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@geetorusai/plugin-sdk/ui` | UI entry: hooks, host navigation, HTTP-safe clipboard copy, shared components, and slot prop types |
| `@geetorusai/plugin-sdk/ui/hooks` | Hooks only |
| `@geetorusai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@geetorusai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@geetorusai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@geetorusai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@geetorusai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@geetorusai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @geetorusai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Geetorus app. They can call ordinary Geetorus HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Geetorus instance at runtime.
...
- The host ships a small shared React component kit through `@geetorusai/plugin-sdk/ui`. Use it for native Geetorus controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@geetorusai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@geetorusai/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@geetorusai/plugin-sdk`.
...
metadata. Slow or failing plugins are isolated: Geetorus logs the failure and
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:geetorus.missions:feature",
...
await ctx.issues.createComment(issueId, replyText, companyId, {
  actorUserId: verifiedSlackUser.geetorusUserId,
});
```
...
```tsx
import { usePluginData, usePluginAction } from "@geetorusai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@geetorusai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@geetorusai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@geetorusai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@geetorusai/plugin-sdk/ui";
...
Routes Geetorus-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.

```tsx
import { useHostNavigation } from "@geetorusai/plugin-sdk/ui";
...
Avoid raw same-origin `href`s or `window.location.assign()` for Geetorus-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
```tsx
import { usePluginStream } from "@geetorusai/plugin-sdk/ui";
...
The host provides selected shared UI components through `@geetorusai/plugin-sdk/ui`.
...
```tsx
import { copyTextToClipboard } from "@geetorusai/plugin-sdk/ui";
...
used by Geetorus issue comments and documents:

```tsx
import { MarkdownBlock, MarkdownEditor } from "@geetorusai/plugin-sdk/ui";
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/geetorus|Geetorus]]."}
  enableWikiLinks
...
```tsx
import { FileTree, type FileTreeNode } from "@geetorusai/plugin-sdk/ui";
...
or configure work against Geetorus entities. Both are controlled components and
load their options from the host for the provided company.

```tsx
import { AssigneePicker, ProjectPicker } from "@geetorusai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@geetorusai/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@geetorusai/plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@geetorusai/plugin-sdk/bundlers";
...
```bash
geetorus-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@geetorusai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__geetorus__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__geetorus__/events` streams `reload` SSE events on UI build changes
