...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".geetorus-sdk");
...
/**
 * Generate a complete Geetorus plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Geetorus plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "geetorus-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["geetorus", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@geetorusai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@geetorusai/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@geetorusai/plugin-sdk/ui";
...
} from "@geetorusai/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@geetorusai/plugin-sdk/ui";
...
When this package is installed from a local path, Geetorus watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@geetorusai/plugin-sdk\` and \`@geetorusai/shared\` from a local Geetorus checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.geetorus-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Geetorus

\`\`\`bash
geetorusai plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
