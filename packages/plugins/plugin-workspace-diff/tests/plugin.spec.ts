...
async function createGitWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-workspace-diff-plugin-"));
  tempRoots.push(root);
  await fs.mkdir(path.join(root, "src"), { recursive: true });
  await git(root, ["init"]);
  await git(root, ["config", "user.email", "geetorus@example.com"]);
  await git(root, ["config", "user.name", "Geetorus Test"]);
