...
import type { PluginExecutionWorkspaceMetadata } from "@geetorusai/plugin-sdk";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-plugin-workspace-diff-"));
...
describe("plugin workspace diff service", () => {
  ...
  it("does not follow untracked symlinks outside the repo", async () => {
    const repoRoot = await createTempRepo();
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-plugin-workspace-diff-secret-"));
  ...
  it("surfaces missing cwd, non-git, invalid base refs, and unsafe path filters as plugin errors", async () => {
    ...
    const nonGitDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-plugin-workspace-diff-non-git-"));
