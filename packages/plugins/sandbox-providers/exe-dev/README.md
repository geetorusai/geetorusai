# `@geetorusai/plugin-exe-dev`

Published exe.dev sandbox provider plugin for Geetorus.

This package lives in the Geetorus monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That lets operators install it from the Plugins page by package name without introducing root lockfile churn.
...
- When you save an environment, Geetorus stores pasted API keys and pasted SSH private keys as company secrets.
...
- SSH access from the Geetorus host to the resulting `*.exe.xyz` VMs.
