...
// has passwordless sudo. The Geetorus sandbox callback bridge is a Node script, so
// every Geetorus workload on this provider needs node on PATH before the bridge can
// start. When the operator hasn't supplied their own setup script, install Node 24 via
// nodesource so the VM comes up ready for Geetorus out of the box.
...
function normalizeNamePrefix(value: string | null): string {
  const normalized = (value ?? "geetorus")
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-");
  return normalized.length > 0 ? normalized.slice(0, 24) : "geetorus";
}
...
async function prepareSshIdentity(config: ExeDevDriverConfig): Promise<{
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "geetorus-exe-dev-ssh-"));
...
function formatSshFailure(
  ...
  if (
    ...
    return [
      `Failed to ${action} exe.dev VM ${vmName}: the Geetorus host SSH key is not registered with exe.dev.`,
...
async function buildLease(
  ...
  const remoteCwd = requestedCwd?.trim() || path.posix.join(remote.homeDir, "geetorus-workspace");
