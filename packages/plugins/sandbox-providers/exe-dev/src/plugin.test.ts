...
describe("exe.dev sandbox provider plugin", () => {
  ...
  it("normalizes config and emits SSH guidance warnings", async () => {
    ...
    expect(result).toEqual({
      ok: true,
      warnings: [
        "The Geetorus host must have SSH access to the created exe.dev VM, and its SSH key must be registered with exe.dev. The API token only covers provisioning.",
  ...
  it("acquires a lease by creating a VM and preparing the SSH workspace", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({
        vm_name: "geetorus-env-run",
        ssh_dest: "geetorus-env-run.exe.xyz",
        https_url: "https://geetorus-env-run.exe.xyz",
        status: "running",
      }), { status: 200 }),
    );
    ...
    expect(lease).toMatchObject({
      providerLeaseId: "geetorus-env-run",
      metadata: {
        provider: "exe-dev",
        vmName: "geetorus-env-run",
        sshDest: "geetorus-env-run.exe.xyz",
  ...
  it("surfaces exe.dev SSH onboarding guidance during lease acquisition", async () => {
    ...
    await expect(plugin.definition.onEnvironmentAcquireLease?.({
      ...
      "the Geetorus host SSH key is not registered with exe.dev",
    );

    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'geetorus-env-run'");
  });
  ...
  it("surfaces invalid SSH key-format guidance during lease acquisition", async () => {
    ...
    queueSpawnResult({
      code: 255,
      stderr: 'Load key "/tmp/geetorus-exe-dev-ssh-abc/id_ed25519": invalid format\n',
    });
  ...
  it("returns exe.dev SSH onboarding guidance for command execution failures", async () => {
    ...
    expect(String(result?.stderr ?? "")).toContain("the Geetorus host SSH key is not registered with exe.dev");
  ...
  it("probes by creating and then deleting a VM after SSH verification", async () => {
    fetchMock
      .mockResolvedValueOnce(
        new Response(JSON.stringify({
          vm_name: "geetorus-probe",
          ssh_dest: "geetorus-probe.exe.xyz",
          status: "running",
        }), { status: 200 }),
      )
    ...
    expect(result).toMatchObject({
      ok: true,
      summary: "Connected to exe.dev VM geetorus-probe.",
      metadata: {
        provider: "exe-dev",
        vmName: "geetorus-probe",
        sshDest: "geetorus-probe.exe.xyz",
        shellCommand: "bash",
      },
    });
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'geetorus-probe'");
  });
  ...
  it("returns onboarding guidance when probe hits exe.dev SSH registration", async () => {
    ...
    expect(String(result?.metadata?.error ?? "")).toContain("the Geetorus host SSH key is not registered with exe.dev");
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'geetorus-probe'");
  });
  ...
  it("realizes a workspace by mkdir-ing the remote cwd over SSH when VM metadata is present", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: {
          sshDest: "vm-1.exe.xyz",
          remoteCwd: "/srv/geetorus/run-1",
        },
      },
      workspace: {
        localPath: "/local/geetorus",
    ...
    expect(sshCommand).toContain("/srv/geetorus/run-1");
    expect(result).toMatchObject({
      cwd: "/srv/geetorus/run-1",
      metadata: {
        provider: "exe-dev",
        remoteCwd: "/srv/geetorus/run-1",
      },
  ...
  it("falls back through workspace.remotePath then workspace.localPath when lease.metadata.remoteCwd is missing", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      workspace: {
        localPath: "/local/geetorus",
        remotePath: "/srv/geetorus/remote-fallback",
      },
    });

    expect(result?.cwd).toBe("/srv/geetorus/remote-fallback");
  });
  ...
  it("skips ensureRemoteWorkspace and returns the resolved cwd when no VM metadata is available", async () => {
    ...
    expect(result?.cwd).toBe("/srv/geetorus/no-vm");
  });
});
