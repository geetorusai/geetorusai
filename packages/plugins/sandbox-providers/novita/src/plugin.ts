...
export function parseNovitaDriverConfig(raw: Record<string, unknown>): NovitaDriverConfig {
  return {
    ...
    requestedCwd: parseOptionalString(raw.requestedCwd) ?? "/home/user/geetorus-workspace",
...
function buildStdinPath(): string {
  return `/tmp/.geetorus-stdin-${randomUUID()}`;
}
...
async function createSandbox(params: PluginEnvironmentAcquireLeaseParams | PluginEnvironmentProbeParams, config: NovitaDriverConfig) {
  const metadata = {
    "geetorus-provider": "novita",
    "geetorus-company-id": params.companyId,
    "geetorus-environment-id": params.environmentId,
    ...(params.issueId ? { "geetorus-issue-id": params.issueId } : {}),
    ...("runId" in params ? { "geetorus-run-id": params.runId } : {}),
  };
  const opts = {
    ...sandboxOpts(config),
    metadata,
