...
describe("Novita sandbox provider plugin", () => {
  ...
  it("parses defaults", () => {
    expect(parseNovitaDriverConfig({})).toMatchObject({
      ...
      domain: null,
      template: null,
      requestedCwd: "/home/user/geetorus-workspace",
      timeoutMs: 300_000,
  ...
  it("builds a quoted shell command with cwd, env, args, and stdin", () => {
    ...
    expect(command).toContain("printf '%s' 'input body' > '/tmp/.geetorus-stdin-");
    expect(command).toMatch(/< '\/tmp\/\.geetorus-stdin-[^']+'/);
    expect(command).toMatch(/rm -f '\/tmp\/\.geetorus-stdin-[^']+'/);
  ...
  it("uses a unique stdin path for each command", () => {
    ...
    const firstPath = first.match(/\/tmp\/\.geetorus-stdin-[^']+/)?.[0];
    const secondPath = second.match(/\/tmp\/\.geetorus-stdin-[^']+/)?.[0];

    expect(firstPath).toBeTruthy();
    expect(secondPath).toBeTruthy();
    expect(firstPath).not.toBe(secondPath);
    expect(first).not.toContain("/tmp/.geetorus-stdin <<");
    expect(second).not.toContain("/tmp/.geetorus-stdin <<");
  });
