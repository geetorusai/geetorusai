...
// Design note: no ingress rules are defined here. Geetorus-server does NOT
// push to agent pods — agents make outbound (egress) callbacks to
// geetorus-server on port 3100. If server→agent push is ever needed, add a
// targeted ingress rule scoped to the geetorus-server endpoint selector.
export function buildCiliumNetworkPolicyManifest(input: BuildCiliumNetworkPolicyInput): Record<string, unknown> {
  ...
  if (input.includeBaseRules !== false) egress.push({
    toEndpoints: [
      {
        matchLabels: {
          "k8s:io.kubernetes.pod.namespace": input.geetorusServerNamespace,
          app: "geetorus-server",
        },
      },
    ],
