export interface BuildNetworkPolicyInput {
  namespace: string;
  geetorusServerNamespace: string;
...
// Geetorus-server does NOT push to agent pods — the agent shim makes
// outbound calls to geetorus-server via the egress allow-list (port 3100).
...
// add a targeted ingress rule here scoped to the geetorus-server pod
// selector.
export function buildNetworkPolicyManifests(input: BuildNetworkPolicyInput): Record<string, unknown>[] {
  const denyAll = {
    ...
    kind: "NetworkPolicy",
    metadata: {
      name: "geetorus-deny-all",
      namespace: input.namespace,
      labels: { "geetorus.io/managed-by": "geetorus-k8s-plugin" },
    },
    spec: {
      podSelector: {},
  ...
  const egressAllow: Record<string, unknown> = {
    ...
    spec: {
      podSelector: { matchLabels: input.podSelector ?? { "geetorus.io/role": "agent" } },
      policyTypes: ["Egress"],
      egress: [
        ...
        ...(input.includeBaseRules === false ? [] : [{
          to: [
            {
              namespaceSelector: { matchLabels: { "kubernetes.io/metadata.name": input.geetorusServerNamespace } },
              podSelector: { matchLabels: { app: "geetorus-server" } },
            },
          ],
  ...
  (egressAllow.metadata as Record<string, unknown>).name = input.name ?? "geetorus-egress-allow";
