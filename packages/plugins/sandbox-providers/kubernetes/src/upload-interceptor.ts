 ...
 * `@geetorusai/adapter-utils`'s `command-managed-runtime.writeFile()`.
 *
 ...
 *
 * Where `<B64>` is `<TARGET>.geetorus-upload.b64`. Over k8s exec each call costs
...
const INIT_RE =
  /^mkdir -p '([^']+)' && rm -f '([^']+)\.geetorus-upload\.b64' && : > '\2\.geetorus-upload\.b64'$/;
const CHUNK_RE =
  /^printf '%s' '([A-Za-z0-9+/=]+)' >> '([^']+)\.geetorus-upload\.b64'$/;
const FINALIZE_RE =
  /^base64 -d < '([^']+)\.geetorus-upload\.b64' > '\1' && rm -f '\1\.geetorus-upload\.b64'$/;
...
/**
 * Stateful interceptor. Keyed by the base64 temp path (`<TARGET>.geetorus-upload.b64`).
...
export class FastUploadInterceptor {
  ...
  decide(command: string): InterceptDecision {
    ...
    if (initMatch) {
      ...
      const b64Path = `${targetPath}.geetorus-upload.b64`;
