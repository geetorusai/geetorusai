...
const PLUGIN_ID = "geetorus.kubernetes-sandbox-provider";
...
const manifest: GeetorusPluginManifestV1 = {
  ...
  description:
    "Built on kubernetes-sigs/agent-sandbox (v1alpha1). ALPHA — expect breaking changes as the upstream CRD evolves. Falls back to stable batch/v1 Job mode for clusters without agent-sandbox installed. First-party Geetorus sandbox-provider plugin for Kubernetes.",
  author: "Geetorus",
      ...
      configSchema: {
        ...
        properties: {
          inCluster: {
            type: "boolean",
            description:
              "When true, the plugin uses the in-pod ServiceAccount credentials. Requires geetorus-server to be running inside the target cluster.",
          },
          kubeconfig: {
            type: "string",
            format: "secret-ref",
            description:
              "Inline kubeconfig YAML. Paste a kubeconfig or an existing Geetorus secret reference; pasted values are stored as company secrets.",
          },
          namespacePrefix: {
            type: "string",
            description: "Prefix for the per-company tenant namespace (default: geetorus-).",
          },
          companySlug: {
            type: "string",
          ...
          imageRegistry: {
            type: "string",
            description: "Override the default registry for agent runtime images (default: ghcr.io/geetorusai).",
          },
          imageAllowList: {
            type: "array",
            items: { type: "string" },
