...
// The namespace geetorus-server itself runs in. Used when building
// NetworkPolicy manifests so the tenant namespace allows inbound traffic
// from the server pod.
const GEETORUS_SERVER_NAMESPACE = "geetorus";

// Name of the ServiceAccount created inside each tenant namespace by ensureTenant.
const TENANT_SERVICE_ACCOUNT = "geetorus-tenant-sa";
...
function generateBootstrapToken(): string {
  ...
  // callback auth scheme; geetorus-server's callback auth is out of scope for
...
const plugin = definePlugin({
  ...
  async onEnvironmentAcquireLease(
    ...
    // it in geetorus-server logs even if they missed the validation step.
    ...
    await ensureTenant(clients, {
      ...
      geetorusServerNamespace: GEETORUS_SERVER_NAMESPACE,
    ...
    adapterEnv.GEETORUS_NETWORK_EGRESS_POLICY = "kubernetes-default-deny";
    adapterEnv.GEETORUS_NETWORK_EGRESS_GRANT_PATH = NETWORK_EGRESS_GRANT_PATH;
    adapterEnv.GEETORUS_NETWORK_EGRESS_ALLOW_FQDNS = scopedNetworkEgress.allowFqdns.join(",");
    adapterEnv.GEETORUS_NETWORK_EGRESS_ALLOW_CIDRS = scopedNetworkEgress.allowCidrs.join(",");
  ...
  async onEnvironmentExecute(
    ...
    if (leaseBackend === "sandbox-cr") {
      ...
      // The container entrypoint is baked into the Job spec (Tini + geetorus-agent-shim).
