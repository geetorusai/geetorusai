...
const SERVICE_ACCOUNT_NAME = "geetorus-tenant-sa";
const ROLE_NAME = "geetorus-tenant-role";
const ROLE_BINDING_NAME = "geetorus-tenant-rb";
const RESOURCE_QUOTA_NAME = "geetorus-quota";
const LIMIT_RANGE_NAME = "geetorus-limits";
 ...
 * old geetorus-egress-allow NetworkPolicy in place alongside the new
    ...
    clients.core.createNamespace({
        body: {
          ...
          metadata: {
            ...
            labels: {
              "geetorus.io/company-id": input.companyId,
              "geetorus.io/managed-by": "geetorus-k8s-plugin",
...
async function ensureNetworkPolicies(clients: KubeClients, input: EnsureTenantInput): Promise<void> {
  const [denyAll, egressStd] = buildNetworkPolicyManifests({
    namespace: input.namespace,
    geetorusServerNamespace: input.geetorusServerNamespace,
