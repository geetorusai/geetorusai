...
describe("buildSandboxCrManifest", () => {
  ...
  it("applies provided labels to CR metadata and pod template labels (with role=agent added)", () => {
    const cr = buildSandboxCrManifest(baseInput);
    expect(cr.metadata.labels["geetorus.io/run-id"]).toBe("r1");
    expect(
      cr.spec.podTemplate.metadata.labels["geetorus.io/run-id"],
    ).toBe("r1");
    expect(cr.spec.podTemplate.metadata.labels["geetorus.io/role"]).toBe(
      "agent",
    );
  });
