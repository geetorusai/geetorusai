...
const baseInput = {
  ...
  image: "ghcr.io/geetorusai/agent-runtime-claude:v1",
  envSecretName: "r-01h00000000000000000000000-env",
  serviceAccountName: "geetorus-tenant-sa",
  labels: { "geetorus.io/run-id": "r1" },
...
describe("buildJobManifest", () => {
  ...
  it("wraps the entrypoint in tini for PID 1", () => {
    const job = buildJobManifest(baseInput);
    const container = job.spec.template.spec.containers[0];
    expect(container.command).toEqual(["/usr/bin/tini", "--", "/usr/local/bin/geetorus-agent-shim"]);
  });
  ...
  it("declares explicit writable emptyDir mounts for the standard agent paths", () => {
    ...
    expect(mountPaths).toEqual(["/home/geetorus", "/home/geetorus/.cache", "/tmp", "/workspace"]);
  ...
  it("applies the provided labels to both Job metadata and pod template", () => {
    const job = buildJobManifest(baseInput);
    expect(job.metadata.labels["geetorus.io/run-id"]).toBe("r1");
    expect(job.spec.template.metadata.labels["geetorus.io/run-id"]).toBe("r1");
    expect(job.spec.template.metadata.labels["geetorus.io/role"]).toBe("agent");
  });
});
