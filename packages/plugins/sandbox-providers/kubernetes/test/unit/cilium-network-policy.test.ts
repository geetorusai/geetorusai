...
describe("buildCiliumNetworkPolicyManifest", () => {
  ...
  it("targets agent pods by role label", () => {
    const cnp = buildCiliumNetworkPolicyManifest(baseInput);
    expect(cnp.spec.endpointSelector.matchLabels["geetorus.io/role"]).toBe("agent");
  });
  ...
  it("includes a rule for geetorus-server callback", () => {
    const cnp = buildCiliumNetworkPolicyManifest(baseInput);
    const cb = cnp.spec.egress.find((e: { toEndpoints?: { matchLabels: Record<string, string> }[] }) =>
      e.toEndpoints?.some((ep) => ep.matchLabels.app === "geetorus-server"),
    );
  ...
  it("targets only the granted run when building a scoped policy", () => {
    const cnp = buildCiliumNetworkPolicyManifest({
      ...baseInput,
      name: "pc-run-egress",
      endpointSelector: { "geetorus.io/run-id": "run-123" },
    ...
    expect(cnp.spec.endpointSelector.matchLabels).toEqual({ "geetorus.io/run-id": "run-123" });
