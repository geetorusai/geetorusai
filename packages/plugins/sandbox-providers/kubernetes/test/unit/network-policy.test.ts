...
describe("buildNetworkPolicyManifests", () => {
  const baseInput = {
    namespace: "geetorus-acme",
    geetorusServerNamespace: "geetorus",
  ...
  it("produces a deny-all + egress allow pair", () => {
    ...
    expect(manifests[0].metadata.name).toBe("geetorus-deny-all");
    expect(manifests[1].metadata.name).toBe("geetorus-egress-allow");
  });
  ...
  it("egress allow includes kube-dns and geetorus-server callback", () => {
    ...
    const geetorusRule = rules.find((r: { to: { namespaceSelector?: { matchLabels?: Record<string, string> } }[] }) =>
      r.to.some((t) => t.namespaceSelector?.matchLabels?.["kubernetes.io/metadata.name"] === "geetorus"),
    );
    expect(geetorusRule).toBeDefined();
  });
  ...
  it("uses geetorus-server pod label selector for callback ingress to geetorus ns", () => {
    const [, egress] = buildNetworkPolicyManifests(baseInput);
    const callbackRule = egress.spec.egress.find((r: { to: { podSelector?: { matchLabels?: Record<string, string> } }[] }) =>
      r.to.some((t) => t.podSelector?.matchLabels?.app === "geetorus-server"),
    );
  ...
  it("builds a task-scoped allow policy without namespace-wide base rules", () => {
    const [, egress] = buildNetworkPolicyManifests({
      ...baseInput,
      name: "pc-run-egress",
      podSelector: { "geetorus.io/run-id": "run-123" },
    ...
    expect(egress.spec.podSelector.matchLabels).toEqual({ "geetorus.io/run-id": "run-123" });
