...
describe("globMatch", () => {
  it("matches exact image", () => {
    expect(globMatch("ghcr.io/geetorusai/agent-runtime-claude:v1", "ghcr.io/geetorusai/agent-runtime-claude:v1")).toBe(true);
  });
  ...
  it("matches multi-character wildcard", () => {
    expect(globMatch("ghcr.io/geetorusai/*:v1", "ghcr.io/geetorusai/agent-runtime-claude:v1")).toBe(true);
    expect(globMatch("ghcr.io/geetorusai/*:v1", "docker.io/other/img:v1")).toBe(false);
  });

  it("does not allow wildcard to span slashes by default", () => {
    expect(globMatch("ghcr.io/*:v1", "ghcr.io/geetorusai/agent-runtime-claude:v1")).toBe(false);
  });
});

describe("resolveImage", () => {
  const defaults = { runtimeImage: "ghcr.io/geetorusai/agent-runtime-claude:v1" };
  ...
  it("uses adapter default when no override", () => {
    expect(resolveImage({ imageOverride: null }, defaults, { imageAllowList: [], imageRegistry: undefined })).toBe(
      "ghcr.io/geetorusai/agent-runtime-claude:v1",
    );
  });
  ...
  it("rewrites registry when imageRegistry is set", () => {
    expect(
      resolveImage(
        { imageOverride: null },
        defaults,
        { imageAllowList: [], imageRegistry: "registry.example.com/geetorus" },
      ),
    ).toBe("registry.example.com/geetorus/agent-runtime-claude:v1");
  });
