...
import { deriveCompanySlug, deriveNamespaceName, newRunUlidDns, geetorusLabels } from "../../src/utils.js";
...
describe("deriveNamespaceName", () => {
  it("concatenates prefix and slug", () => {
    expect(deriveNamespaceName("geetorus-", "acme-co")).toBe("geetorus-acme-co");
  });
});
...
describe("geetorusLabels", () => {
  it("returns canonical label map", () => {
    const labels = geetorusLabels({ runId: "r1", agentId: "a1", companyId: "c1", adapterType: "claude_local" });
    expect(labels["geetorus.io/run-id"]).toBe("r1");
    expect(labels["geetorus.io/agent-id"]).toBe("a1");
    expect(labels["geetorus.io/company-id"]).toBe("c1");
    expect(labels["geetorus.io/adapter"]).toBe("claude_local");
    expect(labels["geetorus.io/managed-by"]).toBe("geetorus-k8s-plugin");
  });
});
