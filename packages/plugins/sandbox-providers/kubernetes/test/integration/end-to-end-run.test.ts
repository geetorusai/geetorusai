 ...
 *        docker tag alpine:3.20 localhost/geetorus-agent:latest
 *        kind load docker-image localhost/geetorus-agent:latest --name geetorus
 ...
 *      And a tini-bearing image pre-loaded (e.g. the same localhost/geetorus-agent:latest
 ...
 * ("geetorus-"), resolving to "geetorus-spike-e2e".
 */
...
const NAMESPACE = "geetorus-spike-e2e";
...
describe("plugin-kubernetes end-to-end", () => {
  ...
  it.runIf(process.env.RUN_K8S_INTEGRATION_TESTS === "1")(
    ...
    async () => {
      ...
      expect(all).toContain("serviceaccount/geetorus-tenant-sa");
      expect(all).toContain("role.rbac.authorization.k8s.io/geetorus-tenant-role");
      expect(all).toContain("rolebinding.rbac.authorization.k8s.io/geetorus-tenant-rb");
      expect(all).toContain("resourcequota/geetorus-quota");
      expect(all).toContain("limitrange/geetorus-limits");
      expect(all).toContain("networkpolicy.networking.k8s.io/geetorus-deny-all");
      expect(all).toContain("networkpolicy.networking.k8s.io/geetorus-egress-allow");
