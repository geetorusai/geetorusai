...
import { decodeChannelBytes, definePlugin, NOOP_PLUGIN_TRACER } from "@geetorusai/plugin-sdk";
import type {
  PluginContext,
  PluginTracer,
...
// `createLoginPtyTransport` factory from `@geetorusai/adapter-utils` to
...
const WORKSPACE_SENTINEL_RELATIVE_PATH = ".geetorus-runtime/reusable-sandbox-lease.json";
...
function buildSandboxLabels(input: {
  ...
  return {
    ...
    "geetorus-company-id": input.companyId,
    "geetorus-environment-id": input.environmentId,
    "geetorus-reuse-lease": input.reuseLease ? "true" : "false",
    ...(input.runId ? { "geetorus-run-id": input.runId } : {}),
    ...(input.setupSessionId ? { "geetorus-setup-session-id": input.setupSessionId } : {}),
    ...(input.purpose ? { "geetorus-purpose": input.purpose } : {}),
  };
}
...
async function resolveSandboxWorkingDirectory(sandbox: Sandbox): Promise<string> {
  const root = (await sandbox.getWorkDir())?.trim()
    ...
    || "/home/daytona";
  const remoteCwd = path.posix.join(root, "geetorus-workspace");
...
function leaseMetadata(input: {
  ...
  return {
    ...
    ...(input.resumedLease
      ...
      : {}),
    // Record the resources Geetorus attempted to request so future diagnosis
...
// before the caller-requested deadline, even after a Geetorus crash or outage.
...
async function getOrCreateSession(sandbox: Sandbox, scope: SandboxScope): Promise<string> {
  ...
  return sandboxHandleSessionStore.runSingle(scope, async () => {
    const sessionId = `geetorus-${randomUUID()}`;
...
async function executeOneShot(
  ...
  const stdinPath = params.stdin != null ? `/tmp/geetorus-stdin-${randomUUID()}` : null;
...
const plugin = definePlugin({
  ...
  async onEnvironmentAcquireLease(
    ...
    try {
      ...
      // an abandoned sandbox self-destroys even if Geetorus is down. The lease
  ...
  async onEnvironmentRealizeWorkspace(
    ...
    const remoteCwd =
      ...
      params.lease.metadata.remoteCwd.trim().length > 0
        ...
        : params.workspace.remotePath ?? params.workspace.localPath ?? "/geetorus-workspace";
  ...
  async onEnvironmentCaptureTemplate(
    ...
    const templateRef = sanitizeSnapshotName(
      ...
      `geetorus-${params.environmentId}-${randomUUID().slice(0, 8)}`,
    );
