...
const PLUGIN_ID = "geetorus.daytona-sandbox-provider";
...
// 0.1.6 adds private authenticated WebSocket ingress for geetorus_runner.
...
const manifest: GeetorusPluginManifestV1 = {
  ...
  description:
    "First-party sandbox provider plugin that provisions Daytona sandboxes as Geetorus execution environments.",
  author: "Geetorus",
  ...
  environmentDrivers: [
    {
      ...
      // Geetorus bridge. So the login has no extra runtime prerequisite, and the
      ...
      configSchema: {
        ...
        properties: {
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Environment-specific Daytona API key. Paste a key or an existing Geetorus secret reference; saved environments store pasted values as company secrets. Falls back to DAYTONA_API_KEY if omitted.",
          },
          apiUrl: {
            type: "string",
            description:
          ...
          runnerLifecycleMode: {
            ...
            description:
              "geetorus_runner lifecycle for this environment. Inherit uses the agent setting; warm keeps runnerd and the sandbox available between turns.",
            default: "inherit",
          },
          runnerIdleTimeoutMs: {
            type: "integer",
            description:
              "How long an idle warm geetorus_runner stays alive before it checkpoints and suspends.",
            minimum: 1000,
            maximum: 86400000,
            default: 300000,
          },
