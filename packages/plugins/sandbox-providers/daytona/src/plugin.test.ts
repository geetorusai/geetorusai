...
import type { PluginContext } from "@geetorusai/plugin-sdk";
...
function createMockSandbox(overrides: {
  ...
  return {
    ...
    name: overrides.name ?? "geetorus-sandbox",
...
describe("Daytona sandbox provider plugin", () => {
  ...
  it("opens a duplex channel, forwards a host write, and closes it on lease release", async () => {
    ...
    try {
      ...
      const open = await plugin.definition.onDuplexChannelOpen?.({
        ...
        command: ["node", "/geetorus/gateway.mjs"],
      });
      ...
      expect(inputs[0]).toContain("exec 'node' '/geetorus/gateway.mjs'");
      expect(inputs[0]).toMatch(/2>'\/tmp\/geetorus-duplex-.+\.log'/);
  ...
  it("probes by creating and then deleting a sandbox", async () => {
    ...
    expect(sandbox.fs.createFolder).toHaveBeenCalledWith("/home/daytona/geetorus-workspace", "755");
  ...
  it("acquires a lease from a created sandbox", async () => {
    ...
    expect(lease).toMatchObject({
      ...
      metadata: {
        ...
        reuseLease: true,
        workspaceSentinel: {
          path: "/home/daytona/geetorus-workspace/.geetorus-runtime/reusable-sandbox-lease.json",
          result: "written",
        },
      },
    });
    expect(sandbox.fs.createFolder).toHaveBeenCalledWith(
      "/home/daytona/geetorus-workspace/.geetorus-runtime",
      "755",
    );
    expect(sandbox.fs.uploadFile).toHaveBeenCalledWith(
      expect.any(Buffer),
      "/home/daytona/geetorus-workspace/.geetorus-runtime/reusable-sandbox-lease.json",
      300,
    );
  });
  ...
  it("starts an interactive setup sandbox with redacted metadata and one-time SSH payload", async () => {
    ...
    expect(createParams).toMatchObject({
      ...
      labels: {
        "geetorus-provider": "daytona",
        "geetorus-setup-session-id": "setup-1",
        "geetorus-purpose": "interactive_setup",
      },
    });
  ...
  it("starts interactive setup from an image source when the environment is image-backed", async () => {
    ...
    expect(createParams).toMatchObject({
      ...
      labels: {
        "geetorus-provider": "daytona",
        "geetorus-setup-session-id": "setup-image-1",
        "geetorus-purpose": "interactive_setup",
      },
    });
  ...
  it("captures a Daytona snapshot from a live setup sandbox with redacted metadata", async () => {
    ...
    expect(sandbox._experimental_createSnapshot).toHaveBeenCalledWith("geetorus-env-1", 120);
    expect(result).toMatchObject({
      templateKind: "snapshot",
      templateRef: "geetorus-env-1",
      metadata: {
        provider: "daytona",
  ...
  describe("session model lifecycle (per-lease session store)", () => {
    ...
    it("creates one session on the first execute and reuses it on the next", async () => {
      ...
      expect(sessionId).toMatch(/^geetorus-/);
    });
    ...
    it("emits a session.open span on create and a session.close span on delete", async () => {
      ...
      try {
        ...
        expect(setup!.attributes["geetorus.sandbox.startup.provider"]).toBe("daytona");
        ...
        expect(teardown!.attributes["geetorus.sandbox.startup.provider"]).toBe("daytona");
      } finally {
        restore();
      }
    });
  ...
  it("stages stdin in the sandbox filesystem when execution needs redirected input", async () => {
    ...
    expect(sandbox.fs.uploadFile).toHaveBeenCalledWith(
      ...
      expect.stringMatching(/^\/tmp\/geetorus-stdin-/),
      1,
    );
    ...
    expect(command).not.toMatch(/nvm\.sh/);
    expect(command).toMatch(/&& cd '\/workspace'/);
    expect(command).toMatch(/env .* 'cat' < '\/tmp\/geetorus-stdin-/);
    expect(command).not.toMatch(/(?:^|&& )exec /);
    expect(sandbox.fs.deleteFile).toHaveBeenCalledWith(expect.stringMatching(/^\/tmp\/geetorus-stdin-/));
    expect(result).toMatchObject({
      exitCode: 0,
      timedOut: false,
    });
  ...
  it("runs the one-shot command plain with no bwrap or su wrapper", async () => {
    ...
    await plugin.definition.onEnvironmentExecute?.({
      ...
      config: { timeoutMs: 300000, reuseLease: false },
      bypassSession: true,
      lease: { providerLeaseId: "sandbox-123", metadata: { remoteCwd: "/home/daytona/geetorus-workspace" } },
      command: "printf",
      args: ["hello"],
      cwd: "/workspace",
      timeoutMs: 1000,
  ...
  describe("started-sandbox handle cache", () => {
    ...
    it("waits for an in-flight syncIn before interactive cancel cleanup starts", async () => {
      ...
      const remoteDir = "/home/daytona/geetorus-workspace";
    ...
    it("realizes a resumed lease after the provider fills in an unspecified target", async () => {
      ...
      await plugin.definition.onEnvironmentRealizeWorkspace!({
        ...
        workspace: { remotePath: "/home/daytona/geetorus-workspace" },
      });
      ...
      await expect(plugin.definition.onEnvironmentRealizeWorkspace!({
        ...
        workspace: { remotePath: "/home/daytona/geetorus-workspace" },
      })).resolves.toMatchObject({ cwd: "/home/daytona/geetorus-workspace" });
    });
    ...
    it("realizes the workspace from the acquire-seeded handle without a client.get", async () => {
      ...
      expect(mockGet).not.toHaveBeenCalled();
      expect(sandbox.fs.createFolder).toHaveBeenCalledWith("/home/daytona/geetorus-workspace", "755");
      expect(realize?.cwd).toBe("/home/daytona/geetorus-workspace");
    });
  });
});

describe("daytona native file-sync hooks", () => {
  const REMOTE_DIR = "/home/daytona/geetorus-workspace";
  ...
  it("records the writablePath destination of a staging-tar rw mapping, not the staging parent", async () => {
          ...
          files: [
            {
              ...
              sourcePath: source,
              targetPath: `${REMOTE_DIR}/.geetorus-runtime/workspace-upload.tar`,
              kind: "file" as const,
              access: "rw" as const,
    ...
    expect(recorded).not.toContain(`${REMOTE_DIR}/.geetorus-runtime`);
  });
      ...
      operations: [
        {
          ...
          files: [
            {
              sourcePath: roSource,
              targetPath: `${REMOTE_DIR}/.geetorus-runtime/project-proj-first`,
            ...
            },
            {
              ...
              targetPath: `${REMOTE_DIR}/.geetorus-runtime/default-upload.tar`,
              kind: "file" as const,
            },
          ],
        },
  ...
  it("syncIn coalesces file mappings into one uploadFiles batch to reserved temp destinations, then one batched mv, applying secret mode via setFilePermissions before the rename", async () => {
    ...
    for (const upload of uploads) {
      expect(path.posix.basename(upload.destination)).toMatch(/^\.geetorus-upload-/);
    ...
    expect(path.posix.basename(permPath)).toMatch(/^\.geetorus-upload-/);
  ...
  it("opens a transfer span with the guard round-trip count around the bulk file upload", async () => {
    ...
    expect(transfer!.attributes["geetorus.sandbox.startup.transfer.guard.count"]).toBe(1);
    expect(transfer!.attributes["geetorus.sandbox.startup.provider"]).toBe("daytona");
    expect(typeof transfer!.attributes["geetorus.sandbox.startup.transfer.wall_ms"]).toBe("number");
  ...
  it("marks the inbound transfer span with the inbound direction attribute", async () => {
    ...
    const transfer = spans.find((span) => span.name === "transfer");
    expect(transfer).toBeDefined();
    expect(transfer!.attributes["geetorus.sandbox.startup.transfer.direction"]).toBe("inbound");
  });
  ...
  it("marks the outbound transfer span with the outbound direction attribute", async () => {
    ...
    const transfer = spans.find((span) => span.name === "transfer");
    expect(transfer).toBeDefined();
    expect(transfer!.attributes["geetorus.sandbox.startup.transfer.direction"]).toBe("outbound");
  });
  ...
  it("opens a pack span and a transfer span around a directory mapping sync", async () => {
        ...
        operations: [
          {
            ...
            files: [
              { sourcePath: sourceDir, targetPath: `${REMOTE_DIR}/.geetorus-runtime/assets`, kind: "directory" },
            ],
          },
        ],
      });
    ...
    expect(typeof pack!.attributes["geetorus.sandbox.startup.pack.wall_ms"]).toBe("number");
  ...
  it("opens ensureDirectory, transfer, promote spans in call order for a file-mapping sync", async () => {
    ...
    for (const span of spans) {
      ...
      expect(span.attributes["geetorus.sandbox.startup.provider"]).toBe("daytona");
      ...
      if (span.name !== "transfer") {
        expect(span.attributes["geetorus.sandbox.startup.ensureDirectory.wall_ms"]).toBeUndefined();
        expect(span.attributes["geetorus.sandbox.startup.promote.wall_ms"]).toBeUndefined();
      }
    }
  });
  ...
  it("records a pack span for a traced directory mapping", async () => {
    ...
    expect(pack).toBeDefined();
    expect(pack!.ended).toBe(true);
    expect(pack!.attributes["geetorus.sandbox.startup.provider"]).toBe("daytona");
  });
  ...
  it("opens a postUploadCommand span for a post-upload command with a working directory", async () => {
    ...
    expect(provision!.attributes["geetorus.sandbox.startup.provider"]).toBe("daytona");
  });
  ...
  it("syncIn tars a directory mapping host-side honoring excludes and the followSymlinks flag, then extracts it in-sandbox via a single quoted tar command", async () => {
          ...
          files: [
            {
              ...
              targetPath: `${REMOTE_DIR}/.geetorus-runtime/assets`,
              kind: "directory",
              exclude: ["*.log"],
            },
          ],
    ...
    expect(path.posix.basename(uploads[0].destination)).toMatch(/^\.geetorus-upload-.*\.tar$/);
    ...
    const mkdirCall = sandbox.process.executeCommand.mock.calls.find(
      ([cmd]) =>
        ...
        String(cmd).includes(`'${REMOTE_DIR}/.geetorus-runtime/assets'`) &&
    ...
    expect(extractCommand).toContain(".geetorus-runtime/assets");
    expect(extractCommand).toContain("tar -xf");
    expect(extractCommand).toMatch(/rm -f .*\.geetorus-upload-.*\.tar/);
  });
  ...
  it("syncOut reads all file mappings via one downloadFiles batch, writes each to its host target, and returns per-operation counts", async () => {
    ...
    for (const req of requests) {
      expect(path.basename(req.destination)).toMatch(/^\.geetorus-upload-/);
      ...
      expect(path.posix.basename(req.source)).toMatch(/^\.geetorus-upload-/);
    }
  ...
  it("syncIn sweeps staged temps when the batched rename fails mid-promotion", async () => {
    ...
    // `.geetorus-upload-*` temps that the error path must sweep with `rm -f`.
    sandbox.process.executeCommand.mockImplementation(async (command: string) => {
    ...
    const cleanupCall = sandbox.process.executeCommand.mock.calls.find(
      ([cmd]) => String(cmd).includes("rm -f") && String(cmd).includes(".geetorus-upload-"),
    );
  ...
  it("stages a merged git-workspace operation as one uploadFiles batch and one rename exec, both extracts in order", async () => {
    ...
    const runtimeDir = `${REMOTE_DIR}/.geetorus-runtime/adapter`;
  ...
  it("runs two concurrent inbound syncIn calls with separate reserved scratch names", async () => {
    ...
    for (const destination of destinations) {
      expect(path.posix.basename(destination)).toMatch(/^\.geetorus-upload-/);
  ...
  it("runs two concurrent outbound syncOut calls that both reach downloadFiles before either opens", async () => {
    ...
    for (const source of sources) {
      ...
      expect(path.posix.basename(source)).toMatch(/^\.geetorus-upload-/);
    }
  ...
  it("opens a transfer span around a directory-mapping download with the guard round-trip count", async () => {
    ...
    // tar.
    expect(transfer!.attributes["geetorus.sandbox.startup.transfer.guard.count"]).toBe(2);
    expect(typeof transfer!.attributes["geetorus.sandbox.startup.transfer.wall_ms"]).toBe("number");
  });
});
