...
const SPAN_ATTR_PREFIX = "geetorus.sandbox.startup.";
const SPAN_ATTR = {
...
// The runtime's base64 fallback stages to `<path>.geetorus-upload`; the native
...
const SCRATCH_PREFIX = ".geetorus-upload";
...
async function withHostTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-daytona-sync-"));
  try {
    return await fn(dir);
  } finally {
...
const ZSTD_PROBE_MARKER = "GEETORUS_ZSTD_AVAILABLE";
...
async function compressFileToHostTemp(sourcePath: string): Promise<{ dir: string; path: string; bytesOut: number }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-daytona-zstd-"));
 ...
 * so a failed transfer never accumulates `.geetorus-upload-*` scratch in the
...
async function syncInFileMappings(input: {
  ...
  // any error so a retry never accumulates stale `.geetorus-upload-*` scratch.
...
async function syncInDirectoryMapping(input: {
  ...
  return withHostTempDir(async (tmp) => {
    ...
    // `.geetorus-upload-*` entry. The extract script below removes it only on
    // success. On an upload or extract failure the scratch tar can remain, and the
    // runtime workspace wipe preserves every `.geetorus-upload-*` entry, so a
