...
const GATEWAY = ["node", "/geetorus/gateway.mjs"];
...
const GATEWAY_QUOTED = "'node' '/geetorus/gateway.mjs'";
...
describe("buildDuplexChannelLaunchWrapper", () => {
  ...
  it("quotes each command argument that holds shell metacharacters", () => {
    const wrapper = buildDuplexChannelLaunchWrapper(
      ["node", "/geetorus/gateway.mjs", "; rm -rf ~"],
      "/tmp/diag.log",
    );
    ...
    expect(wrapper).toContain(`exec 'node' '/geetorus/gateway.mjs' '; rm -rf ~'`);
    expect(wrapper).not.toContain("; rm -rf ~;");
    expect(wrapper).not.toContain("exec node /geetorus/gateway.mjs ; rm -rf ~");
  });
...
describe("openDaytonaDuplexChannelSession", () => {
  ...
  it("redirects diagnostics to the given file path", async () => {
    ...
    await openDaytonaDuplexChannelSession(process, GATEWAY, {
      diagnosticsPath: "/tmp/geetorus-duplex-fixed.log",
    });

    expect(process.handle?.inputs[0]).toContain("2>'/tmp/geetorus-duplex-fixed.log'");
  });
  ...
  it("defaults the diagnostics path under /tmp when the caller gives none", async () => {
    ...
    expect(process.handle?.inputs[0]).toMatch(/2>'\/tmp\/geetorus-duplex-[0-9a-f-]+\.log'/);
  });
  ...
  it("passes the working directory to the pseudo-terminal", async () => {
    ...
    await openDaytonaDuplexChannelSession(process, GATEWAY, { cwd: "/geetorus-workspace" });

    expect(process.createOptions?.cwd).toBe("/geetorus-workspace");
  });
...
describe("createDaytonaDuplexChannelSessionOpener", () => {
  it("opens a session for the command on each call", async () => {
    const process = createFakeProcess();
    const opener = createDaytonaDuplexChannelSessionOpener(process, { cwd: "/geetorus-workspace" });
