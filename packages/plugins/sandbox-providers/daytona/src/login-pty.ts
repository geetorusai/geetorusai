...
// authority to call the Geetorus API. The sandbox contract in
...
// `@geetorusai/adapter-utils`, so the transport factory `createLoginPtyTransport`
...
const LOGIN_SESSION_HOME_ROOT = "/tmp/geetorus-adapter-login";
 ...
 * `@geetorusai/adapter-utils`, so the transport factory there accepts a session
...
export async function openDaytonaLoginPtySession(
  ...
  const handle = await process.createPty({
    id: `geetorus-login-pty-${randomUUID()}`,
 ...
 * to call the Geetorus API. The real command lands against a live sandbox; a
