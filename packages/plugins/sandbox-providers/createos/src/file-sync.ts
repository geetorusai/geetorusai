...
import type { PluginEnvironmentSyncInParams, PluginEnvironmentSyncResult } from "@geetorusai/plugin-sdk";
import { CreateosClient, identifier } from "./client.js";
import { execute, shellQuote } from "./execute.js";

const ROOT = "/geetorus-workspace";
...
export async function syncFiles(
  ...
  for (const operation of params.operations) {
    ...
    for (const mapping of operation.files) {
      ...
      const scratch = `/tmp/geetorus-createos-transfer-${randomUUID()}`;
      ...
      const temp = await fs.mkdtemp(path.join(parent, ".geetorus-createos-"));
