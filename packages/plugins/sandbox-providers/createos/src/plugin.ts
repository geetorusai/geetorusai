...
const CWD = "/geetorus-workspace";
// The host excludes .geetorus-runtime from workspace export, so the lease
// marker never becomes a user repository file.
const MARKER = `${CWD}/.geetorus-runtime/.geetorus-createos-lease`;
    ...
    async onEnvironmentProbe(params) {
      ...
      try {
        ...
        const result = await execute(new CreateosClient(parseConfig(params.config)), {
          ...params, lease, command: "/bin/echo", args: ["geetorus-createos-ready"], cwd: CWD,
        }, AbortSignal.timeout(parseConfig(params.config).timeoutMs));
        if (result.timedOut || result.exitCode !== 0 || !result.stdout.includes("geetorus-createos-ready")) throw new Error("CreateOS command probe failed.");
