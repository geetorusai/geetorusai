...
const manifest: GeetorusPluginManifestV1 = {
  id: "geetorus.createos-sandbox-provider",
  apiVersion: 1,
  version: "0.1.0",
  displayName: "CreateOS Sandbox Provider",
  description: "Runs Geetorus agents in CreateOS sandboxes.",
  author: "CreateOS",
  ...
  environmentDrivers: [{
    ...
    configSchema: {
      ...
      properties: {
        ...
        apiKey: { type: "string", format: "secret-ref", description: "CreateOS API key or Geetorus secret reference. Saved keys become company secrets. The official API endpoint can use CREATEOS_API_KEY from the host; custom endpoints require an explicit key." },
        shape: {
          type: "string",
          title: "Shape",
