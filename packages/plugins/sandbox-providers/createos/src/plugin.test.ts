...
function provider() {
  ...
  const fetchMock = vi.fn(async (url: string | URL | Request, init: RequestInit = {}) => {
    ...
    if (parsed.pathname.endsWith("/files") && method === "PUT") {
      if (parsed.searchParams.get("path")?.endsWith(".geetorus-createos-lease")) marker = raw;
      return success();
    }
    ...
    if (parsed.pathname.endsWith("/connect")) return ndjson([data(1, "geetorus-createos-ready\n"), { type: "exit", exit_code: 0 }]);
...
describe("CreateOS lifecycle", () => {
  it("acquires, executes, realizes, pauses, resumes, and destroys using public API envelopes", async () => {
    ...
    expect(await hooks.onEnvironmentRealizeWorkspace!({ ...params, lease, workspace: { localPath: "/private/host/path" } })).toMatchObject({ cwd: "/geetorus-workspace" });
