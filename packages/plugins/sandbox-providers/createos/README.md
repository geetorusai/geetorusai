# `@geetorusai/plugin-createos`

CreateOS sandbox provider for Geetorus. This package lives alongside Daytona
...
Requires Node 24.11+, pnpm, and an installed Geetorus checkout.
...
From the Geetorus checkout, with your instance running:

```sh
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts plugin install /absolute/path/to/geetorus/packages/plugins/sandbox-providers/createos
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts plugin list
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts plugin inspect geetorus.createos-sandbox-provider
```
...
```sh
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts plugin disable geetorus.createos-sandbox-provider
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts plugin enable geetorus.createos-sandbox-provider
```

The plugin uses the same package entrypoints and publish manifest helper as
other sandbox providers. Plugin workers are trusted code on the Geetorus host.
...
- `apiKey`: your CreateOS key. Geetorus saves pasted keys as company secrets;
...
- Workspace realization at `/geetorus-workspace` and native binary file sync,
...
Using it additionally requires a reachable Geetorus runner endpoint and the
