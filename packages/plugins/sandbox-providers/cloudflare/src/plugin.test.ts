...
describe("Cloudflare sandbox provider plugin", () => {
  ...
  it("maps acquire lease responses from the bridge", async () => {
    ...
    const lease = await plugin.definition.onEnvironmentAcquireLease?.({
      ...
      issueId: "issue-1",
      runId: "run-1",
      requestedCwd: "/workspace/geetorus",
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://bridge.example.workers.dev/api/geetorus-sandbox/v1/leases/acquire",
      expect.objectContaining({
        method: "POST",
        headers: expect.any(Headers),
      }),
    );
    expect(requestHeadersAt().get("X-Geetorus-Run-Id")).toBe("run-1");
    expect(requestHeadersAt().get("X-Geetorus-Environment-Id")).toBe("env-1");
    expect(requestHeadersAt().get("X-Geetorus-Issue-Id")).toBe("issue-1");
  ...
  it("defaults the sleepAfter passed to the bridge to 1h so long runs don't idle out", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        providerLeaseId: "pc-run-1-abcd1234",
        metadata: { provider: "cloudflare", remoteCwd: "/workspace/geetorus", resumedLease: false },
      }),
    );
  ...
  it("returns expired lease semantics when resume reports lost state", async () => {
    ...
    const lease = await plugin.definition.onEnvironmentResumeLease?.({
      ...
      leaseMetadata: { remoteCwd: "/workspace/geetorus" },
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
        bridgeAuthToken: "resolved-token",
      },
  ...
  it("routes bridge-channel execute calls through a dedicated session", async () => {
    ...
    expect(requestBodyAt()).toMatchObject({
      ...
      sessionId: "geetorus-bridge",
      env: {
        KEEP_ME: "visible",
      },
    });
    expect(requestBodyAt().env).not.toHaveProperty("GEETORUS_SANDBOX_EXEC_CHANNEL");
  ...
  it("wraps realizeWorkspace bridge failures and forwards the issue header", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        providerLeaseId: "pc-run-1-abcd1234",
        metadata: { remoteCwd: "/workspace/geetorus" },
      },
      workspace: {
    ...
    })).rejects.toThrow("Failed to prepare Cloudflare sandbox workspace at /workspace/geetorus: mkdir: permission denied");
