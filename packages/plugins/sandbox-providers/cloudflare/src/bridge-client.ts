...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-Geetorus-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-Geetorus-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-Geetorus-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/geetorus-sandbox/v1";
