...
This Worker is the operator-facing bridge used by `@geetorusai/plugin-cloudflare-sandbox`.

It exposes a small authenticated JSON API under `/api/geetorus-sandbox/v1` and translates Geetorus lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so Geetorus does not inherit wedged background processes
...
After deploy, configure Geetorus with:
