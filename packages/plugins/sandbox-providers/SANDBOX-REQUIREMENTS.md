...
must meet this contract. The Geetorus runtime does not build the environment at
...
Geetorus runs agent work inside a sandbox. Geetorus protects the host from
the code in that sandbox. This section states the rule for that protection. It
does not state the implementation.

A sandbox is an untrusted execution environment. Geetorus assumes that a
sandbox process can read or change all accessible data.

Geetorus does not protect sandbox files, processes, credentials, or code from
...
Geetorus protects two authorities across the boundary:

- The authority to write host files
- The authority to call the Geetorus API

Sandbox code holds each authority through one surface only:

1. **Outbound workspace synchronization.** Geetorus copies sandbox files to
   host paths that the Geetorus orchestrator selects.
2. **The Geetorus HTTP bridge.** Sandbox code calls the Geetorus API through
   the bridge.
...
This repository does not enforce these provider rules today, and Geetorus
...
Geetorus does not enforce this policy inside the sandbox.
...
- The change lets sandbox code write host files outside workspace
  synchronization.
- The change lets sandbox code call the Geetorus API outside the HTTP bridge.
...
Geetorus or the provider must enforce each boundary control outside the
...
### Surface 2: Geetorus HTTP bridge

Sandbox code must call the Geetorus API only through the HTTP bridge.
...
Geetorus sends commands from the host to the sandbox. Command execution can
return output to the host. This output does not give sandbox code authority to
write host files or call the Geetorus API.
...
Geetorus probes each CLI before launch. Geetorus uses the same detection
...
Geetorus probes each CLI with `command -v <cmd>`. Geetorus fails loudly when
...
- The Geetorus runtime never modifies the login profile. The runtime never
  writes a profile file. The runtime never writes an rc file.
- The Geetorus runtime never sources `nvm` on the exec path.
