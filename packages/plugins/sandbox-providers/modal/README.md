...
Geetorus and this plugin require **Node 24 or later**. The plugin logs a startup warning when it detects an older host runtime.
...
| `tokenId` / `tokenSecret` | yes | Modal auth tokens. Both must be provided together. Geetorus stores pasted values as company secrets. The plugin worker runs in a child process that does not inherit host env vars, so `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` set on the Geetorus server are **not** read by the plugin — provide the tokens in this form. |
| `environment` | no | Optional Modal environment name. Falls back to the SDK profile default. |
| `workdir` | no | Remote working directory inside the sandbox. Defaults to `/workspace/geetorus`. |
...
These commands assume the repo root has already been installed once so the local `@geetorusai/plugin-sdk` workspace package is available to the compiler during development.

## Operator verification

1. Provision Modal credentials in your Modal account (`modal token new`) or use a service account.
2. Install the plugin from the Geetorus Plugins page.
...
5. Run at least one Geetorus task with a remote-managed adapter (for example `claude_local`) bound to that environment. The adapter should provision the sandbox, run commands in it, and clean it up.
...
- `geetorusPlugin.manifest` and `geetorusPlugin.worker` point the host at the built plugin entrypoints in `dist/`
