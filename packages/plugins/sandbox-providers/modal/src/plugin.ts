...
const DEFAULT_WORKDIR = "/workspace/geetorus";
...
function resolveAuth(config: ModalDriverConfig): { tokenId: string; tokenSecret: string } | null {
  ...
  // environment config, which Geetorus stores as company secrets.
...
function warnIfUnsupportedNode(logger: { warn: (msg: string) => void } | undefined): void {
  ...
  if (Number.isFinite(major) && major < 24) {
    const message = `Modal sandbox provider is running on Node ${process.versions.node}; Geetorus requires Node 24+. Upgrade the host runtime before using this plugin.`;
    logger?.warn(message);
  }
}
  ...
  async onEnvironmentProbe(
    ...
    try {
      ...
      try {
        ...
        const proc = await sandbox.exec(["sh", "-lc", "printf geetorus-probe"]);
        const { stdout, exitCode } = await readProcessStreams(proc);
        if (exitCode !== 0 || stdout.trim() !== "geetorus-probe") {
          return {
            ok: false,
  ...
  async onEnvironmentExecute(
    ...
    try {
      ...
      const stdinPath = params.stdin != null
        ? `/tmp/geetorus-stdin-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
        : null;
      try {
