...
const EXPANDED_STORAGE_KEY = `geetorusai.plugin-llm-wiki:route-sidebar-expanded:v2:${COMPANY_ID}`;
...
describe("WikiRouteSidebar", () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as BridgeGlobal).__geetorusPluginBridge__ = {
      sdkUi: {
  ...
  afterEach(() => {
    ...
    container.remove();
    window.localStorage.clear();
    delete (globalThis as BridgeGlobal).__geetorusPluginBridge__;
  });
  ...
  it("keeps sidebar tree selection scoped to sidebar navigation", () => {
    ...
    expect(navigatedTo).toEqual({
      ...
      options: { state: { geetorusWikiSidebarTreePath: "wiki/concepts/sidebar-navigation.md" } },
    });
    ...
    hostLocation = {
      pathname: "/PAP/wiki/page/wiki/entities/geetorus.md",
      search: "",
      hash: "",
    };
    act(() => {
...
describe("WikiPage", () => {
  ...
  it("renders structured Geetorus source refs as text", () => {
    act(() => {
