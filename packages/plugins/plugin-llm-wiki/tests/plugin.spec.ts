...
import { createTestHarness } from "@geetorusai/plugin-sdk/testing";
import type { Agent, Issue, PluginManagedRoutineResolution, Project } from "@geetorusai/plugin-sdk";
import manifest, {
  CURSOR_WINDOW_ROUTINE_KEY,
  INDEX_REFRESH_ROUTINE_KEY,
  NIGHTLY_LINT_ROUTINE_KEY,
  GEETORUS_DISTILL_SKILL_KEY,
...
const ORIGINAL_DEPLOYMENT_MODE = process.env.GEETORUS_DEPLOYMENT_MODE;
const ORIGINAL_DEPLOYMENT_EXPOSURE = process.env.GEETORUS_DEPLOYMENT_EXPOSURE;
type TestBridgeGlobal = typeof globalThis & {
  __geetorusPluginBridge__?: {
...
beforeEach(() => {
  if (ORIGINAL_DEPLOYMENT_MODE == null) {
    delete process.env.GEETORUS_DEPLOYMENT_MODE;
  } else {
    process.env.GEETORUS_DEPLOYMENT_MODE = ORIGINAL_DEPLOYMENT_MODE;
  }
  if (ORIGINAL_DEPLOYMENT_EXPOSURE == null) {
    delete process.env.GEETORUS_DEPLOYMENT_EXPOSURE;
  } else {
    process.env.GEETORUS_DEPLOYMENT_EXPOSURE = ORIGINAL_DEPLOYMENT_EXPOSURE;
  }
  ...
  (globalThis as TestBridgeGlobal).__geetorusPluginBridge__ = {
    sdkUi: {
...
afterEach(() => {
  ...
  delete (globalThis as TestBridgeGlobal).__geetorusPluginBridge__;
});
...
function geetorusIssue(overrides: Partial<Issue> = {}): Issue {
  const now = new Date();
  return {
    ...
    description: "Decide which Geetorus issues, comments, and documents can be ingested into the wiki.",
    status: "todo",
    workMode: "standard",
    priority: "medium",
    reviewPolicy: null,
...
describe("LLM Wiki plugin scaffold", () => {
  it("declares standalone plugin surfaces without core wiki coupling", () => {
    expect(manifest.id).toBe("geetorusai.plugin-llm-wiki");
  ...
  it("ships Karpathy-pattern schema and workflow prompts by default", () => {
    ...
    expect(manifest.agents?.[0]?.adapterConfig?.geetorusSkillSync).toEqual({
  ...
  it("routes legacy Wiki operations URLs to the History issue table", () => {
    ...
    expect(markup).toContain("Issues table · project-1 · plugin:geetorusai.plugin-llm-wiki:operation");
...
`;
    ...
    expect(markup).toContain("geetorus");
...
`;
  ...
  it("includes local wiki files in browse data before metadata is indexed", async () => {
    ...
    harness.ctx.localFolders.list = async (_companyId, folderKey, options) => ({
      ...
      entries: options?.relativePath === "wiki"
        ? [
            ...
            { path: "wiki/entities/geetorus.md", name: "geetorus.md", kind: "file", size: 10, modifiedAt },
    ...
    expect(result.pages).toEqual([
      ...
      expect.objectContaining({ path: "wiki/entities/geetorus.md", title: null }),
  ...
  it("does not ingest Geetorus events until operator controls enable them", async () => {
    const harness = createTestHarness({ manifest });
    const issue = geetorusIssue();
  ...
  it("records enabled Geetorus issue events as cursor observations without creating ingest operations", async () => {
    const harness = createTestHarness({ manifest });
    const issue = geetorusIssue();
    harness.seed({ issues: [issue] });
    const writes: Array<{ path: string; contents: string }> = [];
    ...
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("wiki_sources"))).toBe(false);
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("wiki_operations"))).toBe(false);
    const cursorUpsert = harness.dbExecutes.find((execute) => execute.sql.includes("geetorus_distillation_cursors"));
  ...
  it("preserves Geetorus event ingestion sources when only enabled changes", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("keeps Geetorus event cursor observations company scoped and ignores plugin-operation issues", async () => {
    const harness = createTestHarness({ manifest });
    const visibleIssue = geetorusIssue({ projectId: "77777777-7777-4777-8777-777777777777" });
    const otherCompanyIssue = geetorusIssue({
      id: "77777777-7777-4777-8777-777777777778",
      companyId: OTHER_COMPANY_ID,
      projectId: "77777777-7777-4777-8777-777777777779",
      identifier: "PAP-9999",
    });
    const operationIssue = geetorusIssue({
    ...
    const cursorWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("geetorus_distillation_cursors"));
  ...
  it("routes Geetorus issue, comment, and document event cursors only to the default space", async () => {
    const harness = createTestHarness({ manifest });
    const issue = geetorusIssue({ projectId: "77777777-7777-4777-8777-777777777777" });
    harness.seed({ issues: [issue] });
  ...
  it("routes Geetorus events into an explicitly enabled shared non-default space", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = geetorusIssue({ projectId: project.id });
    ...
    await harness.performAction("update-geetorus-ingestion-profile", {
      companyId: COMPANY_ID,
      spaceSlug: created.space.slug,
      profile: enabledProfile,
    });
    mockPersistedWikiSpace(harness, { ...(created.space as unknown as Record<string, unknown>), settings: { geetorusIngestion: enabledProfile } });
  ...
  it("stops new non-default observations after the per-space profile is disabled", async () => {
    ...
    mockPersistedWikiSpace(harness, { ...(created.space as unknown as Record<string, unknown>), settings: { geetorusIngestion: { ...enabledProfile, enabled: false } } });
    await harness.emit("issue.updated", {}, {
      companyId: COMPANY_ID,
      entityId: issue.id,
      entityType: "issue",
    ...
    const nonDefaultCursorWrites = harness.dbExecutes.filter((execute) =>
      execute.sql.includes("geetorus_distillation_cursors") && execute.params?.[10] === created.space.id);
    expect(nonDefaultCursorWrites).toHaveLength(1);
  });

  it("assembles deterministic Geetorus source bundles with issue, document, and comment provenance", async () => {
    const harness = createTestHarness({ manifest });
    const root = geetorusIssue({
    ...
    }>("assemble-geetorus-source-bundle", {
      companyId: COMPANY_ID,
      rootIssueId: root.id,
      maxCharacters: 20000,
    });
    const second = await harness.performAction<typeof first>("assemble-geetorus-source-bundle", {
      companyId: COMPANY_ID,
      rootIssueId: root.id,
      maxCharacters: 20000,
    });
  ...
  it("suppresses secret-like comment and document bodies before storing distillation snapshots", async () => {
    ...
    }>("create-geetorus-distillation-run", {
      companyId: COMPANY_ID,
    ...
    const snapshotInsert = harness.dbExecutes.find((execute) => execute.sql.includes("geetorus_source_snapshots"));
  ...
  it("creates source snapshots and only advances cursors after successful distillation outcomes", async () => {
    ...
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("geetorus_source_snapshots"))).toBe(true);

    const failed = await harness.performAction<{ cursorAdvanced: boolean }>("record-geetorus-distillation-outcome", {
      companyId: COMPANY_ID,
      runId: run.runId,
    ...
    const cursorUpdatesAfterFailure = harness.dbExecutes.filter((execute) =>
      execute.sql.trim().startsWith("UPDATE") && execute.sql.includes("geetorus_distillation_cursors"));
    expect(cursorUpdatesAfterFailure).toHaveLength(0);

    const succeeded = await harness.performAction<{ cursorAdvanced: boolean }>("record-geetorus-distillation-outcome", {
      companyId: COMPANY_ID,
      runId: run.runId,
      cursorId: run.cursorId,
      status: "succeeded",
  ...
  it("uses the existing distillation cursor id after an upsert conflict", async () => {
    ...
    const originalQuery = harness.ctx.db.query.bind(harness.ctx.db);
    harness.ctx.db.query = async <T,>(sql: string, params?: unknown[]) => {
      if (sql.includes("SELECT id") && sql.includes("geetorus_distillation_cursors")) {
    ...
    await plugin.definition.setup(harness.ctx);
    const result = await harness.performAction<{ cursorId: string }>("create-geetorus-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    });
    ...
    const runInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("geetorus_distillation_runs") && execute.sql.includes("'source_ready'"));
  ...
  it("creates explicit distillation work items for manual, retry, and backfill lanes", async () => {
    ...
    await harness.performAction("create-geetorus-distillation-work-item", {
      companyId: COMPANY_ID,
      kind: "manual",
      projectId: "77777777-7777-4777-8777-777777777777",
    ...
    const workItemWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("geetorus_distillation_work_items"));
    ...
    await expect(harness.performAction("create-geetorus-distillation-work-item", {
      companyId: COMPANY_ID,
      kind: "backfill",
  ...
  it("records estimated Geetorus distillation cost without refusing on legacy cost config", async () => {
    const harness = createTestHarness({
      manifest,
      config: {
        maxGeetorusRoutineRunCostCents: 1,
        maxGeetorusDistillationTaskCostCents: 1,
        maxGeetorusDistillationProjectCostCents: 1,
        geetorusCostCentsPerThousandSourceCharacters: 100,
      },
    });
    const project = existingProject();
    const issue = geetorusIssue({
  ...
  it("queues visible manual distill operation issues for a company-wide stale cursor scan", async () => {
    ...
    expect(result.operation.issue.description).toContain("Prompt source: LLM Wiki plugin action `distill-geetorus-now`");
    expect(result.operation.issue.description).toContain(`Required skill: use the installed \`${GEETORUS_DISTILL_SKILL_KEY}\` skill`);
    ...
    const workItemInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("geetorus_distillation_work_items") && execute.params?.[3] === "manual");
    ...
    const runInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("geetorus_distillation_runs") && execute.sql.includes("operation_issue_id"));
  ...
  it("enables distillation cursors for the most recently active non-plugin projects", async () => {
    ...
    }>("enable-geetorus-distillation-active-projects", {
      companyId: COMPANY_ID,
      limit: 2,
    });
  ...
  it("backfills only the selected Geetorus project and date window", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const inWindow = geetorusIssue({
    ...
    const outOfWindow = geetorusIssue({
    ...
    const otherProject = geetorusIssue({
    ...
    }>("backfill-geetorus-distillation", {
      companyId: COMPANY_ID,
      projectId: project.id,
    ...
    const workItemInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("geetorus_distillation_work_items") && execute.params?.[3] === "backfill");
    expect(String(workItemInsert?.params?.[9])).toContain('"backfillStartAt":"2026-04-01T00:00:00Z"');
  });

  it("generates review-required Geetorus project page patches with provenance, index, and log updates", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = geetorusIssue({
    ...
    expect(result.patches[3].proposedContents).toContain("geetorus-distill | proposed");
  ...
  it("auto-applies Geetorus project page patches by default when policy allows and records page bindings", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = geetorusIssue({
    ...
    expect(files.get("wiki/log.md")).toContain("geetorus-distill | proposed");
    const bindingWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("geetorus_page_bindings"));
  ...
  it("refuses auto-apply Geetorus project page patches in authenticated/public deployments", async () => {
    process.env.GEETORUS_DEPLOYMENT_MODE = "authenticated";
    process.env.GEETORUS_DEPLOYMENT_EXPOSURE = "public";
  ...
  it("refuses stale project page hashes before writing generated Geetorus pages", async () => {
    const harness = createTestHarness({ manifest, config: { autoApplyIngestPatches: true } });
    const project = existingProject();
    const issue = geetorusIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    await expect(harness.performAction("distill-geetorus-project-page", {
      companyId: COMPANY_ID,
      projectId: project.id,
      autoApply: true,
  ...
  it("skips low-signal Geetorus source windows without proposing wiki writes", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = geetorusIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    const result = await harness.performAction<{ status: string; reason: string; patches: unknown[] }>("distill-geetorus-project-page", {
      companyId: COMPANY_ID,
      projectId: project.id,
      maxCharacters: 20000,
    });
  ...
  it("preserves manual source ingest for non-default spaces while refusing Geetorus distillation there", async () => {
    const harness = createTestHarness({ manifest });
    const writes: Array<{ path: string; contents: string }> = [];
    harness.ctx.localFolders.writeTextAtomic = async (_companyId, _folderKey, relativePath, contents) => {
      writes.push({ path: relativePath, contents });
    ...
    await expect(harness.performAction("distill-geetorus-now", {
      companyId: COMPANY_ID,
      spaceSlug: created.space.slug,
      projectId: existingProject().id,
    })).rejects.toThrow("Geetorus ingestion policy denied queue");
  });

  it("fails closed for direct Geetorus ingestion actions against restricted spaces", async () => {
    const harness = createTestHarness({ manifest });
    harness.seed({
      agents: [wikiMaintainerAgent()],
      projects: [existingProject()],
    ...
    expect(operations).toHaveLength(0);
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("geetorus_distillation_work_items"))).toBe(false);
  });

  it("re-checks Geetorus ingestion policy at execution time for queued work", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = geetorusIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    const queued = await harness.performAction<{ status: string }>("distill-geetorus-now", {
      companyId: COMPANY_ID,
      projectId: project.id,
    });
    ...
    await expect(harness.performAction("create-geetorus-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    })).rejects.toThrow("personal spaces cannot ingest Geetorus sources");
    expect(harness.dbExecutes.some((execute) =>
      execute.sql.includes("geetorus_distillation_runs") && execute.sql.includes("'source_ready'"))).toBe(false);
  });

  it("queues Geetorus ingestion backfills for every selected project scope", async () => {
    const harness = createTestHarness({ manifest });
    await plugin.definition.setup(harness.ctx);
    ...
    }>("queue-geetorus-ingestion-backfill", {
      companyId: COMPANY_ID,
      sourceScope: {
    ...
    expect(harness.dbExecutes.filter((execute) => execute.sql.includes("geetorus_distillation_work_items"))).toHaveLength(2);
  });

  it("rejects oversized Geetorus ingestion profile and source-scope payloads", async () => {
    const harness = createTestHarness({ manifest });
    ...
    await expect(harness.performAction("update-event-ingestion-settings", {
      ...
      maxCharacters: 20001,
      sources: { issues: true },
    })).rejects.toThrow("maxCharacters exceeds the hard Geetorus ingestion cap");

    await expect(harness.performAction("enable-geetorus-distillation-active-projects", {
      companyId: COMPANY_ID,
      limit: 26,
    })).rejects.toThrow("fan-out exceeds the hard cap");

    await expect(harness.performAction("assemble-geetorus-source-bundle", {
      companyId: COMPANY_ID,
      projectId: "77777777-7777-4777-8777-777777777777",
