---
name: geetorus-distill
description: Use when an operation issue is a Geetorus cursor-window, distill, or backfill. Turn source-bundled Geetorus activity into wiki-insightful project standups, durable project pages, decisions, and history without asset dereferencing.
---

# Geetorus Distill

Distill Geetorus project, issue, comment, and document activity into durable wiki pages. The success criterion is **wiki-insightful, not procedural**: a reader who has never seen Geetorus should learn what the project is, what was decided, what is at risk, and what the current state is — without scanning a list of `## [YYYY-MM-DD]` headers.

## When this skill is needed

- Cursor-window distillation: the routine fed you a bounded source bundle of recent Geetorus activity for one project or root issue.
...
- Manual `distill-geetorus-now` request from the UI.
...
In Phase 1, every Geetorus distill, backfill, and cursor-window operation writes into the
...
write Geetorus-derived pages into a non-default space.

This rule is destination-only. The Geetorus source scope (which projects, root issues,
comments, documents are read) is set elsewhere in the operation issue and is independent of
the destination.

## Inputs

- A Geetorus source bundle (issue list, comment refs, document refs, source hash, cursor window).
...
Do not treat Geetorus assets/attachments or issue work products as source text for this skill.

- Allowed Geetorus body text: issue descriptions, comment bodies, document bodies.
...
1. **Datestamp-as-section-header.** Lines like `## [2026-04-15] geetorus-distill | proposed` belong in `wiki/log.md`, not in the project page. The project page is durable knowledge; the log is the audit trail.
2. **Procedural status lists.** `Issue mix: 3 todo, 5 in_progress, 2 done` tells the reader nothing they could not read off Geetorus directly. State *what is happening and why it matters*, then cite the issues that constitute the evidence.
...
4. **Write `wiki/projects/<slug>/standup.md` first.** Every Geetorus project represented in the wiki must have this file. It is the executive standup: where the project stands today, what changed recently, what is blocked or risky, and what happens next. Use stable sections, in this order:
   ...
   - **Next Actions** — concrete next actions and owners inferred from Geetorus issues, not vague aspirations.
   - **Links** — durable wiki project page and relevant Geetorus project/issues/documents.
   Rewrite the standup to today's state. Do not append endless dated sections; the audit trail belongs in `wiki/log.md` and Geetorus comments.
5. **Write `wiki/projects/<slug>/index.md`** with these stable sections, in this order:
   ...
   - **References** — readable links to the current standup and supporting Geetorus tasks/documents. Keep hashes and cursor ids out of the narrative.
...
9. **Append `wiki/log.md`** entry — this is where the datestamp belongs:
   ```
   ## [YYYY-MM-DD] geetorus-distill | <project name>
...
- Cite Geetorus source refs inline using their issue identifier (e.g. `PAP-3179`), not opaque UUIDs.
...
- Append a `geetorus-distill | low-signal skip` log entry naming the cursor window.
...
- [ ] The project page reads as wiki content, not as a Geetorus status report. A reader new to the company should understand what the project is.
...
`wiki_search`, `wiki_read_page`, `wiki_write_page`, `wiki_list_sources`, `wiki_read_source`. Always include the operation issue's `wikiId` and `spaceSlug`. The Geetorus source bundle arrives as part of the operation context — you do not need to assemble it.
