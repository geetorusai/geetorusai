...
export const PLUGIN_ID = "geetorusai.plugin-llm-wiki";
...
export const GEETORUS_DISTILL_SKILL_KEY = "geetorus-distill";
...
export const DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS = 12000;
export const DEFAULT_MAX_GEETORUS_CURSOR_WINDOW_CHARS = 60000;
export const DEFAULT_MAX_GEETORUS_ROUTINE_RUN_CHARS = 120000;
export const DEFAULT_GEETORUS_COST_CENTS_PER_1K_CHARS = 1;
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/geetorusai-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Geetorus issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Geetorus-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Geetorus ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Geetorus issue with the exact files and evidence.
...
const manifest: GeetorusPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Geetorus project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    {
      skillKey: GEETORUS_DISTILL_SKILL_KEY,
      displayName: "Geetorus Distill",
      slug: GEETORUS_DISTILL_SKILL_KEY,
      description: "Turn Geetorus cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
      markdown: skillMarkdown(GEETORUS_DISTILL_SKILL_KEY)
    },
    {
