...
import {
  bootstrapWikiRoot,
  bootstrapSpace,
  assembleGeetorusSourceBundle,
  archiveSpace,
  captureWikiSource,
  createSpace,
  createGeetorusDistillationRun,
  createGeetorusDistillationWorkItem,
  createOperationIssue,
  distillGeetorusProjectPage,
  ...
  listGeetorusIngestionCandidates,
  getGeetorusIngestionProfile,
  getOverview,
  listSpaces,
  handleGeetorusEventIngestion,
  ...
  readTemplate,
  readWikiPage,
  recordGeetorusDistillationOutcome,
  ...
  updateGeetorusIngestionProfile,
  updateSpace,
  writeTemplate,
  writeWikiPage,
} from "./wiki.js";
...
const GEETORUS_EVENT_INGESTION_EVENTS = [
  "issue.created",
  "issue.updated",
...
function buildManualDistillPrompt(input: { companyId: string; projectId?: string | null; rootIssueId?: string | null }) {
  ...
  return [
    ...
    "",
    "Prompt source: LLM Wiki plugin action `distill-geetorus-now` (`packages/plugins/plugin-llm-wiki/src/worker.ts`).",
    `Required skill: use the installed \`${GEETORUS_DISTILL_SKILL_KEY}\` skill before changing wiki files.`,
    "",
    "Scope:",
    ...
    !input.projectId && !input.rootIssueId
      ? "- Do not hardcode a single project. Find non-plugin Geetorus issues/comments/documents that changed in any project after the last processed cursor and are old enough for the stale/debounce threshold."
      : null,
    "",
    "Process:",
    "1. Read the wiki root AGENTS.md, wiki/index.md, and recent wiki/log.md entries.",
    "2. Assemble bounded Geetorus source bundles for every eligible project or root issue, excluding LLM Wiki plugin-operation issues.",
    "3. Turn durable signal into project standups, wiki-insightful project pages, decisions, history, index, and log updates per the geetorus-distill skill.",
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    for (const eventName of GEETORUS_EVENT_INGESTION_EVENTS) {
      ctx.events.on(eventName, async (event) => {
        const result = await handleGeetorusEventIngestion(ctx, event);
        if (result.status === "recorded") {
          ctx.logger.info("LLM Wiki recorded Geetorus event for cursor discovery", {
    ...
    ctx.data.register("geetorus-ingestion-profile", async (params) => {
      return getGeetorusIngestionProfile(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
      });
    ...
    ctx.data.register("geetorus-ingestion-candidates", async (params) => {
      return listGeetorusIngestionCandidates(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("update-geetorus-ingestion-profile", async (params) => {
      return updateGeetorusIngestionProfile(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("queue-geetorus-ingestion-backfill", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      ...
      for (const scope of scopes) {
        ...
        const workItem = await createGeetorusDistillationWorkItem(ctx, {
          companyId,
          wikiId,
          spaceSlug,
          kind: "backfill",
          ...
          metadata: { backfillStartAt, backfillEndAt, requestedFrom: "queue-geetorus-ingestion-backfill" },
        });
        const operation = await createOperationIssue(ctx, {
          ...
          spaceSlug,
          operationType: "backfill",
          title: scope.rootIssueId ? "Backfill Geetorus root issue wiki history" : "Backfill Geetorus project wiki history",
          prompt: [
            "Backfill LLM Wiki distillation was queued from a per-space Geetorus ingestion profile.",
    ...
    ctx.actions.register("assemble-geetorus-source-bundle", async (params) => {
      return assembleGeetorusSourceBundle(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("create-geetorus-distillation-run", async (params) => {
      return createGeetorusDistillationRun(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        projectId: stringField(params.projectId),
    ...
    ctx.actions.register("record-geetorus-distillation-outcome", async (params) => {
      ...
      return recordGeetorusDistillationOutcome(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        runId,
    ...
    ctx.actions.register("distill-geetorus-project-page", async (params) => {
      return distillGeetorusProjectPage(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        projectId: stringField(params.projectId),
    ...
    ctx.actions.register("distill-geetorus-now", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      ...
      const workItem = await createGeetorusDistillationWorkItem(ctx, {
        ...
        metadata: { requestedFrom: "distill-geetorus-now" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        ...
        operationType: "distill",
        title: rootIssueId
          ? "Distill Geetorus root issue into wiki"
          : projectId
            ? "Distill Geetorus project into wiki"
            : "Distill Geetorus changes into wiki",
        prompt: buildManualDistillPrompt({ companyId, projectId, rootIssueId }),
      });
      return { status: "queued", workItem, operation };
    });

    ctx.actions.register("enable-geetorus-distillation-active-projects", async (params) => {
    ...
    ctx.actions.register("backfill-geetorus-distillation", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      const rootIssueId = stringField(params.rootIssueId);
      ...
      const workItem = await createGeetorusDistillationWorkItem(ctx, {
        ...
        metadata: { backfillStartAt, backfillEndAt, requestedFrom: "backfill-geetorus-distillation" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        operationType: "backfill",
        title: rootIssueId ? "Backfill Geetorus root issue wiki history" : "Backfill Geetorus project wiki history",
        prompt: [
          "Backfill LLM Wiki distillation requested for a bounded Geetorus source window.",
      ...
      const result = await distillGeetorusProjectPage(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        projectId,
    ...
    ctx.actions.register("create-geetorus-distillation-work-item", async (params) => {
      ...
      return createGeetorusDistillationWorkItem(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        kind,
