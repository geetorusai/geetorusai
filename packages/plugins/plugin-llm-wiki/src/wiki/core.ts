...
import type { Agent, AgentSessionEvent, Issue, IssueComment, PluginContext, PluginEvent, PluginLocalFolderEntry, Project, ToolResult } from "@geetorusai/plugin-sdk";
import type { IssueDocument, PluginIssueOriginKind, PluginManagedRoutineResolution, PluginManagedSkillResolution } from "@geetorusai/plugin-sdk/types";
import {
  DEFAULT_MAX_SOURCE_BYTES,
  DEFAULT_MAX_GEETORUS_CURSOR_WINDOW_CHARS,
  DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS,
  DEFAULT_MAX_GEETORUS_ROUTINE_RUN_CHARS,
  DEFAULT_GEETORUS_COST_CENTS_PER_1K_CHARS,
  PLUGIN_ID,
...
const MAX_GEETORUS_INGESTION_PROFILE_SOURCE_COUNT = 3;
const MAX_GEETORUS_DISTILLATION_FAN_OUT = 25;
const MAX_GEETORUS_PROFILE_SELECTED_PROJECTS = 25;
const MAX_GEETORUS_PROFILE_ROOT_ISSUES = 25;
...
export type GeetorusDistillationScope = "company" | "project" | "root_issue";
export type GeetorusDistillationWorkItemKind = "manual" | "retry" | "backfill" | "priority_override" | "review_patch";
...
export type GeetorusIngestionSourceScope =
...
export type GeetorusIngestionProfileV1 = {
  version: 1;
  enabled: boolean;
  sourceScopes: GeetorusIngestionSourceScope[];
...
export type GeetorusIngestionProfileEffectiveState =
  | "enabled"
  | "disabled"
  | "policy_blocked"
  | "pending_approval"
...
export type GeetorusIngestionProfileRead = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: GeetorusIngestionProfileV1;
  effectiveState: GeetorusIngestionProfileEffectiveState;
  policyBlocks: string[];
...
type GeetorusIngestionPolicyPurpose =
  | "profile_read"
  | "profile_update"
  | "candidate_search"
  | "queue"
...
type GeetorusIngestionPolicyDecision =
...
type GeetorusIngestionCandidatesInput = SpaceInput & {
  query?: string | null;
};
...
type GeetorusSourceBundleInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  projectId?: string | null;
...
type GeetorusSourceBundle = {
  markdown: string;
  sourceRefs: GeetorusSourceRef[];
  sourceHash: string;
...
type GeetorusDistillationRunInput = GeetorusSourceBundleInput;

type GeetorusDistillationOutcomeInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  runId: string;
...
type GeetorusDistillationWorkItemInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  kind: GeetorusDistillationWorkItemKind;
  projectId?: string | null;
  rootIssueId?: string | null;
...
type GeetorusProjectPageDistillationInput = GeetorusSourceBundleInput & {
  autoApply?: boolean;
...
type GeetorusDistillationPatchOperation =
  | "standup_update"
  ...
  | "index_refresh"
  | "log_append";
type GeetorusDistillationPatch = {
  pagePath: string;
  operationType: GeetorusDistillationPatchOperation;
...
type GeetorusEventIngestResult =
...
async function requireGeetorusIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; wikiId: string; spaceSlug?: string | null },
  purpose: GeetorusIngestionPolicyPurpose,
  ...
  const decision = evaluateGeetorusProfilePolicy({
    space,
    profile,
    purpose,
...
function assertGeetorusSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("Geetorus source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard Geetorus ingestion cap of ${max} characters.`);
  }
}
...
function normalizeCostRate(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_GEETORUS_COST_CENTS_PER_1K_CHARS;
  return Math.max(0, value);
}

type GeetorusDistillationLimits = {
  maxCharacters: number;
  maxCharactersPerSource: number;
...
type DistillationSourceProtectionResult = {
  body: string;
  warning: string | null;
  refPatch: Pick<GeetorusSourceRef, "redactionStatus" | "redactionReasons">;
};
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original Geetorus source directly if a human needs the unredacted material.",
    ].join("\n"),
...
async function resolveGeetorusDistillationLimits(
  ctx: PluginContext,
  input: Pick<GeetorusSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun">,
): Promise<GeetorusDistillationLimits> {
  assertRequestedCharacterLimit("maxCharacters", input.maxCharacters, DEFAULT_MAX_GEETORUS_CURSOR_WINDOW_CHARS);
  assertRequestedCharacterLimit("maxCharactersPerSource", input.maxCharactersPerSource, DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS);
  const config = await ctx.config.get(input.companyId) as Record<string, unknown>;
  const maxCharactersPerSource = Math.min(
    normalizeBundleLimit(input.maxCharactersPerSource, DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS),
    normalizeBundleLimit(config.maxGeetorusIssueSourceCharacters, DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS),
  );
  const cursorWindowCap = normalizeBundleLimit(
    config.maxGeetorusCursorWindowCharacters,
    DEFAULT_MAX_GEETORUS_CURSOR_WINDOW_CHARS,
  );
  const routineRunCap = normalizeBundleLimit(
    config.maxGeetorusRoutineRunCharacters,
    DEFAULT_MAX_GEETORUS_ROUTINE_RUN_CHARS,
  );
  ...
  return {
    ...
    costCentsPerThousandSourceCharacters: normalizeCostRate(config.geetorusCostCentsPerThousandSourceCharacters),
  };
}

async function resolveGeetorusDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<GeetorusSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: WikiSpace },
): Promise<GeetorusDistillationLimits> {
  const [base, profile] = await Promise.all([
    resolveGeetorusDistillationLimits(ctx, input),
...
function defaultGeetorusIngestionProfile(input: {
  space: Pick<WikiSpace, "slug">;
  legacySettings?: WikiEventIngestionSettings | null;
}): GeetorusIngestionProfileV1 {
  ...
  return {
    ...
    cursor: {
      maxWindowCharacters: DEFAULT_MAX_GEETORUS_CURSOR_WINDOW_CHARS,
      maxCharactersPerSource: DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS,
...
function normalizeGeetorusIngestionSourceScope(value: unknown): GeetorusIngestionSourceScope | null {
  ...
  if (kind === "active_projects") {
    ...
    return {
      kind,
      limit: normalizeLimit(record.limit, 3, MAX_GEETORUS_PROFILE_SELECTED_PROJECTS),
      ...(statuses && statuses.length > 0 ? { statuses: [...new Set(statuses)] } : {}),
    };
  }
  if (kind === "selected_projects") {
    return { kind, projectIds: stringArray(record.projectIds).slice(0, MAX_GEETORUS_PROFILE_SELECTED_PROJECTS) };
  }
  if (kind === "root_issues") {
    return { kind, issueIds: stringArray(record.issueIds).slice(0, MAX_GEETORUS_PROFILE_ROOT_ISSUES) };
  }
  if (kind === "company_all") {
...
function normalizeGeetorusIngestionProfile(
  value: unknown,
  input: { space: Pick<WikiSpace, "slug">; legacySettings?: WikiEventIngestionSettings | null },
): GeetorusIngestionProfileV1 {
  const fallback = defaultGeetorusIngestionProfile(input);
  ...
  return {
    ...
    sourceScopes: Array.isArray(record.sourceScopes)
      ? record.sourceScopes.map(normalizeGeetorusIngestionSourceScope).filter((scope): scope is GeetorusIngestionSourceScope => Boolean(scope))
    ...
    cursor: {
      maxWindowCharacters: normalizeLimit(cursor.maxWindowCharacters, fallback.cursor.maxWindowCharacters, DEFAULT_MAX_GEETORUS_CURSOR_WINDOW_CHARS),
      maxCharactersPerSource: normalizeLimit(cursor.maxCharactersPerSource, fallback.cursor.maxCharactersPerSource, DEFAULT_MAX_GEETORUS_ISSUE_SOURCE_CHARS),
...
async function profileForSpace(ctx: PluginContext, companyId: string, space: WikiSpace): Promise<GeetorusIngestionProfileV1> {
  const legacySettings = space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, companyId) : null;
  return normalizeGeetorusIngestionProfile(space.settings.geetorusIngestion, { space, legacySettings });
}
...
function evaluateGeetorusProfilePolicy(input: {
  space: WikiSpace;
  profile?: GeetorusIngestionProfileV1 | null;
  purpose: GeetorusIngestionPolicyPurpose;
  requireEnabledProfile?: boolean;
}): GeetorusIngestionPolicyDecision {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `Geetorus ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `Geetorus ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest Geetorus sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `Geetorus ingestion policy denied ${purpose}: Geetorus ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `Geetorus ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
  return { allowed: true, space };
}

export async function getGeetorusIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; wikiId?: string | null; spaceSlug?: string | null },
): Promise<GeetorusIngestionProfileRead> {
  ...
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluateGeetorusProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalPageCount = await countGeetorusHistoricalPages(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceId: space.id,
  });
  const overlapCount = await countGeetorusProfileOverlaps(ctx, {
    companyId: input.companyId,
    wikiId,
    space,
    profile,
  });
  const effectiveState: GeetorusIngestionProfileEffectiveState = !policy.allowed
    ? "policy_blocked"
...
async function countGeetorusHistoricalPages(ctx: PluginContext, input: { companyId: string; wikiId: string; spaceId: string }): Promise<number> {
...
function scopeIdentity(scope: GeetorusIngestionSourceScope): string[] {
...
async function countGeetorusProfileOverlaps(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  profile: GeetorusIngestionProfileV1;
...
async function validateGeetorusIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  space: WikiSpace;
  profile: GeetorusIngestionProfileV1;
}) {
  const policy = evaluateGeetorusProfilePolicy({
    space: input.space,
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("Geetorus ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_GEETORUS_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Geetorus ingestion profile sources exceed the hard cap of ${MAX_GEETORUS_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  for (const scope of input.profile.sourceScopes) {
    ...
    if (scope.kind === "selected_projects") {
      if (scope.projectIds.length > MAX_GEETORUS_PROFILE_SELECTED_PROJECTS) {
        throw new Error(`selected_projects exceeds the hard cap of ${MAX_GEETORUS_PROFILE_SELECTED_PROJECTS}.`);
      }
    ...
    if (scope.kind === "root_issues") {
      if (scope.issueIds.length > MAX_GEETORUS_PROFILE_ROOT_ISSUES) {
        throw new Error(`root_issues exceeds the hard cap of ${MAX_GEETORUS_PROFILE_ROOT_ISSUES}.`);
      }
...
export async function updateGeetorusIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  profile: unknown;
  ...
  const profile = normalizeGeetorusIngestionProfile(input.profile, { space, legacySettings: space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, input.companyId) : null });
  await validateGeetorusIngestionProfile(ctx, { companyId: input.companyId, space, profile });
  await updateSpace(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceSlug: space.slug,
    settings: { geetorusIngestion: profile },
  });
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated Geetorus ingestion profile for ${space.displayName}`,
    entityType: "llm_wiki_space",
    entityId: space.id,
    metadata: {
      type: "plugin.llm_wiki.geetorus_ingestion_profile_updated",
      wikiId,
      spaceSlug: space.slug,
  ...
  return getGeetorusIngestionProfile(ctx, { companyId: input.companyId, wikiId, spaceSlug: space.slug });
}

export async function listGeetorusIngestionCandidates(ctx: PluginContext, input: GeetorusIngestionCandidatesInput): Promise<{
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search");
  ...
  await requireGeetorusIngestionPolicy(ctx, {
    companyId: input.companyId,
  ...
  if (sourceKeys.length > MAX_GEETORUS_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Geetorus ingestion profile sources exceed the hard cap of ${MAX_GEETORUS_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  ...
  const profile = normalizeGeetorusIngestionProfile(
    {
      ...defaultGeetorusIngestionProfile({ space: defaultSpace, legacySettings: next }),
      ...
      cursor: {
        ...defaultGeetorusIngestionProfile({ space: defaultSpace, legacySettings: next }).cursor,
...
function assertWikiPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    !(options.allowMetadata && trimmed.startsWith(".geetorus/"))
  ) {
...
function distillationCursorTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "geetorus_distillation_cursors");
}

function distillationRunTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "geetorus_distillation_runs");
}

function sourceSnapshotTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "geetorus_source_snapshots");
}

function distillationWorkItemTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "geetorus_distillation_work_items");
}

function pageBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "geetorus_page_bindings");
}
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  const geetorusDerived = input.operationType === "distill" || input.operationType === "backfill";
  return [
    ...
    geetorusDerived
      ? "- Geetorus-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Geetorus-derived pages into a non-default space."
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
...
function geetorusDistillationScope(input: { projectId?: string | null; rootIssueId?: string | null }): GeetorusDistillationScope {
  if (input.rootIssueId) return "root_issue";
  if (input.projectId) return "project";
  return "company";
}

function geetorusCursorScopeMetadata(input: { projectId?: string | null; rootIssueId?: string | null }) {
  const sourceScope = geetorusDistillationScope(input);
...
async function upsertGeetorusDistillationCursor(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  spaceId: string;
  projectId?: string | null;
  ...
  const scope = geetorusCursorScopeMetadata(input);
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS geetorus_distillation_cursors
     ...
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'geetorus_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, wiki_id, space_id, source_scope, scope_key, source_kind)
     DO UPDATE SET last_observed_at = GREATEST(
                       COALESCE(geetorus_distillation_cursors.last_observed_at, EXCLUDED.last_observed_at),
                       COALESCE(EXCLUDED.last_observed_at, geetorus_distillation_cursors.last_observed_at)
                     ),
                   pending_event_count = geetorus_distillation_cursors.pending_event_count + EXCLUDED.pending_event_count,
                   metadata = geetorus_distillation_cursors.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      cursorId,
      input.companyId,
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      WHERE company_id = $1
        ...
        AND source_scope = $4
        AND scope_key = $5
        AND source_kind = 'geetorus_issue_history'
      LIMIT 1`,
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_GEETORUS_DISTILLATION_FAN_OUT) {
    throw new Error(`Geetorus ingestion fan-out exceeds the hard cap of ${MAX_GEETORUS_DISTILLATION_FAN_OUT} enabled profiles.`);
  }
  ...
  for (const project of activeProjects) {
    ...
    const cursorId = await upsertGeetorusDistillationCursor(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceId: space.id,
...
function appendBoundedSection(input: {
  ...
  title: string;
  body: string;
  refs: GeetorusSourceRef[];
  ref: GeetorusSourceRef;
...
function sourceRefUpdatedAt(ref: GeetorusSourceRef): string | null {
  return ref.updatedAt ?? ref.createdAt ?? null;
}

function issueInBackfillWindow(issue: Issue, input: Pick<GeetorusSourceBundleInput, "backfillStartAt" | "backfillEndAt">): boolean {
...
async function listGeetorusBundleIssues(ctx: PluginContext, input: GeetorusSourceBundleInput): Promise<Issue[]> {
...
export async function assembleGeetorusSourceBundle(ctx: PluginContext, input: GeetorusSourceBundleInput): Promise<GeetorusSourceBundle> {
  const wikiId = normalizeWikiId(input.wikiId);
  assertGeetorusSourceScopePayload(input);
  const space = await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const limits = await resolveGeetorusDistillationLimitsForSpace(ctx, { ...input, space });
  ...
  const issues = await listGeetorusBundleIssues(ctx, input);
  const scope = geetorusCursorScopeMetadata(input);
  const sourceRefs: GeetorusSourceRef[] = [];
  const warnings: string[] = [];
  const lines = [
    `# Geetorus source bundle`,
    "",
    "## Bundle Metadata",
    "",
...
export async function createGeetorusDistillationRun(ctx: PluginContext, input: GeetorusDistillationRunInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertGeetorusSourceScopePayload(input);
  const space = await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const scope = geetorusCursorScopeMetadata(input);
  ...
  const bundle = await assembleGeetorusSourceBundle(ctx, input);
...
export async function recordGeetorusDistillationOutcome(ctx: PluginContext, input: GeetorusDistillationOutcomeInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createGeetorusDistillationWorkItem(ctx: PluginContext, input: GeetorusDistillationWorkItemInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertGeetorusSourceScopePayload(input);
  const space = await requireGeetorusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true });
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS geetorus_distillation_work_items
     ...
     DO UPDATE SET priority = EXCLUDED.priority,
                   metadata = geetorus_distillation_work_items.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      itemId,
      input.companyId,
...
function sourceRefLabel(ref: GeetorusSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "Geetorus source");
  if (ref.kind === "document") return `${issue} document:${ref.documentKey ?? "unknown"}`;
  if (ref.kind === "comment") return `${issue} comment`;
  return issue;
}

function sourceRefMarkdown(ref: GeetorusSourceRef): string {
  const metadata = [
...
function issueSourceRef(issue: Issue): GeetorusSourceRef {
  return {
    kind: "issue",
    issueId: issue.id,
    issueIdentifier: issue.identifier ?? null,
...
function projectPageSlug(input: { project: Project | null; rootIssue: Issue | null }): string {
  return slugify(input.project?.name ?? input.rootIssue?.title ?? "geetorus-project");
}
...
function hasDurableSignal(bundle: GeetorusSourceBundle, issues: Issue[]): boolean {
...
function standupPageContents(input: {
  ...
  issues: Issue[];
  bundle: GeetorusSourceBundle;
  pagePath: string;
  durablePagePath: string;
}): string {
  const currentAsOf = input.bundle.sourceWindowEnd ?? new Date().toISOString();
  const title = input.project?.name ?? input.rootIssue?.title ?? "Geetorus Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful Geetorus work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes Geetorus issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify Geetorus before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
function decisionsPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: GeetorusSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Geetorus Project";
...
function historyPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: GeetorusSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Geetorus Project";
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; pagePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  ...
  const warningLines = input.warnings.length
    ...
    : ["- warnings: none"];
  const entry = [
    `## [${new Date().toISOString().slice(0, 10)}] geetorus-distill | ${input.status}`,
...
async function upsertPageBinding(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${pageBindingTable(ctx)} AS geetorus_page_bindings
     ...
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = geetorus_page_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export function getDistillationAutoApplyRestriction(): DistillationAutoApplyRestriction {
  const rawMode = process.env.GEETORUS_DEPLOYMENT_MODE;
  const rawExposure = process.env.GEETORUS_DEPLOYMENT_EXPOSURE;
...
export async function distillGeetorusProjectPage(ctx: PluginContext, input: GeetorusProjectPageDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const run = await createGeetorusDistillationRun(ctx, input);
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordGeetorusDistillationOutcome(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceSlug: space.slug,
      runId: run.runId,
      ...
      warning: "Skipped low-signal Geetorus source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal Geetorus source window."],
      patches: [] as GeetorusDistillationPatch[],
    };
  }
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordGeetorusDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged Geetorus source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged Geetorus source hash."],
      patches: [] as GeetorusDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "Geetorus Project";
  ...
  const patches: GeetorusDistillationPatch[] = [
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `Geetorus distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function rawPathForGeetorusEvent(input: {
  ...
  return assertRawPath(`raw/geetorus/${input.sourceKind}/${eventDate}-${slugify(identifier)}-${slugify(input.label)}-${contentHash(input.contents).slice(0, 8)}.md`);
}

function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Geetorus issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Geetorus comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Geetorus document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
async function recordGeetorusCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  sourceKind: WikiEventIngestionSource;
  ...
  issue: Issue;
  event: PluginEvent;
}): Promise<Extract<GeetorusEventIngestResult, { status: "recorded" }>> {
  const cursorId = await upsertGeetorusDistillationCursor(ctx, {
    companyId: input.companyId,
    wikiId: input.wikiId,
    spaceId: input.space.id,
...
async function geetorusProfileIncludesIssue(ctx: PluginContext, input: {
  companyId: string;
  issue: Issue;
  profile: GeetorusIngestionProfileV1;
}): Promise<boolean> {
...
async function routeGeetorusCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  sourceKind: WikiEventIngestionSource;
  sourceId: string;
  issue: Issue;
  event: PluginEvent;
}): Promise<GeetorusEventIngestResult> {
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, wikiId: DEFAULT_WIKI_ID });
  const recorded: Array<Extract<GeetorusEventIngestResult, { status: "recorded" }>> = [];
  let eligibleProfileCount = 0;
  for (const space of spaces) {
    const profile = await profileForSpace(ctx, input.companyId, space);
    if (!profile.enabled) continue;
    const policy = evaluateGeetorusProfilePolicy({ space, profile, purpose: "event_routing", requireEnabledProfile: true });
    if (!policy.allowed) continue;
    if (!profile.sourceKinds[input.sourceKind]) continue;
    if (!(await geetorusProfileIncludesIssue(ctx, { companyId: input.companyId, issue: input.issue, profile }))) continue;
    eligibleProfileCount += 1;
    if (eligibleProfileCount > MAX_GEETORUS_DISTILLATION_FAN_OUT) {
      throw new Error(`Geetorus ingestion fan-out exceeds the hard cap of ${MAX_GEETORUS_DISTILLATION_FAN_OUT} enabled profiles.`);
    }
    if (await ctx.state.get(eventIngestionDedupKey(input.companyId, space.wikiId, space.id, input.sourceKind, input.sourceId))) {
      continue;
    }
    recorded.push(await recordGeetorusCursorObservation(ctx, {
      ...input,
      wikiId: space.wikiId,
      space,
    }));
...
export async function handleGeetorusEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<GeetorusEventIngestResult> {
  ...
  if (event.eventType === "issue.created" || event.eventType === "issue.updated") {
    ...
    return routeGeetorusCursorObservation(ctx, {
      companyId,
      sourceKind: "issues",
      sourceId,
      issue,
