...
here rather than in Geetorus core.
...
- opt-in company-scoped Geetorus event ingestion controls for issues, comments, and documents; event ingestion is disabled by default and routes captured raw provenance into the default space only
- manual Geetorus project/root issue distillation and bounded backfill actions with explicit work items, operation issues, source caps, and estimated cost recording
- Geetorus-derived distillation (cursor windows, manual `distill-now`, backfill) always writes into the default wiki space in Phase 1; non-default spaces remain on manual / raw-file ingest until per-space Geetorus ingestion profiles ship
- Geetorus-derived distillation maintains `wiki/projects/<slug>/standup.md` as the executive current-state view for each represented project, alongside durable `wiki/projects/<slug>/index.md` knowledge pages
...
Geetorus-derived text ingestion stays limited to issue titles/descriptions, issue comments, and issue documents.
...
The accepted policy lives in [doc/plans/2026-05-06-llm-wiki-geetorus-asset-security-gate.md](../../../doc/plans/2026-05-06-llm-wiki-geetorus-asset-security-gate.md).
...
From the Geetorus repo root:

```bash
pnpm --filter @geetorusai/plugin-llm-wiki typecheck
pnpm --filter @geetorusai/plugin-llm-wiki test
pnpm --filter @geetorusai/plugin-llm-wiki build
```

## Alpha Verification

Run these commands from the Geetorus repo root before handing off alpha plugin
changes:
...
- disabled and enabled Geetorus event ingestion paths
...
- Host-level plugin install and live agent invocation still need Geetorus
...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/Users/dotta/geetorus/.geetorus/worktrees/PAP-3179-design-a-llm-wiki-plugin/packages/plugins/plugin-llm-wiki","isLocalPath":true}'
```
...
The package-local `dist/` directory is ignored by git, but local Geetorus
...
restart the Geetorus dev server so the host imports the fresh bundle.
...
write the starter files. The plugin uses Geetorus's local folder API for path
...
`CLAUDE.md` renamed to `AGENTS.md` and Geetorus project overviews, standups,
