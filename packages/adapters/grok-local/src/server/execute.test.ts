    ...
    prepareRuntimeMock: vi.fn(
      async (input: { assets?: Array<{ key: string; localDir: string; followSymlinks?: boolean }> }) => {
        ...
        const assetDirs =
          override?.assetDirs ??
          Object.fromEntries(
            (input.assets ?? []).map((asset) => [asset.key, `/remote/workspace/.geetorus-runtime/grok/${asset.key}`]),
          );
...
vi.mock("@geetorusai/adapter-utils/execution-target", () => ({
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-grok-local-"));
  tempRoots.push(root);
  return root;
}
...
function makeRestoreWorkspace(
  ...
  return async () => {
    for (const asset of assets) {
      ...
      await asset.restore({
        assetDir: "/remote/workspace/.geetorus-runtime/grok/home",
        readFile: async () => {
...
describe("grok_local execute", () => {
  ...
  it("stages Grok-native instructions and skills into the workspace for the run and cleans them up afterward", async () => {
    ...
    const skillSource = path.join(root, "runtime-skills", "geetorus");
    ...
    runProcessMock.mockImplementation(async (_runId, _target, _command, args, options) => {
      ...
      expect(await pathExists(path.join(root, ".claude", "skills", "geetorus", "SKILL.md"))).toBe(true);
    ...
    const ctx: AdapterExecutionContext = {
      ...
      config: {
        ...
        geetorusSkillSync: { desiredSkills: ["geetorus"] },
      },
      context: {},
      authToken: "run-token",
    ...
    expect(await pathExists(path.join(root, ".claude", "skills", "geetorus"))).toBe(false);
  ...
  describe("local lane GROK_HOME", () => {
    ...
    beforeEach(async () => {
      ...
      process.env.GEETORUS_HOME = await makeTempRoot();
      delete process.env.XAI_API_KEY;
  ...
  describe("remote lane credential staging", () => {
    let previousApiKey: string | undefined;
    let previousGeetorusHome: string | undefined;
    let geetorusHomeRoot: string;

    beforeEach(async () => {
      previousApiKey = process.env.XAI_API_KEY;
      previousGeetorusHome = process.env.GEETORUS_HOME;
      // Point the managed Grok home at a private tmp root, so staging never
      // touches a real developer or CI-host `~/.geetorus` tree.
      geetorusHomeRoot = await makeTempRoot();
      process.env.GEETORUS_HOME = geetorusHomeRoot;
      ...
      prepareRuntimeMock.mockImplementationOnce(
        async (input: { assets?: Array<{ key: string; localDir: string; followSymlinks?: boolean; provision?: unknown; restore?: unknown }> }) => {
          ...
          return {
            ...
            assetDirs: { home: "/remote/workspace/.geetorus-runtime/grok/home" },
    ...
    it("sets GROK_HOME from assetDirs.home in a remote subscription run", async () => {
      ...
      expect(seenEnv.GROK_HOME).toBe("/remote/workspace/.geetorus-runtime/grok/home");
    });
    ...
    it("stages an empty company home instead of a configured host login for remote runs", async () => {
      ...
      expect(runProcessMock.mock.calls[0][4].env.GROK_HOME).toBe("/remote/workspace/.geetorus-runtime/grok/home");
    });
    ...
    it("uses the fallback remote path when assetDirs.home is absent", async () => {
      ...
      expect(seenEnv.GROK_HOME).toBe(
        "/remote/fallback-workspace/.geetorus-runtime/grok/home",
      );
    });
