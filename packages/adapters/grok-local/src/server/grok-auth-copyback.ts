...
export async function copyBackGrokAuth(input: CopyBackGrokAuthInput): Promise<CopyBackGrokAuthOutcome> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException | null)?.code === "ENOENT") {
      await log(
        "[geetorus] Grok auth copy-out: no sandbox credential to copy back (absent auth.json); host credential kept.",
      );
      return "kept-host";
    }
    throw error;
  ...
  try {
      ...
      async (canonicalHostHomeDir) => {
        ...
        try {
          ...
          if (decision === USE_SOURCE_EXIT) {
            ...
            await log(
              "[geetorus] Grok auth copy-out: sandbox credential is strictly newer for the same identity; installed to the host at mode 0600.",
            );
            return "copied";
          }

          await log(
            "[geetorus] Grok auth copy-out: host credential kept (sandbox copy is not a strictly-newer same-identity credential).",
          );
          return "kept-host";
        } finally {
    ...
    await Promise.resolve(
      log(`[geetorus] Grok auth copy-out failed (${code}); host credential kept.`),
    ).catch(() => undefined);
    throw error;
  }
}
