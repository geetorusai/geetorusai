...
export async function checkStagedGrokCredentialReadiness(
  ...
  const scratchHome = await mkdtemp(path.join(os.tmpdir(), "geetorus-grok-login-readiness-"));
  try {
...
export async function promoteGrokDeviceLoginCredential(
  ...
  if (!userInitiated) {
    await log(
      "[geetorus] Grok device-login promotion: skipped (an automatic background login never seeds a company slot).",
    );
  ...
  if (!soleOwner) {
    await log(
      "[geetorus] Grok device-login promotion: skipped (the session no longer holds the sole active claim on the slot).",
    );
  ...
  return await withDirectoryMergeLock(
    ...
    async (canonicalCompanyHome) => {
      ...
      if (existingState.kind === "unreadable") {
        await log(
          "[geetorus] Grok device-login promotion: kept the company credential home (the existing file is present but this step cannot read it as a usable Grok credential).",
        );
        return "kept_foreign_identity";
      }
      if (existingState.kind === "identity" && existingState.identityKey !== payload.identityKey) {
        await log(
          "[geetorus] Grok device-login promotion: kept the company credential home (the login is a different account than the one already set for this company).",
        );
        return "kept_foreign_identity";
      }
      ...
      if (existingState.kind === "identity") {
        ...
        if (!wrote) {
          await log(
            "[geetorus] Grok device-login promotion: kept the company credential home (the existing same-identity credential is not older than the device-login credential).",
          );
          return "kept";
        }
        await log("[geetorus] Grok device-login promotion: wrote the company credential home at mode 0600.");
        return "promoted";
      }
