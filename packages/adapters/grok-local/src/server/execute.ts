...
async function stageGrokProjectAssets(input: {
  ...
  if (input.instructionsFilePath) {
    if (!await pathExists(instructionsTarget)) {
      ...
      await input.onLog(
        "stdout",
        `[geetorus] Grok workspace already contains ${instructionsTarget}; using --rules @${input.instructionsFilePath} instead of overwriting it.\n`,
      );
    }
  } else {
  ...
  if (selectedSkills.length > 0) {
    ...
    for (const skill of selectedSkills) {
      ...
      if (await pathExists(target)) {
        await input.onLog(
          "stdout",
          `[geetorus] Grok skill target already exists at ${target}; leaving it unchanged.\n`,
        );
        continue;
      }
      await materializeGeetorusSkillCopy(skill.source, target);
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const grokSkillEntries = await readGeetorusRuntimeSkillEntries(config, __moduleDir);
  const desiredGrokSkillNames = resolveLegacyGeetorusDesiredSkillNames(config, grokSkillEntries);
  ...
  try {
    ...
    if (executionTargetIsRemote) {
      await onLog(
        "stdout",
        `[geetorus] Syncing Grok workspace to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
      ...
      if (isGrokSubscriptionMode) {
        env.GROK_HOME =
          ...
          path.posix.join(effectiveExecutionCwd, ".geetorus-runtime", "grok", "home");
      }
    }
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[geetorus] Grok session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[geetorus] Grok session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    const commandNotes = (() => {
      ...
      if (stagedAssets.stagedSkillsCount > 0) {
        notes.push(`Staged ${stagedAssets.stagedSkillsCount} Geetorus skill(s) into .claude/skills for native Grok discovery.`);
      }
      return notes;
    })();
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[geetorus] Grok resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
    ...
    if (stagedGrokHomeDir) {
      await fs.rm(stagedGrokHomeDir, { recursive: true, force: true }).catch(async (error) => {
        await onLog(
          "stderr",
          `[geetorus] Failed to remove staged Grok home "${stagedGrokHomeDir}": ${
