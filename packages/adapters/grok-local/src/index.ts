...
Use when:
- You want Geetorus to run the native Grok Build CLI locally on the host machine
- You want resumable Grok sessions across heartbeats via \`--resume\`
- You want Geetorus-managed instructions and skills staged into the execution workspace using Grok's native discovery paths (\`Agents.md\` and \`.claude/skills\`)

Don't use when:
- You need a webhook-style external invocation (use http or openclaw_gateway)
- You only need a one-shot script without an AI coding agent loop (use process)
- Grok CLI is not installed or authenticated on the machine that runs Geetorus

Core fields:
- cwd (string, optional): default absolute working directory fallback for the agent process (created if missing when possible)
- instructionsFilePath (string, optional): absolute path to a markdown instructions file. Geetorus stages it into the execution workspace as \`Agents.md\` when safe, otherwise falls back to \`--rules @file\`
...
- Geetorus stages desired runtime skills into \`.claude/skills\` inside the execution workspace so Grok discovers them as project skills.
