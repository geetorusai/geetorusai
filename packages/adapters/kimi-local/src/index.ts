...
/**
 * Map a Geetorus effort value onto Kimi's supported thinking-effort set.
...
Use when:
- You want Geetorus to run the Kimi Code CLI (kimi) locally on the host machine
- You want Kimi sessions resumed across heartbeats with -r
- You want Geetorus skills injected into the Kimi skills home without polluting the agent workspace

Don't use when:
- You need webhook-style external invocation (use http or openclaw_gateway)
- You only need a one-shot script without an AI coding agent loop (use process)
- Kimi Code CLI is not installed on the machine that runs Geetorus
...
- Desired Geetorus skills are delivered to local runs via \`--skills-dir\` pointing at a per-run managed directory, so skills load reliably without polluting the user's \`~/.kimi-code/skills\` home. Remote runs sync skills into the remote skills home.
