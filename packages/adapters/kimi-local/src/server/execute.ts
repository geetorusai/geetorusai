...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "Use shell commands with curl to make Geetorus API requests when needed.",
    "Include X-Geetorus-Run-Id on mutating requests.",
    "",
    "",
  ].join("\n");
}
...
async function buildKimiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-kimi-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const kimiSkillEntries = await readGeetorusRuntimeSkillEntries(config, __moduleDir);
  const desiredKimiSkillNames = resolveLegacyGeetorusDesiredSkillNames(config, kimiSkillEntries);
  const envConfig = parseObject(config.env);

  const hasExplicitApiKey =
    typeof envConfig.GEETORUS_API_KEY === "string" && envConfig.GEETORUS_API_KEY.trim().length > 0;
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[geetorus] Syncing workspace and Kimi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (!executionTargetIsRemote && desiredKimiSkillNames.length > 0) {
    ...
    await onLog(
      "stderr",
      `[geetorus] Prepared ${desiredKimiSkillNames.length} Kimi skill(s) for --skills-dir delivery.\n`,
    );
  }
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[geetorus] Kimi session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[geetorus] Kimi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const buildArgs = (resumeSessionId: string | null) => {
    ...
    // Load desired Geetorus skills from the dedicated per-run directory
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[geetorus] Kimi resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
