...
vi.mock("@geetorusai/adapter-utils/execution-target", () => ({
  ...
  adapterExecutionTargetUsesGeetorusBridge: () => false,
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-kimi-local-"));
  tempRoots.push(root);
  return root;
}
...
describe("kimi_local execute", () => {
  ...
  it("loads the operational skill when no optional skills are configured", async () => {
    ...
    expect(seenArgs[seenArgs.indexOf("--skills-dir") + 1]).toContain("geetorus-kimi-skills-");
  });
});
