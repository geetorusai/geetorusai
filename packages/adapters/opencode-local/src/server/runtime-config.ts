...
import { asBoolean } from "@geetorusai/adapter-utils/server-utils";
...
function parseProviderConfig(
  ...
  try {
    ...
    notes.push("GEETORUS_OPENCODE_PROVIDERS contains invalid JSON; custom providers ignored.");
    return null;
  }
  ...
  if (skipped.length > 0) {
    notes.push(
      `GEETORUS_OPENCODE_PROVIDERS: skipped provider(s) with non-object values: ${skipped.join(", ")}.`,
    );
  }
...
export async function prepareOpenCodeRuntimeConfig(input: {
  ...
  // Merge gateway/custom provider definitions supplied via GEETORUS_OPENCODE_PROVIDERS
  ...
  const gatewayProviders = parseProviderConfig(
    input.env.GEETORUS_OPENCODE_PROVIDERS ?? process.env.GEETORUS_OPENCODE_PROVIDERS,
    resolveEnv,
    notes,
  );
  ...
  if (gatewayProviders) {
    notes.push(
      `Injected ${Object.keys(gatewayProviders).length} custom OpenCode provider(s) from GEETORUS_OPENCODE_PROVIDERS: ${Object.keys(gatewayProviders).join(", ")}.`,
    );
  }
  ...
  // user config or GEETORUS_OPENCODE_PROVIDERS.
  ...
  // other helper tasks) via GEETORUS_OPENCODE_SMALL_MODEL. OpenCode otherwise
  ...
  const smallModel = (input.env.GEETORUS_OPENCODE_SMALL_MODEL ?? process.env.GEETORUS_OPENCODE_SMALL_MODEL)?.trim();
  if (smallModel) {
