...
describe("opencode remote execution", () => {
  ...
  beforeEach(async () => {
    const configHome = await mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-test-config-"));
  ...
  it.each([false, true])("prepares the workspace, syncs OpenCode skills, and restores workspace changes for remote SSH execution (managed=%s)", async (managed) => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-remote-"));
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.geetorus-runtime/opencode/xdgConfig`,
    }));
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.geetorus-runtime/opencode/skills`,
      followSymlinks: true,
    }));
    ...
    expect(modelProbeCall?.[3].env.XDG_CONFIG_HOME).toBe(
      `${managedRemoteWorkspace}/.geetorus-runtime/opencode/xdgConfig`,
    );
    ...
    if (managed) {
      const home = `${managedRemoteWorkspace}/.geetorus-runtime/opencode/managed-auth/run-1`;
    ...
    expect(call?.[3].env.XDG_CONFIG_HOME).toBe(`${managedRemoteWorkspace}/.geetorus-runtime/opencode/xdgConfig`);
  ...
  it("fails before the remote run when the configured model is unavailable on the SSH target", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-remote-model-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
  ...
  it("resumes saved OpenCode sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
