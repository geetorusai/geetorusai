...
async function ensureOpenCodeSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[geetorus] Removed maintainer-only OpenCode skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[geetorus] ${result === "repaired" ? "Repaired" : "Injected"} OpenCode skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[geetorus] Failed to inject OpenCode skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildOpenCodeSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const desiredOpenCodeSkillNames = resolveLegacyGeetorusDesiredSkillNames(config, openCodeSkillEntries);
  ...
  try {
    ...
    if (executionTarget?.kind === "remote") {
      ...
      await onLog(
        "stdout",
        `[geetorus] Syncing workspace and OpenCode runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[geetorus] OpenCode session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[geetorus] OpenCode session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    // log file is unreachable. Toggle via GEETORUS_OPENCODE_PRINT_LOGS (run env,
    // then process env).
    const printLogs = isTruthyEnvFlag(
      env.GEETORUS_OPENCODE_PRINT_LOGS ?? process.env.GEETORUS_OPENCODE_PRINT_LOGS,
    );
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[geetorus] OpenCode session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
        );
