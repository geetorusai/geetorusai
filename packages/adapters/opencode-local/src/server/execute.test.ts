...
describe("OpenCode local skill injection", () => {
  ...
  beforeEach(async () => {
    configHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-test-config-"));
  ...
  it("injects runtime skills into the configured child HOME", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-configured-home-"));
  ...
  it("passes an OpenRouter key and complete model to OpenCode without logging the key", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-openrouter-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "opencode");
