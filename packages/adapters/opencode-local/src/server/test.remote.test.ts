...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
      ...
      runtimeRootDir: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/opencode",
      assetDirs: {
        xdgConfig: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/opencode/xdgConfig",
      },
      restoreWorkspace,
    })),
  };
...
describe("opencode remote environment diagnostics", () => {
  ...
  beforeEach(async () => {
    configHome = await mkdtemp(path.join(os.tmpdir(), "geetorus-opencode-test-config-"));
  ...
  it.each([false, true])("stages remote runtime config assets for sandbox hello probes (managed=%s)", async (managed) => {
    ...
    if (managed) {
      expect(probeCall?.[4].env.HOME).toContain("/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/opencode/managed-auth/");
    ...
    expect(probeCall?.[4].env.XDG_CONFIG_HOME).toBe(
      "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/opencode/xdgConfig",
    );
  });
});
