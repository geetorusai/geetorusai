...
Use when:
- You want Geetorus to run OpenCode locally as the agent runtime
...
- dangerouslySkipPermissions (boolean, optional): inject a runtime OpenCode config that allows \`external_directory\` access without interactive prompts; defaults to true for unattended Geetorus runs
...
- Geetorus requires an explicit \`model\` value for \`opencode_local\` agents.
...
- When \`dangerouslySkipPermissions\` is enabled, Geetorus injects a temporary \
