...
## Scope
This plan is now **gateway-only**. Geetorus supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Geetorus skill.
...
```json
{
  "geetorusApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a Geetorus task.
...
```bash
pnpm --filter @geetorusai/server test:run -- openclaw-gateway
pnpm --filter @geetorusai/server typecheck
pnpm --filter @geetorusai/ui typecheck
pnpm --filter geetorusai typecheck
```
