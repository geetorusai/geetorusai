...
Use when:
- You want Geetorus to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Geetorus server.
...
- geetorusApiUrl (string, optional): absolute Geetorus base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/geetorus-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default geetorus)

Wake payload notes:
- Geetorus wake context is embedded into the generated message text
- No top-level geetorus field is sent; the gateway agent schema rejects unknown root params
