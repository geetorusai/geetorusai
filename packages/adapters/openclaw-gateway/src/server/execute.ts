...
const DEFAULT_CLIENT_VERSION = "geetorus";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "geetorus";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`geetorus:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`geetorus:issue:${input.issueId}`, input.agentId);
  }
...
function resolveGeetorusApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/geetorus-claimed-api-key.json";

export function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildGeetorusEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const geetorusApiUrlOverride = resolveGeetorusApiUrlOverride(ctx.config.geetorusApiUrl);
  const geetorusEnv: Record<string, string> = {
    ...buildGeetorusEnv(ctx.agent),
    ...buildRuntimeToolsEnv(ctx.runtimeTools),
    GEETORUS_RUN_ID: ctx.runId,
  };

  if (geetorusApiUrlOverride) {
    geetorusEnv.GEETORUS_API_URL = geetorusApiUrlOverride;
  }
  if (wakePayload.taskId) geetorusEnv.GEETORUS_TASK_ID = wakePayload.taskId;
  const issueWorkMode = readGeetorusIssueWorkModeFromContext(ctx.context);
  if (issueWorkMode) geetorusEnv.GEETORUS_ISSUE_WORK_MODE = issueWorkMode;
  if (wakePayload.wakeReason) geetorusEnv.GEETORUS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) geetorusEnv.GEETORUS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) geetorusEnv.GEETORUS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) geetorusEnv.GEETORUS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    geetorusEnv.GEETORUS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  geetorusEnv: Record<string, string>,
  ...
  for (const key of orderedKeys) {
    const value = geetorusEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = geetorusEnv.GEETORUS_API_URL ?? "<set GEETORUS_API_URL>";

  if (conversationTaskMarkdown !== undefined) {
    return [
      "Geetorus conversation turn for a cloud adapter.",
      "Set these values in your run context:",
      ...envLines,
      `Load GEETORUS_API_KEY from ${claimedApiKeyPath} (the token saved after claim-api-key).`,
      "Use Authorization: Bearer $GEETORUS_API_KEY on every API call and X-Geetorus-Run-Id: $GEETORUS_RUN_ID on every mutation.",
  ...
  const lines = [
    "Geetorus wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `GEETORUS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load GEETORUS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $GEETORUS_API_KEY on every API call.",
    "- Use X-Geetorus-Run-Id: $GEETORUS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: GEETORUS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    '   Replace {issueId} in every endpoint below with that determined id. Never send the literal text "{issueId}" in a URL.',
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$GEETORUS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$GEETORUS_COMPANY_ID/issues?assigneeAgentId=$GEETORUS_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "geetorus-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geetorusEnv = buildGeetorusEnvForWake(ctx, wakePayload);
  // No heartbeat prompt template is sent over the gateway, so the wake prompt
  // must carry the execution contract itself.
  const structuredWakePrompt = renderGeetorusWakePrompt(ctx.context.geetorusWake, {
    includeExecutionContract: true,
    conversationMode: ctx.context.conversationMode === true,
  });
  const structuredWakeJson = stringifyGeetorusWakePayload(ctx.context.geetorusWake);
  const wakeText = buildWakeText(
    ...
    ctx.context.conversationMode === true
      ? selectGeetorusTaskMarkdown(ctx.context, { resumedSession: Boolean(ctx.runtime?.sessionId) })
      : undefined,
  );
