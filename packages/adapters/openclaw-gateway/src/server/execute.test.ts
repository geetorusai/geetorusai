...
describe("resolveSessionKey", () => {
  it("prefixes run-scoped session keys with the configured agent", () => {
    ...
    ).toBe("agent:meridian:geetorus:run:run-123");
  });
  ...
  it("prefixes issue-scoped session keys with the configured agent", () => {
    ...
    ).toBe("agent:meridian:geetorus:issue:issue-456");
  });
  ...
  it("prefixes fixed session keys with the configured agent", () => {
    expect(
      resolveSessionKey({
        strategy: "fixed",
        configuredSessionKey: "geetorus",
        agentId: "meridian",
        runId: "run-123",
        issueId: null,
      }),
    ).toBe("agent:meridian:geetorus");
  });
  ...
  it("does not double-prefix an already-routed session key", () => {
    expect(
      resolveSessionKey({
        strategy: "fixed",
        configuredSessionKey: "agent:meridian:geetorus",
        agentId: "meridian",
        runId: "run-123",
        issueId: null,
      }),
...
describe("buildAgentParams", () => {
  it("strips root-level geetorus fields from gateway agent params", () => {
    expect(
      buildAgentParams({
        payloadTemplate: {
          text: "old text",
        ...
        message: "wake text",
        sessionKey: "agent:meridian:geetorus:issue:issue-456",
        runId: "run-123",
...
describe("resolveClaimedApiKeyPath", () => {
  const DEFAULT_PATH = "~/.openclaw/workspace/geetorus-claimed-api-key.json";

  it("returns the configured per-agent path when set", () => {
    expect(
      resolveClaimedApiKeyPath("~/.openclaw/workspace/geetorus-keys/happy.json"),
    ).toBe("~/.openclaw/workspace/geetorus-keys/happy.json");
  });
