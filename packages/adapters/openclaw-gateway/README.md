...
This document describes how `@geetorusai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Geetorus logs/transcript parsing
...
Like every Geetorus adapter, this one must treat the local execution-workspace
...
the OpenClaw side, use the round-trip helpers in `@geetorusai/adapter-utils`
