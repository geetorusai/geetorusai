/**
 * Hermes Agent adapter for Geetorus.
 *
 * Runs Hermes Agent (https://github.com/NousResearch/hermes-agent)
 * as a managed employee in a Geetorus company. Hermes Agent is a
 ...
 *
 * Hermes supports any model via any provider. The Geetorus UI should
...
/**
 * Documentation shown in the Geetorus UI when configuring a Hermes agent.
 */
...
This adapter package also ships a Hermes-facing Geetorus task bridge skill:
\`geetorus-task-bridge\`. Use it when a user starts in Hermes and asks Hermes
to create, comment on, update, or list Geetorus tasks.
...
- \`GEETORUS_BRIDGE_API_KEY\` - Geetorus agent API key created with \`scope.kind = "task_bridge"\`
- optional fallback \`GEETORUS_API_KEY\` - must still be a task_bridge key, never a normal claimed agent key
- optional \`GEETORUS_COMPANY_ID\`, \`GEETORUS_AGENT_ID\`, and \`GEETORUS_RUN_ID\`

The bridge is separate from adapter execution:

- \`hermes_local\` means Geetorus shells out to local \`hermes chat\`.
- \`hermes_gateway\` means Geetorus wakes remote Hermes through Hermes's API server.
- \`geetorus-task-bridge\` means Hermes calls Geetorus's REST API to manage tasks.

Create task bridge keys with a parent issue or project boundary. Do not expose
normal claimed Geetorus agent API keys to internet-facing Hermes chat/webhook
...
/**
 * External adapter plugin entrypoint expected by Geetorus's adapter manager.
 */
