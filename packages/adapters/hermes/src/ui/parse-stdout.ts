...
/**
 * Parse Hermes Agent stdout into TranscriptEntry objects for the Geetorus UI.
 *
 ...
 *   System:     "[hermes] ..."
 *
 * We emit structured tool_call/tool_result pairs so Geetorus renders proper
 ...
 * Geetorus uses this to match them in normalizeTranscript.
 */
 ...
 * Geetorus renders proper tool cards with status icons and expand/collapse.
 *
...
export function parseHermesStdoutLine(
  ...
  if (trimmed.startsWith("[hermes]") || trimmed.startsWith("[geetorus]")) {
  ...
  // Emit as stderr so Geetorus groups them into the amber accordion.
