 ...
 *
 * Translates Geetorus's CreateConfigValues into the adapterConfig
...
/**
 * Build a Hermes Agent adapter config from the Geetorus UI form values.
 */
export function buildHermesConfig(
  ...
  // NOTE: Provider is NOT set here because the Geetorus UI form
  ...
  // Execution limits — let the user configure these from the Geetorus UI.
  ...
  // Heartbeat config is handled by Geetorus itself
