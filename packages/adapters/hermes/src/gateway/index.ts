import type { AdapterSessionManagement, ServerAdapterModule } from "@geetorusai/adapter-utils";
...
- Geetorus should create Hermes runs through POST /v1/runs and observe them through SSE events.
- You need remote Hermes session continuity with X-Hermes-Session-Key.

Don't use when:
- Hermes should run as a local child process on the Geetorus host; use hermes_local instead.
- The Hermes API server is not enabled or is only reachable over an unsafe public HTTP endpoint.
- You need Hermes-originated Geetorus task creation only; that is a Geetorus skill/API bridge, not this wake adapter.
...
- geetorusApiUrl (string): Geetorus API URL reachable from the Hermes host. This is not a credential.
...
- Sends Idempotency-Key equal to the Geetorus run id for correlation only; Hermes v0.16.0 did not dedupe duplicate creates.
...
- Do not put Hermes apiKey or Geetorus bearer tokens in prompts, comments, logs, or result JSON.
