...
describe("resolveSessionKey", () => {
  it("derives issue-scoped session keys by default", () => {
    ...
    ).toBe("geetorus:company:company-1:agent:agent-1:issue:issue-1");
  });
...
describe("execute", () => {
  ...
  it("constructs POST /v1/runs with auth, idempotency, and Hermes session headers", async () => {
    ...
    expect(init.headers).toMatchObject({
      ...
      "X-Hermes-Session-Key": "geetorus:company:company-1:agent:agent-1:issue:issue-1",
    });
    const body = JSON.parse(String(init.body));
    expect(body.input).toContain("Do the thing");
    expect(body.session_id).toBe("geetorus:company:company-1:agent:agent-1:issue:issue-1");
  });
  ...
  it("sends the task brief once on fresh runs and compacts it on stable-session resumes", async () => {
    ...
    const compactTaskMarkdown = ["Geetorus task context:", '- Issue: "PAP-1"'].join("\n");
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith("/v1/runs")) {
        return new Response(JSON.stringify({ run_id: "run-hermes-1", status: "started" }), { status: 200 });
    ...
    expect(runBodies[1]!.input).toContain("Geetorus task context:");
  ...
  it("redacts echoed auth material from stream logs and summaries", async () => {
    ...
    expect(result.summary).not.toContain("geetorus:company:company-1:agent:agent-1:issue:issue-1");
    ...
    expect(logText).not.toContain("geetorus:company:company-1:agent:agent-1:issue:issue-1");
  });

  it("redacts agent-scoped Geetorus session keys from logs and public result metadata", async () => {
    const ctx = makeCtx({
      apiBaseUrl: "http://127.0.0.1:8642",
      apiKey: "secret-key",
    ...
    const agentSessionKey = "geetorus:company:company-1:agent:agent-1";
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith("/v1/runs")) {
        return new Response(JSON.stringify({ run_id: "run-hermes-1", status: "started" }), { status: 200 });
  ...
  it("redacts echoed auth material from HTTP error payloads", async () => {
      ...
      vi.fn(async () =>
        new Response(
          JSON.stringify({
            ...
            detail: "X-Hermes-Session-Key: geetorus:company:company-1:agent:agent-1:issue:issue-1",
            nested: {
              note: "session geetorus:company:company-1:agent:agent-1",
            },
          }),
          { status: 401 },
        )),
    ...
    expect(result.errorMessage).not.toContain("geetorus:company:company-1:agent:agent-1:issue:issue-1");
  });
