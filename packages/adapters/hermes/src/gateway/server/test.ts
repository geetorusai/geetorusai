...
export async function testEnvironment(
  ...
  if (parsed && mappedDefaultDashboardRoot) {
    checks.push({
      ...
      hint: "Hermes dashboard routes such as /chat are browser UI routes. Geetorus gateway calls use /api/health and /api/v1/runs.",
    });
  }
  ...
  try {
    ...
    checks.push({
      ...
      hint: response.ok
        ? undefined
        : "Check apiBaseUrl, API_SERVER_KEY, and that the Hermes API server is reachable from Geetorus.",
    });
  } catch (err) {
    checks.push({
      ...
      hint: "Check apiBaseUrl and make sure the Hermes API server is running where Geetorus can reach it.",
    });
  }
