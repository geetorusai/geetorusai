...
const GEETORUS_SESSION_KEY_PATTERN =
  /\bgeetorus:(?:company:[A-Za-z0-9-]+:agent:[A-Za-z0-9-]+(?::(?:issue|run):[A-Za-z0-9-]+)?|run:[A-Za-z0-9-]+)\b/gi;
...
export function resolveSessionKey(input: {
  ...
  if (input.strategy === "agent") {
    return `geetorus:company:${input.companyId}:agent:${input.agentId}`;
  }
  if (input.strategy === "run") {
    return `geetorus:run:${input.runId}`;
  }
  const issuePart = input.issueId ? `issue:${input.issueId}` : `run:${input.runId}`;
  return `geetorus:company:${input.companyId}:agent:${input.agentId}:${issuePart}`;
}
...
function sanitizeSensitiveText(value: string): string {
  return value
    ...
    .replace(GEETORUS_SESSION_KEY_PATTERN, "[redacted-session-key]");
}
...
function buildInput(ctx: AdapterExecutionContext, geetorusApiUrl: string | null): string {
  ...
  const taskMarkdown = nonEmpty(selectGeetorusTaskMarkdown(ctx.context, { resumedSession }));
  const wakePrompt = renderGeetorusWakePrompt(ctx.context.geetorusWake, {
  ...
  const wakePayloadJson = stringifyGeetorusWakePayload(ctx.context.geetorusWake, {
    omitIssueDescription: Boolean(taskMarkdown),
  });
  const sessionHandoff = nonEmpty(ctx.context.geetorusSessionHandoffMarkdown);
  const issueWorkMode = readGeetorusIssueWorkModeFromContext(ctx.context);
  const lines = [
    `You are ${ctx.agent.name}, an AI agent employee in a Geetorus-managed company.`,
    "",
    "Geetorus runtime identity:",
    `- Agent ID: ${ctx.agent.id}`,
    `- Company ID: ${ctx.agent.companyId}`,
    `- Run ID: ${ctx.runId}`,
    ...(geetorusApiUrl ? [`- Geetorus API URL: ${geetorusApiUrl}`] : []),
    ...(issueWorkMode ? [`- Issue work mode: ${issueWorkMode}`] : []),
    "",
    ...(ctx.context.conversationMode === true || isGeetorusRecoveryWakePayload(ctx.context.geetorusWake)
      ? []
      : [
          "Execution contract:",
          ...
          "- Use X-Geetorus-Run-Id on mutating Geetorus API requests when a Geetorus API key is available.",
          "",
        ]),
    wakePrompt,
...
function buildRunBody(ctx: AdapterExecutionContext, sessionKey: string | null): Record<string, unknown> {
  const geetorusApiUrl = nonEmpty(ctx.config.geetorusApiUrl);
  ...
  const input = configuredInput && ctx.context.conversationMode === true
    ? `${configuredInput}\n\n${buildInput(ctx, geetorusApiUrl)}`
    : configuredInput ?? buildInput(ctx, geetorusApiUrl);
  const instructions =
    nonEmpty(ctx.config.instructions) ??
    nonEmpty(payloadTemplate.instructions) ??
    "Follow the Geetorus wake instructions exactly. Do not expose secrets in logs, comments, or final output.";
  return {
    ...payloadTemplate,
    input,
    instructions,
