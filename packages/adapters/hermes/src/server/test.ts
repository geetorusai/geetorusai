...
function checkModel(
  ...
  if (!model) {
    return {
      ...
      hint: "Set a model explicitly in Geetorus only if you want to override your local Hermes configuration.",
...
async function checkApiKeys(
  ...
  // not export them to the parent process, so Geetorus's process.env won't
...
async function checkProviderConsistency(
  ...
  if (!explicitProvider && resolvedFrom.startsWith("hermesConfigUnsupported:")) {
    ...
    return {
      ...
      hint: "Geetorus will avoid model-name provider inference here and let Hermes resolve the provider from ~/.hermes/config.yaml at runtime.",
