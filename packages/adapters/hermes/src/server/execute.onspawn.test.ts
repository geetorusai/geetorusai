 ...
 *
 * @see https://github.com/geetorusai/geetorus/issues/8723
 */
...
vi.mock("@geetorusai/adapter-utils/server-utils", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@geetorusai/adapter-utils/server-utils")>();
  return {
    ...actual,
...
describe("hermes-local adapter onSpawn forwarding", () => {
  ...
  it("does not inherit GEETORUS_API_KEY without a harness token", async () => {
    const previousApiKey = process.env.GEETORUS_API_KEY;
    process.env.GEETORUS_API_KEY = "parent-process-key";
    ...
    try {
      ...
      expect(opts.env.GEETORUS_API_KEY).toBeUndefined();
    } finally {
