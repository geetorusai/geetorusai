...
async function buildSkillEntry(
  ...
  return {
    ...
    readOnly: true, // Hermes manages its own skills — Geetorus can't toggle them
...
async function buildHermesSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  // 1. Scan Geetorus-managed skills (bundled with the adapter)
  const geetorusEntries = await readGeetorusRuntimeSkillEntries(config, __moduleDir);
  const desiredSkills = resolveLegacyGeetorusDesiredSkillNames(config, geetorusEntries);
  const desiredSet = new Set(desiredSkills);
  const availableByKey = new Map(geetorusEntries.map((e) => [e.key, e]));
  ...
  // 3. Merge: Geetorus skills first (ephemeral), then Hermes skills
  const entries: AdapterSkillEntry[] = [];
  const warnings: string[] = [];

  // Geetorus-managed skills
  for (const entry of geetorusEntries) {
  ...
  for (const entry of hermesSkillEntries) {
    // Skip if Geetorus already manages a skill with the same key
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(
      `Desired skill "${desiredSkill}" is not available in Geetorus or Hermes skills.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Cannot find this skill in Geetorus or ~/.hermes/skills/.",
    });
  }
...
export async function reconcileHermesGeetorusSkills(
  ...
  const desiredSkills = requestedDesiredSkills
    ...
    : resolveLegacyGeetorusDesiredSkillNames(config, availableEntries);
  const desiredSet = new Set(desiredSkills);
  ...
  for (const entry of availableEntries) {
    if (!desiredSet.has(entry.key) || isGeetorusSkillSourceMissing(entry)) continue;
    const target = path.join(skillsHome, entry.runtimeName);
    await ensureGeetorusSkillSymlink(entry.source, target);
...
export async function syncHermesSkills(
  ...
  await reconcileHermesGeetorusSkills(ctx.config, desiredSkills);
  return buildHermesSkillSnapshot(ctx.config);
}
