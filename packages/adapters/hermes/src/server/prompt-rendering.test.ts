...
test("renders standard assignment wake with task authority and no backlog discovery guidance", () => {
  ...
  expect(prompt).toContain("Geetorus task context:");
...
test("renders scoped planning wake authority before the Hermes default workflow", () => {
  const prompt = buildPrompt(baseContext(), {
    geetorusApiUrl: "http://127.0.0.1:3101/api",
  });
...
test("renders resume deltas instead of full scoped-wake boilerplate when continuing a session", () => {
  ...
  expect(prompt).toContain("## Geetorus Resume Delta");
  expect(prompt).toContain("You are resuming an existing Geetorus session.");
...
test("renders safe Geetorus API examples from environment variables with multiline update preservation", () => {
  const prompt = buildPrompt(baseContext(), {
    geetorusApiUrl: "http://geetorus.local/api",
  });

  expect(prompt).toContain("Use `$GEETORUS_API_URL`, `$GEETORUS_API_KEY`, and `$GEETORUS_RUN_ID`");
  expect(prompt).toContain("Displayed command logs may redact secrets");
  expect(prompt).toContain('-H "Authorization: Bearer $GEETORUS_API_KEY"');
  expect(prompt).toContain('-H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID"');
...
test("preserves custom prompt templates while exposing runtime and wake variables", () => {
  const prompt = buildPrompt(baseContext(), {
    ...
    promptTemplate: [
      ...
      "agent={{agent.name}}",
      "api={{geetorusApiUrl}}",
      "keyEnv={{geetorusApiKeyEnv}}",
      "runEnv={{geetorusRunIdEnv}}",
      "wakePrompt={{geetorusWakePrompt}}",
      "task={{geetorusTaskMarkdown}}",
      "wakeJson={{geetorusWakeJson}}",
  ...
  expect(prompt).toContain("api=http://geetorus.local/api");
  expect(prompt).toContain("keyEnv=GEETORUS_API_KEY");
  expect(prompt).toContain("runEnv=GEETORUS_RUN_ID");
  expect(prompt).toContain("wakePrompt=## Geetorus Wake Payload");
  expect(prompt).toContain("task=Geetorus task context:");
  expect(prompt).toContain("wakeJson={\"reason\":\"issue_assigned\"");
  expect(prompt).toContain('"reason":"issue_assigned"');
  expect(prompt).toContain("## Geetorus Wake Payload");
  expect(prompt).toContain("Issue description:\n```text\nUse the wake payload as runtime authority.\n```");
  expect(prompt).not.toContain("Geetorus runtime identity:");
});
...
test.each([false, true])("conversation prompts preserve the handoff policy (resumed=%s)", (resumedSession) => {
  ...
  const ctx = baseContext({
    ...
    geetorusTaskMarkdownCompact: directive,
  });
  ctx.context.geetorusWake.interactionKind = "request_confirmation";
  ctx.context.geetorusWake.interactionStatus = "accepted";
