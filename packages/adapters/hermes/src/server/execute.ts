 ...
 * and returns structured results to Geetorus.
 *
...
import { reconcileHermesGeetorusSkills } from "./skills.js";
...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
  'You are "{{agent.name}}", an AI agent employee in a Geetorus-managed company.',
  "",
  "Geetorus runtime identity:",
  "- Agent ID: {{agent.id}}",
  "- Company ID: {{agent.companyId}}",
  "- Run ID: {{run.id}}",
  "- API base: {{geetorusApiUrl}}",
  "",
  "Geetorus API guidance:",
  "- Use `curl` from the terminal for Geetorus API calls; browser/web extraction tools may not reach localhost.",
  "- Use `$GEETORUS_API_URL`, `$GEETORUS_API_KEY`, and `$GEETORUS_RUN_ID`; do not hard-code local ports or copy secrets into comments.",
  "- Displayed command logs may redact secrets; rely on environment variables instead of printed token values.",
  "- Include `-H \"Authorization: Bearer $GEETORUS_API_KEY\"` on API requests.",
  "- Include `-H \"X-Geetorus-Run-Id: $GEETORUS_RUN_ID\"` on mutating issue requests.",
...
export function buildPrompt(
  ...
  let geetorusApiUrl =
    cfgString(config.geetorusApiUrl) ||
    process.env.GEETORUS_API_URL ||
    "http://127.0.0.1:3100/api";
  // Ensure /api suffix
  if (!geetorusApiUrl.endsWith("/api")) {
    geetorusApiUrl = geetorusApiUrl.replace(/\/+$/, "") + "/api";
  }

  const geetorusTaskMarkdown = selectGeetorusTaskMarkdown(context, {
  ...
  const wakePrompt = renderGeetorusWakePrompt(context.geetorusWake, {
    ...
    suppressIssueDescription: geetorusTaskMarkdown.length > 0,
  });
  const sessionHandoffMarkdown = cfgString(context.geetorusSessionHandoffMarkdown)?.trim() || "";
  ...
  const vars: Record<string, unknown> = {
    ...
    projectName,
    geetorusApiUrl,
    geetorusWakePrompt: wakePrompt,
    geetorusTaskMarkdown,
    taskContext: geetorusTaskMarkdown,
    geetorusWakeJson: wakePayloadJson,
    wakePayloadJson,
    geetorusApiKeyEnv: "GEETORUS_API_KEY",
    geetorusRunIdEnv: "GEETORUS_RUN_ID",
  };

  const rendered = isGeetorusRecoveryWakePayload(context.geetorusWake)
    ? ""
...
function cleanResponse(raw: string): string {
  return raw
    ...
    .filter((line) => {
      ...
      if (t.startsWith("[tool]") || t.startsWith("[hermes]") || t.startsWith("[geetorus]")) return false;
...
export async function execute(
  ...
  // library calls that did not opt into Geetorus runtime skills.
  if (Object.prototype.hasOwnProperty.call(config, "geetorusRuntimeSkills")) {
    try {
      const selectedSkills = await reconcileHermesGeetorusSkills(config);
      if (selectedSkills.length > 0) {
        await ctx.onLog(
          "stdout",
          `[hermes] Reconciled ${selectedSkills.length} Geetorus-managed skill(s) into the Hermes skills home.\n`,
        );
      }
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      await ctx.onLog("stderr", `[hermes] Cannot start without the required Geetorus-managed skills: ${reason}\n`);
      throw err;
    }
  }
  ...
  // ── Load agent instructions file (Geetorus instruction bundles) ──────
  // Geetorus can materialize managed instructions into instructionsFilePath;
  ...
  if (instructionsFilePath) {
    try {
      ...
      // Geetorus UI doesn't render this as a red error (stderr output is
  ...
  // Geetorus agents run as non-interactive subprocesses with no TTY,
  ...
  if (ctx.runId) env.GEETORUS_RUN_ID = ctx.runId;

  // GEETORUS_API_KEY is never accepted from config — the harness-minted run
  // token is the only source of Geetorus API identity.
  delete env.GEETORUS_API_KEY;
  if ((ctx as any).authToken) env.GEETORUS_API_KEY = (ctx as any).authToken;
  ...
  if (envTaskId) env.GEETORUS_TASK_ID = envTaskId;
  const envWakeReason = cfgString(ctxContext.wakeReason) || cfgString(ctx.config?.wakeReason);
  if (envWakeReason) env.GEETORUS_WAKE_REASON = envWakeReason;
  const envCommentId = cfgString(ctxContext.commentId) || cfgString(ctxContext.wakeCommentId) || cfgString(ctx.config?.commentId);
  if (envCommentId) env.GEETORUS_WAKE_COMMENT_ID = envCommentId;
  const wakePayloadJson = stringifyGeetorusWakePayload(ctxContext.geetorusWake);
  ...
  // Geetorus renders all stderr as red/error in the UI.
  ...
  // Set resultJson so Geetorus can persist run metadata (used for UI display + auto-comments)
