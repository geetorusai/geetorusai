...
test("root package export exposes Geetorus external adapter entrypoint", () => {
...
test("Hermes adapter exposes bundled Geetorus task bridge skill", async () => {
  const adapter = createServerAdapter();
  ...
  expect(snapshot?.entries.some((entry) => entry.runtimeName === "geetorus-task-bridge")).toBe(true);
});

test("Hermes keeps the operational Geetorus skill linked after an empty replacement", async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-hermes-core-skill-"));
  try {
    const source = path.join(home, "runtime-skills", "geetorus");
    await fs.mkdir(source, { recursive: true });
    await fs.writeFile(path.join(source, "SKILL.md"), "# Geetorus\n", "utf8");
    const adapter = createServerAdapter();
    ...
    expect(snapshot?.desiredSkills).toContain("geetorusai/geetorus/geetorus");
    expect((await fs.lstat(path.join(home, ".hermes", "skills", "geetorus"))).isSymbolicLink()).toBe(true);
  } finally {
...
test("Hermes rejects a conflicting operational skill target", async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-hermes-core-conflict-"));
  try {
    const source = path.join(home, "runtime-skills", "geetorus");
    const target = path.join(home, ".hermes", "skills", "geetorus");
    await fs.mkdir(source, { recursive: true });
    await fs.writeFile(path.join(source, "SKILL.md"), "# Geetorus\n", "utf8");
...
test("Hermes rejects a live symlink owned by another operational skill", async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-hermes-core-link-conflict-"));
  try {
    const source = path.join(home, "runtime-skills", "geetorus");
    const conflictingSource = path.join(home, "external-skills", "geetorus");
    const target = path.join(home, ".hermes", "skills", "geetorus");
    await fs.mkdir(source, { recursive: true });
    await fs.writeFile(path.join(source, "SKILL.md"), "# Geetorus\n", "utf8");
