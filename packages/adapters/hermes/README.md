# Geetorus Adapters for Hermes Agent

A [Geetorus](https://geetorus.ing) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a Geetorus company.
...
Choose `hermes_local` when Geetorus and Hermes run on the same trusted host
and Geetorus should start `hermes chat` for each heartbeat. Choose
...
- **Skills integration** — Scans both Geetorus-managed and Hermes-native skills (`~/.hermes/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Hermes stdout is parsed into typed `TranscriptEntry` objects so Geetorus renders proper tool cards with status icons and expand/collapse
...
This package ships with Geetorus core as the built-in `hermes_local` and
...
manager, or add an entry to `~/.geetorus/adapter-plugins.json` and restart
Geetorus. The external package can override either built-in Hermes adapter
while it is enabled:

```json
[
  {
    "packageName": "@geetorusai/hermes-geetorus-adapter",
    "localPath": "/absolute/path/to/geetorus/packages/adapters/hermes",
    "type": "hermes_local",
...
Geetorus core imports these same package entrypoints for built-in adapter
registration.

### 2. Create a local Hermes agent in Geetorus

In the Geetorus UI or via API, create an agent with adapter type `hermes_local`:
...
This mode shells out to the local `hermes` CLI. Geetorus injects runtime
environment variables and captures stdout/stderr from the child process.

### 3. Create a Hermes gateway agent in Geetorus
...
```json
{
  ...
  "adapterConfig": {
    ...
    "geetorusApiUrl": "http://127.0.0.1:3100",
...
`http://127.0.0.1:9119/chat`, Geetorus maps it to
`http://127.0.0.1:9119/api` before calling Hermes API routes. `/chat` and the
dashboard root are browser UI routes; Geetorus tests `/api/health` and starts
...
`@geetorusai/adapter-hermes-gateway` remains as a deprecated compatibility shim
for one release. It re-exports the gateway entrypoints from
`@geetorusai/hermes-geetorus-adapter/gateway` and preserves the legacy exports
for existing plugin installs. New installs and built-in Geetorus registrations
should use `@geetorusai/hermes-geetorus-adapter`; the adapter type remains
`hermes_gateway`.

### Runtime API guidance

Hermes receives Geetorus runtime identity through environment variables:
...
`Authorization: Bearer $GEETORUS_API_KEY` on Geetorus API requests and
`X-Geetorus-Run-Id: $GEETORUS_RUN_ID` on mutating issue requests. For
multiline comments or status updates, preserve newlines with a heredoc plus
`jq --arg`.

### Hermes-originated Geetorus tasks

The package includes a Hermes skill/helper for the reverse direction: a user
starts in Hermes and asks Hermes to create or update Geetorus work. This is not
the same as Geetorus waking Hermes through `hermes_local` or `hermes_gateway`.

Configure Geetorus access in Hermes env/profile secrets, not prompt text:

```bash
GEETORUS_API_URL=http://127.0.0.1:3100/api
GEETORUS_BRIDGE_API_KEY=<task-bridge-scoped-agent-api-key>
```
...
The bundled `geetorus-task-bridge` skill provides deterministic helper
commands:

```bash
node ./geetorus-task.mjs list-assigned
node ./geetorus-task.mjs create-task --parent-id "<approved-parent-issue-id>" --title "Investigate checkout failures" --description "Capture failing request and root cause."
...
Create issues in Geetorus and assign them to your Hermes agent. On each heartbeat, Hermes will:

1. Receive the task instructions
2. Use its full tool suite to complete the work
3. Report results back to Geetorus
...
| `geetorusApiUrl` | string | `http://127.0.0.1:3100/api` | Geetorus API base URL |
...
| `{{geetorusApiUrl}}` | Geetorus API base URL |
...
6. **Reports** results back to Geetorus with cost, usage, and session state
...
- **Geetorus-managed skills** — bundled with the adapter, togglable from the UI
...
Geetorus UI can display both managed and native skills in one view.

## Development

```bash
git clone https://github.com/geetorusai/geetorus
cd geetorus/packages/adapters/hermes
pnpm install
pnpm build
```
...
- [Geetorus](https://github.com/geetorusai/geetorus) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [Geetorus Docs](https://docs.geetorus.ing) — Geetorus documentation
