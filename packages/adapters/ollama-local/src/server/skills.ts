...
function resolveOllamaSkillsHome(config: Record<string, unknown>) {
  ...
  return path.join(home, ".ollama", "skills");
}
...
async function buildOllamaSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    installed,
    skillsHome,
    locationLabel: "~/.ollama/skills",
    missingDetail: "Configured but not currently linked into the Ollama skills home.",
...
export async function syncOllamaSkills(
  ...
  const skillsHome = resolveOllamaSkillsHome(ctx.config);
