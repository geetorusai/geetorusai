...
vi.mock("@geetorusai/adapter-utils/execution-target", async () => {
  ...
  return {
    ...actual,
    startAdapterExecutionTargetGeetorusBridge: async () => null,
  };
});
...
describe("cursor execute", () => {
  it("installs the default agent command on a fresh sandbox lease before execution", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-fresh-lease-"));
    ...
    try {
      ...
      expect(prompt).toContain("Follow the geetorus heartbeat.");
    } finally {
  ...
  it("reruns sandbox command resolution after managed runtime setup and keeps the original sandbox home", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-fresh-lease-managed-"));
