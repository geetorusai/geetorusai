...
Use when:
- You want Geetorus to run Pi (the AI coding agent) locally as the agent runtime
...
- Geetorus requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/geetoruss/ and resumed with --session.
