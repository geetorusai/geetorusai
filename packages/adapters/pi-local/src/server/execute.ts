...
const GEETORUS_SESSIONS_DIR = path.join(os.homedir(), ".pi", "geetoruss");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[geetorus] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[geetorus] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[geetorus] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-pi-skills-"));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(GEETORUS_SESSIONS_DIR, { recursive: true });
  return GEETORUS_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(GEETORUS_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readGeetorusRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveLegacyGeetorusDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // Materialize custom Pi providers (GEETORUS_PI_PROVIDERS) into a managed
  ...
  try {
    ...
    // invoke skill binaries (e.g. `geetorus-get-issue`) by name. Without this,
    ...
    if (executionTargetIsRemote) {
      try {
        ...
        await onLog(
          "stdout",
          `[geetorus] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
    ...
    if (runtimeSessionId && !canResumeSession) {
      const staleSessionCwdNote =
        savedSessionCwd !== null && !sessionHeaderCwdMatches
          ? ` Pi stored cwd "${savedSessionCwd}" in the session header, so Geetorus will start a fresh session for "${effectiveExecutionCwd}".`
          : "";
      await onLog(
        "stdout",
        executionTargetIsRemote
          ? `[geetorus] Pi session "${runtimeSessionId}" does not match the current remote execution state and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote} Starting a fresh remote session.\n`
          : `[geetorus] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote}\n`,
      );
    }
    ...
    const taskContextNote = context.conversationMode === true
      ? selectGeetorusTaskMarkdown(context, { resumedSession: canResumeSession })
      : "";
    ...
    const renderedHeartbeatPrompt = shouldUseResumeDeltaPrompt || isGeetorusRecoveryWakePayload(context.geetorusWake)
      ? ""
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[geetorus] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
        );
