...
function parseProviderConfig(
  ...
  try {
    ...
    if (!isPlainObject(parsed)) {
      return {
        providers: null,
        warning: "GEETORUS_PI_PROVIDERS is set but is not a JSON object; custom providers ignored.",
      };
    }
    ...
    return {
      ...
      warning: skipped.length > 0
        ? `GEETORUS_PI_PROVIDERS: skipped provider(s) with non-object values: ${skipped.join(", ")}.`
        : null,
    };
  } catch {
    return {
      providers: null,
      warning: "GEETORUS_PI_PROVIDERS contains invalid JSON; custom providers ignored.",
    };
  }
}

// Materialize custom Pi providers supplied via GEETORUS_PI_PROVIDERS (a JSON
...
//
// When GEETORUS_PI_PROVIDERS is set we write {"providers": ...} to a fresh temp
...
export async function preparePiRuntimeConfig(input: {
  ...
  const { providers, warning } = parseProviderConfig(
    input.env.GEETORUS_PI_PROVIDERS ?? process.env.GEETORUS_PI_PROVIDERS,
    resolveEnv,
  );
  if (!providers) {
    return {
  ...
  const agentConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-pi-agent-config-"));
  try {
    await fs.writeFile(
  ...
  return {
    ...
    notes: [
      ...(warning ? [warning] : []),
      `Injected ${Object.keys(providers).length} custom Pi provider(s) from GEETORUS_PI_PROVIDERS into a managed models.json: ${Object.keys(providers).join(", ")}.`,
    ],
    agentConfigDir,
    cleanup: async () => {
