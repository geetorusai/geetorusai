...
describe("preparePiRuntimeConfig", () => {
  it("is a no-op when GEETORUS_PI_PROVIDERS is unset", async () => {
  ...
  it("writes the providers JSON verbatim to a managed models.json and points PI_CODING_AGENT_DIR at it", async () => {
    ...
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: JSON.stringify(providers) },
    });
  ...
  it("reads GEETORUS_PI_PROVIDERS from process.env when absent from the run env", async () => {
    const providers = { tensorix: { baseUrl: "http://gw/anthropic", api: "anthropic-messages", models: [] } };
    process.env.GEETORUS_PI_PROVIDERS = JSON.stringify(providers);
    try {
      ...
      delete process.env.GEETORUS_PI_PROVIDERS;
    }
  });
  ...
  it("expands {env:VAR} placeholders from process.env when absent from the run env", async () => {
    const providers = {
      tensorix: { baseUrl: "http://gw/anthropic", apiKey: "{env:GEETORUS_PI_TEST_KEY}", api: "anthropic-messages", models: [] },
    };
    process.env.GEETORUS_PI_TEST_KEY = "sk-from-process-env";
    try {
      const prepared = await preparePiRuntimeConfig({
        env: { GEETORUS_PI_PROVIDERS: JSON.stringify(providers) },
      });
      ...
      delete process.env.GEETORUS_PI_TEST_KEY;
    }
  });
  ...
  it("ignores malformed GEETORUS_PI_PROVIDERS without writing a config", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: "not json" },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
    expect(prepared.notes).toEqual([
      "GEETORUS_PI_PROVIDERS contains invalid JSON; custom providers ignored.",
    ]);
    await prepared.cleanup();
  });
  ...
  it("ignores provider entries that are not objects and names them in the note", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: JSON.stringify({ tensorix: "nope" }) },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
    expect(prepared.agentConfigDir).toBeNull();
    expect(prepared.notes).toEqual([
      "GEETORUS_PI_PROVIDERS: skipped provider(s) with non-object values: tensorix.",
    ]);
    await prepared.cleanup();
  });
  ...
  it("surfaces skipped non-object entries while keeping the usable ones", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: {
        GEETORUS_PI_PROVIDERS: JSON.stringify({
          bad: "http://gw/v1",
    ...
    expect(prepared.notes).toEqual([
      "GEETORUS_PI_PROVIDERS: skipped provider(s) with non-object values: bad.",
      "Injected 1 custom Pi provider(s) from GEETORUS_PI_PROVIDERS into a managed models.json: tensorix.",
    ]);
    await prepared.cleanup();
  });

  it("surfaces a note when GEETORUS_PI_PROVIDERS contains invalid JSON", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: "{not json" },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
  ...
  it("surfaces a note when GEETORUS_PI_PROVIDERS is not a JSON object", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: "[1,2]" },
    });
    expect(prepared.notes).toEqual([
      "GEETORUS_PI_PROVIDERS is set but is not a JSON object; custom providers ignored.",
    ]);
    await prepared.cleanup();
  });
  ...
  it("surfaces the skipped entries when no provider objects remain", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: '{"a": 1}' },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
    expect(prepared.notes).toEqual([
      "GEETORUS_PI_PROVIDERS: skipped provider(s) with non-object values: a.",
    ]);
    await prepared.cleanup();
  });

  it("stays silent when GEETORUS_PI_PROVIDERS is an empty object", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { GEETORUS_PI_PROVIDERS: "{}" },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
    expect(prepared.notes).toEqual([]);
    await prepared.cleanup();
