...
Why: Geetorus resolves a local execution workspace (a worktree) for each
...
- Never `git push` from adapter runtime code. Never assume the local worktree
  ...
  read it from the local cwd Geetorus handed you.
- If your adapter runs the agent on a different host (ssh, sandbox, remote
  container), use the round-trip helpers in `@geetorusai/adapter-utils`:
...
`// geetorus:allow-git-push: <reason>` comment on the line (or the line
