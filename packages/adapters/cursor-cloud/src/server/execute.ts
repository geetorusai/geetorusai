...
import type { AdapterExecutionContext, AdapterExecutionResult, AdapterInvocationMeta } from "@geetorusai/adapter-utils";
import {
...
function buildWakeEnv(ctx: AdapterExecutionContext, configEnv: Record<string, string>): Record<string, string> {
  ...
  // issued a Geetorus run JWT (registry: supportsLocalAgentJwt=false).
  // buildGeetorusEnv always sets GEETORUS_API_URL, defaulting to the local
  // runtime host — which a remote worker can neither reach nor authenticate
  // against, so any agent-initiated Geetorus API call would fail with a 401
  // (or be unreachable) and add noise. When there is no usable key, drop the
  // callback wiring so cloud-side Geetorus tools degrade to a clean no-op.
  // Run results are delivered server-side via the Cursor Agent SDK (getRun /
  // wait), not through this callback, so nothing is lost.
  if (!trimNullable(env.GEETORUS_API_KEY)) {
    delete env.GEETORUS_API_URL;
    delete env.GEETORUS_API_BRIDGE_MODE;
  }

  const workspace = parseObject(context.geetorusWorkspace);
  const workspaceMappings: Array<[string, unknown]> = [
    ["GEETORUS_WORKSPACE_CWD", workspace.cwd],
    ["GEETORUS_WORKSPACE_SOURCE", workspace.source],
    ["GEETORUS_WORKSPACE_ID", workspace.workspaceId],
    ["GEETORUS_WORKSPACE_REPO_URL", workspace.repoUrl],
    ["GEETORUS_WORKSPACE_REPO_REF", workspace.repoRef],
    ["GEETORUS_WORKSPACE_BRANCH", workspace.branch],
    ["GEETORUS_WORKSPACE_WORKTREE_PATH", workspace.worktreePath],
  ...
  // Geetorus may use empty values to unset optional host credentials; remote
...
function renderGeetorusEnvNote(env: Record<string, string>): string {
  ...
  return [
    ...
    `The following GEETORUS_* environment variables are available in the cloud agent shell: ${keys.join(", ")}`,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const taskContextNote = context.conversationMode === true
    ? selectGeetorusTaskMarkdown(context, { resumedSession: canReuseSession })
    : "";
  ...
  const renderedPrompt =
    (canReuseSession && wakePrompt.length > 0) || isGeetorusRecoveryWakePayload(context.geetorusWake)
      ? ""
      : renderTemplate(promptTemplate, templateData).trim();
  const geetorusEnvNote = renderGeetorusEnvNote(remoteEnv);
