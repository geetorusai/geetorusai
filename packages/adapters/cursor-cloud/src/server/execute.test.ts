...
describe("cursor_cloud execute", () => {
  ...
  it.each([false, true])("sends the central chat directive to Cursor Cloud (custom=%s)", async (custom) => {
    ...
    expect(prompt).toContain(custom ? "Do the work for" : "Continue your Geetorus conversation");
    expect(prompt).not.toContain("Execution contract:");
    expect(prompt).not.toContain("Create child issues");
  });

  it("creates a fresh Cursor agent and injects Geetorus env without CURSOR_API_KEY", async () => {
    ...
    expect(createMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "Geetorus Cursor Cloud Agent",
      model: { id: "gpt-5.4" },
      cloud: {
        env: { type: "cloud" },
        repos: [{ url: "https://github.com/geetorusai/geetorus.git", startingRef: "main" }],
      },
    });
    expect(createMock.mock.calls[0]?.[0]?.cloud?.envVars).toMatchObject({
      EXTRA_FLAG: "1",
      GEETORUS_RUN_ID: "run-heartbeat-1",
      GEETORUS_TASK_ID: "issue-1",
      GEETORUS_WAKE_REASON: "issue_commented",
      GEETORUS_API_KEY: "geetorus-run-jwt",
    });
    // When a run JWT is present the callback URL is retained so the worker can
    // authenticate its Geetorus API calls.
    expect(createMock.mock.calls[0]?.[0]?.cloud?.envVars).toHaveProperty("GEETORUS_API_URL");
  ...
  it("omits the Geetorus API callback when no run JWT is issued (remote worker cannot call home)", async () => {
    ...
    expect(envVars).not.toHaveProperty("GEETORUS_API_KEY");
    expect(envVars).not.toHaveProperty("GEETORUS_API_URL");
    expect(envVars).not.toHaveProperty("GEETORUS_API_BRIDGE_MODE");
    // Informational Geetorus env (non-credential) still flows through.
  ...
  it("makes Cursor repository setup failures actionable without launching another run", async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.errorMessage).toContain("Cursor's GitHub integration can access https://github.com/geetorusai/geetorus.git");
    expect(result.errorMessage).toContain("https://cursor.com/dashboard/cloud-agents");
    expect(sdkAgent.send).toHaveBeenCalledTimes(1);
  });

  it("maps non-finished Cursor results to failing Geetorus runs", async () => {
