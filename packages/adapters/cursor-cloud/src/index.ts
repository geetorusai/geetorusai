...
Use when:
- You want Geetorus to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across Geetorus heartbeats
- You want Geetorus to keep task state while Cursor handles remote code execution
...
Notes:
- Geetorus reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each Geetorus heartbeat maps to a Cursor run on that durable agent.
- Geetorus injects GEETORUS_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- Geetorus remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
