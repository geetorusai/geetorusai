...
vi.mock("@geetorusai/adapter-utils/execution-target", async (importOriginal) => {
  const actual = (await importOriginal()) as Record<string, unknown>;
  return { ...actual, runAdapterExecutionTargetProcess: vi.fn() };
});

import { ensureRemoteOpenAICompatibleModelConfiguredAndAvailable, execute } from "./execute.js";
import { runAdapterExecutionTargetProcess } from "@geetorusai/adapter-utils/execution-target";
...
describe("OpenAI Compatible local skill injection", () => {
  let configHome: string;

  beforeEach(async () => {
    configHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-openai_compatible-test-config-"));
  ...
  it.each([false, true])("keeps chat policy with a legacy OpenAI Compatible prompt (custom=%s)", async (custom) => {
    const commandPath = path.join(configHome, "fake-openai_compatible");
    ...
    const result = await execute({
      runId: "chat-run",
      agent: { id: "agent-1", companyId: "company-1", name: "OpenAI Compatible", adapterType: "openai_compatible_local", adapterConfig: {} },
      runtime: { sessionId: null, sessionParams: null, sessionDisplayId: null, taskKey: null },
      config: {
        command: commandPath, cwd: configHome, model: "openai/gpt-5", env: { OPENAI_COMPATIBLE_ALLOW_ALL_MODELS: "1" },
        ...(custom ? { promptTemplate: "Custom agent instruction." } : {}),
      },
      context: {
        conversationMode: true,
        geetorusTaskMarkdown: directive,
        geetorusWake: {
    ...
    expect(prompt).toContain(custom ? "Custom agent instruction." : "Continue your Geetorus conversation");
    expect(prompt).not.toContain("Execution contract:");
    expect(prompt).not.toContain("Create child issues");
  });

  it("injects runtime skills into the configured child HOME", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-openai_compatible-configured-home-"));
    const processHome = path.join(root, "process-home");
    const configuredHome = path.join(root, "configured-home");
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "openai_compatible");
    const skillSource = await createSkillDir(path.join(root, "runtime-skills"), "geetorus");
    ...
    try {
      const result = await execute({
        ...
        config: {
          ...
          env: {
            HOME: configuredHome,
            OPENAI_COMPATIBLE_ALLOW_ALL_MODELS: "1",
          },
      ...
      expect(result.exitCode).toBe(0);
      const installedSkill = path.join(configuredHome, ".claude", "skills", "geetorus");
      ...
      await expect(fs.lstat(path.join(processHome, ".claude", "skills", "geetorus"))).rejects.toThrow();
    } finally {
  ...
  it("passes an OpenRouter key and complete model to OpenAI Compatible without logging the key", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-openai_compatible-openrouter-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "openai_compatible");
...
describe("ensureRemoteOpenAICompatibleModelConfiguredAndAvailable", () => {
  afterEach(() => {
    delete process.env.OPENAI_COMPATIBLE_ALLOW_ALL_MODELS;
  });

  // The remote/sandbox execution path must honour OPENAI_COMPATIBLE_ALLOW_ALL_MODELS just
  // like the local path: gateway-routed models (e.g. anthropic/<gateway>/<model>
  // via Bifrost) never appear in `openai_compatible models`, so the availability probe
  ...
  it("skips the remote availability probe when OPENAI_COMPATIBLE_ALLOW_ALL_MODELS is set in the run env", async () => {
    await expect(
      ensureRemoteOpenAICompatibleModelConfiguredAndAvailable({
        runId: "run-1",
...
describe("ensureRemoteOpenAICompatibleModelConfiguredAndAvailable — probe is non-fatal when it cannot run", () => {
  ...
  it("proceeds when the remote probe exits non-zero (e.g. a transient `Unexpected error`)", async () => {
    ...
    await expect(
      ensureRemoteOpenAICompatibleModelConfiguredAndAvailable({ ...base, model: "openai/gpt-5" }),
    ).resolves.toBeUndefined();
  });
