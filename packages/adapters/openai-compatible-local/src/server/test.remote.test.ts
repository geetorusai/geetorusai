...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
      ...
      runtimeRootDir: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/openai_compatible",
      assetDirs: {
        xdgConfig: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/openai_compatible/xdgConfig",
      },
      restoreWorkspace,
    })),
  };
...
describe("openai_compatible remote environment diagnostics", () => {
  const configHomes: string[] = [];
  let configHome: string;

  beforeEach(async () => {
    configHome = await mkdtemp(path.join(os.tmpdir(), "geetorus-openai_compatible-test-config-"));
  ...
  it.each([false, true])("stages remote runtime config assets for sandbox hello probes (managed=%s)", async (managed) => {
    const configHome = await mkdtemp(path.join(os.tmpdir(), "openai_compatible-remote-test-config-"));
    ...
    expect(runtimeInput?.adapterKey).toBe("openai_compatible");
    ...
    const probeCall = runAdapterExecutionTargetProcess.mock.calls[0] as unknown as
      ...
      | undefined;
    expect(probeCall?.[4].cwd).toBe("/remote/workspace/.geetorus-runtime/runs/test/workspace");
    if (managed) {
      expect(probeCall?.[4].env.HOME).toContain("/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/openai_compatible/managed-auth/");
    ...
    expect(probeCall?.[4].env.XDG_CONFIG_HOME).toBe(
      "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/openai_compatible/xdgConfig",
    );
  });
});
