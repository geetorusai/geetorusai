...
import { prepareOpenAICompatibleRuntimeConfig } from "./runtime-config.js";
...
async function makeConfigHome(initialConfig?: Record<string, unknown>) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-openai_compatible-test-"));
  cleanupPaths.add(root);
  const configDir = path.join(root, "openai_compatible");
  await fs.mkdir(configDir, { recursive: true });
  if (initialConfig) {
    await fs.writeFile(
      path.join(configDir, "openai_compatible.json"),
...
describe("prepareOpenAICompatibleRuntimeConfig", () => {
  it("injects an external_directory allow rule by default", async () => {
    ...
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
    ...
    const runtimeConfig = JSON.parse(
      await fs.readFile(
        path.join(prepared.env.XDG_CONFIG_HOME, "openai_compatible", "openai_compatible.json"),
        "utf8",
      ),
  ...
  it("merges custom providers from GEETORUS_OPENAI_COMPATIBLE_PROVIDERS into the config", async () => {
    ...
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: {
        ...
        GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: JSON.stringify(providers),
      },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    ...
    const runtimeConfig = JSON.parse(
      await fs.readFile(path.join(prepared.env.XDG_CONFIG_HOME, "openai_compatible", "openai_compatible.json"), "utf8"),
    ) as Record<string, unknown>;
    expect(runtimeConfig).toMatchObject({
  ...
  it("reads GEETORUS_OPENAI_COMPATIBLE_PROVIDERS from process.env when absent from the run env", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const providers = { bifrost: { npm: "@ai-sdk/openai-compatible", models: { "example/model-a": {} } } };
    process.env.GEETORUS_OPENAI_COMPATIBLE_PROVIDERS = JSON.stringify(providers);
    try {
      const prepared = await prepareOpenAICompatibleRuntimeConfig({
        env: { XDG_CONFIG_HOME: configHome },
        config: {},
      ...
      delete process.env.GEETORUS_OPENAI_COMPATIBLE_PROVIDERS;
    }
  });
  ...
  it("expands {env:VAR} placeholders in custom providers using the run/process env (bakes the literal vk)", async () => {
    ...
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: JSON.stringify(providers), ANTHROPIC_API_KEY: "sk-bf-REALVK" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
    ...
    // The {env:...} placeholder must be replaced with the literal value, so OpenAI Compatible
  ...
  it("leaves an unresolvable {env:VAR} placeholder intact", async () => {
    ...
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: JSON.stringify(providers) },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
  ...
  it("pins small_model from GEETORUS_OPENAI_COMPATIBLE_SMALL_MODEL", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, GEETORUS_OPENAI_COMPATIBLE_SMALL_MODEL: "example/model-a" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
  ...
  it("ignores malformed GEETORUS_OPENAI_COMPATIBLE_PROVIDERS without writing a provider block and surfaces a note", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: "not json" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
    ...
    expect(prepared.notes).toContain(
      "GEETORUS_OPENAI_COMPATIBLE_PROVIDERS contains invalid JSON; custom providers ignored.",
    );
    await prepared.cleanup();
  });

  it("surfaces a note when GEETORUS_OPENAI_COMPATIBLE_PROVIDERS is valid JSON but not an object", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: "[1,2,3]" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
    ...
    expect(runtimeConfig.provider).toBeUndefined();
    expect(prepared.notes).toContain(
      "GEETORUS_OPENAI_COMPATIBLE_PROVIDERS is set but is not a JSON object; custom providers ignored.",
    );
    await prepared.cleanup();
  });
  ...
  it("surfaces skipped provider entries with non-object values and keeps the usable ones", async () => {
    ...
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: {
        XDG_CONFIG_HOME: configHome,
        GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: JSON.stringify({
    ...
    expect(prepared.notes).toContain(
      "GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: skipped provider(s) with non-object values: bifrost.",
    );
    await prepared.cleanup();
  });
  ...
  it("surfaces skipped provider entries when no usable entries remain", async () => {
    ...
    const prepared = await prepareOpenAICompatibleRuntimeConfig({
      env: {
        XDG_CONFIG_HOME: configHome,
        GEETORUS_OPENAI_COMPATIBLE_PROVIDERS: JSON.stringify({ bifrost: "http://gateway.example/v1" }),
      },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
  ...
  it("registers a configured model missing from the catalog on its provider", async () => {
    ...
    expect(prepared.notes).toContain(
      "Registered configured model openrouter/openai/gpt-oss-120b:nitro in the runtime OpenAI Compatible config.",
    );
    await prepared.cleanup();
  });
