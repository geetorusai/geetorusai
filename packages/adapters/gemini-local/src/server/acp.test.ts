...
describe("gemini_local ACP lane", () => {
  ...
  it("keeps ACP selected and reports unavailable prerequisites for default and explicit engines", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-default-");
  ...
  it("executes Gemini through the shared ACP runtime", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-run-");
  ...
  it("creates the ACP session on the in-sandbox workspace cwd for runner-backed remote runs", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-remote-cwd-");
    process.env.HOME = path.join(root, "home");
  ...
  it("seeds the managed Gemini home into the sandbox, repoints HOME, and keeps the key file-only", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-home-seed-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    ...
    const result = await execute(
      buildContext(localCwd, {
        config: {
          ...
          promptTemplate: "Do the assigned work.",
          geetorusRuntimeSkills: [{ key: "company/review", runtimeName: "review", source: skillSource }],
          geetorusSkillSync: { desiredSkills: ["company/review"] },
        },
        context: {
          issueId: "issue-1",
          geetorusWorkspace: { cwd: localCwd, source: "project_workspace", workspaceId: "workspace-1" },
    ...
    expect(remappedHome).toContain(".geetorus-runtime");
  ...
  it("test_gemini_acp_seam_registers_workspace_sync_back", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-syncback-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    await fs.mkdir(remoteCwd, { recursive: true });
  ...
  it("passes the Gemini-specific teardown messages to the shared workspace-restore-teardown factory", async () => {
    ...
    const root = await makeTempRoot("geetorus-gemini-acp-teardown-wiring-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    await fs.mkdir(remoteCwd, { recursive: true });
    ...
    expect(mockCreateWorkspaceRestoreTeardown).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        failurePrefix: "[geetorus] Gemini ACP teardown workspace restore failed",
      }),
    );
  });

  it("does not persist an api-key auth selector from a host-only credential", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-hostkey-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const hostHome = path.join(root, "home");
    await fs.mkdir(localCwd, { recursive: true });
  ...
  it("reports Gemini ACP environment readiness", async () => {
    const root = await makeTempRoot("geetorus-gemini-acp-env-");
