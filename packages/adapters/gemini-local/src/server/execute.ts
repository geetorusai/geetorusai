...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "GEETORUS_API_URL") || !hasNonEmptyEnvValue(env, "GEETORUS_API_KEY")) return "";
  return [
    "Geetorus API access note:",
    "Use run_shell_command with curl to make Geetorus API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $GEETORUS_API_KEY\\" \\"$GEETORUS_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $GEETORUS_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Geetorus-Run-Id: $GEETORUS_RUN_ID\\" -d '{...}' \\"$GEETORUS_API_URL/api/issues/$GEETORUS_TASK_ID/checkout\\"" })`,
    "When GEETORUS_TASK_ID is not set, substitute a real issue id from the current context; never send a placeholder like {id} in the URL.",
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject Geetorus skills directly into the effective Gemini skills home.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[geetorus] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[geetorus] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[geetorus] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[geetorus] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildGeminiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readGeetorusRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveLegacyGeetorusDesiredSkillNames(config, geminiSkillEntries);
  if (!executionTargetIsRemote) {
    await ensureGeminiSkillsInjected(
      onLog,
      geminiSkillEntries.filter((entry) => !isGeetorusSkillSourceMissing(entry)),
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[geetorus] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && adapterExecutionTargetUsesGeetorusBridge(executionTarget)) {
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[geetorus] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[geetorus] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const geetorusEnvNote = renderGeetorusEnvNote(env);
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[geetorus] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
