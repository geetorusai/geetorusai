...
export const DEFAULT_CODEX_LOCAL_MODEL = GEETORUS_RUNNER_DEFAULT_MODELS.codex;
...
- networkScope (string, optional): "deny" blocks all network egress; "allowlist" permits only networkAllowlist targets through Geetorus's HTTP(S) proxy. Off by default.
...
- If instructionsFilePath is configured, Geetorus prepends that file's contents to the stdin prompt on every run.
- Codex exec automatically applies repo-scoped AGENTS.md instructions from the active workspace. Geetorus cannot suppress that discovery in exec mode, so repo AGENTS.md files may still apply even when you only configured an explicit instructionsFilePath.
- Geetorus injects desired local skills into the effective CODEX_HOME/skills/ directory at execution time so Codex can discover "$geetorus" and related skills without polluting the project working directory. For new and updated agents, Geetorus assigns an isolated managed home at ~/.geetorus/instances/<id>/companies/<companyId>/agents/<agentId>/codex-home/skills/; when CODEX_HOME is explicitly overridden in adapter config, that override is used instead.
...
- Fast mode is supported on GPT-6 Astra, GPT-5.6 (sol/terra/luna), GPT-5.5, GPT-5.4 and manual model IDs. When enabled for those models, Geetorus applies \`service_tier="fast"\` and \`features.fast_mode=true\`.
- When Geetorus realizes a workspace/runtime for a run, it injects GEETORUS_WORKSPACE_* and GEETORUS_RUNTIME_* env vars for agent-side tooling.
- The ACP engine keeps its workspace sandbox and enables network access on each turn. Explicit sandbox_workspace_write.network_access overrides in extraArgs (or env.GEETORUS_CODEX_ACP_NETWORK_ACCESS="false") disable it; execution-target network denial wins. The bundled ACP patch is needed because upstream mode presets override Codex config.toml on every turn.
- The CLI engine defaults to a writable workspace sandbox with network access for unattended work and Geetorus API calls. It does not enable the dangerous bypass flag. Explicit sandbox modes/profiles and network overrides in extraArgs retain their meaning. An execution-target network denial remains enforced.
