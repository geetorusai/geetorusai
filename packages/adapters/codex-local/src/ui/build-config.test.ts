...
import { buildCodexLocalConfig, buildGeetorusRunnerConfig } from "./build-config.js";
...
describe("buildGeetorusRunnerConfig", () => {
  it("keeps only settings implemented by the Codex runner profile", () => {
    const config = buildGeetorusRunnerConfig(makeValues({
      codexEngine: "acp",
  ...
  it("persists bounded Codex permission and warm lifecycle values", () => {
    const config = buildGeetorusRunnerConfig(makeValues({
      ...
      codexPermissionMode: "never",
      geetorusRunnerLifecycleMode: "warm",
      geetorusRunnerIdleTimeoutMs: 45_000,
    }));
  ...
  it("rejects an unsupported persisted Codex permission instead of coercing it", () => {
    expect(() => buildGeetorusRunnerConfig(makeValues({
      adapterSchemaValues: {
        provider: "unknown",
  ...
  it.each(["claude-opus-5", "my-custom-model"])("preserves the selected ACPX Claude model %s", (model) => {
    expect(buildGeetorusRunnerConfig(makeValues({ model, adapterSchemaValues: { provider: "acpx" } })))
      .toMatchObject({ provider: "acpx", acpxAgent: "claude", model });
  });

  it("normalizes the removed ACPX Codex configuration to native Codex", () => {
    const config = buildGeetorusRunnerConfig(makeValues({ model: "gpt-5.6-sol", adapterSchemaValues: { provider: "acpx", acpxAgent: "codex" } }));
  ...
  it("uses the Codex default when no model was selected", () => {
    expect(buildGeetorusRunnerConfig(makeValues({ model: "" }))).toMatchObject({
      provider: "codex",
      model: "gpt-5.6-sol",
    });
  });
  ...
  it("bounds warm lifecycle values to the shared safe default", () => {
    expect(buildGeetorusRunnerConfig(makeValues({
      geetorusRunnerLifecycleMode: "warm",
      geetorusRunnerIdleTimeoutMs: 86_400_001,
    }))).toMatchObject({
      lifecycleMode: "warm",
  ...
  it("omits an idle timeout for turn-by-turn sessions", () => {
    const config = buildGeetorusRunnerConfig(makeValues({
      geetorusRunnerLifecycleMode: "per_turn",
      geetorusRunnerIdleTimeoutMs: 45_000,
    }));
