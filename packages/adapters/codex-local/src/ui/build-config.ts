...
export function buildGeetorusRunnerConfig(v: CreateConfigValues): Record<string, unknown> {
  ...
  const provider = isGeetorusRunnerProvider(providerCandidate)
    ? providerCandidate
    : "codex";
  ...
  const lifecycleCandidate = v.geetorusRunnerLifecycleMode ?? schemaValues.lifecycleMode;
  const lifecycleMode = lifecycleCandidate === "warm" ? "warm" : "per_turn";
  const configuredIdleTimeoutMs =
    v.geetorusRunnerIdleTimeoutMs ?? schemaValues.idleTimeoutMs;
  const idleTimeoutMs = resolveGeetorusRunnerIdleTimeoutMs(
  ...
  if (
    ...
    throw new Error(
      "Geetorus Runner currently supports Codex only with codexPermissionMode set to never. Select Full auto (never ask) before saving.",
    );
  }
  ...
  return {
    ...
    provider,
    ...(provider === "codex"
      ? { model: resolveGeetorusRunnerModel("codex", config.model) }
      : {}),
    ...
    ...(provider === "acpx"
      ? {
          acpxAgent: "claude",
          model: configuredModel || schemaModel || resolveGeetorusRunnerModel("acpx", undefined),
        }
      : {}),
