...
import { inferOpenAiCompatibleBiller } from "@geetorusai/adapter-utils";
import {
...
export function buildCodexAcpConfig(config: Record<string, unknown>): Record<string, unknown> {
  ...
  let networkAccess = env.GEETORUS_CODEX_ACP_NETWORK_ACCESS !== "false";
  ...
  return {
    ...config,
    env: { ...env, GEETORUS_CODEX_ACP_NETWORK_ACCESS: String(networkAccess) },
    agent: "codex",
    mode,
    permissionMode,
  ...
  if (!targetIsRemote) {
    ...
    if (credentialReadiness.ready && credentialReadiness.authMode === "api") {
      ...
      checks.push({
        ...
        level: "warn",
        message: "No Codex ACP credentials visible to the Geetorus server were detected.",
        hint: "Set OPENAI_API_KEY in the agent adapter env, set it in the Geetorus server environment, or run `codex login` for the same OS user that runs the Geetorus server before starting a Codex ACP agent. A `/login` in a separate Codex/chat session does not authenticate the server.",
      });
    }
  } else if (targetIsSandbox) {
    // The ACP Test does not probe the sandbox, so it predicts readiness from the
    // credentials the Geetorus server can seed into the sandbox. The host
