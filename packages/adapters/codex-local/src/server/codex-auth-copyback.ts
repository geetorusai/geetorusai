...
export async function copyBackCodexAuth(input: CopyBackCodexAuthInput): Promise<CopyBackCodexAuthOutcome> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException | null)?.code === "ENOENT") {
      await log(
        "[geetorus] Codex auth copy-back: no sandbox credential to copy back (absent auth.json); host credential kept.",
      );
      return "kept-host";
    }
    throw error;
    ...
    async () => {
      ...
      try {
        ...
        if (decision === USE_SOURCE_EXIT) {
          ...
          await log(
            "[geetorus] Codex auth copy-back: sandbox credential is strictly newer for the same subscription identity; installed to the host at mode 0600.",
          );
          return "copied";
        }
        ...
        if (decision === IMPLAUSIBLE_LAST_REFRESH_EXIT) {
          await log(
            `[geetorus] Codex auth copy-back: WARNING host credential kept (decision exit ${IMPLAUSIBLE_LAST_REFRESH_EXIT}, codex auth copy-back) — the sandbox copy's last_refresh sat further ahead of the host clock than the plausible skew allowance. Check the host clock if this is unexpected.`,
          );
          return "kept-host";
        }

        await log(
          "[geetorus] Codex auth copy-back: host credential kept (sandbox copy is not a strictly-newer same-identity subscription credential).",
        );
        return "kept-host";
      } finally {
  ...
  if (resolveCacheEntryPath && isCodexAuthCacheEnabled(env)) {
    try {
      ...
      await Promise.resolve(
        log(
          `[geetorus] Codex auth cache: additive cache write failed (${code}); host copy-back result kept.`,
        ),
      ).catch(() => undefined);
    }
  }
