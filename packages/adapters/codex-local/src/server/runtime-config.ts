...
// Marker comments delimiting the Geetorus-managed regions of config.toml.
...
const MANAGED_ROOT_BEGIN = "# >>> geetorus codex providers (root) -- managed, do not edit >>>";
const MANAGED_ROOT_END = "# <<< geetorus codex providers (root) <<<";
const MANAGED_TABLES_BEGIN = "# >>> geetorus codex providers (tables) -- managed, do not edit >>>";
const MANAGED_TABLES_END = "# <<< geetorus codex providers (tables) <<<";
...
// GEETORUS_CODEX_PROVIDERS is a JSON object that maps 1:1 onto codex's
// config.toml schema:
//
//   {
//     "providers": {
...
function parseCodexProvidersConfig(
  ...
  try {
    ...
    notes.push("GEETORUS_CODEX_PROVIDERS contains invalid JSON; custom providers ignored.");
    return null;
  }
  if (!isPlainObject(parsed)) {
    notes.push("GEETORUS_CODEX_PROVIDERS is set but is not a JSON object; custom providers ignored.");
    return null;
  }
  const rawProviders = parsed.providers;
  if (!isPlainObject(rawProviders)) {
    notes.push(
      'GEETORUS_CODEX_PROVIDERS has no "providers" object; custom providers ignored.',
    );
    return null;
  }
  ...
  if (Object.keys(providers).length === 0) {
    notes.push(
      `GEETORUS_CODEX_PROVIDERS "providers" contains no usable entries${
        skipped.length > 0
  ...
  if (skipped.length > 0) {
    notes.push(
      `GEETORUS_CODEX_PROVIDERS: skipped provider(s) with empty names or non-object values: ${skipped.join(", ")}.`,
    );
  }
  const modelProvider =
  ...
  if (modelProvider !== null && !(modelProvider in providers)) {
    notes.push(
      `GEETORUS_CODEX_PROVIDERS: model_provider "${modelProvider}" does not match any usable provider entry; custom providers ignored.`,
    );
    return null;
  }
...
function configTomlBackupPath(configTomlPath: string): string {
  return `${configTomlPath}.geetorus-backup`;
}

// Merge custom Codex model providers supplied via GEETORUS_CODEX_PROVIDERS
...
// written alongside config.toml (including when GEETORUS_CODEX_PROVIDERS is
...
export async function prepareCodexRuntimeConfig(input: {
  ...
  const parsed = parseCodexProvidersConfig(
    input.env.GEETORUS_CODEX_PROVIDERS ?? process.env.GEETORUS_CODEX_PROVIDERS,
    resolveEnv,
    notes,
  );
  ...
  if (!parsed) {
    ...
    if (input.codexHome) {
      ...
      const reason = notes.length === 0 ? " (GEETORUS_CODEX_PROVIDERS is no longer set)" : "";
      ...
      if (backup !== null) {
        ...
        return {
          notes: [
            ...notes,
            `Restored "${configTomlPath}" from its pre-run backup, removing stale Geetorus-managed model providers left by an interrupted run${reason}.`,
          ],
          cleanup: async () => {},
        };
      }
      ...
      if (existing !== null) {
        ...
        if (stripped !== existing) {
          ...
          return {
            notes: [
              ...notes,
              `Removed stale Geetorus-managed model provider blocks from "${configTomlPath}"${reason}.`,
            ],
            cleanup: async () => {},
          };
        }
  ...
  if (!input.codexHome) {
    return {
      notes: [
        ...notes,
        "GEETORUS_CODEX_PROVIDERS is set but the adapter config explicitly sets env.CODEX_HOME; leaving the user-managed Codex home untouched (no model provider merge).",
      ],
      cleanup: async () => {},
    };
  }
  ...
  return {
    notes: [
      ...notes,
      `Merged ${providerNames.length} custom Codex model provider(s) from GEETORUS_CODEX_PROVIDERS into "${configTomlPath}": ${providerNames.join(", ")}${
