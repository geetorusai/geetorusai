...
describe("codex auth cache store", () => {
  ...
  async function makeInstanceRoot(): Promise<string> {
    const dir = await mkdtemp(path.join(os.tmpdir(), "geetorus-codex-cache-"));
  ...
  describe("Phase 5: cache-clear operator action and off-switch", () => {
    ...
    it("isCodexAuthCacheEnabled defaults to on and turns off on an explicit falsy flag", () => {
      ...
      expect(isCodexAuthCacheEnabled({ GEETORUS_CODEX_AUTH_CACHE: "1" })).toBe(true);
      expect(isCodexAuthCacheEnabled({ GEETORUS_CODEX_AUTH_CACHE: "on" })).toBe(true);
      expect(isCodexAuthCacheEnabled({ GEETORUS_CODEX_AUTH_CACHE: "0" })).toBe(false);
      expect(isCodexAuthCacheEnabled({ GEETORUS_CODEX_AUTH_CACHE: "false" })).toBe(false);
      expect(isCodexAuthCacheEnabled({ GEETORUS_CODEX_AUTH_CACHE: "off" })).toBe(false);
      expect(isCodexAuthCacheEnabled({ GEETORUS_CODEX_AUTH_CACHE: "no" })).toBe(false);
    });
  });
