import { asBoolean, asString, asStringArray } from "@geetorusai/adapter-utils/server-utils";
import {
...
export function buildCodexExecArgs(
  ...
  // Geetorus work. Keep the sandbox, but make unattended workspace work and
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Geetorus will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
