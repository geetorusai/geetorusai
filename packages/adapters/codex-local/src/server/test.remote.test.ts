...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async (input: { assets?: Array<{ key: string; localDir: string }> }) => {
      ...
      return {
        target: null,
        workspaceRemoteDir: "/remote/workspace/.geetorus-runtime/runs/test/workspace",
        runtimeRootDir: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/codex",
        assetDirs: {
          home: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/codex/home",
        },
        restoreWorkspace,
      };
    }),
    prepareManagedCodexHome: vi.fn(async () => {
      ...
      const dir = await fs.mkdtemp(`${os.tmpdir()}/geetorus-managed-codex-home-`);
...
describe("codex remote environment diagnostics", () => {
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("CODEX_HOME", await makeScratchDir("geetorus-test-shared-codex-"));
    vi.stubEnv("GEETORUS_HOME", await makeScratchDir("geetorus-test-instance-"));
  ...
  it("stages managed CODEX_HOME in an isolated runtime dir and keeps the probe cwd on the original remote workspace", async () => {
    ...
    expect(homeAsset?.localDir).toContain(`${os.tmpdir()}/geetorus-codex-probe-home-`);
    expect(capturedHomeAssetFiles.value).toEqual(["auth.json", "config.toml"]);
    expect(runtimeInput?.workspaceLocalDir).toContain(`${os.tmpdir()}/geetorus-codex-envtest-`);
    ...
    expect(probeCall?.[4]).toMatchObject({
      ...
      env: expect.objectContaining({
        CODEX_HOME: "/remote/workspace/.geetorus-runtime/runs/test/workspace/.geetorus-runtime/codex/home",
      }),
    });
  ...
  it("avoids /tmp CODEX_HOME for remote API-key hello probes", async () => {
    ...
    const probeCall = runAdapterExecutionTargetProcess.mock.calls[0] as unknown as
      ...
      | undefined;
    expect(probeCall?.[4].env.CODEX_HOME).toContain("/remote/workspace/.geetorus-runtime/codex/probe-home-codex-envtest-");
  ...
  it("emits the canonical adapter_auth_missing check when a sandbox hello probe reports missing auth", async () => {
    ...
    prepareManagedCodexHome.mockImplementationOnce(async () => {
      const dir = await fs.mkdtemp(`${os.tmpdir()}/geetorus-managed-codex-home-noauth-`);
      await fs.writeFile(`${dir}/config.toml`, "model = \"gpt-5\"\n");
      return dir;
    });
  ...
  it("stages an external CODEX_HOME's credentials as-is and never seeds or mutates it", async () => {
    const externalHome = await makeScratchDir("geetorus-test-external-codex-");
