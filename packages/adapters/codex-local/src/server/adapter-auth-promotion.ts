...
export async function checkStagedCredentialReadiness(
  ...
  const scratchHome = await mkdtemp(path.join(os.tmpdir(), "geetorus-login-readiness-"));
  try {
...
export async function promoteDeviceLoginCredential(
  ...
  if (!userInitiated) {
    await log("[geetorus] Codex device-login promotion: skipped (an automatic background login never seeds a home).");
  ...
  if (!soleOwner) {
    await log("[geetorus] Codex device-login promotion: skipped (the session no longer holds the sole active claim on the slot).");
  ...
  const accountHomeOutcome = await writeCredentialSeedOrNewer({
    ...
    log,
    writtenLine: "[geetorus] Codex device-login promotion: wrote this account's own home at mode 0600.",
    keptLine: "[geetorus] Codex device-login promotion: kept this account's own home (the login is not a seed or a strictly-newer credential).",
  ...
  try {
    ...
    await writeCredentialSeedOrNewer({
      ...
      log,
      writtenLine:
        "[geetorus] Codex device-login promotion: wrote the company default home (seed or strictly-newer refresh).",
      keptLine: "[geetorus] Codex device-login promotion: kept the company default home.",
    ...
    await log(
      "[geetorus] Codex device-login promotion: seeding the company default home failed; this account's own home is durable, so the login stays successful.",
    );
  }
