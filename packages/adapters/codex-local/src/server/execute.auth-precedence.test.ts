...
const {
  ...
  prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
    ...
    runtimeRootDir: "/sandbox/.geetorus-runtime",
    assetDirs: { home: "/sandbox/.geetorus-runtime/codex/home" },
...
describe("codex sandbox auth precedence warning", () => {
  ...
  it("logs and emits a run event when sandbox login is shadowed by host auth", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-precedence-"));
    ...
    expect(homeAsset?.localDir).toContain("geetorus-codex-home-sync");
