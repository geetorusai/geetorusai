...
const {
  ...
  readGeetorusRuntimeSkillEntries: vi.fn(async () => []),
  resolveAdapterExecutionTargetCommandForLogs: vi.fn(async () => "codex"),
  runAdapterExecutionTargetProcess: vi.fn(),
  tempCodexHome: "/tmp/geetorus-codex-stderr-error-test-home",
}));

vi.mock("./acp.js", () => ({
  createCodexAcpExecutor: () => vi.fn(),
  formatCodexAcpFallbackMessage: (reason: string) =>
    `[geetorus] Codex ACP default unavailable; falling back to Codex CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`,
...
describe("codex_local stderr fallback error derivation", () => {
  ...
  it("skips adapter-injected [geetorus] diagnostic lines when picking the fallback error", async () => {
    mockFailedProcess(
      [
        "[geetorus] Codex ACP default unavailable; falling back to Codex CLI. Set engine=acp to require ACP or engine=cli to silence this fallback.",
        YOLO_WARNING,
...
describe("firstMeaningfulStderrLine", () => {
  it("returns the first line that is not a known benign warning", () => {
    ...
    expect(firstMeaningfulStderrLine("[geetorus] Confining Codex with workspace scope.\nError: boom")).toBe(
      "Error: boom",
    );
  });
