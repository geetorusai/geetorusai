...
describe("codex home auth merge on sandbox asset extract", () => {
  ...
  async function runCodexHomeAssetExtract(input: {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-merge-"));
    cleanupDirs.push(rootDir);

    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localHomeDir = path.join(rootDir, "local-codex-home");
    const remoteHomeDir = path.join(remoteWorkspaceDir, ".geetorus-runtime", "codex", "home");
  ...
  it("keeps a newer same-account sandbox auth.json and installs it atomically with mode 0600", async () => {
    ...
    expect(result.commandText).not.toContain("geetorus-extract");
  ...
  it("routes the Codex home asset through a single native syncIn operation whose post-command is the auth-merge (#4, C5/C6)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-codex-native-route-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localHomeDir = path.join(rootDir, "local-codex-home");
...
describe("codex-auth-merge-decision predicate (source/destination)", () => {
  ...
  async function runDecision(input: {
    ...
    const dir = await mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-decision-"));
