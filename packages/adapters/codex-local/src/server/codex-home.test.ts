...
describe("codex managed home", () => {
  ...
  it("treats a concurrently-created expected auth symlink as success", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-home-"));
  ...
  // Regression for #5028: older Geetorus versions copied auth.json into the
  ...
  it("replaces a stale regular-file auth.json with a symlink to the live source (#5028)", async () => {
    ...
    try {
      ...
      // Simulate a stale copy left by a previous Geetorus version.
  ...
  it("ensureSymlink: unlinks a stale regular file and recreates the symlink", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-ensure-symlink-"));
    try {
  ...
  // this path as operator-owned, not a stale Geetorus copy.
  it("ensureSymlink: leaves an unexpected directory in place instead of throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-ensure-symlink-dir-"));
    try {
      const source = path.join(root, "live-source.json");
...
describe("isManagedCodexHomePath", () => {
  const env = {
    GEETORUS_HOME: "/srv/geetorus",
    GEETORUS_INSTANCE_ID: "default",
  } satisfies NodeJS.ProcessEnv;
  const companyRoot = path.resolve(
    "/srv/geetorus/instances/default/companies/company-1",
  );
  ...
  it("treats a path outside the company tree as an external override", () => {
    ...
    expect(
      isManagedCodexHomePath(
        env,
        "company-1",
        path.resolve("/srv/geetorus/instances/default/companies/company-2/codex-home"),
      ),
    ).toBe(false);
  });
...
describe("codexHomeHasUsableAuth", () => {
  it("is true for credential-bearing auth.json and false when missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-"));
    try {
  ...
  it("recognizes the Codex 0.143 AuthDotJson subscription shape", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-modern-"));
    try {
      await fs.writeFile(
  ...
  it("treats subscription auth without account_id or token material as unusable", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-modern-invalid-"));
    try {
      await fs.writeFile(
        path.join(root, "auth.json"),
        JSON.stringify({
  ...
  it("is false for a dangling auth.json symlink", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-auth-dangling-"));
    try {
...
describe("seedManagedCodexHome", () => {
  it("symlinks auth.json from the shared source into an explicit per-agent home", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-"));
    try {
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("writes an API-key auth.json into the home when an apiKey is supplied", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-apikey-"));
    try {
  ...
  it("keeps a promoted subscription auth.json when the shared source has no auth", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-promoted-"));
    try {
  ...
  it("keeps a promoted subscription auth.json whose identity differs from the shared source", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-foreign-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
      const promoted = subscriptionAuth("acct-promoted", "promoted");
  ...
  it("still replaces a same-identity stale regular copy with the shared symlink once the source is strictly fresher (#5028)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-stale-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("keeps a same-identity promoted auth.json that is strictly newer than the shared source", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-newer-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("keeps a same-identity auth.json when freshness ties or cannot be compared", async () => {
    ...
    for (const { source, target } of cases) {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-tie-"));
      try {
        const companyHome = path.join(root, "company-home");
        const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("keeps the usable target when the shared source exists but cannot be read", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-src-err-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("never symlinks or heals a credential-store entry's auth.json, even for a fresher same-identity source", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-store-"));
    try {
      const sharedCodexHome = path.join(root, "shared-codex-home");
      const entryHome = path.join(
        root, "geetorus-home", "instances", "default", "companies", "company-1", "codex-auth-cache", "acct-bound",
      );
      const env = {
        CODEX_HOME: sharedCodexHome,
        GEETORUS_HOME: path.join(root, "geetorus-home"),
        GEETORUS_INSTANCE_ID: "default",
      };
  ...
  it("refuses an API-key rewrite of a credential-store entry's auth.json", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-store-apikey-"));
    try {
      const sharedCodexHome = path.join(root, "shared-codex-home");
      const entryHome = path.join(
        root, "geetorus-home", "instances", "default", "companies", "company-1", "codex-auth-cache", "acct-bound",
  ...
  it("still removes an apikey-mode auth.json so the chatgpt-mode symlink is restored", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-seed-apikey-residue-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
...
describe("reconcileManagedCodexHome", () => {
  async function makeFixture() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-reconcile-"));
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const geetorusHome = path.join(root, "geetorus-home");
    const agentHome = path.join(
      geetorusHome,
...
describe("evaluateCodexCredentialReadiness", () => {
  async function makeFixture() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-readiness-"));
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const geetorusHome = path.join(root, "geetorus-home");
  ...
  it("replaces the managed MCP block and clears stale servers for an empty runtime set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-mcp-config-"));
    try {
      const alphaHome = path.join(root, "agent-alpha");
      const zeroHome = path.join(root, "agent-zero");
      await writeManagedCodexMcpConfig({
        codexHome: alphaHome,
        apiBaseUrl: "https://geetorus.example",
        gateways: [{
          name: "alpha",
          endpointPath: "https://geetorus.example/api/tool-gateway/gateways/alpha/mcp",
...
describe("stageCodexHomeForSync", () => {
  ...
  async function buildFakeHome(root: string): Promise<{ home: string; authBytes: string; skillBytes: string }> {
    ...
    await fs.writeFile(path.join(home, "config.toml"), "model_provider = \"geetorus\"\n", "utf8");
  ...
  it("stages exactly the allowlist, derefs auth.json to bytes, and excludes decoys", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-"));
  ...
  it("writes the staged auth.json with mode 0600", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-mode-"));
    let staged: string | null = null;
    try {
  ...
  it("writes the staged config.toml (managed MCP bearer header) with mode 0600", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-toml-mode-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(home, { recursive: true });
  ...
  it("writes every staged regular file with mode 0600", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-all-mode-"));
    let staged: string | null = null;
    try {
      const { home } = await buildFakeHome(root);
  ...
  it("creates the staged dir with mode 0700", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-dir-"));
    let staged: string | null = null;
    try {
      const { home } = await buildFakeHome(root);
  ...
  it("skips absent optional entries without throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-absent-"));
    let staged: string | null = null;
    try {
  ...
  it("treats a dangling auth.json symlink as absent (skips it, no throw)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-dangling-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(home, { recursive: true });
  ...
  it("fails closed and removes the temp dir on an unexpected I/O error", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-fail-"));
    try {
      const { home } = await buildFakeHome(root);
  ...
  // Skill symlinks that point OUTSIDE skills/ are intentional design (Geetorus
  ...
  it("skips circular skill symlinks (ELOOP) without throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-circular-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(path.join(home, "skills"), { recursive: true });
  ...
  it("writes nested skill files with mode 0600 regardless of source mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-skill-mode-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
  ...
  it("does not infinitely traverse an ancestor directory link (back -> .) inside a skill", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-cycle-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
  ...
  it("does not stage a nested skill symlink that escapes the skill root", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-escape-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      const skillDir = path.join(home, "skills", "my-skill");
  ...
  it("skips a top-level skills entry that resolves to an ancestor of skills/", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-stage-ancestor-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(path.join(home, "skills"), { recursive: true });
