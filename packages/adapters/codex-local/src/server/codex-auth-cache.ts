...
import { withDirectoryMergeLock } from "@geetorusai/adapter-utils/workspace-restore-merge";
import { toAccountHandle } from "@geetorusai/shared";
...
export const CODEX_AUTH_CACHE_OFF_SWITCH_ENV = "GEETORUS_CODEX_AUTH_CACHE";
...
export async function writeCodexAuthCacheEntry(input: {
  ...
  const outcome = await writeCredentialSeedOrNewer({
    ...
    log: input.log,
    writtenLine: "[geetorus] Codex auth cache: wrote the per-identity cache slot at mode 0600.",
    keptLine:
      "[geetorus] Codex auth cache: kept the cache slot (source is not a strictly-newer same-identity subscription credential).",
...
export async function selectVendCredential(
  ...
  return withDirectoryMergeLock(hostDir, async () => {
    ...
    if (!cacheBytes) {
      await log("[geetorus] Codex auth cache: no cached credential for the host identity; host credential kept.");
      return "kept-host";
    }
    ...
    try {
      ...
      if (decision === USE_SOURCE_EXIT) {
        ...
        await log(
          "[geetorus] Codex auth cache: refreshed the host credential with a strictly-newer cached copy of the same identity at mode 0600.",
        );
        return "vended";
      }
      await log(
        "[geetorus] Codex auth cache: host credential kept (the cached copy is not strictly newer for the same identity).",
      );
      return "kept-host";
    } finally {
