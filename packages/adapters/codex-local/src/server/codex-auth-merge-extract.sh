...
asset_tar=$2
auth_name=auth.json
stage_root="$asset_dir.geetorus-extract.$$"
...
if [ -f "$sandbox_auth" ]; then
  if command -v node >/dev/null 2>&1; then
    ...
    echo "[geetorus] node not found in PATH; cannot evaluate auth-merge decision - aborting sandbox restore" >&2
    exit 1
  fi
fi
...
if [ -f "$source_auth" ]; then
  ...
  target_tmp="$asset_dir/.auth.json.geetorus.$$"
