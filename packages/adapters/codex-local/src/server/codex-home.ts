...
import type { AdapterExecutionContext } from "@geetorusai/adapter-utils";
import { resolveGeetorusInstanceRootForAdapter } from "@geetorusai/adapter-utils/server-utils";
import { isCodexAuthCachePath, readSubscriptionAccountId } from "./codex-auth-cache.js";

const TRUTHY_ENV_RE = /^(1|true|yes|on)$/i;
const COPIED_SHARED_FILES = ["config.json", "config.toml", "instructions.md"] as const;
const SYMLINKED_SHARED_FILES = ["auth.json"] as const;
const MANAGED_MCP_BLOCK_START = "# BEGIN GEETORUS MANAGED MCP";
const MANAGED_MCP_BLOCK_END = "# END GEETORUS MANAGED MCP";
...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.GEETORUS_IN_WORKTREE ?? "");
}
...
export function resolveManagedCodexHomeDir(
  ...
  const instanceRoot = resolveGeetorusInstanceRootForAdapter({
    homeDir: nonEmpty(env.GEETORUS_HOME) ?? undefined,
    instanceId: nonEmpty(env.GEETORUS_INSTANCE_ID) ?? undefined,
    env,
  });
  return companyId
...
/**
 * True when `homePath` lives under the Geetorus-managed company tree
 ...
 * external/user-supplied override that Geetorus must not seed or overwrite.
 */
...
export async function ensureSymlink(target: string, source: string): Promise<void> {
  ...
  if (!existing.isSymbolicLink()) {
    // A previous Geetorus version copied this file into the managed home
    ...
    // Geetorus-managed company home, never the user's real ~/.codex.
    ...
    // a Geetorus-written stale copy and warrants operator inspection rather
    // than silent removal.
...
function buildManagedMcpBlock(input: {
  ...
  const lines = [
    ...
    "# Written by Geetorus for governed MCP gateway access. Do not edit this block by hand.",
  ];
  input.gateways.forEach((gateway, index) => {
    ...
    let managedName = directOverlap ? `geetorus-${baseName}` : baseName;
    let suffix = 2;
    while (usedNames.has(managedName) || input.existingNames.has(managedName)) {
      managedName = `geetorus-${baseName}-${suffix}`;
      suffix += 1;
    }
    usedNames.add(managedName);
    if (directOverlap) {
      warnings.push(
        `Found unmanaged Codex MCP server "${gateway.name}" overlapping a Geetorus-governed gateway; leaving the direct entry in place and adding managed gateway "${managedName}". Geetorus cannot enforce policies for that direct entry.`,
      );
    }
 ...
 *
 * `sourceDir`'s *direct* children are the Geetorus-injected skill symlinks that
...
export async function stageCodexHomeForSync(
  ...
  const stagedHome = await fs.mkdtemp(
    path.join(os.tmpdir(), `geetorus-codex-home-sync-${runIdPart ? `${runIdPart}-` : ""}`),
  );
  try {
...
/**
 * Seeds auth/config into an explicit Geetorus-managed `targetHome`. Symlinks
...
export async function seedManagedCodexHome(
    ...
    if (existing && !existing.isSymbolicLink()) {
      ...
      if (targetIdentity) {
        ...
        if (keepPromotedAuth && sourceReadErrorCode) {
          ...
          await onLog(
            "stdout",
            `[geetorus] Keeping the existing subscription auth.json in Codex home "${targetHome}" (shared source read failed: ${sourceReadErrorCode}); the next seed with a readable source reconciles it.\n`,
          );
        }
      }
      if (keepPromotedAuth) {
        await onLog(
          "stdout",
          `[geetorus] Keeping the promoted subscription auth.json in Codex home "${targetHome}".\n`,
        );
      } else {
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[geetorus] Using ${isWorktreeMode(env) ? "worktree-isolated" : "Geetorus-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
  ...
  if (apiKey) {
    if (credentialStoreEntry) {
      ...
      await onLog(
        "stdout",
        `[geetorus] Refusing to write an API-key auth.json into credential-store entry "${targetHome}"; the bound account's stored login stays authoritative.\n`,
      );
    } else {
      await writeApiKeyAuthJson(targetHome, apiKey);
      await onLog(
        "stdout",
        `[geetorus] Wrote API-key auth.json into Codex home "${targetHome}" from configured OPENAI_API_KEY.\n`,
      );
    }
  }
}
...
export interface CodexCredentialReadiness {
  /** True when Geetorus owns the effective home and is responsible for its auth. */
  managed: boolean;
 ...
 *   is always treated as ready (Geetorus must not seed or inspect it).
...
export async function evaluateCodexCredentialReadiness(
  ...
  if (!effectiveHomeIsManaged) {
    // Genuine external override: Geetorus never seeds or inspects it.
    return {
      managed: false,
