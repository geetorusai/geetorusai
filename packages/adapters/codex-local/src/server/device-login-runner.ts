...
export async function runDeviceLogin(
  ...
  try {
    if (signal?.aborted) {
      log("[geetorus] Device login cancelled before start.");
    ...
    if (raced.kind === "timeout") {
      log("[geetorus] Device login timed out; disposing the sandbox.");
      return { outcome: "timeout", exitCode: null, promptSurfaced };
    }
    if (raced.kind === "cancelled") {
      log("[geetorus] Device login cancelled; disposing the sandbox.");
      return { outcome: "cancelled", exitCode: null, promptSurfaced };
    }

    const exitCode = raced.exitCode;
    if (exitCode !== 0) {
      log("[geetorus] Device login command ended with a non-zero exit code.");
    ...
    log("[geetorus] Device login command ended successfully.");
    ...
    try {
      ...
      log("[geetorus] Device login: the sandbox dispose step errored.");
    }
  }
}
