...
async function makeCodexHome(configToml?: string): Promise<string> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-home-"));
...
describe("prepareCodexRuntimeConfig", () => {
  it("is a no-op when GEETORUS_CODEX_PROVIDERS is unset", async () => {
  ...
  it("merges providers + model_provider into a fresh config.toml and cleans it up", async () => {
    ...
    const prepared = await prepareCodexRuntimeConfig({
      env: { GEETORUS_CODEX_PROVIDERS: JSON.stringify(BIFROST_PROVIDERS) },
      codexHome: home,
    });
  ...
  it("emits plain-object fields as inline tables and arrays as TOML arrays", async () => {
    ...
    const prepared = await prepareCodexRuntimeConfig({
      env: {
        GEETORUS_CODEX_PROVIDERS: JSON.stringify({
          providers: {
            gw: {
  ...
  it("expands {env:VAR} placeholders from the run env and process.env, leaving unresolvable ones intact", async () => {
    const home = await makeCodexHome();
    process.env.GEETORUS_CODEX_TEST_PROCESS_KEY = "from-process-env";
    try {
      const prepared = await prepareCodexRuntimeConfig({
        env: {
          GEETORUS_CODEX_PROVIDERS: JSON.stringify({
            providers: {
              gw: {
                base_url: "http://gw.example/v1",
                http_headers: {
                  "X-Run": "{env:GEETORUS_CODEX_TEST_RUN_KEY}",
                  "X-Process": "{env:GEETORUS_CODEX_TEST_PROCESS_KEY}",
          ...
          GEETORUS_CODEX_TEST_RUN_KEY: "from-run-env",
        },
        codexHome: home,
      });
      ...
      delete process.env.GEETORUS_CODEX_TEST_PROCESS_KEY;
    }
  });

  it("reads GEETORUS_CODEX_PROVIDERS from process.env when absent from the run env", async () => {
    const home = await makeCodexHome();
    process.env.GEETORUS_CODEX_PROVIDERS = JSON.stringify(BIFROST_PROVIDERS);
    try {
      const prepared = await prepareCodexRuntimeConfig({ env: {}, codexHome: home });
      const content = await readConfigToml(home);
      expect(content).toContain("[model_providers.bifrost]");
      ...
      delete process.env.GEETORUS_CODEX_PROVIDERS;
    }
  });

  it("surfaces a note for malformed GEETORUS_CODEX_PROVIDERS without touching config.toml", async () => {
    const home = await makeCodexHome("model = \"gpt-5.1-codex\"\n");
    ...
    for (const [raw, noteFragment] of cases) {
      const prepared = await prepareCodexRuntimeConfig({
        env: { GEETORUS_CODEX_PROVIDERS: raw },
        codexHome: home,
      });
  ...
  it("stays silent when GEETORUS_CODEX_PROVIDERS is unset or empty", async () => {
    const home = await makeCodexHome("model = \"gpt-5.1-codex\"\n");
    const envs: Array<Record<string, string>> = [
      {},
      { GEETORUS_CODEX_PROVIDERS: "" },
      { GEETORUS_CODEX_PROVIDERS: "  " },
    ];
  ...
  it("restores user provider sections from the pre-run backup after an interrupted run", async () => {
    ...
    await expect(fs.access(path.join(home, "config.toml.geetorus-backup"))).rejects.toThrow();
    await prepared.cleanup();
  });
  ...
  it("removes the pre-run backup on cleanup", async () => {
    ...
    const backupPath = path.join(home, "config.toml.geetorus-backup");
  ...
  it("rejects the block when model_provider names a filtered or missing provider", async () => {
    ...
    expect(prepared.notes).toContain(
      'GEETORUS_CODEX_PROVIDERS: model_provider "bad" does not match any usable provider entry; custom providers ignored.',
    );
    const content = await readConfigToml(home);
