...
import type { SandboxManagedRuntimeAsset } from "@geetorusai/adapter-utils/sandbox-managed-runtime";
...
const REMOTE_RUNTIME_ROOT = "/remote/workspace/.geetorus-runtime/codex";
...
const {
  ...
  startAdapterExecutionTargetGeetorusBridge: vi.fn(async () => null),
}));
...
describe("codex execute — outbound auth copy-back restore contribution", () => {
  ...
  async function runTeardown(input: { sandboxAuth: string; hostAuth: string }) {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "geetorus-codex-copyback-e2e-"),
    );
