...
} from "@geetorusai/adapter-utils/local-process-sandbox";
import {
  parseCodexJsonl,
...
// "[geetorus] ..." lines are diagnostics the adapter injected (e.g. ACP
...
async function isLikelyGeetorusRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelyGeetorusRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyGeetorusRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableGeetorusSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyGeetorusRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog(
      "stdout",
      `[geetorus] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
...
function managedMcpGatewaysFromContext(context: Record<string, unknown>): ManagedCodexMcpGateway[] {
  const managedMcp = parseObject(context.geetorusManagedMcp);
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries
    ?? (await readGeetorusRuntimeSkillEntries({}, __moduleDir)).filter(
      (entry) => !isGeetorusSkillSourceMissing(entry),
    );
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelyGeetorusRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
          ...
          await onLog(
            "stdout",
            `[geetorus] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }

      const result = await ensureGeetorusSkillSymlink(entry.source, target, linkSkill);
      if (result === "skipped") continue;

      await onLog(
        "stdout",
        `[geetorus] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[geetorus] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailableGeetorusSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const workspaceHints = Array.isArray(context.geetorusWorkspaces)
    ...
    : [];
  const runtimeServiceIntents = Array.isArray(context.geetorusRuntimeServiceIntents)
    ? context.geetorusRuntimeServiceIntents.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimeServices = Array.isArray(context.geetorusRuntimeServices)
    ? context.geetorusRuntimeServices.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimePrimaryUrl = asString(context.geetorusRuntimePrimaryUrl, "");
  ...
  const connectorSkillDigest = typeof config.geetorusConnectorSkillDigest === "string"
    && /^[a-f0-9]{64}$/.test(config.geetorusConnectorSkillDigest) ? config.geetorusConnectorSkillDigest : null;
  ...
  const codexSkillEntries = (await readGeetorusRuntimeSkillEntries(config, __moduleDir))
    // A missing-source entry would become a dangling skill symlink; skip it.
    .filter((entry) => !isGeetorusSkillSourceMissing(entry));
  ...
  const configuredOpenAiApiKey =
    typeof envConfig.OPENAI_API_KEY === "string" && envConfig.OPENAI_API_KEY.trim().length > 0
      ...
      : null;
  // A configured CODEX_HOME that lives under the Geetorus-managed company tree
  ...
  if (!config.managedAiConnection && isCodexAuthCacheEnabled(process.env)) {
    ...
    await selectVendCredential(
      ...
      await onLog(
        "stderr",
        `[geetorus] Codex auth cache: vend skipped after an error; using the shared credential as-is.\n`,
      );
      void error;
    });
  }
  ...
  // Merge custom model providers (GEETORUS_CODEX_PROVIDERS) into the managed
  ...
  try {
    for (const note of preparedRuntimeConfig.notes) {
      await onLog("stdout", `[geetorus] ${note}\n`);
    }
    const geetorusBaseEnv = buildGeetorusEnv(agent);
    ...
    const managedMcp = await writeManagedCodexMcpConfig({
      ...
      apiBaseUrl: geetorusBaseEnv.GEETORUS_API_URL,
      gateways: managedMcpGateways,
    });
    if (managedMcpGateways.length > 0) {
      await onLog(
        "stdout",
        `[geetorus] Wrote ${managedMcpGateways.length} managed MCP gateway(s) into Codex config "${managedMcp.configPath}".\n`,
      );
    }
    for (const warning of managedMcp.warnings) {
      await onLog("stderr", `[geetorus] ${warning}\n`);
    }
    ...
    const preparedExecutionTargetRuntime = executionTargetIsRemote
      ? await (async () => {
          await onLog(
            "stdout",
            `[geetorus] Syncing ${targetWorkspaceRealization?.mode === "in_place" ? "CODEX_HOME" : "workspace and CODEX_HOME"} to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
          );
    ...
    const remoteCodexHome = executionTargetIsRemote
      ? preparedExecutionTargetRuntime?.assetDirs.home ??
        path.posix.join(effectiveExecutionCwd, ".geetorus-runtime", "codex", "home")
      : null;
    ...
    const env: Record<string, string> = { ...geetorusBaseEnv };
    ...
    if (wakeTaskId) {
      env.GEETORUS_TASK_ID = wakeTaskId;
    }
    if (issueWorkMode) {
      env.GEETORUS_ISSUE_WORK_MODE = issueWorkMode;
    }
    if (wakeReason) {
      env.GEETORUS_WAKE_REASON = wakeReason;
    }
    if (wakeCommentId) {
      env.GEETORUS_WAKE_COMMENT_ID = wakeCommentId;
    }
    if (approvalId) {
      env.GEETORUS_APPROVAL_ID = approvalId;
    }
    if (approvalStatus) {
      env.GEETORUS_APPROVAL_STATUS = approvalStatus;
    }
    if (linkedIssueIds.length > 0) {
      env.GEETORUS_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
    }
    if (wakePayloadJson) {
      env.GEETORUS_WAKE_PAYLOAD_JSON = wakePayloadJson;
    }
    refreshGeetorusWorkspaceEnvForExecution({
      env,
      envConfig,
    ...
    if (targetWorkspaceRealization) {
      env.GEETORUS_WORKSPACE_REALIZATION_MODE = targetWorkspaceRealization.mode;
      env.GEETORUS_WORKSPACE_AUTHORITATIVE_ROOT = targetWorkspaceRealization.authoritativeRoot;
    }
    if (runtimeServiceIntents.length > 0) {
      env.GEETORUS_RUNTIME_SERVICE_INTENTS_JSON = JSON.stringify(runtimeServiceIntents);
    }
    if (runtimeServices.length > 0) {
      env.GEETORUS_RUNTIME_SERVICES_JSON = JSON.stringify(runtimeServices);
    }
    if (runtimePrimaryUrl) {
      env.GEETORUS_RUNTIME_PRIMARY_URL = runtimePrimaryUrl;
    }
    ...
    if (executionTargetIsRemote && adapterExecutionTargetUsesGeetorusBridge(runtimeExecutionTarget)) {
      ...
      if (geetorusBridge) {
        Object.assign(env, geetorusBridge.env);
      }
    }
    ...
    const localProcessSandbox: LocalProcessSandboxOptions | null =
      (filesystemScope || networkScope) && !executionTargetIsRemote
        ? {
            ...
            networkTrustedUrls: [
              geetorusBaseEnv.GEETORUS_API_URL,
    ...
    if (localProcessSandbox) {
      ...
      await onLog(
        "stdout",
        `[geetorus] Confining Codex with ${scopes} scope.\n`,
      );
    }
    ...
    if (monitorResolution.mode === "disabled") {
      await onLog(
        "stdout",
        `[geetorus] Codex output inactivity monitor is DISABLED via adapterConfig.outputInactivityTimeoutMs=null. Hung codex runs will only be detected by the platform-level silent-run safety net.\n`,
      );
    } else if (monitorResolution.mode === "default" && "reason" in monitorResolution) {
      await onLog(
        "stdout",
        `[geetorus] Ignoring non-positive adapterConfig.outputInactivityTimeoutMs; falling back to default ${monitorResolution.timeoutMs}ms.\n`,
      );
    }
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[geetorus] Codex session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[geetorus] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    const renderedBootstrapPrompt =
      !sessionId && bootstrapPromptTemplate.trim().length > 0
        ...
        : "";
    const taskContextNote = selectGeetorusTaskMarkdown(context, { resumedSession: Boolean(sessionId) });
    ...
    const continuationSummary = parseObject(context.geetorusContinuationSummary);
    ...
    const runAttempt = async (resumeSessionId: string | null) => {
      const execArgs = buildCodexExecArgs(
        forceSaferInvocation ? { ...config, fastMode: false } : config,
        {
          ...
          networkAccess: env.GEETORUS_RUNNER_NETWORK_ACCESS !== "disabled",
        },
      );
        ...
        monitorResolution.mode === "disabled"
          ...
          : createCodexOutputInactivityMonitor({
              ...
              onFire: (state) => {
                ...
                const logLine =
                  `[geetorus] adapter.invoke ${message}; ` +
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[geetorus] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
        );
      ...
      if (geetorusBridge) {
        await geetorusBridge.stop();
      }
      if (restoreRemoteWorkspace) {
        try {
          await onLog(
            "stdout",
            `[geetorus] Restoring workspace changes from ${describeAdapterExecutionTarget(executionTarget)}.\n`,
          );
          ...
          await Promise.resolve(
            onLog(
              "stderr",
              `[geetorus] Failed to restore workspace changes from ${describeAdapterExecutionTarget(
                executionTarget,
    ...
    if (stagedCodexHomeDir) {
      await fs.rm(stagedCodexHomeDir, { recursive: true, force: true }).catch(async (error) => {
        await onLog(
          "stderr",
          `[geetorus] Failed to remove staged Codex home "${stagedCodexHomeDir}": ${
            error instanceof Error ? error.message : String(error)
          }\n`,
        );
      });
    }
    // Restore the managed config.toml so GEETORUS_CODEX_PROVIDERS changes
