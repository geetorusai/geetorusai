...
const PROBE_CLEANUP_WARNING = "[geetorus] Codex probe cleanup incomplete";
...
async function prepareCodexHelloProbe(input: {
  ...
  if (input.targetIsRemote && !input.probeApiKey) {
    ...
    // Geetorus-managed override (the per-agent home) seeded in place — and
    ...
    probeHomeLocalDir = await fs.mkdtemp(
      path.join(os.tmpdir(), `geetorus-codex-probe-home-${input.runId}-`),
    );
    ...
    preparedRuntimeWorkspaceLocalDir = await fs.mkdtemp(
      path.join(os.tmpdir(), `geetorus-codex-envtest-${input.runId}-`),
    );
    preparedRuntime = await prepareAdapterExecutionTargetRuntime({
      ...
      // `.geetorus-runtime/runs/<runId>/workspace` to whatever it gets, so
  ...
  if (input.probeApiKey) {
    const probeHome = input.targetIsRemote
      ? path.posix.join(input.cwd, ".geetorus-runtime", "codex", `probe-home-${input.runId}`)
      : path.join(os.tmpdir(), `geetorus-codex-probe-${input.runId}`);
    ...
    return {
      ...
      args: [
        "-c",
        `set -e; umask 077; mkdir -p "$CODEX_HOME"; printf "%s" "$_GEETORUS_CODEX_AUTH_JSON" > "$CODEX_HOME/auth.json"; unset _GEETORUS_CODEX_AUTH_JSON; cleanup() { result=$?; trap - EXIT; rm -f "$CODEX_HOME/auth.json" || true; rm -rf "$CODEX_HOME" || printf '%s\\n' '${PROBE_CLEANUP_WARNING}' >&2; exit "$result"; }; trap cleanup EXIT; trap 'exit 130' INT; trap 'exit 143' TERM; "$0" "$@"`,
        input.command,
        ...input.args,
      ],
      env: {
        ...
        _GEETORUS_CODEX_AUTH_JSON: JSON.stringify({ OPENAI_API_KEY: input.probeApiKey }),
      },
      cleanup,
    };
  }
        ...
        if (probe.timedOut) {
          ...
          checks.push({
            ...
            hint: probeApiKey
              ...
              : "Codex CLI does not read OPENAI_API_KEY from the environment; set OPENAI_API_KEY in this adapter's config (so Geetorus writes it to `$CODEX_HOME/auth.json`) or run `codex login` on the host first.",
          });
          if (targetIsSandbox) {
