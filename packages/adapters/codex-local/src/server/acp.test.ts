...
import type { AdapterExecutionContext, AdapterInvocationMeta } from "@geetorusai/adapter-utils";
...
vi.mock("@geetorusai/adapter-utils/workspace-restore-teardown", async (importOriginal) => {
...
const originalGeetorusHome = process.env.GEETORUS_HOME;
const originalGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;
...
// given runId (`geetorus-codex-home-sync-<runId>-<random>` under os.tmpdir()).
...
async function listCodexHomeSyncDirs(runId: string): Promise<string[]> {
  const prefix = `geetorus-codex-home-sync-${runId}-`;
...
afterEach(async () => {
  ...
  if (originalGeetorusHome === undefined) delete process.env.GEETORUS_HOME;
  else process.env.GEETORUS_HOME = originalGeetorusHome;
  if (originalGeetorusInstanceId === undefined) delete process.env.GEETORUS_INSTANCE_ID;
  else process.env.GEETORUS_INSTANCE_ID = originalGeetorusInstanceId;
...
async function makeTempRoot(prefix: string) {
  ...
  tempRoots.push(root);
  process.env.GEETORUS_HOME = path.join(root, "geetorus-home");
  process.env.GEETORUS_INSTANCE_ID = "test";
  return root;
}
...
function buildContext(root: string, overrides: Partial<AdapterExecutionContext> = {}): AdapterExecutionContext {
  return {
    ...
    context: {
      issueId: "issue-1",
      geetorusTaskMarkdown: "Task context",
      geetorusWorkspace: {
        cwd: root,
...
describe("codex_local ACP lane", () => {
  it("keeps ACP selected and reports unavailable prerequisites for default and explicit engines", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-default-");
  ...
  it("enables workspace networking for ACP without changing other env settings", () => {
    expect(buildCodexAcpConfig({ env: { CUSTOM: "kept" } })).toMatchObject({
      env: { CUSTOM: "kept", GEETORUS_CODEX_ACP_NETWORK_ACCESS: "true" },
    });
  });

  it.each([
    { env: { GEETORUS_CODEX_ACP_NETWORK_ACCESS: "false" } },
  ...
  it("reports ACP prerequisites for the ACP lane", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-env-");
    const commandPath = path.join(root, "bin", "codex-acp");
    await fs.mkdir(path.dirname(commandPath), { recursive: true });
    await fs.writeFile(commandPath, "#!/usr/bin/env sh\n", "utf8");
    setNodeVersion("v24.11.0");
  ...
  it("detects shared managed Codex auth in ACP environment tests", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-managed-auth-");
    const commandPath = path.join(root, "bin", "codex-acp");
  ...
  it("explains the Geetorus server credential boundary when ACP auth is missing", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-missing-auth-");
    const commandPath = path.join(root, "bin", "codex-acp");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const managedAgentHome = path.join(
      root,
    ...
    expect(result.checks).toContainEqual(
      expect.objectContaining({
        code: "codex_acp_credentials_missing",
        level: "warn",
        message: expect.stringContaining("Geetorus server"),
  ...
  it("emits the canonical adapter_auth_missing check for a missing-auth sandbox target", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-sandbox-missing-auth-");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const managedAgentHome = path.join(
      root,
      "geetorus-home",
  ...
  it("executes through ACPX with Codex session config and ephemeral skills", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-exec-");
  ...
  it("creates the ACP session on the in-sandbox workspace cwd for runner-backed remote runs", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-remote-cwd-");
    ...
    const result = await execute(
      buildContext(localCwd, {
        ...
        context: {
          issueId: "issue-1",
          geetorusTaskMarkdown: "Task context",
          geetorusWorkspace: { cwd: localCwd, source: "project_workspace", workspaceId: "workspace-1" },
        },
        executionTarget: {
          kind: "remote",
          transport: "sandbox",
  ...
  it("seeds the managed Codex home into the sandbox and repoints CODEX_HOME to the in-sandbox path", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-home-seed-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    ...
    expect(remappedCodexHome).toContain(".geetorus-runtime");
  ...
  it("copies a strictly-newer sandbox Codex auth back to the shared host on teardown", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-copyback-newer-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const sourceHome = path.join(root, "codex-home");
    const sharedHostHome = path.join(root, "shared-codex-home");
  ...
  it("keeps the shared host Codex auth when the sandbox copy is not strictly newer", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-copyback-older-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const sourceHome = path.join(root, "codex-home");
    const sharedHostHome = path.join(root, "shared-codex-home");
  ...
  it("keeps the host staged Codex home after a clean teardown so a compatible resume can reuse it", async () => {
    ...
    const root = await makeTempRoot("geetorus-codex-acp-keep-staged-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const sourceHome = path.join(root, "codex-home");
    const sharedHostHome = path.join(root, "shared-codex-home");
  ...
  it("removes the host staged Codex home when a failed turn drops the staged runtime", async () => {
    ...
    const root = await makeTempRoot("geetorus-codex-acp-drop-staged-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const sourceHome = path.join(root, "codex-home");
    const sharedHostHome = path.join(root, "shared-codex-home");
  ...
  it("passes the Codex-specific teardown messages to the shared workspace-restore-teardown factory", async () => {
    ...
    const root = await makeTempRoot("geetorus-codex-acp-teardown-wiring-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    await fs.mkdir(remoteCwd, { recursive: true });
    ...
    expect(mockCreateWorkspaceRestoreTeardown).toHaveBeenCalledWith(
      expect.objectContaining({
        startMessage: "[geetorus] Restoring workspace changes and Codex auth from the sandbox.\n",
        failurePrefix: "[geetorus] Codex ACP teardown restore/copy-back failed",
      }),
    );
  });
  ...
  it("classifies ACP refresh-token auth failures", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-refresh-token-");
    const execute = createCodexAcpExecutor({
  ...
  it("resumes compatible ACP sessions on later Codex ACP runs", async () => {
    const root = await makeTempRoot("geetorus-codex-acp-resume-");
    const runtimes: FakeRuntime[] = [];
    const execute = createCodexAcpExecutor({
      createRuntime: (options: FakeRuntimeOptions) => {
        const runtime = new FakeRuntime(options);
