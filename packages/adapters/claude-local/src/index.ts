...
/** Resolve Geetorus's default without replacing an explicit provider model. */
...
- stateDir (string, optional): ACP session state directory; defaults to Geetorus-managed company/agent scoped storage
...
- For ACP runs, model selection is passed through ANTHROPIC_MODEL at ACP server startup; Geetorus-managed Claude permissions and ephemeral skill materialization are handled by the shared ACP engine.
