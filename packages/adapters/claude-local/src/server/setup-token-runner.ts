...
async function stopAndDispose(driver: SetupTokenPtyDriver, log: (line: string) => void): Promise<void> {
  try {
    ...
    log("[geetorus] Setup-token login: the process stop step errored.");
  }
  try {
    await driver.dispose();
  } catch {
    log("[geetorus] Setup-token login: the driver dispose step errored.");
  }
}
...
export async function runSetupTokenLogin(
  ...
  const submitCode = async (): Promise<void> => {
    ...
    try {
      ...
      log("[geetorus] Setup-token login: the code input step errored.");
      return;
    }
    ...
    try {
      ...
      codeSubmitted = true;
      log("[geetorus] Setup-token login: sent the browser code to the prompt.");
    } catch {
      log("[geetorus] Setup-token login: the code input step errored.");
    }
  };
  ...
  const deliverCredential = async (): Promise<boolean> => {
    ...
    try {
      ...
      log("[geetorus] Setup-token login: delivered the credential to the sink.");
      return true;
    } catch {
      log("[geetorus] Setup-token login: the credential delivery step errored.");
      return false;
    } finally {
      bytes.fill(0);
    }
  ...
  const onData = (chunk: string): void => {
    if (!promptSurfaced) {
      ...
      if (prompt) {
        ...
        buffer = "";
        onPrompt(prompt);
        log("[geetorus] Setup-token login: surfaced the sign-in prompt.");
        if (!submitStarted) {
          submitStarted = true;
  ...
  try {
    if (signal?.aborted) {
      log("[geetorus] Setup-token login cancelled before start.");
    ...
    if (raced.kind === "timeout") {
      log("[geetorus] Setup-token login timed out; stopping the process.");
      return result("timeout", null);
    }
    if (raced.kind === "cancelled") {
      log("[geetorus] Setup-token login cancelled; stopping the process.");
      return result("cancelled", null);
    }

    const exitCode = raced.exitCode;
    if (exitCode !== 0) {
      log("[geetorus] Setup-token login command ended with a non-zero exit code.");
    ...
    if (!delivered) {
      log("[geetorus] Setup-token login: the credential did not land; treating the run as a failure.");
      return result("failure", exitCode);
    }
    credentialDelivered = true;

    log("[geetorus] Setup-token login command ended successfully.");
