...
} from "@geetorusai/adapter-utils/acpx-engine/constants";
import type {
  AcpxEngineExecutorOptions,
  AcpxRemoteManagedHomeContext,
  AcpxRemoteManagedHomeResult,
} from "@geetorusai/adapter-utils/acpx-engine/execute";
import {
  asBoolean,
  asNumber,
  asString,
...
import { createWorkspaceRestoreTeardown } from "@geetorusai/adapter-utils/workspace-restore-teardown";
...
async function prepareClaudeRemoteManagedHome(
  ...
  const registerWorkspaceSyncBack = (
    ...
    createWorkspaceRestoreTeardown({
      stagedRuntime,
      onLog,
      startMessage: "[geetorus] Restoring workspace changes from the sandbox.\n",
      failurePrefix: "[geetorus] Claude ACP teardown workspace restore failed",
    });
  ...
  if (explicitClaudeConfigDir && !input.config.managedAiConnection) {
    ...
    if (isUnderWorkspace) {
      ...
      await onLog(
        "stdout",
        `[geetorus] Remapped operator CLAUDE_CONFIG_DIR from host path ${explicitClaudeConfigDir} onto the in-sandbox workspace path ${remappedConfigDir} for the remote ACP run.\n`,
      );
      return { stagedRuntime, teardown: registerWorkspaceSyncBack(stagedRuntime) };
    }
    await onLog(
      "stderr",
      `[geetorus] operator-provided CLAUDE_CONFIG_DIR=${explicitClaudeConfigDir} is outside the staged workspace and cannot reach the remote sandbox; ignoring the host-only path and seeding the managed Claude config instead.\n`,
    );
  }
  ...
  // selected skill's files (`materializeGeetorusSkillCopy` never copies a
  ...
  const remoteClaudeRuntimeRoot =
    ...
    path.posix.join(stagedRuntime.workspaceRemoteDir ?? input.workspaceLocalDir, ".geetorus-runtime", "claude");
  ...
  await onLog("stdout", `[geetorus] Materializing Claude auth/config into ${remoteClaudeConfigDir}.\n`);
...
export function createClaudeAcpExecutor(options: ClaudeAcpExecutorOptions = {}): ClaudeAcpExecutor {
  ...
  return async (ctx) => {
    ...
    if (!currentExecutor) {
      const { createAcpxEngineExecutor } = await import("@geetorusai/adapter-utils/acpx-engine/execute");
...
export async function probeClaudeAcpSandboxLogin(input: {
  ...
  if (targetIsRemote && target) {
    ...
    if (!built.command) {
      return [buildAcpLoginProbeUnavailableCheck("Claude is not installed on the Geetorus host.")];
    }
...
export async function testClaudeAcpEnvironment(
  ...
  if (targetIsRemote) {
    checks.push({
      ...
      hint: "Remote ACP requires a bidirectional process target such as SSH or Geetorus's sandbox process-session bridge.",
    });
  }
