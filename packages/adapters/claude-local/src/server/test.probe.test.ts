...
const {
  ...
  return {
    ...
    resolveAdapterExecutionTargetCwd: vi.fn(() => "/home/daytona/geetorus-workspace"),
  };
});
...
const initLine =
  '{"type":"system","subtype":"init","cwd":"/home/daytona/geetorus-workspace","session_id":"abc","tools":["Bash","Read"]}';
...
describe("claude CLI local hello probe hardening", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-cli-localprobe-"));
    ...
    resolveAdapterExecutionTargetCwd.mockReturnValue("/home/daytona/geetorus-workspace");
  });
  ...
  it("warns without executing when runtime PATH selects a different local Claude executable", async () => {
    const runtimeDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-cli-runtime-path-"));
  ...
  it("names the local host target on every result", async () => {
    ...
    expect(targetCheck?.message).toContain("Geetorus host");
  });
});
