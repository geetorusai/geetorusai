...
describe("claude_local ACP startup fallback", () => {
  ...
  it("trusts the Geetorus API URL when network access is allowlisted", async () => {
    const geetorusApiUrl = "http://127.0.0.1:4310";
    vi.stubEnv("GEETORUS_API_URL", geetorusApiUrl);
    ...
    expect(runAdapterExecutionTargetProcess).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        localProcessSandbox: expect.objectContaining({
          ...
          networkTrustedUrls: [geetorusApiUrl],
        }),
      }),
    );
  });
