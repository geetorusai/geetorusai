...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@geetorusai/adapter-utils";
import type { RunProcessResult } from "@geetorusai/adapter-utils/server-utils";
import {
...
import { buildSkillLibraryManifestMarkdown } from "@geetorusai/adapter-utils/skill-library-manifest";
import {
...
async function buildClaudeRuntimeConfig(input: ClaudeExecutionInput): Promise<ClaudeRuntimeConfig> {
  ...
  const env: Record<string, string> = { ...buildGeetorusEnv(agent) };
  ...
  if (shapedWorkspaceEnv.workspaceHints.length > 0) {
    env.GEETORUS_WORKSPACES_JSON = JSON.stringify(shapedWorkspaceEnv.workspaceHints);
  }
  ...
  for (const [key, value] of Object.entries(shapedEnvConfig)) {
    ...
    // Runtime GEETORUS_* always wins over config, and GEETORUS_API_KEY is
    // never accepted from config — the harness-minted run token is the only
    // source. Other GEETORUS_* keys Geetorus did not assign flow through.
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const claudeSkillEntries = await readGeetorusRuntimeSkillEntries(config, __moduleDir);
  ...
  const mountableSkillEntries = desiredSkillEntries.filter((entry) => !isGeetorusSkillSourceMissing(entry));
  for (const entry of desiredSkillEntries) {
    if (!isGeetorusSkillSourceMissing(entry)) continue;
    await onLog(
      "stderr",
      `[geetorus] Warning: skill "${entry.key}" is enabled for this agent but its files are unavailable and it was not mounted${entry.missingDetail ? `: ${entry.missingDetail}` : "."}\n`,
    );
  }
  ...
  const localMcpConfigPath = await writeGeetorusClaudeMcpConfig({
  ...
  if (localProcessSandbox) {
    ...
    await onLog(
      "stdout",
      `[geetorus] Confining Claude with ${scopes} scope.\n`,
    );
  }
  ...
  const preparedExecutionTargetRuntime = executionTargetIsRemote
    ? await (async () => {
        await onLog(
          "stdout",
          `[geetorus] Syncing workspace and Claude runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
  ...
  const effectivePromptBundleAddDir = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.assetDirs.skills ??
      path.posix.join(effectiveExecutionCwd, ".geetorus-runtime", "claude", "skills")
    : promptBundle.addDir;
  ...
  const effectiveMcpConfigPath = executionTargetIsRemote
    ? path.posix.join(
        preparedExecutionTargetRuntime?.assetDirs["mcp-config"] ??
          path.posix.join(effectiveExecutionCwd, ".geetorus-runtime", "claude", "mcp-config"),
        path.basename(localMcpConfigPath),
      )
    : localMcpConfigPath;
  const remoteClaudeRuntimeRoot = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.runtimeRootDir ??
      path.posix.join(effectiveExecutionCwd, ".geetorus-runtime", "claude")
    : null;
  ...
  if (remoteClaudeConfigDir && remoteClaudeConfigSeedDir) {
    ...
    await onLog(
      "stdout",
      `[geetorus] Materializing Claude auth/config into ${remoteClaudeConfigDir}.\n`,
    );
  ...
  if (executionTargetIsSandbox && effort) {
    ...
    if (supportsEffort === false) {
      ...
      await onLog(
        "stderr",
        `[geetorus] Claude CLI in the environment does not advertise --effort; omitting configured effort "${effort}". Upgrade the environment CLI/image to restore reasoning-effort control.\n`,
      );
    }
  }
  ...
  if (runtimeSessionId && !isValidUuid) {
    await onLog(
      "stdout",
      `[geetorus] Claude session "${runtimeSessionId}" is not a valid UUID and will not be passed to --resume.\n`,
    );
  }
  if (
    executionTargetIsRemote &&
    ...
    await onLog(
      "stdout",
      `[geetorus] Claude session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (
    runtimeSessionId &&
    isValidUuid &&
    ...
    await onLog(
      "stdout",
      `[geetorus] Claude session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  if (runtimeSessionId && runtimePromptBundleKey.length > 0 && runtimePromptBundleKey !== promptBundle.bundleKey) {
    await onLog(
      "stdout",
      `[geetorus] Claude session "${runtimeSessionId}" was saved for prompt bundle "${runtimePromptBundleKey}" and will not be resumed with "${promptBundle.bundleKey}".\n`,
    );
  }
  if (runtimeSessionId && !hasMatchingMcpServers) {
    await onLog(
      "stdout",
      `[geetorus] Claude session "${runtimeSessionId}" was saved with a different runtime MCP server set and will not be resumed.\n`,
    );
  }
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    if (runtimeMcpServers.length > 0) {
      commandNotes.push(
        `Using ${runtimeMcpServers.length} Geetorus-managed MCP server(s) from strict config ${effectiveMcpConfigPath}.`,
      );
    }
    if (onMeta) {
      await onMeta({
  ...
  const toAdapterResult = (
    ...
    // successful run to Geetorus and the heartbeat stalls silently. See RY-604.
  ...
  try {
    ...
    if (minimumCliVersion) {
      ...
      if (
        ...
        await onLog("stderr", `[geetorus] ${errorMessage}\n`);
        return {
          exitCode: 1,
          signal: null,
          timedOut: false,
    ...
    if (sessionErrorKind !== null) {
      ...
      await onLog(
        "stdout",
        `[geetorus] Claude resume session "${sessionId}" ${reason}; retrying with a fresh session.\n`,
      );
      if (sessionErrorKind === "poisoned" && !executionTargetIsRemote) {
        ...
        if (unlinked) {
          try {
            await onLog("stdout", `[geetorus] Removed poisoned session file: ${poisonedJsonlPath}\n`);
          } catch {
