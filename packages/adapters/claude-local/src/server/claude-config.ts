...
export async function writeGeetorusClaudeMcpConfig(input: {
  stateDir: string;
  runId: string;
...
export async function prepareClaudeConfigSeed(
  ...
  if (copiedFiles.length > 0) {
    await onLog(
      "stdout",
      `[geetorus] Prepared Claude config seed "${targetDir}" from "${sourceDir}" (${copiedFiles.map((file) => file.name).join(", ")}).\n`,
    );
  } else {
    await onLog(
      "stdout",
      `[geetorus] No local Claude config seed files were found in "${sourceDir}". Remote Claude auth may still require login.\n`,
    );
  }
 ...
 * materializes the Geetorus-managed Claude config directory. Both the CLI
  ...
  if (
    ...
    try {
      ...
      tempWorkspaceDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "geetorus-claude-envtest-workspace-"),
      );
      ...
      const runtimeRootDir =
        ...
        path.posix.join(managedRemoteCwd, ".geetorus-runtime", "claude");
      ...
      checks.push({
        ...
        level: "info",
        message: "The environment probe is using Geetorus-managed Claude config materialization.",
      ...
      logSandboxProbeDiagnostic(
        "Could not materialize Geetorus-managed Claude config for the environment probe",
        "spawn_error",
      ...
      checks.push({
        ...
        level: "error",
        message: "Could not materialize Geetorus-managed Claude config for the environment probe.",
