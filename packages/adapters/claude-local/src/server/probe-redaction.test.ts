...
// step resolves the Geetorus instance root first. This mock makes that resolve
...
vi.mock("@geetorusai/adapter-utils/server-utils", async () => {
  ...
  return {
    ...actual,
    resolveGeetorusInstanceRootForAdapter: (...args: unknown[]) => {
      if (resolveInstanceRoot.throwError) throw resolveInstanceRoot.throwError;
      return (
        actual.resolveGeetorusInstanceRootForAdapter as (...a: unknown[]) => string
      )(...args);
    },
  };
});
