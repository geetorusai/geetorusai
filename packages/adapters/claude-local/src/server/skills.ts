...
async function buildClaudeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildRuntimeMountedSkillSnapshot({
    ...
    availableEntries,
    desiredSkills,
    configuredDetail: "Will be materialized into the stable Geetorus-managed Claude prompt bundle on the next run.",
    externalInstalled: installed,
    externalLocationLabel: "~/.claude/skills",
    externalDetail: "Installed outside Geetorus management in the Claude skills home.",
    skillsHome,
  });
}
