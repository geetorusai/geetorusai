...
describe("claude remote execution", () => {
  ...
  it("prepares the workspace, syncs Claude runtime assets, and restores workspace changes for remote SSH execution", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-claude-remote-"));
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.geetorus-runtime/claude/skills`,
      followSymlinks: true,
    }));
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.geetorus-runtime/claude/mcp-config`,
      followSymlinks: true,
    }));
    ...
    expect(call?.[2]).toContain(
      `${managedRemoteWorkspace}/.geetorus-runtime/claude/skills/agent-instructions.md`,
    );
    expect(call?.[2]).toContain("--add-dir");
    expect(call?.[2]).toContain(`${managedRemoteWorkspace}/.geetorus-runtime/claude/skills`);
    expect(call?.[3].env.GEETORUS_WORKSPACE_CWD).toBe(managedRemoteWorkspace);
    expect(call?.[3].env.GEETORUS_WORKSPACE_WORKTREE_PATH).toBeUndefined();
  ...
  it("does not resume saved Claude sessions for remote SSH execution without a matching remote identity", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-claude-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
  ...
  it("resumes saved Claude sessions for remote SSH execution when the remote identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-claude-remote-resume-match-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
  ...
  it("forwards the duplex_channel_lost transport code on the unparsed Claude result path", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-claude-remote-duplex-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    runChildProcess.mockResolvedValueOnce({
      ...
      stderr:
        "[geetorus] The sandbox duplex control channel was lost (provider_exit) before the run completed.\n",
      pid: 123,
      startedAt: new Date().toISOString(),
  ...
  describe("CLI-lane model pass-through", () => {
    ...
    it("passes the exact configured Fable 5.1 ID as --model on the CLI lane", async () => {
      const { args } = await executeWithModel("geetorus-claude-model-direct-", {
    ...
    it("passes the Bedrock-native Fable 5.1 ID as --model under Bedrock auth", async () => {
      const { args } = await executeWithModel("geetorus-claude-model-bedrock-", {
    ...
    it("skips --model for a direct Anthropic ID under Bedrock auth", async () => {
      const { args } = await executeWithModel("geetorus-claude-model-bedrock-skip-", {
        model: "claude-fable-5-1",
        env: { CLAUDE_CODE_USE_BEDROCK: "1" },
      });
    ...
    it("rejects Fable 5.1 before launch when the CLI is older than 2.1.251", async () => {
      ...
      const { args, result } = await executeWithModel("geetorus-claude-model-old-cli-", {
        model: "claude-fable-5-1",
      });
    ...
    it("leaves Fable compatibility to explicitly configured custom CLI wrappers", async () => {
      const { args, result } = await executeWithModel("geetorus-claude-model-wrapper-", {
        command: "/opt/geetorus/claude-wrapper",
        model: "claude-fable-5-1",
      });
