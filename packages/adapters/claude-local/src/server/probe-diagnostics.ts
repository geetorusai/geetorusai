import type { AdapterEnvironmentCheck } from "@geetorusai/adapter-utils";

/**
 * The fixed label a Test result shows when the probe runs on the local
 * Geetorus host. The label is a constant, so a local target check never
 * carries an environment ID, a config value, or a credential-derived string.
 */
export const ADAPTER_TEST_HOST_TARGET_LABEL = "Geetorus host";
...
export function logSandboxProbeDiagnostic(
  ...
  console.warn(`[geetorus] ${context}`, detail);
}
