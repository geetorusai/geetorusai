...
import { prepareAdapterExecutionTargetRuntime } from "@geetorusai/adapter-utils/execution-target";
import {
...
const originalEnv: Record<string, string | undefined> = {
  GEETORUS_HOME: process.env.GEETORUS_HOME,
  GEETORUS_INSTANCE_ID: process.env.GEETORUS_INSTANCE_ID,
...
describe("claude_local ACP lane", () => {
  ...
  it("reports ACP prerequisites for the ACP lane", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-env-");
    const commandPath = path.join(root, "bin", "claude-agent-acp");
    await fs.mkdir(path.dirname(commandPath), { recursive: true });
    await fs.writeFile(commandPath, "#!/usr/bin/env sh\n", "utf8");
    setNodeVersion("v24.11.0");
  ...
  it("executes through ACPX with Claude model env, settings.local.json, and ephemeral skills", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-exec-");
  ...
  it("stages the skill bundle as a no-follow-symlinks asset for a remote ACP run, and points the prompt at the in-sandbox skill root", async () => {
    ...
    const root = await makeTempRoot("geetorus-claude-acp-skills-remote-");
    const skill = await createRuntimeSkill(root);
  ...
  it("stages no skills asset for a remote ACP run with no selected skill", async () => {
    vi.mocked(prepareAdapterExecutionTargetRuntime).mockClear();
    const root = await makeTempRoot("geetorus-claude-acp-skills-remote-empty-");
    ...
    // set (that fallback only fires when `geetorusRuntimeSkills` is absent).
    const skill = await createRuntimeSkill(root);
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
  ...
  it("stages the skill bundle inside the sandbox but never syncs it back into the host workspace", async () => {
    // The staged skill bundle lives under `.geetorus-runtime/claude/skills`,
    // inside the same in-sandbox directory the workspace restore reads. The
    // restore excludes the whole `.geetorus-runtime` tree
    ...
    const root = await makeTempRoot("geetorus-claude-acp-skills-no-syncback-");
    const skill = await createRuntimeSkill(root);
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    ...
    await expect(fs.access(path.join(localCwd, ".geetorus-runtime"))).rejects.toThrow();
  });

  it("passes the exact configured Fable 5.1 ID through ANTHROPIC_MODEL on the ACP lane", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-fable51-");
    const meta: AdapterInvocationMeta[] = [];
    const execute = createClaudeAcpExecutor({
      createRuntime: (options: FakeRuntimeOptions) => new FakeRuntime(options) as never,
    });
  ...
  it("creates the ACP session on the in-sandbox workspace cwd for runner-backed remote runs", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-remote-cwd-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    await fs.mkdir(remoteCwd, { recursive: true });
  ...
  it("seeds the managed Claude config into the sandbox and repoints CLAUDE_CONFIG_DIR to the in-sandbox path", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-home-seed-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    ...
    expect(remappedConfigDir).toContain(".geetorus-runtime");
  ...
  it("test_claude_acp_seam_registers_workspace_sync_back", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-syncback-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const sharedClaudeConfig = path.join(root, "shared-claude-config");
    await fs.mkdir(localCwd, { recursive: true });
  ...
  it("test_claude_acp_teardown_restore_failure_sanitizes_the_run_log", async () => {
    ...
    const root = await makeTempRoot("geetorus-claude-acp-restore-failure-");
  ...
  it("remaps a workspace-relative explicit CLAUDE_CONFIG_DIR onto the in-sandbox workspace path", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-explicit-inworkspace-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    await fs.mkdir(remoteCwd, { recursive: true });
    ...
    expect(String(meta[0]?.env?.CLAUDE_CONFIG_DIR ?? "")).not.toContain(".geetorus-runtime");
  ...
  it("ignores a host-only explicit CLAUDE_CONFIG_DIR that cannot reach the sandbox and seeds the managed config instead", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-explicit-hostonly-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const sharedClaudeConfig = path.join(root, "shared-claude-config");
  ...
  it("delivers the issue description exactly once per prompt and compacts non-assignment resume deltas", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-brief-");
    const runtimes: FakeRuntime[] = [];
    const execute = createClaudeAcpExecutor({
      createRuntime: (options: FakeRuntimeOptions) => {
        const runtime = new FakeRuntime(options);
    ...
    const wakeContext = (reason: string) => ({
      issueId: "issue-1",
      geetorusTaskMarkdown: fullTaskMarkdown,
      geetorusTaskMarkdownCompact: compactTaskMarkdown,
      geetorusWake: {
        reason,
        issue: {
          id: "issue-1",
    ...
    expect(freshPrompt).toContain("Geetorus task context:");
    ...
    expect(resumePrompt).toContain("Geetorus task context:");
  ...
  it("resumes compatible ACP sessions on later Claude ACP runs", async () => {
    const root = await makeTempRoot("geetorus-claude-acp-resume-");
    const runtimes: FakeRuntime[] = [];
    const execute = createClaudeAcpExecutor({
      createRuntime: (options: FakeRuntimeOptions) => {
        const runtime = new FakeRuntime(options);
