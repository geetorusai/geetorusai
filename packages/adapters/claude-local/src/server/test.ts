...
export async function testEnvironment(
  ...
  // When probing a remote target, the Geetorus host's process.env does not
  ...
  if (canRunProbe && configuredModelIsCompatible) {
    if (!claudeCommandLooksLike(command, "claude")) {
      ...
      checks.push({
        ...
        level: "warn",
        message: "Skipped the Claude hello probe because `claude` is not installed on the Geetorus host.",
        hint: "Install the `claude` CLI on the Geetorus host, then retry the Test.",
      });
    } else {
