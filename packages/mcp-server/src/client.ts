import type { GeetorusMcpConfig } from "./config.js";

export class GeetorusApiError extends Error {
...
export class GeetorusApiClient {
  constructor(private readonly config: GeetorusMcpConfig) {}
  ...
  resolveCompanyId(companyId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("companyId is required because GEETORUS_COMPANY_ID is not set");
    }
    return resolved;
  }
  ...
  resolveAgentId(agentId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("agentId is required because GEETORUS_AGENT_ID is not set");
    }
    return resolved;
  }
  ...
  async requestJson<T>(method: string, path: string, options: JsonRequestOptions = {}): Promise<T> {
    ...
    if ((options.includeRunId ?? isWriteMethod(method)) && this.config.runId) {
      headers["X-Geetorus-Run-Id"] = this.config.runId;
    }
