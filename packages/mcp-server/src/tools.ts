...
async function callRuntimeConnectionTool(
  endpointEnv: "GEETORUS_RUNTIME_TOOLS_CONNECTIONS_SEARCH_URL" | "GEETORUS_RUNTIME_TOOLS_CONNECTION_REQUEST_URL",
  body: unknown,
) {
  const endpoint = process.env[endpointEnv]?.trim();
  const token = process.env.GEETORUS_RUNTIME_TOOLS_TOKEN?.trim();
  if (!endpoint || !token) {
    throw new Error("Connection intent tools are available only inside an active Geetorus heartbeat run");
  }
...
async function getIssueWorkspaceRuntime(client: GeetorusApiClient, issueId: string) {
...
export function createToolDefinitions(client: GeetorusApiClient): ToolDefinition[] {
  return [
    makeTool(
      "connections_search",
      CONNECTIONS_SEARCH_TOOL_DESCRIPTION,
    ...
    makeTool(
      "geetorusMe",
      "Get the current authenticated Geetorus actor details",
      z.object({}),
    ...
    makeTool(
      "geetorusRequestCheckboxConfirmation",
    ...
    makeTool(
      "geetorusApiRequest",
      "Make a JSON request to an existing Geetorus /api endpoint for unsupported operations",
      apiRequestSchema,
