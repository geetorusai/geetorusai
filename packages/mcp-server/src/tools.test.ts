...
describe("geetorus MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("geetorusUpdateIssue");
    await tool.execute({
      issueId: "PAP-1135",
      status: "done",
    });
    ...
    expect((init.headers as Record<string, string>)["X-Geetorus-Run-Id"]).toBe(
  ...
  it("lists the company skill library with the default company id", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockJsonResponse([{ key: "geetorusai/bundled/product/wireframe", name: "wireframe" }]),
    );
    vi.stubGlobal("fetch", fetchMock);

    const tool = getTool("geetorusListSkills");
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("geetorusListIssues");
    const response = await tool.execute({});
  ...
  it("uses default agent id for checkout requests", async () => {
    ...
    const tool = getTool("geetorusCheckoutIssue");
    await tool.execute({
      issueId: "PAP-1135",
    });
  ...
  it("allows create issue requests to omit status so the API applies assignee defaults", async () => {
    ...
    const tool = getTool("geetorusCreateIssue");
    await tool.execute({
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("geetorusUpsertIssueDocument");
    await tool.execute({
      issueId: "PAP-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("controls issue workspace services through the current execution workspace", async () => {
    ...
    const tool = getTool("geetorusControlIssueWorkspaceServices");
    await tool.execute({
      issueId: "PAP-1135",
      action: "restart",
  ...
  it("waits for an issue workspace runtime service URL", async () => {
    ...
    const tool = getTool("geetorusWaitForIssueWorkspaceService");
    const response = await tool.execute({
      issueId: "PAP-1135",
      serviceName: "web",
      timeoutSeconds: 1,
  ...
  it("creates suggest_tasks interactions with the expected issue-scoped payload", async () => {
    ...
    const tool = getTool("geetorusSuggestTasks");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates request_confirmation interactions with plan target payloads", async () => {
    ...
    const tool = getTool("geetorusRequestConfirmation");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates request_checkbox_confirmation interactions with checkbox payloads", async () => {
    ...
    const tool = getTool("geetorusRequestCheckboxConfirmation");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("geetorusCreateApproval");
    await tool.execute({
      type: "hire_agent",
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("geetorusApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
