...
Geetorus Runner Protocol versions 1 and 2. The fixtures in `fixtures/` define
...
`unsupported` or `policy_disabled`. Geetorus sends controls only when the
...
Goal state is separate from Geetorus's company/business goal hierarchy. The
