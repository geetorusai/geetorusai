...
`geetorus-runnerd` supports one provider in this layer: Codex app-server as a
local supervised process. The Geetorus server selects this path only for the
default-off `geetorus_runner` adapter. Every direct adapter keeps its existing
...
- `request.resolve` translates a validated `geetorus.question_response.v1`
