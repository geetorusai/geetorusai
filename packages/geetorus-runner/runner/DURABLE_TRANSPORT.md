...
This layer gives `geetorus-runnerd` a provider-neutral PRP v1 transport. The
Geetorus server invokes durable mode only for a selected, flag-enabled
`geetorus_runner` agent or recovery of its persisted native run. Codex is the
...
- A bootstrap ticket is read from `GEETORUS_RUNNER_BOOTSTRAP_TICKET`, removed
...
Durable mode is selected only when `geetorus-runnerd` receives
