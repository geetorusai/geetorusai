...
const MANAGED_PROVIDER_STATE_SCHEMA: &str = "geetorus.runner.managed-provider-state.v1";
...
fn valid_agentcore_usage_snapshot(value: &Value) -> bool {
    ...
    reconciliation_valid
        ...
        && object.get("costSource").and_then(Value::as_str) == Some("geetorus_estimate")
}
            ...
            if lifecycle == "prepared" {
                ...
                if kind == ManagedProviderKind::AwsAgentcore {
                    state.provider_session_id =
                        Some(format!("geetorus-{}-{}", Uuid::new_v4(), Uuid::new_v4()));
                }
                self.save_state()?;
            } else if !matches!(
                lifecycle.as_str(),
...
fn managed_usage_event(
    ...
    NormalizedProviderEvent {
        ...
        payload: json!({
            ...
            "runDelta": Value::Null,
            "costSource": if descriptor.kind() == ManagedProviderKind::AwsAgentcore { "geetorus_estimate" } else { "provider_reported" },
        }),
    }
}
...
mod tests {
    ...
    fn managed_failure_event_types(recovery: bool) -> Vec<String> {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-managed-terminal-test-{}",
            Uuid::new_v4()
        ));
    ...
    fn durable_pending_agentcore_usage_is_charged_once_before_turn_admission() {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-managed-provider-test-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&directory).unwrap();
        #[cfg(unix)]
    ...
    fn claude_skill_ids_cannot_be_supplied_by_the_prepare_caller() {
        ...
        payload["provider"]["claudeManagedSkills"] = json!([{
            "skillId": "skill_not_owned_by_geetorus",
            "version": "skver_not_owned_by_geetorus"
        }]);
        let error = executor
