...
pub const TOOL_SET_SCHEMA: &str = "geetorus.runner.authorized-tools.v1";
pub const TOOL_CALL_SCHEMA: &str = "geetorus.prp.semantic_tool.v1";
...
const COMPLETION_INPUT_SCHEMA_HINT: &str = "Invalid geetorus_finish arguments. Required fields: reportedWorkDisposition, summary, completionClaim, evidence, and verification. When reportedWorkDisposition is yielded, continuation must include kind=response_wake, summary, and idempotencyKey.";
const BLOCK_INPUT_SCHEMA_HINT: &str = "Invalid geetorus_block arguments. Required fields: reportedWorkDisposition=blocked, summary, completionClaim, evidence, verification, and blocker. blocker must include reasonCode, owner, unblockAction, and scope.";
...
impl ProviderBridgeError {
    ...
    fn input_schema_validation(operation_id: &str) -> Self {
        let safe_provider_message = match operation_id {
            "geetorus_finish" => Some(COMPLETION_INPUT_SCHEMA_HINT),
            "geetorus_block" => Some(BLOCK_INPUT_SCHEMA_HINT),
            _ => None,
        };
        Self {
...
impl ProviderToolBridge {
    ...
    pub fn apply_result(&mut self, result: ToolResult) -> Result<Value, ProviderBridgeError> {
        ...
        if !result.is_error {
            // Geetorus semantic dispatchers return an authoritative envelope;
