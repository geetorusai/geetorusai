...
impl AcpxProviderSession {
    ...
    pub fn deliver_tool_result(&mut self, result: &ToolResult) -> Result<(), LocalRunnerError> {
        ...
        let resolution = if result.is_error {
            ...
            json!({
                ...
                "turnId":turn_id,
                "error":{"message":"Geetorus semantic operation failed"},
            })
        } else {
            json!({
                "callId":result.call_id,
    ...
    ///
    /// A governed Geetorus result can settle the run while the model is still
...
fn reserved_terminal_tool_set() -> Result<AuthorizedToolSet, LocalRunnerError> {
    ...
    .map_err(|_| LocalRunnerError::invalid("embedded Geetorus result schema is invalid"))?;
    let operations = vec![
        AuthorizedTool {
            operation_id: PRP_COMPLETION_TOOL_NAME.to_owned(),
            version: 1,
            description: "Return the authoritative Geetorus completion result.".to_owned(),
        ...
        AuthorizedTool {
            ...
            version: 1,
            description: "Return the authoritative Geetorus blocked result.".to_owned(),
            input_schema: result_schema.clone(),
...
fn validate_prp_run_result(value: &Value) -> Result<(), LocalRunnerError> {
    ...
    let validator = jsonschema::validator_for(&schema).map_err(|_| {
        LocalRunnerError::invalid("embedded Geetorus result schema cannot compile")
    })?;
    if !validator.is_valid(value) {
        return Err(LocalRunnerError::invalid(
            "ACPX reserved semantic result failed the Geetorus result schema",
        ));
    }
    Ok(())
}
