...
const RUNNER_STREAM_SCHEMA: &str = "geetorus.runner.stream.v1";
const RUNNER_COMMAND_SCHEMA: &str = "geetorus.prp.command.v1";
const HARNESS_COMMAND_SCHEMA: &str = "geetorus.fake_harness.command.v1";
const HARNESS_MESSAGE_SCHEMA: &str = "geetorus.fake_harness.message.v1";
...
fn valid_terminal_proposal(value: &Value) -> bool {
    value.get("schema").and_then(Value::as_str) == Some("geetorus.prp.terminal.v1")
        && enum_field(
            value,
            "turnTerminalState",
