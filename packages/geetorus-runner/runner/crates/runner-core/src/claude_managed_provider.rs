...
// Geetorus's generated instruction companion consumes one of those slots.
...
const SESSION_OWNERSHIP_METADATA_KEY: &str = "geetorus_ownership";
...
impl NetworkWorker {
    ...
    fn start_at(
        ...
        let join = thread::Builder::new()
            .name("geetorus-claude-managed-network".to_owned())
            .spawn(move || {
                network_loop(
                    api_key,
                    beta,
...
fn upload_managed_skill(
    ...
    content_identity.update(b"geetorus-managed-skill-v1\0");
    ...
    let boundary = format!("geetorus-{}-{upload_nonce}", &content_identity[..24]);
...
fn upload_managed_runtime_skills_at(
    ...
    let instruction_name = format!("geetorus-instructions-{}", &instruction_identity[..12]);
        ...
        match event_type {
            ...
            "agent.custom_tool_use" => {
                ...
                let validator = jsonschema::validator_for(schema).map_err(|_| {
                    LocalRunnerError::invalid(
                        "authorized Geetorus tool has an invalid JSON Schema",
                    )
                })?;
    ...
    let payload = tools
        ...
        .map(|tool| {
            jsonschema::validator_for(&tool.input_schema).map_err(|_| {
                LocalRunnerError::invalid(format!(
                    "Geetorus tool {} has an invalid JSON Schema",
                    tool.operation_id
                ))
            })?;
...
fn verify_remote_session(
    ...
    {
        return Err(LocalRunnerError::invalid(
            "Anthropic session identity does not match its immutable Geetorus profile",
        ));
    }
    Ok(())
}
...
fn session_ownership_marker(ownership_scope: &str) -> String {
    format!("geetorus-{:x}", Sha256::digest(ownership_scope.as_bytes()))
}
...
fn reconcile_managed_session(
    ...
    match matches.as_slice() {
        ...
        duplicates => {
            ...
            Err(LocalRunnerError::invalid(
                "Anthropic session reconciliation removed duplicate Geetorus-owned sessions",
            ))
        }
    }
}
...
mod tests {
        ...
        if request.method == "POST" && request.path.starts_with("/v1/sessions?") {
            ...
            if keys
                ...
                || body
                    .pointer(&format!("/metadata/{SESSION_OWNERSHIP_METADATA_KEY}"))
                    .and_then(Value::as_str)
                    .is_none_or(|marker| !marker.starts_with("geetorus-"))
            {
                return send_json_response(
                    socket,
                    "400 Bad Request",
    ...
    fn config() -> ClaudeManagedProviderConfig {
        ClaudeManagedProviderConfig {
            ...
            instructions: "Geetorus test system instructions".to_owned(),
    ...
    fn runtime_config_with_one_skill() -> (PathBuf, ClaudeManagedProviderConfig) {
        let root = std::env::temp_dir().join(format!(
            "geetorus-claude-managed-failure-test-{}",
            Uuid::new_v4()
        ));
    ...
    fn managed_overrides_pin_inline_tools_and_never_embed_remote_mcp_credentials() {
        let overrides = managed_agent_overrides(
            &config(),
            "Geetorus test system instructions",
        ...
        assert_eq!(
            ...
            Some(&json!("Geetorus test system instructions"))
        );
    ...
    fn managed_system_replaces_the_local_instruction_path_with_the_companion_skill() {
        ...
        assert!(instructions.contains("attached `geetorus-instructions-"));
    ...
    fn managed_skill_uploads_include_instruction_siblings_and_complete_assigned_skill_trees() {
        ...
        let root = std::env::temp_dir().join(format!(
            "geetorus-claude-managed-context-{}",
            uuid::Uuid::new_v4()
        ));
        let instruction_root = root.join("instructions");
    ...
    fn managed_skill_count_is_rejected_before_any_remote_upload() {
        let service = FakeAnthropicService::start(vec![], vec![]);
        let root = std::env::temp_dir().join(format!(
            "geetorus-claude-managed-context-count-{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn managed_skills_upload_once_reuse_on_cold_recovery_and_delete_with_session() {
        ...
        let root = std::env::temp_dir().join(format!(
            "geetorus-claude-managed-lifecycle-{}",
            uuid::Uuid::new_v4()
        ));
        let instruction_root = root.join("instructions");
        let skill_root = root.join("reviewer");
    ...
    fn managed_transport_streams_reconciles_tools_and_accounts_without_leaking_auth() {
        ...
        assert!(!serialized.contains("GEETORUS_API_KEY"));
    ...
    #[test]
    fn malformed_tool_arguments_fail_closed_before_geetorus_execution() {
        let service = FakeAnthropicService::start(
            vec![json!({
