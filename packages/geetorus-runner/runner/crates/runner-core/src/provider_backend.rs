...
const PROVIDER_STATE_SCHEMA: &str = "geetorus.runner.codex-provider-state.v1";
...
const GENERIC_INVALID_TOOL_CALL_MESSAGE: &str = "Geetorus rejected this semantic tool call";
...
impl ProviderStartupAttempt {
    fn validate(&self) -> Result<(), DurableRunnerError> {
        ...
        if self.schema != "geetorus.provider_startup.v1"
...
impl ProviderEventIdentity {
    ...
    fn source_event_id(&self, executor_event_id: &str) -> String {
        ...
        hasher.update(b"geetorus.executor-event.v1\0");
    ...
    let envelope = safe_result
        ...
        .filter(|receipt| {
            receipt.get("schema").and_then(Value::as_str)
                == Some("geetorus.prp.semantic_tool.v1")
...
fn validate_opencode_run_result(
    ...
    if state.config.provider != "opencode" {
        return Err(DurableRunnerError::invalid(
            "geetorus/runResult is reserved for the verified OpenCode provider",
        ));
    }
    if state.lifecycle != "turn_active" || state.active_provider_turn_id.is_none() {
        return Err(DurableRunnerError::invalid(
            "OpenCode emitted geetorus/runResult without an active provider turn",
        ));
    }
    ...
    {
        return Err(DurableRunnerError::invalid(
            "OpenCode geetorus/runResult is not bound to the active provider turn",
        ));
    }
    let result = params.get("result").cloned().ok_or_else(|| {
        DurableRunnerError::invalid("OpenCode geetorus/runResult omitted its result")
    })?;
...
fn validate_run_result(
    ...
    .map_err(|_| DurableRunnerError::invalid("embedded Geetorus result schema is invalid"))?;
    let validator = jsonschema::validator_for(&schema).map_err(|_| {
        DurableRunnerError::invalid("embedded Geetorus result schema cannot compile")
    })?;
    if !validator.is_valid(result) {
        return Err(DurableRunnerError::invalid(
            "provider semantic result failed the Geetorus result schema",
        ));
    }
    let contract = state.completion_contract.as_ref().ok_or_else(|| {
        DurableRunnerError::invalid("OpenCode geetorus/runResult has no bound completion contract")
    })?;
    let claim = result.get("completionClaim").ok_or_else(|| {
        DurableRunnerError::invalid("OpenCode geetorus/runResult omitted its completion claim")
    })?;
    if claim.get("contractRevision").and_then(Value::as_str) != Some(contract.revision.as_str()) {
        return Err(DurableRunnerError::invalid(
            "OpenCode geetorus/runResult changed its completion contract revision",
        ));
    }
    let criteria = claim
        .get("criteria")
        ...
        .ok_or_else(|| {
            DurableRunnerError::invalid(
                "OpenCode geetorus/runResult omitted its completion criteria",
            )
        })?;
    ...
    for criterion in criteria {
        let criterion_id = criterion
            ...
            .ok_or_else(|| {
                DurableRunnerError::invalid(
                    "OpenCode geetorus/runResult has an invalid completion criterion",
                )
            })?;
        if !reported_criterion_ids.insert(criterion_id) {
            return Err(DurableRunnerError::invalid(
                "OpenCode geetorus/runResult repeated a completion criterion",
            ));
        }
    }
    ...
    {
        return Err(DurableRunnerError::invalid(
            "OpenCode geetorus/runResult changed its bound completion criteria",
        ));
    }
...
fn admit_terminal_tool_authority(
    ...
    if result_is_error || !matches!(operation_id, "geetorus_finish" | "geetorus_block") {
        return Ok(());
    }
    ...
    let disposition_matches_operation = match operation_id {
        ...
        "geetorus_block" => disposition == "blocked",
        _ => false,
    };
...
fn normalize_provider_notification(
    ...
    match (
        ...
        _ => Err(DurableRunnerError::invalid(
            "OpenCode emitted conflicting geetorus/runResult notifications for one turn",
        )),
    }
}
...
fn terminal_events(
    ...
    // Once the correlated semantic-tool result has been accepted, Geetorus's
...
fn goal_event_payload(
    ...
    json!({
        "schema": "geetorus.session_goal.snapshot.v1",
        "goal": goal,
...
impl CodexCommandExecutor {
    ...
    fn bind_opencode_launch_profile(
        ...
        digest.update(b"geetorus.runner.opencode-launch-profile.v1\0");
    ...
    fn begin_startup(
        ...
        state.startup_attempt = Some(ProviderStartupAttempt {
            schema: "geetorus.provider_startup.v1".to_owned(),
        ...
        let admission = (|| -> Result<(), DurableRunnerError> {
            ...
            if legacy_epoch_is_ambiguous {
                ...
                payload: json!({
                    ...
                    "message": format!("{provider_name} recovery could not safely identify and settle active work from a saturated legacy replay epoch; Geetorus terminated the provider and closed the durable run"),
                    "geetorusAccepted": false,
                ...
                .cloned()
            {
                ...
                payload: json!({
                    ...
                    "message": format!("{provider_name} recovery reported a previously settled turn identity as active; Geetorus terminated the provider and closed the durable run"),
                    "geetorusAccepted": false,
                ...
                || recovered_active_turn_id != previous_active_turn_id
            {
                ...
                if recovered_turn_ended {
                    ...
                    if recovered_turn_ended_with_result {
                        // The durable correlated tool receipt proves Geetorus
    ...
    fn stable_launch_args(args: &[String]) -> Vec<String> {
        ...
        while index < args.len() {
            if args[index] == "-c" && index + 1 < args.len() {
                ...
                if matches!(
                    key,
                    "permissions.geetorus-runner-workspace-only.filesystem"
                        | "permissions.geetorus-runner-workspace-read-only.filesystem"
                        | "permissions.geetorus-runner-workspace-only.network.enabled"
                        | "permissions.geetorus-runner-workspace-read-only.network.enabled"
    ...
    fn close_after_rejected_provider_acceptance(
        ...
        // work that Codex accepted without Geetorus accepting its identity.
        self.provider = None;
        let state = self
            .state
            .as_mut()
        ...
        let _ = state.push_terminal_event(NormalizedProviderEvent {
            ...
            payload: json!({
                ...
                "message": match rejected_accepted_turn {
                    RejectedAcceptedTurn::ReusedIdentity(_) => format!("{provider_name} accepted work with a previously settled turn identity; Geetorus terminated the provider and closed the durable run"),
                    RejectedAcceptedTurn::InvalidIdentity => format!("{provider_name} accepted work without a valid bounded turn identity; Geetorus terminated the provider and closed the durable run"),
                },
                "geetorusAccepted": false,
    ...
    fn stop_turn_at_tool_receipt_limit(
        ...
        let rejection = ToolResult {
            ...
            result: json!({
                "error": {
                    "code": "semantic_tool_turn_receipt_limit",
                    "message": "Geetorus stopped this turn at its durable semantic-tool receipt limit",
                    "retryable": false,
                },
            }),
            is_error: true,
            ...
            terminal_tool_input
                ...
                .is_some_and(|(operation_id, _)| {
                    !result.is_error
                        && matches!(
                            operation_id.as_str(),
                            "geetorus_finish" | "geetorus_block"
                        )
                });
                        ...
                        match reconciliation {
                            ...
                            Err(_) => {
                                state.push_event(NormalizedProviderEvent {
                                    ...
                                    priority: EventPriority::P0,
                                    payload: json!({
                                        "schema": "geetorus.provider.notice.v1",
                                        ...
                                        "summary": "Codex goal state could not be reconciled after the turn; Geetorus retained the last durable snapshot.",
                                    }),
                                })?;
                            }
                        }
...
mod tests {
    ...
    fn startup_evidence_save_failure_cannot_become_an_empty_drain_or_admission() {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-startup-write-failure-{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn goal_command_can_persist_provider_restart_ownership() {
        for command_type in ["session.goal.get", "session.goal.set", "session.goal.clear"] {
            let directory = std::env::temp_dir()
                .join(format!("geetorus-goal-startup-{}", uuid::Uuid::new_v4()));
            let mut executor = CodexCommandExecutor::new(&directory);
    ...
    fn failed_authenticated_identity_commit_keeps_the_durable_startup_fence() {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-startup-identity-commit-{}",
            uuid::Uuid::new_v4()
        ));
        let mut executor = CodexCommandExecutor::new(&directory);
        let mut state = opencode_result_state();
    ...
    fn runtime_launch_rebinding_preserves_protected_arguments() {
        let before: Vec<String> = vec![
            "-c",
            "default_permissions=\"geetorus-runner-workspace-only\"",
            "-c",
        ...
        let after: Vec<String> = vec![
            ...
            "-c",
            "shell_environment_policy.include_only=[\"GEETORUS_GITHUB_BROKER_TOKEN\"]",
            "--disable",
            "image_generation",
            "app-server",
        ]
    ...
    fn invalid_tool_results_expose_only_reserved_static_schema_guidance() {
        ...
        let block_result = schema_rejection("geetorus_block", json!({}));
    ...
    fn preserves_one_verified_opencode_result_before_its_terminal() {
        ...
        let result_events =
            normalize_provider_notification(&mut state, "geetorus/runResult", &params).unwrap();
        let replay_events =
            normalize_provider_notification(&mut state, "geetorus/runResult", &params).unwrap();
    ...
    fn stopped_prepared_checkpoint_rebinds_without_restarting_its_old_provider() {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-provider-stopped-checkpoint-rebind-{}",
            std::process::id()
        ));
    ...
    fn accepted_terminal_tool_suppresses_the_generated_terminal_fallback() {
        ...
        admit_terminal_tool_authority(&mut state, "geetorus_finish", &result, false).unwrap();
        let terminal = terminal_events(&state, "turn.completed", None);
    ...
    fn terminal_tool_authority_rejects_an_unbound_yield() {
        ...
        assert!(
            admit_terminal_tool_authority(&mut state, "geetorus_finish", &result, false,).is_err()
        );
    }
    ...
    fn rejects_unbound_conflicting_or_spoofed_opencode_results() {
        ...
        assert!(normalize_provider_notification(
            &mut malformed,
            "geetorus/runResult",
            &params(json!({"schema": "geetorus.run_result.v1"})),
        )
        .unwrap_err()
        .to_string()
        .contains("failed the Geetorus result schema"));
    ...
    fn semantic_finish_input_preserves_a_complete_long_redacted_summary() {
        ...
        let call = PendingToolCall {
            call_id: "call-1".to_owned(),
            operation_id: "geetorus_finish".to_owned(),
    ...
    fn restore_reattaches_the_durable_tool_result_byte_counter() {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-provider-tool-byte-restore-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&directory);
        fs::create_dir_all(&directory).unwrap();
    ...
    fn receipt_limit_deadline_settlement_preserves_unacknowledged_events() {
        let directory = std::env::temp_dir().join(format!(
            "geetorus-provider-receipt-deadline-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&directory);
        fs::create_dir_all(&directory).unwrap();
