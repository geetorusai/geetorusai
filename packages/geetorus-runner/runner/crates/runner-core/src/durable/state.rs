...
const STATE_SCHEMA: &str = "geetorus.runner.durable.state.v1";
...
impl Command {
    pub fn validate(&self) -> Result<(), DurableRunnerError> {
        let schema_version = match self.schema.as_str() {
            "geetorus.prp.command.v1" => 1,
            "geetorus.prp.command.v2" => 2,
            _ => {
                return Err(DurableRunnerError::invalid(
                    "command requires a supported geetorus.prp.command schema",
                ));
            }
        };
        if schema_version == 1 && self.command_type.starts_with("session.goal.") {
            return Err(DurableRunnerError::invalid(
                "session goal commands require the geetorus.prp.command.v2 schema",
            ));
        }
...
impl WarmRunTransition {
    pub(crate) fn new(
        ...
        let mut receipt = Self {
            schema: "geetorus.runner.warm-transition.v1".to_owned(),
            ...
            connection: command
                .payload
                .pointer("/geetorusNextAuthority/connection")
                .cloned()
...
impl DurableState {
    ...
    pub(crate) fn enqueue_event_with_source_event_id(
        ...
        let envelope = json!({
            ...
            "itemId": self.item_id,
            "payload": {
                "schema": format!("geetorus.prp.event.v{schema_version}"),
...
pub(crate) fn sanitize_semantic_tool_input(
    ...
    if !matches!(operation_id, "geetorus_finish" | "geetorus_block") {
        return Ok(sanitized);
    }
...
fn finalize_semantic_tool_input_payload(
    ...
    if original_tool.get("schema").and_then(Value::as_str) != Some("geetorus.prp.semantic_tool.v1")
...
fn validate_semantic_tool_input_digest(
    ...
    if semantic_tool.get("schema").and_then(Value::as_str) != Some("geetorus.prp.semantic_tool.v1")
...
mod tests {
    ...
    fn session_goal_commands_require_and_accept_the_v2_schema() {
        ...
        goal.schema = "geetorus.prp.command.v2".to_owned();
        assert!(goal.validate().is_ok());
    }

    fn temporary_directory(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "geetorus-runner-durable-{label}-{}",
            std::process::id()
        ))
    }
    ...
    fn warm_transition_snapshot_is_exact_and_never_a_legacy_state() {
        ...
        attach.payload = json!({"geetorusNextAuthority": {
        ...
        for (pointer, replacement) in [
            ...
            (
                "/warmTransition/command/payload/geetorusNextAuthority/identity/itemId",
                json!("foreign_item"),
            ),
    ...
    fn session_goal_events_use_the_prp_v2_event_schema() {
        ...
        assert_eq!(
            ...
            Some(&json!("geetorus.prp.event.v2")),
        );
        assert_eq!(
    ...
    fn semantic_finish_digest_covers_the_exact_finally_persisted_long_summary() {
        ...
        let once_sanitized_input =
            sanitize_semantic_tool_input("geetorus_finish", &input).unwrap();
        let payload = json!({
            "semantic_tool": {
    ...
    fn semantic_summary_capacity_and_digest_resealing_fail_closed_outside_exact_finish_input() {
        ...
        let safe_input = sanitize_semantic_tool_input("geetorus_finish", &long_input).unwrap();
        ...
        let tampered_schema_payload = json!({
            "semantic_tool": {
                "schema": "geetorus.prp.semantic_tool.tampered",
                "phase": "input",
                "operationId": "geetorus_finish",
    ...
    fn semantic_redaction_preserves_benign_token_system_prose() {
        ...
        state.begin_command(&command).unwrap();
        let result = json!({
            "result": {"schema": "geetorus.prp.run_result.v1", "summary": observed_prose},
    ...
    fn sentence_period_redaction_keeps_credentials_and_embedded_dots_private() {
        ...
        state.complete_command(&command, json!({
            "result": {
                "schema": "geetorus.prp.run_result.v1",
