...
const SECURE_FRAME_SCHEMA: &str = "geetorus.runner.secure-frame.v1";
        ...
        Self {
            credential_id: format!(
                ...
                hex_encode(&digest_domain(
                    "geetorus-runner-credential-id-v1",
                    &[token.as_bytes()]
                ))
            ),
            auth_key: digest_domain("geetorus-runner-auth-key-v1", &[token.as_bytes()]),
        }
    }
}
...
impl SecureChannel {
    ...
    fn new(
        ...
        let session_binding = digest_domain(
            "geetorus-runner-session-binding-v1",
...
mod tests {
    ...
    fn reconnect_without_reexecuting(renewal_reply_lost: bool) {
        ...
        let directory = std::env::temp_dir().join(format!(
            "geetorus-runner-reconnect-fault-{}-{renewal_reply_lost}",
            std::process::id()
        ));
