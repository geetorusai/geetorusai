...
//!
//! The Harness owns the remote model loop. Geetorus supplies only caller-side
//! inline functions, then executes those functions through the durable PRP
//! bridge. No Geetorus credential or callback address enters AgentCore.
...
// InvokeHarness accepts at most 50 HarnessSkill entries. Geetorus reserves one
...
fn restored_usage_snapshot(snapshot: Option<&Value>) -> Result<Value, LocalRunnerError> {
    ...
    if object.get("costSource").and_then(Value::as_str) != Some("geetorus_estimate") {
...
impl NetworkWorker {
    fn start(
        ...
        let join = thread::Builder::new()
            .name("geetorus-aws-agentcore-network".to_owned())
            .spawn(move || {
    ...
    for (relative, bytes) in files {
            ...
            .await
        {
            let metadata_digest = existing
                .metadata()
                .and_then(|metadata| metadata.get("geetorus-sha256"))
                .map(String::as_str);
        ...
        client
            ...
            .metadata("geetorus-sha256", content_digest)
...
fn prepare_agentcore_runtime_context_assets(
    ...
    let context = config.runtime_context.as_ref().ok_or_else(|| {
        "AgentCore requires geetorus.native-execution-input.v3 runtimeContext".to_owned()
    })?;
    ...
    let instruction_companion = format!(
        "---\nname: geetorus-instructions-{}\ndescription: Geetorus agent instruction sibling bundle\n---\nRead `instructions/{entry_path}` and its sibling files as read-only context.\n",
...
fn agentcore_system_instructions(config: &AwsAgentCoreProviderConfig) -> Result<String, String> {
    ...
    config
        ...
        .map(|prefix| format!(
            "{prefix}Read-only instruction siblings are in the attached Geetorus HarnessSkill under `instructions/`."
        ))
    ...
    match event {
        ...
        InvokeHarnessStreamOutput::ContentBlockStop(value) => {
                ...
                tool_blocks.remove(&value.content_block_index())
            {
                ...
                // across PRP as a Geetorus semantic operation.
...
impl AwsAgentCoreHarnessProvider {
    pub fn start(
        ...
        let actor_id = format!("geetorus-{}", sha_hex(&session_id, 32));
    ...
    fn record_usage(
        ...
        if enforce_budget && estimate.is_some_and(|value| value >= self.max_estimated_cost_usd) {
            ...
            self.queue.push_back(ProviderEvent::Notification {
                ...
                params: json!({ "turnId": self.current_turn_id, "status": "limit_reached", "stopReason": "estimated_session_cost", "estimatedCostUsd": estimate, "costSource": "geetorus_estimate" }),
            });
        }
    ...
    fn invoke_tool_result_continuation(&mut self) -> Result<(), LocalRunnerError> {
        ...
        // transport ambiguity therefore never repeats the Geetorus mutation;
...
impl Provider for AwsAgentCoreHarnessProvider {
    ...
    fn increase_budget(&mut self, value: f64) -> Result<Value, LocalRunnerError> {
        ...
        Ok(json!({ "maxEstimatedSessionCostUsd": value, "costSource": "geetorus_estimate" }))
    }
        ...
        match kind {
            ...
            NetworkEventKind::ToolUse {
                ...
                if !validator_for(schema)
                    .map_err(|_| {
                        LocalRunnerError::invalid(
                            "authorized Geetorus tool has invalid JSON Schema",
                        )
                    })?
                    .is_valid(&input)
                {
            ...
            NetworkEventKind::InvocationComplete => {
                ...
                match reason.as_str() {
                    ...
                    | "model_context_window_exceeded" => Ok(Some(ProviderEvent::Notification {
                        method: "provider/budgetReached".to_owned(),
                        params: json!({ "turnId": self.current_turn_id, "status": "limit_reached", "stopReason": reason, "costSource": "geetorus_estimate" }),
                    })),
...
> {
    ...
    for tool in tools {
        validator_for(&tool.input_schema).map_err(|_| {
            LocalRunnerError::invalid("authorized Geetorus tool has invalid JSON Schema")
        })?;
...
fn invocation_allowed_tools(mut allowed: Vec<String>, has_skills: bool) -> Vec<String> {
            ...
            .any(|tool| tool == AGENTCORE_HARNESS_SKILLS_TOOL)
    {
        ...
        // scoped Geetorus inline functions whenever skills are attached.
...
fn new_runtime_session_id() -> String {
    format!("geetorus-{}-{}", Uuid::new_v4(), Uuid::new_v4())
}
...
mod tests {
    ...
    fn config() -> AwsAgentCoreProviderConfig {
        AwsAgentCoreProviderConfig {
            ...
            invocation_role_arn: "arn:aws:iam::123456789012:role/geetorus-agentcore".to_owned(),
            context_bucket: "geetorus-context-test".to_owned(),
            ...
            timeout_seconds: 300,
            instructions: "Geetorus test instructions".to_owned(),
    ...
    fn provider_with_worker(worker: NetworkWorker, usage: Value) -> AwsAgentCoreHarnessProvider {
        AwsAgentCoreHarnessProvider {
            config: config(),
            session_id: "geetorus-test-session".to_owned(),
            actor_id: "geetorus-test-actor".to_owned(),
            worker,
            tools: Vec::new(),
    ...
    fn agentcore_system_replaces_the_local_instruction_path_with_the_harness_skill() {
        ...
        let local_root = "/geetorus/runtime/instructions";
        ...
        assert!(instructions.starts_with("geetorus prompt\n\nAGENTS entry\n\n"));
        assert!(instructions.ends_with("attached Geetorus HarnessSkill under `instructions/`."));
    ...
    fn agentcore_rejects_too_many_assigned_skills_before_reading_any_skill_root() {
        let root = std::env::temp_dir().join(format!(
            "geetorus-agentcore-context-count-{}",
            uuid::Uuid::new_v4()
        ));
    ...
    #[test]
    fn tool_allowlist_is_confined_to_the_geetorus_inline_namespace() {
