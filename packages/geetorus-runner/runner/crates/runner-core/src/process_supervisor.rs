...
const VERIFIED_RUNTIME_EXECUTABLE_ENV: &str = "GEETORUS_VERIFIED_RUNTIME_EXECUTABLE";
...
fn sealed_snapshot(
    ...
    let fd = match rustix::fs::memfd_create("geetorus-verified-launch", flags) {
        Ok(fd) => fd,
        Err(rustix::io::Errno::INVAL) => rustix::fs::memfd_create(
            "geetorus-verified-launch",
...
mod tests {
    ...
    fn commonjs_artifacts_use_the_bounded_descriptor_loader() {
        ...
        let directory = std::env::temp_dir().join(format!(
            "geetorus-commonjs-launch-{}-{nonce}",
            std::process::id()
        ));
    ...
    fn executable_arguments_use_the_private_nested_launch_contract() {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let directory = std::env::temp_dir().join(format!(
            "geetorus-executable-argument-launch-{}-{nonce}",
            std::process::id()
        ));
        fs::create_dir_all(&directory).unwrap();
        let program = verified_artifact(&directory.join("node"), b"node");
        ...
        assert!(private_directory
            ...
            .unwrap()
            .to_string_lossy()
            .starts_with(".geetorus-verified-executable-"));
        assert_eq!(
