...
pub fn normalize_codex_notification(method: &str, params: &Value) -> Vec<NormalizedProviderEvent> {
    ...
    match method {
        ...
        "geetorus/resumeUsageSnapshot" => push(
            &mut events,
            "harness.diagnostic",
            EventPriority::P0,
            json!({
...
fn acpx_message_item_id(message_id: &str, turn_id: &str, channel: &str) -> String {
    ...
    digest.update(b"geetorus.acpx.message-item.v1\0");
...
fn acpx_opaque_item_id(value: &str, fallback: &str, kind: &str) -> String {
    ...
    let mut digest = Sha256::new();
    digest.update(b"geetorus.acpx.opaque-item.v1\0");
...
fn normalize_acpx_status(
    ...
    if tag == "usage_update" {
        ...
        // that feeds a Geetorus token budget is explicitly present.
...
fn safe_acpx_location(value: Option<&Value>, allow_create_target: bool) -> Value {
    ...
    if value.get("pathBoundary").and_then(Value::as_str)
        != Some("geetorus.workspace_relative_display.v2")
    {
        return Value::Null;
    }
    let raw_path = value
    ...
    if requires_entry_attestation {
        ...
        if attestation != Some("geetorus.workspace_entry.v1")
            && !(allow_create_target && attestation == Some("geetorus.workspace_create_target.v1"))
        {
            return Value::Null;
        }
    }
    ...
    fn preserves_dynamic_tool_identity_without_arguments() {
        for method in ["item/started", "item/completed"] {
            let events = normalize_codex_notification(
                method,
                &json!({"item": {
                    "id": "finish-1", "type": "dynamicToolCall", "tool": "geetorus_finish",
                    "status": if method == "item/started" { "inProgress" } else { "completed" },
                    "arguments": {"secret": "not-for-the-log"}
                }}),
            );
            assert_eq!(events.len(), 1);
            assert_eq!(events[0].payload["name"], "geetorus_finish");
        ...
        for location in [
            ...
            assert_eq!(
                safe_acpx_location(
                    Some(&json!({
                        "path": location,
                        "pathBoundary": "geetorus.workspace_relative_display.v2"
                    })),
                    false,
                ),
                Value::Null,
            ...
            assert_eq!(
                safe_acpx_location(
                    Some(&json!({
                        "path": location,
                        "pathBoundary": "geetorus.workspace_relative_display.v2",
                        "pathAttestation": "geetorus.workspace_entry.v1"
                    })),
                    false,
                ),
