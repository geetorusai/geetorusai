...
const ACPX_PROVIDER_STATE_SCHEMA: &str = "geetorus.runner.acpx-provider-state.v3";
...
impl AcpxCommandExecutor {
    ...
    fn goal_control(
        ...
        if projection.get("schema").and_then(Value::as_str)
            != Some("geetorus.session_goal.snapshot.v1")
        {
            return Err(DurableRunnerError::invalid(
...
mod tests {
    ...
    fn temporary_directory(label: &str) -> PathBuf {
        ...
        let directory = std::env::temp_dir().join(format!(
            "geetorus-acpx-backend-{label}-{}-{nonce}",
            std::process::id()
        ));
    ...
    fn active_turn_recovery_closes_without_starting_the_provider() {
        ...
        assert_eq!(events[1].payload["schema"], "geetorus.prp.terminal.v1");
