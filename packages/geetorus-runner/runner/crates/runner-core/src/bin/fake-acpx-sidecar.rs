...
use geetorus_runner_core::generated_acpx_sidecar_contract::GENERATED_ACPX_SIDECAR_PROTOCOL_VERSION;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in stdin.lock().lines() {
        ...
        if mode == "goals" && command.starts_with("session.goal.") {
            ...
            let projection = json!({
                "schema":"geetorus.session_goal.snapshot.v1", "goal":goal,"workingNow":false,
            ...
            "mcp-environment" => {
                write_json(
                    ...
                    &json!({
                        ...
                        "id": id, "ok": true,
                        "result": {
                            "name": std::env::var("GEETORUS_NATIVE_MCP_NAME").ok(),
                            "url": std::env::var("GEETORUS_NATIVE_MCP_URL").ok(),
                            "hasToken": std::env::var("GEETORUS_NATIVE_MCP_TOKEN").is_ok(),
...
fn bootstrap_success(
    ...
    let result = match command {
            ...
            "resolved":if mode == "resolutions-error-redaction" {
                params.get("callId").and_then(Value::as_str) == Some("call-1")
                    ...
                    && params.pointer("/error/message").and_then(Value::as_str)
                        == Some("Geetorus semantic operation failed")
        ...
        "session.goal.get" => json!({
            "schema":"geetorus.session_goal.snapshot.v1", "goal":null, "workingNow":false,
