...
use geetorus_runner_core::local_runner::{run_local_runner, LocalRunnerError, RunnerConfig};
use geetorus_runner_core::native_provider_backend::NativeProviderCommandExecutor;
use serde_json::json;

const RUNNERD_BUILD_METADATA_SCHEMA: &str = "geetorus-runner/runnerd-build-metadata/v1";
...
fn install_diagnostic_panic_hook(directory: Option<PathBuf>) {
    ...
    std::panic::set_hook(Box::new(move |panic| {
        let _ = persist_runner_diagnostic(&directory, &format!("geetorus-runnerd panic: {panic}"));
    }));
}
    ...
    let ticket = capture_bootstrap_ticket()
        ...
        .ok_or_else(|| {
            LocalRunnerError::invalid(
                "GEETORUS_RUNNER_BOOTSTRAP_TICKET is required for durable mode",
            )
        })?;
...
mod tests {
    ...
    fn persistent_diagnostic_is_private_bounded_and_redacted() {
        let unique = format!(
            "geetorus-runnerd-diagnostic-test-{}-{}",
            std::process::id(),
