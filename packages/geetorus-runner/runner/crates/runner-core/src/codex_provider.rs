...
const OPENCODE_PROVIDER_ENVIRONMENT_KEYS: &[&str] = &[
    ...
    "GEETORUS_RUNNER_INSTANCE_ID",
    "GEETORUS_RUN_ID",
    "GEETORUS_NORMALIZED_SESSION_ID",
    "GEETORUS_NATIVE_RUNTIME_CONTEXT_PATH",
];
const TRUSTED_OPENCODE_EXECUTABLE_ARG: &str = "--geetorus-trusted-opencode-executable";
...
const OPENCODE_RUNTIME_REQUEST_METHOD: &str = "geetorus/runtimeRequest";
...
impl ProviderTraceSink {
    fn from_environment() -> Option<Self> {
        let trace_path = std::env::var_os("GEETORUS_PROVIDER_TRACE_PATH")?;
        let max_bytes = std::env::var("GEETORUS_PROVIDER_TRACE_MAX_BYTES")
            .ok()
    ...
    fn frame(&mut self, direction: &str, raw: &[u8]) -> Option<u64> {
        ...
        self.send_record(json!({
            "kind": "frame",
            "schema": "geetorus.provider_trace_frame.v1",
            "frameId": frame_id,
            "timestamp": timestamp,
            "direction": direction,
...
const GITHUB_CREDENTIAL_ENVIRONMENT_KEYS: &[&str] = &[
    "GEETORUS_RUNNER_NETWORK_ACCESS",
    "GEETORUS_RUNNER_NETWORK_ROOTS",
    "GEETORUS_GITHUB_AUTH_MODE",
    "GEETORUS_GITHUB_HOST_HOME",
    "GEETORUS_GIT_METADATA_ROOTS",
    "GIT_SSH",
    "ZDOTDIR",
    "BASH_ENV",
    "GEETORUS_GITHUB_BROKER_URL",
    "GEETORUS_GITHUB_BROKER_TOKEN",
    "GEETORUS_GITHUB_LAUNCHER_DIR",
    "GH_CONFIG_DIR",
    "GH_ENTERPRISE_TOKEN",
...
impl CodexProvider {
    ...
    pub(crate) fn start_with_tools_observed(
        ...
        let permission_profile = codex_permission_profile(
            ...
            config.externally_sandboxed
                || std::env::var("GEETORUS_RUNNER_EXTERNAL_SANDBOX").as_deref() == Ok("1"),
        );
        ...
        let initialized_result = (|| -> Result<(), LocalRunnerError> {
            let initialized = provider.request(
                ...
                json!({
                    "clientInfo": {
                        "name": "geetorus-runnerd",
                        "title": "Geetorus Runner",
                        "version": "1",
                    },
                    "capabilities": {
            ...
            if provider.permission_profile == "geetorus-runner-external-sandbox" {
    ...
    pub(crate) fn restart_idle_identity_epoch_observed(
        ...
        if replacement.active_provider_turn_id.is_some() {
            ...
            // This process has work Geetorus never dispatched in the new
    ...
    pub fn start_turn_with_skills(
        ...
        if self.permission_profile == "geetorus-runner-external-sandbox" {
    ...
    pub fn poll(&mut self) -> Result<Option<CodexProviderEvent>, LocalRunnerError> {
            ...
            if method == "item/tool/call" {
                ...
                if !self.authorized_tool_ids.contains(&operation_id) {
                    self.send_frame(&json!({
                        "id": rpc_id,
                        "result": codex_tool_failure("Geetorus did not authorize this tool for the run"),
                    }))?;
                    return Err(LocalRunnerError::invalid(format!(
            ...
            if matches!(
                    ...
                    || retained_request_bytes.is_none()
                {
                    self.send_frame(&json!({
                        ...
                        "error": {
                            "code": -32000,
                            "message": "Geetorus rejected this runtime request because the pending input capacity was reached",
                        },
                    }))?;
                    return Ok(Some(CodexProviderEvent::Notification {
                        method: "warning".to_owned(),
        ...
        if let Some(method) = message.get("method").and_then(Value::as_str) {
                    ...
                    .is_some_and(|turn_id| self.settled_provider_turn_ids.contains(turn_id))
            {
                ...
                {
                    return Ok(Some(CodexProviderEvent::Notification {
                        method: "geetorus/resumeUsageSnapshot".to_owned(),
                        params: json!({
                            "threadId": self.thread_id,
    ...
    fn cancel_pending_requests(&mut self) -> Result<(), LocalRunnerError> {
        ...
        for request in pending.into_values() {
            if let Err(error) = self.send_frame(&json!({
                "id": request.rpc_id,
                "result": codex_tool_failure("Geetorus stopped the active provider turn"),
            })) {
                first_error.get_or_insert(error);
            }
        }
...
fn classify_notification_thread(
    ...
    {
        if method.starts_with("geetorus/") {
            return Err(LocalRunnerError::invalid(
...
fn opencode_question_set(params: &Value) -> Result<QuestionSetMapping, LocalRunnerError> {
    ...
    if request.get("schema").and_then(Value::as_str) != Some("geetorus.runtime_request.v2")
...
fn validate_opencode_question_set(question_set: &Value) -> Result<(), LocalRunnerError> {
    ...
    if !validator.is_valid(question_set) {
        return Err(LocalRunnerError::invalid(
            "OpenCode runtime request input failed the Geetorus question-set schema",
        ));
    }
...
fn codex_question_response(
    ...
    if response.get("schema").and_then(Value::as_str) != Some("geetorus.question_response.v1") {
        return Err(LocalRunnerError::invalid(
            "runtime response requires geetorus.question_response.v1",
        ));
    }
...
"#;
    ...
    fn verified_opencode_proxy_executes_the_descriptor_safe_commonjs_bundle() {
        ...
        let directory = std::env::temp_dir().join(format!(
            "geetorus-opencode-launch-{}-{nonce}",
            std::process::id()
        ));
    ...
    fn does_not_forward_an_ambient_opencode_command_override() {
        assert!(!OPENCODE_PROVIDER_ENVIRONMENT_KEYS.contains(&"GEETORUS_OPENCODE_COMMAND"));
    }
    ...
    fn codex_provider_accepts_only_the_controller_derived_external_sandbox_bit() {
        assert!(CODEX_PROVIDER_ENVIRONMENT_KEYS.contains(&"GEETORUS_RUNNER_EXTERNAL_SANDBOX"));
        assert!(!CODEX_PROVIDER_ENVIRONMENT_KEYS.contains(&"GEETORUS_SANDBOX_MODE"));
        assert_eq!(
    ...
    fn converts_codex_questions_and_responses_without_provider_leakage() {
        ...
        assert_eq!(question_set["schema"], "geetorus.question_set.v1");
        let pending = PendingRuntimeRequest {
            rpc_id: json!(41),
...
mod notification_identity_tests {
    ...
    fn classifies_provider_lineage_before_root_authority() {
        ...
        for (method, params) in [
            ("geetorus/runResult", json!({"threadId":"child"})),
