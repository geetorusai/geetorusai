...
use geetorus_runner_core::provider_bridge::{authorized_tool_catalog_digest, AuthorizedTool};
...
fn temporary_directory(label: &str) -> PathBuf {
    ...
    let directory = std::env::temp_dir().join(format!(
        "geetorus-native-provider-{label}-{}-{nonce}",
        std::process::id()
    ));
...
fn assert_valid_terminal(payload: &Value) {
    ...
    let registry = jsonschema::Registry::new()
        .add(
            "https://geetorus.dev/schemas/prp/v1/stop-reason.schema.json",
            stop_reason,
        )
        .unwrap()
        .prepare()
