...
use geetorus_runner_core::local_runner::HarnessCommand;
use geetorus_runner_core::process_supervisor::{
...
fn verified_launch_uses_open_command_and_script_after_atomic_path_replacement() {
    ...
    let original_command = "#!/bin/sh\nprintf '%s\\n' old-command\nprintf '%s\\n' \"$GEETORUS_VERIFIED_RUNTIME_EXECUTABLE\"\nexec /bin/sh \"$1\"\n";
    ...
    #[cfg(target_os = "macos")]
    {
        assert!(inherited_runtime.contains(".geetorus-verified-executable-"));
        assert_eq!(
...
fn spawn_linger_process() -> (SupervisedProcess, u32, u64) {
    ...
    process
        .send(&HarnessCommand {
            schema: "geetorus.fake_harness.command.v1".to_owned(),
