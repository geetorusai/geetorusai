...
use geetorus_runner_core::acpx_sidecar_transport::{
...
fn assigned_gateway_binding_reaches_qualified_sidecar_without_unrelated_secrets() {
    const CHILD: &str = "GEETORUS_TEST_MCP_ENV_CHILD";
    if std::env::var_os(CHILD).is_none() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env(CHILD, "1")
            .env("GEETORUS_NATIVE_MCP_NAME", "geetorus-assigned")
            .env("GEETORUS_NATIVE_MCP_URL", "http://127.0.0.1:3100/mcp")
            .env(
    ...
    for agent in ["claude", "codex"] {
        ...
        let response = sidecar
            .request(GeneratedAcpxSidecarCommand::Initialize, json!({}))
            .unwrap();
        assert_eq!(response["name"], "geetorus-assigned");
