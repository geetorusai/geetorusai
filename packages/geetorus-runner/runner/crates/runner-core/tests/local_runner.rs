...
fn run_fixture(script_name: &str, commands: &[Value]) -> (std::process::ExitStatus, Vec<Value>) {
    let runnerd = PathBuf::from(env!("CARGO_BIN_EXE_geetorus-runnerd"));
...
fn runnerd_startup_reports_build_metadata_without_panicking() {
    let output = Command::new(env!("CARGO_BIN_EXE_geetorus-runnerd"))
        .arg("--build-metadata")
        .output()
    ...
    assert_eq!(metadata["binaryName"], "geetorus-runnerd");
}
