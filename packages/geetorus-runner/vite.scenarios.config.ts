 ...
 *
 * `@geetorus-runner-local/*` is deliberately not the published SDK name: the
 * scenario runtime is package-local and stays outside the frozen 0.1.2 export
 * surface, while the explorer consumes `@geetorusai/geetorus-runner/react`
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      {
        find: "@geetorusai/geetorus-runner/react",
        replacement: resolve(packageRoot, "src/react/index.ts"),
      },
      {
        find: "@geetorusai/geetorus-runner/styles.css",
        replacement: resolve(packageRoot, "styles.css"),
      },
      {
        find: "@geetorus-runner-local/capability",
