...
function sourceLinks(run) {
  ...
  const workflow = /^https:\/\/github\.com\/geetorusai\/geetorus\/actions\/runs\/[1-9][0-9]*$/.test(url ?? "")
    ? `<small><a href="${url}">GitHub Actions ↗</a></small>` : "";
  return `${commit("geetorusai/geetorus", run.source?.geetorus?.sha, "Geetorus")}${commit("geetorusai/geetorus-evals", run.source?.evals?.sha, "Evals")}${workflow}<small>${escape(run.source?.geetorus?.ref ?? "")}</small>`;
}
