 ...
 * `GEETORUS_LIVE_CONSOLE_DRIVER=codex` swaps in the real Codex app-server driver
...
async function createWorkingDirectory() {
  const scratchRoot =
    process.env.GEETORUS_RUN_SCRATCH_DIR ?? process.env.GEETORUS_SCRATCH_DIR ?? tmpdir();
...
export function createLiveConsoleBrowserMiddleware(options = {}) {
  ...
  const driverMode = options.driverMode ?? process.env.GEETORUS_LIVE_CONSOLE_DRIVER ?? "demo";
  const chunkDelayMs = Number.parseInt(
    options.chunkDelayMs ?? process.env.GEETORUS_LIVE_CONSOLE_CHUNK_DELAY_MS ?? "45",
    10,
  );
  ...
  const maxActiveSessions = Number.parseInt(
    options.maxActiveSessions ?? process.env.GEETORUS_LIVE_CONSOLE_MAX_SESSIONS ?? "",
    10,
  );
  let bootstrap = null;
...
export function liveConsoleBrowserServerPlugin(options = {}) {
  ...
  return {
    name: "geetorus-runner-live-console-server",
