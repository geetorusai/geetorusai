...
const EVAL_PROGRAM_RELATIVE_PATH =
  "evals/geetorus-runner/tools/eval_program.py";
...
export function runnerWorkflowEvalbookAttempt({
  ...
  const infrastructure =
    disposition(result) === "infrastructure_failure"
      ...
      : undefined;
  const artifact = {
    schema: "geetorus-runner/workflow-eval-artifact/v1",
    attemptId,
  ...
  const evalCase = {
    schema: "geetorus-runner/workflow-eval-case/v1",
  ...
  const config = {
    schema: "geetorus-runner/workflow-eval-config/v1",
...
export async function resolveCanonicalEvalProgram(packageRoot, environment) {
  const fromRoot = environment.GEETORUS_EVALS_ROOT
    ? resolve(environment.GEETORUS_EVALS_ROOT, EVAL_PROGRAM_RELATIVE_PATH)
    : null;
  const program = await existingPath([
    environment.GEETORUS_EVALBOOK_PROGRAM,
    fromRoot,
    resolve(packageRoot, "../../.geetorus-evals", EVAL_PROGRAM_RELATIVE_PATH),
    resolve(
      packageRoot,
      "../../../geetorus-evals",
  ...
  throw new Error(
    `Canonical Evalbook generator not found. Set GEETORUS_EVALBOOK_PROGRAM to ${EVAL_PROGRAM_RELATIVE_PATH} in a geetorus-evals checkout.`,
  );
}
...
export async function renderRunnerWorkflowWithCanonicalEvalbook({
  ...
  const viewerRoot = resolve(
    environment.GEETORUS_EVAL_VIEWER_ROOT ??
      resolve(packageRoot, "dist-issue-thread"),
  );
  await access(resolve(viewerRoot, "index.html")).catch(() => {
    throw new Error(
      "Evalbook requires the chat viewer. Run pnpm --filter @geetorusai/geetorus-runner build:issue-thread first.",
    );
  });
  ...
  const manifest = {
    schema: "geetorus.runner.workflow-evalbook-render.v1",
    generatedAt: report.generatedAt,
    generator: {
      path: program,
