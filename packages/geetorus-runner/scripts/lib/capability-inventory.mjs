...
const legacyMcpFoldTargets = {
  geetorusMe: "eval:hb-inbox-lite-01",
  geetorusInboxLite: "eval:hb-inbox-lite-01",
  geetorusListAgents: "eval:rf-api-mgr-heartbeat-01",
  geetorusListSkills: "eval:rf-cskill-audit-01",
  geetorusGetAgent: "eval:rf-api-mgr-heartbeat-01",
  geetorusListIssues: "eval:se-q-filters-01",
  geetorusGetIssue: "eval:se-get-issue-01",
  geetorusGetHeartbeatContext: "eval:hb-context-01",
  geetorusListComments: "eval:se-get-issue-01",
  geetorusGetComment: "eval:hb-wake-comment-01",
  geetorusListIssueApprovals: "eval:ap-board-approval-01",
  geetorusListDocuments: "eval:dp-base-revision-01",
  geetorusGetDocument: "eval:dp-base-revision-01",
  geetorusListDocumentRevisions: "eval:dp-base-revision-01",
  geetorusListProjects: "eval:rf-wf-project-setup-01",
  geetorusGetProject: "eval:rf-wf-project-setup-01",
  geetorusGetIssueWorkspaceRuntime: "eval:rf-iws-start-url-01",
  geetorusControlIssueWorkspaceServices: "eval:rf-iws-start-url-01",
  geetorusWaitForIssueWorkspaceService: "eval:rf-iws-target-restart-01",
  geetorusListGoals: "eval:su-parent-goal-01",
  geetorusGetGoal: "eval:su-parent-goal-01",
  geetorusListApprovals: "eval:ap-approval-wake-01",
  geetorusCreateApproval: "eval:ap-board-approval-01",
  geetorusGetApproval: "eval:ap-approval-wake-01",
  geetorusGetApprovalIssues: "eval:ap-approval-wake-01",
  geetorusListApprovalComments: "eval:ap-approval-deny-01",
  geetorusCreateIssue: "eval:su-parent-goal-01",
  geetorusUpdateIssue: "eval:st-done-comment-01",
  geetorusCheckoutIssue: "eval:co-body-contract-01",
  geetorusReleaseIssue: "eval:er-release-01",
  geetorusAddComment: "eval:cm-multiline-01",
  geetorusSuggestTasks: "eval:ix-suggest-tasks-01",
  geetorusAskUserQuestions: "eval:ix-questions-01",
  geetorusRequestConfirmation: "eval:ix-confirmation-plan-01",
  geetorusRequestCheckboxConfirmation: "eval:ix-checkbox-01",
  geetorusUpsertIssueDocument: "eval:dp-plan-doc-01",
  geetorusRestoreIssueDocumentRevision: "eval:dp-base-revision-01",
  geetorusLinkIssueApproval: "eval:ap-board-approval-01",
  geetorusUnlinkIssueApproval: "eval:ap-board-approval-01",
  geetorusApprovalDecision: "eval:ap-approval-wake-01",
  geetorusAddApprovalComment: "eval:ap-approval-deny-01",
  geetorusApiRequest: "eval:rf-api-404-report-01",
};
...
export async function buildInventories({ repoRoot, evalRoot }) {
  ...
  const evalDirectory = resolve(evalRoot, "skills/geetorus/tests/cases");
  ...
  const evalRows = await Promise.all(caseNames.map(async (name) => parseCase(
    ...
    `geetorus-evals/geetorus-skill-optimization/skills/geetorus/tests/cases/${name}`,
  )));
  ...
  const evaluations = foldLegacyMcpAliases({
    ...
    generatedFrom: "geetorus-evals/geetorus-skill-optimization/skills/geetorus/tests/cases",
    rows: evalRows,
  }, legacyMcpAliases);
  return {
    capabilities,
...
export async function buildSkillInventory(repoRoot) {
  const skillRoot = resolve(repoRoot, "skills/geetorus");
  ...
  const rows = (await Promise.all(skillFiles.map(async (file) => parseSkillHeadings(
    ...
    `skills/geetorus/${file}`,
  )))).flat();
  return {
    schemaVersion: 2,
    inventoryRole: "normative",
    generatedFrom: skillFiles.map((file) => `skills/geetorus/${file}`),
    rows,
  };
}
