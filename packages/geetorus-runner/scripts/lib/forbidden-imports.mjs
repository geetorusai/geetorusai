...
function violationReason({ file, packageRoot, specifier }) {
  ...
  const publicRunnerImports = new Set([
    "@geetorusai/geetorus-runner/browser",
    "@geetorusai/geetorus-runner/react",
    "@geetorusai/geetorus-runner/standalone",
    "@geetorusai/geetorus-runner/testing",
    "@geetorusai/geetorus-runner/styles.css",
  ]);
  if (
    specifier.startsWith("@geetorusai/geetorus-runner/") &&
    !publicRunnerImports.has(specifier)
  ) {
    return "runner consumers may import only declared public subpaths";
  }
  if (isExampleConsumer && specifier === "@geetorusai/geetorus-runner") {
    return "runner consumers may import only declared public subpaths";
  }
  if (
    specifier.startsWith("@geetorusai/") &&
    specifier !== "@geetorusai/geetorus-runner" &&
    !publicRunnerImports.has(specifier)
  ) {
    return "Geetorus workspace packages are outside the standalone boundary";
  }
  ...
  if (["server", "ui", "cli"].some((root) => specifier === root || specifier.startsWith(`${root}/`))) {
    return "Geetorus application internals are outside the standalone boundary";
  }

  if (specifier.startsWith(".") || specifier.startsWith("/")) {
    const resolvedImport = resolve(dirname(file), specifier);
    if (!isInside(packageRoot, resolvedImport)) {
      return "relative imports may not escape packages/geetorus-runner";
    }
...
async function manifestViolations(packageRoot) {
  ...
  const unreviewedDevelopmentDependencies = Object.keys(
    ...
    (name) => name.startsWith("@geetorusai/"),
  );
  return [...runtimeDependencies, ...unreviewedDevelopmentDependencies]
    .filter(
      (name) => name.startsWith("@geetorusai/") && name !== "@geetorusai/geetorus-runner",
    )
    .map((specifier) => ({
      file: manifestPath,
      line: 1,
  ...
  for (const manifest of manifests.sort()) {
    ...
    for (let match = pathPattern.exec(source); match !== null; match = pathPattern.exec(source)) {
      ...
      violations.push({
        ...
        specifier: match[1],
        reason: "Cargo path dependencies may not escape packages/geetorus-runner",
      });
    }
  }
