...
rootless_cache_base="${GEETORUS_RUNNER_BROWSER_DEPS_DIR:-${GEETORUS_RUN_SCRATCH_DIR:-${XDG_CACHE_HOME:-${HOME:?HOME is not set}/.cache}/geetorus-runner/playwright-libs}}"
...
if [[ ! -f "$rootless_marker" ]]; then
  ...
  download_dir="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-runner-browser-deps.XXXXXX")"
