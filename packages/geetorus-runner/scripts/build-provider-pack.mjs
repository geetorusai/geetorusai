...
const temporaryParent = mkdtempSync(join(tmpdir(), "geetorus-provider-pack-"));
...
try {
  ...
  const configuredRevision =
    process.env.GEETORUS_RUNNER_SOURCE_REVISION?.trim();
  const revision =
    configuredRevision ??
  ...
  if (!/^[0-9a-f]{40}$/.test(revision)) {
    throw new Error("GEETORUS_RUNNER_SOURCE_REVISION must be a full Git SHA");
  }
  const dirty = configuredRevision
    ? false
    : spawnSync("git", ["diff", "--quiet", "--", "packages/geetorus-runner"], {
        cwd: workspaceRoot,
      }).status !== 0;
  const payload = {
    pins: {
