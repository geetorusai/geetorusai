...
async function maintainedRosterSelection(programRoot) {
  ...
  if (
    campaign.schema !== "geetorus-runner/live-campaign/v1" ||
...
export async function buildProtocolEvalCatalog({
  ...
  const programRoot = resolve(evalsRoot, "evals/geetorus-runner");
  ...
  for (const rosterFile of rosterFiles) {
    ...
    if (roster.schema !== "geetorus-runner/live-roster/v1") {
  ...
  return {
    schema: "geetorus.runner-protocol-eval.catalog/v1",
    campaignId,
    source,
...
async function syntheticAttempt({ evalsRoot, runsOut, cell, campaignId }) {
  ...
  const artifact = {
    schema: "geetorus-runner/eval-session-artifact/v1",
    attemptId,
    createdAt,
  ...
  const score = {
    schema: "geetorus-runner/eval-score/v1",
    attemptId,
    caseId: cell.caseId,
...
export async function aggregateProtocolEvalCampaign({
  ...
  if (catalog.schema !== "geetorus.runner-protocol-eval.catalog/v1") {
  ...
  const campaign = {
    schema: "geetorus.runner-protocol-eval.campaign/v1",
    campaignId: catalog.campaignId,
    generatedAt,
    selection: catalog.selection,
    costs: sumAttemptCosts(attemptUsages),
    source: {
      geetorus: source.geetorus,
      evals: source.evals,
...
async function main() {
  ...
  if (command === "catalog") {
    ...
    const catalog = await buildProtocolEvalCatalog({
      evalsRoot: resolve(argument(args, "--evals-root", ".geetorus-evals")),
      ...
      source: {
        geetorusSha: process.env.GEETORUS_PROTOCOL_EVAL_SOURCE_SHA ?? null,
        evalsSha: process.env.GEETORUS_PROTOCOL_EVALS_SHA ?? null,
      },
    });
  ...
  if (command === "aggregate") {
    const campaign = await aggregateProtocolEvalCampaign({
      ...
      source: {
        geetorus: {
          sha: process.env.GEETORUS_PROTOCOL_EVAL_SOURCE_SHA,
          ref: process.env.GEETORUS_PROTOCOL_EVAL_SOURCE_REF,
        },
        evals: {
          repository: "geetorusai/geetorus-evals",
          sha: process.env.GEETORUS_PROTOCOL_EVALS_SHA,
        },
        workflowRunUrl: process.env.GEETORUS_PROTOCOL_EVAL_WORKFLOW_URL,
      },
    });
