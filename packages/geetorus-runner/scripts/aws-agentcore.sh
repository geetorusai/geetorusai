...
TEMPLATE="$PACKAGE_DIR/infra/aws-agentcore-geetorus.yaml"
STACK_DESCRIPTION="Geetorus proof-of-concept Amazon Bedrock AgentCore Harness and least-privilege invocation roles."
LOCAL_DIR="$PACKAGE_DIR/.geetorus-local"
...
STACK_NAME="geetorus-agentcore-development"
STACK_NAME_EXPLICIT=false
ENDPOINT_NAME="geetorus"
MODEL_ID="global.anthropic.claude-sonnet-4-6"
MARKETPLACE_PRODUCT_ID="prod-ffvjxvh4ltq64"
MARKETPLACE_PRODUCT_ID_EXPLICIT=false
TRUSTED_PRINCIPAL=""
GITHUB_OIDC_PROVIDER_ARN="${GEETORUS_AWS_AGENTCORE_GITHUB_OIDC_PROVIDER_ARN:-}"
GITHUB_REPOSITORY="geetorusai/geetorus"
...
if [[ -z "$CONTEXT_PREFIX" ]]; then CONTEXT_PREFIX="geetorus/agentcore/$STACK_NAME"; fi
...
load_local_env() {
  ...
  if $STACK_NAME_EXPLICIT; then STACK_NAME="$requested_stack"; else STACK_NAME="${GEETORUS_AWS_AGENTCORE_STACK_NAME:-$requested_stack}"; fi
  export AWS_PROFILE="$AWS_PROFILE_NAME"
  export AWS_REGION="$AWS_REGION_NAME"
  export AWS_DEFAULT_REGION="$AWS_REGION_NAME"
  export GEETORUS_AWS_AGENTCORE_STACK_NAME="$STACK_NAME"
}
...
write_teardown_metadata() {
  ...
  printf '%q\n' \
    ...
    "GEETORUS_AWS_AGENTCORE_STACK_NAME=$STACK_NAME" >"$tmp"
...
assume_runner_role() {
  ...
  if ! response="$(aws_cli sts assume-role --role-arn "$role_arn" --role-session-name geetorus-agentcore-probe --duration-seconds 900 --output json 2>&1)"; then
    local caller
...
probe() {
  ...
  assume_runner_role "$GEETORUS_AWS_AGENTCORE_INVOCATION_ROLE_ARN"
  AWS_PROFILE_NAME=""
  harness_json="$(aws --region "$AWS_REGION_NAME" --no-cli-pager bedrock-agentcore-control get-harness \
    --harness-id "$GEETORUS_AWS_AGENTCORE_HARNESS_ID" \
    --harness-version "$GEETORUS_AWS_AGENTCORE_HARNESS_VERSION" --output json)"
  printf '%s' "$harness_json" | node -e '
    const fs = require("fs");
    const h = JSON.parse(fs.readFileSync(0, "utf8")).harness;
    const expectedModel = process.env.GEETORUS_AWS_AGENTCORE_MODEL;
  ...
  aws --region "$AWS_REGION_NAME" --no-cli-pager bedrock-agentcore-control get-harness-endpoint \
    --harness-id "$GEETORUS_AWS_AGENTCORE_HARNESS_ID" \
    --endpoint-name "$GEETORUS_AWS_AGENTCORE_ENDPOINT_QUALIFIER" >/dev/null
  aws --region "$AWS_REGION_NAME" --no-cli-pager bedrock-agentcore-control get-memory \
    --memory-id "$GEETORUS_AWS_AGENTCORE_MEMORY_ID" --view full >/dev/null
  [[ -n "$GEETORUS_AWS_AGENTCORE_CONTEXT_BUCKET" && -n "$GEETORUS_AWS_AGENTCORE_CONTEXT_PREFIX" && -n "$GEETORUS_AWS_AGENTCORE_CONTEXT_KMS_KEY_ARN" ]] || {
  ...
  printf 'AWS AgentCore profile qualified: %s (%s, Harness %s, Memory %s; 90-day retention).\n' \
    "$GEETORUS_AWS_AGENTCORE_PROFILE_ID" "$AWS_REGION_NAME" \
    "$GEETORUS_AWS_AGENTCORE_HARNESS_VERSION" "$GEETORUS_AWS_AGENTCORE_MEMORY_ID"
}
...
provision() {
  ...
  if existing_stack="$(aws_cli cloudformation describe-stacks --stack-name "$STACK_NAME" --output json 2>&1)"; then
    ...
    existing_owned="$(printf '%s' "$existing_stack" | stack_tag_value geetorus:owned)"
    existing_environment="$(printf '%s' "$existing_stack" | stack_tag_value geetorus:environment)"
    existing_cost_center="$(printf '%s' "$existing_stack" | stack_tag_value geetorus:cost-center)"
    if [[ -z "$existing_status" || "$existing_description" != "$STACK_DESCRIPTION" || "$existing_owned" != "true" || "$existing_environment" != "development" || "$existing_cost_center" != "runner-lab" ]]; then
      printf 'Refusing to modify stack %s: Geetorus ownership tags or template provenance do not match. Inspect or remove it manually, or choose another --stack-name.\n' "$STACK_NAME" >&2
      exit 1
    fi
  elif [[ "$existing_stack" == *"(ValidationError)"* && "$existing_stack" == *"does not exist"* ]]; then
    existing_stack=""
  else
    printf 'Unable to verify whether stack %s exists and is owned by Geetorus; refusing to provision.\n' "$STACK_NAME" >&2
    exit 1
  fi
  if [[ "$existing_status" == "ROLLBACK_COMPLETE" ]]; then
    if ! $REPLACE_FAILED_STACK; then
      printf 'Stack %s is ROLLBACK_COMPLETE. Re-run with --replace-failed-stack to explicitly authorize deleting this verified Geetorus stack.\n' "$STACK_NAME" >&2
      exit 1
    fi
    if ! $YES; then
      printf 'Delete verified Geetorus stack %s before retrying provisioning? [y/N] ' "$STACK_NAME"
      read -r answer || true
      [[ "$answer" == y || "$answer" == Y ]] || { printf 'Cancelled.\n'; return; }
    fi
    printf 'Removing verified failed Geetorus stack %s before retrying provisioning.\n' "$STACK_NAME"
  ...
  aws_cli cloudformation deploy \
    ...
    --tags geetorus:owned=true geetorus:environment=development geetorus:cost-center=runner-lab
  ...
  if aws_cli bedrock-agentcore-control get-harness-endpoint --harness-id "$harness_id" --endpoint-name "$ENDPOINT_NAME" >/dev/null 2>&1; then
    aws_cli bedrock-agentcore-control update-harness-endpoint --harness-id "$harness_id" --endpoint-name "$ENDPOINT_NAME" \
      --target-version "$harness_version" --description "Geetorus pinned Harness endpoint" \
      --client-token "geetorus-update-$harness_id-$harness_version" >/dev/null
  else
    aws_cli bedrock-agentcore-control create-harness-endpoint --harness-id "$harness_id" --endpoint-name "$ENDPOINT_NAME" \
      --target-version "$harness_version" --description "Geetorus pinned Harness endpoint" \
      --client-token "geetorus-create-$harness_id-$harness_version" \
      --tags geetorus:owned=true,geetorus:environment=development >/dev/null
  fi
  ...
  printf '%q\n' \
    ...
    "AWS_REGION=$AWS_REGION_NAME" \
    "AWS_DEFAULT_REGION=$AWS_REGION_NAME" \
    "GEETORUS_AWS_AGENTCORE_STACK_NAME=$STACK_NAME" \
    "GEETORUS_AWS_AGENTCORE_PROFILE_ID=$STACK_NAME" \
    "GEETORUS_AWS_AGENTCORE_ACCOUNT_ID=$account_id" \
    "GEETORUS_AWS_AGENTCORE_HARNESS_ARN=$harness_arn" \
    "GEETORUS_AWS_AGENTCORE_HARNESS_ID=$harness_id" \
    "GEETORUS_AWS_AGENTCORE_HARNESS_VERSION=$harness_version" \
    "GEETORUS_AWS_AGENTCORE_ENDPOINT_ARN=$endpoint_arn" \
    "GEETORUS_AWS_AGENTCORE_ENDPOINT_QUALIFIER=$ENDPOINT_NAME" \
    "GEETORUS_AWS_AGENTCORE_RUNTIME_ARN=$runtime_arn" \
    "GEETORUS_AWS_AGENTCORE_MEMORY_ARN=$memory_arn" \
    "GEETORUS_AWS_AGENTCORE_MEMORY_ID=$memory_id" \
    "GEETORUS_AWS_AGENTCORE_INVOCATION_ROLE_ARN=$role_arn" \
    "GEETORUS_AWS_AGENTCORE_EXECUTION_ROLE_ARN=$role_arn" \
    "GEETORUS_AWS_AGENTCORE_CONTEXT_BUCKET=$context_bucket" \
    "GEETORUS_AWS_AGENTCORE_CONTEXT_PREFIX=$context_prefix" \
    "GEETORUS_AWS_AGENTCORE_CONTEXT_KMS_KEY_ARN=$context_kms_key_arn" \
    "GEETORUS_AWS_AGENTCORE_MODEL=$MODEL_ID" \
    "GEETORUS_AWS_AGENTCORE_QUALIFICATION_REVISION=$qualification_revision" \
    "GEETORUS_AWS_AGENTCORE_EVENT_EXPIRY_DAYS=90" >"$tmp"
  if [[ -n "${AWS_CONFIG_FILE:-}" ]]; then
    printf 'AWS_CONFIG_FILE=%q\n' "$AWS_CONFIG_FILE" >>"$tmp"
  fi
  chmod 600 "$tmp"
...
destroy() {
  ...
  aws_cli bedrock-agentcore-control delete-harness-endpoint --harness-id "$harness_id" --endpoint-name "$ENDPOINT_NAME" \
    --client-token "geetorus-delete-$harness_id-$ENDPOINT_NAME" >/dev/null 2>&1 || true
