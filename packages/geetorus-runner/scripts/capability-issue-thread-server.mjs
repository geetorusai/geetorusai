...
const CAPABILITY_COOKIES = Object.freeze({
  issue: "geetorus_capability_issue",
  cleanroom: "geetorus_capability_chat",
});
...
function scratchRoot() {
  return process.env.GEETORUS_RUN_SCRATCH_DIR ?? process.env.GEETORUS_SCRATCH_DIR ?? tmpdir();
}

async function createWorkingDirectory(root, prefix = "capability-issue-thread-") {
  // Managed previews can outlive the heartbeat that launched them. Geetorus
...
function issueThreadSeed(runner, scenario) {
  return runner.createCapabilityFixtureState({
    ...
    company: { id: "company-1", name: "Mock Geetorus Company", issuePrefix: "MCK" },
    actors: [
      {
        id: "actor-1",
        companyId: "company-1",
...
function harnessConfiguration(source, fallbackModel) {
  ...
  const configuredManagedProfileId = process.env.GEETORUS_CLAUDE_MANAGED_PROFILE_ID?.trim();
  ...
  const configuredAgentCoreProfileId = process.env.GEETORUS_AWS_AGENTCORE_PROFILE_ID?.trim();
...
function resolveManagedProfile(configuration) {
  const profileId = process.env.GEETORUS_CLAUDE_MANAGED_PROFILE_ID?.trim() || configuration.managedProfileId;
...
function resolveAgentCoreProfile(configuration) {
  ...
  const profileId = required("GEETORUS_AWS_AGENTCORE_PROFILE_ID");
  ...
  return {
    ...
    accountId: required("GEETORUS_AWS_AGENTCORE_ACCOUNT_ID"),
    harnessArn: required("GEETORUS_AWS_AGENTCORE_HARNESS_ARN"),
    harnessVersion: required("GEETORUS_AWS_AGENTCORE_HARNESS_VERSION"),
    endpointArn: required("GEETORUS_AWS_AGENTCORE_ENDPOINT_ARN"),
    endpointQualifier: required("GEETORUS_AWS_AGENTCORE_ENDPOINT_QUALIFIER"),
    agentRuntimeArn: required("GEETORUS_AWS_AGENTCORE_RUNTIME_ARN"),
    memoryArn: required("GEETORUS_AWS_AGENTCORE_MEMORY_ARN"),
    memoryId: required("GEETORUS_AWS_AGENTCORE_MEMORY_ID"),
    invocationRoleArn: required("GEETORUS_AWS_AGENTCORE_INVOCATION_ROLE_ARN"),
    contextBucket: required("GEETORUS_AWS_AGENTCORE_CONTEXT_BUCKET"),
    contextPrefix: required("GEETORUS_AWS_AGENTCORE_CONTEXT_PREFIX"),
    contextKmsKeyArn: required("GEETORUS_AWS_AGENTCORE_CONTEXT_KMS_KEY_ARN"),
    qualificationRevision: required("GEETORUS_AWS_AGENTCORE_QUALIFICATION_REVISION"),
    eventExpiryDays: 90,
...
export function capabilityIssueThreadServerPlugin(options = {}) {
  ...
  return {
    name: "geetorus-runner-capability-issue-thread-server",
