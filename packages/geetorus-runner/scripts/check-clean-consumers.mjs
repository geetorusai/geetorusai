...
const scratchParent = process.env.GEETORUS_RUN_SCRATCH_DIR
  ?? process.env.GEETORUS_SCRATCH_DIR
  ?? tmpdir();
await mkdir(scratchParent, { recursive: true });
const scratchRoot = await mkdtemp(join(scratchParent, "geetorus-package-consumer-"));
const artifactsRoot = resolve(scratchRoot, "artifacts");
const publicationRoot = process.env.GEETORUS_CLEAN_CONSUMER_OUTPUT_DIR === undefined
  ? undefined
  : resolve(process.env.GEETORUS_CLEAN_CONSUMER_OUTPUT_DIR);
...
try {
  ...
  const conformanceRecord = resolve(artifactsRoot, "geetorus-runner-consumer-conformance.json");
  const sourceCommit = (
    process.env.GEETORUS_SOURCE_COMMIT
    ?? capture("git", ["rev-parse", "HEAD"], runnerRoot)
  ).trim();
  if (!/^[a-f0-9]{40}$/.test(sourceCommit)) {
    throw new Error("GEETORUS_SOURCE_COMMIT must be a full lowercase Git commit SHA");
  }
  ...
  if (process.env.GEETORUS_KEEP_PACKAGE_CONSUMERS !== "1") {
...
async function stageRunnerdArtifact(destination) {
  ...
  const source = resolve(runnerRoot, `runner/target/release/geetorus-runnerd${suffix}`);
  const executablePath = resolve(destination, `geetorus-runnerd-${process.platform}-${process.arch}${suffix}`);
...
async function verifyRunnerConsumer({
  ...
  await writeFile(resolve(consumerRoot, "package.json"), `${JSON.stringify({
    name: "geetorus-runner-clean-consumer",
    private: true,
    type: "module",
    packageManager: "pnpm@9.15.4",
    dependencies: {
      "@geetorusai/geetorus-runner": `file:${runnerTarball}`,
    },
...
import * as runtime from "@geetorusai/geetorus-runner";
import * as evals from "@geetorusai/geetorus-runner/evals";
import * as testing from "@geetorusai/geetorus-runner/testing";
...
const nativeBundle = await evals.loadGeetorusNativeExecutionFixture();
if (nativeBundle.schema !== "geetorus-runner/native-execution/v1") {
...
runtime.assertGeetorusRunnerCompatibility({
  consumer: "geetorus-runner-clean-consumer",
...
const runnerd = await evals.resolveGeetorusRunnerdArtifact({
  executablePath: process.env.GEETORUS_RUNNERD_ARTIFACT,
  expectedSha256: process.env.GEETORUS_RUNNERD_SHA256,
});
const evalCompatibility = evals.assertGeetorusRunnerEvalCompatibility({
  consumer: "geetorus-runner-clean-consumer",
  packageVersion: evals.GEETORUS_RUNNER_BUILD_METADATA.package.version,
  runnerd: runnerd.buildMetadata,
  nativeExecutionVersion: 1,
  prp: { minimumVersion: 1, maximumVersion: 1 },
  catalog: evals.GEETORUS_RUNNER_BUILD_METADATA.semanticCatalog,
  driver: {
...
try {
  evals.assertGeetorusRunnerEvalCompatibility({
  ...
  mismatchFailedClosed = error?.code === "geetorus_runner_eval_incompatible"
...
const packageArtifactPath = process.env.GEETORUS_RUNNER_PACKAGE_ARTIFACT;
const runnerdArtifactPath = process.env.GEETORUS_RUNNERD_ARTIFACT;
...
if (runnerdDigest !== process.env.GEETORUS_RUNNERD_SHA256) {
  throw new Error("runnerd artifact digest changed after staging");
}

await writeFile(process.env.GEETORUS_CONFORMANCE_RECORD, JSON.stringify({
  schema: "geetorus-runner/clean-consumer-conformance/v1",
  recordedAt: new Date().toISOString(),
  sourceCommit: process.env.GEETORUS_SOURCE_COMMIT,
  ...
  artifacts: {
    ...
    runnerd: {
      ...
      buildProfile: process.env.GEETORUS_RUNNERD_BUILD_PROFILE,
    },
  },
  consumer: {
    installMode: "offline-packed-artifact",
    imports: [
      "@geetorusai/geetorus-runner",
      "@geetorusai/geetorus-runner/evals",
      "@geetorusai/geetorus-runner/testing",
    ],
...
`);
  installAndRun(consumerRoot, {
    GEETORUS_RUNNERD_ARTIFACT: runnerdArtifact.executablePath,
    GEETORUS_RUNNERD_SHA256: runnerdArtifact.sha256,
    GEETORUS_RUNNERD_BUILD_PROFILE: runnerdArtifact.buildProfile,
    GEETORUS_RUNNER_PACKAGE_ARTIFACT: runnerTarball,
    GEETORUS_CONFORMANCE_RECORD: conformanceRecord,
    GEETORUS_SOURCE_COMMIT: sourceCommit,
  });
}
