...
export async function validatePublicProtocolEvalReport(
  ...
  if (
    campaign.schema !== "geetorus.runner-protocol-eval.campaign/v1" ||
...
export async function createProtocolEvalBundleManifest(
  ...
  return {
    schema: "geetorus.runner-protocol-eval.bundle/v1",
    campaignId,
...
export function emptyProtocolEvalHistory() {
  return {
    schema: "geetorus.runner-protocol-eval.history/v1",
...
export function mergeProtocolEvalHistory(history, record) {
  if (history.schema !== "geetorus.runner-protocol-eval.history/v1") {
...
export function buildProtocolEvalPointers(history) {
  ...
  const project = (id) => {
    ...
    return campaign
      ? {
          ...
          geetorusSha: campaign.source?.geetorus?.sha ?? null,
  ...
  return {
    latest: {
      schema: "geetorus.runner-protocol-eval.pointer/v1",
...
async function main() {
  const result = await publishProtocolEvalHistory({
    viewerRoot: process.env.GEETORUS_RUNNER_PROTOCOL_EVAL_VIEWER_DIR,
    reportRoot: resolve(
      process.env.GEETORUS_RUNNER_PROTOCOL_EVAL_PUBLIC_REPORT_DIR ??
