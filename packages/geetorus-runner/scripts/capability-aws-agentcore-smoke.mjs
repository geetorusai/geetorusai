...
const envPath = new URL("../.geetorus-local/aws-agentcore.env", import.meta.url);
...
async function main() {
  ...
  try {
    const configuration = {
      ...
      model: process.env.GEETORUS_AWS_AGENTCORE_MODEL,
      agentCoreProfileId: process.env.GEETORUS_AWS_AGENTCORE_PROFILE_ID,
    ...
    assert(read.toolResult && typeof read.toolResult === "object", "the mock Geetorus read operation succeeded");
    ...
    process.stdout.write(`${JSON.stringify({
      schema: "geetorus.capability.aws-agentcore-smoke.v1",
      sessionId: opened.sessionId,
