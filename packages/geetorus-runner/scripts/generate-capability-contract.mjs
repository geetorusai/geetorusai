...
function parseCase(entry, group, contract) {
  ...
  return {
    ...
    title,
    group,
    sourceAnchor: `geetorus-evals/geetorus-skill-optimization/${group}.yaml#${id}`,
    primaryDisposition,
...
function renderHandoff() {
  return [
    ...
    "## Stable Inputs",
    "",
    "- `capabilities.yaml`: every current Geetorus skill and reference heading, including its source anchor, disposition, semantic operation, and mock-state expectation.",
    ...
    "",
    "`pnpm --dir packages/geetorus-runner check:capability-contract` is the drift gate before consuming these artifacts.",
  ].join("\n") + "\n";
}
