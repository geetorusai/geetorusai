...
async function fixture() {
  ...
  roots.push(root);
  const program = join(root, "evals/geetorus-runner");
  await Promise.all([
  ...
  const config = {
    schema: "geetorus-runner/eval-config/v1",
  ...
  const evalCase = {
    schema: "geetorus-runner/eval-case/v1",
    id: "get-task-context",
    title: "Get context",
  ...
  const roster = {
    schema: "geetorus-runner/live-roster/v1",
  ...
  await Promise.all([
    ...
    writeFile(
      ...
      JSON.stringify({
        schema: "geetorus-runner/live-campaign/v1",
        lanes: [
          {
            id: "opencode-model",
...
test("aggregates retained attempts and synthesizes missing cells as infrastructure", async () => {
  ...
  const campaign = await aggregateProtocolEvalCampaign({
    ...
    source: {
      geetorus: { sha: "a".repeat(40), ref: "refs/heads/master" },
      evals: { repository: "geetorusai/geetorus-evals", sha: "b".repeat(40) },
