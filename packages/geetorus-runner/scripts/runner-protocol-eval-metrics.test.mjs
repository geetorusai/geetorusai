...
function campaign(id, failed = [], overrides = {}) {
  ...
  return {
    ...
    source: { geetorus: { sha: "a".repeat(40), ref: "refs/heads/master" }, evals: { sha: "b".repeat(40) }, workflowRunUrl: `https://github.com/geetorusai/geetorus/actions/runs/${id}` },
...
test("history graphs exclude refreshes, link exact SHAs and Actions, and open the latest presentation", async () => {
  ...
  assert.ok(html.includes(`https://github.com/geetorusai/geetorus/commit/${"a".repeat(40)}`));
  assert.ok(html.includes(`https://github.com/geetorusai/geetorus-evals/commit/${"b".repeat(40)}`));
