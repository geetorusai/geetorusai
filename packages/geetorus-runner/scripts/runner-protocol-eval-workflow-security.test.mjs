...
test("pull request CI builds the canonical Evalbook viewer", async () => {
  ...
  assert.match(
    buildJob,
    /name: Build Runner Evalbook viewer[\s\S]*pnpm --filter @geetorusai\/geetorus-runner build:issue-thread/u,
  );
});
...
test("resolves both repositories immutably and bounds total matrix concurrency", async () => {
  ...
  assert.match(
    authorize,
    /repos\/geetorusai\/geetorus-evals\/commits\/\$EVALS_SHA/u,
  );
  assert.match(authorize, /COMMITPERCLIP_KEY/u);
  assert.match(authorize, /GH_REPO: geetorusai\/geetorus-evals/u);
  assert.match(
    authorize,
  ...
  const privateCheckouts = [
    ...workflow.matchAll(
      /repository: geetorusai\/geetorus-evals[\s\S]*?persist-credentials: false/gmu,
    ),
  ];
  ...
  for (const tokenStep of privateTokenSteps) {
    assert.match(
      ...
      /^ {10}GH_REPO: geetorusai\/geetorus-evals$/mu,
  ...
  assert.match(
    workflow,
    /pnpm --filter @geetorusai\/geetorus-runner deploy --prod/u,
  );
  assert.match(
    workflow,
...
test("publishes only the separately sanitized Evalbook through trusted OIDC code", async () => {
  ...
  assert.match(publisher, /GEETORUS_RUNNER_PROTOCOL_EVAL_VIEWER_DIR/u);
  ...
  assert.doesNotMatch(publisher, /geetorusai\/geetorus-evals/u);
