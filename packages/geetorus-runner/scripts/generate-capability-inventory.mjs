...
const evalCandidates = [
  resolve(repoRoot, "../geetorus-evals/geetorus-skill-optimization"),
  resolve(repoRoot, "../../../../geetorus-evals/geetorus-skill-optimization"),
];
const evalRoot = process.env.GEETORUS_EVALS_ROOT
  ?? (await (async () => {
    for (const candidate of evalCandidates) {
      if (await access(candidate).then(() => true, () => false)) return candidate;
    }
    throw new Error(`Geetorus eval corpus not found. Set GEETORUS_EVALS_ROOT. Checked:\n${evalCandidates.join("\n")}`);
  })());
const outputPaths = {
