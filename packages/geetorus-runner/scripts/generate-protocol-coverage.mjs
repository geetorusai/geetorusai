...
import { GEETORUS_PROTOCOL_ACTIONS } from "../dist/protocol-actions/index.js";
...
const catalog = GEETORUS_PROTOCOL_ACTIONS.map((action) => ({
  id: action.id,
...
const lifecycleRequirements = [
  ...
  ["single-daemon", "one Geetorus-owned sandbox daemon and no TypeScript sandbox dispatcher", "src/live/runnerd-codex-transport.test.ts"],
  ...
  ["geetorus-adapter-selection", "geetorus_runner is selectable without changing legacy adapter behavior", "server/src/services/native-runtime/runtime-mode.test.ts; server/src/__tests__/heartbeat-native-runner-selection.test.ts"],
  ["real-control-plane-binding", "advertised runner tools re-authorize the live company, issue, agent, and run before using real Geetorus services", "server/src/services/native-runtime/geetorus-runner-tool-authority.test.ts; server/src/services/native-runtime/geetorus-runner-real-server.integration.test.ts"],
  ["shared-geetorus-prp-route", "Rust runnerd authenticates to the shared Geetorus server route and completes a real bound semantic tool call", "server/src/realtime/runner-prp-ws.test.ts; server/src/services/native-runtime/geetorus-runner-real-server.integration.test.ts"],
  ["real-control-plane-idempotency", "real service mutations are replay-safe and reject conflicting idempotency-key reuse", "server/src/services/native-runtime/geetorus-runner-tool-authority.test.ts"],
  ["geetorus-thread-projection", "committed PRP reasoning and semantic tool items render in the existing Geetorus task chat", "ui/src/adapters/geetorus-runner/index.test.ts; server/src/services/native-runtime/geetorus-control-plane-port.test.ts"],
];
...
const document = {
  schema: "geetorus-runner/protocol-coverage/v1",
  generatedFrom: [
