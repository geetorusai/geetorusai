...
const smokeRoot = await mkdtemp(
  join(tmpdir(), "geetorus-local-provider-smoke-"),
);
if (!basename(smokeRoot).startsWith("geetorus-local-provider-smoke-")) {
  throw new Error("Refusing an unrecognized local provider smoke root");
}
await Promise.all(
  ["tmp", "home", "geetorus-home", "codex-home", "claude-home"].map(
...
Object.assign(process.env, {
  ...
  GEETORUS_HOME: join(smokeRoot, "geetorus-home"),
...
try {
  ...
  // module can snapshot the developer's normal Geetorus or provider state.
  const {
      ...
      const observation = await withIsolatedProfileCredentials({
        ...
        run: () =>
          executeLiveRunnerWorkflow({
            ...
            expectedAssistantText: "GEETORUS_LOCAL_PROVIDER_SMOKE_OK",
            promptOverride:
              "Reply with exactly GEETORUS_LOCAL_PROVIDER_SMOKE_OK and no other text. Do not call tools.",
