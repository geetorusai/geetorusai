...
const now = process.env.GEETORUS_EVAL_GENERATED_AT ?? new Date().toISOString();
const rotationDay = Number(
  process.env.GEETORUS_EVAL_ROTATION_DAY ?? evals.runnerLiveRotationWeek(now),
);
const seed =
  process.env.GEETORUS_EVAL_SCHEDULE_SEED ?? "runner-live-seven-week-v1";
const outputDirectory = resolve(
  packageRoot,
  ".geetorus-local/evals/workflows",
);
const historyDirectory = resolve(
  process.env.GEETORUS_EVAL_HISTORY_DIR ?? resolve(outputDirectory, "history"),
);
...
function selectionLimit() {
  ...
  const raw =
    index < 0 ? process.env.GEETORUS_EVAL_LIMIT : process.argv[index + 1];
...
const selection = {
  candidateIds: selectorValues("--candidate", "GEETORUS_EVAL_CANDIDATE"),
  caseIds: selectorValues("--case", "GEETORUS_EVAL_CASE"),
...
function safeBundleId(schedule) {
  const runnerBuild =
    process.env.GEETORUS_EVAL_RUNNER_BUILD ?? packageManifest.version;
...
if (mode === "nightly") {
  ...
  const campaignCostLimit = evals.parseRunnerLiveCampaignCostLimit(
    process.env.GEETORUS_EVAL_MAX_CAMPAIGN_COST_USD,
  );
  ...
  const alerts = evals.runnerWorkflowAlerts({
    ...
    history,
    baselineReady:
      process.env.GEETORUS_EVAL_BASELINE_READY === "true" &&
      history.length >= 7,
  });
  await Promise.all([
    writeFile(
  ...
  const payload = {
    schema: "geetorus.runner.chaos-eval-schedule.v1",
    generatedAt: now,
    seed,
