...
const templateUrl = new URL("../infra/aws-agentcore-geetorus.yaml", import.meta.url);
...
test("AgentCore wrapper is valid shell and writes only nonsecret profile metadata", async () => {
  ...
  assert.match(teardownBlock, /GEETORUS_AWS_AGENTCORE_STACK_NAME=\$STACK_NAME/);
  ...
  assert.match(provisionBlock, /existing_owned.*geetorus:owned/s);
  assert.match(provisionBlock, /existing_environment.*geetorus:environment/s);
  assert.match(provisionBlock, /existing_cost_center.*geetorus:cost-center/s);
  ...
  assert.match(provisionBlock, /Unable to verify whether stack .* exists and is owned by Geetorus/);
  ...
  assert.match(generatedBlock, /GEETORUS_AWS_AGENTCORE_CONTEXT_BUCKET=\$context_bucket/);
  assert.match(generatedBlock, /GEETORUS_AWS_AGENTCORE_CONTEXT_PREFIX=\$context_prefix/);
  assert.match(generatedBlock, /GEETORUS_AWS_AGENTCORE_CONTEXT_KMS_KEY_ARN=\$context_kms_key_arn/);
  assert.match(generatedBlock, /GEETORUS_AWS_AGENTCORE_EXECUTION_ROLE_ARN=\$role_arn/);
...
test("AgentCore wrapper never implicitly deletes a colliding failed stack", async () => {
  const temp = await mkdtemp(join(tmpdir(), "geetorus-agentcore-test-"));
...
if [[ " $* " == *" --version "* || "$1" == "--version" ]]; then
  ...
  printf '{"Account":"123456789012","Arn":"arn:aws:iam::123456789012:role/geetorus-test"}\\n'
elif [[ " $* " == *" cloudformation describe-stacks "* ]]; then
  printf '{"Stacks":[{"StackStatus":"ROLLBACK_COMPLETE","Description":"%s","Tags":[{"Key":"geetorus:owned","Value":"%s"},{"Key":"geetorus:environment","Value":"development"},{"Key":"geetorus:cost-center","Value":"runner-lab"}]}]}\\n' "\${MOCK_STACK_DESCRIPTION}" "\${MOCK_STACK_OWNED}"
...
`, { mode: 0o700 });
  ...
  const baseEnv = {
    ...
    MOCK_STACK_DESCRIPTION: "Geetorus proof-of-concept Amazon Bedrock AgentCore Harness and least-privilege invocation roles.",
...
test("Runner Lab accepts and resolves the complete qualified AgentCore profile", async () => {
  ...
  for (const field of [
    "GEETORUS_AWS_AGENTCORE_CONTEXT_BUCKET",
    "GEETORUS_AWS_AGENTCORE_CONTEXT_PREFIX",
    "GEETORUS_AWS_AGENTCORE_CONTEXT_KMS_KEY_ARN",
...
test("Runner Lab qualifies Claude Managed and exposes remote governance for both remote providers", async () => {
  ...
  for (const field of [
    "GEETORUS_CLAUDE_MANAGED_PROFILE_ID",
    "ANTHROPIC_API_KEY",
