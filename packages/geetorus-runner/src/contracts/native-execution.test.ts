...
describe("NativeExecutionInputV1", () => {
  it("parses v3 immutable runtime context without changing the model task envelope", () => {
    ...
    const parsed = parseNativeExecutionInput({
      ...input,
      schema: "geetorus.native-execution-input.v3",
      executionMode: "default",
      planningContext: null,
      runtimeContext: { ...context, aggregateDigest: canonicalNativeRuntimeContextDigest(context) },
    });
    expect(parsed.schema).toBe("geetorus.native-execution-input.v3");
    ...
    expect(JSON.stringify(envelope)).not.toContain(GEETORUS_EXECUTION_PROMPT);
  ...
  it("rejects runtime-context traversal and aggregate digest drift", () => {
    ...
    expect(() => parseNativeExecutionInput({ ...input, schema: "geetorus.native-execution-input.v3", executionMode: "default", planningContext: null, runtimeContext: context })).toThrow("bundle root");
    expect(() => parseNativeExecutionInput({
      ...input,
      schema: "geetorus.native-execution-input.v3",
      executionMode: "default",
  ...
  it("rejects unknown context or environment escape hatches", () => {
    ...
    expect(() => parseNativeExecutionInput({
      ...input,
      workspace: { ...input.workspace, env: { GEETORUS_API_KEY: "canary" } },
    ...
    const awsAgentCore = {
      ...
      provider: {
        ...
        agentCoreProfile: {
          ...
          harnessVersion: "3",
          endpointArn: "arn:aws:bedrock-agentcore:us-east-1:123456789012:harness-endpoint/harness-1/geetorus",
          endpointQualifier: "geetorus",
          ...
          memoryId: "memory-1",
          invocationRoleArn: "arn:aws:iam::123456789012:role/geetorus-agentcore-runner",
          contextBucket: "geetorus-agentcore-context",
          contextPrefix: "geetorus/runtime",
...
describe("NativeExecutionInputV2 planning", () => {
  ...
  it("round-trips native plan mode and its pinned canonical revision into model context", () => {
    ...
    expect(parsed).toMatchObject({ schema: "geetorus.native-execution-input.v2", executionMode: "plan" });
    expect(buildNativeModelEnvelope(parsed)).toMatchObject({
      schema: "geetorus.native-model-envelope.v2",
      executionMode: "plan",
      planningContext: { baseRevisionId: "revision-3", baseRevisionNumber: 3 },
    });
  });

  it("fails closed when plan mode omits its pinned Geetorus context", () => {
