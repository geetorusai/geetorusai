...
export const NATIVE_RUNTIME_ASSET_SCHEMA = "geetorus.runtime-asset.v1" as const;
export const GEETORUS_EXECUTION_PROMPT_REVISION = "geetorus-execution.v3" as const;
export const GEETORUS_EXECUTION_PROMPT = "You are running as a Geetorus agent. Complete the assigned task in the provided execution environment. Follow the attached agent instructions and use assigned skills and tools when relevant. Use Geetorus tools for coordination. Hire persistent teammates through Geetorus hiring; provider helper threads do not create Geetorus agents. Delegate with create_task. When remaining work depends on a child task, use set_dependencies to add its ID while preserving existing blocker IDs. Complete independent work, then call geetorus_block with the child agent as owner and child completion as the unblock action. End the turn so the child can use the workspace. Do not sleep or poll for child results while holding the workspace. Geetorus resumes the parent when the dependency completes. When a task needs an external service, use installed tools if available; otherwise use connections_search to discover catalog services or authorized configured connections, then connection_request with the returned service identifier. The request appears as a card in the task. Finish independent work before yielding for access; do not poll or request the same connection repeatedly. Geetorus will continue automatically with updated tools after resolution. After a decline, pursue alternatives unless the user explicitly asks to retry. Finish exactly once with `geetorus_finish` or `geetorus_block`." as const;
...
export interface NativeRuntimeContextSnapshot {
  prompt: { revision: typeof GEETORUS_EXECUTION_PROMPT_REVISION; text: typeof GEETORUS_EXECUTION_PROMPT; digest: string };
...
export function nativeRuntimePromptDigest(): string { return sha256(GEETORUS_EXECUTION_PROMPT); }
...
export function parseNativeRuntimeContext(value: unknown): NativeRuntimeContextSnapshot {
  ...
  if (prompt.revision !== GEETORUS_EXECUTION_PROMPT_REVISION || prompt.text !== GEETORUS_EXECUTION_PROMPT) {
    throw new NativeRuntimeContextError("input.runtimeContext.prompt must match the fixed Geetorus prompt revision");
  }
  ...
  const parsed = {
    prompt: { revision: GEETORUS_EXECUTION_PROMPT_REVISION, text: GEETORUS_EXECUTION_PROMPT, digest: nativeRuntimePromptDigest() },
