...
interface QuestionAdapterFixture {
  schema: "geetorus.question_adapter_fixture.v1";
  adapter: "codex" | "acpx";
...
describe("question adapter conformance fixtures", () => {
  it("normalizes equivalent Codex and ACPX forms to one canonical question set", async () => {
    ...
    const codexExpected = parseGeetorusQuestionSet(codex.canonicalQuestionSet);
    const acpxExpected = parseGeetorusQuestionSet(acpx.canonicalQuestionSet);
    ...
    expect(
      parseGeetorusQuestionResponse(codexExpected, codex.canonicalResponse),
    ).toEqual(codex.canonicalResponse);
    expect(
      parseGeetorusQuestionResponse(acpxExpected, acpx.canonicalResponse),
  ...
  it("converts one canonical response back to each provider shape", async () => {
    ...
    const codexResponse = parseGeetorusQuestionResponse(
      codexQuestionSet,
    ...
    const acpxResponse = parseGeetorusQuestionResponse(
...
function questionPresentation(questionSet: GeetorusQuestionSet): unknown {
  return {
    ...questionSet,
