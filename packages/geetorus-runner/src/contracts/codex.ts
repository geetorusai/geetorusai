...
export const CODEX_TASK_ENVELOPE_SCHEMA = "geetorus.skillless_task.v1" as const;
...
export const CODEX_SKILLLESS_BASE_INSTRUCTIONS =
  "Complete only the supplied task envelope. Do not discover or invoke skills. Do not call a control-plane API. Return exactly one semantic completion result; use geetorus_finish when the work is done, needs review, or is explicitly yielding for the next response, and geetorus_block only when work cannot continue." as const;
...
export interface CodexRunMetadata {
  schema: "geetorus.runner.codex.metadata.v1";
  fixtureName: string;
  identity: {
...
export interface CodexRunTrace {
  schema: "geetorus.runner.codex.trace.v1";
...
export function isSkilllessCodexContext(
  ...
  return (
    ...
    !serialized.includes("geetorus_api_key") &&
