...
import { GEETORUS_RUNNER_BUILD_METADATA } from "../evals/build-metadata.js";
...
const EVAL_RUNTIME_INSTRUCTIONS = [
  "# Geetorus direct live evaluation",
  "",
  "Use the provided Geetorus semantic tools to inspect and act on the assigned task.",
  ...
  "Task-state changes in this mock control plane use finish_task and block_task. Native geetorus_finish and geetorus_block report the provider run result but do not update the mock task. When asked to finish or block the assigned task, use its task-state semantic operation before reporting the run result.",
...
export async function prepareEvalRuntimeContext(
  ...
  const contextRoot = await mkdtemp(
    resolve(workingDirectory, ".geetorus-eval-runtime-context-"),
  );
  ...
  const manifestText = `${JSON.stringify({
    schema: "geetorus.runtime-asset-manifest.v1",
    digest: assetDigest,
  ...
  const semanticCatalogDigest =
    GEETORUS_RUNNER_BUILD_METADATA.semanticCatalog.sha256.replace(
      /^sha256:/,
      "",
    );
  const context = {
    ...
    skills: [],
    mcp: {
      assignmentSetId: "geetorus-runner-direct-eval-v1",
...
export async function runEvalSessionCli(
  ...
  try {
    ...
    await writeFile(cli.outputPath, `${JSON.stringify({
      ...
      attemptId: request.attemptId,
      build: GEETORUS_RUNNER_BUILD_METADATA,
