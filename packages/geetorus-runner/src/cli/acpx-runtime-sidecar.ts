...
function reportRetainedAcpxCleanupFailure(
  ...
  process.emitWarning(
    JSON.stringify({
      schema: "geetorus.runner.retained_cleanup_failure.v1",
      resource: input.resource,
      attempt: input.attempt,
      errorName,
    }),
    {
      code: "GEETORUS_ACPX_RETAINED_CLEANUP_FAILURE",
      type: "GeetorusRunnerCleanupWarning",
    },
  );
}
...
async function dispatch(
  ...
  if (request.command === "initialize") {
    ...
    return {
      ...
      capabilities: {
        ...
        structuredInput: "geetorus.question_set.v1",
      },
    };
  }
  ...
  if (request.command === "tool.resolve") {
    ...
    if (request.params.error) {
      pending.reject(
        new Error(
          safeText(
            text(record(request.params.error).message, "Geetorus tool failed"),
          ),
        ),
      );
    } else {
  ...
  if (request.command === "session.suspend") {
    ...
    await closeSidecarHostForCommand(
      activeHost,
      boundedOptionalText(request.params.reason, "Geetorus suspension", 4_000),
      undefined,
  ...
  if (request.command === "session.close") {
    ...
    if (host) {
      ...
      await closeSidecarHostForCommand(
        activeHost,
        boundedOptionalText(request.params.reason, "Geetorus close", 4_000),
        undefined,
        (cleanup) => retainActiveHostCleanup(activeHost, cleanup),
      );
    }
...
type BoundedRuntimeToolEvent = AcpRuntimeEvent & {
  geetorusBoundedTool: true;
  geetorusOutput: Record<string, unknown>;
};
...
function boundRuntimeEventForNormalization(
  ...
  return {
    ...
    geetorusBoundedTool: true,
    geetorusOutput: safeOutput(event.rawOutput),
    ...
    return boundedSidecarValue(
      {
        ...toolCallIdentity,
        locations:
          boundedTool.geetorusBoundedTool === true
            ? (event.locations ?? [])
            : safeAcpxLocations(
                event.locations,
                openParams?.workingDirectory,
...
function diagnostic(code: string, message: string): void {
  ...
  process.stderr.write(`[geetorus-acpx-sidecar] ${code}: ${safe}\n`);
...
function writeFrame(value: AcpxSidecarEvent | AcpxSidecarResponse): void {
  ...
  if (Buffer.byteLength(line) > ACPX_SIDECAR_MAX_FRAME_BYTES) {
    process.stderr.write("[geetorus-acpx-sidecar] output_frame_too_large\n");
    return;
  }
...
function stableProviderIdentity(value: string, kind: string): string {
  ...
  const digest = createHash("sha256")
    .update("geetorus.acpx.provider-identity.v1\0")
    .update(kind)
    .update("\0")
    .update(value)
    .digest("hex");
