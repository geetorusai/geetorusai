...
export const TRUSTED_OPENCODE_EXECUTABLE_ARG =
  "--geetorus-trusted-opencode-executable";

export function withoutAmbientOpenCodeCommand(
  environment: NodeJS.ProcessEnv,
): NodeJS.ProcessEnv {
  const sanitized = { ...environment };
  delete sanitized.GEETORUS_OPENCODE_COMMAND;
  return sanitized;
}
...
export function trustedOpenCodeLaunchBinding(
  ...
  const validateMacSnapshot = (expected?: { dev: number; ino: number }) => {
    ...
    if (!snapshotIsValid) {
      throw new Error(
        `OpenCode ${QUALIFIED_OPENCODE_VERSION} runner-owned executable binding is unavailable; refusing ambient PATH or GEETORUS_OPENCODE_COMMAND fallback`,
      );
    }
    return metadata!;
  };
  if (
    ...
    /^\.geetorus-verified-executable-[0-9a-f]{32}$/.test(
