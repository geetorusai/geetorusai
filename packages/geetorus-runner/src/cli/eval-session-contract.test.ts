...
function request(overrides: Record<string, unknown> = {}): unknown {
  return {
    schema: "geetorus-runner/eval-session-request/v1",
    attemptId: "attempt-1",
    prompt: "Inspect the governed task.",
    model: "gpt-5.6-sol",
    provider: "codex",
    runnerd: { path: "/tmp/geetorus-runnerd", sha256: "a".repeat(64) },
    limits: {
...
function agentCoreProfile(overrides: Record<string, unknown> = {}) {
  return {
    ...
    invocationRoleArn: "arn:aws:iam::123456789012:role/geetorus-agentcore",
    contextBucket: "geetorus-agentcore-context",
    contextPrefix: "geetorus/agentcore/test",
...
describe("eval-session request contract", () => {
  it("materializes a production-v3 runtime context for direct live providers", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "geetorus-eval-context-"));
    ...
    try {
      ...
      expect(context.mcp.assignmentSetId).toBe("geetorus-runner-direct-eval-v1");
      expect(context.instructions.entryPath).toBe("AGENTS.md");
      expect(await readFile(
        join(context.instructions.bundle.rootPath, "AGENTS.md"),
        "utf8",
      )).toContain("Geetorus direct live evaluation");
      const systemInstructions = evalRuntimeSystemInstructions(context);
      expect(systemInstructions).toContain("Geetorus direct live evaluation");
