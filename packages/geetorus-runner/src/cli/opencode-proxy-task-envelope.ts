...
export function openCodeProxyTaskEnvelope(params: Record<string, unknown>) {
  ...
  return createCodexTaskEnvelope({
    objective: "Complete the provider turn supplied by Geetorus Runner.",
    ...(revision && criterionIds.length > 0
      ? {
          contractRevision: revision,
          criteria: criterionIds.map((id) => ({
            id,
            requirement: `Satisfy the Geetorus completion criterion ${id}.`,
          })),
        }
      : {}),
    constraints: [
      ...
      "Use Geetorus MCP tools for semantic operations.",
    ],
  });
}
