...
async function main(): Promise<void> {
  ...
  try {
    ...
    const assertions = {
      ...
      noRealGeetorusRequest: beforeShutdown.networkEvidence.realGeetorusRequests === 0,
      noGeetorusAuthorityInChild:
        beforeShutdown.networkEvidence.childGeetorusEnvironmentKeys.length === 0,
      authorityCleared: afterShutdown.authority.active === false,
      runnerExited: afterShutdown.process?.runnerExited === true,
    };
    const output = {
      schema: "geetorus.capability.live-smoke.v1",
      providerThreadId,
