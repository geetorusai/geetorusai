...
function runtimeDirectory(): string {
  const configured = process.env.GEETORUS_OPENCODE_RUNTIME_DIR?.trim();
  if (!configured)
    throw new Error("GEETORUS_OPENCODE_RUNTIME_DIR is required");
...
async function open(
  ...
  const runtimeContextPath =
    process.env.GEETORUS_NATIVE_RUNTIME_CONTEXT_PATH?.trim();
  ...
  driver = new OpenCodeServerDriver({
    ...
    permissionMode: parseOpenCodeProxyPermissionMode(
      process.env.GEETORUS_OPENCODE_PERMISSION_MODE,
    ),
    ...
    runnerInstanceId:
      process.env.GEETORUS_RUNNER_INSTANCE_ID ?? "geetorus-runnerd-opencode",
  ...
  if (resume) {
    ...
    const snapshot: PersistedHarnessSession = {
      ...
      runId: process.env.GEETORUS_RUN_ID ?? "runnerd-run",
      normalizedSessionId:
        process.env.GEETORUS_NORMALIZED_SESSION_ID ?? threadId,
      activeTurnId: null,
    ...
    session = await driver.openSession({
      runId: process.env.GEETORUS_RUN_ID ?? "runnerd-run",
      normalizedSessionId:
        process.env.GEETORUS_NORMALIZED_SESSION_ID ?? `runnerd-${Date.now()}`,
    ...
    if (event.eventType === "turn.started") {
      ...
      void (async () => {
        const controllerResponse = record(
          await requestController("geetorus/runtimeRequest", {
            request,
          }),
        );
