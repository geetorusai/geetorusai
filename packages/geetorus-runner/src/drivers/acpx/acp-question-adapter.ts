...
import {
  ...
  type GeetorusQuestion,
  type GeetorusQuestionOption,
...
interface AcpFieldBinding {
  ...
  question: GeetorusQuestion;
  optionValues: Map<string, string>;
}

export interface NormalizedAcpForm {
  questionSet: GeetorusQuestionSet;
  /** Convert a validated Geetorus response back into typed ACP content. */
 ...
 * allowed to cross the Geetorus runtime-request boundary.
 */
export function normalizeAcpFormElicitation(
  ...
  const questionSet = parseGeetorusQuestionSet({
  ...
  return {
    questionSet,
    accept(response: unknown): AcpAcceptElicitationResponse {
      const parsed = parseGeetorusQuestionResponse(questionSet, response);
      return {
        action: "accept",
...
): { options: GeetorusQuestionOption[]; values: Map<string, string> } {
  const values = new Map<string, string>();
  const options = nativeOptions.map(
    (option, index): GeetorusQuestionOption => {
