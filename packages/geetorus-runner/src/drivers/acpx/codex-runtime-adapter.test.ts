...
describe("Codex ACPX runtime adapter", () => {
  ...
  it("opens a persistent Codex session without persisting launch secrets", async () => {
    ...
    const port = await openCodexAcpxRuntime(openOptions(command), {
      createRegistry: ({ overrides }) => {
        expect(overrides).toEqual({
          codex: ["geetorus-verified-acpx-command"],
        });
        return registry();
      },
    ...
    expect(runtime.ensureSession).toHaveBeenCalledWith({
      ...
      sessionOptions: {
        model: "gpt-5.6-sol",
        systemPrompt: { append: "Use Geetorus tools." },
      },
    });
    expect(
  ...
  it.each([["claude" as const, "claude-sonnet-5", "claude-sonnet-5"]])(
    ...
    async (agent, model, providerModel) => {
      ...
      await openCodexAcpxRuntime(options, {
        createRegistry: ({ overrides }) => {
          expect(overrides).toEqual({
            [agent]: [agent === "claude" ? "/geetorus-verified/claude-agent-acp" : "geetorus-verified-acpx-command"],
          });
          return registry();
        },
        createStore: () => store(),
      ...
      expect(runtimeOptions?.spawnEnvironment?.()).toEqual({
        PATH: "/verified/bin",
        GEETORUS_ACPX_ISOLATED_CONTEXT: "1",
        GEETORUS_ACPX_TASK_TOOL_BRIDGE_URL: "",
      });
      expect(runtime.ensureSession).toHaveBeenCalledWith(
  ...
  it.each(["runner-owned", "unowned", "absent"])(
    ...
    async (binding) => {
      ...
      options.launchEnvironment = { GEETORUS_ACPX_TASK_TOOL_BRIDGE_URL: "http://untrusted.invalid/mcp" };
      options.mcpServers = binding === "absent" ? [] : [{
        name: "geetorus", url: "http://127.0.0.1:3210/mcp",
      ...
      expect(runtimeOptions?.spawnEnvironment?.()).toEqual({
        GEETORUS_ACPX_ISOLATED_CONTEXT: "1",
        GEETORUS_ACPX_TASK_TOOL_BRIDGE_URL: binding === "runner-owned" ? "http://127.0.0.1:3210/mcp" : "",
      });
    },
  );
...
function openOptions(
  ...
  return {
    ...
    systemInstructions: "Use Geetorus tools.",
    mcpServers: [],
