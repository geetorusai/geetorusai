...
describe("ACPX installation integrity", () => {
  ...
  it("anchors dynamic provider package resolution at an explicit root", async () => {
    const parent = await mkdtemp(
      join(tmpdir(), "geetorus-acpx-package-parent-"),
    );
    ...
    const runnerPackage = join(root, "packages", "geetorus-runner");
    ...
    async () => {
      ...
      await writeFile(
        ...
        [
          ...
          `const credential = await credentials.stageManagedCodexCredential({ agentHomeDirectory: ${JSON.stringify(credentialHome)}, environment: { GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"owner":"original"}' } });`,
          ...
          "const lease = await installation.openCommand();",
          `const provider = lease.spawn([], { env: { ...process.env, GEETORUS_PROVIDER_PID_FILE: ${JSON.stringify(pidFile)} } }, { credentialFenceFds: credential.lifetimeFenceFds, activateCredentialFenceOwner: (pid) => credential.activateLifetimeOwner(pid) });`,
  ...
  it.runIf(process.platform === "linux")(
    ...
    async () => {
      ...
      const guardian = (await installation.openCommand()).spawn(
        [],
        { env: { ...process.env, GEETORUS_PROVIDER_PID_FILE: pidFile } },
        {
...
async function expectOutput(
  ...
  const normalized = process.platform === "darwin"
    ? stdout.replace(/\/private\/var\/[^"\s]*\/geetorus-acpx-[^/]+\/0/g, "/proc/self/fd/4")
    : stdout;
...
async function persistentInstallationFixture() {
  ...
  const command = [
    ...
    "fs.writeFileSync(process.env.GEETORUS_PROVIDER_PID_FILE, String(process.pid));",
...
async function installationFixture() {
  const root = await mkdtemp(join(tmpdir(), "geetorus-acpx-installation-"));
