...
const VERIFIED_COMMAND_SENTINEL = "geetorus-verified-acpx-command";
...
/**
 * Adapt the pinned ACPX library to Geetorus's admitted runtime port. The
  ...
  const runtimeOptions: GoalAwareAcpRuntimeOptions = {
    ...
    agentRegistry: createRegistry({
      ...
      overrides: { [options.profile.agent]: [options.profile.agent === "claude"
        ? "/geetorus-verified/claude-agent-acp"
    ...
    spawnEnvironment: () => ({
      ...
      ...(options.profile.agent === "claude"
        ? {
            ...
            GEETORUS_ACPX_TASK_TOOL_BRIDGE_URL: options.mcpServers.find(
              (server) => server.runnerOwned && server.name === "geetorus",
            )?.url ?? "",
          }
        : {}),
    }),
...
function requireIdentity(handle: AcpRuntimeHandle): AcpxRuntimePortIdentity {
  ...
  return {
    ...
    // it explicitly rather than inventing a Geetorus-owned identifier.
    agentSessionId:
