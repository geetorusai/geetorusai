...
describe("ACPX runtime host", () => {
  it.each([
    ...
    { GEETORUS_NATIVE_MCP_NAME: "geetorus", GEETORUS_NATIVE_MCP_URL: "http://127.0.0.1:3211/mcp", GEETORUS_NATIVE_MCP_TOKEN: "x".repeat(40) },
    { GEETORUS_NATIVE_MCP_NAME: "geetorus-assigned", GEETORUS_NATIVE_MCP_URL: "http://external.example/mcp", GEETORUS_NATIVE_MCP_TOKEN: "x".repeat(40) },
  ...
  it("registers the assigned connection gateway alongside the Claude task bridge", async () => {
    ...
    const host = await AcpxRuntimeHost.open({
      ...
      environment: { ...fixture.options.environment,
        GEETORUS_NATIVE_MCP_NAME: "geetorus-assigned",
        GEETORUS_NATIVE_MCP_URL: "http://127.0.0.1:3211/mcp/gateway",
        GEETORUS_NATIVE_MCP_TOKEN: "fixture-gateway-token-".repeat(3),
      },
    ...
    try {
      expect(servers.map(server => server.name)).toEqual(["geetorus", "geetorus-assigned"]);
      expect(servers[1]).toEqual({ name: "geetorus-assigned", url: "http://127.0.0.1:3211/mcp/gateway",
        bearerToken: "fixture-gateway-token-".repeat(3), runnerOwned: true });
    } finally { await host.close({ reason: "gateway test complete" }); }
  });

  it("automatically permits only admitted Geetorus reads in the Claude SDK", async () => {
    const fixture = await hostFixture();
    const dependencies = fixture.dependencies({
      openRuntime: async (options) => {
        ...
        expect(settings.permissions).toEqual({
          allow: ["mcp__geetorus__get_task_context"],
        });
        expect(options.mcpServers).toMatchObject([
          { name: "geetorus", runnerOwned: true },
        ]);
        return runtimePort({
  ...
  it("composes admission, isolation, model verification, and cleanup", async () => {
    ...
    expect(host.identity()).toMatchObject({
      schema: "geetorus.runner.acpx-identity.v2",
...
async function hostFixture() {
  const root = await mkdtemp(join(tmpdir(), "geetorus-acpx-host-"));
