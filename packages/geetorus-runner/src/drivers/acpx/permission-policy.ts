...
import { geetorusSemanticAction } from "../../catalog/semantic-action-catalog.js";
...
export class AcpxApprovalRequiredError extends Error {
  ...
  constructor() {
    super(
      "Approval required. This operation is not an automatically allowed Geetorus read, " +
...
/** Exact SDK rules for the run's runner-owned Geetorus MCP connection. */
export function claudeReadPermissionRules(
  ...
  const names = tools.flatMap((tool) => {
    ...
    // Effects come from Geetorus's implementation catalog, never tool hints
    ...
    return geetorusSemanticAction(name)?.effect === "read"
      ? [`mcp__geetorus__${name}`]
      : [];
  });
...
export function acpxRuntimePermissionPolicy(
  ...
  // auto-approve here. Verified Geetorus reads are allowed earlier at the
...
export function decideAcpxPermission(
  ...
  // Verified Geetorus reads use exact SDK rules on the runner-owned MCP
