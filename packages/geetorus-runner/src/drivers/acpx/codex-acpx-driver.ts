...
import { GEETORUS_RUNTIME_REQUEST_SCHEMA_V2 } from "../../contracts/question-set.js";
...
export async function probeQualifiedAcpxEnvironment(
  ...
  const driver = new CodexAcpxDriver({
    ...
    systemInstructions:
      "Geetorus Runner environment qualification probe. Do not execute a provider turn.",
...
class CodexAcpxSession implements HarnessSession {
    ...
    const cleanup = Promise.resolve()
      .then(() =>
        this.#host.interruptActiveTurn(
          "Geetorus parked the ACPX input on a durable wait.",
        ),
      )
      .catch((error: unknown) => {
        if (
  ...
  async dispatchTool(call: RunnerToolCall): Promise<unknown> {
    ...
    if (!this.#dynamicToolHandler) {
      throw new Error(`Unsupported Geetorus operation ${tool}`);
    }
