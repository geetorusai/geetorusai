...
describe("ACPX permission policy", () => {
  it("uses only catalogued reads assigned to this run, regardless of tool hints", () => {
    const tools = [
      "geetorus__get_task_context", "read_document", "write_document",
    ...
    expect(claudeReadPermissionRules(tools)).toEqual([
      "mcp__geetorus__get_task_context", "mcp__geetorus__read_document",
    ]);
  ...
  it("keeps deny-all closed against provider-supplied semantic metadata", () => {
    for (const [agent, raw] of [
      ["claude", { toolCall: { name: "mcp__geetorus__geetorus_finish" } }],
      ["claude", { toolCall: { rawInput: { serverName: "geetorus" } } }],
      [
        "codex",
        {
  ...
  it("does not let provider metadata widen approve-reads", () => {
    for (const [agent, inferredKind, raw, options] of [
      ...
      [
        "claude",
        "write",
        { toolCall: { rawInput: { serverName: "geetorus" } } },
        {},
      ],
      [
        "claude",
        "execute",
        { toolCall: { name: "mcp__geetorus__geetorus_finish" } },
        {},
      ],
      [
        "claude",
        "write",
        {
          toolCall: {
            _meta: {
              claudeCode: { toolName: "mcp.geetorus.get_task_context" },
            },
          },
        },
        {},
      ...
      expect(
        decideAcpxPermission("codex", "approve-reads", {
          ...
          raw: {
            _meta: { is_mcp_tool_approval: true },
            toolCall: {
              name: "mcp__geetorus__get_task_context",
              rawInput: { serverName: "geetorus" },
            },
          },
        }),
      ).toBe("delegate");
