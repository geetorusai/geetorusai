...
afterEach(async () => {
  ...
  (
    globalThis as typeof globalThis & {
      __geetorusDirectorySyncHelperRegistryV1?: {
        activeParentOperations: Set<symbol>;
      };
    }
  ).__geetorusDirectorySyncHelperRegistryV1?.activeParentOperations.clear();
  await Promise.all(
...
describe("managed Codex credentials", () => {
  ...
  it("stages inline JSON privately and removes it idempotently", async () => {
    ...
    const lease = await stageManagedCodexCredential({
      ...
      environment: {
        GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: JSON.stringify({
  ...
  it("fences overlapping leases for the same isolated home", async () => {
    ...
    const firstLease = await stageManagedCodexCredential({
      agentHomeDirectory: fixture.home,
      environment: {
        GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"owner":"first"}',
      },
    });
    ...
    await expect(
      stageManagedCodexCredential({
        agentHomeDirectory: fixture.home,
        environment: {
          GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"owner":"second"}',
        },
      }),
  ...
  it("fences a contender loaded through a fresh module instance", async () => {
    ...
    await expect(
      freshCredentials.stageManagedCodexCredential({
        agentHomeDirectory: fixture.home,
        environment: {
          GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"owner":"contender"}',
        },
      }),
    ).rejects.toThrow("already has an active lease");
    ...
    async (_label, occupiedIndex) => {
      ...
      try {
        const lease = await stageManagedCodexCredential({
          agentHomeDirectory: fixture.home,
          environment: {
            GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"owner":"geetorus"}',
          },
        });
  ...
  it("fences another process and recovers only after its kernel lease dies", async () => {
    ...
    const credentialModule = new URL("./codex-credentials.ts", import.meta.url)
      .href;
    // geetorus-runner intentionally does not ship a TS runtime dependency.
    ...
    await writeFile(
      ...
      [
        ...
        '  environment: { GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: \'{"owner":"first"}\' },',
        "});",
    ...
    try {
      ...
      expect(
        (await readdir(fixture.home)).filter(
          (name) =>
            name.includes("geetorus-auth-lease") ||
            name.includes("geetorus-auth-home-claim"),
        ),
      ).toEqual([]);
    } finally {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const lease = await stageManagedCodexCredential({
        agentHomeDirectory: fixture.home,
        environment: { GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: "{}" },
      });
    ...
    expect(
      (await readdir(fixture.home)).filter(
        (name) =>
          name.includes("geetorus-auth-lease") ||
          name.includes("geetorus-auth-home-claim") ||
          name.includes("geetorus-auth-lock"),
      ),
    ).toEqual([]);
  });
  it("recovers a persisted cleanup intent before admitting another provider", async () => {
    ...
    await writeFile(cleanupIntent, "geetorus-managed-codex-cleanup-v1\n", {
      mode: 0o600,
    });
  ...
  it("scrubs a crash-left credential staging file before admission", async () => {
    const fixture = await credentialFixture();
    const stagingPath = join(fixture.home, ".geetorus-auth-staging-v1");
    const cleanupIntent = join(
      fixture.home,
      ".geetorus-auth-cleanup-required",
    );
    ...
    const lease = await freshCredentials.stageManagedCodexCredential({
      agentHomeDirectory: fixture.home,
      environment: {
        GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"owner":"successor"}',
      },
    });
  ...
  it("replaces a stale regular auth destination in JSON modes", async () => {
    ...
    const lease = await stageManagedCodexCredential({
      agentHomeDirectory: fixture.home,
      environment: {
        GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: '{"fresh":true}',
      },
    });
...
async function credentialFixture(): Promise<{ root: string; home: string }> {
  const root = await mkdtemp(join(tmpdir(), "geetorus-acpx-credential-"));
...
function credentialLeasePortsForScope(
  ...
  const digest = createHash("sha256")
    .update("geetorus-managed-codex-lease-v2:")
    .update(userScope)
    .update("\0")
    .update(home)
    .digest();
