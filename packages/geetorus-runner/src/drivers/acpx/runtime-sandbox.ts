...
export async function prepareAcpxRuntimeSandbox(input: {
  ...
  /** Public operations on the runner-owned Geetorus MCP bridge only. */
  ...
  Object.assign(launchEnvironment, {
    ...
    GEETORUS_ACPX_PROFILE: input.agent,
...
function isPersistableEnvironmentName(name: string): boolean {
  return (
    ...
    /^(?:GEETORUS_ACPX_PROFILE|GEETORUS_ACPX_ISOLATED_CONTEXT)$/.test(name) ||
