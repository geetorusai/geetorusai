...
const CREDENTIAL_CLEANUP_INTENT = ".geetorus-auth-cleanup-required";
const CREDENTIAL_STAGING_FILE = ".geetorus-auth-staging-v1";
...
const processDirectorySyncHelperState = globalThis as typeof globalThis & {
  __geetorusDirectorySyncHelperRegistryV1?: DirectorySyncHelperRegistry;
};
const directorySyncHelperRegistry =
  processDirectorySyncHelperState.__geetorusDirectorySyncHelperRegistryV1 ?? {
...
async function stageClaimedManagedCodexCredential(
  ...
  const inlineJson = environment.GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET;
...
async function createCredentialCleanupIntent(
  ...
  try {
    ...
    await handle.writeFile("geetorus-managed-codex-cleanup-v1\n", "utf8");
    await handle.sync();
    await handle.close();
    handle = undefined;
