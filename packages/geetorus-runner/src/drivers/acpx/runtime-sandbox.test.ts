    ...
    async (agent, credentialName, homeSuffix) => {
      ...
      const sandbox = await prepareAcpxRuntimeSandbox({
        ...
        environment: {
          ...
          GEETORUS_NATIVE_MCP_TOKEN: "native-secret",
        },
      });
      ...
      expect(
        sandbox.launchEnvironment.GEETORUS_NATIVE_MCP_TOKEN,
      ).toBeUndefined();
      ...
      expect(
        sandbox.persistedEnvironment.GEETORUS_NATIVE_MCP_URL,
      ).toBeUndefined();
      expect(
        sandbox.persistedEnvironment.GEETORUS_NATIVE_MCP_TOKEN,
      ).toBeUndefined();
...
async function sandboxFixture(agent: "pi" | "claude" | "codex") {
  const root = await mkdtemp(join(tmpdir(), "geetorus-acpx-sandbox-"));
