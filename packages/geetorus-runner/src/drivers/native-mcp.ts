...
export function nativeMcpLaunchBinding(
  ...
  const name = environment.GEETORUS_NATIVE_MCP_NAME?.trim();
  const url = environment.GEETORUS_NATIVE_MCP_URL?.trim();
  const token = environment.GEETORUS_NATIVE_MCP_TOKEN?.trim();
