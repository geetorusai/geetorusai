...
import {
  ...
  GEETORUS_RUNTIME_REQUEST_SCHEMA_V2,
...
import { geetorusWorkspaceFileReferencesFromText } from "../../live/workspace-file-reference.js";
import {
...
export class OpenCodeServerDriver implements HarnessDriver {
  ...
  async #open(
    ...
    const trace = await createProviderTraceFileSink({
      path: this.#options.environment?.GEETORUS_PROVIDER_TRACE_PATH,
      provider: "opencode",
      channel: "typescript_opencode_native",
      maxBytes: this.#options.environment?.GEETORUS_PROVIDER_TRACE_MAX_BYTES,
    });
      ...
      try {
        ...
        if (providerSessionId !== null) {
          ...
          const created = await api(fetcher, runtime, "/session", {
            method: "POST",
            body: JSON.stringify({ title: `Geetorus ${input.runId}` }),
          });
...
class OpenCodeHarnessSession implements HarnessSession {
  ...
  readonly #pendingRuntimeRequests = new Map<
    string,
    {
      ...
      submittedResponse?: GeetorusQuestionResponse;
  ...
  async startTurn(input: {
    ...
    // every Geetorus continuation can overflow smaller context windows and
    ...
    await api(
      ...
      `/session/${encodeURIComponent(this.#providerSessionId)}/prompt_async`,
      {
        ...
        body: JSON.stringify({
          ...
          // lookup. Keep Geetorus's persisted provider/model form, but adapt it
  ...
  async transcript(): Promise<HarnessTranscriptSnapshot> {
    return {
      schema: "geetorus-runner/harness-transcript/v1",
      complete: true,
  ...
  async dispatchTool(call: {
    ...
    if (!this.#dynamicToolHandler)
      throw new Error("Unsupported Geetorus operation");
    try {
  ...
  #acceptQuestion(properties: Record<string, unknown>): void {
    ...
    this.#emit(
      ...
      {
        request: {
          schema: GEETORUS_RUNTIME_REQUEST_SCHEMA_V2,
          requestKind: "runtime",
          requestId,
          type: "input",
          status: "pending",
  ...
  #mapProviderEventValue(value: unknown): void {
    ...
    if (type === "session.error" && turnId) {
      ...
      if (
        ...
        // endpoint is also how Geetorus parks a provider turn after a durable
  ...
  #emitAssistantPart(part: Record<string, unknown>, turnId: string): void {
    ...
    if (partType === "text" && content.length > 0) {
      for (const reference of geetorusWorkspaceFileReferencesFromText(
...
export function normalizeOpenCodeQuestionSet(
  ...
  const questions = nativeQuestions.map(
    (question, index): GeetorusQuestion => {
  ...
  return parseGeetorusQuestionSet({
...
function openCodeAnswers(
  ...
  response: GeetorusQuestionResponse,
): string[][] {
