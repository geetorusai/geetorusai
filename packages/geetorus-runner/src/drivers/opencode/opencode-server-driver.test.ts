...
describe("OpenCodeServerDriver", () => {
  it("advertises within-turn plans as unsupported", async () => {
    const driver = new OpenCodeServerDriver({
      ...
      runtimeDirectory: "/tmp/geetorus-opencode-capabilities",
      command: fixture,
    });
  ...
  it("normalizes raw OpenCode boundary events into the local-integrity golden identities", async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), "geetorus-opencode-boundary-"));
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-boundary-workspace-"),
    );
  ...
  it("starts an authenticated isolated server, creates a session, streams usage, aborts, and cleans up", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(join(tmpdir(), "geetorus-opencode-driver-"));
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-workspace-"),
    );
    roots.push(root, workspace);
    const tracePath = join(root, "provider-trace.ndjson");
    ...
    const driver = new OpenCodeServerDriver({
      ...
      environment: {
        ...
        GEETORUS_API_KEY: "must-not-leak",
    ...
    expect(
      events.find((event) => event.eventType === "workspace.diff.recorded")
        ?.payload,
    ).toMatchObject({ schema: "geetorus.workspace.diff.v1", complete: true });
    expect(
    ...
    expect(environment.keys).not.toContain("GEETORUS_API_KEY");
    expect(environment.keys).not.toContain("UNRELATED_SECRET");
    expect(environment.keys).not.toContain("GEETORUS_PROVIDER_TRACE_PATH");
    expect(environment.projectConfigDisabled).toBe("true");
    expect(mcpEvidence.tools).toEqual(
      expect.arrayContaining(["geetorus_finish", "geetorus_block"]),
    );
  ...
  it("maps OpenCode's normal abort error to cancellation without a false provider failure notice", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(join(tmpdir(), "geetorus-opencode-abort-"));
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-abort-workspace-"),
    );
    roots.push(root, workspace);
    const driver = new OpenCodeServerDriver({
      model: "openrouter/deepseek/deepseek-v4-flash-0731",
  ...
  it("uses the native system channel, selected skill tree, instruction root, and assigned MCP gateway", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(join(tmpdir(), "geetorus-opencode-context-"));
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-context-workspace-"),
    );
    ...
    const systemInstructions = `${GEETORUS_EXECUTION_PROMPT}\n\nRead sibling.md\n\nRead-only instruction sibling root: ${instructionRoot}`;
    ...
    const driver = new OpenCodeServerDriver({
      ...
      environment: {
        ...
        GEETORUS_NATIVE_MCP_TOKEN: "x".repeat(40),
      },
    ...
    expect(config).toMatchObject({
      ...
      mcp: {
        geetorus: { enabled: true },
        "geetorus-assigned": {
          url: "https://geetorus.example/mcp",
          enabled: true,
        },
      },
    });
  ...
  it("sends only the authoritative wake envelope when resuming an existing provider session", async () => {
    ...
    const root = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-resume-context-"),
    );
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-resume-workspace-"),
    );
    roots.push(root, workspace);
    let submittedPrompt: Record<string, unknown> | null = null;
    const driver = new OpenCodeServerDriver({
  ...
  it("normalizes native question.asked events and replies with ordered OpenCode answer arrays", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(join(tmpdir(), "geetorus-opencode-question-"));
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-question-workspace-"),
    );
    roots.push(root, workspace);
    const driver = new OpenCodeServerDriver({
      model: "openrouter/deepseek/deepseek-v4-flash-0731",
  ...
  it("hands a native question to the durable interaction path without touching permissions", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-question-handoff-"),
    );
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-question-handoff-workspace-"),
    );
    roots.push(root, workspace);
    const driver = new OpenCodeServerDriver({
      model: "openrouter/deepseek/deepseek-v4-flash-0731",
  ...
  it("recovers a missed pending question from the list endpoint and rejects it", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-question-recovery-"),
    );
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-question-recovery-workspace-"),
    );
    roots.push(root, workspace);
  ...
  it.each([
    ...
    async ({ style, action, reply }) => {
      await chmod(fixture, 0o755);
      const root = await mkdtemp(
        join(tmpdir(), `geetorus-opencode-permission-${style}-`),
      );
      const workspace = await mkdtemp(
        join(tmpdir(), `geetorus-opencode-permission-${style}-workspace-`),
      );
      roots.push(root, workspace);
      const driver = new OpenCodeServerDriver({
        model: "openrouter/deepseek/deepseek-v4-flash-0731",
  ...
  it.each(["allow", "ask", "deny"] as const)(
    ...
    async (permissionMode) => {
      await chmod(fixture, 0o755);
      const root = await mkdtemp(
        join(tmpdir(), `geetorus-opencode-mode-${permissionMode}-`),
      );
      const workspace = await mkdtemp(
        join(tmpdir(), `geetorus-opencode-mode-${permissionMode}-workspace-`),
      );
      roots.push(root, workspace);
      const driver = new OpenCodeServerDriver({
        model: "openrouter/deepseek/deepseek-v4-flash-0731",
      ...
      expect(config.permission.geetorus_finish).toBeUndefined();
      expect(config.permission["geetorus_*"]).toBe("allow");
  ...
  it("clears a stale active turn that already has a persisted terminal fingerprint", async () => {
    await chmod(fixture, 0o755);
    const root = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-stale-turn-"),
    );
    const workspace = await mkdtemp(
      join(tmpdir(), "geetorus-opencode-stale-workspace-"),
    );
    roots.push(root, workspace);
    const driver = new OpenCodeServerDriver({
      model: "openrouter/deepseek/deepseek-v4-flash-0731",
