...
describe("OpenCode MCP bridge", () => {
  ...
  it("admits runner-private extension operations without advertising them to the model", async () => {
    const handler = vi.fn(async ({ tool }) => ({ decision: tool === "__geetorus_permission" ? "accept" : "decline" }));
    const bridge = await startOpenCodeMcpBridge({
      tools: [{ name: "documents.read", inputSchema: { type: "object" } }],
      privateTools: [{ name: "__geetorus_permission", inputSchema: { type: "object" } }],
      handler,
    });
    bridges.push(bridge);
    const listed = await (await rpc(bridge, { id: 1, method: "tools/list" })).json() as { result: { tools: Array<{ name: string }> } };
    expect(listed.result.tools.map((tool) => tool.name)).not.toContain("__geetorus_permission");
    expect(await (await rpc(bridge, {
      id: "permission-1",
      method: "tools/call",
      params: { name: "__geetorus_permission", arguments: {} },
    })).json()).toMatchObject({ result: { content: [{ text: "{\"decision\":\"accept\"}" }] } });
    expect(handler).toHaveBeenCalledWith(expect.objectContaining({ tool: "__geetorus_permission" }));
  });
  ...
  it("maps prefixed names and replays identical duplicate calls exactly once", async () => {
    ...
    bridges.push(bridge);
    const request = { id: "call-1", method: "tools/call", params: { name: "geetorus_documents.read", arguments: { id: "doc" } } };
  ...
  it("normalizes all supported OpenCode MCP prefixes", () => {
    expect(canonicalOpenCodeMcpToolName("geetorus_documents.read")).toBe("documents.read");
    expect(canonicalOpenCodeMcpToolName("geetorus__documents.read")).toBe("documents.read");
    expect(canonicalOpenCodeMcpToolName("geetorus.documents.read")).toBe("documents.read");
    expect(canonicalOpenCodeMcpToolName("geetorus_geetorus_finish")).toBe("geetorus_finish");
  });
  ...
  it("times out and cancels in-flight calls", async () => {
    ...
    expect(await response.json()).toMatchObject({ result: { isError: true, content: [{ text: "Geetorus tool call timed out" }] } });
  ...
  it("allows human-gated private calls to outlive ordinary tool timeouts", async () => {
    const bridge = await startOpenCodeMcpBridge({
      tools: [{ name: "documents.read", inputSchema: { type: "object" } }],
      privateTools: [{ name: "__geetorus_runtime_input", inputSchema: { type: "object" } }],
      timeoutMs: 20,
    ...
    const response = await rpc(bridge, {
      id: "human-input",
      method: "tools/call",
      params: { name: "__geetorus_runtime_input", arguments: {} },
    });
  ...
  it("honors MCP cancellation notifications", async () => {
    ...
    expect(await (await pending).json()).toMatchObject({
      result: { isError: true, content: [{ text: "Geetorus tool call cancelled" }] },
    });
  });
});
