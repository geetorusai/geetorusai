...
describe("runner semantic MCP bridge", () => {
  ...
  it("keeps private operations callable but undiscoverable", async () => {
    ...
    const bridge = await startRunnerToolBridge({
      tools: [tool("documents.read")],
      privateTools: [tool("__geetorus_permission")],
      handler,
    });
    bridges.push(bridge);
    const listed = (await (
    ...
    expect(
      ...
      result: {
        content: [{ text: expect.stringContaining("__geetorus_permission") }],
      },
    });
  });
  ...
  it("times out calls and honors MCP cancellation", async () => {
    ...
    expect(
      ...
      result: {
        isError: true,
        content: [{ text: "Geetorus tool call timed out" }],
      },
    });
    ...
    expect(await (await pending).json()).toMatchObject({
      result: {
        isError: true,
        content: [{ text: "Geetorus tool call cancelled" }],
      },
    });
  });
  ...
  it("preserves successful mutation identity when the result is oversized", async () => {
    ...
    for (const response of [
      ...
      expect(manifest).toMatchObject({
        schema: "geetorus.semantic_tool_result_chunks.v1",
        encoding: "json",
  ...
  it("returns a complete tagged receipt for cyclic results without re-executing", async () => {
    ...
    expect(JSON.parse(first.result.content[0]!.text)).toMatchObject({
      schema: "geetorus.semantic_tool_result.v1",
      status: "completed",
      tool: "documents.read",
      callIdentitySha256: expect.stringMatching(/^[0-9a-f]{64}$/),
      encoding: "geetorus.tagged_graph.v1",
      result: {
        root: { $ref: 1 },
        nodes: [
          {
  ...
  it("prevents catalog ambiguity and reserved schema replacement", async () => {
    await expect(
      startRunnerToolBridge({
        tools: [tool("documents.read"), tool("geetorus_documents.read")],
        handler: async () => null,
      }),
    ).rejects.toThrow("duplicated");
    await expect(
      startRunnerToolBridge({
        tools: [tool("geetorus_finish")],
        handler: async () => null,
      }),
  ...
  it("normalizes every supported provider prefix", () => {
    expect(canonicalRunnerToolName("geetorus_documents.read")).toBe(
      "documents.read",
    );
    expect(canonicalRunnerToolName("geetorus__documents.read")).toBe(
      "documents.read",
    );
    expect(canonicalRunnerToolName("geetorus.documents.read")).toBe(
      "documents.read",
    );
  });
});
