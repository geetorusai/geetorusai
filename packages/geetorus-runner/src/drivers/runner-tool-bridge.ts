...
export function canonicalRunnerToolName(name: string): string {
  ...
  for (const prefix of ["geetorus__", "geetorus_", "geetorus."]) {
...
async function handleRequest(
  ...
  if (method === "initialize") {
    ...
    writeRpc(response, id, {
      ...
      serverInfo: { name: "geetorus-runner", version: "1" },
    });
    return;
  }
  if (method === "ping") {
...
function withCancellationAndTimeout<T>(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () =>
      finish(() => reject(new Error("Geetorus tool call cancelled")));
    const timer = setTimeout(() => {
      finish(() => reject(new Error("Geetorus tool call timed out")));
      controller.abort();
    }, timeoutMs);
    timer.unref();
...
function chunkedToolResult(
  tool: string,
  callId: string,
  encoding: "json" | "geetorus.tagged_graph.v1",
  serialized: string,
): RunnerToolCallResult {
