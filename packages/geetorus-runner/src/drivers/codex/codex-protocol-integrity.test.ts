...
async function authenticatedRunner(
  ...
  const authKey = digest("geetorus-runner-auth-key-v1", [credential]);
  const mac = (domain: string, parts: Buffer[]) =>
    createHmac("sha256", authKey).update(framed(domain, parts)).digest();
  const credentialId = `sha256:${digest("geetorus-runner-credential-id-v1", [credential]).toString("hex")}`;
  ...
  expect(serverProof).toBe(
    mac("geetorus-runner-server-proof-v1", [canonical]).toString("hex"),
  );
  const clientProof = mac("geetorus-runner-client-proof-v1", [
    canonical,
  ...
  const binding = digest("geetorus-runner-session-binding-v1", [
    canonical,
    Buffer.from(String(serverProof)),
  ...
  const aad = (direction: string, counter: number) =>
    Buffer.from(
      `geetorus.runner.secure-frame.v1\0${sessionId}\0${direction}\0${counter}`,
    );
  ...
  const decipher = createDecipheriv(
    "aes-256-gcm",
    mac("geetorus-runner-core-to-client-key-v1", [binding]),
    nonce("P3S1", 0),
  );
  ...
  return {
    ...
    send(value: Record<string, unknown>) {
      const cipher = createCipheriv(
        "aes-256-gcm",
        mac("geetorus-runner-client-to-core-key-v1", [binding]),
        nonce("P3C1", counter),
      );
  ...
  it.each([
    ...
    async (scenario) => {
      const directory = mkdtempSync(
        join(tmpdir(), "geetorus-composed-integrity-"),
      );
      ...
      const driver = new CodexAppServerDriver({
        ...
        environment: { GEETORUS_WORKSPACE_CWD: directory },
