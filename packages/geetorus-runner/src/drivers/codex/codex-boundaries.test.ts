...
describe("Codex value and workspace boundaries", () => {
  it("accepts assigned workspaces below HOME while rejecting host-state and containment escapes", () => {
    const fixture = mkdtempSync(join(tmpdir(), "geetorus-codex-boundaries-"));
    try {
      const hostRoot = join(fixture, "host");
      const hostHome = join(hostRoot, "home");
      const workspaceRoot = join(hostHome, ".geetorus", "workspaces");
      ...
      expect(
        validateCodexWorkingDirectory(workspace, {
          ...
          GEETORUS_WORKSPACE_CWD: workspaceRoot,
        }),
      ...
      expect(
        validateCodexWorkingDirectory(ordinaryHomeWorkspace, {
          HOME: hostHome,
          CODEX_HOME: codexHome,
          GEETORUS_WORKSPACE_CWD: join(hostHome, "projects"),
        }),
  ...
  it("defers provider-owned workspace existence without weakening its assignment", () => {
    const remoteWorkspace = "/home/daytona/geetorus-workspace";
    const remoteEnvironment = {
      HOME: remoteWorkspace,
      CODEX_HOME: `${remoteWorkspace}/.codex`,
      GEETORUS_WORKSPACE_CWD: remoteWorkspace,
    };
  ...
  it("keeps completion and block dispositions distinct", () => {
    expect(isCodexSemanticTool("geetorus_finish")).toBe(true);
    expect(isCodexSemanticTool("geetorus_block")).toBe(true);
    expect(isCodexSemanticTool("shell")).toBe(false);
    expect(codexToolAcceptsDisposition("geetorus_finish", "done")).toBe(true);
    expect(codexToolAcceptsDisposition("geetorus_finish", "yielded")).toBe(true);
    expect(codexToolAcceptsDisposition("geetorus_finish", "blocked")).toBe(
      false,
    );
    expect(codexToolAcceptsDisposition("geetorus_block", "blocked")).toBe(
      true,
    );
    expect(codexToolAcceptsDisposition("unknown_tool", "done")).toBe(false);
    expect(codexToolAcceptsResult("geetorus_finish", {
