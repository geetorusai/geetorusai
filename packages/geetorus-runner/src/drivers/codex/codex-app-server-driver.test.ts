...
class FakeCodexTransport implements CodexAppServerTransport {
  ...
  async request(
    ...
    if (method === "thread/start" || method === "thread/resume") {
      const planMode =
        params.permissions === "geetorus-runner-workspace-read-only";
      return {
        thread: {
          id: this.threadId,
  ...
  pushTraced(
    ...
    this.queue.push({
      method,
      params,
      geetorusTrace: { sourceEventId, sourceEventType },
    });
  }
...
function makeDriver(
  ...
  return new CodexAppServerDriver({
    ...
    environment: {
      ...
      LANG: "C.UTF-8",
      GEETORUS_API_KEY: "must-not-pass",
...
describe("Codex app-server Codex driver", () => {
  ...
  it("places Geetorus runtime instructions in Codex's system channel and enables only selected skill instructions", async () => {
    const transport = new FakeCodexTransport();
    const baseInstructions = [
      "You are running as a Geetorus agent.",
      "Follow the attached AGENTS.md instructions.",
      "Read-only instruction sibling root: /geetorus/context/instructions",
    ].join("\n\n");
    const driver = makeDriver([transport], {
      baseInstructions,
  ...
  it("passes the common typed-event contract and reports one provider turn terminal", async () => {
    ...
    const capabilities: PrpCapabilities = {
      schema: "geetorus.prp.capabilities.v1",
    ...
    const metadata = {
      ...
      identity: {
        schema: "geetorus.prp.identity.v1" as const,
        companyId: "company-1",
        issueId: "issue-1",
        runId: "run-1",
  ...
  it("admits a strictly bound semantic result from the durable runner", async () => {
    ...
    transport.push("geetorus/runResult", {
      threadId: "thread-1",
      turnId: turn.turnId,
  ...
  it("correlates rehydrated notifications with the final canonical PRP event ids", async () => {
    ...
    expect(JSON.stringify(events)).not.toContain("geetorusTrace");
  });
  ...
  it("maps canonical workspace snapshots with monotonic revisions and one terminal diff", async () => {
    ...
    transport.pushTraced(
      "geetorus/workspaceChange/updated",
    ...
    transport.push("geetorus/workspaceChange/updated", {
      threadId: "thread-1",
      turnId: turn.turnId,
    ...
    expect(
      transport.traceInterpretations.filter((entry) =>
        entry.providerMethod === "geetorus/workspaceChange/updated"
  ...
  it("captures an exact skillless model/environment snapshot with credentials absent", async () => {
    ...
    expect(
      ...
      permissions: "geetorus-runner-workspace-only",
      runtimeWorkspaceRoots: [TEST_WORKING_DIRECTORY],
      dynamicTools: [{ name: "geetorus_finish" }, { name: "geetorus_block" }],
    });
    ...
    expect(appServerArgs).toEqual(
      expect.arrayContaining([
        ...
        "permissions.geetorus-runner-workspace-only.network.enabled=false",
  ...
  it("exposes and dispatches only explicit chat tools across fresh and resumed direct chat", async () => {
    ...
    const requestHumanInput = {
      ...
      description: "Ask one structured question through Geetorus.",
      inputSchema: { type: "object", properties: {} },
    };
  ...
  it("normalizes finish and block tools into one canonical result contract", async () => {
    ...
    expect(
      await transport.invoke({
        ...request,
        id: 9,
        params: { ...request.params, tool: "geetorus_finish" },
      }),
    ).toMatchObject({ success: false });
    expect(await transport.invoke(request)).toMatchObject({ success: true });
