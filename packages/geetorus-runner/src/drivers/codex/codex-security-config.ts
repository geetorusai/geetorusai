...
export function codexNetworkReadOnlyRoots(source: NodeJS.ProcessEnv): string[] {
  ...
  if (source.GEETORUS_RUNNER_NETWORK_ROOTS !== undefined) {
    try {
      const projected: unknown = JSON.parse(source.GEETORUS_RUNNER_NETWORK_ROOTS);
...
export function codexNetworkAccess(source: NodeJS.ProcessEnv = process.env): boolean {
  return source.GEETORUS_RUNNER_NETWORK_ACCESS === "enabled";
}
...
function gitFilesystemRoots(source: NodeJS.ProcessEnv): { read: string[]; write: string[] } {
  ...
  const write: string[] = [];
  try {
    const roots: unknown = JSON.parse(source.GEETORUS_GIT_METADATA_ROOTS ?? "[]");
  ...
  if (source.GEETORUS_GITHUB_AUTH_MODE === "host" && source.GEETORUS_GITHUB_HOST_HOME) {
    for (const relative of [".gitconfig", ".git-credentials", ".config/git", ".config/gh", ".ssh"]) {
      read.push(join(source.GEETORUS_GITHUB_HOST_HOME, relative));
    }
...
function usesExternalRunnerSandbox(source: NodeJS.ProcessEnv): boolean {
  return source.GEETORUS_RUNNER_EXTERNAL_SANDBOX === "1";
}
...
export function codexCommandEnvironment(
  ...
  if (source.GEETORUS_GITHUB_AUTH_MODE === "host" && source.GEETORUS_GITHUB_HOST_HOME) {
    environment.HOME = source.GEETORUS_GITHUB_HOST_HOME;
  } else if (source.GEETORUS_GITHUB_LAUNCHER_DIR) {
    environment.HOME = source.GEETORUS_GITHUB_LAUNCHER_DIR;
    environment.ZDOTDIR = source.GEETORUS_GITHUB_LAUNCHER_DIR;
    environment.BASH_ENV = `${source.GEETORUS_GITHUB_LAUNCHER_DIR}/.bashrc`;
  }
  return environment;
}
...
export function createIsolatedCodexAppServerArgs(
  ...
  if (source.GEETORUS_GITHUB_LAUNCHER_DIR) readOnlyRoots = [...readOnlyRoots, source.GEETORUS_GITHUB_LAUNCHER_DIR];
  ...
  const filesystemRules = [
    ...
    ...(source.GEETORUS_GITHUB_BROKER_TOKEN && source.GH_CONFIG_DIR
