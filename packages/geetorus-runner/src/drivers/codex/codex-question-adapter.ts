...
interface NormalizedQuestionOptions {
  options: NonNullable<GeetorusQuestion["options"]>;
...
export function normalizeCodexQuestionSet(
  ...
  if (method === "item/tool/requestUserInput" || method === "tool/requestUserInput") {
    ...
    const questions = params.questions.map((rawQuestion, index): GeetorusQuestion => {
    ...
    return retainNativeOptionValues(parseGeetorusQuestionSet({
  ...
  const nativeValues = new Map<string, ReadonlyMap<string, unknown>>();
  const questions = propertyEntries.map(([id, rawProperty]): GeetorusQuestion => {
    ...
    const answerMode: GeetorusQuestion["answerMode"] = propertyType === "array" && normalizedOptions.options.length > 0
      ? "multi_select"
