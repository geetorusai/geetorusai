...
describe("Codex security configuration", () => {
  ...
  it("preserves target DNS symlink resources without opening all of /run", () => {
    const source = { GEETORUS_RUNNER_NETWORK_ACCESS: "enabled", GEETORUS_RUNNER_NETWORK_ROOTS: '["/run/systemd/resolve/stub-resolv.conf","/etc/ssl/certs"]' };
    ...
    expect(codexNetworkReadOnlyRoots({ ...source, GEETORUS_RUNNER_NETWORK_ACCESS: "disabled" })).toEqual([]);
  });
  ...
  it("honors an explicit network restriction independently of GitHub", () => {
    for (const GH_TOKEN of [undefined, "managed-token"]) {
      const args = createIsolatedCodexAppServerArgs({ GH_TOKEN, GEETORUS_RUNNER_NETWORK_ACCESS: "disabled" }).join("\n");
      expect(args).toContain("network.enabled=false");
      expect(args).not.toContain("network.enabled=true");
    }
  });
  ...
  it("restores host Git resources without exposing the provider home", () => {
    const args = createIsolatedCodexAppServerArgs({
      HOME: "/provider", CODEX_HOME: "/provider", PATH: "/usr/bin:/bin", GEETORUS_GITHUB_AUTH_MODE: "host",
      OPENAI_API_KEY: "must-not-cross", DATABASE_URL: "must-not-cross", GEETORUS_API_KEY: "must-not-cross",
      GEETORUS_GITHUB_HOST_HOME: "/legacy", GH_CONFIG_DIR: "/legacy/.config/gh",
      SSH_AUTH_SOCK: "/agent/socket", GEETORUS_GIT_METADATA_ROOTS: '["/repo/.git","/repo/.git"]',
    }).join("\n");
    const allowlist = JSON.parse(args.split("\n").find((arg) => arg.startsWith("shell_environment_policy.include_only="))!.split("=", 2)[1]!);
    expect(allowlist).toEqual(["GH_CONFIG_DIR", "HOME", "GEETORUS_GITHUB_AUTH_MODE", "GEETORUS_GITHUB_HOST_HOME", "GEETORUS_GIT_METADATA_ROOTS", "PATH", "SSH_AUTH_SOCK"]);
    expect(args).not.toContain("must-not-cross");
    expect(args).not.toContain("OPENAI_API_KEY");
    expect(args).not.toContain("DATABASE_URL");
    expect(args).not.toContain("GEETORUS_API_KEY");
  ...
  it("keeps automatic execution inside the workspace without host credentials and with normal network access", () => {
    ...
    expect(serialized).toContain('shell_environment_policy.include_only=["LANG","GEETORUS_RUNNER_NETWORK_ACCESS","PATH"]');
  ...
  it("inherits only projected GitHub credentials without serializing their values", () => {
    const args = createIsolatedCodexAppServerArgs({
      ...
      GEETORUS_GIT_TOKEN: "must-remain-in-process-environment",
  ...
  it("isolates managed launcher profiles and never serializes broker capabilities or host API credentials", () => {
    const serialized = createIsolatedCodexAppServerArgs({
      ...
      GEETORUS_GITHUB_LAUNCHER_DIR: "/runtime/run-B",
      GEETORUS_GITHUB_BROKER_TOKEN: "private-run-capability",
      GEETORUS_GITHUB_BRIDGE_TOKEN: "private-bridge-capability",
      GEETORUS_API_KEY: "forbidden-agent-token",
    ...
    expect(serialized).toContain('"GEETORUS_GITHUB_BRIDGE_TOKEN"');
    expect(serialized).not.toContain("GEETORUS_API_KEY");
  ...
  it("uses a read-only permission profile for plan mode", () => {
    expect(createSecuredCodexThreadParams("/workspace", "plan")).toMatchObject({
      cwd: "/workspace",
      permissions: "geetorus-runner-workspace-read-only",
  ...
  it("uses the outer sandbox for default-mode commands only when the controller authorizes it", () => {
    const source = { GEETORUS_RUNNER_EXTERNAL_SANDBOX: "1", GEETORUS_RUNNER_NETWORK_ACCESS: "enabled" };
    ...
    expect(serializedExternalArgs).toContain(
      'default_permissions="geetorus-runner-external-sandbox"',
    );
    expect(serializedExternalArgs).toContain(
      'permissions.geetorus-runner-external-sandbox.filesystem={":root"="write"}',
    );
    expect(serializedExternalArgs).toContain(
      "permissions.geetorus-runner-external-sandbox.network.enabled=true",
    );
    expect(
      createSecuredCodexThreadParams(
        "/workspace",
      ...
      permissions: "geetorus-runner-external-sandbox",
    });
    expect(
      createSecuredCodexThreadParams(
        "/workspace",
