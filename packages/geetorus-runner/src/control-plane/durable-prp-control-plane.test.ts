...
import { digestGeetorusSemanticContent } from "../semantic-tools/receipts.js";
import {
...
function renewalRequest(client: AuthenticatedClient, expiresAt: number): Record<string, unknown> {
  return {
    protocol: "geetorus.runner", version: client.welcome.version,
...
it("pins the ACPX launch profile in runner startup arguments and restarts", () => {
  ...
  const handle = spawnRunner({
    connection: { mode: "connect", connectUrl: "ws://127.0.0.1:43127" },
    stateDirectory: "/tmp/geetorus-runner-test",
    identity,
    ticket: "bootstrap-ticket",
    maxOutboxBytes: 256 * 1024,
    p0ReserveBytes: 64 * 1024,
...
it("preserves an explicit OpenCode permission mode at the runner spawn boundary", () => {
  ...
  spawnRunner({
    ...
    environment: {
      ...
      GEETORUS_OPENCODE_RUNTIME_DIR: "/runner/opencode",
  ...
  expect(launches[0]!.environment.GEETORUS_API_KEY).toBeUndefined();
  expect(launches[0]!.environment.NODE_OPTIONS).toBeUndefined();
  expect(launches[0]!.environment.GEETORUS_OPENCODE_COMMAND).toBeUndefined();
});
...
it("preserves only bounded GitHub credential projection at the runner spawn boundary", () => {
  ...
  spawnRunner({
    ...
    environment: {
      ...
      GEETORUS_GIT_TOKEN: "github-token",
...
it("preserves the controller-selected ACPX provider package root", () => {
  ...
  expect(launches).toHaveLength(1);
  expect(launches[0]!.environment.GEETORUS_ACPX_PROVIDER_PACKAGE_ROOT).toBe(
    "/verified/provider-pack",
  );
  expect(
    launches[0]!.environment.GEETORUS_ACPX_PROVIDER_PACKAGE_MANIFEST,
...
function credentialMaterial(token: string): {
  ...
  const authKey = domainDigest("geetorus-runner-auth-key-v1", [bytes]);
  return {
    credentialId: `sha256:${domainDigest("geetorus-runner-credential-id-v1", [bytes]).toString("hex")}`,
    authKey,
  };
}
...
async function authenticate(
  ...
  const binding = domainDigest("geetorus-runner-session-binding-v1", [
    Buffer.from(canonicalChallenge),
...
function secureAad(
  ...
  return Buffer.from(
    `geetorus.runner.secure-frame.v1\0${client.sessionId}\0${direction}\0${counter}`,
  );
}
...
function sendSecure(
  ...
  const key = domainHmac(
    client.authKey,
    "geetorus-runner-client-to-core-key-v1",
    [binding],
  );
  ...
  sendMaskedJson(client.socket, {
    schema: "geetorus.runner.secure-frame.v1",
...
async function receiveSecure(
  ...
  const key = domainHmac(
    client.authKey,
    "geetorus-runner-core-to-client-key-v1",
    [binding],
  );
    ...
    payload: {
      ...
      payload: {
        semantic_tool: {
          ...
          idempotencyKey: null,
          content: {
            digest: digestGeetorusSemanticContent({}),
...
describe.sequential("DurablePrpControlPlane", () => {
  it.each(["pending_first", "all_pending", "completed_first"] as const)(
    ...
    async (mode) => {
      const root = mkdtempSync(
        resolve(tmpdir(), "geetorus-prp-semantic-window-"),
      );
      ...
      if (mode === "completed_first") {
        ...
        core.store.state.commands.push({
          schema: "geetorus.prp.command.v1",
          commandId,
          controllerSeq: 1,
  ...
  it.each([false, true])(
    ...
    async (throwingObserver) => {
      const root = mkdtempSync(
        resolve(tmpdir(), "geetorus-prp-transport-integrity-"),
      );
  ...
  it("latches an authenticated semantic integrity fault without ACK or dispatch and still permits suspension", async () => {
    const root = mkdtempSync(resolve(tmpdir(), "geetorus-prp-integrity-"));
  ...
  it.each([
    ...
    async (reason) => {
      const root = mkdtempSync(
        resolve(tmpdir(), "geetorus-prp-typed-commit-integrity-"),
      );
  ...
  it("latches an authenticated replay conflict against exactly committed bytes", async () => {
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-replay-integrity-"),
    );
    const onProtocolIntegrityError = vi.fn();
    const onCommittedEvent = vi.fn(async () => undefined);
    const onSemanticToolInput = vi.fn(
    ...
    try {
      ...
      (semantic.content as Record<string, unknown>).digest =
        digestGeetorusSemanticContent(semantic.input);
  ...
  it.each([
    ...
    async (mismatch) => {
      const root = mkdtempSync(
        resolve(tmpdir(), "geetorus-prp-unbound-integrity-"),
      );
      const onProtocolIntegrityError = vi.fn();
      const onCommittedEvent = vi.fn(async () => undefined);
      const onSemanticToolInput = vi.fn(async () => ({ result: { ok: true } }));
  ...
  it("does not dispatch an earlier in-flight commit after a replacement connection proves an integrity fault", async () => {
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-integrity-commit-race-"),
    );
  ...
  it("does not turn a missing semantic handler into an integrity fault", async () => {
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-integrity-no-handler-"),
    );
    const onProtocolIntegrityError = vi.fn();
    const core = new DurablePrpControlPlane({
      stateDirectory: root,
  ...
  it.each([
    ...
    async (commitFailure) => {
      const root = mkdtempSync(
        resolve(tmpdir(), "geetorus-prp-transient-commit-"),
      );
      const onProtocolIntegrityError = vi.fn();
  ...
  it("requires a fresh bootstrap to replace a v1 lease with v2 on the same authority", async () => {
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-version-bootstrap-"),
    );
    const core = new DurablePrpControlPlane({
      stateDirectory: root,
      identity,
  ...
  it("exchanges a one-use bootstrap for a run-bound reconnect lease", async () => {
    const root = mkdtempSync(resolve(tmpdir(), "geetorus-prp-auth-"));
  ...
  it("recovers one semantic call from the durable event after a coordinator restart", async () => {
    const root = mkdtempSync(resolve(tmpdir(), "geetorus-prp-recovery-"));
    let firstCalls = 0;
  ...
  it("does not acknowledge an event before the caller commits it", async () => {
    const root = mkdtempSync(resolve(tmpdir(), "geetorus-prp-commit-order-"));
    const first = new DurablePrpControlPlane({
      stateDirectory: root,
      identity,
      expectedRunnerVersion,
  ...
  it("keeps a recovered runner attached when it reports an indeterminate command", async () => {
    const root = mkdtempSync(resolve(tmpdir(), "geetorus-prp-indeterminate-"));
    const controlPlane = new DurablePrpControlPlane({
      stateDirectory: root,
      identity,
      expectedRunnerVersion,
  ...
  it("retains an old warm attach result replay lane when its result ACK is lost after core rotation", async () => {
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-attach-result-ack-loss-"),
    );
    const controlPlane = new DurablePrpControlPlane({
      stateDirectory: root,
      identity,
  ...
  it.each([
    ...
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-attach-transition-"),
    );
  ...
  it("fails closed after a warm receipt rename when parent directory fsync fails", async () => {
    const root = mkdtempSync(
      resolve(tmpdir(), "geetorus-prp-transition-fsync-"),
    );
  ...
  it.each(["nonparticipant", "no-capability"] as const)(
    ...
    async (mode) => {
      const root = mkdtempSync(
        resolve(tmpdir(), "geetorus-prp-transition-held-auth-"),
      );
      let release!: () => void;
      let entered!: () => void;
      const gate = new Promise<void>((resolveGate) => {
  ...
  it("acknowledges terminal command results after persisting them", async () => {
    const root = mkdtempSync(resolve(tmpdir(), "geetorus-prp-terminal-ack-"));
    const controlPlane = new DurablePrpControlPlane({
      stateDirectory: root,
      identity,
      expectedRunnerVersion,
