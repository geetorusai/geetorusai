...
const protocol = "geetorus.runner";
const protocolMinVersion = 1;
const protocolVersion = 2;
const secureFrameSchema = "geetorus.runner.secure-frame.v1";
const websocketGuid = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
const coreStateSchema = "geetorus.runner.durable.control-plane-state.v1";
...
function credentialMaterial(token: string): {
  ...
  return {
    ...
    authKey: domainDigest("geetorus-runner-auth-key-v1", [bytes]),
  };
}
...
function warmTransitionReceipt(
  ...
  const boundary = command.payload.geetorusNextAuthority;
  if (
  ...
  const body = {
    schema: "geetorus.runner.warm-transition.v1" as const,
...
function isStoredCoreState(
  ...
  if (
    !commands.every(
      (command, index) =>
        isRecord(command) &&
        (command.schema === "geetorus.prp.command.v1" ||
          command.schema === "geetorus.prp.command.v2") &&
...
function warmTransitionRecoveryProof(input: WarmTransitionInspectionInput): {
  ...
  try {
    ...
    if (
      ...
      !isRecord(runner) ||
      runner.schema !== "geetorus.runner.durable.state.warm-transition.v1"
    )
      return null;
...
function createSecureChannel(
  ...
  const binding = domainDigest("geetorus-runner-session-binding-v1", parts);
  return {
    sendKey: domainHmac(authKey, "geetorus-runner-core-to-client-key-v1", [
      binding,
    ]),
    receiveKey: domainHmac(authKey, "geetorus-runner-client-to-core-key-v1", [
      binding,
    ]),
    sendCounter: 0n,
    receiveCounter: 0n,
...
export class DurablePrpControlPlane {
  ...
  rotateRunIdentity(
    ...
    if (
      ...
      const { geetorusNextAuthority: _boundary, ...template } =
    ...
    if (transition) {
      ...
      if (runAttachTemplate !== undefined) {
        const { geetorusNextAuthority: _boundary, ...expectedTemplate } =
  ...
  queueCommand(
    ...
    if (
      type === "run.attach" &&
      payload.geetorusNextAuthority !== undefined &&
  ...
  handleUpgrade(
    ...
    const requestPath = new URL(request.url ?? "/", "http://geetorus.invalid")
      .pathname;
  ...
  /** Attach either an accepted inbound WebSocket or a Geetorus-opened peer. */
      ...
      if (completed) {
        ...
        !this.#store.state.commands.some(
          (command) =>
            command.status === "pending" &&
            command.type === "run.attach" &&
            command.payload.geetorusNextAuthority !== undefined,
        )
      )
        return null;
    }
...
const runnerExplicitProviderEnvironmentKeys = [
  ...
  "GEETORUS_ACPX_PROVIDER_PACKAGE_ROOT",
...
function runnerEnvironment(
  ...
  const environment: NodeJS.ProcessEnv = {
    GEETORUS_RUNNER_BOOTSTRAP_TICKET: ticket,
  };
