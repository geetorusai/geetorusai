...
import { parseGeetorusQuestionSet } from "./contracts/question-set.js";
import {
  ...
  const consumer = (async () => {
    ...
    while (true) {
      ...
      if (
        ...
        try {
          parseGeetorusQuestionSet(request.input);
      ...
      if (governedResult !== null && !isTurnTerminal(event) && !sessionGoalObserved) {
        ...
        return settleDurableResult(
          event,
          governedResult,
          "Geetorus parked this turn on a durable governed interaction.",
        );
      }
...
/**
 * Package-owned normalized session loop. Geetorus supplies persistence and
        ...
        if (options.sessionGoalControl) {
          ...
          if (dispositionOnlyRecovery && !effectFreeInitialAcpxTurn) {
            modelEnvelope.task.prompt = [
              "Geetorus semantic-result recovery for a prior completed provider turn.",
              ...
              "Use the existing session context to invoke exactly one geetorus_finish or geetorus_block with the accurate current disposition, then stop without additional user-facing prose.",
            ].join("\n");
          }
