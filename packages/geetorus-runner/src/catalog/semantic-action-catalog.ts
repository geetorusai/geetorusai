...
const stringArray = (description: string): GeetorusJsonSchema => ({
  type: "array",
  description,
...
const object = (
  properties: Readonly<Record<string, GeetorusJsonSchema>> = {},
...
const openObject: GeetorusJsonSchema = {
  type: "object",
...
function descriptor(input: DescriptorInput): GeetorusSemanticActionDescriptor {
  return {
    schema: "geetorus.semantic-action.v1",
...
const descriptors: readonly GeetorusSemanticActionDescriptor[] = [
 ...
 */
export const GEETORUS_SEMANTIC_ACTION_CATALOG = Object.freeze([
  ...byId.values(),
]);

export function geetorusSemanticAction(
  operationId: string,
): GeetorusSemanticActionDescriptor | undefined {
  return byId.get(operationId as GeetorusSemanticActionId);
}

export function canonicalGeetorusSemanticActionCatalog(): string {
  return `${JSON.stringify(sortKeys(GEETORUS_SEMANTIC_ACTION_CATALOG), null, 2)}\n`;
}
