...
import { GEETORUS_PROTOCOL_ACTIONS } from "../protocol-actions/index.js";

describe("canonical Geetorus protocol action contracts", () => {
  const ajv = new Ajv2020({ allErrors: true, allowUnionTypes: true, strict: false });

  it.each(GEETORUS_PROTOCOL_ACTIONS.map((action) => [action.id, action] as const))(
