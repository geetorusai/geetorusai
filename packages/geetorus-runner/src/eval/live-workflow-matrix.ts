...
export const RUNNER_LIVE_CANDIDATE_SCHEMA =
  "geetorus.runner.live-eval-candidate.v1" as const;
export const RUNNER_LIVE_SCHEDULE_SCHEMA =
  "geetorus.runner.live-eval-schedule.v1" as const;
...
export function parseRunnerLiveCampaignCostLimit(
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(
      "GEETORUS_EVAL_MAX_CAMPAIGN_COST_USD must be a positive finite number",
    );
  }
  return parsed;
}
