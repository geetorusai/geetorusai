...
export const RUNNER_WORKFLOW_REPORT_SCHEMA = "geetorus.runner.workflow-eval-report.v1" as const;
...
export function renderRunnerWorkflowJUnit(report: RunnerWorkflowEvalReport): string {
  ...
  return [
    ...
    `<testsuite name="geetorus-runner-workflows" tests="${report.results.length}" failures="${failures}" skipped="${skipped}">`,
    ...cases,
    "</testsuite>",
    "",
  ].join("\n");
