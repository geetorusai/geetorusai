...
function promptFor(evalCase: RunnerWorkflowEvalCase): string {
  const common =
    "Use only the typed Geetorus tools. Do not call REST APIs. Reply with concise user-facing prose after the final tool result.";
...
export async function executeLiveRunnerWorkflow(input: {
  ...
  const runtimeRoot = await mkdtemp(
    join(tmpdir(), "geetorus-runner-live-eval-"),
  );
