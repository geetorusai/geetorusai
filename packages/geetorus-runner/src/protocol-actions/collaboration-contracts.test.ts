...
import { GEETORUS_COLLABORATION_PROTOCOL_ACTIONS } from "./collaboration.js";

describe("collaboration Geetorus protocol action contracts", () => {
  const ajv = new Ajv2020({ allErrors: true, allowUnionTypes: true, strict: false });

  it.each(GEETORUS_COLLABORATION_PROTOCOL_ACTIONS.map((action) => [action.id, action] as const))(
