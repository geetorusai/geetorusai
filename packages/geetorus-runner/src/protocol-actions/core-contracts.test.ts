...
import { GEETORUS_CORE_PROTOCOL_ACTIONS } from "./core.js";

describe("core Geetorus protocol action contracts", () => {
  const ajv = new Ajv2020({ allErrors: true, allowUnionTypes: true, strict: false });

  it.each(GEETORUS_CORE_PROTOCOL_ACTIONS.map((action) => [action.id, action] as const))(
