...
const INTERACTION_RESULT_SCHEMA = "geetorus.semantic-interaction-result.v1";
 ...
 *
 * Both the clean-room chat and the scenario path route every Geetorus
 * operation through the in-process mock port, so "no request reached a real
 * Geetorus API" is a fact the session already carries. Projecting it into the
...
function networkGuardRecord(
  ...
  const { realGeetorusRequests, childGeetorusEnvironmentKeys } = snapshot.networkEvidence;
  return {
    id: `network-guard-${snapshot.sessionId}`,
    turnId: "turn-0",
    category: "session",
    outcome: realGeetorusRequests === 0 ? "no_real_geetorus_request" : "real_geetorus_request",
    reason:
      `Real Geetorus API requests: ${realGeetorusRequests}. ` +
      `Child GEETORUS_* environment keys: ${
        childGeetorusEnvironmentKeys.length === 0
          ? "none"
          : childGeetorusEnvironmentKeys.join(", ")
      }.`,
