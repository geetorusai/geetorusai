...
import type { GeetorusWorkspaceDiff } from "../live/workspace-diff.js";
import type { GeetorusWorkspaceFileReference } from "../live/workspace-file-reference.js";
...
export const CAPABILITY_ISSUE_THREAD_VIEW_SCHEMA = "geetorus.capability.issue-thread-view.v1" as const;
...
export interface CapabilityThreadLink {
  label: string;
  /** In-explorer route only. Real Geetorus URLs never appear (§11). */
  href: string;
}
...
export type CapabilityThreadItem =
  ...
  | {
      ...
      id: string;
      at: string;
      changeSet: GeetorusWorkspaceDiff;
    }
  | {
