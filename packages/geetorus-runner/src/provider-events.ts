...
export function canonicalOpenCodeDisplayToolName(
  ...
  // OpenCode exposes MCP tools as `<server>_<tool>`. Geetorus's semantic
  // tools already carry the geetorus prefix, producing names such as
  // `geetorus_geetorus_finish` in native events. Keep the transport-native
  // name in the raw trace, while presenting the stable protocol operation.
  while (name.startsWith("geetorus_geetorus_")) {
    name = name.slice("geetorus_".length);
  }
  return name;
}
