import type { GeetorusSemanticActionDescriptor } from "../catalog/semantic-action-types.js";
import type {
  GeetorusSemanticAuthorizationDecision,
  GeetorusSemanticAuthorizationPhase,
...
export function decideGeetorusSemanticAuthorization(
  descriptor: GeetorusSemanticActionDescriptor,
  context: GeetorusSemanticRunContext,
  phase: GeetorusSemanticAuthorizationPhase,
...
function validAuthorityContext(
  context: unknown,
  requestedRunId: string,
): context is GeetorusSemanticRunContext {
  if (
    !isRecord(context) ||
