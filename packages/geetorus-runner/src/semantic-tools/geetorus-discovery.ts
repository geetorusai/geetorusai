import { GEETORUS_SEMANTIC_ACTION_CATALOG } from "../catalog/semantic-action-catalog.js";
import type {
...
const NAMESPACE: Readonly<Record<GeetorusSemanticActionId, string>> =
  Object.freeze({
...
export function geetorusSemanticActionNamespace(
  operationId: GeetorusSemanticActionId,
): string {
  return NAMESPACE[operationId];
}

export function projectGeetorusSemanticTools(input: {
  readonly runId: string;
  readonly context: GeetorusSemanticRunContext;
...
export function discoverGeetorusSemanticTools(input: {
  readonly runId: string;
  readonly context: GeetorusSemanticRunContext;
  readonly boundOperationIds: ReadonlySet<GeetorusSemanticActionId>;
  ...
  const candidates = authorizedBoundDescriptors({
    ...
    .filter(
      (descriptor) =>
        namespace === undefined ||
        geetorusSemanticActionNamespace(descriptor.operationId) === namespace,
    )
    .map((descriptor) => ({
      descriptor,
  ...
  return deepFreeze({
    schema: "geetorus.semantic-discovery.v1",
    query: input.query,
...
export function toGeetorusSemanticToolDefinition(
...
function authorizedBoundDescriptors(input: {
  ...
  return GEETORUS_SEMANTIC_ACTION_CATALOG.filter(
    (descriptor) =>
...
function scoreDescriptor(
  ...
  const namespace = geetorusSemanticActionNamespace(descriptor.operationId);
  const haystack =
