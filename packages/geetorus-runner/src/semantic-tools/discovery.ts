...
export interface CapabilityDiscoveryResult {
  readonly schema: "geetorus.capability-discovery.v1";
...
export const CAPABILITY_DISCOVERY_GATEWAY_DEFINITIONS = Object.freeze([{
  ...
  description: `Search authorized optional Geetorus capabilities. Namespaces: ${CAPABILITY_DISCOVERY_NAMESPACES.map((item) => `${item.name} (${item.description})`).join("; ")}`,
  inputSchema: {
  ...
  outputSchema: {
    type: "object", properties: {
      schema: { const: "geetorus.capability-discovery.v1" },
  ...
  description: "Invoke one optional Geetorus operation returned by discover_capabilities. Authority is rechecked at invocation.",
  inputSchema: {
    type: "object", properties: {
...
export function discoverCapabilityDefinitions(
  ...
  return Object.freeze({
    schema: "geetorus.capability-discovery.v1",
    query,
