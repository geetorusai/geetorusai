...
import type { GeetorusSemanticActionBinding } from "./types.js";
import { GeetorusSemanticDispatcher } from "./geetorus-dispatcher.js";
import type {
...
import { GEETORUS_SEMANTIC_REDACTED } from "./redaction.js";
import {
  validatePrpEvent,
  type PrpEvent,
...
describe("run-scoped semantic tool authority", () => {
  it("projects only bound and currently authorized actions", async () => {
    ...
    const dispatcher = new GeetorusSemanticDispatcher({
  ...
  it("redacts binding output and emits schema-valid digest-only receipts", async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      value: { accessToken: GEETORUS_SEMANTIC_REDACTED },
      duplicate: false,
    });
  ...
  it("fails closed when durable storage or binding metadata is invalid", async () => {
    let executions = 0;
    const unavailableStore = new GeetorusSemanticDispatcher({
      contextProvider: () => runContext(),
      idempotencyStore: {
        claim: () => {
    ...
    const invalidBinding = new GeetorusSemanticDispatcher({
      contextProvider: () => runContext(),
      bindings: [
        {
          operationId: "get_task_context",
  ...
  it("denies cross-company contexts and malformed protocol identities", async () => {
    const crossCompany = new GeetorusSemanticDispatcher({
    ...
    const malformedAuthority = new GeetorusSemanticDispatcher({
      contextProvider: () =>
...
function binding(
  operationId: GeetorusSemanticActionBinding["operationId"],
...
class MemoryIdempotencyStore implements GeetorusSemanticIdempotencyStore {
  ...
  claim(input: {
    scope: string;
    operationId: GeetorusSemanticStoredOutcome["operationId"];
    inputDigest: string;
  }): GeetorusSemanticIdempotencyClaim {
  ...
  complete(token: string, outcome: GeetorusSemanticStoredOutcome): void {
  ...
  recover(token: string, outcome: GeetorusSemanticStoredOutcome): void {
    this.recoveryCount += 1;
    this.#storeOutcome(token, outcome);
  }

  #storeOutcome(token: string, outcome: GeetorusSemanticStoredOutcome): void {
