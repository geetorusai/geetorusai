...
import {
  loadGeetorusNativeExecutionFixture,
  geetorusNativeExecutionFixtureUrl,
  geetorusNativeExecutionSchemaUrl,
  parseGeetorusNativeExecution,
} from "./native-execution.js";
import { GEETORUS_RUNNER_BUILD_METADATA } from "./build-metadata.js";
import { serializeCapabilityGeneratedSemanticContracts } from "../semantic-tools/provider-neutral.js";

describe("geetorus-runner/native-execution/v1", () => {
  it("refreshes a stale seeded catalog and its manifest with one semantic generator invocation", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-semantic-generator-"));
    ...
    try {
      ...
      await writeFile(join(root, "dist/evals/build-metadata.js"),
        `export const GEETORUS_RUNNER_BUILD_METADATA = ${JSON.stringify(GEETORUS_RUNNER_BUILD_METADATA)};`);
      ...
      expect(JSON.parse(await readFile(fixturePath, "utf8")).runner.catalogSha256)
        .toBe(GEETORUS_RUNNER_BUILD_METADATA.semanticCatalog.sha256);
  ...
  it("keeps the shipped seeded fixture valid against the published JSON Schema", async () => {
    ...
    const schema = JSON.parse(
      await readFile(geetorusNativeExecutionSchemaUrl, "utf8"),
    ) as Record<string, unknown>;
    const validate = ajv.compile(schema);
    const fixture = JSON.parse(
      await readFile(geetorusNativeExecutionFixtureUrl, "utf8"),
    ) as unknown;
  ...
  it("loads the shipped seeded attempt and preserves nested additive fields", async () => {
    const bundle = await loadGeetorusNativeExecutionFixture();
    ...
    expect(bundle.runner.package).toEqual(GEETORUS_RUNNER_BUILD_METADATA.package);
    expect(bundle.runner.catalogSha256).toBe(
      GEETORUS_RUNNER_BUILD_METADATA.semanticCatalog.sha256,
    );
    ...
    expect(parseGeetorusNativeExecution(extended)).toMatchObject({
  ...
  it("rejects unknown native schema versions", async () => {
    const bundle = await loadGeetorusNativeExecutionFixture();
    expect(() => parseGeetorusNativeExecution({ ...bundle, schema: "geetorus-runner/native-execution/v2" }))
      .toThrow(/unsupported schema/);
  });

  it("rejects a terminal that conflicts with the event stream", async () => {
    const bundle = await loadGeetorusNativeExecutionFixture();
    expect(() => parseGeetorusNativeExecution({
      ...bundle,
  ...
  it("rejects semantic indexes that reinterpret or omit PRP tool events", async () => {
    ...
    expect(() => parseGeetorusNativeExecution(mismatched))
    ...
    expect(() => parseGeetorusNativeExecution(mismatchedCorrelation))
  ...
  it("rejects a non-interrupted terminal bundle with an unresolved semantic call", async () => {
    ...
    expect(() => parseGeetorusNativeExecution(unresolved))
  ...
  it("allows an unresolved semantic call only for interrupted or cancelled states", async () => {
    for (const terminalState of [
      ...
      expect(parseGeetorusNativeExecution(unfinished)).toMatchObject({
  ...
  it("rejects unordered native events", async () => {
    ...
    expect(() => parseGeetorusNativeExecution({ ...bundle, events }))
    ...
    expect(() => parseGeetorusNativeExecution({ ...bundle, events: duplicateIds }))
