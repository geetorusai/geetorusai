...
import {
  ...
  GEETORUS_RUNNER_EVAL_INTEGRATION_SCHEMA,
} from "./build-metadata.js";
import type { GeetorusRunnerdBuildMetadata } from "./runnerd-artifact.js";
...
export type GeetorusRunnerEvalCompatibilityIssueCode =
...
export interface GeetorusRunnerEvalCompatibilityIssue {
  code: GeetorusRunnerEvalCompatibilityIssueCode;
...
export interface GeetorusRunnerEvalCompatibilityRequirement {
  consumer: string;
  packageVersion: string;
  runnerd: GeetorusRunnerdBuildMetadata;
...
export interface GeetorusRunnerEvalCompatibilityReceipt {
  schema: typeof GEETORUS_RUNNER_EVAL_INTEGRATION_SCHEMA;
  consumer: string;
  packageVersion: string;
...
export class GeetorusRunnerEvalCompatibilityError extends Error {
  readonly code = "geetorus_runner_eval_incompatible" as const;

  constructor(
    readonly consumer: string,
    readonly issues: readonly GeetorusRunnerEvalCompatibilityIssue[],
  ) {
    super(
      `Geetorus runner eval compatibility check failed for ${consumer}: ${issues
        .map((issue) => `${issue.code}: ${issue.message}`)
        .join("; ")}`,
    );
    this.name = "GeetorusRunnerEvalCompatibilityError";
 ...
 */
export function assertGeetorusRunnerEvalCompatibility(
  requirement: GeetorusRunnerEvalCompatibilityRequirement,
): GeetorusRunnerEvalCompatibilityReceipt {
  const expected = GEETORUS_RUNNER_BUILD_METADATA;
  const issues: GeetorusRunnerEvalCompatibilityIssue[] = [];
  const issue = (
    code: GeetorusRunnerEvalCompatibilityIssueCode,
    component: GeetorusRunnerEvalCompatibilityIssue["component"],
  ...
  if (
    ...
    issue(
      ...
      "consumer and runnerd must both emit geetorus-runner/native-execution/v1",
    );
  }
  ...
  if (issues.length > 0) {
    throw new GeetorusRunnerEvalCompatibilityError(requirement.consumer, issues);
  }

  return {
    schema: GEETORUS_RUNNER_EVAL_INTEGRATION_SCHEMA,
