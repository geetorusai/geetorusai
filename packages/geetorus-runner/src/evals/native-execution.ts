...
import { GEETORUS_RUNNER_NATIVE_EXECUTION_SCHEMA } from "./build-metadata.js";
...
export const geetorusNativeExecutionSchemaUrl = new URL(
  "../../protocol/schemas/native-execution.schema.json",
  import.meta.url,
);
export const geetorusNativeExecutionFixtureUrl = new URL(
...
export interface GeetorusNativeExecutionV1 {
  schema: typeof GEETORUS_RUNNER_NATIVE_EXECUTION_SCHEMA;
...
export class GeetorusNativeExecutionError extends Error {
  readonly code = "geetorus_native_execution_invalid" as const;
  ...
  constructor(
    ...
    super(`${path}: ${message}`);
    this.name = "GeetorusNativeExecutionError";
  }
}

function fail(path: string, message: string): never {
  throw new GeetorusNativeExecutionError(message, path);
}
 ...
 */
export function parseGeetorusNativeExecution(
  value: unknown,
): GeetorusNativeExecutionV1 {
  const bundle = record(value, "bundle");
  if (bundle.schema !== GEETORUS_RUNNER_NATIVE_EXECUTION_SCHEMA) {
    fail(
      "bundle.schema",
      `unsupported schema ${String(bundle.schema)}; expected ${GEETORUS_RUNNER_NATIVE_EXECUTION_SCHEMA}`,
    );
  }
  ...
  if (terminal.schema !== "geetorus.prp.terminal.v1") {
    fail("bundle.terminal.schema", "must be geetorus.prp.terminal.v1");
  }
  ...
  const calls = array(semanticTools.calls, "bundle.semanticTools.calls").map((entry, index) => {
    ...
    return parsed as GeetorusNativeExecutionV1["semanticTools"]["calls"][number];
  });
  const results = array(semanticTools.results, "bundle.semanticTools.results").map((entry, index) => {
    ...
    return parsed as GeetorusNativeExecutionV1["semanticTools"]["results"][number];
  });
  const denials = array(semanticTools.denials, "bundle.semanticTools.denials").map((entry, index) => {
    ...
    return parsed as GeetorusNativeExecutionV1["semanticTools"]["denials"][number];
  });
  ...
  return {
    ...
    schema: GEETORUS_RUNNER_NATIVE_EXECUTION_SCHEMA,
...
function semanticToolEnvelope(
  ...
  if (
    envelope.schema !== "geetorus.prp.semantic_tool.v1"
    || envelope.schemaVersion !== 1
    || envelope.phase !== phase
  ) {
    fail(
      `${path}.payload.semantic_tool`,
      `must be a geetorus.prp.semantic_tool.v1 ${phase} envelope`,
    );
  }
...
export async function loadGeetorusNativeExecutionFixture(
  url: URL = geetorusNativeExecutionFixtureUrl,
): Promise<GeetorusNativeExecutionV1> {
  const value = JSON.parse(await readFile(url, "utf8")) as unknown;
  return parseGeetorusNativeExecution(value);
}
