...
// GEETORUS_FINAL_BURST_BENCHMARK=1 pnpm exec vitest run src/live/runnerd-final-output-burst.benchmark.test.ts
// GEETORUS_FINAL_BURST_BINARY optionally selects an isolated comparison build;
...
const enabled = process.env.GEETORUS_FINAL_BURST_BENCHMARK === "1";
const repetitions = Number(
  process.env.GEETORUS_FINAL_BURST_REPETITIONS ?? "1",
);
if (
  enabled &&
...
it.skipIf(!enabled).each(cases)(
  ...
  async ({ deltaCount, repeat }) => {
    const root = await mkdtemp(
      join(tmpdir(), "geetorus-final-burst-benchmark-"),
    );
    try {
      ...
      const staged = process.env.GEETORUS_FINAL_BURST_BINARY
        ? resolve(process.env.GEETORUS_FINAL_BURST_BINARY)
        : defaultCapabilityRunnerdBinary();
      const runnerBinary = join(root, "geetorus-runnerd");
      ...
      try {
        ...
        process.stdout.write(
          `FINAL_BURST_BENCHMARK ${JSON.stringify({
            schema: "geetorus.final_output_burst_benchmark.v1",
            deltaCount,
            repeat,
            binarySha256,
            providerEmissionMs:
