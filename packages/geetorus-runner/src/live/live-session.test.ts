...
function fakeTransportFactory(state: FakeProviderState): CapabilityLiveTransportFactory {
  return (options) => {
    const evidence = {
      ...
      diagnostics: ["geetorus-runnerd: capability codex proxy started"],
    };
...
describe("Capability live runnerd and Codex session", () => {
  ...
  it("turns an assistant local-file link into a verified dynamic-chat file card record", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-live-file-reference-"));
  ...
  it("records runner-verified file changes for a dynamic chat turn", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-live-workspace-"));
  ...
  it("returns typed mock results to the same multi-turn Codex thread without Geetorus network calls", async () => {
    const state = providerState();
