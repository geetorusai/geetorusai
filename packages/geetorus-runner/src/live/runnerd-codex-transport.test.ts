...
const defaultCapabilityRunnerdBinary = () =>
  process.env.GEETORUS_ATTACH_TRANSITION_RUNNER ??
...
async function expectTurnStarted(
  ...
  for (let index = 0; index < 32; index += 1) {
    ...
    expect(next.value.method).toBe("geetorus/canonicalProviderEvent");
  }
  throw new Error(
        ...
        if (startupFailureProof) {
          ...
          for (const fact of facts) {
            ...
            expect(fact).toMatchObject({
              schema: "geetorus.provider_startup.v1",
...
it("preserves answer and internal wait descriptions in the serialized native tool catalog", () => {
  ...
  const finish = catalog.operations.find(
    (operation: { operationId: string }) =>
      operation.operationId === "geetorus_finish",
  );
...
it("rejects caller-selected local ACPX artifacts even when they are self-hashed", async () => {
  const directory = await mkdtemp(join(tmpdir(), "geetorus-acpx-authority-"));
...
it.each(["acpx-runtime-sidecar.cjs", "opencode-app-server-proxy.cjs"] as const)(
  ...
  async (artifact) => {
    const directory = await mkdtemp(
      join(tmpdir(), "geetorus-provider-artifact-"),
    );
...
it("keeps a self-rooted pnpm deployment inside its dependency authority", async () => {
  const deploymentRoot = await mkdtemp(
    join(tmpdir(), "geetorus-deployed-provider-root-"),
  );
...
it("keeps a scoped npm-installed package inside its portable dependency root", async () => {
  const deploymentRoot = await mkdtemp(
    join(tmpdir(), "geetorus-npm-provider-root-"),
  );
...
it("adds Codex-style turn updates only when collaboration instructions are enabled", () => {
  const base = "Base Geetorus instructions.";
...
it("preserves OpenCode runtime bindings when a durable runner is respawned", () => {
  const environment = createCapabilityRunnerdProviderEnvironment({
    ...
    options: {
      ...
      environment: {
        ...
        GEETORUS_API_KEY: "must-not-reach-runnerd",
  ...
  expect(environment).toMatchObject({
    GEETORUS_OPENCODE_PERMISSION_MODE: "deny",
    GEETORUS_OPENCODE_RUNTIME_DIR: "/isolated/session/opencode",
    GEETORUS_RUNNER_INSTANCE_ID: "runner-1",
    GEETORUS_RUN_ID: "run-1",
    GEETORUS_NORMALIZED_SESSION_ID: "session-1",
    GEETORUS_NATIVE_RUNTIME_CONTEXT_PATH: "/isolated/runtime-context.json",
    OPENROUTER_API_KEY: "test-provider-key",
  });
  expect(environment.HOME).toBeUndefined();
  expect(environment.CODEX_HOME).toBeUndefined();
  expect(environment.DATABASE_URL).toBeUndefined();
  expect(environment.GEETORUS_API_KEY).toBeUndefined();
  expect(environment.NODE_OPTIONS).toBeUndefined();
  expect(environment.GEETORUS_OPENCODE_COMMAND).toBeUndefined();
  ...
  expect(defaultPermissionEnvironment.GEETORUS_OPENCODE_PERMISSION_MODE).toBe(
    "ask",
  );
});
  ...
  const environment = createCapabilityRunnerdProviderEnvironment({
    ...
    options: {
      ...
      environment: {
        ...
        GEETORUS_API_KEY: "must-not-reach-provider",
      },
    },
    identity: {
      runnerInstanceId: "runner-1",
...
it("passes only the Anthropic credential to Claude Managed runnerd", () => {
  const environment = createCapabilityRunnerdProviderEnvironment({
    ...
    options: {
      ...
      environment: {
        ...
        GEETORUS_NATIVE_MCP_NAME: "geetorus",
        GEETORUS_NATIVE_MCP_URL: "https://geetorus.example/mcp",
        GEETORUS_NATIVE_MCP_TOKEN: "must-not-reach-provider",
        GEETORUS_API_KEY: "must-not-reach-provider",
  ...
  expect(environment.GEETORUS_NATIVE_MCP_NAME).toBeUndefined();
  expect(environment.GEETORUS_NATIVE_MCP_URL).toBeUndefined();
  expect(environment.GEETORUS_NATIVE_MCP_TOKEN).toBeUndefined();
  expect(environment.GEETORUS_API_KEY).toBeUndefined();
  expect(environment.DATABASE_URL).toBeUndefined();
});

it("uses file-backed AWS workload identity without forwarding access keys or Geetorus tokens", () => {
  const environment = createCapabilityRunnerdProviderEnvironment({
...
it.each([
  {
    ...
    denied: [
      ...
      "OPENAI_API_KEY",
      "CODEX_API_KEY",
      "GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET",
    ],
  },
  {
  ...
  ({ agent, allowed, denied }) => {
    const credentialEnvironment: Record<string, string> = {
      ...
      GEETORUS_ACPX_CODEX_AUTH_JSON_SECRET: "managed-codex-canary",
    };
    const environment = createCapabilityRunnerdProviderEnvironment({
      provider: "acpx",
      options: {
        ...
        environment: {
          ...
          GEETORUS_ACPX_PROVIDER_PACKAGE_ROOT: "/attacker/package-root",
          GEETORUS_ACPX_PROVIDER_PACKAGE_MANIFEST:
    ...
    expect(environment).toMatchObject({
      ...
      GEETORUS_NORMALIZED_SESSION_ID: "session-1",
      GEETORUS_NATIVE_RUNTIME_CONTEXT_PATH: "/isolated/runtime-context.json",
      GEETORUS_ACPX_PROVIDER_PACKAGE_ROOT: "/verified/provider-pack",
      GEETORUS_ACPX_PROVIDER_PACKAGE_MANIFEST:
...
it.each(["opencode", "acpx"] as const)(
  ...
  async (provider) => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-runner-plan-mode-"));
...
it("denies the isolated Codex home without denying a remote execution workspace", () => {
  const args = createRunnerdCodexAppServerArgs({
    ...
    codexHome:
      "/workspaces/task/.geetorus-runtime/geetorus-runner/sessions/session/filesystem/codex-home",
    readOnlyRoots: ["/usr/local"],
  });
  const serialized = args.join("\n");

  expect(serialized).toContain(
    '"/workspaces/task/.geetorus-runtime/geetorus-runner/sessions/session/filesystem/codex-home"="none"',
  );
...
it("rejects remote OpenCode before spawn when provider-pack paths are absent", async () => {
  const root = await mkdtemp(join(tmpdir(), "geetorus-runner-remote-pack-"));
  const { transport } = createCapabilityRunnerdCodexTransport({
    provider: "opencode",
    stateDirectory: root,
    runnerFilesystemRoot: "/workspaces/task/.geetorus-runtime/session",
  });
  try {
    await expect(
...
it("binds a durable semantic result to the active provider turn", () => {
  expect(
    rehydrateRunnerdResultNotification(
      { schema: "geetorus.run_result.v1", reportedWorkDisposition: "done" },
      "opened-thread-1",
      "provider-turn-1",
      "finish-1",
    ),
...
it.each([false, true])("binds goal turns through the full Codex harness (autonomous continuation: %s)", async (autocontinue) => {
  ...
  const driver = new CodexAppServerDriver({
    taskEnvelope: {
      schema: "geetorus.skillless_task.v1",
      ...
      constraints: [],
      expectedResultSchema: "geetorus.run_result.v1",
    },
...
it.each([
  ...
  const driver = new CodexAppServerDriver({
    ...
    environment: {
      ...
      GEETORUS_WORKSPACE_CWD: stateDirectory,
    },
    approvalPolicy: "never",
    transportFactory: () => bundle.transport,
  });
  ...
  bundle.transport.setServerRequestHandler(async (request) => {
    ...
    await bundle.transport.resolveRuntimeRequest?.({
      ...
      resolution: {
        action: "submit",
        response: {
          schema: "geetorus.question_response.v1",
          answers: {
...
it("captures exact provider frames and correlates Rust and TypeScript interpretation stages", async () => {
  ...
  const bundle = createCapabilityRunnerdCodexTransport({
    ...
    environment: {
      GEETORUS_PROVIDER_TRACE_PATH: tracePath,
      GEETORUS_PROVIDER_TRACE_MAX_BYTES: String(64 * 1024 * 1024),
    },
  });
  const driver = new CodexAppServerDriver({
    taskEnvelope: createCodexTaskEnvelope({
    ...
    environment: {
      PATH: process.env.PATH,
      HOME: hostHome,
      GEETORUS_WORKSPACE_CWD: traceDirectory,
    },
    approvalPolicy: "never",
    transportFactory: () => bundle.transport,
  });
...
it.each([
  ...
  ...[
    "attach-wait",
    ...(process.env.GEETORUS_ATTACH_TRANSITION_LEGACY_RUNNER
      ? ["attach-capability"]
      : []),
  ].map((recoveryFault) => ({
  ...
  async (testCase) => {
    ...
    const options = {
      ...
      prpIdentity: (() => {
        ...
        return {
          ...
          environmentLeaseId:
            process.env.GEETORUS_ATTACH_TRANSITION_FIXTURE_SCOPE ===
            "transient"
              ? runId
              : randomUUID(),
          runId,
    ...
    try {
      ...
      const fixtureOutput =
        process.env.GEETORUS_ATTACH_TRANSITION_FIXTURE_DIRECTORY;
      if (fixtureOutput && recoveryFault === "none") {
        const retainedArtifact = join(fixtureOutput, "geetorus-runnerd");
        ...
        await writeFile(
          ...
          JSON.stringify(
            {
              schema: "geetorus.test.warm-transition-fixture.v1",
              oldIdentity,
              newIdentity: desired,
              lossPoint,
              routed,
...
it("cold-restores a suspended provider session under its durable run binding", async () => {
  ...
  const options = {
    ...
    stateDirectory,
    environment: {
      GEETORUS_GITHUB_BROKER_TOKEN: "test-run-A-capability",
      GEETORUS_PROVIDER_TRACE_PATH: tracePath,
      GEETORUS_PROVIDER_TRACE_MAX_BYTES: String(64 * 1024 * 1024),
    },
    lifecyclePolicy: { mode: "per_turn" as const, idleTimeoutMs: null },
  ...
  const rotated = createCapabilityRunnerdCodexTransport({
    ...options,
    environment: { ...options.environment, GEETORUS_GITHUB_BROKER_TOKEN: "test-run-B-capability" },
  ...
  try {
    ...
    for await (const event of rotated.transport.notifications()) {
      if (event.method === "geetorus/runResult") break;
    }
...
it.each(["claude", "codex"] as const)("keeps the explicitly assigned gateway in the %s runner environment", (agent) => {
  const gateway = {
    GEETORUS_NATIVE_MCP_NAME: "geetorus-assigned",
    GEETORUS_NATIVE_MCP_URL: "http://127.0.0.1:3100/mcp/gateway",
    GEETORUS_NATIVE_MCP_TOKEN: "fixture-scoped-gateway-token-1234567890",
  };
  const environment = createCapabilityRunnerdProviderEnvironment({
    provider: "acpx",
    options: { provider: "acpx", acpxAgent: agent, environment: {
      PATH: "/bin", ...gateway, GEETORUS_API_KEY: "must-not-cross", DATABASE_URL: "must-not-cross",
    } },
