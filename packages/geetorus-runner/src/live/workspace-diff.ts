...
export const GEETORUS_WORKSPACE_DIFF_SCHEMA = "geetorus.workspace.diff.v1" as const;

export type GeetorusWorkspaceChangeSource = "harness_reported" | "runner_verified";
export type GeetorusWorkspaceFileOperation =
  | "create"
  | "modify"
  | "delete"
  | "rename"
...
export interface GeetorusWorkspaceFileChange {
  path: string;
  operation: GeetorusWorkspaceFileOperation;
...
export interface GeetorusWorkspaceDiff {
  schema: typeof GEETORUS_WORKSPACE_DIFF_SCHEMA;
  changeSetId: string;
  revision: number;
  source: GeetorusWorkspaceChangeSource;
  complete: boolean;
  files: GeetorusWorkspaceFileChange[];
...
export type GeetorusWorkspaceSnapshot = Map<string, WorkspaceFileSnapshot>;
 ...
 */
export async function captureGeetorusWorkspace(
  root: string,
  options: { signal?: AbortSignal } = {},
): Promise<GeetorusWorkspaceSnapshot> {
  const result: GeetorusWorkspaceSnapshot = new Map();
  let retainedBytes = 0;
...
export function diffGeetorusWorkspace(
  before: GeetorusWorkspaceSnapshot,
  after: GeetorusWorkspaceSnapshot,
  changeSetId: string,
): GeetorusWorkspaceDiff | null {
  const files: GeetorusWorkspaceFileChange[] = [];
  ...
  const deletedByHash = new Map<string, GeetorusWorkspaceFileChange>();
  ...
  return {
    schema: GEETORUS_WORKSPACE_DIFF_SCHEMA,
    changeSetId,
    revision: 1,
