...
import { captureGeetorusWorkspace, diffGeetorusWorkspace } from "./workspace-diff.js";

describe("runner-verified workspace diffs", () => {
  it("captures create, modify, delete, and rename without runner-owned noise", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-workspace-diff-"));
    await mkdir(join(root, ".geetorus-runner-prp"));
    await writeFile(join(root, ".geetorus-runner-prp", "state.json"), "before");
    ...
    const before = await captureGeetorusWorkspace(root);

    await writeFile(join(root, ".geetorus-runner-prp", "state.json"), "after");
    ...
    const after = await captureGeetorusWorkspace(root);
    const diff = diffGeetorusWorkspace(before, after, "turn-1:workspace");

    expect(diff?.schema).toBe("geetorus.workspace.diff.v1");
    ...
    expect(JSON.stringify(diff)).not.toContain(".geetorus-runner-prp");
  });

  it("returns no record when the workspace did not change", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-workspace-stable-"));
    await writeFile(join(root, "stable.txt"), "same\n");
    const before = await captureGeetorusWorkspace(root);
    const after = await captureGeetorusWorkspace(root);
    expect(diffGeetorusWorkspace(before, after, "turn-1:workspace")).toBeNull();
  });
});
