...
import {
  ...
  geetorusWorkspaceFileReferencesFromText,
...
describe("workspace file references", () => {
  it("normalizes Markdown links without retaining unproven file bytes", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-file-reference-"));
    try {
      await writeFile(join(root, "guide.md"), "# Guide\n\nSafe content.\n");
      const canonicalRoot = await realpath(root);
      const references = await discoverGeetorusWorkspaceFileReferences(
        root,
  ...
  it("rejects external URLs and paths outside the authorized workspace", () => {
    const references = geetorusWorkspaceFileReferencesFromText(
      "/workspace",
  ...
  it("does not verify a symlink that resolves outside the workspace", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-file-reference-root-"));
    const outside = await mkdtemp(join(tmpdir(), "geetorus-file-reference-outside-"));
    try {
      ...
      await expect(discoverGeetorusWorkspaceFileReferences(
        root,
        "[linked](linked.md)",
        "turn-1",
      )).resolves.toEqual([]);
  ...
  it("anchors parsing and verification to a canonical workspace root", async () => {
    const parent = await mkdtemp(join(tmpdir(), "geetorus-file-reference-parent-"));
