...
import {
  captureGeetorusWorkspace,
  diffGeetorusWorkspace,
  type GeetorusWorkspaceDiff,
  type GeetorusWorkspaceSnapshot,
} from "./workspace-diff.js";
import {
  discoverGeetorusWorkspaceFileReferences,
  type GeetorusWorkspaceFileReference,
} from "./workspace-file-reference.js";

const LIVE_SESSION_SCHEMA = "geetorus.capability.live-session.v1" as const;
const LIVE_COMPLETION_CONTRACT = Object.freeze({
  revision: "geetorus-capability-live-v1",
  criterionIds: ["objective"],
});
const LIVE_BASE_INSTRUCTIONS = [
  "You are operating one mock Geetorus issue through typed semantic tools.",
  "Use only the tools exposed in this thread; never call a Geetorus REST API.",
  "Treat every tool result as authoritative mock state and use it in your next response.",
  "The user conversation and provider are real, but all Geetorus records are mock records.",
  "Do not discover skills, credentials, endpoints, or hidden control-plane capabilities.",
].join(" ");
const CODEX_PERMISSION_PROFILE = "geetorus-runner-workspace-only";
...
export interface CapabilityLiveWorkspaceDiffEntry {
  turnId: string;
  at: string;
  diff: GeetorusWorkspaceDiff;
}

export interface CapabilityLiveWorkspaceFileReferenceEntry {
  turnId: string;
  at: string;
  reference: GeetorusWorkspaceFileReference;
}
...
export interface CapabilityLiveSessionSnapshot {
  ...
  networkEvidence: {
    realGeetorusRequests: 0;
    childGeetorusEnvironmentKeys: string[];
  };
  ...
  nativeResume?: {
    schema: "geetorus.runner.native-resume-proof/v1";
    triggered: true;
...
async function captureWorkspaceBaselineBeforeAdmission(
  workingDirectory: string,
): Promise<GeetorusWorkspaceSnapshot | null> {
  const controller = new AbortController();
  let timeout: ReturnType<typeof setTimeout> | null = null;
  try {
    return await Promise.race([
      captureGeetorusWorkspace(workingDirectory, { signal: controller.signal }),
...
export class CapabilityLiveSession {
  ...
  snapshot(): CapabilityLiveSessionSnapshot {
    ...
    return {
      ...
      networkEvidence: {
        realGeetorusRequests: 0,
        childGeetorusEnvironmentKeys: childKeys.filter((key) => key.startsWith("GEETORUS_")),
      },
  ...
  async sendMessage(
    ...
    let workspaceBefore: GeetorusWorkspaceSnapshot | null = null;
    try {
    ...
    const workspaceFileReferences = await discoverGeetorusWorkspaceFileReferences(
      this.#config.workingDirectory,
    ...
    const workspaceDiff = workspaceBefore === null
      ...
      : diffGeetorusWorkspace(
          workspaceBefore,
          await captureGeetorusWorkspace(this.#config.workingDirectory),
  ...
  async resolveInteraction(input: CapabilityInteractionResolution): Promise<CapabilityLiveTurnResult> {
    ...
    return this.sendMessage(JSON.stringify({
      schema: "geetorus.semantic-interaction-result.v1",
      interactionId: input.interactionId,
  ...
  async #connect(resume: boolean): Promise<void> {
    ...
    const transportBundle = this.#transportFactory({
      ...
      stateDirectory: resolve(this.#config.workingDirectory, ".geetorus-runner-prp", identityDigest),
      prpIdentity: {
    ...
    const initialized = await this.#transport.request("initialize", {
      clientInfo: {
        name: "geetorus-runner",
        title: "Geetorus Runner Capability",
