...
export const GEETORUS_WORKSPACE_FILE_REFERENCE_SCHEMA = "geetorus.workspace.file_reference.v1" as const;

export interface GeetorusWorkspaceFileReference {
  schema: typeof GEETORUS_WORKSPACE_FILE_REFERENCE_SCHEMA;
  referenceId: string;
...
function media(path: string): Pick<GeetorusWorkspaceFileReference, "mediaType" | "presentation"> {
...
export function geetorusWorkspaceFileReferencesFromText(
  workspace: string,
  assistantText: string,
  turnId: string,
  source: GeetorusWorkspaceFileReference["source"] = "harness_reported",
): GeetorusWorkspaceFileReference[] {
  const root = resolve(workspace);
  const references: GeetorusWorkspaceFileReference[] = [];
  ...
  for (const match of links) {
    ...
    references.push({
      schema: GEETORUS_WORKSPACE_FILE_REFERENCE_SCHEMA,
...
export async function discoverGeetorusWorkspaceFileReferences(
  workspace: string,
  assistantText: string,
  turnId: string,
): Promise<GeetorusWorkspaceFileReference[]> {
  ...
  const references = geetorusWorkspaceFileReferencesFromText(canonicalRoot, assistantText, turnId, "runner_verified");
  const verified: GeetorusWorkspaceFileReference[] = [];
