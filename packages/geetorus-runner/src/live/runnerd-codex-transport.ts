...
- Reserve \`report_progress\` for meaningful durable milestones on longer work. Do not call it merely to create a completion comment on a short run; Geetorus materializes the final assistant response as the durable completion comment.
- Invoke the semantic completion tool exactly once before the final assistant response. After it succeeds, send one self-contained final response with the outcome and verification, then do not call another tool. The completion tool records task disposition; Geetorus keeps receiving your answer until the provider turn ends.`;
...
function assertSuspendedRunnerState(
  ...
  if (
    state.schema !== "geetorus.runner.durable.state.v1" ||
...
function bridgedCodexQuestionParams(
  ...
  if (
    questionSet.schema !== "geetorus.question_set.v1" ||
...
export interface CapabilityRunnerdCodexTransportOptions {
  ...
  /** Root path visible to runnerd when it is not on the Geetorus host. */
  ...
  /** Registers the run-bound PRP authority on Geetorus's shared HTTP server. */
  ...
  return (
    {
      ...
      "workspace.change.updated": "geetorus/workspaceChange/updated",
      "run.result.proposed": "geetorus/runResult",
...
function appendRunnerdRehydrationTrace(
  ...
  try {
    ...
    appendFileSync(
      ...
      `${JSON.stringify({
        kind: "interpretation",
        schema: "geetorus.provider_trace_interpretation.v1",
        ...
        fieldMappings: [
          {
            ...
            outputPath: "geetorusTrace.sourceEventId",
            action: "copied",
            reason:
          ...
          {
            inputPath: "eventType",
            outputPath: "geetorusTrace.sourceEventType",
            action: "renamed",
            reason:
...
function assertMaintenanceBinding(
  ...
  if (
    ...
    state.runner.schema !== "geetorus.runner.durable.state.v1" ||
    state.provider.schema !== "geetorus.runner.codex-provider-state.v1" ||
...
async function settleRetainedRunnerdSessionOwned(
  ...
  for (let epoch = 0; epoch < 4; epoch++) {
    ...
    const epochIdentity = {
      schema: "geetorus.native_cleanup_runner_epoch.v1" as const,
    ...
    for (const raw of [
      ...
      const sourceEventId = `event_executor_${createHash("sha256")
        .update("geetorus.executor-event.v1\0")
...
function resolveBuildOwnedCliArtifact(
  ...
  throw new Error(
    `runner_local_provider_artifact_missing: ${artifact} is absent; build @geetorusai/geetorus-runner TypeScript artifacts with build:typescript before starting a local JS-backed provider`,
  );
}
...
function acpxProviderPackageAuthority(
  ...
  // A local source build lives at <workspace>/packages/geetorus-runner and
  ...
  // <deploy>/node_modules/@geetorusai/geetorus-runner. The verifier always
  ...
  if (!options.runnerFilesystemRoot) {
    ...
    return {
      authorityDigest: commandDigest({
        schema: "geetorus.runner.local-acpx-authority.v1",
        commandSha256,
        sidecarScriptSha256,
      }),
      command: buildCommand,
...
function withRunnerdProviderTrace(
  ...
  for (const key of [
    "GEETORUS_PROVIDER_TRACE_PATH",
    "GEETORUS_PROVIDER_TRACE_MAX_BYTES",
  ] as const) {
...
export function createCapabilityRunnerdProviderEnvironment(input: {
  ...
  const commonIdentity = {
    GEETORUS_RUNNER_INSTANCE_ID: input.identity.runnerInstanceId,
    GEETORUS_RUN_ID: input.identity.runId,
    GEETORUS_NORMALIZED_SESSION_ID: input.identity.normalizedSessionId,
    ...(input.hasRuntimeContext
      ? { GEETORUS_NATIVE_RUNTIME_CONTEXT_PATH: input.runtimeContextPath }
      : {}),
  };
  if (input.provider === "opencode") {
    return {
      ...createSanitizedOpenCodeRunnerEnvironment(input.options.environment),
      GEETORUS_OPENCODE_PERMISSION_MODE:
        input.options.opencodePermissionMode ?? "ask",
      GEETORUS_OPENCODE_RUNTIME_DIR:
  ...
  if (input.provider === "acpx") {
    ...
    return {
      ...(assignedGateway ? {
        GEETORUS_NATIVE_MCP_TOKEN: assignedGateway.token,
      } : {}),
...
const OPEN_CODE_RUNNER_ENVIRONMENT_KEYS = new Set([
  ...
  "RUST_BACKTRACE",
  "OPENROUTER_API_KEY",
  "GEETORUS_NATIVE_MCP_NAME",
  "GEETORUS_NATIVE_MCP_URL",
  "GEETORUS_NATIVE_MCP_TOKEN",
]);
...
function unwrapToolResponse(response: Record<string, unknown>): {
  readonly __geetorusSemanticToolOutcome: true;
  ...
  return {
    __geetorusSemanticToolOutcome: true as const,
    result,
...
class DurablePrpCodexTransport implements CodexAppServerTransport {
  ...
  #handler: CodexServerRequestHandler = async () => ({
    ...
    contentItems: [
      {
        type: "inputText",
        text: "No Geetorus control-plane tool handler is installed.",
      },
    ],
  });
  ...
  constructor(readonly options: CapabilityRunnerdCodexTransportOptions) {
    ...
    this.#root =
      ...
      mkdtempSync(resolve(tmpdir(), "geetorus-runner-lab-prp-"));
        ...
        thread: {
          ...
          cwd:
            typeof snapshot.cwd === "string" && snapshot.cwd.length > 0
              ? snapshot.cwd
              : (this.options.environment?.GEETORUS_WORKSPACE_CWD ??
  ...
  async attachRun(input: {
    ...
    try {
      ...
      const payload = {
        ...runAttachTemplate,
        geetorusNextAuthority: { identity: desired, connection },
      };
  ...
  recordTraceInterpretation(input: CodexTraceInterpretation): void {
    const tracePath = this.options.environment?.GEETORUS_PROVIDER_TRACE_PATH;
    if (!tracePath) return;
  ...
  async #start(
    ...
    const unboundBaseInstructions = String(
      params.baseInstructions ?? "You are a Geetorus agent.",
    );
          ...
          : provider === "claude_managed"
            ...
            : provider === "aws_agentcore"
              ...
              : {
                  ...
                  collaborationMode:
                    params.permissions ===
                    "geetorus-runner-workspace-read-only"
                      ? "plan"
                      : "default",
  ...
  async #resume(): Promise<void> {
    ...
    const hasRunnerWarmBoundary =
      ...
      candidateRunnerState?.schema ===
        "geetorus.runner.durable.state.warm-transition.v1";
        ...
        (Array.isArray(controlPlaneState.commands) &&
          controlPlaneState.commands.some((entry) => {
            ...
            return (
              ...
              command.status === "pending" &&
              record(command.payload).geetorusNextAuthority !== undefined
            );
          })))
    ) {
    ...
    for (;;) {
      ...
      const notifications =
        isCanonicalProviderEventType(event.eventType) && !canonicalMethod
          ? [{ method: "geetorus/canonicalProviderEvent", params: {
      ...
      for (const payload of notifications) {
        ...
        const params =
          method === "thread/tokenUsage/updated"
            ...
            : method === "geetorus/workspaceChange/updated"
              ? rehydrateRunnerdWorkspaceChangeNotification(
                  rawParams,
                  this.#threadId,
                  this.#turnId,
                )
            : method === "geetorus/runResult"
        ...
        this.#queue.push({
          method,
          params,
          ...(this.options.environment?.GEETORUS_PROVIDER_TRACE_PATH
            ? {
                geetorusTrace: {
                  sourceEventId: event.sourceEventId,
                  sourceEventType: event.eventType,
      ...
      if (this.options.environment?.GEETORUS_PROVIDER_TRACE_PATH) {
        const pending = {
          sourceEventId: event.sourceEventId,
...
export function defaultCapabilityRunnerdBinary(): string {
  const staged = resolve(
    packageRoot,
    `dist/bin/geetorus-runnerd${executableSuffix}`,
  );
  if (existsSync(staged)) return staged;
  return resolve(
    packageRoot,
    `runner/target/debug/geetorus-runnerd${executableSuffix}`,
  );
}
