...
describe("provider-neutral events", () => {
  ...
  it("preserves ACPX identity across title-less progress and completion updates", () => {
    ...
    const started = normalize({
      ...
      tag: "tool_call",
      toolCallId: "tool-1",
      title: "mcp__geetorus__search_tasks",
      kind: "other",
      status: "pending",
      text: "mcp__geetorus__search_tasks (pending)",
    ...
    const dotted = canonicalProviderEventsFromAcpxRuntimeEvent(
      {
        ...
        tag: "tool_call",
        toolCallId: "mcp-1",
        title: "mcp.geetorus.get_task_context",
        kind: "other",
        status: "pending",
        text: "Starting",
      } as never,
