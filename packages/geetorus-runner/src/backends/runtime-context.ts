...
export function nativeTaskConstraints(input: NativeExecutionInput): string[] {
  const finalResponseConstraint =
    "Obtain one accepted result from geetorus_finish or geetorus_block before writing the complete user-facing final response. Use geetorus_finish with yielded and a response_wake continuation only when explicitly waiting for the next response. If the tool rejects an incomplete report, correct it and retry. When it succeeds, read its outcome and explain any pending approval with the supplied link and required action. Do not claim the task is done when completion is still gated. Then write the final response exactly once and do not call another tool.";
