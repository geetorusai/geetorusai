...
function createTransportBackedNativeSessionBackend(
  ...
  if (
    ...
    throw new Error(
      "geetorus_runner_codex_permission_mode_unqualified: set codexPermissionMode to never before starting or recovering this native run",
    );
  }
  ...
  return new HarnessDriverBackend(
    new CodexAppServerDriver({
        ...
        constraints: [
          ...
          input.executionMode === "plan"
            ? [
                ...
                "Treat the supplied Geetorus planning context as the canonical pinned base revision.",
                "Complete one structured provider plan item; Geetorus will synchronize it after completion.",
      ...
      runnerInstanceId:
        options.runnerInstanceId ?? `geetorus-native-${input.binding.runId}`,
