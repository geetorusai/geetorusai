 ...
 *
 * The adapter deliberately models Geetorus semantics instead of Geetorus
...
export class CapabilityMockControlPlaneAdapter implements CapabilityMockControlPlanePort {
  ...
  static restore(
    ...
    if (!isFixtureState(parsed)) {
      throw new CapabilityMockControlPlaneError(
        "fixture_state_invalid",
        "fixture state schema must be geetorus.capability.mock-state.v1",
      );
    }
...
function isSemanticToolRuntimeSnapshot(
  ...
  if (
    snapshot.schema !== "geetorus.capability.semantic-tool-runtime.v1" ||
...
function isFixtureState(value: unknown): value is CapabilityFixtureState {
  return (
    ...
    value !== null &&
    "schema" in value &&
    value.schema === "geetorus.capability.mock-state.v1"
  );
}
