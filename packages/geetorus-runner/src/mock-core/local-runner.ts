...
interface RunnerStreamEnvelope {
  schema: "geetorus.runner.stream.v1";
...
function metadata(scenario: LocalRunnerScenario): LocalRunnerRunMetadata {
  ...
  return {
    schema: "geetorus.runner.local-runner.metadata.v1",
    scenario,
    fixtureName: `Local runner ${scenario}`,
    identity: {
      schema: "geetorus.prp.identity.v1",
...
class LocalRunnerController implements LocalRunnerRunHandle {
  ...
  static async start(options: StartLocalRunnerScenarioOptions): Promise<LocalRunnerController> {
    ...
    await Promise.all([access(runnerBinary), access(fakeHarnessBinary)]).catch(() => {
      throw new Error(
        `Local runner Rust binaries are missing. Run pnpm --filter @geetorusai/geetorus-runner build:rust first. PATH entries: ${(process.env.PATH ?? "").split(delimiter).length}.`,
      );
    });
  ...
  #handleLine(line: string): void {
    ...
    if (envelope.schema !== "geetorus.runner.stream.v1") {
  ...
  #finish(code: number | null, signal: NodeJS.Signals | null): void {
    ...
    const trace: LocalRunnerRunTrace = {
      ...this.metadata,
      schema: "geetorus.runner.local-runner.trace.v1",
...
export function localRunnerTraceAsFixture(trace: LocalRunnerRunTrace): PrpFixture {
  ...
  const fixture: PrpFixture = {
    schema: "geetorus.prp.fixture.v1",
    fixtureVersion: 1,
    protocolVersion: 1,
    name: trace.fixtureName,
    description: `Live Local runner ${trace.scenario} trace captured from geetorus-runnerd.`,
