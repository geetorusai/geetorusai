...
| --- | --- | --- |
| **Real Codex** | A real Codex app-server session driving the turn loop. The agent has no Geetorus skill; it sees only the semantic tools the scenario exposes. | Not a scripted stand-in, and not a Geetorus-aware agent. |
| **Real runnerd** | The real package-local `geetorus-runnerd` binary. It owns the Codex child process group and proxies newline-delimited JSON-RPC over stdio. | Not an in-process fake and not the production Geetorus runtime. |
| **Mock Geetorus** | The deterministic in-process `ControlPlanePort` adapter. It holds every issue, comment, document, interaction, approval, and audit record as mock state. | Not the Geetorus control plane, database, or API. No request leaves the process for a Geetorus service. |

The live surface renders a `Real Codex`, `Real runnerd`, and `Mock Geetorus`
...
Live mode starts a real `geetorus-runnerd` process and a real Codex
