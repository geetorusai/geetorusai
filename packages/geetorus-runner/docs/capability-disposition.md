...
This page explains how Capability classifies every Geetorus capability. The
...
1. The Geetorus skill and its seven references (`SKILL.md` plus
   `references/*.md`), contributing **153 headings**.
2. The Geetorus Evals corpus, contributing **106 cases across 16 groups**.

Together these produce **259 normative rows**. The legacy Geetorus MCP tool
...
source, and the Geetorus Evals corpus, so it **requires** the external eval
repository (via `GEETORUS_EVALS_ROOT` or a known local path) and is not part of
...
```sh
# Rewrite every generated file. Requires the external Geetorus Evals corpus.
pnpm --filter @geetorusai/geetorus-runner generate:capability-inventory
