...
Every file lives under `packages/geetorus-runner/`. The console does not
import or change Geetorus server, UI, database, or control-plane code.
...
| --- | --- | --- |
| `GEETORUS_LIVE_CONSOLE_DRIVER` | `demo` | `codex` starts the real `codex app-server` driver behind the same routes |
| `GEETORUS_LIVE_CONSOLE_CHUNK_DELAY_MS` | `45` | Milliseconds between streamed chunks in the demo driver |
...
No provider or Geetorus credential reaches the browser. The Node process owns
