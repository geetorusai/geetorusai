...
or AI SDK dependency to `@geetorusai/geetorus-runner`. A later browser phase
...
|---|---|---|
| shadcn/ui | Official documentation describes open-code component distribution and provides first-class Vite and monorepo setup paths. | Compatible with Geetorus's Vite UI model. Reuse or adapt checked-in source while keeping Geetorus tokens and accessibility rules authoritative. |
...
- data contract maps to the shared runner reducer rather than component-local
  state;
- styles use Geetorus's token layer with no raw color, spacing, radius,
  typography, shadow, or motion values in Geetorus UI files;
