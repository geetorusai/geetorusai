...
and evidence stay inside `packages/geetorus-runner/`. They do not import or
change Geetorus server, UI, database, or production control-plane behavior.
...
The model receives one text input containing `geetorus.skillless_task.v1`:
...
valid, but when `GEETORUS_WORKSPACE_CWD` is present its canonical path must be
...
Geetorus bearer values, `OPENAI_API_KEY`, arbitrary skill paths, and other
...
- `geetorus_finish` accepts `done` or `needs_review`;
- `geetorus_block` accepts `blocked` and requires a blocker owner, action,
  reason, and scope.

Both normalize through the canonical `geetorus.run_result.v1` validator. The
