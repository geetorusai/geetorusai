# Capability Geetorus-style issue-thread UI

Capability renders the mock Geetorus issue as a native issue thread. A board user
...
```text
browser  ->  package server  ->  CapabilityLiveSession  ->  geetorus-runnerd -> codex
...
the reserved `MCK-` prefix, and no real Geetorus URL is ever rendered.
...
The two surfaces use separate cookie names (`geetorus_capability_issue`,
`geetorus_capability_chat`) because they are separate pages of one origin: a single
...
- **Header** — three identity chips (`Real Codex` / `Fake agent` / `Replay`,
  `Real runnerd` / `In-process runner`, and `Mock Geetorus` in every mode),
...
pnpm --filter @geetorusai/geetorus-runner exec vitest run src/issue-thread
...
`pnpm --filter @geetorusai/geetorus-runner verify:rootless` or set
`GEETORUS_RUNNER_CHROMIUM_PATH` to a preinstalled Chromium.

The committed PNGs are pinned to the Chromium build listed in
`.geetorus-local/evidence/capability/ui/index.md`, so `check:capability:ui` needs that same
browser. Point `GEETORUS_RUNNER_CHROMIUM_PATH` at the recorded browser before
comparing — and when that path is the agent-browser wrapper, also set
`GEETORUS_CHROMIUM_BIN` to the exact binary, because the wrapper otherwise
...
`.geetorus-local/evidence/capability/ui-live/` and is excluded from the determinism gate.
