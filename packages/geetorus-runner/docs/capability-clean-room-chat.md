...
a mock Geetorus issue through real runnerd. There is no scenario to pick, no
...
`CapabilityLiveSession` routes every Geetorus operation through the in-process mock
`ControlPlanePort`; no code path reaches a Geetorus URL. The projection turns
...
`Real Geetorus API requests: 0. Child GEETORUS_* environment keys: none.`

The child environment is allowlisted by `createSanitizedCodexEnvironment`, so no
`GEETORUS_*` value reaches runnerd or Codex, and the browser receives no
...
MCP/dynamic-tool, assistant-stream, and Geetorus semantic-tool lifecycle events
...
| Surface | Command |
| --- | --- |
| Seed, exposure profile, and identity rotation | `pnpm --filter @geetorusai/geetorus-runner test:scenarios` |
| Clean-room HTTP routes end to end (stub provider) | included in `test:scenarios` |
| Browser entry, blank state, evidence-on-demand, narrow layout, axe | `pnpm --filter @geetorusai/geetorus-runner test:browser:issue-thread` |
| Real Codex through real runnerd | `pnpm --filter @geetorusai/geetorus-runner smoke:capability:cleanroom` |
| Live screenshots | `pnpm --filter @geetorusai/geetorus-runner recorded-evidence campaign (deferred)` |
