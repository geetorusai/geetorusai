...
(annotated desktop + mobile renders in `.geetorus-local/evidence/capability/`).
...
not evaluate claims, does not apply redaction, and holds no Geetorus or
...
from static assets plus checked-in fixtures with **no Geetorus services and
no network dependency** in fake-agent mode. Reuse the frozen `0.1.2` SDK
surface (`@geetorusai/geetorus-runner/react` + `./styles.css`) through its
...
- Fake mode performs zero network I/O; Codex mode talks only to the local
  relay; no route, storage key, or bundle string may contain a Geetorus or
...
- The generic escape hatch (`geetorusApiRequest` successor), when a test
...
- **Reuse first.** Shell, transcript, and inspector compose the frozen SDK:
  ...
  `@geetorusai/geetorus-runner/react`. Semantic call detail uses the
...
Evidence lands in `.geetorus-local/evidence/capability/` as
...
- No live-Geetorus or ACPX affordances; the future binding boundary stays a
