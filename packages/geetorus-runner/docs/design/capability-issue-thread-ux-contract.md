...
Status: **binding** for track 7G (Geetorus-style Web UI) and reviewable by tracks 7E/7K.
...
1. **Three identity chips**, rendered in the issue header and never hidden by scroll
   ...
   - `Real runnerd` — neutral chip with a live process dot (pulse while a runnerd session is
     ...
     `In-process runner`.
   - `Mock Geetorus` — amber chip, shown in **all** modes. Tooltip: "All issue records are
     mock. No real Geetorus API is reachable."
...
│  [Fake agent|Real Codex] [Real runnerd] [Mock Geetorus]     │  (collapsed   │
...
- Thread column measure: max-width 760px, centered in the remaining space; `text-sm` body
  scale per the Geetorus type ramp.
...
| # | Item | Source | Anatomy |
|---|------|--------|---------|
| T1 | **User message** | thread record | Right-aligned bubble style is **not** used; Geetorus comment card with author "You (board user)", timestamp, markdown body. |
...
- **Replay**: mode `replay` re-renders a recorded run from canonical events. Composer
  `disabled` (`Replay is read-only`), identity chip row shows `Replay` + `Mock Geetorus`,
...
Follow the Geetorus design language without importing the product `ui/` package:
...
- Interaction cards use the product interaction-card anatomy (title row + prompt + controls +
  state chip) so the mock thread reads as a Geetorus issue thread (Jakob's Law is the point
...
`.geetorus-local/evidence/capability/ui/<slug>--<desktop|mobile>.png` (package-local). Every capture
...
`.geetorus-local/evidence/capability/cases/<group>/<case-id>--<viewport>.png`. The §10.2 matrix is
...
- No provider, runner, or control-plane credential ever reaches the browser; redactions render
  by rule name. Real Geetorus URLs/API paths never appear.
