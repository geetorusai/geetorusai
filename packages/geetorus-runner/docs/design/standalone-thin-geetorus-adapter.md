# Standalone Thin Geetorus Adapter Boundary

> **Scope correction — 2026-08-09:** The board superseded real Geetorus
...
> `packages/geetorus-runner/`. Product/server/database sections below are
...
Date: 2026-08-09
Decision scope: the first feature-flagged Geetorus native run, including the
normative native finalizer and server-owned Section 18 status arbitration

## Decision

Geetorus will integrate the native runner through two public package ports:
...
package supplies the `NativeSessionBackend` implementation; Geetorus supplies
...
Geetorus types. It does not implement a native backend or contain runner
behavior.
...
server heartbeat orchestration
  -> GeetorusNativeRuntimeAdapter (core seam)
      -> @geetorusai/geetorus-runner public contracts
          -> NativeSessionBackend -> package-owned driver/runner logic
          -> ControlPlanePort      -> server-bound Geetorus implementation
```

`packages/geetorus-runner/` never imports `server/`, `packages/db/`,
`packages/shared/`, or a Geetorus service. The server may import the public
package. The core seam may translate company-scoped Geetorus records into the
...
the accepted semantic result stored in the harness snapshot. Geetorus core
...
Everything before the branch remains Geetorus-owned preparation. Shared
...
- the local agent JWT, `GEETORUS_API_KEY`, a board session, or a board API key;
- managed MCP gateway credentials, runner-lease/bootstrap credentials, or
  credential-broker secret material;
- `GEETORUS_WAKE_PAYLOAD_JSON`, rendered Geetorus wake text, Geetorus skill
  instructions, the Geetorus API manual, or run-scoped skill material;
...
| Geetorus concern | Standalone mapping | Owner |
|---|---|---|
...
2. `GeetorusControlPlanePort`, with a real test database, heartbeat run, issue,
...
| ID | Concern | Mock | Real Geetorus | Legacy assertion |
|---|---|---:|---:|---|
...
5. Implement the server-bound `GeetorusControlPlanePort` with constructor
...
packages/geetorus-runner/src/contracts/control-plane-port.ts
packages/geetorus-runner/src/contracts/native-session-backend.ts
packages/geetorus-runner/src/contracts/types.ts
packages/geetorus-runner/src/mock-core/mock-control-plane-adapter.ts
packages/geetorus-runner/src/mock-core/mock-control-plane-adapter.test.ts
packages/geetorus-runner/src/backends/harness-driver-backend.ts              (new)
packages/geetorus-runner/src/backends/harness-driver-backend.test.ts         (new)
packages/geetorus-runner/src/conformance/control-plane-port.ts               (new)
packages/geetorus-runner/src/conformance/control-plane-port.test.ts          (new)
packages/geetorus-runner/src/cli/geetorus-adapter.ts                            (new)
packages/geetorus-runner/protocol/fixtures/standalone/*                         (new)
packages/geetorus-runner/docs/standalone-thin-geetorus-adapter.md              (new)
packages/geetorus-runner/docs/tutorials/standalone-thin-geetorus-adapter.md
packages/geetorus-runner/docs/tutorials/end-to-end.md
packages/geetorus-runner/docs/index.md
packages/geetorus-runner/docs/architecture.md
packages/geetorus-runner/.geetorus-local/log.md
```

### Geetorus storage, adapter, finalizer, and read seam
...
server/src/services/native-runtime/geetorus-control-plane-port.ts        (new)
...
```text
packages/geetorus-runner/src/native-session-runtime.ts
packages/geetorus-runner/src/native-session-runtime.test.ts
packages/geetorus-runner/src/contracts/native-execution.ts
packages/geetorus-runner/src/contracts/native-execution.test.ts
packages/geetorus-runner/src/backends/codex-native-backend.ts
packages/geetorus-runner/spec/fixtures/status-authority-sdk.json
...
server/src/services/native-runtime/geetorus-control-plane-port.test.ts
server/src/services/native-runtime/status-arbiter.test.ts
server/src/services/recovery/service.ts
packages/geetorus-runner/docs/design/standalone-thin-geetorus-adapter.md
```
...
```text
GeetorusControlPlanePort.completeRun
...
`GeetorusControlPlanePort.completeRun` and then calls the same production
...
`experimental.enableNativeRunner` turns off. A fresh `geetorus_runner` start
is rejected with `geetorus_runner_rollout_disabled`; direct adapters remain on
...
packages/geetorus-runner/README.md
packages/geetorus-runner/docs/design/standalone-thin-geetorus-adapter.md
packages/geetorus-runner/docs/index.md
packages/geetorus-runner/docs/standalone-thin-geetorus-adapter.md
packages/geetorus-runner/docs/tutorials/standalone-thin-geetorus-adapter.md
packages/geetorus-runner/.geetorus-local/log.md
packages/geetorus-runner/package.json
packages/geetorus-runner/spec/fixtures/status-authority-sdk.json
packages/geetorus-runner/src/backends/codex-native-backend.ts
packages/geetorus-runner/src/backends/harness-driver-backend.test.ts
packages/geetorus-runner/src/backends/harness-driver-backend.ts
packages/geetorus-runner/src/cli/standalone-geetorus.ts
packages/geetorus-runner/src/conformance/control-plane-port.test.ts
packages/geetorus-runner/src/conformance/control-plane-port.ts
packages/geetorus-runner/src/contracts/control-plane-port.ts
packages/geetorus-runner/src/contracts/native-execution.test.ts
packages/geetorus-runner/src/contracts/native-execution.ts
packages/geetorus-runner/src/contracts/native-session-backend.ts
...
server/src/services/native-runtime/native-workspace-finalizer.ts
server/src/services/native-runtime/geetorus-control-plane-port.test.ts
server/src/services/native-runtime/geetorus-control-plane-port.ts
...
pnpm --filter @geetorusai/geetorus-runner exec vitest run \
  src/conformance/control-plane-port.test.ts \
  src/backends/harness-driver-backend.test.ts

pnpm --filter @geetorusai/geetorus-runner trace:standalone -- \
  --target mock --scenario happy-path
```

First real Geetorus tracer and inspection (against an isolated local dev
instance with the five `GEETORUS_*` identifiers/auth variables already set):

```sh
pnpm --filter @geetorusai/geetorus-runner trace:standalone -- \
  --target geetorus --scenario happy-path

GEETORUS_API_BASE="${GEETORUS_API_URL%/}"
GEETORUS_API_BASE="${GEETORUS_API_BASE%/api}"
curl -fsS \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  "$GEETORUS_API_BASE/api/heartbeat-runs/$GEETORUS_RUN_ID/events?after=0&limit=200" \
...
```sh
pnpm --filter @geetorusai/server exec vitest run \
...
```sh
pnpm --filter @geetorusai/geetorus-runner trace:standalone -- \
  --target geetorus --scenario legacy-fallback
...
[Standalone tutorial](../tutorials/standalone-thin-geetorus-adapter.md) with:
