...
Geetorus activity for every turn** instead of having it hidden behind chat.
...
1. **Mode names: `Scripted (deterministic)` / `Codex (bounded)`, and the
   ...
   acceptance — "a user can complete successful and denied mock Geetorus
...
- All 7F §10 rejections stand (no Tailwind/Radix, no dark mode, no markdown
  renderer, no virtualization, no parity re-judging, no live-Geetorus
  affordances).
