...
> in [`../../../doc/plans/2026-08-20-single-daemon-runner-tool-bridge.md`](../../../doc/plans/2026-08-20-single-daemon-runner-tool-bridge.md): external control plane → PRP → one Rust `geetorus-runnerd` → provider. The TypeScript dispatcher below does not run in the sandbox.

Capability binds the provider-neutral semantic catalog to a real package-local
`geetorus-runnerd` process and a real Codex app-server session. Geetorus data
...
```text
CapabilityLiveSession -> geetorus-runnerd -> codex app-server
                  -> CapabilitySemanticDispatcher -> ControlPlanePort mock
```

`geetorus-runnerd` owns the Codex child and proxies newline-delimited JSON-RPC
...
Only the allowlisted Codex host environment is copied. `GEETORUS_*` variables,
...
} from "@geetorusai/geetorus-runner/live";
...
ownership/cleanup, cleared authority, and zero Geetorus network/child-env
exposure.
