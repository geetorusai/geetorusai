# Geetorus Evals Integration Contract

## Stable consumer inputs

Geetorus Evals consumes two explicit App artifacts:

1. a packed/released `@geetorusai/geetorus-runner` package; and
2. an explicit `geetorus-runnerd` executable path plus its
...
`resolveGeetorusRunnerdArtifact` resolves only the supplied path, verifies its
...
`assertGeetorusRunnerEvalCompatibility` checks the complete join before any
...
| Native output | Consumer and runnerd both select `geetorus-runner/native-execution/v1`. |

Failures use `geetorus_runner_eval_incompatible` and include all detected
...
`geetorus-runner/native-execution/v1` is the App-owned raw attempt. Its checked
...
Set `GEETORUS_CLEAN_CONSUMER_OUTPUT_DIR` to retain the qualifying inputs and
...
`geetorus-runner-consumer-conformance.json`. That record is produced by the clean
...
```sh
GEETORUS_CLEAN_CONSUMER_OUTPUT_DIR=/absolute/release/directory \
  pnpm --filter @geetorusai/geetorus-runner check:clean-consumers
```
