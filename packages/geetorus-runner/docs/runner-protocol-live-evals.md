...
the [Geetorus evaluation guide](../../../doc/evals.md). This document remains
...
`pnpm --filter @geetorusai/geetorus-runner build:issue-thread` and provide
`--viewer-root` or `GEETORUS_EVAL_VIEWER_ROOT` to the canonical Python renderer.
...
`GEETORUS_RUNNER_PROTOCOL_EVAL_VIEWER_DIR` to the publisher. The workflow sends
...
`node packages/geetorus-runner/scripts/verify-runner-evalbook-viewer.mjs --report-root /path/to/report`.
...
```sh
node packages/geetorus-runner/scripts/refresh-runner-protocol-eval-report.mjs \
  --source /path/to/downloaded-aggregate \
  --evals-root /path/to/geetorus-evals \
  --viewer-root packages/geetorus-runner/dist-issue-thread \
...
`geetorusai/geetorus-evals/evals/geetorus-runner`. It is intentionally
...
`campaigns/live-direct-full.json` at one immutable `geetorus-evals` commit.
...
- `target_branch`: the Geetorus branch to build and test;
- `evals_sha`: an exact 40-character commit from
  `geetorusai/geetorus-evals`;
...
The authorization job resolves the Geetorus branch to a commit and verifies
...
same `geetorus.native-execution-input.v3` contract as production, including
...
state changes from native `geetorus_finish`/`geetorus_block` run-result
...
`GEETORUS_CLAUDE_MANAGED_*` profile variables. AgentCore requires the
nonsecret `GEETORUS_AWS_AGENTCORE_*` profile variables, including
`GEETORUS_AWS_AGENTCORE_EXECUTION_ROLE_ARN` and the immutable
`GEETORUS_AWS_AGENTCORE_QUALIFICATION_REVISION`; the eval fails closed when
...
role admits only the `geetorusai/geetorus` repository's protected
...
- Exact Geetorus and eval-suite commit links (full SHA on hover), the source ref,
...
```sh
pnpm --filter @geetorusai/geetorus-runner test:runner-protocol-eval-publish
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner \
