...
|---|---|
| Geetorus App | PRP schemas and fixtures, canonical semantic catalog and dispatcher, runnerd/client interfaces, `ControlPlanePort`, the production binding, deterministic mock, and mock/real parity fixtures |
...
`server/src/services/native-runtime/geetorus-control-plane-port.ts`. It
...
|---|---|
| `@geetorusai/geetorus-runner` | Runtime contracts, runner clients/backends, PRP validation/replay, canonical catalog/dispatcher, and compatibility preflight |
| `@geetorusai/geetorus-runner/evals` | Versioned native-attempt/build metadata, compatibility negotiation, and explicit runnerd artifact resolution |
| `@geetorusai/geetorus-runner/testing` | Deterministic mocks, PRP port conformance, and provider-neutral semantic conformance kit |
...
workspace-private `@geetorusai/geetorus-eval-kernel` package. It contains no
...
```text
Geetorus App production binding
            |
            v
@geetorusai/geetorus-runner (runtime contracts)
            ^
            |
Eval consumers ----> @geetorusai/geetorus-runner/evals
       |             @geetorusai/geetorus-runner/testing
       +-----------> @geetorusai/geetorus-eval-kernel
```
...
contracts are published in `GEETORUS_RUNNER_COMPATIBILITY`:
...
`assertGeetorusRunnerCompatibility` performs a fail-closed preflight. It emits
`geetorus_runner_incompatible` with stable issue codes for component mismatch,
...
```sh
pnpm --filter @geetorusai/geetorus-runner check:package-boundaries
pnpm --filter @geetorusai/geetorus-runner check:clean-consumers
```
...
- Existing tests importing mock/conformance values from the package root must
  migrate to `@geetorusai/geetorus-runner/testing`.
