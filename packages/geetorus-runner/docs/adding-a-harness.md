...
Every Geetorus Runner harness must declare its permission behavior as part of
...
validation is `GEETORUS_RUNNER_PERMISSION_CAPABILITIES` in
`@geetorusai/adapter-utils`. Native process boundaries validate the pinned
...
access, unrestricted network access, Geetorus credentials, or write access in
...
5. Authorize Geetorus semantic and question tools through authenticated PRP.
...
7. Keep standalone local adapters unchanged unless their own contract is
   deliberately revised. Geetorus Runner defaults apply only to
   `geetorus_runner` executions.
