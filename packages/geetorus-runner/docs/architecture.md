...
Geetorus App production binding --> implements ControlPlanePort
...
The standalone package does not reach backward into a Geetorus implementation.
The production implementation lives in
`server/src/services/native-runtime/geetorus-control-plane-port.ts`; it depends
...
- The workspace-private `@geetorusai/geetorus-eval-kernel` contains generic
...
`geetorus-runnerd` and `fake-harness` binaries without changing that dependency
direction.
...
- Rust is the production direction for deterministic runner behavior,
  supervision, durable delivery, and the eventual `geetorus-runnerd` binary.
...
- Files within `packages/geetorus-runner/`.
...
- `@geetorusai/db` and production database schema or client modules;
- `@geetorusai/shared`, adapter utilities, and other Geetorus workspace
  internals unless a boundary review explicitly allows a public contract;
- relative or absolute imports that escape `packages/geetorus-runner/`.
...
```sh
pnpm --filter @geetorusai/geetorus-runner check:forbidden-imports
pnpm --filter @geetorusai/geetorus-runner test
pnpm --filter @geetorusai/geetorus-runner check:replay-parity
```
...
The mock core is an in-memory adapter, not a Geetorus server. Starting it only
...
No socket, database, browser, Geetorus process, or model process is started.
...
TypeScript mock core
  ...
  v
geetorus-runnerd (Rust WebSocket client)
...
production Geetorus. Its focused tests cover lost ACKs, socket loss, malformed
input, process restarts, lease expiry, storage pressure, drain, and revoke.

## Capability-model boundary

```text
Geetorus skill + 7 references        Geetorus Evals corpus (106 cases)
            |                                      |
            +-------------------+------------------+
                                v
...
Capability is a package-local model of a native Geetorus run. It classifies every
...
authorization rules, or conformance suite. Capability contacts no Geetorus
...
```text
 ...
 package server ── projectCapabilityIssueThread ──> CapabilityIssueThreadSnapshot
        ...
        v
 geetorus-runnerd (real binary, owns the Codex process group)
...
session), **Real runnerd** (the package-local binary), and **Mock Geetorus**
...
Geetorus API, and the evidence suite proves no such request occurred. The same
...
Geetorus core may implement these contracts behind a separately reviewed
...
[Standalone Thin Geetorus Adapter Boundary](design/standalone-thin-geetorus-adapter.md).
It keeps one dependency direction, branches only after Geetorus workspace and
environment realization, composes a package-owned `NativeSessionBackend` with
a server-bound `ControlPlanePort`, and returns to the existing Geetorus
