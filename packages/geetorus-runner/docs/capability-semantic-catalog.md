...
`ControlPlanePort`. It does not contain Geetorus REST routes, authentication
...
`geetorus.semantic-denial.v1` result. The mock command boundary independently
...
```sh
pnpm --dir packages/geetorus-runner generate:semantic-contracts
pnpm --dir packages/geetorus-runner check:semantic-contracts
```
