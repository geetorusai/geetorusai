...
Rust `geetorus-runnerd` process is the client. The TypeScript mock core is the
remote peer. Neither side imports Geetorus server, UI, database, or shared
control-plane code.
...
2. The ticket is passed to the runner through
   `GEETORUS_RUNNER_BOOTSTRAP_TICKET`. It is not a command-line argument.
