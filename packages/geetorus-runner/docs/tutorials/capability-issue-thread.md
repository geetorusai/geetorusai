# Capability Clean-Start Tutorial: The Geetorus-Style Issue Thread Over a Mock Control Plane

**Time to first success: about 5 minutes.** Two commands take you from a clean
checkout to 106 passing conformance cases and a Geetorus-style issue thread you
can click through in a browser. The whole tutorial runs from the repository
root. It starts no Geetorus service, contacts no Geetorus control plane, clones
no external eval repository, and holds no provider credential. Everything the
default path needs is checked in under `packages/geetorus-runner/`.
...
for why none of this touches real Geetorus — that is future upload integration and needs separate
approval.
...
- No Rust toolchain for the fake-mode steps (steps 1–4). The optional live steps
  (step 5) build the `geetorus-runnerd` binary and need a Rust toolchain and a
...
```sh
NODE_ENV=development pnpm install --filter @geetorusai/geetorus-runner --frozen-lockfile --offline --ignore-scripts
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner test:capability-evals
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner report:capability-evals
```
...
The report is written to `.geetorus-local/evidence/capability/eval-parity-report.{json,md}`.
...
```sh
pnpm --filter @geetorusai/geetorus-runner test:scenarios
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner console:issue-thread
```

Open <http://127.0.0.1:4184/#/issue/hb-baseline?shot=thread-baseline>.

You are looking at a Geetorus issue thread, not a dashboard. The header carries
three identity chips that stay visible in every mode — `Fake agent`,
`In-process runner`, and `Mock Geetorus` — plus the mock issue identifier
...
pnpm --filter @geetorusai/geetorus-runner check:capability:ui
```

The first writes 24 images — the twelve slugs at 1440×900 and 390×844 — to
`.geetorus-local/evidence/capability/ui/`. Every mobile capture asserts no horizontal
...
real `geetorus-runnerd` process and a real Codex app-server session; the
...
```sh
pnpm --filter @geetorusai/geetorus-runner smoke:capability:ui
```

This creates a session, runs live turns, and asserts the identity reads
`Real Codex` / `Real runnerd` / `Mock Geetorus`, that a real tool call was
...
The frames land in `.geetorus-local/evidence/capability/ui-live/`. They are intentionally
...
```sh
pnpm --filter @geetorusai/geetorus-runner trace:live-runner -- --json
```
...
critically — `noRealGeetorusRequest` and `noGeetorusAuthorityInChild`.
...
- No ACPX implementation and no real control-plane integration. Real Geetorus
