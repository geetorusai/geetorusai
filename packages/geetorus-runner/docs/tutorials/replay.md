...
Replay is a static replay path for a Geetorus Runner Protocol fixture. It uses the same reducer in the CLI and browser.
...
```sh
pnpm install --filter @geetorusai/geetorus-runner --lockfile=false --ignore-scripts --dev
pnpm --filter @geetorusai/geetorus-runner exec playwright install chromium
```

On a minimal Linux host, Playwright may also request system browser libraries:

```sh
pnpm --filter @geetorusai/geetorus-runner exec playwright install-deps chromium
```

## 1. Check the contract and golden corpus

```sh
pnpm --filter @geetorusai/geetorus-runner typecheck
pnpm --filter @geetorusai/geetorus-runner check:replay-goldens
...
```sh
pnpm --filter @geetorusai/geetorus-runner replay:fixture packages/geetorus-runner/protocol/fixtures/replay/source-gap.json
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner replay:fixture packages/geetorus-runner/protocol/fixtures/replay/unsupported-required-version.json
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner check:browser-tokens
pnpm --filter @geetorusai/geetorus-runner test:browser
```

Expected: the Playwright tests pass and temporary screenshots are written under
the ignored `packages/geetorus-runner/test-results/` directory. The regression
...
- No Geetorus server, UI, CLI, adapter, or production database module starts or
