...
brand-new mock Geetorus issue through a real `geetorus-runnerd` process. No
scenario to pick, no recorded transcript, and no real Geetorus service anywhere
in the picture.
...
- Node.js 20 or newer and pnpm 9 or newer (verified with Node 22.22.2 and pnpm
  9.15.4).
- A Rust toolchain, because this path builds the real `geetorus-runnerd`
...
```sh
pnpm --filter @geetorusai/geetorus-runner smoke:capability:cleanroom -- --json
```

This builds the TypeScript output and `geetorus-runnerd`, starts the package
...
```json
{
  "schema": "geetorus.capability.clean-room-smoke.v1",
  "firstIssue": "MCK-2114",
...
- three identity chips — `Real Codex`, `Real runnerd`, `Mock Geetorus` — plus a
...
Codex tools, and Geetorus semantic calls also appear as individual activity
