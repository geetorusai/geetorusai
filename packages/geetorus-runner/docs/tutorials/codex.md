...
Codex gets one small task envelope. Codex does not get the Geetorus skill.
Codex does not get a Geetorus API key or API instructions.
...
- Do not add a Geetorus or OpenAI API key for this tutorial.
...
`AGENTS.md`. In a Geetorus-managed agent run, `GEETORUS_WORKSPACE_CWD` also
...
```sh
pnpm --filter @geetorusai/geetorus-runner typecheck:typescript
...
- the semantic tools are `geetorus_finish` and `geetorus_block`;
...
```sh
if rg -ni 'authorization: bearer|geetorus_api_key|/api/issues/' "$codex_trace"; then
