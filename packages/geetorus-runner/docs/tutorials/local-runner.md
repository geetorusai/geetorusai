...
Geetorus, a network provider, or a real model. It proves cleanup, request
...
```sh
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario happy-path --quiet
```
...
The command writes
`packages/geetorus-runner/.geetorus-local/evidence/local-runner-happy-path.json`.

## 3. Run the scripted control flows

```sh
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario permission-input --quiet
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario interrupted --quiet
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario error --quiet
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario duplicate-terminal --quiet
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario happy-path --duplicate-turn-command --quiet
```
...
ignored `packages/geetorus-runner/test-results/` directory. It never rewrites
