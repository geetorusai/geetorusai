...
The whole tutorial runs from the repository root. It starts no Geetorus
service, contacts no Geetorus control plane, clones no external eval
repository, and holds no provider credential. Everything it needs is checked in
under `packages/geetorus-runner/`.

Capability does not integrate the runner into Geetorus. It builds a package-local
model of what a native Geetorus run *would* do — a deterministic mock control
plane, a transport-neutral semantic tool catalog, an authorization engine, a
106-case conformance suite derived from the Geetorus Evals corpus, and a
...
`.geetorus-local/evidence/capability/eval-parity-report.{json,md}`. It is a
generated-on-demand artifact, not a committed file; a clean checkout does not
contain it. Delete it before you run `pnpm --filter @geetorusai/geetorus-runner docs:validate`
...
```sh
pnpm --filter @geetorusai/geetorus-runner demo:scenarios
```
...
`@geetorus-runner-local/capability` alias — deliberately not the published package
...
pnpm --filter @geetorusai/geetorus-runner check:capability-inventory
pnpm --filter @geetorusai/geetorus-runner test:capability-inventory
...
- **No Geetorus integration.** Nothing here touches a real control plane,
