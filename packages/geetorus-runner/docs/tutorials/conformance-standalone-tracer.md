...
Conformance is the smallest standalone runner path. It uses a mock control plane and does not start Geetorus.
...
the `geetorus-runner-core` crate under `runner/target/`.
...
```json
{"schemaVersion":"geetorus.runner.conformance.output.v1","runIdentity":{"runId":"run_conformance_0001","sessionId":"session_conformance_0001"},"result":{"status":"succeeded","summary":"Standalone Conformance fixture accepted."}}
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner check:conformance-parity
```
...
```sh
sed -n '1,240p' packages/geetorus-runner/protocol/fixtures/conformance-minimal-run.json
```
