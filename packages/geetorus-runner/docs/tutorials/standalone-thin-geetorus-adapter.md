...
`packages/geetorus-runner/`. It does not install the runner into Geetorus,
call a Geetorus API, change an instance flag, edit an agent profile, create a
Geetorus task, or use the repository server/database integration.
...
pnpm --filter @geetorusai/geetorus-runner run build:standalone
```

These checks require no running Geetorus service or credentials.
...
5. Confirm no Geetorus instance or repository path outside
   `packages/geetorus-runner/` was needed.
