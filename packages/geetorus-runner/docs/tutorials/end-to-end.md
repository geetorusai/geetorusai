...
It currently includes Conformance through the Standalone Geetorus adapter tracer.
...
together. It does not use the Geetorus control plane. Codex uses a real
...
Standalone consumes the public runner contract from Geetorus behind a default-off
...
4. Confirm the shell prompt returns and no Geetorus service was started.
...
11. Inspect the exact model-context snapshot and confirm that it has no
    Geetorus instructions, bearer credentials, or unrelated skills.
...
18. Follow [Standalone: Run the Thin Geetorus Adapter](standalone-thin-geetorus-adapter.md).
19. Run the unchanged port conformance suite against mock and database-backed
    Geetorus ports, then inspect one local feature-flagged task.
...
- deliberate TypeScript and Cargo references to Geetorus core are rejected;
- documentation and journal indexes are machine checked;
- the package remains runnable without Geetorus core;
...
- CLI and browser diagnostics do not expose bootstrap or connection-lease
  tokens.
- the Codex child receives an allowlisted environment without Geetorus or
...
`GEETORUS_LIVE_CONSOLE_DRIVER=codex` to run the identical screens against a real
Codex session.
