...
Local runner proves one native local session. It uses no Geetorus service, database,
network provider, or model. The TypeScript mock core, Rust runner, and Rust fake
harness are all inside `packages/geetorus-runner/`.

## Processes and transports

| Boundary | Transport | Input | Output |
|---|---|---|---|
| Mock core → runner | stdin JSONL | `geetorus.prp.command.v1` | — |
| Runner → mock core | stdout JSONL | — | `geetorus.runner.stream.v1` |
| Runner → fake harness | stdin JSONL | `geetorus.fake_harness.command.v1` | — |
| Fake harness → runner | stdout JSONL | — | `geetorus.fake_harness.message.v1` |
...
the inherited environment before it starts the fake harness. Geetorus keys,
...
Geetorus API. Vite dev and preview default to `127.0.0.1`. Every
...
harness, production Geetorus bridge, or browser-to-runner connection. Those
