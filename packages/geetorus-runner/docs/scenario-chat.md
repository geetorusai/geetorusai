...
send prompts, and watch the mock Geetorus activity for every turn instead of
having it hidden behind the conversation. It contacts no Geetorus service,
...
```sh
pnpm --filter @geetorusai/geetorus-runner run demo:scenarios       # open the surface
pnpm --filter @geetorusai/geetorus-runner run test:scenarios       # runtime + component tests
