...
`plan.updated` / `geetorus.plan.updated.v1` is a complete, ordered snapshot of
the provider's checklist for one active turn. It is not a Geetorus Plan
...
Harness-initiated forms cross PRP as `geetorus.runtime_request.v2` with
`requestKind: "runtime"`, `type: "input"`, and an embedded
`geetorus.question_set.v1`. A submission is always
`{ "action": "submit", "response": geetorus.question_response.v1 }`.
...
`geetorus.question_adapter_fixture.v1` fixture proving its native request
...
process is lost first, Geetorus emits one non-replayable
...
`geetorus.native-execution-input.v4` pins the effective harness permission
...
- Mock-core commands use `geetorus.prp.command.v1` over stdin JSONL.
- Runner output uses `geetorus.runner.stream.v1` over stdout JSONL.
- Fake-harness commands use `geetorus.fake_harness.command.v1`.
- Fake-harness output uses `geetorus.fake_harness.message.v1`.
...
declared by `GEETORUS_RUNNER_COMPATIBILITY` and checked before execution by
`assertGeetorusRunnerCompatibility`. A mismatch fails with
`geetorus_runner_incompatible` and stable per-issue codes; a provider-specific
...
App/Evals join. `assertGeetorusRunnerEvalCompatibility` requires simultaneous
...
`geetorus_runner_eval_incompatible` with expected/received values for every
mismatch and must run before launching a provider.

runnerd itself reports `geetorus-runner/runnerd-build-metadata/v1` from
`--build-metadata`. The consumer passes its path and expected content digest to
`resolveGeetorusRunnerdArtifact`; implicit PATH or source-tree discovery is
not part of the contract. Native attempt output is
`geetorus-runner/native-execution/v1`, whose parser accepts unknown additive
