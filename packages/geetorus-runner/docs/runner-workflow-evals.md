...
The workspace-private `@geetorusai/geetorus-eval-kernel` package owns only
...
rendered by the canonical `geetorus-evals` report program so live results use
...
- `pnpm --filter @geetorusai/geetorus-runner report:runner-workflow-evals`
  validates the deterministic fail-closed fixture matrix and writes JSON,
  Markdown, JUnit, and GitHub-safe artifacts under
  `.geetorus-local/evals/workflows/`. It makes no network requests.
- `pnpm --filter @geetorusai/geetorus-runner report:runner-live-evals` runs
...
environment. `GEETORUS_EVAL_MAX_CAMPAIGN_COST_USD` must be a positive finite
...
`.geetorus-local/evals/workflows/evalbook-runs/`, then invoke
`evals/geetorus-runner/tools/eval_program.py report` from a `geetorus-evals`
checkout. That program writes the canonical matrix to
`.geetorus-local/evals/workflows/index.html`, plus `latest.html`, test pages,
and attempt pages. Set `GEETORUS_EVALBOOK_PROGRAM` to the program's absolute
path or `GEETORUS_EVALS_ROOT` to its repository root. Conventional sibling
worktree locations are discovered automatically. GitHub Actions checks out a
pinned `geetorus-evals` revision, so every hosted run uses the same reviewed
