...
Capability turns the Geetorus Evals corpus into an executable, offline
...
The reporter writes `.geetorus-local/evidence/capability/eval-parity-report.{json,md}`.
...
```sh
pnpm --filter @geetorusai/geetorus-runner report:capability-live-evals
```
