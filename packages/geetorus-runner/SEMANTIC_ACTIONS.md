...
bindings, Codex receives no dynamic Geetorus tools.
...
`GEETORUS_SEMANTIC_ACTION_CATALOG` and every nested declaration are frozen.
`geetorusSemanticAction` returns `undefined` for unknown operation IDs.

## Run-scoped authority

`GeetorusSemanticDispatcher` accepts a current-context provider and an
...
```sh
pnpm --filter @geetorusai/geetorus-runner generate:semantic-action-catalog
```
