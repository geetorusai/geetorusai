...
## Real-binding status (before real Geetorus service binding)

No operation is bound to a real Geetorus service yet; the deterministic mock is
...
operation for the well-known aliases (for example `geetorusListAgents` →
`list_agents`, `geetorusGetAgent` → `get_agent`, `geetorusListIssues` →
`search_tasks`, `geetorusCreateIssue` → `create_task`).
