...
3. The arbiter is always the subject of status sentences: "*Geetorus kept In Progress — evidence incomplete*".
...
- Claim chip: `rounded-full border border-dashed border-border px-2 text-xs text-muted-foreground`, leading `MessageSquareQuote` (14px). Tooltip: "What the agent reported. Geetorus decides issue status separately."
...
[glyph] Kept In Progress — completion claim not accepted: evidence incomplete
        by Geetorus arbiter · triggered by run finalization · 12m ago     [∨]
```
...
3. `FinalizationErrorCard`: "**Finalization failed — completion claim preserved, not accepted.** Reconciliation scheduled · owner: Geetorus recovery · retry 1 of 3." For `result_schema_rejected`: "Agent report couldn't be read — status unchanged" + preserved raw payload link.
...
1. On any finalized native run, a stranger can answer separately: did the turn finish, did the run succeed, what did the agent claim, what did Geetorus decide — without opening a drawer (spec §18.11 consistency gate, UI bullet).
