...
const PANEL_OPEN_KEY = "geetorus-runner.capability.panel.open";
/**
 ...
 */
const CHAT_PANEL_OPEN_KEY = "geetorus-runner.capability.chat.panel.open";
const PANEL_WIDTH_KEY = "geetorus-runner.capability.panel.width";
const PANEL_MIN = 320;
const PANEL_MAX = 960;
...
const CHAT_HISTORY_KEY = "geetorus-runner.capability.chat.history.v1";
const CHAT_HARNESS_KEY = "geetorus-runner.capability.chat.harness.v1";
const MODEL_PRESETS = {
...
declare global {
  interface Window { __GEETORUS_EVAL_REPORT__?: EmbeddedEvalReport }
}
...
function currentTurnActivity(snapshot: CapabilityIssueThreadSnapshot): string {
  ...
  if (activity?.kind === "tool_activity") {
    return activity.status === "running"
      ? `Geetorus tool · ${activity.operationId}`
      : `Geetorus tool completed · ${activity.operationId}`;
  }
...
export function App() {
  const embeddedEval = window.__GEETORUS_EVAL_REPORT__ ?? null;
  ...
  useEffect(() => {
    if (embeddedEval !== null) {
      document.title = `${embeddedEval.passed ? "✓" : "✕"} ${embeddedEval.caseId} · geetorus-runner eval`;
      return;
    }
    document.title = chat
      ? "🫧 Mock Geetorus · Issue thread"
      : "🧯 Mock Geetorus · Issue thread";
  }, [chat, embeddedEval]);
                ...
                {snapshot.turns.length === 0 ? (
                  <section className="pit-empty-thread" data-testid="clean-room-empty">
                    ...
                    <p>
                      ...
                      <strong>Mock Geetorus (clean room)</strong>. Nothing has been said, called, or
                      recorded yet.
                    </p>
                    <p className="pit-muted">
                      ...
                      in the mock control plane only — never a real Geetorus API. Detailed tool,
