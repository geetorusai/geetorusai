...
test.describe("Capability issue thread", () => {
  test("loads the bundled sans and mono faces before the thread settles", async ({ page }) => {
    ...
    const probe = await page.evaluate(async () => {
      ...
      return {
        ...
        sansWeightsLoaded: [400, 500, 600, 700].every((weight) =>
          document.fonts.check(`${weight} 16px "${sansFamily}"`, "Geetorus"),
        ),
    ...
    expect(probe).toEqual({
      sansFamily: "Geetorus Issue Thread Inter",
      monoFamily: "Geetorus Issue Thread DejaVu Sans Mono",
      symbolFamily: "Geetorus Issue Thread Symbols",
      sansStatuses: ["loaded"],
  ...
  test("baseline thread renders identity, turns, and the §3 item types", async ({ page }) => {
    ...
    await expect(chips.getByTestId("control-plane-chip")).toHaveText("Mock Geetorus");
...
function devtoolsPayload() {
  const state = {
    company: { id: "company-1", name: "Mock Geetorus" },
  ...
  return {
    schema: "geetorus.capability.devtools.v1",
    currentRevision: 4,
...
function cleanRoomView(identifier: string, withTurn: boolean) {
  const guard = {
    ...
    turnId: "turn-0",
    category: "session",
    outcome: "no_real_geetorus_request",
    reason: "Real Geetorus API requests: 0. Child GEETORUS_* environment keys: none.",
    stateRevision: 3,
    threadAnchorId: null,
  };
  return {
    schema: "geetorus.capability.issue-thread-view.v1",
...
function turnStreamBody(payload: unknown, interim: unknown[] = []): string {
  const frames = interim.map((view, index) => ({
    schema: "geetorus.capability.turn-stream.v1",
    type: "frame",
    seq: index + 1,
    reason: "delta",
    turnId: "turn-1",
...
test.describe("Capability clean-room chat", () => {
  ...
  test("a new chat opens a blank live thread with no scenario controls", async ({ page }) => {
    ...
    await expect(page).toHaveTitle("🫧 Mock Geetorus · Issue thread");
  ...
  test("evidence stays collapsed until it is asked for", async ({ page }) => {
    ...
    await expect(page.getByText("Real Geetorus API requests: 0")).toBeVisible();
  });
  ...
  test("Runner events expand completely while stream deltas stay grouped", async ({ page }) => {
      ...
      view.evidence.runner = [
        {
          ...
          details: [
            ...
            { label: "Runner", value: "geetorus-runnerd" },
          ],
        },
        ...[2, 3].map((ordinal) => ({
          id: `delta-${ordinal}`,
    ...
    await expect(session).toContainText("geetorus-runnerd");
    ...
    await page.route(CLEAN_ROOM_API, async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          message: "geetorus-runnerd exited before the Codex app-server was ready",
        }),
      });
    });
    await page.goto("/#/chat");
