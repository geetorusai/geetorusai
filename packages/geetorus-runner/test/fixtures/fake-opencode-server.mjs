...
const session = { id: "ses_fake_1", title: "Geetorus fake" };
...
const mcp = runtimeConfig.mcp?.geetorus;
let mcpRequestId = 1;
...
async function callFirstGeetorusTool() {
  if (!mcp?.url) return;
...
async function callTerminalTool(promptBody) {
  ...
  return mcpRequest("tools/call", {
    name: blocked ? "geetorus_block" : "geetorus_finish",
    arguments: result,
  });
}
          ...
          emit({
            ...
            properties: {
              ...
              part: {
                ...
                type: "tool",
                tool: "unknown",
                callID: "functions.geetorus_geetorus_finish:24",
                state: {
                  status: "error",
