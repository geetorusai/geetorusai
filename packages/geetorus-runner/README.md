# Geetorus Native Runner

This package is the standalone development boundary for Geetorus's native
...
imports or starts Geetorus's server, UI, CLI, or production database.

## Public package surfaces

- `@geetorusai/geetorus-runner` — production contracts, clients/backends,
  PRP validation/replay, canonical catalog/dispatcher, and compatibility check.
- `@geetorusai/geetorus-runner/testing` — deterministic mocks plus PRP and
  semantic conformance kits. Tests and external conformance consumers import
  this explicitly.
- `@geetorusai/geetorus-runner/evals` — versioned native-attempt metadata,
...
`@geetorusai/geetorus-eval-kernel`; scenario content and provider-backed eval
...
decisions. The production adapter stays App-owned and invokes Geetorus's real
...
The package also builds `geetorus-runner-acpx-sidecar`. This bounded v2
...
For ACPX Claude, `approve-reads` is shown as **Allow Geetorus reads**. The host
...
`geetorus` connection is always the runner's authenticated tool bridge; ambient
...
spend, and invocation limits. No provider process receives a Geetorus API
...
```sh
pnpm install --filter @geetorusai/geetorus-runner --lockfile=false --offline --ignore-scripts --dev
pnpm --filter @geetorusai/geetorus-runner verify
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner verify:rootless
```
...
```json
{
  "schemaVersion": "geetorus.runner.conformance.output.v1",
  "runIdentity": {
...
```sh
pnpm --filter @geetorusai/geetorus-runner trace:conformance
```
...
```sh
pnpm --filter @geetorusai/geetorus-runner replay:fixture
pnpm --filter @geetorusai/geetorus-runner trace:local-runner -- --scenario happy-path
pnpm --filter @geetorusai/geetorus-runner trace:codex
pnpm --filter @geetorusai/geetorus-runner demo:live-console -- --host 127.0.0.1 --port 4174

# Live console: chat with a live session in the browser.
pnpm --filter @geetorusai/geetorus-runner console:live-console
pnpm --filter @geetorusai/geetorus-runner browser:dev --host 127.0.0.1 --port 4179

# SDK: open the public-SDK reference console and mini consumer.
pnpm --filter @geetorusai/geetorus-runner console:sdk
...
pnpm --filter @geetorusai/geetorus-runner trace:standalone -- --feature-flag enabled
pnpm --filter @geetorusai/geetorus-runner trace:standalone -- --feature-flag enabled --kill-switch enabled
pnpm --filter @geetorusai/geetorus-runner demo:standalone
...
`geetorus-evals` repository under `evals/geetorus-runner/`. Its
`live-mini.json` roster is the complete 35-case Codex qualification lane. Build
this package's TypeScript output, release `geetorus-runnerd`, package tarball,
...
`geetorus-runner-eval-session` (`dist/cli/eval-session.js`). Evalbook owns the
...
```sh
pnpm --filter @geetorusai/geetorus-runner test:runner-workflow-evals
pnpm --filter @geetorusai/geetorus-runner report:runner-chaos-evals
```
...
`GEETORUS_EVAL_MAX_CAMPAIGN_COST_USD` to a positive finite number to bound
additional scheduling after the observed campaign total reaches that value:

```sh
GEETORUS_EVAL_MAX_CAMPAIGN_COST_USD=12 \
  GEETORUS_EVALS_ROOT=/path/to/geetorus-evals \
  pnpm --filter @geetorusai/geetorus-runner report:runner-live-evals
...
```sh
pnpm --filter @geetorusai/geetorus-runner test:aws-agentcore-provisioning
pnpm --filter @geetorusai/geetorus-runner aws-agentcore:provision -- --dry-run
pnpm --filter @geetorusai/geetorus-runner aws-agentcore:provision
pnpm --filter @geetorusai/geetorus-runner aws-agentcore:probe
pnpm --filter @geetorusai/geetorus-runner aws-agentcore:lab
pnpm --filter @geetorusai/geetorus-runner smoke:capability:aws-agentcore
pnpm --filter @geetorusai/geetorus-runner aws-agentcore:destroy -- --yes
```
...
Geetorus ownership tags and template description match. A verified
...
| `check:forbidden-imports`                               | Reject TypeScript imports and Cargo path dependencies that cross into Geetorus core.                                                       |
...
| `report:runner-live-evals`                              | Execute the paid provider schedule and render its immutable attempts with the canonical `geetorus-evals` HTML grid.                        |
...
| `console:issue-thread`                                  | Start the Geetorus-style issue thread on `127.0.0.1:4184`.                                                                                 |
...
two standalone consumers. Runtime production Geetorus integration remains
