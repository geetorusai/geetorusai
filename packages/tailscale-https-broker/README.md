# Geetorus Tailscale HTTPS broker

Least-privilege host broker that manages **only** Geetorus-owned, tailnet-only,
same-number HTTPS-to-loopback listeners for managed branch runtimes.

It exists so the Geetorus app/agent account never gains Tailscale operator
...
for the configured Geetorus service UID. Connection deadlines destroy the
...
## One-time host installation (`geetorus-dev`)

These steps require **root** and must be run by CloudOps/host owner, not the
Geetorus agent account. They install the broker as a dedicated
Tailscale-operator service account distinct from the Geetorus app account.
...
2. **Create the dedicated operator account and socket group.**
   ...
   ```sh
   sudo useradd --system --home /var/lib/geetorus-tailscale-broker \
     --shell /usr/sbin/nologin geetorus-tsbroker
   sudo groupadd --system geetorus-tsbroker-sock
   # The Geetorus *app* service account must have this as its PRIMARY group so
   # its SO_PEERCRED gid matches the socket group (supplemental membership is
   # intentionally NOT accepted).
   sudo usermod -g geetorus-tsbroker-sock <geetorus-app-account>
   ```

3. **Grant Tailscale operator authority to the broker account only.**

   ```sh
   sudo tailscale set --operator=geetorus-tsbroker
   ```

   Do **not** grant `--operator` to the Geetorus app/agent account (that grant
   was explicitly rejected in PAP-16989).

4. **Create state directories (not writable by the Geetorus app).** The
   packaged unit creates these automatically; for a manual install use:

   ```sh
   sudo install -d -o geetorus-tsbroker -g geetorus-tsbroker-sock -m 0750 /run/geetorus-tailscale-broker
   sudo install -d -o geetorus-tsbroker -g geetorus-tsbroker-sock -m 0700 /var/lib/geetorus-tailscale-broker
   sudo install -d -o geetorus-tsbroker -g geetorus-tsbroker-sock -m 0700 /var/log/geetorus-tailscale-broker
   ```

   The broker refuses to start if the registry path's parent is group/other
   writable.

5. **Build, install the package under `/opt/geetorus`, and install the
   packaged systemd unit.** The unit's `ExecStart` (and the doctor command
   below) run the build output from
   `/opt/geetorus/packages/tailscale-https-broker/dist`, so copy it there
   ...
   ```sh
   pnpm --filter @geetorusai/tailscale-https-broker build
   sudo install -d -m 0755 /opt/geetorus/packages/tailscale-https-broker
   sudo cp -r packages/tailscale-https-broker/dist \
     /opt/geetorus/packages/tailscale-https-broker/
   sudo install -D -m 0644 \
     packages/tailscale-https-broker/deploy/geetorus-tailscale-https-broker.service \
     /etc/systemd/system/geetorus-tailscale-https-broker.service
   sudo install -d -m 0750 /etc/geetorus
   sudoedit /etc/geetorus/tailscale-https-broker.env
   ```
   ...
   ```ini
   [Unit]
   Description=Geetorus Tailscale HTTPS broker
   After=tailscaled.service
   Requires=tailscaled.service

   [Service]
   Type=simple
   User=geetorus-tsbroker
   # Socket must end up 0660 geetorus-tsbroker:geetorus-tsbroker-sock. Set the group here and
   # the broker chmods the socket to 0660 on bind.
   Group=geetorus-tsbroker-sock
   EnvironmentFile=/etc/geetorus/tailscale-https-broker.env
   ExecStart=/usr/bin/node /opt/geetorus/packages/tailscale-https-broker/dist/main.js
   Restart=on-failure
   NoNewPrivileges=true
   ProtectSystem=strict
   ReadWritePaths=/run/geetorus-tailscale-broker /var/lib/geetorus-tailscale-broker /var/log/geetorus-tailscale-broker
   ...
   `GEETORUS_TAILSCALE_BROKER_SOCKET=/run/geetorus-tailscale-broker/broker.sock`
   on the Geetorus service only if overriding its default.
   ...
   | `BROKER_SERVICE_UID` | yes | — | UID of the Geetorus **app** account allowed to connect |
   | `BROKER_SERVICE_GID` | yes | — | GID of the dedicated socket group (caller's primary GID) |
   | `BROKER_RUNTIME_UID` | yes | — | UID that owns Geetorus-managed runtime processes (normally the Geetorus app service account); only its loopback listeners are eligible |
   | `BROKER_TAILSCALE_BIN` | no | `/usr/bin/tailscale` | absolute path to the Tailscale CLI |
   | `BROKER_SOCKET_PATH` | no | `/run/geetorus-tailscale-broker/broker.sock` | Unix socket path |
   | `BROKER_REGISTRY_PATH` | no | `/var/lib/geetorus-tailscale-broker/registry.json` | root-owned `0600` ownership registry |
   | `BROKER_AUDIT_PATH` | no | `/var/log/geetorus-tailscale-broker/audit.log` | append-only security audit log |
   ...
   *provenance-blind*: it protects only entries the broker has no lease for. It
   ...
   ```sh
   sudo -u geetorus-tsbroker \
     env $(cat /etc/geetorus/tailscale-https-broker.env | xargs) \
     node /opt/geetorus/packages/tailscale-https-broker/dist/main.js --doctor
   ```
...
7. **Enable.** `sudo systemctl daemon-reload && sudo systemctl enable --now
   geetorus-tailscale-https-broker`. Confirm the socket is `0660
   geetorus-tsbroker:geetorus-tsbroker-sock`.

## Upgrade

Deploy new package output to
`/opt/geetorus/packages/tailscale-https-broker/dist`, then
`sudo systemctl restart geetorus-tailscale-https-broker`. On
...
1. Disable the exposure flag on the project runtime (Geetorus stops requesting
   `expose`). Existing previews drain on runtime stop.
2. Drain owned listeners: stop each managed runtime so Geetorus issues `remove`
   for its own leases (proven by handle).
3. `sudo systemctl disable --now geetorus-tailscale-https-broker`.
...
```sh
pnpm --filter @geetorusai/tailscale-https-broker test        # 72 tests
pnpm --filter @geetorusai/tailscale-https-broker typecheck
pnpm --filter @geetorusai/tailscale-https-broker build
```
