...
First-party MCP server for Google Sheets API v4. It can run as a Geetorus
`local_stdio` gallery connection or as a local Streamable HTTP server for
Geetorus's `remote_http` connect-by-link flow.
...
```sh
geetorus-google-sheets-mcp-server \
...
## Geetorus `local_stdio` Test Path

Use the Google Sheets gallery app when you want Geetorus to supervise the
server as a stdio MCP process:

1. Configure Geetorus's Google Sheets service account environment so the
...
2. Share every spreadsheet you want to test with the service account's
   `client_email`.
3. In Geetorus, open the tool app gallery, choose **Google Sheets**, and paste
   one or more Google Sheets links.
4. Save the app connection. Geetorus creates a `local_stdio` connection using
   the `geetorus.google-sheets` template and passes the selected spreadsheet
...
geetorus-google-sheets-mcp-http-server
```
...
Then in Geetorus, choose the remote HTTP or "connect with a link" path, paste
...
`GOOGLE_SHEETS_SPREADSHEET_IDS`), not a per-connection Geetorus gallery wizard
