...
const EXPECTED_BUNDLED_KEYS = [
  "geetorusai/bundled/docs/doc-maintenance",
  "geetorusai/bundled/geetorus-operations/issue-triage",
  "geetorusai/bundled/geetorus-operations/reflection-coach",
  "geetorusai/bundled/geetorus-operations/status-card-query",
  "geetorusai/bundled/geetorus-operations/summarize-status",
  "geetorusai/bundled/geetorus-operations/task-planning",
  "geetorusai/bundled/product/geetorus-capsules",
  "geetorusai/bundled/product/wireframe",
  "geetorusai/bundled/quality/qa-acceptance",
  "geetorusai/bundled/software-development/github-pr-workflow",
];

const EXPECTED_OPTIONAL_KEYS = [
  "geetorusai/optional/browser/agent-browser",
  "geetorusai/optional/content/release-announcement",
  "geetorusai/optional/content/simplified-english",
  "geetorusai/optional/finance/ramp",
  "geetorusai/optional/product/design-critique",
  "geetorusai/optional/research/last30days",
  "geetorusai/optional/software-development/prepare-mcp-integration",
];
...
function listSkillFiles(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    ...
    // shipped Geetorus skills and must not participate in this repo audit.
...
describe("shipped skills catalog", () => {
  ...
  it("uses canonical geetorusai keys derived from kind/category/slug", () => {
    const violations: string[] = [];
    for (const skill of catalogSkills) {
      const expectedKey = `geetorusai/${skill.kind}/${skill.category}/${skill.slug}`;
      const expectedId = `geetorusai:${skill.kind}:${skill.category}:${skill.slug}`;
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@geetorusai/skills-catalog");
  ...
  it("resolves shipped skills by id, key, and unique slug", () => {
    const sample = catalogSkills.find((skill) => skill.key === "geetorusai/bundled/software-development/github-pr-workflow");
  ...
  it("keeps the Ramp wrapper fail-closed on mixed-provenance playbooks", () => {
    ...
    expect(rampSkill).toContain("do not execute them inside Geetorus unless a Geetorus approval explicitly names the playbook");
