...
const CATALOG_PACKAGE_NAME = "@geetorusai/skills-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @geetorusai/skills-catalog build:manifest.");
    }
  }
...
function skillIdForCandidate(candidate: BaseSkillCandidate) {
  return `geetorusai:${candidate.kind}:${candidate.category}:${candidate.slug}`;
}
