...
- **No reflection on yourself.** If `targetAgentId == GEETORUS_AGENT_ID`, refuse and ask for another coach.
...
```sh
curl -sS "$GEETORUS_API_URL/api/agents/<targetAgentId>" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```

Record `name`, `role`, `reportsTo`, `adapterType`, `adapterConfig.instructionsFilePath` (where `AGENTS.md` lives), and current assigned skills via `GET /api/agents/<targetAgentId>/skills`. Refuse and exit if `targetAgentId == $GEETORUS_AGENT_ID`.

### 2) Pull the recent record

```sh
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues?assigneeAgentId=<targetAgentId>&status=done,in_review,blocked&limit=25" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```sh
curl -sS "$GEETORUS_API_URL/api/issues/<issueId>" -H "Authorization: Bearer $GEETORUS_API_KEY"
curl -sS "$GEETORUS_API_URL/api/issues/<issueId>/comments" -H "Authorization: Bearer $GEETORUS_API_KEY"
```

Keep status transitions, blocker reasons, reviewer comments, approval outcomes, human corrections, and PR-link comments. Comments are the closest thing Geetorus has to an execution trace — treat them as first-class evidence.
...
- [ ] `targetAgentId != $GEETORUS_AGENT_ID`
