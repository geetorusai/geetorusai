...
Use this reference when the request is to generate Geetorus capsule graphics, capsule identicons, or reusable visual assets.
...
2. Deterministic identicon/profile-pill prototype:
   - `geetorus-content/design/PAP-11825/geetorus-capsule-identicon-prototyper/README.md`
   - `src/identicon.ts`
   - `src/App.tsx`
3. Website embedded generator:
   - `geetorus-website/public/brand/generator.js`
   - `geetorus-website/src/components/brand/sections/08-imagery.html`
...
Use this when the task needs reproducible generated capsule art beyond the small embedded website tool. The contract here is intentionally mirrored into Geetorus so agents can proceed if the original prototype repository is renamed, deleted, or private.
...
If an implementation needs source code, prefer the Geetorus website embedded generator or a Geetorus-owned tool. Use any external prototype link only as optional historical context, not as required task input.

Palette caution:

- The `duotones` palette matches the website 12-preset capsule palette.
- Experimental palettes such as vaporwave, cyberpunk, ocean, and jewel are generator options, not canonical Geetorus brand palettes.
- Do not use experimental palettes when the request asks for strict Geetorus brand work.
...
- Divergence from canonical Geetorus rendering:
```
