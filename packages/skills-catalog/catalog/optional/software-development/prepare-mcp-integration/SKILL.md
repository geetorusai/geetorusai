...
description: >
  ...
  exact-revision human approval, and one governed Geetorus connector PR per
...
key: geetorusai/optional/software-development/prepare-mcp-integration
recommendedForRoles:
  - engineer
  - product-manager
  - researcher
...
research PR in `geetorus-content`, then connector implementation in Geetorus
App only after a human accepts the exact research revision and connection set.

## Preserve These Boundaries

- Treat `geetorus-content/integrations/README.md` as the research contract and
  `geetorus-content/integrations/skills/integration-harness/SKILL.md` as its
...
- Treat `geetorus/doc/connections/CONNECTOR-PLAYBOOK.md` on the implementation
...
- Create one Geetorus App PR per approved connection. Shared prerequisite
...
- Request the broadest vendor permissions and scopes the connection can
  ...
  Geetorus's action catalog, resource boundaries, ask-first policies,
...
- Treat vendor permission breadth and Geetorus execution governance as
...
1. Load the current Geetorus skill for checkout, comments, interactions,
   durable state, and final disposition. Load the standard PR-preparation skill
   (`prepare-geetorus-pr` for Geetorus agents) before opening any PR.
2. Resolve the input URL(s), vendor/platform, intended MCP endpoint or API,
   target repositories and branches, and the Geetorus issue that owns the
...
6. Prefer official vendor documentation, protocol/RFC sources, safe live
   probes, and current Geetorus code. Use third-party sources only to find or
...
Maintain one concise checkpoint in the Geetorus issue or an issue document:
...
## Phase A: Research In geetorus-content
...
3. Build the complete reviewable OKF/planning package required by the current
   ...
   - exact service involvement and system boundaries in Geetorus.
4. Ground every Geetorus-surface claim in the maintained surface map and
...
5. Open one research-only PR to `geetorus-content`. Include the full planning
   package and any tightly coupled content-playbook correction, but no
   Geetorus App implementation.
...
2. Create a Geetorus `request_confirmation` interaction targeted at that issue
...
## Phase B: Implement In Geetorus App
