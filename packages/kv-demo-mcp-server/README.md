...
mutate — so you can call a tool from a Geetorus agent and watch the value
...
the right fixture for showing what Geetorus stores versus what the demo
package stores — the package stores the values, Geetorus stores the
...
- Four MCP tools (over the Streamable HTTP transport):
  ...
  - `kv_delete` (destructive) — delete a key. Carries `destructiveHint: true`
    so Geetorus's catalog quarantines it on first sight.
...
```sh
pnpm --filter @geetorusai/kv-demo-mcp-server build
...
MCP tools share state, and listens on a fixed loopback port. Geetorus's
...
4. If you launched the server with `KV_DEMO_TOKEN`, paste the token in the
   **App key** field. The wizard stores it as an `Authorization: Bearer …`
   header secret.
5. Pick the profile defaults (read/write/destructive) and finish. Geetorus
...
- **`remote_http` (recommended for this demo)** — required if you want the
  ...
  from that process. Geetorus's `remote_http` gateway forwards every call to
...
- **`local_stdio` (not used here)** — runs MCP servers as supervised child
  processes inside Geetorus's runtime slots. Reserved for *trusted local
  deployments* (developer laptop, `local_trusted` or
  `authenticated/private` with `GEETORUS_TRUSTED_MCP_RUNTIME_HOST` set on a
  ...
  ship in the Geetorus build when you need stdio; do not try to shoehorn
...
## What you should see in Geetorus
...
| Concern | Stored in this package | Stored in Geetorus |
| --- | --- | --- |
| Key/value entries (`kv_*` data) | In-memory `Map`, lost on restart. | Not stored. Geetorus never sees the values directly; the gateway only sees the MCP request/response envelope and the redacted-by-policy view the audit log keeps. |
...
Resetting the demo state usually means resetting this process; Geetorus's
...
- **Free the port** — if startup logs `EADDRINUSE`, another `kv-demo`
  ...
  kill <pid>
  ```
- **Quiesce the Geetorus side** — disable the connection so the gateway
