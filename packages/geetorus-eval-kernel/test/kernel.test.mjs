...
import {
  GEETORUS_EVAL_KERNEL_COMPATIBILITY,
  GeetorusEvalKernelConfigurationError,
  runGeetorusEvalMatrix,
} from "../dist/index.js";

test("runs a caller-owned scenario/candidate matrix", async () => {
  const results = await runGeetorusEvalMatrix({
  ...
  assert.equal(GEETORUS_EVAL_KERNEL_COMPATIBILITY.apiVersion, 1);
  ...
  await assert.rejects(
    runGeetorusEvalMatrix({
      ...
      candidates: [{
        id: "candidate-a",
        config: null,
        preflight: () => { throw new Error("geetorus_runner_incompatible"); },
      }],
      execute: async () => { executed = true; },
      score: () => null,
    }),
    /geetorus_runner_incompatible/,
  );
