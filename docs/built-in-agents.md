...
Built-in agents are first-party, company-scoped agents that Geetorus can resolve by a stable registry key. They are normal rows in `agents`, but they carry immutable metadata under `metadata.geetorusBuiltInAgent` so services can find them without hardcoding a database id.
...
- Marker: `server/src/services/built-in-agent-metadata.ts` reads and writes `metadata.geetorusBuiltInAgent`.
...
8. Run the focused tests from the repo root with `pnpm --filter @geetorusai/server exec vitest run src/__tests__/built-in-agents.test.ts src/__tests__/built-in-agent-routes.test.ts`.
...
+    defaultInstructions:
+      "You are Geetorus's built-in Digest agent. Produce short, sourced summaries of recent company activity, decisions, blockers, and next actions.",
+    defaultRole: "general",
...
```sh
pnpm --filter @geetorusai/server exec vitest run src/__tests__/built-in-agents.test.ts src/__tests__/built-in-agent-routes.test.ts
```
