...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Geetorus saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
# Installed CLI
geetorusai feedback report

# Point to a different server or company
npx geetorusai feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Geetorus feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Geetorus envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
npx geetorusai feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Geetorus falls back to `https://telemetry.geetorus.ing`
