...
The **Skills Store** is Geetorus's library of reusable skills. A skill is a markdown
...
|---|---|---|
| **The catalog** | A curated, read-only set of skills that ships with Geetorus | The `@geetorusai/skills-catalog` package |
...
- **`bundled`** — first-party Geetorus skills (e.g. `issue-triage`, `task-planning`,
  `qa-acceptance`, `wireframe`, `github-pr-workflow`, `doc-maintenance`). These carry the
  reserved `geetorusai/geetorus/...` key namespace.
...
- **`category`** — grouping such as `software-development`, `quality`, `product`,
  `research`, `content`, `browser`, `geetorus-operations`, `docs`.
...
|---|---|---|
| `catalog` | Geetorus / catalog | Installed from the bundled catalog |
...
`ramp` skill is the model: Geetorus ships the stable governance wrapper, source
...
a vendored snapshot would go stale, and when Geetorus can keep the safety boundary in
the wrapper. For financial, legal, or account-control domains, the wrapper must require
Geetorus approvals before spend, incorporation, account authorization, card issuance,
...
Paste a source and Geetorus fetches and imports it. Accepted forms include:
...
The catalog is its own publishable package, `@geetorusai/skills-catalog`:
