...
Geetorus's MCP permission work uses deterministic fixture servers so policy
...
checks the local Geetorus `/api/health` endpoint when available, then exercises:
...
```sh
pnpm smoke:mcp-fixtures -- --geetorus-url http://127.0.0.1:3100
```

Require the dev instance health check:

```sh
pnpm smoke:mcp-fixtures -- --require-geetorus
```
...
The browser side uses the same `GEETORUS_PLAYWRIGHT_CHANNEL` override as the
rest of `tests/e2e`. In minimal containers, install the Playwright system
dependencies or point `GEETORUS_PLAYWRIGHT_CHANNEL` at the managed branch
