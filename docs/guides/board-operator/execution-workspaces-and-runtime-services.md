...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Geetorus.

Geetorus now presents this as a workspace-command model:
...
- Geetorus does not automatically restart workspace services on server boot — services only come back up when the next run (or a manual start) brings them up.
...
2. Geetorus realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Geetorus persists execution-workspace metadata such as paths, refs, and provisioning settings.
...
A managed service that runs Geetorus itself needs one canonical origin for Better Auth and tool OAuth callbacks. Geetorus resolves that origin in this order:

1. Explicit service/runtime configuration such as `GEETORUS_PUBLIC_URL` or `BETTER_AUTH_URL`.
...
`GITHUB_TOKEN`, `GH_TOKEN`, or `GEETORUS_GITHUB_TOKEN` (checked in that order; Settings →
