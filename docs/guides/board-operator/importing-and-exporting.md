...
Geetorus companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .geetorus.yaml     # Adapter config, env inputs, routines
```
...
- **.geetorus.yaml** holds Geetorus-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
geetorusai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
geetorusai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
geetorusai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.geetorus.yaml`
...
```sh
# From a local folder
geetorusai company import ./my-export

# From a GitHub URL
geetorusai company import https://github.com/org/repo

# From a GitHub subfolder
geetorusai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
geetorusai company import org/repo
geetorusai company import org/repo/companies/acme
```
...
If `--target` is not specified, Geetorus infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
geetorusai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
geetorusai company import org/company-templates/engineering-team \
  --target new \
...
```sh
geetorusai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
geetorusai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
geetorusai company import ./package \
  --target new \
  --yes \
  --json
```
...
Geetorus supports several GitHub URL formats:
