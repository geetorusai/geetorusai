...
Use `hermes_local` when Geetorus should start the local Hermes CLI. Use
`hermes_gateway` when Hermes is already running as an API server and Geetorus
should call that server. Both are built-in adapter types from the unified
`@geetorusai/hermes-geetorus-adapter` package.

For `opencode_local`, configure an explicit `adapterConfig.model` (`provider/model`).
Geetorus validates the selected model against live `opencode models` output.
...
- OpenAI lists ChatGPT accounts saved by Geetorus's Codex sign-in flow. Choose
