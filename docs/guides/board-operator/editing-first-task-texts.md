...
The templates support `{{agentName}}`, `{{organizationName}}`, and `{{proposalMode}}`. Geetorus fills them when it creates the first agent and first task. The proposal mode is `confirmation` or `plan`; the full policy lives in the skill.
...
Geetorus reads this setting once, when it creates an organization's first task. Changing it later does not alter an existing first task.
