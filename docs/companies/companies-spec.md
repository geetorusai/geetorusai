...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Geetorus.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.geetorus.yaml`
...
- Geetorus must not require extra top-level fields for skill validity
- Geetorus-specific extensions must live under `metadata.geetorus` or `metadata.sources`
...
For Geetorus, the preferred fidelity extension is:
...
- issue/task Geetorus-only metadata
...
- Geetorus tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Geetorus shape:
...
Additional rules for Geetorus exporters:
...
- omit empty and default-valued Geetorus fields when possible
...
- Geetorus exporters should emit `.geetorus.yaml` as a sidecar by default
...
Geetorus can map this spec to its runtime model like this:
...
- Geetorus extension:
  - `.geetorus.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Geetorus-specific fidelity

Inline Geetorus-only metadata that must live inside a shared markdown file should use:

- `metadata.geetorus`

That keeps the base format broader than Geetorus.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Geetorus.

## 20. Cutover

Geetorus should cut over to this markdown-first package model as the primary portability format.

`geetorus.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Geetorus, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
