...
Run this against a dev Geetorus instance with a board token or an agent token that can manage pipelines, routines, and issues.

```sh
export GEETORUS_API_URL=http://localhost:3100
export GEETORUS_COMPANY_ID=<company-id>
export GEETORUS_API_KEY=<token>
...
geetorusai pipelines create \
  -C "$GEETORUS_COMPANY_ID" \
...
geetorusai pipelines set-transitions \
  -C "$GEETORUS_COMPANY_ID" \
  "$RELEASE_PIPELINE" \
...
geetorusai pipelines guidance put \
  -C "$GEETORUS_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
...
export DRAFTING_ROUTINE_ID="$(
  geetorusai routine create \
    -C "$GEETORUS_COMPANY_ID" \
    --payload-json "$(jq -c . /tmp/drafting-routine.json)" \
    --json | jq -r '.id'
)"

geetorusai pipelines set-automation \
  -C "$GEETORUS_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
  --stage drafting \
...
geetorusai pipelines ingest-batch \
  -C "$GEETORUS_COMPANY_ID" \
  "$FEATURE_PIPELINE" \
...
geetorusai pipelines review-bulk \
  -C "$GEETORUS_COMPANY_ID" \
...
export SUGGESTION_ID="$(
  geetorusai pipelines case suggest \
    -C "$GEETORUS_COMPANY_ID" \
    "$BLOG_CASE" \
    --to assets \
...
geetorusai pipelines case resolve-suggestion \
  -C "$GEETORUS_COMPANY_ID" \
  "$BLOG_CASE" \
...
curl -sS -X POST \
  ...
  "$GEETORUS_API_URL/api/cases/$TWEET_CASE/issue-links" >/dev/null

geetorusai pipelines case edit \
  -C "$GEETORUS_COMPANY_ID" \
  "$BLOG_CASE" \
  --expected-version 2 \
...
# Recovery:
geetorusai pipelines case get -C "$GEETORUS_COMPANY_ID" "$BLOG_CASE" --json
```
...
```sh
export BLOG_VERSION="$(geetorusai pipelines case get -C "$GEETORUS_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$HERO_CASE" --to published --expected-version 1 --reason "Hero image done."
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$CARD_CASE" --to dropped --expected-version 1 --reason "Social card not needed."
```

When both asset cases are terminal, move the blog case to `assembly`.

```sh
export BLOG_ASSETS_VERSION="$(geetorusai pipelines case get -C "$GEETORUS_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

geetorusai pipelines case transition \
  -C "$GEETORUS_COMPANY_ID" \
  "$BLOG_CASE" \
  --to assembly \
...
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$ASSEMBLY_CASE" --to published --expected-version 1 --reason "Assembly complete."
```
...
```sh
export BLOG_REVIEW_VERSION="$(geetorusai pipelines case get -C "$GEETORUS_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

geetorusai pipelines case review \
  -C "$GEETORUS_COMPANY_ID" \
  "$BLOG_CASE" \
  --approve \
...
```sh
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 1 --reason "Draft ready for final review."
...
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 4 --reason "Revised draft ready."
geetorusai pipelines case review -C "$GEETORUS_COMPANY_ID" "$CHANGELOG_CASE" --approve --expected-version 5
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$CHANGELOG_CASE" --to published --expected-version 6 --reason "Published after request-changes loop."
```
...
```sh
geetorusai pipelines case transition -C "$GEETORUS_COMPANY_ID" "$TWEET_CASE" --to final_review --expected-version 1 --reason "Blog blocker is now done."
...
```sh
geetorusai pipelines case rollup \
  -C "$GEETORUS_COMPANY_ID" \
  "$RELEASE_CASE_ID" \
  --json
```
...
```sh
geetorusai pipelines case events \
  -C "$GEETORUS_COMPANY_ID" \
  "$CHANGELOG_CASE" \
  --json
```
