...
summary: Build, package, and distribute adapters as plugins without modifying Geetorus source
---

Geetorus supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Geetorus's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `geetorus-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Geetorus | Published to npm or linked via `file:` |
| Updates | Requires Geetorus release | Independent versioning |
...
- `hermes_local` starts the local Hermes CLI from
  `@geetorusai/hermes-geetorus-adapter`.
- `hermes_gateway` calls an already-running Hermes API server through
  `@geetorusai/hermes-geetorus-adapter/gateway`.

The legacy `@geetorusai/adapter-hermes-gateway` package is a deprecated
...
depend on or link `@geetorusai/hermes-geetorus-adapter` and declare the type
...
```json
{
  ...
  "dependencies": {
    "@geetorusai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^24.0.0",
...
| `geetorus.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@geetorusai/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Geetorus vars injected
  const env = buildGeetorusEnv(agent);
...
#### Available Helpers from `@geetorusai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildGeetorusEnv(agent)` | Inject `GEETORUS_*` environment variables |
...
```sh
# Via the Geetorus UI
# Settings → Adapters → Install from npm → "my-geetorus-adapter"
...
Local adapters are symlinked into Geetorus's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.geetorus/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-geetorus-adapter",
...
Other Geetorus users can then install your adapter by package name from the UI or API.
