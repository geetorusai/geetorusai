...
Build a custom adapter to connect Geetorus to any agent runtime.
...
|---|---|---|
| Source | Inside `geetorus-fork` | Separate npm package |
| Distribution | Ships with Geetorus | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Geetorus's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@geetorusai/adapter-utils/server-utils`
2. Build environment with `buildGeetorusEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@geetorusai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildGeetorusEnv(agent)` | `@geetorusai/adapter-utils/server-utils` | Inject `GEETORUS_*` env vars |
| `renderTemplate(tpl, data)` | `@geetorusai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@geetorusai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@geetorusai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Geetorus's source:
...
`format-event.ts` — pretty-prints stdout for `geetorusai run --watch` using `picocolors`.
...
With these flags set, the Geetorus UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Geetorus source changes required.
...
Make Geetorus skills discoverable to your agent runtime without writing to the agent's working directory:
