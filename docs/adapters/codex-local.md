...
- Either a host Codex login with `~/.codex/auth.json`, or a per-agent
  `OPENAI_API_KEY` configured in adapter env (Geetorus materializes this into
...
The adapter symlinks Geetorus skills into the global Codex skills directory (`~/.codex/skills`). Existing user skills are not overwritten.

## Fast Mode

When `fastMode` is enabled, Geetorus adds Codex config overrides equivalent to:

```sh
-c 'service_tier="fast"' -c 'features.fast_mode=true'
```

Geetorus currently applies that only when the selected model is `gpt-5.4`. On other models, the toggle is preserved in config but ignored at execution time to avoid unsupported runs.

## Managed `CODEX_HOME`

When Geetorus is running inside a managed worktree instance (`GEETORUS_IN_WORKTREE=true`), the adapter instead uses a worktree-isolated `CODEX_HOME` under the Geetorus instance so Codex skills, sessions, logs, and other runtime state do not leak across checkouts. It seeds that isolated home from the user's main Codex home for shared auth/config continuity.
...
- **Genuine external overrides** (a `CODEX_HOME` outside the Geetorus-managed company tree) are treated as self-managed and are never seeded or overwritten.
...
3. **External `CODEX_HOME`:** if adapter env points `CODEX_HOME` outside the
   Geetorus-managed company tree, that home is self-managed. Geetorus does
   not seed or overwrite it, so its own `auth.json` wins.

For sandbox or SSH execution, Geetorus uploads the effective managed
...
`$HOME/.codex/auth.json`, and the Geetorus host is logged in with a ChatGPT
subscription. For a managed `codex_local` agent, Geetorus symlinks the host
...
If `instructionsFilePath` is configured, Geetorus reads that file and prepends it to the stdin prompt sent to `codex exec` on every run.

This is separate from any workspace-level instruction discovery that Codex itself performs in the run `cwd`. Geetorus does not disable Codex-native repo instruction files, so a repo-local `AGENTS.md` may still be loaded by Codex in addition to the Geetorus-managed agent instructions.
