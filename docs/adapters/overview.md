...
summary: What adapters are and how they connect agents to Geetorus
---

Adapters are the bridge between Geetorus's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Geetorus:
...
| Hermes | `hermes_local` | Runs the local Hermes CLI through `@geetorusai/hermes-geetorus-adapter` |
| Hermes Gateway | `hermes_gateway` | Calls an already-running Hermes API server through `@geetorusai/hermes-geetorus-adapter/gateway` |
...
Local CLI adapters can run on the Geetorus host, SSH targets, or managed
...
|---------|---------------------|-------------------------------------------------------|
| [`codex_local`](/adapters/codex-local) | Host-owns-auth for Geetorus-managed `CODEX_HOME` | A host-owned `auth.json` is symlinked into the managed `CODEX_HOME` and uploaded to the sandbox. If a per-agent `OPENAI_API_KEY` is configured, Geetorus writes an API-key `auth.json` instead and that file wins. A login baked into the sandbox image is shadowed because Codex runs with Geetorus's uploaded `CODEX_HOME`. |
| [`claude_local`](/adapters/claude-local) | Snapshot-owns-auth for managed remote Claude config | A configured `ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN` (agent or environment env) wins over any stored login. Otherwise Geetorus uploads only sanitized settings and skill/runtime assets, and when the remote managed config has no Claude credential files it copies `.credentials.json` or `credentials.json` from the sandbox image's own `$HOME/.claude`, so the image's login wins. |
...
- **Claude sandbox with image login:** Geetorus materializes a remote
...
Use `hermes_local` when Geetorus should start the local `hermes` CLI on the
same host for each heartbeat. Use `hermes_gateway` when Hermes is already
running as an HTTP/SSE API server and Geetorus should call that server instead
...
`@geetorusai/adapter-hermes-gateway` package remains only as a deprecated
compatibility shim that re-exports the gateway entrypoints for one release.
New plugin overrides should target `@geetorusai/hermes-geetorus-adapter` and
...
|---------|---------|----------|-------------|
| Droid | `@henkey/droid-geetorus-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to Geetorus's source code required. External adapters are loaded at startup via the plugin system.

```sh
# Install from npm via API
curl -X POST http://localhost:3102/api/adapters \
  -d '{"packageName": "my-geetorus-adapter"}'
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `geetorusai run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the Geetorus web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
