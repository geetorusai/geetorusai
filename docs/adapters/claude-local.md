...
Geetorus default and leaves the setting unset until you select a model.
...
Geetorus Runner's qualified provider profiles.
...
The adapter creates a temporary directory with symlinks to Geetorus skills and passes it via `--add-dir`. This makes skills discoverable without polluting the agent's working directory.
...
`CLAUDE_CONFIG_DIR` is configured, Geetorus creates a remote
...
its own Claude Code login. Geetorus starts a managed remote `claude_local` run,
...
Geetorus-managed sandbox run uploads a host-owned `CODEX_HOME/auth.json` and
...
This installs Geetorus skills in `~/.claude/skills`, creates an agent API key, and prints shell exports to run as that agent.
