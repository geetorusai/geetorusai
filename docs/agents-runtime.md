...
Audience: Operators setting up and running agents in Geetorus

## 1. What this system does

Agents in Geetorus do not run continuously.  
...
- `hermes_local`: starts your local `hermes` CLI through `@geetorusai/hermes-geetorus-adapter`
- `hermes_gateway`: calls an already-running Hermes API server through `@geetorusai/hermes-geetorus-adapter/gateway`
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-geetorus-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Geetorus assumes the CLI is already installed and authenticated on the host machine. For `hermes_gateway`, Geetorus assumes the Hermes API server is already running, reachable from the Geetorus server, and configured with an API key. The older `@geetorusai/adapter-hermes-gateway` npm package is only a deprecated compatibility shim; the adapter type remains `hermes_gateway`.
...
Geetorus stores session IDs for resumable adapters.
...
Geetorus pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Geetorus surfaces this as a warning in environment tests, not a hard error.
