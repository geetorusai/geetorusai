...
Geetorus `external_reference` secrets without copying plaintext into
Geetorus.
...
`ListSecrets` only and import stores a Geetorus external reference plus
...
remote metadata, or copies secret plaintext into Geetorus.
...
- `conflict`: the suggested Geetorus `name` or `key` is already in use.
...
Geetorus-managed AWS secret through a broad runtime role.
...
Each row may include an optional Geetorus `description` entered during review;
...
the Geetorus runtime role can list the AWS secret but lacks
