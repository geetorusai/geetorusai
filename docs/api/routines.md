...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Geetorus-Signature` + `X-Geetorus-Timestamp` header pair matching the trigger's signing mode.
