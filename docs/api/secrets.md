...
An agent can ask Geetorus to bind an existing secret under a new path without
...
- `failed` means the card was accepted but execution failed. The card renders
  **FAILED**, exposes a non-secret `errorCode`, and Geetorus posts a **Secret
...
To link a provider-owned secret without copying the value into Geetorus, create
...
```json
{
  ...
  "externalRef": "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod/stripe",
  "providerVersionRef": "version-id-or-label"
}
```

Geetorus stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in Geetorus `company_secrets`.

The equivalent CLI check is:

```sh
npx geetorusai secrets doctor --company-id {companyId}
```
...
Remote import links existing AWS Secrets Manager entries into Geetorus as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into Geetorus.
...
- `duplicate`: a Geetorus secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under Geetorus's own managed namespace are
blocked as external references; use the Geetorus-managed secret flow for those
resources instead.
...
Geetorus `name`, `key`, optional Geetorus `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into Geetorus
...
- `imported`: Geetorus created an active `external_reference` secret and one
...
real value into the agent process environment. Geetorus's custody guarantees
...
empty string or another user's value. Geetorus records value-free access
...
```sh
npx geetorusai secrets declarations --company-id {companyId}
```
