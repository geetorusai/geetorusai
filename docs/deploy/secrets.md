...
Geetorus encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Geetorus protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Geetorus does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Geetorus can no longer guarantee
...
At runtime, Geetorus resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Geetorus injects its
own `GEETORUS_*` runtime variables.

Separately from env bindings, the **server itself** consumes a company secret
named `GITHUB_TOKEN`, `GH_TOKEN`, or `GEETORUS_GITHUB_TOKEN` (by name, no
...
Geetorus stores the feature as value-free metadata plus the user's own secret
value record:
...
identity, not from Geetorus secrets.
...
- Keep Geetorus-managed AWS values under the Geetorus managed namespace.
  External refs under that namespace are blocked by guardrails; use the
  Geetorus-managed flow when Geetorus should create and rotate the value.

### IAM Caveats

Geetorus enforces company scoping, responsible-user derivation, declaration
...
For AWS Secrets Manager, the Geetorus runtime role needs `GetSecretValue` and
any required KMS decrypt permission for every user-scoped value it may resolve.
If you link user-specific external refs outside the Geetorus managed prefix,
...
refs. Geetorus can prevent an agent from choosing another credential owner,
but a runtime role with broad external vault read permissions can still read
what IAM allows if another code path is introduced outside Geetorus.
...
```
~/.geetorus/instances/default/secrets/master.key
```

This key is auto-created during onboarding. The key never leaves your machine.
Geetorus best-effort enforces `0600` permissions when it creates or loads the
key file. `geetorusai doctor` and the provider health API warn when the file is
...
|----------|-------------|
| `GEETORUS_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `GEETORUS_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `GEETORUS_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
configuration or `GEETORUS_SECRETS_STRICT_MODE=false`.

## External References

Provider-owned secrets can be linked without copying values into Geetorus by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Geetorus stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Geetorus Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Geetorus company secrets**. Allowed credential sources are
workload identity attached to the Geetorus server (instance profile, IRSA, ECS
...
can override) the deployment-level `GEETORUS_SECRETS_AWS_*` env. Bootstrap
...
Geetorus `external_reference` secrets. This is a metadata-only link: Geetorus
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Geetorus name and key, then
   submit.
...
cursors. Do not expect Geetorus to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Geetorus runtime
...
- Keep managed create/rotate/delete permissions scoped to the Geetorus
...
dedicated Geetorus runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Geetorus
...
- `aws_secrets_manager`: back up Geetorus's database for vault metadata
  ...
  same Geetorus company at the same AWS namespace and confirming the runtime
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Geetorus. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Geetorus
...
For Geetorus Cloud, provision the server runtime IAM role/workload identity,
KMS key, deployment prefix, and non-secret `GEETORUS_SECRETS_AWS_*` environment
...
Geetorus secrets. Bootstrap material belongs in infrastructure IAM/workload
...
```sh
npx geetorusai secrets migrate-inline-env --company-id <company-id>
...
Use the CLI command for normal operations because it goes through the Geetorus
...
the value custodian; Geetorus stores metadata and provider version references,
