...
Geetorus supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
npx geetorusai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Geetorus emits a one-time claim URL at startup:
...
```sh
GEETORUS_DEPLOYMENT_MODE=authenticated GEETORUS_BIND=lan pnpm geetorusai run
```
