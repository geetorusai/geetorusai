...
Geetorus uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.geetorus/instances/default/db/` for storage
2. Ensures the `geetorus` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.geetorus/instances/default/db`.
...
| `DATABASE_APPLICATION_NAME` | `geetorus` | Value of `application_name` in `pg_stat_activity`, so you can find Geetorus's backends: `SELECT * FROM pg_stat_activity WHERE application_name = 'geetorus';` |
