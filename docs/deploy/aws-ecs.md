...
summary: Deploy Geetorus to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Geetorus to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The Geetorus repo cloned locally
...
export GEETORUS_DOMAIN=geetorus.example.com   # your domain
...
aws ecr create-repository \
  --repository-name geetorus-server \
...
# Build
docker build -t geetorus-server .

# Tag and push
docker tag geetorus-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/geetorus-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name geetorus-alb \
  --description "Geetorus ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name geetorus-ecs \
  --description "Geetorus ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name geetorus-rds \
  --description "Geetorus RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name geetorus-efs \
  --description "Geetorus EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name geetorus-db-subnet \
  --db-subnet-group-description "Geetorus RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier geetorus-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username geetorus \
...
DATABASE_URL="postgresql://geetorus:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/geetorus"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=geetorus-data \
...
aws secretsmanager create-secret \
  --name geetorus/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name geetorus/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name geetorus/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name geetorus/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name geetorus/github-token \
...
# Task execution role
aws iam create-role \
  --role-name geetorus-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:geetorus/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name geetorus-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name geetorus

aws logs create-log-group --log-group-name /ecs/geetorus
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$GEETORUS_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/geetorus-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/geetorus-task-def.json
```
...
- Create a CNAME or ALIAS record for `$GEETORUS_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster geetorus \
  --service-name geetorus-server \
  --task-definition geetorus-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster geetorus \
  --services geetorus-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster geetorus --service-name geetorus-server
TASK_ARN=$(aws ecs list-tasks --cluster geetorus --service-name geetorus-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster geetorus --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/geetorus --since 10m --follow

# Hit the health endpoint
curl -sf https://$GEETORUS_DOMAIN/api/health
```
...
aws ecs update-service \
  --cluster geetorus \
  --service geetorus-server \
  --task-definition geetorus-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier geetorus-db
aws rds start-db-instance --db-instance-identifier geetorus-db
```
...
aws ecs update-service --cluster geetorus --service geetorus-server --desired-count 0
aws ecs delete-service --cluster geetorus --service geetorus-server --force
aws ecs delete-cluster --cluster geetorus
...
aws rds delete-db-instance \
  --db-instance-identifier geetorus-db \
  --final-db-snapshot-identifier geetorus-db-final
aws rds wait db-instance-deleted --db-instance-identifier geetorus-db
aws rds delete-db-subnet-group --db-subnet-group-name geetorus-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id geetorus/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name geetorus-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name geetorus-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name geetorus-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name geetorus-ecs-execution
aws iam delete-role --role-name geetorus-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/geetorus
```
