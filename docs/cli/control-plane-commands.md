...
```sh
# List issues
npx geetorusai issue list [--status todo,in_progress] [--assignee-agent-id <id>] [--match text]

# Get issue details
npx geetorusai issue get <issue-id-or-identifier>

# Create issue
npx geetorusai issue create --title "..." [--description "..."] [--status todo] [--priority high]

# Update issue
npx geetorusai issue update <issue-id> [--status in_progress] [--comment "..."]

# Add comment
npx geetorusai issue comment <issue-id> --body "..." [--reopen]

# Checkout task
npx geetorusai issue checkout <issue-id> --agent-id <agent-id>
...
npx geetorusai company export <company-id> --out ./exports/acme --include company,agents

# Preview import (no writes)
npx geetorusai company import \
  <owner>/<repo>/<path> \
  --target existing \
...
forbidden, it falls back to `--company-id`, `GEETORUS_COMPANY_ID`, context, or
...
npx geetorusai skills browse [--kind bundled|optional] [--category software-development] [--query github]
npx geetorusai skills search "pull request" [--json]
...
npx geetorusai skills install github-pr-workflow --as pr-flow --force --company-id <company-id>

# External sources still use import instead of catalog install
npx geetorusai skills import ./skills/my-skill --company-id <company-id>
npx geetorusai skills import owner/repo/path/to/skill --company-id <company-id>

# Attach desired company skills to an agent after install/import
npx geetorusai skills agent sync <agent-id> --skill github-pr-workflow --mode add --company-id <company-id>
```
...
```sh
# List approvals
npx geetorusai approval list [--status pending]

# Get approval
npx geetorusai approval get <approval-id>

# Create approval
npx geetorusai approval create --type hire_agent --payload '{"name":"..."}' [--issue-ids <id1,id2>]
...
```sh
npx geetorusai activity list [--agent-id <id>] [--entity-type issue] [--entity-id <id>]
```

## Dashboard

```sh
npx geetorusai dashboard get
```
...
```sh
npx geetorusai heartbeat run --agent-id <agent-id> [--api-base http://localhost:3100]
```
