# GeeTorus: Marketplace for Geetorus Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Geetorus configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**GeeTorus** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Geetorus-managed companies.

| Dimension | GeeTorus |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Geetorus company (agents, projects, governance) |
...
A complete Geetorus company configuration:
...
Single-agent configurations designed to plug into a Geetorus org:
...
Portable skill files that any Geetorus agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Geetorus's skill loading system
...
interface Listing {
  ...
  geetorusVersionMin: string;     // Minimum Geetorus version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Geetorus company
...
| `/about` | About GeeTorus | Mission, how it works |
...
| `POST` | `/api/listings/:id/install` | Install to Geetorus company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Geetorus company (or create new)
  → GeeTorus API calls Geetorus API to:
  ...
  → Redirect to Geetorus dashboard with new team running
```
...
### 5.3 Creator: Export from Geetorus → Publish

```
Running Geetorus company → "Export as Blueprint" (CLI or UI)
  → Geetorus exports:
      ...
      - Project templates
      - Skill files
  → Upload to GeeTorus as new listing
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Geetorus brand blue/purple)
...
| Install flow | One-click deploy to Geetorus company |
...
{
  "targetCompanyId": "uuid",         // Existing Geetorus company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `geetorus-create-agent` supports it, or via approval flow)
...
| Platform fee | 10% of sale price | GeeTorus's cut |
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Geetorus
- **Backend**: Node.js API (or extend Geetorus server)
- **Database**: Postgres (can share Geetorus's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Geetorus auth (or OAuth2)

### 9.2 Integration with Geetorus

GeeTorus can be:
- **Option A**: A separate app that calls Geetorus's API to install blueprints
- **Option B**: A built-in section of the Geetorus UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Geetorus UI and API.
...
- [ ] Install flow: blueprint → Geetorus company
...
- [ ] "Export from Geetorus" CLI command
