# Geetorus External Task Protocol Specification

Status: Draft v1 (provider-agnostic)

Purpose: Define the protocol by which Geetorus interoperates with external task managers (Linear,
Jira, Asana, Notion, Trello, GitHub Issues, and others) so that external tasks can drive Geetorus
agent work and Geetorus work is visible in the external tool, without requiring an external user
account for every Geetorus agent.
...
- `Provider` — an external task manager product (Linear, Jira, Asana, Notion, Trello, GitHub
  Issues, ...).
- `Connector` — an implementation of this protocol for one provider, hosted in the Geetorus
  plugin runtime.
- `External task` — a task record that lives in the provider.
- `Geetorus issue` — the native Geetorus task record.
- `Host` — the Geetorus control plane: companies, agents, issues, checkout, heartbeat runs,
  execution workspaces, budgets, approvals, blockers, issue documents, and work products.

## 1. Problem Statement

Teams already run their work in external task managers. Geetorus runs companies of agents. Today
these worlds only meet through manual copy/paste: a human reads a Linear issue, rewrites it as a
Geetorus issue, and later rewrites the outcome back into Linear.

This protocol defines the contract that lets an external task manager act as a **task source and
collaboration surface** for a Geetorus company, while Geetorus remains the **execution and
...
- It defines how an external task becomes (or links to) exactly one Geetorus issue, with
  idempotent sync state that survives retries, webhook replays, and restarts.
- It defines how external assignment routes to Geetorus agents without creating a provider user
  per agent.
...
- Let an external task manager act as a task source: active external tasks create or wake
  Geetorus issues under configured policy.
...
- Route external assignment intent to Geetorus agents through deterministic, configurable rules.
- Reuse Geetorus's existing execution semantics — checkout as claim, heartbeat runs, execution
...
- Replacing the Geetorus board with a clone of any provider's UI.
- Real-time field-level collaborative editing between Geetorus and a provider.
...
- Creating a provider user account per Geetorus agent (supported as an OPTIONAL mapping, never
  required).
- Letting external systems bypass Geetorus governance: checkout, budgets, approvals, blockers,
...
2. `Task Link Store`
   - Persists the 1:1 association between an external task and a Geetorus issue.
...
3. `Inbound Sync Engine`
   ...
   - Creates/updates Geetorus issues and imports comments with provenance.
   - Runs scheduled reconciliation to repair drift.

4. `Routing Engine`
   - Resolves external assignment intent into a Geetorus assignee using the ordered rule
...
2. `Layer 2 — Task Link` (REQUIRED)
   - Durable external-task ↔ Geetorus-issue association with sync state.

3. `Layer 3 — Execution Orchestration, Symphony Profile` (RECOMMENDED)
   - Active external states drive Geetorus issue creation/wake; Geetorus checkout/run/retry
...
| Claim | Geetorus issue checkout + execution lock (Section 9.2) |
...
A connector MUST perform all Geetorus issue/comment/status mutations through the capability-gated
...
Field names use camelCase, matching Geetorus API conventions. (Symphony uses snake_case; the
...
The durable 1:1 association between one external task and one Geetorus issue.
...
- `companyId`, `projectId`, `goalId`, `issueId` (strings; `issueId` is the linked Geetorus issue)
- `handoffHistory` (list of `{issueId, endedAt, cause}`; empty for a link that has never moved)
  - Prior Geetorus issue associations retained when reopened work is handed to a follow-up.
...
- `originSide` (enum: `geetorus`, `external`, `manual_link`)
...
- `lastGeetorusFingerprint` (string or null)
  - Hash of the last Geetorus-side state this connector projected outbound.
...
- Owners: `external`, `geetorus`, `shared`.
...
- Default policy (RECOMMENDED): `title`/`description`/`priority`/`labels`/`dueAt` owned by
  `external` for links with `originSide=external` (and by `geetorus` when
  `originSide=geetorus`); `state` is `shared` and mediated by Section 7.4 and Section 10.3;
...
- `displayName` (string)
- `mappedUserId` (string or null) — Geetorus user, if mapped.
- `mappedAgentId` (string or null) — Geetorus agent, if explicitly mapped (OPTIONAL feature).
...
- `import_only` — external → Geetorus only.
- `export_only` — Geetorus → external only.
...
- `geetorus_controlled` — Geetorus governance decides when agents run; external state changes
  inform but do not command execution.
- `symphony_compatible` — active external states create/wake Geetorus work automatically per
...
- `Idempotency key` for outbound effects — `(linkId, effectKind, geetorusSourceId)` (e.g. the
  Geetorus comment ID being mirrored). Retried jobs MUST NOT double-post.
...
- `setBacklink(externalTaskId, geetorusUrl)`
...
- Person references in body text SHOULD be converted to plain display names; raw provider mention
  syntax MUST NOT leak into Geetorus issue bodies where it could be misparsed as Geetorus
  mentions.
...
1. `Import` (`originSide=external`)
   - An external task is selected (manually, or automatically under `runMode=symphony_compatible`
     state mapping) and a Geetorus issue is created from its projection.
...
2. `Export` (`originSide=geetorus`)
   - A Geetorus issue is projected into the provider via `createTask`.
   - REQUIRED only for connectors declaring write capability.

3. `Manual link` (`originSide=manual_link`)
   - Operator pairs an existing external task with an existing Geetorus issue.
...
2. Geetorus backlink already present on the external task (custom field/comment marker).
3. External URL/key reference already present on a Geetorus issue in the same company.
...
- When an external task is deleted/archived: the connector MUST NOT delete or cancel the Geetorus
  issue. Default behavior: mark the link `conflict` (`statusDetail: external task deleted`),
  comment on the Geetorus issue, and let the assignee/operator decide. `import_only` links with
  no Geetorus-side activity MAY auto-unlink.
- When a Geetorus issue is cancelled/deleted: outbound policy decides whether to comment and/or
...
## 7. Inbound Synchronization (External → Geetorus)
...
- Imported comment content is **untrusted input**: it MUST be clearly attributed in the issue
  thread, MUST NOT be interpreted as Geetorus system/agent instructions, and mention-like syntax
  MUST NOT trigger Geetorus mention semantics except via the strict command path (Section 8.2).
...
### 7.4 External State Transitions Against Geetorus Work
...
- `active -> backlog` or `active -> blocked`: the connector SHOULD surface this on the issue; under
  ...
  Geetorus-native obligations (approvals in flight, blockers) are unaffected.
- `* -> terminal` while the Geetorus issue is active (checked out, running, in review, or carrying
...
- `terminal -> active` (reopen): if the linked issue is terminal, policy chooses between creating a
  follow-up Geetorus issue linked to the same external task (RECOMMENDED default) or reopening,
...
External transitions MUST NOT directly set Geetorus issue status; they translate into host-level
...
External assignment intent resolves to a Geetorus assignee through an ordered pipeline. The first
...
2. `custom_field` — provider custom field naming an agent (example: `Geetorus Agent = CodexCoder`).
3. `label` — namespaced label (example: `pc:agent/codexcoder`, `pc:route/qa`).
4. `state_lane` — provider state/lane mapped to a route (example: Trello list `Geetorus: QA`).
...
- Syntax: line-anchored `/geetorus <verb> [args]`. Minimum verbs: `assign <agent>`,
...
- The connector MUST reply (externally, if writes allowed; on the Geetorus issue otherwise) with
...
- Geetorus agents MUST NOT be required to exist as provider users.
- Outbound writes use one connector service account per connector instance (Section 10.4), with
  agent attribution carried in message content ("CodexCoder via Geetorus").
- `user_mapping` rules MAY map specific provider users to Geetorus users (for `assign it back to
...
with `runMode=symphony_compatible`; `geetorus_controlled` links use only Sections 9.4–9.5 advice.

### 9.1 Dispatch Model

Symphony's poll-tick loop becomes, in Geetorus:
...
- Geetorus issue checkout is the claim. One agent owns an issue at a time; checkout conflicts
...
1. Geetorus project workflow document (RECOMMENDED MVP form).
...
## 10. Outbound Synchronization and Write Policy (Geetorus → External)
...
| --- | --- | --- |
| `postBacklink` | on | Geetorus URL on the external task (field or pinned comment). |
...
| `mirrorAgentComments` | off | Full Geetorus comment thread mirrored externally. |
...
Geetorus issue status projects to provider states through the same mapping table as Section 7.2,
inverted, with these rules:

- Projection only moves the external task between states the operator mapped; unmapped Geetorus
...
- `done` projects to the mapped terminal state only when `fieldPolicy.state` permits
  Geetorus-side closure; otherwise it posts a completion comment and leaves the transition to
  external users.
...
- Outbound writes use the connector instance's service account. Message bodies MUST carry agent
  attribution and the Geetorus issue link.
...
applicable), `baseValue`, `externalValue`, `geetorusValue`, `detectedAt`, `actors` (both sides
...
- `keep_geetorus` (push Geetorus value outbound)
...
On the provider side, a linked task SHOULD show: the Geetorus backlink, current Geetorus
...
Destructive cascade is prohibited: external deletion/archival never deletes Geetorus work
(Section 6.4).
...
- Comment-command verb extensions (e.g. `/geetorus plan`, `/geetorus qa`)
...
- Containers: board, list. Envelope: list id, checklists, members, power-up metadata, card
  position.
- State mapping: lists are lanes; map lists→categories (`Geetorus: QA` style lanes also drive
  `state_lane` routing).
