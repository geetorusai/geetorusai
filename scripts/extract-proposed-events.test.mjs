...
function withFixtureRepo(source, callback) {
  const repoRoot = mkdtempSync(join(tmpdir(), "geetorus-proposed-events-"));
...
test("provenance paths are repo-relative and reject dev-host path shapes", () => {
  ...
  const repoRoot = mkdtempSync(join(tmpdir(), "geetorus-provenance-root-"));
  try {
...
function diagnosticsFor(sourceText) {
  const repoRoot = mkdtempSync(join(tmpdir(), "geetorus-ts2578-"));
