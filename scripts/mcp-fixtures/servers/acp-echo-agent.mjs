...
async function handleRequest(request) {
  if (request.method === "initialize") {
    ...
    process.stderr.write("geetorus-acp-echo-agent started\n");
    return {
      protocolVersion: 1,
      agentCapabilities: {
        loadSession: false,
      ...
      agentInfo: { name: "geetorus-acp-echo-agent", version: "1.0.0" },
    };
  }
  if (request.method === "session/new") return { sessionId: randomUUID() };
  if (request.method === "session/prompt") {
    const typedFailureCanary = process.env.GEETORUS_ACPX_TYPED_FAILURE_CANARY;
    ...
    const typedWarningCanary = process.env.GEETORUS_ACPX_TYPED_WARNING_CANARY;
    let responseMeta;
    ...
    writeMessage({
      ...
      params: {
        ...
        update: {
          ...
          content: {
            type: "text",
            text: process.env.GEETORUS_ACPX_SPAWN_SMOKE ?? "missing",
          },
        },
      },
    });
