...
const root = process.env.GEETORUS_STOP_FIXTURE_ROOT ?? process.cwd();
const send = (message) => process.stdout.write(`${JSON.stringify(message)}\n`);
let active;
let timer;
if (process.env.GEETORUS_STOP_FIXTURE_HANG_ON_CLOSE === '1') {
  process.on('SIGTERM', () => {});
  setInterval(() => {}, 1000);
}
...
async function request(message) {
  ...
  switch (message.method) {
    ...
    case 'session/prompt': {
      ...
      fs.appendFileSync(`${root}/run-env`, `${JSON.stringify({ runId: process.env.GEETORUS_RUN_ID, tokenHash: createHash('sha256').update(process.env.GEETORUS_API_KEY ?? '').digest('hex'), scratchDir: process.env.GEETORUS_RUN_SCRATCH_DIR })}\n`);
      if (fs.existsSync(`${root}/continued`)) {
        ...
        if (!paused) {
          ...
          if (process.env.GEETORUS_STOP_FIXTURE_FINISH_TASK === '1') {
            const base = process.env.GEETORUS_API_URL.replace(/\/api\/?$/, '').replace(/\/$/, '');
            const response = await fetch(`${base}/api/issues/${process.env.GEETORUS_TASK_ID}`, {
              method: 'PATCH',
              headers: { 'content-type': 'application/json', authorization: `Bearer ${process.env.GEETORUS_API_KEY}`, 'X-Geetorus-Run-Id': process.env.GEETORUS_RUN_ID },
      ...
      active = message;
      if (process.env.GEETORUS_STOP_FIXTURE_TOOL === 'read') {
      ...
      if (process.env.GEETORUS_STOP_FIXTURE_TOOL === 'write') {
