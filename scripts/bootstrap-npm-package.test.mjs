...
test("parseArgs recognizes the publish flag", () => {
  assert.deepEqual(parseArgs(["@geetorusai/adapter-kimi-local", "--publish"]), {
    help: false,
    packageName: "@geetorusai/adapter-kimi-local",
    publish: true,
  });
});

test("parseArgs defaults to a dry run", () => {
  assert.deepEqual(parseArgs(["@geetorusai/adapter-kimi-local"]), {
    help: false,
    packageName: "@geetorusai/adapter-kimi-local",
    publish: false,
  });
...
test("parseArgs rejects a second package name", () => {
  assert.throws(() => parseArgs(["@geetorusai/a", "@geetorusai/b"]), /exactly one package name/);
});

test("parseArgs rejects unknown options", () => {
  assert.throws(() => parseArgs(["@geetorusai/a", "--skip-build"]), /unknown option/);
  assert.throws(() => parseArgs(["@geetorusai/a", "--otp", "123456"]), /unknown option/);
});

test("validatePackageName accepts @geetorusai scoped names", () => {
  validatePackageName("@geetorusai/adapter-kimi-local");
  validatePackageName("@geetorusai/plugin-workspace-diff");
});

test("validatePackageName rejects names outside the @geetorusai scope", () => {
  assert.throws(() => validatePackageName("left-pad"), /@geetorusai scope/);
  assert.throws(() => validatePackageName("@evil/adapter-kimi-local"), /@geetorusai scope/);
  assert.throws(() => validatePackageName("@geetorusai/UPPER"), /@geetorusai scope/);
});

test("buildPlaceholderFiles produces a publishable manifest at the placeholder version", () => {
  const files = buildPlaceholderFiles("@geetorusai/adapter-kimi-local");
  const manifest = JSON.parse(files["package.json"]);

  assert.equal(manifest.name, "@geetorusai/adapter-kimi-local");
...
test("buildPlaceholderFiles entry point throws with a pointer to the repo", () => {
  ...
  assert.match(files["index.js"], /github\.com\/geetorusai\/geetorus/);
...
test("waitForPackageVisible requires consecutive sightings before reporting success", async () => {
  ...
  const visible = await waitForPackageVisible("@geetorusai/x", {
    attempts: 10,
    consecutive: 2,
