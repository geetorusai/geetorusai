#!/usr/bin/env bash
# End-to-end proof that `geetorusai update` works ACROSS VERSIONS, including
...
#
# Env knobs:
#   E2E_REPO                  GitHub repo (default: geetorusai/geetorus)
...
# Clean environment, then isolate ALL Geetorus state (managed store, config,
...
export CI="${CI:-1}"
export GEETORUS_HOME="$HOME/.geetorus-e2e-update"

SHIM="$HOME/.local/bin/geetorusai"
STORE="$GEETORUS_HOME/cli"
BACKUP_DIR="$GEETORUS_HOME/instances/default/data/backups"
RESULTS=()
FAILED=0
...
[ -d "$GEETORUS_HOME" ] && { echo "$GEETORUS_HOME already exists — refusing to run"; exit 2; }
...
echo "repo=$E2E_REPO base=$BASE_REF next=$NEXT_REF geetorus_home=$GEETORUS_HOME"
...
if [ -n "${E2E_BOOTSTRAP_CLI:-}" ] && [ -f "$E2E_BOOTSTRAP_CLI" ]; then
  ...
  ( cd "$BOOT" \
      ...
      && GEETORUS_README_ASSET_REF="$BASE_REF" bash scripts/build-npm.sh --skip-checks --skip-typecheck > "$HOME/e2e-upd-bootstrap-build.log" 2>&1 ) \
  ...
  BOOTSTRAP_CLI="$HOME/e2e-upd-bootstrap-cli/node_modules/geetorusai/dist/index.js"
fi
...
PG_VERSION_FILE="$(find "$GEETORUS_HOME" -name PG_VERSION 2>/dev/null | head -1)"
