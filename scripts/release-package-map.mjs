...
const CHANNEL_ENTRYPOINT_PACKAGE = "geetorusai";
...
// Sections whose @geetorusai workspace deps are rewritten to the calver release
...
// release, and every @geetorusai/* workspace: dep it declares is rewritten to that
  ...
  for (const pkg of packages) {
    ...
    for (const section of RESOLVED_DEP_SECTIONS) {
      ...
      for (const [depName, spec] of Object.entries(deps)) {
        if (!depName.startsWith("@geetorusai/")) continue;
...
function replaceWorkspaceDeps(deps, version) {
  ...
  for (const [name, value] of Object.entries(next)) {
    if (!name.startsWith("@geetorusai/")) continue;
