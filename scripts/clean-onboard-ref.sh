...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Geetorus data dir to use
  GEETORUS_HOST=127.0.0.1    Host passed to the onboarded server
  GEETORUS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/geetorus-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/geetorus-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export GEETORUS_HOST="${GEETORUS_HOST:-127.0.0.1}"
export GEETORUS_PORT="${GEETORUS_PORT:-3100}"
export GEETORUS_OPEN_ON_LISTEN="${GEETORUS_OPEN_ON_LISTEN:-false}"
...
echo "GEETORUS_HOST: $GEETORUS_HOST"
echo "GEETORUS_PORT: $GEETORUS_PORT"
...
env \
  ...
  HOST="$GEETORUS_HOST" \
  PORT="$GEETORUS_PORT" \
  GEETORUS_OPEN_ON_LISTEN="$GEETORUS_OPEN_ON_LISTEN" \
  pnpm geetorusai onboard --yes --data-dir "$PC_DATA"
