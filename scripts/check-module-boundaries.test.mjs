...
test("extractImportSpecifiers recognizes supported TypeScript dependency forms", () => {
  assert.deepEqual(
    ...
    ["@geetorusai/db", "./helper.js", "../adapters/postgres.js", "postgres", "node:fs"],
  );
});

test("scanModuleBoundaries rejects outward dependencies and module-internal imports", () => {
  const serverSrc = mkdtempSync(join(tmpdir(), "geetorus-module-boundaries-"));
  ...
  try {
    ...
    write(
      ...
      [
        ...
        'import db = require("@geetorusai/db");',
      ].join("\n"),
    );
    ...
    assert.deepEqual(
      ...
      [
        ...
        { specifier: "@geetorusai/db", reason: "application cannot import database packages" },
