...
publish_package_to_npm() {
  ...
  publish_log="$(mktemp "${TMPDIR:-/tmp}/geetorus-npm-publish.XXXXXX")"
...
wait_for_npm_package_versions() {
  ...
  (
    ...
    status_dir="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-release-visibility.XXXXXX")"
...
verify_npm_installable() {
  ...
  install_dir="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-release-install.XXXXXX")"
  ...
  installed_version="$(node -e "console.log(require(process.argv[1]).version)" "$install_dir/node_modules/geetorusai/package.json")"
  rm -rf "$install_dir"
