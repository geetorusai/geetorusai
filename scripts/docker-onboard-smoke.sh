...
IMAGE_NAME="${IMAGE_NAME:-geetorus-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
GEETORUSAI_VERSION="${GEETORUSAI_VERSION:-latest}"
...
GEETORUS_DEPLOYMENT_MODE="${GEETORUS_DEPLOYMENT_MODE:-authenticated}"
GEETORUS_DEPLOYMENT_EXPOSURE="${GEETORUS_DEPLOYMENT_EXPOSURE:-private}"
...
GEETORUS_PUBLIC_URL="${GEETORUS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
# Seconds to wait for /api/health after the container starts. The container
# cold-installs geetorusai from npm and initializes embedded postgres before
...
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@geetorus.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-geetorus-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_GEETORUSAI_VERSION=%q\n' "$GEETORUSAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e GEETORUS_DEPLOYMENT_MODE="$GEETORUS_DEPLOYMENT_MODE" \
      -e GEETORUS_DEPLOYMENT_EXPOSURE="$GEETORUS_DEPLOYMENT_EXPOSURE" \
      -e GEETORUS_PUBLIC_URL="$GEETORUS_PUBLIC_URL" \
      -e GEETORUS_HOME="/geetorus" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "geetorusai@${GEETORUSAI_VERSION}" auth bootstrap-ceo --data-dir "$GEETORUS_HOME" --base-url "$GEETORUS_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$GEETORUS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$GEETORUS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$GEETORUS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$GEETORUS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$GEETORUS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$GEETORUS_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg GEETORUSAI_VERSION="$GEETORUSAI_VERSION" \
...
echo "    Public URL: $GEETORUS_PUBLIC_URL"
...
echo "    Deployment: $GEETORUS_DEPLOYMENT_MODE/$GEETORUS_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$GEETORUS_PUBLIC_URL/api/health" "$SMOKE_READY_TIMEOUT_SECONDS" 1; then
  echo "Smoke bootstrap failed: server did not become ready at $GEETORUS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$GEETORUS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $GEETORUS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
