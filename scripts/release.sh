...
export GEETORUS_RELEASE_REUSE_UI_DIST=1
...
if [ "$dry_run" = true ]; then
  ...
  preview_logs_dir="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-release-previews.XXXXXX")"
  ...
  while IFS=$'\t' read -r pkg_dir _pkg_name _pkg_version; do
    ...
    (
      ...
      if [ "$publish_tool" = "npm" ]; then
        publish_dir="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-release-package.XXXXXX")"
...
if [ "$dry_run" = true ]; then
  ...
  release_info "  Installing geetorusai@$DIST_TAG into a clean prefix..."
  if ! verify_npm_installable "geetorusai@$DIST_TAG" "$TARGET_PUBLISH_VERSION"; then
    release_fail "geetorusai@$DIST_TAG did not install cleanly at expected version ${TARGET_PUBLISH_VERSION}"
  fi
...
if [ "$dry_run" = true ]; then
  ...
  case "$channel" in
    canary|nightly|beta)
      ...
      release_info "Install with: npx geetorusai@$channel onboard"
