...
const corpus =
  process.env.GEETORUS_CONTENT_TEMPLATES ??
  path.resolve(
    root,
    "../../geetorus-content/research/connections/vercel/templates",
  );
...
const channelMethod = (
  ...
  whenToUse: `Let people in ${chatProviderName(provider)} start and continue work with one Geetorus agent.`,
  credentialFields,
  guidanceMd,
  consoleLinks,
  riskTier: "S3",
...
const posthogConfigFields = () => [
  ...
  },
  {
    ...
    helperMd:
      "Geetorus uses individual tools so every action can be governed. CLI mode remains unavailable until nested execution is governed.",
...
const apps = [
  ...
  [
    ...
    method(
      ...
      "S4",
      "Sign in to Railway and select the workspaces your agents may use. Geetorus adds direct service, deployment, and bounded log tools when Railway accepts the connection for API access. Container commands require the separate SSH setup on the connection. Project tokens are not supported by Railway's hosted connection.",
      {
        ...
        warnings: [
          ...
          "The general Railway agent and committing staged changes are unavailable because their internal changes cannot be individually reviewed in Geetorus.",
          "Live Railway qualification is pending. If Railway rejects API access, reconnect with the required permissions; Geetorus never falls back to another credential.",
        ],
  ...
  [
    ...
    [
      method(
        ...
        "S3",
        "Authorize Geetorus, then choose selected repositories in GitHub. You can edit repository access later from GitHub's installation settings.",
        {
          label: "Use this connection as an agent tool",
          purpose: "tool",
          oauthStrategy: "geetorus_cloud_connector",
  ...
  [
    "slack",
    "Slack",
    "Give agents Slack tools or let people start and continue Geetorus work from Slack.",
    "communication",
    "slack.com",
    ...
    [
      ...
      channelMethod(
        ...
        ["workspace", "channel"],
        "Create and install one Slack App for this agent. Geetorus receives verified Events API requests and interactive callbacks, acknowledges with reactions, responds in direct messages, and starts one Geetorus task per new mentioned channel thread.",
        {
  ...
  [
    "microsoft-teams",
    "Microsoft Teams",
    "Let people start and continue Geetorus work with an agent from Microsoft Teams.",
    "communication",
    "teams.microsoft.com",
  ...
  [
    "imessage-photon", "iMessage Photon",
    "Message a Geetorus agent from Apple Messages using Photon Cloud. Pro supports DMs; dedicated lines also support groups.",
    ...
    channelMethod("imessage-photon", [field("projectSecret", "Project secret", "Photon project secret")], ["direct_message", "group_chat"],
      "Connect a Photon Cloud project. Pro shared lines support DMs after sender enrollment in Photon and identity linking in Geetorus. Dedicated lines also support individually enabled groups.",
      { register: "https://photon.codes/", docs: "https://photon.codes/docs/spectrum-ts/providers/imessage/connection-and-routing" }),
  ],
  [
    "telegram",
    "Telegram",
    "Let people start and continue Geetorus work with an agent from Telegram.",
    "communication",
    "telegram.org",
    ...
    channelMethod(
      ...
      ["chat", "group", "topic"],
      "Create one dedicated bot with BotFather, then connect its token to the public Geetorus webhook endpoint.",
      {
  ...
  [
    "discord",
    "Discord",
    "Let people start and continue Geetorus work with an agent from Discord.",
    "communication",
    "discord.com",
    ["https://discord.com/*"],
    channelMethod(
      ...
      ["channel"],
      "Create one dedicated Discord application and bot, enable the Message Content intent, install it in one server with the documented bot permissions, then connect its bot token, Application ID, and server ID. Geetorus starts one Discord thread per root bot mention and keeps the linked Geetorus task authoritative.",
      {
  ...
  [
    ...
    method(
      ...
      "S2",
      "Register a Linear OAuth app and add Geetorus's redirect URI before connecting.",
      {
        ownershipModes: ["customer"],
  ...
  [
    ...
    method(
      ...
      "local_stdio",
      "none",
      { templateKey: "geetorus.google-sheets" },
      "S3",
      "Share each spreadsheet with the Geetorus robot email, then paste the sheet links.",
  ...
  [
    ...
    [
      method(
        ...
        "S3",
        "Connect Shopify's current UCP server for shopper-facing catalog and commerce tools. Geetorus supplies the required agent profile automatically.",
        {
          ...
          warnings: [
            ...
            "Geetorus currently uses Shopify's documented hosted agent-profile fixture while Geetorus's production UCP profile is being established.",
          ],
    ...
    ],
    {
      ...
      setupPrerequisite: {
        ...
        description:
          "Shopify's Storefront MCP is a public, no-auth endpoint. Geetorus cannot use the merchant's Shopify Admin session to bypass a private storefront.",
        steps: [
          ...
          "Use the permanent <store>.myshopify.com domain in Geetorus, even if the store also has a custom domain.",
        ],
  ...
  [
    "composio",
    "Composio",
    "Connect Composio so Geetorus can discover and manage the toolkits in your project.",
    "productivity",
    "composio.dev",
  ...
  [
    ...
    method(
      ...
      {},
      "S3",
      "Register an OAuth client with the provider and add Geetorus's redirect URI.",
      {
        credentialFields: [
          {
...
apps.push({
  ...
  methods: [
    {
      key: "geetorus-id-oauth",
      label: "Connect Gmail",
      transport: "mcp_remote",
      auth: "oauth",
      oauthStrategy: "geetorus_id_connector",
      grantKinds: ["user"],
      ownershipModes: ["customer"],
      whenToUse:
        "Use Geetorus ID for a personal Gmail connection with centrally registered Google OAuth.",
      defaults: {
      ...
      guidanceMd:
        "Connect your Gmail identity. Geetorus can search and read mail and create drafts. Sending mail is not enabled.",
      warnings: [
...
const specialMethodsFor = (entry) => {
  ...
  if (entry.slug === "supabase") {
    ...
    return [
      oauthMethodFor(entry, "mcp-oauth", entry.serverUrl, {
        guidanceMd:
          "Connect Supabase in the browser and scope the connection to one development project. Write tools start enabled and remain governed by Geetorus's action policies.",
        tenantFields,
      ...
      apiKeyMethodFor(entry, "mcp-api-key", entry.serverUrl, {
        guidanceMd:
          "Use a customer-created Supabase key scoped to one development project. Write tools start enabled and remain governed by Geetorus's action policies.",
        tenantFields,
        warnings: [entry.prerequisite, warning],
        requiredResourceFilters: ["project"],
      }),
...
const inferState = (slug, state) => {
  ...
  // a Geetorus connection. Keep those concepts separate: importing this review
