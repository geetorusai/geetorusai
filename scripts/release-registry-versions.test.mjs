...
function makeFixture() {
  const fixtureDir = mkdtempSync(join(tmpdir(), "geetorus-release-registry-"));
...
test("fetch prints a JSON version map and treats missing packages as empty", () => {
  ...
  const result = runScript(["fetch", "@geetorusai/present", "@geetorusai/missing"], fixture);

  assert.equal(result.status, 0);
  const map = JSON.parse(result.stdout);
  assert.deepEqual(map["@geetorusai/present"], [
    "1.0.0",
    "2026.707.0",
    "2026.707.1",
    "2026.707.1-canary.4",
  ]);
  assert.deepEqual(map["@geetorusai/missing"], []);
  assert.match(result.calls, /^npm view @geetorusai\/present versions --json$/m);
  assert.match(result.calls, /^npm view @geetorusai\/missing versions --json$/m);
});

test("assert-absent succeeds when no package has the version", () => {
  const fixture = makeFixture();
  const result = runScript(
    ["assert-absent", "2026.707.2", "@geetorusai/absent", "@geetorusai/absent"],
    fixture,
  );

  assert.equal(result.status, 0);
  assert.match(result.calls, /^npm view @geetorusai\/absent@2026\.707\.2 version$/m);
});

test("assert-absent fails and names packages that already have the version", () => {
  const fixture = makeFixture();
  const result = runScript(
    ["assert-absent", "2026.707.2", "@geetorusai/present", "@geetorusai/absent"],
    fixture,
  );

  assert.equal(result.status, 1);
  assert.match(result.stderr, /npm version @geetorusai\/present@2026\.707\.2 already exists\./);
  assert.doesNotMatch(result.stderr, /@geetorusai\/absent@/);
});

test("invalid concurrency fails instead of skipping registry checks", () => {
  const fixture = makeFixture();
  const result = runScript(["assert-absent", "2026.707.2", "@geetorusai/present"], fixture, {
...
test("next_stable_version reads RELEASE_PACKAGE_VERSIONS_FILE without calling npm", () => {
  ...
  writeFileSync(
    versionsFile,
    JSON.stringify({
      "@geetorusai/a": ["2026.707.0", "2026.707.1", "2026.707.1-canary.4"],
      "@geetorusai/b": [],
    }),
  );

  const result = runReleaseLibHelper(
    'next_stable_version 2026-07-07 "@geetorusai/a" "@geetorusai/b"',
    fixture,
...
test("next_canary_version reads RELEASE_PACKAGE_VERSIONS_FILE without calling npm", () => {
  ...
  const result = runReleaseLibHelper('next_canary_version 2026.707.1 "@geetorusai/a"', fixture, {
    RELEASE_PACKAGE_VERSIONS_FILE: versionsFile,
  });
...
test("next_stable_version falls back to npm view without a versions file", () => {
  const fixture = makeFixture();
  const result = runReleaseLibHelper('next_stable_version 2026-07-07 "@geetorusai/present"', fixture);
...
test("next_prerelease_version counts per channel so nightly numbering ignores canaries", () => {
  ...
  writeFileSync(
    versionsFile,
    JSON.stringify({
      "@geetorusai/a": ["2026.707.1-canary.4", "2026.707.1-nightly.0", "2026.707.1-nightly.1"],
    }),
  );

  const result = runReleaseLibHelper(
    'next_prerelease_version nightly 2026.707.1 "@geetorusai/a"',
    fixture,
    { RELEASE_PACKAGE_VERSIONS_FILE: versionsFile },
  );
...
test("next_prerelease_version counts beta numbering independently of other channels", () => {
  ...
  writeFileSync(
    versionsFile,
    JSON.stringify({
      "@geetorusai/a": ["2026.707.1-canary.4", "2026.707.1-nightly.3", "2026.707.1-beta.0"],
    }),
  );

  const result = runReleaseLibHelper(
    'next_prerelease_version beta 2026.707.1 "@geetorusai/a"',
    fixture,
    { RELEASE_PACKAGE_VERSIONS_FILE: versionsFile },
  );
...
test("next_prerelease_version rejects unknown channels", () => {
  const fixture = makeFixture();
  const result = runReleaseLibHelper(
    'next_prerelease_version weekly 2026.707.1 "@geetorusai/a"',
    fixture,
  );
