...
// Requires GEETORUS_API_URL and GEETORUS_API_KEY in the environment.
...
const apiUrl = process.env.GEETORUS_API_URL;
const apiKey = process.env.GEETORUS_API_KEY;
if (!apiUrl || !apiKey) {
  console.error("GEETORUS_API_URL and GEETORUS_API_KEY are required.");
  process.exit(1);
}
...
const headers = {
  ...
  ...(process.env.GEETORUS_RUN_ID ? { "X-Geetorus-Run-Id": process.env.GEETORUS_RUN_ID } : {}),
};
