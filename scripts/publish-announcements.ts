...
export function announcementPublishPrefix(staging?: string, hostPrefix?: string) {
  if (hostPrefix !== undefined && !/^[a-z0-9-]+(?:\/[a-z0-9-]+)*$/.test(hostPrefix)) {
    throw new Error("Invalid GEETORUS_PAGE_DEFAULT_PREFIX: use lowercase path segments without leading or trailing slashes");
  }
...
async function main() {
  ...
  const hostPrefix = process.env.GEETORUS_PAGE_DEFAULT_PREFIX;
  const prepared = await prepareAnnouncementPublish(sourceDirectory, staging, hostPrefix);
  const bucket = process.env.GEETORUS_PAGE_BUCKET;
  const baseUrl = process.env.GEETORUS_PAGE_BASE_URL?.replace(/\/+$/, "") ?? "https://pages.geetorus.ing";
  ...
  if (!publish) return;
  if (!bucket) throw new Error("Set GEETORUS_PAGE_BUCKET before publishing");
  const env = { ...process.env };
  const key = env.GEETORUS_PAGE_AWS_ACCESS_KEY_ID;
  const secret = env.GEETORUS_PAGE_AWS_SECRET_ACCESS_KEY;
  if (Boolean(key) !== Boolean(secret)) throw new Error("Set both namespaced page uploader credential variables");
  if (key && secret) {
    env.AWS_ACCESS_KEY_ID = key;
    env.AWS_SECRET_ACCESS_KEY = secret;
    delete env.AWS_SESSION_TOKEN;
    if (env.GEETORUS_PAGE_AWS_SESSION_TOKEN) env.AWS_SESSION_TOKEN = env.GEETORUS_PAGE_AWS_SESSION_TOKEN;
  } else if (env.GEETORUS_PAGE_AWS_PROFILE) {
    delete env.AWS_ACCESS_KEY_ID;
    delete env.AWS_SECRET_ACCESS_KEY;
    delete env.AWS_SESSION_TOKEN;
    env.AWS_PROFILE = env.GEETORUS_PAGE_AWS_PROFILE;
  }
