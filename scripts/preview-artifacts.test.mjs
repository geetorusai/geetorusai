...
const manifest = (name) => previewManifest({ name, version: "0.0.0", dependencies: name.endsWith("/db") ? { "@geetorusai/shared": "workspace:*" } : {}, publishConfig: { exports: { ".": "./dist/index.js" } } }, sha);
function pack(pkg) {
...
test("migrator-only planning never waits for GHCR and reuses complete exact-source packages", async () => {
  for (const available of [[], ["@geetorusai/shared"], ["@geetorusai/shared", "@geetorusai/db"]]) {
    const calls = [];
    ...
    assert.ok(calls.includes("@geetorusai/shared"));
    if (available.length) assert.ok(calls.includes("@geetorusai/db"));
  }
});

test("migrator-only planning rejects registry outages and mismatched source identity", async () => {
  for (const response of [json({}, 403), json({}, 503), json({ ...manifest("@geetorusai/shared"), gitHead: "b".repeat(40) })]) {
...
test("preview manifests carry exact source, isolated versions and shared dependency", () => {
  const pkg = manifest("@geetorusai/db");
  assert.equal(pkg.version, `0.0.0-preview.g${sha}`);
  assert.equal(pkg.dependencies["@geetorusai/shared"], pkg.version);
  assert.deepEqual(pkg.exports, { ".": "./dist/index.js" });
  assertMetadata(pkg, "@geetorusai/db", sha);
  assert.throws(() => assertMetadata({ ...pkg, gitHead: "b".repeat(40) }, pkg.name, sha));
  assert.throws(() => assertMetadata({ ...pkg, dependencies: { "@geetorusai/shared": "latest" } }, pkg.name, sha));
  assert.deepEqual(tarManifest(pack(pkg)), pkg);
});

test("only 404 means an artifact is missing; auth and outages are fatal", async () => {
  assert.equal(await packageExists("@geetorusai/db", sha, async () => json({}, 404)), false);
  await assert.rejects(packageExists("@geetorusai/db", sha, async () => json({}, 403)));
  await assert.rejects(packageExists("@geetorusai/db", sha, async () => json({}, 503)));
  ...
  await assert.rejects(packageExists("@geetorusai/db", sha, async () => json({ ...manifest("@geetorusai/db"), gitHead: "b".repeat(40) })));
});
...
test("publishing reuses existing previews and never executes package lifecycle hooks", async () => {
  ...
  const published = new Set(["@geetorusai/shared"]);
  const calls = [];
  try {
    for (const short of ["shared", "db"]) writeFileSync(path.join(dir, `${short}.tgz`), pack({ ...manifest(`@geetorusai/${short}`), scripts: { prepublishOnly: "do-not-run" } }));
    await publishPreview(dir, sha, {
      ...
      exec: (command, args) => { calls.push({ command, args }); published.add("@geetorusai/db"); },
      sleep: async () => {},
    });
...
test("publishing submits both packages before waiting for either to propagate", async () => {
  ...
  let polls = 0;
  try {
    for (const short of ["shared", "db"]) writeFileSync(path.join(dir, `${short}.tgz`), pack(manifest(`@geetorusai/${short}`)));
    await publishPreview(dir, sha, {
...
test("a visibility timeout identifies the missing package after both were submitted", async () => {
  ...
  try {
    ...
    await assert.rejects(publishPreview(dir, sha, {
      ...
      fetchImpl: async (url) => {
        const name = decodeURIComponent(new URL(url).pathname.split("/")[1]);
        return name === "@geetorusai/db" && submitted.includes("db")
          ? json({ ...manifest(name), dist: { integrity: "test-integrity", tarball: "https://registry.npmjs.org/package.tgz" } })
          : json({}, 404);
      },
      sleep: async () => {},
    }), /not yet visible: @geetorusai\/shared\./);
    assert.deepEqual(submitted, ["shared", "db"]);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test("invalid DB package metadata prevents publication of either package", async () => {
  ...
  try {
    writeFileSync(path.join(dir, "shared.tgz"), pack(manifest("@geetorusai/shared")));
    writeFileSync(path.join(dir, "db.tgz"), pack({ ...manifest("@geetorusai/db"), gitHead: "b".repeat(40) }));
    await assert.rejects(publishPreview(dir, sha, {
...
test("preview workflow separates branch compilation from trusted publishing", () => {
  ...
  assert.match(image, /GEETORUS_BUILD_COMMIT=\$\{\{ inputs.source_ref \}\}/);
...
test("image publisher verifies source and platform before pushing exactly one immutable tag", async () => {
  for (const revision of [sha, "c".repeat(40)]) {
    ...
    if (revision === sha) {
      await operation;
      assert.deepEqual(calls.filter((call) => call.args[0] === "push").map((call) => call.args), [["push", `ghcr.io/geetorusai/geetorus:sha-${sha}-cloud`]]);
...
test("commits sharing a short prefix use separate full-SHA image addresses", async () => {
  ...
  assert.deepEqual(urls.filter((url) => url.includes("/manifests/")), [sha, other].map((commit) => `https://ghcr.io/v2/geetorusai/geetorus/manifests/sha-${commit}-cloud`));
});
...
test("cloud builds bake the managed runtime identity and verify it before publication", () => {
  ...
  assert.ok(step.includes('test -w "$GEETORUS_HOME"'));
});
...
test("cloud cache imports are bounded, follow master ancestry, and retain the legacy fallback", () => {
  ...
  try {
    ...
    commits.unshift(git("rev-parse", "HEAD"));
    const available = `ghcr.io/geetorusai/geetorus:buildcache-cloud-${commits[2]}`;
...
`, { mode: 0o755 });
    const result = spawnSync(process.execPath, [script], {
      cwd: dir, encoding: "utf8", env: {
        ...env, PATH: `${dir}${path.delimiter}${env.PATH}`, CACHE_IMAGE: "ghcr.io/geetorusai/geetorus",
    ...
    assert.deepEqual(readFileSync(inspections, "utf8").trim().split("\n"), commits.slice(0, 3).map((commit) => `ghcr.io/geetorusai/geetorus:buildcache-cloud-${commit}`));
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test("normal cloud builds publish the checked digest only when source and platform match", () => {
  ...
  const image = `ghcr.io/geetorusai/geetorus@sha256:${"b".repeat(64)}`;
  const tag = `ghcr.io/geetorusai/geetorus:sha-${sha}-cloud`;
  try {
