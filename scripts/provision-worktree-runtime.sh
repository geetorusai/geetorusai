...
# The CLI derives the source from GEETORUS_WORKSPACE_BASE_CWD, which the control
...
if [[ -L "$base_cwd/.geetorus" && ! -d "$base_cwd/.geetorus" ]]; then
  echo "Registered base project workspace .geetorus is a broken symlink: $base_cwd/.geetorus" >&2
  exit 1
fi
source_config_args=()
if [[ ! -e "$base_cwd/.geetorus/config.json" && ! -L "$base_cwd/.geetorus/config.json" ]]; then
  ...
  # GEETORUS_CONFIG at the target. Naming the target as its own source is never
...
run_ensure_seeded() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm geetorusai --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        pnpm geetorusai worktree ensure-seeded --config "$worktree_config_path" ${source_config_args[@]+"${source_config_args[@]}"}
    )
    return
  fi
  ...
  if command -v geetorusai >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        geetorusai worktree ensure-seeded --config "$worktree_config_path" ${source_config_args[@]+"${source_config_args[@]}"}
    )
    return
  fi
...
if run_ensure_seeded; then
  ...
  if [[ "$exit_code" -eq 127 ]]; then
    echo "No usable geetorusai CLI found; cannot seed the worktree database." >&2
  fi
  exit "$exit_code"
fi
