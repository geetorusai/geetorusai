...
async function ensureWorkspaceLinksCurrent(workspaceDir: string) {
  ...
  console.log(`[geetorus] detected stale workspace package links for ${workspaceDir}; relinking dependencies...`);
  for (const mismatch of mismatches) {
    console.log(
      `[geetorus]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
