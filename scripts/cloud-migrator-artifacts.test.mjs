...
function fixture(t) {
  ...
  for (const name of ["db", "shared"]) {
    const bytes = Buffer.from(JSON.stringify(previewManifest({ name: `@geetorusai/${name}`, dependencies: {} }, sha)));
  ...
  const manifest = buildBundle(dir, sha, { exec: (cmd, args, options) => {
    ...
    assert.deepEqual(localRoot.dependencies, { "@geetorusai/db": "file:db.tgz", "@geetorusai/shared": "file:shared.tgz" });
    const packages = { "": localRoot };
    for (const name of ["db", "shared"]) packages[`node_modules/@geetorusai/${name}`] = {
      version: versionFor(sha), integrity: descriptor(readFileSync(path.join(dir, `${name}.tgz`)), "tgz").integrity,
      resolved: `file:${name}.tgz`, ...(name === "db" ? { dependencies: { "@geetorusai/shared": versionFor(sha) } } : {}),
    };
...
test("lockfile rejects mutable, foreign, linked, and mismatched dependencies", (t) => {
  ...
  for (const mutate of [
    (l) => { l.packages[""].dependencies["@geetorusai/db"] = "latest"; },
    (l) => { l.packages["node_modules/@geetorusai/shared"].version = "0.0.0"; },
    (l) => { l.packages["node_modules/@geetorusai/db"].link = true; },
    ...
    (l) => { l.packages["node_modules/a/node_modules/@geetorusai/shared"] = l.packages["node_modules/@geetorusai/shared"]; },
  ]) {
...
test("real npm ci installs the new pair from pinned archives with an empty cache", async (t) => {
  ...
  for (const name of ["db", "shared"]) lock.packages[`node_modules/@geetorusai/${name}`].resolved = `${base}/${name}.tgz`;
  writeFileSync(path.join(dir, "package.json"), JSON.stringify({ name: "geetorus-migrator-install-root", version: "0.0.0", private: true, dependencies: { "@geetorusai/db": versionFor(sha) } }));
  ...
  for (const name of ["db", "shared"]) assert.equal(JSON.parse(readFileSync(path.join(dir, `node_modules/@geetorusai/${name}/package.json`))).version, manifest.packageVersion);
});
...
test("AWS trust is master-only and publication policy cannot overwrite objects", () => {
  ...
  assert.equal(read("trust-policy").Statement[0].Condition.StringEquals["token.actions.githubusercontent.com:sub"], "repo:geetorusai/geetorus:ref:refs/heads/master");
