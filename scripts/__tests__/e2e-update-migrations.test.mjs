...
const cleanEnv = Object.fromEntries(
  Object.entries(process.env).filter(([name]) => !name.startsWith("GEETORUS_") && !name.startsWith("E2E_UPDATE_")),
);
...
test("cross-version migration harness requires both refs before side effects", () => {
  for (const testCase of [
    ...
    const testHome = mkdtempSync(path.join(os.tmpdir(), "geetorus-update-migrations-"));
    try {
