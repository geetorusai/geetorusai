...
// reachable, but a globally installed `geetorusai` must not shadow the paths
// under test.
...
function makeInstanceHome() {
  const home = makeTempDir("geetorus-provision-instance-home-");
...
function makeBaseWorkspace({ helpExit, initExit, ensureExit = 0 }) {
  const baseCwd = makeTempDir("geetorus-provision-base-");
...
if (cliArgs[0] === "worktree" && cliArgs[1] === "init") {
  ...
  fs.mkdirSync(".geetorus", { recursive: true });
  fs.writeFileSync(".geetorus/config.json", JSON.stringify({ $meta: { source: "fake-cli" } }));
  fs.writeFileSync(".geetorus/.env", "GEETORUS_IN_WORKTREE=true\\n");
  process.exit(0);
}
if (cliArgs[0] === "worktree" && cliArgs[1] === "ensure-seeded") {
  ...
  fs.rmSync(".geetorus/seed-pending", { force: true });
  fs.rmSync(".geetorus/seed-complete", { force: true });
  fs.writeFileSync(".geetorus/seed-manifest.json", JSON.stringify({
    version: 2,
    source: { instanceId: "base-source", configPath: ${JSON.stringify(path.join(baseCwd, ".geetorus", "config.json"))} },
...
function runProvision(baseCwd, { pathPrefix, setupWorktree, existingWorktree } = {}) {
  const worktreeCwd = existingWorktree ?? makeTempDir("geetorus-provision-worktree-");
  setupWorktree?.(worktreeCwd);
  const worktreesHome = makeTempDir("geetorus-provision-home-");
  const geetorusHome = makeInstanceHome();
  const result = spawnSync("bash", [script], {
    ...
    env: {
      ...
      HOME: os.homedir(),
      GEETORUS_WORKSPACE_BASE_CWD: baseCwd,
      GEETORUS_WORKSPACE_CWD: worktreeCwd,
      GEETORUS_WORKSPACE_BRANCH: "feature/provision-test",
      GEETORUS_WORKTREES_DIR: worktreesHome,
      GEETORUS_HOME: geetorusHome,
      GEETORUS_PROJECT_WORKSPACE_ID: "project-workspace-1",
      GEETORUS_SEED_EXPECTED_COMPANY_ID: "company-1",
    },
  });
  return { result, worktreeCwd, worktreesHome, geetorusHome };
}

function runRuntimeProvision(baseCwd, worktreeCwd) {
  const worktreesHome = makeTempDir("geetorus-provision-runtime-home-");
  const geetorusHome = makeInstanceHome();
  return spawnSync("bash", [runtimeScript], {
    ...
    env: {
      ...
      GEETORUS_WORKSPACE_BASE_CWD: baseCwd,
      GEETORUS_WORKSPACE_CWD: worktreeCwd,
      GEETORUS_WORKSPACE_BRANCH: "feature/provision-runtime-test",
      GEETORUS_WORKTREES_DIR: worktreesHome,
      GEETORUS_HOME: geetorusHome,
      GEETORUS_PROJECT_WORKSPACE_ID: "project-workspace-1",
      GEETORUS_COMPANY_ID: "company-1",
    },
  });
}
...
function readWorktreeConfig(worktreeCwd) {
  const configPath = path.join(worktreeCwd, ".geetorus", "config.json");
...
test("uses the base CLI when its import graph boots", () => {
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(worktreeCwd, ".geetorus", "seed-manifest.json"), "utf8")).state,
    "pending",
  );
...
test("rejects a dangling base workspace .geetorus symlink instead of falling back", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  // `-e`/`-L` on the config resolve `.geetorus` first, so the config reads as absent
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /\.geetorus is a broken symlink/);
});
...
test("falls back to an isolated config when the base CLI cannot boot", () => {
  ...
  const env = fs.readFileSync(path.join(worktreeCwd, ".geetorus", ".env"), "utf8");
  assert.match(env, /GEETORUS_IN_WORKTREE=true/);
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(worktreeCwd, ".geetorus", "seed-manifest.json"), "utf8")).state,
    "pending",
  );
...
test("reconciles deployment mode from the registered source when reusing a guest config", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 1, initExit: 0 });
  const { result: first, worktreeCwd, worktreesHome, geetorusHome } = runProvision(baseCwd);
  ...
  fs.mkdirSync(path.join(baseCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(
    path.join(baseCwd, ".geetorus", "config.json"),
    `${JSON.stringify({
      server: {
  ...
  assert.match(second.stderr, /Reusing existing isolated Geetorus worktree config/);
  assert.match(second.stderr, /Reconciled isolated Geetorus worktree deployment mode/);
...
test("repairs an unhealthy base install under the lock and then uses the CLI", (t) => {
  ...
  const baseCwd = makeTempDir("geetorus-provision-repair-base-");
...
`,
  ...
  const fakeBin = makeTempDir("geetorus-provision-fakebin-");
...
`,
  ...
  assert.ok(
    fs.existsSync(path.join(baseCwd, ".git", "geetorus-provision-repair.lock")),
...
test("a failed CLI init fails provisioning instead of being masked as success", () => {
  ...
  assert.ok(!fs.existsSync(path.join(worktreeCwd, ".geetorus", "config.json")));
});

test("runtime provisioning invokes ensure-seeded once and fast-exits after success", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  const worktreeCwd = makeTempDir("geetorus-provision-runtime-worktree-");
  fs.mkdirSync(path.join(worktreeCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "config.json"), "{}\n");
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "seed-pending"), "{}\n");
  ...
  assert.ok(!fs.existsSync(path.join(worktreeCwd, ".geetorus", "seed-pending")));
...
test("runtime provisioning omits the source override when the base config exists", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  fs.mkdirSync(path.join(baseCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(path.join(baseCwd, ".geetorus", "config.json"), "{}\n");
  const worktreeCwd = makeTempDir("geetorus-provision-runtime-base-config-");
  fs.mkdirSync(path.join(worktreeCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "config.json"), "{}\n");
...
test("runtime provisioning seeds a worktree config that has no seed markers", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  const worktreeCwd = makeTempDir("geetorus-provision-runtime-unmarked-config-");
  fs.mkdirSync(path.join(worktreeCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "config.json"), "{}\n");
...
test("runtime provisioning bootstraps and seeds an empty .geetorus directory", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  ...
  const worktreeCwd = makeTempDir("geetorus-provision-runtime-empty-state-");
  fs.mkdirSync(path.join(worktreeCwd, ".geetorus"), { recursive: true });
...
test("runtime provisioning leaves seed-pending in place when ensure-seeded fails", () => {
  ...
  const worktreeCwd = makeTempDir("geetorus-provision-runtime-failure-");
  fs.mkdirSync(path.join(worktreeCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "config.json"), "{}\n");
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "seed-pending"), "{}\n");
  ...
  assert.ok(fs.existsSync(path.join(worktreeCwd, ".geetorus", "seed-pending")));
  assert.ok(!fs.existsSync(path.join(worktreeCwd, ".geetorus", "seed-complete")));
});

test("runtime provisioning does not trust a truncated verified manifest", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0, ensureExit: 4 });
  const worktreeCwd = makeTempDir("geetorus-provision-runtime-truncated-");
  fs.mkdirSync(path.join(worktreeCwd, ".geetorus"), { recursive: true });
  fs.writeFileSync(path.join(worktreeCwd, ".geetorus", "config.json"), "{}\n");
  fs.writeFileSync(
    path.join(worktreeCwd, ".geetorus", "seed-manifest.json"),
...
for (const failure of ["ERR_PNPM_LOCKFILE_CONFIG_MISMATCH", "ERR_PNPM_OUTDATED_LOCKFILE", "ENOTFOUND", "retry-fails"]) {
  test(`dependency provisioning preserves failures and bounds recovery: ${failure}`, () => {
    const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
    const bin = makeTempDir("geetorus-fake-pnpm-");
...
`, { mode: 0o700 });
    ...
    assert.equal(fs.existsSync(path.join(worktreeCwd, ".geetorus/pnpm-install-fingerprint")), recovers);
...
test("patch content changes invalidate an otherwise matching install fingerprint", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  const bin = makeTempDir("geetorus-patch-pnpm-");
