...
function createReleaseFixture() {
  const fixtureDir = mkdtempSync(join(tmpdir(), "geetorus-release-dry-run-"));
...
if (mode === "fetch") {
  process.stdout.write('{"geetorusai":[]}\\n');
  process.exit(0);
}
...
function preparePreviewFixture(fixture) {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "geetorusai", version: "2026.710.0-canary.0" }),
  );
