...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-build-lock-"));
...
`);
  ...
  const lock = path.join(root, "node_modules/.cache/geetorus-plugin-build-deps.lock");
  const launch = (env = {}) => {
...
test("recovers the old empty lock left by interrupted startup", async (t) => {
  ...
  assert.match(result.output, /Building @geetorusai\/shared/);
...
test("rejects partial output from an interrupted direct compiler", async (t) => {
  ...
  const completionTime = fs.statSync(path.join(target, "dist/.geetorus-build-complete")).mtimeMs;
  await sleep(20);
...
test("shared source changes invalidate both shared and dependent SDK output", async (t) => {
  ...
  const retry = await f.launch().done;
  assert.equal(retry.code, 0, retry.output);
  assert.match(retry.output, /Building @geetorusai\/shared/);
  assert.match(retry.output, /Building @geetorusai\/plugin-sdk/);
  assert.equal(fs.readFileSync(path.join(f.root, "builds"), "utf8").trim().split("\n").length, 4);
});
