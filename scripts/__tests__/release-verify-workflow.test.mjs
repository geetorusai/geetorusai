...
test("canary reuses exact-source proof while stable keeps full verification", () => {
  ...
  assert.match(canary, /github\.repository == 'geetorusai\/geetorus' && github\.event_name == 'push' && github\.ref == 'refs\/heads\/master'/);
...
test("published canaries are gated by the exact-version onboarding browser smoke", () => {
  ...
  assert.match(
    releaseWorkflow,
    /GEETORUSAI_VERSION: \$\{\{ needs\.publish_canary\.outputs\.canary_version \}\}/,
  );
  ...
  assert.match(
    releaseWorkflow,
    /name: Smoke exact published canary through onboarding\n\s+env:\n\s+GEETORUS_CANARY_SMOKE_SERVER_LOG: \$\{\{ runner\.temp \}\}\/canary-onboarding-server\.log/,
  );
  assert.match(
    releaseWorkflow,
...
test("release smoke workflow extends the container readiness budget for CI", () => {
  ...
  // CI containers cold-install geetorusai and embedded postgres, so the
  ...
  assert.match(
    harness,
    /wait_for_http "\$GEETORUS_PUBLIC_URL\/api\/health" "\$SMOKE_READY_TIMEOUT_SECONDS" 1/,
  );
});
...
test("release verify workflow covers the same split test surface as stable PR verification", () => {
  ...
  const runnerScripts = JSON.parse(readFileSync(path.join(repoRoot, "packages/geetorus-runner/package.json"), "utf8")).scripts;
  ...
  assert.match(verifyWorkflow, /pnpm --filter @geetorusai\/geetorus-runner "\$check"/);
...
test("Runner eval workflows pin actions and gate paid live execution", () => {
  ...
  for (const [base, block] of [
    [path.join(repoRoot, "packages/geetorus-runner"), runnerBlock],
