...
function readTrustedPrWorkflow() {
  ...
  assert.match(
    caller,
    /^\s+uses: geetorusai\/geetorus\/\.github\/workflows\/pr-trusted\.yml@master\s*$/m,
...
function runStackScope(stack, prBaseRef) {
  ...
  const script = match[1]
    ...
    .join("\n");
  const scratch = mkdtempSync(path.join(tmpdir(), "geetorus-stack-scope-"));
...
test("the trusted PR workflow limits full CI to merge-relevant stack layers", () => {
  ...
  assert.match(
    verify,
    /^ {4}needs: \[gate, policy, typecheck_release_registry, general_tests, verify_geetorus_runner, build, docker_context_integrity\]$/m,
  );
  ...
  assert.match(verify, /RUNNER_VERIFICATION_RESULT: \$\{\{ needs\.verify_geetorus_runner\.result \}\}/);
