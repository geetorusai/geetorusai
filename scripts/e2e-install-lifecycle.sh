#!/usr/bin/env bash
# End-to-end proof of the geetorusai managed install lifecycle on a CLEAN machine.
#
...
#
# Env knobs:
#   E2E_REPO          GitHub repo to install from (default: geetorusai/geetorus)
...
E2E_REPO="${E2E_REPO:-geetorusai/geetorus}"
E2E_REF="${E2E_REF:-master}"
E2E_SERVICE_TIMEOUT_SECS="${E2E_SERVICE_TIMEOUT_SECS:-300}"

# A clean environment: no inherited Geetorus or build-mode state.
for var in $(env | grep -o '^GEETORUS_[A-Z_]*' || true); do unset "$var"; done
unset NODE_ENV npm_config_prefix 2>/dev/null || true
export COREPACK_ENABLE_DOWNLOAD_PROMPT=0
export CI="${CI:-1}"

SHIM="$HOME/.local/bin/geetorusai"
STORE="$HOME/.geetorus/cli"
RESULTS=()
FAILED=0
...
# bootstrap simulates what `npx geetorusai@<channel> install` will run post-release:
...
if GEETORUS_README_ASSET_REF="$E2E_REF" \
...
BOOTSTRAP_CLI="$HOME/e2e-bootstrap-cli/node_modules/geetorusai/dist/index.js"
...
if [ "${E2E_SKIP_NPM:-0}" != "1" ]; then
  ...
  [ -x "$SHIM" ] && pass "2b shim created at ~/.local/bin/geetorusai" || fail_ "2b shim created"
  ...
  [ -n "$NPM_VERSION" ] && pass "2e shim runs: geetorusai --version = $NPM_VERSION" || fail_ "2e shim runs geetorusai --version"
else
...
mkdir -p "$HOME/.geetorus" && touch "$HOME/.geetorus/e2e-user-data-marker"
...
[ -f "$HOME/.geetorus/e2e-user-data-marker" ] && pass "10d user data under ~/.geetorus preserved" || fail_ "10d user data preserved"
