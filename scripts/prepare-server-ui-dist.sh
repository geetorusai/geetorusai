...
# This keeps @geetorusai/server publish artifacts self-contained for static UI serving.
# When GEETORUS_RELEASE_REUSE_UI_DIST=1 and ui/dist already exists, reuse that
...
case "${GEETORUS_RELEASE_REUSE_UI_DIST:-}" in
  1|true|TRUE|yes|YES)
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @geetorusai/ui dist output"
else
  echo "  -> Building @geetorusai/ui..."
  pnpm --dir "$REPO_ROOT" --filter @geetorusai/ui build
fi
