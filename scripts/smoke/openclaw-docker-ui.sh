...
GEETORUS_HOST_PORT="${GEETORUS_HOST_PORT:-3100}"
GEETORUS_HOST_FROM_CONTAINER="${GEETORUS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.geetorus-openclaw.override.yml"
...
detect_geetorus_base_url() {
  ...
  for candidate in "$GEETORUS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${GEETORUS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${GEETORUS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
geetorus_base_url="$(detect_geetorus_base_url || true)"
...
Geetorus URL for OpenClaw container:
EOF
  if [[ -n "$geetorus_base_url" ]]; then
    cat <<EOF
  $geetorus_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${GEETORUS_HOST_PORT}
  (Do not use http://127.0.0.1:${GEETORUS_HOST_PORT} inside the container.)
  If Geetorus rejects the host, run on host machine:
    pnpm geetorusai allowed-hostname host.docker.internal
  Then restart Geetorus and re-run this script.
EOF
  fi
  cat <<EOF
