...
const COMPLETE_ENV = {
  GEETORUS_E2E_BASE_URL: "https://geetorus.example.test",
  GEETORUS_E2E_EMAIL: "operator@example.test",
  GEETORUS_DEV_LOGIN_PASSWORD: "not-a-real-password",
  GEETORUS_API_URL: "https://geetorus.example.test/api",
  GEETORUS_API_KEY: "not-a-real-agent-key",
  GEETORUS_RUN_ID: "run-123",
  GEETORUS_TASK_ID: "issue-123",
};

test("preflight reports binding names without exposing supplied values", () => {
  assert.throws(
    () => preflightNotionGenericLive({ GEETORUS_DEV_LOGIN_PASSWORD: "present" }),
    (error) => {
...
test("preflight requires explicit credential-free HTTPS target and control-plane URLs", () => {
  for (const baseUrl of [
    "http://127.0.0.1:3100",
    "http://geetorus.example.test",
    "https://user:secret@geetorus.example.test",
    "https://geetorus.example.test/?code=secret",
  ]) {
    assert.throws(
      () => preflightNotionGenericLive({ ...COMPLETE_ENV, GEETORUS_E2E_BASE_URL: baseUrl }),
      (error) => error instanceof NotionGenericLivePreflightError && error.code === "unsafe_base_url",
    );
  }
  const split = preflightNotionGenericLive({
    ...COMPLETE_ENV,
    GEETORUS_API_URL: "https://control-plane.example.test/api",
  });
  assert.equal(split.baseUrl, "https://geetorus.example.test");
...
test("health and binding metadata pass before browser loading, without fetching the value", async () => {
  ...
  assert.equal(prepared.config.callbackUrl, "https://geetorus.example.test/api/tools/oauth/callback");
...
test("authorization proof requires automatic registration, PKCE, callback, resource, and safe endpoints", () => {
  ...
  const baseUrl = "https://geetorus.example.test";
...
test("authorization proof rejects a provider login page after OAuth parameters were consumed", () => {
  assert.throws(
    () => inspectAuthorizationUrl("https://id.notion.test/login", {
      callbackUrl: "https://geetorus.example/api/tools/oauth/callback",
      resource: "https://mcp.notion.com/mcp",
      registrationSource: "dcr",
      baseUrl: "https://geetorus.example",
    }),
...
test("workspace proof extraction and fresh-run comments retain only sanitized identity", () => {
    ...
    result: {
      content: [{
        ...
        text: JSON.stringify({
          id: "bot-123",
          type: "bot",
          bot: { workspace_id: "workspace-123", workspace_name: "Geetorus" },
          token: "discard-me",
        }),
      }],
    },
  ...
  assert.deepEqual(identity, {
    ...
    workspaceName: "Geetorus",
    botId: "bot-123",
  });
  assert.deepEqual(
    parseSanitizedAgentProof(
      '{"workspaceId":"workspace-123","workspaceName":"Geetorus","invocationId":"inv-123"}',
      identity,
    ),
    { workspaceId: "workspace-123", workspaceName: "Geetorus", invocationId: "inv-123" },
  );
