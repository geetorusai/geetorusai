...
: "${GEETORUS_API_URL:?Set GEETORUS_API_URL for the target dev instance.}"
: "${GEETORUS_API_KEY:?Set GEETORUS_API_KEY for the target dev instance.}"
: "${GEETORUS_COMPANY_ID:?Set GEETORUS_COMPANY_ID for the target dev company.}"

read -r -a PC_CMD <<< "${GEETORUSAI_CMD:-pnpm --silent geetorusai}"
...
pc_json() {
  "${PC_CMD[@]}" "$@" --json -C "$GEETORUS_COMPANY_ID"
}
