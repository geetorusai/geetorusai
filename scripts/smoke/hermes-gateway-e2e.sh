...
GEETORUS_API_URL="${GEETORUS_API_URL:-http://127.0.0.1:3100}"
API_BASE="${GEETORUS_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${GEETORUS_COMPANY_ID:-}}"
COMPANY_SELECTOR="${COMPANY_SELECTOR:-}"

RUN_SUFFIX="${HERMES_SMOKE_RUN_SUFFIX:-$(date +%Y%m%d-%H%M%S)-$$}"
HERMES_IMAGE="${HERMES_IMAGE:-geetorus-hermes-gateway-smoke:local}"
...
HERMES_CONTAINER_NAME="${HERMES_CONTAINER_NAME:-geetorus-hermes-gateway-smoke-${RUN_SUFFIX}}"
...
HERMES_SMOKE_STATE_DIR="${HERMES_SMOKE_STATE_DIR:-${TMPDIR:-/tmp}/geetorus-hermes-gateway-smoke-${RUN_SUFFIX}}"
HERMES_SMOKE_DIAG_DIR="${HERMES_SMOKE_DIAG_DIR:-${TMPDIR:-/tmp}/geetorus-hermes-gateway-e2e-diag-${RUN_SUFFIX}}"
...
GEETORUS_API_URL_FOR_HERMES="${GEETORUS_API_URL_FOR_HERMES:-http://host.docker.internal:3100}"
...
joins it to Geetorus as a hermes_gateway agent, wakes that agent on a smoke
issue, verifies the issue result, captures redacted diagnostics, and cleans up
Geetorus and Docker state unless HERMES_SMOKE_KEEP=1.

Required:
  GEETORUS_API_URL=http://127.0.0.1:3100
  GEETORUS_AUTH_HEADER='Bearer <board-token>'     # or GEETORUS_COOKIE

Common flags:
  COMPANY_ID=<uuid> or COMPANY_SELECTOR=<prefix|name|uuid>
  HERMES_VERSION=0.17.0
  HERMES_IMAGE=geetorus-hermes-gateway-smoke:local
  HERMES_GATEWAY_PORT=8642
  HERMES_GATEWAY_API_BASE_URL=http://127.0.0.1:8642
  HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642
  GEETORUS_API_URL_FOR_HERMES=http://host.docker.internal:3100
...
Mode notes:
  HERMES_GATEWAY_API_BASE_URL is the URL stored on the Geetorus adapter and
  must be reachable by the Geetorus server. HERMES_GATEWAY_PROBE_URL is the URL
  this operator shell uses for direct gateway checks. They can differ for Docker
  network and reverse-proxy smoke runs.

  Raw Hermes and Geetorus API keys are redacted from logs and diagnostic files.
...
AUTH_HEADERS=()
if [[ -n "${GEETORUS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${GEETORUS_AUTH_HEADER}")
elif [[ -n "${GEETORUS_API_KEY:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: Bearer ${GEETORUS_API_KEY}")
fi
if [[ -n "${GEETORUS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${GEETORUS_COOKIE}")
  GEETORUS_BROWSER_ORIGIN="${GEETORUS_BROWSER_ORIGIN:-${GEETORUS_API_URL%/}}"
  AUTH_HEADERS+=(-H "Origin: ${GEETORUS_BROWSER_ORIGIN}" -H "Referer: ${GEETORUS_BROWSER_ORIGIN}/")
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board/operator auth required. Set GEETORUS_COOKIE, GEETORUS_AUTH_HEADER, or a board-capable GEETORUS_API_KEY."
  fi
...
capture_diagnostics() {
  mkdir -p "$HERMES_SMOKE_DIAG_DIR"
  {
    ...
    echo "geetorusApiUrl=${GEETORUS_API_URL}"
    echo "geetorusApiUrlForHermes=${GEETORUS_API_URL_FOR_HERMES}"
  ...
  capture_container_logs
  capture_issue_diagnostics "$SMOKE_ISSUE_ID" "geetorus-smoke"
  capture_run_diagnostics "$RUN_ID" "geetorus-smoke"
}

cleanup_geetorus_state() {
...
start_container() {
  ...
  local args=(
    ...
    -e GEETORUS_API_URL="$GEETORUS_API_URL_FOR_HERMES"
    -e NO_COLOR=1
...
assert_fresh_container_state() {
  ...
  docker exec "$HERMES_CONTAINER_NAME" sh -lc 'env | sort | grep -E "^(HOME|HERMES_HOME|XDG_|API_SERVER_|GEETORUS_API_URL)=" || true' > "${HERMES_SMOKE_DIAG_DIR}/container-env.txt"
  ...
  if docker exec "$HERMES_CONTAINER_NAME" sh -lc 'env | grep -q "^GEETORUS_API_KEY="'; then
    fail "container unexpectedly has GEETORUS_API_KEY before join/key claim"
  fi
}

probe_container_to_geetorus() {
  log "probing container-to-Geetorus connectivity at ${GEETORUS_API_URL_FOR_HERMES}/api/health"
  if ! docker exec "$HERMES_CONTAINER_NAME" curl -fsS --max-time 8 "${GEETORUS_API_URL_FOR_HERMES%/}/api/health" > "${HERMES_SMOKE_DIAG_DIR}/container-geetorus-health.json"; then
    fail "Hermes container cannot reach Geetorus. Set GEETORUS_API_URL_FOR_HERMES to a URL reachable from inside Docker, or keep HERMES_DOCKER_ADD_HOST=1 for Linux host.docker.internal."
  fi
}
...
probe_gateway_readiness() {
  ...
  if [[ "$HERMES_GATEWAY_PROBE_URL" != "$HERMES_GATEWAY_API_BASE_URL" ]]; then
    log "Geetorus will store Hermes gateway URL ${HERMES_GATEWAY_API_BASE_URL}"
  fi
...
assert_direct_gateway_run() {
  ...
  payload="$(jq -nc \
    --arg marker "$marker" \
    --arg session "geetorus-smoke-direct-${RUN_SUFFIX}" \
    '{input: ("Reply with exactly " + $marker + " and no other text."), instructions: "You are running a Geetorus Hermes gateway smoke direct API assertion.", session_id: $session}')"
...
assert_stop_behavior_if_deterministic() {
  ...
  local payload
  payload="$(jq -nc \
    --arg session "geetorus-smoke-stop-${RUN_SUFFIX}" \
...
join_hermes_agent() {
  ...
  GEETORUS_API_URL_FOR_HERMES="$GEETORUS_API_URL_FOR_HERMES" \
  GEETORUS_AUTH_HEADER="${GEETORUS_AUTH_HEADER:-}" \
  GEETORUS_API_KEY="${GEETORUS_API_KEY:-}" \
  GEETORUS_COOKIE="${GEETORUS_COOKIE:-}" \
...
install_claimed_key_in_container() {
  log "placing newly claimed Geetorus key in container workspace"
  local key_file="${HERMES_SMOKE_STATE_DIR}/workspace/geetorus-claimed-api-key.json"
  ...
  chmod 644 "$key_file"
  docker exec "$HERMES_CONTAINER_NAME" sh -lc 'test -f /home/hermes/workspace/geetorus-claimed-api-key.json && test ! -e "$HERMES_HOME/host-sentinel.txt"'
}

patch_agent_instructions_with_claimed_key() {
  log "patching Hermes agent instructions with claimed Geetorus API context"
  api_request "GET" "/agents/${AGENT_ID}"
  assert_status "200"

  local instructions patch_payload
  instructions="For this smoke run only, call Geetorus at ${GEETORUS_API_URL_FOR_HERMES}. Read /home/hermes/workspace/geetorus-claimed-api-key.json and use its token as GEETORUS_API_KEY for Geetorus API requests. Do not reveal this key. When mutating Geetorus, include X-Geetorus-Run-Id with the current Geetorus run id when available."
...
create_smoke_issue() {
  local marker="HERMES_GEETORUS_E2E_OK_${RUN_SUFFIX}"
  local title="[Hermes Gateway Smoke] ${RUN_SUFFIX}"
  local description
  description="Hermes gateway full Docker e2e smoke.\n\n1. Read this issue.\n2. Post a Geetorus issue comment containing exactly: ${marker}\n3. Mark this issue done.\n\nUse the Geetorus API URL and key provided in your run instructions. Do not reveal secrets."
  ...
  echo "$marker" > "${HERMES_SMOKE_DIAG_DIR}/geetorus-marker.txt"
}
...
trigger_wakeup() {
  ...
  log "triggered Geetorus run ${RUN_ID}"
}
...
assert_geetorus_wake_success() {
  local marker
  marker="$(cat "${HERMES_SMOKE_DIAG_DIR}/geetorus-marker.txt")"

  trigger_wakeup
  local run_status issue_status marker_found
  run_status="$(wait_for_run_terminal "$RUN_ID" "$RUN_TIMEOUT_SEC")"
  log "Geetorus run ${RUN_ID} status=${run_status}"
  ...
  if [[ "$STRICT_CASES" == "1" ]]; then
    [[ "$run_status" == "succeeded" ]] || fail "Geetorus Hermes gateway run did not succeed"
...
main() {
  ...
  assert_status "200"
  log "Geetorus health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
  require_board_auth
  resolve_company_id
