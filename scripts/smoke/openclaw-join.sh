...
SMOKE_IMAGE="${SMOKE_IMAGE:-geetorus-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-geetorus-openclaw-smoke}"
...
assert_status "200"
if ! grep -q "Geetorus OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.geetorus.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
