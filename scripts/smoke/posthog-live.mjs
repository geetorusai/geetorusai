...
async function gotoGeetorusPage(
  page,
  url,
  readyLocator,
  checkpoint,
...
async function safePageState(page, resourceFailures, geetorusOrigin) {
  let current;
  try {
  ...
  return {
    location: current.origin === geetorusOrigin
...
async function completePosthogAuthorization(page, config) {
  ...
  while (Date.now() < deadline) {
    ...
    if (current.origin === geetorusOrigin && current.pathname.includes("/apps/")) return;
...
async function runSmoke({ config, chromium }) {
  ...
  const outputDirectory = process.env.POSTHOG_EVIDENCE_DIR
    ...
    : path.join(process.env.GEETORUS_RUN_SCRATCH_DIR || process.cwd(), `posthog-live-${runKey}`);
  ...
  try {
    ...
    activeCheckpoint = "A.geetorus-login";
    await gotoGeetorusPage(
      page,
    ...
    await page.waitForURL((url) => url.pathname !== "/auth", { timeout: 30_000 }).catch(() => {
      fail("A.geetorus-login", "login_redirect_missing");
    });
    ...
    const parentIssueId = process.env.POSTHOG_PROOF_PARENT_ISSUE_ID || process.env.GEETORUS_TASK_ID;
    ...
    const child = await apiJson(
      ...
      `/api/issues/${parentIssueId}/children`,
      {
        ...
        description: [
          ...
          "Then post exactly one JSON object with keys `projectId`, `projectName`, and `invocationId` (the Geetorus invocation ID), and mark this issue done.",
        ...
        acceptanceCriteria: [
          ...
          "The comment contains only sanitized project ID/name and Geetorus invocation ID.",
