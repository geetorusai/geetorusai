...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-geetorus-smoke}"
...
GEETORUS_API_URL_FOR_OPENCLAW="${GEETORUS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set GEETORUS_COOKIE or GEETORUS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg geetorusApiUrl "$GEETORUS_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/geetorus"
  local claimed_file="${workspace_dir}/geetorus-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/geetorus-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/geetorus-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/geetorus-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/geetorus"
    assert_status "200"
    {
      echo "GEETORUS_API_URL=${GEETORUS_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set GEETORUS_API_KEY=${AGENT_API_KEY} in your run context before running Geetorus heartbeat steps."
...
main() {
  ...
  wait_http_ready "${GEETORUS_API_URL%/}/api/health" 15 || fail "Geetorus API health endpoint not reachable"
