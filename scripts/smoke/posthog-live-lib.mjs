const REQUIRED_ENVIRONMENT = [
  ...
  "INTEGRATIONS_POSTHOG_GEETORUS_DEV_LOGIN_PASSWORD",
...
export function preflightPosthogLive(environment = process.env, options = {}) {
  ...
  const baseUrlValue = typeof options.baseUrl === "string" && options.baseUrl.trim()
    ...
    : requiredValue(environment, "GEETORUS_API_URL");
  if (!baseUrlValue) {
  ...
  const email = requiredValue(environment, "INTEGRATIONS_POSTHOG_GEETORUS_E2E_EMAIL");
  if (!email.includes("@")) {
  ...
  return {
    ...
    email,
    password: environment.INTEGRATIONS_POSTHOG_GEETORUS_DEV_LOGIN_PASSWORD,
    projectId,
  };
}
...
export function preflightFailureMessage(error) {
  ...
  if (error.code === "missing_base_url") {
    return "PostHog live smoke preflight failed: pass the target Geetorus URL or run inside a Geetorus heartbeat.";
  }
  if (error.code === "invalid_arguments") {
    return "PostHog live smoke preflight failed: expected an optional Geetorus URL or --base-url <url>.";
  }
