...
export function preflightNotionGenericLive(environment = process.env) {
  ...
  const base = explicitHttpsOrigin(requiredValue(environment, "GEETORUS_E2E_BASE_URL"), "unsafe_base_url");
  const api = explicitHttpsOrigin(requiredValue(environment, "GEETORUS_API_URL"), "unsafe_api_url");

  const email = requiredValue(environment, "GEETORUS_E2E_EMAIL");
  if (!email.includes("@")) throw new NotionGenericLivePreflightError("invalid_geetorus_email");
  ...
  return {
    ...
    geetorusEmail: email,
    geetorusPassword: environment.GEETORUS_DEV_LOGIN_PASSWORD,
    agentApiKey: environment.GEETORUS_API_KEY,
    runId: environment.GEETORUS_RUN_ID,
    taskId: environment.GEETORUS_TASK_ID,
