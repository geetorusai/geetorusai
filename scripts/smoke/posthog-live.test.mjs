...
const COMPLETE_ENV = {
  GEETORUS_API_URL: "https://geetorus.example.test/api",
  INTEGRATIONS_POSTHOG_GEETORUS_E2E_EMAIL: "operator@example.test",
  INTEGRATIONS_POSTHOG_GEETORUS_DEV_LOGIN_PASSWORD: "not-a-real-password",
...
test("preflight reports only missing binding names", () => {
  assert.throws(
    () => preflightPosthogLive({
      GEETORUS_API_URL: "https://geetorus.example.test/api",
      INTEGRATIONS_POSTHOG_GEETORUS_DEV_LOGIN_PASSWORD: "present",
    }),
    (error) => {
      ...
      assert.deepEqual(error.details.missing, [
        "INTEGRATIONS_POSTHOG_GEETORUS_E2E_EMAIL",
...
test("preflight rejects credential-bearing and non-HTTPS remote URLs", () => {
  for (const baseUrl of [
    ...
    assert.throws(
      () => preflightPosthogLive({ ...COMPLETE_ENV, GEETORUS_API_URL: baseUrl }),
...
test("preflight derives the current Geetorus origin and accepts an explicit target", () => {
  assert.equal(preflightPosthogLive(COMPLETE_ENV).baseUrl, "https://geetorus.example.test");
  assert.equal(
    preflightPosthogLive(COMPLETE_ENV, { baseUrl: "https://other-geetorus.example.test" }).baseUrl,
    "https://other-geetorus.example.test",
  );
  assert.throws(
    () => preflightPosthogLive({ ...COMPLETE_ENV, GEETORUS_API_URL: "" }),
...
test("live smoke arguments accept a target URL without another environment binding", () => {
  ...
  assert.deepEqual(
    parsePosthogLiveArguments(["https://geetorus.example.test"]),
    { baseUrl: "https://geetorus.example.test" },
  );
  assert.deepEqual(
    parsePosthogLiveArguments(["--base-url", "https://geetorus.example.test"]),
    { baseUrl: "https://geetorus.example.test" },
  );
  assert.throws(
...
test("project proof extraction retains only the expected id and name", () => {
  const result = {
    data: {
      content: [{ type: "text", text: JSON.stringify({ id: 483530, name: "Geetorus", token: "discard-me" }) }],
    },
  };
  assert.deepEqual(extractProjectSummary(result, "483530"), { id: "483530", name: "Geetorus" });
  assert.equal(extractProjectSummary(result, "42"), null);

  assert.deepEqual(
    parseSanitizedAgentProof(
      '{"projectId":"483530","projectName":"Geetorus","invocationId":"inv-123"}',
      "483530",
    ),
    { projectId: "483530", projectName: "Geetorus", invocationId: "inv-123" },
  );
  assert.equal(
    parseSanitizedAgentProof(
      'Done: {"projectId":"483530","projectName":"Geetorus","invocationId":"inv-123"}',
      "483530",
    ),
    null,
  );
  assert.equal(
    parseSanitizedAgentProof(
      '{"projectId":"483530","projectName":"Geetorus","invocationId":"inv-123","token":"unsafe"}',
      "483530",
    ),
    null,
  );
