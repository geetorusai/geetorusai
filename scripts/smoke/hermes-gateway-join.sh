...
Creates a Hermes gateway agent from an agent-only Geetorus invite, approves the
join request, claims the one-time Geetorus API key, and verifies the stored
...
Notes:
  HERMES_GATEWAY_API_BASE_URL is stored on the Geetorus adapter and must be
  reachable by the Geetorus server. HERMES_GATEWAY_PROBE_URL is only used by
  this operator shell to preflight /health, which is useful when Geetorus talks
  ...
  claimed Geetorus agent API key and is written chmod 600.
...
redact_text() {
  local text="$1"
  local secret
  for secret in "${HERMES_GATEWAY_API_KEY:-}" "${CLAIM_SECRET:-}" "${AGENT_API_KEY:-}" "${GEETORUS_AUTH_HEADER:-}" "${GEETORUS_COOKIE:-}" "${GEETORUS_API_KEY:-}"; do
    if [[ -n "$secret" ]]; then
...
Provide one of:
  GEETORUS_AUTH_HEADER="Bearer <board-token>"
  GEETORUS_COOKIE="<board-session-cookie>"
...
probe_hermes_gateway() {
  ...
  [[ -n "$HERMES_GATEWAY_API_KEY" ]] || fail "HERMES_GATEWAY_API_KEY or API_SERVER_KEY is required before any Geetorus state is mutated"
  ...
  if [[ "$code" != "200" ]]; then
    fail "Hermes gateway health probe failed before mutating Geetorus state: ${health_url} returned HTTP ${code}. Start Hermes with API_SERVER_ENABLED=true API_SERVER_KEY=<key> hermes gateway run --replace --accept-hooks, or set HERMES_GATEWAY_API_BASE_URL/HERMES_GATEWAY_API_KEY."
  fi
}

log "checking Geetorus health"
...
JOIN_PAYLOAD="$(jq -nc \
  ...
  --arg geetorusApiUrl "$GEETORUS_API_URL_FOR_HERMES" \
  ...
  | if $geetorusApiUrl != "" then .agentDefaultsPayload.geetorusApiUrl = $geetorusApiUrl else . end
...
if [[ -n "$GEETORUS_API_URL_FOR_HERMES" ]]; then
  STORED_GEETORUS_API_URL="$(jq -r '.adapterConfig.geetorusApiUrl // empty' <<<"$RESPONSE_BODY")"
  [[ "$STORED_GEETORUS_API_URL" == "$GEETORUS_API_URL_FOR_HERMES" || "$(strip_trailing_slash "$STORED_GEETORUS_API_URL")" == "$(strip_trailing_slash "$GEETORUS_API_URL_FOR_HERMES")" ]] \
    || fail "stored geetorusApiUrl mismatch"
fi
