...
function issueMutationHeaders(config, method, pathname) {
  ...
  return { "X-Geetorus-Run-Id": config.runId };
}
...
async function gotoWithVisibleMarker(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      // A credential-free Geetorus navigation is safe to repeat once. Do not
...
async function loginGeetorusBoard(page, context, config) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    const geetorusEmailInput = page.getByLabel(/^email$/i);
    const geetorusPasswordInput = page.getByLabel(/^password$/i);
    await expectVisible(geetorusPasswordInput, "A.geetorus-login", "password_field_missing");
    await geetorusEmailInput.fill(config.geetorusEmail);
    await geetorusPasswordInput.fill(config.geetorusPassword);
    const loginResponsePromise = page.waitForResponse((response) =>
      response.request().method() === "POST" && new URL(response.url()).pathname === "/api/auth/sign-in/email",
    );
    await page.getByRole("button", { name: /^sign in$/i }).click();
    const loginResponse = await loginResponsePromise;
    if (!loginResponse.ok()) fail("A.geetorus-login", `http_${loginResponse.status()}`);
  ...
  fail("A.geetorus-login", "session_cookie_missing");
}
...
async function completeNotionAuthorization(page, config, credential, connectionId) {
  const geetorusOrigin = new URL(config.baseUrl).origin;
  ...
  while (Date.now() < deadline) {
    ...
    if (current.origin === geetorusOrigin) {
...
async function safeScreenshot(page, outputPath, config, credential, checkpoint) {
  ...
  if (current.origin !== new URL(config.baseUrl).origin) fail(checkpoint, "screenshot_not_on_geetorus");
  ...
  if (bodyText.includes(config.geetorusPassword)
  ...
  await page.screenshot({
    ...
    animations: "disabled",
    mask: [
      page.getByText(config.geetorusEmail, { exact: false }),
...
async function runSmoke({ config, chromium }) {
  ...
  const outputDirectory = process.env.NOTION_EVIDENCE_DIR
    ...
    : path.join(process.env.GEETORUS_RUN_SCRATCH_DIR || process.cwd(), `notion-generic-live-${runKey}`);
  ...
  let activeCheckpoint = "A.geetorus-login";
  ...
  try {
    ...
    await loginGeetorusBoard(page, context, config);
    ...
    // Fetch only after URL, health, Geetorus login, company, agent, and binding
    ...
    const authorizationRequestPromise = page.waitForRequest((request) => {
      ...
      try {
        ...
        return target.protocol === "https:" && target.origin !== geetorusOrigin;
      } catch {
        return false;
      }
    }, { timeout: 90_000 });
    ...
    assertNoCredentialMaterial(connection, [config.geetorusPassword, credential.username, credential.password], "C.connection-detail");
    summary.connection = {
      id: connectionId,
    ...
    const proofIssue = await createAndWaitForProofIssue(context.request, config, {
      ...
      description: [
        ...
        "Post exactly one JSON object with keys `workspaceId`, `workspaceName`, and `invocationId` (the Geetorus invocation ID), then mark this issue done.",
      ...
      acceptanceCriteria: [
        ...
        "Only sanitized workspace ID/name and Geetorus invocation ID are reported.",
