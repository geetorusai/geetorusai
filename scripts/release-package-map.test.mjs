...
test("release package list publishes the installable channel entrypoint last", () => {
  ...
  assert.equal(enabledPackages.at(-1)?.name, "geetorusai");
  assert.ok(enabledPackages.slice(0, -1).some((pkg) => pkg.name === "@geetorusai/server"));
});
...
test("Hermes release surface publishes the unified built-in package and keeps gateway as a shim", () => {
  const packages = buildReleasePackagePlan();
  const hermes = packages.find((pkg) => pkg.name === "@geetorusai/hermes-geetorus-adapter");
  const gatewayShim = packages.find((pkg) => pkg.name === "@geetorusai/adapter-hermes-gateway");
...
test("guard flags a publishFromCi:true package depending on a publishFromCi:false package", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@geetorusai/server", {
      publishFromCi: true,
      dependencies: { "@geetorusai/skills-catalog": "workspace:*" },
    }),
    pkg("@geetorusai/skills-catalog", { publishFromCi: false }),
  ]);

  assert.equal(problems.length, 1);
  assert.match(problems[0], /@geetorusai\/server/);
  assert.match(problems[0], /@geetorusai\/skills-catalog/);
});
...
test("guard inspects optional and peer dependency sections too", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@geetorusai/server", {
      publishFromCi: true,
      optionalDependencies: { "@geetorusai/opt": "workspace:^" },
      peerDependencies: { "@geetorusai/peer": "workspace:*" },
    }),
    pkg("@geetorusai/opt", { publishFromCi: false }),
    pkg("@geetorusai/peer", { publishFromCi: false }),
  ]);

  assert.equal(problems.length, 2);
});

test("guard treats a workspace dep on an unknown @geetorusai package as unpublishable", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@geetorusai/server", {
      publishFromCi: true,
      dependencies: { "@geetorusai/private-internal": "workspace:*" },
    }),
  ]);
...
test("guard allows true->true workspace edges", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@geetorusai/server", {
      publishFromCi: true,
      dependencies: { "@geetorusai/shared": "workspace:*" },
    }),
    pkg("@geetorusai/shared", { publishFromCi: true }),
  ]);
...
test("guard ignores non-workspace specs, non-internal deps, and edges from off-train packages", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@geetorusai/server", {
      publishFromCi: true,
      dependencies: {
        "@geetorusai/pinned": "0.3.1",
        zod: "^3.0.0",
      },
    }),
    pkg("@geetorusai/pinned", { publishFromCi: false }),
    pkg("@geetorusai/offtrain", {
      publishFromCi: false,
      dependencies: { "@geetorusai/also-off": "workspace:*" },
    }),
    pkg("@geetorusai/also-off", { publishFromCi: false }),
  ]);
