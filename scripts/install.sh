...
usage() {
  cat <<'EOF'
Install Geetorus on macOS, Linux, or WSL2.

Usage:
  curl -fsSLO https://geetorus.ing/install.sh
  bash install.sh [options]
  curl -fsSL https://geetorus.ing/install.sh | bash -s -- --no-prompt [options]
...
Options:
  ...
  --install-service        Install the per-user Geetorus service
  --dry-run                Print the install plan without changing files
  --verbose                Enable verbose installer output
  -h, --help               Show this help

Every option also has a GEETORUS_INSTALL_* environment equivalent, for example
GEETORUS_INSTALL_VERSION=2026.722.0 and GEETORUS_INSTALL_NO_PROMPT=1.

To install from a git branch, tag, or commit, use the Geetorus CLI directly:
npx geetorusai install --ref <ref>
EOF
}
...
CANARY="$(parse_bool GEETORUS_INSTALL_CANARY "${GEETORUS_INSTALL_CANARY:-}")"
VERSION="${GEETORUS_INSTALL_VERSION:-}"
REF="${GEETORUS_INSTALL_REF:-}"
REPO="${GEETORUS_INSTALL_REPO:-}"
NO_ONBOARD="$(parse_bool GEETORUS_INSTALL_NO_ONBOARD "${GEETORUS_INSTALL_NO_ONBOARD:-}")"
NO_PROMPT="$(parse_bool GEETORUS_INSTALL_NO_PROMPT "${GEETORUS_INSTALL_NO_PROMPT:-}")"
INSTALL_SERVICE="$(parse_bool GEETORUS_INSTALL_INSTALL_SERVICE "${GEETORUS_INSTALL_INSTALL_SERVICE:-}")"
DRY_RUN="$(parse_bool GEETORUS_INSTALL_DRY_RUN "${GEETORUS_INSTALL_DRY_RUN:-}")"
VERBOSE="$(parse_bool GEETORUS_INSTALL_VERBOSE "${GEETORUS_INSTALL_VERBOSE:-}")"
...
if [ -n "$REF" ] || [ -n "$REPO" ]; then
  fail "git-ref installs are not supported by install.sh; run 'npx geetorusai install --ref <ref>' instead"
fi
...
confirm_command() {
  ...
  local answer
  printf '[geetorus] Run this command? [y/N] ' >/dev/tty
...
ensure_temp_dir() {
  if [ -z "$TEMP_DIR" ]; then
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-install.XXXXXX")"
  fi
}
...
PACKAGE_SPEC="$GEETORUS_PACKAGE@latest"
if [ "$CANARY" = "1" ]; then
  PACKAGE_SPEC="$GEETORUS_PACKAGE@canary"
elif [ -n "$VERSION" ]; then
  PACKAGE_SPEC="$GEETORUS_PACKAGE@$VERSION"
fi
...
log "Delegating to the Geetorus CLI"
if [ "$DRY_RUN" = "1" ]; then
...
if [ "$INSTALL_SERVICE" = "1" ]; then
  log "Installing the Geetorus service"
...
if [ "$NO_ONBOARD" = "0" ] && [ -t 0 ] && [ -t 1 ]; then
  if command -v geetorusai >/dev/null 2>&1; then
    exec geetorusai onboard
  elif [ -x "${HOME:-}/.local/bin/geetorusai" ]; then
    exec "${HOME}/.local/bin/geetorusai" onboard
  else
    fail "Geetorus was installed, but 'geetorusai' is not available on PATH. Open a new shell and run 'geetorusai onboard'."
  fi
fi

if [ "$NO_ONBOARD" = "0" ]; then
  log "Installation complete. Next: geetorusai onboard"
else
