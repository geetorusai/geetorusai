...
const DEFAULT_QUERY = "\"Co-Authored-By: Geetorus <noreply@geetorus.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/geetorus-commit-metrics-cache.json");
...
const GEETORUS_EMAIL = "noreply@geetorus.ing";
const GEETORUS_NAME = "geetorus";
...
function printHelp() {
  console.log(`Usage: tsx scripts/geetorus-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === GEETORUS_EMAIL) || (displayName && displayName.toLowerCase() === GEETORUS_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Geetorus commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "geetorus-commit-metrics",
