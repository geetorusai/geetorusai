...
const env = {
  ...process.env,
  GEETORUS_UI_DEV_MIDDLEWARE: "true",
};
...
if (tailscaleAuth) {
  ...
  env.GEETORUS_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[geetorus] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@geetorusai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
...
async function maybePreflightMigrations(options = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[geetorus] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
