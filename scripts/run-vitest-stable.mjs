...
const nonServerProjects = [
  "@geetorusai/shared",
  "@geetorusai/skills-catalog",
  "@geetorusai/db",
  "@geetorusai/adapter-utils",
  "@geetorusai/adapter-claude-local",
  "@geetorusai/adapter-codex-local",
  "@geetorusai/adapter-grok-local",
  "@geetorusai/adapter-openclaw-gateway",
  "@geetorusai/adapter-opencode-local",
...
const generalWorkspacesAProjects = ["@geetorusai/ui", "geetorusai"];
...
function runVitest(args, label, testShard = null) {
  ...
  const env = {
    ...process.env,
    NODE_ENV: "test",
    GEETORUS_HOME: path.join(testRoot, "h"),
    // Config discovery otherwise prefers the checkout's .geetorus/config.json
    // over GEETORUS_HOME, importing preview scheduling policy into unit tests.
    GEETORUS_CONFIG: path.join(testRoot, "h", "config.json"),
    GEETORUS_INSTANCE_ID: `vt-${process.pid}-${invocationIndex}`,
...
function runGeneralGroup(routeTests, groupName, shardIndex = null, shardCount = null) {
  if (groupName === generalChatGroupName) {
    runVitest(["--project", "@geetorusai/server", ...serializedServerVitestArgs, chatSuite],
