#!/usr/bin/env bash
#
# Kill all local Geetorus workspace runtime service processes.
#
# This targets managed workspace services such as preview/dev commands started
# from project or execution workspaces. Use scripts/kill-dev.sh for Geetorus
# server processes.
#
# Usage:
...
geetorus_home="$(expand_home "${GEETORUS_HOME:-$HOME/.geetorus}")"
geetorus_instance_id="${GEETORUS_INSTANCE_ID:-default}"
append_runtime_dir "$geetorus_home/instances/$geetorus_instance_id/runtime-services"

if [[ "${GEETORUS_KILL_WORKSPACES_ONLY_CURRENT:-}" != "1" ]]; then
  for dir in \
    "$HOME"/.geetorus/instances/*/runtime-services \
    "$HOME"/.geetorus-worktrees/instances/*/runtime-services \
    "$REPO_ROOT"/.geetorus/instances/*/runtime-services \
    "$REPO_ROOT"/.geetorus/runtime-services/instances/*/runtime-services
  do
    append_runtime_dir "$dir"
  done

  for sibling_root in "$REPO_PARENT"/geetorus*; do
    [[ -d "$sibling_root" ]] || continue
    for dir in \
      "$sibling_root"/.geetorus/instances/*/runtime-services \
      "$sibling_root"/.geetorus/runtime-services/instances/*/runtime-services
    do
      append_runtime_dir "$dir"
    done
  done
...
if [[ ${#active_files[@]} -eq 0 && ${#stale_files[@]} -eq 0 ]]; then
  echo "No Geetorus workspace runtime services found."
  exit 0
fi

if [[ ${#active_files[@]} -gt 0 ]]; then
  echo "Found ${#active_files[@]} Geetorus workspace runtime service record(s):"
  echo ""
  printf '%s\n' "${display_lines[@]}"
  echo ""
fi
