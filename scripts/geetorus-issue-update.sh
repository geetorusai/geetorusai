...
usage() {
  cat <<'EOF'
Usage:
  scripts/geetorus-issue-update.sh [--issue-id ID] [--status STATUS] [--comment TEXT] [--dry-run]
...
Examples:
  scripts/geetorus-issue-update.sh --issue-id "$GEETORUS_TASK_ID" --status in_progress <<'MD'
  ...
  scripts/geetorus-issue-update.sh --issue-id "$GEETORUS_TASK_ID" --status done --dry-run <<'MD'
  Done
...
if [[ -z "$issue_id" ]]; then
  printf 'Missing issue id. Pass --issue-id or set GEETORUS_TASK_ID.\n' >&2
  exit 1
fi
...
while :; do
  ...
  response="$(
    curl -sS -m 30 -X PATCH \
      "$GEETORUS_API_URL/api/issues/$issue_id" \
