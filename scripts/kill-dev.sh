#!/usr/bin/env bash
#
# Kill all local Geetorus dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all geetorus dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/geetorus(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.geetorus/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.geetorus/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.geetorus/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/geetorus*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.geetorus/instances/*/db/postmaster.pid
    "$sibling_root"/.geetorus/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No Geetorus dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Geetorus dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Geetorus node processes..."
