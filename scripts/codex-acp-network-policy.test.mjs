...
const require = createRequire(new URL("../packages/geetorus-runner/package.json", import.meta.url));
const source = readFileSync(require.resolve("@agentclientprotocol/codex-acp"), "utf8");
const helperStart = source.indexOf("function geetorusSandboxPolicy(");
...
test("ACP forwards workspace networking to the actual Codex turn without changing confinement", async () => {
  ...
  const result = await turnPolicy(policy, { GEETORUS_CODEX_ACP_NETWORK_ACCESS: "true" });
...
test("ACP preserves operator and execution-target network denials", async () => {
  ...
  for (const env of [
    { GEETORUS_CODEX_ACP_NETWORK_ACCESS: "false" },
    { GEETORUS_CODEX_ACP_NETWORK_ACCESS: "true", GEETORUS_RUNNER_NETWORK_ACCESS: "disabled" },
  ]) {
...
test("ACP leaves other sandbox modes and callers without a valid override unchanged", async () => {
  for (const policy of [{ type: "readOnly", networkAccess: false }, { type: "dangerFullAccess" }]) {
    assert.equal((await turnPolicy(policy, { GEETORUS_CODEX_ACP_NETWORK_ACCESS: "true" })).sandboxPolicy, policy);
  }
  const policy = { type: "workspaceWrite", networkAccess: false };
  for (const value of [undefined, "", "1", "invalid"]) {
    assert.equal((await turnPolicy(policy, { GEETORUS_CODEX_ACP_NETWORK_ACCESS: value })).sandboxPolicy, policy);
  }
});
