...
const lockDir = path.join(rootDir, "node_modules", ".cache", "geetorus-plugin-build-deps.lock");
...
const buildTargets = [
  {
    ...
    completion: path.join(rootDir, "packages/shared/dist/.geetorus-build-complete"),
    sourceDir: path.join(rootDir, "packages/shared/src"),
    tsconfig: path.join(rootDir, "packages/shared/tsconfig.json"),
    dependencies: [],
  },
  {
    name: "@geetorusai/plugin-sdk",
    output: path.join(rootDir, "packages/plugins/sdk/dist/index.js"),
    completion: path.join(rootDir, "packages/plugins/sdk/dist/.geetorus-build-complete"),
...
function recoverAbandonedLock() {
  try {
    ...
    console.log("[geetorus] Recovered abandoned workspace build lock.");
  } catch (error) {
...
async function acquireLock() {
  ...
  try {
    while (!stoppingSignal) {
      ...
      if (!reportedWait) {
        console.log(`[geetorus] Waiting for another workspace build (${lockDir})...`);
        reportedWait = true;
      }
...
async function build(target) {
  console.log(`[geetorus] Building ${target.name}...`);
