...
const SCOPE_RE = /^@geetorusai\/[a-z0-9][a-z0-9._-]*$/;
...
function usage() {
  process.stderr.write(
    [
      ...
      "",
      "Examples:",
      "  node scripts/bootstrap-npm-package.mjs @geetorusai/new-package",
      "  node scripts/bootstrap-npm-package.mjs @geetorusai/new-package --publish",
      "",
    ].join("\n"),
  );
}
  ...
  if (!SCOPE_RE.test(packageName)) {
    throw new Error(
      `refusing to publish a placeholder for ${JSON.stringify(packageName)}: ` +
        "the name must be a lowercase package inside the @geetorusai scope " +
...
export function buildPlaceholderFiles(packageName) {
  const deprecationNote =
    ...
    "for Geetorus's release pipeline. It contains no functionality; the first real release " +
    "supersedes it. See https://github.com/geetorusai/geetorus";

  const packageJson = {
    name: packageName,
    version: PLACEHOLDER_VERSION,
    description:
      "Placeholder publish reserving this name for Geetorus's release pipeline. Do not install this version.",
    license: "MIT",
    main: "index.js",
    files: ["index.js"],
    repository: {
      type: "git",
      url: "git+https://github.com/geetorusai/geetorus.git",
    },
    homepage: "https://github.com/geetorusai/geetorus",
    publishConfig: {
      access: "public",
    },
  };
  ...
  const readme = [
    ...
    "Geetorus's release-bootstrap CI gate can pass before the package's first real",
    "release ships from CI. It intentionally contains no functionality.",
    "",
    "Real versions are published by the release workflow of",
    "[geetorusai/geetorus](https://github.com/geetorusai/geetorus).",
    "",
  ].join("\n");
  ...
  if (/\bE401\b|401 Unauthorized/i.test(output)) {
    throw new Error(
      [
        ...
        "Run `npm logout --registry=https://registry.npmjs.org/` and then `npm login` or `npm adduser` on this maintainer machine with an npm account that can publish to the @geetorusai scope, then rerun with --publish.",
...
async function stageAndPublish(packageName, { publish }) {
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "geetorus-npm-placeholder-"));
