...
async function readPreviousServerInfo() {
  const apiBase = normalizeApiBase(process.env.GEETORUS_API_URL);
  if (!apiBase) return { version: null, identity: null };
  try {
    const apiKey = process.env.GEETORUS_API_KEY?.trim();
...
async function readPreflightActiveRunIds() {
  ...
  const dbUrl = process.env.DATABASE_URL?.trim()
    || config.databaseUrl
    || `postgres://geetorus:geetorus@127.0.0.1:${config.embeddedPostgresPort}/geetorus`;
