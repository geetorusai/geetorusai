...
probe_dir="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-runner-cache.XXXXXX")"
...
RUN ./runner/target/release/geetorus-runnerd --build-metadata > /metadata.json
...
p=Path('packages/geetorus-runner/runner/crates/runner-core/src/bin/geetorus-runnerd.rs')
s=p.read_text(); needle='geetorus-runner/runnerd-build-metadata/v1'
...
assert before['schema']=='geetorus-runner/runnerd-build-metadata/v1'
...
assert 'Compiling geetorus-runner-core' in log
print('PASS: fresh builder imported compiled dependencies; real binary changed.')
p=Path('packages/geetorus-runner/runner/Cargo.toml')
