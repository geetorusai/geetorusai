#!/usr/bin/env node
/**
 * Screenshot utility for Geetorus UI.
 *
 * Reads the board token from ~/.geetorus/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/geetorus-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".geetorus/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.geetorus/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Geetorus origin only
