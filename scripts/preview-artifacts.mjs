...
export function previewManifest(pkg, sha) {
  if (!["@geetorusai/shared", "@geetorusai/db"].includes(pkg.name)) throw new Error("Unexpected preview package.");
  ...
  result.gitHead = sha;
  result.geetorusPreviewCommit = sha;
  if (pkg.name === "@geetorusai/db") result.dependencies = { ...result.dependencies, "@geetorusai/shared": version };
  return result;
}

export function assertMetadata(pkg, name, sha) {
  if (pkg?.publishConfig !== undefined || pkg?.name !== name || pkg.version !== versionFor(sha) || pkg.gitHead !== sha || pkg.geetorusPreviewCommit !== sha ||
      (name === "@geetorusai/db" && pkg.dependencies?.["@geetorusai/shared"] !== versionFor(sha))) {
...
export async function planArtifacts(sha, { migrator = false, image = true, fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    packages: migrator && !(await packageExists("@geetorusai/shared", sha, fetchImpl) && await packageExists("@geetorusai/db", sha, fetchImpl)),
  };
}

export async function imageExists(sha, fetchImpl = fetch) {
  versionFor(sha);
  const tokenRes = await fetchImpl("https://ghcr.io/token?service=ghcr.io&scope=repository:geetorusai/geetorus:pull", { signal: AbortSignal.timeout(30_000) });
  ...
  const base = "https://ghcr.io/v2/geetorusai/geetorus";
...
export async function publishImage(file, sha, { exec = execFileSync, fetchImpl = fetch } = {}) {
  versionFor(sha);
  const image = `ghcr.io/geetorusai/geetorus:sha-${sha}-cloud`;
...
export function packPreview(source, output, sha, { exec = execFileSync } = {}) {
  ...
  for (const short of ["shared", "db"]) {
    exec("pnpm", ["--filter", `@geetorusai/${short}`, "build"], { cwd: source, stdio: "inherit" });
    ...
    assertMetadata(tarManifest(readFileSync(path.join(output, `${short}.tgz`))), `@geetorusai/${short}`, sha);
  }
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    if (command === "plan" || command === "plan-migrator") {
      ...
      if (process.env.PREVIEW_MIGRATOR === "true" && !(await packageExists("@geetorusai/shared", sha) && await packageExists("@geetorusai/db", sha))) throw new Error("Preview packages are still missing.");
