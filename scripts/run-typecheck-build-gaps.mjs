...
function checkServerRuntimeAssets() {
  ...
  if (missingAssets.length > 0) {
    ...
    fail(
      `Missing server runtime asset(s) in dist:\n${missingList}\nRun pnpm --filter @geetorusai/server build and ensure source runtime asset trees are copied into dist.`,
    );
  }
...
if (buildGapPackages.length > 0) {
  run("pnpm", ["--filter", "@geetorusai/plugin-sdk", "ensure-build-deps"]);
