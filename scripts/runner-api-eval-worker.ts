...
/** JSONL worker for the companion geetorus-evals API suite. Never selects cases or retries. */
...
import { createRunnerdCodexTransport, defaultCapabilityRunnerdBinary } from "../packages/geetorus-runner/src/live/runnerd-codex-transport.js";
import { createSkilllessCodexThreadConfig } from "../packages/geetorus-runner/src/drivers/codex/codex-app-server-driver.js";
import { AttemptJournal } from "../packages/geetorus-runner/src/evals/attempt-journal.js";
import { estimateModelCostNanodollars } from "../packages/geetorus-runner/src/evals/model-pricing.js";
...
process.env.GEETORUS_AGENT_JWT_SECRET = randomUUID() + randomUUID();
  ...
  for await (const line of createInterface({ input: process.stdin })) {
    ...
    try {
      if (request.calls) {
        ...
        if (isOpenRouter) {
          providerVersion = execFileSync(resolve("packages/geetorus-runner/node_modules/opencode-ai/bin/opencode.exe"), ["--version"], { encoding: "utf8" }).trim();
        ...
        bundle = createRunnerdCodexTransport({
          ...
          environment: { ...providerEnvironment, GEETORUS_PROVIDER_TRACE_PATH: join(directory, "provider-trace.jsonl"), GEETORUS_PROVIDER_TRACE_MAX_BYTES: String(32 * 1024 * 1024) },
    ...
    const artifact = {
      schema: "geetorus-runner/eval-session-artifact/v1", attemptId: request.attemptId,
