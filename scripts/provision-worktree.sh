...
base_cwd="${GEETORUS_WORKSPACE_BASE_CWD:?GEETORUS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${GEETORUS_WORKSPACE_CWD:?GEETORUS_WORKSPACE_CWD is required}"
geetorus_home="${GEETORUS_HOME:-$HOME/.geetorus}"
geetorus_instance_id="${GEETORUS_INSTANCE_ID:-default}"
geetorus_dir="$worktree_cwd/.geetorus"
worktree_config_path="$geetorus_dir/config.json"
worktree_env_path="$geetorus_dir/.env"
seed_manifest_path="$geetorus_dir/seed-manifest.json"
seed_pending_marker_path="$geetorus_dir/seed-pending"
seed_complete_marker_path="$geetorus_dir/seed-complete"
worktree_name="${GEETORUS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
if [[ -L "$canonical_base_cwd/.geetorus" && ! -d "$canonical_base_cwd/.geetorus" ]]; then
  ...
  echo "Registered base project workspace .geetorus is a broken symlink: $canonical_base_cwd/.geetorus" >&2
  exit 1
fi
source_config_path="$canonical_base_cwd/.geetorus/config.json"
if [[ ! -e "$source_config_path" && ! -L "$source_config_path" ]]; then
  ...
  source_config_path="${GEETORUS_CONFIG:-$geetorus_home/instances/$geetorus_instance_id/config.json}"
fi
if [[ ! -f "$source_config_path" || -L "$source_config_path" ]]; then
  echo "Registered Geetorus seed source config is missing or is not a canonical file: $source_config_path" >&2
  exit 1
fi
canonical_source_dir="$(cd "$(dirname "$source_config_path")" && pwd -P)"
if [[ "$canonical_source_dir/config.json" != "$source_config_path" ]]; then
  echo "Registered Geetorus seed source config uses a symlink alias: $source_config_path" >&2
  exit 1
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$geetorus_dir"
...
repair_base_workspace_install() {
  ...
  if command -v flock >/dev/null 2>&1 && [[ -d "$repair_lock_dir" ]]; then
    ...
    (
      cd "$base_cwd" || exit 1
      exec 9>"$repair_lock_dir/geetorus-provision-repair.lock"
      flock 9
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm geetorusai --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        pnpm geetorusai worktree init --force --no-seed --seed-mode minimal --name "$worktree_name" --instance "$worktree_instance_id" --from-config "$source_config_path"
    )
    return
  fi
  ...
  if command -v geetorusai >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        geetorusai worktree init --force --no-seed --seed-mode minimal --name "$worktree_name" --instance "$worktree_instance_id" --from-config "$source_config_path"
    )
    return
  fi
...
const envConfigPath = expandHomePrefix(env.GEETORUS_CONFIG);
if (envConfigPath && path.resolve(envConfigPath) !== configPath) {
  fail(`existing worktree env points at ${envConfigPath}, not ${configPath}`);
}

const homeDir = expandHomePrefix(env.GEETORUS_HOME);
const instanceId = env.GEETORUS_INSTANCE_ID;
const expectedInstanceId = process.env.WORKTREE_INSTANCE_ID;
if (!homeDir || !instanceId) {
  fail("existing worktree env is missing GEETORUS_HOME or GEETORUS_INSTANCE_ID");
}
...
console.error(`Reconciled isolated Geetorus worktree deployment mode from ${sourceConfigPath}: ${deploymentMode}/${exposure}`);
EOF
}
...
if (fs.existsSync(sourceEnvPath)) {
  const match = fs.readFileSync(sourceEnvPath, "utf8").match(/^\s*(?:export\s+)?GEETORUS_INSTANCE_ID\s*=\s*["']?([^\s"'#]+)["']?/m);
...
write_fallback_worktree_config() {
  ...
  GEETORUS_DIR="$geetorus_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  WORKTREE_INSTANCE_ID="$worktree_instance_id" \
  GEETORUS_WORKTREES_DIR="${GEETORUS_WORKTREES_DIR:-}" \
  node <<'EOF'
const fs = require("node:fs");
const os = require("node:os");
const path = require("node:path");
...
async function main() {
  ...
  const geetorusDir = process.env.GEETORUS_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.GEETORUS_WORKTREES_DIR) ?? "~/.geetorus-worktrees"));
  ...
  const configPath = path.resolve(geetorusDir, "config.json");
  const envPath = path.resolve(geetorusDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "geetorus",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.GEETORUS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.GEETORUS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.GEETORUS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "GEETORUS_HOME=" + JSON.stringify(worktreeHome),
    "GEETORUS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "GEETORUS_CONFIG=" + JSON.stringify(configPath),
    "GEETORUS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "GEETORUS_IN_WORKTREE=true",
    "GEETORUS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  // Secrets that must be carried over from the source instance so the worktree's
  // dev server behaves like the real one. GEETORUS_TOOL_ACTION_SIGNING_SECRET is
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]] && existing_worktree_config_is_usable; then
  echo "Reusing existing isolated Geetorus worktree config at $worktree_config_path" >&2
else
  if [[ -e "$worktree_config_path" || -e "$worktree_env_path" ]]; then
    echo "Existing isolated Geetorus worktree config is stale for this host; regenerating." >&2
  fi
  if geetorusai_command_available; then
    if run_isolated_worktree_init; then
      ...
      if [[ "$init_exit_code" -eq 127 ]]; then
        ...
        echo "No usable geetorusai CLI found; writing isolated fallback config without DB seeding." >&2
        write_fallback_worktree_config
      else
        # A CLI that ran and failed signals a real problem; do not paper over
        # it with an unseeded fallback config.
        echo "geetorusai worktree init failed (exit $init_exit_code); failing provisioning instead of writing an unseeded fallback config." >&2
        exit "$init_exit_code"
      fi
    fi
  else
    echo "geetorusai worktree init unavailable; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
...
const ignoredDirs = new Set([".git", ".geetorus", "node_modules", "dist", "storybook-static"]);
const files = [];
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  needs_install=0
  install_fingerprint_path="$geetorus_dir/pnpm-install-fingerprint"
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".geetorus-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
