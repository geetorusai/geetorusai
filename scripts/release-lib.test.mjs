...
function runPublishHelper({
  ...
  const fixtureDir = mkdtempSync(join(tmpdir(), "geetorus-release-lib-"));
...
`,
  ...
  const packageInfo = (
    visibilityPackages ?? ["@geetorusai/example"]
  )
...
wait_for_npm_package_versions "$VERIFY_ATTEMPTS" "$VERIFY_DELAY_SECONDS" "$PACKAGE_INFO"`
    : waitForRegistry
      ? `publish_package_to_npm ${distTag} @geetorusai/example 1.2.3 ${publishTool}
PACKAGE_INFO="$(printf '${packageInfo}')"
wait_for_npm_package_versions "$VERIFY_ATTEMPTS" "$VERIFY_DELAY_SECONDS" "$PACKAGE_INFO"`
      : `publish_package_to_npm ${distTag} @geetorusai/example 1.2.3 ${publishTool}`
}
...
test("publish_package_to_npm retries bundled directory tlog failures without provenance", () => {
  ...
  assert.equal(result.status, 0);
  assert.match(result.calls, /^npm view @geetorusai\/example@1\.2\.3 version$/m);
  assert.match(
    result.calls,
...
test("wait_for_npm_package_versions blocks the release and names the straggler", () => {
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.calls, /^npm view @geetorusai\/example@1\.2\.3 version$/m);
  assert.match(result.output, /did not become registry-visible: @geetorusai\/example@1\.2\.3/);
});
...
test("wait_for_npm_package_versions polls every package concurrently", () => {
  ...
  const result = runPublishHelper({
    pnpmMode: "success",
    visibilityPackages: ["@geetorusai/alpha", "@geetorusai/beta"],
    verifyAttempts: 50,
    verifyDelaySeconds: 0.2,
  });

  assert.equal(result.status, 0, result.output);
  assert.match(result.output, /@geetorusai\/alpha@1\.2\.3 is registry-visible/);
  assert.match(result.output, /@geetorusai\/beta@1\.2\.3 is registry-visible/);
});
