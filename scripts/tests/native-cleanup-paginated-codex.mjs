...
// GEETORUS_TEST_CODEX_BINARY may select the exact installed Codex 0.153.4 binary.
...
const codexBinary = process.env.GEETORUS_TEST_CODEX_BINARY ?? "codex";
if (
...
const home = await mkdtemp(join(tmpdir(), "geetorus-paginated-probe-"));
const methods = [];
async function withServer(home, callback) {
  ...
  try {
    await rpc("initialize", {
      clientInfo: { name: "geetorus-fixture", version: "1" },
