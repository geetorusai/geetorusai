...
import {
  ...
  spawnRunner,
} from "../../packages/geetorus-runner/src/control-plane/durable-prp-control-plane.ts";
import {
  createRunnerdCodexAppServerArgs,
  defaultCapabilityRunnerdBinary,
} from "../../packages/geetorus-runner/src/live/runnerd-codex-transport.ts";
...
if (args.length === 0 || (args.length === 1 && args[0] === "--help")) {
  ...
  try {
    ...
    fixture = await realpath(
      await mkdtemp(join(tmpdir(), "geetorus-real-startup-")),
    );
    ...
    environment.GEETORUS_PROVIDER_TRACE_PATH = trace;
    environment.GEETORUS_PROVIDER_TRACE_MAX_BYTES = "1048576";
    const identity = {
    ...
    for (const fact of facts) {
      assert.equal(fact.schema, "geetorus.provider_startup.v1");
