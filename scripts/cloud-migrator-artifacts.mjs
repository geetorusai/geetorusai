...
export const artifactBucket = "geetorusai-runner-e2e-history-078455283791-us-east-1";
...
export function assertLockfile(lock, manifest) {
  ...
  if (lock?.lockfileVersion !== 3 || !lock.packages || Array.isArray(lock.packages) ||
      JSON.stringify(lock.packages[""]?.dependencies) !== JSON.stringify({ "@geetorusai/db": version })) throw new Error("Invalid migrator lockfile root.");
  for (const name of names) {
    const pin = lock.packages[`node_modules/@geetorusai/${name}`];
  ...
  if (lock.packages["node_modules/@geetorusai/db"].dependencies?.["@geetorusai/shared"] !== version) throw new Error("Migrator shared dependency mismatch.");
  for (const [key, entry] of Object.entries(lock.packages)) {
    if (key === "") continue;
    if (!entry || typeof entry !== "object" || entry.link) throw new Error("Invalid migrator lockfile entry.");
    if (/(?:^|\/)node_modules\/@geetorusai\/[^/]+$/.test(key) && !names.some((name) => key === `node_modules/@geetorusai/${name}`)) throw new Error("Unexpected internal migrator dependency.");
    if (entry.inBundle === true) {
      if (!key.startsWith("node_modules/@geetorusai/db/node_modules/")) throw new Error("Unexpected bundled dependency.");
      continue;
    }
    if (!/^sha512-[A-Za-z0-9+/]{86}==$/.test(entry.integrity ?? "")) throw new Error("Migrator dependency has no strong integrity pin.");
    if (names.some((name) => key === `node_modules/@geetorusai/${name}`)) continue;
...
export function buildBundle(directory, sha, { exec = execFileSync } = {}) {
  ...
  for (const name of names) {
    ...
    assertMetadata(tarManifest(bytes), `@geetorusai/${name}`, sha);
  ...
  try {
    ...
    const root = { name: "geetorus-migrator-install-root", version: "0.0.0", private: true,
      dependencies: { "@geetorusai/db": "file:db.tgz", "@geetorusai/shared": "file:shared.tgz" } };
    ...
    lock.packages[""].dependencies = { "@geetorusai/db": versionFor(sha) };
    for (const name of names) lock.packages[`node_modules/@geetorusai/${name}`].resolved = packages[name].url;
...
export function verifyInstall(directory, sha, { exec = execFileSync } = {}) {
  ...
  try {
    ...
    for (const name of names) {
      ...
      lock.packages[`node_modules/@geetorusai/${name}`].resolved = `file:${name}.tgz`;
    }
    writeFileSync(path.join(scratch, "package.json"), JSON.stringify({ name: "geetorus-migrator-install-root", version: "0.0.0", private: true,
      dependencies: { "@geetorusai/db": manifest.packageVersion } }));
    ...
    for (const name of names) assertMetadata(JSON.parse(readFileSync(path.join(scratch, "node_modules", "@geetorusai", name, "package.json"), "utf8")), `@geetorusai/${name}`, sha);
    exec(process.execPath, ["--input-type=module", "--eval", "await import('@geetorusai/db'); await import('@geetorusai/shared');"], { cwd: scratch, stdio: "inherit", timeout: 30_000 });
  } finally { rmSync(scratch, { recursive: true, force: true }); }
}
