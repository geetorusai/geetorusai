...
DATA_DIR="${DATA_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/geetorus-service-smoke.XXXXXX")}"
...
SERVICE_NAME="geetorusai.service"
SHIM_PATH="${GEETORUS_SHIM_PATH:-$HOME/.local/bin/geetorusai}"
...
echo "==> Onboarding geetorusai@$GEETORUSAI_VERSION with --install-service"
echo "    Data dir: $DATA_DIR"
if ! timeout "$ONBOARD_TIMEOUT_SECONDS" \
  npx --yes "geetorusai@${GEETORUSAI_VERSION}" onboard --yes --install-service --data-dir "$DATA_DIR"; then
  diagnostics
