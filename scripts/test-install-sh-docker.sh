...
RESULTS_DIR="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-install-sh.XXXXXX")"
...
run_with_node() {
  ...
  shift
  docker run --rm \
    -v "$REPO_ROOT/scripts:/geetorus-scripts:ro" \
    -v "$RESULTS_DIR:/results" \
    -e "GEETORUS_INSTALL_TEST_LOG=/results/$name.args" \
    -e PATH="/geetorus-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
...
echo "==> existing Node"
run_with_node with-node bash /geetorus-scripts/install.sh --no-prompt --no-onboard
assert_line "$RESULTS_DIR/with-node.args" "geetorusai@latest"
...
docker run --rm \
  ...
  -e npm_config_registry=http://attacker-registry.invalid \
  -e GEETORUS_INSTALL_TEST_LOG=/results/hostile.args \
  -e PATH="/geetorus-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  node:24-bookworm-slim \
  bash /geetorus-scripts/install.sh --no-prompt --no-onboard
...
echo "==> --ref master"
if run_with_node ref-master bash /geetorus-scripts/install.sh --ref master --no-onboard; then
...
if run_with_node piped-rejected bash -c 'cat /geetorus-scripts/install.sh | bash -s -- --no-onboard'; then
  echo "Expected piped install without --no-prompt to fail" >&2
  exit 1
fi

echo "==> piped --no-prompt"
run_with_node piped bash -c 'cat /geetorus-scripts/install.sh | bash -s -- --no-prompt --no-onboard'
...
if docker run --rm \
  ...
  ubuntu:24.04 \
  bash -c 'cat /geetorus-scripts/install.sh | bash -s -- --no-prompt --no-onboard' \
  >"$RESULTS_DIR/piped-no-node.out" 2>&1; then
  echo "Expected piped install without Node.js to fail before privileged bootstrap" >&2
  exit 1
fi
assert_line "$RESULTS_DIR/piped-no-node.out" "[geetorus] error: Node.js bootstrap is disabled for piped installs; download install.sh, review it, and run 'bash install.sh --no-prompt'"

echo "==> dry run"
run_with_node dry-run bash /geetorus-scripts/install.sh --no-prompt --dry-run --no-onboard
...
docker run --rm \
  -v "$REPO_ROOT/scripts:/geetorus-scripts:ro" \
  -v "$RESULTS_DIR:/results" \
  -e GEETORUS_INSTALL_TEST_LOG=/results/env.args \
  -e GEETORUS_INSTALL_VERSION=2026.722.0 \
  -e GEETORUS_INSTALL_INSTALL_SERVICE=1 \
  -e GEETORUS_INSTALL_NO_ONBOARD=1 \
  -e GEETORUS_INSTALL_NO_PROMPT=1 \
  -e PATH="/geetorus-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  node:24-bookworm-slim \
  bash /geetorus-scripts/install.sh
assert_line "$RESULTS_DIR/env.args" "geetorusai@2026.722.0"
...
docker run --rm \
  -v "$REPO_ROOT/scripts:/geetorus-scripts:ro" \
  -v "$RESULTS_DIR:/results" \
  -e GEETORUS_INSTALL_TEST_LOG=/results/no-node.args \
  -e PATH="/geetorus-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  ubuntu:24.04 \
  bash -c 'apt-get update >/dev/null && apt-get install -y ca-certificates curl >/dev/null && bash /geetorus-scripts/install.sh --no-prompt --no-onboard'
assert_line "$RESULTS_DIR/no-node.args" "geetorusai@latest"
