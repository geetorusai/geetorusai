/** Isolated, repeatable preparation benchmark. Run with pnpm --filter @geetorusai/server exec tsx ../scripts/benchmark-skill-preparation.ts. */
...
const home = child ? process.env.GEETORUS_HOME! : await fs.mkdtemp(path.join(os.tmpdir(), "skill-preparation-benchmark-"));
