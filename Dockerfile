...
RUN usermod -u $USER_UID --non-unique node \
  ...
  && usermod -g $USER_GID -d /geetorus node
...
COPY packages/geetorus-eval-kernel/package.json packages/geetorus-eval-kernel/
COPY packages/geetorus-runner/package.json packages/geetorus-runner/
...
COPY packages/plugins/geetorus-plugin-fake-sandbox/package.json packages/plugins/geetorus-plugin-fake-sandbox/
...
COPY packages/geetorus-runner/rust-toolchain.toml /tmp/runner-toolchain/rust-toolchain.toml
...
WORKDIR /app/packages/geetorus-runner
COPY packages/geetorus-runner/rust-toolchain.toml ./
COPY packages/geetorus-runner/runner ./runner
RUN cd runner && cargo chef prepare --recipe-path /tmp/runner-recipe.json

FROM rust-chef AS runner-deps
WORKDIR /app/packages/geetorus-runner/runner
COPY packages/geetorus-runner/rust-toolchain.toml ../
...
RUN cargo chef cook --release --locked --package geetorus-runner-core --bin geetorus-runnerd --recipe-path /tmp/runner-recipe.json \
...
COPY packages/geetorus-runner/rust-toolchain.toml ./
COPY packages/geetorus-runner/runner ./runner
COPY packages/geetorus-runner/protocol ./protocol
...
RUN find runner protocol -type f -exec touch -d @0 {} + \
  ...
  && cargo build --release --manifest-path runner/Cargo.toml --locked -p geetorus-runner-core --bin geetorus-runnerd

FROM runner-build AS build
WORKDIR /app
COPY --from=deps /app /app
COPY . .
RUN find packages/geetorus-runner/runner packages/geetorus-runner/protocol -type f -exec touch -d @0 {} + \
  && touch -d @0 packages/geetorus-runner/rust-toolchain.toml
RUN pnpm --filter @geetorusai/ui build
RUN pnpm --filter @geetorusai/plugin-sdk build
...
# script reads GEETORUS_BUILD_COMMIT instead. Docker exposes an ARG to the
...
ARG GEETORUS_BUILD_COMMIT=""
ENV NODE_OPTIONS=--max-old-space-size=4096
RUN pnpm --filter @geetorusai/server build
RUN test -f server/dist/index.js || (echo "ERROR: server build output missing" && exit 1)
RUN rm -rf packages/geetorus-runner/runner/target
...
ENV NODE_ENV=production \
  ...
  GEETORUS_BUILD_VERSION=${GEETORUS_BUILD_VERSION} \
  GEETORUS_BUILD_COMMIT=${GEETORUS_BUILD_COMMIT} \
  USER_UID=${USER_UID} \
  USER_GID=${USER_GID} \
  GEETORUS_CONFIG=/geetorus/instances/default/config.json \
...
# `plugins.autoInstall` key list through GEETORUS_MANAGED_CONFIG and
...
RUN set -eu; \
  ...
  echo '{"name":"geetorus-cloud-server-deps","private":true}' > package.json; \
  specifiers=""; \
