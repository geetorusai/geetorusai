...
//   2. The `GEETORUS_BUILD_COMMIT` environment variable.
// A Docker image build excludes `.git`, so the git lookup fails there. The
// image build passes the commit in `GEETORUS_BUILD_COMMIT` instead, so the
// stamp still records the true built commit.
//
// The build must not fail when no commit is available. A missing `git`, a
// checkout with no `.git`, and an unset `GEETORUS_BUILD_COMMIT` together write
// no stamp and exit 0.
 ...
 * `GEETORUS_BUILD_COMMIT` when `.git` is absent. Return null when neither
 * source gives a non-empty value.
 *
 * @param {unknown} gitCommit The `git rev-parse` result, or null on failure.
 * @param {unknown} suppliedCommit The `GEETORUS_BUILD_COMMIT` value.
 * @returns {string | null}
 */
 ...
 */
function main() {
  const commit = resolveBuildCommit(readGitCommit(), process.env.GEETORUS_BUILD_COMMIT);
