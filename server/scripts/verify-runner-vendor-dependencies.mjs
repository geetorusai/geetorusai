// Verify every npm package the vendored geetorus-runner actually imports
// at runtime is also declared as a direct dependency of server/package.json.
//
// packages/geetorus-runner is private and never published, so the server
// build vendors its compiled dist/ tree wholesale with `cp -R` (see the
// `build` script's `... dist/vendor/geetorus-runner/` step) -- code only,
...
// geetorus-runner declares dependencies (react-markdown, the codex/opencode
...
const runnerRoot = resolve(serverRoot, "../packages/geetorus-runner");
const runnerDist = resolve(runnerRoot, "dist");

// The only entry points server/src actually imports from the vendored
// runner (server/src/**/*.ts import "../vendor/geetorus-runner/index.js"
// or ".../testing.js").
...
export async function findRunnerExternalPackages(entryPoints) {
  for (const entryPoint of entryPoints) {
    if (!existsSync(entryPoint)) {
      throw new Error(
        `geetorus-runner vendor check: expected build output at ${entryPoint}. ` +
          `Run "pnpm --filter @geetorusai/geetorus-runner build" first.`,
      );
    }
  }
...
function explainMissingDependencies(missing, runnerDependencyNames) {
  const lines = missing.map((name) => {
    ...
    return range
      ? `  "${name}": "${range}"  (matches packages/geetorus-runner/package.json)`
      : `  "${name}"  (not declared as a geetorus-runner dependency either -- check for a missing or mistyped dependency there first)`;
  });
  return (
    `geetorus-runner vendor check: server/package.json is missing the runtime ` +
    `${missing.length === 1 ? "dependency" : "dependencies"} the vendored runner ` +
    `imports at runtime:\n${lines.join("\n")}\n\n` +
    `packages/geetorus-runner/dist is copied into server's own published package ` +
