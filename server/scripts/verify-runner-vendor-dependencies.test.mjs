...
describe("findMissingVendorDependencies", () => {
  ...
  it("flags a runner dependency that isn't mirrored into server/package.json", () => {
    ...
    // packages/geetorus-runner/package.json grew a new runtime dependency
...
describe("findRunnerExternalPackages", () => {
  ...
  // matching packages/geetorus-runner's shape (testing.js re-exports
  ...
  function writeFixture() {
    fixtureDir = mkdtempSync(join(tmpdir(), "geetorus-runner-vendor-fixture-"));
    writeFileSync(
  ...
  it("throws a clear, actionable error when an entry point is missing", async () => {
    ...
    await expect(findRunnerExternalPackages([missingEntryPoint])).rejects.toThrow(
      /expected build output at .*index\.js.*Run "pnpm --filter @geetorusai\/geetorus-runner build" first/s,
    );
  });
});
