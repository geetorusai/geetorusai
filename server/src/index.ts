...
async function startServerWithDatabaseTeardown(
  ...
  if (process.env.GEETORUS_SECRETS_PROVIDER === undefined) {
    process.env.GEETORUS_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.GEETORUS_SECRETS_STRICT_MODE === undefined) {
    process.env.GEETORUS_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.GEETORUS_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.GEETORUS_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.GEETORUS_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set GEETORUS_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@geetorus.local";
  ...
  if (config.databaseUrl) {
    ...
    try {
      ...
      // racing Geetorus's later heartbeat snapshot query. Geetorus explicitly
    ...
    const verboseEmbeddedPostgresLogs = process.env.GEETORUS_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const actualDataDir = await getPostgresDataDirectory(`postgres://geetorus:geetorus@127.0.0.1:${port}/postgres`);
      ...
      const configuredAdminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "geetorus");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "geetorus");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: geetorus");
    }
  
    const embeddedConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${port}/geetorus`;
  ...
  // Cloud managed-config contract (harness → app). Parse GEETORUS_MANAGED_CONFIG
  ...
  try {
    ...
    logger.error({ err }, "invalid GEETORUS_MANAGED_CONFIG; refusing to start (fail closed)");
    throw err;
  }

  // Operator setting defaults (GEETORUS_SETTING_DEFAULTS). Same fail-closed
  ...
  try {
    ...
    logger.error({ err }, "invalid GEETORUS_SETTING_DEFAULTS; refusing to start (fail closed)");
    throw err;
  }
  ...
  const databaseBackupMaxAgeHours = Math.max(
    1,
    Number(process.env.GEETORUS_DB_BACKUP_MAX_AGE_HOURS) ||
      Math.max(26, Math.ceil((config.databaseBackupIntervalMinutes / 60) * 2)),
  );
  const databaseBackupAlertFile =
    process.env.GEETORUS_DB_BACKUP_ALERT_FILE ||
  ...
  const configuredApiUrl = process.env.GEETORUS_API_URL?.trim() || runtimeApiUrl;
  ...
  process.env.GEETORUS_LISTEN_HOST = runtimeListenHost;
  process.env.GEETORUS_LISTEN_PORT = String(listenPort);
  process.env.GEETORUS_RUNTIME_API_URL = runtimeApiUrl;
  process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.GEETORUS_API_URL = configuredApiUrl;
  ...
  setupLiveEventsWebSocketServer(server, db as any, {
    ...
    // Cloud-proxied browsers carry trusted x-geetorus-cloud-* headers instead
    ...
    // self-gating: it returns null unless GEETORUS_CLOUD_TENANT_SERVER_TOKEN
  ...
  // env (GEETORUS_EXECUTION_MODE=kubernetes). Runs BEFORE the heartbeat resumes
  // queued runs so the policy + managed k8s environments are in place. A bad
  // GEETORUS_EXECUTION_MODE / GEETORUS_K8S_* value throws and fails startup
  ...
  // a contradictory deployment that also forces GEETORUS_EXECUTION_MODE
  ...
  const tools = toolAccessService(db as any, {
    deploymentMode: config.deploymentMode,
    deploymentExposure: config.deploymentExposure,
    trustedLocalStdioRuntimeHost: process.env.GEETORUS_TRUSTED_MCP_RUNTIME_HOST
  ...
  // set (GEETORUS_ADAPTERS). Must run after external adapters are loaded so the
  ...
  try {
    ...
    logger.error({ err }, "failed to reconcile adapter availability from GEETORUS_ADAPTERS");
    throw err;
  }
  ...
  void systemdNotify(["--ready", `--status=Listening on ${config.host}:${listenPort}`]).then((notified) => {
    if (notified) logger.info("Notified systemd that Geetorus is ready");
  });
  if (process.env.GEETORUS_OPEN_ON_LISTEN === "true") {
  ...
  const shutdown = async (
    ...
    const appShutdown = (app as { locals?: { geetorusShutdown?: () => Promise<void> } }).locals
      ?.geetorusShutdown;
...
if (isMainModule(import.meta.url)) {
  void startServer().catch(async (err) => {
    logger.error({ err }, "Geetorus server failed to start");
