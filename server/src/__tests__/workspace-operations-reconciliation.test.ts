...
describeEmbeddedPostgres("workspace operation reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-workspace-operation-reconcile-");
    db = createDb(tempDb.connectionString);
    logRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-workspace-operation-logs-"));
