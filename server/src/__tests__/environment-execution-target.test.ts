...
describe("resolveEnvironmentExecutionTarget", () => {
  ...
  it("keeps sandbox targets on bridge mode even when lease metadata includes a Geetorus API URL", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      driver: "sandbox",
      config: {
        provider: "fake-plugin",
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        geetorusApiUrl: "https://geetorus.example.test",
      },
      lease: null,
      environmentRuntime: null,
    });
    ...
    expect(target).not.toHaveProperty("geetorusApiUrl");
    expect(target).not.toHaveProperty("geetorusTransport");
  });
  ...
  it("keeps sandbox targets on callback bridge execution even when lease metadata advertises SSH access", async () => {
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        remoteCwd: "/home/sandbox/geetorus-workspace",
        sshAccess: {
          type: "ssh",
  ...
  // span ended. Each bridge step (`bridge.geetorus`, `bridge.process-session`)
  ...
  it("opens an unparented exec span for a geetorus bridge continuation", async () => {
    const execSpan = await runContinuationExec("bridge.geetorus", { wrap: true });
    expect(execSpan).toBeTruthy();
    expect(execSpan!.parent).toBeNull();
  });
