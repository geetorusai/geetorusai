...
import { execute } from "@geetorusai/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects geetorus env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the geetorus heartbeat.");
      ...
      expect(capture.prompt).toContain("Geetorus runtime note:");
      expect(capture.prompt).toContain("GEETORUS_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-execute-runtime-skill-"));
    ...
    const geetorusDir = await createSkillDir(runtimeSkillsRoot, "geetorus");
  ...
  it("prefers ~/.local/bin/cursor-agent for remote sandbox execution when using the default command", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-sandbox-execute-"));
  ...
  it("keeps explicit command overrides for remote sandbox execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cursor-sandbox-explicit-"));
    const homeDir = path.join(root, "home");
    const workspace = path.join(root, "workspace");
    const remoteWorkspace = path.join(root, "remote-workspace");
    const capturePath = path.join(root, "capture.json");
