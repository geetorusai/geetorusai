...
describe("agent local JWT", () => {
  const secretEnv = "GEETORUS_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "GEETORUS_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "GEETORUS_AGENT_JWT_ISSUER";
  const audienceEnv = "GEETORUS_AGENT_JWT_AUDIENCE";
  const disableLegacyFallbackEnv = "GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK";
  const instanceIdEnv = "GEETORUS_INSTANCE_ID";
  ...
  it("falls back to BETTER_AUTH_SECRET when GEETORUS_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "geetorus";
    process.env[audienceEnv] = "geetorus-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
  ...
  // GEETORUS_INSTANCE_ID but deliberately shares GEETORUS_AGENT_JWT_SECRET
  ...
  it("defaults TTL to 48h when GEETORUS_AGENT_JWT_TTL_SECONDS is unset", () => {
    ...
    // GEETORUS_API_KEY dead on arrival.
  ...
  it("accepts master-secret-signed tokens when GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK is unset", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    delete process.env[disableLegacyFallbackEnv];
  ...
  it("rejects master-secret-signed tokens when GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK is enabled", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    process.env[disableLegacyFallbackEnv] = "true";
    const legacyToken = craftLegacyMasterSecretToken(process.env[secretEnv]!, "company-legacy");
    expect(verifyLocalAgentJwt(legacyToken)).toBeNull();
  ...
  it("still verifies per-company-signed tokens when GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK is enabled", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    process.env[disableLegacyFallbackEnv] = "true";
    const token = createLocalAgentJwt("agent-1", "company-1", "claude_local", "run-1");
