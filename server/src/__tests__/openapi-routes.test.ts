...
import { COMPANY_IMPORT_TRANSFERS_ROUTE_PATH } from "@geetorusai/shared/company-import-transfer";
...
const explicitOpenApiOperationCoverageExclusions = new Set([
  ...
  // Geetorus board/agent credential. It intentionally stays out of the public
...
describe("openapi routes", () => {
  it("documents personal board-only announcements and private responses", () => {
    ...
    for (const operation of [current, image, animation, dismiss]) {
      ...
      expect(operation["x-geetorus-authorization"]).toEqual({ actor: "board" });
  ...
  it("serves the generated OpenAPI document", async () => {
    ...
    expect(res.body.info.title).toBe("Geetorus API");
    ...
    expect(res.body.paths["/api/mcp/project-tools"].post["x-geetorus-authorization"]).toEqual({ actor: "agent", heartbeatBound: true, taskBound: true });
  ...
  it("publishes the complete board contract for chat channels", () => {
    ...
    for (const [method, routePath] of operations) {
      ...
      expect(operation["x-geetorus-authorization"]).toEqual({
        actor: "board",
      });
  ...
  it("documents auth and reviewed response-code invariants", () => {
    ...
    expect(
      spec.paths["/api/plugins/install"].post["x-geetorus-authorization"],
    ).toEqual({
      actor: "board",
      instanceAdmin: true,
    });
