...
describe.sequential("plugin install and upgrade authz", () => {
  ...
  it("lists bundled monorepo plugin packages", async () => {
    ...
    expect(packageNames).toContain("@geetorusai/plugin-workspace-diff");
    expect(packageNames).toContain("@geetorusai/plugin-llm-wiki");
    expect(packageNames).toContain("@geetorusai/plugin-modal");
    expect(packageNames).toContain("@geetorusai/plugin-authoring-smoke-example");
    expect(packageNames).not.toContain("@geetorusai/plugin-sdk");
    expect(byPackageName.get("@geetorusai/plugin-workspace-diff")?.experimental).toBe(true);
    expect(byPackageName.get("@geetorusai/plugin-llm-wiki")?.experimental).toBe(true);
    expect(byPackageName.get("@geetorusai/plugin-modal")?.experimental).toBe(true);
    expect(byPackageName.get("@geetorusai/plugin-authoring-smoke-example")?.experimental).toBe(false);
    expect(typeof byPackageName.get("@geetorusai/plugin-workspace-diff")?.hasBuiltEntrypoints).toBe("boolean");
  }, 20_000);
  ...
  it("rejects plugin installation for non-admin board users", async () => {
    ...
    const res = await request(app)
      ...
      .send({ packageName: "geetorus-plugin-example" });
  ...
  it("allows instance admins to install plugins", async () => {
    const pluginId = "11111111-1111-4111-8111-111111111111";
    const pluginKey = "geetorus.example";
    const discovered = {
      manifest: {
        id: pluginKey,
      },
    ...
    mockRegistry.getByKey.mockResolvedValue({
      id: pluginId,
      pluginKey,
      packageName: "geetorus-plugin-example",
      version: "1.0.0",
    });
    mockRegistry.getById.mockResolvedValue({
      id: pluginId,
  ...
  it("resolves plugin keys without probing the UUID id column for core plugin actions", async () => {
    const pluginKey = "geetorusqa.hello-plugin";
    const plugin = {
      id: pluginId,
      pluginKey,
      version: "1.0.0",
...
describe.sequential("scoped plugin API routes", () => {
  ...
  it("dispatches manifest-declared scoped routes after company access checks", async () => {
    ...
    const res = await request(app)
      .get("/api/plugins/geetorus.example/api/smoke")
    ...
    for (const [label, rows] of cases) {
      ...
      const { app } = await createApp(boardActor(), {}, {
        ...
        toolDeps: {
          toolDispatcher: {
            listToolsForAgent: vi.fn(),
            getTool: vi.fn(() => ({ name: "geetorus.example:search" })),
            executeTool,
          },
        },
      });
    ...
    const { app } = await createApp(
      ...
      {},
      {
        ...
        toolDeps: {
          toolDispatcher: {
            listToolsForAgent: vi.fn(),
            getTool: vi.fn(() => ({ name: "geetorus.example:search", pluginDbId: pluginId })),
            executeTool,
          },
        },
      },
...
describe.sequential("operator-hidden plugin management floor", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.GEETORUS_HIDDEN_SETTINGS = "instance.plugins";
  });
  afterEach(() => {
    delete process.env.GEETORUS_HIDDEN_SETTINGS;
  });
  ...
  it("floors plugin lifecycle and config writes for instance admins", async () => {
    ...
    const attempts: Array<[string, request.Test]> = [
      ["install", request(app).post("/api/plugins/install").send({ packageName: "@geetorusai/plugin-modal" })],
