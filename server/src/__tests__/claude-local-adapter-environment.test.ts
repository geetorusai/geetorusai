...
import { resetClaudeCliCapabilitiesCacheForTests, testEnvironment } from "@geetorusai/adapter-claude-local/server";
...
const ORIGINAL_GEETORUS_HOME = process.env.GEETORUS_HOME;
const ORIGINAL_GEETORUS_INSTANCE_ID = process.env.GEETORUS_INSTANCE_ID;
...
afterEach(() => {
  ...
  if (ORIGINAL_GEETORUS_HOME === undefined) {
    delete process.env.GEETORUS_HOME;
  } else {
    process.env.GEETORUS_HOME = ORIGINAL_GEETORUS_HOME;
  }
  if (ORIGINAL_GEETORUS_INSTANCE_ID === undefined) {
    delete process.env.GEETORUS_INSTANCE_ID;
  } else {
    process.env.GEETORUS_INSTANCE_ID = ORIGINAL_GEETORUS_INSTANCE_ID;
  }
});
...
describe("claude_local environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `geetorus-claude-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
    ...
    expect(
      result.checks.some(
        (check) =>
          ...
          check.message === "Working directory is valid: /srv/geetorus/workspace",
      ),
    ).toBe(true);
  ...
  it("uses the managed Claude config seed for sandbox hello probes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-envtest-managed-config-"));
...
if (!configDir.includes(".geetorus-runtime/claude/config")) {
...
`, "utf8");
  ...
  it("warns and omits --effort for sandbox probes when the installed Claude CLI does not advertise it", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-envtest-sandbox-effort-"));
