...
function registerModuleMocks() {
  ...
  // ~/.geetorus/adapter-settings.json.
...
describe("agent routes adapter validation", () => {
  ...
  it("selects and refreshes the runner provider catalog independently", async () => {
    ...
    try {
      ...
      for (const [provider, adapter] of [["acpx", "claude_local"], ["codex", "codex_local"], ["opencode", "opencode_local"]]) {
        const res = await requestApp(app, (baseUrl) => request(baseUrl).get(`/api/companies/company-1/adapters/geetorus_runner/models?provider=${provider}`));
        expect(res.status).toBe(200);
        expect(res.body).toEqual([{ id: adapter, label: adapter }]);
        const refreshed = await requestApp(app, (baseUrl) => request(baseUrl).get(`/api/companies/company-1/adapters/geetorus_runner/models?provider=${provider}&refresh=true`));
        expect(refreshed.status).toBe(200);
        expect(refreshed.body).toEqual([{ id: `${adapter}-fresh`, label: adapter }]);
      }
      const invalid = await requestApp(app, (baseUrl) => request(baseUrl).get("/api/companies/company-1/adapters/geetorus_runner/models?provider=acpx_codex"));
  ...
  it("restores a saved agent's redacted CODEX_HOME before testing its adapter", async () => {
    const agentId = "11111111-1111-4111-8111-111111111111";
    const storedHome = "/geetorus/companies/company-1/agents/agent-1/codex-home";
    mockAgentService.getById.mockResolvedValue({
      ...(await mockAgentService.getById()),
      id: agentId,
      adapterType: "external_test",
  ...
  it("refuses to create an agent on an adapter the instance has disabled", async () => {
    ...
    // GEETORUS_ADAPTERS). Creating an agent on it "succeeds" and then every
  ...
  it("rejects a new geetorus_runner selection while the rollout flag is off", async () => {
    const app = await createApp();
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl)
        .post("/api/companies/company-1/agents")
        .send({ name: "Native Codex", adapterType: "geetorus_runner" }),
    );

    expect(res.status, JSON.stringify(res.body)).toBe(422);
    expect(res.body.details).toMatchObject({ code: "geetorus_runner_rollout_disabled" });
    expect(mockAgentService.create).not.toHaveBeenCalled();
  });

  it("allows a new geetorus_runner selection while the rollout flag is on", async () => {
      ...
      request(baseUrl)
        ...
        .send({
          ...
          adapterConfig: {
            provider: "codex",
            geetorusSkillSync: {
              desiredSkills: ["geetorusai/geetorus/geetorus", "company-1/reviewer"],
            },
          },
        }),
    );
    ...
    expect(mockAgentInstructionsService.materializeManagedBundle).toHaveBeenCalledWith(
      expect.objectContaining({ adapterType: "geetorus_runner" }),
      expect.any(Object),
      expect.objectContaining({ entryFile: "AGENTS.md", replaceExisting: false }),
    );
  });

  it("normalizes legacy skills and permissions when switching to geetorus_runner", async () => {
    mockInstanceSettingsService.getExperimental.mockResolvedValue({ enableNativeRunner: true });
    ...
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl)
        .patch("/api/agents/11111111-1111-4111-8111-111111111111")
        .send({ adapterType: "geetorus_runner", replaceAdapterConfig: true, adapterConfig: {} }),
    );
  ...
  it.each([
    ...
    "rejects a same-provider Geetorus Runner edit with %s",
  ...
  it("rejects a same-provider rollback to an invalid runner config", async () => {
    ...
    expect(response.body.details).toMatchObject({
      code: "geetorus_runner_claude_managed_retention_required",
    });
    expect(mockAgentService.rollbackConfigRevision).not.toHaveBeenCalled();
  });

  it("keeps an existing geetorus_runner agent editable after the flag is disabled", async () => {
    const existing = await mockAgentService.getById();
    mockAgentService.getById.mockResolvedValue({
      ...existing,
      adapterType: "geetorus_runner",
