...
import { resolveCodexAuthCacheDir, withAccountHomeSecretMutationLock } from "@geetorusai/adapter-codex-local/server";
import {
  activityLog,
  agents,
  companies,
...
import { LOW_TRUST_REVIEW_PRESET } from "@geetorusai/shared";
...
describeEmbeddedPostgres("secretService", () => {
  ...
  const previousKeyFile = process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-secrets-service-${randomUUID()}`);

  beforeAll(async () => {
    mkdirSync(secretsTmpDir, { recursive: true });
    process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = path.join(secretsTmpDir, "master.key");
  ...
  afterAll(async () => {
    ...
    if (previousKeyFile === undefined) {
      ...
      process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = previousKeyFile;
    }
  ...
  it("serializes an aws_secrets_manager secret create against a concurrent account-home mutation lock holder", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        ...
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/aws-secret",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/aws-secret",
  ...
  it("serializes an aws_secrets_manager secret rotate against a concurrent account-home mutation lock holder", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/aws-secret-rotate",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/aws-secret-rotate",
      providerVersionRef: "aws-version-1",
    });
  ...
  it("reports current-user secret rollback failures when AWS create cleanup cannot remove the reserved row", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockRejectedValueOnce(
      new SecretProviderClientError({
        ...
        rawMessage:
          "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/Geetorus cannot create secret",
      }),
    );
    ...
    await expect(
      ...
      status: 500,
      message: "Secret create failed and Geetorus could not roll back the local secret reservation.",
      details: {
  ...
  it("reports current-user secret persistence rollback failures when local cleanup cannot remove the reserved row", async () => {
    ...
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/user/github-token";
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
  ...
  it("removes user secret values and provider material when deleting a definition", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockImplementation(async (input) => {
      nextVersion += 1;
      const externalRef =
        `arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/${input.context.secretKey}`;
      return {
        material: {
          scheme: "aws_secrets_manager_v1",
          secretId: externalRef,
  ...
  it("removes user secret values and provider material when update deletes a definition", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/user-secret",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/user-secret",
      providerVersionRef: "aws-version-1",
    });
    const deleteSpy = vi.spyOn(awsSecretsManagerProvider, "deleteOrArchive").mockResolvedValue();
  ...
  it("blocks coming-soon provider vaults from secret selection", async () => {
    ...
    const draftVault = await svc.createProviderConfig(companyId, {
      ...
      config: { projectId: "geetorus-prod1" },
    });
  ...
  it("passes selected provider vault config through create, rotate, and resolve", async () => {
    ...
    const createSpy = vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/openai-api-key",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/openai-api-key",
      providerVersionRef: "aws-version-1",
    });
  ...
  it("cleans up managed provider secrets when create persistence fails", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/create-rollback",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("keeps a local cleanup handle when create rollback cleanup fails", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/create-cleanup-handle",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
    ...
    await expect(
      ...
      status: 500,
      message: "Secret create failed and Geetorus could not clean up the remote provider secret.",
      details: {
  ...
  it("reports managed secret persistence rollback failures when local cleanup cannot remove the reserved row", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/create-local-cleanup",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("archives managed provider versions when rotate persistence fails", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/rotate-rollback",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("keeps a disabled version cleanup handle when rotate rollback cleanup fails", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/rotate-cleanup-handle",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("rejects generic provider vault reassignment for managed secrets", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company/vault-reassign",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("sanitizes AWS remote import preview provider errors before crossing the service boundary", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Geetorus is not authorized to perform secretsmanager:ListSecrets";
  ...
  it("sanitizes AWS managed secret create failures and removes the reserved row", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Geetorus is not authorized to perform secretsmanager:CreateSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1";
  ...
  it("previews AWS provider vault discovery from draft config without persisting a provider vault", async () => {
    ...
    const discoverSpy = vi.spyOn(awsSecretsManagerProvider, "discoverProviderConfigs").mockResolvedValue({
      ...
      skippedForeignGeetorusSampleCount: 0,
      candidates: [
        {
          provider: "aws_secrets_manager",
          displayName: "AWS production",
          ...
          sampleCount: 1,
          samples: [
            { name: "geetorus/prod-use1/company-1/openai", hasKmsKey: false, tagKeys: ["geetorus:environment"] },
          ],
          signals: {
            namespace: "prod-use1",
            secretNamePrefix: "geetorus",
  ...
  it("sanitizes AWS provider vault discovery errors before crossing the service boundary", async () => {
    ...
    expect(thrown).toMatchObject({
      ...
      details: {
        ...
        region: "us-east-1",
        credentialPath: "Geetorus server runtime/provider credential path",
        requiredCapability: "secretsmanager:ListSecrets",
        actionableMessage:
          "AWS discovery preview needs secretsmanager:ListSecrets in the selected region for the Geetorus server runtime/provider credential path.",
        safeAlternative:
  ...
  it("sanitizes AWS remote import row provider errors", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Geetorus is not authorized to perform secretsmanager:DescribeSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
  ...
  it("rejects Geetorus-managed AWS namespace refs during preview and import commit", async () => {
    const companyId = await seedCompany();
    const svc = secretService(db);
    const awsVault = await svc.createProviderConfig(companyId, {
      provider: "aws_secrets_manager",
    ...
    vi.spyOn(awsSecretsManagerProvider, "listRemoteSecrets").mockResolvedValue({
      secrets: [
        {
          externalRef:
            "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-b/openai",
          name: "geetorus/prod-use1/company-b/openai",
          providerVersionRef: null,
          metadata: {
            arn: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-b/openai",
            description: "must not leak",
            tags: [{ Key: "geetorus:company-id", Value: "company-b" }],
          },
        },
      ],
    });
    ...
    expect(JSON.stringify(preview)).not.toContain("geetorus:company-id");
    ...
    expect(result.results[0]?.reason).toMatch(/Geetorus-managed namespace/i);
  ...
  it("rejects externalRef overrides on managed secrets", async () => {
    ...
    await expect(
      svc.update(secret.id, {
        externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod/company-b/openai-api-key",
      }),
  ...
  it("passes managed AWS secret context into provider delete during removal", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-1/openai-api-key";
  ...
  it("renames name and key during removal before provider deletion", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-1/remove-failure";
    const secret = await db
      .insert(companySecrets)
      .values({
        companyId,
  ...
  it("treats missing provider secrets as already removed during removal retry", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-1/retry-delete";
  ...
  it("removes DB rows even when the attached provider vault is disabled", async () => {
    ...
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod/company-1/openai-api-key";
    const secret = await db
      .insert(companySecrets)
      .values({
        companyId,
