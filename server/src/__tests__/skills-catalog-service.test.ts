...
import type { CatalogSkill } from "@geetorusai/shared";
...
function catalogSkill(slug: string, name = slug): CatalogSkill {
  return {
    id: `geetorusai:bundled:software-development:${slug}`,
    key: `geetorusai/bundled/software-development/${slug}`,
    kind: "bundled",
...
describe("skills catalog service", () => {
  ...
  beforeEach(() => {
    ...
    mockRequireResolve.mockImplementation((specifier: string) => {
      if (specifier === "@geetorusai/skills-catalog/package.json") {
        return "/published/node_modules/@geetorusai/skills-catalog/package.json";
      }
      if (specifier === "@geetorusai/skills-catalog/catalog.json") {
        return "/published/node_modules/@geetorusai/skills-catalog/generated/catalog.json";
      }
  ...
  it("caches and reloads the generated catalog manifest when it changes", async () => {
    ...
    expect(service.listCatalogSkills().map((skill) => skill.key)).toEqual([
      "geetorusai/bundled/software-development/old-skill",
    ]);
    expect(service.listCatalogSkills().map((skill) => skill.key)).toEqual([
      "geetorusai/bundled/software-development/old-skill",
    ]);
    ...
    expect(service.listCatalogSkills().map((skill) => skill.key)).toEqual([
      "geetorusai/bundled/software-development/new-skill",
    ]);
  ...
  it("resolves the manifest and bundled skill files from the published package layout", async () => {
    ...
    expect(mockReadFileSync).toHaveBeenCalledWith(
      "/published/node_modules/@geetorusai/skills-catalog/generated/catalog.json",
      "utf8",
    );
    expect(mockReadFile).toHaveBeenCalledWith(
      "/published/node_modules/@geetorusai/skills-catalog/catalog/bundled/software-development/published-skill/SKILL.md",
    );
    expect(mockRequireResolve).toHaveBeenCalledWith("@geetorusai/skills-catalog/package.json");
    expect(mockRequireResolve).toHaveBeenCalledWith("@geetorusai/skills-catalog/catalog.json");
  });
