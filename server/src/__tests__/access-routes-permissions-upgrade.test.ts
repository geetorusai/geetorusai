...
vi.hoisted(() => {
  process.env.GEETORUS_HOME = "/tmp/geetorus-test-home";
  process.env.GEETORUS_INSTANCE_ID = "vitest";
  process.env.GEETORUS_LOG_DIR = "/tmp/geetorus-test-home/logs";
...
describeEmbeddedPostgres("access routes permissions upgrade compatibility", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-access-routes-permissions-upgrade-");
