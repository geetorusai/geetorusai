...
describeEmbeddedPostgres("issue watchdog routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issue-watchdogs-routes-");
  ...
  async function seedCompany(name = "Geetorus") {
  ...
  it("routes watchdog-discovered product bugs outside the watched source tree with evidence links", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        description: "Watchdog found a Geetorus follow-up routing bug.",
