...
describe("teams catalog routes", () => {
  beforeEach(() => {
    ...
    mockCatalogModule.readCatalogTeamFile.mockResolvedValue({
      catalogTeamId: "geetorusai:bundled:software-development:product-engineering",
      path: "TEAM.md",
      kind: "team",
    ...
    mockTeamsCatalogService.installCatalogTeam.mockResolvedValue({
      team: catalogTeam(),
      portabilityImport: {
        company: { id: companyId, name: "Geetorus", action: "unchanged" },
        agents: [],
        projects: [],
        envInputs: [],
        warnings: [],
  ...
  it("previews catalog teams with company access and actor/source policy context", async () => {
    ...
    const res = await request(app)
      .post(`/api/companies/${companyId}/teams/catalog/ref/preview?ref=geetorusai%2Fbundled%2Fsoftware-development%2Fproduct-engineering`)
      .send({
