...
const cloudEnv = {
  GEETORUS_CLOUD_TENANT_SERVER_TOKEN: "tenant-secret",
  GEETORUS_CLOUD_STACK_ID: "stack-current",
  GEETORUS_CLOUD_API_ORIGIN: "https://cloud.example.test/control-plane",
};
...
describe("GET /api/cloud/stacks", () => {
  it("returns the actor's portfolio without forwarding client-supplied identity", async () => {
    ...
    const res = await request(app)
      ...
      .set("x-geetorus-cloud-user-id", "spoofed-header-user")
    ...
    expect(init?.headers).toEqual({
      ...
      "x-geetorus-cloud-user-id": "actor-user",
      "x-geetorus-cloud-stack-id": "stack-current",
    });
  ...
  it("keeps cache entries isolated by the server-derived actor user id", async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockImplementation(async (_url, init) => {
      ...
      return jsonResponse({ userId: headers.get("x-geetorus-cloud-user-id") });
    });
    const app = express();
    app.use((req, _res, next) => {
