...
import { LOW_TRUST_REVIEW_PRESET, type AgentApiKeyScope } from "@geetorusai/shared";
...
describeEmbeddedPostgres("agent secret routes", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-agent-secret-routes-${randomUUID()}`);
  ...
  it("deduplicates concurrent redaction registrations for the same run and value", async () => {
    ...
    expect(run.contextSnapshot).toMatchObject({
      geetorusSecretRedactions: [expect.objectContaining({ fingerprintSha256: expect.any(String) })],
    });
    expect((run.contextSnapshot as { geetorusSecretRedactions: unknown[] }).geetorusSecretRedactions).toHaveLength(1);
  });
