...
import type { AttentionArchiveManifestEntry, AttentionArchiveTargetSnapshot } from "@geetorusai/shared";
...
describePg("decisionService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-decisions-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  beforeEach(async () => {
    process.env.GEETORUS_DECISION_SIGNING_SECRET = "0123456789abcdef0123456789abcdef";
  ...
  afterEach(async () => {
    delete process.env.GEETORUS_DECISIONS_SWEEP_BATCH_SIZE;
    delete process.env.GEETORUS_DECISIONS_RECOVERY_GRACE_MS;
  ...
  it("fails closed when the configured signing secret is removed after proposal", async () => {
    ...
    const tempHome = mkdtempSync(path.join(tmpdir(), "geetorus-decision-rotate-"));
    process.env.GEETORUS_HOME = tempHome;
    delete process.env.GEETORUS_DECISION_SIGNING_SECRET;
    process.env.GEETORUS_AGENT_JWT_SECRET = "agent-jwt-secret-must-not-sign-decisions";
    try {
      await expect(service().decide({ id: created.id, optionId: "yes", decidedByUserId, userActor: boardActor() }))
  ...
  it("signs and verifies with an auto-generated key when no secret is configured", async () => {
    const originalHome = process.env.GEETORUS_HOME;
    const tempHome = mkdtempSync(path.join(tmpdir(), "geetorus-decision-generated-"));
    process.env.GEETORUS_HOME = tempHome;
    delete process.env.GEETORUS_DECISION_SIGNING_SECRET;
    try {
      const created = await createCommentDecision();
  ...
  it("recovers stale running decisions from the bounded server sweep", async () => {
    process.env.GEETORUS_DECISIONS_RECOVERY_GRACE_MS = "0";
    const created = await createCommentDecision();
    await db.update(decisions).set({ status: "decided", executionStatus: "running", chosenOptionId: "yes", decidedByUserId,
  ...
  it("bounds expiration work to the configured batch size", async () => {
    process.env.GEETORUS_DECISIONS_SWEEP_BATCH_SIZE = "1";
  ...
  it("falls back to the default sweep batch size for invalid configuration", async () => {
    process.env.GEETORUS_DECISIONS_SWEEP_BATCH_SIZE = "not-a-number";
