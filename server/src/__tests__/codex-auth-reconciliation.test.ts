...
describe("reconcileCodexLocalManagedHomesOnStartup", () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-startup-"));
    geetorusHome = path.join(root, "geetorus-home");
    ...
    for (const key of ["GEETORUS_HOME", "GEETORUS_INSTANCE_ID", "CODEX_HOME"]) {
