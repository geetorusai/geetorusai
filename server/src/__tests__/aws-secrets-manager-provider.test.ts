...
describe("awsSecretsManagerProvider", () => {
  const previousEnv = {
    GEETORUS_SECRETS_AWS_REGION: process.env.GEETORUS_SECRETS_AWS_REGION,
    AWS_REGION: process.env.AWS_REGION,
    AWS_DEFAULT_REGION: process.env.AWS_DEFAULT_REGION,
    GEETORUS_SECRETS_AWS_DEPLOYMENT_ID: process.env.GEETORUS_SECRETS_AWS_DEPLOYMENT_ID,
    GEETORUS_SECRETS_AWS_KMS_KEY_ID: process.env.GEETORUS_SECRETS_AWS_KMS_KEY_ID,
  ...
  it("creates Geetorus-managed AWS secrets without persisting plaintext in provider material", async () => {
    ...
    const provider = createAwsSecretsManagerProvider({
      ...
      gateway: {
        async createSecret(input) {
          ...
          return {
            ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-1/openai-api-key",
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          Name: "geetorus/prod-use1/company-1/openai-api-key",
          KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/test",
        }),
      }),
    ]);
    expect(JSON.stringify(prepared)).not.toContain("super-secret-value");
    expect(prepared.externalRef).toContain("geetorus/prod-use1/company-1/openai-api-key");
  ...
  it("creates AWS secrets from selected provider vault config without deployment env fallback", async () => {
    delete process.env.GEETORUS_SECRETS_AWS_REGION;
    delete process.env.AWS_REGION;
    delete process.env.AWS_DEFAULT_REGION;
    delete process.env.GEETORUS_SECRETS_AWS_DEPLOYMENT_ID;
    delete process.env.GEETORUS_SECRETS_AWS_KMS_KEY_ID;
      ...
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          ...
          Tags: expect.arrayContaining([
            { Key: "geetorus:provider-owner", Value: "platform" },
            { Key: "geetorus:environment", Value: "production" },
          ]),
        }),
      }),
    ]);
  ...
  it("signs AWS Secrets Manager JSON requests with default runtime credentials", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod/company-1/openai-api-key",
          VersionId: "aws-version-1",
        }),
        { status: 200 },
      ),
    ...
    expect(calls).toEqual([
      {
        ...
        input: {
          ...
          VersionStages: ["GEETORUS_PENDING"],
        },
      },
    ]);
  ...
  it("rejects linked external references under the Geetorus-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
    ...
    await expect(
      provider.linkExternalSecret({
        externalRef:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-2/openai-api-key",
        providerVersionRef: "linked-version-7",
      }),
    ).rejects.toThrow(/Geetorus-managed namespace/i);
  });
  ...
  it("writes new values through to externally referenced AWS secrets as AWSCURRENT", async () => {
    ...
    const prepared = await provider.updateExternalSecretValue!({
      ...
      context: {
        ...
        secretName: "geetorus-cloud/prod/provider/neon/admin-api-key",
        version: 2,
      },
    });
  ...
  it("rejects external value writes under the Geetorus-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
      ...
      gateway: {
        ...
        async listSecrets(input) {
          ...
          return {
            ...
            SecretList: [
              {
                ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-1/openai",
                Name: "geetorus/prod-use1/company-1/openai",
                KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/prod",
                Tags: [
                  { Key: "geetorus:managed-by", Value: "geetorus" },
                  { Key: "geetorus:deployment-id", Value: "prod-use1" },
                  { Key: "geetorus:company-id", Value: "company-1" },
                  { Key: "geetorus:environment", Value: "production" },
                  { Key: "geetorus:provider-owner", Value: "platform" },
                ],
              },
              {
                ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-2/stripe",
                Name: "geetorus/prod-use1/company-2/stripe",
                Tags: [
                  { Key: "geetorus:managed-by", Value: "geetorus" },
                  { Key: "geetorus:company-id", Value: "company-2" },
                ],
              },
            ],
          };
    ...
    expect(calls).toEqual([
      {
        ...
        input: {
          ...
          NextToken: undefined,
          IncludePlannedDeletion: false,
          Filters: [{ Key: "all", Values: ["geetorus"] }],
        },
      },
    ]);
  ...
  it("redacts AWS provider exception text when remote listing fails", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Geetorus is not authorized to perform secretsmanager:ListSecrets on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
  ...
  it("resolves AWS secret values by provider version reference", async () => {
    ...
    const resolved = await provider.resolveVersion({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/company-1/openai-api-key",
        versionId: "aws-version-2",
        source: "managed",
      },
      externalRef:
  ...
  it("warns when AWS provider configuration is incomplete and blocks managed writes", async () => {
    ...
    expect(health.message).toContain("missing GEETORUS_SECRETS_AWS_REGION");
    ...
    ).rejects.toThrow(/GEETORUS_SECRETS_AWS_REGION|AWS_REGION/i);
  });

  it("deletes only Geetorus-managed AWS secrets", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
  ...
  it("archives pending Geetorus-managed AWS versions without deleting the secret", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
