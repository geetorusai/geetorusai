...
const ORIGINAL_GEETORUS_CONFIG = process.env.GEETORUS_CONFIG;
...
describe("readConfigFile", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-config-file-test-"));
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    if (ORIGINAL_GEETORUS_CONFIG === undefined) {
      delete process.env.GEETORUS_CONFIG;
    } else {
      process.env.GEETORUS_CONFIG = ORIGINAL_GEETORUS_CONFIG;
    }
  ...
  it("throws a path-specific error when the config file is invalid JSON", () => {
    ...
    expect(() => readConfigFile()).toThrow(
      new RegExp(`Invalid Geetorus config at ${escapeRegExp(configPath)}: failed to read or parse JSON`),
    );
  });
  ...
  it("throws a field-specific error when the config file fails schema validation", () => {
    ...
    expect(() => readConfigFile()).toThrow(/Invalid Geetorus config .* \$meta\.source:/);
  });
