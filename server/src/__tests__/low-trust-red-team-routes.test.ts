  ...
  () => {
    ...
    beforeAll(async () => {
      tempDb = await startEmbeddedPostgresTestDatabase(
        "geetorus-low-trust-red-team-routes-",
      );
    ...
    it("redacts quarantined low-trust output from higher-trust wake and continuation contexts", async () => {
      ...
      const heartbeat = heartbeatService(db, {
        runtimeEnv: {
          ...
          GEETORUS_DATABASE_RESTORE_IN_PROGRESS: "false",
          GEETORUS_RESTORE_IN_PROGRESS: "false",
        },
      });
      ...
      try {
        ...
        // generated message rather than a top-level `geetorus` field.
