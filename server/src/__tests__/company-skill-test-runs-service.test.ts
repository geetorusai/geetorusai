...
describeEmbeddedPostgres("companySkillService skill test runs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-company-skill-test-runs-");
  ...
  async function seedSkillAndAgent() {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-test-run-"));
  ...
  it("snapshots output and keeps run history after harness issue retention", async () => {
    ...
    await db.insert(agents).values({
      ...
      adapterConfig: {
        model: "gpt-5.4",
        geetorusSkillSync: { desiredSkills: [`company/${companyId}/review`] },
