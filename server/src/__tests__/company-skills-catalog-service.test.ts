...
import { companies, companySkills, createDb, folders } from "@geetorusai/db";
import {
  getEmbeddedPostgresTestSupport,
  startEmbeddedPostgresTestDatabase,
} from "./helpers/embedded-postgres.js";
import type { CatalogSkill, CatalogSkillFile } from "@geetorusai/shared";
...
const sampleCatalogSkill: CatalogSkill = {
  id: "geetorusai:bundled:software-development:review",
  key: "geetorusai/bundled/software-development/review",
  kind: "bundled",
...
describeEmbeddedPostgres("companySkillService.installFromCatalog", () => {
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-company-skills-catalog-");
    db = createDb(tempDb.connectionString);
    svc = await createService();
  }, 20_000);

  beforeEach(async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-catalog-home-"));
  ...
  it("repairs an existing unfiled Geetorus catalog skill during inventory refresh", async () => {
    const companyId = await createCompany();
  ...
  it("restores portable catalog provenance when importing packaged skills", async () => {
    const companyId = await createCompany();
    const importedFiles = {
      "skills/geetorusai/bundled/software-development/review/SKILL.md": [
        "---",
        `key: "${sampleCatalogSkill.key}"`,
        'slug: "review"',
        'name: "review"',
      ...
      "skills/geetorusai/bundled/software-development/review/references/checklist.md": sampleReferenceMarkdown,
    };
  ...
  it("rejects duplicate slug conflicts", async () => {
    const companyId = await createCompany();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-existing-skill-"));
