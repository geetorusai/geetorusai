...
describeEmbeddedPostgres("agent service clearError", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-agent-clear-error-");
  ...
  it("resets converted agent sessions while preserving identity, configuration and run history", async () => {
    ...
    const updated = await agentService(db).update(agentId, { adapterType: "geetorus_runner", adapterConfig: { ...config, provider: "acpx", acpxAgent: "claude", model: "custom-claude-model" } });
    ...
    expect(runtime).toMatchObject({ adapterType: "geetorus_runner", sessionId: null, stateJson: {}, lastRunId: runId });
