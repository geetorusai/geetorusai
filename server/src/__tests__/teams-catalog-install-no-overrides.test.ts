...
import { agents, companies, createDb } from "@geetorusai/db";
import {
...
describeEmbeddedPostgres("teams catalog install with no caller adapter overrides", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-teams-catalog-no-overrides-"));
    process.env.GEETORUS_HOME = tempHome;
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-teams-catalog-no-overrides-");
