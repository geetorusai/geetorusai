...
const PLUGIN_KEY = "geetorus.daytona-sandbox-provider";
...
describe("listReadyPluginEnvironmentDrivers worker recovery", () => {
  ...
  it("does not lazy-start ready plugins outside the managed bundled allowlist", async () => {
    ...
    const drivers = await listReadyPluginEnvironmentDrivers({
      ...
      workerManager: worker.workerManager,
      recoverMissingWorker: {
        pluginKeys: ["geetorus.kubernetes-sandbox-provider"],
        startWorker,
      },
    });
  ...
  it("lets callers suppress shared error-state writes on activation failure", async () => {
    const plugin = {
      ...createPlugin("ready"),
      packageName: "@geetorusai/missing-sandbox-provider",
      packagePath: null,
      version: "1.0.0",
    };
  ...
  it("tears down partially-activated runtime state when error writes are suppressed", async () => {
    ...
    try {
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "@geetorusai/daytona-sandbox-provider",
          version: "1.0.0",
          geetorusPlugin: { manifest: "manifest.mjs" },
        }),
      );
      fs.writeFileSync(
      ...
      const plugin = {
        ...createPlugin("ready"),
        packageName: "@geetorusai/daytona-sandbox-provider",
  ...
  it("bounds slow managed bundled recovery attempts without serial waits", async () => {
    ...
    try {
      ...
      const driversPromise = listReadyPluginEnvironmentDrivers({
        ...
        workerManager: worker.workerManager,
        recoverMissingWorker: {
          pluginKeys: [PLUGIN_KEY, "geetorus.modal-sandbox-provider"],
          startWorker,
          timeoutMs: 25,
        },
      });
