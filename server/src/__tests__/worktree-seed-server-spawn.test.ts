...
const originalConfig = process.env.GEETORUS_CONFIG;
const originalWorktreesDir = process.env.GEETORUS_WORKTREES_DIR;
...
afterEach(async () => {
  if (originalConfig === undefined) delete process.env.GEETORUS_CONFIG;
  else process.env.GEETORUS_CONFIG = originalConfig;
  if (originalWorktreesDir === undefined) delete process.env.GEETORUS_WORKTREES_DIR;
  else process.env.GEETORUS_WORKTREES_DIR = originalWorktreesDir;
...
describe("managed worktree seed source through the server spawn path", () => {
  it("re-derives an ambient-instance manifest written before provisioning", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-server-seed-source-"));
    ...
    const registeredConfigPath = path.join(repoRoot, ".geetorus", "config.json");
...
const stateDir = path.join(cwd, ".geetorus");
...
const instanceRoot = path.join(process.env.GEETORUS_WORKTREES_DIR, "instances", instanceId);
...
fs.writeFileSync(path.join(stateDir, ".env"), [
  "GEETORUS_HOME=" + JSON.stringify(process.env.GEETORUS_WORKTREES_DIR),
  "GEETORUS_INSTANCE_ID=" + JSON.stringify(instanceId),
  "GEETORUS_CONFIG=" + JSON.stringify(targetConfigPath),
  "",
].join("\\n"));
fs.writeFileSync(path.join(stateDir, "seed-manifest.json"), JSON.stringify({
  version: 2,
  source: { instanceId: "ambient-instance", configPath: process.env.GEETORUS_CONFIG },
  snapshotAt: null,
  seedMode: "minimal",
...
`,
    ...
    process.env.GEETORUS_CONFIG = ambientConfigPath;
    process.env.GEETORUS_WORKTREES_DIR = worktreeHome;
    ...
    expect(readWorktreeSeedManifest(path.join(workspace.cwd, ".geetorus", "config.json"))).toMatchObject({
      source: { instanceId: "ambient-instance", configPath: ambientConfigPath },
      state: "pending",
    });

    await expect(ensureWorktreeSeeded({
      config: path.join(workspace.cwd, ".geetorus", "config.json"),
