...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Geetorus Agent Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/geetorus");
    expect(text).toContain("Suggested Geetorus base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("geetorusApiUrl");
    ...
    expect(text).toContain("agentDefaultsPayload.geetorusApiUrl");
    expect(text).toContain("hermes_local");
    expect(text).toContain("Hermes-originated Geetorus API usage");
    ...
    expect(text).toContain("Do not rotate or invent a Geetorus key manually");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.geetorusApiUrl");
    expect(text).toContain("GEETORUS_API_KEY");
    ...
    expect(text).toContain("Decide which Geetorus adapter type matches your runtime.");
  });
  ...
  it("includes LAN candidates when the advertised host is tailnet-only", () => {
    ...
    try {
      const req = buildReq("geetorus.example.test:3103");
      const invite = {
        id: "invite-4",
        companyId: "company-1",
        inviteType: "company_join",
      ...
      const text = buildInviteOnboardingTextDocument(req, "token-999", invite as any, {
        ...
        deploymentExposure: "private",
        bindHost: "0.0.0.0",
        allowedHostnames: ["geetorus.example.test", "203.0.113.42"],
      });
