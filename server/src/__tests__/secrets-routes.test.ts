...
describe("secret routes", () => {
  ...
  it("previews provider vault discovery and logs only aggregate metadata", async () => {
      ...
      candidates: [
        {
          ...
          sampleCount: 2,
          samples: [
            { name: "geetorus/prod-use1/company-1/openai", hasKmsKey: false, tagKeys: ["environment"] },
          ],
          signals: {
            namespace: "prod-use1",
            secretNamePrefix: "geetorus",
    ...
    expect(JSON.stringify(mockLogActivity.mock.calls)).not.toContain("geetorus/prod-use1/company-1/openai");
  });
  ...
  it("returns actionable sanitized provider vault discovery errors", async () => {
    ...
    expect(res.body.details.actionableMessage).toContain("Geetorus server runtime/provider credential path");
        ...
        secrets: [
          {
            ...
            name: "OpenAI API key",
            key: "openai-api-key",
            description: "Operator-entered Geetorus description",
          },
        ],
      });
