...
describe("resolveServerDevWatchIgnorePaths", () => {
  it("includes both the worktree UI paths and their real shared targets", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-dev-watch-"));
    const sharedUiRoot = path.join(tempRoot, "shared-ui");
    const worktreeRoot = path.join(tempRoot, "repo", ".geetorus", "worktrees", "PAP-884");
    ...
    const sharedWorktreesRoot = path.join(tempRoot, "repo", ".geetorus", "worktrees");
