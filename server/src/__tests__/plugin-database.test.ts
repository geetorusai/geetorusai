...
import type { GeetorusPluginManifestV1 } from "@geetorusai/shared";
import {
...
const multiMigrationPluginKey = "geetorus.dbfixture";
const llmWikiPluginKey = "geetorusai.plugin-llm-wiki";
...
describe("buildPluginWorkerEnv", () => {
  ...
  it("passes only model provider keys through to environment driver plugins", () => {
    ...
    expect(env).toEqual({
      ...
      GEETORUS_DEPLOYMENT_EXPOSURE: "public",
      ANTHROPIC_API_KEY: "anthropic-token",
      OPENAI_API_KEY: "openai-token",
    });
  });
  ...
  it.each([
    { packagePath: null, packageName: "@geetorusai/plugin-createos", driverKey: "createos", allowed: true },
    { packagePath: "/app/packages/plugins/sandbox-providers/createos", packageName: "@geetorusai/plugin-createos", driverKey: "createos", allowed: true },
    { packagePath: "/home/operator/plugins/fake-createos", packageName: "@geetorusai/plugin-createos", driverKey: "createos", allowed: false },
    { packagePath: null, packageName: "@acme/plugin-createos", driverKey: "createos", allowed: false },
    { packagePath: null, packageName: "@geetorusai/plugin-createos", driverKey: "daytona", allowed: false },
  ...
  it("passes a first-party sandbox provider's documented credential env var to its own worker", () => {
    const env = buildPluginWorkerEnv({
      ...
      packageName: "@geetorusai/plugin-daytona",
      packagePath: null,
      instanceInfo,
      processEnv: {
  ...
  it("does not pass the credential to a local plugin that self-declares the first-party name", () => {
    const env = buildPluginWorkerEnv({
      ...
      packageName: "@geetorusai/plugin-daytona",
      packagePath: "/home/operator/.geetorus/plugins/fake-daytona",
      trustedLocalPluginRoots: ["/app/packages/plugins"],
      instanceInfo,
      processEnv: {
        DAYTONA_API_KEY: "daytona-token",
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["geetorus.dbtest", "geetorus.escape", "geetorus.refresh", multiMigrationPluginKey, llmWikiPluginKey]) {
  ...
  async function createPluginPackage(manifest: GeetorusPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-plugin-package-"));
  ...
  function llmWikiManifest(): GeetorusPluginManifestV1 {
    return {
      id: llmWikiPluginKey,
      apiVersion: 1,
      version: "0.1.0",
  ...
  async function createInstallablePluginPackage(
    pluginManifest: GeetorusPluginManifestV1,
    migrationSql: string,
  ) {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        geetorusPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  async function installPluginRecord(manifest: GeetorusPluginManifestV1) {
  ...
  function manifest(pluginKey = "geetorus.dbtest"): GeetorusPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("applies the bundled LLM Wiki migrations through the production validator", async () => {
    ...
    expect(constraints).not.toContain("geetorus_distillation_cursor_company_id_wiki_id_source_sco_key");
    expect(constraints).not.toContain("geetorus_distillation_work_i_company_id_wiki_id_idempotenc_key");
    expect(constraints).not.toContain("geetorus_page_bindings_company_id_wiki_id_page_path_key");
    expect(uniqueColumnSets).not.toContain("wiki_pages:company_id,wiki_id,path");
    expect(uniqueColumnSets).not.toContain("geetorus_distillation_cursors:company_id,wiki_id,source_scope,scope_key,source_kind");
    expect(uniqueColumnSets).not.toContain("geetorus_distillation_work_items:company_id,wiki_id,idempotency_key");
    expect(uniqueColumnSets).not.toContain("geetorus_page_bindings:company_id,wiki_id,page_path");
  });
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("geetorus.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
  ...
  it("refreshes persisted manifests from disk before activation", async () => {
    const staleManifest = manifest("geetorus.refresh");
    const refreshedManifest: GeetorusPluginManifestV1 = {
      ...staleManifest,
