...
describe("redaction", () => {
  it("keeps the discriminator allowlist in exact PRP v1 schema parity", () => {
    const schema = JSON.parse(
      readFileSync(
        new URL(
          "../../../packages/geetorus-runner/protocol/schemas/event.schema.json",
          import.meta.url,
        ),
        "utf8",
      ),
  ...
  it("preserves Geetorus protocol schema identifiers", () => {
    expect(
      sanitizeRecord({
        schema: "geetorus.question_set.v1",
        nested: {
          schema: "geetorus.question_response.v1",
          runtimeSchema: "geetorus.runtime_request.v2",
          arbitraryProviderValue: "geetorus.question_set.v1",
        },
      }),
    ).toEqual({
      schema: "geetorus.question_set.v1",
  ...
  it("does not trust discriminators inside a forged unknown schema", () => {
    expect(
      redactEventPayload({
        schema: "geetorus.attacker.control.v1",
        runtimeSchema: "geetorus.attacker.runtime.v1",
        schemaVersion: 1,
        eventType: "tool.execution.started",
      }),
    ).toEqual({
  ...
  it("preserves native run span identities without weakening hostname redaction", () => {
    ...
    for (const span of spanNames) {
      const input = {
        schema: "geetorus.run-performance-span.v1",
        span,
  ...
  it("redacts common secret shapes from unstructured text", () => {
    ...
    const input = [
      ...
      `geetorus {"GEETORUS_API_KEY":"geetorus-json-secret"}`,
      `escaped {\\"apiKey\\":\\"escaped-json-secret\\"}`,
      `export GEETORUS_API_KEY='geetorus-shell-secret'`,
    ...
    expect(result).not.toContain("geetorus-json-secret");
    expect(result).not.toContain("escaped-json-secret");
    expect(result).not.toContain("geetorus-shell-secret");
...
second-line\" status=401`,
  ...
  it("redacts inline secrets from command metadata without hiding safe command text", () => {
    ...
    expect(result?.env).toEqual({
      GEETORUS_RESOLVED_COMMAND: `env OPENAI_API_KEY=${REDACTED_EVENT_VALUE} custom-acp --token ${REDACTED_EVENT_VALUE}`,
      SAFE_VALUE: "visible",
    });
  });
