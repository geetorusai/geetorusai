...
describeEmbeddedPostgres("secret proposal routes", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-secret-proposals-${randomUUID()}`);
  ...
  afterAll(async () => {
    await stopDb?.();
    if (previousKeyFile === undefined) delete process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
    else process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = previousKeyFile;
  ...
  it("uses the selected provider vault when approving a secret proposal", async () => {
    ...
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/prod-use1/proposed-token";
  ...
  it("requires agent JWT and atomically cascade-approves a secret plus binding with dual audits", async () => {
    ...
    expect(registeredRun.contextSnapshot).toMatchObject({
      geetorusSecretRedactions: [expect.objectContaining({ fingerprintSha256: expect.any(String), material: expect.any(Object) })],
    });
  ...
  it("serializes binding creation with concurrent dependency rejection", async () => {
    ...
    await db.execute(sql.raw(`
      CREATE OR REPLACE FUNCTION geetorus_test_pause_binding_proposal()
      RETURNS trigger
      LANGUAGE plpgsql
      AS $function$
      BEGIN
      ...
      END
      $function$;
      CREATE TRIGGER geetorus_test_pause_binding_proposal
      BEFORE INSERT ON company_secret_proposals
      FOR EACH ROW EXECUTE FUNCTION geetorus_test_pause_binding_proposal();
    `));
    ...
    try {
      ...
      await db.execute(sql.raw(`
        DROP TRIGGER IF EXISTS geetorus_test_pause_binding_proposal ON company_secret_proposals;
        DROP FUNCTION IF EXISTS geetorus_test_pause_binding_proposal();
      `));
    }
  ...
  it("holds the org graph stable until binding approval commits", async () => {
    ...
    await db.execute(sql.raw(`
      CREATE OR REPLACE FUNCTION geetorus_test_pause_secret_approval()
      RETURNS trigger
      LANGUAGE plpgsql
      AS $function$
      BEGIN
      ...
      END
      $function$;
      CREATE TRIGGER geetorus_test_pause_secret_approval
      BEFORE INSERT ON company_secrets
      FOR EACH ROW EXECUTE FUNCTION geetorus_test_pause_secret_approval();
    `));
    ...
    try {
      ...
      await db.execute(sql.raw(`
        DROP TRIGGER IF EXISTS geetorus_test_pause_secret_approval ON company_secrets;
        DROP FUNCTION IF EXISTS geetorus_test_pause_secret_approval();
      `));
    }
