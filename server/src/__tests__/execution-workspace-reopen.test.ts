...
describeEmbeddedPostgres("reopen archived isolated execution workspace", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-reopen-");
  ...
  async function makeExistingDir(): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), "geetorus-reopen-cwd-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  async function seedCompanyProject() {
    ...
    await db.insert(projectWorkspaces).values({
      ...
      cwd: "/tmp/geetorus-reopen-project",
      isPrimary: true,
    });
  ...
  it("fails closed and keeps the row closed when the rebuild fails", async () => {
    ...
    const missingDir = join(tmpdir(), `geetorus-reopen-missing-${randomUUID()}`);
  ...
  it("resolves the managed base checkout for a git_worktree row when the project workspace cwd is null", async () => {
    ...
    const tempHome = await mkdtemp(join(tmpdir(), "geetorus-reopen-home-"));
