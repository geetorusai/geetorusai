...
describe("HTTP body limits", () => {
  ...
  it("lets operators override the zip upload cap via GEETORUS_IMPORT_ZIP_MAX_BYTES", async () => {
    vi.stubEnv(
      "GEETORUS_IMPORT_ZIP_MAX_BYTES",
  ...
  it("falls back to the default when the env override is not a usable number", async () => {
    for (const raw of ["unlimited", "0", "-5", "0.5"]) {
      vi.stubEnv("GEETORUS_IMPORT_ZIP_MAX_BYTES", raw);
      vi.resetModules();
      const reloaded = await import("../http/body-limits.js");
      expect(
  ...
  it("clamps an oversized override so derived guard math cannot overflow", async () => {
    vi.stubEnv("GEETORUS_IMPORT_ZIP_MAX_BYTES", String(Number.MAX_VALUE));
    vi.resetModules();
    const reloaded = await import("../http/body-limits.js");
    expect(reloaded.PORTABLE_ZIP_UPLOAD_LIMIT_BYTES).toBe(
      64 * 1024 * 1024 * 1024,
