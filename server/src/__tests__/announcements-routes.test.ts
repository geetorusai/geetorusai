...
import { activityLog, announcementDismissals, announcementPublications, companies, createDb, startEmbeddedPostgresTestDatabase, type EmbeddedPostgresTestDatabase } from "@geetorusai/db";
...
describe("announcement routes and durable dismissals", () => {
  ...
  beforeAll(async () => {
    database = await startEmbeddedPostgresTestDatabase("geetorus-announcements-");
