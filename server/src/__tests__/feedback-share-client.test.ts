...
describe("feedback trace share client", () => {
  ...
  it("defaults to telemetry.geetorus.ing when no backend url is configured", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://telemetry.geetorus.ing/feedback-traces",
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    const client = createFeedbackTraceShareClientFromConfig({
      feedbackExportBackendUrl: "https://telemetry.geetorus.ing",
    ...
    expect(call?.[0]).toBe("https://telemetry.geetorus.ing/feedback-traces");
