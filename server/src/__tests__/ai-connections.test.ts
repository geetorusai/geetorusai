...
beforeAll(async () => {
  home = await mkdtemp(path.join(os.tmpdir(), "geetorus-ai-tests-"));
  vi.stubEnv("GEETORUS_HOME", home);
  vi.stubEnv("GEETORUS_INSTANCE_ID", "ai-connection-fixture");
  database = await startEmbeddedPostgresTestDatabase("geetorus-ai-db-");
...
describe("managed AI connections", () => {
  ...
  it("enforces the shared transport discriminator and existing harness compatibility", () => {
    ...
    expect(isAiConnectionCompatible({ provider: "anthropic", method: "api_key", mode: "responsible_user" }, "geetorus_runner", "same-model", "acpx", "claude")).toBe(true);
    expect(isAiConnectionCompatible(binding, "geetorus_runner", "same-model", "acpx", "claude")).toBe(true);
    expect(isAiConnectionCompatible(binding, "geetorus_runner", "same-model", "acpx", "codex")).toBe(false);
