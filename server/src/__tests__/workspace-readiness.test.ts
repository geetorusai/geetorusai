...
import type { WorkspaceReadinessProbeResult } from "@geetorusai/shared";
import {
...
function createMarkerDir(files: Record<string, string> = {}) {
  const dir = mkdtempSync(path.join(os.tmpdir(), "geetorus-workspace-readiness-"));
  tempDirs.push(dir);
...
describe("resolveWorkspaceReadiness", () => {
  ...
  it("resolves the marker directory from the configured config path", () => {
    ...
    expect(resolveWorkspaceSeedMarkerDir({ GEETORUS_CONFIG: configPath })).toBe(dir);
  });

  it("does not re-stat marker files on every health request", () => {
    const { configPath } = createMarkerDir({ "seed-manifest.json": verifiedManifest() });
    const env = { GEETORUS_CONFIG: configPath };
    let clock = 0;
  ...
  it("only treats a process with clone evidence as a managed workspace", () => {
    const { configPath } = createMarkerDir();
    expect(isManagedWorkspaceInstance({ GEETORUS_CONFIG: configPath })).toBe(false);
    expect(
      isManagedWorkspaceInstance({ GEETORUS_CONFIG: configPath, [WORKSPACE_HANDOFF_KEY_ENV_KEY]: "key" }),
    ).toBe(true);
    const seeded = createMarkerDir({ "seed-manifest.json": verifiedManifest() });
    expect(isManagedWorkspaceInstance({ GEETORUS_CONFIG: seeded.configPath })).toBe(true);
  });
});
...
describe("probeManagedWorkspaceReadiness", () => {
  ...
  it("fails closed when any part of the workspace identity is unresolved", () => {
    ...
    const complete = {
      ...
      executionWorkspaceId: "ews-1",
      companyId: "company-1",
      env: { GEETORUS_WORKSPACE_HANDOFF_SECRET: "root" },
    };
  ...
  it("hands the guest only derived per-workspace material", () => {
    expect(buildManagedWorkspaceGuestEnv(identity)).toEqual({
      ...
      [WORKSPACE_EXECUTION_WORKSPACE_ID_ENV_KEY]: "ews-1",
      GEETORUS_EXECUTION_WORKSPACE_COMPANY_ID: "company-1",
    });
  });
});
...
describe("shouldBlockPublicationOnReadiness", () => {
  ...
  it("reads the deployment-level mode from the environment", () => {
    ...
    expect(resolveWorkspaceReadinessGateMode({ GEETORUS_WORKSPACE_READINESS_GATE: "STRICT" })).toBe("strict");
    expect(resolveWorkspaceReadinessGateMode({ GEETORUS_WORKSPACE_READINESS_GATE: "anything-else" })).toBe("auto");
  });
});
