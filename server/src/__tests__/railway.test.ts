...
describe("Railway governed operations", () => {
  it("binds project listing and the access probe to an explicit workspace", async () => {
    ...
    await expect(f.client.call("geetorus-railway-list-projects", {})).rejects.toMatchObject({ code: "railway_invalid_arguments" });
    expect(f.request).not.toHaveBeenCalled();
    await f.client.probe(target.projectId);
    await f.client.call("geetorus-railway-list-projects", { workspaceId: target.projectId, first: 5 });
  ...
  it.each(["structured", "sse"])("discovers workspace access from the hosted %s response without account profile scopes", async (format) => {
    ...
    const payload = { jsonrpc: "2.0", id: "geetorus-railway-workspace-probe", result: format === "structured" ? { structuredContent: data } : { content: [{ type: "text", text: JSON.stringify(data) }] } };
  ...
  it("requires exact provider identity and treats shell and remote agents as privileged", () => {
    ...
    expect(isRailwayConnection({ transport: "mcp_remote", authKind: "oauth", credentialSource: "geetorus_vault", config: { url: "https://mcp.railway.com", sourceTemplateKey: "railway", connectionMethodKey: "mcp-oauth" } })).toBe(true);
    expect(railwayRisk("railwayAgent")).toBe("destructive");
    expect(isRailwayToolBlocked("accept_deploy")).toBe(true);
    expect(railwayRisk("geetorus-railway-run-command")).toBe("destructive");
    expect(railwayRisk("unfamiliar-tool")).toBe("write");
    expect(RAILWAY_TOOLS).toHaveLength(11);
    expect(RAILWAY_TOOLS.map((tool) => tool.name)).not.toContain("geetorus-railway-deploy-revision");
  });
  ...
  it("checks full deployment membership before a single fixed mutation", async () => {
    const f = fixture();
    await expect(f.client.call("geetorus-railway-restart", target)).resolves.toEqual({ deploymentRestart: true, targetDeploymentId: target.deploymentId });
  ...
  it.each(["project", "environment", "service", "deployment"])("denies a mismatched %s before mutation", async (field) => {
    ...
    await expect(f.client.call("geetorus-railway-restart", target)).rejects.toMatchObject({ code: "railway_target_mismatch" });
  ...
  it("bounds and redacts logs without selecting variables", async () => {
    ...
    const result = await f.client.call("geetorus-railway-read-logs", { ...target, limit: 10 });
    ...
    await expect(f.client.call("geetorus-railway-read-logs", { ...target, limit: 501 })).rejects.toMatchObject({ code: "railway_invalid_arguments" });
  ...
  it("reports when a single long log line was cut", async () => {
    ...
    await expect(f.client.call("geetorus-railway-read-logs", target)).resolves.toMatchObject({ truncated: true, limitReached: false });
  });
  ...
  it("does not expose GraphQL error details or retry an ambiguous mutation", async () => {
    ...
    await expect(f.client.call("geetorus-railway-restart", target)).rejects.toMatchObject({ code: "railway_api_error" });
  ...
  it.each([
    ...
    await expect(f.client.call(`geetorus-railway-${operation}`, target)).rejects.toMatchObject({ code: "railway_operation_unconfirmed" });
    expect(f.request).toHaveBeenCalledTimes(3);
  });
  ...
  it("rejects oversized responses, cancelled calls and GraphQL passthrough", async () => {
    ...
    await expect(f.client.call("geetorus-railway-list-projects", { query: "mutation Evil" })).rejects.toMatchObject({ code: "railway_invalid_arguments" });
    f.controller.abort();
    await expect(f.client.probe(target.projectId)).rejects.toMatchObject({ name: "AbortError" });
    expect(f.request).toHaveBeenCalledTimes(1);
  });

  it.each(["geetorus-railway-deploy-revision", "geetorus_railway_deploy_revision", "geetorusRailwayDeployRevision"])("blocks %s before any repository preflight or deployment mutation", async (name) => {
  ...
  it("allows only an instance in the exact running deployment to reach SSH", async () => {
    const f = fixture();
    await expect(f.client.call("geetorus-railway-run-command", { ...target, deploymentInstanceId: target.serviceId, command: "true" })).rejects.toMatchObject({ code: "railway_target_mismatch" });
    expect(f.runCommand).not.toHaveBeenCalled();
    await f.client.call("geetorus-railway-run-command", { ...target, deploymentInstanceId: instanceId, command: "true" });
