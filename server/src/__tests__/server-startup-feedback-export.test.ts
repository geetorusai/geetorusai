...
const ORIGINAL_GEETORUS_RUNTIME_API_CANDIDATES_JSON = process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON;
...
function buildTestConfig(overrides: Record<string, unknown> = {}) {
  return {
    ...
    databaseUrl: "postgres://geetorus:geetorus@127.0.0.1:5432/geetorus",
    embeddedPostgresDataDir: "/tmp/geetorus-test-db",
    embeddedPostgresPort: 54329,
    databaseBackupEnabled: false,
    databaseBackupIntervalMinutes: 60,
    databaseBackupRetentionDays: 30,
    databaseBackupDir: "/tmp/geetorus-test-backups",
    serveUi: false,
    uiDevMiddleware: false,
    secretsProvider: "local_encrypted",
    secretsStrictMode: false,
    secretsMasterKeyFilePath: "/tmp/geetorus-master.key",
    storageProvider: "local_disk",
    storageLocalDiskBaseDir: "/tmp/geetorus-storage",
    storageS3Bucket: "geetorus-test",
...
describe("startServer feedback export wiring", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.GEETORUS_DECISION_SIGNING_SECRET = "fedcba9876543210fedcba9876543210";
    process.env.GEETORUS_AGENT_JWT_SECRET = "0123456789abcdef0123456789abcdef";
  ...
  it("starts without GEETORUS_DECISION_SIGNING_SECRET by generating a persisted key", async () => {
    const originalHome = process.env.GEETORUS_HOME;
    const originalInstanceId = process.env.GEETORUS_INSTANCE_ID;
    const tempHome = mkdtempSync(path.join(tmpdir(), "geetorus-decision-key-"));
    process.env.GEETORUS_HOME = tempHome;
    process.env.GEETORUS_INSTANCE_ID = "default";
    delete process.env.GEETORUS_DECISION_SIGNING_SECRET;
    try {
      ...
      if (originalHome === undefined) delete process.env.GEETORUS_HOME;
      else process.env.GEETORUS_HOME = originalHome;
      if (originalInstanceId === undefined) delete process.env.GEETORUS_INSTANCE_ID;
      else process.env.GEETORUS_INSTANCE_ID = originalInstanceId;
  ...
  it("repairs permissive permissions on an existing generated decision signing key", async () => {
    const originalHome = process.env.GEETORUS_HOME;
    const originalInstanceId = process.env.GEETORUS_INSTANCE_ID;
    const tempHome = mkdtempSync(path.join(tmpdir(), "geetorus-decision-key-mode-"));
    const keyPath = path.join(tempHome, "instances", "default", "secrets", "decision-signing.key");
  ...
  it("refuses a symlink planted as the generated decision signing key", async () => {
    ...
    const originalHome = process.env.GEETORUS_HOME;
    const originalInstanceId = process.env.GEETORUS_INSTANCE_ID;
    const tempHome = mkdtempSync(path.join(tmpdir(), "geetorus-decision-key-symlink-"));
    const keyPath = path.join(tempHome, "instances", "default", "secrets", "decision-signing.key");
  ...
  it("refuses startup when an explicit decision signing secret is too short", async () => {
    process.env.GEETORUS_DECISION_SIGNING_SECRET = "too-short";
    await expect(startServer()).rejects.toThrow("GEETORUS_DECISION_SIGNING_SECRET must be at least 32 characters");
  ...
  it("refuses authenticated public startup when DATABASE_URL is not a postgres URL", async () => {
    loadConfigMock.mockReturnValue(buildTestConfig({
      ...
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://tenant.example.com",
      databaseUrl: "secret://geetorus-cloud/stacks/alpha/database/runtime-url",
    }));
...
describe("startServer GEETORUS_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.GEETORUS_DECISION_SIGNING_SECRET = "fedcba9876543210fedcba9876543210";
    loadConfigMock.mockReturnValue(buildTestConfig());
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_GEETORUS_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_GEETORUS_RUNTIME_API_CANDIDATES_JSON;
    }
  ...
  it("uses the externally set GEETORUS_API_URL when provided", async () => {
    process.env.GEETORUS_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.GEETORUS_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when GEETORUS_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.GEETORUS_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("keeps loopback as the runtime API URL when allowed hostnames are present", async () => {
    ...
    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.GEETORUS_RUNTIME_API_URL).toBe("http://127.0.0.1:3210");
    expect(process.env.GEETORUS_API_URL).toBe("http://127.0.0.1:3210");
    expect(JSON.parse(process.env.GEETORUS_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
  ...
  it("preserves explicit-port external auth public URLs when detect-port selects a new port", async () => {
    ...
    // URL that leaked to spawned agents as a dead GEETORUS_API_URL. (BRO-1558)
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("http://my-host.ts.net:3100");
    expect(process.env.GEETORUS_RUNTIME_API_URL).toBe("http://my-host.ts.net:3100");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    loadConfigMock.mockReturnValueOnce(buildTestConfig({
      port: 3100,
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://geetorus.example",
    }));
    detectPortMock.mockResolvedValueOnce(3110);
    ...
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("https://geetorus.example");
    expect(process.env.GEETORUS_RUNTIME_API_URL).toBe("https://geetorus.example");
  });
});
