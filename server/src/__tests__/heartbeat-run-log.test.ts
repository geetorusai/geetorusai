...
describe("compactRunLogChunk", () => {
  ...
  it("truncates oversized chunks after sanitizing them", () => {
    ...
    expect(compacted).toContain("[geetorus truncated run log chunk:");
    expect(compacted.endsWith("tail")).toBe(true);
  });

  it("redacts Geetorus credential shapes before persisting run-log chunks", () => {
    const chunk = [
      "Authorization: Bearer live-bearer-token-value",
      `export GEETORUS_API_KEY='geetorus-shell-secret'`,
      `auth {"refresh_token":"refresh-token-fixture-secret"}`,
      `payload {"GEETORUS_API_KEY":"geetorus-json-secret"}`,
      "--geetorus-api-key=geetorus-flag-secret",
    ].join("\n");
    ...
    expect(compacted).not.toContain("geetorus-shell-secret");
    expect(compacted).not.toContain("refresh-token-fixture-secret");
    expect(compacted).not.toContain("geetorus-json-secret");
    expect(compacted).not.toContain("geetorus-flag-secret");
  });
});
