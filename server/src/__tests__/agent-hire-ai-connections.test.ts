...
import { agents, companies, companyMemberships, createDb, heartbeatRuns, issues, principalPermissionGrants, toolConnectionInstalls } from "@geetorusai/db";
...
beforeAll(async () => {
  home = await mkdtemp(path.join(os.tmpdir(), "geetorus-hire-ai-"));
  vi.stubEnv("GEETORUS_HOME", home);
  vi.stubEnv("GEETORUS_INSTANCE_ID", "hire-ai");
  database = await startEmbeddedPostgresTestDatabase("geetorus-hire-ai-db-");
...
describe("agent-created hires use managed AI connections", () => {
  ...
  for (const endpoint of ["agent-hires", "agents"]) {
    it.each([
      ...
      ["anthropic", "geetorus_runner", { provider: "codex" }, "ANTHROPIC_API_KEY"],
      ["openai", "geetorus_runner", { provider: "acpx", acpxAgent: "claude" }, "OPENAI_API_KEY"],
  ...
  it.each(["anthropic", "openai"] as const)("inherits %s when the hire uses the native runner", async (provider) => {
    ...
    const agent = hired(await request(f.app).post(`/api/companies/${f.companyId}/agent-hires`).send({ name: "Native teammate", role: "engineer", adapterType: "geetorus_runner", adapterConfig: provider === "anthropic" ? { provider: "acpx", acpxAgent: "claude" } : { provider: "codex" } }));
    expect(agent.runtimeConfig.aiConnection).toEqual(f.binding);
    const runtime = await prepareManagedAiRuntime(db, { companyId: f.companyId, agentId: agent.id, responsibleUserId: f.userId, adapterType: agent.adapterType, binding: agent.runtimeConfig.aiConnection, config: agent.adapterConfig });
