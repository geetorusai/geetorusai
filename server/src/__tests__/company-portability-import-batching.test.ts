...
// The `.geetorus.yaml` extension is consumed by the importer's own hand-rolled
...
function renderGeetorusYaml(value: Record<string, unknown>): string {
...
function buildSyntheticBundle(options: SyntheticBundleOptions): {
  ...
  files[".geetorus.yaml"] = renderGeetorusYaml({ schemaVersion: 7, tasks });
...
describeEmbeddedPostgres("company import batches inserts", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-import-batching-");
  ...
  it("preserves bundle timestamps and parent hierarchy through a real import", async () => {
    ...
    files[".geetorus.yaml"] = renderGeetorusYaml({
      schemaVersion: 7,
      tasks: {
