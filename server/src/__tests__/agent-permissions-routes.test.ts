...
function registerModuleMocks() {
  vi.doMock("@geetorusai/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@geetorusai/adapter-opencode-local/server")>("@geetorusai/adapter-opencode-local/server");
    return {
      ...actual,
    ...
    const res = await requestApp(app, (baseUrl) => request(baseUrl)
      ...
      .send({
        adapterConfig: {
          workspaceStrategy: {
            type: "git_worktree",
            provisionCommand: "touch /tmp/geetorus-rce",
          },
        },
      }));
