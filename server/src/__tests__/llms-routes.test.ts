...
describe("llm routes", () => {
  ...
  it("documents timer heartbeats as opt-in for new hires", async () => {
    ...
    expect(res.text).toContain("Use the geetorus-create-agent skill for end-to-end hiring");
  ...
  it("serves static Hermes Gateway configuration docs before the plugin is installed", async () => {
    ...
    expect(res.text).toContain("agentDefaultsPayload.geetorusApiUrl");
    expect(res.text).toContain("hermes_local");
    expect(res.text).toContain("Hermes-originated Geetorus API usage");
