...
import { createDb, instanceSettings } from "@geetorusai/db";
import {
...
import { geetorusCloudConnectorEnrollmentStatus } from "../services/geetorus-cloud-connector-enrollment.js";
...
const POOL_ORIGIN = "https://pool-123.staging.geetorus.app";
const CANONICAL_ORIGIN = "https://gonzo.staging.geetorus.app";
...
describeEmbeddedPostgres("Cloud runtime identity", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-cloud-runtime-identity-");
  ...
  beforeEach(async () => {
    ...
    resetCloudRuntimeIdentityForTests();
    process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN = "test-tenant-server-token";
    process.env.GEETORUS_CLOUD_STACK_ID = STACK_ID;
    process.env.GEETORUS_CLOUD_API_ORIGIN = "https://my-staging.geetorus.app";
    process.env.GEETORUS_PUBLIC_URL = POOL_ORIGIN;
    process.env.GEETORUS_AUTH_PUBLIC_BASE_URL = POOL_ORIGIN;
    process.env.GEETORUS_API_URL = POOL_ORIGIN;
    process.env.GEETORUS_PRIMARY_HOST = "pool-123.staging.geetorus.app";
    process.env.GEETORUS_STACK_SLUG = "pool-123";
    process.env.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS = JSON.stringify({ keys: [publicJwk] });
    process.env.GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID = "managed-instance";
    process.env.GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY = "managed-signing-key";
    process.env.GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY = "managed-sealing-key";
    process.env.GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT = "staging";
    process.env.GEETORUS_CLOUD_CONNECTOR_BASE_URL = "https://my-staging.geetorus.app";
    await initializeCloudRuntimeIdentity(db);
  });

  afterEach(() => {
    for (const key of [
      "GEETORUS_CLOUD_TENANT_SERVER_TOKEN",
      "GEETORUS_CLOUD_STACK_ID",
      "GEETORUS_CLOUD_API_ORIGIN",
      "GEETORUS_PUBLIC_URL",
      "GEETORUS_AUTH_PUBLIC_BASE_URL",
      "GEETORUS_API_URL",
      "GEETORUS_PRIMARY_HOST",
      "GEETORUS_STACK_SLUG",
      "GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS",
      ...
      "GEETORUS_CLOUD_CONNECTOR_BASE_URL",
      "GEETORUS_RUNTIME_API_CANDIDATES_JSON",
    ]) {
  ...
  it("persists and immediately applies a valid one-time claim", async () => {
    ...
    expect(process.env.GEETORUS_PUBLIC_URL).toBe(CANONICAL_ORIGIN);
    expect(process.env.GEETORUS_AUTH_PUBLIC_BASE_URL).toBe(CANONICAL_ORIGIN);
    expect(process.env.GEETORUS_API_URL).toBe(CANONICAL_ORIGIN);
    expect(process.env.GEETORUS_PRIMARY_HOST).toBe("gonzo.staging.geetorus.app");
    expect(process.env.GEETORUS_STACK_SLUG).toBe("gonzo");
    expect(routineWebhookUrl("hook-1")).toBe(
      "https://gonzo.staging.geetorus.app/api/routine-triggers/public/hook-1/fire",
    );
  ...
  it("applies the assertion on the existing health request and acknowledges the exact origin", async () => {
    ...
    const response = await request(app)
      .get("/api/health")
      .set("x-geetorus-cloud-runtime-identity", assertion({
  ...
  it("accepts the identical claim after a restart with already-aligned provider variables", async () => {
    await applyCloudRuntimeIdentityAssertion({ db, compactJws: assertion(), now: NOW });
    resetCloudRuntimeIdentityForTests();
    process.env.GEETORUS_PUBLIC_URL = CANONICAL_ORIGIN;
    process.env.GEETORUS_AUTH_PUBLIC_BASE_URL = CANONICAL_ORIGIN;
    process.env.GEETORUS_API_URL = CANONICAL_ORIGIN;
    await initializeCloudRuntimeIdentity(db);
  ...
  it.each([
    ...
    ["wrong previous origin", { previousOrigin: "https://another.staging.geetorus.app" }],
