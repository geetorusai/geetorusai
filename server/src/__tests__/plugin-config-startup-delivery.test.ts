...
import { companies, createDb, pluginConfig, plugins } from "@geetorusai/db";
...
describeEmbeddedPostgres("registry.listConfigs (startup config delivery)", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-config-delivery-");
  ...
  async function seedPlugin(pluginKey: string, installOrder: number) {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey,
      packageName: `@geetorusai/${pluginKey}`,
      version: "0.0.1",
      apiVersion: 1,
  ...
  it("returns every company-scoped config row for a plugin", async () => {
    ...
    const pluginId = await seedPlugin("geetorus.gateway-test", 1);
  ...
  it("only returns rows for the requested plugin (no cross-plugin bleed)", async () => {
    const registry = pluginRegistryService(db);
    const pluginId = await seedPlugin("geetorus.gateway-test", 1);
    const otherPluginId = await seedPlugin("geetorus.other-test", 2);
    const companyA = await seedCompany();
