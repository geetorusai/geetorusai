...
const mockExecuteGeetorusNativeSession = vi.hoisted(() =>
  vi.fn<
    typeof import("../services/native-runtime/native-session-executor.js").executeGeetorusNativeSession
  >(),
);
...
vi.mock("../services/native-runtime/native-session-executor.js", async () => {
  ...
  mockExecuteGeetorusNativeSession.mockImplementation(
    actual.executeGeetorusNativeSession,
  );
  ...
  return {
    ...
    executeGeetorusNativeSession: mockExecuteGeetorusNativeSession,
...
vi.mock("@geetorusai/shared/telemetry", async () => {
  const actual = await vi.importActual<
    typeof import("@geetorusai/shared/telemetry")
  >("@geetorusai/shared/telemetry");
  return {
    ...actual,
...
import { GeetorusControlPlanePort } from "../services/native-runtime/geetorus-control-plane-port.js";
...
import * as geetorusRunner from "../vendor/geetorus-runner/index.js";
import {
  CONTROL_PLANE_CONFORMANCE_OPEN,
  CONTROL_PLANE_CONFORMANCE_RESULT,
  CONTROL_PLANE_CONFORMANCE_TERMINAL,
} from "../vendor/geetorus-runner/testing.js";
...
const externalTestDatabaseUrl = process.env.GEETORUS_TEST_DATABASE_URL?.trim();
...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it.each(["settled", "rejected", "late_callback"] as const)(
    ...
    async (outcome) => {
      await withTempGeetorusHome(async () => {
        ...
        const port = new GeetorusControlPlanePort(db, {
          companyId,
          issueId,
          runId,
          agentId,
        ...
        try {
          ...
          expect(mockExecuteGeetorusNativeSession).not.toHaveBeenCalled();
          release();
  ...
  async function withTempGeetorusHome<T>(
    fn: (home: string) => Promise<T>,
  ): Promise<T> {
    const home = await fs.mkdtemp(
      path.join(os.tmpdir(), "geetorus-hot-restart-"),
    );
  ...
  it("fences native selection when cancellation wins during preparation", async () => {
    await withTempGeetorusHome(async () => {
      ...
      await db.update(agents).set({ adapterType: "geetorus_runner",
  ...
  it("dispatches local native external chat inside the server-selected task root", async () => {
    await withTempGeetorusHome(async () => {
      const { companyId, agentId, issueId, runId } =
        await seedQueuedIssueRunFixture();
      await fs.mkdir(resolveGeetorusInstanceRoot(), { recursive: true });
      await db
        .update(agents)
        .set({
          adapterType: "geetorus_runner",
      ...
      expect(input.workspace.cwd).toBe(
        path.join(
          await fs.realpath(resolveGeetorusInstanceRoot()),
          "chat-workspaces",
          companyId,
          agentId,
          issueId,
    ...
    await withTempGeetorusHome(async () => {
      ...
      const nativeExecutionInput = buildNativeExecutionInput({
        ...
        completionContract: {
          ...
          schemaVersion: "geetorus.run-result.v1",
          contract: {
            revision: "1",
      ...
      expect(mockTerminateLocalService).not.toHaveBeenCalled();
      await expect(
        fs.stat(path.join(resolveGeetorusInstanceRoot(), "chat-workspaces")),
  ...
  it("snapshots and drains a server-stdio ACP run before embedded database shutdown", async () => {
    ...
    await withTempGeetorusHome(async (home) => {
      await writeHotRestartIntent({
        previousServerPid: process.pid,
  ...
  it("suspends native Geetorus Runner ownership on graceful restart without cancelling or creating a retry run", async () => {
    const { agentId, runId, issueId, wakeupRequestId } = await seedRunFixture({
      adapterType: "geetorus_runner",
      agentStatus: "running",
      runtimeMode: "native",
  ...
  it("schedules an infra retry for a setup failure caused by a transient sandbox provider worker restart", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.kubernetes-sandbox-provider",
      packageName: "@geetorusai/kubernetes-sandbox-provider",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.kubernetes-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("classifies only the installed-but-not-ready sandbox provider plugin message as a configuration gap", () => {
    expect(
      parseSandboxProviderPluginNotReadyFailureMessage(
        'Sandbox provider "kubernetes" is installed via plugin "geetorus.kubernetes-sandbox-provider", but that plugin is currently error.',
      ),
    ).toEqual({
      provider: "kubernetes",
      pluginKey: "geetorus.kubernetes-sandbox-provider",
    ...
    expect(
      parseSandboxProviderPluginNotReadyFailureMessage(
        'Failed to acquire lease: Sandbox provider "daytona" is installed via plugin "geetorus.daytona-sandbox-provider", but that plugin is currently upgrade_pending.',
      ),
    ...
    expect(
      parseSandboxProviderPluginNotReadyFailureMessage(
        'Sandbox provider "kubernetes" is installed via plugin "geetorus.kubernetes-sandbox-provider", but its worker is not running.',
      ),
    ).toBeNull();
  ...
  it("blocks the issue instead of re-dispatching when the sandbox provider plugin is stuck in error", async () => {
    ...
    expect(failedRun?.resultJson).toMatchObject({
      configurationIncomplete: {
        ...
        pluginStatus: "error",
        fingerprint:
          "sandbox_provider_plugin:geetorus.kubernetes-sandbox-provider:error",
      },
    });
    ...
    const notice = await waitForValue(async () => {
      ...
      return (
        rows.find((comment) =>
          comment.body.includes("geetorus.kubernetes-sandbox-provider"),
        ) ?? null
      );
    });
  ...
  it("blocks a git-sensitive local adapter before launch when a project-workspace-linked issue is missing its project id", async () => {
    ...
    await db.insert(projectWorkspaces).values({
      ...
      cwd: `/tmp/geetorus-missing-workspace-${randomUUID()}`,
      isPrimary: true,
    });
    await db
      .update(issues)
  ...
  it("dispatches interrupted CLI input after the executor releases its lease", async () => {
    ...
    const [wake] = await db.insert(agentWakeupRequests).values({
      ...
      payload: { issueId, commentId: comment!.id, _geetorusWakeContext: { issueId, wakeReason: "issue_commented", wakeCommentIds: [comment!.id] } },
    }).returning();
  ...
  it.each(["pending", "discarded", "wrong queue"] as const)(
    ...
    async (state) => {
      ...
      const [deferred] = await db.insert(agentWakeupRequests).values({
        ...
        requestedByActorType: "user", requestedByActorId: "responsible-user",
        payload: { issueId, commentId: commentIds[0], _geetorusWakeContext: {
      ...
      await db.insert(agentWakeupRequests).values({
        ...
        payload: { issueId, commentId: otherComment!.id, _geetorusWakeContext: {
  ...
  it("preserves deferred input on a clean Stop and adopts it once on the next explicit comment", async () => {
    ...
    const [deferred] = await db.insert(agentWakeupRequests).values({ companyId, agentId, source: "automation", reason: "issue_execution_deferred", status: "deferred_issue_execution",
      payload: { issueId, commentId: pending!.id, _geetorusWakeContext: { issueId, wakeReason: "issue_commented", wakeCommentIds: [pending!.id] } },
    }).returning();
  ...
  it("lets a child waiting for the shared workspace run before recovering its lead", async () => {
    ...
    await db.update(heartbeatRuns).set({ contextSnapshot: { issueId, geetorusWorkspace: { mode: "shared_workspace" } } }).where(eq(heartbeatRuns.id, runId));
