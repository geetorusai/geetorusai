...
import type { Issue, GeetorusPluginManifestV1 } from "@geetorusai/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: GeetorusPluginManifestV1["capabilities"]): GeetorusPluginManifestV1 {
  return {
    id: "geetorus.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:geetorus.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:geetorus.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:geetorus.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:geetorus.test-orchestration");
    await expect(
  ...
  it("supports generic plugin operation issue visibility in the test harness", async () => {
    ...
    expect(created.originKind).toBe("plugin:geetorus.test-orchestration:operation");
