...
function tenantHeaders(input: { stackId: string; userId: string; companyName?: string }) {
  const headers: Record<string, string> = {
    "x-geetorus-cloud-tenant-token": SERVER_TOKEN,
    "x-geetorus-cloud-user-id": input.userId,
    "x-geetorus-cloud-user-email": `${input.userId}@example.com`,
    "x-geetorus-cloud-stack-id": input.stackId,
    "x-geetorus-cloud-stack-role": "owner",
  };
  if (input.companyName) headers["x-geetorus-cloud-geetorus-company-name"] = input.companyName;
...
function legacyProvisionedDescription(stackId: string) {
  return `Provisioned by Geetorus Cloud for stack ${stackId}.`;
}
...
describeEmbeddedPostgres("cloud tenant company provisioning", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-tenant-provisioning-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  beforeEach(() => {
    process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN = SERVER_TOKEN;
  });
  ...
  describe("claim time", () => {
    ...
    it("derives the prefix from the humanized stack slug when no company name header is sent", async () => {
      const actor = await resolveCloudTenantActor(
        db,
        tenantHeaders({ stackId: "geetorus-stack-borealis", userId: "user-claim-2" }),
      );
