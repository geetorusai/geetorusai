...
async function createGitRepo() {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-accepted-plan-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["checkout", "-B", "master"]);
  await runGit(repoRoot, ["config", "user.email", "geetorus-test@example.com"]);
...
async function createGitRepoWithOrigin() {
  ...
  const originRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-accepted-plan-origin-"));
...
describeEmbeddedPostgres("accepted plan workspace refresh", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-accepted-plan-workspace-");
  ...
  it("realizes an isolated workspace and drops stale shared task-session params before executing", async () => {
    ...
    expect(adapterInput.context.geetorusWorkspace).toEqual(expect.objectContaining({
      mode: "isolated_workspace",
      strategy: "git_worktree",
    }));
    expect((adapterInput.context.geetorusWorkspace as { cwd: string }).cwd).not.toBe(repoRoot);
  ...
  it("keeps accepted-plan children strategy-only until first realization after the base ref moves", async () => {
    ...
    adapterExecute.mockImplementationOnce(async (input) => {
      ...
      const workspace = context.geetorusWorkspace as Record<string, unknown> | undefined;
  ...
  it("forces a fresh session and suppresses accepted-plan continuation when another issue owns the in-flight claim", async () => {
    ...
    expect(adapterInput.context.geetorusTaskMarkdown).toContain("Make the plan only.");
    expect(adapterInput.context.geetorusTaskMarkdown).not.toContain("Create child issues from the approved plan only");
  }, 20_000);
  ...
  it("guards cross-issue accepted-plan retries even when the waking issue is standard work mode", async () => {
    ...
    expect(adapterInput.context.geetorusTaskMarkdown).toContain("Issue: \"PAP-9401\"");
    expect(adapterInput.context.geetorusTaskMarkdown).not.toContain("Create child issues from the approved plan only");
  }, 20_000);
  ...
  it("preserves accepted-plan continuation resume state when the wake issue owns the in-flight claim", async () => {
    ...
    expect(adapterInput.context.geetorusTaskMarkdown).toContain(
