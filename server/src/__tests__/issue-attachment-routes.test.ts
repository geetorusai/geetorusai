...
describe("MAX_ATTACHMENT_BYTES", () => {
  it("reads the deployment-level attachment cap from the environment", async () => {
    const previous = process.env.GEETORUS_ATTACHMENT_MAX_BYTES;
    process.env.GEETORUS_ATTACHMENT_MAX_BYTES = "5";
    vi.resetModules();
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.GEETORUS_ATTACHMENT_MAX_BYTES = previous;
      }
      vi.resetModules();
    }
  });
...
describe("issue attachment routes", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@geetorusai/shared/telemetry");
  ...
  it("canonicalizes geetorus artifact metadata before creating a work product", async () => {
    const storage = createStorageService();
    const issue = {
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-1",
  ...
  it("rejects geetorus artifact metadata that references another issue's attachment", async () => {
    const storage = createStorageService();
    const issue = {
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-1",
  ...
  it("canonicalizes geetorus artifact metadata on work product updates", async () => {
    const storage = createStorageService();
    const issue = {
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-1",
