...
describe("adapter routes", () => {
  beforeEach(async () => {
    ...
    mockAdapterPluginStore.getAdapterPluginsDir.mockReturnValue("/tmp/geetorus-adapter-routes-test");
  ...
  it("keeps geetorus_runner hidden from selection unless the rollout flag is enabled", async () => {
    const disabledResponse = await request(createApp()).get("/api/adapters");
    expect(disabledResponse.status).toBe(200);
    expect(disabledResponse.body.find((adapter: any) => adapter.type === "geetorus_runner"))
    ...
    expect(enabledResponse.body.find((adapter: any) => adapter.type === "geetorus_runner"))
      .toMatchObject({
        disabled: false,
        capabilities: {
  ...
  it("serves provider-scoped Geetorus Runner configuration fields", async () => {
    const app = createApp();

    const res = await request(app).get("/api/adapters/geetorus_runner/config-schema");
