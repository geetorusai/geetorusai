...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@geetorusai/adapter-cursor-local/server";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$GEETORUS_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $GEETORUS_API_KEY\"";
