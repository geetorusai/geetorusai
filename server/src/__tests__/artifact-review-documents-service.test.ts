...
describeEmbeddedPostgres("artifactReviewDocumentService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-artifact-review-docs-");
  ...
  it("rejects work products that are not attachment-backed Geetorus artifacts", async () => {
    const fixture = await seedFixture();
    await expect(
