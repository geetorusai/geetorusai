...
type SupertestTestConstructor = {
  prototype: {
    ...
    __geetorusLoopbackPatched?: boolean;
  };
};
...
if (!process.env.CODEX_HOME) {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-vitest-codex-home-"));
...
// service named `geetorus-dev` behave differently on a broker-capable host
// than on CI. Tests that exercise the default opt in explicitly.
if (!process.env.GEETORUS_MANAGED_RUNTIME_HTTPS) {
  process.env.GEETORUS_MANAGED_RUNTIME_HTTPS = "off";
}

if (!SupertestTest.prototype.__geetorusLoopbackPatched) {
  ...
  SupertestTest.prototype.__geetorusLoopbackPatched = true;
}
