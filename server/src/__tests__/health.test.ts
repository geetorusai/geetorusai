...
describe("GET /health", () => {
  ...
  it("exposes public stack metadata on cloud-simulated health", async () => {
    ...
    expect(res.body.cloud).toEqual({
      managed: true,
      managedBy: "geetorus-cloud",
      stackSlug: "acme",
      cloudBaseUrl: "https://app.geetorus.app",
    });
  });

  it("lists operator-hidden settings and drops unknown keys", async () => {
    const app = createApp(undefined, testServerInfo, undefined, {
      GEETORUS_HIDDEN_SETTINGS: "instance.plugins,instance.adapters,instance.bogus",
    });
  ...
  it("surfaces a stale database backup warning in full health details", async () => {
    const backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-health-backups-"));
    const backupFile = path.join(backupDir, "geetorus-20260705-031702.sql.gz");
    ...
    expect(res.body.databaseBackup).toMatchObject({
      ...
      maxAgeHours: 26,
      latestBackup: {
        name: "geetorus-20260705-031702.sql.gz",
        ageHours: 33.7,
      },
      warnings: [
        {
  ...
  it("surfaces database backup failure markers in full health details", async () => {
    const backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-health-backups-"));
    const backupFile = path.join(backupDir, "geetorus-20260706-031702.sql.gz");
  ...
  it("finds conventional database backup failure markers without an explicit alert file", async () => {
    const backupRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-health-backups-root-"));
  ...
  it("surfaces redacted database backup warnings for anonymous authenticated probes", async () => {
    const backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-health-redacted-backups-"));
    const backupFile = path.join(backupDir, "geetorus-20260705-031702.sql.gz");
    fs.writeFileSync(backupFile, "backup");
    fs.utimesSync(
      backupFile,
  ...
  it("reports bootstrapStatus ready for cloud-managed instances regardless of instance admin count", async () => {
    vi.stubEnv("GEETORUS_CLOUD_TENANT_SERVER_TOKEN", "test-tenant-server-token");
    const { healthRoutes } = await import("../routes/health.js");
    const db = {
      execute: vi.fn().mockResolvedValue([{ "?column?": 1 }]),
      select: vi.fn(() => ({
