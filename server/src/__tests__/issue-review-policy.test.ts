...
import { activityLog, agents, companies, createDb, issues, type Db } from "@geetorusai/db";
...
describeEmbeddedPostgres("issue review verdict policy", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issue-review-policy-");
