...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-orchestration-");
  ...
  async function makeLocalRoot() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-plugin-host-folder-"));
    tempRoots.push(root);
    return root;
  }
  ...
  it("returns plugin-safe execution workspace metadata scoped to the company", async () => {
    ...
    await db.insert(executionWorkspaces).values({
      ...
      repoUrl: "https://example.com/geetorus.git",
      baseRef: "main",
      branchName: "feature/workspace",
      providerType: "git_worktree",
      providerRef: "/tmp/geetorus-feature",
      metadata: {
    ...
    const services = buildHostServices(db, "plugin-record-id", "geetorus.workspace", createEventBusStub());
  ...
  it("creates plugin-origin issues with full orchestration fields and audit activity", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "geetorus.missions", createEventBusStub());
    ...
    expect(stored?.originKind).toBe("plugin:geetorus.missions");
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            ...
            sourcePluginKey: "geetorus.missions",
  ...
  it("enforces plugin origin namespaces", async () => {
    ...
    expect(featureIssue.originKind).toBe("plugin:geetorus.missions:feature");
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:geetorus.missions");
  ...
  it("creates plugin operation issues with the generic operation origin", async () => {
    ...
    expect(issue.originKind).toBe("plugin:geetorus.missions:operation");
  ...
  it("lets bootstrap-style actions initialize required local folders from an empty root", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorusai.plugin-llm-wiki",
      packageName: "@geetorusai/plugin-llm-wiki",
      version: "0.1.0",
      manifestJson: {
        id: "geetorusai.plugin-llm-wiki",
        apiVersion: 1,
        version: "0.1.0",
        ...
        localFolders: [
          {
            ...
            access: "readWrite",
            requiredDirectories: ["raw", "wiki", "wiki/concepts", ".geetorus"],
  ...
  it("rejects worker local-folder access for undeclared manifest keys", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.local-folders",
      packageName: "@geetorus/plugin-local-folders",
      version: "0.1.0",
      manifestJson: {
        id: "geetorus.local-folders",
        apiVersion: 1,
        version: "0.1.0",
  ...
  it("resolves plugin-managed projects by stable key without overwriting user edits", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.missions",
      packageName: "@geetorus/plugin-missions",
      version: "0.1.0",
      apiVersion: 1,
      categories: ["automation"],
      status: "ready",
      manifestJson: {
        id: "geetorus.missions",
        apiVersion: 1,
        version: "0.1.0",
    ...
    const services = buildHostServices(db, pluginId, "geetorus.missions", createEventBusStub());
  ...
  it("rejects a human-attributed plugin comment when actorUserId is not an active company member", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "geetorus.gateway", createEventBusStub());
    await expect(
