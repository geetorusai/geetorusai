...
describe("project workspace skill discovery", () => {
  ...
  it("finds bounded skill roots under supported workspace paths", async () => {
    const workspace = await makeTempDir("geetorus-skill-workspace-");
    ...
    await writeSkillDir(path.join(workspace, "skills", ".system", "geetorus"), "Geetorus");
    ...
    expect(discovered).toEqual([
      ...
      {
        skillDir: path.resolve(workspace, "skills", ".system", "geetorus"),
        directoryRoot: "skills/.system",
        relativePath: "skills/.system/geetorus",
        inventoryMode: "full",
      },
      {
  ...
  it("limits root SKILL.md imports to skill-related support folders", async () => {
    const workspace = await makeTempDir("geetorus-root-skill-");
  ...
  it("rejects symlinks reachable from a project-scanned skill", async () => {
    const workspace = await makeTempDir("geetorus-linked-skill-file-");
    const skillDir = path.join(workspace, ".codex", "skills", "linked-file");
    const outsideFile = path.join(await makeTempDir("geetorus-linked-skill-outside-"), "outside.md");
  ...
  it("parses inline object array items in skill frontmatter metadata", async () => {
    const workspace = await makeTempDir("geetorus-inline-skill-yaml-");
  ...
  it("parses folded and literal block scalar descriptions in skill frontmatter", async () => {
    const foldedWorkspace = await makeTempDir("geetorus-folded-skill-yaml-");
    ...
    const literalWorkspace = await makeTempDir("geetorus-literal-skill-yaml-");
    ...
    const stripWorkspace = await makeTempDir("geetorus-strip-skill-yaml-");
  ...
  it("parses YAML block-scalar chomping variants from SKILL.md frontmatter", async () => {
    const workspace = await makeTempDir("geetorus-block-scalar-chomp-skill-");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(
      path.join(workspace, "SKILL.md"),
      [
...
describe("missing local skill reconciliation", () => {
  it("flags local-path skills whose directory was removed", async () => {
    const workspace = await makeTempDir("geetorus-missing-skill-dir-");
  ...
  it("flags local-path skills whose SKILL.md file was removed", async () => {
    const workspace = await makeTempDir("geetorus-missing-skill-file-");
    const skillDir = path.join(workspace, "skills", "ghost");
    await writeSkillDir(skillDir, "Ghost");
