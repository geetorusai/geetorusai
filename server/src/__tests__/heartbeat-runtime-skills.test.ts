...
describeEmbeddedPostgres("heartbeat runtime skill version pins", () => {
  ...
  let oldGeetorusApiUrl: string | undefined;
  ...
  beforeAll(async () => {
    ...
    geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-skills-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    // The server normalizes GEETORUS_API_URL into its own env at boot
    ...
    oldGeetorusApiUrl = process.env.GEETORUS_API_URL;
    process.env.GEETORUS_API_URL = "http://127.0.0.1:3100/api";
    registerServerAdapter({
      ...
      execute: async (ctx) => {
        ...
        capturedRuns.push({
          ...
          skills: (ctx.config.geetorusRuntimeSkills ?? []) as GeetorusSkillEntry[],
  ...
  afterAll(async () => {
    ...
    if (oldGeetorusApiUrl === undefined) delete process.env.GEETORUS_API_URL;
    else process.env.GEETORUS_API_URL = oldGeetorusApiUrl;
    if (geetorusHome) {
  ...
  it("materializes different pinned skill versions for different agents at runtime", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-versioned-runtime-skill-"));
  ...
  it("delivers installed connections without exposing gateway bearers in adapter config or logs", async () => {
    ...
    expect(captured?.config).not.toHaveProperty("geetorusRuntimeMcpServers");
