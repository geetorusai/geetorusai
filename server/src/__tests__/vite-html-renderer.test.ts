...
describe("createCachedViteHtmlRenderer", () => {
  ...
  it("caches the branded template until index.html changes while transforming every request", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-vite-html-"));
    ...
    const brandHtml = vi.fn((html: string) => html.replace("<body>", '<body data-brand="geetorus">'));
    ...
    expect(first).toContain('data-brand="geetorus"');
