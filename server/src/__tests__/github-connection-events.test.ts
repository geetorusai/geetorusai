...
import type { GeetorusCloudConnector } from "../services/geetorus-cloud-connector.js";
import {
...
describeEmbeddedPostgres.sequential("GitHub connection event delivery", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-github-events-");
  ...
  it("applies a normalized merged PR once, updates the snapshot, and acknowledges Cloud", async () => {
    ...
    await db.insert(companies).values({ id: companyId, name: "Geetorus", issuePrefix: "GHE" });
    ...
    await db.insert(connectionGrants).values({
      ...
      isDefault: false,
      providerTenant: {
        oauth: { strategy: "geetorus_cloud_connector", accessTokenExpiresAt: null },
        github: {
          userId: "42",
          login: "octocat",
          installationCount: 1,
          ...
          repositories: [{ id: "203", fullName: "geetorusai/removed", installationId: "101" }],
    ...
    await db.insert(externalObjects).values({
      ...
      externalId: "geetorusai/geetorus#pull/123",
      statusCategory: "open",
      statusTone: "info",
    ...
    const leasedEvent = {
      ...
      payload: {
        repository: "geetorusai/geetorus",
        number: 123,
        state: "closed",
        merged: true,
        mergedAt: "2026-09-04T11:59:00.000Z",
        updatedAt: "2026-09-04T11:59:01.000Z",
        url: "https://github.com/geetorusai/geetorus/pull/123",
        headRef: "feature",
        headSha: "a".repeat(40),
        baseRef: "master",
        baseSha: "b".repeat(40),
    ...
    } as unknown as GeetorusCloudConnector;
  ...
  it.each([false, true])("applies installation events once without discarding newer verified access (refreshed: %s)", async (refreshed) => {
    ...
    const connectionId = randomUUID();
    const grantId = randomUUID();
    await db.insert(companies).values({ id: companyId, name: "Geetorus", issuePrefix: "GHI" });
    await db.insert(toolApplications).values({
      id: applicationId,
      companyId,
      applicationKey: `github-${randomUUID()}`,
