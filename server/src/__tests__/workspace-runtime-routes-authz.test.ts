...
describe.sequential("workspace runtime service route authorization", () => {
      ...
      .send({
        ...
        executionWorkspacePolicy: {
          ...
          workspaceStrategy: {
            ...
            runtimeProvisionCommand: "touch /tmp/geetorus-runtime-rce",
          },
        },
      });
  ...
  it("rejects agent callers that persist workspace-runtime service commands", async () => {
    ...
    const projectRes = await request(app)
      ...
      .send({
        ...
        executionWorkspacePolicy: {
          enabled: true,
          workspaceRuntime: {
            services: [{ name: "web", command: "touch /tmp/geetorus-rce" }],
          },
        },
      });
    ...
    const workspaceRes = await request(app)
      ...
      .send({
        runtimeConfig: {
          workspaceRuntime: {
            jobs: [{ name: "build", command: "touch /tmp/geetorus-rce" }],
          },
        },
      });
  ...
  it("rejects agent callers that update project workspace cleanup commands", async () => {
    ...
    const res = await request(app)
      .patch(`/api/projects/${projectId}/workspaces/${workspaceId}`)
      .send({
        cleanupCommand: "rm -rf /tmp/geetorus-rce",
      });
