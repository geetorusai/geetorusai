...
vi.mock("@geetorusai/adapter-claude-local/server", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@geetorusai/adapter-claude-local/server")>();
  return {
    ...actual,
...
describeEmbeddedPostgres("agents adapter-config user-secret resolution routes", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-adapter-user-secret-${randomUUID()}`);
