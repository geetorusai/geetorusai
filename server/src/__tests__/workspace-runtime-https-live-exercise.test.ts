 ...
 *
 *   - `GEETORUS_LIVE_BROKER_EXERCISE=1`
 *   - the broker socket exists (`GEETORUS_TAILSCALE_BROKER_SOCKET` or the default)
 *
 * Run it on a broker-provisioned host with:
 *
 *   GEETORUS_LIVE_BROKER_EXERCISE=1 pnpm --filter @geetorusai/server exec \
...
const DEFAULT_BROKER_SOCKET = "/run/geetorus-tailscale-broker/broker.sock";
const brokerSocketPath = process.env.GEETORUS_TAILSCALE_BROKER_SOCKET ?? DEFAULT_BROKER_SOCKET;
...
const optedIn = process.env.GEETORUS_LIVE_BROKER_EXERCISE === "1";
...
(live ? describe : describe.skip)("PAP-17158 live HTTPS backfill exercise", () => {
  ...
  it("upgrades a pre-existing HTTP workspace in place to a browser-trusted HTTPS URL", async () => {
    ...
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "pap17158-live-home-"));
    const previousHome = process.env.GEETORUS_HOME;
    const previousInstance = process.env.GEETORUS_INSTANCE_ID;
    const previousMode = process.env.GEETORUS_MANAGED_RUNTIME_HTTPS;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `pap17158-live-${randomUUID()}`;
    ...
    try {
      ...
      const before = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Geetorus", companyId },
        issue: null,
        workspace: {
      ...
      if (previousMode === undefined) delete process.env.GEETORUS_MANAGED_RUNTIME_HTTPS;
      else process.env.GEETORUS_MANAGED_RUNTIME_HTTPS = previousMode;
    }
  }, 180_000);
});
