...
describe("P6-31 Section 18.13 executable status-authority corpus", () => {
  ...
  beforeAll(async () => {
    temporary = await startEmbeddedPostgresTestDatabase("geetorus-status-corpus-");
  ...
  async function executeFixture(
    ...
    const mode = resolveNativeRuntimeMode({
      ...
      agent: {
        id: agentId,
        status: "running",
        adapterType: fixture.mode === "native" ? "geetorus_runner" : "codex_local",
      },
    ...
    if (["turn_scope", "run_scope", "issue_scope_authorized", "replacement_turn_accepted"].includes(completionState)) {
      ...
      if (
        ...
        const activeResolution = resolveHeartbeatNativeRuntimeMode({
          ...
          runtimeConfig,
          adapterConfig: { provider: "codex" },
          agent: { id: agentId, status: "running", adapterType: "geetorus_runner" },
          issue: { id: seeded.issueId, workMode: "standard" },
          target: { kind: "local" },
          workspaceId: "fixture-workspace",
        });
        ...
        if (
          ...
          || freshMode !== "rejected"
          || freshReason !== "geetorus_runner_rollout_disabled"
    ...
    for (const matrixRow of Object.values(fixture.covers).flat()) {
      ...
      if (semanticRow) {
        ...
        if (
          ...
          || execution.observed.freshReason !== "geetorus_runner_rollout_disabled"
  ...
  it("executes all 53 fixtures in their production consumers and joins all 70 matrix rows", async () => {
    expect(corpus.schema).toBe("geetorus.status-authority-conformance.v1");
