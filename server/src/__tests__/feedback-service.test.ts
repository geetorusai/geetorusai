...
import { writeGeetorusSkillSyncPreference } from "@geetorusai/adapter-utils/server-utils";
import {
  agents,
  companies,
  companySkills,
...
describeEmbeddedPostgres("feedbackService.saveIssueVote", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("geetorus-feedback-service-");
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-feedback-instructions-"));
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeGeetorusSkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["geetorusai/geetorus/geetorus", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a sanitized shared bundle without reading external instruction roots", async () => {
    ...
    expect(trace?.schemaVersion).toBe("geetorus-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("geetorus-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("geetorus-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("geetorus-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-feedback-opencode-"));
    ...
    vi.stubEnv("GEETORUS_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "geetorus_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
