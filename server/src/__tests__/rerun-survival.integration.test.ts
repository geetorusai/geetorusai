...
import { mergeGeetorusConfig, geetorusConfigSchema } from "@geetorusai/shared";
import {
  updateEnvFileContents,
  writeEnvFileAtomicallyIfChanged,
} from "@geetorusai/shared/env-file";
 ...
 *   - config.json          -> mergeGeetorusConfig + geetorusConfigSchema (shared core
...
function writerRoundTrip(configPath: string, update: unknown): void {
  const parsedUpdate = geetorusConfigSchema.parse(update);
  const source = geetorusConfigSchema.parse(
    JSON.parse(fsSync.readFileSync(configPath, "utf8")),
  );
  const next = geetorusConfigSchema.parse(mergeGeetorusConfig(source, parsedUpdate));
...
describe("setup/sync rerun survival — cross-cutting", () => {
  it("preserves unknown top-level and nested config.json keys across a writer rerun", async () => {
    ...
    const seeded = geetorusConfigSchema.parse({
      ...BASE_CONFIG,
