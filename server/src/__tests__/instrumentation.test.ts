...
describe("checkExactPeerVersions", () => {
  ...
  it("reports a package as missing when it cannot be resolved", async () => {
    ...
    const result = checkExactPeerVersions(["@geetorusai/does-not-exist-anywhere"], {
      "@geetorusai/does-not-exist-anywhere": "1.0.0",
    });

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.diagnostic).toContain("@opentelemetry/* packages are not installed");
      expect(result.diagnostic).toContain("@geetorusai/does-not-exist-anywhere");
    }
  });
