...
describe("merged pull-request confirmation extraction", () => {
  ...
  it("extracts and deduplicates full GitHub URLs and owner/repo#N shorthand", () => {
    expect(extractGitHubPullRequestReferences([
      "Merge https://github.com/GeetorusAI/geetorus/pull/39.",
      "Also geetorusai/geetorus#40 and GEETORUSAI/geetorus#40.",
    ])).toEqual([
      { host: "github.com", owner: "GeetorusAI", repo: "geetorus", number: 39 },
      { host: "github.com", owner: "geetorusai", repo: "geetorus", number: 40 },
    ]);
  });
  ...
  it("requires merge intent and excludes document and tool-action confirmations", () => {
    const base = {
      ...
      title: "Merge GeetorusAI/geetorus#39?",
      summary: null,
  ...
  it("finds references in merge-confirmation body and custom target fields", () => {
    expect(getMergeConfirmationPullRequestReferences({
      ...
      payload: {
        ...
        detailsMarkdown: "Primary: geetorusai/geetorus#39",
        target: {
          type: "custom",
          key: "github-pr-40",
          href: "https://github.com/geetorusai/geetorus/pull/40",
        },
      },
    })).toEqual([
      { host: "github.com", owner: "geetorusai", repo: "geetorus", number: 39 },
  ...
  it("denies governed actions mentioned in details or custom target metadata", () => {
    ...
    expect(getMergeConfirmationPullRequestReferences({
      ...
      payload: {
        version: 1,
        prompt: "Merge the linked pull request?",
        detailsMarkdown: "Deploy to production after geetorusai/geetorus#39 merges.",
      },
    })).toEqual([]);
    expect(getMergeConfirmationPullRequestReferences({
      ...base,
      payload: {
        ...
        target: {
          ...
          href: "https://github.com/geetorusai/geetorus/pull/39",
        },
      },
    })).toEqual([]);
  });
  ...
  it("fails closed when a merge confirmation includes an additional action clause", () => {
    expect(getMergeConfirmationPullRequestReferences({
      ...
      payload: {
        version: 1,
        prompt: "Merge geetorusai/geetorus#39?",
        detailsMarkdown: "Erase all customer records after geetorusai/geetorus#39 merges.",
      },
    })).toEqual([]);
  });
});
...
describeEmbeddedPostgres.sequential("merged pull-request confirmation sweep", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-merged-pr-confirmations-");
    ...
    await db.insert(issueThreadInteractions).values([
      {
        ...
        title: "Merge https://github.com/geetorusai/geetorus/pull/39?",
      ...
      },
      {
        ...
        title: "Ready to merge geetorusai/geetorus#39 and geetorusai/geetorus#41?",
      ...
      },
      {
        ...
        title: "Merge geetorusai/geetorus#42?",
        payload: {
          version: 1,
          prompt: "Merge the PR?",
    ...
    expect(audit).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        details: expect.objectContaining({
          ...
          pullRequests: ["geetorusai/geetorus#39"],
        }),
      }),
    ]));
  });
