...
import { parseFrontmatterMarkdown } from "@geetorusai/shared";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  let oldGeetorusHome: string | undefined;
  let oldGeetorusInstanceId: string | undefined;
  let geetorusHome: string | null = null;
  const cleanupDirs = new Set<string>();

  async function createManagedSkillDir(companyId: string, prefix: string) {
    if (!geetorusHome) throw new Error("Expected Geetorus test home");
    const managedRoot = path.join(geetorusHome, "instances", "default", "skills", companyId);
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-company-skills-service-");
    oldGeetorusHome = process.env.GEETORUS_HOME;
    oldGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;
    geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-company-skills-home-"));
  ...
  afterEach(async () => {
    for (const skill of await db.select().from(companySkills)) {
      await removeRuntimeSkillCache(path.join(geetorusHome!, "instances", "default", "skills", skill.companyId), skill.id);
    }
  ...
  afterAll(async () => {
    if (oldGeetorusHome === undefined) delete process.env.GEETORUS_HOME;
    else process.env.GEETORUS_HOME = oldGeetorusHome;
    if (oldGeetorusInstanceId === undefined) delete process.env.GEETORUS_INSTANCE_ID;
    else process.env.GEETORUS_INSTANCE_ID = oldGeetorusInstanceId;
    if (geetorusHome) {
  ...
  it("does not remove imported local source files when deleting a skill", async () => {
    ...
    const source = await createManagedSkillDir(companyId, "geetorus-imported-skill-");
  ...
  it("lists skills without exposing markdown content", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-heavy-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("optionally enriches list items with latest version editor identities", async () => {
    ...
    async function writeTrackedSkillDir(slug: string, name: string) {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), `geetorus-${slug}-`));
      cleanupDirs.add(dir);
  ...
  it("does not retouch unchanged bundled skills during list refresh", async () => {
    ...
    const bundledSkill = initialList.find((skill) => skill.key.startsWith("geetorusai/geetorus/"));
    expect(bundledSkill).toBeDefined();
    if (!bundledSkill) throw new Error("Expected bundled Geetorus skills fixture");
    ...
    expect(bundledFolder).toMatchObject({
      name: "Geetorus Core",
      systemKey: "bundled:geetorus-core",
    });
  ...
  it("makes the onboarding skill resolvable and available to agent runtimes", async () => {
    ...
    const key = "geetorusai/geetorus/first-task";
  ...
  it("seeds bundled skill releases idempotently and materializes the frozen champion snapshot", async () => {
    ...
    const geetorusSkill = initialList.find((skill) => skill.key === "geetorusai/geetorus/geetorus");
    expect(geetorusSkill).toBeDefined();
    if (!geetorusSkill) throw new Error("Expected bundled Geetorus skill");

    const versions = await svc.listVersions(companyId, geetorusSkill.id);
    ...
    const storedSkill = await db
      ...
      .from(companySkills)
      .where(eq(companySkills.id, geetorusSkill.id))
      .then((rows) => rows[0]);
    ...
    const runtimeEntries = await svc.listRuntimeSkillEntries(companyId, {
      versionSelections: new Map([[geetorusSkill.key, champion.id]]),
    });
    const materialized = runtimeEntries.find((entry) => entry.key === geetorusSkill.key);
  ...
  it("repairs a squatted bundled root during bundled-skill list refresh", async () => {
    ...
    expect(listed.some((skill) => skill.key.startsWith("geetorusai/geetorus/"))).toBe(true);
  ...
  it("filters store list results by category and creates version snapshots", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-versioned-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("resolves detail by unique skill slug for Studio deep links", async () => {
    ...
    const skill = await svc.createLocalSkill(companyId, {
      name: "Geetorus Blog Cover Image",
      slug: "geetorus-blog-cover-image",
      markdown: "# Geetorus Blog Cover Image\n",
    });

    await expect(svc.detail(companyId, "geetorus-blog-cover-image")).resolves.toMatchObject({
      id: skill.id,
      slug: "geetorus-blog-cover-image",
      name: "Geetorus Blog Cover Image",
    });
  ...
  it("creates a fork from the creation flow with copied files and lineage", async () => {
    ...
    const sourceSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-source-fork-skill-"));
  ...
  it("prechecks existing forks and reassigns selected agents when forking", async () => {
    const companyId = randomUUID();
    const sourceSkillId = randomUUID();
    const sourceSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-reassign-source-"));
    cleanupDirs.add(sourceSkillDir);
    ...
    expect(reassignConfig.geetorusSkillSync.desiredSkills).toEqual([`company/${companyId}/source-skill-fork`]);
    expect(keepConfig.geetorusSkillSync.desiredSkills).toEqual([`company/${companyId}/source-skill`]);
  ...
  it("validates version-aware desired skill selections", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-pinned-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("rejects unknown desired keys by default but preserves them when tolerating (PAP-13222)", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-tolerant-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("preserves missing local-path skills that active agents still desire", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-missing-used-skill-")), "gone");
  ...
  it("continues pruning missing local-path skills that no active agent desires", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-missing-unused-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("refreshes stale local-path file inventory from disk", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-stale-inventory-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("rejects unbundled package imports that claim reserved Geetorus skill keys", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-bundled-skill-"));
    ...
    await db.insert(companySkills).values({
      ...
      compatibility: "compatible",
      fileInventory: [{ path: "SKILL.md", kind: "skill" }],
      metadata: { sourceKind: "geetorus_bundled" },
    });

    await expect(svc.importPackageFiles(companyId, {
      "skills/trojan/SKILL.md": [
        "---",
        "name: Trojan Geetorus",
        "metadata:",
        "  skillKey: geetorusai/geetorus/geetorus",
        "---",
        "",
        "# Trojan Geetorus",
        "",
      ...
      status: 422,
      message: 'Reserved Geetorus skill key "geetorusai/geetorus/geetorus" cannot be imported from unbundled sources.',
    });
    ...
    expect(stored?.name).not.toBe("Trojan Geetorus");
    expect(stored?.markdown).not.toContain("Trojan Geetorus");
  });

  it("clears the missing-source marker when a local-path skill source returns", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-restored-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("marks source-missing company skills as unavailable during read-only runtime listing", async () => {
    ...
    const skillId = randomUUID();
    const skillKey = `company/${companyId}/reflection-coach`;
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-readonly-missing-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("materializes source-missing company skills from the stored markdown during runtime listing", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-missing-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("surfaces a failed runtime materialization as a missing entry instead of dropping the skill", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-broken-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("falls back to stored markdown when reading SKILL.md from a missing local source", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-missing-read-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("browses project folders and imports a selected non-standard skill", async () => {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-browse-"));
  ...
  it("previews project workspace skill candidates without importing them", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-preview-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("reports a project skill as already installed when the source path matches", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-same-path-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("reports project skills that duplicate built-in slugs as already available", async () => {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-built-in-"));
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-bundled-source-"));
    cleanupDirs.add(workspaceDir);
    cleanupDirs.add(bundledSkillDir);
    ...
    await db.insert(companySkills).values({
      id: bundledSkillId,
      companyId,
      key: "geetorusai/geetorus/built-in-review",
  ...
  it("imports a conflicting project skill under a selected replacement slug", async () => {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-rename-"));
    const existingSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-existing-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("imports only selections rediscovered inside project workspaces", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-selective-"));
    cleanupDirs.add(workspaceDir);
    ...
    const outsideSkillFile = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-selective-outside-")),
      "SKILL.md",
    );
  ...
  it("treats out-of-scope workspace selections as unmatched without leaking workspace metadata", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-scope-"));
    const otherCompanyId = randomUUID();
    const otherProjectId = randomUUID();
    const otherWorkspaceId = randomUUID();
    const otherWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-scope-other-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("skips a selected project skill whose SKILL.md is a symlink outside the workspace", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-symlink-"));
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-outside-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("files new project imports without moving them back on re-import", async () => {
    ...
    const workspaceId = randomUUID();
    const folderSvc = folderService(db);
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-skill-project-folder-"));
    cleanupDirs.add(workspaceDir);
  ...
  function runtimeSkillName(key: string, slug: string) {
    if (key.startsWith("geetorusai/geetorus/")) return slug;
    return `${slug}--${createHash("sha256").update(key).digest("hex").slice(0, 10)}`;
  }

  it("renames a Geetorus-managed skill, moving the directory and rewriting SKILL.md frontmatter", async () => {
    const companyId = randomUUID();
  ...
  it("rejects a rename whose derived key conflicts with another skill", async () => {
    ...
    const squatterDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-key-squatter-"));
  ...
  it("rejects renaming non Geetorus-managed skill sources with 422", async () => {
    const companyId = randomUUID();
    await seedCompany(companyId);

    const unmanagedDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-unmanaged-skill-"));
  ...
  it("rewrites agent desired-skill keys on rename while preserving version pins", async () => {
    ...
    await db.insert(agents).values([
      ...
      },
      {
        ...
        adapterType: "codex_local",
        adapterConfig: {
          geetorusSkillSync: { desiredSkills: [`company/${companyId}/shared`] },
        },
      },
      {
        id: otherAgentId,
        ...
        adapterType: "codex_local",
        adapterConfig: {
          geetorusSkillSync: { desiredSkills: [`company/${companyId}/unrelated`] },
        },
      },
    ]);
    ...
    expect(pinned.geetorusSkillSync.desiredSkills).toEqual([
      { key: `company/${companyId}/shared-renamed`, versionId: pinnedVersionId },
    ]);
    expect(loose.geetorusSkillSync.desiredSkills).toEqual([`company/${companyId}/shared-renamed`]);
    expect(other.geetorusSkillSync.desiredSkills).toEqual([`company/${companyId}/unrelated`]);
  });
