...
describeEmbeddedPostgres("environmentService leases", () => {
  ...
  async function seedEnvironment() {
    ...
    await db.insert(environments).values({
      ...
      config: {
        ...
        port: 22,
        username: "fixture",
        remoteWorkspacePath: "/srv/geetorus",
      },
      createdAt: new Date(),
      updatedAt: new Date(),
    });
  ...
  it("adopts a pre-existing local row on a cloud-managed instance by stamping the platform marker", async () => {
    ...
    try {
      ...
      expect(adopted.metadata).toEqual({ owner: "operator", managedByGeetorus: true });
      ...
      const adoptedRow = await db
        ...
        .then((rows) => rows[0]);
      const reused = await svc.ensureLocalEnvironment(companyId);
      expect(reused.metadata).toEqual({ owner: "operator", managedByGeetorus: true });
  ...
  it("ensures and refreshes a managed sandbox environment for an arbitrary provider", async () => {
    ...
    expect(created.metadata?.managedByGeetorus).toBe(true);
  ...
  it("preserves an existing unmanaged sandbox row holding the desired name", async () => {
    ...
    expect(handMade.metadata?.managedByGeetorus).toBeUndefined();
    ...
    expect(reconciliation.environment.metadata?.managedByGeetorus).toBeUndefined();
  ...
  it("rejects a second managed-sandbox row for the same company at the DB level", async () => {
    ...
    // second row matching driver='sandbox' AND managedByGeetorus=true for the
    ...
    // Index does NOT cover tenant-created sandbox rows (no managedByGeetorus
