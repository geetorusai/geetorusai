...
import { companies, companySkills, createDb, projects, projectWorkspaces } from "@geetorusai/db";
...
describeEmbeddedPostgres("company skill local import boundary", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-skill-import-boundary-");
  ...
  it("allows configured workspace imports and rejects out-of-tree and symlink escapes", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-approved-workspace-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-outside-skill-"));
