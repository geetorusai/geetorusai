...
describeEmbeddedPostgres("tool OAuth legacy backfill", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-tool-oauth-backfill-");
  ...
  it("preserves an existing deterministic OAuth secret provider when rotating legacy material", async () => {
    ...
    const externalRef =
      `arn:aws:secretsmanager:us-east-1:123456789012:secret:geetorus/oauth/${connection!.id}`;
    ...
    const awsVault = await secrets.createProviderConfig(company.id, {
      ...
      config: { region: "us-east-1", namespace: "oauth-test", secretNamePrefix: "geetorus" },
    });
