...
import { models as claudeFallbackModels } from "@geetorusai/adapter-claude-local";
import { resetClaudeModelsCacheForTests } from "@geetorusai/adapter-claude-local/server";
import { models as codexFallbackModels } from "@geetorusai/adapter-codex-local";
import { models as cursorFallbackModels } from "@geetorusai/adapter-cursor-local";
import { models as opencodeFallbackModels } from "@geetorusai/adapter-opencode-local";
import { resetOpenCodeModelsCacheForTests } from "@geetorusai/adapter-opencode-local/server";
...
describe("adapter model listing", () => {
  beforeEach(() => {
    ...
    delete process.env.GEETORUS_OPENCODE_COMMAND;
  ...
  it("returns opencode fallback models including gpt-5.4", async () => {
    process.env.GEETORUS_OPENCODE_COMMAND = "__geetorus_missing_opencode_command__";
  ...
  describe("GEETORUS_ADAPTER_MODELS declared models", () => {
    afterEach(() => {
      delete process.env.GEETORUS_ADAPTER_MODELS;
    });

    it("prefers declared env models over adapter discovery", async () => {
      process.env.GEETORUS_ADAPTER_MODELS = JSON.stringify({
        opencode_local: [
    ...
    it("fails soft on malformed values: falls back to adapter models instead of throwing", async () => {
      process.env.GEETORUS_ADAPTER_MODELS = "{not json";
      process.env.GEETORUS_OPENCODE_COMMAND = "__geetorus_missing_opencode_command__";
