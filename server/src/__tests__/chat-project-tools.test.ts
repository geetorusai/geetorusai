...
import { issues, heartbeatRuns } from "@geetorusai/db";
...
(support.supported ? describe : describe.skip)("chat project tool handoff", () => {
  ...
  beforeAll(async () => { process.env.GEETORUS_AGENT_JWT_SECRET = randomUUID(); server = await startRunnerApiTestServer(); }, 60_000);
  afterAll(async () => { await server?.close(); if (originalSecret === undefined) delete process.env.GEETORUS_AGENT_JWT_SECRET; else process.env.GEETORUS_AGENT_JWT_SECRET = originalSecret; });
  ...
  it("allows a conversation reply to enter review without manufacturing a review interaction", async () => {
    ...
    const token = createLocalAgentJwt(f.agentId, f.companyId, "geetorus_runner", f.runId, f.responsibleUserId)!;
  ...
  it("hands off through the same API used by Claude/Codex MCP with the plan present on return", async () => {
    ...
    const result = await callProjectTool({ name: "create_task", arguments: { title: "MCP handoff", projectId: f.projectId, initialPlan: "# Plan\nImplement in the execution task.", idempotencyKey: "mcp" },
      apiUrl: server.apiUrl, token: createLocalAgentJwt(f.agentId, f.companyId, "geetorus_runner", f.runId, f.responsibleUserId)!,
