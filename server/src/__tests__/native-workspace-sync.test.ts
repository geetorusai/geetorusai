...
} from "@geetorusai/adapter-utils/workspace-restore-merge";
...
describe("native workspace sync durable metadata", () => {
  ...
  afterEach(async () => {
    ...
    if (originalGeetorusInstanceId === undefined)
  ...
  it("reads backward-compatible references and the resource disposition", () => {
    const base = {
      schema: "geetorus.native-workspace-sync/v1",
      state: "prepared",
  ...
  it.each([false, true])("writes one immutable descriptor when the same state is replayed (multiple repositories: %s)", async (multipleRepositories) => {
    const geetorusHome = await mkdtemp(
      path.join(os.tmpdir(), "geetorus-native-workspace-sync-"),
    );
    cleanupDirs.push(geetorusHome);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "descriptor-test";
    const baseline = {
    ...
    const descriptor = {
      schema: "geetorus.native-workspace-sync/v1" as const,
      binding: {
        ...
        leaseId: "lease-1",
        providerLeaseId: "sandbox-1",
        localCwd: path.join(geetorusHome, "workspace"),
        remoteCwd: "/workspace",
      },
      ...
      gitSnapshot: multipleRepositories ? {
        ...
        repositories: [{ path: ".geetorus-repositories/backend", snapshot: {
  ...
  it("repairs finalized remote and lease stamps after an interrupted commit", async () => {
    const geetorusHome = await mkdtemp(
      path.join(os.tmpdir(), "geetorus-native-workspace-sync-repair-"),
    );
    cleanupDirs.push(geetorusHome);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "descriptor-repair-test";
    const baseline = {
      exclude: [".geetorus-runtime"],
      entries: new Map([
        [
