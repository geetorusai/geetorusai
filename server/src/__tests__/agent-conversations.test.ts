...
import { renderGeetorusWakePrompt } from "@geetorusai/adapter-utils/server-utils";
...
(support.supported ? describe : describe.skip)(
  ...
  () => {
    ...
    it("resets only this task, keeps history, and fences stale replies and retries", async () => {
      ...
      const payload = await buildGeetorusWakePayload({
        db, companyId,
    ...
    it("projects the resolved chat plan review into fresh and resumed prompts without replaying old reviews", async () => {
      ...
      const payload = await buildGeetorusWakePayload(input);
      ...
      for (const resumedSession of [false, true]) {
        const prompt = renderGeetorusWakePrompt(payload, { resumedSession });
      ...
      const later = await buildGeetorusWakePayload({ ...input,
      ...
      expect((await buildGeetorusWakePayload(input))?.planReviewContext).toBeNull();
    });
    it("includes an initial handoff plan in the first execution prompt and pins approved revisions", async () => {
      ...
      for (const includeDescription of [true, false]) {
        const prompt = buildGeetorusTaskMarkdown({
          issue: task,
          taskPlan: initial,
          includeDescription,
        });
    ...
    it.each([false, true])("runs real process turns and resets without replaying pre-Stop queued input (queued=%s)", async (queuedBeforeStop) => {
      ...
      const script = `fetch("http://127.0.0.1:${address.port}/respond", {method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({runId:process.env.GEETORUS_RUN_ID})}).then(async r=>{if(!r.ok){console.error(r.status,await r.text());process.exitCode=1}})`;
      await db
        .insert(agents)
        .values({
          id: runtimeAgent,
        ...
        if (queuedBeforeStop) {
          ...
          const [stoppedQueuedWake] = await db.insert(agentWakeupRequests).values({ companyId: runtimeCompany, agentId: runtimeAgent,
            ...
            payload: { issueId: chat.id, commentId: pending.id,
              _geetorusWakeContext: { issueId: chat.id, wakeReason: "issue_commented", wakeCommentId: pending.id, wakeCommentIds: [pending.id] } },
          }).returning();
    ...
    it("rejects late replies and session events from cancelled conversation turns", async () => {
      ...
      const previousSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
      process.env.GEETORUS_AGENT_JWT_SECRET = "test-conversation-cancellation-secret";
      try {
        const app = express();
        app.use(actorMiddleware(db, { deploymentMode: "local_trusted" }));
        ...
        if (previousSecret === undefined) delete process.env.GEETORUS_AGENT_JWT_SECRET;
        else process.env.GEETORUS_AGENT_JWT_SECRET = previousSecret;
      }
...
describe("native conversation finalization", () => {
  it("does not require execution completion or schedule a continuation after a successful chat turn", () => {
    const decision = {
      policyVersion: "geetorus.native-status-arbiter.v1",
