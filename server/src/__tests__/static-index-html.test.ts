...
describe("static SPA fallback HTML", () => {
  ...
  it("includes the operator snippet only in Cloud-served static HTML", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-cloud-html-"));
    tempDirs.push(dir);
    fs.writeFileSync(path.join(dir, "index.html"), "<html><body>App</body></html>");
    vi.stubEnv("GEETORUS_CLOUD_UI_SNIPPET", '<script src="https://example.com/chat.js"></script>');
    vi.stubEnv("GEETORUS_CLOUD_TENANT_SERVER_TOKEN", undefined);
    vi.stubEnv("GEETORUS_MANAGED_CONFIG", undefined);
    expect(readBrandedStaticIndexHtml(dir)).not.toContain("chat.js");
    vi.stubEnv("GEETORUS_MANAGED_CONFIG", "{}");
  ...
  it("serves the current index.html instead of reusing stale asset hashes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-static-index-"));
