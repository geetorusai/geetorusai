...
describeEmbeddedPostgres("POST /api/companies/:companyId/onboarding-seed", () => {
  ...
  it("applies the mission, the first agent and the first task before it answers", async () => {
    ...
    // A seeded CEO arrives with the core geetorus skills enabled. Skills only
  ...
  it("keeps server-seeded onboarding on a legacy adapter when native runner is requested", async () => {
    const previous = process.env.GEETORUS_ONBOARDING_SEED_ADAPTER_TYPE;
    process.env.GEETORUS_ONBOARDING_SEED_ADAPTER_TYPE = "geetorus_runner";
    try {
      const { companyId, app } = await seedCompany();
      ...
      if (previous === undefined) {
        delete process.env.GEETORUS_ONBOARDING_SEED_ADAPTER_TYPE;
      } else {
        process.env.GEETORUS_ONBOARDING_SEED_ADAPTER_TYPE = previous;
      }
    }
  });
  ...
  it("never reads the seed from a trusted Cloud header", async () => {
    ...
    // The `x-geetorus-cloud-*` set is the trusted identity channel, derived
    // server-side. A mission planted there must be ignored entirely — only the
    // body is read.
    const response = await request(app)
      .post(`/api/companies/${companyId}/onboarding-seed`)
      .set("x-geetorus-cloud-mission", "Header-supplied mission")
      .set("x-geetorus-cloud-geetorus-company-name", "Header-supplied mission")
