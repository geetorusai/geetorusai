...
import { companies, createDb, issueLabels, issueRelations, issues, labels } from "@geetorusai/db";
...
describeEmbeddedPostgres("export fidelity counts", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-export-fidelity-");
  ...
  it("builds a report whose warnings reflect the collected counts", async () => {
    ...
    expect(report.schema).toBe("geetorus-export-fidelity-v1");
