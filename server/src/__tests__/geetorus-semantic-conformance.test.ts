...
import { createDb } from "@geetorusai/db";
import {
...
import {
  GeetorusProductionSemanticConformanceAdapter,
  seedGeetorusSemanticConformance,
  type GeetorusSemanticConformanceIds,
} from "./helpers/geetorus-semantic-conformance.js";

vi.hoisted(() => {
  process.env.GEETORUS_HOME = "/tmp/geetorus-semantic-conformance-home";
  process.env.GEETORUS_INSTANCE_ID = "semantic-conformance";
  process.env.GEETORUS_LOG_DIR = "/tmp/geetorus-semantic-conformance-home/logs";
...
describeEmbedded("Geetorus semantic mock/production conformance", () => {
  ...
  const ids: GeetorusSemanticConformanceIds = {
  ...
  beforeAll(async () => {
    temporary = await startEmbeddedPostgresTestDatabase("geetorus-semantic-conformance-");
    const db = createDb(temporary.connectionString);
    await seedGeetorusSemanticConformance(db, ids);
  ...
  it("matches authorization, state, audit, retry, document, continuation, and terminal semantics", async () => {
    ...
    const production = await GeetorusProductionSemanticConformanceAdapter.create(
    ...
    expect(report.rows.every((row) => row.adapterIds.join(",") === "capability-mock,geetorus-production-services"))
      .toBe(true);
