...
import { GeetorusRunnerToolAuthority } from "../services/native-runtime/geetorus-runner-tool-authority.js";
import { renderGeetorusWakePrompt, resolveGeetorusDesiredSkillNames, resolveLegacyGeetorusDesiredSkillNames } from "@geetorusai/adapter-utils/server-utils";
...
import { emailSendSchema } from "@geetorusai/shared";
import { chatChannelService } from "../services/chat-channels.js";

describe("AgentMail durable email pipeline", () => {
  let database: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>>;
  let db: ReturnType<typeof createDb>;
  const folder = mkdtempSync(path.join(os.tmpdir(), "geetorus-email-"));
  const previous = process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
  const services: EmailChannelService[] = [];
  beforeAll(async () => {
    process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = path.join(
      folder,
      "master.key",
    );
    database = await startEmbeddedPostgresTestDatabase("geetorus-email-");
  ...
  afterAll(async () => {
    ...
    else process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = previous;
  ...
  it("installs one connector skill and provider tools only for the assigned agent", async () => {
    ...
    const base = { geetorusSkillSync: { desiredSkills: [] } };
    const configured = await applyConnectorSkills(base, [], assignments);
    expect(base.geetorusSkillSync.desiredSkills).toEqual([]);
    expect(resolveGeetorusDesiredSkillNames(configured, configured.geetorusRuntimeSkills)).toEqual(["geetorusai/geetorus/agentmail"]);
    expect(resolveLegacyGeetorusDesiredSkillNames(configured, configured.geetorusRuntimeSkills)).toContain("geetorusai/geetorus/agentmail");
    const markdown = readFileSync(path.join(configured.geetorusRuntimeSkills[0].source, "SKILL.md"), "utf8");
    ...
    for (const adapterType of ["cursor_local", "gemini_local", "opencode_local", "pi_local", "codex_local"]) {
      ...
      expect(delivery.config.geetorusRuntimeSkills).toEqual([]);
      expect(delivery.config.geetorusConnectorSkillDigest).toBe(configured.geetorusConnectorSkillDigest);
      for (const resumedSession of [false, true]) {
        expect(renderGeetorusWakePrompt({ connectorSkillInstructions: delivery.instructions }, { resumedSession })).toContain(f.endpointId);
      }
    }
    const nativeDelivery = await prepareConnectorSkillDelivery(configured, "geetorus_runner");
    expect(nativeDelivery.instructions).toBe("");
    expect(nativeDelivery.config.geetorusRuntimeSkills).toHaveLength(1);
    const authority = new GeetorusRunnerToolAuthority(db, { ...binding, issueId: randomUUID(), runId: randomUUID(), connectorAssignments: assignments });
    ...
    const disconnected = await applyConnectorSkills(configured, configured.geetorusRuntimeSkills, []);
    expect(disconnected.geetorusRuntimeSkills).toEqual([]);
    expect(disconnected.geetorusConnectorSkillDigest).toBeNull();
    expect(resolveGeetorusDesiredSkillNames(disconnected, [])).toEqual([]);
    expect(new GeetorusRunnerToolAuthority(db, { ...binding, issueId: randomUUID(), runId: randomUUID() }).definitions().some((tool) => String(tool.name).startsWith("agentmail_"))).toBe(false);
  ...
  it("deduplicates multiple inboxes into one skill and changes the runtime bundle on reassignment", async () => {
    ...
    const after = await applyConnectorSkills(before, before.geetorusRuntimeSkills, assignments);
    expect(after.geetorusRuntimeSkills).toHaveLength(1);
    expect(after.geetorusConnectorSkillDigest).not.toBe(before.geetorusConnectorSkillDigest);
    expect(after.geetorusRuntimeSkills[0].source).not.toBe(before.geetorusRuntimeSkills[0].source);
    const markdown = readFileSync(path.join(after.geetorusRuntimeSkills[0].source, "SKILL.md"), "utf8");
  ...
  async function fixture(mode: "websocket" | "webhook" = "webhook", storage?: StorageService) {
    ...
    const service = emailChannelService(db, {
      heartbeat: { wakeup },
      fetch: fetcher,
      publicBaseUrl: "https://geetorus.example.test",
      createSocket,
      storage,
    });
  ...
  it("allows a bound normal agent to queue email, preserves budget limits, and rejects generic chat publication", async () => {
    ...
    const authority = new GeetorusRunnerToolAuthority(db, {
