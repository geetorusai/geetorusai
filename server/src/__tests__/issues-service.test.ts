...
import { buildAgentMentionHref, buildProjectMentionHref, MAX_ISSUE_REQUEST_DEPTH, type IssueWorkMode } from "@geetorusai/shared";
...
describeEmbeddedPostgres("issueService run attachment artifacts", () => {
  ...
  it("registers a run-produced attachment as an attachment-backed artifact work product", async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-run-attachment-artifact-");
...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-service-");
  ...
  it("ranks direct description matches ahead of comment-only matches", async () => {
    ...
    await db.insert(issueComments).values({
      ...
      body: "Reference: https://github.com/geetorusai/geetorus/pull/3303",
    });
  ...
  it("hides plugin operation issues from default lists and inbox-style filters while preserving explicit retrieval", async () => {
    ...
    await db.insert(issues).values([
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:geetorus.missions:operation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:geetorus.missions:operation:evaluation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:geetorusai.content-machine:evaluation",
    ...
    await expect(svc.list(companyId, { originKind: "plugin:geetorus.missions:operation" }))
    ...
    await expect(svc.list(companyId, { originId: "mission-alpha:operation-1" }))
      .resolves.toEqual([expect.objectContaining({ id: operationIssueId })]);
    await expect(svc.list(companyId, { originKindPrefix: "plugin:geetorus.missions:operation" }))
...
describeEmbeddedPostgres("issueService.findOpenAncestorCreatedByAgent", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-ancestor-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-create-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
  ...
  it("createChild targeting another project does not forward the parent project workspace", async () => {
    ...
    await db.insert(projects).values([
      { id: parentProjectId, companyId, name: "Geetorus App", status: "in_progress" },
      { id: targetProjectId, companyId, name: "Geetorus ID", status: "in_progress" },
    ]);
    ...
    const { issue: child } = await svc.createChild(parentIssueId, {
      title: "Implement the Geetorus ID connect broker",
      status: "todo",
      priority: "medium",
      projectId: targetProjectId,
      executionWorkspaceInheritanceMode: "strategy_only",
...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-blockers-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-mentioned-projects-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.clearExecutionRunIfTerminal", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-execution-lock-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("accepted plan decomposition", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-accepted-plan-decomposition-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.assertCheckoutOwner stale checkout adoption", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-checkout-owner-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.addComment createdByRunId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issues-comment-runid-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  ...
  it("nulls out a non-UUID x-geetorus-run-id instead of 500-ing", async () => {
