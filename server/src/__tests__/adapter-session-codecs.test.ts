...
import { sessionCodec as claudeSessionCodec } from "@geetorusai/adapter-claude-local/server";
import { sessionCodec as codexSessionCodec, isCodexUnknownSessionError } from "@geetorusai/adapter-codex-local/server";
import {
...
import { sessionCodec as acpxSessionCodec } from "@geetorusai/adapter-utils/acpx-engine/session-codec";
...
describe("adapter session codecs", () => {
  ...
  it("preserves claude ACP session params for ACP lane resumes", () => {
    const parsed = claudeSessionCodec.deserialize({
      sessionKey: "geetorus:company:agent:task:fingerprint",
