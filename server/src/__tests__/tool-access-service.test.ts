...
import { invalidateGeetorusCloudConnectorCapabilities, type GeetorusCloudConnector } from "../services/geetorus-cloud-connector.js";
...
async function createBrokerConnection(
  ...
  const [application] = await db
    ...
    .values({
      ...
      name: `Geetorus Pages ${randomUUID()}`,
      type: "mcp_http",
      status: "active",
    })
    .returning();
...
describeEmbeddedPostgres("tool access service", () => {
  ...
  beforeAll(async () => {
    const externalDatabaseUrl =
      process.env.GEETORUS_TOOL_ACCESS_TEST_DATABASE_URL?.trim();
    if (externalDatabaseUrl) {
      db = createDb(externalDatabaseUrl);
      return;
    }
    tempDb = await startEmbeddedPostgresTestDatabase(
      "geetorus-tool-access-service-",
    );
  ...
  it("allows an explicitly allowlisted internal token broker through the guarded fetch", async () => {
    vi.stubEnv(
      "GEETORUS_TOKEN_BROKER_ALLOWED_HOSTS",
  ...
  it.each(
    [
      { sourceTemplateKey: "anthropic", connectionMethodKey: "api-key" },
      {
        ...
        templateId: "geetorus.echo-calculator-time",
      },
    ].flatMap((config) =>
  ...
  it("registers an approved local stdio template and exposes its runtime slot", async () => {
    ...
    const connection = await service.createConnection(company.id, {
      ...
      config: { templateId: "geetorus.echo-calculator-time" },
      enabled: true,
      status: "active",
    });
    const health = await service.checkHealth(connection.id);
    ...
    expect(health.runtimeSlot).toMatchObject({
      ...
      status: "stopped",
      commandTemplateKey: "geetorus.echo-calculator-time",
    });
    ...
    expect(runtimeSlots).toEqual([
      expect.objectContaining({
        connectionId: connection.id,
        providerRef: "template:geetorus.echo-calculator-time",
        healthStatus: "ok",
      }),
    ]);
  });
  ...
  it("implicitly allowlists the configured Pages API host for internal token brokers", async () => {
    vi.stubEnv("GEETORUS_PAGES_API_URL", "http://127.0.0.1:8787");
    const company = await createCompany(db);
    const service = createTestToolAccessService(db, {
      deploymentMode: "authenticated",
      deploymentExposure: "public",
  ...
  it("cancels an ask-first test request when approval signing is unavailable", async () => {
    vi.stubEnv("GEETORUS_TOOL_ACTION_SIGNING_SECRET", "");
    const company = await createCompany(db);
    const userId = `tool-tester-${randomUUID()}`;
    await grantBoardUser(db, company.id, userId, ["tools:use"]);
    const agent = await createAgent(db, company.id);
    ...
    expect(res.body).toMatchObject({
      ...
      error: expect.stringContaining("GEETORUS_TOOL_ACTION_SIGNING_SECRET"),
    });
    const [actionRequest] = await db
      .select()
      .from(toolActionRequests)
  ...
  it("exposes managed Google methods only for profiles signed for this enrolled instance", async () => {
    ...
    const nonPilotConnector: GeetorusCloudConnector = {
      ...pilotConnector,
    ...
    const nonPilot = await request(
      createRouteApp(
        ...
        undefined,
        { geetorusCloudConnector: nonPilotConnector },
      ),
    ...
    expect(
      nonPilotGmail.methods.some(
        (method: { oauthStrategy?: string }) =>
          method.oauthStrategy === "geetorus_cloud_connector",
      ),
    ).toBe(false);
    expect(
    ...
    const pilot = await request(
      createRouteApp(
        ...
        boardSessionActor(company.id, "owner", userId),
        undefined,
        { geetorusCloudConnector: pilotConnector },
      ),
    ).get(`/api/companies/${company.id}/tools/gallery`);
  ...
  it("validates a reviewed Vercel connector and stores no provider bearer or vault secret", async () => {
    vi.stubEnv("GEETORUS_VERCEL_CONNECT_ENABLED", "true");
    vi.stubEnv(
      "GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN",
    ...
    const connected = await service.connectGalleryApp(
      company.id,
      {
        ...
        vercelConnect: { connector: "posthog-geetorus" },
        configValues: { projectId: "12345", mode: "tools" },
        grantKind: "organization",
      },
      { actorType: "user", actorId: "board" },
    ...
    await expect(
      service.connectGalleryApp(
        company.id,
        {
          ...
          connectionMethodKey: "mcp-api-key",
          credentialSource: "vercel_connect",
          vercelConnect: { connector: "linear-geetorus" },
          configValues: { projectId: "12345", mode: "tools" },
        },
        { actorType: "user", actorId: "board" },
      ),
  ...
  it("binds the Vercel OAuth callback to one company, actor, session, and one-time state", async () => {
    ...
    const getToken = vi.fn<VercelConnectClient["getToken"]>(async () => ({
      ...
      tokenId: "stk_notion",
      expiresAt: Date.now() + 60_000,
      connector: { id: "scl_notion", uid: "notion-geetorus", type: "oauth" },
    ...
    const connected = await service.connectGalleryApp(
      company.id,
      {
        galleryKey: "notion",
        credentialSource: "vercel_connect",
        vercelConnect: { connector: "notion-geetorus" },
        grantKind: "user",
      },
  ...
  it("rejects raw Google Sheets connection patches that claim another company's spreadsheet", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        config: {
          templateId: "geetorus.google-sheets",
  ...
  it.each(GOOGLE_WORKSPACE_CONNECTOR_PROFILE_IDS.flatMap((profile) => [
    ...
    ["authenticated", "public", "https://tenant.geetorus.app"] as const,
    ...
    async ({ profile, deploymentMode, deploymentExposure, origin }) => {
      ...
      const app = createRouteApp(db,
        ...
        undefined, { deploymentMode, deploymentExposure, geetorusCloudConnector: connector });
      ...
      const service = createTestToolAccessService(db, { geetorusCloudConnector: connector });
      ...
      mockToolsList([]);
      const completed = await service.completeGeetorusCloudConnectorCallback({
  ...
  it.each(GOOGLE_WORKSPACE_CONNECTOR_PROFILE_IDS)("connects advertised Workspace %s with a Cloud-delivered environment identity", async (profile) => {
    ...
    vi.stubEnv("GEETORUS_AUTH_PUBLIC_BASE_URL", "https://tenant.geetorus.app");
    vi.stubEnv("GEETORUS_CLOUD_CONNECTOR_BASE_URL", "https://my.geetorus.app");
    vi.stubEnv("GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT", "production");
    vi.stubEnv("GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID", "inst-cloud-workspace-regression");
    vi.stubEnv("GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY", signing.privateKey.export({ type: "pkcs8", format: "pem" }).toString());
    vi.stubEnv("GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY", sealing.privateKey.export({ type: "pkcs8", format: "pem" }).toString());
    invalidateGeetorusCloudConnectorCapabilities();
    const cloudRequest = vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      ...
      if (String(url) === "https://my.geetorus.app/v1/connector/instance-status") {
        expect(claims.op).toBe("status");
        return Response.json({ active: true, status: "active", profiles: [profile] });
      }
      expect(String(url)).toBe("https://my.geetorus.app/v1/connector/sessions");
      expect(claims).toMatchObject({
        op: "session", prf: profile, cid: company.id, sub: userId,
        ruri: "https://tenant.geetorus.app/api/tools/oauth/cloud-connector/callback",
      });
      return Response.json({
        confirmationUrl: "https://my.geetorus.app/connections/confirm?id=test-workspace-session",
        expiresAt: new Date(Date.now() + 600_000).toISOString(),
      });
    });
    try {
      ...
      expect(result.body.auth.startUrl).toBe("https://my.geetorus.app/connections/confirm?id=test-workspace-session");
  ...
  it.each(GOOGLE_WORKSPACE_CONNECTOR_PROFILE_IDS.flatMap((profile) =>
    ...
    const app = createRouteApp(db, undefined, undefined, { geetorusCloudConnector: connector });
  ...
  it("completes brokered Gmail OAuth with a single database connection", async () => {
    ...
    const service = createTestToolAccessService(callbackDb, {
      geetorusCloudConnector: connector,
    });
    const actor = { actorType: "user" as const, actorId: userId };
    ...
    try {
      ...
      const connected = await service.connectGalleryApp(
        ...
        {
          galleryKey: "gmail",
          connectionMethodKey: "geetorus-draft",
          grantKind: "user",
      ...
      const started = await service.startOAuth(
        ...
        {
          redirectUri:
            "https://geetorus.example/api/tools/oauth/cloud-connector/callback",
          actor,
        },
      );
      ...
      const completed = await Promise.race([
        service.completeGeetorusCloudConnectorCallback({
          state,
          claimId: "gmail-claim",
          actor,
        }),
        ...
        expect(grant).toMatchObject({
          ...
          providerTenant: {
            ...
            github: {
              ...
              installationIds: ["101"],
              installationUrl:
                "https://github.com/apps/geetorus-development/installations/new",
  ...
  it("replaces an archived dedicated GitHub identity with an explicitly selected personal identity", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
      ...
      if (
        ...
        return mcpHttpResponse({
          total_count: 1,
          repositories: [{ id: 1, full_name: "geetorusai/repo-1" }],
        });
      }
      if (href === GITHUB_CONNECTOR_PROFILES["github.code"].serverUrl) {
        return mcpHttpResponse({
    ...
    try {
      ...
      await service.completeGeetorusCloudConnectorCallback({
    ...
    try {
      const connected = await service.connectGalleryApp(
        company.id,
        {
          ...
          connectionMethodKey: "geetorus-read",
          grantKind: "user",
  ...
  it("routes a managed Drive callback into the shared organization vault when selected", async () => {
    ...
    try {
      ...
      expect(grants[0]).toMatchObject({
        ...
        providerTenant: {
          externalId: userId,
          oauth: { strategy: "geetorus_cloud_connector" },
        },
      });
      expect(
  ...
  it("activates allowed Drive write actions without approval defaults after a managed callback", async () => {
    ...
    try {
      const connected = await service.connectGalleryApp(
        ...
        {
          galleryKey: "google-drive",
          connectionMethodKey: "geetorus-write",
          grantKind: "user",
  ...
  it("serializes brokered Gmail OAuth completion behind membership revocation", async () => {
    ...
    const removalDb = createDb(tempDb!.connectionString, { maxConnections: 1 });
    const service = createTestToolAccessService(callbackDb, {
      geetorusCloudConnector: fakeGmailConnector(company.id, userId),
    });
    const actor = { actorType: "user" as const, actorId: userId };
    const gmailDefinition = getConnectableAppDefinition("gmail")!;
    const previousOwnershipAvailability = gmailDefinition.ownershipAvailability;
  ...
  it("synchronizes shared OAuth credentials to the organization grant used by gateway calls", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_SLACK_CLIENT_ID", "slack-client-id");
    vi.stubEnv(
      "GEETORUS_TOOL_OAUTH_SLACK_CLIENT_SECRET",
      "slack-client-secret",
    );
    const company = await createCompany(db);
    ...
    const started = await service.startOAuth(
      ...
      connected.connectionId,
      {
        redirectUri: "https://geetorus.example/api/tools/oauth/callback",
  ...
  it("returns a pre-scoped personal Notion callback directly to Permissions", async () => {
    vi.stubEnv("GEETORUS_PUBLIC_URL", "https://geetorus.example");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_NOTION_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_NOTION_CLIENT_SECRET", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_SECRET", "");
    const company = await createCompany(db);
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      ...
      if (href === "https://mcp.notion.com/register") {
        return mcpHttpResponse({
          ...
          redirect_uris: ["https://geetorus.example/api/tools/oauth/callback"],
  ...
  it("does not derive an OAuth callback origin from a non-loopback request host", async () => {
    ...
    const connectRes = await request(app)
      .post(`/api/companies/${company.id}/tools/apps/connect`)
      .set("Host", "geetorus.example.test")
    ...
    expect(connectRes.body).toMatchObject({
      ...
      error:
        "This Geetorus needs a browser-reachable HTTPS address (or loopback HTTP) before browser sign-in can start.",
    });
  });
  ...
  it("uses an authenticated same-origin HTTPS browser request without public URL config", async () => {
    ...
    const connectRes = await request(app)
      .post(`/api/companies/${company.id}/tools/apps/connect`)
      .set("Host", "geetorus.tail123.ts.net")
      .set("Origin", "https://geetorus.tail123.ts.net")
      .send({ galleryKey: "slack", name: "Tailscale Slack workspace" });

    expect(connectRes.status).toBe(201);
    expect(
      new URL(connectRes.body.auth.startUrl).searchParams.get("redirect_uri"),
    ).toBe("https://geetorus.tail123.ts.net/api/tools/oauth/callback");
  });
  ...
  it("requires non-viewer board access to start OAuth for active app connections", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_SLACK_CLIENT_ID", "slack-client-id");
    vi.stubEnv("GEETORUS_PUBLIC_URL", "http://geetorus.test");
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
    const connect = await service.connectGalleryApp(
      company.id,
  ...
  it("discovers Notion MCP OAuth metadata, registers one public client, and reuses it", async () => {
    ...
    const redirectUri =
      "https://geetorus-dev.tail29c1aa.ts.net/api/tools/oauth/callback";
    ...
    expect(registrationBodies).toEqual([
      {
        client_name: "Geetorus (geetorus-dev.tail29c1aa.ts.net)",
        redirect_uris: [redirectUri],
        grant_types: ["authorization_code", "refresh_token"],
        response_types: ["code"],
        token_endpoint_auth_method: "none",
        // PAP-17087: Geetorus's callback is a server-side HTTPS endpoint, so
  ...
  it("supports confidential DCR clients without exposing their registration secret", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_SUPABASE_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_SUPABASE_CLIENT_SECRET", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_SECRET", "");
    const company = await createCompany(db);
    ...
    const redirectUri = "https://geetorus.example/api/tools/oauth/callback";
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockImplementation(async (url, init) => {
        const href = String(url);
  ...
  it("preserves the provider's DCR client-auth ordering for Miro token exchange", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_MIRO_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_MIRO_CLIENT_SECRET", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_SECRET", "");
    const company = await createCompany(db);
    await grantBoardUser(db, company.id, "board", ["tools:manage_connections"]);
  ...
  it("accepts provider-added DCR grants without adopting them", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_HUGGING_FACE_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_HUGGING_FACE_CLIENT_SECRET", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_SECRET", "");
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
  ...
  it("does not request refresh-token registration from a provider that explicitly omits it", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CODA_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CODA_CLIENT_SECRET", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_SECRET", "");
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
  ...
  it("normalizes a public DCR client's zero secret expiry when no secret was issued", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_MIXPANEL_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_MIXPANEL_CLIENT_SECRET", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_SECRET", "");
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
  ...
  it("fails fast when Notion DCR is attempted from a non-loopback HTTP origin", async () => {
    ...
    await expect(
      service.startOAuth(company.id, connected.connectionId, {
        redirectUri: "http://geetorus-dev:3100/api/tools/oauth/callback",
        actor: { actorType: "user", actorId: "board" },
      }),
    ).rejects.toMatchObject({
      status: 422,
  ...
  it("leases rotating OAuth refresh tokens across service instances before concurrent remote app calls", async () => {
    ...
    const start = await service.startOAuth(company.id, connect.connectionId, {
      redirectUri: "http://geetorus.test/api/tools/oauth/callback",
      actor: { actorType: "user", actorId: "board" },
    });
  ...
  it("uses OAuth client credentials for shared machine-to-machine MCP connections", async () => {
    vi.stubEnv("GEETORUS_TOOL_OAUTH_M2M_CLIENT_ID", "m2m-client-id");
    vi.stubEnv("GEETORUS_TOOL_OAUTH_M2M_CLIENT_SECRET", "m2m-client-secret");
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
    const connection = await service.createConnection(company.id, {
      name: "Machine OAuth",
  ...
  it("cancels invalid-signature pending action requests but keeps unsigned in-flight ones out of the review queue without cancelling them", async () => {
    vi.stubEnv("GEETORUS_TOOL_ACTION_SIGNING_SECRET", "current-secret");
    const company = await createCompany(db);
    const [application] = await db
      .insert(toolApplications)
      .values({
  ...
  it("discovers OAuth for pasted MCP links and completes sign-in without a gallery entry", async () => {
    vi.stubEnv(
      "GEETORUS_TOOL_OAUTH_GENERIC_EXAMPLE_TEST_CLIENT_ID",
      "generic-client-id",
    );
    vi.stubEnv(
      "GEETORUS_TOOL_OAUTH_GENERIC_EXAMPLE_TEST_CLIENT_SECRET",
  ...
  it("starts OAuth only for the marked Smoke Lab HTTP fixture", async () => {
    ...
    const [application] = await db
      ...
      .values({
        companyId: company.id,
        applicationKey: "geetorus.smoke-lab.http-fixture",
    ...
    expect(`${authorizationUrl.origin}${authorizationUrl.pathname}`).toBe(
      `http://geetorus.test/api/companies/${company.id}/smoke-lab/oauth/authorize`,
    );
    ...
    expect(fetchMock.mock.calls.map(([url]) => String(url))).toContain(
      `http://geetorus.test/api/companies/${company.id}/smoke-lab/oauth/token`,
    );
    expect(fetchMock.mock.calls.map(([url]) => String(url))).toContain(
  ...
  it("summarizes runtime health and flags stale slots plus degraded connections", async () => {
    ...
    const [connection] = await db
      ...
      .values({
        ...
        enabled: true,
        config: { templateId: "geetorus.echo-calculator-time" },
        transportConfig: { templateId: "geetorus.echo-calculator-time" },
    ...
    await db.insert(toolRuntimeSlots).values({
      ...
      applicationId: application.id,
      connectionId: connection.id,
      slotKey: `${connection.id}:geetorus.echo-calculator-time`,
      ownerScopeType: "connection",
      ownerScopeId: connection.id,
      runtimeKind: "local_stdio",
      status: "running",
    ...
    expect(preview.drafts).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          warnings: [expect.stringContaining("Geetorus secret")],
        }),
        expect.objectContaining({
          name: "local",
          transport: "local_stdio",
          ...
          warnings: [expect.stringContaining("approved Geetorus template")],
        }),
      ]),
    );
  });
  ...
  it("sweeps enabled active connection health and records failing connections", async () => {
    ...
    const [pluginApplication] = await db.insert(toolApplications).values({
      companyId: company.id,
      applicationKey: `geetorus_plugin:fixture-${randomUUID()}`,
      name: "Plugin placeholder",
      type: "geetorus_plugin",
      status: "active",
    ...
    const [pluginConnection] = await db.insert(toolConnections).values({
      ...
      status: "active",
      enabled: true,
      config: { type: "geetorus_plugin" },
      transportConfig: { type: "geetorus_plugin" },
      healthStatus: "ok",
  ...
  it("builds a concrete Shopify endpoint from the validated store domain", () => {
    expect(
      normalizeConnectionMethodConfig(shopifyMethod, {
        storeDomain: "geetorus-demo.myshopify.com",
      }),
    ).toEqual({
      values: { storeDomain: "geetorus-demo.myshopify.com" },
      url: "https://geetorus-demo.myshopify.com/api/mcp",
    });
  });
