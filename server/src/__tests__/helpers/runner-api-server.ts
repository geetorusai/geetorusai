...
export async function startRunnerApiTestServer() {
  const root = await mkdtemp(join(tmpdir(), "geetorus-api-eval-"));
  const temporary = await startEmbeddedPostgresTestDatabase("geetorus-api-eval-db-");
  ...
  return {
    ...
    async fixture(options: { mode?: "standard" | "ask" | "planning"; apiToolsEnabled?: boolean; reset?: boolean; conversation?: boolean; connectionScenario?: RunnerConnectionScenario } = {}) {
      ...
      await db.insert(agents).values({ id: agentId, companyId, name: "API eval agent", adapterType: "geetorus_runner", adapterConfig: { provider: "codex", cwd: workspace }, runtimeConfig: { heartbeat: { enabled: false } }, status: "active" });
      ...
      if (options.connectionScenario === "stale_owner") {
        ...
        await db.insert(agents).values({ id: replacementAgentId, companyId, name: "New task owner", adapterType: "geetorus_runner", adapterConfig: { provider: "codex" }, runtimeConfig: { heartbeat: { enabled: false } }, status: "active" });
      ...
      return {
        ...
        authority: new GeetorusRunnerToolAuthority(db, binding),
        async snapshot() {
          return {
    ...
    async close() {
      ...
      await app.locals.geetorusShutdown();
