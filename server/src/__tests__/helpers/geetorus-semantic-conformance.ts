...
export interface GeetorusSemanticConformanceIds {
...
interface ProductionWorld {
  readonly port: GeetorusRouteSemanticPort;
 ...
 */
export class GeetorusProductionSemanticConformanceAdapter implements SemanticConformanceAdapter {
  readonly id = "geetorus-production-services";
  readonly kind = "production_binding" as const;

  private constructor(readonly worlds: ReadonlyMap<string, ProductionWorld>) {}

  static async create(db: Db, ids: GeetorusSemanticConformanceIds): Promise<GeetorusProductionSemanticConformanceAdapter> {
    ...
    for (const [id, binding] of Object.entries(ids.worlds)) {
      const port = new GeetorusRouteSemanticPort(db, app, ids, binding);
      await port.refresh();
      worlds.set(id, {
        port,
    ...
    return new GeetorusProductionSemanticConformanceAdapter(worlds);
  }
...
export async function seedGeetorusSemanticConformance(
  db: Db,
  ids: GeetorusSemanticConformanceIds,
): Promise<void> {
...
class GeetorusRouteSemanticPort {
  ...
  constructor(
    ...
    readonly ids: GeetorusSemanticConformanceIds,
  ...
  context(runId: string): CapabilityRunContext {
    ...
    return {
      schema: "geetorus.capability.run-context.v1",
      company: {
  ...
  async refresh(): Promise<void> {
    ...
    state.runs = [{
      ...
      backendKind: "runner",
      sourceInstanceId: "geetorus-production-services",
  ...
  private post(path: string, body: unknown) {
    return request(this.app).post(path).set("X-Geetorus-Run-Id", this.binding.runId).send(body);
  }

  private put(path: string, body: unknown) {
    return request(this.app).put(path).set("X-Geetorus-Run-Id", this.binding.runId).send(body);
  }

  private patch(path: string, body: unknown) {
    return request(this.app).patch(path).set("X-Geetorus-Run-Id", this.binding.runId).send(body);
  }
}
...
function createProductionAuthorityApp(db: Db, ids: GeetorusSemanticConformanceIds): Application {
  const app = express();
  ...
  app.use((req, _res, next) => {
    req.actor = {
      ...
      agentId: ids.actorId,
      companyId: ids.companyId,
      runId: req.header("X-Geetorus-Run-Id") ?? undefined,
      source: "agent_jwt",
    };
    next();
  });
