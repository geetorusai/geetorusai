...
import type { InstanceExperimentalSettings } from "@geetorusai/shared";
import {
...
describe("instance settings service", () => {
  ...
  it("defaults enableGeetorusDeveloperMode to false for empty and legacy settings", () => {
    expect(normalizeExperimentalSettings(undefined).enableGeetorusDeveloperMode).toBe(false);
    expect(normalizeExperimentalSettings({}).enableGeetorusDeveloperMode).toBe(false);
    expect(
    ...
    const next = applyExperimentalSettingsPatch(
      ...
      { enableWorktreeRunExecution: true },
      {
        ...
        runtimeEnv: {
          ...
          GEETORUS_INSTANCE_ID: "worktree-instance",
        },
      },
    );
  ...
  it("fails closed when worktree run execution was activated by another instance", async () => {
    ...
    await expect(
      resolveWorktreeRunExecutionActivationState({
        ...
        runtimeEnv: {
          GEETORUS_IN_WORKTREE: "true",
          GEETORUS_INSTANCE_ID: "target-instance",
        },
      }),
    ).resolves.toMatchObject({
      armed: false,
