...
import { agents, companies, createDb, heartbeatRuns, issues, projects, projectWorkspaces } from "@geetorusai/db";
import { runLocalGit, setExpensiveWorkspaceGitExecutor } from "@geetorusai/adapter-utils/git-workspace-sync";
...
suite("task project repository provisioning", () => {
  ...
  beforeAll(async () => {
    root = await mkdtemp(path.join(os.tmpdir(), "geetorus-project-repos-"));
    vi.stubEnv("GEETORUS_HOME", path.join(root, "home"));
    vi.stubEnv("GEETORUS_MULTI_PROJECT_WORKSPACE_SYNC", "false");
  ...
  it.each([
    ...
    const copies = (execute.mock.calls.find(([input]) => input.runId === retry!.id)![0].context.geetorusWorkspaces as Array<{ cwd: string }>).filter((hint) => hint.cwd.includes(".geetorus-repositories"));
  ...
  it.each([1, 2])("gives a task all %i repositories without any configured local folders", async (count) => {
    ...
    const hints = input.context.geetorusWorkspaces as Array<{ workspaceId: string; cwd: string }>;
    for (let index = 0; index < count; index++) {
