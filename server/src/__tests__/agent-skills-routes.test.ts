...
vi.mock("@geetorusai/shared/telemetry", () => ({
...
function registerModuleMocks() {
  vi.doMock("@geetorusai/shared/telemetry", () => ({
    trackAgentCreated: mockTrackAgentCreated,
    trackErrorHandlerCrash: vi.fn(),
  }));
...
describe.sequential("agent skill routes", () => {
  beforeEach(async () => {
    ...
    mockCompanySkillService.resolveRequestedSkillEntries.mockImplementation(
      async (_companyId: string, requested: Array<{ key: string; versionId?: string | null }>) => ({
        resolved: requested.map((entry) => ({
          key: entry.key === "geetorus" ? "geetorusai/geetorus/geetorus" : entry.key,
    ...
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["geetorusai/geetorus/geetorus"],
      entries: [],
      warnings: [],
    });
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          geetorusRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  }, 10_000);
  ...
  it("threads a non-undefined actor secret context into resolveAdapterConfigForRuntime on both skills routes (audit fidelity, skipUserSecrets preserved)", async () => {
    ...
    const syncRes = await requestApp(
      ...
      (baseUrl) => request(baseUrl)
        ...
        .send({ desiredSkills: ["geetorus"], mode: "replace" }),
    );
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          agent: "codex",
          geetorusSkillSync: expect.objectContaining({
            desiredSkills: ["geetorusai/geetorus/geetorus"],
          }),
        }),
      }),
  ...
  it("adds only named desired skills while preserving existing assignments", async () => {
    mockAgentService.getById.mockResolvedValue({
      ...makeAgent("claude_local"),
      adapterConfig: {
        geetorusSkillSync: { desiredSkills: ["company-1/keep"] },
      },
    });

    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["geetorus"], mode: "add" }));
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          geetorusSkillSync: {
            desiredSkills: ["company-1/keep", "geetorusai/geetorus/geetorus"],
          },
        }),
      }),
      expect.any(Object),
  ...
  it("removes only named desired skills while preserving other assignments", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["geetorus"], mode: "remove" }));
  ...
  it("accepts version pins while beta skills are enabled", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      ...
      .send({
        mode: "replace",
        desiredSkills: [{ key: "geetorusai/geetorus/geetorus", versionId }],
      }));
  ...
  it("preserves stale desired keys instead of 422-ing when syncing (PAP-13222)", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["geetorus", "stale/removed/skill"], mode: "replace" }));
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          geetorusSkillSync: expect.objectContaining({
            desiredSkills: ["geetorusai/geetorus/geetorus", "stale/removed/skill"],
          }),
        }),
      }),
      expect.any(Object),
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["geetorusai/geetorus/geetorus"], mode: "replace" }));

    expect(res.status, JSON.stringify(res.body)).toBe(200);
    expect(mockAdapter.syncSkills).toHaveBeenCalled();
  });

  it("ignores the reserved legacy Geetorus skill for geetorus_runner", async () => {
    mockAgentService.getById.mockResolvedValue(makeAgent("geetorus_runner"));
  ...
  it("allows geetorus_runner to remove a pre-existing legacy Geetorus skill", async () => {
    mockAgentService.getById.mockResolvedValue({
      ...makeAgent("geetorus_runner"),
      adapterConfig: {
        geetorusSkillSync: {
          desiredSkills: ["company-1/keep", "geetorusai/geetorus/geetorus"],
        },
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["geetorusai/geetorus/geetorus"], mode: "remove" }));
  ...
  it("materializes the bundled default instruction set for non-CEO agents with no prompt template", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(mockAgentInstructionsService.materializeManagedBundle).toHaveBeenCalledWith(
        expect.any(Object),
        expect.objectContaining({
          "AGENTS.md": expect.stringContaining("skills/geetorus/scripts/geetorus-upload-artifact.sh"),
        }),
        expect.any(Object),
      );
    });
  ...
  it("seeds the chief-of-staff persona for the onboarding first agent", async () => {
    ...
    expect(mockAgentService.create.mock.calls[0][1].adapterConfig.geetorusSkillSync.desiredSkills)
      .toContain("geetorusai/geetorus/first-task");
  });

  it.each(["codex_local", "claude_local"].flatMap((adapterType) => [
    ["agents", "geetorusai/geetorus/geetorus-create-agent"],
    ["agent-hires", "geetorusai/geetorus/geetorus-create-agent"],
    ["agents", "geetorus"],
    ["agent-hires", "geetorus"],
    ["agents", "geetorusai/geetorus/first-task"],
    ["agent-hires", "geetorusai/geetorus/first-task"],
    ...
    const canonicalKey = skill === "geetorus" ? "geetorusai/geetorus/geetorus" : skill;
    const expected = ["geetorus", "geetorus-board", "geetorus-converting-plans-to-tasks", "geetorus-create-agent", "para-memory-files", "first-task"]
      .map((name) => ({ key: `geetorusai/geetorus/${name}`, versionId: `geetorusai/geetorus/${name}` === canonicalKey ? versionId : null }));
    expect(input.adapterConfig.geetorusSkillSync.desiredSkills).toEqual(expect.arrayContaining(expected));
    expect(input.adapterConfig.geetorusSkillSync.desiredSkills).toHaveLength(6);
  });
  ...
  it.each(["agents", "agent-hires"])("leaves ordinary general agents' defaults unchanged via %s", async (route) => {
    ...
    expect(res.status, JSON.stringify(res.body)).toBe(201);
    expect(mockAgentService.create.mock.calls[0][1].adapterConfig.geetorusSkillSync).toBeUndefined();
  });
  ...
  it.each(["agents", "agent-hires"])("does not assign first-task to ordinary CEOs via %s", async (route) => {
    ...
    expect(res.status, JSON.stringify(res.body)).toBe(201);
    const desiredSkills = mockAgentService.create.mock.calls[0][1].adapterConfig.geetorusSkillSync.desiredSkills;
    expect(desiredSkills).toContain("geetorusai/geetorus/geetorus");
    expect(desiredSkills).not.toContain("geetorusai/geetorus/first-task");
  });
  ...
  it("gives a CEO hire the core geetorus skills when none are requested", async () => {
  ...
  it("omits the legacy operational skill from geetorus_runner CEO defaults", async () => {
    ...
    const createInput = mockAgentService.create.mock.calls[0]?.[1] as {
      adapterConfig: { geetorusSkillSync: { desiredSkills: string[] } };
    };
    expect(createInput.adapterConfig.geetorusSkillSync.desiredSkills).not.toContain(
      "geetorusai/geetorus/geetorus",
    );
    expect(createInput.adapterConfig.geetorusSkillSync.desiredSkills).toContain(
      "geetorusai/geetorus/geetorus-board",
    );
  });
  ...
  it("unions requested skills with the CEO defaults instead of replacing them", async () => {
    ...
    const desired = createInput.adapterConfig.geetorusSkillSync.desiredSkills;
    // "geetorus" resolves to its canonical key and dedupes with the default.
    expect(desired).toHaveLength(5);
    expect(desired).toContain("geetorusai/geetorus/geetorus");
  });
  ...
  it("does not add default skills to non-CEO hires", async () => {
    ...
    expect(createInput.adapterConfig.geetorusSkillSync).toBeUndefined();
  });
