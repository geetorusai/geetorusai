...
describe("assertCompanyAccess", () => {
  ...
  it("logs only in shadow mode for responsible-user company access denials", () => {
    const previous = process.env.GEETORUS_RESPONSIBLE_USER_AUTHZ_SHADOW;
    process.env.GEETORUS_RESPONSIBLE_USER_AUTHZ_SHADOW = "true";
    try {
      const req = makeReq({
        method: "PATCH",
        actor: {
      ...
      if (previous === undefined) delete process.env.GEETORUS_RESPONSIBLE_USER_AUTHZ_SHADOW;
      else process.env.GEETORUS_RESPONSIBLE_USER_AUTHZ_SHADOW = previous;
    }
  });
