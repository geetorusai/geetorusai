...
describe("local service supervision", () => {
  ...
  it("keeps request-logging runtime stdio usable after the supervisor side closes", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-service-stdio-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-service-home-"));
    ...
    process.env.GEETORUS_INSTANCE_ID = `service-stdio-${randomUUID()}`;
