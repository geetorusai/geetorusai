...
describeEmbeddedPostgres("smoke lab service pack and results API", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-smoke-lab-");
  ...
  it("runs the deterministic fake OAuth code, refresh, userinfo, and revoke flow", async () => {
    ...
    expect(page.text).toContain("smoke@geetorus.test");
    ...
    const userinfo = await request(app)
      ...
      .expect(200);
    expect(userinfo.body).toMatchObject({ sub: "smoke-user-1", email: "smoke@geetorus.test" });
  ...
  it("requires a loopback or same-origin HTTP(S) redirect URI before rendering or completing consent", async () => {
    ...
    await enableSmokeLab(db);
    const app = createRouteApp(db);
    vi.stubEnv("GEETORUS_PUBLIC_URL", "http://geetorus-dev:45439");
    ...
    await request(app)
      .get(`/api/companies/${company.id}/smoke-lab/oauth/authorize`)
      .query({ client_id: "smoke-client", redirect_uri: "http://geetorus-dev:45439/callback", response_type: "code" })
      .expect(200);
    ...
    await request(app)
      ...
      .send({
        client_id: "smoke-client",
        redirect_uri: "http://geetorus-dev:45439/api/tools/oauth/callback",
        email: "smoke@geetorus.test",
        password: "smoke-password",
      })
      .expect(302);
  ...
  it("creates runs and lets an agent JWT actor record step results", async () => {
    ...
    const step = await request(agentApp)
      ...
      .send({
        ...
        screenshotArtifactRef: { provider: "geetorus", attachmentId: randomUUID() },
        durationMs: 42,
      })
      .expect(201);
