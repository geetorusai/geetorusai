...
function craftAgentJwtWithoutResponsibleClaim(input: {
  ...
  // instance defaults to "default" (beforeEach clears GEETORUS_INSTANCE_ID),
...
describe("agent auth middleware", () => {
  const originalSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
  const originalTtl = process.env.GEETORUS_AGENT_JWT_TTL_SECONDS;
  const originalInstanceId = process.env.GEETORUS_INSTANCE_ID;

  beforeEach(() => {
    process.env.GEETORUS_AGENT_JWT_SECRET = "auth-middleware-secret";
    process.env.GEETORUS_AGENT_JWT_TTL_SECONDS = "3600";
  ...
  afterEach(() => {
    if (originalSecret === undefined) delete process.env.GEETORUS_AGENT_JWT_SECRET;
    else process.env.GEETORUS_AGENT_JWT_SECRET = originalSecret;
    if (originalTtl === undefined) delete process.env.GEETORUS_AGENT_JWT_TTL_SECONDS;
    else process.env.GEETORUS_AGENT_JWT_TTL_SECONDS = originalTtl;
  ...
  it("reports an expired agent JWT specifically", async () => {
    ...
    const token = craftAgentJwtWithoutResponsibleClaim({
      secret: process.env.GEETORUS_AGENT_JWT_SECRET!,
      agentId,
      companyId,
  ...
  it("rejects mismatched run headers for agent JWTs and audits the spoof attempt", async () => {
    ...
    const res = await request(createApp(db))
      .get("/actor")
      .set("Authorization", `Bearer ${token}`)
      .set("X-Geetorus-Run-Id", spoofedRunId);
  ...
  it("rejects fork-minted run JWTs before issue reads or writes reach live issue data", async () => {
    ...
    process.env.GEETORUS_INSTANCE_ID = "pap-12899-worktree";
