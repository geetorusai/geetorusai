...
function createRegisteredRepairFixture(
  ...
  const configDir = path.join(workspaceCwd, ".geetorus");
  const sourceConfigPath = path.join(baseCwd, ".geetorus", "config.json");
  ...
  fs.writeFileSync(path.join(path.dirname(sourceConfigPath), ".env"), "GEETORUS_INSTANCE_ID=repair-source\n");
  fs.writeFileSync(path.join(configDir, "config.json"), "{}\n");
  fs.writeFileSync(path.join(configDir, ".env"), `GEETORUS_INSTANCE_ID=${targetInstanceId}\n`);
...
describe.sequential("execution workspace runtime control conflict and failure reconciliation", () => {
  ...
  it("returns 422 with a stable reason when the repair seed manifest is malformed", async () => {
    const workspaceCwd = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-route-repair-malformed-"));
    try {
      const configDir = path.join(workspaceCwd, ".geetorus");
      fs.mkdirSync(configDir, { recursive: true });
      fs.writeFileSync(path.join(configDir, "config.json"), "{}\n");
  ...
  it.each(["sibling", "foreign_instance", "symlink", "instance_mismatch"] as const)(
    ...
    async (variant) => {
      const fixture = createRegisteredRepairFixture(`geetorus-route-repair-${variant}-`);
      ...
      try {
        ...
        fs.writeFileSync(
          ...
          `GEETORUS_INSTANCE_ID=${variant === "foreign_instance" ? "foreign-source" : "repair-source"}\n`,
        );
  ...
  it("rejects repair when the execution workspace has no registered base workspace", async () => {
    const workspaceCwd = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-route-repair-no-source-"));
    try {
      const configDir = path.join(workspaceCwd, ".geetorus");
      fs.mkdirSync(configDir, { recursive: true });
      fs.writeFileSync(path.join(configDir, "config.json"), "{}\n");
  ...
  it("rejects repair when the registered base workspace has no runnable Geetorus CLI", async () => {
    const fixture = createRegisteredRepairFixture("geetorus-route-repair-no-cli-", { withCli: false });
    try {
      const res = await request(await createApp())
        .post(`/api/execution-workspaces/${executionWorkspaceId}/runtime-commands/repair`)
        .send({});
  ...
  it("returns 422 with a stable reason when the verified manifest names another instance", async () => {
    const fixture = createRegisteredRepairFixture("geetorus-route-repair-wrong-instance-");
    try {
  ...
  it("uses the recorded instance pointer consistently for readiness, handoff, and repair", async () => {
    ...
    const fixture = createRegisteredRepairFixture("geetorus-route-repair-success-", {
    ...
    try {
      ...
      expect(resolveManagedWorkspaceIdentity({
        ...
        executionWorkspaceId,
        companyId: "company-1",
        env: { GEETORUS_WORKSPACE_HANDOFF_SECRET: "test-root-secret" },
      ...
      mockSpawn.mockImplementation((_command: string, args: string[], options: { env?: NodeJS.ProcessEnv }) => {
        ...
        expect(options.env).toMatchObject({
          ...
          GEETORUS_WORKSPACE_BASE_CWD: fixture.baseCwd,
          GEETORUS_PROJECT_WORKSPACE_ID: projectWorkspaceId,
        });
        const child = new EventEmitter() as EventEmitter & {
          stdout: PassThrough;
          stderr: PassThrough;
  ...
  it("completes database-only repair when no managed runtime service is configured", async () => {
    const fixture = createRegisteredRepairFixture("geetorus-route-repair-db-only-");
    try {
      mockExecutionWorkspaceService.getById.mockResolvedValue(buildExecutionWorkspace({
  ...
  it("fails repair at the exact seed phase and leaves managed services stopped", async () => {
    const fixture = createRegisteredRepairFixture("geetorus-route-repair-failure-");
    try {
      mockEnsurePersistedExecutionWorkspaceAvailable.mockResolvedValue({ cwd: fixture.workspaceCwd });
      mockSpawn.mockImplementation(() => {
        const child = new EventEmitter() as EventEmitter & {
