...
import { agentWakeupRequests, agents, companies, createDb, heartbeatRuns, heartbeatRunEvents, issueComments, issueThreadInteractions, issues } from "@geetorusai/db";
...
(support.supported ? describe : describe.skip)("durable execution identity", () => {
  ...
  beforeAll(async () => {
    database = await startEmbeddedPostgresTestDatabase("geetorus-run-identity-");
  ...
  async function seedInterrupt() {
    ...
    await db.insert(agentWakeupRequests).values([
      { id: queueId, companyId: input.companyId, agentId: input.agentId, source: "automation",
        ...
        payload: { issueId: input.issueId, _geetorusWakeContext: { wakeCommentIds: input.messageIds },
      ...
      if (fault === "malformed" || fault === "missing") {
        ...
        if (fault === "other-agent") {
          ...
          { payload: { ...receipt.payload, ...(fault === "other-task" ? { issueId: randomUUID() } :
            { _geetorusWakeContext: { wakeCommentIds: [randomUUID()] } }) } },
