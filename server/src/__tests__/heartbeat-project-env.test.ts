...
import { buildSkillMentionHref } from "@geetorusai/shared";
import {
...
describe("resolveExecutionRunAdapterConfig", () => {
  it("does not preflight or resolve legacy GitHub token bindings for managed executions", async () => {
    const assertNoGitHubBinding = (env: Record<string, unknown>) => {
      for (const key of Object.keys(env)) if (/^(GH_TOKEN|GITHUB_TOKEN|GH_ENTERPRISE_TOKEN|GITHUB_ENTERPRISE_TOKEN|GEETORUS_GIT_TOKEN)$/.test(key)) {
    ...
    const result = await resolveExecutionRunAdapterConfig({
      ...
      routineEnv: { GITHUB_ENTERPRISE_TOKEN: missing, GEETORUS_GIT_TOKEN: missing, ROUTINE_VALUE: "ok" },
      secretsSvc: {
  ...
  it("drops GEETORUS_API_KEY bindings but forwards other GEETORUS_-named env to resolution", async () => {
    ...
    const result = await resolveExecutionRunAdapterConfig({
      ...
      environmentId: "environment-1",
      environmentEnv: {
        GEETORUS_API_KEY: "environment-api-key",
        GEETORUS_RUNNER_NETWORK_ACCESS: "enabled",
        GEETORUS_CLOUD_PROVIDER_TOKEN_ENV: "environment-cloud",
        ENV_ONLY: "environment-only",
      },
      executionRunConfig: {
        env: {
          GEETORUS_API_KEY: { type: "secret_ref", secretId: "secret-api-key", version: "latest" },
          GEETORUS_CLOUD_PROVIDER_TOKEN_AGENT: "agent-cloud",
          AGENT_ONLY: "agent-only",
        },
      },
      projectEnv: {
        GEETORUS_API_KEY: "project-api-key",
        GEETORUS_CLOUD_PROVIDER_TOKEN_PROJECT: "project-cloud",
        PROJECT_ONLY: "project-only",
      },
      routineEnv: {
        GEETORUS_API_KEY: "routine-api-key",
        GEETORUS_CLOUD_PROVIDER_TOKEN_ROUTINE: "routine-cloud",
        ROUTINE_ONLY: "routine-only",
      },
      routineId: "routine-1",
      secretsSvc: {
    ...
    expect(JSON.stringify(result.resolvedConfig.env)).not.toContain("GEETORUS_API_KEY");
    expect(JSON.stringify(result.resolvedConfig.env)).not.toContain("GEETORUS_RUNNER_NETWORK_ACCESS");
  });
...
describe("resolveExecutionRunAdapterConfig codex_local credential pre-dispatch gate", () => {
  ...
  async function stubManagedCodexEnv(options: { seedSharedAuth: boolean }) {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-gate-"));
    ...
    vi.stubEnv("GEETORUS_HOME", geetorusHome);
    vi.stubEnv("GEETORUS_INSTANCE_ID", "default");
