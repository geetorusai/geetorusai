...
import { activityLog, companies, companyMemberships, instanceUserRoles } from "@geetorusai/db";
import {
  actorMiddleware,
...
describe("actorMiddleware authenticated session profile", () => {
  const originalCloudTenantToken = process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN;

  afterEach(() => {
    if (originalCloudTenantToken === undefined) delete process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN;
    else process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN = originalCloudTenantToken;
  });
  ...
  it("trusts Cloud tenant identity headers and seeds board access", async () => {
    process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN = "tenant-token";
    ...
    const res = await request(app)
      .get("/actor")
      .set("x-geetorus-cloud-tenant-token", "tenant-token")
      .set("x-geetorus-cloud-user-id", "global-user-1")
      .set("x-geetorus-cloud-user-email", "owner@example.com")
      .set("x-geetorus-cloud-user-name", "Stack Owner")
      .set("x-geetorus-cloud-stack-id", "stack-alpha")
      .set("x-geetorus-cloud-geetorus-company-id", "geetorus-stack-alpha")
      .set("x-geetorus-cloud-geetorus-company-name", "Purple Rain")
      .set("x-geetorus-cloud-stack-role", "owner");
  ...
  it("lets the cloud tenant actor through assertCompanyAccess for a company it holds a membership row in", async () => {
    ...
    const cloudHeaders = {
      "x-geetorus-cloud-tenant-token": "tenant-token",
      "x-geetorus-cloud-user-id": "global-user-1",
      "x-geetorus-cloud-user-email": "owner@example.com",
      "x-geetorus-cloud-stack-id": "stack-alpha",
      "x-geetorus-cloud-stack-role": "member",
    };
  ...
  it("repairs a legacy machine company name from the trusted human-name header", async () => {
      ...
      select: vi.fn(() => ({
        from: (table: unknown) => ({
          where: () =>
            Promise.resolve(
              table === companies
                ? [{ name: "geetorus-stack-purple-rain" }]
                : [],
            ),
        }),
      })),
    ...
    const res = await request(app)
      ...
      .set("x-geetorus-cloud-user-id", "global-user-1")
      .set("x-geetorus-cloud-user-email", "owner@example.com")
      .set("x-geetorus-cloud-stack-id", "stack-purple-rain")
      .set(
        "x-geetorus-cloud-geetorus-company-id",
        "geetorus-stack-purple-rain",
      )
      .set("x-geetorus-cloud-geetorus-company-name", "Purple Rain")
      .set("x-geetorus-cloud-stack-role", "owner");
    ...
    expect(activities).toEqual([
      expect.objectContaining({
        ...
        details: expect.objectContaining({
          ...
          previousName: "geetorus-stack-purple-rain",
          name: "Purple Rain",
        }),
      }),
    ]);
...
describe("Cloud tenant company naming", () => {
  const ids = {
    ...
    geetorusCompanyId: "geetorus-stack-purple-rain",
  };

  it.each([
    "geetorus-stack-purple-rain",
    "stack-purple-rain Geetorus",
    ids.companyId,
  ...
  it("humanizes the stack slug for old harnesses without a name header", () => {
    ...
    expect(humanizeCloudStackSlug("geetorus-stack-purple-rain")).toBe(
      "Purple Rain",
    );
  });
});
