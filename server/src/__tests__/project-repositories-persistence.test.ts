...
import { companies, companyMemberships, companySecrets, connectionGrants, connectionGrantMembers, toolApplications, toolConnections, createDb, projects as projectTable } from "@geetorusai/db";
...
(support.supported ? describe : describe.skip)("project repository persistence", () => {
  ...
  beforeAll(async () => {
    temp = await startEmbeddedPostgresTestDatabase("geetorus-repositories-");
