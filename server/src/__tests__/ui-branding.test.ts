...
<head>
    <!-- GEETORUS_RUNTIME_BRANDING_START -->
    <!-- GEETORUS_RUNTIME_BRANDING_END -->
    <!-- GEETORUS_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- GEETORUS_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from GEETORUS_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ GEETORUS_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ GEETORUS_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ GEETORUS_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      GEETORUS_IN_WORKTREE: "true",
      GEETORUS_WORKTREE_NAME: "geetorus-pr-432",
      GEETORUS_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("geetorus-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="geetorus-worktree-name"');
    expect(meta).toContain('content="geetorus-pr-432"');
    expect(meta).toContain('name="geetorus-worktree-color"');
  });
  ...
  it("omits the instance-id meta when the runtime id is unset", () => {
    ...
    expect(renderRuntimeBrandingMeta(branding)).not.toContain('name="geetorus-instance-id"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="geetorus-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="geetorus-worktree-name"');
  });
});
