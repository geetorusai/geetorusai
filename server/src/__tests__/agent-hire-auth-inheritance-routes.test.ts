...
describeEmbeddedPostgres("hired agent provider credential inheritance", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-agent-hire-auth-inheritance-${randomUUID()}`);

  beforeAll(async () => {
    mkdirSync(secretsTmpDir, { recursive: true });
    process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = path.join(secretsTmpDir, "master.key");
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-agent-hire-auth-inheritance-");
