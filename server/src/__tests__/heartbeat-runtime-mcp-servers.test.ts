...
import { buildGeetorusRuntimeMcpServers, createManagedMcpRunConfig } from "../services/heartbeat.js";
...
describeEmbeddedPostgres("heartbeat runtime MCP servers", () => {
  ...
  const originalApiUrl = process.env.GEETORUS_API_URL;

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-heartbeat-runtime-mcp-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    if (originalApiUrl === undefined) delete process.env.GEETORUS_API_URL;
    else process.env.GEETORUS_API_URL = originalApiUrl;
  ...
  it("provisions one aggregate gateway and omits unavailable access without blocking any runtime", async () => {
    process.env.GEETORUS_API_URL = "https://geetorus.example.test";
    ...
    const first = await buildGeetorusRuntimeMcpServers({ db, agent: agent!, runId: randomUUID() });
    const second = await buildGeetorusRuntimeMcpServers({ db, agent: agent!, runId: randomUUID() });

    expect(first).toHaveLength(1);
    expect(first[0]).toMatchObject({
      name: "geetorus-assigned",
      connectionId: expect.stringMatching(/^assignment:[a-f0-9]{64}$/),
      url: expect.stringMatching(/^https:\/\/geetorus\.example\.test\/mcp\/gateways\/gw_[a-f0-9]{32}$/),
    ...
    await expect(
      buildGeetorusRuntimeMcpServers({
        db,
        agent: agent!,
        runId: randomUUID(),
  ...
  it("preserves exact permissions when an aggregate assignment exceeds the public 250-entry edit limit", async () => {
    ...
    const servers = await buildGeetorusRuntimeMcpServers({ db, agent: agent!, runId: randomUUID() });
    ...
    const reused = await buildGeetorusRuntimeMcpServers({ db, agent: agent!, runId: randomUUID() });
  ...
  it("exposes only the dedicated GitHub connection when a personal connection is also installed", async () => {
    ...
    const servers = await buildGeetorusRuntimeMcpServers({ db, agent: agent!, runId: run!.id });
  ...
  it("audits permitted remote MCP connections that were not installed when delivery is empty", async () => {
    ...
    const servers = await buildGeetorusRuntimeMcpServers({ db, agent: agent!, runId });
