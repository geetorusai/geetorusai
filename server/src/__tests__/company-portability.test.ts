...
import type { CompanyPortabilityFileEntry } from "@geetorusai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "geetorus",
        name: "geetorus",
        description: "Geetorus coordination skill",
        markdown: "---\nname: geetorus\ndescription: Geetorus coordination skill\n---\n\n# Geetorus\n",
        sourceType: "github",
        sourceLocator: "https://github.com/geetorusai/geetorus/tree/master/skills/geetorus",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/geetorusai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "geetorusai",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/geetorusai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "geetorusai",
  ...
  it("exports referenced skills as stubs by default with sanitized Geetorus extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Geetorus"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${geetorusKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/geetorus/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/geetorus/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/geetorusai/geetorus/geetorus/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".geetorus.yaml"]);
    expect(extension).toContain('schema: "geetorus/v1"');
    ...
    expect(extension).not.toContain("geetorusSkillSync");
  ...
  it("exports agent permission grants through the Geetorus extension and manifest", async () => {
    const db = {
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".geetorus.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });
  ...
  it("exports default sidebar order into the Geetorus extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".geetorus.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/geetorus/SKILL.md"])).toContain("# Geetorus");
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/geetorus/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/geetorus/references/api.md"])).toContain("# API");
  });

  it("exports catalog skill provenance in portable Geetorus frontmatter", async () => {
    const portability = companyPortabilityService({} as any);
    const catalogKey = "geetorusai/bundled/software-development/review";
    ...
    const catalogSkill = {
      ...
      markdown: "---\nname: review\ndescription: Catalog review skill\n---\n\n# Review\n",
      sourceType: "catalog",
      sourceLocator: "/tmp/geetorus/catalog/review",
      ...
      metadata: {
        sourceKind: "catalog",
        skillKey: catalogKey,
        catalogId: "geetorusai:bundled:software-development:review",
        catalogKey,
        catalogKind: "bundled",
        catalogCategory: "software-development",
        catalogPath: "catalog/bundled/software-development/review",
        packageName: "@geetorusai/skills-catalog",
    ...
    const skillMarkdown = asTextFile(exported.files["skills/geetorusai/bundled/software-development/review/SKILL.md"]);
    expect(skillMarkdown).toContain("geetorus:");
    expect(skillMarkdown).toContain("catalog:");
    expect(skillMarkdown).toContain(`sourceRef: "${originHash}"`);
    expect(skillMarkdown).toContain('catalogId: "geetorusai:bundled:software-development:review"');
    ...
    expect(skillMarkdown).toContain('packageName: "@geetorusai/skills-catalog"');
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/geetorusai/geetorus/geetorus/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/geetorusai/geetorus/geetorus/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .geetorus.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".geetorus.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-geetorus",
        companyId: "company-1",
        key: "geetorusai/geetorus/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/geetorusai/geetorus/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/geetorusai/geetorus/release-changelog/SKILL.md"])).toContain("geetorusai/geetorus/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/geetorus",
            repoUrl: "https://github.com/geetorusai/geetorus.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .geetorus-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/geetorusai/geetorus.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/geetorus");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Geetorus",
      },
  ...
  it("normalizes invalid imported project icon names to null", async () => {
    ...
    await portability.importBundle({
      source: { type: "inline", rootPath: "geetorus-demo", files },
      include: { company: true, agents: false, projects: true, issues: false },
      target: { mode: "new_company", newCompanyName: "Imported Geetorus" },
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/geetorusai/geetorus.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".geetorus.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/geetorusai/geetorus.git"');
    expect(extension).toContain('projectWorkspaceKey: "geetorus"');
  ...
  it("reads env inputs back from .geetorus.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .geetorus.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Geetorus routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("suffixes a manifest-derived company name that collides with an existing company", async () => {
    ...
    companySvc.list.mockResolvedValue([
      ...
      { name: "imported geetorus (2)" },
    ]);
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Geetorus (3)",
    });
    accessSvc.ensureMembership.mockResolvedValue(undefined);

    const files = {
      "COMPANY.md": ["---", 'schema: "agentcompanies/v1"', 'name: "Imported Geetorus"', "---", ""].join("\n"),
    };
  ...
  it("skips name de-duplication for agent-safe imports so collisions stay unobservable", async () => {
    ...
    companySvc.list.mockResolvedValue([{ name: "Imported Geetorus" }]);
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Geetorus",
    });
  ...
  it("imports recurring tasks without a project or assignee as paused routines", async () => {
    ...
    const request = {
      source: { type: "inline" as const, rootPath: "geetorus-demo", files },
      include: { company: true, agents: false, projects: false, issues: true, skills: false },
      target: { mode: "new_company" as const, newCompanyName: "Imported Geetorus" },
  ...
  it("imports a vendor-neutral package without .geetorus.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Geetorus");
  ...
  it("imports packaged skills and restores desired skill refs on agents", async () => {
    ...
    companySkillSvc.importPackageFiles.mockResolvedValueOnce([{
      ...
      action: "renamed",
      originalKey: "geetorus",
      originalSlug: "geetorus",
      requestedRefs: ["geetorus"],
      reason: "Existing skill matched; renamed to geetorus-2.",
    }]);
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    const geetorusYaml = `${exported.files[".geetorus.yaml"]}`;
    expect(geetorusYaml).not.toContain("company:");
    exported.files[".geetorus.yaml"] =
      `${geetorusYaml}company:\n  logoPath: "images/company-logo.png"\n`;
  ...
  it("skips blockers and documents of tasks excluded from the import selection", async () => {
    ...
    const result = await portability.importBundle({
      ...
      agents: "all",
      collisionStrategy: "rename",
      selectedFiles: ["COMPANY.md", ".geetorus.yaml", "tasks/pap-2/TASK.md"],
    }, "user-1");
  ...
  it("skips attachment export with a per-task warning when storage is unavailable", async () => {
    ...
    expect(asTextFile(exported.files[".geetorus.yaml"])).not.toContain("attachments:");
  });
  ...
  it("skips oversized and missing-blob attachments with warnings instead of failing", async () => {
    ...
    const previousCap = process.env.GEETORUS_ATTACHMENT_MAX_BYTES;
    process.env.GEETORUS_ATTACHMENT_MAX_BYTES = "10";
    vi.resetModules();
    try {
      ...
      if (previousCap === undefined) {
        delete process.env.GEETORUS_ATTACHMENT_MAX_BYTES;
      } else {
        process.env.GEETORUS_ATTACHMENT_MAX_BYTES = previousCap;
      }
      vi.resetModules();
    }
  });
  ...
  it("skips embedded image references that are foreign or dangling with one aggregate warning", async () => {
    ...
    expect(asTextFile(exported.files[".geetorus.yaml"])).not.toContain("embeddedAssets:");
    expect(exported.manifest.embeddedAssets).toEqual([]);
    expect(exported.warnings).toContain(
  ...
  it("prunes embedded asset entries and blobs when the referencing files are excluded from the export selection", async () => {
    ...
    const kept = await portability.exportBundle("company-1", {
      include: { company: true, agents: false, projects: false, issues: true },
      selectedFiles: ["COMPANY.md", ".geetorus.yaml", "tasks/pap-1/TASK.md", "tasks/pap-1/documents/spec.md", `blobs/${sha}`],
    });
    expect(asTextFile(kept.files[".geetorus.yaml"])).toContain("embeddedAssets:");
    expect(kept.files[`blobs/${sha}`]).toBeDefined();

    const pruned = await portability.exportBundle("company-1", {
      include: { company: true, agents: false, projects: false, issues: true },
      selectedFiles: ["COMPANY.md", ".geetorus.yaml"],
    });
    expect(Object.keys(pruned.files).some((filePath) => filePath.startsWith("blobs/"))).toBe(false);
    const prunedYaml = asTextFile(pruned.files[".geetorus.yaml"]);
  ...
  it("rejects packages produced by a newer Geetorus", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    await expect(portability.importBundle({
      ...
      agents: "all",
      collisionStrategy: "rename",
    }, "user-1")).rejects.toThrow(/newer Geetorus/);
    expect(issueSvc.importIssues).not.toHaveBeenCalled();
  });
    ...
    issueSvc.listComments.mockResolvedValue([
      {
        ...
        authorAgentId: null,
        authorUserId: null,
        body: "Geetorus needs a disposition before this issue can continue.",
        presentation,
        metadata,
      ...
      source: {
        ...
        files: {
          ...
          ".geetorus.yaml": [
            ...
            "      default:",
            "        name: App",
            "        repoUrl: https://github.com/geetorusai/geetorus",
            "        setupCommand: pnpm install",
            "",
          ].join("\n"),
        },
  ...
  it("rejects runner imports while disabled and accepts the same selection when enabled", async () => {
    ...
    await expect(portability.importBundle(request, "user-1")).rejects.toMatchObject({
      status: 422,
      details: { code: "geetorus_runner_rollout_disabled" },
    });
    expect(agentSvc.create).not.toHaveBeenCalled();
...
describe("dedupeImportedCompanyName", () => {
  it("returns the base name when nothing collides", () => {
    expect(dedupeImportedCompanyName("Geetorus", ["Other Co"])).toBe("Geetorus");
    expect(dedupeImportedCompanyName("Geetorus", [])).toBe("Geetorus");
  });

  it("suffixes past every taken candidate, case-insensitively", () => {
    expect(dedupeImportedCompanyName("Geetorus", ["geetorus"])).toBe("Geetorus (2)");
    expect(dedupeImportedCompanyName("Geetorus", ["Geetorus", "Geetorus (2)"])).toBe("Geetorus (3)");
    expect(dedupeImportedCompanyName("Geetorus", ["GEETORUS", "geetorus (2)"])).toBe("Geetorus (3)");
  });
