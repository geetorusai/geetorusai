...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "geetorus", LOGNAME: "geetorus", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "geetorus" }),
      },
    );

    expect(tokens).toEqual(["geetorus", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "geetorus" }, {
        userInfo: () => ({ username: "geetorus" }),
      });

      expect(tokens).toEqual(["geetorus", "custom-token"]);
    } finally {
