...
import { MCP_CONFIG_HELP_PROMPT } from "@geetorusai/shared";
import {
...
const PUBLIC_BASE_URL = "https://geetorus.fixture.test";
 ...
 * protocol parameters Geetorus sent (RFC 8707 `resource`, DCR metadata, PKCE).
 */
function installMcpOAuthFixture(options: FixtureOptions = {}) {
  ...
  const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
    ...
    if (href === MCP_URL && method === "POST") {
      ...
      return jsonResponse({ jsonrpc: "2.0", id: "geetorus-catalog-refresh", result: { tools } });
    }
    ...
    if (href === `${ISSUER}/register` && method === "POST") {
      ...
      return jsonResponse({
        ...
        // A conforming server echoes back what it registered, and Geetorus
...
describeEmbeddedPostgres("generic remote MCP connections", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-generic-mcp-");
  ...
  it("emits deployment guidance without exposing server env-var names", async () => {
    ...
    vi.stubEnv("GEETORUS_MANAGED_RUNTIME_PUBLIC_URL", "");
    const app = createRouteApp(db, {
      deploymentMode: "local_trusted",
      deploymentExposure: "private",
    });
    ...
    expect(JSON.stringify(response.body)).not.toContain("GEETORUS_PUBLIC_URL");
  });
  ...
  it("creates, refreshes, and disconnects a Composio toolkit child", async () => {
    ...
    expect(sessionRequests).toEqual([
      expect.objectContaining({ userId: `geetorus:${company.id}`, options: expect.objectContaining({ toolkits: ["github"], mcp: true }) }),
    ]);
    ...
    expect(connectRequests).toEqual([
      expect.objectContaining({ authConfigId: "auth-github", userId: `geetorus:${company.id}` }),
    ]);
  ...
  it("rejects header names Geetorus refuses to send", async () => {
    installMcpOAuthFixture({ auth: "public" });
    const company = await createCompany(db);
    const service = toolAccessService(db);
  ...
  it("discovers OAuth for a personal URL connection before its user grant exists", async () => {
    ...
    const company = await createCompany(db);
    const app = createRouteApp(db);
    vi.stubEnv("GEETORUS_PUBLIC_URL", PUBLIC_BASE_URL);
  ...
  it("creates one personal grant when two public URL setup retries probe concurrently", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async () => {
      ...
      if (probes === 2) release();
      await bothProbed;
      return jsonResponse({ jsonrpc: "2.0", id: "geetorus-catalog-refresh", result: { tools: FIXTURE_TOOLS } });
    });
  ...
  it("rolls back partial catalog and profile writes without removing the established public identity", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async () => jsonResponse({
      jsonrpc: "2.0", id: "geetorus-catalog-refresh",
  ...
  it("replaces a private-only Client ID Metadata Document with dynamic registration", async () => {
    ...
    const firstStart = await service.startOAuth(company.id, connected.connectionId, {
      redirectUri: "https://geetorus.tailnet.test:42001/api/tools/oauth/callback",
      actor: { actorType: "user", actorId: "board-user" },
    });
  ...
  it("prefers a deployment-preconfigured client over any registration", async () => {
    const fixture = installMcpOAuthFixture({ auth: "oauth", cimd: true });
    vi.stubEnv("GEETORUS_TOOL_OAUTH_CLIENT_ID", "preconfigured-client");
    const company = await createCompany(db);
    const service = toolAccessService(db);
  ...
  const HOSTILE_ERROR_DESCRIPTION =
    `\u001b[31mFATAL\u001b[0m **Geetorus needs your recovery key**: ${PROVIDER_CANARY} <script>alert(1)</script>`;
  ...
  it("redacts a hostile provider error from the token exchange", async () => {
    ...
    // Geetorus's own copy for `invalid_grant`, not a syllable of the provider's.
  ...
  it("normalizes an unrecognized provider error code instead of echoing it", async () => {
    ...
    // Geetorus's generic copy rather than naming the provider's code.
    expect(thrown).toMatchObject({
      status: 502,
  ...
  it("redacts a hostile provider error from dynamic client registration", async () => {
    ...
    expect(thrown).toMatchObject({
      status: 502,
      message: "The authorization server rejected Geetorus's callback URL.",
      details: {
  ...
  it("validates the callback state before acting on a provider-reported error", async () => {
    ...
    // An unsolicited callback carries no state Geetorus issued, so it is
    // rejected on that ground and never reaches the provider-error branch.
    await expect(service.completeOAuthCallback({
      state: "state-geetorus-never-issued",
      error: "access_denied",
      redirectUri: REDIRECT_URI,
      actor: { actorType: "user", actorId: "board-user" },
    })).rejects.toMatchObject({
  ...
  describe("denied and cancelled callbacks", () => {
    ...
    it("treats a cancel the same as a denial even when the provider names it its own way", async () => {
      ...
      // Real providers invent their own cancel codes. Whether or not Geetorus
    ...
    it("resolves the board's pending authorization prompt as rejected", async () => {
      ...
      // The prompt's reason is Geetorus's own copy, never the provider's.
  ...
  it("will not auto-re-register over a client the operator supplied", async () => {
    ...
    // The callback moved. Geetorus cannot re-register in the operator's console,
  ...
  /**
   * PAP-17099 — the authorization endpoint is the one discovered value Geetorus
  ...
  describe("unsafe advertised authorization endpoints", () => {
    ...
    it("never navigates to a stored authorization endpoint that is unsafe", async () => {
      ...
      // trusted just because it is in Geetorus's own database.
  ...
  it("uses the configured auth origin for self-hosted OAuth callbacks", async () => {
    vi.stubEnv("GEETORUS_PUBLIC_URL", "https://public.geetorus.example");
    vi.stubEnv("GEETORUS_AUTH_PUBLIC_BASE_URL", "https://auth.geetorus.example");
    const app = createRouteApp(db);

    const response = await request(app).get("/api/tools/oauth/client-metadata").expect(200);

    expect(response.body.redirect_uris).toEqual([
      "https://auth.geetorus.example/api/tools/oauth/callback",
    ]);
  });
