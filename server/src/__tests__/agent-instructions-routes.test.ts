...
describe("agent instructions bundle routes", () => {
  ...
  it("preserves geetorus skill-sync selections when switching adapters", async () => {
    // Desired skills live inside the per-adapter config under
    // `geetorusSkillSync`, yet they are adapter-agnostic company-level
    ...
    mockAgentService.getById.mockResolvedValue({
      ...
      adapterConfig: {
        model: "claude-sonnet-4",
        geetorusSkillSync: { desiredSkills: ["research", "code-review"] },
      },
    });
