...
describe("extract-search query validation", () => {
  it("accepts supported extraction filters and rejects unsafe or ambiguous input", () => {
    const parsed = companySearchExtractQuerySchema.parse({
      contains: "github.com/geetorusai/geetorus/pull",
      kind: "url",
      scope: "comments",
...
describeEmbeddedPostgres("companySearchExtractService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-company-search-extract-");
  ...
  async function createCompany(name = "Geetorus") {
  ...
  it("expands and deduplicates URLs across issue, comment, and document sources", async () => {
    ...
    const firstUrl = "https://github.com/geetorusai/geetorus/pull/123";
    const secondUrl = "https://github.com/geetorusai/geetorus/pull/456";
    const thirdUrl = "https://github.com/geetorusai/geetorus/pull/789";
  ...
  it("keeps URL sources selected by scheme-less queries", async () => {
    const companyId = await createCompany();
    const titleUrl = "https://github.com/geetorusai/geetorus/pull/101";
    const descriptionUrl = "https://github.com/geetorusai/geetorus/pull/102";
    const documentTitleUrl = "https://github.com/geetorusai/geetorus/pull/103";
    const documentBodyUrl = "https://github.com/geetorusai/geetorus/pull/104";
    const issueId = await createIssue(companyId, {
  ...
  it("uses the default distinct-match cap and marks truncation explicitly", async () => {
    ...
    const urls = Array.from(
      ...
      (_, index) => `https://github.com/geetorusai/geetorus/pull/${index + 1}`,
    );
