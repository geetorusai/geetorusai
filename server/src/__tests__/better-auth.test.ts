...
const ORIGINAL_INSTANCE_ID = process.env.GEETORUS_INSTANCE_ID;
const ORIGINAL_PUBLIC_URL = process.env.GEETORUS_PUBLIC_URL;

afterEach(() => {
  if (ORIGINAL_INSTANCE_ID === undefined) delete process.env.GEETORUS_INSTANCE_ID;
  else process.env.GEETORUS_INSTANCE_ID = ORIGINAL_INSTANCE_ID;
  if (ORIGINAL_PUBLIC_URL === undefined) delete process.env.GEETORUS_PUBLIC_URL;
  else process.env.GEETORUS_PUBLIC_URL = ORIGINAL_PUBLIC_URL;
});

describe("Better Auth cookie scoping", () => {
  it("derives an instance-scoped cookie prefix", () => {
    expect(deriveAuthCookiePrefix("default")).toBe("geetorus-default");
    expect(deriveAuthCookiePrefix("PAP-1601-worktree")).toBe("geetorus-PAP-1601-worktree");
  });

  it("uses GEETORUS_INSTANCE_ID for the Better Auth cookie prefix", () => {
    process.env.GEETORUS_INSTANCE_ID = "sat-worktree";

    const advanced = buildBetterAuthAdvancedOptions({ disableSecureCookies: false });

    expect(advanced).toEqual({
      cookiePrefix: "geetorus-sat-worktree",
    });
    expect(getCookies({ advanced } as BetterAuthOptions).sessionToken.name).toMatch(
      /geetorus-sat-worktree\.session_token$/,
    );
  });

  it("keeps local http auth cookies non-secure while preserving the scoped prefix", () => {
    process.env.GEETORUS_INSTANCE_ID = "pap-worktree";

    expect(buildBetterAuthAdvancedOptions({ disableSecureCookies: true })).toEqual({
      cookiePrefix: "geetorus-pap-worktree",
      useSecureCookies: false,
    });
    expect(getCookies({
      advanced: buildBetterAuthAdvancedOptions({ disableSecureCookies: true }),
    } as BetterAuthOptions).sessionToken.name).toBe("geetorus-pap-worktree.session_token");
  });
  ...
  it("uses an explicit public URL when deciding whether secure cookies are required", () => {
    expect(shouldDisableSecureAuthCookies({
      ...
      authBaseUrlMode: "auto",
      authPublicBaseUrl: undefined,
      publicUrl: "https://geetorus.example.test",
    })).toBe(false);
    ...
    expect(shouldDisableSecureAuthCookies({
      ...
      authPublicBaseUrl: "http://geetorus.local.test:3100",
      publicUrl: undefined,
    })).toBe(true);
  });
  ...
  it("derives secure cookie behavior from the configured public auth URL", () => {
    ...
    expect(shouldDisableSecureAuthCookies({
      deploymentMode: "authenticated",
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "http://geetorus-dev:46259",
    } as Parameters<typeof shouldDisableSecureAuthCookies>[0])).toBe(true);
    expect(shouldDisableSecureAuthCookies({
      deploymentMode: "authenticated",
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://geetorus.example.test",
  ...
  it("uses the caller-resolved public URL for cookie security", () => {
    process.env.GEETORUS_PUBLIC_URL = "https://ignored.example.test";

    expect(shouldDisableSecureAuthCookies({
      deploymentMode: "authenticated",
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://geetorus.example.test",
      publicUrl: "http://geetorus-dev:46259",
    } as Parameters<typeof shouldDisableSecureAuthCookies>[0])).toBe(true);
  });
