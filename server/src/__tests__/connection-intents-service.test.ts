...
describeEmbeddedPostgres("connectionIntentService", () => {
  ...
  beforeAll(async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("geetorus-connection-intents-");
  ...
  it("native authority discovers and requests services on an empty tool snapshot", async () => {
    ...
    const authority = new GeetorusRunnerToolAuthority(db, { companyId: claims.company_id, issueId, agentId: claims.sub, runId });
