...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    process.env.GEETORUS_INSTANCE_ROOT?.trim() ||
    path.join(process.env.HOME ?? "/tmp", ".geetorus-worktrees", "instances", "live-ssh-test");
  return path.join(instanceRoot, "env-lab", "ssh-fixture", "state.json");
}

/** Attempt to build config from explicit GEETORUS_ENV_LIVE_SSH_* env vars. */
function tryExplicitConfig(): {
  ...
  const host = process.env.GEETORUS_ENV_LIVE_SSH_HOST?.trim() ?? "";
  const username = process.env.GEETORUS_ENV_LIVE_SSH_USERNAME?.trim() ?? "";
  const remoteWorkspacePath =
    process.env.GEETORUS_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH?.trim() ?? "";
  const port = Number.parseInt(process.env.GEETORUS_ENV_LIVE_SSH_PORT ?? "22", 10);
 ...
 * 1. Explicit GEETORUS_ENV_LIVE_SSH_* env vars
...
async function resolveSshConfig(): Promise<SshConnectionConfig | null> {
  ...
  if (explicit) {
    return {
      ...
      privateKey: await readOptionalSecret(
        process.env.GEETORUS_ENV_LIVE_SSH_PRIVATE_KEY,
        process.env.GEETORUS_ENV_LIVE_SSH_PRIVATE_KEY_PATH,
      ),
      knownHosts: await readOptionalSecret(
        process.env.GEETORUS_ENV_LIVE_SSH_KNOWN_HOSTS,
        process.env.GEETORUS_ENV_LIVE_SSH_KNOWN_HOSTS_PATH,
      ),
      strictHostKeyChecking:
        (process.env.GEETORUS_ENV_LIVE_SSH_STRICT_HOST_KEY_CHECKING ?? "true").toLowerCase() !== "false",
    };
  }
  ...
  if (process.env.GEETORUS_ENV_LIVE_SSH_NO_AUTO_FIXTURE !== "true") {
...
const describeLiveSsh = (() => {
  ...
  if (process.env.GEETORUS_ENV_LIVE_SSH_NO_AUTO_FIXTURE === "true") {
    console.warn(
      "Skipping live SSH smoke test. Set GEETORUS_ENV_LIVE_SSH_HOST, GEETORUS_ENV_LIVE_SSH_USERNAME, and GEETORUS_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH to enable it, or remove GEETORUS_ENV_LIVE_SSH_NO_AUTO_FIXTURE to auto-start env-lab.",
    );
...
describeLiveSsh("live SSH environment smoke", () => {
  ...
  it("connects to the configured SSH environment and verifies basic runtime tools", async () => {
    ...
    if (!resolvedConfig) {
      console.warn(
        "Live SSH smoke test could not resolve SSH config from env vars or env-lab fixture. Set GEETORUS_ENV_LIVE_SSH_NO_AUTO_FIXTURE=true to mark this suite skipped intentionally.",
      );
      return;
    }
