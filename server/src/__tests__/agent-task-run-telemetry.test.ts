...
import { agents, companies, createDb, heartbeatRuns } from "@geetorusai/db";
import {
...
vi.mock("@geetorusai/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@geetorusai/shared/telemetry")>(
...
describeEmbeddedPostgres("emitAgentTaskRun", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-agent-task-run-telemetry-");
