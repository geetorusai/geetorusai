...
describeEmbeddedPostgres("plugin tenant isolation (company_id FK)", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-tenant-isolation-");
  ...
  async function seedPlugin() {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.tenant-isolation-test",
      packageName: "@geetorusai/plugin-tenant-isolation-test",
      version: "0.0.1",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.tenant-isolation-test",
        apiVersion: 1,
        version: "0.0.1",
