 ...
 *
 * - Cloud-managed instances (GEETORUS_MANAGED_CONFIG present) may only
...
describeEmbeddedPostgres("plugin install route security floor", () => {
  ...
  const originalManagedConfig = process.env["GEETORUS_MANAGED_CONFIG"];

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-install-guard-");
  ...
  afterEach(async () => {
    ...
    if (originalManagedConfig === undefined) {
      ...
      process.env["GEETORUS_MANAGED_CONFIG"] = originalManagedConfig;
    }
  });
  ...
  describe("cloud-managed instances", () => {
    it("rejects npm installs outright", async () => {
      process.env["GEETORUS_MANAGED_CONFIG"] = CLOUD_MANAGED_CONFIG;
      const app = await createInstallApp(db);

      const res = await request(app)
        .post("/api/plugins/install")
        .send({ packageName: "geetorus-plugin-anything", version: "1.0.0" });
    ...
    it("rejects npm installs even when the managed config document is corrupted", async () => {
      ...
      process.env["GEETORUS_MANAGED_CONFIG"] = CLOUD_MANAGED_CONFIG;
      const app = await createInstallApp(db);
      process.env["GEETORUS_MANAGED_CONFIG"] = "{definitely not json";

      const res = await request(app)
        .post("/api/plugins/install")
        .send({ packageName: "geetorus-plugin-anything" });
  ...
  describe("self-hosted npm installs (behavior unchanged)", () => {
    ...
    it("does not apply the cloud floor when GEETORUS_MANAGED_CONFIG is absent", async () => {
      const app = await createInstallApp(db);
      ...
      const res = await request(app)
        .post("/api/plugins/install")
        .send({ packageName: `geetorus-plugin-guard-missing-${randomUUID().slice(0, 8)}` });
