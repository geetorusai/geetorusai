 ...
 * local Geetorus instance's on-disk service registry still names, even with
 ...
 * the guest binds it; this suite does not close that window. `GEETORUS_HOME`
...
/**
 * The real, unredirected Geetorus home directory. `beforeEach` later points
 * `GEETORUS_HOME` at a throwaway temp dir for the suite under test, so this
 * must be read before that happens. Mirrors the default in
 * `resolveGeetorusHomeDir`.
 */
const REAL_GEETORUS_HOME = process.env.GEETORUS_HOME?.trim() || path.join(os.homedir(), ".geetorus");

/**
 * Every port a local Geetorus instance's on-disk service registry currently
 * records, read once per case from the real, unredirected Geetorus home. A
...
async function readLocallyLeasedPorts(): Promise<Set<number>> {
  ...
  const instancesDir = path.join(REAL_GEETORUS_HOME, "instances");
...
(embeddedPostgresSupport.supported ? describe : describe.skip)(
  ...
  () => {
    ...
    let geetorusHome: string;

    beforeAll(async () => {
      tempDb = await startEmbeddedPostgresTestDatabase("pap17419-reservation-");
      db = createDb(tempDb.connectionString);
      previousHttpsMode = process.env.GEETORUS_MANAGED_RUNTIME_HTTPS;
      process.env.GEETORUS_MANAGED_RUNTIME_HTTPS = "auto";
    }, 60_000);
    ...
    beforeEach(async () => {
      ...
      geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "pap17419-home-"));
      ...
      // Geetorus instance on this host and could confuse a live server.
      ...
      process.env.GEETORUS_INSTANCE_ID = `pap17419-${randomUUID()}`;
    });
    ...
    async function seedIncident(options?: { leaseStatus?: string; leasedRowStatus?: string }) {
      ...
      await db.insert(projects).values({ id: projectId, companyId, name: "Geetorus App", status: "in_progress" });
    ...
    function startInput(seed: Awaited<ReturnType<typeof seedIncident>>, executionWorkspaceId: string) {
      return {
        ...
        actor: { id: null, name: "Geetorus", companyId: seed.companyId },
        issue: null,
        db,
        workspace: {
