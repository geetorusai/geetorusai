...
async function createGitRepo() {
  ...
  const repoRoot = await realpath(await mkdtemp(path.join(os.tmpdir(), "geetorus-branch-containment-repo-")));
...
function readAdapterWorkspace(input: unknown) {
  ...
  const wake = context.geetorusWake as { executionWorkspace?: { branchName?: string } } | undefined;
...
async function seedBranchContainmentRun(
  ...
  await db.insert(executionWorkspaces).values([
    ...
    },
    {
      ...
      status: "active",
      cwd: path.join(repoRoot, ".geetorus", "worktrees", "other-workspace"),
      repoUrl: null,
      baseRef: "HEAD",
      branchName: "other-workspace",
      providerType: "git_worktree",
      providerRef: path.join(repoRoot, ".geetorus", "worktrees", "other-workspace"),
      lastUsedAt: now,
      openedAt: now,
      createdAt: now,
      updatedAt: now,
...
describeEmbeddedPostgres("heartbeat workspace branch containment", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-branch-containment-");
