...
describeEmbeddedPostgres("company skill import authorization routes", () => {
  ...
  const previousAgentJwtSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
  const previousGeetorusHome = process.env.GEETORUS_HOME;
  const previousGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;

  beforeAll(async () => {
    process.env.GEETORUS_AGENT_JWT_SECRET = "company-skills-import-authz-test-secret";
    geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-company-skills-import-authz-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "default";
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-company-skills-import-authz-");
  ...
  afterAll(async () => {
    ...
    if (previousAgentJwtSecret === undefined) delete process.env.GEETORUS_AGENT_JWT_SECRET;
    else process.env.GEETORUS_AGENT_JWT_SECRET = previousAgentJwtSecret;
  ...
  async function writeSkillFixture(companyId: string) {
    ...
    const skillDir = await fs.mkdtemp(path.join(managedRoot, "geetorus-import-authz-skill-"));
