...
describe.sequential("iMessage Photon channel control plane", () => {
  ...
  let secrets: string;
  const oldKey = process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
  const services: ChatChannelService[] = [];
  const companyIds: string[] = [];
  beforeAll(async () => {
    database = await startEmbeddedPostgresTestDatabase("geetorus-photon-");
  ...
  afterAll(async () => {
    ...
    else process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = oldKey;
  });
  async function setup(shared = false) {
    ...
    const makeService = () => {
      const service = chatChannelService(db, {
        runtime,
        publicBaseUrl: "https://geetorus.example",
        heartbeat: { wakeup },
        storage,
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(chatIdentityLinks.geetorusUserId, second),
        ),
      );
    expect(
