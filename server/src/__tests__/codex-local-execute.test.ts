...
import { runChildProcess } from "@geetorusai/adapter-utils/server-utils";
import { execute } from "@geetorusai/adapter-codex-local/server";
import { buildGeetorusTaskMarkdown } from "../services/heartbeat.js";
...
const capturePath = process.env.GEETORUS_TEST_CAPTURE_PATH;
const payload = {
  ...
  codexConfigContents: process.env.CODEX_HOME && fs.existsSync(process.env.CODEX_HOME + "/config.toml")
    ...
    : null,
  geetorusWakePayloadJson: process.env.GEETORUS_WAKE_PAYLOAD_JSON || null,
  geetorusApiUrl: process.env.GEETORUS_API_URL || null,
  geetorusApiKey: process.env.GEETORUS_API_KEY || null,
  geetorusApiBridgeMode: process.env.GEETORUS_API_BRIDGE_MODE || null,
  geetorusEnvKeys: Object.keys(process.env)
...
type CapturePayload = {
  ...
  geetorusWakePayloadJson: string | null;
  geetorusApiUrl?: string | null;
  geetorusApiKey?: string | null;
  geetorusApiBridgeMode?: string | null;
  geetorusEnvKeys: string[];
};
...
describe("codex execute", () => {
  it("uses a Geetorus-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-default-"));
    ...
    const previousGeetorusInWorktree = process.env.GEETORUS_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.GEETORUS_HOME = geetorusHome;
    delete process.env.GEETORUS_INSTANCE_ID;
    delete process.env.GEETORUS_IN_WORKTREE;
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            GEETORUS_TEST_CAPTURE_PATH: capturePath,
          },
          promptTemplate: "Follow the geetorus heartbeat.",
        },
        context: {},
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Geetorus-managed Codex home"),
        }),
      );
    } finally {
      ...
      if (previousGeetorusInWorktree === undefined) delete process.env.GEETORUS_IN_WORKTREE;
      else process.env.GEETORUS_IN_WORKTREE = previousGeetorusInWorktree;
  ...
  it("writes managed MCP gateways into Codex config and warns on overlapping direct entries without logging tokens", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-managed-mcp-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const previousHome = process.env.HOME;
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    const previousGeetorusApiUrl = process.env.GEETORUS_API_URL;
    const previousGeetorusRuntimeApiUrl = process.env.GEETORUS_RUNTIME_API_URL;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_API_URL = "http://geetorus.local:3100";
    process.env.GEETORUS_RUNTIME_API_URL = "http://geetorus.local:3100";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      const result = await execute({
        ...
        runtimeMcp: {
          getServers: () => [
            {
              name: "github",
              url: "http://geetorus.local:3100/api/tool-gateway/gateways/gateway-1/mcp",
      ...
      expect(configText).toContain("[mcp_servers.\"geetorus-github\"]");
      expect(configText).toContain('url = "http://geetorus.local:3100/api/tool-gateway/gateways/gateway-1/mcp"');
      expect(configText).toContain('Authorization = "Bearer pcgw_secret-managed-token"');
      expect(logs).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            stream: "stderr",
            chunk: expect.stringContaining("Geetorus cannot enforce policies for that direct entry"),
          }),
        ]),
      );
      ...
      if (previousGeetorusApiUrl === undefined) delete process.env.GEETORUS_API_URL;
      else process.env.GEETORUS_API_URL = previousGeetorusApiUrl;
      if (previousGeetorusRuntimeApiUrl === undefined) delete process.env.GEETORUS_RUNTIME_API_URL;
      else process.env.GEETORUS_RUNTIME_API_URL = previousGeetorusRuntimeApiUrl;
      if (previousCodexHome === undefined) delete process.env.CODEX_HOME;
      else process.env.CODEX_HOME = previousCodexHome;
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(commandNotes).toContain(
        "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; Geetorus does not currently suppress that discovery.",
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      ...
      expect(loggedEnv.GEETORUS_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-sandbox-"));
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.codexHome).toBe(path.join(remoteWorkspace, ".geetorus-runtime", "codex", "home"));
      expect(capture.geetorusApiUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(capture.geetorusApiKey).not.toBe("run-jwt-token");
      expect(capture.geetorusApiBridgeMode).toBe("queue_v1");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPath === undefined) delete process.env.PATH;
  ...
  it("injects structured Geetorus wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.geetorusEnvKeys).toContain("GEETORUS_WAKE_PAYLOAD_JSON");
      expect(capture.geetorusWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.geetorusWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## Geetorus Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it.each([true, false])("retries missing resume only before a session starts (started=%s)", async (started) => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-resume-stop-"));
    const commandPath = path.join(root, "codex");
...
`, "utf8");
  ...
  it("classifies mid-turn harness crashes as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-harness-crash-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
...
`;
  ...
  it("persists retry-not-before metadata for codex provider quota failures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-usage-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingCodexCommand(
  ...
  it("classifies Codex refresh-token auth failures without credential telemetry", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-refresh-token-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("Geetorus session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            GEETORUS_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the geetorus heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it.each([{ conversationMode: false, resumedSession: true }, { conversationMode: true, resumedSession: true }, { conversationMode: true, resumedSession: false }])("retains current task policy (conversation=$conversationMode, resumed=$resumedSession)", async ({ conversationMode, resumedSession }) => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      const result = await execute({
        ...
        config: {
          ...
          promptTemplate: conversationMode ? undefined : "Follow the geetorus heartbeat.",
        },
        context: {
          conversationMode,
          geetorusTaskMarkdown: `Full description that must not replay\n${policy}`,
          geetorusTaskMarkdownCompact: policy,
          issueId: "issue-1",
          taskId: "issue-1",
          wakeReason: "issue_commented",
          wakeCommentId: "comment-2",
      ...
      expect(capture.prompt).toContain(resumedSession ? "## Geetorus Resume Delta" : "## Geetorus Wake Payload");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      ...
      expect(capture.prompt).not.toContain("Follow the geetorus heartbeat.");
      if (resumedSession) {
        expect(capture.prompt).not.toContain("You are managed instructions.");
        expect(invocationPrompt).toContain("## Geetorus Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME and mounts the operational skill by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "geetorus");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), `${fakeCodexAuthJson}\n`, "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "worktree-1";
    ...
    try {
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the geetorus heartbeat.");
      expect(capture.geetorusEnvKeys).toEqual(
        expect.arrayContaining([
          "GEETORUS_AGENT_ID",
          "GEETORUS_API_KEY",
          "GEETORUS_API_URL",
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "geetorus"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("isolates connector skills by agent and revision without changing the selected model identity", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-connector-codex-"));
    const workspace = path.join(root, "workspace");
    ...
    const keys = ["GEETORUS_HOME", "GEETORUS_INSTANCE_ID", "CODEX_HOME"] as const;
    const previous = Object.fromEntries(keys.map((key) => [key, process.env[key]]));
    process.env.GEETORUS_HOME = path.join(root, "geetorus");
    process.env.GEETORUS_INSTANCE_ID = "connectors";
    ...
    const invoke = async (agentId: string, digest: string | null, source = skillSource, connectorSkillInstructions = "") => {
      const config = {
        ...
        env: { CODEX_HOME: sourceHome, GEETORUS_TEST_CAPTURE_PATH: capture },
        geetorusConnectorSkillDigest: digest,
        geetorusSkillSync: { desiredSkills: digest ? ["geetorusai/geetorus/agentmail"] : [] },
        geetorusRuntimeSkills: digest ? [{ key: "geetorusai/geetorus/agentmail", runtimeName: "agentmail", source }] : [],
      };
      const result = await execute({ runId: `run-${agentId}-${digest?.slice(0, 1) ?? "none"}`,
        ...
        config, context: { geetorusWake: { connectorSkillInstructions } }, authToken: "test-token", onLog: async () => {},
      });
      expect(result.errorMessage).toBeNull();
      expect(result.exitCode).toBe(0);
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "geetorus"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(geetorusHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousGeetorusHome === undefined) delete process.env.GEETORUS_HOME;
