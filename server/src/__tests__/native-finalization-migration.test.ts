...
describe("P6-18 / MIG-01..04 native finalization migration", () => {
  it("repairs only later duplicates and preserves legacy event bytes and cursors", async () => {
    const temporary = await startEmbeddedPostgresTestDatabase("geetorus-native-migration-");
    ...
    try {
      ...
      await rawDb.execute(sql.raw(`
        ...
        DROP TRIGGER IF EXISTS geetorus_issue_status_version_trigger ON issues;
        DROP FUNCTION IF EXISTS geetorus_bump_issue_status_version();
