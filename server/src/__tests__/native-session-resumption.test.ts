...
describe("P6-25 pre-result native session recovery", () => {
  ...
  const persistedProfile = {
    mode: "native",
    nativeExecutionInput: { schema: "geetorus.native-execution-input.v1", binding: { runId } },
    sessionCheckpoint: {
  ...
  beforeAll(async () => {
    temporary = await startEmbeddedPostgresTestDatabase("geetorus-native-resume-");
  ...
  it("blocks ambiguous observed ownership and a live unrelated persisted PID without replacement effects", async () => {
    ...
    try {
      ...
      const heartbeat = heartbeatService(db, {
        runtimeEnv: { GEETORUS_INSTANCE_ID: "observed-owner-test" },
...
describe.each(["unchanged", "newer_active", "stale_idle"] as const)(
  ...
  (variant) => {
    ...
    afterAll(async () => {
      ...
      if (temporary) {
        await drainHeartbeatRunsToQuiescence(
          db,
          heartbeatService(db, {
            runtimeEnv: { GEETORUS_INSTANCE_ID: "phase6-recovery-test" },
    ...
    it("preserves the old provider contract and admits newer direction only in a separate turn", async () => {
      ...
      // The persisted Geetorus Runner run above remains recoverable while the
