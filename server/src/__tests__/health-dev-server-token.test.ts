...
function createDevServerStatusFile(payload: unknown) {
  const dir = mkdtempSync(path.join(os.tmpdir(), "geetorus-health-dev-server-"));
  tempDirs.push(dir);
...
describe("GET /health dev-server supervisor access", () => {
  it("exposes dev-server metadata to the supervising dev runner in authenticated mode", async () => {
    const previousFile = process.env.GEETORUS_DEV_SERVER_STATUS_FILE;
    const previousToken = process.env.GEETORUS_DEV_SERVER_STATUS_TOKEN;
    process.env.GEETORUS_DEV_SERVER_STATUS_FILE = createDevServerStatusFile({
      dirty: true,
    ...
    process.env.GEETORUS_DEV_SERVER_STATUS_TOKEN = "dev-runner-token";
    ...
    try {
      ...
      const res = await request(app)
        .get("/health")
        .set("X-Geetorus-Dev-Server-Status-Token", "dev-runner-token");
      ...
      if (previousFile === undefined) {
        delete process.env.GEETORUS_DEV_SERVER_STATUS_FILE;
      } else {
        process.env.GEETORUS_DEV_SERVER_STATUS_FILE = previousFile;
      }
      if (previousToken === undefined) {
        delete process.env.GEETORUS_DEV_SERVER_STATUS_TOKEN;
      } else {
        process.env.GEETORUS_DEV_SERVER_STATUS_TOKEN = previousToken;
      }
    }
  });
});
...
describe("POST /health/dev-server/restart", () => {
  it("records a manual restart request for the dev runner", async () => {
    ...
    process.env.GEETORUS_HOME = path.dirname(
      process.env.GEETORUS_DEV_SERVER_STATUS_FILE,
    );
    ...
    try {
      ...
      const requestPath = path.join(
        path.dirname(process.env.GEETORUS_DEV_SERVER_STATUS_FILE),
  ...
  it("rolls back the hot intent when the supervisor request cannot be written", async () => {
    ...
    const home = mkdtempSync(path.join(os.tmpdir(), "geetorus-health-restart-home-"));
    tempDirs.push(home);
