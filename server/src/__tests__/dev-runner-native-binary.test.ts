...
function createRunnerCheckout(): { root: string; source: string; binary: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-dev-runner-binary-"));
  tempRoots.add(root);

  const runnerRoot = path.join(root, "packages", "geetorus-runner", "runner");
...
describe("geetorus runner native dev prerequisite", () => {
  ...
  it("does not build a workspace binary when an explicit binary is configured", () => {
    expect(
      geetorusRunnerBinaryNeedsBuild({
        repoRoot: "/checkout/without/a/staged/binary",
        nativeRunnerRequired: true,
        configuredBinary: "/opt/geetorus/geetorus-runnerd",
      }),
    ).toBe(false);
  });
});
