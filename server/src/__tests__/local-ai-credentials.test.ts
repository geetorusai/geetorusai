...
vi.mock("@geetorusai/adapter-claude-local/server", () => ({ readClaudeToken: mocks.claude, readIsolatedClaudeKeychainToken: mocks.claudeIsolatedKeychain, fetchClaudeQuota: mocks.claudeQuota }));
vi.mock("@geetorusai/adapter-codex-local/server", () => ({ readCodexAuthInfo: mocks.codex, fetchCodexQuota: mocks.codexQuota }));
...
describe("explicit local subscription import", () => {
  ...
  it("rejects missing and invalid logins with actionable, redacted errors", async () => {
    ...
    await expect(readVerifiedLocalAiCredential("anthropic")).rejects.toThrow(/^Could not verify the local subscription\. Run claude auth login in a terminal on the machine running Geetorus, then try Connect again\.$/);
