...
describeEmbeddedPostgres("createPluginSecretsHandler shared vault integration", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-plugin-secrets-${randomUUID()}`);
  ...
  async function seedPlugin() {
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.plugin-secrets-test",
      packageName: "@geetorusai/plugin-secrets-test",
      version: "0.0.1",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.plugin-secrets-test",
        apiVersion: 1,
        version: "0.0.1",
