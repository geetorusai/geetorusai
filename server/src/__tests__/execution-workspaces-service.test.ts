...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Geetorus Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@geetorus.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-execution-workspaces-service-");
  ...
  async function seedTerminalWorkspace(options: {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-terminal-${randomUUID()}`);
    ...
    if (options.mergedPr) {
      await db.insert(issueWorkProducts).values({
        ...
        provider: "github",
        title: "Delivered PR",
        url: "https://github.com/geetorusai/geetorus/pull/10623",
        status: "merged",
      });
    }
  ...
  it("reports a squash cross-branch delivery as merged_via_pr and suppresses the ancestry warning", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-delivery-${randomUUID()}`);
    tempDirs.add(worktreePath);
    await runGit(repoRoot, ["branch", "PAP-16015-delivery"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "PAP-16015-delivery"]);
    await fs.writeFile(path.join(worktreePath, "delivered.txt"), "delivered\n", "utf8");
  ...
  async function seedAncestryTerminalWorkspace(overrides: { updatedAt?: Date } = {}) {
    ...
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-ancestry-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("fails closed before archive when git status inspection is unavailable", async () => {
    ...
    try {
      ...
      expect(readiness).toMatchObject({
        ...
        blockingReasons: [
          "Geetorus could not verify the workspace git status. Retry before destructive cleanup.",
        ],
      });
  ...
  it("does not treat an unrelated inbound issue mention as delivery evidence", async () => {
    ...
    await db.insert(issueWorkProducts).values({
      ...
      url: "https://github.com/geetorusai/geetorus/pull/10624",
      status: "merged",
    });
  ...
  it("does not trust directly linked merged products for a different repository or branch", async () => {
    ...
    await db.insert(issueWorkProducts).values([
      ...
      },
      {
        ...
        url: "https://github.com/unrelated/geetorus/pull/10623",
        status: "merged",
      },
    ]);
  ...
  it("does not treat a descendant PR on the shared workspace as parent delivery evidence", async () => {
    ...
    await db.insert(issueWorkProducts).values({
      ...
      url: "https://github.com/geetorusai/geetorus/pull/10625",
      status: "merged",
    });
  ...
  it("reconciles a forward branch record, comments on the source issue, and resolves matching workspace recovery", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("reconciles forward when the recorded branch has no resolvable commit and the worktree is clean", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-missing-recorded-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("keeps forward reconciliation fail-closed when the recorded branch is missing but the worktree is dirty", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-missing-recorded-dirty-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("keeps forward reconciliation fail-closed when the checked-out branch ref does not resolve either", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-missing-both-refs-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("quarantine_restore rescues dirty live-branch work, resolves recovery, and returns the source issue to todo", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-quarantine-restore-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    expect(result.rescueRef).toMatchObject({
      branchName: expect.stringMatching(/^geetorus\/rescue\/PAP-124\/\d{8}T\d{6}Z$/),
      fileCount: 2,
    });
  ...
  it("quarantine_restore rejects active runtime services before creating a rescue branch", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-quarantine-running-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    await expect(readGit(
      repoRoot,
      ["for-each-ref", "--format=%(refname:short)", "refs/heads/geetorus/rescue"],
    )).resolves.toBeNull();
  ...
  it.each(["review", "approval"] as const)(
    ...
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-quarantine-${stageType}-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it.each([
    ...
    async ({ claimantIssueIdentifier, claimantHasActiveRun, expectedReason }) => {
      const repoRoot = await createTempRepo();
      tempDirs.add(repoRoot);
      const worktreePath = path.join(path.dirname(repoRoot), `geetorus-quarantine-claimant-${randomUUID()}`);
      tempDirs.add(worktreePath);
      ...
      const claimantWorkspacePath = path.join(path.dirname(repoRoot), `geetorus-claimant-${randomUUID()}`);
      const now = new Date();
  ...
  it("rejects branch reconciliation when the worktree is dirty", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-dirty-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation while the workspace lifecycle is active", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-active-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation if the workspace becomes active before the branch record update", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-race-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation while runtime services are active", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-running-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation when a runtime service starts before the locked update", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-raced-service-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation when runtime service activation is already spawning", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-spawning-service-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    const issueId = randomUUID();
    const executionWorkspaceId = randomUUID();
    const runtimeStartedMarker = path.join(os.tmpdir(), `geetorus-runtime-started-${randomUUID()}.marker`);
  ...
  it("rejects forward branch reconciliation for diverged branches", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-diverged-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("serializes the verified HTTPS URL as canonical and never falls back to the HTTP backend", async () => {
    ...
    const hostname = "geetorus-dev.tail29c1aa.ts.net";
    ...
    await db.insert(projectWorkspaces).values({
      ...
      metadata: {
        runtimeConfig: {
          workspaceRuntime: { services: [{ name: "geetorus-dev", command: "pnpm dev" }] },
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `geetorus-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "geetorus-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "geetorus-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      cwd: worktreePath,
      providerRef: worktreePath,
      branchName: "geetorus-close-check",
      baseRef: "main",
      metadata: {
        createdByRuntime: true,
        gitBranchOwnershipVersion: 1,
