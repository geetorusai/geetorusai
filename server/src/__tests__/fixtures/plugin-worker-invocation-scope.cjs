...
function sendNestedHostRequest(originalRequest, invocationId) {
  ...
  if (mode === "echo") {
    nestedRequest.geetorusInvocationId = invocationId;
  } else if (mode === "unknown") {
    nestedRequest.geetorusInvocationId = "unknown-invocation";
  }
...
rl.on("line", (line) => {
  ...
  if (method === "getData" || method === "performAction") {
    sendNestedHostRequest(message, message.geetorusInvocation?.id);
    return;
  }
