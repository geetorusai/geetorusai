...
rl.on("line", (line) => {
  ...
  if (method === "environmentExecute") {
    const invocationId = message.geetorusInvocation && message.geetorusInvocation.id;
    ...
    if (oversizedChars > 0) {
      send({
        jsonrpc: "2.0",
        method: "execute.log",
        geetorusInvocationId: invocationId,
    ...
    for (const entry of logs) {
      ...
      if (tag === "echo") {
        note.geetorusInvocationId = invocationId;
      } else if (tag === "unknown") {
        note.geetorusInvocationId = "unknown-invocation";
      } else if (tag === "forge-previous") {
        note.geetorusInvocationId = forgeableId;
      }
