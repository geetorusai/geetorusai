...
import { execute } from "@geetorusai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("injects runtime skills into the configured child HOME", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-configured-home-"));
    ...
    try {
      ...
      const installedSkill = path.join(configuredHome, ".gemini", "skills", "geetorus");
      ...
      await expect(fs.lstat(path.join(processHome, ".gemini", "skills", "geetorus"))).rejects.toThrow();
    } finally {
  ...
  it("passes prompt via --prompt and injects geetorus env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the geetorus heartbeat.");
      expect(promptArg).toContain("Geetorus runtime note:");
      ...
      expect(invocationPrompt).toContain("Geetorus runtime note:");
      expect(invocationPrompt).toContain("GEETORUS_API_URL");
      expect(invocationPrompt).toContain("Geetorus API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes turn-limit exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes Gemini exit code 53 as max-turn exhaustion", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-exit-53-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("does not normalize unstructured turn-limit text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-max-turn-text-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Geetorus Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the geetorus heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
