...
import { companies, createDb } from "@geetorusai/db";
import {
...
describeEmbeddedPostgres("company import transfer routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-import-transfer-routes-");
    ...
    spoolRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-import-transfer-spool-"));
    app = await createApp();
  });
  ...
  it("reports an already-applied transfer on re-declaration", async () => {
    ...
    expect(redeclared.body.company).toEqual({ id: companyId, name: "Geetorus", issuePrefix: "PAPA" });
