...
beforeAll(async () => {
  originalHome = process.env.GEETORUS_HOME;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-managed-clone-"));
...
async function createLocalSourceRepo() {
  const sourceRepo = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-clone-source-"));
  await execFile("git", ["init"], { cwd: sourceRepo });
  await execFile("git", ["config", "user.email", "geetorus@example.com"], { cwd: sourceRepo });
  await execFile("git", ["config", "user.name", "Geetorus Test"], { cwd: sourceRepo });
...
describe("ensureManagedProjectWorkspace clone credentials", () => {
  it("materializes every repository-only project row inside the task workspace and reuses local edits", async () => {
    ...
    try {
      ...
      expect(path.relative(anchor.cwd, additional[0]!.cwd)).toMatch(/^\.geetorus-repositories\//);
  ...
  it("does not blame the credential when an authenticated clone fails for non-auth reasons", async () => {
    ...
    const missingRepo = path.join(os.tmpdir(), "geetorus-definitely-missing", "repo.git");
    const resolveGitAuth = vi.fn(async () => ({
      configArgs: [],
  ...
  it("keeps the credential env alive through the sanitizer spread order", () => {
    ...
    // sanitizer strips every GEETORUS_* key, so the token env must be spread after it.
