...
describeEmbedded("PAP-9522 QA: routine secrets end-to-end", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `geetorus-qa-routine-secrets-${randomUUID()}`);
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-qa-routine-secrets-");
