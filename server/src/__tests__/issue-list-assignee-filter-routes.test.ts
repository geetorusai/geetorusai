...
import { activityLog, agents, companies, companyMemberships, createDb, heartbeatRuns, issues, principalPermissionGrants } from "@geetorusai/db";
import {
...
describeEmbeddedPostgres("issue list routes assigneeAgentId filter", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-issue-list-routes-");
  ...
  it("coalesces simultaneous identical compact issue-list requests into one service computation", async () => {
    ...
    expect(responses.some((res) => res.headers["x-geetorus-request-cache"] === "coalesced")).toBe(true);
  });
  ...
  it("keeps compact issue-list cache keys separated by board user identity", async () => {
    ...
    expect(first.headers["x-geetorus-request-cache"]).toBe("miss");
    expect(second.headers["x-geetorus-request-cache"]).toBe("miss");
  });
  ...
  it("serves repeated compact issue-list requests from the short server cache", async () => {
    ...
    expect(computeCount).toBe(1);
    expect(first.headers["x-geetorus-request-cache"]).toBe("miss");
    expect(second.headers["x-geetorus-request-cache"]).toBe("hit");
  });
  ...
  it("logs request_storm_detected for identical in-flight compact issue-list fanout without query values", async () => {
    ...
    const responses = await Promise.all(Array.from({ length: 5 }, () =>
      request(app)
        ...
        .set("X-Geetorus-Tab-Visible", "visible")
