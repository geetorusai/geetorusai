...
import {
  ...
  resolveManagedGeetorusRuntimePublicOrigin,
...
import { resolveGeetorusConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@geetorusai/shared";
...
async function writeRegisteredSourceConfig(baseCwd: string, instanceId = "source-instance") {
  const configDir = path.join(baseCwd, ".geetorus");
  await fs.mkdir(configDir, { recursive: true });
  await fs.writeFile(path.join(configDir, "config.json"), "{}\n", "utf8");
  await fs.writeFile(
    path.join(configDir, ".env"),
    `GEETORUS_INSTANCE_ID=${instanceId}\n`,
    "utf8",
  );
}

async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "geetorus@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Geetorus Test"]);
...
async function createClonedRepoWithRemote() {
  ...
  const remoteDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-remote-"));
  const remotePath = path.join(remoteDir, "geetorus.git");
  await execFileAsync("git", ["clone", "--bare", sourceRepo, remotePath]);

  const cloneRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-clone-"));
  const repoRoot = path.join(cloneRoot, "geetorus");
...
afterEach(async () => {
  ...
  delete process.env.GEETORUS_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Geetorus and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/geetorus",
      GEETORUS_HOME: "/tmp/geetorus-home",
      GEETORUS_INSTANCE_ID: "runtime-instance",
    ...
    expect(sanitized.GEETORUS_HOME).toBeUndefined();
    expect(sanitized.GEETORUS_INSTANCE_ID).toBeUndefined();
...
describe("resolveManagedGeetorusRuntimePublicOrigin", () => {
  const baseInput = {
    serviceName: "geetorus-dev",
    command: "pnpm dev --bind lan",
  };

  it("leaves explicit operator origin configuration unchanged", () => {
    expect(resolveManagedGeetorusRuntimePublicOrigin({
      ...baseInput,
      environment: { GEETORUS_PUBLIC_URL: "https://operator.example.com" },
  ...
  it("infers browser-reachable HTTPS and loopback origins", () => {
    expect(resolveManagedGeetorusRuntimePublicOrigin({
      ...baseInput,
      environment: {},
      exposedUrl: "https://geetorus-dev.tail29c1aa.ts.net/path?ignored=true",
      exposedUrlTemplate: "https://{{workspace.branchName}}.tail29c1aa.ts.net",
    })).toBe("https://geetorus-dev.tail29c1aa.ts.net");
    expect(resolveManagedGeetorusRuntimePublicOrigin({
      ...baseInput,
      environment: {},
  ...
  it("rejects internal-only and unsafe inferred origins with actionable guidance", () => {
    expect(() => resolveManagedGeetorusRuntimePublicOrigin({
      ...baseInput,
      environment: {},
      exposedUrl: "http://geetorus-dev:45439",
    })).toThrow(/internal-only.*Configure GEETORUS_PUBLIC_URL or BETTER_AUTH_URL/);
    expect(() => resolveManagedGeetorusRuntimePublicOrigin({
      ...baseInput,
      environment: {},
  ...
  it("keeps interpolated hostnames inside the operator-configured domain", () => {
    ...
    expect(() => resolveManagedGeetorusRuntimePublicOrigin({
      ...baseInput,
      environment: {},
      exposedUrl: "https://managed-worktree.geetorus.dev",
...
describe("resolveRuntimeProvisionCommand", () => {
  it("backfills deferred seeding for legacy managed git worktrees", async () => {
    const baseCwd = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-provision-"));
    ...
    try {
      ...
      await fs.mkdir(path.join(cwd, ".geetorus"), { recursive: true });
      await fs.writeFile(path.join(cwd, ".geetorus", "config.json"), "{}\n");
      expect(resolveRuntimeProvisionCommand({ config: {}, workspace })).toBe(
        "bash ./scripts/provision-worktree-runtime.sh",
      );

      await fs.writeFile(path.join(cwd, ".geetorus", "seed-pending"), "{}\n");
      expect(resolveRuntimeProvisionCommand({ config: {}, workspace })).toBe(
        "bash ./scripts/provision-worktree-runtime.sh",
      );
      ...
      await fs.writeFile(path.join(cwd, ".geetorus", "seed-complete"), "{}\n");
      expect(resolveRuntimeProvisionCommand({ config: {}, workspace })).toBe(
        "bash ./scripts/provision-worktree-runtime.sh",
      );

      await fs.writeFile(
        path.join(cwd, ".geetorus", "seed-manifest.json"),
...
describe("refreshRemoteTrackingBaseRef git auth", () => {
  ...
  it("attributes a failed authenticated fetch to the credential that was used", async () => {
    ...
    await runGit(repoRoot, ["remote", "set-url", "origin", path.join(os.tmpdir(), "geetorus-missing-remote", "repo.git")]);
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-links-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-links-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@geetorusai");
    ...
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/geetorus-main/.git/worktrees/runtime-links\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
        name: "@geetorusai/server",
        dependencies: {
          "@geetorusai/db": "workspace:*",
        },
      }),
      "utf8",
    );
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@geetorusai/db" }),
      "utf8",
    );
    await fs.writeFile(
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-links-current-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@geetorusai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    ...
    await fs.mkdir(expectedPackageDir, { recursive: true });
    await fs.mkdir(serverNodeModulesScopeDir, { recursive: true });
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/geetorus-main/.git/worktrees/runtime-links-current\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
  ...
  it("skips relinking outside linked git worktrees", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-links-non-worktree-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-links-non-worktree-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@geetorusai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    const stalePackageDir = path.join(staleRoot, "db");
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".geetorus", "worktrees"));
  ...
  it("defaults the repo-provided worktree provisioner for git worktree strategies", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "set -euo pipefail",
        "mkdir -p .geetorus",
        "printf 'provisioned\\n' > .geetorus/default-provision-ran",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(
      fs.readFile(path.join(workspace.cwd, ".geetorus", "default-provision-ran"), "utf8"),
  ...
  it("stops before git worktree add when the base ref is absent on origin", async () => {
    ...
    await expect(
      fs.stat(path.join(repoRoot, ".geetorus", "worktrees", "PAP-447-add-worktree-support")),
    ).rejects.toThrow();
  });
  ...
  it("surfaces an authenticated fetch failure as an unresolved base ref, not a crash", async () => {
    ...
    await runGit(repoRoot, ["remote", "set-url", "origin", path.join(os.tmpdir(), "geetorus-missing-remote.git")]);
  ...
  it("rejects reusing an empty directory that only looks like a worktree because it sits inside the repo", async () => {
    ...
    const poisonedPath = path.join(repoRoot, ".geetorus", "worktrees", branchName);
  ...
  it("reuses the current linked worktree instead of nesting another worktree inside it", async () => {
    ...
    const currentWorktree = path.join(repoRoot, ".geetorus", "worktrees", branchName);
  ...
  it("reuses an already checked out branch from git worktree metadata even when the target path differs", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PAP-1355-worktree-reuse";
    const existingWorktree = path.join(repoRoot, ".geetorus", "worktrees", branchName);
    const { recorder, operations } = createWorkspaceOperationRecorderDouble();
    ...
    const realized = await realizeExecutionWorkspace({
      ...
      config: {
        workspaceStrategy: {
          type: "git_worktree",
          branchTemplate: "{{issue.identifier}}-{{slug}}",
          worktreeParentDir: ".geetorus/other-worktrees",
        },
      },
      issue: {
        id: "issue-1",
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$GEETORUS_WORKSPACE_BRANCH\" > .geetorus-provision-branch",
        "printf '%s\\n' \"$GEETORUS_WORKSPACE_BASE_CWD\" > .geetorus-provision-base",
        "printf '%s\\n' \"$GEETORUS_WORKSPACE_CREATED\" > .geetorus-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".geetorus-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".geetorus-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".geetorus-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".geetorus-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".geetorus-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".geetorus-provision-version"), "utf8")).resolves.toBe("v2\n");
  }, 30_000);

  it("writes an isolated repo-local Geetorus config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    await writeRegisteredSourceConfig(repoRoot, "worktree-base-source");
    const previousCwd = process.cwd();
    const previousPath = process.env.PATH;
    const previousConfig = process.env.GEETORUS_CONFIG;
    const previousHome = process.env.GEETORUS_HOME;
    const previousInstanceId = process.env.GEETORUS_INSTANCE_ID;
    const previousWorktreesDir = process.env.GEETORUS_WORKTREES_DIR;
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktrees-"));
    const isolatedBin = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-bin-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(geetorusHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = instanceId;
    process.env.GEETORUS_WORKTREES_DIR = isolatedWorktreeHome;
    delete process.env.GEETORUS_CONFIG;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/geetorus"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".geetorus", "config.json");
      const envPath = path.join(workspace.cwd, ".geetorus", ".env");
      ...
      expect(envVars.GEETORUS_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.GEETORUS_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.GEETORUS_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.GEETORUS_IN_WORKTREE).toBe("true");
      expect(envVars.GEETORUS_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveGeetorusConfigPath()).toBe(configPath);
      ...
      await fs.writeFile(envPath, `${envContents}GEETORUS_WORKTREE_COLOR="#112233"\n`, "utf8");
      ...
      expect(reusedEnvContents).toContain('GEETORUS_WORKTREE_COLOR="#112233"');
    } finally {
      ...
      for (const [key, value] of [
        ["GEETORUS_CONFIG", previousConfig],
        ["GEETORUS_HOME", previousHome],
        ["GEETORUS_INSTANCE_ID", previousInstanceId],
        ["GEETORUS_WORKTREES_DIR", previousWorktreesDir],
      ] as const) {
        if (value === undefined) {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".geetorus", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);

  it("reinstalls worktree-local pnpm dependencies when package metadata changes", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-stale-deps-"));
    ...
    try {
      ...
      const runScript = () => execFileAsync(scriptPath, [], {
        ...
        env: {
          ...
          GEETORUS_WORKSPACE_BASE_CWD: baseRoot,
          GEETORUS_WORKSPACE_CWD: worktreeRoot,
        },
      });
  ...
  it("fails instead of writing an unseeded fallback config when worktree init errors after CLI detection succeeds", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-provision-fail-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "  exit 0",
          "fi",
          "if [ \"$1\" = \"geetorusai\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
      ...
      await expect(fs.stat(path.join(worktreeRoot, ".geetorus", "config.json"))).rejects.toThrow();
      await expect(fs.stat(path.join(worktreeRoot, ".geetorus", ".env"))).rejects.toThrow();
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("regenerates stale worktree config that points at another host", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-stale-config-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: "/Users/example/.geetorus-worktrees/instances/stale/db",
          },
          logging: {
            mode: "file",
            logDir: "/Users/example/.geetorus-worktrees/instances/stale/logs",
          },
          storage: {
            provider: "local_disk",
            localDisk: {
              baseDir: "/Users/example/.geetorus-worktrees/instances/stale/data/storage",
            },
          },
          secrets: {
            provider: "local_encrypted",
            localEncrypted: {
              keyFilePath: "/Users/example/.geetorus-worktrees/instances/stale/secrets/master.key",
            },
          },
        }),
        "utf8",
      ...
      await fs.writeFile(
        ...
        [
          "GEETORUS_HOME=/Users/example/.geetorus-worktrees",
          "GEETORUS_INSTANCE_ID=stale",
          `GEETORUS_CONFIG=/Users/example/geetorus/${path.basename(worktreeRoot)}/.geetorus/config.json`,
          "",
        ].join("\n"),
        "utf8",
      );
      await fs.writeFile(
        ...
        [
          ...
          "fi",
          "if [ \"$1\" = \"geetorusai\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
          "  mkdir -p \"$PWD/.geetorus\"",
          "  printf '%s\\n' '{\"database\":{\"embeddedPostgresDataDir\":\"'$PWD'/.geetorus/runtime/db\"}}' > \"$PWD/.geetorus/config.json\"",
          "  printf '%s\\n' \"GEETORUS_HOME=$PWD/.geetorus/runtime\" \"GEETORUS_INSTANCE_ID=healthy\" \"GEETORUS_CONFIG=$PWD/.geetorus/config.json\" > \"$PWD/.geetorus/.env\"",
          "  exit 0",
          "fi",
          "exit 0",
          "",
      ...
      expect(result.stderr).toContain("Existing isolated Geetorus worktree config is stale for this host; regenerating.");
      await expect(fs.readFile(path.join(geetorusDir, ".env"), "utf8")).resolves.toContain(
        `GEETORUS_CONFIG=${worktreeRoot}/.geetorus/config.json`,
      );
      await expect(fs.readFile(path.join(geetorusDir, "config.json"), "utf8")).resolves.toContain(worktreeRoot);
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("retries worktree-local pnpm install without a frozen lockfile when the lockfile is outdated", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-outdated-lockfile-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await expect(fs.readFile(path.join(worktreeRoot, ".geetorus", "config.json"), "utf8")).resolves.toContain(
        "\"database\"",
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("reattaches a missing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$GEETORUS_WORKSPACE_BRANCH\" > .geetorus-restored-branch",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".geetorus-restored-branch"), "utf8")).resolves.toBe(`${branchName}\n`);
  ...
  it("repairs a clean persisted git worktree branch mismatch when both branches point at the same commit", async () => {
    ...
    const realWorktreeRoot = path.join(repoRoot, ".geetorus", "real-worktrees");
    const symlinkedWorktreeRoot = path.join(repoRoot, ".geetorus", "worktrees");
  ...
  it("reattaches a clean forward detached HEAD to the recorded persisted git worktree branch", async () => {
    ...
    const worktreePath = path.join(repoRoot, ".geetorus", "worktrees", branchName);
  ...
  it("rejects dirty persisted git worktree branch incoherence with bounded recovery evidence", async () => {
    ...
    const worktreePath = path.join(repoRoot, ".geetorus", "worktrees", expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
    await runGit(repoRoot, ["branch", expectedBranch]);
  ...
  it("routes non-reusable persisted git worktrees through workspace validation recovery", async () => {
    ...
    const detachedWorktreePath = path.join(repoRoot, ".geetorus", "worktrees", expectedBranch);
  ...
  it("does not reuse a missing persisted local filesystem workspace", async () => {
    const baseCwd = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-workspace-base-"));
  ...
  it("reprovisions an existing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.rm(path.join(initial.cwd, ".geetorus-restored-state"), { force: true });
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".geetorus-restored-state"), "utf8")).resolves.toBe("reprovisioned\n");
  }, 15_000);
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
  ...
  it("records teardown and cleanup operations when a recorder is provided", async () => {
    ...
    const worktreesDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-cleanup-instances-"));
    ...
    await fs.mkdir(path.join(workspace.cwd, ".geetorus"), { recursive: true });
    await fs.writeFile(
      path.join(workspace.cwd, ".geetorus", ".env"),
      `GEETORUS_HOME=${JSON.stringify(worktreesDir)}\nGEETORUS_INSTANCE_ID=${JSON.stringify(instanceId)}\n`,
      "utf8",
    );
    process.env.GEETORUS_WORKTREES_DIR = worktreesDir;
...
describe("ensureRuntimeServicesForRun", () => {
  function configureRuntimeProvisionTestHome(workspaceRoot: string, suffix: string) {
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    const previousGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;
    process.env.GEETORUS_HOME = workspaceRoot;
    process.env.GEETORUS_INSTANCE_ID = `${suffix}-${randomUUID()}`;
    return () => {
      if (previousGeetorusHome === undefined) delete process.env.GEETORUS_HOME;
      else process.env.GEETORUS_HOME = previousGeetorusHome;
      if (previousGeetorusInstanceId === undefined) delete process.env.GEETORUS_INSTANCE_ID;
      else process.env.GEETORUS_INSTANCE_ID = previousGeetorusInstanceId;
    };
  }
  ...
  it("runs runtime provisioning once when service starts race for the same workspace", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-provision-race-"));
    const restoreGeetorusEnv = configureRuntimeProvisionTestHome(workspaceRoot, "runtime-provision-race");
  ...
  it("logs runtime provisioning failure and retries it on the next service start", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-provision-retry-"));
    const restoreGeetorusEnv = configureRuntimeProvisionTestHome(workspaceRoot, "runtime-provision-retry");
  ...
  it("records the built-in deferred seed as failed when its manifest is not verified", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-workspace-seed-operation-"));
    const restoreGeetorusEnv = configureRuntimeProvisionTestHome(workspaceRoot, "workspace-seed-operation");
    const scriptsDir = path.join(workspaceRoot, "scripts");
    const markerDir = path.join(workspaceRoot, ".geetorus");
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        `printf '%s\\n' '${JSON.stringify({ version: 2, state: "failed", phase: "source_validation" })}' > .geetorus/seed-manifest.json`,
      ].join("\n"),
      "utf8",
    );
    const config = runtimeProvisionTestConfig({});
  ...
  it("keeps an explicit command matching the built-in seed command as runtime provisioning", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-explicit-runtime-provision-"));
    const restoreGeetorusEnv = configureRuntimeProvisionTestHome(workspaceRoot, "explicit-runtime-provision");
    const scriptsDir = path.join(workspaceRoot, "scripts");
    await fs.mkdir(scriptsDir, { recursive: true });
    await fs.writeFile(
      path.join(scriptsDir, "provision-worktree-runtime.sh"),
  ...
  it("does not create a runtime provision operation when the command is absent", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-provision-noop-"));
    const restoreGeetorusEnv = configureRuntimeProvisionTestHome(workspaceRoot, "runtime-provision-noop");
    const workspace = buildWorkspace(workspaceRoot);
    const config = runtimeProvisionTestConfig({});
    const { recorder, operations } = createWorkspaceOperationRecorderDouble();
  ...
  it("preserves the selected persisted runtime id when starting one configured service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-selected-id-"));
    const workspace = buildWorkspace(workspaceRoot);
    const restoreGeetorusEnv = configureRuntimeProvisionTestHome(workspaceRoot, "runtime-selected-id");
  ...
  it("leaves manual runtime services untouched during agent runs", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-manual-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("enables UI dev middleware by default for managed Geetorus worktree runtimes", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-ui-dev-"));
    const workspace = buildWorkspace(workspaceRoot);
    const serviceScript =
      ...
      + "res.end(process.env.GEETORUS_UI_DEV_MIDDLEWARE||'missing');"
  ...
  it("injects isolated browser callback origins into separate worktree runtimes", async () => {
    const firstRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-origin-first-"));
    const secondRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-origin-second-"));
  ...
  it("requires Geetorus dev runtime services to pass /api/health readiness", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-health-"));
    const workspace = buildWorkspace(workspaceRoot);
    const runId = "run-geetorus-health";
  ...
  it("replaces a reused Geetorus dev runtime whose 2xx health payload is unhealthy", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-misreported-health-"));
    const workspace = buildWorkspace(workspaceRoot);
    const serviceCommand =
  ...
  it("reuses a shared Geetorus dev runtime after one transient unhealthy response", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-transient-health-"));
    const workspace = buildWorkspace(workspaceRoot);
    const serviceCommand =
  ...
  it("rejects an unreachable exposed origin even when readiness uses a local probe", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-explicit-readiness-"));
    const workspace = buildWorkspace(workspaceRoot);
    const runId = "run-geetorus-explicit-readiness";
    const serviceCommand =
    ...
    try {
      await expect(ensureRuntimeServicesForRun({
            ...
            services: [
              {
                ...
                expose: {
                  type: "url",
                  urlTemplate: "http://not-a-real-geetorus-host.invalid:{{port}}",
                },
                lifecycle: "shared",
                stopPolicy: {
                  type: "manual",
        ...
        adapterEnv: {},
      })).rejects.toThrow(/internal-only or non-resolvable.*Configure GEETORUS_PUBLIC_URL or BETTER_AUTH_URL/);
    } finally {
      await releaseRuntimeServicesForRun(runId);
    }
  });
  ...
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-workspace-"));
    const workspace = buildWorkspace(workspaceRoot);
    const serviceCommand =
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".geetorus", "worktrees", "PAP-874-chat-speed-issues");
    ...
    // A Geetorus dev runtime must answer `/api/health` semantically before it may
    // be published, so the fake serves the same shape a real one does.
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>{if(req.url==='/api/health'){res.setHeader('content-type','application/json');res.end(JSON.stringify({status:'ok'}));return;}res.end(process.env.GEETORUS_HOME)}).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              GEETORUS_HOME: "{{workspace.cwd}}/.geetorus/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".geetorus", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".geetorus", "runtime-services"));
  });

  it("does not leak parent Geetorus instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "geetorusConfig: process.env.GEETORUS_CONFIG ?? null,",
          "geetorusHome: process.env.GEETORUS_HOME ?? null,",
          "geetorusInstanceId: process.env.GEETORUS_INSTANCE_ID ?? null,",
    ...
    process.env.GEETORUS_CONFIG = "/tmp/base-geetorus-config.json";
    process.env.GEETORUS_HOME = "/tmp/base-geetorus-home";
    process.env.GEETORUS_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/geetorus";
    ...
    expect(captured.geetorusConfig).toBeNull();
    expect(captured.geetorusHome).toBeNull();
    expect(captured.geetorusInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-sibling-"));
  ...
  it("starts only the selected workspace-controlled runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-control-start-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("stops only the selected execution workspace runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-control-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
...
describe("readLocalServicePortOwner", () => {
  ...
  it("attributes a Windows listener to a descendant of the launched process", async () => {
    const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-windows-tools-"));
    const previousPath = process.env.PATH;
    const port = 43_123;
  ...
  it("accepts service cwd nested within the requested workspace", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-workspace-"));
  ...
  it("preserves newlines and trailing whitespace from Darwin lsof cwd output", async () => {
    const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-lsof-tools-"));
    const previousPath = process.env.PATH;
    const reportedCwd = path.join(os.tmpdir(), "geetorus-runtime-line\nbreak ");
  ...
  it("returns null for invalid PIDs and a missing Darwin lsof binary", async () => {
    ...
    const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-missing-lsof-"));
    const previousPath = process.env.PATH;
    Object.defineProperty(process, "platform", { value: "darwin" });
    process.env.PATH = fakeBin;
  ...
  it("keeps a live registry record adoptable when Darwin cwd inspection confirms it", async () => {
    ...
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `unsupported-cwd-${randomUUID()}`;
    expect(port).toBeTypeOf("number");
    ...
    try {
      ...
      await fs.rm(geetorusHome, { recursive: true, force: true });
    }
  });
  ...
  it("refuses to adopt a listener whose real cwd belongs to another workspace", async () => {
    ...
    const targetWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-target-"));
    const ownerWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-owner-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `cross-workspace-${randomUUID()}`;
  ...
  it("adopts a port owner running inside the workspace when the registry record is gone", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-adopt-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `adopt-port-owner-${randomUUID()}`;
  ...
  it("refuses to adopt a listener whose cwd differs only by trailing whitespace", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-ws-"));
    ...
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `adopt-whitespace-${randomUUID()}`;
...
describeEmbeddedPostgres("workspace dirty quarantine branch repair", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-workspace-dirty-quarantine-");
  ...
  async function createDirtyMismatchRepo(input: {
    ...
    const repoRoot = await createTempRepo();
    const worktreePath = path.join(repoRoot, ".geetorus", "worktrees", input.expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
  ...
  async function seedDirtyQuarantineRecords(input: {
    ...
    if (input.claimant && input.claimant !== "none") {
      ...
      await db.insert(executionWorkspaces).values({
        ...
        status: "active",
        cwd: path.join(input.repoRoot, ".geetorus", "claimants", claimantWorkspaceId),
        providerRef: path.join(input.repoRoot, ".geetorus", "claimants", claimantWorkspaceId),
        baseRef: "HEAD",
  ...
  it("quarantines dirty foreign-branch work into a rescue branch before restoring the recorded branch", async () => {
    ...
    expect(rescueBranch).toMatch(/^geetorus\/rescue\/PAP-455\/\d{8}T\d{6}Z$/);
  ...
  it("quarantines a worktree wedged mid-rebase and clears the interrupted rebase state", async () => {
    ...
    const rescueBranch = warning?.match(/"([^"]+)"/)?.[1] ?? "";
    expect(rescueBranch).toMatch(/^geetorus\/rescue\/PAP-456\/\d{8}T\d{6}Z$/);
  ...
  it("refuses dirty quarantine repair while the execution workspace has an active runtime service", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      status: "running",
      lifecycle: "shared",
      reuseKey: `execution_workspace:${ids.sourceWorkspaceId}:geetorus-dev`,
      command: "pnpm dev",
      cwd: worktreePath,
      port: 49195,
...
describeEmbeddedPostgres("workspace runtime service control persistence", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-workspace-runtime-control-");
    db = createDb(tempDb.connectionString);
  }, 20_000);
  ...
  it("persists provisioning before starting and excludes provision time from readiness timeout", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-slow-control-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-control-home-"));
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    const previousGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-control-${randomUUID()}`;
  ...
  async function createRuntimeFixture(input?: {
    ...
    const projectId = randomUUID();
    const projectWorkspaceId = randomUUID();
    const baseRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-port-fixture-"));
  ...
  async function createRuntimeHome() {
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-port-home-"));
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    const previousGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-ports-${randomUUID()}`;
    return async () => {
      if (previousGeetorusHome === undefined) delete process.env.GEETORUS_HOME;
      else process.env.GEETORUS_HOME = previousGeetorusHome;
      if (previousGeetorusInstanceId === undefined) delete process.env.GEETORUS_INSTANCE_ID;
    ...
    return {
      workspaceRuntime: {
        services: [{
          name: "geetorus-dev",
          command,
          env: { GEETORUS_PUBLIC_URL: "http://127.0.0.1:3100" },
  ...
  it("does not accept an occupied allocated port when listener ownership is unavailable", async () => {
    ...
    const workspace = fixture.workspaces[0]!;
    const basePort = await findFreePort();
    const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-no-lsof-"));
  ...
  it("keeps fixed ports strict for shared persisted workspaces and identity-less starts", async () => {
    ...
    try {
      ...
      const identitylessCwd = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-identityless-"));
      try {
        await expect(startRuntimeServicesForWorkspaceControl({
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-workspace-runtime-");
    db = createDb(tempDb.connectionString);
  }, 20_000);
  ...
  it("restores desired services when one row is stopped and a live registered service has no row", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-desired-reconcile-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-desired-reconcile-${randomUUID()}`;
    ...
    const actor = { id: null, name: "Geetorus", companyId };
    const workspace = {
      ...buildWorkspace(workspaceRoot),
      projectId,
      workspaceId: projectWorkspaceId,
  ...
  it("backfills a pre-existing HTTP-only managed worktree runtime to verified HTTPS in place", async () => {
    ...
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-https-backfill-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    const previousGeetorusInstanceId = process.env.GEETORUS_INSTANCE_ID;
    const previousHttpsMode = process.env.GEETORUS_MANAGED_RUNTIME_HTTPS;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-https-backfill-${randomUUID()}`;
    ...
    try {
      ...
      process.env.GEETORUS_MANAGED_RUNTIME_HTTPS = "off";
      ...
      // ---- Deploy: the feature turns on and Geetorus restarts. ----
      await resetRuntimeServicesForTests();
      delete process.env.GEETORUS_MANAGED_RUNTIME_HTTPS;
      installExposureDeps();
      ...
      if (previousHttpsMode === undefined) delete process.env.GEETORUS_MANAGED_RUNTIME_HTTPS;
      else process.env.GEETORUS_MANAGED_RUNTIME_HTTPS = previousHttpsMode;
    }
  }, 40_000);

  it("re-adopts a request-logging service on the same auto port after supervisor stdio closes", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-reconcile-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("re-adopts a live service whose shell command differs from the surviving process argv", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-pnpm-reconcile-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    const previousInstanceId = process.env.GEETORUS_INSTANCE_ID;
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-pnpm-reconcile-${randomUUID()}`;
    ...
    try {
      ...
      if (previousInstanceId === undefined) delete process.env.GEETORUS_INSTANCE_ID;
      else process.env.GEETORUS_INSTANCE_ID = previousInstanceId;
      await fs.rm(geetorusHome, { recursive: true, force: true });
      await fs.rm(workspaceRoot, { recursive: true, force: true });
    }
  }, 20_000);
  ...
  it("does not reuse a stopped auto-port service port while another process owns it", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-unhealthy-adopt-"));
    const geetorusHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-home-"));
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = `runtime-unhealthy-adopt-${randomUUID()}`;
  ...
  it("does not adopt a live registry process from another workspace with the same runtime service ID", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:geetorus-dev`,
      command: "pnpm dev",
      cwd: "/tmp/geetorus-primary",
      port: 49195,
      url: "http://127.0.0.1:49195",
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-geetorus-dev-stale",
      profileKind: "workspace-runtime",
      serviceName: "geetorus-dev",
      command: "pnpm dev",
      cwd: process.cwd(),
  ...
  it("adopts stopped persisted local services when a matching registry process is alive", async () => {
    ...
    const reuseKey = `project_workspace:${projectWorkspaceId}:geetorus-dev`;
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-geetorus-dev-live-stopped",
      profileKind: "workspace-runtime",
      serviceName: "geetorus-dev",
      command: "node",
      cwd,
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
  ...
  it("restarts a stopped auto-port service on the same port when rendered env changes", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-port-reuse-env-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const executionWorkspaceId = randomUUID();
        ...
        services: [
          {
            name: "web",
            command: serviceCommand,
            env: { GEETORUS_TEST_RUNTIME_FLAG: flag },
            port: { type: "auto" },
            readiness: {
              type: "http",
              urlTemplate: "http://127.0.0.1:{{port}}",
  ...
  it("restarts a stopped auto-port service on the same port when it is available", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-port-reuse-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const executionWorkspaceId = randomUUID();
...
describe("realizeExecutionWorkspace with an exact existing branch", () => {
  ...
  it("attaches an isolated worktree checked out on exactly the requested branch", async () => {
    ...
    expect(workspace.cwd).toContain(path.join(".geetorus", "worktrees"));
  ...
  it("fails closed when the requested branch does not exist and creates nothing", async () => {
    ...
    expect(
      existsSync(path.join(repoRoot, ".geetorus", "worktrees", "feature", "never-created")),
    ).toBe(false);
  });
  ...
  it("fails closed instead of reconciling when the managed worktree path holds another branch", async () => {
    ...
    const managedPath = path.join(repoRoot, ".geetorus", "worktrees", "feature/pinned");
