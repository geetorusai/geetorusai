...
describe("docker-entrypoint.sh", () => {
  ...
  it("remaps the node user and chowns /geetorus before gosu when root requests a different UID/GID", async () => {
    ...
    const { stdout, calls } = await runEntrypoint({ USER_UID: "1001", USER_GID: "1001", GEETORUS_HOME: stubDir });
  ...
  it("chowns a root-owned home before gosu even with the default UID/GID (fresh volume mount)", async () => {
    ...
    const { stdout, calls } = await runEntrypoint({ GEETORUS_HOME: stubDir });
  ...
  it("repairs ownership on a GID-only remap (stale group on persisted descendants)", async () => {
    ...
    const { calls } = await runEntrypoint({ USER_GID: "1001", GEETORUS_HOME: stubDir });
  ...
  it("keeps a fully node-owned tree chown-free (no per-boot recursive chown)", async () => {
    ...
    const { calls } = await runEntrypoint({ GEETORUS_HOME: stubDir });

    expect(calls).not.toContain("chown");
    expect(calls).toContain("gosu node echo ENTRYPOINT-CMD-RAN");
  });

  it("honours GEETORUS_HOME for the ownership probe", async () => {
    installStubs({ uid: 0, gid: 0, homeMismatch: true });
