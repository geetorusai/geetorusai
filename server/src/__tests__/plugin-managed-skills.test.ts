...
function manifest(): GeetorusPluginManifestV1 {
  return {
    id: "geetorus.managed-skills-test",
    apiVersion: 1,
    version: "0.1.0",
...
describeEmbeddedPostgres("plugin-managed skills", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-managed-skills-");
  ...
  async function seedCompanyAndPlugin(pluginManifest = manifest()) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@geetorusai/plugin-managed-skills-test",
  ...
  it("installs and resolves managed company skills by stable resource key", async () => {
    ...
    expect(created.skill).toMatchObject({
      ...
      key: "plugin/geetorus-managed-skills-test/wiki-maintainer",
      sourceType: "catalog",
  ...
  it("preserves operator edits during reconcile and restores manifest defaults on reset", async () => {
    ...
    expect(reset.skill?.markdown).toContain("key: \"plugin/geetorus-managed-skills-test/wiki-maintainer\"");
  });
  ...
  it("adds the canonical managed key to manifest-provided markdown skills", async () => {
    ...
    expect(created.status).toBe("created");
    expect(created.skill).toMatchObject({
      key: "plugin/geetorus-managed-skills-test/markdown-skill",
      name: "markdown-skill",
    });
    expect(created.skill?.markdown).toContain("key: \"plugin/geetorus-managed-skills-test/markdown-skill\"");
  });
});
