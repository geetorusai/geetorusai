...
describe("buildGeetorusTaskMarkdown", () => {
  it("keeps a durable task plan in full and resumed context without granting execution approval", () => {
    ...
    expect(buildGeetorusTaskMarkdown({
  ...
  it("surfaces every coalesced wake comment in provider order", () => {
    const markdown = buildGeetorusTaskMarkdown({
      issue: {
        id: "issue-burst",
        identifier: "PAP-5000",
  ...
  it("surfaces exact wake-comment attachment descriptors and inspection guidance", () => {
    ...
    expect(markdown).toContain("GEETORUS_API_URL");
    expect(markdown).toContain("GEETORUS_API_KEY");
  ...
  it.each(["slack", "discord", "telegram", "microsoft-teams", "github"])(
    ...
    (externalChatProvider) => {
      ...
      expect(markdown).toContain("geetorus-upload-artifact.sh --chat-comment");
  ...
  it.each(["slack", "discord", "telegram", "microsoft-teams", "github"])(
    ...
    (externalChatProvider) => {
      ...
      expect(markdown).not.toContain("geetorus-upload-artifact.sh");
      expect(markdown).not.toContain("GEETORUS_API_KEY");
  ...
  it("does not imply that GitHub chat grants attachment or repository-tool access", () => {
    ...
    expect(markdown).toContain(
      "attach the file directly to this Geetorus task or paste the needed text",
    );
    expect(markdown).toContain(
  ...
  it("adds planning directives for assignment and comment task context", () => {
    const assignment = buildGeetorusTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
    ...
    const commentWake = buildGeetorusTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
    ...
    const acceptedConfirmation = buildGeetorusTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
  ...
  it("strips the description for the compact resume variant but keeps directives and the wake comment", () => {
    ...
    const full = buildGeetorusTaskMarkdown(input);
    ...
    const compact = buildGeetorusTaskMarkdown({ ...input, includeDescription: false });
...
describe("summarizeHeartbeatRunContextSnapshot", () => {
  ...
  it("returns null when no allowed fields are present", () => {
    expect(
      summarizeHeartbeatRunContextSnapshot({
        geetorusWake: { comments: [{ body: "hello" }] },
      }),
    ).toBeNull();
  });
});
