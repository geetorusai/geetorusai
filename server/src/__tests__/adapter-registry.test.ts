import { probeAcpxClaudeInstallation } from "@geetorusai/geetorus-runner/live";
import { describe, expect, it, beforeEach, afterEach, vi } from "vitest";
import { buildSandboxNpmInstallCommand } from "@geetorusai/adapter-utils";
...
vi.mock("@geetorusai/geetorus-runner/live", () => ({ probeAcpxClaudeInstallation: vi.fn(async () => undefined) }));
...
describe("server adapter registry", () => {
  ...
  it("rejects an incomplete managed runner provider before probing Codex", async () => {
    const adapter = requireServerAdapter("geetorus_runner");
    expect(adapter.supportsInstructionsBundle).toBe(true);
    expect(adapter.instructionsPathKey).toBe("instructionsFilePath");
    ...
    expect(result).toMatchObject({
      ...
      status: "fail",
      checks: [{
        code: "geetorus_runner_claude_managed_profile_required",
        level: "error",
      }],
    });
  });
  ...
  it.each([
    ...
    const result = await requireServerAdapter("geetorus_runner").testEnvironment({
      companyId: "company-1",
      adapterType: "geetorus_runner",
  ...
  it.each([true, false])("checks actual local ACPX installation readiness (%s)", async (ready) => {
    ...
    const result = await requireServerAdapter("geetorus_runner").testEnvironment({
      companyId: "company-1", adapterType: "geetorus_runner",
  ...
  it("keeps the ACPX Pi profile unavailable", async () => {
    ...
    expect(result).toMatchObject({
      status: "fail",
      checks: [{ code: "geetorus_runner_acpx_agent_unavailable" }],
    });
  });
  it("wraps built-in npm runtime installs with the sandbox-aware install helper", () => {
    ...
    expect(findActiveServerAdapter("geetorus_runner")?.getRuntimeCommandSpec?.({ provider: "codex" })).toEqual({
      command: "codex",
      detectCommand: "codex",
      installCommand: expectedRunnerCodexInstall,
    });
    expect(findActiveServerAdapter("geetorus_runner")?.getRuntimeCommandSpec?.({ provider: "opencode" })).toEqual({
      command: "opencode",
      detectCommand: "opencode",
      installCommand: expectedRunnerOpenCodeInstall,
    });
    expect(findActiveServerAdapter("geetorus_runner")?.getRuntimeCommandSpec?.({ provider: "acpx" })).toEqual({
      command: "geetorus-runnerd",
      detectCommand: null,
      installCommand: null,
    });
  });
