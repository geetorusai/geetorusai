...
describe("Cloud UI snippet", () => {
  it("leaves self-hosted HTML unchanged even when a snippet is configured", () => {
    expect(injectCloudUiSnippet(html, { GEETORUS_CLOUD_UI_SNIPPET: snippet })).toBe(html);
    expect(injectCloudUiSnippet(html, { GEETORUS_CLOUD_UI_SNIPPET_B64: encoded })).toBe(html);
  });

  it.each([
    { GEETORUS_CLOUD_TENANT_SERVER_TOKEN: "test-token" },
    { GEETORUS_MANAGED_CONFIG: "{}" },
  ])("injects only on a configured Cloud instance: %j", (cloud) => {
    expect(injectCloudUiSnippet(html, { ...cloud, GEETORUS_CLOUD_UI_SNIPPET: snippet }))
      .toBe(html.replace("</body>", `${snippet}\n</body>`));
    expect(injectCloudUiSnippet(html, cloud)).toBe(html);
    expect(injectCloudUiSnippet(html, { ...cloud, GEETORUS_CLOUD_UI_SNIPPET: "  " })).toBe(html);
  });
  ...
  it("preserves literal replacement tokens in operator JavaScript", () => {
    ...
    const result = injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}", GEETORUS_CLOUD_UI_SNIPPET: script,
    });
  ...
  it("decodes a base64 snippet on a Cloud instance", () => {
    expect(injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}", GEETORUS_CLOUD_UI_SNIPPET_B64: encoded,
    })).toBe(html.replace("</body>", `${snippet}\n</body>`));
  });
  ...
  it("tolerates whitespace and line wrapping in the base64 value", () => {
    ...
    expect(injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}", GEETORUS_CLOUD_UI_SNIPPET_B64: wrapped,
    })).toBe(html.replace("</body>", `${snippet}\n</body>`));
  });
  ...
  it("wraps a bare script body that carries no markup in a <script> element", () => {
    ...
    expect(injectCloudUiSnippet(html, { GEETORUS_MANAGED_CONFIG: "{}", GEETORUS_CLOUD_UI_SNIPPET: body }))
      .toBe(html.replace("</body>", `${wrapped}\n</body>`));
    expect(injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}",
      GEETORUS_CLOUD_UI_SNIPPET_B64: Buffer.from(body, "utf-8").toString("base64"),
    })).toBe(html.replace("</body>", `${wrapped}\n</body>`));
  });
  ...
  it("preserves literal replacement tokens when wrapping a bare script body", () => {
    ...
    const result = injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}", GEETORUS_CLOUD_UI_SNIPPET: body,
    });
  ...
  it("prefers the plain snippet when both variables are set", () => {
    ...
    const result = injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}",
      GEETORUS_CLOUD_UI_SNIPPET: snippet,
      GEETORUS_CLOUD_UI_SNIPPET_B64: other,
    });
  ...
  it("treats a blank plain variable as disabled even when a base64 value is set", () => {
    expect(injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}",
      GEETORUS_CLOUD_UI_SNIPPET: "  ",
      GEETORUS_CLOUD_UI_SNIPPET_B64: encoded,
    })).toBe(html);
    expect(injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}",
      GEETORUS_CLOUD_UI_SNIPPET: "",
  ...
  it.each([
    ...
    expect(injectCloudUiSnippet(html, {
      GEETORUS_MANAGED_CONFIG: "{}", GEETORUS_CLOUD_UI_SNIPPET_B64: value,
    })).toBe(html);
  });
});
