...
import { withAccountHomeSecretMutationLock } from "@geetorusai/adapter-codex-local/server";
...
vi.mock("@geetorusai/adapter-codex-local/server", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@geetorusai/adapter-codex-local/server")>();
  return {
    ...actual,
  ...
  return {
    async acquireLoginLease(input) {
      ...
      const lease: LoginSessionLease = {
        ...
        authPath: `/tmp/geetorus-adapter-login/${input.sessionId}/auth.json`,
        driver: {
...
describe("adapter device-login routes", () => {
  ...
  beforeEach(() => {
    ...
    mockDeviceLoginPromotion.mockResolvedValue({
      ...
      accountHomeDir: "/tmp/geetorus-codex-account-home/acct-default",
    });
  ...
  it("lets a second owner start an active login in the same company", async () => {
    ...
    mockSecretService.resolveSecretValueForDeviceLoginCheck.mockResolvedValue(
      "/tmp/geetorus-codex-account-home/acct-default",
    );
    const app = await createApp();
  ...
  it("an authenticated login's owner read carries the account-binding claim with the identity verdict", async () => {
    ...
    // company home.
    const instanceRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-login-binding-"));
    try {
      vi.stubEnv("GEETORUS_HOME", instanceRoot);
  ...
  it("a login for the account the company home already holds reports no identity mismatch", async () => {
    ...
    const instanceRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-login-binding-same-"));
    try {
      vi.stubEnv("GEETORUS_HOME", instanceRoot);
      vi.stubEnv("GEETORUS_INSTANCE_ID", "default");
      const companyHome = path.join(instanceRoot, "instances", "default", "companies", COMPANY_1, "codex-home");
  ...
  it("a successful Codex login creates the company secret naming the account home", async () => {
    ...
    mockSecretService.resolveSecretValueForDeviceLoginCheck.mockResolvedValue(
      "/tmp/geetorus-codex-account-home/acct-second",
    );
    mockDeviceLoginPromotion.mockResolvedValueOnce({
      outcome: "promoted",
      accountId: "acct-second",
      accountHomeDir: "/tmp/geetorus-codex-account-home/acct-second",
    });
    const app = await createApp();
    ...
    expect(mockSecretService.create).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        value: "/tmp/geetorus-codex-account-home/acct-second",
      }),
      expect.anything(),
    );
  });
  ...
  it("a repeat login for the same account creates no second secret", async () => {
    ...
    // login is idempotent.
    const accountHomeDir = "/tmp/geetorus-codex-account-home/acct-default";
  ...
  it("fails closed when a pre-existing secret names a different account home", async () => {
    ...
    mockDeviceLoginPromotion.mockResolvedValueOnce({
      ...
      accountHomeDir: "/tmp/geetorus-codex-account-home/acct-stale",
    });
    const app = await createApp();
  ...
  it("fails closed and removes the account home when secret creation fails", async () => {
    ...
    const accountHomeDir = await mkdtemp(path.join(os.tmpdir(), "geetorus-codex-account-home-test-"));
  ...
  it("fails closed when the account identifier cannot form a valid account home name", async () => {
    ...
    mockDeviceLoginPromotion.mockResolvedValueOnce({
      ...
      accountHomeDir: "/tmp/geetorus-codex-account-home/unused",
    });
    const app = await createApp();
