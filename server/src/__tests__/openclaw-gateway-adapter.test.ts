...
import { execute, testEnvironment } from "@geetorusai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@geetorusai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("geetorus:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("GEETORUS_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("GEETORUS_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Geetorus Wake Payload");
      ...
      expect(payload?.geetorus).toBeUndefined();
