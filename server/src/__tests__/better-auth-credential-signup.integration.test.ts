...
import { authAccounts, createDb } from "@geetorusai/db";
import {
...
describeEmbeddedPostgres("Better Auth credential sign-up against the real schema", () => {
  ...
  const originalEnv = {
    ...
    rateLimit: process.env.GEETORUS_AUTH_RATE_LIMIT_ENABLED,
  };
  ...
  beforeAll(async () => {
    ...
    process.env.GEETORUS_AUTH_RATE_LIMIT_ENABLED = "false";

    database = await startEmbeddedPostgresTestDatabase("geetorus-better-auth-signup-");
  ...
  afterAll(async () => {
    ...
    if (originalEnv.rateLimit === undefined) delete process.env.GEETORUS_AUTH_RATE_LIMIT_ENABLED;
    else process.env.GEETORUS_AUTH_RATE_LIMIT_ENABLED = originalEnv.rateLimit;
  });
