...
import { canonicalizeExternalObjectUrl } from "@geetorusai/shared/external-objects-server";
...
describeEmbeddedPostgres("externalObjectService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-external-objects-");
  ...
  it("uses a mock plugin provider to detect and resolve non-GitHub objects", async () => {
    ...
    const manifest: GeetorusPluginManifestV1 = {
      id: "geetorus.mock-object-provider",
      apiVersion: 1,
      version: "1.0.0",
    ...
    const [plugin] = await db.insert(plugins).values({
      ...
      packageName: "@geetorus/mock-object-provider",
