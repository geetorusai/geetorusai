...
function manifest(): GeetorusPluginManifestV1 {
  return {
    id: "geetorus.managed-agents-test",
    apiVersion: 1,
    version: "0.1.0",
...
function pausedManifest(): GeetorusPluginManifestV1 {
...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: GeetorusPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@geetorusai/plugin-managed-agents-test",
  ...
  it("records plugin provenance when a manifest creates a paused managed agent", async () => {
    ...
    expect(created.agent).toMatchObject({
      status: "paused",
      pauseReason: "Provisioned paused by plugin geetorus.managed-agents-test; requires explicit activation.",
    });
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-managed-agent-wiki-")));
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "geetorus.managed-agents-test",
