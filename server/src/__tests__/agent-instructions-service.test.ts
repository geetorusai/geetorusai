...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const geetorusHome = await makeTempDir("geetorus-agent-instructions-home-");
    const externalRoot = await makeTempDir("geetorus-agent-instructions-external-");
    cleanupDirs.add(geetorusHome);
    cleanupDirs.add(externalRoot);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("geetorus-agent-instructions-new-external-");
    cleanupDirs.add(geetorusHome);
    cleanupDirs.add(externalRoot);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("geetorus-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it.skipIf(process.platform === "win32")("rejects instruction symlinks for immutable runner snapshots without changing legacy exports", async () => {
    const externalRoot = await makeTempDir("geetorus-agent-instructions-symlink-");
    const outsideRoot = await makeTempDir("geetorus-agent-instructions-outside-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const geetorusHome = await makeTempDir("geetorus-agent-instructions-recover-");
    cleanupDirs.add(geetorusHome);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const geetorusHome = await makeTempDir("geetorus-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("geetorus-agent-instructions-stale-root-");
    cleanupDirs.add(geetorusHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const geetorusHome = await makeTempDir("geetorus-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("geetorus-agent-instructions-heal-write-stale-");
    cleanupDirs.add(geetorusHome);
    cleanupDirs.add(staleRoot);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const geetorusHome = await makeTempDir("geetorus-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("geetorus-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(geetorusHome);
    cleanupDirs.add(staleRoot);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const geetorusHome = await makeTempDir("geetorus-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("geetorus-agent-instructions-partial-root-");
    cleanupDirs.add(geetorusHome);
    cleanupDirs.add(staleRoot);
    process.env.GEETORUS_HOME = geetorusHome;
    process.env.GEETORUS_INSTANCE_ID = "test-instance";
