...
import { createDb, plugins } from "@geetorusai/db";
import {
...
async function createBundledPluginFixture(
  ...
  const packageName = `@geetorusai/${slug}`;
  const pluginKey = `geetorus.${slug.replace(/^plugin-/, "").replace(/-/g, "_")}`;
...
describe("ensureLocalPluginBuilt", () => {
  ...
  it("skips auto-build for local plugin paths outside the repo", async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-plugin-outside-"));
    ...
    await ensureLocalPluginBuilt(
      packageRoot,
      {
        name: "@geetorusai/plugin-outside",
        geetorusPlugin: {
          manifest: "./dist/manifest.js",
          worker: "./dist/worker.js",
        },
  ...
  it("skips auto-build when GEETORUS_DISABLE_PLUGIN_AUTOBUILD=1", async () => {
    ...
    await ensureLocalPluginBuilt(
      ...
      {
        processEnv: { GEETORUS_DISABLE_PLUGIN_AUTOBUILD: "1" },
        execFileAsyncImpl: execStub,
      },
    );
  ...
  it.each([
    ...
    const execStub = vi.fn(async (_file: string, args: readonly string[]) => {
      if (args.join(" ") === installArgs.join(" ")) {
        await mkdir(path.join(fixture.packageRoot, "node_modules", "@geetorusai", "plugin-sdk"), { recursive: true });
      }
      if (args.join(" ") === "build") {
...
describeEmbeddedPostgres("plugin install auto-build route", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-plugin-autobuild-");
  ...
  afterEach(async () => {
    ...
    cleanupPaths.clear();
    delete process.env["GEETORUS_DISABLE_PLUGIN_AUTOBUILD"];
  });
  ...
  it("auto-builds standalone bundled local plugins outside the root pnpm workspace", async () => {
    ...
    expect(existsSync(path.join(fixture.distDir, "worker.js"))).toBe(true);
    expect(existsSync(path.join(fixture.distDir, "ui", "index.js"))).toBe(true);
    expect(existsSync(path.join(fixture.packageRoot, "node_modules", "@geetorusai", "plugin-sdk"))).toBe(true);
    expect(mockLifecycle.load).toHaveBeenCalledTimes(1);
  }, 60_000);
  ...
  it("bootstraps standalone bundled local plugin runtime dependencies when dist already exists", async () => {
    ...
    expect(existsSync(path.join(fixture.distDir, "manifest.js"))).toBe(true);
    expect(existsSync(path.join(fixture.packageRoot, "node_modules", "@geetorusai", "plugin-sdk"))).toBe(false);
  ...
  it("returns the manual build command when auto-build is disabled and dist is missing", async () => {
    process.env["GEETORUS_DISABLE_PLUGIN_AUTOBUILD"] = "1";
    ...
    expect(res.body.error).toContain("does not appear to be a Geetorus plugin (no manifest found)");
