...
import type { ChatProvider } from "@geetorusai/shared";
...
describeEmbeddedPostgres(
  ...
  () => {
    ...
    beforeAll(async () => {
      process.env.GEETORUS_PUBLIC_URL = "https://geetorus.example";
      tempDb = await startEmbeddedPostgresTestDatabase(
        "geetorus-terminal-chat-interaction-",
      );
    ...
    it("keeps unsupported governance interactions authoritative in Geetorus", async () => {
      const fixture = await seedBoundIssue();
    ...
    it("settles UI answers and skips on every provider-native card with no live actions", async () => {
      ...
      expect(
        skippedSettlements.every(
          (row) => row.payload.card?.body === "Skipped in Geetorus.",
        ),
      ).toBe(true);
      expect(
        skippedSettlements.every(
    ...
    it("settles native and link-only confirmation cards without inventing controls", async () => {
      ...
      for (const outcome of ["accept", "reject"] as const) {
        ...
        expect(
          ...
          expect.objectContaining({ type: "link", label: "Open in Geetorus" }),
        ]);
        expect(
          originals.find(
