...
function createWorkspaceCwd(instanceId: string | null) {
  const cwd = mkdtempSync(path.join(os.tmpdir(), "geetorus-handoff-issuer-"));
  tempDirs.push(cwd);
  mkdirSync(path.join(cwd, ".geetorus"), { recursive: true });
  if (instanceId) {
    writeFileSync(
      path.join(cwd, ".geetorus", ".env"),
      `GEETORUS_HOME=/srv/home\nGEETORUS_INSTANCE_ID=${instanceId}\n`,
      "utf8",
    );
  }
  return cwd;
...
beforeEach(() => {
  previousSecret = process.env.GEETORUS_WORKSPACE_HANDOFF_SECRET;
  process.env.GEETORUS_WORKSPACE_HANDOFF_SECRET = "issuer-test-root-secret";
});

afterEach(() => {
  if (previousSecret === undefined) delete process.env.GEETORUS_WORKSPACE_HANDOFF_SECRET;
  else process.env.GEETORUS_WORKSPACE_HANDOFF_SECRET = previousSecret;
...
describe("issueWorkspaceLoginHandoff", () => {
  ...
  it("falls back to credentials when no signing secret is configured at all", async () => {
    delete process.env.GEETORUS_WORKSPACE_HANDOFF_SECRET;
    const previousAuthSecret = process.env.BETTER_AUTH_SECRET;
    const previousJwtSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
    ...
    try {
      ...
      if (previousJwtSecret !== undefined) process.env.GEETORUS_AGENT_JWT_SECRET = previousJwtSecret;
    }
  });
