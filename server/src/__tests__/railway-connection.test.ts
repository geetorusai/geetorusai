...
import { agents, approvals, companies, companyMemberships, companySecrets, connectionGrants, createDb, heartbeatRuns, issues, toolCatalogEntries, toolActionRequests, toolConnections, toolPolicies, toolProfileBindings, toolProfiles, toolAccessAuditEvents } from "@geetorusai/db";
...
(support.supported ? describe : describe.skip)("Railway connection lifecycle and gateway", () => {
  ...
  beforeAll(async () => { temp = await startEmbeddedPostgresTestDatabase("geetorus-railway-"); db = createDb(temp.connectionString); }, 20000);
    ...
    const request = vi.fn(async (url: string, init: RequestInit) => {
      ...
      if (url.replace(/\/$/, "") === RAILWAY_MCP_URL && init.method === "POST") {
        ...
        if (JSON.parse(body).method === "tools/call") {
          ...
          return Response.json({ jsonrpc: "2.0", id: "geetorus-railway-workspace-probe", result: { structuredContent: { workspaces: [{ id: target.projectId }] } } });
        }
        return Response.json({ jsonrpc: "2.0", id: "geetorus-catalog-refresh", result: { tools } });
      }
  ...
  it("discovers direct tools, blocks opaque actions and quarantines changed tools even on reconnect", async () => {
    ...
    expect(rows.find((r) => r.toolName === "geetorus-railway-read-logs")?.status).toBe("active");
    ...
    f.setTools([{ name: "geetorus_railway_restart" }]);
    await expect(f.service.refreshCatalog(f.connectionId, actor)).rejects.toThrow("Railway advertised a reserved Geetorus action");
  });
  ...
  it("denies retired source-deployment entries before refresh and disables them on refresh", async () => {
    ...
    const [existing] = await db.select().from(toolCatalogEntries).where(and(eq(toolCatalogEntries.connectionId, f.connectionId), eq(toolCatalogEntries.toolName, "geetorus-railway-restart")));
    const names = ["geetorus-railway-deploy-revision", "geetorus_railway_deploy_revision", "geetorusRailwayDeployRevision"];
    ...
    const restart = listed.find((tool) => tool.upstreamToolName === "geetorus-railway-restart")!;
    expect(restart).toBeTruthy();
    f.request.mockClear();
    const { deploymentId: _, ...ids } = target;
    const parameters = { ...ids, repository: "example/app", commitSha: "a".repeat(40) };
    await expect(gateway.executeTool({ sessionToken: session.token, tool: restart.name.replace("geetorus-railway-restart", names[0]), parameters, idempotencyKey: randomUUID() })).rejects.toMatchObject({ reasonCode: "tool_not_found" });
  ...
  it("enforces policy, grant, run and company boundaries before API execution", async () => {
    ...
    const tool = (await gateway.listToolsForSession(session.token)).find((r) => r.upstreamToolName === "geetorus-railway-restart")!;
