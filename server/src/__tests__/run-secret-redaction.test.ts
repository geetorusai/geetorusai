...
describe("registered run secret redaction", () => {
  ...
  it("redacts run detail, event, and transcript fields and strips registry material", () => {
    const result = redactRegisteredSecretValues({
      contextSnapshot: {
        issueId: "issue-1",
        geetorusSecretRedactions: [{ material: { ciphertext: "encrypted" } }],
      },
...
describe("batched run secret redaction", () => {
  ...
  it("reads only registry JSON once for 200 runs and resolves shared secrets once", async () => {
    const contextSnapshot = { geetorusSecretRedactions: [{ fingerprintSha256: "shared", material: { value: secret } }] };
    ...
    expect(dialect.sqlToQuery(select.mock.calls[0][0].contextSnapshot).sql).toContain("-> 'geetorusSecretRedactions'");
  });

  it("keeps each run's registry separate and observes new registrations on the next request", async () => {
    const rows = [{ id: "a", contextSnapshot: { geetorusSecretRedactions: [{ fingerprintSha256: "one", material: { value: secret } }] } }];
    ...
    rows[0].contextSnapshot.geetorusSecretRedactions.push({ fingerprintSha256: "two", material: { value: "new-secret" } });
  ...
  it("does not query for an empty list and fails closed on decryption failure", async () => {
    const { registry, select } = fixture([{ id: "a", contextSnapshot: { geetorusSecretRedactions: [{ fingerprintSha256: "one", material: {} }] } }]);
