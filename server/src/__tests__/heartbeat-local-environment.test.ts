...
describeEmbeddedPostgres("heartbeat local environment lifecycle", () => {
  ...
  beforeAll(async () => {
    previousAgentJwtSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
    process.env.GEETORUS_AGENT_JWT_SECRET = "heartbeat-local-environment-test-secret";
  ...
  afterAll(async () => {
    ...
    if (previousAgentJwtSecret === undefined) {
      ...
      process.env.GEETORUS_AGENT_JWT_SECRET = previousAgentJwtSecret;
    }
  });
  ...
  it("runs work through the default Local environment lease", async () => {
    ...
    expect(context.geetorusEnvironment).toMatchObject({
      id: localRows[0]?.id,
      name: "Local",
      driver: "local",
      leaseId: leases[0]?.id,
  ...
  it("injects run-scoped Geetorus env into process agents", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issuePrefix = `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`;
    const tempDir = await mkdtemp(join(tmpdir(), "geetorus-process-env-"));
      ...
      adapterConfig: {
        ...
        args: [
          ...
          [
            ...
            "agentId: process.env.GEETORUS_AGENT_ID ?? null,",
            "companyId: process.env.GEETORUS_COMPANY_ID ?? null,",
            "apiUrl: process.env.GEETORUS_API_URL ?? null,",
            "runId: process.env.GEETORUS_RUN_ID ?? null,",
            "apiKeyPresent: Boolean(process.env.GEETORUS_API_KEY),",
            "}));",
          ].join(" "),
        ],
      },
