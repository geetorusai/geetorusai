...
import { activityLog, agents, companies, createDb, executionWorkspaces, goals, issues, projects, projectWorkspaces, type Db } from "@geetorusai/db";
...
async function makeWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-file-resources-"));
...
describeEmbeddedPostgres("workspace file resources", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-file-resources-");
...
describeEmbeddedPostgres("file resource route guards", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-file-resource-guards-");
    db = createDb(tempDb.connectionString);
  }, 60_000);
