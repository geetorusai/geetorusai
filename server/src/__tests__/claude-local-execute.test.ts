...
import type { AdapterRuntimeMcpServer } from "@geetorusai/adapter-utils";
...
const argv = process.argv.slice(2);
if (argv.includes("--help")) {
  const helpCountPath = process.env.GEETORUS_TEST_HELP_COUNT_PATH;
  if (helpCountPath) {
...
const capturePath = process.env.GEETORUS_TEST_CAPTURE_PATH;
const statePath = process.env.GEETORUS_TEST_STATE_PATH;
const payload = {
...
describe("claude execute", () => {
  it.each([
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-default-"));
    ...
    try {
      const result = await execute({
        ...
        config: {
          ...
          env: { GEETORUS_TEST_CAPTURE_PATH: capturePath },
        },
        context: {}, onLog: async () => {},
      });
  ...
  it("uses a strict per-agent MCP config only when managed servers are present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-mcp-config-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root);
    try {
      ...
      const alpha = await run("run-alpha", "agent-alpha", [{
        name: "alpha",
        url: "https://geetorus.example/api/tool-gateway/gateways/alpha/mcp",
        token: "alpha-token",
  ...
  /**
   * Regression tests for https://github.com/geetorusai/geetorus/issues/2848
   *
  ...
  it("passes --append-system-prompt-file on a fresh session when instructionsFile is set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-fresh-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root);
  ...
  it("omits --append-system-prompt-file on a resumed session even when instructionsFile is set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-resume-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root);
    const instructionsFile = path.join(root, "instructions.md");
    await fs.writeFile(instructionsFile, "# Agent instructions", "utf-8");
    try {
  ...
  it("commandNotes reports injection on a fresh session with instructionsFile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-notes-fresh-"));
  ...
  it("commandNotes is empty on a resumed session even when instructionsFile is set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-notes-resume-"));
    const { workspace, commandPath, restore } = await setupExecuteEnv(root);
    const instructionsFile = path.join(root, "instructions.md");
    await fs.writeFile(instructionsFile, "# Agent instructions", "utf-8");
  ...
  it("rebuilds the combined instructions file when an unknown resumed session falls back to fresh", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-resume-fallback-"));
      ...
      const result = await execute({
        ...
        config: {
          ...
          env: {
            ...
            GEETORUS_TEST_STATE_PATH: statePath,
          },
          promptTemplate: "Do work.",
          instructionsFilePath: instructionsFile,
        },
  ...
  it("normalizes max-turn exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-max-turns-"));
    const resultEvent = {
      type: "result",
  ...
  it("does not normalize unstructured max-turn text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-max-turn-text-"));
    const resultEvent = {
      type: "result",
      subtype: "error",
      session_id: "11111111-1111-4111-8111-111111111111",
  ...
  it("does not normalize fallback stdout/stderr max-turn text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-max-turn-fallback-"));
    const { workspace, commandPath, restore } = await setupExecuteEnv(root, {
  ...
  it("logs HOME, CLAUDE_CONFIG_DIR, and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-meta-"));
    const workspace = path.join(root, "workspace");
    const binDir = path.join(root, "bin");
    const commandPath = path.join(binDir, "claude");
    const capturePath = path.join(root, "capture.json");
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-sandbox-"));
    ...
    try {
      const result = await execute({
        ...
        config: {
          ...
          cwd: localWorkspace,
          env: {
            GEETORUS_TEST_CAPTURE_PATH: capturePath1,
          },
          promptTemplate: "Follow the geetorus heartbeat.",
        },
        context: {},
      ...
      expect(capture.claudeConfigDir).toBe(path.join(remoteWorkspace, ".geetorus-runtime", "claude", "config"));
  ...
  it("omits --effort for sandbox-managed runs when the installed Claude CLI does not advertise it", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-sandbox-effort-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root, {
  ...
  it("passes through --effort and reuses the sandbox capability probe across sandbox leases when the installed Claude CLI advertises it", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-sandbox-effort-supported-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root, {
    ...
    const baseInput = {
      ...
      config: {
        ...
        env: {
          GEETORUS_TEST_CAPTURE_PATH: capturePath,
          GEETORUS_TEST_HELP_COUNT_PATH: helpCountPath,
        },
  ...
  it("reuses a stable Geetorus-managed Claude prompt bundle across equivalent runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-bundle-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      const first = await execute({
        ...
        runtimeMcp: {
          getServers: () => [{
            name: "Geetorus projects",
            url: "http://localhost:3100/api/mcp/project-tools",
            connectionId: "geetorus-project-tools",
            token: "run-jwt-token",
          }],
        },
        authToken: "run-jwt-token",
      ...
      const second = await execute({
        ...
        config: {
          ...
          instructionsFilePath: instructionsPath,
          env: {
            GEETORUS_TEST_CAPTURE_PATH: capturePath2,
          },
          promptTemplate: "Follow the geetorus heartbeat.",
          geetorusSkillSync: {
            desiredSkills: ["geetorus"],
      ...
      expect(capture1.skillEntries).toContain("geetorus");
      expect(capture2.argv).toContain("--resume");
      expect(capture2.argv).toContain("11111111-1111-4111-8111-111111111111");
      expect(capture2.prompt).toContain("## Geetorus Resume Delta");
      expect(capture2.prompt).not.toContain("Follow the geetorus heartbeat.");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("starts a fresh Claude session when the stable prompt bundle changes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-reset-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    ...
    try {
      ...
      expect(after.prompt).toContain("Follow the geetorus heartbeat.");
  ...
  it("classifies Claude 'out of extra usage' failures as provider quota errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("treats subtype=success results as successful even when the process exits nonzero", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-success-subtype-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingClaudeCommand(commandPath, {
  ...
  it("classifies rate-limit / overloaded failures without reset metadata as transient", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-rate-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingClaudeCommand(commandPath, {
  ...
  it("does not reclassify deterministic Claude failures (auth, max turns) as transient", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-execute-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingClaudeCommand(commandPath, {
  ...
  it("auto-rotates session on previous_message_id 400 (synthetic-msg poisoning) and succeeds on retry", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-poisoned-msgid-"));
    const { workspace, commandPath, capturePath, statePath, restore } = await setupExecuteEnv(root, {
  ...
  it("drops sessionId and forces clearSession when a fresh run reports a poisoned previous_message_id", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-poisoned-fresh-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root, {
  ...
  it("forces clearSession when the recovery retry also reports a poisoned previous_message_id", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-claude-exec-poisoned-retry-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root, {
      commandWriter: writeAlwaysPoisonedMessageIdClaudeCommand,
    });
    try {
