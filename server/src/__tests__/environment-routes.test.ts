...
const originalSecretsProviderEnv = process.env.GEETORUS_SECRETS_PROVIDER;
...
describe("environment routes", () => {
  afterAll(async () => {
    if (originalSecretsProviderEnv === undefined) {
      ...
      process.env.GEETORUS_SECRETS_PROVIDER = originalSecretsProviderEnv;
    }
    if (!server) return;
  ...
  describe("platform-provisioned environment floor on cloud-managed instances", () => {
    function createPlatformSandboxEnvironment() {
      ...
      return {
        ...
        metadata: { managedByGeetorus: true, managedSandboxProvider: "daytona" },
        createdAt: now,
        updatedAt: now,
      };
    }
    ...
    beforeEach(() => {
      process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN = "test-server-token";
    });
    afterEach(() => {
      delete process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN;
    });
    ...
    it("never echoes credential-shaped config keys, while tenant env vars round-trip", async () => {
      ...
      expect(res.body.metadata).toMatchObject({ managedByGeetorus: true });
    ...
    it("exposes structural config to restricted company readers instead of blanking it", async () => {
      ...
      expect(res.body[0].metadata).toMatchObject({ managedByGeetorus: true });
    });
    ...
    it("hides the local environment from every read surface under managed-sandbox-only", async () => {
      ...
      const localRow = {
        ...
        config: {},
        envVars: {},
        metadata: { managedByGeetorus: true, defaultForInstance: true },
      };
    ...
    it("keeps the local environment visible when managed-sandbox-only is off", async () => {
      const localRow = {
        ...
        config: {},
        envVars: {},
        metadata: { managedByGeetorus: true },
      };
    ...
    it("allows a marker-clear-only patch to unblock a row with a stale legacy kubernetes marker", async () => {
      ...
      // `managedByGeetorus`), and with the persisted execution mode not
    ...
    it("allows a marker-clear-only patch on a non-slot driver with a stale platform marker", async () => {
      ...
      const res = await request(app)
        ...
        .send({ metadata: { managedByGeetorus: false, managedKubernetesSandbox: false } });
    ...
    it("refuses the marker-clear patch on the sandbox slot row while managed provisioning is configured", async () => {
      ...
      // managedByGeetorus is THE provisioner-owned slot row, adopted and
      ...
      process.env.GEETORUS_MANAGED_CONFIG = MANAGED_CONFIG_WITH_SANDBOX_ENTRY;
      try {
        mockEnvironmentService.getById.mockResolvedValue(createPlatformSandboxEnvironment());
        const app = createApp(ownerAdminActor);
    ...
    it("refuses the marker-clear patch on the sandbox slot row under the forced kubernetes execution mode", async () => {
      ...
      try {
        ...
        delete process.env.GEETORUS_EXECUTION_MODE;
      }
    });
    ...
    it("refuses the marker-clear patch on the managed local row", async () => {
      ...
      const res = await request(app)
        ...
        .send({ metadata: { managedByGeetorus: false } });
    ...
    it("rejects non-marker-only patches to platform-provisioned rows even from instance admins", async () => {
      ...
      const res = await request(app)
        .patch("/api/environments/env-managed-1")
        .send({ name: "Renamed", metadata: { managedByGeetorus: false } });
    ...
    it("accepts platform markers in client payloads on self-hosted instances", async () => {
      ...
      mockEnvironmentService.update.mockResolvedValue({
        ...existing,
        metadata: { source: "manual", managedByGeetorus: true },
      });
      const app = createApp({
        type: "board",
        userId: "admin-1",
      ...
      const res = await request(app)
        .patch("/api/environments/env-tenant-1")
        .send({ metadata: { source: "manual", managedByGeetorus: true } });
  ...
  it("rejects deleting the current instance default environment", async () => {
    const environment = {
      ...
      config: {
        ...
        port: 22,
        username: "ssh-user",
        remoteWorkspacePath: "/srv/geetorus/workspace",
        privateKey: null,
  ...
  it("rejects deleting an environment with a live reusable sandbox lease", async () => {
    ...
    expect(res.status).toBe(409);
    expect(res.body.error).toBe(
      "Cannot delete this environment while it has a reusable sandbox lease. Remove the associated execution workspace or issue so Geetorus can destroy the sandbox, then retry.",
    );
  ...
  it("uses the configured provider for SSH private key secret materialization", async () => {
    process.env.GEETORUS_SECRETS_PROVIDER = "aws_secrets_manager";
    const environment = {
      ...createEnvironment(),
      id: "env-ssh",
      name: "SSH Fixture",
