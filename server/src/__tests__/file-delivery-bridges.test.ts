...
import { agents, heartbeatRuns, issueAttachments, issueComments, issues, issueWorkProducts } from "@geetorusai/db";
import { startAdapterExecutionTargetGeetorusBridge, type AdapterSandboxExecutionTarget } from "@geetorusai/adapter-utils/execution-target";
...
const liveDaytona = process.env.GEETORUS_FILE_DELIVERY_DAYTONA === "1";
const helper = path.resolve(import.meta.dirname, "../../../skills/geetorus/scripts/geetorus-upload-artifact.sh");
...
describe(`durable file delivery (${liveDaytona ? "Daytona" : "local processes"})`, () => {
  ...
  beforeAll(async () => {
    vi.stubEnv("GEETORUS_AGENT_JWT_SECRET", "isolated-file-delivery-test-secret");
  ...
  it.each([false, true])("runs the actual Bash helper with HTTP/2 enabled=%s", async duplex => {
    ...
    const installedHelper = await remote.write("geetorus-upload-artifact.sh", await readFile(helper));
    ...
    expect(bridge?.env.GEETORUS_API_BRIDGE_MODE).toBe(duplex ? "http2_v1" : "queue_v1");
    const env = { ...bridge!.env, GEETORUS_RUN_ID: fixture.runId,
      GEETORUS_COMPANY_ID: fixture.companyId, GEETORUS_TASK_ID: fixture.issueId,
      GEETORUS_HELPER_STATE_DIR: path.join(remote.workspace, ".helper-state") };
    const headers = { authorization: `Bearer ${bridge!.env.GEETORUS_API_KEY}` };
    ...
    try {
      for (const file of files) {
        ...
        const download = await remote.fetch(bridge!.env.GEETORUS_API_URL + receipt.attachment.downloadPath, headers);
        expect(download.status).toBe(200);
        expect(Buffer.from(await download.arrayBuffer())).toEqual(file.body);
        receipts.push(receipt);
      }
      const foreign = await server.fixture();
      const denied = await remote.fetch(`${bridge!.env.GEETORUS_API_URL}/api/issues/${foreign.issueId}/attachments`, headers);
      expect(denied.status).toBe(404);
      const unauthorized = await remote.fetch(`${bridge!.env.GEETORUS_API_URL}/api/issues/${fixture.issueId}/attachments`, { authorization: "Bearer wrong-token" });
  ...
  it("registers native files with API tools disabled and returns durable download receipts", async () => {
    const fixture = await server.fixture({ apiToolsEnabled: false });
    const remote = await execution(fixture.workspace);
    const authority = new GeetorusRunnerToolAuthority(server.db, { ...fixture,
    ...
    const token = createLocalAgentJwt(fixture.agentId, fixture.companyId, "geetorus_runner", fixture.runId)!;
