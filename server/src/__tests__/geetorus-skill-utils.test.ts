...
const artifactHelperPath = path.resolve(
  "skills/geetorus/scripts/geetorus-upload-artifact.sh",
);
...
async function makeArtifactHelperHarness(
  ...
  const root = await makeTempDir("geetorus-artifact-helper-");
...
attachment_json() {
  originating_run_id="$GEETORUS_RUN_ID"
...
if [[ "$method" == "POST" && "$url" == */attachments ]]; then
  ...
  if [[ -n "\${FAKE_CURL_COMMIT_AFTER_DROP_DELAY:-}" && ! -f "$FAKE_CURL_STATE_DIR/upload-drop-used" ]]; then
    ...
    (
      ...
      printf '%s' "$GEETORUS_RUN_ID" >"$FAKE_CURL_STATE_DIR/originating-run-id"
    ) >/dev/null 2>&1 &
    exit 56
  fi
...
if [[ "$method" == "POST" && "$url" == */work-products ]]; then
  respond "$(jq -nc --arg runId "$GEETORUS_RUN_ID" '{ id: "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", createdByRunId: $runId }')" 201
  exit 0
fi
...
`,
  ...
  const env = {
    ...
    GEETORUS_API_KEY: "test-run-key",
    GEETORUS_API_URL: options.apiUrl ?? "http://geetorus.invalid",
    GEETORUS_COMPANY_ID: "company-1",
    GEETORUS_HELPER_STATE_DIR: lockDir,
    GEETORUS_RUN_ID: "dddddddd-dddd-4ddd-8ddd-dddddddddddd",
...
describe("geetorus skill utils", () => {
  ...
  it("lists bundled runtime skills from ./skills without pulling in .agents/skills", async () => {
    const root = await makeTempDir("geetorus-skill-roots-");
    cleanupDirs.add(root);

    const moduleDir = path.join(root, "a", "b", "c", "d", "e");
    await fs.mkdir(moduleDir, { recursive: true });
    await fs.mkdir(path.join(root, "skills", "geetorus"), { recursive: true });
    await fs.mkdir(path.join(root, "skills", "geetorus-create-agent"), { recursive: true });
    await fs.mkdir(path.join(root, ".agents", "skills", "diagnose-why-work-stopped"), { recursive: true });
    await fs.mkdir(path.join(root, ".agents", "skills", "geetorus-create-plugin"), { recursive: true });
    ...
    expect(entries.map((entry) => entry.runtimeName)).toEqual([
      "geetorus",
      "geetorus-create-agent",
    ]);
    expect(entries[0]?.source).toBe(path.join(root, "skills", "geetorus"));
    expect(entries[1]?.source).toBe(path.join(root, "skills", "geetorus-create-agent"));
  });

  it("documents artifact uploads in the installed Geetorus skill", async () => {
    const skillBody = await fs.readFile(path.resolve("skills/geetorus/SKILL.md"), "utf8");
    const referenceBody = await fs.readFile(path.resolve("skills/geetorus/references/artifacts.md"), "utf8");
    const helperBody = await fs.readFile(path.resolve("skills/geetorus/scripts/geetorus-upload-artifact.sh"), "utf8");
    ...
    expect(skillBody).not.toContain("/api/companies/$GEETORUS_COMPANY_ID/issues/$GEETORUS_TASK_ID/attachments");
    expect(referenceBody).toContain("Generated Artifacts and Work Products");
    expect(referenceBody).toContain("scripts/geetorus-upload-artifact.sh");
    expect(referenceBody).toContain("POST");
    expect(referenceBody).toContain("/api/companies/$GEETORUS_COMPANY_ID/issues/$GEETORUS_TASK_ID/attachments");
    expect(referenceBody).toContain("/api/issues/$GEETORUS_TASK_ID/work-products");
    ...
    expect(referenceBody).not.toContain("npx geetorusai issue comment");
    ...
    await expect(
      fs.access(path.resolve("skills/geetorus/scripts/geetorus-upload-artifact.sh")),
    ).resolves.toBeUndefined();
    await expect(fs.access(path.resolve("scripts/geetorus-upload-artifact.sh"))).rejects.toThrow();
  });
  ...
  it("accepts GEETORUS_API_URL with an existing trailing API path", async () => {
    const harness = await makeArtifactHelperHarness(cleanupDirs, {
      apiUrl: "http://geetorus.invalid/api/",
    });
    ...
    expect(requestLog).toContain(
      "GET http://geetorus.invalid/api/issues/issue-1/attachments",
    );
  ...
  it("documents governed agent interaction resolution invariants", async () => {
    const apiReference = await fs.readFile(path.resolve("skills/geetorus/references/api-reference.md"), "utf8");
  ...
  it("requires issue-update writes to be verified, not inferred", async () => {
    ...
    // geetorus-issue-update-helper.test.ts against a live local server.
  });
  ...
  it("keeps the create-issue-interaction-ui guide as a maintainer-only skill", async () => {
    ...
    expect(normalizedLowerSkillBody).toContain(
      "not the operational agents that run inside a deployed geetorus company",
    );
  ...
  it("removes stale maintainer-only symlinks from a shared skills home", async () => {
    const root = await makeTempDir("geetorus-skill-cleanup-");
    cleanupDirs.add(root);

    const skillsHome = path.join(root, "skills-home");
    const runtimeSkill = path.join(root, "skills", "geetorus");
    ...
    await fs.symlink(runtimeSkill, path.join(skillsHome, "geetorus"));
    await fs.symlink(customSkill, path.join(skillsHome, "release-notes"));
    await fs.symlink(staleMaintainerSkill, path.join(skillsHome, "release"));

    const removed = await removeMaintainerOnlySkillSymlinks(skillsHome, ["geetorus"]);
