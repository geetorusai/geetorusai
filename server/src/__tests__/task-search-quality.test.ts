...
import { companies, createDb, documents, issueComments, issueDocuments, issues, getEmbeddedPostgresTestSupport, startEmbeddedPostgresTestDatabase } from "@geetorusai/db";
import { companySearchQuerySchema } from "@geetorusai/shared";
...
describe.skipIf(!support.supported)("task search relevance rubric (real PostgreSQL)", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-search-quality-");
