...
const originalGeetorusApiUrlEnv = process.env.GEETORUS_API_URL;
...
describeEmbeddedPostgres("routine service live-execution coalescing", () => {
  ...
  beforeAll(async () => {
    process.env.GEETORUS_API_URL = "http://localhost:3100";
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-routines-service-");
  ...
  afterAll(async () => {
    ...
    if (originalGeetorusApiUrlEnv === undefined) {
      delete process.env.GEETORUS_API_URL;
    } else {
      process.env.GEETORUS_API_URL = originalGeetorusApiUrlEnv;
    }
  });
  ...
  it("interpolates routine variables into the execution issue and stores resolved values", async () => {
    ...
    expect(storedIssue?.title).toBe("repo triage for geetorus");
    expect(storedIssue?.description).toBe("Review geetorus for high bugs");
  ...
  it("rejects an HMAC webhook replay when automatic execution is suppressed", async () => {
    const runtimeEnv = { GEETORUS_IN_WORKTREE: "yes", GEETORUS_INSTANCE_ID: "worktree-routines-test" };
  ...
  it("dispatches only post-cutoff scheduled routines in an armed worktree", async () => {
    const runtimeEnv = { GEETORUS_IN_WORKTREE: "true", GEETORUS_INSTANCE_ID: "worktree-routines-test" };
