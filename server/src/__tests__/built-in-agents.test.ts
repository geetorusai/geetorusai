...
const BUILT_IN_MARKER_UNIQUE_INDEX_DDL = `
  CREATE UNIQUE INDEX IF NOT EXISTS "${BUILT_IN_MARKER_UNIQUE_INDEX}"
    ON "agents" ("company_id", ((metadata -> 'geetorusBuiltInAgent' ->> 'key')))
    WHERE (metadata -> 'geetorusBuiltInAgent' ->> 'key') IS NOT NULL
...
describe("built-in agent asset loading", () => {
  it("uses the first readable candidate path", () => {
    const dir = mkdtempSync(path.join(tmpdir(), "geetorus-built-in-agent-"));
    try {
...
describeEmbeddedPostgres("built-in agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-built-in-agents-");
  ...
  it("reconciles an enabled Reflection Coach bundle with skill sync and a disabled routine", async () => {
    ...
    const [skill] = await db
      .select()
      .from(companySkills)
      .where(eq(companySkills.key, "geetorusai/bundled/geetorus-operations/reflection-coach"));
    expect(skill).toMatchObject({
      key: "geetorusai/bundled/geetorus-operations/reflection-coach",
      slug: "reflection-coach",
    });
    expect(readGeetorusSkillSyncPreference(state.agent!.adapterConfig as Record<string, unknown>).desiredSkills).toContain(
  ...
  it("prevents direct marker add, remove, or mutation", async () => {
    ...
    await expect(agentService(db).update(builtIn.agentId!, {
      ...
      metadata: {
        note: "allowed",
        geetorusBuiltInAgent: { key: "briefs", featureKeys: ["briefs"] },
      },
    });
  });
  ...
  it("automatically materializes the Reflection Coach bundle without enabling background work", async () => {
    ...
    expect(readGeetorusSkillSyncPreference(state.agent!.adapterConfig).desiredSkills).toContain(skill!.key);
  ...
  it("materializes the Summarizer bundle paused on Claude Haiku with a disabled routine", async () => {
    ...
    const [skill] = await db
      .select()
      .from(companySkills)
      .where(eq(companySkills.key, "geetorusai/bundled/geetorus-operations/summarize-status"));
    expect(skill).toMatchObject({
