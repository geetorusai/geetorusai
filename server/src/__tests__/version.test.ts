...
describe("resolveServerVersion", () => {
  ...
  it("skips git metadata probing for packaged installs under node_modules", () => {
    ...
    expect(
      resolveServerVersion({
        ...
        debugLog,
        packageRoot: "/tmp/npm/_npx/example/node_modules/@geetorusai/server",
      }),
  ...
  it("uses git metadata for source checkouts whose path contains node_modules", () => {
    ...
    expect(
      resolveServerVersion({
        ...
        debugLog,
        gitDescribeCommand,
        packageRoot: "/tmp/node_modules/source/geetorus/server",
        pathExists: (path) => path === "/tmp/node_modules/source/geetorus/.git",
        realpath: (path) => path,
      }),
    ).toBe("2026.626.0+58.git.518fc71ce");
  ...
  it("keeps fallback diagnostics quiet by default", () => {
    const previousDebugFlag = process.env.GEETORUS_DEBUG_VERSION_RESOLUTION;
    delete process.env.GEETORUS_DEBUG_VERSION_RESOLUTION;
    ...
    try {
      ...
      if (previousDebugFlag === undefined) {
        ...
        process.env.GEETORUS_DEBUG_VERSION_RESOLUTION = previousDebugFlag;
      }
    }
  });
});
