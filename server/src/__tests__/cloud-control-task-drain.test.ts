...
const ENV = {
  GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS: JSON.stringify({ keys: [publicJwk] }),
  GEETORUS_CLOUD_STACK_ID: STACK_ID,
...
describe("verifyCloudControlAssertion", () => {
  ...
  it("rejects a runtime identity assertion replayed as a control assertion", () => {
    ...
    const payload = encodeJson({
      ...
      sub: STACK_ID,
      claimId: "claim-1",
      previousOrigin: "https://pool-1.staging.geetorus.app",
      canonicalOrigin: "https://gonzo.staging.geetorus.app",
      stackSlug: "gonzo",
      iat,
      exp: iat + 60,
    });
  ...
  it("rejects an assertion for another stack, and any assertion when the instance is self-hosted", () => {
    ...
    expect(() =>
      verifyCloudControlAssertion({
        ...
        env: { GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS: ENV.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS } as NodeJS.ProcessEnv,
        now: NOW,
      }),
...
describe("cloudControlMiddleware", () => {
  ...
  beforeEach(() => {
    resetCloudControlReplayFenceForTests();
    savedEnv.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS = process.env.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS;
    savedEnv.GEETORUS_CLOUD_STACK_ID = process.env.GEETORUS_CLOUD_STACK_ID;
    process.env.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS = ENV.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS;
