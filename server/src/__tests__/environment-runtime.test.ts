...
} from "@geetorusai/adapter-utils/ssh";
import {
  agents,
...
} from "@geetorusai/adapter-utils/acpx-engine/startup-timing";
...
describeEmbeddedPostgres("environmentRuntimeService", () => {
  ...
  it("resumes the same provider lease on the next per-turn run", async () => {
    ...
    const workspaceSyncStamp = {
      schema: "geetorus.native-workspace-stamp/v1",
  ...
  it("destroys a disposable geetorus_runner sandbox after the turn", async () => {
    const { pluginId, runId, reusableLease } = await seedReusablePluginSandboxLease();
    const workerManager = {
      isRunning: vi.fn((id: string) => id === pluginId),
      call: vi.fn(async (_pluginId: string, method: string) => {
  ...
  it("acquires and releases an SSH run lease through the runtime seam", async () => {
    ...
    const fixtureRoot = await mkdtemp(path.join(os.tmpdir(), "geetorus-environment-runtime-ssh-"));
  ...
  it("destroys the remote plugin sandbox when the lease insert rejects a foreign-company binding", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.fake-plugin-sandbox-provider",
      packageName: "@geetorusai/plugin-fake-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.fake-plugin-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("reports the plugin cleanup worker not ready while its worker is down, then ready after it recovers", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.fake-plugin-ready-sandbox-provider",
      packageName: "@geetorusai/plugin-fake-ready-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.fake-plugin-ready-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("reports the plugin cleanup provider not ready while the plugin is not ready, then ready after it recovers", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.fake-plugin-reload-sandbox-provider",
      packageName: "@geetorusai/plugin-fake-reload-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.fake-plugin-reload-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("reports the plugin cleanup provider not ready while the plugin is missing, then ready after it returns", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "geetorus.fake-plugin-missing-sandbox-provider",
      packageName: "@geetorusai/plugin-fake-missing-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "geetorus.fake-plugin-missing-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  const SECRET_REF_PLUGIN_KEY = "geetorus.secret-plugin-sandbox-provider";
  ...
  async function registerSecretRefPluginProvider(): Promise<string> {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@geetorusai/plugin-secret-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
  ...
  it("does not allocate a native-runner replacement without a verified backup", async () => {
    const seeded = await seedReusablePluginSandboxLease("geetorus_runner");
    const workerManager = {
      isRunning: vi.fn((id: string) => id === seeded.pluginId),
      call: vi.fn(async (_pluginId: string, method: string) => {
        if (method === "environmentResumeLease") {
  ...
  it("permits native-runner replacement only after verifying the stamped backup", async () => {
    const seeded = await seedReusablePluginSandboxLease("geetorus_runner");
    const backupBase = await mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-replacement-"));
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = backupBase;
    try {
  ...
  it("destroys reusable plugin-backed sandbox leases scoped to an environment", async () => {
    ...
    await expect(environmentService(db).getLeaseById(reusableLease.id)).resolves.toMatchObject({
      ...
      failureReason: "environment_deleted",
      cleanupStatus: "success",
      metadata: { remoteExecutionTermination: { schema: "geetorus.remote-termination.v1",
  ...
  it("does not reuse a sandbox lease owned by a different agent for the same execution workspace", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "acme.environments",
      packageName: "@acme/geetorus-environments",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
