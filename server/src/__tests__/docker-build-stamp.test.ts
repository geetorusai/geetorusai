 ...
 * of its stage. So the build stage must declare `ARG GEETORUS_BUILD_COMMIT`
...
it("keeps per-build runtime metadata out of the weekly CLI-install cache", () => {
  ...
  for (const name of ["GEETORUS_BUILD_VERSION", "GEETORUS_BUILD_COMMIT"]) {
...
describe("docker build-stamp wiring", () => {
  it("declares GEETORUS_BUILD_COMMIT in the build stage before the server build", () => {
    const build = stageBody(dockerfile, "build");
    const argIdx = build.search(/^ARG GEETORUS_BUILD_COMMIT\b/m);
    const serverBuildIdx = build.search(/^RUN pnpm --filter @geetorusai\/server build\b/m);
    expect(argIdx, "build stage must declare ARG GEETORUS_BUILD_COMMIT").toBeGreaterThanOrEqual(0);
    expect(serverBuildIdx, "build stage must run the server build").toBeGreaterThanOrEqual(0);
    expect(
      argIdx,
      "ARG GEETORUS_BUILD_COMMIT must precede the server build so the stamp script reads it",
    ).toBeLessThan(serverBuildIdx);
  });

  it("passes GEETORUS_BUILD_COMMIT as a build-arg for both image targets", () => {
    const argLines = [...`${workflow}\n${cloudWorkflow}`.matchAll(/^\s*GEETORUS_BUILD_COMMIT=.*$/gm)];
    expect(
      argLines.length,
      "the docker workflow must pass GEETORUS_BUILD_COMMIT for the production and cloud builds",
...
describe("Docker Rust dependency cache", () => {
  it("caches the locked dependency recipe separately from source and per-build metadata", () => {
    ...
    expect(planner).toContain("COPY packages/geetorus-runner/runner ./runner");
    ...
    expect(dependencies).toContain("cargo chef cook --release --locked --package geetorus-runner-core --bin geetorus-runnerd");
    expect(dependencies).not.toMatch(/COPY .*\.\/runner|COPY .*\.\/protocol|COPY \. \.|GEETORUS_BUILD_COMMIT/);
  });
  ...
  it("rebuilds real workspace code and embedded protocol inputs after cooking dependencies", () => {
    ...
    for (const source of ["runner", "protocol"]) {
      expect(native.indexOf(`COPY packages/geetorus-runner/${source} ./${source}`))
        .toBeLessThan(native.indexOf("cargo build --release"));
      expect(native).toContain(`COPY packages/geetorus-runner/${source} ./${source}`);
    }
    expect(native).toContain("cargo build --release --manifest-path runner/Cargo.toml --locked -p geetorus-runner-core --bin geetorus-runnerd");
