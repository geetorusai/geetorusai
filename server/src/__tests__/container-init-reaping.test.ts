...
describe("deployment manifest parity", () => {
  ...
  it("leaves ECS to inherit the image's init rather than enabling its own", () => {
    ...
    const server = ecsTaskDefinition.containerDefinitions.find((c) => c.name === "geetorus-server");
    expect(server, "the task definition must define a geetorus-server container").toBeDefined();
