...
import type { CatalogTeam } from "@geetorusai/shared";
...
const CORE_EXEC_TEAM_ID = "geetorusai:bundled:company-defaults:core-exec-team";
    ...
    metadata: {
      geetorus: {
        catalogTeam: {
          ...
          catalogKey: "geetorusai/bundled/company-defaults/core-exec-team",
...
describe("teamsCatalogService", () => {
  beforeEach(() => {
    ...
    mockCompanyPortabilityService.importBundle.mockResolvedValue({
      company: { id: "company-1", name: "Geetorus", action: "unchanged" },
      agents: [],
      projects: [],
      envInputs: [],
      warnings: [],
    ...
    mockCompanySkillService.installFromCatalog.mockResolvedValue({
      action: "created",
      skill: { key: "geetorusai/bundled/geetorus-operations/task-planning" },
      catalogSkill: { id: "geetorusai:bundled:geetorus-operations:task-planning" },
      warnings: [],
    });
  ...
  it("builds an inline portability source with catalog skill keys and target-manager reparenting", async () => {
    ...
    expect(prepared.source.files["agents/ceo/AGENTS.md"]).toEqual(expect.stringContaining("geetorusai/bundled/geetorus-operations/task-planning"));
    expect(prepared.source.files["agents/cto/AGENTS.md"]).toEqual(expect.stringContaining("geetorusai/bundled/software-development/github-pr-workflow"));
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentId: \"manager-1\""));
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentSlug: \"engineering-manager\""));
  });
  ...
  it("resolves target-manager slug against same-company agents before rendering reparent metadata", async () => {
    ...
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentId: \"manager-1\""));
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentSlug: \"ceo\""));
  });

  it("preserves package-declared Geetorus sidecar permissions while adding generated catalog provenance", async () => {
    const svc = teamsCatalogService({} as any);

    const prepared = await svc.prepareCatalogTeamSource("company-1", "product-engineering");

    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("permissions:"));
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("canCreateAgents: true"));
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("catalogTeam:"));
    expect(prepared.source.files[".geetorus.yaml"]).toEqual(expect.stringContaining("catalogSlug: \"product-engineering\""));
  });
  ...
  it("uses the configured safe adapter default for bundled agents", async () => {
    const previousDefault = process.env.GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE;
    process.env.GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE = "opencode_local";
    try {
      const svc = teamsCatalogService({} as any);
      ...
      if (previousDefault === undefined) {
        delete process.env.GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE;
      } else {
        process.env.GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE = previousDefault;
      }
    }
  });
  ...
  describe("readCatalogTeamProvenance", () => {
    it("reads catalogTeam provenance from agent metadata", () => {
      expect(
        readCatalogTeamProvenance({
          geetorus: { catalogTeam: { catalogId: "team-x", catalogKey: "k", originHash: "sha256:1" } },
        }),
    ...
    it("returns null when there is no catalogTeam provenance", () => {
      ...
      expect(readCatalogTeamProvenance({ geetorus: { catalog: { skillKey: "s" } } })).toBeNull();
      expect(readCatalogTeamProvenance({ geetorus: { catalogTeam: { originHash: "h" } } })).toBeNull();
    });
  });
      ...
      mockAgentService.list.mockResolvedValue([
        {
          id: "removed",
          companyId: "company-1",
          metadata: { geetorus: { catalogTeam: { catalogId: "geetorusai:bundled:gone:removed", originHash: "sha256:x" } } },
        },
      ]);
      const svc = teamsCatalogService({} as any);
  ...
  it("classifies unresolved and unsafe external skill requirements as blocked", () => {
    const fakeTeam: CatalogTeam = {
      id: "geetorusai:optional:test:unsafe",
      key: "geetorusai/optional/test/unsafe",
      kind: "optional",
      category: "test",
      slug: "unsafe",
      name: "Unsafe",
