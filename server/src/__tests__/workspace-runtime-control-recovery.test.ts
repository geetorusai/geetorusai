...
describeEmbeddedPostgres("managed runtime-control operation recovery", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("geetorus-runtime-control-recovery-");
    stopDb = started.cleanup;
    db = createDb(started.connectionString);
    logRoot = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-runtime-control-logs-"));
