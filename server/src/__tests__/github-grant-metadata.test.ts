...
describe("GitHub grant metadata", () => {
  it("lists every page of installations and repositories, persisting only display metadata", async () => {
    const request = vi.fn<typeof fetch>(async (input) => {
      ...
      if (url.pathname === "/user/installations") {
        return json({ installations: secondPage
          ...
          : [{ id: 101, repository_selection: "selected", html_url: "https://github.com/settings/installations/101", account: { login: "geetorusai" } }],
        }, !secondPage);
      }
      if (url.pathname === "/user/installations/101/repositories") {
        return json({ total_count: 2, repositories: secondPage
          ? [{ id: 2, full_name: "geetorusai/b", private: true, description: "must-not-persist", clone_url: "must-not-persist" }]
          : [{ id: 1, full_name: "geetorusai/a", private: false }],
        }, !secondPage);
      }
    ...
    const metadata = await loadGitHubGrantMetadata("ghu_secret", request, "geetorus-development");
    expect(metadata).toMatchObject({
      ...
      installationOwnerLogins: ["geetorusai", "octocat"],
      repositories: [
        { id: "3", fullName: "octocat/c", installationId: "102" },
        { id: "1", fullName: "geetorusai/a", installationId: "101", private: false },
        { id: "2", fullName: "geetorusai/b", installationId: "101", private: true },
      ],
      installationUrl: "https://github.com/apps/geetorus-development/installations/new",
  ...
  it("recovers a legacy grant's app chooser from GitHub installation metadata", async () => {
    const request = vi.fn<typeof fetch>()
      ...
      .mockResolvedValueOnce(json({ installations: [{ id: 101, app_slug: "geetorus-staging", repository_selection: "selected" }] }))
    ...
    await expect(loadGitHubGrantMetadata("ghu_secret", request)).resolves.toMatchObject({
      appSlug: "geetorus-staging",
      installationUrl: "https://github.com/apps/geetorus-staging/installations/new",
    });
  });
