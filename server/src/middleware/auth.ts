...
import { isUuidLike, normalizeAgentApiKeyScope, type DeploymentMode } from "@geetorusai/shared";
...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    const runIdHeader = req.header("x-geetorus-run-id");
    ...
    if (!key) {
      ...
      if (normalizedRunIdHeader && normalizedRunIdHeader !== claims.run_id) {
        ...
        next(
          unprocessable("X-Geetorus-Run-Id does not match signed agent JWT run_id", {
...
async function resolveCloudTenantActorOnce(
  ...
  const expectedToken = process.env.GEETORUS_CLOUD_TENANT_SERVER_TOKEN?.trim();
  if (!expectedToken) return null;

  const token = req.header("x-geetorus-cloud-tenant-token")?.trim();
  if (!token || !constantTimeStringEqual(token, expectedToken)) return null;

  const userId = requiredCloudHeader(req, "x-geetorus-cloud-user-id");
  const userEmail = requiredCloudHeader(req, "x-geetorus-cloud-user-email").toLowerCase();
  const stackId = requiredCloudHeader(req, "x-geetorus-cloud-stack-id");
  const stackRole = stackMembershipRole(req.header("x-geetorus-cloud-stack-role"));
  const userName = req.header("x-geetorus-cloud-user-name")?.trim() || userEmail;
  const geetorusCompanyId = req.header("x-geetorus-cloud-geetorus-company-id")?.trim();
  const geetorusCompanyName = req
    .header("x-geetorus-cloud-geetorus-company-name")
    ?.trim();
  const companyId = cloudTenantCompanyId(stackId);
  const companyName = geetorusCompanyName || humanizeCloudStackSlug(stackId);
  const now = new Date();
  const membershipRole = stackRole === "owner" || stackRole === "admin" ? "owner" : stackRole;
  const syncFingerprint = [userEmail, userName, stackId, stackRole, geetorusCompanyId ?? ""].join(":");
  ...
  if (shouldSync && geetorusCompanyName) {
...
function cloudTenantCompanyId(stackId: string): string {
  const bytes = createHash("sha256").update(`geetorus-cloud-tenant-company:${stackId}`).digest();
...
export function isKnownBadCloudCompanyName(
  name: string,
  ids: { companyId: string; geetorusCompanyId?: string },
): boolean {
  const normalized = name.trim();
  return (
    /^geetorus-stack-.+/i.test(normalized) ||
    /^stack-.+\s+geetorus$/i.test(normalized) ||
    normalized === ids.companyId ||
    (ids.geetorusCompanyId !== undefined &&
      normalized === ids.geetorusCompanyId)
  );
}
...
async function repairCloudTenantCompanyName(
  ...
  input: {
    companyId: string;
    geetorusCompanyId?: string;
    geetorusCompanyName: string;
    now: Date;
  },
): Promise<void> {
  try {
    ...
    if (
      ...
      !isKnownBadCloudCompanyName(existing.name, {
        companyId: input.companyId,
        geetorusCompanyId: input.geetorusCompanyId,
      })
    ) {
      return;
    }
    await db.transaction(async (tx) => {
      const [updated] = await tx
        .update(companies)
        .set({ name: input.geetorusCompanyName, updatedAt: input.now })
        .where(
          and(
      ...
      await tx.insert(activityLog).values({
        ...
        details: {
          ...
          name: input.geetorusCompanyName,
        },
      });
    });
  } catch (err) {
...
const LEGACY_PROVISIONED_DESCRIPTION_PREFIX = "Provisioned by Geetorus Cloud for stack ";
