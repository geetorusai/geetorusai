...
import { trackErrorHandlerCrash } from "@geetorusai/shared/telemetry";
...
function getGeetorusDb(req: Request): Db | null {
  const locals = req.app?.locals as { geetorusDb?: Db; db?: Db } | undefined;
  return locals?.geetorusDb ?? locals?.db ?? null;
}
