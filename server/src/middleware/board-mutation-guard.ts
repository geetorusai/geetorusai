...
function trustedOriginsForRequest(req: Request) {
  ...
  // explicitly-configured GEETORUS_PUBLIC_URL when it's set.
  const publicUrl = parseOrigin(process.env.GEETORUS_PUBLIC_URL?.trim());
  if (publicUrl) origins.add(publicUrl);
  return origins;
}

/**
 * Return the browser origin only when it is the same origin Geetorus's CSRF
