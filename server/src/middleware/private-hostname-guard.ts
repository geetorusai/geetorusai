...
const BLOCKED_HOSTNAME_MESSAGE =
  "This hostname is not allowed for this Geetorus instance. " +
  "If you want to allow a hostname, run npx geetorusai allowed-hostname <host>.";
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. If you want to allow a hostname, run npx geetorusai allowed-hostname <host>.";
      if (wantsJson) {
