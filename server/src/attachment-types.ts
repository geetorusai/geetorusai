 ...
 * `GEETORUS_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set for routes that use this allowlist.
 *
 * Examples:
 *   GEETORUS_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   GEETORUS_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
...
export function normalizeContentType(contentType: string | null | undefined): string {
  ...
  // example Discord uses `text/plain; charset=utf-8`) while Geetorus's
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.GEETORUS_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.GEETORUS_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
