...
if (legacyFallbackUsed) {
  ...
  console.warn(
    "[geetorus] SENTRY_DSN_FRONTEND or SENTRY_DSN_BACKEND is not set. " +
...
export function captureException(error: unknown): void {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error("[geetorus] Sentry captureException failed", err);
  }
}
...
export function captureRunFailure(event: RunFailureEvent): void {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error("[geetorus] Sentry captureRunFailure failed", err);
  }
}
...
export function shutdownSentry(): Promise<void> {
  shutdownPromise ??= (async () => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[geetorus] Sentry shutdown failed", err);
    }
  })();
...
async function bootstrapSentry(dsn: string): Promise<void> {
  ...
  if (!versionCheck.ok) {
    // eslint-disable-next-line no-console
    console.warn(
      "[geetorus] The backend Sentry DSN is set, but the @sentry/node " +
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[geetorus] The backend Sentry DSN is set, and @sentry/node passed " +
