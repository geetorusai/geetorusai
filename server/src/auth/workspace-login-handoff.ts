...
const HANDOFF_KEY_ENV = "GEETORUS_WORKSPACE_HANDOFF_KEY";
const HANDOFF_ROOT_SECRET_ENV = "GEETORUS_WORKSPACE_HANDOFF_SECRET";
const READINESS_TOKEN_ENV = "GEETORUS_WORKSPACE_READINESS_TOKEN";
const EXECUTION_WORKSPACE_ID_ENV = "GEETORUS_EXECUTION_WORKSPACE_ID";
const EXECUTION_WORKSPACE_COMPANY_ID_ENV = "GEETORUS_EXECUTION_WORKSPACE_COMPANY_ID";
...
export const WORKSPACE_READINESS_TOKEN_HEADER = "x-geetorus-workspace-readiness-token";
export const WORKSPACE_READINESS_USER_ID_HEADER = "x-geetorus-workspace-readiness-user-id";
export const WORKSPACE_READINESS_USER_EMAIL_HEADER = "x-geetorus-workspace-readiness-user-email";
...
export function deriveWorkspaceHandoffKey(input: {
  ...
  return createHmac("sha256", input.rootSecret)
    .update(`geetorus.workspace-login-handoff.${WORKSPACE_HANDOFF_TICKET_VERSION}\n`)
...
export function deriveWorkspaceReadinessToken(input: {
  ...
  return createHmac("sha256", input.rootSecret)
    .update(`geetorus.workspace-readiness-probe.${WORKSPACE_HANDOFF_TICKET_VERSION}\n`)
    .update(`${input.instanceId}\n`)
    .update(`${input.executionWorkspaceId}`)
    .digest("hex");
}
 ...
 *
 * A dedicated `GEETORUS_WORKSPACE_HANDOFF_SECRET` is preferred. When it is
...
export function resolveWorkspaceHandoffRootSecret(
  ...
  const fallback = env.BETTER_AUTH_SECRET?.trim() || env.GEETORUS_AGENT_JWT_SECRET?.trim();
  if (!fallback) return null;
  return {
    secret: createHmac("sha256", fallback)
      .update(`geetorus.workspace-login-handoff.root.${WORKSPACE_HANDOFF_TICKET_VERSION}`)
      .digest("hex"),
    source: "derived",
  };
}
...
export function workspaceHandoffKeyFingerprint(key: string): string {
  return createHmac("sha256", "geetorus.workspace-login-handoff.fingerprint").update(key).digest("hex").slice(0, 12);
}
