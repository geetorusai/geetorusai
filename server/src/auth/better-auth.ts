...
export function resolveWorkspaceHandoffIdentity(
  ...
  const configuredOrigin =
    normalizeWorkspaceHandoffOrigin(env.GEETORUS_PUBLIC_URL)
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins: string[]): BetterAuthInstance {
  const baseUrl = config.authBaseUrlMode === "explicit" ? config.authPublicBaseUrl : undefined;
  const publicUrl = process.env.GEETORUS_PUBLIC_URL?.trim() || baseUrl;
  const managedRuntimePublicUrl = process.env.GEETORUS_MANAGED_RUNTIME_PUBLIC_URL?.trim() || undefined;
  const secret = process.env.BETTER_AUTH_SECRET ?? process.env.GEETORUS_AGENT_JWT_SECRET;
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or GEETORUS_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=geetorus-dev-secret in your .env file.",
    );
  }
  ...
  const authConfig = {
    ...
    rateLimit: buildBetterAuthRateLimitOptions({
      deploymentMode: config.deploymentMode,
      deploymentExposure: config.deploymentExposure,
      override: process.env.GEETORUS_AUTH_RATE_LIMIT_ENABLED,
    }),
