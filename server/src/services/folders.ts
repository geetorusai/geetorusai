...
import { companySkills, folders, routines } from "@geetorusai/db";
import type {
  CreateFolder,
  Folder,
  FolderKind,
...
export function folderService(db: Db, mutationLockHeld = false) {
  async function withCompanyFolderLock<T>(companyId: string, operation: (lockedDb: Db) => Promise<T>) {
    ...
    return db.transaction(async (tx) => {
      await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`geetorus:folders:${companyId}`}, 0))`);
