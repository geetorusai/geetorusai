...
export function parseAdapterModelsEnv(
  ...
  const raw = env.GEETORUS_ADAPTER_MODELS?.trim();
  if (!raw) return null;
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (e) {
    throw new Error(`GEETORUS_ADAPTER_MODELS must be valid JSON: ${(e as Error).message}`);
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("GEETORUS_ADAPTER_MODELS must be a JSON object mapping adapterType to an array of {id,label}");
  }
  const out: Record<string, AdapterModelEntry[]> = {};
  for (const [type, list] of Object.entries(parsed as Record<string, unknown>)) {
    if (!Array.isArray(list)) {
      throw new Error(`GEETORUS_ADAPTER_MODELS[${type}] must be an array`);
    }
    out[type] = list.map((m) => {
      const o = m as Record<string, unknown>;
      if (typeof o.id !== "string" || !o.id) {
        throw new Error(`GEETORUS_ADAPTER_MODELS[${type}] entries require a non-empty string id`);
      }
