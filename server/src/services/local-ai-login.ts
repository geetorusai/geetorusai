...
import { adapterAuthSessions, ADAPTER_AUTH_SESSION_ACTIVE_STATES, environments, type Db } from "@geetorusai/db";
import type { AiConnectionLoginIntent, LocalAiLoginAttempt, LocalAiLoginStatus } from "@geetorusai/shared";
...
function loginHome(id: string) {
  return path.join(resolveGeetorusInstanceRoot(), "ai-local-logins", id);
}
