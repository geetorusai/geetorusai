...
describe("external Geetorus task links", () => {
  ...
  it("does not use webhook ingress as the question or confirmation destination", () => {
    for (const name of [
      ...
      "GEETORUS_MANAGED_RUNTIME_PUBLIC_URL",
    ])
      vi.stubEnv(name, "");
    vi.stubEnv("GEETORUS_CHAT_WEBHOOK_PUBLIC_URL", "https://ingress.example");
    expect(publicChatInteractionTaskUrl("issue-1")).toBeNull();
    vi.stubEnv("GEETORUS_PUBLIC_URL", "http://127.0.0.1:3103");
    expect(publicChatInteractionTaskUrl("issue-1")).toBeNull();
    vi.stubEnv("GEETORUS_PUBLIC_URL", "https://board.example");
    expect(publicChatInteractionTaskUrl("issue-1")).toBe(
      "https://board.example/issues/issue-1",
    );
    vi.stubEnv("GEETORUS_AUTH_PUBLIC_BASE_URL", "https://canonical.example");
    expect(publicChatInteractionTaskUrl("issue-1")).toBe(
  ...
  it("uses the configured board origin ahead of the managed-runtime fallback", () => {
    ...
    vi.stubEnv("GEETORUS_PUBLIC_URL", "https://environment.example");
    expect(publicChatInteractionTaskUrl("issue-1")).toBe(
