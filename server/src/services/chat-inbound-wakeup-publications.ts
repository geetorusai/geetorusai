import type { agentWakeupRequests, chatActions } from "@geetorusai/db";
import {
...
export function inboundWakePublicationText(
  ...
  return state === "queued"
    ...
    : "This follow-up was not started. Open the task in Geetorus for details.";
}
...
function hasComment(
  ...
  const context = payload?._geetorusWakeContext;
  const nested =
