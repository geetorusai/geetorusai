...
import { connectionIntentDeliveries } from "@geetorusai/db";
...
} from "@geetorusai/shared/telemetry";
import type {
...
function parseStoredInteractionResult<S extends z.ZodTypeAny>(
  ...
  console.warn(
    `[geetorus] Dropping unparseable ${row.kind} interaction result for interaction ${row.id}`,
    parsed.error.issues,
  );
  return null;
}
  ...
  try {
    ...
    if (!args?.creatorRoleByAgentId) {
      try {
        ...
        console.error(
          "[geetorus] Failed to load interaction.resolved creator role",
          error,
        );
      }
    }
    ...
    console.error(
      "[geetorus] Failed to emit interaction.resolved telemetry",
      error,
    );
  }
}
...
function emitInteractionCreatedTelemetry(args: {
  ...
  try {
    ...
    console.error(
      "[geetorus] Failed to emit interaction.created telemetry",
      error,
    );
  }
}
...
async function emitResolvedInteractionsTelemetry(
  ...
  try {
    ...
    console.error(
      "[geetorus] Failed to load interaction.resolved creator roles",
      error,
    );
  }
  await Promise.all(
