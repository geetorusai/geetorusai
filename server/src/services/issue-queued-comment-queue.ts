...
import type { IssueComment, IssueQueuedCommentQueue } from "@geetorusai/shared";

const QUEUE_CONTEXT_KEY = "_geetorusWakeContext";
...
export function withQueuedCommentIdsInRunContext(
  ...
  delete context.geetorusWake;
  delete context.geetorusWakeComment;
  delete context.geetorusTaskMarkdown;
  delete context.geetorusTaskMarkdownCompact;
  return context;
}

export type QueuedCommentQueueProtocol = "geetorus_runner_v1" | "legacy";
...
export type QueuedCommentQueueSteeringDecision =
  ...
  | { protocol: "geetorus_runner_v1"; kind: "probe"; steeringRunId: string };
 ...
 * `geetorus_runner_v1`/`legacy` protocol and the steering answer; every
...
export function decideQueuedCommentQueueSteering(facts: {
  ...
  const protocol: QueuedCommentQueueProtocol =
    persistedRuntimeMode === "native"
      || (persistedRuntimeMode === null && facts.assignedAgentAdapterType === "geetorus_runner")
      ? "geetorus_runner_v1"
      : "legacy";

  if (protocol !== "geetorus_runner_v1") {
