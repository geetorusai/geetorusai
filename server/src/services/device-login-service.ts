...
import { adapterAuthSessions } from "@geetorusai/db";
import type {
...
import { toPublicAdapterAuthSessionStatus } from "@geetorusai/shared";
import {
...
import type { AdapterLoginPrompt } from "@geetorusai/adapter-utils";
import {
...
export function adapterLoginPromotionLockKey(
  ...
  return `geetorus:adapter-login-promotion:${companyId}:${startedByUserId}:${adapterType}`;
}
...
export function sessionLoginHomePath(sessionId: string): string {
  return `/tmp/geetorus-adapter-login/${sessionId}`;
}
...
export function createWorkerBoundLoginPtyOpener(
  ...
  return async (binding) => {
    ...
    if (!binding.providerLeaseId || !pluginId || !driverKey) {
      log("[geetorus] Device login: the lease carries no sandbox worker binding.");
    ...
    try {
      ...
      log("[geetorus] Device login: the adapter type has no login command key.");
      throw new Error(CODEX_LOGIN_PTY_BIND_FAILED);
    }
