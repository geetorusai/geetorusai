...
describe("parseExecutionPolicyBootstrapEnv", () => {
  ...
  it("returns null when execution mode is explicitly any", () => {
    expect(
      parseExecutionPolicyBootstrapEnv(env({ GEETORUS_EXECUTION_MODE: "any" })),
    ).toBeNull();
  });
  ...
  it("parses the forced kubernetes policy with a job/gvisor/cilium config", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(
      env({
        GEETORUS_EXECUTION_MODE: "kubernetes",
        GEETORUS_K8S_BACKEND: "job",
        GEETORUS_K8S_IN_CLUSTER: "true",
        GEETORUS_K8S_RUNTIME_CLASS_NAME: "gvisor",
        GEETORUS_K8S_EGRESS_MODE: "cilium",
        GEETORUS_K8S_EGRESS_ALLOW_FQDNS: "api.anthropic.com, api.openai.com",
        GEETORUS_K8S_EGRESS_ALLOW_CIDRS: "10.0.0.0/8",
      }),
    );
  ...
  it("defaults inCluster false and omits unset optional fields", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(
      env({ GEETORUS_EXECUTION_MODE: "kubernetes" }),
    );
  ...
  it("throws on an unknown execution mode", () => {
    expect(() =>
      parseExecutionPolicyBootstrapEnv(env({ GEETORUS_EXECUTION_MODE: "vm" })),
    ).toThrow(/GEETORUS_EXECUTION_MODE/);
  });
  ...
  it("attaches the declared adapter registry to the kubernetes config", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(
      env({
        GEETORUS_EXECUTION_MODE: "kubernetes",
        GEETORUS_ADAPTERS: JSON.stringify([
  ...
  it("leaves adapters undefined when GEETORUS_ADAPTERS is absent", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(env({ GEETORUS_EXECUTION_MODE: "kubernetes" }));
    expect(parsed?.kubernetesConfig.adapters).toBeUndefined();
  });

  it("reads GEETORUS_K8S_RPC_TIMEOUT_MS into kubernetesConfig.timeoutMs", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(
      env({
        GEETORUS_EXECUTION_MODE: "kubernetes",
        GEETORUS_K8S_RPC_TIMEOUT_MS: "600000",
      }),
    );
    expect(parsed?.kubernetesConfig.timeoutMs).toBe(600000);
  });

  it("omits timeoutMs when GEETORUS_K8S_RPC_TIMEOUT_MS is absent", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(env({ GEETORUS_EXECUTION_MODE: "kubernetes" }));
    expect(parsed?.kubernetesConfig.timeoutMs).toBeUndefined();
  });

  it("throws when GEETORUS_K8S_RPC_TIMEOUT_MS is not a positive integer", () => {
    expect(() =>
      parseExecutionPolicyBootstrapEnv(
        env({ GEETORUS_EXECUTION_MODE: "kubernetes", GEETORUS_K8S_RPC_TIMEOUT_MS: "0" }),
      ),
    ).toThrow(/GEETORUS_K8S_RPC_TIMEOUT_MS/);
    expect(() =>
      parseExecutionPolicyBootstrapEnv(
        env({ GEETORUS_EXECUTION_MODE: "kubernetes", GEETORUS_K8S_RPC_TIMEOUT_MS: "abc" }),
      ),
    ).toThrow(/GEETORUS_K8S_RPC_TIMEOUT_MS/);
  });
});
