...
import { authUsers, companies, companyMemberships, issues } from "@geetorusai/db";
import type {
  WorkspaceReadiness,
 ...
 *
 * `GEETORUS_CONFIG` is the authoritative pointer a seeded worktree is started
...
export function resolveWorkspaceSeedMarkerDir(env: NodeJS.ProcessEnv = process.env): string {
  const configPath = env.GEETORUS_CONFIG?.trim();
  if (configPath) return path.dirname(path.resolve(configPath));
  return path.resolve(process.cwd(), ".geetorus");
}
