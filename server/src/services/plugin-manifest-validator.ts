 ...
 */
import { pluginManifestV1Schema } from "@geetorusai/shared";
import type { GeetorusPluginManifestV1 } from "@geetorusai/shared";
import { PLUGIN_API_VERSION } from "@geetorusai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): GeetorusPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as GeetorusPluginManifestV1,
        };
      }
