...
function serviceCommand() {
  // Answers `/api/health` the way a real Geetorus dev runtime does: managed
 ...
 * argv and **ignores `GEETORUS_BIND` / `GEETORUS_MANAGED_RUNTIME_EXPOSURE`
...
function startInput(options?: {
  ...
  return {
    ...
    actor: { id: null, name: "Geetorus", companyId: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee" },
    issue: null,
    workspace: {
      baseCwd: process.cwd(),
...
describe("automatic tailscale_https default for managed worktree runtimes", () => {
  it("defaults a legacy geetorus-dev service with no exposure block, relocating its pinned port", async () => {
    const { broker, calls } = createBroker();
    installDeps({ broker });
  ...
  it("preserves a deliberate opt-out and leaves the service on plain HTTP", async () => {
    ...
    expect(runtime.url).toBe(`http://geetorus-dev:${runtime.port}`);
  }, 15_000);
  ...
  it("still honors an explicit opt-in when the broker socket is missing, failing loudly", async () => {
    ...
    await expect(
      startRuntimeServicesForWorkspaceControl(startInput({ serviceName: "geetorus-dev" })),
    ).rejects.toThrow();
...
describe("loopback bind is forced on the guest, not merely requested (PAP-17256)", () => {
  ...
  it("leaves a non-Geetorus service's --bind argument alone", async () => {
  ...
  it("does not rewrite a Geetorus dev command when the service is not exposed", async () => {
    const { broker, calls } = createBroker();
    installDeps({ broker });
