...
describe("chat publication projection", () => {
  ...
  it("redacts common credentials and private connection material", () => {
    ...
    const input = [
      ...
      `key ${openAiKeyCanary}`,
      "database postgresql://geetorus:hunter2@example.com/db",
      "-----BEGIN PRIVATE KEY-----",
      "definitely-private",
      "-----END PRIVATE KEY-----",
    ].join("\n");
        ...
        card: {
          ...
          actions: [
            { type: "callback", actionId: "choice.one", label: "First", style: "primary" },
            {
              type: "link",
              label: "Open Geetorus",
              url: "https://geetorus.example/tasks/123?handoff=secret#private",
            },
      ...
      card: {
        ...
        actions: [
          { type: "callback", actionId: "choice.one", label: "First", style: "primary" },
          {
            type: "link",
            label: "Open Geetorus",
            url: "https://geetorus.example/tasks/123",
          },
        ],
      },
    });
