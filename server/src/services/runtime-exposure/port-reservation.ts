 ...
 * Geetorus went on reporting the leased lane `stopped` and its exposure
 * `removed`.
 *
 ...
 */
import { deriveViteHmrPort, isRuntimeExposureAppPort } from "@geetorusai/shared";
...
export type ExposureReservationSource =
  ...
  | "in_memory_runtime"
  /** The broker reports a live Geetorus-owned Serve mapping on the port. */
  | "broker_mapping"
...
/** One Geetorus-owned listener as reported by the broker's `list()`. */
...
export interface ExposureReservationDrift {
  /** The row Geetorus believes is stopped/removed. */
  runtimeServiceId: string;
  owner: ExposureOwnerIdentity;
  port: number;
