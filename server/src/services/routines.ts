...
import { trackRoutineRun } from "@geetorusai/shared/telemetry";
...
export function routineWebhookUrl(publicId: string): string {
  const baseUrl = runtimePublicOrigin() ?? process.env.GEETORUS_API_URL?.trim();
  if (!baseUrl) throw new Error("GEETORUS_API_URL is required to create a routine webhook");
...
function resolveRoutineVariableValues(
  ...
  for (const variable of variables) {
    ...
    // Geetorus manages from execution context, so callers cannot override them.
...
export function routineService(
  ...
  async function getAutomaticRoutineDispatchEligibility(
    ...
    if (!isTruthyRuntimeEnvValue(runtimeEnv.GEETORUS_IN_WORKTREE)) return { eligible: true };
  ...
  return {
    ...
    create: async (companyId: string, input: CreateRoutine, actor: Actor): Promise<Routine> => {
      ...
      const env = input.env === undefined || input.env === null
        ...
        : await secretsSvc.normalizeEnvBindingsForPersistence(companyId, input.env, {
            strictMode: process.env.GEETORUS_SECRETS_STRICT_MODE === "true",
            fieldPath: "env",
          });
    ...
    firePublicTrigger: async (publicId: string, input: {
      ...
      if (trigger.signingMode === "none") {
        ...
        // generic X-Geetorus-Signature header so operators can use github_hmac
    ...
    tickScheduledTriggers: async (now: Date = new Date()) => {
      const worktreeActivation = isTruthyRuntimeEnvValue(runtimeEnv.GEETORUS_IN_WORKTREE)
