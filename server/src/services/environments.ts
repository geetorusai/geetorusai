...
const DEFAULT_LOCAL_ENVIRONMENT_DESCRIPTION =
  "Default execution environment for Geetorus runs on this machine.";
...
export interface KubernetesEnvironmentConfigInput {
   ...
   */
  timeoutMs?: number;
  adapters?: import("@geetorusai/shared").AdapterRegistryEntry[];
  [key: string]: unknown;
}
...
export interface ManagedSandboxEnvironmentInput {
   ...
   * this row (currently only true for the GEETORUS_MANAGED_CONFIG applier,
...
const MANAGED_ENVIRONMENT_ARCHIVE_TOKEN_METADATA_KEY = "_geetorusManagedArchiveToken";
const MANAGED_ENVIRONMENT_ARCHIVE_TOKEN_DEFAULTS_KEY = "_geetorusManagedArchiveToken";
...
export function environmentService(db: Db) {
  /**
   * Idempotently ensure THE Geetorus-managed sandbox environment for this
    ...
    const reconciliation = await db.transaction(
      async (tx): Promise<ManagedSandboxEnvironmentReconcileResult> => {
        ...
        let row = sandboxRows.find(
          (candidate) => (candidate.metadata as Record<string, unknown> | null)?.managedByGeetorus === true,
        ...
        // `platformFullyManaged` callers (currently: the GEETORUS_MANAGED_CONFIG
        ...
        //   row that was never Geetorus-managed (the fallback lookup above).
        ...
        //   Geetorus's own provider-unavailability archival, so that path
        ...
        if (stockStatus === "operator_modified") {
          ...
          // an operator edit. If the binding records that Geetorus archived
  ...
  /**
   * Archive the Geetorus-managed sandbox row when its provider became
  ...
  const archiveManagedSandboxEnvironment = async (
    ...
    const archived = await db.transaction(async (tx) => {
      const existing = await tx
        ...
        .then(
          (rows) => rows.find(
            (row) => (row.metadata as Record<string, unknown> | null)?.managedByGeetorus === true,
          ) ?? null,
        );
      ...
      // Archival is a Geetorus-owned availability transition. Record only
  ...
  return {
     ...
     * stamped `managedByGeetorus: true` (other metadata preserved) — so the
    ...
    ensureLocalEnvironment: async (_companyId?: string): Promise<Environment> => {
      ...
      if (isCloudManagedInstance() && existingMetadata.managedByGeetorus !== true) {
        const adopted = await db
          .update(environments)
          .set({
            metadata: { ...existingMetadata, managedByGeetorus: true },
            updatedAt: new Date(),
          })
     ...
     * `managedByGeetorus`-marked slot row), if one exists. Read-only
