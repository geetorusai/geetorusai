 ...
 *
 * One source (`GEETORUS_ADAPTERS` inline JSON, or `GEETORUS_ADAPTERS_FILE` a
...
import { adapterRegistrySchema, type AdapterRegistryEntryParsed } from "@geetorusai/shared";
...
export function parseAdapterRegistryEnv(
  ...
  const inline = env.GEETORUS_ADAPTERS?.trim();
  const filePath = env.GEETORUS_ADAPTERS_FILE?.trim();
  ...
  if (inline) {
    ...
    try {
      ...
      throw new Error(
        `GEETORUS_ADAPTERS_FILE could not be read at "${filePath}": ${(err as Error).message}`,
      );
    }
  }
  ...
  try {
    ...
    throw new Error(`GEETORUS_ADAPTERS must be valid JSON: ${(err as Error).message}`);
  }

  const result = adapterRegistrySchema.safeParse(parsed);
  if (!result.success) {
    throw new Error(
      `GEETORUS_ADAPTERS failed validation: ${result.error.issues
...
export function reconcileAdapterAvailability(
  ...
  if (missing.length > 0) {
    throw new Error(
      `GEETORUS_ADAPTERS declares adapter type(s) with no installed adapter: ${missing.join(", ")}`,
    );
  }
  ...
  logger.info({ enabled, disabled }, "reconciled adapter availability from GEETORUS_ADAPTERS");
