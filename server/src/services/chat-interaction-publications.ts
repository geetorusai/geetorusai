 ...
 * connection, or tool side effect stay in Geetorus, where the complete
...
function textForQuestionInteraction(
  ...
  lines.push(
    taskUrl
      ? `Open the task in Geetorus to respond: ${taskUrl}`
      : "Open the task in Geetorus to respond.",
  );
  return lines.join("\n");
}
...
function genericInteractionText(taskUrl: string | null): string {
  return [
    "This task needs an authorized response in Geetorus.",
    taskUrl
      ? `Open the task in Geetorus to respond: ${taskUrl}`
      : "Open the task in Geetorus to respond.",
  ]
...
export async function enqueueIssueInteractionChatPublications(
  ...
  // terminal semantics that are authoritative in Geetorus; projecting a
          ...
          title:
            interaction.kind === "ask_user_questions"
              ...
              : interaction.kind === "request_confirmation"
                ...
                : "Response needed in Geetorus",
          body:
            interaction.kind === "ask_user_questions"
              ? (question?.helpText ?? undefined)
              : interaction.kind === "request_confirmation"
                ? (interaction.payload.detailsMarkdown ?? undefined)
                : "Open the task in Geetorus to review and respond.",
          actions,
        },
      },
    });
 ...
 * providers with buttons, so an "Open in Geetorus" prompt never remains
 ...
 * Geetorus UI wins, all still-issued callback tokens expire in this same
