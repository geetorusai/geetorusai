...
> {
  ...
  // Geetorus comment even if a fast provider answer resolves the interaction
...
export function safeMilestoneText(input: {
  ...
  const recovery =
    input.milestone === "waiting_for_input"
      ? `${input.agentName} needs a Geetorus admin to safely recover this turn before more work can start.`
      : input.errorCode === "low_trust_isolation_unavailable"
        ? `${input.agentName} couldn't safely start this turn because this task was started for an unlinked external guest and isolated guest execution isn't available. Ask a Geetorus admin to create a private identity link for this account or enable isolated guest execution, then start a new task.`
        : input.errorCode === "native_provider_usage_limit"
          ? `${input.agentName} couldn't complete this turn because the model provider's usage allowance is exhausted. A Geetorus admin needs to restore capacity before retrying.`
          : input.errorCode === "native_event_replay_conflict"
            ? `${input.agentName} couldn't safely continue this turn. A Geetorus admin needs to review the run before it can be retried.`
            : input.errorCode === "native_session_cleanup_quarantined"
              ? `${input.agentName} couldn't start this turn because an earlier session needs recovery. Your request is saved. Ask a Geetorus admin to recover that session before retrying; sending the request again won't repair it.`
              : `${input.agentName} stopped before completing this turn.`;
  return `${recovery}${
    taskUrl
      ? ` Open the task in Geetorus: ${taskUrl}`
      : " Open the task in Geetorus for details."
  }`;
}
 ...
 * and time; native messages and payloads stay inside Geetorus.
 */
        ...
        const explicitlyAuthoredFinal = await tx
          ...
          .where(
            and(
              ...
              sql`(
                ${issueComments.metadata} ->> 'authorizationReason' = 'geetorus_runner_protocol'
 ...
 * output, errors, tool events, and reasoning stay in Geetorus. Idempotency is
