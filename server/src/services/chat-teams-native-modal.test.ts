...
// Chat SDK into Geetorus's callbacks. Only the instance JWT check is replaced;
...
describe("Teams native task module adapter-to-runtime boundary", () => {
  ...
  async function harness(
    ...
    const dispatch = async (
      ...
      const response = await runtime.handleWebhook(
        new Request(
          "https://geetorus.test/api/chat-webhooks/synthetic/microsoft-teams",
          {
            method: "POST",
            headers: {
  ...
  it("round-trips the Geetorus form through task/fetch and task/submit with exact opaque fields and source", async () => {
