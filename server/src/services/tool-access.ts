import { connectionPurposeTransportSchema } from "@geetorusai/shared";
...
import type { Db } from "@geetorusai/db";
import {
  agents,
...
import type { ConfigureRailwaySsh, RailwaySshSetup } from "@geetorusai/shared";
...
import {
  appWithGeetorusCloudConnectorAvailability,
  geetorusCloudConnectorCapabilitiesFromEnv,
  createGeetorusCloudConnector,
  isGeetorusCloudConnectorStrategy,
  geetorusCloudConnectorConfigFromEnv,
  GeetorusCloudConnectorError,
  type GeetorusCloudConnector,
 ...
 * `error_description`, `error_uri`, and the response body. Geetorus surfaces
 ...
 * key here") into Geetorus's own voice.
 *
 * The rule is therefore: the operator only ever reads text Geetorus authored.
...
/**
 * Stable, Geetorus-authored operator copy for each allowlisted provider error.
 * Deliberately keyed on the code alone: the calling context is already carried
 * by the Geetorus `code` in `details`, so one table serves the callback,
...
const OAUTH_PROVIDER_ERROR_MESSAGES: Record<string, string> = {
  ...
  invalid_client: "The authorization server rejected Geetorus's OAuth client.",
  invalid_client_metadata:
    "The authorization server rejected Geetorus's client registration details.",
  invalid_grant:
    "The authorization server rejected the authorization code or refresh token.",
  invalid_redirect_uri:
    "The authorization server rejected Geetorus's callback URL.",
  invalid_request:
  ...
  unapproved_software_statement:
    "The authorization server rejected Geetorus's client registration details.",
  unauthorized_client:
    "The authorization server refused to authorize Geetorus's OAuth client.",
  unsupported_grant_type:
    "The authorization server does not support the grant Geetorus uses.",
  unsupported_response_type:
    "The authorization server does not support the sign-in flow Geetorus uses.",
};
...
/** Geetorus's own message for a provider failure, never the provider's. */
 ...
 * own URL is the `client_id` Geetorus presents, so this path is a stable part of
...
/**
 * Resolve the URL Geetorus would use as a CIMD client id, but only when its
 * hostname is not known to resolve into a private network.
 *
 * An authorization server fetches this URL from outside Geetorus's network and
...
/**
 * Geetorus's client metadata for CIMD (RFC 7591 metadata, served rather than
...
export function oauthClientIdMetadataDocument(input: {
  ...
  return {
    ...
    client_name: `Geetorus (${new URL(input.redirectUri).host})`,
...
type OAuthProviderEndpoints = {
   ...
   * restrict the access token to this server rather than to everything Geetorus
   * has ever connected.
   */
...
type ToolAccessServiceOptions = {
  ...
  geetorusCloudConnector?: GeetorusCloudConnector | null;
  /** @deprecated Use geetorusCloudConnector. */
  geetorusIdGmailConnector?: GeetorusCloudConnector | null;
...
> = {
  "geetorus.echo-calculator-time": {
    name: "Geetorus Echo / Calculator / Time fixture",
    tools: [
      {
        name: "echo",
  ...
  "geetorus.synthetic-todo-kv": {
    name: "Geetorus Synthetic Todo / KV fixture",
    tools: [
      {
        name: "list_items",
  ...
  "geetorus.google-sheets": {
    name: "Google Sheets",
    command: "geetorus-google-sheets-mcp-server",
    args: [],
    envKeys: [
...
const GOOGLE_SHEETS_TEMPLATE_ID = "geetorus.google-sheets";
...
const TOOL_EXAMPLES: ToolExampleDefinition[] = [
  {
    ...
    applicationKey: "geetorus.examples.safe-read-only-todo-kv",
    applicationName: "Geetorus example: Safe read-only Todo / KV",
    applicationDescription:
      "Deterministic MCP fixture for first-run tool governance checks.",
    connectionName: "Geetorus example: Safe read-only Todo / KV",
    templateId: "geetorus.synthetic-todo-kv",
    profileKey: "geetorus.examples.safe-read-only-todo-kv.profile",
    profileName: "Example safe read-only tools",
    profileDescription:
      "Allows only the read-only tools from the Geetorus Todo / KV example fixture.",
  },
];
...
function connectionMethodFor(app: AppDefinition, methodKey?: string | null) {
  const normalizedMethodKey =
    app.slug === "gmail" && methodKey === "geetorus-id-oauth"
      ? "geetorus-draft"
      : methodKey;
  ...
  const toolMethods = app.methods.filter(
    (candidate) =>
      ...
      && (availableMethods.has(candidate) || isGeetorusCloudConnectorStrategy(candidate.oauthStrategy)),
  );
...
export async function loadGitHubGrantMetadata(
  ...
  if (installationIds.length === 0 || repositoryCount === 0) {
    ...
    throw unprocessable(
      "GitHub access is required. Install Geetorus and grant at least one repository before refreshing access.",
      {
 ...
 * is Geetorus's own decision about an unsafe address, so it must keep its code
...
function sanitizeHttpFailure(error: unknown): {
  ...
  if (error instanceof HttpError) {
    ...
    return { status: "error", message: error.message, code: "geetorus_error" };
  }
...
export function toolAccessService(
  ...
  const configuredCloudConnector =
    options.geetorusCloudConnector ?? options.geetorusIdGmailConnector;
  const connectorWasProvided =
    options.geetorusCloudConnector !== undefined ||
    options.geetorusIdGmailConnector !== undefined;
  let cachedCloudConnector = configuredCloudConnector ?? null;
  const currentCloudConnector = (): GeetorusCloudConnector | null => {
    ...
    cachedCloudConnector = config
      ? createGeetorusCloudConnector({ config, now: () => now().getTime() })
      : null;
    return cachedCloudConnector;
  };
  async function appForConnectionSetup(app: AppDefinition): Promise<AppDefinition> {
    if (!app.methods.some((method) => isGeetorusCloudConnectorStrategy(method.oauthStrategy))) {
      return app;
    }
    const profiles = connectorWasProvided
      ? (await currentCloudConnector()?.getCapabilities() ?? [])
      : await geetorusCloudConnectorCapabilitiesFromEnv();
    return appWithGeetorusCloudConnectorAvailability(app, profiles);
  }
  ...
  function tokenBrokerAllowedPrivateHosts(): Set<string> {
    const configured = (process.env.GEETORUS_TOKEN_BROKER_ALLOWED_HOSTS ?? "")
      .split(/[,\s]+/)
      .map(normalizeTokenBrokerAllowedHost)
      .filter((host): host is string => host !== null);
    const pagesApiHost = normalizeTokenBrokerAllowedHost(
      process.env.GEETORUS_PAGES_API_URL ?? "",
    );
   ...
   *
   * Every OAuth endpoint Geetorus acts on is attacker-influenced: discovered
   ...
   * Geetorus's own origin is exempt from the transport rule because a
  ...
  /**
   * Origins that are Geetorus itself: this deployment's configured public URL,
  ...
  function firstPartyOrigins(candidate?: string | null): string[] {
    const configured =
      process.env.GEETORUS_PUBLIC_URL?.trim() ||
      process.env.GEETORUS_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  /** Throws unless `value` is an endpoint Geetorus may use (and navigate to). */
  ...
  function trustedRuntimeHost() {
    return (
      ...
      process.env.GEETORUS_TRUSTED_MCP_RUNTIME_HOST ??
      process.env.GEETORUS_TOOL_RUNTIME_TRUSTED_HOST ??
      null
    );
  }
  ...
  function isPagesTokenConnection(
    ...
    return Boolean(
      applicationKey === "geetorus-pages" ||
      applicationKey === "geetorus.pages" ||
      applicationKey === "pages.geetorus" ||
  ...
  async function loadBrokerRunContext(input: {
    ...
    const geetorusIssue = asRecord(snapshot.geetorusIssue);
    ...
    return {
      ...
      issueId:
        ...
        runSnapshotString(geetorusIssue, "id"),
      projectId:
        runSnapshotString(snapshot, "projectId") ??
        runSnapshotString(geetorusIssue, "projectId"),
  ...
  function exchangeTokenUrl(
    ...
    const url =
      ...
      readConfigString(config, "pagesTokenExchangeUrl");
    if (url) return url;
    const pagesApiBase = process.env.GEETORUS_PAGES_API_URL?.trim();
  ...
  function buildRuntimeAlerts(input: {
    ...
    return [
      ...
      runtimeAlert({
        ...
        firstResponderAction:
          "Check upstream MCP health, Geetorus runtime capacity, and recent gateway audit failures before retrying workloads.",
        runbookSection,
      }),
      runtimeAlert({
   ...
   *    Geetorus secret rather than a per-user credential.
  ...
  async function remoteTools(
    ...
    if (usedInitializedSession) {
      const sessionHeaders = await initializeMcpHttpSession({
        send: sendRemote,
        headers,
        requestId: "geetorus-catalog-refresh",
      });
    ...
    if (response.status === 401 && composioChild) {
      ...
      response = await requestRemoteHttpEndpoint(new URL(refreshed.url), {
        ...
        body: JSON.stringify({
          jsonrpc: "2.0",
          id: "geetorus-catalog-refresh-retry",
          method: "tools/list",
          params: {},
        }),
      });
    ...
    if (
      response.status === 401 &&
      connection.authKind === "oauth" &&
      connection.credentialSource === "geetorus_vault"
    ) {
      headers = {
        ...projectedConnectionHeaders(connection),
        ...(await resolveCredentialHeaders(connection, actor, {
      ...
      if (
        response.status === 401 &&
        isGeetorusCloudConnectorStrategy(oauthConfig(connection).strategy)
      ) {
    ...
    if (descriptors.some((tool) => normalizeRailwayToolName(tool.name).startsWith(RAILWAY_TOOL_PREFIX))) {
      throw unprocessable("Railway advertised a reserved Geetorus action name. Refresh is blocked pending review.", { code: "railway_tool_name_collision" });
    }
  ...
  async function assertComposioConnectedAccountActive(
    ...
    const accounts = await client.listConnectedAccounts({
      ...
      userIds: [`geetorus:${child.companyId}`],
      limit: 100,
    });
    ...
    try {
      ...
      accounts = (
        await client.listConnectedAccounts({
          userIds: [`geetorus:${parent.companyId}`],
          limit: 1000,
        })
      ).items;
    } catch {
  ...
  async function syncComposioToolkit(
    ...
    const client = await composioClientForParent(parent);
    const userId = `geetorus:${parent.companyId}`;
  ...
  async function startComposioServiceConnect(
    ...
    const link = await client.createConnectLink({
      authConfigId,
      userId: `geetorus:${parent.companyId}`,
      alias: `geetorus-${parent.companyId}-${toolkitSlug}`,
    ...
    const due = await db
      ...
      .where(
        and(
          ...
          ne(toolApplications.type, "geetorus_plugin"),
    ...
    const rows = await db
      ...
      .values(
        readEntries.map((entry) => ({
          ...
          conditions: { source: "geetorus_example" },
        })),
      )
      .returning();
  ...
  function sampleArguments(toolName: string): Record<string, unknown> {
    ...
    if (toolName === "set_value") return { key: "project", value: "geetorus" };
  ...
  function oauthEnvName(
    ...
    return `GEETORUS_TOOL_OAUTH_${provider.replace(/[^a-z0-9]+/gi, "_").toUpperCase()}_${suffix}`;
  }
  ...
  function oauthClientConfig(provider: string) {
    ...
    return {
      ...
      clientId:
        ...
        process.env.GEETORUS_TOOL_OAUTH_CLIENT_ID ??
        null,
      clientSecret:
        process.env[clientSecretEnv] ??
        process.env.GEETORUS_TOOL_OAUTH_CLIENT_SECRET ??
        null,
    };
  }
  ...
  async function oauthProviderEndpoints(
    ...
    // A gallery default is Geetorus's own data, but it is still a URL that ends
  ...
  async function registerOAuthClient(input: {
    ...
    const requestedMetadata = {
      ...
      // RFC 7591: Geetorus's callback is a server-side HTTPS endpoint, so this
    ...
    // client. Geetorus still uses only the exact redirect, grant and response
    ...
    // metadata inherits the values Geetorus requested. Additional provider-owned
    // values do not widen Geetorus's behavior because they are never persisted as
   ...
   * `client_id` *is* the https URL of Geetorus's published client metadata, so
   ...
   * as `manual`, which is the conservative answer: Geetorus will not silently
  ...
  function oauthClientBindingMatches(
    ...
    // durable proof that Geetorus minted that client. Force a fresh registration
    ...
    // Geetorus's callback, so it has no recorded callback until first use.
  ...
  /**
   * May Geetorus mint client material for this connection without an operator
  ...
  async function ensureOAuthClient(input: {
    ...
    if (
      ...
      // Geetorus cannot re-register on the operator's behalf: the credentials
  ...
  async function exchangeOAuthToken(input: {
    ...
    // Geetorus without re-checking the transport it would leave over.
    const tokenUrl = assertOAuthEndpointUrl("token", input.tokenUrl, {
      // Geetorus's own callback origin, so a first-party token endpoint keeps
  ...
  async function refreshOAuthGrantCredentials(input: {
    ...
    if (
      connection.authKind === "oauth" &&
      connection.credentialSource === "geetorus_vault" &&
      isGeetorusCloudConnectorStrategy(oauth.strategy)
    ) {
        ...
        async () => {
          ...
          try {
            ...
            try {
              ...
              if (
                error instanceof GeetorusCloudConnectorError &&
            ...
            const providerTenant = {
              ...
              oauth: {
                ...
                strategy: "geetorus_cloud_connector",
  ...
  async function connectGalleryApp(
    ...
    const credentialSource: ToolConnectionCredentialSource =
      input.credentialSource ?? "geetorus_vault";
    if (
      retainedConnection &&
    ...
    if (credentialSource === "vercel_connect") {
      ...
      if (!integration.enabled || !integration.configured || !vercelConnect) {
        throw unprocessable(
          "Vercel Connect setup is not available on this Geetorus instance",
          {
            code: "vercel_connect_unavailable",
          },
        );
      ...
      if (expectedPrincipalMode === "app") {
        ...
        if (connectorInUse) {
          throw conflict(
            "App-subject Vercel connectors are dedicated to one Geetorus connection. Create or attach a separate connector in Vercel.",
            {
    ...
    if (method && isGeetorusCloudConnectorStrategy(method.oauthStrategy)) {
    ...
    // the client id now; the secret becomes an encrypted Geetorus secret below.
    ...
    const credentialPolicy: ToolConnectionCredentialPolicy =
      requestedGrantKind === "user"
        ...
        : requestedGrantKind === "agent"
          ? "per_agent"
          : "shared";
    const connectionOwnership = isGeetorusCloudConnectorStrategy(
    ...
    try {
      ...
      if (existingApplication) {
        ...
        if (!applicationRow) {
          throw conflict(
            "Geetorus could not allocate a unique connection name",
            {
        ...
        if (
          ...
          const endpoints = await discoverOAuthEndpoints(oauthConnection).catch(
            (discoveryError: unknown) => {
              // "This server advertised an address Geetorus refuses to open" is a
  ...
  async function reconnectGalleryApp(
    ...
    if (connection.credentialSource === "vercel_connect") {
      throw conflict(
        "Manage this connector in Vercel Connect, then run a Geetorus health check to verify it.",
        {
  ...
  async function startOAuth(
    ...
    if (
      galleryMethod &&
      isGeetorusCloudConnectorStrategy(galleryMethod.oauthStrategy)
    ) {
      ...
      if (!cloudConnector) {
        throw unprocessable(
          `${providerName} connections through Geetorus are not available on this instance yet`,
          {
            code: "geetorus_cloud_connector_unavailable",
          },
        );
      }
      const binding = starterBinding;
      ...
      await db.insert(toolOauthStates).values({
        ...
        companyId,
        connectionId: connection.id,
        // Geetorus Cloud owns PKCE for this flow. The local state row remains the
        // single-use browser correlator and never stores broker token material.
        codeVerifier: "geetorus-cloud-connector",
        createdByActorType: binding.actorType,
        createdByActorId: binding.actorId,
        createdBySessionId: binding.sessionId,
        subjectUserId,
    ...
    const authorizationUrl = new URL(
      assertOAuthEndpointUrl("authorization", endpoints.authorizationUrl, {
        // Geetorus's own callback origin: a first-party authorization endpoint is
    ...
    // MCP URLs retain discovery-first behavior because Geetorus has no manifest
    ...
    const nextConfig = {
      ...
      oauth: {
        ...
        // universe and must never silently become Geetorus's requested set.
    ...
    await db
      ...
      .set({
        ...
        result: {
          version: 1,
          outcome: "rejected",
          // Geetorus's own words: the provider's explanation is untrusted and
  ...
  async function completeGeetorusCloudConnectorCallback(input: {
    state: string;
    claimId?: string | null;
    error?: string | null;
    actor?: ActorInfo;
        ...
        `${providerName} authorization did not complete. Start a new ${providerName} connection to try again.`,
        {
          code:
            input.error === "access_denied"
              ...
              : "geetorus_cloud_connector_failed",
        },
      );
    }
    if (!input.claimId)
    ...
    if (
      !method ||
      !isGeetorusCloudConnectorStrategy(method.oauthStrategy) ||
  ...
  async function completeOAuthCallback(input: {
    ...
    // acted on once the callback is bound to a state Geetorus issued and to the
  ...
  return {
    ...
    completeGeetorusCloudConnectorCallback,
    ...
    listProjectRepositories: async (
      ...
      const repositories = new Map<
        string,
        import("@geetorusai/shared").ProjectRepository
      >();
    ...
    revokeConnectionGrant: async (
      ...
      if (
        ...
        // profiles intentionally share one Geetorus-owned client, so revoking
      ...
      const drafts = Object.entries(mcpServers).map(([name, rawServer]) => {
        ...
        if (
          ...
          const credentialFields = Object.keys(headers)
            ...
            .map((key) => {
              warnings.push(
                `Header ${key} will be stored as a Geetorus secret before activation.`,
              );
              return {
        ...
        if (typeof server.command === "string") {
          warnings.push(
            "Imported stdio commands stay draft-only unless mapped to an approved Geetorus template.",
          );
          return {
            name,
