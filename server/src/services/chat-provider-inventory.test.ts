  ...
  it("exchanges a GitHub App JWT and never exposes the installation token", async () => {
          ...
          repositories: [
            {
              id: 101,
              name: "repo",
              full_name: "geetorus/repo",
              html_url: "https://github.com/geetorus/repo",
              owner: { id: 12, login: "geetorus" },
              private: true,
            },
          ],
        }),
    ...
    expect(result.resources).toEqual([
      expect.objectContaining({
        ...
        label: "geetorus/repo",
        providerUrl: "https://github.com/geetorus/repo",
      }),
    ]);
  ...
  it("discovers the one active installation without asking for an ID", async () => {
    const fetch = vi.fn(async () =>
      response([
        {
          id: 44,
          account: { id: 12, login: "geetorus", type: "Organization" },
          permissions: {
            issues: "write",
            metadata: "read",
            pull_requests: "write",
