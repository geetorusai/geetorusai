...
const DEFERRED_WAKE_CONTEXT_KEY = "_geetorusWakeContext";
...
export function recoveryService(
  ...
  async function hasCurrentNativePassiveWait(
    ...
    if (
      result.schema !== "geetorus.run_result.v1" ||
  ...
  async function hasPersistedDurableWaitPath(
    ...
    // turn. The Geetorus issue intentionally remains in progress so the next
    ...
    for (const candidate of candidates) {
      ...
      await issuesSvc.addComment(
        ...
        [
          ...
          "",
          `Geetorus found this issue is blocking ${blockingLinks} but had no assignee, so no heartbeat could pick it up.`,
          "",
  ...
  function buildRecoveryIssueInPlaceEscalationComment(input: {
    ...
    return [
      "Geetorus stopped automatic stranded-work recovery for this recovery issue.",
      "",
  ...
  async function resolveContinuationWaitingOnReview(
    ...
    await issuesSvc.addComment(
      ...
      `This task is waiting on ${waitingOn} to finish. ` +
        ...
        "Geetorus turned that into a normal dependency wait instead of flagging it as stuck.)",
      {},
      {
        authorType: "system",
        presentation: compactRecoveryPresentation(
  ...
  async function escalateDispositionRepair(input: {
    ...
    await issuesSvc.addComment(
      input.issue.id,
      [
        "Geetorus exhausted the bounded original-owner disposition repair without a durable source-state change.",
        "",
  ...
  async function reconcileStrandedAssignedIssues(opts?: {
    ...
    for (const issue of candidates) {
      ...
      if (issue.status !== "in_review" && !agentInvokable) {
        ...
        if (
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "Geetorus cannot safely continue automatic recovery because the original assignee is not invokable. " +
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            recoveryCause:
              issue.status === "in_review"
                ...
                : undefined,
            comment:
              "Geetorus cannot safely continue automatic recovery because the original recovery target is over budget. " +
              "The source assignment is unchanged and the board must choose the next action.",
          });
          if (updated) {
            result.escalated += 1;
      ...
      if (latestRun && adapterFailureClassification) {
        ...
        if (adapterFailureClassification.kind === "provider_quota") {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            recoveryCause: "configuration_incomplete",
            comment:
              "Geetorus classified the latest adapter failure as `configuration_incomplete`. " +
        ...
        if (!successfulRunSinceResolution) {
          ...
          if (
            ...
            const updated = await escalateStrandedAssignedIssue({
              ...
              comment:
                `Geetorus stopped requeueing accepted interaction \`${acceptedContinuationInteraction.id}\` after ` +
      ...
      if (issue.status === "in_review") {
        ...
        if (
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            recoveryCause: "configuration_incomplete",
            comment:
              "Geetorus classified the active review participant's latest adapter failure as " +
      ...
      if (issue.status === "todo") {
        ...
        if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
          const updated = await escalateStrandedAssignedIssue({
            ...
            notice: {
              body:
                "Geetorus automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
      ...
      if (isSuccessfulInProgressContinuationRun(latestRun)) {
        ...
        const workspace = parseObject(parseObject(successfulRun.contextSnapshot).geetorusWorkspace);
        ...
        if (isRepeatedProductiveContinuationRecovery(successfulRun)) {
          ...
          if (!exempted) {
            const updated = await escalateStrandedAssignedIssue({
              ...
              comment:
                "Geetorus automatically retried continuation for this assigned `in_progress` issue and the retry " +
      ...
      if (isUnsuccessfulTerminalIssueRun(latestRun)) {
        ...
        if (classification.kind === "non_retryable") {
          const updated = await escalateStrandedAssignedIssue({
            ...
            notice: {
              body:
                "Geetorus detected a non-retryable failure on this issue's continuation run " +
        ...
        if (didAutomaticRecoveryFail(latestRun, "issue_continuation_needed")) {
          ...
          if (consecutive >= classification.maxAttempts) {
            ...
            const updated = await escalateStrandedAssignedIssue({
              ...
              notice: {
                body:
                  "Geetorus automatically retried continuation for this assigned `in_progress` issue after its live " +
