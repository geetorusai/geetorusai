...
import { UNMANAGED_BACKGROUND_TASK_LIVENESS_REASON } from "@geetorusai/adapter-utils/server-utils";
...
describe("successful run handoff decision", () => {
  ...
  it("does not queue when a plugin owns the issue's lifecycle", () => {
    expect(decide({ issue: { ...issue, originKind: "plugin:geetorus.workflow-engine" } as any })).toEqual({
      kind: "skip",
      reason: "issue lifecycle is owned by a plugin",
    });
    expect(decide({ issue: { ...issue, originKind: "plugin:geetorus.workflow-engine:advance" } as any })).toEqual({
      kind: "skip",
      reason: "issue lifecycle is owned by a plugin",
    });
  });
  ...
  describe("isPluginManagedIssueLifecycle", () => {
    it("is true for any plugin: prefixed origin kind", () => {
      expect(isPluginManagedIssueLifecycle({ originKind: "plugin:geetorus.workflow-engine" })).toBe(true);
      expect(isPluginManagedIssueLifecycle({ originKind: "plugin:geetorus.workflow-engine:advance" })).toBe(true);
    });
