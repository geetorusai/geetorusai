import type { IssueCommentMetadata, IssueCommentPresentation } from "@geetorusai/shared";
import {
  agentLinkRow,
  keyValueRow,
  runLinkRow,
...
export const DEFAULT_STRANDED_RECOVERY_NOTICE_BODY =
  "Geetorus could not restore a live execution path for this issue automatically. " +
...
export function buildImmediateExecutionPathRecoveryNoticeSeed(input: {
  ...
  const retryDescription = input.status === "todo"
    ? "Geetorus automatically retried dispatch for this assigned `todo` issue during terminal run recovery"
    : "Geetorus automatically retried continuation for this assigned `in_progress` issue during terminal run recovery";
  return {
    body:
...
export function buildWorkspaceValidationRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Geetorus stopped before launching the local adapter because the issue workspace failed validation. " +
...
export function buildConfigurationIncompleteRecoveryNoticeSeed(
  ...
  if (readNonEmptyStringField(configurationIncomplete, "reason") === SANDBOX_PROVIDER_PLUGIN_NOT_READY_REASON) {
    ...
    return {
      body:
        `Geetorus stopped before dispatching the adapter because the sandbox provider plugin \`${pluginKey}\` ` +
  ...
  return {
    body:
      "Geetorus stopped before dispatching the adapter because required secret/env bindings are missing. " +
...
export function buildExecutionReviewParticipantRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Geetorus retried the pending execution-review participant once, but the review stage still has no " +
...
export function buildExecutionReviewParticipantUnavailableNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Geetorus cannot continue the pending execution-review participant because the participant is not " +
...
export function buildStrandedRecoveryEscalationNotice(input: {
  ...
  const seed = workspaceScan ? {
    ...workspaceScan,
    body: `Geetorus could not prepare the workspace before the agent started. Automatic recovery could not continue. ${workspaceScan.nextAction}`,
