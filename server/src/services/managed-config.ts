 ...
 *
 * Instances managed by the Geetorus Cloud harness receive one environment
 * variable, `GEETORUS_MANAGED_CONFIG`, holding a single JSON document:
 *
 *   {
 *     "v": 1,
 *     "mode": "cloud",
...
export const MANAGED_CONFIG_ENV_KEY = "GEETORUS_MANAGED_CONFIG";
...
/**
 * Parse `GEETORUS_MANAGED_CONFIG` from a raw env map. Returns null only when
...
export function parseManagedConfigEnv(env: ManagedConfigEnv): ManagedInstanceConfig | null {
  ...
  if (doc.environments !== undefined) {
    ...
    // The DB enforces at most ONE Geetorus-managed sandbox row per instance
    ...
    if (doc.environments.length > 1) {
      fail(
        `"environments" supports at most one entry: each entry provisions the single Geetorus-managed sandbox environment (DB invariant environments_managed_sandbox_idx)`,
      );
    }
