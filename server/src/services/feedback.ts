...
import { readGeetorusSkillSyncPreference } from "@geetorusai/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@geetorusai/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@geetorusai/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@geetorusai/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveGeetorusInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "geetorus-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "geetorus-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "geetorus-feedback-v1";
const FEEDBACK_DESTINATION = "geetorus_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "geetorus_run" &&
    file.source !== "geetorus_run_events" &&
    file.source !== "geetorus_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.GEETORUS_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readGeetorusSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let geetorusRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      geetorusRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "geetorus/run.json",
        contentType: "application/json",
        source: "geetorus_run",
        contents: `${JSON.stringify(geetorusRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.geetorusRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "geetorus/run-events.json",
        contentType: "application/json",
        source: "geetorus_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "geetorus/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "geetorus_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.geetorusRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
