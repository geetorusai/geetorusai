...
import { WORKSPACE_GIT_SCAN_SATURATED_CODE } from "@geetorusai/adapter-utils/git-workspace-sync";

const tempPaths: string[] = [];

async function makeWorkspace(name = "workspace"): Promise<string> {
  const parent = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-git-scheduler-"));
...
describe("WorkspaceGitOperationScheduler", () => {
  it("loads bounded process defaults and overrides from the environment", () => {
    ...
    expect(workspaceGitSchedulerOptionsFromEnv({
      GEETORUS_WORKSPACE_GIT_SCAN_CONCURRENCY: "4",
      GEETORUS_WORKSPACE_GIT_SCAN_QUEUE_CAPACITY: "12",
      GEETORUS_WORKSPACE_GIT_SCAN_TIMEOUT_MS: "5000",
      GEETORUS_WORKSPACE_GIT_SCAN_CACHE_TTL_MS: "7000",
    })).toEqual({
      concurrency: 4,
      queueCapacity: 12,
      timeoutMs: 5_000,
  ...
  it("kills a hung subprocess after the hard timeout and releases the slot for the next scan", async () => {
    ...
    await fs.writeFile(scriptPath, [
      ...
      'fs.writeFileSync(process.env.GEETORUS_FAKE_GIT_PID_PATH, String(process.pid));',
    ...
    const env = {
      ...
      GEETORUS_FAKE_GIT_PID_PATH: pidPath,
    };
