...
import { readGeetorusSkillSyncPreference, writeGeetorusSkillSyncPreference } from "@geetorusai/adapter-utils/server-utils";
import type { GeetorusDesiredSkillEntry, GeetorusSkillEntry } from "@geetorusai/adapter-utils/server-utils";
import type {
...
function assertImportedSkillKeyAllowed(skill: ImportedSkill) {
  if (!skill.key.startsWith("geetorusai/geetorus/")) return;
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  const sourceKind = asString(metadata?.sourceKind);
  if (sourceKind === "geetorus_bundled") return;
  throw unprocessable(
    `Reserved Geetorus skill key "${skill.key}" cannot be imported from unbundled sources.`,
    {
      skillKey: skill.key,
 ...
 */
export const GEETORUS_CORE_SKILL_KEYS = [
  "geetorusai/geetorus/geetorus",
  "geetorusai/geetorus/geetorus-board",
  "geetorusai/geetorus/geetorus-converting-plans-to-tasks",
  "geetorusai/geetorus/geetorus-create-agent",
  "geetorusai/geetorus/para-memory-files",
] as const;

export const ONBOARDING_FIRST_TASK_SKILL_KEY = "geetorusai/geetorus/first-task";
...
function resolveBundledSkillReleasesRoot() {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  return [
    path.resolve(moduleDir, "../../skills-releases/geetorus"),
    path.resolve(process.cwd(), "skills-releases/geetorus"),
    path.resolve(moduleDir, "../../../skills-releases/geetorus"),
  ];
}
...
function deriveImportedSkillSource(
  ...
  if (kind === "github-dir" || kind === "github-file") {
    ...
    if (repo && owner && repoName) {
      const sourceKind = owner === "geetorusai"
        && repoName === "geetorus"
...
function isGeetorusBundledSkillKey(key: string) {
  return key.startsWith("geetorusai/geetorus/");
}

function geetorusBundledFolderCategory(key: string, metadata?: unknown) {
  const keyParts = key.split("/");
  if (keyParts[0] === "geetorusai" && keyParts[1] === "bundled" && keyParts[2]) {
    return keyParts[2];
  }
  if (isGeetorusBundledSkillKey(key)) return "geetorus-core";
  if (isPlainRecord(metadata) && asString(metadata.sourceKind) === "geetorus_bundled") {
    return "geetorus-core";
  }
  return null;
}
...
function stripDerivedGeetorusBundledMetadata(key: string, metadata: unknown): unknown {
  ...
  if (out.sourceKind === "geetorus_bundled") delete out.sourceKind;
...
function importedSkillMetadataEqual(existing: CompanySkill, values: ImportedSkillPersistValues) {
  ...
  if (isGeetorusBundledSkillKey(values.key) && asString(incomingMetadata?.sourceKind) === "geetorus_bundled") {
    return JSON.stringify(stripDerivedGeetorusBundledMetadata(existing.key, existing.metadata))
      === JSON.stringify(stripDerivedGeetorusBundledMetadata(values.key, values.metadata));
  }
...
const BUILT_IN_SKILL_TEST_RUN_TEMPLATE_BODY = [
  ...
  "",
  "Invoke and use the selected skill under test: `{{skillInvocation}}`. Use the pinned skill revision supplied by Geetorus as the source of truth, regardless of any other runtime skills.",
  "",
...
function builtInSkillTestRunTemplate(companyId: string): CompanySkillTestRunTemplate {
  return {
    ...
    description: "Geetorus's read-only default harness instructions for Skills Studio runs.",
...
function normalizeRequestedDesiredSkillSelection(value: string | AgentDesiredSkillEntry): GeetorusDesiredSkillEntry {
...
export interface ResolvedRequestedSkillEntries {
  ...
  resolved: GeetorusDesiredSkillEntry[];
  /**
...
async function resolveRequestedSkillEntriesOrThrow(
  ...
  const missing = new Set<string>();
  const ambiguous = new Set<string>();
  const resolved = new Map<string, GeetorusDesiredSkillEntry>();
...
function resolveDesiredSkillEntries(
  ...
  const preference = readGeetorusSkillSyncPreference(config);
  const out = new Map<string, GeetorusDesiredSkillEntry>();
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but Geetorus cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but Geetorus cannot find a valid local runtime source for it.`;
}
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveGeetorusInstanceRoot(), "skills", companyId);
}

/**
 * A rename target must be a true Geetorus-managed local skill: a `local_path`
 ...
 */
function isGeetorusManagedRenameTarget(skill: CompanySkill): boolean {
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "geetorus_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Geetorus skills are read-only.",
      sourceLabel: "Geetorus bundled",
      sourceBadge: "geetorus",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Geetorus workspace",
        sourceBadge: "geetorus",
...
export function companySkillService(db: Db) {
  ...
  async function assertLocalImportSourceAllowed(companyId: string, source: string) {
    ...
    if (!allowed) {
      throw forbidden("Local skill source is outside approved company workspace roots", {
        ...
        remediation: "Import from a configured Geetorus workspace or the company managed-skill directory.",
      });
    }
  }
  ...
  async function ensureBundledSkillReleases(companyId: string, bundledSkills: CompanySkill[]) {
    const geetorusSkill = bundledSkills.find((skill) => skill.key === "geetorusai/geetorus/geetorus");
    if (!geetorusSkill) return;
    for (const release of await readBundledSkillReleaseRegistry()) {
      ...
      const existing = await db
        ...
        .where(and(
          eq(companySkillVersions.companyId, companyId),
          eq(companySkillVersions.companySkillId, geetorusSkill.id),
      ...
      await createVersion(companyId, geetorusSkill.id, { label: release.releaseName }, null, {
        fileInventory,
  ...
  async function reconcileGeetorusSkillFolders(companyId: string) {
    const shippedSkills = await db
      ...
      .then((rows) => rows.flatMap((skill) => {
        const category = geetorusBundledFolderCategory(skill.key, skill.metadata);
  ...
  async function reconcileLocalPathSkillSources(companyId: string) {
    ...
    for (const skill of skills) {
      if (skill.sourceType !== "local_path") continue;
      if (isGeetorusBundledSkillKey(skill.key) || asString(skill.metadata?.sourceKind) === "geetorus_bundled") continue;
  ...
  async function ensureSkillInventoryCurrent(companyId: string) {
    ...
    const refreshPromise = (async () => {
      ...
      await reconcileGeetorusSkillFolders(companyId);
    ...
    await db.transaction(async (tx) => {
      for (const item of planned) {
        ...
        const updated = await tx
          .update(agentsTable)
          .set({
            adapterConfig: writeGeetorusSkillSyncPreference(adapterConfig, nextEntries),
            updatedAt: new Date(),
          })
          .where(and(eq(agentsTable.companyId, companyId), eq(agentsTable.id, item.agentId)))
    ...
    await removeRuntimeSkillCache(managedRoot, skillId, async () => {
      try {
        result = await withSkillFileMutation(companyId, skillId, async (skill, tx) => {
          if (!isGeetorusManagedRenameTarget(skill)) {
            throw unprocessable(
              "Only Geetorus-managed skills can be renamed. Catalog, external, project-scanned, and unmanaged local skills are read-only.",
    ...
    for (const target of scanTargets) {
      ...
      for (const directory of directories) {
        ...
        const existingBundledBySlug = acceptedSkills.find((skill) => (
          ...
          && (isGeetorusBundledSkillKey(skill.key) || asString(skill.metadata?.sourceKind) === "geetorus_bundled")
        )) ?? null;
  ...
  async function installFromCatalog(
    ...
    const bundledCategory = geetorusBundledFolderCategory(catalogSkill.key, metadata);
    ...
    const values = {
      ...
      authorName: storeMetadata.authorName ?? existingByKey?.authorName ?? "Geetorus",
  ...
  async function upsertImportedSkills(
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "geetorus_bundled"
        && incomingKind === "github"
        && incomingOwner === "geetorusai"
        && incomingRepo === "geetorus"
      ) {
        out.push(existing);
        continue;
      }
      ...
      const bundledCategory = geetorusBundledFolderCategory(skill.key, incomingMeta);
      const bundledFolder = bundledCategory
        ? await folderSvc.ensureBundledCategory(companyId, bundledFolderLabel(bundledCategory))
        : null;
  ...
  function snapshotAgentConfig(agent: Awaited<ReturnType<typeof agents.getById>>) {
    ...
    return {
      ...
      adapterConfig,
      runtimeConfig,
      assignedSkills: isPlainRecord(adapterConfig.geetorusSkillSync)
        ? adapterConfig.geetorusSkillSync
        : null,
      instructionsRef:
