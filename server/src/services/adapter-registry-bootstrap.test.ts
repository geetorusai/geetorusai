...
describe("parseAdapterRegistryEnv", () => {
  ...
  it("parses inline GEETORUS_ADAPTERS JSON", () => {
    const r = parseAdapterRegistryEnv({ GEETORUS_ADAPTERS: ENTRY });
  ...
  it("throws on malformed JSON (fail loud)", () => {
    expect(() => parseAdapterRegistryEnv({ GEETORUS_ADAPTERS: "{not json" })).toThrow(
      /GEETORUS_ADAPTERS/,
    );
  });
  ...
  it("throws on schema-invalid content (fail loud)", () => {
    expect(() =>
      parseAdapterRegistryEnv({ GEETORUS_ADAPTERS: JSON.stringify([{ enabled: true }]) }),
    ).toThrow(/GEETORUS_ADAPTERS/);
  });
});
