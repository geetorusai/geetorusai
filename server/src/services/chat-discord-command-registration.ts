...
const fenceSchema = z
  ...
  .strict();
const common = {
  schema: z.literal("geetorus.discord.command-registration.v1"),
  scope: scopeSchema,
  ownerId,
};
...
export function discordGeetorusCommandDefinition(publicOwnerId: string) {
  if (!ownerId.safeParse(publicOwnerId).success)
    throw new Error("Invalid Discord command owner identifier");
  return {
    type: 1,
    name: "geetorus",
    description: `Geetorus session controls [pc:${publicOwnerId}]`,
    options: [
      {
        type: 1,
        name: "status",
        description: "Show the current Geetorus task",
      },
      {
        type: 1,
        name: "new",
...
function priorCloseCopyDefinition(id: string) {
  const definition = discordGeetorusCommandDefinition(id);
  definition.options[2]!.description = "Close the current Geetorus task";
  return definition;
}
    ...
    .update(
      JSON.stringify(
        priorCloseCopy
          ...
          : discordGeetorusCommandDefinition(id),
      ),
    )
    .digest("hex");
}
...
export function createDiscordCommandRegistration(
  ...
  return freezeState({
    schema: "geetorus.discord.command-registration.v1",
    scope: parsed.data,
...
export async function reconcileDiscordCommandRegistration(
  ...
  const namespace = commands.filter(
    (command) => command.type === 1 && command.name === "geetorus",
  );
