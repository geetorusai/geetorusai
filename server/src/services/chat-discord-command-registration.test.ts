...
import {
  ...
  discordGeetorusCommandDefinition,
...
function fixture() {
  ...
  const fetch = vi.fn<typeof globalThis.fetch>(async (url, init) => {
    ...
    expect(body).toEqual(discordGeetorusCommandDefinition(stored.ownerId));
...
describe("Discord owned native-command registration", () => {
  async function priorCopyFixture() {
    ...
    const prior = discordGeetorusCommandDefinition(f.stored.ownerId);
    prior.options[2]!.description = "Close the current Geetorus task";
  ...
  it("upgrades the exact prior close description using its retained owner, command and version", async () => {
    ...
    expect(f.commands[1]).toMatchObject({
      id: commandId,
      ...discordGeetorusCommandDefinition(f.stored.ownerId),
    });
  });
  ...
  it("describes closing a conversation without claiming to close the Geetorus task", () => {
    const definition = discordGeetorusCommandDefinition(
  ...
  it("refuses an existing unowned namespace even if its command shape looks familiar", async () => {
    ...
    f.commands = [
      {
        ...discordGeetorusCommandDefinition("b".repeat(32)),
        id: commandId,
        application_id: scope.applicationId,
        version,
      },
  ...
  it("leaves customer-owned status, new and close commands untouched", async () => {
    ...
    expect(f.commands[4]!.name).toBe("geetorus");
  });
  ...
  it("enforces Discord command limits without deleting customer commands", async () => {
    const f = fixture();
    const body = discordGeetorusCommandDefinition(f.stored.ownerId);
