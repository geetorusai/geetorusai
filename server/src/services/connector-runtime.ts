...
import type { AgentSkillSnapshot } from "@geetorusai/shared";
import {
  resolveGeetorusSkillsDir,
...
const skillKey = (connector: ConnectorDefinition) =>
  `geetorusai/geetorus/${connector.skillName}`;
...
export async function applyConnectorSkills(
  ...
  const desired = readGeetorusSkillSyncPreference(
    config,
  ...
  for (const assignment of assignments) {
    ...
    const root = await resolveGeetorusSkillsDir(
  ...
  const connectorSkillDigest = assignments.length
    ...
    : null;
  return {
    ...writeGeetorusSkillSyncPreference(config, desired),
    geetorusRuntimeSkills: skills,
    geetorusConnectorSkillDigest: connectorSkillDigest,
  };
}
...
export async function prepareConnectorSkillDelivery(
  ...
  const assigned = config.geetorusRuntimeSkills.filter((entry) =>
  ...
  return {
    config: {
      ...stripped,
      geetorusConnectorSkillDigest: config.geetorusConnectorSkillDigest,
    },
    instructions,
  };
}
