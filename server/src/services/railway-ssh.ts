...
export async function generateRailwaySshKey(): Promise<{ publicKey: string; privateKey: string }> {
  const directory = await mkdtemp(path.join(tmpdir(), "geetorus-railway-key-"));
  try {
    const keyPath = path.join(directory, "identity");
    await promisify(execFile)("/usr/bin/ssh-keygen", ["-q", "-t", "ed25519", "-N", "", "-C", "geetorus-railway", "-f", keyPath], { timeout: 10_000, env: { PATH: "/usr/bin:/bin" } });
    return { publicKey: (await readFile(`${keyPath}.pub`, "utf8")).trim(), privateKey: await readFile(keyPath, "utf8") };
  } catch {
    throw new RailwayError("railway_ssh_unavailable", "Generating a Railway key requires system OpenSSH (ssh-keygen) on the Geetorus runtime.", 422);
...
export async function runRailwaySshCommand(input: RailwaySshInput & { privateKey: string; knownHosts: string }) {
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "geetorus-railway-command-"));
  try {
    ...
    return await new Promise<{ exitCode: number | null; stdout: string; stderr: string; truncated: boolean; timedOut: boolean }>((resolve, reject) => {
      ...
      const marker = `geetorus_railway_completed_${randomBytes(16).toString("hex")}`;
      ...
      child.once("error", () => { cleanup(); reject(new RailwayError("railway_ssh_unavailable", "System OpenSSH is unavailable on this Geetorus runtime.", 422)); });
      ...
      child.stdin.end(`sh -c ${quotedCommand} </dev/null\ngeetorus_command_status=$?\nprintf '\\n${marker}:%d\\n' "$geetorus_command_status"\nexit "$geetorus_command_status"\n`);
    });
  } finally { await rm(directory, { recursive: true, force: true }); }
}
