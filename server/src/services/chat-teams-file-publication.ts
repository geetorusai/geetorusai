...
import { z } from "zod";
import { CHAT_FILE_TRANSFER_PHASES } from "@geetorusai/shared";
import {
  chatActions,
  chatConversations,
  chatEndpoints,
...
export async function projectTeamsFilePublication(
  ...
  const transfer = await tx
    .select({
      ...
      operatorConfirmed: sql<boolean>`coalesce(${chatTeamsFileTransfers.privateState}->'resolution'->>'schema' = 'geetorus.teams.file-resolution.v1'
