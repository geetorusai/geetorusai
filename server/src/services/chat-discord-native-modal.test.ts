  ...
  it("does not turn an application-swallowed unsupported modal into a success ACK", async () => {
    ...
    expect(click.reply).toHaveBeenCalledWith(
      expect.objectContaining({
        content:
          "This form could not be opened. Open the linked Geetorus task.",
        flags: 64,
      }),
    );
  });
  ...
  it.each([
    ...
    async (variant) => {
      ...
      expect(input.reply).toHaveBeenCalledWith({
        content:
          "This response was not accepted. Open the linked Geetorus task or reopen the question to try again.",
        flags: 64,
        allowedMentions: { parse: [] },
      });
