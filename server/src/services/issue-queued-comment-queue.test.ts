...
describe("decideQueuedCommentQueueSteering", () => {
  ...
  it("answers temporarily_unavailable for a promoted native queue with no deferred run", () => {
    const decision = decideQueuedCommentQueueSteering({
      ...
      activeRun: null,
      assignedAgentAdapterType: "geetorus_runner",
      queuedCommentCount: 1,
    });

    expect(decision).toEqual({ protocol: "geetorus_runner_v1", kind: "temporarily_unavailable" });
  });
  ...
  it("tells the caller it may probe a running deferred turn on the native protocol", () => {
    ...
    expect(decision).toEqual({ protocol: "geetorus_runner_v1", kind: "probe", steeringRunId: "run-1" });
  });
  ...
  // `geetorus_runner` adapter. The protocol resolves to
  // `geetorus_runner_v1` through the adapter-type fallback, and the
