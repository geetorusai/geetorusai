 ...
 * calls. On startup we:
 *   1. Parse `GEETORUS_EXECUTION_MODE` (+ `GEETORUS_K8S_*`) from the env.
...
import type { InstanceExecutionMode } from "@geetorusai/shared";
...
function parsePositiveIntMs(value: string | undefined): number | undefined {
  ...
  if (!Number.isFinite(parsed) || !Number.isInteger(parsed) || parsed <= 0) {
    throw new Error(
      `GEETORUS_K8S_RPC_TIMEOUT_MS must be a positive integer of milliseconds (got "${value}").`,
    );
  }
  return parsed;
}
 ...
 * unrestricted (no env, or `GEETORUS_EXECUTION_MODE=any`). Throws on an
...
export function parseExecutionPolicyBootstrapEnv(
  ...
  const raw = env.GEETORUS_EXECUTION_MODE?.trim();
  if (!raw || raw === "any") return null;
  if (raw !== "kubernetes") {
    throw new Error(
      `GEETORUS_EXECUTION_MODE must be "kubernetes" or "any" (got "${raw}").`,
    );
  }

  const kubernetesConfig: KubernetesEnvironmentConfigInput = {
    // inCluster defaults to false (matches the plugin schema default); an
    // in-cluster cloud deployment sets GEETORUS_K8S_IN_CLUSTER=true.
    inCluster: parseBool(env.GEETORUS_K8S_IN_CLUSTER) ?? false,
  };

  const backend = env.GEETORUS_K8S_BACKEND?.trim();
  if (backend) {
    if (backend !== "job" && backend !== "sandbox-cr") {
      throw new Error(
        `GEETORUS_K8S_BACKEND must be "job" or "sandbox-cr" (got "${backend}").`,
      );
    }
    kubernetesConfig.backend = backend;
  }

  const egressMode = env.GEETORUS_K8S_EGRESS_MODE?.trim();
  if (egressMode) {
    if (egressMode !== "cilium" && egressMode !== "standard") {
      throw new Error(
        `GEETORUS_K8S_EGRESS_MODE must be "cilium" or "standard" (got "${egressMode}").`,
      );
    }
    kubernetesConfig.egressMode = egressMode;
  }

  const runtimeClassName = env.GEETORUS_K8S_RUNTIME_CLASS_NAME?.trim();
  if (runtimeClassName) kubernetesConfig.runtimeClassName = runtimeClassName;

  const namespacePrefix = env.GEETORUS_K8S_NAMESPACE_PREFIX?.trim();
  if (namespacePrefix) kubernetesConfig.namespacePrefix = namespacePrefix;

  const imageRegistry = env.GEETORUS_K8S_IMAGE_REGISTRY?.trim();
  if (imageRegistry) kubernetesConfig.imageRegistry = imageRegistry;

  const rpcTimeoutMs = parsePositiveIntMs(env.GEETORUS_K8S_RPC_TIMEOUT_MS);
  if (rpcTimeoutMs !== undefined) kubernetesConfig.timeoutMs = rpcTimeoutMs;

  const adapterType = env.GEETORUS_K8S_ADAPTER_TYPE?.trim();
  if (adapterType) kubernetesConfig.adapterType = adapterType;

  const egressAllowFqdns = parseList(env.GEETORUS_K8S_EGRESS_ALLOW_FQDNS);
  if (egressAllowFqdns) kubernetesConfig.egressAllowFqdns = egressAllowFqdns;

  const egressAllowCidrs = parseList(env.GEETORUS_K8S_EGRESS_ALLOW_CIDRS);
