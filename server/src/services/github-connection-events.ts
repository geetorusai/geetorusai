...
export function githubConnectionEventService(
  db: Db,
  options: {
    connector?: GeetorusCloudConnector;
  ...
  return {
    async pollOnce(): Promise<GitHubConnectionEventPollResult> {
      ...
      const config = options.connector ? null : geetorusCloudConnectorConfigFromEnv(options.env);
      const connector = options.connector ?? (config ? createGeetorusCloudConnector({ config }) : null);
      if (!connector) {
        nextPollAt = now().getTime() + 5 * 60_000;
        return { leased: 0, processed: 0, duplicate: 0, ignored: 0, failed: 0 };
      }
