...
import { type Db, companySecrets, connectionGrants } from "@geetorusai/db";
import {
...
import { decideCodexAuthMerge } from "@geetorusai/adapter-codex-local/server";
...
import { decideGrokAuthMerge } from "@geetorusai/adapter-grok-local/server";
...
export async function prepareManagedAiRuntime(
  ...
  try {
    ...
    home = await mkdtemp(
      path.join(
        os.tmpdir(),
        `geetorus-ai-${input.companyId}-${selection.grant.id}-`,
      ),
    );
