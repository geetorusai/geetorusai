...
type ToolActionSigningSecretEnv = Partial<
  Record<"GEETORUS_TOOL_ACTION_SIGNING_SECRET" | "GEETORUS_AGENT_JWT_SECRET" | "BETTER_AUTH_SECRET", string | undefined>
>;
...
export class ToolActionSigningSecretMissingError extends Error {
  constructor() {
    super(
      "GEETORUS_TOOL_ACTION_SIGNING_SECRET is not configured; signed tool action approvals cannot be issued. " +
        "Set GEETORUS_TOOL_ACTION_SIGNING_SECRET in this instance's environment (worktrees inherit it from .geetorus/.env).",
    );
...
export function resolveToolActionSigningSecret(env: ToolActionSigningSecretEnv = process.env as ToolActionSigningSecretEnv) {
  const secret = env.GEETORUS_TOOL_ACTION_SIGNING_SECRET?.trim();
  if (!secret) {
