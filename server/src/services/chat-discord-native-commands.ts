...
export interface DiscordNativeCommandInvocation {
  schema: "geetorus.discord.native-command.v1";
...
export function parseDiscordNativeCommand(
  ...
  if (
    ...
    callback.event.command !== `/geetorus ${command}` ||
    callback.event.text !== "" ||
  ...
  if (
    ...
    return null;
  const invocation = {
    schema: "geetorus.discord.native-command.v1" as const,
