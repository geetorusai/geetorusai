...
export function readWorktreeInstanceId(workspacePath: string): string | null {
  const envPath = path.join(path.resolve(workspacePath), ".geetorus", ".env");
  let contents: string;
  try {
  ...
  const instanceId = parseEnvContents(contents).GEETORUS_INSTANCE_ID?.trim();
...
export async function readWorktreeInstancePointer(workspacePath: string): Promise<WorktreeInstancePointer | null> {
  const envPath = path.join(workspacePath, ".geetorus", ".env");
  try {
    return {
      envPath,
...
function resolveConfiguredInstanceRoot(pointer: WorktreeInstancePointer, expectedInstanceId?: string):
  ...
  const configuredHome = env.GEETORUS_HOME?.trim();
  const instanceId = env.GEETORUS_INSTANCE_ID?.trim();
  ...
  if (!INSTANCE_ID_RE.test(instanceId)) {
    return {
      instanceRoot: null,
      warning: `Refusing worktree instance cleanup from ${pointer.envPath}: GEETORUS_INSTANCE_ID is not a safe path segment.`,
  ...
  if (!path.isAbsolute(expandedHome)) {
    return {
      instanceRoot: null,
      warning: `Refusing worktree instance cleanup from ${pointer.envPath}: GEETORUS_HOME is not absolute.`,
  ...
  if (expectedInstanceId && instanceId !== expectedInstanceId) {
    return {
      instanceRoot,
      warning: `Refusing worktree instance cleanup from ${pointer.envPath}: GEETORUS_INSTANCE_ID "${instanceId}" does not match the expected workspace instance "${expectedInstanceId}".`,
...
function resolveManagedInstancesDir(worktreesDir?: string): string {
  const managedWorktreesDir = path.resolve(
    expandHomePrefix(
      worktreesDir?.trim()
      || process.env.GEETORUS_WORKTREES_DIR?.trim()
      || path.join(os.homedir(), ".geetorus-worktrees"),
    ),
  );
