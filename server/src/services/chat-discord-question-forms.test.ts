...
describe("Discord actor-scoped correction drafts", () => {
  it("retains only bounded known values under one opaque actor/form key with parent TTL", async () => {
    ...
    expect(response.geetorusDiscordCorrection.actionId).toMatch(
      /^pcfr:[\w-]{43}$/,
    );
    expect(response.geetorusDiscordCorrection.message).toContain(
      "Release note: Too long",
    );
    expect(response.geetorusDiscordCorrection.message).toContain(
      "shortened to 12",
    );
    expect(response.geetorusDiscordCorrection.message).not.toContain(inputId);
    const draft = await loadDiscordQuestionFormCorrection(
      persistence,
      scope,
      response.geetorusDiscordCorrection.actionId,
      owner,
      threadId,
      now,
    );
    ...
    expect(retried.geetorusDiscordCorrection.actionId).toBe(
      response.geetorusDiscordCorrection.actionId,
    );
