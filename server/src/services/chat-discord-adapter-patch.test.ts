...
describe("Geetorus Discord adapter patch", () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });
  ...
  it("never logs inbound message content before Geetorus admission", async () => {
    const { adapter, chat, client, handlers, logger } = harness();
    await adapter.initialize(chat as never);
    (
      adapter as unknown as {
  ...
  it("checks Geetorus admission before creating a root provider thread", async () => {
  ...
  it("keeps an oversized Markdown attachment byte-for-byte intact", async () => {
    ...
    expect((file as File).name).toBe("geetorus-response.md");
  ...
  it("does not acknowledge an action that Geetorus durably rejects", async () => {
    const { adapter, chat, client, handlers, logger } = harness();
    ...
    expect(interaction.reply).toHaveBeenCalledWith({
      content:
        "This action is no longer available. Open the linked Geetorus task or ask an operator to link this account.",
      flags: 64,
    });
    expect(logger.info).toHaveBeenCalledWith(
      "Discord Gateway action was not acknowledged after Geetorus rejected it",
      expect.objectContaining({
        event: "interaction",
        messageId: "message-2",
      }),
  ...
  it("acknowledges a duplicate that Geetorus reports as already durable", async () => {
    const { adapter, chat, client, handlers } = harness();
    await adapter.initialize(chat as never);
    (
      adapter as unknown as {
