...
import { z } from "zod";
import type { ChatFileTransferPhase } from "@geetorusai/shared";
...
const privateSchema = z
  .object({
    schema: z.literal("geetorus.teams.transfer-private.v1"),
    binding: z.record(z.string(), z.unknown()),
    response: z.record(z.string(), z.unknown()).optional(),
    upload: z.record(z.string(), z.unknown()).optional(),
    resolution: z
      .object({
        schema: z.literal("geetorus.teams.file-resolution.v1"),
    ...
    quarantine: z
      .object({
        schema: z.literal("geetorus.teams.file-quarantine.v1"),
...
export function teamsFileTransferService(
  ...
  async function quarantine(tx: Tx, row: Row, reason: string) {
    const state = privateSchema.parse(row.privateState);
    state.quarantine ??= {
      schema: "geetorus.teams.file-quarantine.v1",
      fromPhase: row.phase,
  ...
  async function issue(input: TeamsFileTransferAuthority, expiresAt: Date) {
    ...
    return db.transaction(async (tx) => {
      ...
      const privateState: PrivateState = {
        schema: "geetorus.teams.transfer-private.v1",
  ...
  async function recordConsent(
    ...
    return db.transaction(async (tx) => {
      ...
      await tx.insert(chatActions).values({
        ...
        status: "processed",
        payload: {
          schema: "geetorus.teams.consent-receipt.v1",
          transferId: row.id,
          publicationId: row.publicationId,
          receiptDigest,
          action: event.action,
  ...
  async function resolveInTransaction(
    ...
    state.resolution = {
      schema: "geetorus.teams.file-resolution.v1",
      action: input.action,
