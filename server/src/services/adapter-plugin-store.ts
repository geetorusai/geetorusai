 ...
 * ~/.geetorus/adapter-plugins.json. This is the source of truth for which
...
import { resolveGeetorusHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-geetorus-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const geetorusDir = resolveGeetorusHomeDir();
  return {
    adapterPluginsDir: path.join(geetorusDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(geetorusDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(geetorusDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "geetorus-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Geetorus external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
