...
export const HEARTBEAT_RUN_SCRATCH_MARKER = ".geetorus-run-scratch.json";
...
export async function prepareHeartbeatRunScratch(input: {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `geetorus-run-${issueSegment}-${runSegment}-`));
...
export function buildHeartbeatRunScratchEnv(
  ...
  const env: Record<string, string> = {
    GEETORUS_RUN_SCRATCH_DIR: scratch.dir,
    GEETORUS_TASK_SCRATCH_DIR: scratch.dir,
    GEETORUS_SCRATCH_DIR: scratch.dir,
    GEETORUS_TMPDIR: scratch.dir,
  };
...
export async function cleanupHeartbeatRunScratch(input: {
  ...
  if (!isPathInside(tmpRoot, dir) || !path.basename(dir).startsWith("geetorus-run-")) {
