...
function invoke(action: "accept" | "decline") {
  return {
    ...
    value: {
      type: "fileUpload",
      action,
      context: { schema: "geetorus.teams.file-consent.v1", token, action },
      ...(action === "accept"
        ? {
            uploadInfo: {
              name: "report.txt",
...
// Geetorus DB authorization. No request may contact any external service.
  ...
  async function harness() {
    ...
    const dispatch = async (
      ...
      adapter.handleWebhook(
        new Request("https://geetorus.test/chat-webhook", {
          method: "POST",
