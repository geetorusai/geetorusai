...
const SCHEMA = "geetorus.teams.file-consent.v1";
const INSTALLED_HOOK = Symbol("geetorus.teams.file-consent.hook");
  ...
  try {
    const prepared = await getSecretProvider("local_encrypted").createSecret({
      value: JSON.stringify({
        schema: "geetorus.teams.file-private.v1",
        context: parsed.data,
        purpose,
        value,
      }),
...
async function openPrivate(
  ...
  try {
    ...
    const envelope = z
      .object({
        schema: z.literal("geetorus.teams.file-private.v1"),
...
export function buildTeamsFileConsentCard(binding: TeamsFileConsentBinding) {
  ...
  return {
    ...
    content: {
      description: "Allow Geetorus to upload this file to your OneDrive.",
      ...
      content: z
        .object({
          description: z.literal(
            "Allow Geetorus to upload this file to your OneDrive.",
          ),
          sizeInBytes: z
            .number()
            .int()
