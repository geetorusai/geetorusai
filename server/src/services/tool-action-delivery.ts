    ...
    return db.transaction(async (tx) => {
      ...
      if (!(await existing())) {
        ...
        while (Buffer.byteLength(JSON.stringify(context), "utf8") > 32_000 && toolActions.length > 1) {
          toolActions.pop();
          context.geetorusAgentMessage.untrustedToolResults.pop();
        ...
        if (Buffer.byteLength(JSON.stringify(context), "utf8") > 32_000) {
          ...
          Object.assign(context.geetorusAgentMessage.untrustedToolResults[0], { resultSummary: "", error: null, declineReason: null });
        }
