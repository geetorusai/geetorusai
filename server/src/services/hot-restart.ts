...
function resolveHotRestartPath(filename: string, homeDir?: string) {
  return path.join(resolveGeetorusInstanceRoot({ homeDir }), filename);
}

function resolveLegacyHotRestartPath(filename: string, homeDir?: string) {
  return path.join(resolveGeetorusHomeDir(homeDir), filename);
}
...
export async function readHotRestartIntent(homeDir?: string) {
  ...
  if (!instanceIntent) {
    ...
    return resolveGeetorusInstanceId() === "default" ? legacyIntent : null;
  }
...
export async function writeHotRestartIntent(input: {
  ...
  // The legacy location is shared by every instance under GEETORUS_HOME.
