...
import type { LoginPtySessionOpener } from "@geetorusai/adapter-utils/login-pty-transport";
import {
...
describe("worker-bound live pseudo-terminal opener", () => {
  it("resolves the lease to its provider lease id and drives the worker route gate", async () => {
    ...
    const getLeaseById = vi.fn(async () => ({
      ...
      metadata: { pluginId: "geetorus.daytona", provider: "daytona" },
    }));
    ...
    expect(pluginId).toBe("geetorus.daytona");
    ...
    expect(openInput.sessionHome).toMatch(
      /^\/tmp\/geetorus-adapter-login\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
    );
...
const DIAGNOSTIC_ALLOWLIST = new Set<string>([
  "[geetorus] Setup-token login: the process stop step errored.",
  "[geetorus] Setup-token login: the driver dispose step errored.",
  "[geetorus] Setup-token login: the code input step errored.",
  "[geetorus] Setup-token login: sent the browser code to the prompt.",
  "[geetorus] Setup-token login: delivered the credential to the sink.",
  "[geetorus] Setup-token login: the credential delivery step errored.",
  "[geetorus] Setup-token login cancelled before start.",
  "[geetorus] Setup-token login timed out; stopping the process.",
  "[geetorus] Setup-token login cancelled; stopping the process.",
  "[geetorus] Setup-token login command ended with a non-zero exit code.",
  "[geetorus] Setup-token login: the credential did not land; treating the run as a failure.",
  "[geetorus] Setup-token login: surfaced the sign-in prompt.",
  "[geetorus] Setup-token login command ended successfully.",
]);
...
describe("setup-token production transport binding diagnostics", () => {
  it("forwards only allowlisted, non-secret diagnostics on a success path", async () => {
    ...
    expect(captured).toContain("[geetorus] Setup-token login: surfaced the sign-in prompt.");
    expect(captured).toContain(
      "[geetorus] Setup-token login: delivered the credential to the sink.",
    );
    expect(captured).toContain("[geetorus] Setup-token login command ended successfully.");
    // The runner sent the browser code, so the code-input path ran. The code
    // still never reaches a log line.
    expect(captured).toContain("[geetorus] Setup-token login: sent the browser code to the prompt.");
