...
import { agents, companyOnboardingSeeds, goals, issues, projects } from "@geetorusai/db";
import type { ApplyOnboardingSeed } from "@geetorusai/shared";
import { writeGeetorusSkillSyncPreference } from "@geetorusai/adapter-utils/server-utils";
import { findActiveServerAdapter } from "../adapters/registry.js";
import { agentService } from "./agents.js";
import { GEETORUS_CORE_SKILL_KEYS } from "./company-skills.js";
...
function seededAgentAdapterType() {
  const configured = process.env.GEETORUS_ONBOARDING_SEED_ADAPTER_TYPE?.trim()
    || process.env.GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE?.trim()
  ...
  return configured === "geetorus_runner"
    ? FALLBACK_SEEDED_AGENT_ADAPTER_TYPE
    : configured;
}
 ...
 * agent to use the core geetorus skills (hiring, memory, coordination), and
...
function seededAgentAdapterConfig(adapterType: string): Record<string, unknown> {
  ...
  return writeGeetorusSkillSyncPreference(
    {},
    GEETORUS_CORE_SKILL_KEYS.map((key) => ({ key, versionId: null })),
  );
}
...
export function onboardingSeedService(db: Db) {
  ...
  async function applyWithin(
    ...
    //    `firstTask` are unpopulated by the signup wizard and a geetorus-cloud
  ...
  async function apply(
    ...
    const result = await db.transaction(async (tx) => {
      await tx.execute(
        sql`select pg_advisory_xact_lock(hashtextextended(${`geetorus:onboarding-seed:${companyId}`}, 0))`,
      );
