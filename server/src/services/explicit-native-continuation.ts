...
import { completeTerminatedRemoteNativeSessionCleanup } from "../vendor/geetorus-runner/index.js";
...
export async function admitExplicitNativeContinuation(input: {
  ...
  if (!agent || (!isConversationAdapter(agent.adapterType) && agent.adapterType !== "geetorus_runner")) return null;
    ...
    if (remote) {
      ...
      if (!unusedAdmission && !cancelledStartup) {
        ...
        if (!run.processPid && !run.processGroupId &&
            ...
            !await hasHistoricalSuspendedNativeSession(db, run)) return blocked("process_identity_missing", "The previous run has no verified stop record. Geetorus cannot start this message yet.");
