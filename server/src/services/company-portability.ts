...
import { sha256HexOfBytes } from "@geetorusai/shared/portability-hash";
import {
  readGeetorusSkillSyncPreference,
  writeGeetorusSkillSyncPreference,
} from "@geetorusai/adapter-utils/server-utils";
import { requireOpenCodeModelId } from "@geetorusai/adapter-opencode-local/server";
...
import {
  GeetorusRunnerProviderProfileError,
  resolveGeetorusRunnerProviderProfile,
...
// their schemaVersion in the .geetorus.yaml extension at 6; undeclared
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".geetorus.yaml" || normalized === ".geetorus.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const geetorus = isPlainRecord(metadata?.geetorus) ? metadata?.geetorus as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.geetorusSkillKey)
    ?? asString(geetorus?.skillKey)
    ?? asString(geetorus?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "geetorus_bundled") {
    return `geetorusai/geetorus/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "geetorus" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .geetorus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .geetorus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .geetorus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .geetorus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Geetorus will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .geetorus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .geetorus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[geetorusExtensionPath];
  if (selectedFiles.has(geetorusExtensionPath) && typeof extensionEntry === "string") {
    filtered[geetorusExtensionPath] = filterPortableExtensionYaml(
      extensionEntry,
      selectedFiles,
      filtered,
      geetorusExtensionPath,
    );
  }
...
function findGeetorusExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".geetorus.yaml"] === "string") return ".geetorus.yaml";
  if (typeof files[".geetorus.yml"] === "string") return ".geetorus.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.geetorus.yaml") || entry.endsWith("/.geetorus.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "geetorus_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "geetorusai/geetorus",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/geetorusai/geetorus/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  // Bundled Geetorus skills ship with every build and may contain executable
  ...
  if (asString(metadata?.sourceKind) === "geetorus_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.geetorusSkillKey = skill.key;
  metadata.geetorus = {
    ...(isPlainRecord(metadata.geetorus) ? metadata.geetorus : {}),
    skillKey: skill.key,
    slug: skill.slug,
...
function buildManifestFromPackageFiles(
  ...
  const geetorusExtensionPath = findGeetorusExtensionPath(normalizedFiles);
  const geetorusExtension = geetorusExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, geetorusExtensionPath) ?? "")
    : {};
  const declaredSchemaVersion = asInteger(geetorusExtension.schemaVersion);
  ...
  if (bundleSchemaVersion > BUNDLE_SCHEMA_VERSION) {
    throw unprocessable(`Company package declares schemaVersion ${bundleSchemaVersion}, which was produced by a newer Geetorus; this board reads up to schemaVersion ${BUNDLE_SCHEMA_VERSION}.`);
  }
  const geetorusCompany = isPlainRecord(geetorusExtension.company) ? geetorusExtension.company : {};
  const geetorusSidebar = normalizePortableSidebarOrder(geetorusExtension.sidebar);
  const geetorusLabels = normalizePortableLabelDefinitions(geetorusExtension.labels);
  const geetorusBlobs = normalizePortableBlobIndex(geetorusExtension.blobs);
  const geetorusEmbeddedAssets = normalizePortableEmbeddedAssets(geetorusExtension.embeddedAssets);
  const geetorusAgents = isPlainRecord(geetorusExtension.agents) ? geetorusExtension.agents : {};
  const geetorusProjects = isPlainRecord(geetorusExtension.projects) ? geetorusExtension.projects : {};
  const geetorusTasks = isPlainRecord(geetorusExtension.tasks) ? geetorusExtension.tasks : {};
  const geetorusRoutines = isPlainRecord(geetorusExtension.routines) ? geetorusExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      logoPath: asString(geetorusCompany.logoPath) ?? asString(geetorusCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof geetorusCompany.requireBoardApprovalForNewAgents === "boolean"
          ? geetorusCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof geetorusCompany.feedbackDataSharingEnabled === "boolean"
          ? geetorusCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof geetorusCompany.feedbackDataSharingConsentAt === "string"
          ? geetorusCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(geetorusCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(geetorusCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: geetorusSidebar,
    labels: geetorusLabels,
    blobs: geetorusBlobs,
    embeddedAssets: geetorusEmbeddedAssets,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(geetorusAgents[slug]) ? geetorusAgents[slug] : {};
  ...
  for (const skillPath of skillPaths) {
    ...
    if (sourceKind === "github-dir" || sourceKind === "github-file") {
      ...
      const normalizedSourceKind = owner === "geetorusai"
        && repoName === "geetorus"
        && canonicalKey?.startsWith("geetorusai/geetorus/")
        ? "geetorus_bundled"
        : "github";
      sourceType = "github";
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(geetorusProjects[slug]) ? geetorusProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(geetorusTasks[slug]) ? geetorusTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(geetorusRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(geetorusRoutines[slug]) ? geetorusRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.GEETORUS_SECRETS_STRICT_MODE === "true";
  ...
  async function assertImportAdapterConfigConstraints(
    ...
    if (adapterType === "geetorus_runner") {
      let profile;
      try {
        profile = resolveGeetorusRunnerProviderProfile(adapterConfig);
      } catch (error) {
        if (error instanceof GeetorusRunnerProviderProfileError) {
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeGeetorusSkillSyncPreference(
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".geetorus.yaml" ||
          relative === ".geetorus.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const geetorusAgentsOut: Record<string, Record<string, unknown>> = {};
    const geetorusProjectsOut: Record<string, Record<string, unknown>> = {};
    const geetorusTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const geetorusRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      ...
      for (const agent of agentRows) {
        ...
        const desiredSkills = readGeetorusSkillSyncPreference(
        ...
        geetorusAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      geetorusProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      geetorusTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      geetorusRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const extension of Object.values(geetorusTasksOut)) {
    ...
    const geetorusExtensionPath = ".geetorus.yaml";
    const exportedBlobIndex = Array.from(exportedBlobs.values())
      .sort((left, right) => left.sha256.localeCompare(right.sha256));
    const geetorusAgents = Object.fromEntries(
      Object.entries(geetorusAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const geetorusProjects = Object.fromEntries(
      Object.entries(geetorusProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const geetorusTasks = Object.fromEntries(
      Object.entries(geetorusTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const geetorusRoutines = Object.fromEntries(
      Object.entries(geetorusRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[geetorusExtensionPath] = buildYamlFile(
      {
        schema: "geetorus/v1",
        ...
        agents: Object.keys(geetorusAgents).length > 0 ? geetorusAgents : undefined,
        projects: Object.keys(geetorusProjects).length > 0 ? geetorusProjects : undefined,
        tasks: Object.keys(geetorusTasks).length > 0 ? geetorusTasks : undefined,
        routines: Object.keys(geetorusRoutines).length > 0 ? geetorusRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, geetorusExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      ...
      if (
        ...
        throw unprocessable(
          "Geetorus Runner is experimental and disabled on this instance.",
          { code: "geetorus_runner_rollout_disabled" },
        );
      }
    }
