...
import { authUsers, companyMemberships, type Db } from "@geetorusai/db";
import type { WorkspaceReadiness, WorkspaceReadinessProbeResult } from "@geetorusai/shared";
import {
 ...
 *
 * The worktree's `.geetorus/.env` pointer is authoritative because it is what
 ...
 * construction for anything Geetorus provisioned.
 */
...
export function resolveWorkspaceReadinessGateMode(
  ...
  return env.GEETORUS_WORKSPACE_READINESS_GATE?.trim().toLowerCase() === "strict" ? "strict" : "auto";
}
