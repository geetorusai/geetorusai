...
import { companySecrets, companySecretVersions, environmentLeases, heartbeatRuns } from "@geetorusai/db";
import type {
  Environment,
  EnvironmentLease,
...
import { resolveDeclaredSandboxCapabilities } from "@geetorusai/shared";
import type { EffectiveExecutionCapabilities } from "@geetorusai/adapter-utils/execution-target";
import type { RunnerIngressEndpoint } from "@geetorusai/adapter-utils/runner-connectivity";
import type {
  CommandManagedDuplexChannel,
} from "@geetorusai/adapter-utils/command-managed-runtime";
import type {
...
import { ensureSshWorkspaceReady } from "@geetorusai/adapter-utils/ssh";
import {
    ...
    async acquireRunLease(input) {
      ...
      if (!isBuiltinSandboxProvider(parsed.config.provider)) {
        ...
        if (reusableLease?.providerLeaseId) {
          ...
          if (!providerLease) {
            if (
              input.adapterType === "geetorus_runner" &&
        ...
        try {
          ...
          const lease = providerResourceDisposition === "destroy" && driver?.destroyRunLease
            ? await driver.destroyRunLease({
                environment,
                lease: leaseSnapshot,
                failureReason: "geetorus_runner_destroy_after_turn",
              })
            : driver
