...
import { agents, builtInManagedResources, companies, issueThreadInteractions, issues, routines, routineTriggers } from "@geetorusai/db";
import { syncRoutineVariablesWithTemplate } from "@geetorusai/shared";
import type { Agent, Approval, CompanySkill, PermissionKey, Routine, RoutineTrigger, RoutineVariable } from "@geetorusai/shared";
...
const FALLBACK_REFLECTION_COACH_INSTRUCTIONS = [
  "# Reflection Coach",
  "",
  "You are Geetorus's built-in Reflection Coach.",
  ...
  "Do not apply changes in the same run. Present a reviewable diff and wait for the required Geetorus issue-thread approval before any follow-up applies it.",
  "",
].join("\n");
...
const FALLBACK_REFLECTION_COACH_SKILL = [
  ...
  "key: geetorusai/bundled/geetorus-operations/reflection-coach",
  "---",
  "",
  "# Reflection Coach",
  "",
...
const FALLBACK_SUMMARIZER_INSTRUCTIONS = [
  "You are Summarizer, a built-in reporting agent at Geetorus.",
  "",
  "Turn the current state of a Geetorus scope (project, workspaces overview, project workspace, or execution workspace) into a short, honest, human-readable Markdown summary and write it back to that scope's summary slot as a new revision. Use the `summarize-status` skill as your operating procedure.",
  "",
...
export function readBuiltInTextWithFallback(
    ...
    try {
      ...
      if (code !== "ENOENT") {
        ...
        if (!warnedBuiltInTextReadErrors.has(warningKey)) {
          ...
          console.warn(
            "[geetorus] Built-in agent asset " + label + " read error on " + candidatePath + ": " + codeLabel,
          );
        }
      }
  ...
  if (!warnedBuiltInTextFallbacks.has(label)) {
    ...
    console.warn(
      `[geetorus] Built-in agent asset ${label} was not readable; using bundled fallback text. `
...
const skillsCatalogRoot = resolvePackageRoot("@geetorusai/skills-catalog");
...
const REFLECTION_COACH_SKILL = readBuiltInTextWithFallback(
  ...
  [
    path.resolve(
      moduleDir,
      "../../../packages/skills-catalog/catalog/bundled/geetorus-operations/reflection-coach/SKILL.md",
    ),
    ...(skillsCatalogRoot
      ? [path.join(skillsCatalogRoot, "catalog/bundled/geetorus-operations/reflection-coach/SKILL.md")]
      : []),
  ],
...
const SUMMARIZER_SKILL = readBuiltInTextWithFallback(
  ...
  [
    path.resolve(
      moduleDir,
      "../../../packages/skills-catalog/catalog/bundled/geetorus-operations/summarize-status/SKILL.md",
    ),
    ...(skillsCatalogRoot
      ? [path.join(skillsCatalogRoot, "catalog/bundled/geetorus-operations/summarize-status/SKILL.md")]
      : []),
  ],
...
const DEFINITIONS = validateBuiltInAgentDefinitions([
  {
    ...
    defaultInstructions:
      "You are Geetorus's built-in Briefs agent. Produce concise, sourced operational briefs that help the board understand current company work, risks, and next actions.",
    defaultRole: "general",
  ...
  },
  {
    ...
    defaultInstructions:
      "You are Geetorus's built-in Learning agent. Extract durable lessons from completed work, preserve useful patterns, and keep learning artifacts grounded in source context.",
    defaultRole: "general",
    allowedAdapterTypes: ["codex_local", "claude_local", "gemini_local", "opencode_local", "process"],
    defaultBudgetMonthlyCents: 0,
  },
  {
    ...
    bundle: {
      ...
      skill: {
        ...
        canonicalKey: "geetorusai/bundled/geetorus-operations/reflection-coach",
        files: {
  ...
  },
  {
    ...
    bundle: {
      ...
      skill: {
        ...
        canonicalKey: "geetorusai/bundled/geetorus-operations/summarize-status",
        files: {
...
export function builtInAgentService(db: Db) {
  ...
  async function syncBundledSkillToAgent(agent: Agent, skill: CompanySkill) {
    const desired = readGeetorusSkillSyncPreference(agent.adapterConfig as Record<string, unknown>).desiredSkillEntries;
    const nextDesired = [
      ...desired.filter((entry) => entry.key !== skill.key),
      { key: skill.key, versionId: skill.currentVersionId ?? null },
    ];
    const adapterConfig = writeGeetorusSkillSyncPreference(agent.adapterConfig as Record<string, unknown>, nextDesired);
