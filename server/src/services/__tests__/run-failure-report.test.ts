...
import { agents, companies, createDb, heartbeatRuns, type Db } from "@geetorusai/db";
import {
...
describeEmbeddedPostgres("reportRunFailure", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-run-failure-report-");
  ...
  it("removes a database connection string from the error message", async () => {
    ...
    const run = buildRun({
      status: "failed",
      error: `connect failed: connectionString: "postgres://appuser:s3cr3t-pass@db.internal:5432/geetorus"`,
    });
