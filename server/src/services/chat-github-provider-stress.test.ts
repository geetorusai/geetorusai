...
function signedGitHubRequest(
  ...
  return new Request("https://geetorus.example/github", {
    method: "POST",
    headers: {
...
function commentPayload(input: {
  ...
  return {
    ...
    comment: {
      ...
      html_url: `https://github.com/geetorusai/chat-e2e/issues/${input.number}#issuecomment-${input.commentId}`,
      user: {
        id: userId,
        login: userId === 9001 ? "maya-geetorus[bot]" : "alex-e2e",
    ...
    repository: {
      id: 97531,
      name: "chat-e2e",
      full_name: "geetorusai/chat-e2e",
      owner: { id: 1357, login: "geetorusai" },
    },
    sender: {
      id: userId,
      login: userId === 9001 ? "maya-geetorus[bot]" : "alex-e2e",
...
describe("GitHub published adapter stress contract", () => {
  ...
  it("normalizes issue, PR, inline, and subscribed follow-up boundaries and suppresses the bot's own event", async () => {
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        return new Response(
          JSON.stringify({
            id: 1,
            content: "+1",
            user: { id: 9001, login: "maya-geetorus[bot]" },
          }),
    ...
    const selfEvent = commentPayload({
      body: "@maya-geetorus outbound self event",
      commentId: 4203,
      number: 42,
      senderId: 9001,
    });
    ...
    expect(
      ...
      {
        id: "4201",
        threadId: "github:geetorusai/chat-e2e:issue:42",
        trigger: "mention",
      },
      {
        id: "4301",
        threadId: "github:geetorusai/chat-e2e:43",
        trigger: "mention",
      },
      {
        id: "4401",
        threadId: "github:geetorusai/chat-e2e:43:rc:4401",
        trigger: "mention",
      },
      {
        id: "4202",
  ...
  it("posts and edits inline output through the review-thread API boundary", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (
          method === "POST" &&
          url.endsWith(
            "/repos/geetorusai/chat-e2e/pulls/43/comments/4401/replies",
          )
        ) {
        ...
        if (
          method === "PATCH" &&
          url.endsWith("/repos/geetorusai/chat-e2e/pulls/comments/9901")
        ) {
          return Response.json({
            id: 9901,
    ...
    try {
      const sent = await runtime
        .thread("github:geetorusai/chat-e2e:43:rc:4401")
      ...
      expect(providerRequests).toEqual([
        {
          method: "POST",
          url: "https://api.github.com/repos/geetorusai/chat-e2e/pulls/43/comments/4401/replies",
        },
        {
          method: "PATCH",
          url: "https://api.github.com/repos/geetorusai/chat-e2e/pulls/comments/9901",
        },
      ]);
    } finally {
      await runtime.shutdown();
  ...
  it("keeps stable identities across reordered and exactly duplicated raw deliveries", async () => {
    ...
    const newer = commentPayload({
      body: "@maya-geetorus newer comment delivered first",
      commentId: 5102,
      number: 51,
    });
    const older = commentPayload({
      body: "@maya-geetorus older root delivered late",
      commentId: 5101,
      number: 51,
    });
    try {
      ...
      expect(
        ...
        {
          id: "5102",
          threadId: "github:geetorusai/chat-e2e:issue:51",
        },
        {
          id: "5101",
          threadId: "github:geetorusai/chat-e2e:issue:51",
