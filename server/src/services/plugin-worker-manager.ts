...
import { getActiveStepContext } from "@geetorusai/adapter-utils/acpx-engine/startup-timing";
import {
  isLoginCommandKey,
...
export interface LoginPtyOpenInput {
   ...
   * `/tmp/geetorus-adapter-login/<uuid>`.
   */
  sessionHome: string;
}
...
export function createPluginWorkerHandle(
  ...
  // carry no `geetorusInvocationId`, so the governed-access gate
  ...
  // the worker-supplied `geetorusInvocationId` alone as proof of origin. The
  ...
  function routeExecuteLogNotification(notification: JsonRpcNotification): void {
    const invocationId = readNonEmptyString(
      (notification as { geetorusInvocationId?: unknown }).geetorusInvocationId,
    );
  ...
  function contextForWorkerMessage(message: JsonRpcRequest | JsonRpcNotification): WorkerHostCallContext {
    const invocationId = readNonEmptyString(
      (message as { geetorusInvocationId?: unknown }).geetorusInvocationId,
    );
    if (!invocationId) {
