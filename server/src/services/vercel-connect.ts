...
export function vercelConnectFailureMessage(code: VercelConnectFailureCode): string {
  switch (code) {
    case "vercel_connect_unavailable":
      return "Vercel Connect is not configured on this Geetorus instance.";
    case "vercel_connect_auth_failed":
      return "Geetorus could not authenticate to Vercel Connect. Repair or refresh the instance's Vercel authority.";
    ...
    case "vercel_connect_installation_required":
      return "This connector must be installed or attached in Vercel before Geetorus can use it.";
    default:
...
export function vercelConnectIntegrationStatus(env: NodeJS.ProcessEnv = process.env): {
  ...
  const integrationEnabled = enabled(env.GEETORUS_VERCEL_CONNECT_ENABLED);
  const hasAccessToken = Boolean(env.GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN?.trim());
 ...
 * Geetorus's local board commonly runs on 127.0.0.1, which is the same
...
export function vercelConnectSdkOptions(
  ...
  const vercelToken = hasWorkloadOidc
    ? undefined
    : env.GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN?.trim();
  return {
...
export function deriveVercelConnectSubject(input: {
  ...
  const instanceId = process.env.GEETORUS_INSTANCE_ID
    ?? process.env.GEETORUS_DEPLOYMENT_ID
    ?? "geetorus-instance";
