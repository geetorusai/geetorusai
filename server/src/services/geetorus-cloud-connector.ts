...
export type GeetorusCloudConnectorEnvironment = "development" | "staging" | "production";
export type GeetorusCloudConnectorOperation = "status" | "session" | "claim" | "refresh" | "revoke" | "webhook-bind" | "event-lease" | "event-ack";
export type GeetorusCloudConnectorProfileId = GoogleWorkspaceConnectorProfileId | GitHubConnectorProfileId;
export type GeetorusCloudConnectorProvider = "google" | "github";

export type GeetorusCloudConnectorConfig = {
  baseUrl: string;
  instanceId: string;
  environment: GeetorusCloudConnectorEnvironment;
...
export type SealedConnectorCredentials = {
  ...
  instanceId: string;
  environment: GeetorusCloudConnectorEnvironment;
  provider: GeetorusCloudConnectorProvider;
  profile: string;
  appSlug?: string;
};
...
type SealedEnvelope = {
  ...
  provider: GeetorusCloudConnectorProvider;
  profile: GeetorusCloudConnectorProfileId;
  epk: string;
  iv: string;
  ct: string;
};
...
const ENDPOINTS: Record<GeetorusCloudConnectorOperation, string> = {
...
const JWS_TYP = "geetorus-cloud-connector-request+jwt";
...
export class GeetorusCloudConnectorError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "GeetorusCloudConnectorError";
  }
}

export function geetorusCloudConnectorConfigFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): GeetorusCloudConnectorConfig | null {
  const localIdentity = loadGeetorusCloudConnectorIdentity();
  const legacyConfigured = [
    env.GEETORUS_ID_CONNECTOR_INSTANCE_ID,
    env.GEETORUS_ID_CONNECTOR_SIGN_PRIVATE_KEY,
    env.GEETORUS_ID_CONNECTOR_SEAL_PRIVATE_KEY,
    env.GEETORUS_ID_CONNECTOR_ENVIRONMENT,
    env.GEETORUS_ID_CONNECTOR_BASE_URL,
  ].some((value) => Boolean(value?.trim()));
  const managedInstanceId = env.GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID?.trim();
  const managedSignPrivateKey = env.GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY?.trim();
  const managedSealPrivateKey = env.GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY?.trim();
  const managedEnvironment = env.GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT?.trim();
  const hasManagedIdentityOverride = [managedInstanceId, managedSignPrivateKey, managedSealPrivateKey]
    .some(Boolean);
  const localStatus = hasManagedIdentityOverride ? null : geetorusCloudConnectorEnrollmentStatus(env);
  ...
  if (!hasManagedIdentityOverride && !hasActiveLocalIdentity && legacyConfigured) {
    throw new GeetorusCloudConnectorError(
      "Geetorus ID connector settings use an incompatible legacy protocol; enroll this instance with Geetorus Cloud",
  ...
  const baseUrl = env.GEETORUS_CLOUD_CONNECTOR_BASE_URL?.trim()
    || (hasActiveLocalIdentity ? localIdentity!.brokerBaseUrl : undefined)
    || "https://my.geetorus.app";
  const values = [instanceId, signPrivateKey, sealPrivateKey, environment];
  if (values.some((value) => !value)) {
    throw new GeetorusCloudConnectorError("Geetorus Cloud connector configuration is incomplete", "CONNECTOR_CONFIG_INCOMPLETE");
  }
  if (environment !== "development" && environment !== "staging" && environment !== "production") {
    throw new GeetorusCloudConnectorError("Geetorus Cloud connector environment is invalid", "CONNECTOR_CONFIG_INVALID");
  }
  const parsedBaseUrl = new URL(baseUrl);
  if (parsedBaseUrl.protocol !== "https:" && !(parsedBaseUrl.protocol === "http:" && isLoopback(parsedBaseUrl.hostname))) {
    throw new GeetorusCloudConnectorError("Geetorus Cloud connector URL must use HTTPS", "CONNECTOR_CONFIG_INVALID");
  }
  if (parsedBaseUrl.username || parsedBaseUrl.password || parsedBaseUrl.search || parsedBaseUrl.hash) {
    throw new GeetorusCloudConnectorError("Geetorus Cloud connector URL is invalid", "CONNECTOR_CONFIG_INVALID");
  }
  const brokerHost = parsedBaseUrl.hostname.toLowerCase();
  if ((brokerHost === "my.geetorus.app" && environment !== "production")
    || (brokerHost === "my-staging.geetorus.app" && environment !== "staging")) {
    throw new GeetorusCloudConnectorError(
      "Geetorus Cloud connector broker and environment do not match",
...
export function createGeetorusCloudConnector(input: {
  config: GeetorusCloudConnectorConfig;
  ...
  async function call(
    operation: GeetorusCloudConnectorOperation,
    claims: { subject: string; companyId: string; profile?: GeetorusCloudConnectorProfileId; returnUri?: string; returnState?: string; claimId?: string; redemptionId?: string },
    ...
    try {
      ...
      throw new GeetorusCloudConnectorError("Geetorus Cloud connector is unavailable", "CONNECTOR_UNAVAILABLE");
    }
    if (operation === "revoke" && response.status === 204) return {};
    if (!response.ok) {
      throw new GeetorusCloudConnectorError(
        "Geetorus Cloud connector rejected the request",
    ...
    try {
      ...
      throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid response", "CONNECTOR_BAD_RESPONSE");
    }
  }
  ...
  function openCredentials(
    ...
    if (
      ...
      throw new GeetorusCloudConnectorError("Geetorus Cloud credential binding did not match", "CONNECTOR_BINDING_MISMATCH");
    }
    if (credentials.profile !== profile) {
      throw new GeetorusCloudConnectorError("Geetorus Cloud connector profile binding did not match", "CONNECTOR_BINDING_MISMATCH");
    }
    if (!sameStringSet(credentials.scopes, definition.scopes)) {
      throw new GeetorusCloudConnectorError("Geetorus Cloud scope grant did not match", "REAUTHORIZATION_REQUIRED");
    }
    return credentials;
  }
  ...
  return {
    async getInstanceStatus(): Promise<"active" | "suspended" | "removed"> {
      ...
      try {
        ...
        if (error instanceof GeetorusCloudConnectorError && error.status === 401) return "removed";
        throw error;
      }
      ...
      throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid instance status", "CONNECTOR_BAD_RESPONSE");
    },
    async getCapabilities(): Promise<GeetorusCloudConnectorProfileId[]> {
      let response: ConnectorResponse;
      try {
        response = await call("status", {
      ...
      return [...new Set(response.profiles.flatMap((value) =>
        typeof value === "string" && isGeetorusCloudConnectorProfileId(value) ? [value] : []
      ))];
    },
    async startAuthorization(values: { subject: string; companyId: string; profile?: GeetorusCloudConnectorProfileId; returnUri: string; returnState: string }) {
      ...
      if (typeof response.confirmationUrl !== "string" || typeof response.expiresAt !== "string") {
        throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid session", "CONNECTOR_BAD_RESPONSE");
      }
      let confirmationUrl: URL;
      try {
        confirmationUrl = new URL(response.confirmationUrl);
      } catch {
        throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid confirmation URL", "CONNECTOR_BAD_RESPONSE");
      }
      ...
      if (response.authorizationUrl !== undefined) {
        if (typeof response.authorizationUrl !== "string") {
          throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid provider URL", "CONNECTOR_BAD_RESPONSE");
        }
        try {
    ...
    async claim(values: { subject: string; companyId: string; profile?: GeetorusCloudConnectorProfileId; claimId: string; redemptionId: string }) {
      const profile = values.profile ?? "gmail.draft";
    ...
    async refresh(values: { subject: string; companyId: string; profile?: GeetorusCloudConnectorProfileId; refreshToken: string }) {
      const profile = values.profile ?? "gmail.draft";
    ...
    async revoke(values: { subject: string; companyId: string; profile?: GeetorusCloudConnectorProfileId; token: string }) {
    ...
    async leaseEvents(values: { subject: string; companyId: string }): Promise<{ leaseId: string; events: SealedConnectorEvents["events"] } | null> {
      ...
      if (typeof response.leaseId !== "string") {
        throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid event lease", "CONNECTOR_BAD_RESPONSE");
      }
      ...
      if (opened.leaseId !== response.leaseId) {
        throw new GeetorusCloudConnectorError("Geetorus Cloud connector event lease did not match", "CONNECTOR_BINDING_MISMATCH");
      }
    ...
    async acknowledgeEvents(values: { subject: string; companyId: string; leaseId: string; deliveryIds: string[] }): Promise<number> {
      ...
      if (typeof acknowledged !== "number" || !Number.isSafeInteger(acknowledged) || acknowledged < 0) {
        throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid acknowledgement", "CONNECTOR_BAD_RESPONSE");
      }
      return acknowledged;
    },
  };
...
function parseCloudHandoff(value: unknown): { kind: "geetorus_cloud"; session: string } | undefined {
  if (value === undefined) return undefined;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid handoff", "CONNECTOR_BAD_RESPONSE");
  }
  ...
  return { kind: "geetorus_cloud", session };
}

export type GeetorusCloudConnector = ReturnType<typeof createGeetorusCloudConnector>;
export type GeetorusCloudGoogleWorkspaceConnector = GeetorusCloudConnector;

/** Accept persisted Geetorus ID-era records while all new records use the Cloud strategy. */
export function isGeetorusCloudConnectorStrategy(value: unknown): boolean {
  return value === "geetorus_cloud_connector" || value === "geetorus_id_connector";
}

/** Use the same signed instance profiles for catalog display and setup validation. */
export function appWithGeetorusCloudConnectorAvailability(
  app: AppDefinition,
  ...
  return {
    ...
    ownershipAvailability: {
      ...
      platform_shared: methods.some((method) => isGeetorusCloudConnectorStrategy(method.oauthStrategy)),
    },
  };
}

let capabilityCache: { key: string; expiresAt: number; profiles: GeetorusCloudConnectorProfileId[] } | null = null;
let capabilityCacheGeneration = 0;

export function invalidateGeetorusCloudConnectorCapabilities(): void {
  capabilityCacheGeneration += 1;
  capabilityCache = null;
}

export async function geetorusCloudConnectorCapabilitiesFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): Promise<GeetorusCloudConnectorProfileId[]> {
  let config: GeetorusCloudConnectorConfig | null;
  try {
    config = geetorusCloudConnectorConfigFromEnv(env);
  } catch (error) {
    ...
    if (error instanceof GeetorusCloudConnectorError) return [];
    throw error;
  }
  if (!config) return [];
  ...
  const connector = createGeetorusCloudConnector({ config });
...
function privateKey(value: string, curve: "ed25519" | "x25519"): KeyObject {
  try {
    ...
    throw new GeetorusCloudConnectorError(`Geetorus Cloud ${curve} private key is invalid`, "CONNECTOR_CONFIG_INVALID");
  }
}
...
function unseal(
  ...
  try {
    ...
    if (error instanceof GeetorusCloudConnectorError) throw error;
    throw badEnvelope();
  }
}
...
function badEnvelope() {
  return new GeetorusCloudConnectorError("Geetorus Cloud connector returned an invalid sealed credential", "CONNECTOR_BAD_RESPONSE");
}

function sealPurpose(
  kind: "initial" | "access",
  _profile: GeetorusCloudConnectorProfileId,
): SealedEnvelope["purpose"] {
  return kind;
}

function connectorProfileDefinition(profile: GeetorusCloudConnectorProfileId): {
  provider: GeetorusCloudConnectorProvider;
  scopes: readonly string[];
} {
...
function isGeetorusCloudConnectorProfileId(value: string): value is GeetorusCloudConnectorProfileId {
