...
import { companySecretBindings, toolConnections } from "@geetorusai/db";
import type { ToolCredentialSecretRef } from "@geetorusai/shared";
...
export function createComposioSessionManager(db: Db, options: ComposioSessionManagerOptions = {}) {
  ...
  async function mint(
    ...
    const session = await client.createSession(`geetorus:${child.companyId}`, {
      mcp: true,
