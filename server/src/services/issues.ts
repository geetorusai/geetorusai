...
import type { ExecutionProjection } from "@geetorusai/shared";
...
function wakeRequestTargetsIssue(issueId: string) {
  return sql`(
    ...
    or ${agentWakeupRequests.payload} -> '_geetorusWakeContext' ->> 'issueId' = ${issueId}
    or ${agentWakeupRequests.payload} -> '_geetorusWakeContext' ->> 'taskId' = ${issueId}
  )`;
}
...
async function freshChatSourceAfterPublishedControl(
  ...
  try {
    ...
    if (
      ...
      /^geetorus_runner_chat_attachment_(binding|destination|principal)_denied$/.test(
        error.message,
      )
    )
      return false;
...
export function isExplicitExternalAgentComment(
  ...
  return reason === "geetorus_runner_protocol" || reason.startsWith("allow_");
}
  ...
  const rows = await dbOrTx
    ...
    .where(
      and(
        ...
        isNull(issueComments.deletedAt),
        sql<boolean>`(
          coalesce(${issueComments.metadata}->>'authorizationReason', '') = 'geetorus_runner_protocol'
...
const LEGACY_PLUGIN_OPERATION_ORIGIN_KINDS = [
  "plugin:geetorusai.content-machine:case",
  "plugin:geetorusai.content-machine:evaluation",
  "plugin:geetorusai.content-machine:source-sync",
] as const;
    ...
    dbOrTx
      .select({
        ...
        issueId: sql<string | null>`coalesce(
          ...
          ${agentWakeupRequests.payload} -> '_geetorusWakeContext' ->> 'issueId',
          ${agentWakeupRequests.payload} -> '_geetorusWakeContext' ->> 'taskId'
        )`,
...
export function issueService(db: Db) {
  ...
  const service = {
      ...
      if (nodeIds.length > 0) {
        ...
        const wakeTargetIssueIdSql = sql<string>`
          coalesce(
            ...
            wake.payload ->> 'taskId',
            wake.payload -> '_geetorusWakeContext' ->> 'issueId',
            wake.payload -> '_geetorusWakeContext' ->> 'taskId'
          )
        `;
    ...
    create: async (
      ...
      const persist = async (tx: DbTransaction) => {
        ...
        if (workspaceInheritanceIssueId) {
          ...
          // cross-project child (for example, a Geetorus ID issue created from
          // a Geetorus App parent) must fall through to its own project's
    ...
    addComment: async function addComment(
      ...
      // Geetorus even while a bound conversation is active.
      if (authorType === "agent" && isExplicitExternalAgentComment(metadata)) {
        // An external-chat run may perform ordinary Geetorus lifecycle or
