...
import { companyMemberships, decisionBundles, decisionEffectExecutions, decisionRetention, decisions, decisionTargetIssues, heartbeatRuns, issueRelations, issues } from "@geetorusai/db";
import { ATTENTION_SOURCE_KINDS, decisionEffectTargetIssueIds } from "@geetorusai/shared";
import type { AttentionArchiveManifestEntry, DecisionEffect, DecisionInput, DecisionOption, DecisionStatsCounts, DecisionStatsResponse } from "@geetorusai/shared";
...
export function decisionService(db: Db, options: DecisionServiceOptions) {
  ...
  async function createInStore(input: CreateInputWithSnapshots, dbOrTx: Db) {
    ...
    const cap = Number(process.env.GEETORUS_DECISIONS_OPEN_CAP ?? 50);
  ...
  async function sweepExpired(now = new Date()) {
    const configuredBatchSize = Number(process.env.GEETORUS_DECISIONS_SWEEP_BATCH_SIZE ?? 100);
    const batchSize = Number.isFinite(configuredBatchSize)
      ? Math.max(1, Math.trunc(configuredBatchSize))
      : 100;
    const configuredRecoveryGraceMs = Number(process.env.GEETORUS_DECISIONS_RECOVERY_GRACE_MS ?? 60_000);
