...
import type { AdapterRuntimeServiceReport } from "@geetorusai/adapter-utils";
import type { Db } from "@geetorusai/db";
import { executionWorkspaces, issueComments, issues, projectWorkspaces, workspaceRuntimeServices } from "@geetorusai/db";
import {
...
const DEFAULT_TAILSCALE_BROKER_SOCKET = "/run/geetorus-tailscale-broker/broker.sock";
...
export type WorkspaceRuntimeExposureDeps = ExposureManagerDeps & {
   ...
   * automatic default on broker availability is what keeps a Geetorus install
...
function resolveTailscaleBrokerSocketPath(): string {
  return process.env.GEETORUS_TAILSCALE_BROKER_SOCKET?.trim() || DEFAULT_TAILSCALE_BROKER_SOCKET;
}
 ...
 *
 *  - `auto` (default): eligible Geetorus-managed worktree runtimes get
...
export function resolveManagedRuntimeHttpsMode(): ManagedRuntimeHttpsMode {
  const raw = process.env.GEETORUS_MANAGED_RUNTIME_HTTPS?.trim().toLowerCase();
 ...
 *
 * Intentionally narrow: only the Geetorus-managed dev runtime. Unmanaged and
 * custom external services are left exactly as they are, because the broker
 * only publishes allowlisted loopback ports it can prove Geetorus owns and we
 * do not want to relocate a service somebody else addresses by port.
 *
 * A *pinned* port is still a candidate. The pre-feature Geetorus App template
...
function isManagedHttpsDefaultCandidate(input: {
  ...
  return isGeetorusDevRuntimeService(input);
}
 ...
 * name, and the substrings `isGeetorusDevRuntimeService` matches survive
 ...
 * startup-reconciliation suites use this to simulate a Geetorus restart, where
...
export async function resetRuntimeServicesForTests(
  ...
  for (const record of runtimeServicesById.values()) {
    ...
    if (opts.simulateSupervisorExit) {
      ...
      // Geetorus keeping an anonymous stdio peer alive.
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".geetorus", "dist", "node_modules"]);
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  // Geetorus instance. Runtime/service overrides are merged back after sanitizing.
...
function buildDirtyQuarantineRescueBranch(sourceIssue: ExecutionWorkspaceIssueRef | null) {
  ...
  return sanitizeBranchName(`geetorus/rescue/${issueComponent}/${formatUtcBranchTimestamp()}`);
}
...
function explainGitWorktreeBranchIncoherence(input: {
  ...
  if (!input.expectedHeadSha || !input.actualHeadSha) {
    return `Geetorus could not determine branch ancestry because the recorded branch "${input.expectedBranchName}" or checked-out branch "${actualBranch}" is missing a resolvable HEAD commit.`;
  }
  if (input.sameHead) {
  ...
  if (input.ancestryVerdict === "diverged") {
    return `The recorded branch "${input.expectedBranchName}" is not an ancestor of the checked-out branch "${actualBranch}", so Geetorus cannot prove a forward-only reconciliation.`;
  }
  return `Geetorus could not determine whether the checked-out branch "${actualBranch}" is forward of the recorded branch "${input.expectedBranchName}".`;
}
  ...
  try {
    ...
    await recordGitOperation(input.recorder, {
      ...
      args: [
        "commit",
        "-m",
        "Geetorus dirty workspace rescue",
        "-m",
        [
...
export async function ensureGitWorktreeBranchCoherent(input: {
  ...
  if (
    ...
    const reason = evidence.provenance.expectedBranchExists
      ...
      : "Automatic forward reconciliation: the recorded branch no longer exists, so Geetorus adopted the clean checked-out branch.";
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The checked-out branch contains the recorded branch plus newer commits, so Geetorus adopted it for subsequent runs.`,
      ],
    };
  }
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The detached HEAD contained the recorded branch plus newer commits, so Geetorus moved the recorded branch to that HEAD.`,
      ],
    };
  }
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.GEETORUS_WORKSPACE_CWD = input.worktreePath;
  env.GEETORUS_WORKSPACE_PATH = input.worktreePath;
  env.GEETORUS_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.GEETORUS_WORKSPACE_BRANCH = input.branchName;
  env.GEETORUS_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.GEETORUS_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.GEETORUS_WORKSPACE_SOURCE = input.base.source;
  env.GEETORUS_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.GEETORUS_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.GEETORUS_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.GEETORUS_PROJECT_ID = input.base.projectId ?? "";
  env.GEETORUS_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.GEETORUS_AGENT_ID = input.agent.id ?? "";
  env.GEETORUS_AGENT_NAME = input.agent.name;
  env.GEETORUS_COMPANY_ID = input.agent.companyId;
  env.GEETORUS_ISSUE_ID = input.issue?.id ?? "";
  env.GEETORUS_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.GEETORUS_ISSUE_TITLE = input.issue?.title ?? "";
  env.GEETORUS_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.GEETORUS_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.GEETORUS_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.GEETORUS_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.GEETORUS_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.GEETORUS_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.GEETORUS_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.GEETORUS_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.GEETORUS_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.GEETORUS_PROJECT_ID = input.workspace.projectId ?? "";
  env.GEETORUS_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.GEETORUS_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".geetorus", "worktrees");
...
async function deleteGitBranchAtVerifiedTip(input: {
  ...
  const detachedGitDir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-branch-delete-"));
...
/**
 * Rows Geetorus reports stopped/removed whose reserved pair is still live on
...
/** Geetorus-owned Serve mappings, or null when the broker cannot be read. */
...
function isGeetorusDevRuntimeService(input: { serviceName?: string | null; command?: string | null }) {
  const serviceName = (input.serviceName ?? "").trim().toLowerCase();
  const command = (input.command ?? "").trim().toLowerCase();
  return (
    serviceName === "geetorus-dev"
    || serviceName === "geetorus-dev-once"
    || (command.includes("dev:once") && command.includes("tailscale-auth"))
  );
}

export const MANAGED_RUNTIME_PUBLIC_URL_ENV = "GEETORUS_MANAGED_RUNTIME_PUBLIC_URL";

const EXPLICIT_RUNTIME_ORIGIN_ENV_KEYS = [
  "GEETORUS_PUBLIC_URL",
  "GEETORUS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
...
function managedRuntimeOriginError(serviceName: string, reason: string) {
  return new Error(
    ...
    + "Configure GEETORUS_PUBLIC_URL or BETTER_AUTH_URL for this service, or publish an HTTPS expose.urlTemplate "
...
function trustedRuntimeHostnameBoundary(
  ...
  let markerIndex = 0;
  const markerPrefix = "geetorus-runtime-template-";
  ...
  const markers = [...hostname.matchAll(/geetorus-runtime-template-\d+/g)];
...
/**
 * Resolve the low-priority public URL hint injected into a managed Geetorus dev
 * service. Explicit operator origin settings are deliberately left untouched.
 */
export function resolveManagedGeetorusRuntimePublicOrigin(input: {
  serviceName: string;
  command: string;
  ...
  if (!isGeetorusDevRuntimeService(input)) return null;
...
function resolveRuntimeServiceHealthUrl(
  ...
  if (!url || !isGeetorusDevRuntimeService(input ?? {})) return url;
  try {
...
async function isRuntimeServiceUrlHealthy(
  ...
  const localProbeUrl = input?.provider === "local_process" && input.port && isGeetorusDevRuntimeService(input)
  ...
  try {
    ...
    if (!isGeetorusDevRuntimeService(input ?? {})) return true;
...
function readWorkspaceSeedOperationEvidence(worktreePath: string): {
  ...
  const manifestPath = path.join(worktreePath, ".geetorus", "seed-manifest.json");
  try {
...
export function resolveRuntimeProvisionCommand(input: {
  ...
  const stateDir = path.join(input.workspace.cwd, ".geetorus");
...
async function spawnLocalRuntimeService(input: StartLocalRuntimeServiceInput): Promise<LocalRuntimeServiceStart> {
  ...
  // overwrites GEETORUS_BIND from its own `--bind` argv and deletes
  // GEETORUS_BIND_HOST — which is exactly how a branch pinned at plain master
  ...
  // only way a legacy pinned port (the Geetorus App template's 45439) can be
  ...
  // Managed Geetorus worktrees are development environments, so their UI
  ...
  const uiDevMiddlewareHasTransport =
    !exposureConfig || exposureConfig.includeGeetorusViteHmr;
  if (
    uiDevMiddlewareHasTransport
    && isGeetorusDevRuntimeService({ serviceName, command })
  ) {
    env.GEETORUS_UI_DEV_MIDDLEWARE ??= "true";
  }
  if (port) {
  ...
  // the Geetorus dev runtime whether or not it is HTTPS-exposed, because the
  ...
  const managedWorkspaceIdentity = isGeetorusDevRuntimeService({ serviceName, command })
    ? resolveManagedWorkspaceIdentity({
  ...
  if (exposureConfig) {
    // Geetorus dev-runtime-specific hardening. Other managed processes are
    ...
    env.GEETORUS_BIND = RUNTIME_EXPOSURE_BIND_MODE;
    env.GEETORUS_BIND_HOST = RUNTIME_EXPOSURE_BIND_HOST;
    env.HOST = RUNTIME_EXPOSURE_BIND_HOST;
    env.GEETORUS_VITE_HMR_PROTOCOL = "wss";
    env.GEETORUS_MANAGED_RUNTIME_EXPOSURE = "tailscale_https";
    env.GEETORUS_ALLOWED_HOSTNAMES = exposureHostname!;
    env.GEETORUS_AUTH_BASE_URL_MODE = "explicit";
    env.GEETORUS_AUTH_PUBLIC_BASE_URL = `https://${exposureHostname}:${port}`;
    env.GEETORUS_PUBLIC_URL = `https://${exposureHostname}:${port}`;
  }
  ...
  const managedRuntimePublicOrigin = resolveManagedGeetorusRuntimePublicOrigin({
    serviceName,
    command,
  ...
  try {
    child = spawn(shell, ["-lc", command], {
      ...
      // Geetorus (or this parent handle below) cannot strand a request logger
  ...
  const ownershipCheckedPorts = port
    ? exposureConfig
      ? [
          port,
          ...(exposureConfig.includeGeetorusViteHmr ? [deriveViteHmrPort(port)] : []),
        ]
  ...
  const readinessPromise = Promise.race([
    ...
    const exposureAssignedPorts: number[] =
      exposureConfig && port
        ? [port, ...(exposureConfig.includeGeetorusViteHmr ? [deriveViteHmrPort(port)] : [])]
        : [];
...
export async function reconcilePersistedRuntimeServicesOnStartup(db: Db) {
  ...
  for (const row of rows) {
    ...
    // leave `http://geetorus-dev:<port>` as the canonical URL forever. Stopping
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Geetorus", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
