...
const threadId = "github:geetorusai/chat-e2e:issue:42";
...
function message(overrides: Record<string, unknown> = {}): Message {
  return {
    ...
    raw: {
      ...
      prNumber: 42,
      threadType: "issue",
      repository: { full_name: "geetorusai/chat-e2e" },
...
function canonicalComment(overrides: Record<string, unknown> = {}) {
  return {
    id: 4242,
    url: "https://api.github.com/repos/geetorusai/chat-e2e/issues/comments/4242",
    issue_url: "https://api.github.com/repos/geetorusai/chat-e2e/issues/42",
...
describe("GitHub exact-comment private image resolution", () => {
  ...
  it.each([
    ...
    {
      url: "https://api.github.com/repos/geetorusai/chat-e2e/issues/comments/4243",
    },
    {
  ...
  it("uses the documented review-comment media type and exact review-root binding", () => {
    const value = message({
      threadId: "github:geetorusai/chat-e2e:42:rc:4000",
    });
    ...
    expect(requestDescriptor).toEqual({
      url: "https://api.github.com/repos/geetorusai/chat-e2e/pulls/comments/4242",
      accept: "application/vnd.github-commitcomment.full+json",
    });
    const response = canonicalComment({
      url: requestDescriptor.url,
      pull_request_url:
        "https://api.github.com/repos/geetorusai/chat-e2e/pulls/42",
      in_reply_to_id: 4000,
    });
...
describe("public GitHub attachment extraction", () => {
  ...
  it.each([
    ...
    { threadId: "github:geetorusai/chat-e2e:issue:43" },
    { threadId: "github:geetorusai/chat-e2e:42:rc:4242" },
  ...
  it("imports a public file and never persists a signed CDN redirect", async () => {
    ...
    for (const [, init, guard] of request.mock.calls) {
      ...
      expect(init.headers).toEqual({
        accept: "*/*",
        "user-agent": "Geetorus/ChatAttachments",
      });
