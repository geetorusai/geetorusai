...
import { instanceSettings } from "@geetorusai/db";

export const CLOUD_RUNTIME_IDENTITY_HEADER = "x-geetorus-cloud-runtime-identity";
export const CLOUD_RUNTIME_IDENTITY_AUDIENCE = "geetorus-runtime-identity/v1";
export const CLOUD_RUNTIME_IDENTITY_ISSUER = "geetorus-cloud";
export const CLOUD_RUNTIME_IDENTITY_JWS_TYPE = "geetorus-cloud-runtime-identity+jwt";
...
function configuredStartupOrigin(env: NodeJS.ProcessEnv): string | null {
  const candidate = nonEmpty(env.GEETORUS_PUBLIC_URL)
    ?? nonEmpty(env.GEETORUS_AUTH_PUBLIC_BASE_URL)
    ?? nonEmpty(env.GEETORUS_API_URL);
...
function applyCompatibilityEnvironment(identity: CloudRuntimeIdentitySnapshot, env: NodeJS.ProcessEnv) {
  ...
  env.GEETORUS_PUBLIC_URL = identity.canonicalOrigin;
  env.GEETORUS_AUTH_PUBLIC_BASE_URL = identity.canonicalOrigin;
  env.GEETORUS_API_URL = identity.canonicalOrigin;
  env.GEETORUS_PRIMARY_HOST = hostname;
  env.GEETORUS_STACK_SLUG = identity.stackSlug;

  const existingCandidates = (() => {
    try {
      const parsed = JSON.parse(env.GEETORUS_RUNTIME_API_CANDIDATES_JSON ?? "[]");
      return Array.isArray(parsed) ? parsed.filter((value): value is string => typeof value === "string") : [];
    } catch {
      return [];
    }
  })();
  env.GEETORUS_RUNTIME_API_CANDIDATES_JSON = JSON.stringify([
...
function assertPersistedIdentityMatchesStack(row: PersistedRuntimeIdentity, env: NodeJS.ProcessEnv) {
  const configuredStackId = nonEmpty(env.GEETORUS_CLOUD_STACK_ID);
  if (!configuredStackId || configuredStackId !== row.stackId) {
    throw new Error("Persisted Cloud runtime identity does not match GEETORUS_CLOUD_STACK_ID");
  }
...
export async function initializeCloudRuntimeIdentity(
  ...
  if (!nonEmpty(env.GEETORUS_CLOUD_STACK_ID)) {
    initialized = true;
...
function publicKeyForKid(env: NodeJS.ProcessEnv, kid: string) {
  const raw = nonEmpty(env.GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS);
  if (!raw) throw new Error("GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS is not configured");
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error("GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS is invalid");
  }
  const keys = parsed && typeof parsed === "object" && !Array.isArray(parsed)
    ? (parsed as { keys?: unknown }).keys
    : undefined;
  if (!Array.isArray(keys)) throw new Error("GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS is invalid");
...
export const CLOUD_CONTROL_HEADER = "x-geetorus-cloud-control";
export const CLOUD_CONTROL_AUDIENCE = "geetorus-cloud-control/v1";
export const CLOUD_CONTROL_JWS_TYPE = "geetorus-cloud-control+jwt";
 ...
 * (GEETORUS_CLOUD_RUNTIME_IDENTITY_JWKS) but under its own JWS type and
