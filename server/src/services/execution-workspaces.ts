...
import { deriveProjectUrlKey, WORKSPACE_OVERVIEW_LINKED_ISSUE_LIMIT } from "@geetorusai/shared";
...
function explainGitWorktreeBranchReconcileInspection(input: {
  ...
  if (!input.fromSha || !input.toSha) {
    return `Geetorus could not determine branch ancestry because "${input.fromBranch}" or "${input.toBranch}" is missing a resolvable HEAD commit.`;
  }
  if (input.fromSha === input.toSha) {
  ...
  return `Geetorus could not determine whether "${input.toBranch}" is forward of "${input.fromBranch}".`;
}
...
async function inspectExecutionWorkspaceBranchForReconcile(
  ...
  if (!worktreePath) {
    throw unprocessable("Execution workspace needs a local worktree path before Geetorus can reconcile its branch record");
  }
  ...
  if (!toBranch) {
    throw unprocessable("Execution workspace is detached; Geetorus cannot reconcile it to a branch name");
  }
...
async function inspectGitCloseReadiness(workspace: ExecutionWorkspace): Promise<{
  ...
  if (!workspacePath) {
    warnings.push("Workspace has no local path, so Geetorus cannot inspect git status before close.");
    return { git: null, warnings, statusInspectionSucceeded: false };
  }

  if (!(await pathExists(workspacePath))) {
    warnings.push(`Workspace path "${workspacePath}" does not exist, so Geetorus cannot inspect git status before close.`);
    return {
      git: {
        repoRoot: null,
        workspacePath,
  ...
  return {
    ...
    getCloseReadiness: async (id: string): Promise<ExecutionWorkspaceCloseReadiness | null> => {
      ...
      if (!statusInspectionSucceeded) {
        blockingReasons.push("Geetorus could not verify the workspace git status. Retry before destructive cleanup.");
      }
      ...
      if (executionWorkspace.providerType === "git_worktree" && workspacePath) {
        plannedActions.push({
          ...
          description: `Geetorus will run git worktree cleanup for ${workspacePath}.`,
      ...
      if (git?.createdByRuntime && executionWorkspace.branchName) {
        plannedActions.push({
          ...
          description: "Geetorus will try to delete the runtime-created branch after removing the worktree.",
      ...
      if (executionWorkspace.providerType === "local_fs" && git?.createdByRuntime && workspacePath) {
        ...
        if (containsProjectWorkspace) {
          warnings.push(`Geetorus will archive this workspace but keep "${workspacePath}" because it contains the project workspace.`);
        } else {
          plannedActions.push({
            kind: "remove_local_directory",
            label: "Remove runtime-created directory",
            description: `Geetorus will remove the runtime-created directory at ${workspacePath}.`,
    ...
    reconcileExecutionWorkspaceBranch: async (
      ...
      if (!existing.sourceIssueId) {
        throw unprocessable("Execution workspace needs a source issue before Geetorus can audit branch reconciliation");
      }
