...
/** A versioned row returned by the Geetorus persistence implementation. */
...
export interface GeetorusChatSdkStateOptions extends ChatSdkStateScope {
...
/**
 * Chat SDK StateAdapter backed by Geetorus's injected, company-scoped CAS
 * persistence. Instances are endpoint-scoped and never own or close the shared
 * database connection.
 */
export class GeetorusChatSdkStateAdapter implements StateAdapter {
  ...
  constructor(options: GeetorusChatSdkStateOptions) {
    assertBoundedIdentifier(
      "companyId",
      options.companyId,
      MAX_SCOPE_ID_LENGTH,
  ...
  private ensureConnected(): void {
    if (!this.connected) {
      throw new Error(
        "GeetorusChatSdkStateAdapter is not connected. Call connect() first.",
      );
    }
  }
...
export function createGeetorusChatSdkState(
  options: GeetorusChatSdkStateOptions,
): GeetorusChatSdkStateAdapter {
  return new GeetorusChatSdkStateAdapter(options);
}
