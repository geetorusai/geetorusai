...
describe("chat run milestone projection", () => {
  ...
  it("gives unlinked external identities a safe recovery path when isolation is unavailable", () => {
    expect(
      safeMilestoneText({
        ...
        milestone: "failed",
        issueId: "issue-1",
        publicBaseUrl: "https://geetorus.example/path",
      }),
    ).toBe(
      "Maya couldn't safely start this turn because this task was started for an unlinked external guest and isolated guest execution isn't available. Ask a Geetorus admin to create a private identity link for this account or enable isolated guest execution, then start a new task. Open the task in Geetorus: https://geetorus.example/issues/issue-1",
    );
    expect(
      safeMilestoneText({
        agentName: "Maya",
      ...
      "Maya couldn't safely start this turn because this task was started for an unlinked external guest and isolated guest execution isn't available. Ask a Geetorus admin to create a private identity link for this account or enable isolated guest execution, then start a new task. Open the task in Geetorus for details.",
    );
  });
  ...
  it("gives permanent integrity failures a safe operator recovery path", () => {
    ...
    expect(text).toBe(
      "Maya couldn't safely continue this turn. A Geetorus admin needs to review the run before it can be retried. Open the task in Geetorus for details.",
    );
    expect(text).not.toMatch(/digest|source.seq|semantic|replay.conflict/i);
  });

  it("keeps every other run failure generic outside Geetorus", () => {
    expect(
      safeMilestoneText({
        agentName: "Maya",
      ...
      "Maya stopped before completing this turn. Open the task in Geetorus for details.",
    );
  });

  it.each([
    [null, " Open the task in Geetorus for details."],
    [
      "https://geetorus.example",
      " Open the task in Geetorus: https://geetorus.example/issues/issue-1",
    ],
  ])(
    ...
    (publicBaseUrl, suffix) => {
      ...
      expect(text).toBe(
        "Maya couldn't start this turn because an earlier session needs recovery. Your request is saved. Ask a Geetorus admin to recover that session before retrying; sending the request again won't repair it." +
          suffix,
      );
  ...
  it("describes ownership recovery without claiming the retained process stopped", () => {
    expect(
      ...
      "Maya needs a Geetorus admin to safely recover this turn before more work can start. Open the task in Geetorus for details.",
    );
  });

  it.each(["server_shutdown_interrupted", "lease_released_before_terminal"])(
    "keeps interruption bookkeeping for %s inside Geetorus",
    (errorCode) => {
      expect(
        safeMilestoneText({
          agentName: "Maya",
  ...
  it("explains an allowlisted native provider capacity failure without exposing provider details", () => {
    expect(
      ...
      "Maya couldn't complete this turn because the model provider's usage allowance is exhausted. A Geetorus admin needs to restore capacity before retrying. Open the task in Geetorus for details.",
    );
  });
...
describe("chat agent comment publication authorization", () => {
  ...
  it.each([
    ...
    "keeps an internal agent comment with reason %s inside Geetorus",
    (reason) => {
