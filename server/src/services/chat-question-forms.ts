...
import { chatActions } from "@geetorusai/db";
import type {
  AskUserQuestionsAnswer,
  AskUserQuestionsInteraction,
  AskUserQuestionsQuestion,
  GeetorusQuestionSetQuestion,
...
const CHAT_QUESTION_FORM_DENIAL_MESSAGE =
  "This form is no longer authorized. Close it and open the linked Geetorus task.";

/**
 * Provider callbacks must be acknowledged after Geetorus has durably denied a
...
function questionSetQuestion(
  interaction: AskUserQuestionsInteraction,
  questionId: string,
): GeetorusQuestionSetQuestion | null {
  return (
...
function formKind(
  ...
  if (source) {
    if (source.answerMode === "text") {
      ...
      // Keep those uncommon forms in Geetorus, whose canonical validator owns
...
/**
 * Converts opaque submitted values to canonical Geetorus answers. Provider
