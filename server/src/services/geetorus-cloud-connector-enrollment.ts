...
const ENROLLMENT_FILE = "geetorus-cloud-connector.json";
const REQUEST_TYPE = "geetorus-cloud-connector-request+jwt";
...
export type GeetorusCloudConnectorIdentity = {
  version: 1;
  instanceId: string;
...
export type GeetorusCloudConnectorEnrollmentStatus = {
  configured: boolean;
  status: GeetorusCloudConnectorIdentity["status"] | "not_configured" | "suspended" | "unverified";
  brokerBaseUrl: string;
...
export function geetorusCloudConnectorIdentityPath(): string {
  return path.join(resolveGeetorusInstanceRoot(), "secrets", ENROLLMENT_FILE);
}

export function loadGeetorusCloudConnectorIdentity(): GeetorusCloudConnectorIdentity | null {
  const filePath = geetorusCloudConnectorIdentityPath();
...
export function geetorusCloudConnectorEnrollmentStatus(
  env: NodeJS.ProcessEnv = process.env,
): GeetorusCloudConnectorEnrollmentStatus {
  const identity = loadGeetorusCloudConnectorIdentity();
...
export async function startGeetorusCloudConnectorEnrollment(input: {
  origin: string;
  label?: string;
  companyId?: string;
  initiatedBy?: string;
  ...
  return withEnrollmentMutationLock(() => startGeetorusCloudConnectorEnrollmentUnlocked(input));
}

async function startGeetorusCloudConnectorEnrollmentUnlocked(input: {
  origin: string;
  label?: string;
  companyId?: string;
  initiatedBy?: string;
  ...
  if (hasManagedConnectorIdentityOverride(env)) {
    throw new Error("Geetorus Cloud self-host enrollment is unavailable with managed identity configuration");
  }
  const origin = normalizeInstanceOrigin(input.origin);
  const existingIdentity = loadGeetorusCloudConnectorIdentity();
  const target = connectorTarget(env, existingIdentity);
  let identity: GeetorusCloudConnectorIdentity;
  if (!existingIdentity) {
    identity = createIdentity(env);
  } else if (!identityMatchesTarget(existingIdentity, target)) {
    if (existingIdentity.status === "active") {
      throw new Error("Geetorus Cloud connector is enrolled with another target");
    }
    identity = createIdentity(env);
  } else {
  ...
  if (identity.status === "pending" && identity.pending && Date.parse(identity.pending.expiresAt) > Date.now()) {
    if (identity.pending.origin !== origin) {
      throw new Error("Geetorus Cloud enrollment is already pending for another origin");
    }
    if (identity.pending.companyId !== input.companyId) {
      throw new Error("Geetorus Cloud enrollment is already pending for another company");
    }
    if (input.initiatedBy && identity.pending.initiatedBy && identity.pending.initiatedBy !== input.initiatedBy) {
      throw new Error("Geetorus Cloud enrollment is already pending for another administrator");
    }
    return geetorusCloudConnectorEnrollmentStatus(env);
  }
  ...
  const response = await request(`${identity.brokerBaseUrl}/v1/connector/enrollments`, {
    ...
    body: JSON.stringify({
      ...
      returnUri,
      returnState,
      label: input.label?.trim() || process.env.GEETORUS_INSTANCE_ID?.trim() || "Self-hosted Geetorus",
      signPublicKey: identity.signPublicKey,
      sealPublicKey: identity.sealPublicKey,
    }),
    signal: AbortSignal.timeout(15_000),
  }).catch(() => null);
  if (!response?.ok) throw new Error("Geetorus Cloud enrollment is unavailable");
  const body = await response.json().catch(() => null) as Record<string, unknown> | null;
  if (!body || typeof body.enrollmentId !== "string" || typeof body.verificationUrl !== "string" || typeof body.expiresAt !== "string") {
    throw new Error("Geetorus Cloud returned an invalid enrollment response");
  }
  ...
  if (verificationUrl.origin !== identity.brokerBaseUrl
    ...
    throw new Error("Geetorus Cloud returned an invalid enrollment destination");
  }
  identity = {
    ...identity,
    status: "pending",
  ...
  return {
    ...geetorusCloudConnectorEnrollmentStatus(env),
    verificationUrl: verificationUrl.toString(),
    expiresAt: body.expiresAt,
  };
}

export async function completeGeetorusCloudConnectorEnrollment(input: {
  enrollmentId: string;
  approvalCode: string;
  state: string;
  env?: NodeJS.ProcessEnv;
  ...
  return withEnrollmentMutationLock(() => completeGeetorusCloudConnectorEnrollmentUnlocked(input));
}

async function completeGeetorusCloudConnectorEnrollmentUnlocked(input: {
  enrollmentId: string;
  approvalCode: string;
  state: string;
  env?: NodeJS.ProcessEnv;
  ...
  if (hasManagedConnectorIdentityOverride(input.env ?? process.env)
    ...
    throw new Error("Invalid or expired Geetorus Cloud enrollment state");
  }
  ...
  if (!response?.ok) throw new Error("Geetorus Cloud enrollment could not be completed");
  const body = await response.json().catch(() => null) as Record<string, unknown> | null;
  if (!body || body.id !== identity.instanceId || body.environment !== identity.environment
    ...
    throw new Error("Geetorus Cloud returned an invalid enrollment binding");
  }
  saveIdentity({
    ...identity,
    status: "active",
  ...
  return geetorusCloudConnectorEnrollmentStatus(input.env ?? process.env);
}

function createIdentity(env: NodeJS.ProcessEnv): GeetorusCloudConnectorIdentity {
  const signing = generateKeyPairSync("ed25519");
  const sealing = generateKeyPairSync("x25519");
  const { brokerBaseUrl, environment } = connectorTarget(env);
  const identity: GeetorusCloudConnectorIdentity = {
...
function saveIdentity(identity: GeetorusCloudConnectorIdentity): void {
  const filePath = geetorusCloudConnectorIdentityPath();
...
function parseIdentity(value: unknown): GeetorusCloudConnectorIdentity {
  if (!isRecord(value) || value.version !== IDENTITY_VERSION
    ...
    throw new Error("Invalid Geetorus Cloud connector identity");
  }
  return value as GeetorusCloudConnectorIdentity;
}
...
function connectorEnvironment(
  ...
  const inferred = host === "my.geetorus.app"
    ? "production"
    : host === "my-staging.geetorus.app"
      ? "staging"
      : "development";
  const value = env.GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT?.trim() || fallback || inferred;
  if (!isEnvironment(value)) throw new Error("Geetorus Cloud connector environment is invalid");
  if ((host === "my.geetorus.app" && value !== "production")
    || (host === "my-staging.geetorus.app" && value !== "staging")) {
    throw new Error("Geetorus Cloud connector broker and environment do not match");
  }
  return value;
}
...
function connectorTarget(
  env: NodeJS.ProcessEnv,
  identity?: GeetorusCloudConnectorIdentity | null,
): Pick<GeetorusCloudConnectorIdentity, "brokerBaseUrl" | "environment"> {
  const brokerOverride = env.GEETORUS_CLOUD_CONNECTOR_BASE_URL?.trim() || undefined;
  const brokerBaseUrl = normalizeBrokerOrigin(
    brokerOverride ?? identity?.brokerBaseUrl ?? "https://my.geetorus.app",
  );
  return {
    brokerBaseUrl,
...
function identityMatchesTarget(
  identity: GeetorusCloudConnectorIdentity,
  target: Pick<GeetorusCloudConnectorIdentity, "brokerBaseUrl" | "environment">,
): boolean {
...
function hasManagedConnectorIdentityOverride(env: NodeJS.ProcessEnv): boolean {
  return [
    env.GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID,
    env.GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY,
    env.GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY,
...
function normalizeBrokerOrigin(value: string): string {
  ...
  if (url.protocol !== "https:" && !(url.protocol === "http:" && loopback(url.hostname))) {
    throw new Error("Geetorus Cloud connector URL must use HTTPS");
  }
  if (url.username || url.password || url.search || url.hash || (url.pathname !== "/" && url.pathname !== "")) {
    throw new Error("Geetorus Cloud connector URL must be an origin");
  }
  return url.origin;
}
...
function normalizeInstanceOrigin(value: string): string {
  const url = new URL(value.trim());
  if (url.protocol !== "https:" && !(url.protocol === "http:" && loopback(url.hostname))) {
    throw new Error("Geetorus Cloud enrollment requires HTTPS except on loopback");
  }
  if (url.username || url.password || url.search || url.hash || (url.pathname !== "/" && url.pathname !== "")) {
    throw new Error("Geetorus Cloud enrollment requires an exact origin");
  }
  return url.origin;
}
