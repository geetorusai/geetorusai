...
import type { VercelConnectCredentialReference } from "@geetorusai/shared";
...
describe("Vercel Connect credential adapter", () => {
  it("canonicalizes Geetorus's loopback callback to Vercel's accepted localhost form", () => {
    ...
    expect(vercelConnectCallbackUrl(
      "https://geetorus.example/api/tools/oauth/callback",
      "one-time-state",
    )).toBe("https://geetorus.example/api/tools/vercel-connect/callback?state=one-time-state");
  });

  it("gates new setup separately from runtime credential availability", () => {
    expect(vercelConnectIntegrationStatus({
      GEETORUS_VERCEL_CONNECT_ENABLED: "false",
      GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN: "bootstrap-token",
    ...
    expect(vercelConnectIntegrationStatus({
      ...
      GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN: "fallback-token",
    } as NodeJS.ProcessEnv)).toMatchObject({
      enabled: true,
      configured: true,
  ...
  it("prefers workload OIDC over the access-token fallback", () => {
    expect(vercelConnectSdkOptions({
      VERCEL_OIDC_TOKEN: "workload-token",
      GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN: "stale-fallback-token",
  ...
  it("derives stable company- and user-bound subjects without browser input", () => {
    vi.stubEnv("GEETORUS_INSTANCE_ID", "instance-one");
    const base = {
      credential,
  ...
  it("persists only the allow-listed grant summary, never bearer or upstream claims", () => {
    const reference = vercelGrantReference({
      ...
      token: {
        ...
        connector: { id: "scl_posthog", uid: "posthog-geetorus", type: "api-key" },
