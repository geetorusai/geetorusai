import { runIdentityContexts } from "@geetorusai/db";
...
import type { ToolRunContext } from "@geetorusai/plugin-sdk";
import type {
...
const sensitivePassthroughHeaderNames = new Set([
  ...
  "cookie",
  "set-cookie",
  "x-geetorus-tool-gateway-token",
]);

function isSensitivePassthroughHeader(name: string) {
  return (
    name.startsWith("x-geetorus-") ||
  ...
  const envDefaults: McpGatewayProtocolLimitOptions = {
    authFailures: {
      windowMs: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_AUTH_FAILURE_WINDOW_MS,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.authFailures.windowMs,
      ),
      max: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_AUTH_FAILURE_LIMIT,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.authFailures.max,
      ),
    },
    gatewayRequests: {
      windowMs: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_REQUEST_WINDOW_MS,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.gatewayRequests.windowMs,
      ),
      max: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_REQUEST_LIMIT,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.gatewayRequests.max,
      ),
    },
    tokenRequests: {
      windowMs: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_TOKEN_REQUEST_WINDOW_MS,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.tokenRequests.windowMs,
      ),
      max: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_TOKEN_REQUEST_LIMIT,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.tokenRequests.max,
      ),
    },
    sessionSetup: {
      windowMs: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_SESSION_SETUP_WINDOW_MS,
        DEFAULT_MCP_GATEWAY_PROTOCOL_LIMITS.sessionSetup.windowMs,
      ),
      max: positiveInt(
        process.env.GEETORUS_MCP_GATEWAY_SESSION_SETUP_LIMIT,
...
function safeClientMetadata(
  ...
  const clientName =
    safeHeaderValue(headers, "x-geetorus-client-name", 120) ??
...
const BUILTIN_TOOLS: ToolGatewayDescriptor[] = [
  ...
  {
    name: "geetorus-self:list_my_issues",
    displayName: "List my Geetorus issues",
    description:
      "Geetorus self-MCP read fixture that lists the authenticated agent's current issues.",
    parametersSchema: {
      type: "object",
      properties: { limit: { type: "number" } },
      additionalProperties: false,
    },
    pluginId: "geetorus-self",
    providerType: "geetorus_self",
    risk: "read",
  },
  {
    name: "geetorus-self:get_issue_context",
    displayName: "Get issue context",
    description:
      "Geetorus self-MCP read fixture that returns scoped issue context and plan document metadata.",
    parametersSchema: {
      type: "object",
...
const VIRTUAL_SEARCH_TOOLS: ToolGatewayDescriptor = {
  ...
  description:
    "Search the tools available through this Geetorus gateway without loading every target tool into the tool list.",
  parametersSchema: {
    type: "object",
    properties: {
      query: { type: "string" },
  ...
  pluginId: "geetorus-gateway",
  providerType: "geetorus_virtual",
  risk: "read",
};
...
const VIRTUAL_RUN_TOOL: ToolGatewayDescriptor = {
  ...
  description:
    "Run a target tool by name after Geetorus applies the target tool's profile, policy, approval, and rate-limit checks.",
  parametersSchema: {
    type: "object",
    properties: {
      tool: { type: "string" },
...
export function createToolGatewayService(
  ...
  const currentCloudConnector = (): GeetorusCloudConnector | null => {
    ...
    cachedCloudConnector = config
      ? createGeetorusCloudConnector({ config, now: options.now })
      : null;
    return cachedCloudConnector;
  };
  ...
  function pluginTools(): ToolGatewayDescriptor[] {
    return (pluginToolDispatcher?.listToolsForAgent() ?? []).map((tool) => ({
      ...tool,
      providerType: "geetorus_plugin" as const,
  ...
  async function findToolForSession(
    ...
    const tool = [...allTools(), ...connectedTools, ...virtualTools]
      .filter(
        (candidate) =>
          session.agentId ||
          (candidate.providerType !== "geetorus_self" &&
            candidate.providerType !== "geetorus_plugin"),
      )
  ...
  async function listToolsForContext(
    ...
    const tools = [
      ...
      (tool) =>
        session.agentId ||
        (tool.providerType !== "geetorus_self" &&
          tool.providerType !== "geetorus_plugin"),
    );
    const decisions = await Promise.all(
      tools.map(async (tool) => {
  ...
  async function executeBuiltinTool(
    ...
    if (tool.name === "geetorus-self:list_my_issues") {
      if (!session.agentId) {
        throw new ToolGatewayHttpError(
          403,
          "Geetorus self tools require an agent-scoped gateway session",
    ...
    if (tool.name === "geetorus-self:get_issue_context") {
      if (!session.agentId) {
        throw new ToolGatewayHttpError(
          403,
          "Geetorus self tools require an agent-scoped gateway session",
  ...
  function metadataHeadersForSession(
    ...
    for (const key of policy.metadataHeaders) {
      ...
      if (value) headers[`x-geetorus-${key.replace(/_/g, "-")}`] = value;
    }
    return headers;
  }
  ...
  async function maybeRefreshGeetorusCloudGrant(
    session: ToolGatewaySession,
    connection: typeof toolConnections.$inferSelect,
    grant: typeof connectionGrants.$inferSelect,
    forceRefresh = false,
    ...
    if (!oauth || !isGeetorusCloudConnectorStrategy(oauth.strategy))
      return grant;
    ...
    if (
      ...
      return grant;
    if (oauth.strategy === "geetorus_id_connector") {
      // Geetorus ID used different endpoints, signing metadata, envelope
      // purposes, and a different Google client. Its refresh token cannot be
      // exchanged through Geetorus Cloud. Let an unexpired access token finish
      ...
      throw new ToolGatewayHttpError(
        409,
        "Legacy authorization must be reconnected through Geetorus Cloud",
  ...
  async function resolveCredentialHeadersUnrecorded(
    ...
    if (
      ...
      grant = await maybeRefreshGeetorusCloudGrant(
        session,
        connection,
        grant,
    ...
    const payload = {
      ...
      detailsMarkdown:
        grantKind === "organization"
          ...
          : "This run needs your personal authorization. Geetorus will not use another user's identity.",
      target: {
  ...
  async function callLocalStdioMcp(input: {
    ...
    try {
      await request("initialize", {
        ...
        capabilities: {},
        clientInfo: { name: "geetorus-tool-gateway", version: "0.3.1" },
      });
      child.stdin.write(
  ...
  async function callRemoteConnectionProtocol(input: {
    ...
    const response = await guardedRemoteHttpFetch(
      endpoint,
      {
        ...
        body: JSON.stringify({
          jsonrpc: "2.0",
          id: `geetorus-context-${randomUUID()}`,
          method: input.method,
          params: input.params,
        }),
      },
  ...
  function contextHandle(
    ...
    return `geetorus-${kind}://${connectionId}/${Buffer.from(value, "utf8").toString("base64url")}`;
  }

  function parseContextHandle(kind: "resource" | "prompt", value: unknown) {
    if (typeof value !== "string") return null;
    const match = value.match(
      new RegExp(`^geetorus-${kind}://([0-9a-f-]{36})/([A-Za-z0-9_-]+)$`, "i"),
    );
  ...
  async function requestElicitationForRecordedToolCall(input: {
    ...
    await db
      ...
      .set({
        ...
        errorMessage:
          "Remote MCP tool requested elicitation; Geetorus created an issue interaction for the response.",
        updatedAt: now,
      })
  ...
  async function executeRemoteHttpTool(
    ...
    const requestId = `geetorus-tool-${randomUUID()}`;
    ...
    return [
      {
        ...
        notes: ["Use the full Geetorus origin before the endpoint path."],
      },
      {
  ...
  return {
    ...
    async executeTool(input: ExecuteGatewayToolInput) {
      ...
      if (
        ...
        tool.providerType === "geetorus_virtual"
      ) {
        const argumentValidation = validateToolContent({
          value: requestedParameters,
          direction: "arguments",
    ...
    async executePluginTool(input: ExecutePluginToolInput) {
      ...
      if (tool.providerType !== "geetorus_plugin") {
        throw new ToolGatewayHttpError(
          404,
