...
import { agentWakeupRequests, issueThreadInteractions, type Db } from "@geetorusai/db";
import type { IssueComment, IssueQueuedCommentEntry } from "@geetorusai/shared";
...
export async function readQueuedInteractionResponse(db: Db, companyId: string, issueId: string, payload: unknown) {
  ...
  const source: NonNullable<IssueQueuedCommentEntry["source"]> = {
    ...
    requiresFreshSession: record(record(payload)._geetorusWakeContext).forceFreshSession === true,
  };
