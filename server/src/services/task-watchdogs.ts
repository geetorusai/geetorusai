...
import type { IssueWatchdog, IssueWatchdogSummary } from "@geetorusai/shared";
...
function issueIdFromWakePayload(payload: unknown) {
  ...
  const nested = parseObject(parsed._geetorusWakeContext);
      ...
      db
        ...
        .where(and(
          ...
          or(
            ...
            inArray(sql`${agentWakeupRequests.payload}->'_geetorusWakeContext'->>'issueId'`, subtreeIssueIds),
            inArray(sql`${agentWakeupRequests.payload}->'_geetorusWakeContext'->>'taskId'`, subtreeIssueIds),
          ),
        )),
      db
        .select({
