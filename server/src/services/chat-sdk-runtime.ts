...
/** Public Geetorus provider ids. The Teams SDK name remains an internal detail. */
...
/** Chat SDK-normalized inbound message plus Geetorus endpoint identity. */
...
/**
 * Provider-neutral callbacks consumed by the Geetorus control-plane service.
...
const OFFICIAL_TEAMS_CONNECTOR_HOST_SUFFIXES = [
  ...
  // that Geetorus's commercial-cloud-only credential flow supports every
...
interface SlackAdapterInternals {
  geetorusFileUploadReceiptContext?: AsyncLocalStorage<
    (receipt: SlackFileUploadAcceptedReceipt) => Promise<void>
  >;
  geetorusResolveFileUploadReceipt?: (
    fileIds: string[],
    threadId: string,
  ) => Promise<string | null>;
}
...
interface DiscordAdapterInternals {
  ...
  geetorusCompatibilityRevision?: unknown;
...
function installDiscordNativeCommands(
  ...
  const denied =
    "This command is not available here. Open the Geetorus task or ask an operator to check your chat access.";
  const unconfirmed =
    "This command could not be confirmed. Check the Geetorus task before trying again.";
  discord.handleGatewayInteraction = async (interaction) => {
    ...
    const valid =
      ...
      interaction.commandName === "geetorus" &&
...
function assertDiscordAdapterCompatibility(adapter: Adapter, chat: Chat): void {
  ...
  if (discord.geetorusCompatibilityRevision !== "geetorus-discord-v6") {
    throw new DiscordAdapterCompatibilityError(
      "Geetorus patch revision geetorus-discord-v6 is unavailable",
    );
  }
...
interface TeamsAdapterInternals {
  ...
  geetorusRecordAcceptedActivity?: (activity: unknown) => Promise<void>;
  geetorusRecordThreadServiceUrl?: (
    threadId: string,
    serviceUrl: unknown,
  ) => Promise<void>;
  geetorusSendFileCard?: (
    threadId: string,
...
export function scopeMicrosoftTeamsEgress(
  ...
  // members or Graph APIs before Chat dispatches to Geetorus's reach and
  ...
  // through geetorusRecordAcceptedActivity below.
  teams.cacheUserContext = () => {};
  teams.getIncomingUser = async () => null;
  teams.getUser = async () => null;

  teams.geetorusRecordAcceptedActivity = async (activityValue: unknown) => {
  ...
  if (enableFileConsent) {
    ...
    teams.geetorusSendFileCard = async (threadId, kind, input) => {
      const card =
        kind === "consent"
  ...
  teams.geetorusRecordThreadServiceUrl = async (
    threadId: string,
    serviceUrlValue: unknown,
  ) => {
    const decoded = teams.decodeThreadId!(threadId);
  ...
  teams.openDM = async (userId: string) => {
    ...
    return await withServiceUrl(serviceUrl, async () => {
      ...
      if (typeof threadId === "string") {
        await teams.geetorusRecordThreadServiceUrl!(threadId, serviceUrl);
      }
      return threadId;
    });
  };
  ...
  switch (config.provider) {
    ...
    case "slack": {
      const adapterConfig: SlackAdapterConfig = {
        ...
        // Geetorus's durable publication outbox owns retry timing and
      ...
      ).geetorusFileUploadReceiptContext = new AsyncLocalStorage<
        (receipt: SlackFileUploadAcceptedReceipt) => Promise<void>
      >();
      return adapter;
    }
    case "github": {
      ...
      // exchange or API request could outlive Geetorus's publication lease
    ...
    case "discord": {
      ...
      const adapterConfig: DiscordAdapterConfig = {
        ...
        // Geetorus receives Discord messages and interactions over its
 ...
 * application callback runs. That ordering is unsafe for Geetorus: if the
 * `chat_deliveries` insert fails, a provider retry can be discarded by the SDK
 * even though Geetorus never durably accepted it. Geetorus's delivery ledger
 ...
 * durable.
 */
function geetorusAuthoritativeIngressState(
  state: StateAdapter,
    ...
    chat.onAction(async (event) => {
      ...
      if (!acceptsProviderScope(event.raw)) {
        if (transport === "discord_gateway") {
          ...
          // that never reached Geetorus's scoped action authorization.
          throw Object.assign(
...
/** One isolated Chat instance and provider adapter for one Geetorus endpoint. */
export class ChatSdkEndpointRuntime {
  ...
  constructor(options: CreateChatSdkEndpointRuntimeOptions) {
    ...
    const durableState = createGeetorusChatSdkState({
      companyId: options.companyId,
      endpointId: options.endpointId,
      persistence: options.persistence,
      maxValueBytes: options.maxStateValueBytes,
    });
    const state = geetorusAuthoritativeIngressState(durableState, (error) => {
      const attempt = this.webhookIngress.getStore();
    ...
    this.chat = new Chat({
      ...
      // Geetorus acknowledges only after its own durable ledger write and
  ...
  async handleWebhook(
    ...
    const retryableTimeout = () =>
      new Response("Geetorus could not durably accept the event in time", {
        status: 503,
        headers: {
    ...
    return await this.webhookIngress.run(attempt, async () => {
      ...
      if (response.ok && attempt.callbackError !== undefined) {
        return new Response("Geetorus could not durably accept the event", {
          status: 503,
          headers: {
            "content-type": "text/plain; charset=utf-8",
            "retry-after": "1",
  ...
  async streamTelegramDraft(
    ...
    const adapter = this.adapter as unknown as {
      geetorusDraftStopVersion?: number;
      stream(
        threadId: string,
  ...
  async postSlackFilePublication(
    ...
    if (
      !slack.geetorusFileUploadReceiptContext ||
      typeof slack.geetorusResolveFileUploadReceipt !== "function"
    ) {
      throw new SlackAdapterCompatibilityError(
        "file publication receipt capture is unavailable",
      );
    }
    return await slack.geetorusFileUploadReceiptContext.run(
      onUploadAccepted,
  ...
  async resolveSlackFileUploadReceipt(
    ...
    const slack = this.adapter as unknown as SlackAdapterInternals;
    if (typeof slack.geetorusResolveFileUploadReceipt !== "function") {
      throw new SlackAdapterCompatibilityError(
        "file publication receipt lookup is unavailable",
      );
    }
    return await slack.geetorusResolveFileUploadReceipt.call(
      this.adapter,
      fileIds,
      threadId,
    );
  ...
  async recordMicrosoftTeamsRoute(
    ...
    const recorder = teams.geetorusRecordThreadServiceUrl;
    ...
    if (raw !== undefined) {
      const acceptedActivityRecorder = teams.geetorusRecordAcceptedActivity;
  ...
  private async sendTeamsFileCard(
    ...
    if (
      !this.teamsFileConsentEnabled ||
      typeof teams.geetorusSendFileCard !== "function"
    ) {
      throw new TeamsAdapterCompatibilityError(
        "file-consent runtime is not enabled",
      );
    }
    return await teams.geetorusSendFileCard(threadId, kind, card);
  }
   ...
   * separately from the parsed Message, so without this bridge Geetorus
...
/** Process-local lifecycle registry; durable state remains in Geetorus persistence. */
