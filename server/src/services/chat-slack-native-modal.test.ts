...
const botUserId = "U-GEETORUS-BOT";
...
function signedRequest(payload: unknown, secret = signingSecret) {
  ...
  return new Request("https://geetorus.test/webhooks/slack", {
    method: "POST",
    headers: {
...
describe("Slack native multi-question modal adapter-to-runtime boundary", () => {
      ...
      if (method === "conversations.replies") {
        ...
        result = {
          ...
          user: {
            id: params.get("user"),
            name: "geetorus-agent",
            real_name: "Geetorus Agent",
            is_bot: true,
            profile: { display_name: "Geetorus Agent" },
          },
        };
  ...
  it("round-trips the actual Geetorus modal, opaque metadata and Slack state into canonical answers", async () => {
