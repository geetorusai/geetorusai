...
describe("heartbeat run scratch cleanup", () => {
  ...
  it("preserves geetorus-named directories without the ownership marker", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-run-unmarked-"));
    cleanupDirs.add(dir);
  ...
  it("builds explicit scratch env without clobbering configured temp dirs", async () => {
    ...
    expect(result.env.GEETORUS_RUN_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.GEETORUS_TASK_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.GEETORUS_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.GEETORUS_TMPDIR).toBe(scratch.dir);
