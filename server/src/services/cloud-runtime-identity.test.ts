...
describe("verifyCloudRuntimeIdentityAssertion", () => {
  it("preserves distinct self-hosted public and authentication origins", () => {
    const selfHostedEnv = {
      GEETORUS_PUBLIC_URL: "https://app.example.test",
      GEETORUS_AUTH_PUBLIC_BASE_URL: "https://auth.example.test",
      GEETORUS_API_URL: "https://api.example.test",
    } as NodeJS.ProcessEnv;
  ...
  it.each([
    ...
    ["wrong pool origin", { previousOrigin: "https://someone-else.staging.geetorus.app" }],
    ["non-HTTPS destination", { canonicalOrigin: "http://gonzo.staging.geetorus.app" }],
