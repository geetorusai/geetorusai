...
export class PhotonChatAdapter implements Adapter<PhotonThread, PhotonMessage> {
  ...
  parseMessage(raw: PhotonMessage): Message<PhotonMessage> {
    ...
    const isGroup = (raw as PhotonMessage & { geetorusIsGroup?: boolean })
      .geetorusIsGroup;
  ...
  async postMessage(
    ...
    throw new Error(
      "Photon sends require an immutable Geetorus publication identity",
    );
  }
  async editMessage(
    _id: string,
    ...
    throw new Error(
      "Photon edits require an immutable Geetorus publication identity",
    );
  }
