...
export async function reapSetupTokenLeases(
  ...
  for (const record of records) {
    try {
      ...
      failed += 1;
      log("[geetorus] Setup-token reaper: a lease release failed; it stays retryable.");
    }
  }
...
export class SetupTokenSessionService {
  ...
  private async terminateLocked(session: StoredSession, state: SetupTokenSessionState): Promise<void> {
    ...
    try {
      ...
      this.log("[geetorus] Setup-token session: the process stop step errored.");
    }
  ...
  private async runCleanup(session: StoredSession, state: SetupTokenSessionState): Promise<void> {
    ...
    if (!storedClaim) {
      try {
        ...
        this.log("[geetorus] Setup-token session: the cleanup record update failed; it stays retryable.");
      }
    }
    ...
    try {
      ...
      this.log("[geetorus] Setup-token session: the cleanup record removal failed; it stays retryable.");
    }
  ...
  private async releaseLeaseSafely(lease: SetupTokenLease): Promise<void> {
    try {
      ...
      this.log("[geetorus] Setup-token session: the lease release failed; the reaper retries it.");
    }
  }
