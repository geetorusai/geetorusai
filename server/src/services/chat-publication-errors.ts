export type ChatPublicationErrorDisposition =
  | {
      kind: "retry";
      retryAfterMs: number;
      /** True when the provider explicitly asked Geetorus to slow down. */
...
export function classifyChatPublicationError(
  ...
  if (
    ...
    // Geetorus rejected the destination locally before opening a provider
  ...
  // Geetorus. An operator must inspect the native conversation before replay.
