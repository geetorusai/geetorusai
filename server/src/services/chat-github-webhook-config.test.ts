...
const webhookUrl =
  "https://geetorus.example:8443/api/chat-webhooks/public-id/github";
...
describe("GitHub App webhook reconnect", () => {
  ...
  it.each([
    ...
    async (value) => {
      await expect(resync(async () => json(value))).rejects.toThrow(
        "did not confirm the expected secure Geetorus webhook",
      );
    },
  );
  ...
  it.each([
    "http://geetorus.example/hook",
    "https://user:password@geetorus.example/hook",
    "https://geetorus.example/hook?private=secret",
    "https://geetorus.example/hook#secret",
  ])(
...
const repositoryFullName = "geetorus/example";
...
const comment = {
  ...
  issue_url: "https://api.github.com/repos/geetorus/example/issues/3",
};
const delivery = {
  id: deliveryId,
  guid,
...
describe("GitHub App webhook recovery HTTP boundaries", () => {
  ...
  it("gets the current exact issue comment using only installation authority", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      `https://api.github.com/repos/geetorus/example/issues/comments/${comment.id}`,
  ...
  it("uses the pull-comment route, validates its target, and revokes before returning", async () => {
    const fetch = vi
      ...
      .mockResolvedValueOnce(
        losslessJson({
          ...
          issue_url: undefined,
          pull_request_url:
            "https://api.github.com/repos/geetorus/example/pulls/4",
        }),
      )
      .mockResolvedValueOnce(new Response(null, { status: 204 }));
    await expect(
    ...
    expect(fetch.mock.calls[1]?.[0]).toBe(
      `https://api.github.com/repos/geetorus/example/pulls/comments/${comment.id}`,
    );
    expect(fetch).toHaveBeenCalledTimes(3);
  });
  ...
  it.each([
    ...
    {
      ...comment,
      issue_url: "https://evil.example/repos/geetorus/example/issues/3",
    },
