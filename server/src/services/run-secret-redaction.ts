...
import { heartbeatRuns } from "@geetorusai/db";
import { REDACTED_EVENT_VALUE } from "../redaction.js";
import { getSecretProvider } from "../secrets/provider-registry.js";
import type { StoredSecretVersionMaterial } from "../secrets/types.js";

const REGISTRY_KEY = "geetorusSecretRedactions";
// Project only the registry: run contexts can contain megabytes of prompt data.
const registrySnapshot = sql`jsonb_build_object('geetorusSecretRedactions', ${heartbeatRuns.contextSnapshot} -> 'geetorusSecretRedactions')`;
    ...
    const rows = await db.select({ contextSnapshot: registrySnapshot })
      ...
      .where(and(
        ...
        or(
          ...
          sql`${heartbeatRuns.contextSnapshot} -> 'geetorusIssue' ->> 'id' = ${issueId}`,
        ),
      ));
    return valuesForRuns(rows);
  }
