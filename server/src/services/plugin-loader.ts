 ...
 *    (`~/.geetorus/plugins/`) and `node_modules` for packages matching
 *    the `geetorus-plugin-*` naming convention. Aggregates results with
...
import { PLUGIN_RPC_ERROR_CODES } from "@geetorusai/plugin-sdk";
import type {
  GeetorusPluginManifestV1,
...
const STANDALONE_BUNDLED_PLUGIN_SDK_PACKAGE = "@geetorusai/plugin-sdk";
...
/**
 * Naming convention for npm-published Geetorus plugins.
 * Packages matching this pattern are considered Geetorus plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "geetorus-plugin-";
 ...
 * `@geetorusai/plugin-kubernetes`) are allowed to read from the
 ...
 * runs in-cluster (e.g. `@geetorusai/plugin-kubernetes` with inCluster=true)
 ...
 * (`packagePath` null — the `@geetorusai` scope is project-controlled at
...
> = {
  "@geetorusai/plugin-createos": { driverKey: "createos", envVars: ["CREATEOS_API_KEY"] },
  "@geetorusai/plugin-daytona": { driverKey: "daytona", envVars: ["DAYTONA_API_KEY"] },
  "@geetorusai/plugin-e2b": { driverKey: "e2b", envVars: ["E2B_API_KEY"] },
  "@geetorusai/plugin-exe-dev": { driverKey: "exe-dev", envVars: ["EXE_API_KEY"] },
  "@geetorusai/plugin-novita-sandbox": { driverKey: "novita", envVars: ["NOVITA_API_KEY"] },
};

export function buildPluginWorkerEnv(input: {
  manifest: Pick<GeetorusPluginManifestV1, "capabilities"> & {
  ...
  const env: Record<string, string> = {
    GEETORUS_DEPLOYMENT_MODE: input.instanceInfo.deploymentMode ?? "",
    GEETORUS_DEPLOYMENT_EXPOSURE: input.instanceInfo.deploymentExposure ?? "",
  };
...
export interface DiscoveredPlugin {
  ...
  manifest: GeetorusPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.geetorus/plugins/ local directory
  | "npm"               // npm packages matching geetorus-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: GeetorusPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the geetorus-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "geetorus-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: GeetorusPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Geetorus plugins installed as npm packages in the current
   * Node.js environment matching the "geetorus-plugin-*" naming convention.
   *
   ...
   * the "geetorusPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Geetorus plugin.
   * Throws if the package is a Geetorus plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<GeetorusPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: GeetorusPluginManifestV1;
    newManifest: GeetorusPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Geetorus plugin naming convention.
 * Accepts both the "geetorus-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @geetorusai/plugin-*
  if (name.includes("/")) {
...
export function resolveDeclaredPluginEntrypoints(
  ...
  const geetorusPlugin = pkgJson["geetorusPlugin"];
  if (
    geetorusPlugin === null
    || typeof geetorusPlugin !== "object"
    || Array.isArray(geetorusPlugin)
  ) {
    return [];
  }
  ...
  for (const key of ["manifest", "worker", "ui"] as const) {
    const relativePath = (geetorusPlugin as Record<string, unknown>)[key];
...
function formatLocalPluginManualBuildHint(
  ...
  const autoBuildDisabled = (options.processEnv ?? process.env)["GEETORUS_DISABLE_PLUGIN_AUTOBUILD"] === "1"
    ? " Auto-build is disabled by GEETORUS_DISABLE_PLUGIN_AUTOBUILD=1."
    : "";
...
export async function ensureLocalPluginBuilt(
  ...
  if (processEnv["GEETORUS_DISABLE_PLUGIN_AUTOBUILD"] === "1") return;
  if (!isRepoBundledPluginPath(packageRoot, { repoRoot: options.repoRoot })) return;
 ...
 *
 * The spec defines a "geetorusPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  if (
    ...
    const manifestRelPath = (geetorusPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: GeetorusPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumGeetorusVersion;
}
 ...
 *   packageName: "geetorus-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: GeetorusPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as GeetorusPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      const manualBuildHint = localPath
        ...
        : "";
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Geetorus plugin (no manifest found).${manualBuildHint}`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<GeetorusPluginManifestV1> {
    let raw: unknown;
  ...
  async function loadManifestFromPackageRoot(
    packageRoot: string,
  ): Promise<GeetorusPluginManifestV1 | null> {
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Geetorus manifest`);
    }
   ...
   * if the package is not a Geetorus plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasGeetorusPlugin = "geetorusPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasGeetorusPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<GeetorusPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        packageName: string;
        packagePath: string | null;
        manifestJson: GeetorusPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(
    ...
    try {
      ...
      // (for example @geetorusai/shared exports). Run those workers through
