...
import { LOW_TRUST_REVIEW_PRESET, extractAgentMentionIds, type LowTrustBoundary } from "@geetorusai/shared";
import {
...
export function responsibleUserAuthzShadowMode() {
  const mode = process.env.GEETORUS_RESPONSIBLE_USER_AUTHZ_MODE?.trim().toLowerCase();
  const shadow = process.env.GEETORUS_RESPONSIBLE_USER_AUTHZ_SHADOW?.trim().toLowerCase();
