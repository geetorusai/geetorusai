...
export const RAILWAY_TOOL_PREFIX = "geetorus-railway-";
...
export const RAILWAY_QUERIES = {
  projects: `query GeetorusRailwayProjects($workspaceId:String!,$first:Int!,$after:String) { projects(workspaceId:$workspaceId,first:$first,after:$after) { edges { node { id name workspaceId } } ${pageInfo} } }`,
  services: `query GeetorusRailwayServices($projectId:String!,$first:Int!,$after:String) { project(id:$projectId) { id services(first:$first,after:$after) { edges { node { id name projectId } } ${pageInfo} } } }`,
  environments: `query GeetorusRailwayEnvironments($projectId:String!,$first:Int!,$after:String) { project(id:$projectId) { id environments(first:$first,after:$after) { edges { node { id name projectId } } ${pageInfo} } } }`,
  target: `query GeetorusRailwayTarget($projectId:String!,$environmentId:String!,$serviceId:String!) { project(id:$projectId) { id } environment(id:$environmentId) { id projectId } service(id:$serviceId) { id projectId } serviceInstance(environmentId:$environmentId,serviceId:$serviceId) { ${instanceFields} } }`,
  deployment: `query GeetorusRailwayDeployment($deploymentId:String!) { deployment(id:$deploymentId) { ${deploymentFields} instances { id } } }`,
  deployments: `query GeetorusRailwayDeployments($input:DeploymentListInput!,$first:Int!,$after:String) { deployments(input:$input,first:$first,after:$after) { edges { node { ${deploymentFields} } } ${pageInfo} } }`,
  buildLogs: `query GeetorusRailwayBuildLogs($deploymentId:String!,$limit:Int!,$startDate:DateTime,$endDate:DateTime,$filter:String) { buildLogs(deploymentId:$deploymentId,limit:$limit,startDate:$startDate,endDate:$endDate,filter:$filter) { timestamp message severity } }`,
  runtimeLogs: `query GeetorusRailwayRuntimeLogs($deploymentId:String!,$limit:Int!,$startDate:DateTime,$endDate:DateTime,$filter:String) { deploymentLogs(deploymentId:$deploymentId,limit:$limit,startDate:$startDate,endDate:$endDate,filter:$filter) { timestamp message severity } }`,
  redeploy: `mutation GeetorusRailwayRedeploy($deploymentId:String!) { deploymentRedeploy(id:$deploymentId,usePreviousImageTag:true) { id status } }`,
  restart: `mutation GeetorusRailwayRestart($deploymentId:String!) { deploymentRestart(id:$deploymentId) }`,
  rollback: `mutation GeetorusRailwayRollback($deploymentId:String!) { deploymentRollback(id:$deploymentId) }`,
};
...
export async function discoverRailwayWorkspace(options: RailwayClientOptions): Promise<string> {
  ...
  const list = (requestHeaders: Record<string, string>) => send({
    ...
    body: JSON.stringify({ jsonrpc: "2.0", id: "geetorus-railway-workspace-probe", method: "tools/call", params: { name: "list-workspaces", arguments: {} } }),
  });
  let response = await list(headers);
  if (response.status === 400) {
    await response.body?.cancel();
    response = await list(await initializeMcpHttpSession({ send, headers, requestId: "geetorus-railway-workspace-probe" }));
  }
  if (!response.ok) {
    await response.body?.cancel();
