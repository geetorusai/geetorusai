...
async function throwProviderWriteOrReservedRowRollbackError(input: {
  ...
  try {
    ...
    throw new HttpError(500, "Secret create failed and Geetorus could not roll back the local secret reservation.", {
...
function throwProviderCleanupFailedAfterCreateRollback(input: {
  ...
  const providerConfigId = providerConfigIdentifier(input);
  throw new HttpError(500, "Secret create failed and Geetorus could not clean up the remote provider secret.", {
...
function safeRemoteProviderErrorDetails(
  ...
  if (
    ...
    details.credentialPath = "Geetorus server runtime/provider credential path";
...
export function secretService(db: Db | DbTransaction) {
  ...
  async function resolveSecretValueForAgentAccess(
    ...
    if (context.bindingId) {
      ...
      const manifest = (asRecord(runContext.geetorusSecrets) ?? {}).manifest;
    ...
    const effectiveIssueId = context.issueId ?? (
      typeof runContext.issueId === "string"
        ? runContext.issueId
        : typeof (asRecord(runContext.geetorusIssue) ?? {}).id === "string"
          ? String((asRecord(runContext.geetorusIssue) ?? {}).id)
          : null
    );
  ...
  async function createSecretUnlocked(
    ...
    input: {
      ...
      value?: string | null;
      key?: string | null;
      managedMode?: "geetorus_managed" | "external_reference";
      description?: string | null;
    ...
    const managedMode = input.managedMode ?? "geetorus_managed";
    ...
    if (managedMode === "geetorus_managed" && input.externalRef?.trim()) {
      throw unprocessable("Managed secrets cannot override externalRef");
    }
    if (managedMode === "geetorus_managed" && !input.value?.trim()) {
  ...
  async function updateUnlocked(
    ...
    if (deleting && secret.managedMode === "geetorus_managed") {
    ...
    if (
      secret.managedMode === "geetorus_managed" &&
      patch.providerConfigId !== undefined &&
      patch.providerConfigId !== secret.providerConfigId
    ) {
      throw unprocessable(
  ...
  async function createUserSecretValueInternal(
    ...
    const managedMode = definition.managedMode as "geetorus_managed" | "external_reference";
    if (managedMode === "external_reference" && !input.externalRef?.trim()) {
      ...
      try {
        return await db
          ...
          .values({
            ...
            provider: input.provider,
            providerConfigId: input.providerConfigId ?? null,
            managedMode: input.managedMode ?? "geetorus_managed",
            providerMetadata: input.providerMetadata ?? null,
