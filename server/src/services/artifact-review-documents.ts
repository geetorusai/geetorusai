...
import { assets, documentRevisions, documents, issueAttachments, issueDocuments } from "@geetorusai/db";
import {
...
export function artifactReviewDocumentService(db: Db, storage: StorageService) {
  ...
  return {
    ensureForWorkProduct: async (
      ...
      if (!metadata) {
        throw unprocessable("Work product is not an attachment-backed Geetorus artifact", {
