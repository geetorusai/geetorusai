...
vi.mock("@chat-adapter/slack", async (importOriginal) => {
  const candidate = process.env.GEETORUS_SLACK_STREAM_ADAPTER_MODULE;
...
async function withNativeSlack(
  ...
  try {
    ...
    await run({
      stream: (chunks) => adapter.stream("slack:D-GEETORUS:1788.400", chunks),
      calls,
      start,
      append,
      stop,
  ...
  it("bounds actual serialized HTTP chunks after cached alias expansion", async () => {
    await withNativeSlack(async ({ stream, useHttp }) => {
      ...
      useHttp(async ({ method, body }) => {
        ...
        expect(body.get("channel")).toBe("D-GEETORUS");
        if (method === "chat.startStream")
          ...
          return Response.json({
            ...
            result: method.endsWith("Draft")
              ...
              : {
                  ...
                  from: { id: 123, is_bot: true, first_name: "Geetorus" },
                  text: source,
                },
          });
        }),
