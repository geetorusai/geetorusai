...
describe("chat provider conversation links", () => {
  it("links Slack and GitHub threads to their native conversation", () => {
    ...
    expect(
      chatProviderConversationUrl({
        provider: "github",
        threadId: "github:geetorusai/geetorus:issue:42",
        providerMessageId: "99",
      }),
    ).toBe(
      "https://github.com/geetorusai/geetorus/issues/42#issuecomment-99",
    );
    expect(
      chatProviderConversationUrl({
        provider: "github",
        threadId: "github:geetorusai/geetorus:43",
        providerMessageId: "100",
      }),
    ).toBe("https://github.com/geetorusai/geetorus/pull/43#issuecomment-100");
    expect(
      chatProviderConversationUrl({
        provider: "github",
        threadId: "github:geetorusai/geetorus:43:rc:101",
        providerMessageId: "102",
      }),
    ).toBe("https://github.com/geetorusai/geetorus/pull/43#discussion_r101");
  });
  ...
  it("links Telegram public and private forum topics and falls back to the bot DM", () => {
    expect(
      chatProviderConversationUrl({
        ...
        raw: { message_id: 88, chat: { username: "geetorus_e2e" } },
      }),
    ).toBe("https://t.me/geetorus_e2e/77/88");
    expect(
      chatProviderConversationUrl({
        provider: "telegram",
        threadId: "telegram:-100123456:77",
