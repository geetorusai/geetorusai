...
import { readClaudeToken, readIsolatedClaudeKeychainToken, fetchClaudeQuota } from "@geetorusai/adapter-claude-local/server";
import { readCodexAuthInfo, fetchCodexQuota } from "@geetorusai/adapter-codex-local/server";
import { parseGrokAuthPayload, hasUsableGrokAuthValue } from "@geetorusai/adapter-grok-local/server";
import type { AiProvider } from "@geetorusai/shared";
...
export async function readVerifiedLocalAiCredential(provider: AiProvider, loginHome?: string): Promise<string> {
  ...
  try {
    ...
    throw unprocessable(provider === "anthropic" && !loginHome
      ? "Could not verify the local subscription. Run claude auth login in a terminal on the machine running Geetorus, then try Connect again."
