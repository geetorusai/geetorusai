...
import { normalizeAgentUrlKey } from "@geetorusai/shared";
import { parseFrontmatterMarkdown } from "@geetorusai/shared/frontmatter";
...
function buildCatalogPackageRootCandidates() {
  const configuredRoot = process.env.GEETORUS_TEAMS_CATALOG_DIR?.trim();
  const candidates = [
...
async function statCatalogManifest() {
  ...
  throw new Error(
    `Teams catalog manifest not found. Checked: ${catalogPackageRootCandidates.map((root) => path.join(root, "generated/catalog.json")).join(", ")}. Run pnpm --filter @geetorusai/teams-catalog build:manifest.`,
  );
}
...
/**
 * Extract `metadata.geetorus.catalogTeam` provenance written by the team
...
export function readCatalogTeamProvenance(
  ...
  const geetorus = isPlainRecord(metadata.geetorus) ? metadata.geetorus : null;
  const catalogTeam = geetorus && isPlainRecord(geetorus.catalogTeam) ? geetorus.catalogTeam : null;
...
function defaultSafeCatalogAdapterType() {
  return process.env.GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE?.trim() || FALLBACK_SAFE_CATALOG_ADAPTER_TYPE;
}
...
export function teamsCatalogService(db: Db) {
  ...
  async function prepareCatalogTeamSource(
    ...
    const existingExtension =
      typeof files[".geetorus.yaml"] === "string"
        ? parseYamlDocument(files[".geetorus.yaml"])
        : {};
    const generatedExtension = parseYamlDocument(await renderCatalogProvenanceYaml(team, targetManager));
    files[".geetorus.yaml"] = renderYamlFile(mergePlainRecords(existingExtension, generatedExtension));
  ...
  async function installCatalogTeam(
    ...
    const warnings = [
      ...
      ...(defaultedAdapterSlugs.length > 0
        ? [
            `Catalog agents without explicit overrides (${defaultedAdapterSlugs.join(", ")}) default to ${defaultAdapterType}. Pass adapterOverrides or GEETORUS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE to use a different supported adapter.`,
          ]
        : []),
    ];
