 ...
 * `environments` section of `GEETORUS_MANAGED_CONFIG` (parsed fail-closed in
 * `managed-config.ts`). On boot, each declared environment is idempotently
 * ensured as the instance-level Geetorus-managed sandbox row via the
...
export async function applyManagedEnvironments(
  ...
  // The forced-execution-mode bootstrap (`GEETORUS_EXECUTION_MODE=kubernetes`)
  // and this one both own the single Geetorus-managed sandbox row
  ...
  if (parseExecutionPolicyBootstrapEnv(env)) {
    throw new Error(
      `GEETORUS_EXECUTION_MODE and the GEETORUS_MANAGED_CONFIG "environments" section are mutually exclusive: both manage the single instance sandbox environment`,
    );
  }
