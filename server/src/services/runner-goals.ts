...
function capabilityForAgent(agent: AgentBinding): RunnerGoalCapability {
  ...
  if (agent.adapterType === "geetorus_runner") {
    ...
    const reason = provider === "opencode"
      ...
      : provider === "acpx"
        ...
        : "This Geetorus runner provider does not expose a durable session goal lifecycle.";
    return {
  ...
  const reason = agent.adapterType === "opencode_local"
    ? "Unsupported by OpenCode."
    : directAcp
      ? "This direct ACP adapter has no live session goal controller. Use Geetorus Runner with a supported provider."
