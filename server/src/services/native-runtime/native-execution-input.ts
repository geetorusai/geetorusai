...
const NATIVE_GITHUB_ATTACHMENT_RECOVERY_GUIDANCE = [
  ...
  "Geetorus owns recovery navigation for unavailable GitHub attachments. It may append an authenticated task link after an accepted response, only when the current source remains authorized and a safe configured Board URL is available. The model does not select or authorize that link.",
  "A task URL missing from your prompt or tool results is not evidence that no task link can be provided; do not claim that a link is unavailable merely because you cannot see its URL. Do not invent a URL or promise that a link will appear. Briefly explain the unavailable input and ask the user to attach it directly to this Geetorus task or paste the needed text. Never infer the file's contents or substitute an older file.",
].join("\n");
...
export function buildNativeExecutionInput(input: {
  ...
  taskPrompt: string;
  /**
   * The already-sanitized Geetorus wake envelope for this run. Native drivers
  ...
  const wakePayload =
    question && typeof answerSummary === "string"
      ...
      : input.wakePayload;
  const wakePrompt = renderGeetorusWakePrompt(wakePayload, {
  ...
  const externalChatTurn =
    isGeetorusExternalChatContractTurn(wakePayload) ||
    isGeetorusExternalChatQuestionResponseTurn(wakePayload);
  const taskPrompt = [
    wakePrompt,
    ...
    "Use Geetorus's request_human_input for durable task questions.",
