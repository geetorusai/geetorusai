...
import {
  GeetorusSemanticDispatcher,
  type GeetorusJsonValue,
  type GeetorusSemanticActionBinding,
  type GeetorusSemanticActionId,
  type GeetorusSemanticAuthorizationRecord,
  type GeetorusSemanticRunContext,
  type GeetorusSemanticToolCall,
  type GeetorusSemanticToolDefinition,
  type GeetorusSemanticToolResult,
} from "../../vendor/geetorus-runner/index.js";

export interface GeetorusRunnerSemanticBinding {
...
] as const satisfies readonly GeetorusSemanticActionId[];
...
function requiredString(value: unknown): string {
  if (typeof value !== "string" || value.length === 0 || value.length > 240) {
    throw new Error("geetorus_runner_semantic_input_invalid");
  }
  return value;
}

function jsonValue(value: unknown): GeetorusJsonValue {
  return JSON.parse(JSON.stringify(value)) as GeetorusJsonValue;
}
 ...
 */
export class GeetorusRunnerSemanticAuthority {
  readonly #db: Db;
  readonly #binding: GeetorusRunnerSemanticBinding;
  readonly #dispatcher: GeetorusSemanticDispatcher;

  constructor(db: Db, binding: GeetorusRunnerSemanticBinding) {
    this.#db = db;
    this.#binding = structuredClone(binding);
    this.#dispatcher = new GeetorusSemanticDispatcher({
  ...
  listAlwaysAvailableTools(): Promise<
    readonly GeetorusSemanticToolDefinition[]
  > {
    return this.#dispatcher.listAlwaysAvailableTools(this.#binding.runId);
  }

  dispatch(
    call: Omit<GeetorusSemanticToolCall, "runId">,
        ...
        switch (operationId) {
          ...
          case "read_document": {
            ...
            const [row] = await this.#db
              ...
              .limit(1);
            if (!row) throw new Error("geetorus_runner_document_not_found");
            return {
  ...
  async #context(requestedRunId: string): Promise<GeetorusSemanticRunContext> {
    if (requestedRunId !== this.#binding.runId) {
      throw new Error("geetorus_runner_semantic_run_mismatch");
    }
    const context = await this.#loadBoundContext();
    ...
    return {
      ...
      activeTask: {
        ...
        workMode: context.issue
          .workMode as GeetorusSemanticRunContext["activeTask"]["workMode"],
      },
      delegatedClaims: [],
    };
  }
  ...
  async #loadBoundContext(): Promise<BoundContext> {
    const [row] = await this.#db
      ...
      .limit(1);
    if (!row) throw new Error("geetorus_runner_semantic_binding_not_found");
    return row;
  }
  ...
  #assertActiveContext(context: BoundContext, requireOwnership: boolean): void {
    if (
      ...
      throw new Error("geetorus_runner_semantic_binding_inactive");
    }
  }
}
