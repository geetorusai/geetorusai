...
describe("remote deliverable admission", () => {
  it.each([
    ...
    const execute = vi.fn();
    await expect(readVerifiedRemoteWorkspaceFile({ ...request, ...override, runner: { execute } })).rejects.toThrow(/geetorus_runner_file_handoff_/);
  ...
  it("sanitizes a rejected remote transport error", async () => {
    ...
    await expect(readVerifiedRemoteWorkspaceFile({ ...request, runner: { execute } }))
      .rejects.toThrow("geetorus_runner_file_handoff_remote_read_failed");
  });
  ...
  it.each([
    ...
    await expect(readVerifiedRemoteWorkspaceFile({ ...request, runner: { execute } })).rejects.toThrow(/geetorus_runner_file_handoff_/);
  });
});
...
describe.skipIf(!hasLinuxNode)("remote deliverable real Linux descriptor reads", () => {
  ...
  beforeEach(async () => {
    root = await realpath(await mkdtemp(join(tmpdir(), "geetorus-remote-file-")));
  ...
  it.each(["symlink", "parent symlink", "hardlink", "directory", "wrong size", "wrong hash"])("rejects %s without returning file bytes", async (scenario) => {
    ...
    await expect(readVerifiedRemoteWorkspaceFile({ ...request, workspaceRoot, contentRef, runner, ...values })).rejects.toThrow(/geetorus_runner_file_handoff_/);
  });
  ...
  it.each(["replace", "grow", "hardlink"])("detects a %s during the descriptor read", async (mutation) => {
    ...
    `);
    await expect(readVerifiedRemoteWorkspaceFile({ ...request, workspaceRoot, runner })).rejects.toThrow(/geetorus_runner_file_handoff_file_changed/);
  });
