...
export type QualifiedGeetorusRunnerAcpxAgent =
  keyof typeof QUALIFIED_ACPX_RUNNER_MODELS;

export type GeetorusRunnerProviderProfile =
  | {
      provider: "codex";
  ...
  | {
      ...
      model: string;
      acpxAgent: QualifiedGeetorusRunnerAcpxAgent;
    };

export type GeetorusRunnerNativeProviderInput =
  | {
      provider: "codex";
      model: string | null;
...
export class GeetorusRunnerProviderProfileError extends Error {
  constructor(
    readonly code: string,
    message: string,
  ) {
    super(message);
    this.name = "GeetorusRunnerProviderProfileError";
  }
}
...
function positiveNumberOrNull(
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new GeetorusRunnerProviderProfileError(code, message);
  }
  return parsed;
}
...
function boundedPositiveInteger(
  ...
  if (
    ...
    throw new GeetorusRunnerProviderProfileError(
      code,
...
function assertPermissionMode(
  ...
  if (
    ...
    && resolveGeetorusRunnerPermissionMode(provider, configured) !== configured
  ) {
    if (provider === "codex") {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_codex_permission_mode_unqualified",
        "Geetorus Runner currently supports Codex only with codexPermissionMode set to never. Update the agent configuration before starting a new native run.",
      );
    }
    throw new GeetorusRunnerProviderProfileError(
...
export function assertManagedProfileRecoveryBinding(input: {
  ...
  if (
    ...
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_claude_managed_recovery_identity_mismatch",
  ...
  if (boundSecretId !== stored.apiKeySecretId) {
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_claude_managed_recovery_secret_mismatch",
...
export function assertAgentCoreProfileRecoveryBinding(input: {
  ...
  if (
    ...
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_aws_agentcore_recovery_identity_mismatch",
...
/**
 * Resolve the immutable provider identity used for a fresh Geetorus Runner
 ...
 */
export function resolveGeetorusRunnerProviderProfile(
  adapterConfig: unknown,
): GeetorusRunnerProviderProfile {
  const config = asRecord(adapterConfig);
  const candidate = config.provider ?? "codex";
  if (!isGeetorusRunnerProvider(candidate)) {
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_provider_unsupported",
      "Geetorus Runner provider must be Codex, OpenCode, Claude Managed, AWS AgentCore, or ACPX.",
    );
  }
  ...
  if (candidate === "opencode") {
    if (!model || !model.includes("/") || model.endsWith("/")) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_opencode_model_invalid",
        "Geetorus Runner OpenCode requires model in provider/model form.",
      );
    }
    return {
      provider: "opencode",
  ...
  if (candidate === "claude_managed") {
    ...
    if (!managedProfileId) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_claude_managed_profile_required",
        "Geetorus Runner Claude Managed requires a company managed-agent profile.",
      );
    }
    if (config.managedAgentsRetentionAcknowledged !== true) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_claude_managed_retention_required",
        "Geetorus Runner Claude Managed requires acknowledgement of stateful beta retention.",
      );
    }
    if (model !== null && model !== CLAUDE_MANAGED_QUALIFIED_MODEL) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_claude_managed_model_unqualified",
    ...
    return {
      ...
      maxSessionListCostUsd: positiveNumberOrNull(
        ...
        "geetorus_runner_claude_managed_spend_cap_invalid",
        "Geetorus Runner Claude Managed requires a positive session spend ceiling when overridden.",
      ),
    };
  }
  ...
  if (candidate === "aws_agentcore") {
    ...
    if (!agentCoreProfileId) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_aws_agentcore_profile_required",
        "Geetorus Runner AWS AgentCore requires a company remote-agent profile.",
      );
    }
    if (config.agentCoreRetentionAcknowledged !== true) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_aws_agentcore_retention_required",
        "Geetorus Runner AWS AgentCore requires acknowledgement of 90-day Memory retention.",
      );
    }
    if (model !== null && model !== AGENTCORE_QUALIFIED_MODEL) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_aws_agentcore_model_unqualified",
    ...
    return {
      ...
      maxEstimatedSessionCostUsd: positiveNumberOrNull(
        ...
        "geetorus_runner_aws_agentcore_spend_cap_invalid",
        "Geetorus Runner AWS AgentCore requires a positive estimated session spend ceiling when overridden.",
      ),
    };
  }
  ...
  if (acpxAgent !== "claude" && acpxAgent !== "codex") {
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_acpx_agent_unavailable",
      "Geetorus Runner ACPX requires the qualified Claude or Codex agent profile; Pi is not available.",
    );
  }
  const qualifiedModel = QUALIFIED_ACPX_RUNNER_MODELS[acpxAgent];
  if (acpxAgent === "codex" && model !== qualifiedModel) {
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_acpx_model_unqualified",
      `Geetorus Runner ACPX ${acpxAgent} requires exact model ${qualifiedModel}.`,
    );
  }
  return {
    provider: "acpx",
 ...
 */
export function resolveGeetorusRunnerNativeProviderInput(input: {
  backend: GeetorusRunnerProviderProfile["backend"];
  adapterConfig: unknown;
  managedProfile?: {
    id: string;
    profileKey: string;
...
}): GeetorusRunnerNativeProviderInput {
  const config = asRecord(input.adapterConfig);
  const profile = resolveGeetorusRunnerProviderProfile(config);
  if (profile.backend !== input.backend) {
    throw new GeetorusRunnerProviderProfileError(
      "geetorus_runner_provider_changed",
      "Geetorus Runner provider changed after this run selected its native backend.",
    );
  }
  if (profile.provider === "opencode") {
    return {
      provider: "opencode",
      model: profile.model,
      opencodePermissionMode: resolveGeetorusRunnerPermissionMode(
        "opencode",
  ...
  if (profile.provider === "acpx") {
    return {
      ...
      acpxPermissionMode: resolveGeetorusRunnerPermissionMode(
        "acpx",
  ...
  if (profile.provider === "claude_managed") {
    ...
    if (
      ...
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_claude_managed_profile_mismatch",
    ...
    if (stored.betaVersion !== CLAUDE_MANAGED_BETA_VERSION) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_claude_managed_beta_unqualified",
    ...
    if (!model) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_claude_managed_model_invalid",
  ...
  if (profile.provider === "aws_agentcore") {
    ...
    if (
      ...
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_aws_agentcore_profile_mismatch",
    ...
    const required = (key: string): string => {
      ...
      if (!value) {
        throw new GeetorusRunnerProviderProfileError(
          "geetorus_runner_aws_agentcore_profile_invalid",
    ...
    if (remote.eventExpiryDays !== 90) {
      throw new GeetorusRunnerProviderProfileError(
        "geetorus_runner_aws_agentcore_retention_unqualified",
    ...
    return {
      ...
      invocationLimits: {
        maxIterations: boundedPositiveInteger(
          ...
          8,
          8,
          "geetorus_runner_aws_agentcore_max_iterations_invalid",
        ...
        maxOutputTokens: boundedPositiveInteger(
          ...
          4_096,
          4_096,
          "geetorus_runner_aws_agentcore_max_output_tokens_invalid",
        ...
        timeoutSeconds: boundedPositiveInteger(
          ...
          300,
          300,
          "geetorus_runner_aws_agentcore_timeout_invalid",
  ...
  return {
    provider: "codex",
    model: profile.model,
    codexApprovalPolicy: resolveGeetorusRunnerPermissionMode(
      "codex",
