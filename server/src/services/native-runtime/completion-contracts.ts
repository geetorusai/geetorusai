...
import { completionContracts } from "@geetorusai/db";
import type { StrictCompletionContractInput } from "../../vendor/geetorus-runner/index.js";

import { nativeSha256 } from "./canonical.js";

export const NATIVE_COMPLETION_CONTRACT_SCHEMA = "geetorus.completion-contract.v1";
...
export async function ensureNativeCompletionContract(input: {
  ...
  return input.db.transaction(async (tx) => {
    await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${[
      "geetorus:native-completion-contract",
      input.companyId,
      input.issue.id,
    ].join(":")}, 0))`);
