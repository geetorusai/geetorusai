...
import { connectionsSearchInputSchema, connectionRequestInputSchema, CONNECTION_INTENT_AGENT_GUIDANCE } from "@geetorusai/shared";
import { createHash } from "node:crypto";
import { geetorusChatFilePreparationDelivery } from "@geetorusai/adapter-utils/chat-file-delivery";
import {
  isGeetorusExternalChatContractTurn,
  normalizeGeetorusWakePayload,
...
import { CAPABILITY_SEMANTIC_TOOL_CATALOG } from "../../vendor/geetorus-runner/index.js";
...
export class GeetorusRunnerToolAuthority {
  ...
  definitions(): Array<Record<string, unknown>> {
    ...
    const definitions: Array<Record<string, unknown>> =
      CAPABILITY_SEMANTIC_TOOL_CATALOG.filter(
        ...
        description:
          descriptor.operationId === "register_deliverable"
            ? "Prepare one verified workspace file for Geetorus's final task or external-chat response. This records the attachment, work product, and explicit same-run selection; it does not confirm provider delivery."
  ...
  async execute(call: {
    ...
    if (isConnectorTool(call.tool)) {
      ...
      if (isGeetorusExternalChatContractTurn(snapshot.geetorusWake) || String(snapshot.source ?? "").startsWith("chat:") || snapshot.geetorusExternalChatQuestionResponse) throw forbidden("Restricted chat runs cannot use email actions");
    ...
    if (
      ...
      throw new Error("geetorus_runner_tool_not_advertised");
    }
    if (
      !runnerApiToolsEnabled(
        this.binding.companyId,
    ...
    if (call.tool === READ_CHAT_ATTACHMENT_TOOL_NAME) {
      ...
      if (!scope || identityKeys.some((key) => scope.options.binding[key] !== this.binding[key])) {
        throw new Error("geetorus_runner_chat_attachment_read_scope_unavailable");
      }
      if (Object.keys(input).some((key) => key !== "sourceCommentId" && key !== "attachmentId")) {
        throw new Error("geetorus_runner_chat_attachment_read_arguments_invalid");
      }
      return scope.read({
    ...
    if (
      ...
      throw new Error("geetorus_runner_tool_mode_denied");
    }
    switch (call.tool) {
      case "create_skill": {
        const apiUrl = this.binding.apiUrl ?? process.env.GEETORUS_API_URL;
      ...
      case "read_document": {
        ...
        if (!document) throw new Error("geetorus_runner_document_not_found");
        return { document };
      }
      ...
      case "get_agent": {
        ...
        if (!actor || actor.companyId !== this.binding.companyId) throw new Error("geetorus_runner_agent_not_found");
      ...
      default: throw new Error("geetorus_runner_tool_not_bound");
    }
  }
  ...
  async #callApi(callId: string, value: unknown): Promise<unknown> {
    ...
    const apiUrl = this.binding.apiUrl ?? process.env.GEETORUS_API_URL;
    if (!apiUrl) throw new Error("Geetorus API origin is unavailable");
    ...
    if (!token) throw new Error("Geetorus run authentication is unavailable");
    const execute = async () => {
      const current = await this.#boundContext();
      if (!runnerApiToolsEnabled(this.binding.companyId, this.binding.apiToolsEnabled)) throw new Error("geetorus_runner_tool_not_advertised");
  ...
  async #approval(id: string) {
    ...
    if (!approval || approval.companyId !== this.binding.companyId) throw new Error("geetorus_runner_approval_not_found");
    return approval;
  }
  ...
  async #boundContext() {
    ...
    if (
      ...
      throw new Error("geetorus_runner_tool_binding_not_authorized");
    }
    return row;
  }
  ...
  async #reportProgress(input: Record<string, unknown>): Promise<unknown> {
    ...
    if (!body || !idempotencyKey) throw new Error("geetorus_runner_tool_input_invalid");
    ...
    const result = await this.#withMutationReceipt(
      ...
      async (tx, context) => {
        const comment = await issueService(tx).addComment(
          ...
          { authorizationReason: "geetorus_runner_protocol" },
          tx,
        );
        ...
        const activity = await persistActivity(tx, {
          ...
          details: {
            ...
            authorizationReason: "geetorus_runner_protocol",
            source: "geetorus_runner_protocol",
          },
        });
  ...
  async #createTask(input: Record<string, unknown>, identityContextId: string | null): Promise<unknown> {
    ...
    if (!assignee || assignee.companyId !== this.binding.companyId) {
      throw new Error("geetorus_runner_agent_not_found");
    }
    ...
    const durableIdempotencyKey =
      `geetorus-runner:create-task:${this.binding.issueId}:${idempotencyKey}`;
    ...
    const result = await this.#withMutationReceipt("create_task", idempotencyKey, input, async (tx, context) => {
      ...
      if (existingChild) {
        if (existingChild.originFingerprint !== inputFingerprint) {
          throw new Error("geetorus_runner_tool_idempotency_conflict");
        }
        return {
      ...
      if (!deduplicated) {
        const activity = await persistActivity(tx, {
          ...
          details: { identifier: child.identifier, title: child.title, parentId: child.parentId,
            assigneeAgentId: child.assigneeAgentId, status: childStatus, source: "geetorus_runner_protocol" },
        });
        publication = activity.publication;
      }
    ...
    if (this.binding.enqueueWakeup && assignedAgentId && scheduledWakeIds.length > 0) {
      await this.binding.enqueueWakeup(assignedAgentId, {
        ...
        contextSnapshot: {
          issueId: childId,
          source: "geetorus_runner.create_task",
          parentIssueId: task.parentId ?? null,
        },
      });
    }
  ...
  async #setDependencies(input: Record<string, unknown>): Promise<unknown> {
    ...
    if (!Array.isArray(input.blockedByTaskIds)) {
      throw new Error("geetorus_runner_tool_input_invalid");
    }
    ...
    return this.#withMutationReceipt("set_dependencies", idempotencyKey, input, async (tx) => {
      ...
      if (!updated) throw new Error("geetorus_runner_task_not_found");
      return {
  ...
  async #registerDeliverable(input: Record<string, unknown>): Promise<unknown> {
    ...
    if (!workspaceRoot) {
      throw new Error("geetorus_runner_file_handoff_workspace_unavailable");
    }
    let publication:
  ...
  async #reuseChatAttachment(input: Record<string, unknown>): Promise<unknown> {
    ...
    if (title.length > 500) throw new Error("geetorus_runner_tool_input_invalid");
      ...
      async (tx, context) => {
        ...
        const activity = await persistActivity(tx, {
          ...
          details: {
            ...
            identifier: context.issue.identifier,
            issueTitle: context.issue.title,
            source: "geetorus_runner_chat_attachment_reuse",
            sourceCommentId,
  ...
  async #withMutationReceipt(
    ...
    return this.db.transaction(async (tx) => {
      try {
        ...
        const wake = snapshot.geetorusWake;
        const normalized = normalizeGeetorusWakePayload(wake);
        let provider =
          normalized?.issue?.id === this.binding.issueId &&
          isGeetorusExternalChatContractTurn(wake)
            ? normalized.externalChatProvider
            : null;
        if (
          isFilePreparation &&
          snapshot.source === "issue.interaction.respond" &&
          snapshot.geetorusExternalChatQuestionResponse
        ) {
          ...
          if (!endpoint || endpoint.provider === "agentmail") {
            throw new Error("geetorus_runner_chat_attachment_binding_denied");
          }
        ...
        const describeResult = (result: unknown): unknown => {
          ...
          return {
            ...record(result),
            fileDelivery: geetorusChatFilePreparationDelivery(provider),
          };
        };
        const resultJson = record(context.run.resultJson);
  ...
  async #requestHumanInput(input: Record<string, unknown>, identityContextId: string | null): Promise<unknown> {
    ...
    if (!kind) throw new Error("geetorus_runner_interaction_kind_invalid");
    ...
    const result = await this.#withMutationReceipt(
      ...
      async (tx, context) => {
        ...
        if (targetRevisionId !== null && suppliedPayload.target === undefined && inferredPlanningTarget === null) {
          throw new Error("geetorus_runner_interaction_target_incomplete");
        }
...
function requiredString(value: unknown): string {
  if (typeof value !== "string" || value.trim() === "") throw new Error("geetorus_runner_tool_input_invalid");
  return value.trim();
}
