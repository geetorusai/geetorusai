...
describe("native execution input external-chat framing", () => {
  ...
  it.each([false, true])(
    ...
    (resumedSession) => {
      ...
      expect(input.task.prompt).toContain(
        "Geetorus owns recovery navigation for unavailable GitHub attachments",
      );
      expect(input.task.prompt).toContain(
      ...
      expect(input.task.prompt).toContain(
        "Briefly explain the unavailable input and ask the user to attach it directly to this Geetorus task or paste the needed text",
      );
      expect(input.task.prompt).toContain(
  ...
  it.each([
    ...
    const input = buildNativeExecutionInput({
      ...
      completionContract: {
        id: "70000000-0000-4000-8000-000000000007", sha256: `sha256:${"a".repeat(64)}`, schemaVersion: "geetorus.run-result.v1",
    ...
    expect(input.task.prompt).not.toContain("## Questions that need a user response");
    expect(input.task.prompt).toContain("Use Geetorus's request_human_input for durable task questions.");
