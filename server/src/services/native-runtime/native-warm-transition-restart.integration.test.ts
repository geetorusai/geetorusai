...
import type { NativeExecutionInputV1 } from "@geetorusai/geetorus-runner";
...
// Generate with GEETORUS_ATTACH_TRANSITION_FIXTURE_DIRECTORY, then explicitly
// supply that directory and a fresh migrated GEETORUS_TEST_DATABASE_URL here.
...
const fixtureDirectory =
  process.env.GEETORUS_WARM_TRANSITION_FIXTURE_DIRECTORY;
const managedFixtureDirectory =
  process.env.GEETORUS_WARM_TRANSITION_MANAGED_FIXTURE_DIRECTORY;
const databaseUrl = process.env.GEETORUS_TEST_DATABASE_URL;
const generated =
...
vi.mock("../../vendor/geetorus-runner/index.js", async (original) => ({
  ...(await original<
...
import { readRunnerdArtifactBinding } from "../../vendor/geetorus-runner/index.js";
...
generated(
  ...
  () => {
    it.each(cases)("%s / %s", async (fixtureName, fault) => {
      ...
      expect(fixture.schema).toBe("geetorus.test.warm-transition-fixture.v1");
      ...
      const oldEnvironment = {
        state: process.env.GEETORUS_RUNNER_STATE_DIR,
        binary: process.env.GEETORUS_RUNNER_BINARY,
      };
      ...
      try {
        ...
        process.env.GEETORUS_RUNNER_BINARY = fixture.artifact.path;
        ...
        if (fault === "wrong_artifact") {
          ...
          process.env.GEETORUS_RUNNER_BINARY = alternative;
        }
        ...
        try {
          ...
          else process.env.GEETORUS_RUNNER_STATE_DIR = oldEnvironment.state;
          if (oldEnvironment.binary === undefined)
            delete process.env.GEETORUS_RUNNER_BINARY;
          else process.env.GEETORUS_RUNNER_BINARY = oldEnvironment.binary;
