...
import { environmentLeases, heartbeatRuns } from "@geetorusai/db";
import type { EnvironmentLease } from "@geetorusai/shared";
import {
  prepareAdapterExecutionTargetRuntime,
  type AdapterExecutionTarget,
  type PreparedAdapterExecutionTargetRuntime,
} from "@geetorusai/adapter-utils/execution-target";
import type { GitWorkspaceSnapshot } from "@geetorusai/adapter-utils/git-workspace-sync";
import {
...
} from "@geetorusai/adapter-utils/sandbox-managed-runtime";
...
const DESCRIPTOR_SCHEMA = "geetorus.native-workspace-sync/v1";
const STAMP_SCHEMA = "geetorus.native-workspace-stamp/v1";
...
function parseGitSnapshot(
  ...
  if (candidate.repositories !== undefined) {
    ...
    for (const raw of candidate.repositories) {
      ...
      if (typeof repo.path !== "string" || !/^\.geetorus-repositories\/[a-zA-Z0-9_-]+$/.test(repo.path) || seen.has(repo.path)) return undefined;
...
async function finalizePreparedRuntime(input: {
  ...
  const finalSnapshot =
    await import("@geetorusai/adapter-utils/workspace-restore-merge").then(
