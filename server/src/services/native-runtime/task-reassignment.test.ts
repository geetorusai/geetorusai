...
import { activityLog, agents, companies, createDb, heartbeatRuns, issues, issueComments } from "@geetorusai/db";
...
describe("runner task reassignment", () => {
  ...
  async function fixture(status = "todo") {
    ...
    await db.insert(agents).values([
      { id: agentId, companyId, name: "Manager", role: "ceo", status: "active", adapterType: "geetorus_runner" },
      { id: nextId, companyId, name: "Engineer", role: "engineer", status: "active", adapterType: "geetorus_runner" },
    ]);
  ...
  it("replays across a replacement caller run without a second mutation", async () => {
    ...
    await expect(new GeetorusRunnerToolAuthority(db, { ...f.binding, runId: replacement }).execute(f.call)).resolves.toEqual(receipt);
