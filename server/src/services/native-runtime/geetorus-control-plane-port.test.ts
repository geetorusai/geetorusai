...
describe("GeetorusControlPlanePort conformance", () => {
  ...
  beforeAll(async () => {
    temporary = await startEmbeddedPostgresTestDatabase("geetorus-native-port-");
  ...
  it("runs the unchanged package conformance suite against Geetorus persistence", async () => {
    const identity = CONTROL_PLANE_CONFORMANCE_OPEN.identity;
    const committedEventIds: string[] = [];
    const duplicateEventIds: string[] = [];
    const port = new GeetorusControlPlanePort(
      db,
      {
        companyId: identity.companyId,
        issueId: identity.issueId,
  ...
  it("signals committed safe progress after row visibility without forwarding its payload", async () => {
    ...
    const progressEvent: PrpEvent = {
      ...
      payload: {
        schema: "geetorus.tool.execution.v1",
  ...
  it("recovers a runtime question when the event commits before its callback", async () => {
    ...
    const port = new GeetorusControlPlanePort(db, binding, {
  ...
  it("persists a delayed final answer and replay before resolving the selected task response", async () => {
    ...
    const event = (sourceSeq: number, eventType: PrpEvent["eventType"], payload: Record<string, unknown>): PrpEvent => ({
      ...
      runId: taskRunId,
      normalizedSessionId: sessionId,
      turnId: "turn-phase6-geetorus-task",
      eventType,
      schemaVersion: 1,
      priority: 0,
    ...
    const backend: NativeSessionBackend = {
      ...
      async openSession(input) {
        return {
          ...
          async startTurn() { return { turnId: "turn-phase6-geetorus-task" }; },
          cancel() { throw new Error("A completion report must not interrupt the provider"); },
          async result() { return { result: taskResult, terminal: CONTROL_PLANE_CONFORMANCE_TERMINAL, turnId: "turn-phase6-geetorus-task" }; },
          async snapshot() { return { backendKind: "mock", sessionId, identity: input.identity, providerSessionId: "provider-phase6-geetorus-task" }; },
          async close() {},
        };
      },
    };
  ...
  it("revalidates every persisted native binding before open and completion", async () => {
    ...
    const createPort = () => new GeetorusControlPlanePort(db, {
      companyId: identity.companyId,
      issueId,
      runId,
      agentId: identity.agentId,
