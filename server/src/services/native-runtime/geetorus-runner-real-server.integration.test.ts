...
import { agents, companies, createDb, heartbeatRuns, issues } from "@geetorusai/db";
...
const fakeCodexAppServer = resolve(
  import.meta.dirname,
  "../../../../packages/geetorus-runner/runner/target/debug/fake-codex-app-server",
);
...
describe("geetorus-runner real server vertical slice", () => {
  ...
  beforeAll(async () => {
    temporary = await startEmbeddedPostgresTestDatabase("geetorus-runner-real-server-");
  });
  ...
  runnerBinaryIt("runs Rust runnerd through Geetorus PRP and reads the real bound task", async () => {
    ...
    const stateDirectory = await mkdtemp(resolve(tmpdir(), "geetorus-runner-real-resume-"));
    ...
    try {
      ...
      const resumedAuthority = new GeetorusRunnerToolAuthority(db, {
        companyId,
        agentId,
        issueId,
        runId: resumedRunId,
