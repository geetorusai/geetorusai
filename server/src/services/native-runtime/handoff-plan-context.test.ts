...
import { createDb, companies, agents, issues, heartbeatRuns, documents, documentRevisions, issueDocuments, issueThreadInteractions } from "@geetorusai/db";
...
(support.supported ? describe : describe.skip)("handoff approval evidence", () => {
  ...
  async function seed() {
    ...
    await db.insert(agents).values({ id: agentId, companyId, name: "Planner", adapterType: "geetorus_runner" });
