...
import type { SourceTrustMetadata } from "@geetorusai/shared";
...
export const CURRENT_WAKE_COMMENTS_BINDING_SCHEMA =
  "geetorus.current-wake-comments-binding.v1";
export const CURRENT_WAKE_COMMENTS_RECEIPT_SCHEMA =
  "geetorus.current-wake-comments-receipt.v1";
...
type CurrentWakeCommentsCursor = {
  schema: "geetorus.current-wake-comments-cursor.v1";
  bindingDigest: string;
  snapshotDigest: string;
  commentIndex: number;
  bodyOffset: number;
...
export type CurrentWakeCommentsPage = {
  schema: "geetorus.current-wake-comments-page.v1";
  bindingDigest: string;
  snapshotDigest: string;
...
function attachmentImportNotice(
  ...
  const reasons = entries
    ...
    .join(", ");
  return `Geetorus could not import every attachment from this exact external message: ${omitted} attachment${omitted === 1 ? " was" : "s were"} omitted (${reasons}). Treat omitted attachments as unavailable; do not infer their contents or substitute an older workspace file.`;
}
...
export async function resolveCurrentWakeCommentsBinding(
  ...
  const wake = record(record(bound.run.contextSnapshot).geetorusWake);
...
function decodeCursor(
  ...
  if (
    ...
    throw new Error("geetorus_current_wake_comments_cursor_invalid");
  }
  try {
    const parsed = record(
    ...
    if (
      parsed.schema !== "geetorus.current-wake-comments-cursor.v1" ||
    ...
    if (
      ...
      error.message === "geetorus_current_wake_comments_cursor_invalid"
    ) {
      throw error;
    }
    throw new Error("geetorus_current_wake_comments_cursor_invalid");
...
function parseReceipt(value: unknown): CurrentWakeCommentsReceipt | null {
  ...
  if (
    ...
    result.schema !== "geetorus.current-wake-comments-page.v1"
  ) {
    return null;
  }
...
async function currentWakeCommentsSnapshot(
  ...
  for (const attachment of attachments) {
    ...
    current.push({
      ...
      // This closed reader intentionally has no general Geetorus API key.
...
export async function readCurrentWakeComments(
  ...
  if (
    ...
    throw new Error("geetorus_current_wake_comments_input_invalid");
  }
  ...
  return db.transaction(async (tx) => {
    ...
    if (
      ...
      throw new Error("geetorus_current_wake_comments_binding_not_authorized");
    }
    ...
    if (
      ...
      throw new Error("geetorus_current_wake_comments_binding_changed");
    }
    ...
    if (
      ...
      if (inputCursor !== null) {
        throw new Error("geetorus_current_wake_comments_snapshot_changed");
      }
    } else if (!prior && inputCursor !== null) {
      throw new Error("geetorus_current_wake_comments_cursor_out_of_order");
    } else if (prior && prior.nextCursor !== inputCursor) {
      throw new Error(
        prior.complete
          ? "geetorus_current_wake_comments_already_complete"
          : "geetorus_current_wake_comments_cursor_out_of_order",
      );
    }
