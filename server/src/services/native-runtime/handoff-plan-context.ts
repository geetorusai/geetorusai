...
import { documentRevisions, heartbeatRuns, issueDocuments, issues, issueThreadInteractions, type Db } from "@geetorusai/db";
...
export async function handoffPlanContext(db: Db, task: typeof issues.$inferSelect) {
  ...
  const sourceId = sourceRun?.nativeIssueId ?? (sourceRun?.contextSnapshot as Record<string, unknown> | null)?.issueId;
