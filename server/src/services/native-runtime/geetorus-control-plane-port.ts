...
export interface GeetorusControlPlaneBinding {
  companyId: string;
  issueId: string;
  runId: string;
  agentId: string;
...
function assertTerminal(value: unknown): asserts value is PrpTerminalState {
  ...
  if (
    terminal?.schema !== "geetorus.prp.terminal.v1"
...
export class GeetorusControlPlanePort implements ControlPlanePort {
  readonly #db: Db;
  readonly #binding: GeetorusControlPlaneBinding;
  ...
  constructor(
    db: Db,
    binding: GeetorusControlPlaneBinding,
    options: {
