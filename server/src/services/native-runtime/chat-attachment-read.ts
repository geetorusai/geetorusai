...
export class NativeChatAttachmentReadScope {
  ...
  #assertOpen() {
    if (this.#closed)
      throw new Error("geetorus_runner_chat_attachment_read_scope_closed");
  }
  ...
  read(input: { sourceCommentId: string; attachmentId: string }) {
    ...
    if (
      ...
      throw new Error(
        "geetorus_runner_chat_attachment_read_arguments_invalid",
      );
    }
    if (this.options.executionTargetKind !== "local") {
      throw new Error(
        "geetorus_runner_chat_attachment_remote_staging_unsupported",
      );
    }
    if (++this.#readCount > 20)
      throw new Error("geetorus_runner_chat_attachment_read_limit");
  ...
  async #authorized(input: {
    ...
    for (;;) {
      ...
      try {
        ...
        if (remaining <= 0) {
          throw new Error(
            "geetorus_runner_chat_attachment_read_busy: chat authorization is temporarily busy; retry this read shortly",
          );
        }
  ...
  async #authorizeOnce(input: {
    ...
    return this.options.db.transaction(async (transaction) => {
      ...
      if (authorization !== "authorized")
        throw new Error("geetorus_runner_chat_attachment_read_not_authorized");
      const [run] = await tx
  ...
  async #read(input: { sourceCommentId: string; attachmentId: string }) {
    ...
    if (
      ...
      throw new Error("geetorus_runner_chat_attachment_read_source_changed");
    }
  ...
  async #bytes(source: ChatAttachmentReuseSource): Promise<Buffer> {
    ...
    const aborted = new Promise<never>((_, reject) => {
      signal.addEventListener(
        ...
        () => {
          ...
          reject(new Error("geetorus_runner_chat_attachment_read_aborted"));
        },
        { once: true },
      );
    });
    const acquiring = storage
      ...
      .then((value) => {
        if (signal.aborted) {
          ...
          throw new Error("geetorus_runner_chat_attachment_read_aborted");
        }
        object = value;
        return value;
      });
    const read = (async () => {
      ...
      try {
        for await (const chunk of value.stream) {
          ...
          if (length > source.byteSize || length > MAX_ATTACHMENT_BYTES)
            throw new Error(
              "geetorus_runner_chat_attachment_read_size_mismatch",
            );
          chunks.push(bytes);
        }
      } finally {
      ...
      if (
        ...
        throw new Error(
          "geetorus_runner_chat_attachment_read_integrity_mismatch",
        );
      return body;
    })();
  ...
  close(): Promise<void> {
    ...
    this.#closing = (async () => {
      ...
      if (results.some((result) => result.status === "rejected"))
        throw new Error("geetorus_runner_chat_attachment_read_cleanup_failed");
    })();
    return this.#closing;
  }
}
