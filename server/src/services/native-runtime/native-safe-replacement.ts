  ...
  for (const { run, coordinator } of candidates) {
    try {
      ...
      const safeControlReads = new Set([
        ...
        "geetorus_get_agent",
        "geetorus_get_issue",
        "geetorus_list_issues",
        "geetorus_read_document",
      ]);
      const uncertainProviderActions = events.flatMap((event) => {
        ...
        if (event.eventType === "tool.execution.started") {
          ...
          if (name === "geetorus_finish" && Array.isArray(completedTaskControlCallIds) &&
      ...
      const created = await db.transaction(async (tx) => {
        ...
        for (const key of [
          ...
          "geetorusWake", "geetorusWakeComment", "geetorusTaskMarkdown", "geetorusTaskMarkdownCompact",
