...
const PREFIX = "geetorus_runner_file_handoff_";
...
const fail = code => { throw new Error('geetorus_runner_file_handoff_' + code); };
...
(async () => {
  ...
  const allowed = /^geetorus_runner_file_handoff_(path_denied|symlink_denied|file_changed|hash_mismatch)$/;
  process.stderr.write(allowed.test(error.message) ? error.message : 'geetorus_runner_file_handoff_remote_read_failed');
  process.exitCode = 1;
});
`;
...
export async function readVerifiedRemoteWorkspaceFile(input: {
  ...
  if (result.exitCode !== 0) {
    ...
    throw new Error(/^geetorus_runner_file_handoff_(path_denied|symlink_denied|file_changed|hash_mismatch)$/u.test(code)
