...
import { NativeSessionProtocolIntegrityError } from "../../vendor/geetorus-runner/index.js";
...
import { GeetorusRunnerSemanticAuthority } from "./runner-semantic-authority.js";
...
describeEmbeddedPostgres("hidden runner PRP coordinator", () => {
  ...
  it("registers only an exact Codex native binding and exposes read-only tools", async () => {
    ...
    const stateRoot = mkdtempSync(resolve(tmpdir(), "geetorus-runner-state-"));
  ...
  it("rechecks task ownership and returns semantic receipts", async () => {
    const seed = await seedNativeRun();
    const authority = new GeetorusRunnerSemanticAuthority(db, {
      companyId: seed.companyId,
      issueId: seed.issueId,
      runId: seed.runId,
      agentId: seed.agentId,
