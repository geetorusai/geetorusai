...
import { copyBackCodexAuth } from "@geetorusai/adapter-codex-local/server";
...
import type { NativeFinalizationResult } from "@geetorusai/shared";
import type {
...
import type { CommandManagedRuntimeRunner } from "@geetorusai/adapter-utils/command-managed-runtime";
import {
  resolveGeetorusRunnerTransport,
  type GeetorusRunnerTransport,
} from "@geetorusai/adapter-utils/runner-connectivity";
...
import { GeetorusControlPlanePort } from "./geetorus-control-plane-port.js";
import { appendHeartbeatRunEvent } from "../heartbeat-run-events.js";
import { nativeSha256 } from "./canonical.js";
import { GeetorusRunnerToolAuthority } from "./geetorus-runner-tool-authority.js";
...
import { resolveGeetorusInstanceRoot } from "../../home-paths.js";
...
import { resolveGeetorusRunnerBinary } from "./native-codex-runner.js";
import {
...
const RUNNERD_CONTROL_PLANE_STATE_SCHEMA =
  "geetorus.runner.durable.control-plane-state.v1";
const RUNNERD_STATE_SCHEMA = "geetorus.runner.durable.state.v1";
const CODEX_PROVIDER_STATE_SCHEMA = "geetorus.runner.codex-provider-state.v1";
const ACPX_PROVIDER_STATE_SCHEMA = "geetorus.runner.acpx-provider-state.v3";
const MANAGED_PROVIDER_STATE_SCHEMA =
  "geetorus.runner.managed-provider-state.v1";
...
export function buildNativeProviderEnvironment(
  ...
  if (assignedWorkspaceCwd?.trim()) {
    environment.GEETORUS_WORKSPACE_CWD = assignedWorkspaceCwd;
  }
  return environment;
}
...
type RuntimeQuestionFallback = {
  ...
  payload: {
    ...
    questionSet: GeetorusQuestionSet;
    questions: Array<{
      id: string;
      prompt: string;
      helpText?: string;
...
export function runtimeQuestionFallbackFromEvent(
  ...
  if (
    ...
    request.schema !== "geetorus.runtime_request.v2" ||
    ...
    return null;
  let questionSet: GeetorusQuestionSet;
  try {
    questionSet = parseGeetorusQuestionSet(request.input);
  } catch {
    return null;
  }
  ...
  try {
    const confirmation = await issueThreadInteractionService(input.db).create(
      ...
      },
      {
        ...
        summary: "Review the synchronized Geetorus plan.",
        continuationPolicy: "wake_assignee",
        payload: {
          version: 1,
...
class SessionToolAuthorityEpoch {
  ...
  #authority: GeetorusRunnerToolAuthority;
  #revoked = false;

  constructor(runId: string, authority: GeetorusRunnerToolAuthority) {
    this.runId = runId;
  ...
  async execute(call: Parameters<GeetorusRunnerToolAuthority["execute"]>[0]) {
    this.#assertCurrent();
...
function runnerdStateBase(): string {
  return (
    process.env.GEETORUS_RUNNER_STATE_DIR ??
    resolve(
      resolveGeetorusInstanceRoot(),
      "runtime",
      "geetorus-runner",
      "durable-sessions",
    )
...
export function retainedNativeCleanupJournalMatches(input: {
  ...
  if (
    ...
    semantic.schema !== "geetorus.prp.semantic_tool.v1" ||
...
export async function appendRetainedNativeCleanupEvent(
  ...
  const receipt = {
    schema: "geetorus.native_cleanup_event.v1",
...
export async function verifyStoppedNativeSessionForReplacement(
  ...
  try {
    ...
    for (const event of events.filter(event => event.eventType === "semantic_tool.input")) {
      const semantic = record(record(event.payload).semantic_tool);
      const correlation = record(semantic.correlation);
      if (!bound(event) || semantic.operationId !== "geetorus_finish" || semantic.phase !== "input" ||
    ...
    const evidence = { schema: "geetorus.stopped_text_turn.v1", runId: run.id, nativeSessionId: run.nativeSessionId,
...
export async function reconcileRetainedNativeSessionCleanup(
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        schema: "geetorus.native_cleanup_activation.v1",
        companyId: owned.run.companyId,
        issueId: owned.run.nativeIssueId,
        runId: owned.run.id,
        requestId: leaseOwner,
...
async function assertCleanupActivationCommitted(
  ...
  if (
    marker.schema !== "geetorus.native_cleanup_activation.v1" ||
  ...
  for (const [directory, filename, maximum] of [
    ...
    try {
      ...
      if (
        ...
        state.schema === "geetorus.runner.durable.state.warm-transition.v1"
      )
        return true;
    } catch {
...
async function verifyWarmTransitionRestart(input: {
  ...
  const artifact = readRunnerdArtifactBinding(resolveGeetorusRunnerBinary());
...
async function recoverQuiescentRunnerdState(input: {
  ...
  for (const root of candidates) {
    ...
    try {
      ...
      if (
        ...
        record(record(prior.contextSnapshot).geetorusEnvironment).driver !==
          "local" ||
  ...
  await input.onLog?.(
    "stdout",
    `[geetorus-runner] Automatically recovered settled session from run ${candidate.runId}; preserving the existing provider thread.\n`,
  );
}
...
export interface NativeHarnessBackupManifest {
  schema: "geetorus.native-harness-backup.v1";
  normalizedSessionId: string;
  runnerInstanceId: string;
  providerKind: NativeProviderKind;
  driverKind: NativeDriverKind;
...
function compatibleNativeHarnessBackupManifests(input: {
  ...
  for (const candidateRoot of harnessBackupCandidates(input.root)) {
    ...
    if (
      manifest.schema !== "geetorus.native-harness-backup.v1" ||
...
function persistWarmNativeCheckpoint(
  ...
  writeFileSync(
    temporary,
    JSON.stringify({
      schema: "geetorus.native-session-supervisor.v1",
      configDigest,
...
function loadWarmNativeCheckpoint(
  ...
  if (
    envelope.schema !== "geetorus.native-session-supervisor.v1" ||
    !envelope.snapshot
  ) {
...
> {
    ...
    const intent = await options.db.transaction(async (tx) => {
      ...
      if (existingIntentId) {
        const matchingIntent =
          existing.schema === "geetorus.native-cancellation.v1" &&
  ...
  if (decision && decisionContext) {
    ...
    const acknowledgement = await options.db.transaction(async (tx) => {
      ...
      const matchingIntent =
        intent.schema === "geetorus.native-cancellation.v1" &&
...
export async function executeGeetorusNativeSession(input: {
  db: Db;
  execution: NativeExecutionInput;
  runnerInstanceId: string;
  ...
  useRunnerd?: boolean;
  /** Geetorus adapter identity used to scope the durable goal projection. */
  adapterType?: string;
  ...
  if (!input.useRunnerd) {
    return executeGeetorusNativeSessionWithinScope(input);
  }
  ...
  try {
    ...
    cleanupStagedAttachments = async () => {
      ...
      if (cleanupResults.some((result) => result.status === "rejected")) {
        throw new Error("geetorus_runner_attachment_staging_cleanup_failed");
      }
    };
    ...
    return await executeGeetorusNativeSessionWithinScope(preparedInput);
  } catch (error) {
...
async function executeGeetorusNativeSessionWithinScope(
  input: Parameters<typeof executeGeetorusNativeSession>[0],
): Promise<AdapterExecutionResult> {
  if (
    input.execution.provider.kind !== "codex" &&
    ...
    throw new Error("geetorus_runner_provider_unsupported");
  }
  if (
    input.execution.provider.kind === "acpx" &&
    ...
    throw new Error(
      "geetorus_runner_provider_unsupported: ACPX Pi is unavailable until descriptor-confined verified launch is implemented",
    );
  }
          ...
          if (
            ...
            const intentMatchesBinding =
              cancellationIntent.schema ===
                "geetorus.native-cancellation.v1" &&
  ...
  const recordPlanSynchronization = async (event: {
    ...
    if (input.onLog)
      await input.onLog(
        "stdout",
        `${JSON.stringify({ type: "geetorus.plan.synchronization", synchronization })}\n`,
      );
  };
  ...
  const projectSessionGoalEvent = (event: PrpEvent) =>
    applyRunnerGoalPrpEvent(
      input.db,
      {
        ...
        adapterType:
          input.adapterType ??
          (input.useRunnerd ? "geetorus_runner" : "codex_local"),
      },
      {
  ...
  const controlPlane = new GeetorusControlPlanePort(
    input.db,
    {
      companyId: input.execution.binding.companyId,
      issueId: input.execution.binding.issueId,
    ...
    },
    {
      onCommittedEvent: async (event) => {
        ...
        if (input.onLog)
          await input.onLog(
            "stdout",
            `${JSON.stringify({ type: "geetorus.prp.event", event })}\n`,
          );
        if (
          input.onEvent &&
          [
        ...
        if (inputMetric && input.onLog) {
          await input.onLog(
            "stdout",
            `${JSON.stringify({
              type: "geetorus.runtime_input.metric",
              ...inputMetric,
            })}\n`,
          );
        }
  ...
  if (warmSessionId !== null && warmConfigDigest !== null) {
    ...
    if (entry) {
      ...
      const hasBrokerCapability = Boolean(
        input.runnerEnvironment?.GEETORUS_GITHUB_BROKER_TOKEN,
      );
      ...
      if (
        ...
        entry.githubAuthenticationMode !==
          input.runnerEnvironment?.GEETORUS_GITHUB_AUTH_MODE ||
        entry.networkAccess !==
          (input.runnerEnvironment?.GEETORUS_RUNNER_NETWORK_ACCESS ===
            "enabled")
      ) {
  ...
  try {
        ...
        const result = await trace.run(runnerSessionStartupScope, () =>
          executeNativeSession({
            ...
            onPostCompletionEnrichmentFailure: async ({ stage, error }) => {
              ...
              await input.onLog?.(
                "stderr",
                `[geetorus-runner] post-completion ${stage} enrichment failed: ${detail}\n`,
              );
            },
            onSessionQuarantined: async (reason) => {
              await input.onLog?.(
                "stderr",
                `[geetorus-runner] warm native session quarantined: ${redactSensitiveText(reason).slice(-1_000)}\n`,
              );
            },
                  ...
                  warmNativeSessions.set(warmSessionId, {
                    ...
                    networkAccess:
                      input.runnerEnvironment
                        ?.GEETORUS_RUNNER_NETWORK_ACCESS === "enabled",
                    credentialRunId: input.runnerEnvironment
                      ?.GEETORUS_GITHUB_BROKER_TOKEN
                      ? input.execution.binding.runId
                      : undefined,
                    session,
    ...
    try {
      ...
      clearSteeringDeliveries(input.execution.binding.runId);
      clearNativeRuntimeRequestResolutions(input.execution.binding.runId);
      // Stop before geetorus_finish is normal. Bounded provider teardown has
      ...
      await boundedExecutionCleanup(async () => {
        ...
        await attemptFailureStep(() =>
          input.onLog?.(
            "stderr",
            `[geetorus-runner] native session execution failed: ${executionFailureMessage}\n`,
          ),
        );
        if (runnerSessionStartupScope) {
          await attemptFailureStep(() =>
  ...
  if (
    ...
    if (markdown) {
      ...
      await recordPlanSynchronization({
        ...
        payload: {
          schema: "geetorus.plan.updated.v1",
  ...
  const finalization: NativeFinalizationResult = {
    schema: "geetorus.native-finalization.v1",
    runtimeMode: "native",
    runId: input.execution.binding.runId,
    issueId: input.execution.binding.issueId,
    companyId: input.execution.binding.companyId,
...
export function buildRemoteCodexLauncherCommand(
  ...
  return (
    ...
    `[ ! -d ${quote(targetPath)} ] && ` +
    `geetorus_codex_launcher_tmp=$(mktemp ${quote(targetPath + ".tmp.XXXXXX")}) && ` +
    `trap 'rm -f "$geetorus_codex_launcher_tmp"' 0 && ` +
    `printf '%s' ${quote(launcher)} > "$geetorus_codex_launcher_tmp" && ` +
    `chmod 700 "$geetorus_codex_launcher_tmp" && ` +
    `mv -f "$geetorus_codex_launcher_tmp" ${quote(targetPath)}`
  );
}
...
const RUNNERD_BUILD_METADATA_SCHEMA =
  "geetorus-runner/runnerd-build-metadata/v1";
const RUNNERD_BINARY_CONTRACT_VERSION = 2;

const REMOTE_PROVIDER_PACK_SCHEMA = "geetorus-runner/remote-provider-pack/v1";
...
export function assertRemoteRunnerBuildMetadata(
  ...
  if (
    ...
    metadata.binaryName !== "geetorus-runnerd" ||
    metadata.packageName !== "@geetorusai/geetorus-runner" ||
  ...
  if (input.runner.syncIn) {
    ...
    try {
      if (excludeArgs.length > 0) {
        stagingRoot = mkdtempSync(join(tmpdir(), "geetorus-runner-restore-"));
...
export async function syncRemoteRunnerDirectoryOut(input: {
  ...
  if (input.runner.syncOut) {
    ...
    if (excluded.length > 0) {
      ...
      snapshotPath = posix.join(
        ...
        `.geetorus-checkpoint-${checkpointId}`,
      );
  ...
  const stagingRoot = mkdtempSync(join(parent, ".geetorus-checkpoint-"));
  const stagedTarget = join(stagingRoot, "payload");
  const previousTarget = join(
    parent,
    `.geetorus-checkpoint-previous-${randomUUID()}`,
  );
    ...
    const result = await input.runner
      .execute({
        ...
        args: [
          ...
          "geetorus-runner-process-identity",
          input.identityPath,
        ],
        bypassSession: true,
        timeoutMs: 2_000,
  ...
  const result = await input.runner
    .execute({
      ...
      args: [
        ...
        "geetorus-runner-identity-failure-cleanup",
        input.identityPath,
        input.nonce,
        input.runnerInstanceId,
      ],
...
export function createRemoteRunnerProcessLauncher(input: {
  ...
  return (spec) => {
    ...
    const completion = (async () => {
      ...
      // that RPC even when Geetorus requests a longer timeout. Detach runnerd
      ...
      const launchResult = await runner.execute({
        ...
        args: [
          "-c",
          'set -eu; identity_path=$1; identity_nonce=$2; runner_instance_id=$3; child_script=$4; shift 4; umask 077; identity_dir=$(dirname -- "$identity_path"); mkdir -p -- "$identity_dir"; if command -v setsid >/dev/null 2>&1; then nohup setsid sh -c "$child_script" geetorus-runner-child "$identity_path" "$identity_nonce" "$runner_instance_id" "$@" </dev/null >/dev/null 2>&1 & else nohup sh -c "$child_script" geetorus-runner-child "$identity_path" "$identity_nonce" "$runner_instance_id" "$@" </dev/null >/dev/null 2>&1 & fi',
          "geetorus-runner-launch",
          input.processIdentityPath,
          identityNonce,
          input.runnerInstanceId,
...
export async function createRunnerdBackend(input: {
  ...
  try {
    let retainedTransition: VerifiedWarmTransitionBinding | undefined;
    // executeGeetorusNativeSession holds the full session-scope claim and
...
async function createRunnerdBackendWithinSessionClaim(
  ...
  const authority = new GeetorusRunnerToolAuthority(input.db, {
  ...
  if (requiresRemoteProviderPack) {
    if (
      ...
      throw new Error(
        "runner_remote_provider_artifact_incompatible: configure GEETORUS_RUNNER_REMOTE_PROVIDER_PACK_PATH with the build-owned provider pack",
      );
    }
  ...
  const remoteBinary = remoteRuntimeRoot
    ? posix.join(remoteRuntimeRoot, "bin", "geetorus-runnerd")
    : null;
  ...
  const controllerRunnerBinary = remoteTarget
    ? input.runnerRemoteBinaryPath?.trim() || resolveGeetorusRunnerBinary()
    : resolveGeetorusRunnerBinary();
  ...
  const discoverPreinstalledProviderPack = async () => {
    ...
    const result = await remoteCommandRunner.execute({
      ...
      args: [
        "-c",
        'for candidate in /opt/geetorus-runner/provider-pack "$HOME/.local/share/geetorus-runner/provider-pack"; do if [ -f "$candidate/provider-pack.json" ]; then printf \'%s\\n\' "$candidate"; break; fi; done',
      ],
      cwd: remoteTarget.remoteCwd,
      bypassSession: true,
      timeoutMs: 10_000,
  ...
  const discoverPreinstalledExecutable = async (
    ...
    const result = await remoteCommandRunner.execute({
      ...
      args: [
        "-c",
        `for candidate in /opt/geetorus-runner/bin/${name} "$HOME/.local/bin/${name}"; do ` +
  ...
  const prepareRemoteRunner = async (
    ...
    if (mayUsePreinstalledRunnerArtifact(explicitRemoteBinary)) {
      const preinstalledRunner = await measureNativeRunnerSpan(
        ...
        () => discoverPreinstalledExecutable("geetorus-runnerd"),
      );
      if (preinstalledRunner) {
        try {
          ...
          await input.onLog?.(
            "stderr",
            "[geetorus-runner] using preinstalled runnerd from the sandbox image\n",
          );
        } catch {
    ...
    if (!usedPreinstalledRunner) {
      ...
      if (!explicitRemoteBinary) {
        ...
        if (
          ...
          throw new Error(
            "runner_remote_artifact_platform_mismatch: configure GEETORUS_RUNNER_REMOTE_BINARY_PATH for the remote OS and architecture",
          );
        }
      }
    ...
    if (remoteCodexBinary && remoteCodexNpmSpec) {
      ...
      if (preinstalledCodex) {
        try {
          ...
          await input.onLog?.(
            "stderr",
            "[geetorus-runner] using preinstalled Codex from the sandbox image\n",
          );
        } catch {
    ...
    if (remoteCodexBinary && !explicitRemoteCodex && !remoteCodexNpmSpec) {
      ...
      if (!preinstalledCodex) {
        throw new Error(
          "runner_remote_codex_artifact_unavailable: install codex in the sandbox image or configure GEETORUS_RUNNER_REMOTE_CODEX_NPM_SPEC",
        );
      }
      await measureNativeRunnerSpan(
        input.trace,
    ...
    if (
        ...
        usePreinstalled: async () => {
          ...
          if (preinstalledProviderPack) {
            try {
              ...
              await input.onLog?.(
                "stderr",
                "[geetorus-runner] using manifest-matched provider pack from the sandbox image\n",
              );
            } catch {
      ...
      if (packSource === "staged") {
        await input.onLog?.(
          "stderr",
          "[geetorus-runner] reusing manifest-matched provider pack from the workspace\n",
        );
      }
    }
    remotePrepared = true;
  ...
  const claimUntouchedSessionInResumedLease = async (): Promise<boolean> => {
    ...
    const probe = await remoteCommandRunner.execute({
      ...
      args: ["-c",
        ...
        "geetorus-runner-claim-unstarted-session", remoteRuntimeRoot,
  ...
  const checkpointRemoteRunner = async (
    ...
    if (!checkpointable.complete) {
      ...
      await input.onLog?.(
        "stderr",
        `[geetorus-runner] remote checkpoint ${incompleteFailure ? "failed" : "skipped"}: exact suspended harness state unavailable (process=${settlement} reason=${checkpointable.incompleteReason})\n`,
      );
    ...
    try {
      ...
      await input.onLog?.(
        "stderr",
        `[geetorus-runner] remote checkpoint failed: ${detail || "unknown failure"}\n`,
      );
      throw error;
    }
  };
      ...
      ? async (archive: { archiveKey: string }) => {
          ...
          const result = await remoteCommandRunner.execute({
            ...
            args: [
              ...
              "geetorus-runner-authority-archive",
              sourcePath,
              archiveDirectory,
              archivedStatePath,
            ],
  ...
  delete effectiveRunnerEnvironmentBase.GEETORUS_RUNNER_EXTERNAL_SANDBOX;
  const effectiveRunnerEnvironment: NodeJS.ProcessEnv = remoteRuntimeRoot
    ? {
        ...
        GEETORUS_WORKSPACE_CWD: remoteTarget!.remoteCwd,
        ...(remoteTarget!.transport === "sandbox"
    ...
    : {
        ...effectiveRunnerEnvironmentBase,
        GEETORUS_WORKSPACE_CWD: input.execution.workspace.cwd,
      };
    ...
    codexTransportFactory: (recoveryContext) =>
      createRunnerdCodexTransport({
        ...
        ...(input.execution.provider.kind === "acpx"
          ? {
              ...
              acpxPermissionModePinned:
                ...
                "geetorus.native-execution-input.v4",
        ...
        onDiagnostic: (message) => {
          void input.onLog?.(
            "stderr",
            `[geetorus-runner] runnerd diagnostic: ${redactSensitiveText(message).slice(-4_096)}\n`,
          );
        },
          ...
          return measureNativeRunnerSpan(
            ...
            async () => {
              if (target.kind === "local") {
                ...
                await input.onLog?.(
                  "stderr",
                  "[geetorus-runner] transport mode=local_loopback state=connecting\n",
                );
              ...
              let transport: GeetorusRunnerTransport;
              ...
              await input.onLog?.(
                "stderr",
                `[geetorus-runner] transport mode=${transport.mode} state=connecting\n`,
              );
                    ...
                    async () => {
                      ...
                      outbound = connectRunnerPrpIngress({
                        ...
                        onStateChange: (state, failureCode) => {
                          void input.onLog?.(
                            "stderr",
                            `[geetorus-runner] transport mode=provider_ingress state=${state}${failureCode ? ` failure=${failureCode}` : ""}\n`,
                          );
                        },
                      });
                    },
