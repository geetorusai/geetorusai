...
import { heartbeatRuns, type Db } from "@geetorusai/db";
import type { GeetorusSkillEntry } from "@geetorusai/adapter-utils/server-utils";
import { isToolConnectionAttentionHealth } from "@geetorusai/shared";
import {
...
export async function materializeAsset(files: AssetFile[]): Promise<NativeRuntimeAssetReference> {
  ...
  const manifestText = `${JSON.stringify({ schema: "geetorus.runtime-asset-manifest.v1", digest: assetDigest, fileCount: manifestFiles.length, totalBytes, files: manifestFiles })}\n`;
  const manifestDigest = sha256(manifestText);
  const assetsRoot = path.join(resolveGeetorusInstanceRoot(), "runtime-context-assets");
...
async function materializeSelectedSkills(runtimeConfig: Record<string, unknown>, entries: GeetorusSkillEntry[], omitLegacy: boolean) {
  const desiredKeys = resolveGeetorusDesiredSkillNames(runtimeConfig, entries).filter(
    (key) => !omitLegacy || key !== GEETORUS_OPERATIONAL_SKILL_KEY,
  );
...
export async function buildNativeRuntimeContext(input: { db: Db; agent: RuntimeAgent; runId: string; runtimeConfig: Record<string, unknown>; runtimeSkillEntries: GeetorusSkillEntry[] }): Promise<NativeRuntimeContextSnapshot> {
  const [instructions, skills, mcp] = await Promise.all([
    materializeInstructionBundle(input.agent),
    materializeSelectedSkills(input.runtimeConfig, input.runtimeSkillEntries, input.agent.adapterType === "geetorus_runner"),
    resolveNativeRuntimeMcpSnapshot({ db: input.db, agent: input.agent, runId: input.runId }),
  ]);
  const snapshot = { prompt: { revision: GEETORUS_EXECUTION_PROMPT_REVISION, text: GEETORUS_EXECUTION_PROMPT, digest: nativeRuntimePromptDigest() }, instructions, skills, mcp };
