...
beforeEach(async () => {
  ...
  const root = await mkdtemp(path.join(tmpdir(), "geetorus-native-context-"));
  temporaryRoots.push(root);
  process.env.GEETORUS_HOME = root;
  process.env.GEETORUS_INSTANCE_ID = "runtime_context_test";
...
describe("buildNativeRuntimeContext", () => {
  ...
  it("materializes every instruction and selected-skill file as immutable, content-addressed context", async () => {
    ...
    const sourceRoot = await mkdtemp(path.join(tmpdir(), "geetorus-native-skill-"));
    ...
    const entries: GeetorusSkillEntry[] = [
    ...
    const input = {
      ...
      runId: "run-1",
      runtimeConfig: { geetorusSkillSync: { desiredSkills: ["company-1/reviewer"] } },
  ...
  it("fails closed for a missing assigned skill and omits a stale legacy operational skill", async () => {
    ...
    await expect(buildNativeRuntimeContext({
      ...base,
      runtimeConfig: { geetorusSkillSync: { desiredSkills: ["company-1/missing"] } },
      runtimeSkillEntries: [{
    ...
    const supportedRoot = await mkdtemp(path.join(tmpdir(), "geetorus-native-supported-skill-"));
    ...
    const context = await buildNativeRuntimeContext({
      ...
      runtimeConfig: {
        geetorusSkillSync: {
          desiredSkills: [GEETORUS_OPERATIONAL_SKILL_KEY, "company-1/supported"],
        },
      },
      runtimeSkillEntries: [
        {
