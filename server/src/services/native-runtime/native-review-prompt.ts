...
export function buildNativeReviewRequest(
  ...
  return [
    ...
    "After recording the review decision, report your review complete with geetorus_finish. Do not redo the worker's assignment, change dependencies, or wait for the parent task to resume.",
    ...
    `<geetorus-review-evidence>${serializedEvidence}</geetorus-review-evidence>`,
  ].join("\n\n");
}
