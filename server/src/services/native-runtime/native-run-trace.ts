...
import { runWithRuntimeParent } from "@geetorusai/adapter-utils/acpx-engine/startup-timing";
...
/**
 * Run-scoped tracing for the native Geetorus runner path.
 *
...
export function createNativeRunTrace(input: {
  ...
  const tracing =
    input.traceContext ?? getStartupTraceContext("geetorus.native-runner");
  ...
  try {
    rootSpan = tracing.tracer.startSpan("task.run", {
      ...
      attributes: {
        "geetorus.task.run.run_id": runIdHash,
        "geetorus.task.run.runtime": "geetorus_runner",
        "geetorus.task.run.trace_schema_version":
  ...
  const setOtelAttributes = (
    ...
    for (const [key, value] of Object.entries(attributes)) {
      try {
        span.setAttribute(`geetorus.native.span.${key}`, value);
      } catch {
    ...
    try {
      span = tracing.tracer.startSpan(
        name,
        {
          ...
          attributes: {
            "geetorus.native.span.trace_schema_version":
              NATIVE_RUN_TRACE_SCHEMA_VERSION,
            ...Object.fromEntries(
              Object.entries(attributes).map(([key, value]) => [
                `geetorus.native.span.${key}`,
                value,
              ]),
            ),
          },
  ...
  const finish = async (outcome: NativeRunSpanOutcome): Promise<void> => {
    ...
    try {
      ...
      rootSpan.setAttribute("geetorus.task.run.outcome", outcome);
    ...
    await emitCompletedSpanEvent({
      ...
      endedAtMs,
      outcome,
      attributes: { runtime: "geetorus_runner" },
    });
  };
