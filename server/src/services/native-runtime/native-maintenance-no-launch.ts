...
import { validatePrpEvent } from "../../vendor/geetorus-runner/index.js";
...
export function verifyRetainedMaintenanceNoLaunch(
  ...
  try {
    ...
    for (const snapshot of [original, attempted]) {
      ...
      requireProof(
        snapshot.runner.schema === "geetorus.runner.durable.state.v1",
      );
      requireProof(
        snapshot.provider.schema === "geetorus.runner.codex-provider-state.v1",
      );
