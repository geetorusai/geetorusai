...
import { attestReviewedExternalChatRun, buildGeetorusWakePayload } from "../heartbeat.js";
...
import type { AskUserQuestionsInteraction } from "@geetorusai/shared";
...
describe("native external-chat response wait", () => {
  const externalTestDatabaseUrl = process.env.GEETORUS_TEST_DATABASE_URL;
  ...
  async function seedWaitTurn(
    ...
    await db.insert(chatEndpointResources).values({
      ...
      providerResourceId:
        provider === "github" ? "geetorus/test-repository" : "telegram-user",
      label:
        provider === "github"
          ? "Geetorus test repository"
  ...
  async function seedGitHubBoardAnsweredTurn(withLink = true) {
    ...
    const previousPublicUrl = process.env.GEETORUS_PUBLIC_URL;
    process.env.GEETORUS_PUBLIC_URL = withLink
      ? "https://board.geetorus.example"
      : "http://127.0.0.1:3103";
    ...
    try {
      ...
      if (previousPublicUrl === undefined)
        delete process.env.GEETORUS_PUBLIC_URL;
      else process.env.GEETORUS_PUBLIC_URL = previousPublicUrl;
    }
  ...
  it.each([true, false])(
    ...
    async (withLink) => {
      ...
      expect(fixture.context.geetorusWake).toMatchObject({
  ...
  it.each([
    ...
    async (change) => {
      ...
      expect(
        fixture.context.geetorusExternalChatQuestionResponse,
      ).toBeUndefined();
      await expect(
        db
          .select({ status: issueThreadInteractions.status })
  ...
  async function attestAnswer(
    ...
    expect(fixture.context.geetorusExternalChatQuestionResponse).toMatchObject(
      {
    ...
    fixture.context.geetorusExternalChatExecutionBound = true;
    fixture.context.geetorusWake = await buildGeetorusWakePayload({
      db,
      companyId: fixture.companyId,
      agentId: fixture.agentId,
      runId: fixture.runId,
  ...
  async function withAnsweredFileTool(
    ...
    try {
      ...
      const runner = new GeetorusRunnerToolAuthority(db, {
        ...fixture,
        workspaceRoot,
        storage,
      });
  ...
  it.each(["result", "field", "expiry"] as const)(
    ...
    async (mutation) => {
      ...
      expect(fixture.context).not.toHaveProperty(
        "geetorusExternalChatQuestionResponse",
      );
    },
  );
    ...
    async (provider, tool) => {
      await withAnsweredFileTool(
        ...
        async ({ fixture, invoke }) => {
          ...
          expect(fixture.context.geetorusWake).not.toHaveProperty(
            "questionResponse",
          );
          ...
          await expect(invoke()).rejects.toThrow(
            "geetorus_runner_chat_attachment_destination_denied",
          );
          expect(
            await db
              .select()
    ...
    async (tool) => {
      await withAnsweredFileTool(
        ...
        async ({ fixture, invoke }) => {
          ...
          (
            context.geetorusExternalChatQuestionResponse as Record<
              string,
              unknown
            >
          ...
          await db
            ...
            .where(eq(heartbeatRuns.id, fixture.runId));
          await expect(invoke()).rejects.toThrow(
            "geetorus_runner_chat_attachment_binding_denied",
          );
          await expect(
            db
              .select()
        ...
        async ({ fixture, invoke }) => {
          ...
          await expect(invoke()).rejects.toThrow(
            mutation === "membership_revoked"
              ? "geetorus_runner_chat_attachment_principal_denied"
              : "geetorus_runner_chat_attachment_binding_denied",
          );
          expect(
            await db
  ...
  it.each([
    ...
    async (kind) => {
      ...
      if (kind === "missing_parent_marker") {
        ...
        delete context.geetorusExternalChatQuestionResponse;
        await db
          .update(heartbeatRuns)
          .set({ contextSnapshot: context })
          .where(eq(heartbeatRuns.id, parent.runId));
  ...
  it("keeps presentation authority when heartbeat appends only adapter runtime-service display metadata", async () => {
    ...
    const context = {
      ...run!.contextSnapshot,
      geetorusRuntimeServices: [
        { name: "preview", url: "http://127.0.0.1:9000" },
      ],
      geetorusRuntimePrimaryUrl: "http://127.0.0.1:9000",
    };
    await db
      .update(heartbeatRuns)
      .set({ contextSnapshot: context })
