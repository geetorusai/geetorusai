...
describeEmbeddedPostgres("native runner restart recovery with real processes", () => {
  ...
  let apiUrl: string;
  let originalGeetorusHome: string | undefined;
  ...
  beforeAll(async () => {
    ...
    geetorusHome = await mkdtemp(resolve(tmpdir(), "native-restart-home-"));
    originalGeetorusHome = process.env.GEETORUS_HOME;
  ...
  afterAll(async () => {
    ...
    await Promise.all([
      ...
      rm(geetorusHome, { recursive: true, force: true }),
    ]);
