...
function requiredText(value: string, field: string, maxLength: number): string {
  ...
  if (
    ...
    throw new Error(`geetorus_runner_file_handoff_invalid_${field}`);
  }
  return normalized;
}
...
async function assertNoSymlinkComponents(
  ...
  for (const segment of relativePath.split(path.sep)) {
    ...
    if ((await lstat(cursor)).isSymbolicLink()) {
      throw new Error("geetorus_runner_file_handoff_symlink_denied");
    }
  }
}
...
async function openedFilePath(fd: number): Promise<string> {
  if (process.platform === "darwin") {
    ...
    if (paths.length !== 1 || !path.isAbsolute(paths[0]!)) {
      throw new Error("geetorus_runner_file_handoff_descriptor_unverifiable");
    }
...
async function readVerifiedWorkspaceFile(
  ...
  if (path.basename(filename) !== filename || filename.includes("\\")) {
    throw new Error("geetorus_runner_file_handoff_invalid_filename");
  }
  ...
  if (
    ...
    throw new Error("geetorus_runner_file_handoff_size_denied");
  }
  const expectedSha256 = input.sha256.trim().toLowerCase();
  if (!/^[a-f0-9]{64}$/u.test(expectedSha256)) {
    throw new Error("geetorus_runner_file_handoff_invalid_sha256");
  }
  ...
  if (!isAllowedContentType(contentType)) {
    throw new Error("geetorus_runner_file_handoff_content_type_denied");
  }

  if (binding.executionTargetKind === "remote") {
    if (!binding.readRemoteWorkspaceFile) throw new Error("geetorus_runner_file_handoff_remote_unsupported");
    ...
    if (path.posix.isAbsolute(contentRef) || /^[a-z][a-z0-9+.-]*:/iu.test(contentRef)
      ...
      throw new Error("geetorus_runner_file_handoff_path_denied");
    }
    const body = await binding.readRemoteWorkspaceFile({ contentRef, byteSize: input.byteSize, sha256: expectedSha256 });
    if (body.length !== input.byteSize) throw new Error("geetorus_runner_file_handoff_size_denied");
    if (createHash("sha256").update(body).digest("hex") !== expectedSha256) throw new Error("geetorus_runner_file_handoff_hash_mismatch");
  ...
  try {
    ...
    if (
      ...
      throw new Error("geetorus_runner_file_handoff_file_changed");
    }
    ...
    if (actualSha256 !== expectedSha256) {
      throw new Error("geetorus_runner_file_handoff_hash_mismatch");
    }
...
function wakeAttachmentSelections(value: unknown): Array<{
  ...
  const wake = record(record(value).geetorusWake);
...
async function ensurePrivateStagingDirectory(
  ...
  for (const segment of [".geetorus-inbound", processDirectoryName]) {
    cursor = path.join(cursor, segment);
    try {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
      throw new Error("geetorus_runner_attachment_staging_path_denied");
    }
...
async function scrubNativeRunnerStagingResidue(
  ...
  const stagingRootPath = path.join(workspaceRoot, ".geetorus-inbound");
  ...
  for await (const runEntry of rootDirectory) {
    if (!runEntry.isDirectory()) {
      throw new Error("geetorus_runner_attachment_staging_residue_denied");
    }
...
async function readBoundedStorageObject(input: {
  ...
  if (
    ...
    throw new Error("geetorus_runner_attachment_staging_size_denied");
  }
  ...
  for await (const chunk of object.stream) {
    ...
    if (byteSize > MAX_ATTACHMENT_BYTES || byteSize > input.expectedByteSize) {
      ...
      throw new Error("geetorus_runner_attachment_staging_size_mismatch");
    }
    chunks.push(buffer);
  }
  ...
  if (
    ...
    throw new Error("geetorus_runner_attachment_staging_integrity_mismatch");
  }
  return body;
}
...
export async function stageNativeRunnerWakeAttachments(input: {
  ...
  if (
    ...
    throw new Error("geetorus_runner_attachment_staging_not_authorized");
  }
  const selections = wakeAttachmentSelections(run.contextSnapshot);
  if (selections.length > MAX_NATIVE_STAGED_ATTACHMENTS) {
    throw new Error("geetorus_runner_attachment_staging_count_denied");
  }
  const workspaceRoot =
      ...
      cleanup: async () => {
        try {
          ...
          if (results.some((result) => result.status === "rejected")) {
            throw new Error(
              "geetorus_runner_attachment_staging_cleanup_failed",
            );
          }
        } finally {
          releaseActiveStage();
...
export function renderNativeRunnerStagedAttachmentPrompt(
  ...
  const lines = [
    "Geetorus native attachment access:",
...
async function assertCurrentBinding(
  ...
  if (
    ...
    throw new Error("geetorus_runner_file_handoff_not_authorized");
  }
...
export async function prepareNativeRunnerFileHandoff(input: {
  ...
  const [existing] = await input.db
    ...
    .leftJoin(
      ...
      and(
        ...
        eq(issueWorkProducts.provider, "geetorus"),
  ...
  if (existing?.commentId) {
    if (!existing.workProductId) {
      throw new Error("geetorus_runner_file_handoff_work_product_missing");
    }
    const [comment] = await input.db
      ...
      .limit(1);
    if (!comment) {
      throw new Error("geetorus_runner_file_handoff_existing_binding_invalid");
    }
    return {
      result: {
  ...
  try {
    if (
      ...
      throw new Error("geetorus_runner_file_handoff_storage_mismatch");
    }
    ...
    if (
      ...
      throw new Error("geetorus_runner_file_handoff_origin_not_persisted");
    }
    await input.db
