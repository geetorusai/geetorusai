...
import { activityLog, companySkills, companyMemberships, heartbeatRuns, issues } from "@geetorusai/db";
...
describe("runner create_skill through the real skill API", () => {
  ...
  beforeAll(async () => {
    ...
    home = await mkdtemp(join(tmpdir(), "geetorus-create-skill-"));
  ...
  it("recovers a concurrent or replacement-run retry without creating another skill", async () => {
    ...
    const replacement = new GeetorusRunnerToolAuthority(server.db, { ...testFixture, runId: replacementRunId });
  ...
  it("does not create skills in planning mode", async () => {
    ...
    expect(new GeetorusRunnerToolAuthority(server.db, { ...testFixture, workMode: "planning" }).definitions().some(tool => tool.name === "create_skill")).toBe(false);
