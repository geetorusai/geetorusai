...
async function resolveQuestionResponseChain(
  ...
  const sourceWake = record(sourceContext.geetorusWake);
  ...
  if (
    ...
    !(
      (sourceContext.geetorusHarnessCheckedOut === true &&
        sourceWake.checkedOutByHarness === true) ||
      (sourceContext.geetorusExternalChatExecutionBound === true &&
  ...
  if (lockMode !== "read") {
    ...
    if (lockMode === "blocking") {
      ...
      if (!lock?.acquired)
        throw Object.assign(
          new Error("geetorus_external_chat_wait_authorization_contended"),
          { code: "55P03" },
        );
    }
  }
  ...
  if (
    ...
    identity.geetorusUserId !== interaction.resolvedByUserId
  )
    return null;
  const marker: Marker = {
    schema: "geetorus.external_chat_question_response.v1",
    ...
    bindingSha256: nativeSha256({
      ...
      ...(action
        ...
        : {
            boardResponse: {
              schema: "geetorus.github_board_question_response.v1",
  ...
  return {
    ...
    authorizationContext: {
      ...
      geetorusHarnessCheckedOut: false,
