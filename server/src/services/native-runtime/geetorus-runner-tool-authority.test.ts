...
describe("GeetorusRunnerToolAuthority", () => {
  ...
  it("advertises only real bindings and reads the bound task", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_tool_not_advertised");
    await expect(
      authority.execute({
    ...
    ).rejects.toThrow("geetorus_runner_chat_attachment_binding_denied");
    await expect(
      authority.execute({
  ...
  it("preserves direct-chat file tools across the guarded API rollout", () => {
    const previousEnabled = process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED;
    const previousCompanies =
      process.env.GEETORUS_RUNNER_API_TOOLS_COMPANY_IDS;
    const createAuthority = () =>
      new GeetorusRunnerToolAuthority(db, {
        companyId,
        agentId,
        issueId,
        runId,
        workspaceRoot: "/tmp/geetorus-runner-tools",
    ...
    try {
      delete process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED;
      delete process.env.GEETORUS_RUNNER_API_TOOLS_COMPANY_IDS;
      ...
      process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED = "true";
      process.env.GEETORUS_RUNNER_API_TOOLS_COMPANY_IDS = companyId;
      ...
      if (previousEnabled === undefined) {
        ...
        process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED = previousEnabled;
      }
      if (previousCompanies === undefined) {
        delete process.env.GEETORUS_RUNNER_API_TOOLS_COMPANY_IDS;
      } else {
        process.env.GEETORUS_RUNNER_API_TOOLS_COMPANY_IDS = previousCompanies;
      }
    }
  });
  ...
  it.each(["standard", "ask"] as const)(
    ...
    (workMode) => {
      ...
      expect(advertised.description).toContain(
        "current Geetorus task bound to this run",
      );
      expect(advertised.description).toContain(
        "payload.questions for choices",
      );
      expect(advertised.description).toContain(
        "Geetorus renders it and authenticates the response",
      );
      expect(advertised.description).toContain(
  ...
  it.each(["choice", "text"] as const)("executes the advertised %s question once on the bound reviewed task", async (answerMode) => {
    ...
    const authority = new GeetorusRunnerToolAuthority(db, binding);
  ...
  it("writes progress through the real issue service and replays idempotently", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_tool_idempotency_conflict");
  });
  ...
  it("creates ordinary children, preserves blockers, and deduplicates across runs", async () => {
    ...
    const retryAuthority = new GeetorusRunnerToolAuthority(db, {
      companyId,
      agentId,
      issueId,
      runId: nextRunId,
    ...
    ).rejects.toThrow("geetorus_runner_agent_not_found");
    await expect(
      retryAuthority.execute({
        tool: "create_task",
  ...
  it("rejects mutations after reassignment, run replacement, or terminalization", async () => {
    ...
    await expect(authority.execute(mutation)).rejects.toThrow(
      "geetorus_runner_tool_binding_not_authorized",
    );
  ...
  it("captures delegation and approval origins before steering and preserves replay identity", async () => {
    ...
    const authority = new GeetorusRunnerToolAuthority(db, { companyId, agentId, issueId, runId });
