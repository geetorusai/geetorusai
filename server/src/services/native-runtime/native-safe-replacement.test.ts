...
import { buildGeetorusWakePayload, heartbeatService } from "../heartbeat.js";
...
const externalDatabaseUrl = process.env.GEETORUS_TEST_DATABASE_URL;
  ...
  () => {
    ...
    it.each(["verified", "unproven", "unknown_action"] as const)(
      ...
      async (mode) => {
        ...
        const workspace = await mkdtemp(join(tmpdir(), "geetorus-recovery-work-"));
        try {
    ...
    it.each(["unproven", "changed", "verified"] as const)("requires stopped-session proof through commit (%s)", async (mode) => {
      ...
      await appendHeartbeatRunEvent(db, { companyId: source.companyId, runId: source.runId, agentId: source.agentId,
        eventType: "tool.execution.started", stream: "system",
        payload: { name: "geetorus_finish", executionId: "completion", transport: "process" },
      });
      await reconcileSafeNativeReplacements(db, new Date(), { verifyStoppedSession });
      await reconcileSafeNativeReplacements(db, new Date(), { verifyStoppedSession });
      const successors = await db.select().from(heartbeatRuns).where(eq(heartbeatRuns.retryOfRunId, source.runId));
    ...
    it("does not turn an already reconciled continuation into a recovery assignment", async () => {
      ...
      const wake = await buildGeetorusWakePayload({ db, companyId: source.companyId, contextSnapshot: { issueId: source.issueId, recoveryActionId: action.id, wakeReason: "issue_recovery_action_restored" } });
