...
import { buildGeetorusTaskMarkdown } from "../heartbeat.js";
import {
...
describeEmbeddedPostgres("current external-chat wake comment reader", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase(
      "geetorus-current-wake-comments-",
    );
  ...
  it("pages every exact accepted comment in order and excludes task history outside the wake", async () => {
    ...
    ).rejects.toThrow("geetorus_current_wake_comments_cursor_out_of_order");
  ...
  it("requires ordered pages and fails the completion fence after a comment changes", async () => {
    ...
    ).rejects.toThrow("geetorus_current_wake_comments_snapshot_changed");
  ...
  it("surfaces closed attachment omissions and fences their exact snapshot", async () => {
    ...
    expect(page.comments[0]?.attachmentImportNotice).toBe(
      "Geetorus could not import every attachment from this exact external message: 2 attachments were omitted (unsupported type: 1, processing failed: 1). Treat omitted attachments as unavailable; do not infer their contents or substitute an older workspace file.",
    );
    ...
    const wake = contextSnapshot.geetorusWake as Record<string, unknown>;
