...
import {
  ...
  type GeetorusRunnerProviderProfile,
 ...
 */
export const NATIVE_RUNTIME_RESOLVER_VERSION = "geetorus-runner-v1" as const;
...
export type HeartbeatRuntimeResolution =
  ...
  | {
      kind: "native";
      resolverVersion: typeof NATIVE_RUNTIME_RESOLVER_VERSION;
      reason: "explicit_geetorus_runner" | "persisted_native_selection";
      provider: GeetorusRunnerProviderProfile["provider"];
    };
...
export function resolveNativeRuntimeMode(input: {
  ...
  const runnerAdapterSelected = input.agent.adapterType === "geetorus_runner";
  ...
  if (!input.enabled) {
    throw ineligible(
      "geetorus_runner_rollout_disabled",
      "Geetorus Runner is experimental and disabled on this instance.",
    );
  }
  let runnerProfile: GeetorusRunnerProviderProfile;
  try {
    runnerProfile = resolveGeetorusRunnerProviderProfile(input.adapterConfig);
  } catch (error) {
  ...
  if (
    input.agent.adapterType !== "geetorus_runner"
    || input.agent.status !== "active" && input.agent.status !== "running"
  ) {
    throw ineligible(
      "geetorus_runner_agent_ineligible",
      "Geetorus Runner requires an active agent.",
    );
  }
  const allowedWorkModes = ["standard", "planning", "ask"];
  if (!input.issue || !allowedWorkModes.includes(input.issue.workMode)) {
    throw ineligible(
      "geetorus_runner_issue_ineligible",
      "Geetorus Runner requires a standard, planning, or ask task.",
    );
  }
  ...
  if (!rollout.effects.some((effect) => effect.kind === "record_mode_native")) {
    throw ineligible(
      "geetorus_runner_rollout_policy_rejected",
...
export function resolveHeartbeatRuntimeMode(input: {
  ...
  return {
    kind: "native",
    resolverVersion: NATIVE_RUNTIME_RESOLVER_VERSION,
    reason: "explicit_geetorus_runner",
...
export function resolveHeartbeatNativeRuntimeMode(input: {
  ...
  if (input.persisted.runtimeModeResolvedAt) {
    if (input.persisted.runtimeMode === "native") {
      if (input.agent.adapterType !== "geetorus_runner") {
        throw ineligible(
          "geetorus_runner_adapter_binding_mismatch",
          "A persisted native run must remain bound to the Geetorus Runner adapter.",
        );
      }
      if (
        ...
        throw ineligible(
          "geetorus_runner_agent_ineligible",
          "A persisted Geetorus Runner run cannot recover through a non-invokable agent.",
        );
      }
      ...
      if (backend === null) {
        throw ineligible(
          "geetorus_runner_driver_unsupported",
          `Persisted Geetorus Runner driver is unsupported: ${driverKind}`,
        );
      }
      return {
        kind: "native",
