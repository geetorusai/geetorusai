...
describe("stopped Codex turn inventory", () => {
  ...
  const completionRows = (source: string) => [meta, start, context,
    ...
    { type: "event_msg", payload: { type: "item_completed", thread_id: "thread", turn_id: "turn",
      item: { type: "DynamicToolCall", tool: "geetorus_finish", id: completed.callId, arguments: completed.input, status: "completed", success: true } } },
  ...
  it("preserves completion bookkeeping with an exact accepted receipt", () => {
    expect(checkCompletion('const r = await tools.geetorus_finish({summary: "ready"}); text(r);')).toBe(true);
    expect(checkCompletion('const r = await tools.geetorus_finish({summary: "ready"}); text(r);', [])).toBe(false);
    expect(checkCompletion('const r = await tools.geetorus_finish({summary: "different"}); text(r);')).toBe(false);
  });
  it.each([
    'await tools.send_email({}); const r = await tools.geetorus_finish({summary: "ready"}); text(r);',
    'const r = await tools.geetorus_finish({summary: tools.write_file()}); text(r);',
    'const r = await tools.geetorus_finish({get summary() { return "ready"; }}); text(r);',
    'const r = await tools.geetorus_finish({...external, summary: "ready"}); text(r);',
    'const r = await tools.geetorus_finish({summary: `ready`}); text(r);',
    'const r = await tools["geetorus_finish"]({summary: "ready"}); text(r);',
    'const r = await tools.geetorus_finish({summary: "ready"}); tools.send_email(r);',
    'const r = await tools.geetorus_finish({__proto__: {summary: "ready"}}); text(r);',
