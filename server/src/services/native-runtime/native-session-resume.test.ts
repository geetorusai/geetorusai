...
const CONDITIONAL_READER_TOOL_CONTRACT_FINGERPRINT = `sha256:${createHash(
  ...
  .update(
    JSON.stringify({
      schema: "geetorus.native-tool-contract.v1",
      tools: [
        { name: "register_deliverable", version: 1 },
        {
          name: "read_current_wake_comments",
          semanticContract: "geetorus.server-current-wake-comments.v1",
          version: 1,
        },
      ],
    }),
...
const PRE_CHAT_ATTACHMENT_REUSE_TOOL_CONTRACT_FINGERPRINT = `sha256:${createHash(
  ...
  .update(
    JSON.stringify({
      schema: "geetorus.native-tool-contract.v2",
...
const PRE_CHAT_ATTACHMENT_READ_TOOL_CONTRACT_FINGERPRINT = `sha256:${createHash(
  ...
  .update(
    JSON.stringify({
      schema: "geetorus.native-tool-contract.v3",
      executionTargetKind: "local",
      advertisementPolicy: {
        readCurrentWakeComments: "always_advertised_binding_gated.v1",
      ...
      tools: [
        ...
        },
        {
          ...
          semanticContract: "geetorus.server-chat-attachment-reuse.v1",
          version: 1,
        },
        {
...
const PRE_STRUCTURED_HUMAN_INPUT_TOOL_CONTRACT_FINGERPRINT = `sha256:${createHash(
  ...
  .update(
    JSON.stringify({
      schema: "geetorus.native-tool-contract.v4",
      executionTargetKind: "local",
      advertisementPolicy: {
        readCurrentWakeComments: "always_advertised_binding_gated.v1",
        historicalChatAttachments:
      ...
      tools: [
        ...
        },
        {
          ...
          semanticContract: "geetorus.server-chat-attachment-read.v1",
          version: 1,
        },
      ],
    }),
...
const PRE_RESPONSE_WAKE_YIELD_TOOL_CONTRACT_FINGERPRINT = `sha256:${createHash(
  ...
  .update(
    JSON.stringify({
      schema: "geetorus.native-tool-contract.v5",
      executionTargetKind: "local",
      advertisementPolicy: {
        readCurrentWakeComments: "always_advertised_binding_gated.v1",
        historicalChatAttachments:
...
const PRE_EXPLICIT_CHAT_ATTACHMENT_GUIDANCE_TOOL_CONTRACT_FINGERPRINT = `sha256:${createHash(
  ...
  .update(
    JSON.stringify({
      schema: "geetorus.native-tool-contract.v7",
      executionTargetKind: "local",
      advertisementPolicy: {
        readCurrentWakeComments: "always_advertised_binding_gated.v1",
        historicalChatAttachments:
...
const recoveryFakeCodex = resolve(
  import.meta.dirname,
  "../../../../packages/geetorus-runner/test/fixtures/fake-final-burst-codex-app-server.mjs",
);
...
(embeddedSupport.supported &&
  ...
  async () => {
    ...
    const previousStateBase = process.env.GEETORUS_RUNNER_STATE_DIR;
    ...
    try {
      ...
      const makeExecution = (runId: string): NativeExecutionInput => {
        ...
        return {
          ...value,
          schema: "geetorus.native-execution-input.v2",
      ...
      else process.env.GEETORUS_RUNNER_STATE_DIR = previousStateBase;
      await database.cleanup();
      await rm(scratch, { recursive: true, force: true });
    }
  },
...
(embeddedSupport.supported ? describe : describe.skip)(
  ...
  () => {
    it("uses scoped latest checkpoints, preserves progress barriers, and guards bootstrap persistence after the row lock", async () => {
      const database = await startEmbeddedPostgresTestDatabase(
        "geetorus-native-resume-selection-",
      );
      const db = createDb(database.connectionString);
      try {
...
describe("rebindNativeSessionCheckpoint", () => {
  ...
  it.each([
    ...
    "refreshes retained $contract without changing the Geetorus task or prior history",
    ({ retainedFingerprint }) => {
        ...
        buildExecution: (options) => {
          ...
          return {
            ...
            task: {
              ...
              prompt: options.resumedSession
                ? "Geetorus Resume Delta"
                : "Full context for the same Geetorus task",
            },
          };
        },
      });
      ...
      // No deletion/reset of the existing provider checkpoint or Geetorus history.
...
describe("buildNativeExecutionInput wake projection", () => {
  ...
  it("writes native v4 and pins every provider's complete effective configuration", () => {
    ...
    const agentCore = buildNativeExecutionInput({
      ...
      agentCoreProfile: {
        ...
        contextBucket: "geetorus-context",
    ...
    expect(
      ...
      /OPENAI_API_KEY|ANTHROPIC_API_KEY|AWS_SECRET_ACCESS_KEY|GEETORUS_API_KEY/,
    );
  });
  ...
  it.each([false, true])("projects wake context with the appropriate execution contract (conversation=%s)", (conversationMode) => {
    const input = buildNativeExecutionInput({
      ...
      taskPrompt: "Geetorus task context:\n- Issue: DOT-146",
      wakePayload: {
    ...
    expect(input.task.prompt).toContain("## Geetorus Resume Delta");
