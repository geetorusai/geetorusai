import type { PrpStructuredRunResult } from "../../vendor/geetorus-runner/index.js";
...
describe("native runner file handoff", () => {
  ...
  beforeAll(async () => {
    ...
    temporaryRoot = await mkdtemp(
      path.join(tmpdir(), "geetorus-native-file-handoff-"),
    );
  ...
  function authority(
    ...
    return new GeetorusRunnerToolAuthority(db, {
  ...
  it("fails closed for remote targets, symlinks, traversal, hash drift, and foreign bindings", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_file_handoff_remote_unsupported");
    await expect(
      authority().execute(callFor("linked.txt", body, "symlink-denied")),
    ).rejects.toThrow("geetorus_runner_file_handoff_symlink_denied");
    await expect(
      authority().execute(
        callFor(
    ...
    ).rejects.toThrow("geetorus_runner_file_handoff_file_changed");
    await expect(
      authority().execute(callFor("../checked.txt", body, "traversal-denied")),
    ).rejects.toThrow("geetorus_runner_file_handoff_path_denied");
    await expect(
      authority().execute({
    ...
    ).rejects.toThrow("geetorus_runner_file_handoff_hash_mismatch");
    await expect(
      authority({ companyId: "00000000-0000-4000-8000-000000009999" }).execute(
        callFor("checked.txt", body, "foreign-denied"),
      ),
    ).rejects.toThrow("geetorus_runner_tool_binding_not_authorized");
  });
  ...
  it("stages only exact wake-bound inbound bytes without exposing an API credential", async () => {
    ...
    expect(prompt).not.toContain("GEETORUS_API_KEY");
    await stage.cleanup();
    await expect(
      readFile(path.join(workspaceRoot, relativePath!)),
    ...
    ).rejects.toThrow("geetorus_runner_attachment_staging_residue_denied");
    ...
    ).rejects.toThrow("geetorus_runner_attachment_staging_not_authorized");
  ...
  it("stages no historical attachment when the current wake contains only an omission", async () => {
    ...
    const currentComment = await issueService(db).addComment(
      issueId,
      "Inspect the current attachment if Geetorus imported it.",
      { userId: "inbound-user" },
    );
    await db
      .update(heartbeatRuns)
            ...
            comments: [
              {
                ...
                body: "Inspect the current attachment if Geetorus imported it.",
                attachments: [],
              },
            ],
            attachmentOmissions: [
  ...
  it("fails closed on a reminted work product and removes definite pre-commit storage failures", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_file_handoff_work_product_missing");
    ...
    const mismatchingAuthority = new GeetorusRunnerToolAuthority(db, {
      companyId,
      agentId,
      issueId,
      runId,
    ...
    ).rejects.toThrow("geetorus_runner_file_handoff_storage_mismatch");
    ...
    await db.execute(sql`
      CREATE OR REPLACE FUNCTION geetorus_test_fail_native_receipt()
    ...
    `);
    await db.execute(sql`
      CREATE TRIGGER geetorus_test_fail_native_receipt
      BEFORE UPDATE ON heartbeat_runs
      FOR EACH ROW EXECUTE FUNCTION geetorus_test_fail_native_receipt()
    `);
    try {
      const receiptFailureAuthority = new GeetorusRunnerToolAuthority(db, {
        companyId,
        agentId,
        issueId,
        runId,
      ...
      await db.execute(
        sql`DROP TRIGGER IF EXISTS geetorus_test_fail_native_receipt ON heartbeat_runs`,
      );
      await db.execute(
        sql`DROP FUNCTION IF EXISTS geetorus_test_fail_native_receipt()`,
      );
    }
  ...
  it("returns authenticated file-delivery modes and upgrades receipt descriptions without repeating preparation", async () => {
    ...
    try {
      for (const [index, testCase] of cases.entries()) {
        ...
        await db
          .update(heartbeatRuns)
          .set({ contextSnapshot: { issueId, geetorusWake: testCase.wake } })
          .where(eq(heartbeatRuns.id, runId));
  ...
  it.each(["attachment", "work product"])("rejects a previous run's %s for new output and revalidates preserved bytes internally", async (kind) => {
    ...
    try {
      ...
      const replacement = new GeetorusRunnerToolAuthority(db, { companyId, agentId, issueId, runId: nextRunId,
