...
const eligible = {
  ...
  agent: { status: "running", adapterType: "geetorus_runner" },
...
describe("resolveNativeRuntimeMode", () => {
  it("keeps every direct built-in adapter outside native arbitration", () => {
    for (const adapterType of BUILTIN_ADAPTER_TYPES) {
      if (adapterType === "geetorus_runner") continue;
  ...
  it("rejects a fresh Geetorus Runner start while the rollout flag is disabled", () => {
    expect(() => resolveNativeRuntimeMode({
      ...eligible,
      enabled: false,
  ...
  it("rejects unknown Geetorus Runner providers", () => {
    expect(() => resolveNativeRuntimeMode({
      ...eligible,
      runtimeConfig: {},
      adapterConfig: { provider: "claude" },
      agent: { ...eligible.agent, adapterType: "geetorus_runner" },
    })).toThrow(expect.objectContaining({
      code: "geetorus_runner_provider_unsupported",
    }));
  });
  ...
  it("rejects malformed OpenCode and unqualified ACPX profiles", () => {
    expect(() => resolveNativeRuntimeMode({
      ...
      code: "geetorus_runner_opencode_model_invalid",
    }));
    expect(() => resolveNativeRuntimeMode({
      ...eligible,
      adapterConfig: {
      ...
      code: "geetorus_runner_acpx_agent_unavailable",
    }));
    expect(() => resolveNativeRuntimeMode({
      ...eligible,
  ...
  it("rejects incomplete managed-provider selections before a run is persisted", () => {
    ...
    expect(() => resolveNativeRuntimeMode({
      ...
      code: "geetorus_runner_aws_agentcore_profile_required",
    }));
  });
  ...
  it.each(["paused", "terminated", "pending_approval"])(
    ...
    (status) => {
      expect(() => resolveHeartbeatNativeRuntimeMode({
        ...
        code: "geetorus_runner_agent_ineligible",
      }));
    },
  );
  ...
  it("fails closed for an unknown persisted driver", () => {
    expect(() => resolveHeartbeatNativeRuntimeMode({
      ...
      code: "geetorus_runner_driver_unsupported",
    }));
  });
  ...
  it("does not recover a native run through a direct adapter", () => {
    expect(() => resolveHeartbeatNativeRuntimeMode({
      ...
      code: "geetorus_runner_adapter_binding_mismatch",
    }));
  });
  ...
  it("admits remote targets only through geetorus_runner", () => {
    expect(resolveNativeRuntimeMode({
      ...eligible,
      target: { kind: "remote" },
      runtimeConfig: {},
  ...
  it("allows geetorus_runner to use a transient local workspace for projectless issues", () => {
    expect(resolveNativeRuntimeMode({
      ...eligible,
      workspaceId: null,
      agent: { ...eligible.agent, adapterType: "geetorus_runner" },
  ...
  it("admits planning only through geetorus_runner", () => {
    expect(resolveNativeRuntimeMode({
      ...eligible,
  ...
  it("admits ask mode through geetorus_runner while preserving the legacy native boundary", () => {
    expect(resolveNativeRuntimeMode({
      ...eligible,
  ...
  it("keeps every direct built-in adapter on the legacy path", () => {
    for (const adapterType of BUILTIN_ADAPTER_TYPES) {
      ...
      expect(resolveHeartbeatRuntimeMode({
        ...
        kind: "legacy",
        resolverVersion: "geetorus-runner-v1",
        reason: "direct_adapter",
      });
    }
  });
