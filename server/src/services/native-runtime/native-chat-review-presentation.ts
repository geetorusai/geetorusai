...
const SCHEMA = "geetorus.chat_review_response_presentation.v1";
...
function presentationContextSha256(value: unknown): string {
  ...
  const {
    geetorusRuntimeServices: _services,
    geetorusRuntimePrimaryUrl: _url,
    ...scope
  } = record(value);
...
async function reviewPresentationEvidence(
  ...
  if (
    semantic.schema !== "geetorus.run_result.v1" ||
