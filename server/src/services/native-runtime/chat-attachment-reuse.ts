...
class ExternalChatWaitAuthorizationContentionError extends Error {
  constructor() {
    super("geetorus_external_chat_wait_authorization_contended");
...
type ListCursor = {
  schema: "geetorus.chat-attachment-list-cursor.v1";
  conversationId: string;
...
function decodeListCursor(
  ...
  if (typeof value !== "string" || value.length === 0 || value.length > 1024) {
    throw new Error("geetorus_runner_chat_attachment_cursor_invalid");
  }
  try {
    const parsed = record(
    ...
    if (
      parsed.schema !== "geetorus.chat-attachment-list-cursor.v1" ||
...
async function principalAuthorized(
  ...
  const linkQuery = tx
    .select({
      ...
      userId: chatIdentityLinks.geetorusUserId,
    })
...
export async function authorizeChatConversationForBoundRun(
  ...
  if (context.source === "issue.interaction.respond") {
    ...
    if (!answer) throw new Error("geetorus_runner_chat_attachment_binding_denied");
  ...
  if (
    !provider ||
    (context.geetorusHarnessCheckedOut !== true &&
      context.geetorusExternalChatExecutionBound !== true) ||
    commentIds.length === 0
  ) {
  ...
  if (!destinationAllowed(endpoint, conversation, resource)) {
    throw new Error("geetorus_runner_chat_attachment_destination_denied");
  }
  for (const principalId of new Set(links.map((row) => row.principalId!))) {
    if (!(await principalAuthorized(tx, endpoint, principalId, lockMode))) {
      throw new Error("geetorus_runner_chat_attachment_principal_denied");
    }
  }
...
function externalChatWaitCandidate(
  ...
  const commentIds = wakeCommentIds(context);
  const wake = record(context.geetorusWake);
...
export async function resolveExternalChatResponseWaitAuthorizationInTransaction(
  ...
  if (
    ...
    record(run.contextSnapshot).geetorusExternalChatQuestionResponse &&
    !answerContext
  )
    return "revoked";
  const candidate = run
...
async function loadSource(
  ...
  const [sourceComment] = await tx
    ...
    .limit(1);
  if (!sourceComment) {
    throw new Error("geetorus_runner_chat_attachment_source_denied");
  }
  const [row] = await tx
    .select({
  ...
  return input.db.transaction(async (tx) => {
    ...
    for (const candidate of scanned) {
      ...
      try {
        ...
        if (
          !(error instanceof Error) ||
          error.message !== "geetorus_runner_chat_attachment_source_denied"
        ) {
          throw error;
        }
      }
...
async function readSourceBytes(
  ...
  const acquisitionTimer = setTimeout(() => {
    ...
    rejectAcquisition(
      new Error("geetorus_runner_chat_attachment_source_read_timed_out"),
    );
  }, timeoutMs);
  ...
  try {
    for await (const chunk of object.stream) {
      ...
      if (total > source.byteSize || total > MAX_ATTACHMENT_BYTES) {
        object.stream.destroy();
        throw new Error(
          "geetorus_runner_chat_attachment_source_size_mismatch",
        );
      }
      chunks.push(buffer);
    }
  ...
  if (
    ...
    throw new Error(
      "geetorus_runner_chat_attachment_source_integrity_mismatch",
    );
  }
  return body;
}
...
async function putStorageObjectWithin(
  ...
  const timer = setTimeout(() => {
    timedOut = true;
    rejectTimeout(
      new Error("geetorus_runner_chat_attachment_storage_write_timed_out"),
    );
  }, timeoutMs);
  timer.unref?.();
...
export async function prepareReusedChatAttachment(input: {
  ...
  try {
    if (
      ...
      throw new Error("geetorus_runner_chat_attachment_storage_mismatch");
    }
    ...
    if (
      ...
      throw new Error("geetorus_runner_chat_attachment_origin_not_persisted");
    }
