...
import { documents, heartbeatRuns, issues, routineDocuments, routines } from "@geetorusai/db";
import { beforeAll, afterAll, describe, expect, it, vi } from "vitest";
import { startRunnerApiTestServer } from "../../__tests__/helpers/runner-api-server.js";
import { createRunnerdCodexTransport, defaultCapabilityRunnerdBinary } from "../../vendor/geetorus-runner/index.js";
...
describe("runner API against real HTTP routes", () => {
  ...
  const oldSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
  const oldEnabled = process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED;
  beforeAll(async () => {
    process.env.GEETORUS_AGENT_JWT_SECRET = randomUUID();
    process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED = "true";
    server = await startRunnerApiTestServer();
  }, 60_000);
  afterAll(async () => {
    await server?.close();
    if (oldSecret === undefined) delete process.env.GEETORUS_AGENT_JWT_SECRET;
    else process.env.GEETORUS_AGENT_JWT_SECRET = oldSecret;
    if (oldEnabled === undefined) delete process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED;
    else process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED = oldEnabled;
  });

  it.skipIf(!process.env.GEETORUS_REQUIRE_RUNNER_API_INTEGRATION && !existsSync(defaultCapabilityRunnerdBinary()))("runs runnerd → PRP → authority → actual authenticated HTTP", async () => {
...
`);
  ...
  it("cannot opt into API tools through a binding when the operator flag is absent", async () => {
    ...
    } finally { process.env.GEETORUS_RUNNER_API_TOOLS_ENABLED = "true"; }
  });
  ...
  it("revokes advertised API tools without disabling dedicated operations", async () => {
    ...
    vi.stubEnv("GEETORUS_RUNNER_API_TOOLS_ENABLED", "false");
    try {
