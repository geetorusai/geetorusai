...
import { heartbeatRuns, issueThreadInteractions } from "@geetorusai/db";
import type {
...
import type { PrpEvent } from "../../vendor/geetorus-runner/index.js";
import {
  parseGeetorusQuestionResponse,
  parseGeetorusQuestionSet,
  type GeetorusQuestionResponse,
...
const QUESTION_KEY_PREFIX = "geetorus-runner-question:";
const REQUEST_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/;
const TEXT_ANSWER_OPTION_ID = "geetorus_text_answer";
const CUSTOM_ANSWER_OPTION_ID = "geetorus_custom_answer";
...
function toInteractionPayload(questionSet: GeetorusQuestionSet, runtimeRequestId: string) {
  return {
    version: 1 as const,
    ...
    questionSet: questionSet as GeetorusQuestionSetPayload,
    runtimeRequestId,
...
function canonicalResponse(
  ...
  return parseGeetorusQuestionResponse(questionSet, response);
}
...
export async function projectNativeRuntimeRequest(input: {
  ...
  const questionSet = parseGeetorusQuestionSet(request.input);
