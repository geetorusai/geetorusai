...
import type { AdapterExecutionResult } from "@geetorusai/adapter-utils";
import type { Db } from "@geetorusai/db";
import { agentSessionGoalActions, agentTaskSessions } from "@geetorusai/db";
...
interface NativeRunnerPrepareInput {
  ...
  readonly semanticTools: readonly GeetorusSemanticToolDefinition[];
...
export function buildNativeRunnerPreparePayload(
  ...
  return {
    ...
    authorizedTools: createGeetorusRunnerAuthorizedToolSet(input.semanticTools),
  };
}

function executableName(): string {
  return process.platform === "win32" ? "geetorus-runnerd.exe" : "geetorus-runnerd";
}

export function resolveGeetorusRunnerBinary(
  configuredPath = process.env.GEETORUS_RUNNER_BINARY,
): string {
  const candidates = [
    configuredPath,
    resolve(moduleDirectory, "../../vendor/geetorus-runner/bin", executableName()),
    resolve(moduleDirectory, "../../../../packages/geetorus-runner/dist/bin", executableName()),
    resolve(
      moduleDirectory,
      "../../../../packages/geetorus-runner/runner/target/release",
      executableName(),
    ),
  ].filter((candidate): candidate is string => Boolean(candidate));
  if (configuredPath && !isAbsolute(configuredPath)) {
    throw new Error("GEETORUS_RUNNER_BINARY must be an absolute path");
  }
  ...
  throw new Error(
    "geetorus_runner_binary_missing: build @geetorusai/geetorus-runner or set GEETORUS_RUNNER_BINARY",
  );
}
...
export async function executeNativeCodexRunner(input: {
  ...
  const binary = input.runnerBinary ?? resolveGeetorusRunnerBinary();
  ...
  const runtimeRoot = input.runtimeRoot
    ...
    : resolve(resolveGeetorusInstanceRoot(), "runtime", "geetorus-runner");
  ...
  const child = spawn(binary, buildNativeRunnerArguments({
    ...
    env: {
      ...process.env,
      ...input.environment,
      GEETORUS_RUNNER_BOOTSTRAP_TICKET: prepared.bootstrapTicket,
    },
  ...
  try {
    if (!child.pid) throw new Error("geetorus_runner_process_not_started");
    await input.onSpawn({
      pid: child.pid,
    ...
    const completed = await Promise.race([
      ...
      exit.then(async ({ code, signal }) => {
        ...
        throw new Error(
          `geetorus_runner_process_exited: code=${code ?? "null"} signal=${signal ?? "null"}`,
        );
      }),
    ]);
    ...
    return {
      ...
      timedOut: false,
      ...(succeeded ? {} : {
        errorCode: "geetorus_runner_provider_failed",
