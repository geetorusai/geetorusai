...
import type { PrpEvent } from "@geetorusai/geetorus-runner";
...
describeEmbeddedPostgres("native question bridge", () => {
  ...
  beforeAll(async () => {
    temporary = await startEmbeddedPostgresTestDatabase("geetorus-native-question-");
  ...
  it("removes the UI-only marker from a canonical custom response", async () => {
    ...
    const answer = {
      answers: [{
        questionId: "color",
        optionIds: ["geetorus_custom_answer"],
        otherText: "purple",
      }],
    };
    validateNativeQuestionResponseInput(interaction!, answer);
