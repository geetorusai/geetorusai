...
import { activityLog, agents, companies, createDb, heartbeatRuns, issues } from "@geetorusai/db";
...
describe("runner backlog task creation", () => {
  ...
  async function fixture(conversation: boolean) {
    ...
    await db.insert(agents).values({ id: agentId, companyId, name: "Planner", role: "ceo", status: "active", adapterType: "geetorus_runner" });
    ...
    return { companyId, agentId, issueId, runId, binding, enqueueWakeup, authority: new GeetorusRunnerToolAuthority(db, binding) };
  }
  ...
  it.each([false, true])("persists the plan without waking an assigned backlog task (conversation=%s)", async conversation => {
    ...
    await expect(new GeetorusRunnerToolAuthority(db, { ...f.binding, runId: replacement }).execute(call)).resolves.toMatchObject({ task: { id: task!.id, status: "backlog" }, scheduledWakeIds: [] });
    expect(f.enqueueWakeup).not.toHaveBeenCalled();
