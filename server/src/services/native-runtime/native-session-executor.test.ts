...
const durableControlPlaneState = (identity: Record<string, unknown>) => ({
  schema: "geetorus.runner.durable.control-plane-state.v1",
  identity,
});
const durableRunnerState = (
  ...
  schema: "geetorus.runner.durable.state.v1",
  ...identity,
  lifecycle,
});
...
const state = vi.hoisted(() => ({
  ...
  resolveRunnerBinary: vi.fn(() => "/tmp/geetorus-runnerd"),
  release: null as null | (() => void),
}));

vi.mock("../../vendor/geetorus-runner/index.js", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("../../vendor/geetorus-runner/index.js")
  >()),
  ...
  parseGeetorusQuestionSet: (value: unknown) => value,
}));

vi.mock("@geetorusai/adapter-codex-local/server", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("@geetorusai/adapter-codex-local/server")
  >()),
  copyBackCodexAuth: state.copyBackCodexAuth,
}));

vi.mock("./geetorus-runner-tool-authority.js", () => ({
  GeetorusRunnerToolAuthority: class {
...
vi.mock("./native-codex-runner.js", () => ({
  resolveGeetorusRunnerBinary: state.resolveRunnerBinary,
}));
  ...
  it.each(["delivered", "sandbox_missing", "logging_failed"] as const)(
    ...
    const execute = vi.fn(
      async (input: {
        ...
        if (label === "geetorus-runner-launch") {
        ...
        if (label === "geetorus-runner-process-identity") {
          return {
            exitCode: 0,
            signal: null,
            timedOut: false,
        ...
        if (label === "geetorus-runner-monitor") {
          return {
            exitCode: 3,
            signal: null,
            timedOut: false,
        ...
        if (label === "geetorus-runner-diagnostics") {
          return {
            exitCode: 0,
            signal: null,
            timedOut: false,
            stdout: "geetorus-runnerd: provider transport closed",
            stderr: "",
          };
        }
        ...
        if (label === "geetorus-runner-signal") {
    ...
    const launcher = createRemoteRunnerProcessLauncher({
      ...
      remoteBinary: "/runtime/geetorus-runnerd",
    ...
    const handle = launcher({
      command: "/controller/geetorus-runnerd",
    ...
    await expect(handle.completion).resolves.toMatchObject({
      code: null,
      stderr: "geetorus-runnerd: provider transport closed",
    });

    const launch = execute.mock.calls.find(
      ([input]) => input.args?.[2] === "geetorus-runner-launch",
    )?.[0];
    ...
    await vi.waitFor(() =>
      expect(
        execute.mock.calls.some(
          ([input]) =>
            input.args?.[2] === "geetorus-runner-signal" &&
    ...
    try {
      ...
      const execute = vi.fn(
        async (input: { command?: string; args?: string[] }) => {
          ...
          if (label === "geetorus-runner-identity-failure-cleanup") {
      ...
      const cleanup = execute.mock.calls.find(
        ([call]) =>
          call.args?.[2] === "geetorus-runner-identity-failure-cleanup",
      )?.[0];
    ...
    try {
      const execute = vi.fn(
        async (input: { command?: string; args?: string[] }) => {
          ...
          return {
            exitCode:
              label === "geetorus-runner-launch"
                ? 0
                : label === "geetorus-runner-process-identity"
                  ? 3
                  : label === "geetorus-runner-identity-failure-cleanup"
                    ? 4
                    : 1,
            signal: null,
            timedOut: false,
...
describe("remote provider pack manifest", () => {
  ...
  it("accepts a fully digested pack and rejects artifact tampering", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-provider-pack-"));
    ...
    const writeManifest = async () =>
      writeFile(
        ...
        JSON.stringify({
          schema: "geetorus-runner/remote-provider-pack/v1",
...
describe("verified native harness backups", () => {
  ...
  it("accepts a complete digest-matched backup and rejects corruption", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-harness-backup-"));
    try {
  ...
  it("rejects a backup whose provider identity or harness contract changed", async () => {
    const root = await mkdtemp(
      join(tmpdir(), "geetorus-harness-backup-identity-"),
    );
    try {
      const current = join(root, "failover-backups", "current");
      await mkdir(join(current, "runner"), { recursive: true });
  ...
  it("verifies the lease stamp and all backup directory digests before replacement", async () => {
    const stateBase = await mkdtemp(join(tmpdir(), "geetorus-harness-stamp-"));
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    try {
      ...
      if (previousStateDirectory === undefined) {
        delete process.env.GEETORUS_RUNNER_STATE_DIR;
      } else {
        process.env.GEETORUS_RUNNER_STATE_DIR = previousStateDirectory;
      }
  ...
  it("rejects a digest-valid legacy stamp for remote lease authorization", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-legacy-harness-stamp-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    try {
      ...
      expect(
        verifyNativeHarnessBackupStamp(
          {
            schema: "geetorus.native-harness-backup-stamp.v1",
            normalizedSessionId: "native-session",
            runnerInstanceId: "runner-1",
  ...
  it("reads ACPX identity from its provider-owned state after suspension", () => {
    ...
    expect(
      providerSessionIdentityFromDurableProviderState({
        ...
        providerState: {
          schema: "geetorus.runner.acpx-provider-state.v3",
          lifecycle: "suspended",
          activeTurnId: null,
    ...
    (provider, driverKind) => {
      expect(
        providerSessionIdentityFromDurableProviderState({
          ...
          providerState: {
            schema: "geetorus.runner.codex-provider-state.v1",
            lifecycle: "prepared",
    ...
    (provider) => {
      expect(
        providerSessionIdentityFromDurableProviderState({
          ...
          providerState: {
            schema: "geetorus.runner.managed-provider-state.v1",
            lifecycle: "suspended",
            normalizedSessionId: "native-session",
...
describe("remote provider checkpoint snapshots", () => {
  it("excludes Codex scratch and credential state without mutating the live provider home", async () => {
    ...
    await syncRemoteRunnerDirectoryOut({
      ...
      targetPath: "/tmp/geetorus-checkpoint-test-codex-home",
      mode: 0o700,
      excludeEntries: ["tmp", ".tmp", "auth.json", "config.toml"],
    });
    ...
    expect(batch?.files[0]).toMatchObject({
      sourcePath: expect.stringMatching(
        /^\/remote\/session\/filesystem\/\.geetorus-checkpoint-/,
      ),
      targetPath: "/tmp/geetorus-checkpoint-test-codex-home",
      kind: "directory",
      mode: 0o700,
    ...
    expect(String(execute.mock.calls[2]?.[0]?.args?.[1])).toMatch(
      /^rm -rf -- '\/remote\/session\/filesystem\/\.geetorus-checkpoint-/,
    );
  });
  ...
  it("omits nested ACPX-Codex scratch aliases without widening the exclusion", async () => {
    ...
    await syncRemoteRunnerDirectoryOut({
      ...
      targetPath: "/tmp/geetorus-checkpoint-test-acpx",
      mode: 0o700,
  ...
  it("rejects unsafe relative checkpoint exclusions", async () => {
    ...
    await expect(
      syncRemoteRunnerDirectoryOut({
        ...
        targetPath: "/tmp/geetorus-checkpoint-invalid-codex-home",
        mode: 0o700,
  ...
  it("rejects unsafe fallback archives without replacing durable state", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-checkpoint-unsafe-"));
...
describe("remote provider checkpoint restores", () => {
  it("does not upload excluded Codex scratch trees or credentials", async () => {
    const sourcePath = await mkdtemp(
      join(tmpdir(), "geetorus-codex-restore-source-"),
    );
    try {
...
describe("remote preinstalled executable discovery", () => {
  it("stages a relative-path CLI shim without changing its installation or losing arguments", async () => {
    const root = await mkdtemp(join(tmpdir(), "geetorus-codex-shim-"));
    try {
  ...
  it("accepts one normalized absolute executable path", () => {
    expect(
      parseRemoteExecutableCandidate(
        "/home/daytona/.local/bin/geetorus-runnerd\n",
      ),
    ).toBe("/home/daytona/.local/bin/geetorus-runnerd");
  });
  ...
  it("does not accept a merely contract-compatible runnerd when a build-owned artifact is configured", () => {
    expect(
      mayUsePreinstalledRunnerArtifact("/artifacts/geetorus-runnerd"),
    ).toBe(false);
...
describe("remote runner build metadata", () => {
  ...
  const current = {
    schema: "geetorus-runner/runnerd-build-metadata/v1",
...
describe("runtime question fallback", () => {
  const questionSet = {
    schema: "geetorus.question_set.v1" as const,
    ...
    (reason) => {
      const fallback = runtimeQuestionFallbackFromEvent({
        ...
        payload: {
          ...
          replayAllowed: false,
          request: {
            schema: "geetorus.runtime_request.v2",
            requestKind: "runtime",
            requestId: "elicitation-1",
            type: "input",
            status: "pending",
        ...
        payload: {
          ...
          questions: [
            ...
            },
            {
              id: "replicas",
              selectionMode: "single",
              options: [{ id: "__geetorus_text__", freeText: true }],
            },
          ],
        },
      });
...
describe("native provider bootstrap environment", () => {
    ...
    expect(
      buildNativeProviderEnvironment(
        {},
        {
          ...
          GEETORUS_INTERNAL_SECRET: "must-not-leak",
        },
      ),
    ).toEqual({
      PATH: "/opt/homebrew/bin:/usr/bin",
  ...
  it("pins the server-assigned workspace over configured environment input", () => {
    expect(
      buildNativeProviderEnvironment(
        {
          GEETORUS_WORKSPACE_CWD: "/untrusted/configured-workspace",
        },
        { HOME: "/Users/runner" },
        "/Users/runner/.geetorus/instances/default/workspaces/agent-1",
      ),
    ).toEqual({
      HOME: "/Users/runner",
...
const execution = {
  schema: "geetorus.native-execution-input.v1",
  ...
  workspace: {
    cwd: "/tmp/geetorus-native-session-test",
    repoUrl: null,
    repoRef: null,
    branchName: null,
  },
  ...
  completionContract: {
    id: "contract",
    sha256: "sha",
    schemaVersion: "geetorus.completion-contract.v1",
    contract: {
      revision: "1",
...
describe("retained native cleanup activation", () => {
  it.each([
    ...
    const directory = await mkdtemp(
      join(tmpdir(), "geetorus-maintenance-activation-"),
    );
    ...
    const previous = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = directory;
    const canonical = (value: unknown): string =>
    ...
    const key = createHash("sha256")
      .update(
        canonical({
          schema: "geetorus.native-session-scope.v2",
    ...
    const accepted = {
      ...
      resultJson: {
        ...
        terminal: {
          schema: "geetorus.prp.terminal.v1",
    ...
    try {
      ...
      state.cleanup.mockImplementation(async (input) => {
        ...
        const epoch = {
          schema: "geetorus.native_cleanup_runner_epoch.v1",
          requestId: input.requestId,
          epoch: 0,
      ...
      releaseCommit();
      if (previous === undefined) delete process.env.GEETORUS_RUNNER_STATE_DIR;
      else process.env.GEETORUS_RUNNER_STATE_DIR = previous;
...
describe("explicit failed native retry physical evidence", () => {
  it.each([
    ...
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-failed-retry-state-"),
    );
    const previous = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const canonical = (value: unknown): string =>
...
describe("native conversation replies", () => {
  const reply: PrpEvent = {
    schema: "geetorus.prp.event.v1", sourceInstanceId: "runner-1",
...
describe("native governed waits", () => {
  ...
  it("consumes an exact replay observation once without leaking stale state", async () => {
    ...
    const replayedEvent: PrpEvent = {
      schema: "geetorus.prp.event.v1" as const,
...
describe("native startup restart detachment", () => {
  it("remembers shutdown while the session is still opening and detaches its late publication", async () => {
    ...
    await expect(executeGeetorusNativeSession({ db: leaseDb(restarting), execution: restarting, runnerInstanceId: "runner" })).rejects.toBeInstanceOf(NativeControllerDetachedForRestartError);
  });
});
...
describe("native resumed preparation timing", () => {
  it("keeps answered-question ingress at the run root rather than charging it to preparation", async () => {
    ...
    try {
      await executeGeetorusNativeSession({
        db: leaseDb(),
        execution,
...
describe("native session cancellation", () => {
  ...
  it("routes control-plane cancellation to the active normalized session and removes the handle", async () => {
    const running = executeGeetorusNativeSession({
      db: leaseDb(),
      execution,
      runnerInstanceId: "runner",
    });
  ...
  it("binds cancellation to nativeIssueId and merges metadata under a row lock", async () => {
    ...
    expect(cancellationUpdate?.values.resultJson).toMatchObject({
      durableReceipt: { operationId: "operation-1" },
      nativeCancellation: {
        schema: "geetorus.native-cancellation.v1",
...
describe("native runtime request resolution", () => {
  ...
  it("clears completed response reservations when the session tears down", async () => {
    const firstSession = executeGeetorusNativeSession({
      db: leaseDb(),
      execution,
      runnerInstanceId: "runner",
    });
    ...
    state.release = null;
    const secondSession = executeGeetorusNativeSession({
      db: leaseDb(),
      execution,
      runnerInstanceId: "runner",
    });
...
describe("native warm session supervision", () => {
  ...
  it("preserves the active turn when a warm checkpoint resumes the same run", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-warm-same-run-recovery-"),
    );
    const previousGeetorusHome = process.env.GEETORUS_HOME;
    process.env.GEETORUS_HOME = stateBase;
    const activeRun = {
      ...execution,
      binding: {
        ...execution.binding,
    ...
    try {
      ...
      if (previousGeetorusHome === undefined) {
        delete process.env.GEETORUS_HOME;
      } else {
        process.env.GEETORUS_HOME = previousGeetorusHome;
      }
      await rm(stateBase, { recursive: true, force: true });
    }
  });
  ...
  it.each(
    ...
    async ({
      ...
      const stateBase = await mkdtemp(
        join(tmpdir(), "geetorus-runnerd-warm-authority-"),
      );
      const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
      const previousGeetorusHome = process.env.GEETORUS_HOME;
      process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
      ...
      const remoteTarget = (
        local
          ...
          : {
              ...
              environmentId: "environment-runnerd-warm-authority",
              remoteCwd: "/home/daytona/geetorus-workspace",
      ...
      try {
        await executeGeetorusNativeSession({
          ...
          execution: first,
          runnerEnvironment: {
            GEETORUS_GITHUB_AUTH_MODE: firstMode,
            GEETORUS_RUNNER_NETWORK_ACCESS: firstNetwork,
            ...(firstBroker
              ? { GEETORUS_GITHUB_BROKER_TOKEN: "first-run-capability" }
              : {}),
          },
        ...
        if (projectless && replacesProvider) {
          ...
          const checkpointFile = (
            ...
            (path) =>
              path.includes("geetorus-runner/sessions/") &&
              path.endsWith(".json"),
          );
        ...
        await executeGeetorusNativeSession({
          ...
          execution: second,
          runnerEnvironment: {
            GEETORUS_GITHUB_AUTH_MODE: secondMode,
            GEETORUS_RUNNER_NETWORK_ACCESS: secondNetwork,
            ...(secondBroker
              ? { GEETORUS_GITHUB_BROKER_TOKEN: "second-run-capability" }
              : {}),
          },
          runnerInstanceId: "runner-runnerd-warm",
          useRunnerd: true,
  ...
  it("replaces an idle warm provider session when its pinned permission mode changes", async () => {
    ...
    const base = {
      ...execution,
      schema: "geetorus.native-execution-input.v4",
  ...
  it("does not turn an acknowledged Stop before completion into a failure or a retry", async () => {
    ...
    state.execute.mockReset().mockImplementationOnce(async () => {
      Object.assign(stop, { cancelledByActorType: "user", cancelledByUserId: "board", nativeCancellation: {
        schema: "geetorus.native-cancellation.v1", ...execution.binding, scope: "run", reasonCode: "cancellation_run_only",
    ...
    await expect(executeGeetorusNativeSession({
...
describe("native process ownership", () => {
  it("checks the complete wake-comment receipt before finalizing a successful provider turn", async () => {
    const expectedBinding = {
      schema: "geetorus.current-wake-comments-binding.v1",
      companyId: execution.binding.companyId,
      issueId: execution.binding.issueId,
      runId: execution.binding.runId,
      agentId: execution.binding.agentId,
...
describe("runnerd provider runtime wiring", () => {
  ...
  beforeEach(async () => {
    previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    isolatedStateDirectory = await mkdtemp(
      join(tmpdir(), "geetorus-runnerd-wiring-"),
    );
    process.env.GEETORUS_RUNNER_STATE_DIR = isolatedStateDirectory;
  });
  ...
  it("stages from the authenticated run snapshot and cleans up after the provider turn", async () => {
      ...
      attachments: [
        {
          ...
          workspaceRelativePath:
            ".geetorus-inbound/run/00000000-0000-4000-8000-000000009202",
    ...
    state.renderNativeRunnerStagedAttachmentPrompt.mockReturnValueOnce(
      "Geetorus native attachment access: staged.",
    );
    state.execute.mockReset().mockResolvedValueOnce({
      result: { summary: "completed" },
      terminal: { runTerminalState: "succeeded" },
  ...
  it("rejects overlapping runs for the same runnerd provider session scope", async () => {
    ...
    const active = executeGeetorusNativeSession({
      db: leaseDb(first),
      execution: first,
  ...
  it("carries the verified runner and lease binding into a projectless continuation", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-runner-binding-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const prior = {
    ...
    const remoteCwd = "/home/daytona/geetorus-workspace";
    try {
      state.createBackend.mockClear();
      state.createTransport.mockClear();
      await createRunnerdBackend({
    ...
    expect(state.createTransport).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        environment: expect.objectContaining({
          GEETORUS_RUNNER_EXTERNAL_SANDBOX: "1",
        }),
      }),
    );
  ...
  it("uses the native execution workspace as the local provider containment root", async () => {
    ...
    expect(state.createTransport).toHaveBeenCalledWith(
      expect.objectContaining({
        environment: expect.objectContaining({
          GEETORUS_WORKSPACE_CWD: execution.workspace.cwd,
        }),
      }),
    );
    ...
    expect(
      localTransportOptions.environment.GEETORUS_RUNNER_EXTERNAL_SANDBOX,
    ).toBeUndefined();
  });

  it("atomically migrates legacy unscoped state only for its exact durable run identity", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-legacy-runner-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
  ...
  it("migrates the former company/session scope into the full native session scope", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-company-session-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const legacyExecution = {
  ...
  it("migrates a suspended prior-run authority only when its persisted execution has the same full session scope", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-prior-run-session-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
  ...
  it.each([
    ...
    async ({ directLifecycle, backupLifecycle, corrupt, accepted }) => {
      const stateBase = await mkdtemp(
        join(tmpdir(), "geetorus-remote-prior-run-state-"),
      );
      const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
      process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
      const priorExecution = {
  ...
  it("quarantines legacy prior-run state only after the database proves a terminal owner in the same full scope", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-legacy-terminal-unsuspended-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const priorExecution = {
  ...
  it("rejects scoped prior-run state after restart while its heartbeat is still running", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-running-prior-run-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const priorExecution = {
  ...
  it.each([
    ...
    async (scenario) => {
      const stateBase = await mkdtemp(
        join(tmpdir(), "geetorus-quiescent-recovery-"),
      );
      const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
      process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
      const priorExecution = {
                ...
                Promise.resolve([
                  {
                    ...
                    contextSnapshot: {
                      geetorusEnvironment: { driver: "local" },
                    },
                  },
                ]),
            }),
      ...
      try {
        ...
        if (previousStateDirectory === undefined)
          delete process.env.GEETORUS_RUNNER_STATE_DIR;
        else process.env.GEETORUS_RUNNER_STATE_DIR = previousStateDirectory;
        await rm(stateBase, { recursive: true, force: true });
      }
    },
  );
  ...
  it("quarantines scoped prior-run state when the heartbeat is terminal but runnerd is not suspended", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-terminal-unsuspended-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const priorExecution = {
  ...
  it("resumes an existing scoped authority only for the exact current run", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-current-scoped-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const currentExecution = {
  ...
  it.each([
    ...
    async (variant) => {
      const stateBase = await mkdtemp(
        join(tmpdir(), "geetorus-pending-warm-transition-"),
      );
      const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
      process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
      const currentExecution = {
      ...
      try {
        ...
        const pending = {
          ...
          receipt: {
            schema: "geetorus.runner.warm-transition.v1",
            newIdentity: {
              ...identity,
              runId:
        ...
        const core =
          variant === "runner_prepared"
            ...
            : {
                ...durableControlPlaneState(identity),
                schema:
                  "geetorus.runner.durable.control-plane-state.warm-transition.v1",
                ...(variant === "schema_only"
                  ? {}
        ...
        const runner = {
          ...durableRunnerState(identity, "ready"),
          schema: "geetorus.runner.durable.state.warm-transition.v1",
          warmTransition: pending,
        };
        const coreBytes =
          variant === "malformed"
            ? '{"schema":"geetorus.runner.durable.control-plane-state.warm-transition.v1",'
            : JSON.stringify(core);
  ...
  it.each(["unknown_schema", "unknown_lifecycle"] as const)(
    ...
    async (caseName) => {
      const stateBase = await mkdtemp(
        join(tmpdir(), `geetorus-${caseName}-runner-state-`),
      );
      const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
      process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
      const currentExecution = {
        ...
        await writeFile(
          ...
          JSON.stringify(
            caseName === "unknown_schema"
              ? {
                  ...runnerState,
                  schema: "geetorus.runner.durable.state.v999",
                }
              : runnerState,
          ),
        );
  ...
  it.each(["missing", "malformed", "unknown_schema", "mismatched"] as const)(
    ...
    async (caseName) => {
      const stateBase = await mkdtemp(
        join(tmpdir(), `geetorus-${caseName}-scoped-state-`),
      );
      const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
      process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
            ...
            caseName === "malformed"
              ...
              : caseName === "unknown_schema"
                ? JSON.stringify({
                    ...
                    schema: "geetorus.runner.durable.control-plane-state.v999",
                  })
                : JSON.stringify(
                    durableControlPlaneState({
                      runId: scopedExecution.binding.runId,
  ...
  it("does not quarantine an unsafe scoped-root symlink", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-symlink-scoped-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const scopedExecution = {
  ...
  it("rejects a suspended prior-run authority whose persisted execution belongs to another full session scope", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-prior-run-mismatched-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const currentExecution = {
  ...
  it("fails closed instead of claiming a mismatched former session scope", async () => {
    const stateBase = await mkdtemp(
      join(tmpdir(), "geetorus-mismatched-session-state-"),
    );
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const currentExecution = {
  ...
  it("scopes local durable sessions by agent, workspace, and provider profile while reusing them across runs", async () => {
    const stateBase = await mkdtemp(join(tmpdir(), "geetorus-session-scope-"));
    const previousStateDirectory = process.env.GEETORUS_RUNNER_STATE_DIR;
    process.env.GEETORUS_RUNNER_STATE_DIR = stateBase;
    const scopedExecution = (input: {
      ...
      ({
        ...
        workspace: {
          ...
          repoUrl: "https://example.test/geetorus.git",
  ...
  it("uses the remote workspace for both the runner backend and native session", async () => {
    ...
    expect(state.createTransport).toHaveBeenCalledWith(
      expect.objectContaining({
        runnerBinary: "/tmp/geetorus-runnerd",
        environment: expect.objectContaining({
          GEETORUS_WORKSPACE_CWD: remoteCwd,
        }),
      }),
    );
    ...
    expect(
      sshTransportOptions.environment.GEETORUS_RUNNER_EXTERNAL_SANDBOX,
    ).toBeUndefined();
    expect(state.createTransport.mock.calls[0]![0].runnerBinary).not.toBe(
      `${remoteCwd}/.geetorus-runtime/geetorus-runner/bin/geetorus-runnerd`,
    );
  });
  ...
  it.each(["fresh", "existing_state", "symlink_parent", "wrong_identity", "connected", "pending_turn", "remote_probe_failed", "backup_present"])(
    ...
    const runtimeRoot = join(remoteCwd, ".geetorus-runtime", "geetorus-runner");
    ...
    const remoteExecute = vi.fn(async (command: { command: string; args?: string[] }) => {
      if (command.args?.[2] === "geetorus-runner-claim-unstarted-session") {
        let exitCode = 1;
      ...
      if (command.args?.[0] === "--build-metadata") return {
        exitCode: 0, timedOut: false, stdout: JSON.stringify({
          schema: "geetorus-runner/runnerd-build-metadata/v1", binaryName: "geetorus-runnerd",
          packageName: "@geetorusai/geetorus-runner", binaryContractVersion: 2,
    ...
    if (scenario === "fresh") {
      ...
      const claimCommand = remoteExecute.mock.calls.find(([command]) => command.args?.[2] === "geetorus-runner-claim-unstarted-session")![0];
  ...
  it("uses the image's shared Codex without uploading or installing artifacts", async () => {
        ...
        if (command.args?.[0] === "--build-metadata") {
          ...
          if (
            command.command.endsWith(
              "/.geetorus-runtime/geetorus-runner/bin/codex",
            )
          ) {
            throw new Error("reached-preinstalled-codex-verification");
          }
          stdout = "codex-cli 0.153.4";
        } else if (script.includes("command -v geetorus-runnerd")) {
          stdout = "/usr/local/bin/geetorus-runnerd\n";
        } else if (script.includes("command -v codex")) {
          stdout = script.includes("/opt/geetorus-runner/bin/codex")
            ? "/opt/geetorus-runner/bin/codex\n"
            : "/usr/local/bin/codex\n";
        } else if (
          !script.includes("ln -sfn") &&
          !script.includes("geetorus_codex_launcher_tmp")
        ) {
    ...
    expect(remoteExecute).toHaveBeenCalledWith(
      expect.objectContaining({
        command: "/opt/geetorus-runner/bin/codex",
        args: ["--version"],
      }),
    );
    expect(
  ...
  it("binds a remote launch to the configured controller-owned runner artifact", async () => {
    const remoteCwd = "/home/daytona/geetorus-workspace";
    const controllerArtifact = "/controller/artifacts/geetorus-runnerd";
    const remoteExecution = {
      ...execution,
      binding: { ...execution.binding, runId: "run-remote-runner-artifact" },
      workspace: { ...execution.workspace, cwd: "/host/geetorus-workspace" },
    } as NativeExecutionInputV1;
  ...
  it.each([
    ...
    async (providerKind, provider, driverKind) => {
      ...
      await expect(
        ...
        "runner_remote_provider_artifact_incompatible: configure GEETORUS_RUNNER_REMOTE_PROVIDER_PACK_PATH",
      );
      expect(state.createBackend).not.toHaveBeenCalled();
    },
  );
