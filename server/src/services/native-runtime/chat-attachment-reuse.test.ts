...
describe("native same-conversation chat attachment reuse", () => {
  ...
  beforeAll(async () => {
    ...
    const storageRoot = await mkdtemp(
      path.join(tmpdir(), "geetorus-chat-reuse-"),
    );
  ...
  it("authorizes an external-chat response wait only from current durable policy", async () => {
    ...
    try {
      ...
      delete spoofedContext.geetorusWake;
      await db
        .update(heartbeatRuns)
        .set({
  ...
  it("lists metadata only and clones exact bytes once into current-run provenance", async () => {
    ...
    const verifyReceipt = (semanticToolReceipts: unknown) => validateNativeDeliverableEvidence(db, {
      ...
      schema: "geetorus.run_result.v1", reportedWorkDisposition: "done", summary: "Prepared the requested existing file.",
    ...
    const reuseActivity = (
      ...
      (row) => row.details?.source === "geetorus_runner_chat_attachment_reuse",
    );
  ...
  it("bounds a stalled storage write and removes the late object without creating rows", async () => {
    ...
    await expect(
      ...
      "geetorus_runner_chat_attachment_storage_write_timed_out",
    );
    releaseWrite();
  ...
  it("pages a stable deduplicated attachment history with equal timestamps", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_chat_attachment_cursor_invalid");
  });
  ...
  it("keeps an older confirmed lineage when a newer publication was deleted", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_chat_attachment_source_denied");
    await expect(
      authority().execute({
        tool: "reuse_chat_attachment",
  ...
  it("rejects forged source pairs, stale reach, and deleted historical sources", async () => {
    ...
    ).rejects.toThrow("geetorus_runner_chat_attachment_principal_denied");
    await db
      .update(companyMemberships)
      .set({ status: "active" })
      .where(eq(companyMemberships.principalId, userId));
    ...
    ).rejects.toThrow("geetorus_runner_tool_mode_denied");
    await db
      .update(issues)
    ...
    ).rejects.toThrow("geetorus_runner_chat_attachment_destination_denied");
    await db
      .update(chatEndpointResources)
      .set({ enabled: true })
      .where(eq(chatEndpointResources.id, resourceId));
