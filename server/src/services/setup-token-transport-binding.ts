...
} from "@geetorusai/adapter-utils/login-pty-transport";
import {
  runSetupTokenLogin,
...
export function createProductionSetupTokenSandboxProvider(
  ...
  return {
    async acquire({ scope, deadline }) {
      ...
      if (!environment) {
        log("[geetorus] Setup-token login: the selected environment is not found.");
        return failClosed();
      }
      if (environment.driver === "local" || environment.driver === "ssh") {
        ...
        log("[geetorus] Setup-token login: the selected environment has no sandbox.");
        return failClosed();
      }
      if (!deps.openLivePtySession) {
        ...
        log(
          "[geetorus] Setup-token login: the live sandbox pseudo-terminal transport is not bound.",
        );
        return failClosed();
      }
      ...
      if (!Number.isFinite(expiresAtMs) || expiresAtMs > deadline) {
        ...
        log("[geetorus] Setup-token login: the acquired lease expiry does not bound the session deadline.");
        return failClosed();
      }
      ...
      try {
        ...
        // holds no lease.
        await this.release(leaseId);
        log("[geetorus] Setup-token login: the live pseudo-terminal bind failed.");
        void err;
        return failClosed();
      }
    },
  ...
  async function releaseLeaseById(leaseId: string): Promise<void> {
    ...
    if (!environment) {
      ...
      log("[geetorus] Setup-token login: the lease environment is not found for a restart release.");
      return failClosed();
    }
    ...
    if (!driver) {
      ...
      log("[geetorus] Setup-token login: no driver resolves the lease for a restart release.");
      return failClosed();
    }
...
export function createWorkerBoundLoginPtyOpener(
  ...
  return async ({ scope, environmentId, leaseId }) => {
    ...
    if (!providerLeaseId || !pluginId || !driverKey) {
      log("[geetorus] Setup-token login: the lease carries no sandbox worker binding.");
      throw new SetupTokenSessionError(503, SETUP_TOKEN_START_FAILED);
    }
    ...
    try {
      ...
      log("[geetorus] Setup-token login: the adapter type has no login command key.");
      throw new SetupTokenSessionError(503, SETUP_TOKEN_START_FAILED);
    }
