...
function assertOwnedByCurrentUser(stats: Stats, description: string) {
  ...
  if (currentUserId !== undefined && stats.uid !== currentUserId) {
    throw new Error(`${description} must be owned by the Geetorus process user`);
  }
}
...
function readGeneratedSecret(keyPath: string): string {
  ...
  if (existing.length < MIN_SECRET_LENGTH) {
    throw new Error(
      `Invalid decision signing key at ${keyPath} (must be at least ${MIN_SECRET_LENGTH} characters); remove the file to regenerate it or set GEETORUS_DECISION_SIGNING_SECRET`,
    );
  }
  return existing;
}
...
export function resolveDecisionSigningSecret(): string {
  const fromEnv = process.env.GEETORUS_DECISION_SIGNING_SECRET?.trim();
  if (fromEnv) {
    if (fromEnv.length < MIN_SECRET_LENGTH) {
      throw new Error(
        `GEETORUS_DECISION_SIGNING_SECRET must be at least ${MIN_SECRET_LENGTH} characters when set (unset it to use an auto-generated key)`,
      );
    }
    return fromEnv;
  }
 ...
 * GEETORUS_DECISION_SIGNING_SECRET fails fast and the generated key file is
