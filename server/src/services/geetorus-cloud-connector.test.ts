...
import {
  ...
  type GeetorusCloudConnectorConfig,
...
function config() {
  ...
  return {
    ...
    } satisfies GeetorusCloudConnectorConfig,
    sealPublicKey: sealing.publicKey,
  };
}

describe("Geetorus Cloud connector", () => {
  it("refreshes capabilities after enrollment and rejects stale cache writes", async () => {
    const keys = config().config;
    const env = {
      GEETORUS_CLOUD_CONNECTOR_BASE_URL: keys.baseUrl,
      GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID: keys.instanceId,
      GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT: keys.environment,
      GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY: keys.signPrivateKey,
      GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY: keys.sealPrivateKey,
    };
    ...
    invalidateGeetorusCloudConnectorCapabilities();
    try {
      await expect(geetorusCloudConnectorCapabilitiesFromEnv(env)).resolves.toEqual([]);
      await expect(geetorusCloudConnectorCapabilitiesFromEnv(env)).resolves.toEqual([]);
      expect(request).toHaveBeenCalledTimes(1);
      invalidateGeetorusCloudConnectorCapabilities();
      const oldRequest = geetorusCloudConnectorCapabilitiesFromEnv(env);
      invalidateGeetorusCloudConnectorCapabilities();
      await expect(geetorusCloudConnectorCapabilitiesFromEnv(env)).resolves.toEqual(["gmail.read"]);
  ...
  it("starts a signed session with exact endpoint audience and scope contract", async () => {
    ...
    const request = vi.fn(async (_url: string | URL | Request, init?: RequestInit) => {
      ...
      expect(JSON.parse(Buffer.from(encodedHeader!, "base64url").toString("utf8"))).toEqual({
        alg: "EdDSA",
        typ: "geetorus-cloud-connector-request+jwt",
      });
      ...
      expect(claims).toMatchObject({
        ...
        ruri: "https://geetorus.example.test/api/tools/oauth/cloud-connector/callback",
        rst: "state-1",
      });
    ...
    const connector = createGeetorusCloudConnector({ config: keys.config, request: request as typeof fetch });

    await expect(connector.startAuthorization({
      subject,
      companyId,
      returnUri: "https://geetorus.example.test/api/tools/oauth/cloud-connector/callback",
      returnState: "state-1",
  ...
  it("prefers a validated HTTPS provider URL over the legacy confirmation URL", async () => {
    const keys = config();
    const connector = createGeetorusCloudConnector({
      config: keys.config,
  ...
  it("keeps legacy session responses compatible and rejects malformed handoff descriptors", async () => {
    const keys = config();
    const legacy = createGeetorusCloudConnector({
      config: keys.config,
      request: vi.fn(async () => Response.json({
        confirmationUrl: "https://my.example.test/connections/confirm?session=broker-state",
        expiresAt: "2099-08-21T20:00:00.000Z",
    ...
    const malformed = createGeetorusCloudConnector({
      config: keys.config,
      request: vi.fn(async () => Response.json({
        confirmationUrl: "https://my.example.test/connections/confirm?session=broker-state",
  ...
  it("does not expose a broker response body when a request fails", async () => {
    ...
    expect(error).toBeInstanceOf(GeetorusCloudConnectorError);
  ...
  it("requires an all-or-nothing environment configuration and loopback for HTTP", () => {
    expect(geetorusCloudConnectorConfigFromEnv({})).toBeNull();
    expect(() => geetorusCloudConnectorConfigFromEnv({
      GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID: instanceId,
    })).toThrowError(/incomplete/);
    expect(() => geetorusCloudConnectorConfigFromEnv({
      GEETORUS_CLOUD_CONNECTOR_INSTANCE_ID: instanceId,
      GEETORUS_CLOUD_CONNECTOR_SIGN_PRIVATE_KEY: "key",
      GEETORUS_CLOUD_CONNECTOR_SEAL_PRIVATE_KEY: "key",
      GEETORUS_CLOUD_CONNECTOR_ENVIRONMENT: "development",
      GEETORUS_CLOUD_CONNECTOR_BASE_URL: "http://my.example.test",
    })).toThrowError(/HTTPS/);
    const legacyError = (() => {
      try {
        geetorusCloudConnectorConfigFromEnv({
          GEETORUS_ID_CONNECTOR_INSTANCE_ID: instanceId,
          GEETORUS_ID_CONNECTOR_SIGN_PRIVATE_KEY: "key",
          GEETORUS_ID_CONNECTOR_SEAL_PRIVATE_KEY: "key",
          GEETORUS_ID_CONNECTOR_ENVIRONMENT: "development",
  ...
  it("keeps gallery capability discovery available during incomplete enrollment", async () => {
    await expect(geetorusCloudConnectorCapabilitiesFromEnv({
...
function seal(
  ...
  configValue: GeetorusCloudConnectorConfig,
