...
import { authUsers, instanceUserRoles, workspaceRuntimeServices } from "@geetorusai/db";
import type { WorkspaceReadiness } from "@geetorusai/shared";
import {
...
export async function issueWorkspaceLoginHandoff(input: {
  ...
  const { ticket, payload } = issueWorkspaceHandoffTicket({
    ...
    origin,
    issuerInstanceId: resolveGeetorusInstanceId(),
