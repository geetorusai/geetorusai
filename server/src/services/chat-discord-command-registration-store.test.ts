...
import { discordGeetorusCommandDefinition } from "./chat-discord-command-registration.js";
import {
...
const external = process.env.GEETORUS_TEST_DATABASE_URL;
...
suite("Discord command ownership store (real PostgreSQL, no network)", () => {
  ...
  beforeAll(async () => {
    if (external) db = createDb(external);
      ...
      temporary = await startEmbeddedPostgresTestDatabase(
        "geetorus-discord-command-store-",
      );
  ...
  it("keeps prior-copy authority unavailable until its exact owned remote definition is durably upgraded", async () => {
    ...
    const prior = discordGeetorusCommandDefinition(registration.ownerId);
    ...
    expect(f.remote[0]).toMatchObject(
      discordGeetorusCommandDefinition(registration.ownerId),
    );
  });
