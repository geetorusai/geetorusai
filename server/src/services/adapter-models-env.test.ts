...
describe("parseAdapterModelsEnv", () => {
  ...
  it("parses the per-adapter model map", () => {
    const m = parseAdapterModelsEnv({ GEETORUS_ADAPTER_MODELS: ENV });
  ...
  it("defaults label to id when omitted", () => {
    const m = parseAdapterModelsEnv({ GEETORUS_ADAPTER_MODELS: JSON.stringify({ pi_local: [{ id: "tensorix/x/y" }] }) });
    expect(m?.pi_local?.[0]).toEqual({ id: "tensorix/x/y", label: "tensorix/x/y" });
  });
  it("throws on invalid JSON (fail loud)", () => {
    expect(() => parseAdapterModelsEnv({ GEETORUS_ADAPTER_MODELS: "{bad" })).toThrow(/GEETORUS_ADAPTER_MODELS/);
  });
  it("throws when an entry lacks a string id", () => {
    expect(() => parseAdapterModelsEnv({ GEETORUS_ADAPTER_MODELS: JSON.stringify({ a: [{ label: "x" }] }) })).toThrow();
  });
});
