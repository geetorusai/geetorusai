...
import { companies, instanceSettings } from "@geetorusai/db";
...
export function applyExperimentalSettingsPatch(
  ...
  if (!isTruthyRuntimeEnvValue(runtimeEnv.GEETORUS_IN_WORKTREE)) {
    return nextExperimental;
  }
...
export function normalizeExperimentalSettings(raw: unknown): InstanceExperimentalSettings {
  ...
  if (parsed.success) {
    return {
      ...
      enableGeetorusDeveloperMode: parsed.data.enableGeetorusDeveloperMode ?? false,
...
export function applyManagedExperimentalOverlay(
  ...
  >) {
    ...
    next[key] = value;
    managedKeys[key] = { managed: true, managedBy: GEETORUS_CLOUD_MANAGED_BY };
  }
...
export function instanceSettingsService(db: Db, options: InstanceSettingsServiceOptions = {}) {
  // Fail closed: a malformed GEETORUS_MANAGED_CONFIG throws here (and at
  ...
  // Same posture for GEETORUS_SETTING_DEFAULTS: parsed once, applied per
