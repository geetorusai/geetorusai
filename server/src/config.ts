...
import { resolveGeetorusEnvPath } from "./paths.js";
...
const GEETORUS_ENV_FILE_PATH = resolveGeetorusEnvPath();
if (existsSync(GEETORUS_ENV_FILE_PATH)) {
  loadDotenv({ path: GEETORUS_ENV_FILE_PATH, override: false, quiet: true });
}

const CWD_ENV_PATH = resolve(process.cwd(), ".env");
const cwdEnvExists = existsSync(CWD_ENV_PATH);
const isSameFile = cwdEnvExists && existsSync(GEETORUS_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(GEETORUS_ENV_FILE_PATH)
  : CWD_ENV_PATH === GEETORUS_ENV_FILE_PATH;
if (shouldLoadWorkingDirectoryEnv({
  cwdEnvExists,
  isGeetorusEnvFile: isSameFile,
})) {
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.GEETORUS_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const providerFromEnvRaw = process.env.GEETORUS_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.GEETORUS_STORAGE_PROVIDER;
  ...
  const storageLocalDiskBaseDir = resolveHomeAwarePath(
    process.env.GEETORUS_STORAGE_LOCAL_DIR ??
      fileStorage?.localDisk?.baseDir ??
      resolveDefaultStorageDir(),
  );
  const storageS3Bucket = process.env.GEETORUS_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "geetorus";
  const storageS3Region = process.env.GEETORUS_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.GEETORUS_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.GEETORUS_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.GEETORUS_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.GEETORUS_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.GEETORUS_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.GEETORUS_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.GEETORUS_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
    process.env.GEETORUS_TELEMETRY_BACKEND_TOKEN?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = process.env.GEETORUS_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const strictModeFromEnv = process.env.GEETORUS_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? deploymentMode === "authenticated");
  const deploymentExposureFromEnvRaw = process.env.GEETORUS_DEPLOYMENT_EXPOSURE;
  ...
  const bindFromEnvRaw = process.env.GEETORUS_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.GEETORUS_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.GEETORUS_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.GEETORUS_PUBLIC_URL;
  ...
  const managedRuntimePublicUrl = process.env.GEETORUS_MANAGED_RUNTIME_PUBLIC_URL?.trim() || undefined;
  ...
  const disableSignUpFromEnv = process.env.GEETORUS_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.GEETORUS_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.GEETORUS_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.GEETORUS_DB_BACKUP_ENABLED !== undefined
      ? process.env.GEETORUS_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.GEETORUS_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.GEETORUS_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.GEETORUS_DB_BACKUP_DIR ??
  ...
  // GEETORUS_DB_BACKUP_RETENTION_DAYS precedent above.
  const workspaceReaperCooldownDaysEnv =
    process.env.GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS?.trim();
  ...
  return {
    ...
    chatWebhookPublicBaseUrl: parseChatWebhookPublicBaseUrl(
      process.env.GEETORUS_CHAT_WEBHOOK_PUBLIC_URL,
    ),
    authDisableSignUp,
    ...
    uiDevMiddleware: process.env.GEETORUS_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
    secretsMasterKeyFilePath:
      resolveHomeAwarePath(
        process.env.GEETORUS_SECRETS_MASTER_KEY_FILE ??
    ...
    announcementsEnabled: process.env.GEETORUS_ANNOUNCEMENTS_ENABLED !== "false",
    announcementsFeedUrl: process.env.GEETORUS_ANNOUNCEMENTS_FEED_URL?.trim() || "https://pages.geetorus.ing/announcements/v1/current.json",
  };
}
