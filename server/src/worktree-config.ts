...
function emptyWorktreeEnvFileContents(): string {
  return [
    ...
    "# Generated by Geetorus worktree repair",
    "",
  ].join("\n");
}
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.GEETORUS_IN_WORKTREE !== "true") return null;

  const configPath = resolveGeetorusConfigPath(overrideConfigPath);
  const envPath = resolveGeetorusEnvPath(configPath);
  const persistedEnv = readEnvEntries(envPath);

  // GEETORUS_IN_WORKTREE can leak in from a parent process or a sourced env
  ...
  // a target as a worktree when its config sits in a `<root>/.geetorus/`
  ...
  if (path.basename(path.dirname(configPath)) !== ".geetorus") return null;
  if (persistedEnv.GEETORUS_IN_WORKTREE !== "true") return null;

  const persistedConfigPath = nonEmpty(persistedEnv.GEETORUS_CONFIG);
  ...
  const worktreeName =
    nonEmpty(stablePersistedEnv.GEETORUS_WORKTREE_NAME) ??
    nonEmpty(env.GEETORUS_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(stablePersistedEnv.GEETORUS_INSTANCE_ID) ??
    nonEmpty(env.GEETORUS_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(stablePersistedEnv.GEETORUS_HOME) ??
      nonEmpty(env.GEETORUS_HOME) ??
      nonEmpty(env.GEETORUS_WORKTREES_DIR) ??
      "~/.geetorus-worktrees",
  );
...
function writeConfigFile(configPath: string, config: GeetorusConfig): boolean {
  fs.mkdirSync(path.dirname(configPath), { recursive: true });
  const update = geetorusConfigSchema.parse(config);
  const source = fs.existsSync(configPath)
    ? geetorusConfigSchema.parse(JSON.parse(fs.readFileSync(configPath, "utf8")))
    : null;
  const nextConfig = source
    ? geetorusConfigSchema.parse(mergeGeetorusConfig(source, update))
    : update;
...
function collectSiblingWorktreePorts(
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".geetorus", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as GeetorusConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: GeetorusConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: GeetorusConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.GEETORUS_HOME = context.homeDir;
  process.env.GEETORUS_INSTANCE_ID = context.instanceId;
  process.env.GEETORUS_CONFIG = context.configPath;
  process.env.GEETORUS_CONTEXT = context.contextPath;
  process.env.GEETORUS_WORKTREE_NAME = context.worktreeName;
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const runtimeConfig = withWorktreePortRegistryLockSync(context.homeDir, () => {
        const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as GeetorusConfig;
  ...
  const toolActionSigningSecret =
    ...
    nonEmpty(existingEnvEntries.GEETORUS_TOOL_ACTION_SIGNING_SECRET) ??
    randomBytes(32).toString("hex");

  const managedEnvEntries: Record<string, string> = {
    GEETORUS_HOME: context.homeDir,
    GEETORUS_INSTANCE_ID: context.instanceId,
    GEETORUS_CONFIG: context.configPath,
    GEETORUS_CONTEXT: context.contextPath,
    GEETORUS_IN_WORKTREE: "true",
    GEETORUS_DB_BACKUP_ENABLED: "false",
    GEETORUS_WORKTREE_NAME: context.worktreeName,
    GEETORUS_TOOL_ACTION_SIGNING_SECRET: toolActionSigningSecret,
  };

  process.env.GEETORUS_DB_BACKUP_ENABLED = "false";
  process.env.GEETORUS_TOOL_ACTION_SIGNING_SECRET = toolActionSigningSecret;
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: GeetorusConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as GeetorusConfig;
  } catch {
    return;
  }
