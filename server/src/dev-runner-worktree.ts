...
export function resolveWorktreeEnvFilePath(rootDir: string): string {
  return path.resolve(rootDir, ".geetorus", ".env");
}

export function isWorktreeSeedPending(rootDir: string): boolean {
  const markerDir = path.resolve(rootDir, ".geetorus");
...
function resolveDefaultWorktreeHome(env: NodeJS.ProcessEnv): string {
  return path.resolve(expandHomePrefix(env.GEETORUS_WORKTREES_DIR?.trim() || "~/.geetorus-worktrees"));
}
...
function repairStaleMigratedWorktreeEnvEntries(
  ...
  const localConfigPath = path.resolve(rootDir, ".geetorus", "config.json");
  const configuredPath = entries.GEETORUS_CONFIG?.trim();
  ...
  return {
    ...
    GEETORUS_CONFIG: localConfigPath,
    GEETORUS_CONTEXT: path.resolve(homeDir, "context.json"),
  };
}
