...
Run this checklist on every heartbeat. This covers both your local planning/memory work and your organizational coordination via the Geetorus skill.

## 1. Identity and Context

- `GET /api/agents/me` -- confirm your id, role, budget, chainOfCommand.
- Check wake context: `GEETORUS_TASK_ID`, `GEETORUS_WAKE_REASON`, `GEETORUS_WAKE_COMMENT_ID`.
...
If `GEETORUS_APPROVAL_ID` is set:
...
- If `GEETORUS_TASK_ID` is set and assigned to you, prioritize that task.

## 5. Checkout and Work

- For scoped issue wakes, Geetorus may already checkout the current issue in the harness before your run starts.
...
- Use `geetorus-create-agent` skill when hiring new agents.
...
- Always use the Geetorus skill for coordination.
- Always include `X-Geetorus-Run-Id` header on mutating API calls.
