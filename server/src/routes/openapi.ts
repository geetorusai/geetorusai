...
} from "@geetorusai/shared/company-import-transfer";
...
const chatPrincipalLinkResponseSchema = z
  .object({
    ...
    geetorusUserId: z.string().nullable(),
    geetorusUserLabel: z.string().nullable(),
...
const safeExternalChatCardResponseSchema = z
  .object({
    schema: z.literal("geetorus.chat.card.v1"),
...
function applyDocumentFixups(document: any): any {
  ...
  document.components.securitySchemes = {
    [BOARD_SESSION_AUTH_SCHEME]: {
      type: "apiKey",
      in: "cookie",
      name: "geetorus_session",
      description:
        "Board session cookie in authenticated mode. Geetorus uses Better Auth; cookie transport may vary by deployment.",
    },
    ...
    [AGENT_BEARER_AUTH_SCHEME]: {
      ...
      description:
        "Agent API key or Geetorus-issued local agent JWT presented in the Authorization bearer header.",
    },
    ...
    [AGENT_RUN_AUTH_SCHEME]: {
      ...
      description: "Geetorus-issued JWT bound to an active task run. Agent API keys, board sessions, and connection-only tokens are rejected.",
    },
  };
  ...
  for (const [path, pathItem] of Object.entries(document.paths ?? {})) {
    for (const [method, operation] of Object.entries(
      ...
      operation["x-geetorus-authorization"] =
      ...
      z.object({
        ...
        cloud: z
          .object({
            ...
            managedBy: z.literal("geetorus-cloud"),
...
registry.registerPath({
  ...
  description:
    "Creates one provider bot endpoint bound permanently to one Geetorus agent. Provider setup and verification happen in later calls.",
  request: {
...
registry.registerPath({
  ...
  description:
    "Runs a setup or lifecycle action. `configure` and `reconnect` accept provider credentials (Slack: `botToken`, `signingSecret`; GitHub: `appId`, `privateKey` after Geetorus generates the webhook secret; Discord: `applicationId`, `guildId`, `botToken`; Microsoft Teams: `clientId`, `tenantId`, `clientSecret`; Telegram: `botToken`; iMessage Photon: `projectSecret`, with nonsecret `photon.projectId` and `photon.lineId` configuration). Credentials are stored as Geetorus secret references and are never returned. Other actions do not require credentials.",
  request: {
    params: z.object({ endpointId: z.string().uuid() }),
...
registry.registerPath({
  ...
  description:
    "Activates a verifying endpoint only after Geetorus has received a real provider event since the server-issued setup test boundary. iMessage Photon additionally requires a fresh linked sender's task and a successful outbound agent publication.",
  request: { params: z.object({ endpointId: z.string().uuid() }) },
  responses: {
    200: r.ok(chatEndpointResponseSchema),
    401: r.unauthorized,
...
registry.registerPath({
  ...
  description:
    "Lists provider identities and their explicit Geetorus identity-link status for this endpoint's provider account.",
  request: { params: z.object({ endpointId: z.string().uuid() }) },
  responses: {
...
registry.registerPath({
  ...
  description:
    "Creates a short-lived confirmation URL for a human external identity belonging to this endpoint. The signed-in Geetorus user must confirm the link separately.",
  request: {
    params: z.object({
      endpointId: z.string().uuid(),
      principalId: z.string().uuid(),
...
registry.registerPath({
  ...
  description:
    "Links the token's external identity to the currently signed-in Geetorus user after rechecking active company membership and canonical-link conflicts.",
...
registry.registerPath({
  ...
  description:
    "Lists each durable provider conversation-to-Geetorus-task binding for the endpoint, including provider and task links and the latest publication state.",
  request: { params: z.object({ endpointId: z.string().uuid() }) },
  responses: {
...
registry.registerPath({
  ...
  description:
    "After checking the provider, an operator may mark an ambiguous durable provider reply delivered, retry it while accepting duplicate risk, or cancel it. Slack slash-command task starts support explicit retry or cancel only. Geetorus never replays an ambiguous provider action automatically, and every resolution is audited.",
  request: {
    params: z.object({
      endpointId: z.string().uuid(),
...
registry.registerPath({
  ...
  description:
    "Board failed-run retries supply failedRunId with reason retry_failed_run. Geetorus derives the exact request and current authorization; a chat retry may return a durable queued/deferred receipt before a run exists. Caller task/comment markers and fresh-session overrides do not authorize replay.",
  request: {
    params: z.object({ id: z.string() }),
...
registry.registerPath({
  ...
  tags: ["runs"],
  summary: "Resolve a pending Geetorus runner runtime request",
  request: {
...
registry.registerPath({
  ...
  tags: ["companies"],
  summary: "Apply the onboarding seed Geetorus Cloud collected at signup",
  request: { params: z.object({ companyId: z.string() }) },
...
registry.registerPath({
  ...
  description:
    ...
    "`x-geetorus-cloud-async-import: 1` header (browsers cannot — the Cloud harness proxy " +
    "strips inbound `x-geetorus-cloud-*` headers), while board sessions use the proxy-safe " +
...
registerCurrentRoute({
  ...
  tags: ["tool-access"],
  summary: "Handle a brokered Geetorus Cloud OAuth callback",
});

registerCurrentRoute({
  method: "get",
  path: "/api/tools/oauth/geetorus-id/callback",
  tags: ["tool-access"],
  summary: "Handle a legacy brokered Geetorus ID OAuth callback",
});
...
registerCurrentRoute({
  ...
  tags: ["tool-access"],
  summary: "Get Geetorus Cloud connector enrollment status",
});
...
registerCurrentRoute({
  ...
  path: "/api/tools/oauth/cloud-connector/enrollment",
  tags: ["tool-access"],
  summary: "Start Geetorus Cloud connector enrollment",
  body: z
...
registerCurrentRoute({
  ...
  tags: ["tool-access"],
  summary: "Complete Geetorus Cloud connector enrollment",
  query: z
    .object({
...
export function buildOpenApiDocument(): any {
  return applyDocumentFixups({
    ...
    info: {
      title: "Geetorus API",
      version: "1.0.0",
      description: "REST API for the Geetorus AI agent management platform",
    },
    servers: [{ url: "/" }],
