...
import type { PluginPerformActionActorContext, ToolRunContext } from "@geetorusai/plugin-sdk";
import { JsonRpcCallError, PLUGIN_RPC_ERROR_CODES } from "@geetorusai/plugin-sdk";
import {
  assertAuthenticated,
  assertBoard,
  assertBoardOrAgent,
 ...
 * (`instance.plugins` in GEETORUS_HIDDEN_SETTINGS), plugin lifecycle and
...
type PluginUiSlotDeclaration = NonNullable<NonNullable<GeetorusPluginManifestV1["ui"]>["slots"]>[number];
/** Launcher declaration extracted from plugin manifest */
type PluginLauncherDeclaration = NonNullable<GeetorusPluginManifestV1["launchers"]>[number];
...
interface PluginInstallRequest {
  /** npm package name (e.g., @geetorus/plugin-linear) or local path */
  packageName: string;
...
const EXPERIMENTAL_BUNDLED_PLUGIN_PACKAGE_NAMES = new Set([
  ...
  "@geetorusai/plugin-workspace-diff",
]);
/**
...
function manifestSourcePath(packageRoot: string, pkgJson: Record<string, unknown>): string | null {
  ...
  const manifestPath = (geetorusPlugin as Record<string, unknown>).manifest;
    ...
    bundledPlugins.push({
      entry: {
        ...
        description: metadata.description
          ?? `Bundled Geetorus plugin from ${path.relative(REPO_ROOT, packageRoot)}.`,
...
export function pluginRoutes(
   ...
   *   {
   *     "pluginId": "plg_123",
   *     "pluginKey": "geetorus.claude-usage",
   ...
   * `GEETORUS_MANAGED_CONFIG` environment variable) enforce a positive
