...
import { extractIssueReferenceIdentifiers, requiresExecutionReconciliation } from "@geetorusai/shared";
import {
...
import { trackAgentTaskCompleted } from "@geetorusai/shared/telemetry";
...
function requiresGeetorusAttachmentMetadata(
  input: {
    type?: unknown;
    provider?: unknown;
  },
  ...
  return type === "artifact" && provider === "geetorus";
}
    ...
    .wakeup(input.issue.assigneeAgentId, {
      ...
      payload: {
        ...
        ...(rejectionAgentMessage
          ? { geetorusAgentMessage: rejectionAgentMessage }
          : {}),
        ...(reviewPathContext ?? {}),
  ...
  if (existing) {
    ...
    if (
      ...
      const event: IssueListStormEvent = {
        ...
        visibilityHint: input.req.header("x-geetorus-tab-visible") ?? null,
      };
...
export function issueRoutes(
  ...
  async function resolveRunIssueWorkspaceInheritanceSource(
    ...
    if (!context || !readNonEmptyString(context.executionWorkspaceId))
      return null;
    const geetorusIssue =
      context.geetorusIssue && typeof context.geetorusIssue === "object"
        ? (context.geetorusIssue as Record<string, unknown>)
        : null;
    return (
      readNonEmptyString(context.issueId) ??
      readNonEmptyString(geetorusIssue?.id)
    );
  }
  ...
  async function canonicalizeGeetorusArtifactMetadata(input: {
    issue: { id: string; companyId: string };
  ...
  async function deriveRecoveryCommentPresentation(
    ...
    const geetorusWake = readObject(context.geetorusWake);
    const recovery = readObject(geetorusWake.recovery);
    const wakeReason =
      typeof context.wakeReason === "string"
        ? context.wakeReason
        : typeof geetorusWake.reason === "string"
          ? geetorusWake.reason
          : null;
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    res.setHeader("X-Geetorus-Request-Cache", coordinated.cacheStatus);
  ...
  router.get("/issues/:id", async (req, res) => {
    ...
    res.setHeader(
      "Server-Timing",
      `geetorus_issue;dur=${(performance.now() - requestStartedAt).toFixed(1)}`,
    );
    res.json({
      ...issue,
      ...inboxArchiveFields,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (requiresGeetorusAttachmentMetadata(createInput)) {
        createInput.metadata = await canonicalizeGeetorusArtifactMetadata({
          issue,
          metadata: req.body.metadata ?? null,
        });
      }
      const attachmentId =
        createInput.type === "artifact" && createInput.provider === "geetorus"
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      if (requiresGeetorusAttachmentMetadata(patch, existing)) {
        if (patch.metadata !== undefined) {
          patch.metadata = await canonicalizeGeetorusArtifactMetadata({
            issue,
            metadata: patch.metadata ?? null,
          });
        } else if (!requiresGeetorusAttachmentMetadata(existing)) {
          res
            .status(422)
    ...
    async (req, res) => {
      ...
      try {
        queue = await db.transaction(async (tx) => {
          ...
          if (locked.queue.protocol !== "geetorus_runner_v1") {
