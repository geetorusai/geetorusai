...
export function runtimeConnectionIntentRoutes(db: Db) {
  ...
  router.post("/runtime-tools/github/credentials", async (req, res) => {
    ...
    const claims = verifyRuntimeToolsToken(typeof req.headers["x-geetorus-github-capability"] === "string"
      ? req.headers["x-geetorus-github-capability"] : bearer(req), "github_credentials");
  ...
  router.get("/mcp/runtime-tools", async (req, res) => {
    ...
    res.json({ name: "geetorus-runtime-tools", protocolVersion: "2025-03-26" });
  });
    ...
    if (request.method === "initialize") {
      res.json({
        ...
        result: {
          ...
          serverInfo: { name: "geetorus-runtime-tools", version: "1" },
        },
      });
      return;
    }
