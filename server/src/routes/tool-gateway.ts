...
import { agents, toolApplications, toolCallEvents, toolConnections, toolInvocations } from "@geetorusai/db";
import {
...
} from "@geetorusai/shared/validators/tool-access";
...
function gatewayToken(req: { header(name: string): string | undefined }) {
  return req.header("x-geetorus-tool-gateway-token")?.trim() || null;
}
  ...
  try {
    ...
    if (body.method === "initialize") {
      ...
      res.json({
        ...
        result: {
          ...
          serverInfo: { name: "Geetorus MCP Gateway", version: "1.0.0" },
          _meta: {
            "geetorus/mcp-app-ui": "unsupported",
            "geetorus/mcp-app-ui-detail": "Interactive ui:// iframe hosting is not available in Geetorus Runner.",
          },
        },
      });
      return;
      ...
      res.json({
        ...
        result: {
          tools: [
            ...
            {
              name: "geetorus_list_resources",
              description: "List resources from fully assigned MCP connections.",
              inputSchema: { type: "object", properties: {}, additionalProperties: false },
            },
            {
              name: "geetorus_read_resource",
              description: "Read a resource URI returned by geetorus_list_resources.",
              inputSchema: { type: "object", required: ["uri"], properties: { uri: { type: "string" } }, additionalProperties: false },
            },
            {
              name: "geetorus_list_prompts",
              description: "List prompts from fully assigned MCP connections.",
              inputSchema: { type: "object", properties: {}, additionalProperties: false },
            },
            {
              name: "geetorus_get_prompt",
              description: "Get a prompt returned by geetorus_list_prompts.",
    ...
    if (body.method === "tools/call") {
      ...
      const contextMethods = {
        geetorus_list_resources: "resources/list",
        geetorus_read_resource: "resources/read",
        geetorus_list_prompts: "prompts/list",
        geetorus_get_prompt: "prompts/get",
      } as const;
