...
import { issues, projects, projectWorkspaces } from "@geetorusai/db";
import {
...
import type { WorkspaceRuntimeDesiredState, WorkspaceRuntimeServiceStateMap } from "@geetorusai/shared";
import {
...
function resolveFallbackSeedSourceConfigPath(baseWorkspaceCwd: string): string | null {
  return baseWorkspaceDeclaresInstanceConfig(baseWorkspaceCwd) ? null : resolveGeetorusConfigPath();
}
...
export function executionWorkspaceRoutes(db: Db, opts: { pluginWorkerManager?: PluginWorkerManager } = {}) {
  ...
  async function handleExecutionWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Geetorus can run workspace commands" });
      return;
    }
    ...
    if (action === "repair") {
      const manifestPath = path.join(workspaceCwd, ".geetorus", "seed-manifest.json");
      let manifest: {
        attemptId?: unknown;
      ...
      try {
        ...
        repairSeedSource = resolveCanonicalWorktreeSeedSource({
          ...
          targetConfigPath: path.join(workspaceCwd, ".geetorus", "config.json"),
        ...
        repairCliArgs = isReadableFile(cliRunner) && isReadableFile(cliEntry)
          ...
          : isReadableFile(cliDist)
            ...
            : null;
        if (!repairCliArgs) {
          throw new Error("Workspace repair cannot find a runnable Geetorus CLI in the base workspace.");
        }
      } catch (error) {
        throw unprocessable(
      ...
      run: async (reportProgress) => {
        ...
        if (action === "run") {
          ...
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Geetorus can run workspace commands");
          }
        ...
        if (action === "repair") {
          ...
          try {
            ...
            const child = spawn(process.execPath, [
              ...
              env: {
                ...process.env,
                GEETORUS_SEED_EXPECTED_COMPANY_ID: existing.companyId,
                GEETORUS_WORKSPACE_BASE_CWD: baseWorkspaceCwd,
                GEETORUS_PROJECT_WORKSPACE_ID: existing.projectWorkspaceId ?? "",
              },
            ...
            if (repairRestartsRuntimeServices) {
              const availableWorkspace = await ensureWorkspaceAvailable();
              if (!availableWorkspace) {
                throw new Error("Execution workspace needs a local path before Geetorus can restart it.");
              }
        ...
        if (action === "start" || action === "restart") {
          const availableWorkspace = await ensureWorkspaceAvailable();
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Geetorus can manage local runtime services");
          }
          let startedServices;
          try {
            startedServices = await startRuntimeServicesForWorkspaceControl({
