...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole } from "@geetorusai/shared";
import {
  forbidden,
  conflict,
  notFound,
 ...
 * (`instance.access` in GEETORUS_HIDDEN_SETTINGS), instance-admin user
...
/** Resolve the Geetorus repo skills directory (built-in / managed skills). */
function resolveGeetorusSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  const claudeSkillsDir = resolveClaudeSkillsDir();
  const geetorusSkillsDir = resolveGeetorusSkillsDir();

  // Build set of Geetorus-managed skill names
  const geetorusSkillNames = new Set<string>();
  if (geetorusSkillsDir) {
    try {
      for (const entry of fs.readdirSync(geetorusSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) geetorusSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isGeetorusManaged: geetorusSkillNames.has(entry.name),
      });
    }
  ...
  if (geetorusSkillsDir) {
    ...
    try {
      for (const entry of fs.readdirSync(geetorusSkillsDir, { withFileTypes: true })) {
        ...
        const skillMdPath = path.join(geetorusSkillsDir, entry.name, "SKILL.md");
        let description = "";
        try {
          const md = fs.readFileSync(skillMdPath, "utf8");
          description = parseSkillFrontmatter(md).description;
        ...
        skills.push({
          name: entry.name,
          description,
          isGeetorusManaged: true,
        });
      }
    } catch { /* skip Geetorus skills directory */ }
  }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.geetorusApiUrl)) {
    const legacyGeetorusApiUrl = nonEmptyTrimmedString(input.geetorusApiUrl);
    if (legacyGeetorusApiUrl) merged.geetorusApiUrl = legacyGeetorusApiUrl;
  }
...
export function assertLegacyAgentInviteAdapterType(
  ...
  if (adapterType === "geetorus_runner") {
    throw badRequest(
      "Geetorus Runner is not available through agent invite onboarding.",
      { code: "geetorus_runner_invite_onboarding_disabled" },
    );
  }
}
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    geetorusApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.geetorusApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
        ...
        if (apiBaseUrl.protocol !== "http:" && apiBaseUrl.protocol !== "https:") {
          ...
          if (mappedDefaultDashboardRoot) {
            diagnostics.push({
              ...
              hint: "Hermes dashboard and /chat routes are browser UI routes. Geetorus gateway calls use /api/health and /api/v1/runs.",
            });
          }
  ...
  const rawGeetorusApiUrl =
    typeof defaults.geetorusApiUrl === "string"
      ? defaults.geetorusApiUrl.trim()
      : "";
  if (rawGeetorusApiUrl) {
    try {
      const parsedGeetorusApiUrl = new URL(rawGeetorusApiUrl);
      if (
        parsedGeetorusApiUrl.protocol !== "http:" &&
        parsedGeetorusApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_geetorus_api_url_protocol",
          level: "warn",
          message: `geetorusApiUrl must use http:// or https:// (got ${parsedGeetorusApiUrl.protocol}).`
        });
      } else {
        normalized.geetorusApiUrl = parsedGeetorusApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_geetorus_api_url_configured",
          level: "info",
          message: `geetorusApiUrl set to ${parsedGeetorusApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_geetorus_api_url_invalid",
        level: "warn",
        message: `Invalid geetorusApiUrl: ${rawGeetorusApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Geetorus host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Geetorus is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run npx geetorusai allowed-hostname <host>`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const skillPath = `/api/invites/${token}/skills/geetorus`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external Geetorus agent, save your one-time claim secret, wait for board approval, then claim your Geetorus API key through the standard claim endpoint. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. Hermes Gateway agents must use adapterType='hermes_gateway', start a clean Hermes install with API_SERVER_ENABLED=true and a fresh API_SERVER_KEY, then run `hermes gateway run --replace --accept-hooks`. Put the Hermes gateway URL in agentDefaultsPayload.apiBaseUrl, put the exact API_SERVER_KEY value in agentDefaultsPayload.apiKey, and put the reachable Geetorus base URL in agentDefaultsPayload.geetorusApiUrl. If you use the default Hermes dashboard root or /chat URL on port 9119, Geetorus maps it to /api automatically. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Runtime-specific adapter config. OpenClaw Gateway agents must include url (ws:// or wss://) and headers.x-openclaw-token. Hermes Gateway agents must include apiBaseUrl, apiKey set to the Hermes API_SERVER_KEY, and geetorusApiUrl. A default Hermes dashboard root or /chat URL such as http://127.0.0.1:9119/chat is accepted and maps to /api. Other runtimes should include the config their adapter expects."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Geetorus hostname is reachable and allowed via `npx geetorusai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Geetorus API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific Geetorus skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which Geetorus adapter type matches your runtime.

    Use adapterType only when there is a matching Geetorus adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    OpenClaw Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "geetorusApiUrl": "https://geetorus-hostname-your-agent-can-reach:3100",
    ...
    - Set agentDefaultsPayload.apiBaseUrl to the Hermes gateway URL Geetorus can reach.
    - Set agentDefaultsPayload.apiKey to the exact same value as API_SERVER_KEY. This is the Hermes gateway key, not the Geetorus key.
    ...
    - Watch out: /chat and the dashboard root are browser UI routes. Geetorus tests /api/health and starts runs with /api/v1/runs after mapping them to the API base.
    - Set agentDefaultsPayload.geetorusApiUrl to the Geetorus base URL Hermes can reach.
    - Use hermes_local when Geetorus should start Hermes on the Geetorus host.
    - Use hermes_gateway when Geetorus should call an already-running Hermes API server.
    - After board approval, claim the Geetorus API key once with the claim endpoint below and save it as GEETORUS_API_KEY. Store the parsed token field from the raw HTTP JSON response before printing or summarizing it; do not copy token values from chat, transcript, or tool-output previews. A token value containing literal ... or [redacted] is a masked display preview, not a valid key. Do not rotate or invent a Geetorus key manually.
    - Hermes-originated Geetorus API usage means Hermes calls Geetorus with GEETORUS_API_URL and GEETORUS_API_KEY after approval/key claim. Do not confuse that with agentDefaultsPayload.apiBaseUrl, which points Geetorus to Hermes.
    ...
    Hermes Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "geetorusApiUrl": "http://localhost:3100"
      }
    }

    Hermes Gateway network examples:
    - Local loopback API server: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:8642" and agentDefaultsPayload.geetorusApiUrl = "http://127.0.0.1:3100".
    - Local loopback dashboard root or chat URL: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:9119" or "http://127.0.0.1:9119/chat"; Geetorus maps either one to "http://127.0.0.1:9119/api".
    - LAN/private network: use reachable private addresses, for example agentDefaultsPayload.apiBaseUrl = "http://192.168.1.25:8642" and agentDefaultsPayload.geetorusApiUrl = "http://192.168.1.10:3100".
    - Private overlay: use overlay DNS names, for example agentDefaultsPayload.apiBaseUrl = "http://hermes-host.tailnet-name.ts.net:8642" and agentDefaultsPayload.geetorusApiUrl = "http://geetorus-host.tailnet-name.ts.net:3100".
    - Docker: if Geetorus runs in Docker and Hermes runs on the host, use agentDefaultsPayload.apiBaseUrl = "http://host.docker.internal:8642"; if both run in Compose, use the Hermes service name.
    - Reverse proxy/TLS: use HTTPS origins, for example agentDefaultsPayload.apiBaseUrl = "https://hermes-gateway.example" and agentDefaultsPayload.geetorusApiUrl = "https://geetorus.example".
    ...
    The board approves the join request in Geetorus before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set GEETORUS_API_KEY and GEETORUS_API_URL for future Geetorus API calls. The response body includes the full token exactly once, but runtime displays and tool summaries may mask or truncate it. Write the raw response token directly to private storage before logging anything, then verify it with an authenticated Geetorus API call. Do not persist displayed previews containing literal ... or [redacted].
    ...
    ${
      ...
      "Ensure Geetorus is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Geetorus base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.geetorusApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - npx geetorusai allowed-hostname <host>
      - then restart Geetorus and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@geetorus.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "geetorus", path: "/api/skills/geetorus" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "geetorus-create-agent",
          path: "/api/skills/geetorus-create-agent"
        },
        {
          name: "geetorus-converting-plans-to-tasks",
          path: "/api/skills/geetorus-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "geetorus",
          path: `/api/invites/${token}/skills/geetorus`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "geetorus") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              geetorusApiUrl: req.body.geetorusApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.geetorusApiUrl &&
          !persistedDefaults.geetorusApiUrl
        ) {
          missingPersistedFields.push("geetorusApiUrl");
        }
