...
export function cloudRoutes(opts: {
  ...
  router.get("/stacks", async (req, res) => {
    ...
    const tenantToken = runtimeEnv.GEETORUS_CLOUD_TENANT_SERVER_TOKEN?.trim();
    if (!context.cloudOrigin || !context.stackId || !tenantToken) {
      throw new HttpError(503, "Geetorus Cloud portfolio is not configured", {
    ...
    try {
      const upstream = await fetchImpl(portfolioUrl, {
        ...
        headers: {
          ...
          "x-geetorus-cloud-user-id": actorUserId,
          "x-geetorus-cloud-stack-id": context.stackId,
        },
      ...
      if (!upstream.ok) {
        logger.warn(
          ...
          "Geetorus Cloud portfolio request failed",
        );
        throw new HttpError(502, "Geetorus Cloud portfolio request failed", {
      ...
      try {
        ...
        throw new HttpError(502, "Geetorus Cloud portfolio returned invalid JSON", {
