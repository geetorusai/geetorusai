 ...
 * toggle server-side adapter code for the whole Geetorus instance.
 *
 ...
 * (`instance.adapters` in GEETORUS_HIDDEN_SETTINGS), adapter management
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-geetorus-adapter") or local path */
  packageName: string;
...
export function adapterRoutes(options: {
  ...
  router.get("/adapters", async (_req, res) => {
    ...
    if (!nativeRunnerEnabled) disabledSet.add("geetorus_runner");
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-geetorus-adapter@0.3.0" → packageName + version
  ...
  // without modifying Geetorus's source code.
  //
