...
import { activityLog } from "@geetorusai/db";
...
import { trackProjectCreated } from "@geetorusai/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  async function selectedRepositories(req: Request, companyId: string, ids: string[], existing: import("@geetorusai/shared").ProjectWorkspace[] = []) {
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Geetorus can run workspace commands" });
      return;
    }
