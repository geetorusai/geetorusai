...
import { ADAPTER_AUTH_MISSING_CHECK_CODE, AI_CONNECTION_CAPABILITIES, aiConnectionBindingSchema, type AiConnectionBinding } from "@geetorusai/shared";
import { toolConnections } from "@geetorusai/db";
...
import { isAiConnectionCompatible } from "@geetorusai/shared";
...
import { geetorusRunnerTransitionConfig, normalizeLegacyRunnerProvider, isGeetorusRunnerProvider } from "@geetorusai/adapter-utils";
...
import { activityLog, agents as agentsTable, chatConversations, companies, heartbeatRuns, issues as issuesTable, projects as projectsTable } from "@geetorusai/db";
...
import { trackAgentCreated } from "@geetorusai/shared/telemetry";
...
import { ONBOARDING_FIRST_TASK_SKILL_KEY, GEETORUS_CORE_SKILL_KEYS } from "../services/company-skills.js";
...
import { evaluateCodexCredentialReadiness } from "@geetorusai/adapter-codex-local/server";
import type { AdapterAuthSignal, AdapterAuthSignalResponse, CodexAccountBindingClaim } from "@geetorusai/shared";
...
import { runClaudeLogin } from "@geetorusai/adapter-claude-local/server";
...
import { SETUP_TOKEN_TRANSPORT_ADVISORY_CODE } from "@geetorusai/shared";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@geetorusai/adapter-codex-local";
import {
...
import type { AdapterAuthSessionOwnerResponse } from "@geetorusai/shared";
...
import { DEFAULT_OPENCODE_LOCAL_MODEL } from "@geetorusai/adapter-opencode-local";
...
function readRunIssueId(context: Record<string, unknown> | null) {
  ...
  const geetorusIssue = readObject(context?.geetorusIssue);
  const nestedIssueId = geetorusIssue?.id;
...
// the right scope because a Geetorus instance serves its API from one
...
export function agentRoutes(
   ...
   *   adapter probes the Geetorus host (legacy behavior).
  ...
  function buildSandboxIdentityCheck(input: {
    ...
    const detailParts = [
      `geetorusLeaseId=${input.lease.id}`,
   ...
   * declarative registry (GEETORUS_ADAPTERS) curated it out, which
  ...
  async function assertFreshGeetorusRunnerProvider(
    companyId: string,
    adapterType: string,
    adapterConfig: Record<string, unknown>,
  ): Promise<void> {
    if (adapterType !== "geetorus_runner") return;
    let profile;
    try {
  ...
  function resolveGeetorusRunnerAdapterTransition(input: {
    ...
    if (
      input.nextAdapterType !== "geetorus_runner"
      || input.previousAdapterType === input.nextAdapterType
    ) {
      return input.nextAdapterConfig;
    }
    const defaults = geetorusRunnerTransitionConfig(input.previousAdapterType, input.previousAdapterConfig.model, input.nextAdapterConfig.provider);
    if (!["claude_local", "codex_local", "opencode_local"].includes(input.previousAdapterType)
      && !isGeetorusRunnerProvider(input.nextAdapterConfig.provider)) {
      throw unprocessable("Select a Geetorus Runner provider before converting this agent.");
    }
  ...
  async function normalizeMediatedAdapterConfigForPersistence(input: {
    ...
    return normalizeGeetorusRunnerAdapterConfig(
      input.adapterType ?? "",
      normalizedAdapterConfig,
    );
  }
  ...
  function codexLocalAgentHome(companyId: string, agentId: string): string {
    const instanceRoot = resolveGeetorusInstanceRootForAdapter({
      homeDir: asNonEmptyString(process.env.GEETORUS_HOME) ?? undefined,
      instanceId: asNonEmptyString(process.env.GEETORUS_INSTANCE_ID) ?? undefined,
      env: process.env,
    });
  ...
  function applyCreateDefaultsByAdapterType(
    ...
    if (adapterType === "geetorus_runner") {
      return normalizeGeetorusRunnerAdapterConfig(adapterType, next);
    }
  ...
  async function assertAdapterConfigConstraints(
    ...
    if (adapterType === "geetorus_runner") {
      await assertFreshGeetorusRunnerProvider(companyId, adapterType, adapterConfig);
      return;
    }
  ...
  // core geetorus skills (board coordination, planning, hiring, memory).
  ...
  // the Geetorus operational skill as a runtime invariant.
  function defaultRoleSkillSelections(
    ...
    const keys = boardOnboardingFirstAgent
      ? [...GEETORUS_CORE_SKILL_KEYS, ONBOARDING_FIRST_TASK_SKILL_KEY]
      : GEETORUS_CORE_SKILL_KEYS;
    return keys
      .filter((key) => adapterType !== "geetorus_runner" || key !== GEETORUS_OPERATIONAL_SKILL_KEY)
  ...
  async function buildRuntimeSkillConfig(
    ...
    return {
      ...config,
      geetorusRuntimeSkills: runtimeSkillEntries,
    };
  }
  ...
  async function resolveDesiredSkillAssignment(
    ...
    const currentPreference = readGeetorusSkillSyncPreference(adapterConfig);
    ...
    const desiredSkillEntries = mergeDesiredSkillEntries(
      ...
      (entry) => !isConnectorSkill(entry.key) && (adapterType !== "geetorus_runner"
    ...
    return {
      adapterConfig: writeGeetorusSkillSyncPreference(adapterConfig, desiredSkillEntries),
      desiredSkills,
      desiredSkillEntries,
      runtimeSkillEntries,
    };
  ...
  router.get("/companies/:companyId/adapters/:type/models", async (req, res) => {
    ...
    if (type === "geetorus_runner" && provider && !isGeetorusRunnerProvider(provider)) {
      throw unprocessable("Unknown Geetorus Runner provider");
    }
    const modelAdapterType = type === "geetorus_runner"
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (savedAgentId) {
        ...
        const providerAdapter = savedAgent.adapterType === "geetorus_runner"
  ...
  // the Geetorus host, so it is authoritative for the null-environment and
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readGeetorusSkillSyncPreference(
    ...
    const connectorConfig = await applyConnectorSkills(runtimeSkillConfig, runtimeSkillConfig.geetorusRuntimeSkills, connectorAssignments);
    ...
    async (req, res) => {
      ...
      let snapshot = adapter?.syncSkills
        ? await adapter.syncSkills({
          ...
          }, readGeetorusSkillSyncPreference(manualSkillConfig).desiredSkills)
        : adapter?.listSkills
  ...
  router.get("/agents/me/inbox-lite", async (req, res) => {
    ...
    const isWorktreeRuntime = isTruthyRuntimeEnvValue(process.env.GEETORUS_IN_WORKTREE);
  ...
  router.post("/agents/:id/config-revisions/:revisionId/rollback", async (req, res) => {
    ...
    if (
      ...
      rollbackAdapterType === "geetorus_runner"
    ) {
      await assertFreshGeetorusRunnerProvider(
        existing.companyId,
        rollbackAdapterType,
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    ...
    if (touchesAdapterConfiguration) {
      ...
      if (changingAdapterType) {
        ...
        rawEffectiveAdapterConfig = resolveGeetorusRunnerAdapterTransition({
      ...
      if (requestedAdapterType === "geetorus_runner") {
        rawEffectiveAdapterConfig = normalizeGeetorusRunnerAdapterConfig(requestedAdapterType, rawEffectiveAdapterConfig);
      }
      const existingRunnerProvider =
        existing.adapterType === "geetorus_runner"
          ? existingAdapterConfig.provider
          : undefined;
      if (
        changingAdapterType ||
        (requestedAdapterType === "geetorus_runner" &&
   ...
   * Geetorus over plain HTTP on a home server or a Tailscale tailnet. So the
      ...
      try {
        if (rawRequestKind === "runtime") {
          ...
          if (action === "decline" || action === "cancel") {
            ...
            action === "submit" &&
            candidate?.response?.schema === "geetorus.question_response.v1" &&
