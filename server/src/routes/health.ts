...
import { heartbeatRuns, instanceUserRoles, invites } from "@geetorusai/db";
import type { DeploymentExposure, DeploymentMode } from "@geetorusai/shared";
import {
...
function hasDevServerStatusToken(providedToken: string | undefined) {
  return matchesSharedToken(process.env.GEETORUS_DEV_SERVER_STATUS_TOKEN, providedToken);
}
...
function getCloudHealthStatus(env: CloudInstanceEnv) {
  ...
  return {
    ...
    managedBy: "geetorus-cloud" as const,
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-geetorus-dev-server-status-token"));
