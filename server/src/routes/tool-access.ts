...
import { agents, companies, connectionGrants, issueThreadInteractions, toolConnectionInstalls } from "@geetorusai/db";
...
} from "../services/geetorus-cloud-connector.js";
...
} from "../services/geetorus-cloud-connector-enrollment.js";
import { reconcileGeetorusCloudConnectorEnrollmentStatus } from "../services/geetorus-cloud-connector-status.js";
import {
...
export function connectionIntentOAuthOutcomeHtml(input: {
  ...
  const message = JSON.stringify({
    type: "geetorus.connection-intent.oauth",
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Connection authorization</title></head><body><p>Returning to Geetorus…</p><script>const message=${message};const targetOrigin=${targetOrigin}||window.location.origin;if(window.opener&&window.opener!==window){window.opener.postMessage(message,targetOrigin);window.close();}else{window.location.replace(${fallback});}</script></body></html>`;
}
...
function normalizeCloudConnectorEnrollmentReturnTo(returnTo?: string | null): string | null {
  ...
  try {
    const parsed = new URL(returnTo, "http://geetorus.local");
    if (
      parsed.origin !== "http://geetorus.local"
...
export function cloudConnectorEnrollmentOutcomeHtml(issuePrefix: string, returnTo: string, issueId?: string): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Geetorus connected</title></head><body><p>Geetorus is connected. Return to your task to finish connecting the app.</p><script>if(window.opener&&window.opener!==window){window.close();}else{const link=document.createElement("a");link.href=${fallback};link.textContent=${JSON.stringify(issueId ? "Return to task" : "Continue setup")};document.body.append(link);}</script></body></html>`;
}
...
export function cloudConnectorEnrollmentReturnPath(issuePrefix: string, returnTo?: string | null): string {
  ...
  if (normalizedReturnTo) {
    const parsed = new URL(normalizedReturnTo, "http://geetorus.local");
...
export function toolAccessRoutes(
  ...
  function configuredPublicBaseUrl() {
    ...
    const raw = (
      ...
      || process.env.GEETORUS_MANAGED_RUNTIME_PUBLIC_URL?.trim()
    );
    if (!raw) return null;
    try {
...
/**
  ...
  router.post("/agents/me/connections/:connectionId/token", validate(connectionTokenRequestSchema), async (req, res) => {
    ...
    const headerRunId = req.get("X-Geetorus-Run-Id")?.trim();
  ...
  router.get("/companies/:companyId/tools/gallery", async (req, res) => {
    ...
    assertCompanyAccess(req, companyId);
    const advertisedProfiles = options.geetorusCloudConnector === undefined
      ? await geetorusCloudConnectorCapabilitiesFromEnv()
      : options.geetorusCloudConnector
        ? await options.geetorusCloudConnector.getCapabilities()
        : [];
        ...
        vercelConnect: {
          ...
          reason: vercelConnect.enabled
            ? vercelConnect.configured
              ? null
              : "Vercel Connect needs workload OIDC or GEETORUS_VERCEL_CONNECT_ACCESS_TOKEN."
            : "Vercel Connect setup is disabled on this Geetorus instance.",
        },
      },
      apps: APP_STORE_DEFINITIONS.map((app) =>
        appWithGeetorusCloudConnectorAvailability(app, advertisedProfiles)
      ),
    });
  });
  ...
  /**
   * Geetorus's Client ID Metadata Document (PAP-17087).
   *
   * The document's own URL is the `client_id` Geetorus presents to an
   ...
   * Geetorus session. It contains only this deployment's callback and the
   * grant/response/auth methods Geetorus uses: no company, connection or secret
   * data of any kind.
   */
  ...
  router.get("/tools/oauth/cloud-connector/enrollment", async (req, res) => {
    assertBoard(req);
    res.json(await reconcileGeetorusCloudConnectorEnrollmentStatus());
  });
  ...
  router.post("/tools/oauth/cloud-connector/enrollment", async (req, res) => {
    ...
    if (!companyId) throw badRequest("Geetorus Cloud enrollment requires a company");
    assertCompanyAccess(req, companyId);
    ...
    let status;
    try {
      status = await startGeetorusCloudConnectorEnrollment({
        origin,
        companyId,
      ...
      throw unprocessable("Geetorus Cloud enrollment could not be started", {
        code: "geetorus_cloud_connector_enrollment_failed",
      });
    }
    await logActivity(db, {
      companyId,
      actorType: "user",
      actorId: req.actor.userId ?? "board",
      action: "geetorus_cloud_connector.enrollment_started",
  ...
  router.get("/tools/oauth/cloud-connector/enrollment-callback", async (req, res) => {
    ...
    if (!enrollmentId || !approvalCode || !state) throw badRequest("Invalid Geetorus Cloud enrollment callback");
    const pending = loadGeetorusCloudConnectorIdentity()?.pending;
    if (pending?.companyId && !hasCompanyAccess(req, pending.companyId)) {
      throw notFound("Geetorus Cloud enrollment not found");
    }
    ...
    const [company] = pending?.companyId
      ? await db
        ...
        .limit(1)
      : [];
    if (!company) throw notFound("Geetorus Cloud enrollment not found");
    let status;
    try {
      status = await completeGeetorusCloudConnectorEnrollment({ enrollmentId, approvalCode, state });
      invalidateGeetorusCloudConnectorCapabilities();
    } catch {
      throw badRequest("Invalid or expired Geetorus Cloud enrollment callback");
    }
    if (pending?.companyId) {
      await logActivity(db, {
        ...
        actorType: "user",
        actorId: req.actor.userId ?? "board",
        action: "geetorus_cloud_connector.enrollment_completed",
        entityType: "connector_instance",
    ...
    if (returnTo && new URL(returnTo, "http://geetorus.local").searchParams.get("enrollment_host") === "dialog") {
      res.set("Cache-Control", "no-store");
      const intent = new URL(returnTo, "http://geetorus.local").searchParams.get("intent");
  ...
  const handleGeetorusCloudConnectorCallback = async (req: Request, res: Response) => {
    assertBoard(req);
    const state = typeof req.query.state === "string" ? req.query.state : "";
    ...
    try {
      const result = await svc.completeGeetorusCloudConnectorCallback({
        state,
        claimId,
        error,
        actor: getActorInfo(req),
  ...
  router.get("/tools/oauth/cloud-connector/callback", handleGeetorusCloudConnectorCallback);
  router.get("/tools/oauth/geetorus-id/callback", handleGeetorusCloudConnectorCallback);
  ...
  router.get("/tools/oauth/callback", async (req, res) => {
    ...
    // provider's body: they are provider-authored prose, and Geetorus maps the
    ...
    try {
      ...
      await logActivity(db, {
        ...
        details: {
          ...
          // HttpError messages are Geetorus-authored. Provider-authored
