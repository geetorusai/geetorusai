...
import { agents as agentsTable } from "@geetorusai/db";
import type { CompanyPortabilityImportResult } from "@geetorusai/shared";
import {
  MAX_ZIP_ENTRY_DECOMPRESSED_BYTES,
  MAX_ZIP_TOTAL_DECOMPRESSED_BYTES,
  readZipArchive,
} from "@geetorusai/shared/portability-zip";
import {
...
// @geetorusai/shared/company-import-transfer — the browser and CLI clients
...
/**
 * Async job opt-in. Cloud tenants set the `x-geetorus-cloud-async-import`
 ...
 * inbound `x-geetorus-cloud-*` header as anti-spoofing — so they opt in with
 * the proxy-safe `?async=1` query parameter instead. Either signal enters the
 * async path.
 */
function wantsAsyncImport(req: Request) {
  return req.query.async === "1" || req.header("x-geetorus-cloud-async-import") === "1";
}
...
export function companyRoutes(db: Db, storage?: StorageService, options?: CompanyRoutesOptions) {
   ...
   *   `GEETORUS_HIDDEN_SETTINGS` → `settings_operator_managed`): hiding the
  ...
  router.post("/", (req, _res, next) => {
    assertBoard(req);
    if (isCloudManagedInstance()) {
      throw forbidden("Company creation is managed by Geetorus Cloud", {
        code: "cloud_managed",
      });
    }
    next();
...
function cloudTenantRequestKey(req: Request) {
  return [
    req.actor.userId ?? "",
    req.header("x-geetorus-cloud-stack-id")?.trim() ?? "",
    req.header("x-geetorus-cloud-geetorus-company-id")?.trim() ?? "",
  ].join(":");
}
