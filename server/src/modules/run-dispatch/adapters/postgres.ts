...
import { ISSUE_DISPOSITION_REPAIR_RETRY_REASON } from "@geetorusai/shared";
...
export function createPostgresRunDispatchAdapter(
  ...
  async function loadStalenessFacts(
    ...
    if (continuationParkApplies) {
      const queuedWake = parseObject(context.geetorusWake);
      const queuedContinuationSummary =
        readNonEmptyString(parseObject(context.geetorusContinuationSummary).body) ??
