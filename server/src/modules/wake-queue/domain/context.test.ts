...
describe("enrichPromotedWakeContext", () => {
  it("clears stale derived comment projections when the canonical id list is empty, but keeps an independent interaction continuation", () => {
    const contextSnapshot: Record<string, unknown> = {
      ...
      geetorusWake: { commentId: "comment-2" },
      geetorusWakeComment: { id: "comment-2", body: "stale body" },
      geetorusTaskMarkdown: "stale markdown",
      geetorusTaskMarkdownCompact: "stale compact markdown",
    ...
    expect(result.contextSnapshot.geetorusWake).toBeUndefined();
    expect(result.contextSnapshot.geetorusWakeComment).toBeUndefined();
    expect(result.contextSnapshot.geetorusTaskMarkdown).toBeUndefined();
    expect(result.contextSnapshot.geetorusTaskMarkdownCompact).toBeUndefined();
  ...
  it("sets the canonical id and latest-id fields when the recomputed list has entries, and still clears the stale projections", () => {
    const contextSnapshot: Record<string, unknown> = {
      ...
      geetorusWake: { commentId: "comment-1" },
      geetorusWakeComment: { id: "comment-1", body: "will be rebuilt" },
      geetorusTaskMarkdown: "will be rebuilt",
      geetorusTaskMarkdownCompact: "will be rebuilt",
    };
  ...
  it("adds a new id from the payload to the canonical list and clears the stale projections", () => {
    const contextSnapshot: Record<string, unknown> = {
      wakeCommentIds: ["comment-1"],
      geetorusWakeComment: { id: "comment-1", body: "stale body" },
    };
