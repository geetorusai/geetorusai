...
import { activityLog, agentWakeupRequests, agents, companies, createDb, heartbeatRuns, issueComments, issues } from "@geetorusai/db";
import {
...
describeEmbeddedPostgres("queued-comment postgres adapter", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-queued-comment-postgres-adapter-");
  ...
  async function seedDeferredWake(input: {
    ...
    await db.insert(agentWakeupRequests).values({
      ...
      payload: {
        issueId: input.issueId,
        _geetorusWakeContext: { wakeCommentIds: input.commentIds },
      },
    });
    return id;
  }
  ...
  it("answers the steering question for a deferred geetorus_runner queue whose active run has no persisted runtime mode yet", async () => {
    const companyId = await seedCompany();
    const agentId = await seedAgent({ companyId, adapterType: "geetorus_runner" });
    const issueId = await seedIssue({ companyId, assigneeAgentId: agentId });
    const commentId = await seedComment({ companyId, issueId, authorUserId: "user-1" });
    const wakeId = await seedDeferredWake({ companyId, agentId, issueId, commentIds: [commentId] });
    ...
    expect(queue.protocol).toBe("geetorus_runner_v1");
