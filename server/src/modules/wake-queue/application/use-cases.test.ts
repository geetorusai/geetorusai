...
describe("releaseIssueExecution", () => {
  ...
  it("carries the deferred wake's raw issue, interaction, execution-stage, and accepted-plan context onto the promoted run, and clears only the rendered text projections", async () => {
      ...
      findNextDeferredWake: vi.fn(async () =>
        wakeCandidate({
          deferredContextSeed: {
            ...
            geetorusTaskMarkdown: "queue-time markdown",
            geetorusTaskMarkdownCompact: "queue-time compact markdown",
    ...
    // the run dispatches.
    expect(promotedContextSnapshot.geetorusTaskMarkdown).toBeUndefined();
    expect(promotedContextSnapshot.geetorusTaskMarkdownCompact).toBeUndefined();
    expect(promotedContextSnapshot.geetorusWake).toBeUndefined();
    ...
    async (status) => {
      ...
      expect(writer.insertNewDeferredWake).toHaveBeenCalledExactlyOnceWith(
        ...
        expect.objectContaining({
          ...
          payload: {
            issueId: "issue-1",
            _geetorusWakeContext: contextSnapshot,
          },
        }),
      );
    },
