...
import { resolveGeetorusConfigPath, resolveGeetorusEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@geetorusai/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.GEETORUS_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.GEETORUS_AGENT_JWT_SECRET === "string" ? parsed.GEETORUS_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `npx geetorusai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveGeetorusEnvPath();
