const FAVICON_BLOCK_START = "<!-- GEETORUS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- GEETORUS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- GEETORUS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- GEETORUS_RUNTIME_BRANDING_END -->";
...
function createFaviconDataUrl(background: string, foreground: string): string {
  const svg = [
    ...
    `<circle cx="12" cy="12" r="7.5" stroke="${foreground}" stroke-width="3.4"/>`,
    "</svg>",
  ].join("");
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.GEETORUS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.GEETORUS_WORKTREE_NAME) ?? nonEmpty(env.GEETORUS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.GEETORUS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
  ...
  return {
    ...
    instanceId: nonEmpty(env.GEETORUS_INSTANCE_ID),
  };
}
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  const tags = [
    '<meta name="geetorus-worktree-enabled" content="true" />',
    `<meta name="geetorus-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="geetorus-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="geetorus-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ];
  if (branding.instanceId) {
    tags.push(`<meta name="geetorus-instance-id" content="${escapeHtmlAttribute(branding.instanceId)}" />`);
  }
  return tags.join("\n");
}
