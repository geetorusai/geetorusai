...
export function resolveServerDevWatchIgnorePaths(serverRoot: string): string[] {
  ...
  const isLinkedWorktree =
    ...
    path.basename(path.dirname(linkedWorktreesRoot)) === ".geetorus";
  ...
  for (const relativePath of [
    ...
    "../ui/.vite",
    "../ui/dist",
    // Git worktrees live under <repo>/.geetorus/worktrees, each a full
    // checkout (source + its own .geetorus). Watching them can add hundreds
    ...
    // <repo>/.geetorus/worktrees/<branch>/server. In that case, the shared
    // worktree directory is the checkout's parent, not a nested path.
    isLinkedWorktree ? "../.." : "../.geetorus/worktrees",
    ...
    process.env.HOME + "/.geetorus/adapter-plugins",
  ]) {
