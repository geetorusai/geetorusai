...
import { normalizeAgentApiKeyScope, type AgentApiKeyScope } from "@geetorusai/shared";
...
function jwtConfig() {
  const secret = process.env.GEETORUS_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  ...
  return {
    ...
    ttlSeconds: parseNumber(process.env.GEETORUS_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.GEETORUS_AGENT_JWT_ISSUER ?? "geetorus",
    audience: process.env.GEETORUS_AGENT_JWT_AUDIENCE ?? "geetorus-api",
    ...
    // name) even though it deliberately shares GEETORUS_AGENT_JWT_SECRET with
    ...
    instanceId: resolveGeetorusInstanceId(),
    disableLegacyFallback: parseBooleanEnv(process.env.GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK),
  };
}
 ...
 * have expired (set GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true). Normal
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  //
  // Operators should set `GEETORUS_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true`
  // approximately one JWT TTL (~48h by default, see GEETORUS_AGENT_JWT_TTL_SECONDS)
