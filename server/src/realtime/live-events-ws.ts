...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@geetorusai/db";
import type { DeploymentMode } from "@geetorusai/shared";
...
/** Cloud-proxied browser identity resolved from trusted x-geetorus-cloud-* headers. */
...
interface IncomingMessageWithContext extends IncomingMessage {
  geetorusWebSocketHandled?: boolean;
  geetorusUpgradeContext?: UpgradeContext;
}
...
async function authorizeUpgrade(
  ...
  if (!token) {
    ...
    // x-geetorus-cloud-* headers, never a local Better Auth session — the
...
export function setupLiveEventsWebSocketServer(
  ...
  opts: {
     ...
     * x-geetorus-cloud-* headers on the upgrade request. Wired by managed
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).geetorusUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    if ((req as IncomingMessageWithContext).geetorusWebSocketHandled) {
      return;
    }
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.geetorusUpgradeContext = context;
