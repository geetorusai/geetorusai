...
import { createHostClientHandlers } from "@geetorusai/plugin-sdk";
...
export function resolveViteHmrPort(serverPort: number): number {
  return deriveGeetorusViteHmrPort(serverPort);
}
...
export function resolveViteHmrProtocol(
  ...
  throw new Error("GEETORUS_VITE_HMR_PROTOCOL must be ws or wss");
}
...
export async function createApp(
  ...
  opts: {
    ...
    /**
     * `plugins.autoInstall` from the managed config (GEETORUS_MANAGED_CONFIG).
  ...
  // before Geetorus persists or acts on any event.
  ...
  if (opts.uiMode === "static") {
    ...
    if (uiDist) {
      ...
      console.warn("[geetorus] UI dist not found; running in API-only mode");
    }
    if (process.env.GEETORUS_MANAGED_RUNTIME_EXPOSURE === "tailscale_https") {
  ...
  if (opts.uiMode === "vite-dev") {
    ...
    const hmrProtocol = resolveViteHmrProtocol(
      process.env.GEETORUS_VITE_HMR_PROTOCOL,
    );
    const hmrServer = createHttpServer((_req, res) => {
      res.writeHead(426, { "Content-Type": "text/plain" });
      res.end("Upgrade Required");
    ...
    const configuredViteCacheDir = process.env.GEETORUS_VITE_CACHE_DIR?.trim();
  ...
  //
  // Managed instances (`plugins.autoInstall` from GEETORUS_MANAGED_CONFIG)
  ...
  app.locals.geetorusShutdown = shutdownAppServices;
