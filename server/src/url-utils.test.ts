...
describe("rewriteUrlPort", () => {
  ...
  it("leaves URLs without an explicit port stable", () => {
    expect(rewriteUrlPort("https://geetorus.example", 3101)).toBe("https://geetorus.example");
...
describe("rewriteLoopbackUrlPort", () => {
  ...
  it("leaves explicit external base URLs untouched (BRO-1558)", () => {
    ...
    // listen port produced an unreachable URL that leaked to agents as a dead GEETORUS_API_URL.
    const serve = "https://erics-mac-studio-1.tailc54c7.ts.net:8443";
    expect(rewriteLoopbackUrlPort(serve, 3101)).toBe(serve);
  });

  it("leaves URLs without an explicit port stable", () => {
    expect(rewriteLoopbackUrlPort("https://geetorus.example.com", 3101)).toBe(
      "https://geetorus.example.com",
    );
...
describe("isLoopbackHost", () => {
  ...
  it("rejects external hosts", () => {
    ...
    expect(isLoopbackHost("geetorus.example.com")).toBe(false);
  });
});
