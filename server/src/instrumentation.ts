 ...
 */
export function getStartupTracer(name = "geetorus.startup"): StartupTracerHandle {
  try {
    ...
    if (!tracerApiLoadFailed) {
      ...
      console.warn(
        "[geetorus] @opentelemetry/api is not available; startup tracing uses a no-op tracer.",
        err,
      );
    }
    return NOOP_TRACER;
 ...
 */
export function getStartupTraceContext(name = "geetorus.startup"): StartupTraceContextHandle {
  try {
    const require = createRequire(import.meta.url);
    const api = require("@opentelemetry/api") as {
      trace?: {
    ...
    if (!traceContextApiLoadFailed) {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "[geetorus] @opentelemetry/api is not available; startup tracing uses a no-op trace context.",
        err,
      );
    }
    return NOOP_TRACE_CONTEXT;
...
export function recordProviderPluginSpan(input: {
  ...
  try {
    ...
    const tracer = trace.getTracer("geetorus.startup");
...
export function shutdownInstrumentation(): Promise<void> {
  shutdownPromise ??= (async () => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[geetorus] OpenTelemetry shutdown failed", err);
    }
  })();
...
export function resolveProtocol(): {
  ...
  switch (raw) {
    ...
    default:
      // eslint-disable-next-line no-console
      console.warn(
        `[geetorus] Unknown OTEL_EXPORTER_OTLP_PROTOCOL=${raw}; falling back to grpc. ` +
 ...
 * `dist` in a built server; both sit inside the Geetorus checkout. A server
 ...
 * is a Geetorus-specific variable, not an OpenTelemetry SDK variable, so
 * Geetorus controls this precedence.
 */
...
async function bootstrapOtel(endpoint: string): Promise<void> {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(`[geetorus] OpenTelemetry service.version=${serviceVersion}`);

    const sdk = new NodeSDK({
      resource: resourceFromAttributes({
        [ATTR_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || "geetorus",
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        "[geetorus] OpenTelemetry SDK failed to start; continuing without tracing",
        err,
      );
      return;
    }
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[geetorus] OTEL_EXPORTER_OTLP_ENDPOINT is set and the @opentelemetry/* " +
