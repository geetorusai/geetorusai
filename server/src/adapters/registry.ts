...
import type { AdapterLoginCapability } from "@geetorusai/adapter-utils";
import { runAdapterExecutionTargetShellCommand } from "@geetorusai/adapter-utils/execution-target";
import {
...
} from "@geetorusai/adapter-claude-local";
import {
...
} from "@geetorusai/adapter-cursor-local/server";
import {
  agentConfigurationDoc as cursorAgentConfigurationDoc,
  models as cursorModels,
} from "@geetorusai/adapter-cursor-local";
import {
...
} from "@geetorusai/adapter-cursor-cloud/server";
import { agentConfigurationDoc as cursorCloudAgentConfigurationDoc } from "@geetorusai/adapter-cursor-cloud";
import {
...
} from "@geetorusai/adapter-gemini-local/server";
import {
  agentConfigurationDoc as geminiAgentConfigurationDoc,
  models as geminiModels,
} from "@geetorusai/adapter-gemini-local";
import {
...
} from "@geetorusai/adapter-grok-local";
import {
...
} from "@geetorusai/adapter-kimi-local";
import {
  createHermesGatewayServerAdapter,
  createHermesLocalServerAdapter,
} from "@geetorusai/hermes-geetorus-adapter";
import {
  ...
  listOpenCodeModels,
} from "@geetorusai/adapter-opencode-local/server";
import {
...
} from "@geetorusai/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@geetorusai/adapter-openclaw-gateway";
...
Geetorus keeps this tombstone registered so stale acpx_local rows fail clearly instead of falling back to the process adapter.
`;
...
const geetorusRunnerAdapter: ServerAdapterModule = {
  type: "geetorus_runner",
  runtimeToolDelivery: "environment",
  async execute(ctx) {
    const message = "geetorus_runner requires the native runner coordinator";
    ...
    return {
      ...
      errorCode: "geetorus_runner_coordinator_required",
  ...
  async testEnvironment(context) {
    let profile: ReturnType<typeof resolveGeetorusRunnerProviderProfile>;
    try {
      profile = resolveGeetorusRunnerProviderProfile(context.config);
    } catch (error) {
      const profileError = error instanceof GeetorusRunnerProviderProfileError
        ? error
        : new GeetorusRunnerProviderProfileError(
            "geetorus_runner_provider_unsupported",
            "Geetorus Runner provider configuration is invalid.",
          );
      return {
    ...
    if (profile.provider === "acpx") {
      try {
        ...
        if (target?.kind === "remote") {
          ...
          return {
            adapterType: "geetorus_runner", status: "warn" as const, testedAt: new Date().toISOString(),
        ...
        const { probeAcpxClaudeInstallation } = await import("@geetorusai/geetorus-runner/live");
        await probeAcpxClaudeInstallation(profile.model);
        return {
          adapterType: "geetorus_runner", status: "pass" as const, testedAt: new Date().toISOString(),
        ...
        return {
          adapterType: "geetorus_runner", status: "fail" as const, testedAt: new Date().toISOString(),
    ...
    return { ...result, adapterType: "geetorus_runner" };
  },
  listSkills: listCodexSkills,
  syncSkills: syncCodexSkills,
  sessionCodec: codexSessionCodec,
  ...
  getRuntimeCommandSpec: (config) => config.provider === "claude_managed"
    ...
    ? { command: "geetorus-runnerd", detectCommand: null, installCommand: null }
  ...
  agentConfigurationDoc:
    "# Geetorus Runner\n\nAdapter: geetorus_runner\n\nRuns Codex, OpenCode, Claude Managed, AWS AgentCore, or ACPX Claude through the Rust Geetorus runner and authenticated PRP transport. Pi is not available through the qualified ACPX profile. Managed providers use company-scoped qualified profiles, explicit retention acknowledgement, and spend limits.\n",
  getConfigSchema: () => ({
    fields: [
      {
        key: "provider",
      ...
      },
      {
        ...
        type: "select" as const,
        default: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.codex.defaultMode,
        options: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.codex.options.map(
          ({ value, label }) => ({ value, label }),
        ),
        hint: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.codex.description,
      ...
      },
      {
        ...
        type: "select" as const,
        default: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.opencode.defaultMode,
        options: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.opencode.options.map(
          ({ value, label }) => ({ value, label }),
        ),
        hint: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.opencode.description,
      ...
      },
      {
        ...
        type: "select" as const,
        default: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.acpx.defaultMode,
        options: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.acpx.options.map(
          ({ value, label }) => ({ value, label }),
        ),
        hint: GEETORUS_RUNNER_PERMISSION_CAPABILITIES.acpx.description,
      ...
      },
      {
        ...
        type: "number" as const,
        default: 1,
        hint: "Geetorus estimate; AWS does not provide a per-session currency hard stop.",
        meta: { visibleWhen: { key: "provider", value: "aws_agentcore" } },
      },
      {
        key: "maxIterations",
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[geetorus] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[geetorus] Failed to load external adapters:", err);
  }
})();
...
/**
 * Memoized view of GEETORUS_ADAPTER_MODELS, keyed by the raw env string so
...
function getDeclaredAdapterModels(): ReturnType<typeof parseAdapterModelsEnv> {
  const raw = process.env.GEETORUS_ADAPTER_MODELS;
  ...
  try {
    ...
    console.error(
      "[geetorus] Invalid GEETORUS_ADAPTER_MODELS; ignoring declared model lists:",
      err,
    );
  }
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[geetorus] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[geetorus] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
