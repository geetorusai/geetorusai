...
import { validateAdapterLoginCapability } from "@geetorusai/adapter-utils";
...
function extractUiParserSource(
  ...
  const contractVersion = pkg.geetorus?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no geetorus.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
