...
import { assertValidAdapterLoginCapability } from "@geetorusai/adapter-utils";
...
vi.mock("@geetorusai/geetorus-runner/live", () => ({ probeAcpxClaudeInstallation: probeInstallation }));
...
describe("native ACPX environment checks", () => {
  ...
  it("reports unsupported local platforms before a successful CLI login can mask them", async () => {
    ...
    const result = await requireServerAdapter("geetorus_runner").testEnvironment!(context);
  ...
  it.each([
    ...
    const result = await requireServerAdapter("geetorus_runner").testEnvironment!({ ...context, executionTarget: sshTarget });
