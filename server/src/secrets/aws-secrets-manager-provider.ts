...
import type { DeploymentMode, SecretProviderConfigDiscoveryPreviewResult } from "@geetorusai/shared";
...
const DEFAULT_PREFIX = "geetorus";
const DEFAULT_OWNER_TAG = "geetorus";
const DEFAULT_VERSION_STAGE = "AWSCURRENT";
const GEETORUS_PENDING_VERSION_STAGE = "GEETORUS_PENDING";
...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Geetorus server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Geetorus company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function readProviderVaultConfig(input: SecretProviderVaultRuntimeConfig): AwsSecretsManagerConfig {
  ...
  const recoveryWindowRaw = process.env.GEETORUS_SECRETS_AWS_DELETE_RECOVERY_DAYS?.trim();
  ...
  if (!Number.isFinite(recoveryWindow) || recoveryWindow < 7 || recoveryWindow > 30) {
    throw unprocessable(
      "GEETORUS_SECRETS_AWS_DELETE_RECOVERY_DAYS must be an integer between 7 and 30",
    );
  }
  ...
  return {
    region,
    endpoint:
      process.env.GEETORUS_SECRETS_AWS_ENDPOINT?.trim() ||
...
function getAwsConfigReadiness() {
  const region = (
    process.env.GEETORUS_SECRETS_AWS_REGION ??
    process.env.AWS_REGION ??
    process.env.AWS_DEFAULT_REGION
  )?.trim();
  const deploymentId = process.env.GEETORUS_SECRETS_AWS_DEPLOYMENT_ID?.trim();
  const kmsKeyId = process.env.GEETORUS_SECRETS_AWS_KMS_KEY_ID?.trim();
  const missingConfig: string[] = [];

  if (!region) {
    missingConfig.push("GEETORUS_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!deploymentId) {
    missingConfig.push("GEETORUS_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!kmsKeyId) {
    missingConfig.push("GEETORUS_SECRETS_AWS_KMS_KEY_ID");
  }
...
function loadAwsSecretsManagerConfig(): AwsSecretsManagerConfig {
  ...
  const region =
    process.env.GEETORUS_SECRETS_AWS_REGION?.trim() ||
  ...
  if (!region) {
    throw unprocessable(
      "AWS Secrets Manager provider requires GEETORUS_SECRETS_AWS_REGION or AWS_REGION",
    );
  }
  if (!deploymentId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires GEETORUS_SECRETS_AWS_DEPLOYMENT_ID",
    );
  }
  if (!kmsKeyId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires GEETORUS_SECRETS_AWS_KMS_KEY_ID",
    );
  }
  ...
  return {
    ...
    endpoint:
      ...
      `https://secretsmanager.${region}.amazonaws.com`,
    deploymentId,
    prefix: sanitizePathSegment(process.env.GEETORUS_SECRETS_AWS_PREFIX?.trim() || DEFAULT_PREFIX),
    kmsKeyId,
    environmentTag:
      process.env.GEETORUS_SECRETS_AWS_ENVIRONMENT?.trim() ||
      process.env.NODE_ENV?.trim() ||
      "unknown",
    providerOwnerTag:
      process.env.GEETORUS_SECRETS_AWS_PROVIDER_OWNER?.trim() || DEFAULT_OWNER_TAG,
    deleteRecoveryWindowDays: recoveryWindow,
  };
}
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Geetorus-managed namespace secrets cannot be imported as external references",
  );
}
...
function buildManagedSecretTags(
  ...
  if (!context) return [];
  return [
    { Key: "geetorus:managed-by", Value: "geetorus" },
    { Key: "geetorus:provider-owner", Value: config.providerOwnerTag },
    { Key: "geetorus:deployment-id", Value: config.deploymentId },
    { Key: "geetorus:company-id", Value: context.companyId },
    { Key: "geetorus:secret-key", Value: context.secretKey },
    { Key: "geetorus:environment", Value: config.environmentTag },
  ];
}
...
function inferPathSignals(entry: AwsSecretsManagerListSecretEntry, tags: Map<string, string>) {
  ...
  const geetorusDeploymentId = tagValue(tags, ["geetorus:deployment-id"]);
  const geetorusManaged = tagValue(tags, ["geetorus:managed-by"])?.toLowerCase() === "geetorus";

  if (geetorusDeploymentId || geetorusManaged) {
    return {
      prefix: segments[0] ?? DEFAULT_PREFIX,
      namespace: geetorusDeploymentId ?? segments[1] ?? null,
    };
  }
...
function discoverAwsProviderConfigCandidates(input: {
  ...
  type DiscoverySample = {
    ...
    ownerTag: string | null;
    kmsKeyId: string | null;
    geetorusManaged: boolean;
    geetorusCompanyId: string | null;
  };

  const skippedWarnings: string[] = [];
  let skippedForeignGeetorusSampleCount = 0;
  ...
  for (const entry of input.entries) {
    ...
    const geetorusManaged = tagValue(tags, ["geetorus:managed-by"])?.toLowerCase() === "geetorus";
    const geetorusCompanyId = tagValue(tags, ["geetorus:company-id"]);
    if (geetorusManaged && geetorusCompanyId !== input.companyId) {
      skippedForeignGeetorusSampleCount += 1;
      continue;
    }
    ...
    samples.push({
      ...
      environmentTag: tagValue(tags, ["geetorus:environment", "environment", "env", "stage"]),
      ownerTag: tagValue(tags, ["geetorus:provider-owner", "owner", "team", "service", "application"]),
  ...
  if (skippedForeignGeetorusSampleCount > 0) {
    skippedWarnings.push(
      `Skipped ${skippedForeignGeetorusSampleCount} Geetorus-managed AWS secret sample(s) that were not tagged for this company.`,
    );
  }
  ...
  const candidates = [...groups.values()]
    ...
    .map((group) => {
      ...
      if (group.some((sample) => sample.geetorusManaged && sample.geetorusCompanyId === input.companyId)) {
        candidateWarnings.push("Sample includes Geetorus-managed secrets for this company; do not import them as external references.");
      }
      ...
      return {
        ...
        signals: {
          ...
          sampleCount: group.length,
          geetorusManagedSampleCount: group.filter((sample) => sample.geetorusManaged).length,
          skippedForeignGeetorusSampleCount,
        },
...
export function createAwsSecretsManagerProvider(
  ...
  async function validateConfig(
    ...
    if (!config.prefix) {
      warnings.push("GEETORUS_SECRETS_AWS_PREFIX should be set to a deployment-scoped prefix");
    }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Geetorus metadata separately from AWS-managed secrets.",
          "Restoring access requires the Geetorus database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
      ...
      return {
        ...
        details: {
          ...
          requiredProviderConfig: input?.providerConfig
            ? ["region"]
            : [
                "GEETORUS_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION",
                "GEETORUS_SECRETS_AWS_DEPLOYMENT_ID",
                "GEETORUS_SECRETS_AWS_KMS_KEY_ID",
              ],
          optionalProviderConfig: [
            "GEETORUS_SECRETS_AWS_PREFIX",
            "GEETORUS_SECRETS_AWS_ENVIRONMENT",
            "GEETORUS_SECRETS_AWS_PROVIDER_OWNER",
            "GEETORUS_SECRETS_AWS_ENDPOINT",
            "GEETORUS_SECRETS_AWS_DELETE_RECOVERY_DAYS",
          ],
          credentialSource: "AWS SDK default credential provider chain",
          detectedCredentialSources: readiness.credentialSources,
        },
  ...
  return {
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Geetorus secret ${input.context.secretName}` : undefined,
    ...
    async createVersion(input) {
      ...
      try {
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
          VersionStages: [GEETORUS_PENDING_VERSION_STAGE],
        });
        const normalizedSecretId = created.ARN ?? created.Name ?? secretId;
        return {
          material: createManagedMaterial(normalizedSecretId, created.VersionId ?? null),
    ...
    async updateExternalSecretValue(input) {
      ...
      try {
        ...
        // referenced secret (not just Geetorus) picks up the new value.
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
        });
      ...
      try {
        if (input.mode === "archive") {
          if (material.versionId && gateway.updateSecretVersionStage) {
            await gateway.updateSecretVersionStage({
              SecretId: secretId,
              VersionStage: GEETORUS_PENDING_VERSION_STAGE,
