...
function unavailableProvider(
  ...
  return {
    ...
    async createSecret() {
      throw unprocessable(`${id} provider is not configured for Geetorus-managed values`);
    },
    async createVersion() {
      throw unprocessable(`${id} provider is not configured for Geetorus-managed values`);
    },
    ...
    async deleteOrArchive() {
      // External references are metadata-only in Geetorus for unconfigured providers.
    },
    async healthCheck() {
      return {
        provider: id,
