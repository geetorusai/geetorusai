 ...
 *
 * In supervised managed-cloud deployments (`GEETORUS_CLOUD_API_ORIGIN`
...
export function migrationRefusalError(
  ...
  // `GEETORUS_CLOUD_API_ORIGIN` marks the deployment as supervised
...
export function shouldReportStartupFailure(
  ...
  const cloudOrigin = env.GEETORUS_CLOUD_API_ORIGIN?.trim();
  return !cloudOrigin;
}
