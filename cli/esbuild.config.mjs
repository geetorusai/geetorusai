/**
 * esbuild configuration for building the geetorusai CLI for npm.
 *
 * Bundles all workspace packages (@geetorusai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @geetorusai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@geetorusai/") && !bundledCliNpmDependencies.has(name)) {
      externals.add(name);
    }
  }
