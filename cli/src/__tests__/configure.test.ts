...
function writeBaseConfig(configPath: string) {
  const base: GeetorusConfig = {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/geetorus-storage" },
      s3: {
        bucket: "geetorus",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/geetorus-secrets/master.key" },
    },
  };
...
describe("configure command", () => {
  it("sets a failing exit code for unknown sections", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-configure-"));
  ...
  it("sets a failing exit code when no config exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-configure-missing-"));
  ...
  it("backs up invalid config bytes and refuses non-interactive replacement", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-configure-invalid-"));
    const configPath = path.join(root, "config.json");
