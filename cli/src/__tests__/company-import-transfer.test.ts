...
import { readZipArchive } from "@geetorusai/shared/portability-zip";
import {
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "geetorus-company-import-transfer-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("resolveChunkedImportZip", () => {
  ...
  it("takes the chunked path for a small zip whose entries inflate past the threshold", async () => {
    ...
    const zipBytes = buildDeflateZip("dense-package/NOTES.md", "geetorus agent docs\n".repeat(3_200_000));
  ...
  it("names the company the completed transfer created", async () => {
    const { api, putRaw } = fakeApi({
      post: vi.fn().mockResolvedValue({
        ...
        totalParts: 2,
        missingParts: [],
        company: { id: "company-2", name: "Geetorus", issuePrefix: "PAPA" },
      }),
    });

    await expect(uploadCompanyImportTransfer(api, zipBytes)).rejects.toThrow(
      /landed in the company "Geetorus" \(PAPA\)/,
    );
    expect(putRaw).not.toHaveBeenCalled();
  });
});
...
describe("company import command over the chunked transfer path", () => {
  ...
  it("slices an oversized local zip into a transfer and applies it against the spool", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "http://geetorus.test/api/companies/import/transfers",
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://geetorus.test/api/companies/import/transfers/transfer-1/parts/0",
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      3,
      "http://geetorus.test/api/companies/import/transfers/transfer-1/parts/1",
      expect.objectContaining({ method: "PUT" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      4,
      "http://geetorus.test/api/companies/import/transfers/transfer-1/preview",
      expect.objectContaining({ method: "POST" }),
    );
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      5,
      "http://geetorus.test/api/companies/import/transfers/transfer-1/apply",
      expect.objectContaining({ method: "POST" }),
    );
  ...
  it("keeps small local zips on the inline single-shot path", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://geetorus.test/api/companies/import/preview",
      expect.objectContaining({ method: "POST" }),
    );
