...
describe("env-lab cleanup command hint", () => {
  ...
  it("never restores the unsafe pnpm invocation forms", () => {
    ...
    // The bare `pnpm geetorusai` script form is unsafe. The `pnpm exec` form
    // does not resolve the CLI binary. Keep both out of the hint.
    expect(command).not.toContain("pnpm geetorusai");
    expect(command).not.toContain("pnpm exec geetorusai");
  });
...
describe("env-lab doctor cleanup hint instance", () => {
  ...
  afterEach(() => {
    if (originalInstanceId === undefined) {
      ...
      process.env.GEETORUS_INSTANCE_ID = originalInstanceId;
    }
    vi.restoreAllMocks();
  });
  ...
  it("pins the GEETORUS_INSTANCE_ID instance when opts.instance is absent", async () => {
    // The doctor diagnoses the instance that `GEETORUS_INSTANCE_ID` selects.
    // The cleanup hint must target that instance, not the default instance.
    process.env.GEETORUS_INSTANCE_ID = "env-selected-instance";
  ...
  it("pins the explicit instance over GEETORUS_INSTANCE_ID", async () => {
