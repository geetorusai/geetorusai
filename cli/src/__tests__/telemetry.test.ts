...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.GEETORUS_HOME = path.join(root, "home");
    process.env.GEETORUS_INSTANCE_ID = "telemetry-test";
