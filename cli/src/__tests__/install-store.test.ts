...
describe("managed install store", () => {
  ...
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-install-store-"));
    paths = resolveInstallStorePaths({
      homeDir: path.join(root, "home"),
      geetorusHome: path.join(root, "home", ".geetorus"),
    });
  });
  ...
  it("writes a stable shim with the validated runtime and custom store path", () => {
    ...
    expect(shim).not.toContain("GEETORUS_HOME");
    ...
    expect(fs.readFileSync(rcPath, "utf8")).not.toContain("geetorusai managed PATH");
  ...
  it("reports managed provenance only for the payload selected by current", () => {
    ...
    const executable = path.join(manifestPayload, "node_modules", "geetorusai", "dist", "index.js");
