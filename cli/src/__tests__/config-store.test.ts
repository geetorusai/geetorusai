...
import { geetorusConfigSchema, type GeetorusConfig } from "../config/schema.js";
...
function createConfigPath(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-config-store-"));
  roots.push(root);
  return path.join(root, "config.json");
}

function defaultConfig(): GeetorusConfig {
  return geetorusConfigSchema.parse({
    $meta: {
      version: 1,
