...
import { resolveGeetorusInstanceId } from "../config/home.js";
import {
...
describe("managed install commands", () => {
  ...
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-install-command-"));
    process.env = {
      ...ORIGINAL_ENV,
      HOME: path.join(root, "home"),
      GEETORUS_HOME: path.join(root, "home", ".geetorus"),
      PATH: "/usr/bin:/bin",
      SHELL: "/bin/bash",
    };
  ...
  it("resolves branch, tag, full SHA, and short SHA refs through GitHub", async () => {
    ...
    for (const ref of ["master", "v1.2.3", sha, sha.slice(0, 12)]) {
      await expect(resolveGitHubRef("geetorusai/geetorus", ref, runCommand)).resolves.toBe(sha);
    }
    expect(runCommand.mock.calls.map((call) => call[1].at(-1))).toEqual([
      "https://api.github.com/repos/geetorusai/geetorus/commits/master",
      "https://api.github.com/repos/geetorusai/geetorus/commits/v1.2.3",
      `https://api.github.com/repos/geetorusai/geetorus/commits/${sha}`,
      `https://api.github.com/repos/geetorusai/geetorus/commits/${sha.slice(0, 12)}`,
    ]);
  });

  it("supports fork overrides and classifies SHA refs as pinned", () => {
    expect(resolveGitInstallRequest({ ref: "feature/test", repo: "HenkDz/geetorus" })).toEqual({ repo: "HenkDz/geetorus", ref: "feature/test", pinned: false });
    expect(resolveGitInstallRequest({ ref: "abcdef1" })).toEqual({ repo: "geetorusai/geetorus", ref: "abcdef1", pinned: true });
    expect(() => resolveGitInstallRequest({ repo: "HenkDz/geetorus" })).toThrow("requires --ref");
  });

  it("requires explicit non-interactive consent before resolving git refs", async () => {
    const runCommand = vi.fn();

    await expect(installCommand({ ref: "master", repo: "HenkDz/geetorus" }, { runCommand }))
  ...
  it("reuses a SHA-keyed git payload without downloading or rebuilding", async () => {
    ...
    const packageRoot = path.join(payloadPath, "node_modules", "geetorusai");
    ...
    await expect(installGitPayload("geetorusai/geetorus", sha, runCommand, paths)).resolves.toEqual({ payloadPath, reused: true, version: "0.3.1" });
  ...
  const createGitCheckoutRunCommand = (sha: string) =>
    vi.fn(async (file: string, args: string[], _options?: Parameters<CommandRunner>[2]) => {
      ...
      if (file === "tar") {
        ...
        const packages = [
          { dir: "packages/shared", name: "@geetorusai/shared", packageJson: { name: "@geetorusai/shared", version: "0.3.1" } },
          { dir: "packages/db", name: "@geetorusai/db", packageJson: { name: "@geetorusai/db", version: "0.3.1", dependencies: { "@geetorusai/shared": "workspace:*" }, bundleDependencies: ["embedded-postgres"] } },
          { dir: "server", name: "@geetorusai/server", packageJson: { name: "@geetorusai/server", version: "0.3.1", dependencies: { "@geetorusai/db": "workspace:*" } } },
        ];
      ...
      if (file === "corepack") {
        if (args.includes("pack")) {
          ...
          const packageName = packageDir === "server" ? "geetorusai-server" : "geetorusai-shared";
      ...
      if (file === "npm" && args[0] === "pack") {
        const packageName = args[1]?.includes("workspace-package-") ? "geetorusai-db" : "geetorusai";
        fs.writeFileSync(path.join(args[args.indexOf("--pack-destination") + 1], `${packageName}-0.3.1.tgz`), "package");
        return { stdout: "", stderr: "" };
      }
      if (file === "npm" && args[0] === "install") { const prefix = args[args.indexOf("--prefix") + 1]; const packageRoot = path.join(prefix, "node_modules", "geetorusai"); fs.mkdirSync(path.join(packageRoot, "dist"), { recursive: true }); fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ version: "0.3.1" })); fs.writeFileSync(path.join(packageRoot, "dist", "index.js"), "#!/usr/bin/env node\n"); return { stdout: "", stderr: "" }; }
      if (file === process.execPath && args[0]?.endsWith("prepare-bundled-package.mjs")) {
        fs.mkdirSync(args[2], { recursive: true });
        fs.writeFileSync(path.join(args[2], "package.json"), JSON.stringify({ name: "@geetorusai/db", version: "0.3.1" }));
        return { stdout: "", stderr: "" };
      }
  ...
  it("installs a GitHub branch through codeload and reuses the resolved SHA", async () => {
    ...
    await installCommand({ ref: "master", repo: "HenkDz/geetorus", yes: true }, { runCommand });
    await installCommand({ ref: "master", repo: "HenkDz/geetorus", yes: true }, { runCommand });
    const manifest = readInstallManifest(resolveInstallStorePaths());
    expect(manifest).toMatchObject({ source: "git", repo: "HenkDz/geetorus", ref: "master", sha });
  ...
  it("builds git checkouts with NODE_ENV cleared so ambient production mode keeps devDependencies", async () => {
    ...
    const runCommand = createGitCheckoutRunCommand(sha);
    await expect(installGitPayload("geetorusai/geetorus", sha, runCommand, resolveInstallStorePaths())).resolves.toMatchObject({ version: "0.3.1", reused: false });
    ...
    const uiPackCall = buildCalls.find(([file, , options]) => file === "corepack" && options?.env?.GEETORUS_RELEASE_REUSE_UI_DIST === "1");
  ...
  it("resolves the complete server workspace dependency closure in dependency order", () => {
    ...
    const packages = [
      { dir: "packages/shared", name: "@geetorusai/shared", dependencies: {} },
      { dir: "packages/db", name: "@geetorusai/db", dependencies: { "@geetorusai/shared": "workspace:*" } },
      { dir: "server", name: "@geetorusai/server", dependencies: { "@geetorusai/db": "workspace:*" } },
    ];
    fs.mkdirSync(path.join(checkout, "scripts"), { recursive: true });
    fs.writeFileSync(path.join(checkout, "scripts", "release-package-manifest.json"), JSON.stringify(packages.map(({ dir, name }) => ({ dir, name }))));
    for (const workspacePackage of packages) {
  ...
  it("installs through the shim, reports provenance, and uninstalls without deleting user data", async () => {
    ...
    const runCommand = vi.fn(async (file: string, args: string[], _options?: unknown) => {
      ...
      if (file === "npm" && args[0] === "install") {
        ...
        const entrypoint = path.join(prefix, "node_modules", "geetorusai", "dist", "index.js");
    ...
    expect(installCall?.[1]).toContain("--@geetorusai:registry=https://registry.npmjs.org");
    ...
    const entrypoint = path.join(manifest!.payloadPath, "node_modules", "geetorusai", "dist", "index.js");
    expect(resolveCliVersion(entrypoint)).toContain(`managed npm latest; payload ${manifest!.payloadPath}`);

    const userData = path.join(process.env.GEETORUS_HOME!, "instances", "default", "keep.txt");
  ...
  it("preserves the managed install when an existing systemd unit cannot be checked", async () => {
    ...
    const unitPath = path.join(
      ...
      "systemd",
      "user",
      systemdServiceName(resolveGeetorusInstanceId()),
    );
  ...
  it("refuses a symlinked git payload root before downloading", async () => {
    ...
    await expect(installGitPayload("geetorusai/geetorus", "4".repeat(40), runCommand, paths)).rejects.toThrow("unsafe payload root");
    expect(runCommand).not.toHaveBeenCalled();
  });
