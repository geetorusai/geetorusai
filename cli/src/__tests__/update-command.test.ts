...
function createPayload(payloadPath: string, version: string): string {
  const entrypoint = path.join(payloadPath, "node_modules", "geetorusai", "dist", "index.js");
  fs.mkdirSync(path.dirname(entrypoint), { recursive: true });
  fs.writeFileSync(entrypoint, version);
  return entrypoint;
}
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-update-"));
...
afterEach(() => {
  ...
  if (previousGeetorusHome === undefined) delete process.env.GEETORUS_HOME; else process.env.GEETORUS_HOME = previousGeetorusHome;
...
describe("update command", () => {
  it.each(["npm", "git", "global-npm"] as const)("rejects %s updates on unsupported Node before any update work", async (source) => {
    ...
    try {
      await expect(updateCommand({ yes: true }, {
        paths,
        executablePath: source === "global-npm" ? path.join(root, "lib", "node_modules", "geetorusai", "dist", "index.js") : entrypoint,
        runCommand, backup, restartActiveService,
      })).rejects.toThrow("npx geetorusai@latest install --yes");
  ...
  it("detects managed, global npm, npx, and source modes", () => {
    ...
    expect(detectInstallMode(path.join(root, "lib", "node_modules", "geetorusai", "dist", "index.js"), paths)).toBe("global-npm");
    expect(detectInstallMode(path.join(root, ".npm", "_npx", "abc", "node_modules", "geetorusai", "dist", "index.js"), paths)).toBe("npx");
  ...
  it("re-resolves a moving git branch and activates the new SHA payload", async () => {
    ...
    fs.writeFileSync(path.join(oldPayload, "node_modules", "geetorusai", "package.json"), JSON.stringify({ version: "0.3.1" }));
    const newPayload = payloadPathFor(paths, "git", newSha.slice(0, 12));
    createPayload(newPayload, "0.3.1");
    fs.writeFileSync(path.join(newPayload, "node_modules", "geetorusai", "package.json"), JSON.stringify({ version: "0.3.1" }));
    flipCurrentAtomic(oldPayload, paths);
    writeInstallManifestAtomic({ schemaVersion: 1, source: "git", version: "0.3.1", channel: "pinned", repo: "geetorusai/geetorus", ref: "master", sha: oldSha, payloadPath: oldPayload, installedAt: "2026-07-22T00:00:00.000Z", previous: [] }, paths);
  ...
  it("reports SHA git installs as pinned without resolving again", async () => {
    ...
    flipCurrentAtomic(payload, paths);
    writeInstallManifestAtomic({ schemaVersion: 1, source: "git", version: "0.3.1", channel: "pinned", repo: "geetorusai/geetorus", ref: sha.slice(0, 12), sha, payloadPath: payload, installedAt: "2026-07-22T00:00:00.000Z", previous: [] }, paths);
  ...
  it("requires explicit confirmation before a global npm downgrade", async () => {
    ...
    const executable = path.join(root, "lib", "node_modules", "geetorusai", "dist", "index.js");
  ...
  it("isolates global npm updates from hostile registry configuration", async () => {
    ...
    const runCommand = vi.fn<CommandRunner>(async (_file, args, commandOptions) => {
      ...
      expect(args).toContain("--@geetorusai:registry=https://registry.npmjs.org");
  ...
  it("explains how to recover when the pre-update database is unreachable", async () => {
    ...
    await expect(updateCommand({}, { paths, executablePath: executable, runCommand, backup, hasInstanceData: () => true })).rejects.toThrow(
      "Start the service with `geetorusai service start` and retry, or skip the backup with `geetorusai update --no-backup`.",
    );
    expect(backup).toHaveBeenCalledOnce();
    expect(readInstallManifest(paths)?.version).toBe("1.0.0");
  });
  ...
  it("does not inherit a managed pin for global npm updates", async () => {
    ...
    expect(runCommand).toHaveBeenCalledWith("npm", expect.arrayContaining(["view", "geetorusai@latest"]), expect.anything());
  });
