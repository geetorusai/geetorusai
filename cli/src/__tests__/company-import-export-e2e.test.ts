...
function createBaseGeetorusEnv(options: TestGeetorusEnv) {
  ...
  env.GEETORUS_CONFIG = options.configPath;
  env.GEETORUS_HOME = options.geetorusHome;
  env.GEETORUS_INSTANCE_ID = options.instanceId;
  env.GEETORUS_CONTEXT = path.join(options.geetorusHome, "context.json");
  env.GEETORUS_AUTH_STORE = path.join(options.geetorusHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestGeetorusEnv, "configPath">,
) {
  const env = createBaseGeetorusEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.GEETORUS_DB_BACKUP_ENABLED = "false";
  env.GEETORUS_DECISION_SIGNING_SECRET = "company-import-export-decision-signing-secret";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.GEETORUS_MIGRATION_AUTO_APPLY = "true";
  env.GEETORUS_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestGeetorusEnv) {
  const env = createBaseGeetorusEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.GEETORUS_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.GEETORUS_MIGRATION_AUTO_APPLY;
  delete env.GEETORUS_UI_DEV_MIDDLEWARE;
  return env;
}
...
function isPortableAgent(agent: { metadata?: Record<string, unknown> | null }) {
  const marker = agent.metadata?.geetorusBuiltInAgent;
  return typeof marker !== "object" || marker === null;
}

async function runCliJson<T>(
  args: string[],
  opts: TestGeetorusEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "geetorusai", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `geetorusai run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("geetorusai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let geetorusHome = "";
  let cliShellHome = "";
  let geetorusInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "geetorus-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    geetorusHome = path.join(tempRoot, "geetorus-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    geetorusInstanceId = "company-cli-e2e";
    mkdirSync(geetorusHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("geetorus-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["geetorusai", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          geetorusHome,
          instanceId: geetorusInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(geetorusHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".geetorus", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".geetorus.yaml"), "utf8")).toContain('schema: "geetorus/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "geetorus-demo"));
