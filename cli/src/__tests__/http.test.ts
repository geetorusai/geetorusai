...
import { ApiConnectionError, ApiRequestError, GeetorusApiClient } from "../client/http.js";

describe("GeetorusApiClient", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new GeetorusApiClient({
    ...
    expect(headers["x-geetorus-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new GeetorusApiClient({ apiBase: "http://localhost:3100" });
