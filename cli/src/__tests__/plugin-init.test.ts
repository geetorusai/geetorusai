...
vi.mock("../../../packages/plugins/create-geetorus-plugin/src/index.js", async () => {
  const actual =
    await vi.importActual<typeof import("../../../packages/plugins/create-geetorus-plugin/src/index.js")>(
      "../../../packages/plugins/create-geetorus-plugin/src/index.js",
    );
  return {
    ...actual,
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-cli-plugin-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin init", () => {
  ...
  it("maps package name and flags to scaffolder options", () => {
    const cwd = path.resolve("/tmp/geetorus-cli-test");
    const options = buildPluginInitScaffoldOptions(
      "@acme/plugin-linear",
      {
        ...
        author: "Acme",
        sdkPath: "../geetorus/packages/plugins/sdk",
      },
      cwd,
    );
  ...
  it("builds exact next commands using the scaffold path", () => {
    expect(buildPluginInitNextCommands("/tmp/acme plugin")).toEqual([
      ...
      "pnpm install",
      "pnpm dev",
      "geetorusai plugin install '/tmp/acme plugin'",
    ]);
  });
  ...
  it("registers the CLI wrapper and invokes the existing scaffolder", async () => {
    ...
    expect(mocks.scaffoldPluginProject).toHaveBeenCalledWith({
      ...
      outputDir: path.resolve("/tmp/geetorus-init-output", "demo-plugin"),
      template: "workspace",
      category: "workspace",
...
describe("plugin target diagnostics", () => {
  ...
  it("marks the target unreachable when the health probe throws", async () => {
    const get = vi.fn(async () => {
      throw new Error("Could not reach the Geetorus API.\nRequest: GET ...");
    });
    ...
    expect(diag.error).toContain("Could not reach the Geetorus API.");
  });
  ...
  it("formats unreachable diagnostics with a remediation hint", () => {
    ...
    expect(rendered).toContain("GEETORUS_API_URL");
  });
});
