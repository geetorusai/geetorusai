...
function catalogSkill(overrides: Record<string, unknown> = {}) {
  return {
    id: "geetorusai:bundled:software-development:github-pr-workflow",
    key: "geetorusai/bundled/software-development/github-pr-workflow",
    kind: "bundled",
...
describe("skills CLI helpers", () => {
  it("resolves skill refs by id, key, or unique normalized slug", () => {
    const rows = [
      skill({ id: "skill-a", key: "geetorus/a", slug: "alpha", name: "Alpha" }),
      skill({ id: "skill-b", key: "geetorus/b", slug: "beta-skill", name: "Beta" }),
    ];

    expect(resolveCompanySkillReference(rows, "skill-a").key).toBe("geetorus/a");
    expect(resolveCompanySkillReference(rows, "geetorus/b").id).toBe("skill-b");
    expect(resolveCompanySkillReference(rows, "Beta Skill").id).toBe("skill-b");
  });

  it("rejects ambiguous slug refs", () => {
    const rows = [
      skill({ id: "skill-a", key: "geetorus/a", slug: "same", name: "A" }),
      skill({ id: "skill-b", key: "geetorus/b", slug: "same", name: "B" }),
    ];
...
describe("skills CLI commands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.GEETORUS_COMPANY_ID;
    fetchMock = vi.fn();
  ...
  it("lists company skills as JSON through the shared client context", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://geetorus.test/api/companies/company-1/skills",
  ...
  it("resolves a skill slug before reading detail", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://geetorus.test/api/companies/company-1/skills/11111111-1111-1111-1111-111111111111",
  ...
  it("browses catalog skills with filters in table output", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://geetorus.test/api/skills/catalog?kind=bundled&category=software-development&q=github",
      expect.objectContaining({ method: "GET" }),
    );
    const rendered = logSpy.mock.calls.map((call: unknown[]) => String(call[0])).join("\n");
    expect(rendered).toContain("id");
    expect(rendered).toContain("geetorusai:bundled:software-development:github-pr-workflow");
  ...
  it("searches catalog skills as JSON", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://geetorus.test/api/skills/catalog?kind=bundled&q=pull+requests",
      expect.objectContaining({ method: "GET" }),
    );
    expect(JSON.parse(String(logSpy.mock.calls[0]?.[0]))).toEqual(rows);
  });
  ...
  it("inspects catalog skill detail by query ref so keys with slashes work", async () => {
    ...
    await runCommand([
      "skills",
      "inspect",
      "geetorusai/bundled/software-development/github-pr-workflow",
      "--api-base",
      "http://geetorus.test",
      "--api-key",
      "token",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://geetorus.test/api/skills/catalog/ref?ref=geetorusai%2Fbundled%2Fsoftware-development%2Fgithub-pr-workflow",
      expect.objectContaining({ method: "GET" }),
    );
  ...
  it("installs catalog skills into the company library without agent sync", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://geetorus.test/api/companies/company-1/skills/install-catalog",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
  ...
  it("passes force to skill updates", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://geetorus.test/api/companies/company-1/skills/11111111-1111-1111-1111-111111111111/install-update",
      expect.objectContaining({
        method: "POST",
  ...
  it("audits installed skill bytes through the server", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://geetorus.test/api/companies/company-1/skills/11111111-1111-1111-1111-111111111111/audit",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({}),
      }),
  ...
  it("requires confirmation for reset and sends force when confirmed", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://geetorus.test/api/companies/company-1/skills/11111111-1111-1111-1111-111111111111/reset",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ force: true }),
      }),
  ...
  it("syncs desired company skill refs to an agent and returns the runtime snapshot", async () => {
    const snapshot = {
      ...
      supported: true,
      mode: "persistent",
      desiredSkills: ["geetorus/review-prs"],
      entries: [
        {
          key: "geetorus/review-prs",
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "http://geetorus.test/api/agents/coder?companyId=company-1",
      expect.objectContaining({ method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://geetorus.test/api/agents/agent-1/skills/sync",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ desiredSkills: ["review-prs", "geetorus/qa"], mode: "add" }),
      }),
    );
