...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.GEETORUS_AUTH_STORE;
    delete process.env.GEETORUS_SERVER_PORT;
  });
  ...
  it("resolves api base by explicit, env, profile, then config/default precedence", () => {
    ...
    process.env.GEETORUS_API_URL = "http://env:3/";
  ...
  it("prefers explicit and env tokens over profile env and stored board auth", () => {
    ...
    process.env.GEETORUS_AUTH_STORE = authStorePath;
    ...
    process.env.GEETORUS_API_KEY = "env-token";
