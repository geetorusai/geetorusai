...
let previousGeetorusHome: string | undefined;
let previousServiceManaged: string | undefined;

beforeEach(() => {
  previousGeetorusHome = process.env.GEETORUS_HOME;
  previousServiceManaged = process.env.GEETORUS_SERVICE_MANAGED;
  process.env.GEETORUS_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-service-restart-"));
});

afterEach(() => {
  if (previousGeetorusHome === undefined) delete process.env.GEETORUS_HOME;
  else process.env.GEETORUS_HOME = previousGeetorusHome;
  if (previousServiceManaged === undefined) delete process.env.GEETORUS_SERVICE_MANAGED;
  else process.env.GEETORUS_SERVICE_MANAGED = previousServiceManaged;
});

function managerFixture(active = true) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-service-doctor-"));
  const definitionPath = path.join(root, "geetorusai.service");
  fs.writeFileSync(definitionPath, "unit");
  return {
    platform: "systemd" as const,
    instanceId: "default",
    serviceName: "geetorusai.service",
    definitionPath,
...
describe("service health doctor checks", () => {
  ...
  it("reclaims restart locks left by terminated processes", async () => {
    const lockPath = path.join(process.env.GEETORUS_HOME!, "instances", "default", "hot-restart.lock");
  ...
  it("detects a foreground process on the configured port while the service is inactive", async () => {
    ...
    expect(results).toContainEqual(
      expect.objectContaining({
        ...
        status: "fail",
        message: expect.stringContaining("another Geetorus process"),
      }),
    );
  });
});
...
describe("service runtime shim awareness", () => {
  function inactiveManager() {
    return {
      ...
      instanceId: "default",
      serviceName: "ing.geetorus.geetorusai",
  ...
  it("blames the missing binary, not a port conflict, when the shim is gone", async () => {
    ...
    expect(runtime?.repairHint).toContain("geetorusai install");
  });

  it("diagnoses against the executable recorded in the definition, not the current env", async () => {
    const manager = inactiveManager();
    manager.installedExecutablePath = vi.fn(async () => "/custom/bin/geetorusai");
    ...
    const runtime = results.find((r) => r.name === "Service runtime");
    expect(shimPresent).toHaveBeenCalledWith("/custom/bin/geetorusai");
    expect(runtime?.message).toContain("/custom/bin/geetorusai");
    expect(runtime?.repairHint).toContain("/custom/bin/geetorusai");
    expect(runtime?.repairHint).toContain("unset GEETORUS_SHIM_PATH");
    expect(runtime?.repairHint).toContain("`geetorusai install` followed by `geetorusai service install`");
  });
  ...
  it("attributes a healthy foreign responder instead of reporting Healthy", async () => {
    ...
    expect(healthResult?.message).toContain("but not from ing.geetorus.geetorusai");
    const runtime = results.find((r) => r.name === "Service runtime");
    expect(runtime?.message).toContain("serving another Geetorus process");
  });
});

describe("definition executable extraction", () => {
  it("round-trips through both renderers", () => {
    const unit = renderSystemdUnit({ instanceId: "default", shimPath: "/custom/bin/geetorusai", homeDir: "/home/x/.geetorus" });
    expect(extractExecutableFromSystemdUnit(unit)).toBe("/custom/bin/geetorusai");
    const plist = renderLaunchdPlist({ instanceId: "default", shimPath: "/custom/bin/geetorusai", homeDir: "/home/x/.geetorus", stdoutPath: "/tmp/o.log", stderrPath: "/tmp/e.log" });
    expect(extractExecutableFromLaunchdPlist(plist)).toBe("/custom/bin/geetorusai");
  ...
  it("round-trips paths the renderers escape", () => {
    const hostile = '/tmp/we"ird $pa%th & <x>/geetorusai';
    const unit = renderSystemdUnit({ instanceId: "default", shimPath: hostile, homeDir: "/home/x/.geetorus" });
    expect(extractExecutableFromSystemdUnit(unit)).toBe(hostile);
    const plist = renderLaunchdPlist({ instanceId: "default", shimPath: hostile, homeDir: "/home/x/.geetorus", stdoutPath: "/tmp/o.log", stderrPath: "/tmp/e.log" });
