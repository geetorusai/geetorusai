...
afterEach(async () => {
  ...
  delete process.env.GEETORUS_SERVICE_MANAGED;
});

async function temporaryDirectory(): Promise<string> {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "geetorus-service-test-"));
...
describe("service definition generation", () => {
  it("generates a stable systemd notify unit without secrets", () => {
    const unit = renderSystemdUnit({ instanceId: "team-a", shimPath: "/home/alice/.local/bin/geetorusai", homeDir: "/home/alice/.geetorus" });
    expect(unit).toContain("Type=notify");
    expect(unit).toContain("NotifyAccess=all");
    expect(unit).toContain('ExecStart="/home/alice/.local/bin/geetorusai" run --instance "team-a"');
  ...
  it("escapes systemd variable and specifier expansion in configured values", () => {
    const unit = renderSystemdUnit({
      ...
      shimPath: "/home/$USER/%i/geetorusai",
      homeDir: "/home/$USER/%i/.geetorus",
    });

    expect(unit).toContain('ExecStart="/home/$$USER/%%i/geetorusai" run --instance "team-$$USER-%%i"');
    expect(unit).toContain('Environment="GEETORUS_HOME=/home/$$USER/%%i/.geetorus"');
  });

  it.each([
    ["instanceId", { instanceId: "team-a\nExecStartPre=/tmp/attack", shimPath: "/home/alice/.local/bin/geetorusai", homeDir: "/home/alice/.geetorus" }],
    ["shimPath", { instanceId: "team-a", shimPath: "/home/alice/bin/geetorusai\r\nExecStartPre=/tmp/attack", homeDir: "/home/alice/.geetorus" }],
    ["homeDir", { instanceId: "team-a", shimPath: "/home/alice/.local/bin/geetorusai", homeDir: "/home/alice/.geetorus\nEnvironment=ATTACK=1" }],
  ...
  it("generates a launchd agent with keepalive and instance logs", () => {
    const plist = renderLaunchdPlist({ instanceId: "team-a", shimPath: "/Users/alice/.local/bin/geetorusai", homeDir: "/Users/alice/.geetorus", stdoutPath: "/Users/alice/.geetorus/instances/team-a/logs/service.log", stderrPath: "/Users/alice/.geetorus/instances/team-a/logs/service.err.log" });
    expect(plist).toContain("ing.geetorus.geetorusai.team-a");
...
describe("systemd drift regeneration", () => {
  it("rewrites a drifted unit and reloads the user manager", async () => {
    ...
    const manager = new SystemdServiceManager("default", runner, path.join(userHome, ".geetorus"), path.join(userHome, ".local/bin/geetorusai"), userHome);
...
describe("service adapter dispatch", () => {
  ...
  it("returns a foreground-run skip on unsupported hosts", async () => {
    ...
    const detection = await detectServiceManager({ platform: "linux", instanceId: "default", runner });
    expect(detection).toEqual({ supported: false, reason: expect.stringContaining("geetorusai run") });
  });
});
...
describe("launchd lifecycle", () => {
  it("starts without changing the saved login preference", async () => {
    ...
    const runner: CommandRunner = async (command, args) => {
      calls.push([command, ...args].join(" "));
      if (args[0] === "print-disabled") return { stdout: `\"ing.geetorus.geetorusai.team-a\" => true`, stderr: "" };
      return { stdout: "", stderr: "" };
    };
    const manager = new LaunchdServiceManager("team-a", runner, path.join(userHome, ".geetorus"), path.join(userHome, ".local/bin/geetorusai"), userHome);

    await manager.start();

    expect(calls).toContain(`launchctl disable gui/${process.getuid?.() ?? 0}/ing.geetorus.geetorusai.team-a`);
    expect(calls).not.toContain(`launchctl enable gui/${process.getuid?.() ?? 0}/ing.geetorus.geetorusai.team-a`);
  });

  it("preserves disabled state when the service name contains regex metacharacters", async () => {
    const userHome = await temporaryDirectory();
    const calls: string[] = [];
    const serviceName = "ing.geetorus.geetorusai.team[qa]+";
    const runner: CommandRunner = async (command, args) => {
      calls.push([command, ...args].join(" "));
      if (args[0] === "print-disabled") return { stdout: `"${serviceName}" => true`, stderr: "" };
      return { stdout: "", stderr: "" };
    };
    const manager = new LaunchdServiceManager("team[qa]+", runner, path.join(userHome, ".geetorus"), path.join(userHome, ".local/bin/geetorusai"), userHome);
  ...
  it("disables login startup and unloads the keepalive job when stopped", async () => {
    ...
    expect(calls).toContain(`launchctl disable gui/${process.getuid?.() ?? 0}/ing.geetorus.geetorusai.team-a`);
    expect(calls).toContain(`launchctl bootout gui/${process.getuid?.() ?? 0}/ing.geetorus.geetorusai.team-a`);
...
describe("single-writer guard", () => {
  const activeManager = { status: async () => ({ active: true, serviceName: "geetorusai.service" }) } as unknown as ServiceManager;
  ...
  it("allows the supervisor-owned process", async () => {
    process.env.GEETORUS_SERVICE_MANAGED = "1";
  ...
  it("refuses to replace a symlinked service definition", async () => {
    const userHome = await temporaryDirectory();
    const manager = new SystemdServiceManager("default", async () => ({ stdout: "", stderr: "" }), path.join(userHome, ".geetorus"), path.join(userHome, ".local/bin/geetorusai"), userHome);
    await fs.mkdir(path.dirname(manager.definitionPath), { recursive: true });
