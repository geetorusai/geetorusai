...
const dbFixture: DatabaseConfig = {
  mode: "postgres",
  connectionString: "postgres://user:pass@localhost:5432/geetorus",
  embeddedPostgresDataDir: "/var/lib/geetorus/db",
  embeddedPostgresPort: 54329,
  backup: {
    enabled: false,
    intervalMinutes: 30,
    retentionDays: 7,
    dir: "/var/lib/geetorus/backups",
  },
};
...
describe("promptDatabase accepts defaults", () => {
  ...
  it("still requires a connection string when no saved default exists", async () => {
    ...
    expect(connection("postgres://user:pass@localhost:5432/geetorus")).toBeUndefined();
...
describe("promptServer accepts defaults", () => {
  ...
  it("accepts the saved public base URL default when reconfiguring a public deployment", async () => {
    ...
    const { auth } = await promptServer({
      ...
      currentAuth: { publicBaseUrl: "https://geetorus.example.com" },
    });

    expect(auth.publicBaseUrl).toBe("https://geetorus.example.com");
  });
  ...
  it("still requires a public base URL when no saved default exists", async () => {
    ...
    expect(url("https://geetorus.example.com")).toBeUndefined();
    expect(url("ftp://geetorus.example.com")).toBeTruthy();
...
describe("promptStorage accepts defaults", () => {
  ...
  it("accepts the default bucket and region in the s3 flow", async () => {
    ...
    expect(storage.s3.bucket).toBe("geetorus");
