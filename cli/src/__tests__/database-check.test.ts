...
const ORIGINAL_IN_WORKTREE = process.env.GEETORUS_IN_WORKTREE;

function makeBase(): string {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-dbcheck-"));
  created.push(base);
  return base;
}

function embeddedConfig(dataDir: string): GeetorusConfig {
  return {
    database: {
  ...
  } as unknown as GeetorusConfig;
}

afterEach(() => {
  vi.restoreAllMocks();
  if (ORIGINAL_IN_WORKTREE === undefined) delete process.env.GEETORUS_IN_WORKTREE;
  else process.env.GEETORUS_IN_WORKTREE = ORIGINAL_IN_WORKTREE;
...
describe("databaseCheck — embedded postgres temp-dir guard", () => {
  ...
  it("warns when a worktree-mode data dir lives inside the OS temp directory", async () => {
    ...
    expect(result.repairHint).toContain("GEETORUS_HOME");
