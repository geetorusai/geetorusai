...
describe("test-drive data isolation", () => {
  ...
  it("resolves an explicit reusable directory without resetting it", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-test-drive-explicit-"));
  ...
  it("discards inherited Geetorus routing while preserving a custom key source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-test-drive-env-"));
    cleanupDirectories.push(root);
    process.env.GEETORUS_HOME = "/normal/home";
    process.env.GEETORUS_CONFIG = "/normal/config.json";
    process.env.GEETORUS_IN_WORKTREE = "true";
    process.env.GEETORUS_TEST_PROVIDER_KEY = "secret-value";
    process.env.DATABASE_URL = "postgres://normal-instance";

    const prepared = await prepareTestDriveEnvironment(
      { dataDir: root, apiKeyEnv: "GEETORUS_TEST_PROVIDER_KEY" },
      os.tmpdir(),
    );

    expect(prepared.dataDir).toBe(path.resolve(root));
    expect(prepared.linkedWorktree).toBe(false);
    expect(process.env.GEETORUS_HOME).toBe(path.resolve(root));
    expect(process.env.GEETORUS_CONFIG).toBe(
      path.join(path.resolve(root), "instances", "default", "config.json"),
    );
    expect(process.env.GEETORUS_IN_WORKTREE).toBe("false");
    expect(process.env.GEETORUS_DISABLE_CWD_ENV_FILE).toBe("true");
    expect(process.env.GEETORUS_DEPLOYMENT_MODE).toBe("local_trusted");
    expect(process.env.GEETORUS_DEPLOYMENT_EXPOSURE).toBe("private");
    expect(process.env.GEETORUS_BIND).toBe("loopback");
    expect(process.env.HOST).toBe("127.0.0.1");
    expect(process.env.GEETORUS_TEST_PROVIDER_KEY).toBe("secret-value");
...
describe("test-drive bootstrap validation", () => {
  ...
  it("removes literal keys from the JavaScript argv view", () => {
    const splitArgv = ["node", "geetorusai", "test-drive", "--api-key", "literal-secret"];
    const joinedArgv = ["node", "geetorusai", "test-drive", "--api-key=literal-secret"];

    redactTestDriveArgv("literal-secret", splitArgv);
    redactTestDriveArgv("literal-secret", joinedArgv);

    expect(splitArgv).toEqual(["node", "geetorusai", "test-drive", "--api-key", "[REDACTED]"]);
    expect(joinedArgv).toEqual(["node", "geetorusai", "test-drive", "--api-key=[REDACTED]"]);
  });
});
...
describe("test-drive foreground lifecycle", () => {
  ...
  it("skips service-manager integration for an auto-created directory and opens after initialization", async () => {
    process.env.GEETORUS_HOME = "/tmp/test-drive-lifecycle";
    process.env.GEETORUS_INSTANCE_ID = "default";
    process.env.GEETORUS_IN_WORKTREE = "false";
  ...
  it("retains the managed-instance collision guard for an explicitly reused directory", async () => {
    process.env.GEETORUS_HOME = "/tmp/test-drive-reused";
    process.env.GEETORUS_INSTANCE_ID = "default";
    process.env.GEETORUS_IN_WORKTREE = "false";
    let runOptions: RunOptions | undefined;
    const api = {
  ...
  it("uses the credential snapshot captured before downstream server initialization", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-test-drive-credential-"));
    cleanupDirectories.push(root);
  ...
  it("redacts a literal key from downstream errors", async () => {
    process.env.GEETORUS_HOME = "/tmp/test-drive-redaction";
    process.env.GEETORUS_INSTANCE_ID = "default";
    process.env.GEETORUS_IN_WORKTREE = "false";
  ...
  it("redacts a custom environment key when its option name has whitespace", async () => {
    process.env.GEETORUS_HOME = "/tmp/test-drive-custom-env-redaction";
    process.env.GEETORUS_INSTANCE_ID = "default";
    process.env.GEETORUS_IN_WORKTREE = "false";
  ...
  it("honors --no-browser after successful initialization", async () => {
    process.env.GEETORUS_HOME = "/tmp/test-drive-no-browser";
    process.env.GEETORUS_INSTANCE_ID = "default";
    process.env.GEETORUS_IN_WORKTREE = "false";
    const api = {
      get: vi.fn(async <T>() => [company("existing", "Existing")] as T),
