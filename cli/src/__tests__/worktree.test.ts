...
async function seedValidWorktreeSource(
  ...
  await db.insert(authUsers).values({
    id: userId,
    email: userId === "local-board" ? "local@geetorus.local" : "existing@geetorus.ing",
  ...
  if (options.includeCredentialAccount !== false) {
    await db.insert(authAccounts).values({
      ...
      accountId: "existing@geetorus.ing",
...
function buildSourceConfig(): GeetorusConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  it("uses the repo-local config for the current worktree", () => {
    const targetRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-current-worktree-"));
    try {
      const localConfig = path.join(targetRoot, ".geetorus", "config.json");
      fs.mkdirSync(path.dirname(localConfig), { recursive: true });
      fs.writeFileSync(localConfig, "{}\n");
      process.env.GEETORUS_CONFIG = "/tmp/ambient-geetorus/config.json";
  ...
  it("uses the repository config from a nested working directory", () => {
    const targetRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-current-worktree-nested-"));
    try {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("geetorus-pr-432")).toBe(
      path.resolve(os.homedir(), "geetorus-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("geetorus/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://geetorus.example", 3110)).toBe("https://geetorus.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/geetorus-feature",
      homeDir: "/tmp/geetorus-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/geetorus-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/geetorus-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.GEETORUS_HOME).toBe(path.resolve("/tmp/geetorus-worktrees"));
    expect(env.GEETORUS_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.GEETORUS_IN_WORKTREE).toBe("true");
    expect(env.GEETORUS_DB_BACKUP_ENABLED).toBe("false");
    expect(env.GEETORUS_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.GEETORUS_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export GEETORUS_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("requires the seed process to own the target embedded Postgres lifecycle", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-live-target-"));
    try {
      fs.writeFileSync(
  ...
  it("rejects a source migration journal that diverges from the code journal", () => {
    expect(() => resolveWorktreeSeedMigrationRevision({
      ...
      journalEntryCount: 3,
    }, "sourcePrefix")).toThrow("Migration journal is not a prefix of this Geetorus checkout");
  });
  ...
  itEmbeddedPostgres("recognizes positive legacy database schema evidence", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("geetorus-worktree-legacy-evidence-");
    onTestFinished(() => tempDb.cleanup());
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-legacy-config-"));
    try {
      const configPath = path.join(tempRoot, "config.json");
      const sourceConfig = buildSourceConfig();
      const config: GeetorusConfig = {
        ...sourceConfig,
        database: {
  ...
  it("ensure-seeded seeds once and fast-exits on the verified manifest", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-ensure-seeded-"));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
      ...
      fs.writeFileSync(path.join(path.dirname(sourceConfigPath), ".env"), "GEETORUS_INSTANCE_ID=source\n");
      fs.writeFileSync(targetConfigPath, `${JSON.stringify(targetConfig)}\n`);
      fs.writeFileSync(
        path.join(targetRoot, ".geetorus", ".env"),
        `GEETORUS_HOME=${targetPaths.homeDir}\nGEETORUS_INSTANCE_ID=${targetPaths.instanceId}\n`,
      );
      ...
      expect(fs.existsSync(path.join(targetRoot, ".geetorus", "seed-pending"))).toBe(false);
      expect(fs.existsSync(path.join(targetRoot, ".geetorus", "seed-complete"))).toBe(false);
  ...
  it("treats an unregistered markerless config as a normal non-worktree boot", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-unregistered-markerless-"));
    try {
      const configPath = path.join(tempRoot, "config.json");
      fs.writeFileSync(configPath, `${JSON.stringify(buildSourceConfig())}\n`);
      delete process.env.GEETORUS_WORKSPACE_BASE_CWD;
      delete process.env.GEETORUS_PROJECT_WORKSPACE_ID;
      delete process.env.GEETORUS_SEED_EXPECTED_COMPANY_ID;
  ...
  it("honors a legacy complete marker without resolving a seed source", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-complete-marker-"));
    try {
      const configPath = path.join(tempRoot, "config.json");
      fs.writeFileSync(configPath, `${JSON.stringify(buildSourceConfig())}\n`);
  ...
  it("seeds a configured worktree with no seed markers when no legacy database is present", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-unmarked-empty-"));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
  ...
  it("adopts a markerless legacy worktree only after validating its database schema", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-unmarked-legacy-"));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
  ...
  it("managed ensure-seeded derives a valid source from the registered base workspace", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-managed-seed-"));
    try {
      const baseRoot = path.join(tempRoot, "base");
      const sourceConfigPath = path.join(baseRoot, ".geetorus", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
      const targetPaths = resolveWorktreeLocalPaths({
        cwd: targetRoot,
      ...
      fs.mkdirSync(path.dirname(targetConfigPath), { recursive: true });
      fs.writeFileSync(sourceConfigPath, `${JSON.stringify(sourceConfig)}\n`);
      fs.writeFileSync(path.join(path.dirname(sourceConfigPath), ".env"), "GEETORUS_INSTANCE_ID=managed-source\n");
      fs.writeFileSync(targetConfigPath, `${JSON.stringify(targetConfig)}\n`);
      fs.writeFileSync(
        path.join(path.dirname(targetConfigPath), ".env"),
        `GEETORUS_HOME=${targetPaths.homeDir}\nGEETORUS_INSTANCE_ID=managed-target\n`,
      );
      markWorktreeSeedPending({ configPath: targetConfigPath, sourceConfigPath });
      const seedDatabase = vi.fn().mockResolvedValue(mockVerifiedSeedResult());
  ...
  it.each(["sibling", "foreign_instance", "symlink", "instance_mismatch"] as const)(
    ...
    async (variant) => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), `geetorus-worktree-managed-${variant}-`));
      try {
        const baseRoot = path.join(tempRoot, "base");
        const canonicalSource = path.join(baseRoot, ".geetorus", "config.json");
        const targetRoot = path.join(tempRoot, "worktree");
        const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
        ...
        fs.writeFileSync(path.join(path.dirname(canonicalSource), ".env"), "GEETORUS_INSTANCE_ID=registered-source\n");
        fs.writeFileSync(targetConfigPath, `${JSON.stringify(buildSourceConfig())}\n`);
        fs.writeFileSync(
          path.join(path.dirname(targetConfigPath), ".env"),
          `GEETORUS_HOME=${path.join(tempRoot, "worktree-home")}\nGEETORUS_INSTANCE_ID=managed-target\n`,
        );
        fs.writeFileSync(attackerConfig, `${JSON.stringify(buildSourceConfig())}\n`);
        fs.writeFileSync(
          path.join(attackerRoot, ".env"),
          `GEETORUS_INSTANCE_ID=${variant === "foreign_instance" ? "foreign" : "registered-source"}\n`,
        );
        ...
        expect(fs.existsSync(path.join(targetRoot, ".geetorus", "seed.lock"))).toBe(false);
      } finally {
        fs.rmSync(tempRoot, { recursive: true, force: true });
      }
    },
  ...
  it("ensure-seeded records a target shutdown diagnostic when restore fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-ensure-seeded-failure-"));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
  ...
  it("serializes concurrent ensure-seeded calls across the seed marker lock", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-ensure-seeded-lock-"));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
  ...
  it("records an interrupted phase before retrying to a verified terminal state", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-interrupted-seed-"));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".geetorus", "config.json");
      ...
      fs.writeFileSync(
        path.join(targetRoot, ".geetorus", "seed-manifest.json"),
  ...
  it("fails closed instead of racing to reclaim a stale seed lock", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-ensure-seeded-stale-lock-"));
    try {
      const targetConfigPath = path.join(tempRoot, ".geetorus", "config.json");
      const lockPath = path.join(tempRoot, ".geetorus", "seed.lock");
      fs.mkdirSync(path.dirname(targetConfigPath), { recursive: true });
      fs.writeFileSync(
        lockPath,
        `${JSON.stringify({
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("geetorus-worktree-quarantine-");
    onTestFinished(() => tempDb.cleanup());
    ...
    try {
      ...
      await db.insert(projectWorkspaces).values({
        ...
        metadata: {
          ...
          runtimeConfig: {
            workspaceRuntime: { services: [{ name: "geetorus-dev" }] },
      ...
      await db.insert(workspaceRuntimeServices).values({
        ...
        port: 42013,
        url: "https://geetorus-dev.example.test:42013",
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-secrets-"));
    const originalInlineMasterKey = process.env.GEETORUS_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.GEETORUS_SECRETS_MASTER_KEY;
      delete process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.GEETORUS_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.GEETORUS_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.GEETORUS_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          GEETORUS_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.GEETORUS_AGENT_JWT_SECRET;
    const originalToolActionSigningSecret = process.env.GEETORUS_TOOL_ACTION_SIGNING_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.GEETORUS_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.env.GEETORUS_TOOL_ACTION_SIGNING_SECRET = "worktree-tool-action-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".geetorus-worktrees"),
      });

      const envPath = path.join(repoRoot, ".geetorus", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("GEETORUS_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("GEETORUS_TOOL_ACTION_SIGNING_SECRET=worktree-tool-action-secret");
      expect(envContents).toContain("GEETORUS_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/GEETORUS_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.GEETORUS_AGENT_JWT_SECRET;
      } else {
        process.env.GEETORUS_AGENT_JWT_SECRET = originalJwtSecret;
      }
      if (originalToolActionSigningSecret === undefined) {
        delete process.env.GEETORUS_TOOL_ACTION_SIGNING_SECRET;
      } else {
        process.env.GEETORUS_TOOL_ACTION_SIGNING_SECRET = originalToolActionSigningSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves repo-managed worktree checkouts when --force re-runs from the source repo", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-force-preserve-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      const repoConfigDir = path.join(repoRoot, ".geetorus");
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-local-board-seed-"));
      const originalCwd = process.cwd();
      onTestFinished(() => {
        process.chdir(originalCwd);
        fs.rmSync(tempRoot, { recursive: true, force: true });
      ...
      const worktreeHome = path.join(tempRoot, ".geetorus-worktrees");
      const sourceDb = await startEmbeddedPostgresTestDatabase("geetorus-worktree-local-board-source-");
      ...
      const targetConfigPath = path.join(worktreeRoot, ".geetorus", "config.json");
      const targetConfig = JSON.parse(fs.readFileSync(targetConfigPath, "utf8")) as GeetorusConfig;
      expect(readWorktreeSeedManifest(targetConfigPath)).toMatchObject({
        state: "verified",
        phase: "complete",
      });
      ...
      const targetDb = createDb(
        `postgres://geetorus:geetorus@127.0.0.1:${targetConfig.database.embeddedPostgresPort}/geetorus`,
      );
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-auth-seed-"));
      const originalCwd = process.cwd();
      onTestFinished(() => {
        process.chdir(originalCwd);
        fs.rmSync(tempRoot, { recursive: true, force: true });
      ...
      const sourceKeyPath = path.join(sourceConfigDir, "secrets", "master.key");
      const worktreeHome = path.join(tempRoot, ".geetorus-worktrees");
      const sourceCluster = await startEmbeddedPostgresTestDatabase("geetorus-worktree-auth-source-");
      ...
      const targetConfig = JSON.parse(
        fs.readFileSync(path.join(worktreeRoot, ".geetorus", "config.json"), "utf8"),
      ) as GeetorusConfig;
      const manifestText = fs.readFileSync(
        path.join(worktreeRoot, ".geetorus", "seed-manifest.json"),
        "utf8",
      );
      ...
      expect(seededUsers.some((row) => row.email === "existing@geetorus.ing")).toBe(true);
    },
  );

  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".geetorus-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".geetorus", "config.json"), "utf8"));
  ...
  it("reserves distinct ports for postgres-mode siblings under a custom worktree parent", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-custom-parent-"));
    const homeDir = path.join(tempRoot, ".geetorus-worktrees");
    ...
    try {
      ...
      firstConfig.database = {
        ...firstConfig.database,
        mode: "postgres",
        connectionString: "postgres://geetorus:geetorus@127.0.0.1:54330/geetorus",
      };
  ...
  it("defaults the seed source config to the current repo-local Geetorus config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".geetorus", "config.json");
    const originalCwd = process.cwd();
    const originalGeetorusConfig = process.env.GEETORUS_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalGeetorusConfig === undefined) {
        delete process.env.GEETORUS_CONFIG;
      } else {
        process.env.GEETORUS_CONFIG = originalGeetorusConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".geetorus", "config.json");
    const envPath = path.join(worktreeRoot, ".geetorus", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "GEETORUS_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".geetorus", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".geetorus", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Geetorus instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  itEmbeddedPostgres("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".geetorus-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalGeetorusConfig = process.env.GEETORUS_CONFIG;
    ...
    const sourceDb = await startEmbeddedPostgresTestDatabase("geetorus-worktree-reseed-source-");
    onTestFinished(() => sourceDb.cleanup());
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `GEETORUS_HOME=${homeDir}`,
          `GEETORUS_INSTANCE_ID=${currentInstanceId}`,
          "GEETORUS_WORKTREE_NAME=existing-name",
          "GEETORUS_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`GEETORUS_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("GEETORUS_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("GEETORUS_WORKTREE_COLOR=\"#112233\"");
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".geetorus-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `GEETORUS_HOME=${homeDir}\nGEETORUS_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`GEETORUS_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/geetorus",
        targetRepoRoot: "/Users/example/geetorus-pr-432",
        workspaceCwd: "/Users/example/geetorus",
      }),
    ).toBe("/Users/example/geetorus-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/geetorus",
        targetRepoRoot: "/Users/example/geetorus-pr-432",
        workspaceCwd: "/Users/example/geetorus/packages/db",
      }),
    ).toBe("/Users/example/geetorus-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "geetorus-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("geetorus-make-test", {
        seed: false,
        home: path.join(tempRoot, ".geetorus-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".geetorus", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".geetorus", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".geetorus", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".geetorus", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Geetorus metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".geetorus", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".geetorus-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".geetorus-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".geetorus", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".geetorus", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".geetorus", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("geetorus-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
    const projectId = randomUUID();
    const agentId = randomUUID();
