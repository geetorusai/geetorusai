...
import type { Agent, CompanySecret } from "@geetorusai/shared";
...
function configWithSecretsProvider(provider: GeetorusConfig["secrets"]["provider"]): GeetorusConfig {
  return {
    $meta: {
      version: 1,
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/geetorus/db",
      embeddedPostgresPort: 55432,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/geetorus/backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/geetorus/logs",
    },
    server: {
    ...
    secrets: {
      ...
      strictMode: true,
      localEncrypted: {
        keyFilePath: "/tmp/geetorus/secrets/master.key",
      },
    },
  };
}
...
describe("secrets CLI helpers", () => {
  ...
  it("reports the AWS bootstrap config required by doctor", () => {
    ...
    expect(result.message).toContain("GEETORUS_SECRETS_AWS_DEPLOYMENT_ID");
  ...
  it("passes AWS doctor checks when non-secret provider config is present", () => {
    process.env.GEETORUS_SECRETS_AWS_REGION = "us-east-1";
    process.env.GEETORUS_SECRETS_AWS_DEPLOYMENT_ID = "prod-us-1";
    process.env.GEETORUS_SECRETS_AWS_KMS_KEY_ID =
      "arn:aws:kms:us-east-1:123456789012:key/test";
    process.env.AWS_PROFILE = "geetorus-prod";
