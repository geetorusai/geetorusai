...
import type { SecretProvider } from "@geetorusai/shared";
import type { SecretsConfig } from "../config/schema.js";
import { resolveDefaultSecretsKeyFilePath, resolveGeetorusInstanceId } from "../config/home.js";
  ...
  if (provider !== "local_encrypted") {
    p.note(
      provider === "aws_secrets_manager"
        ? "AWS credentials must come from the Geetorus server runtime (IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, or short-lived shell env), not from Geetorus company secrets."
