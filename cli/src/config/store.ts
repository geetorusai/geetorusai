...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".geetorus", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveGeetorusInstanceId());
}
...
export function readConfig(configPath?: string): GeetorusConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = geetorusConfigSchema.safeParse(migrated);
  if (!parsed.success) {
...
function effectiveConfig(config: GeetorusConfig): Record<string, unknown> {
...
export function writeConfig(
  ...
  let nextConfig = geetorusConfigSchema.parse(config);
  if (fs.existsSync(filePath)) {
    try {
      const source = geetorusConfigSchema.parse(migrateLegacyConfig(parseJson(filePath)));
      nextConfig = geetorusConfigSchema.parse(mergeGeetorusConfig(source, nextConfig));
