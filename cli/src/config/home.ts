...
import {
  ...
  resolveGeetorusInstanceRoot as resolveSharedGeetorusInstanceRoot,
...
export function resolveGeetorusInstanceRoot(instanceId?: string): string {
  return resolveSharedGeetorusInstanceRoot({ instanceId });
}

export function resolveDefaultConfigPath(instanceId?: string): string {
  return resolveGeetorusConfigPathForInstance({ instanceId });
}

export function resolveDefaultContextPath(): string {
  return path.resolve(resolveGeetorusHomeDir(), "context.json");
}

export function resolveDefaultCliAuthPath(): string {
  return path.resolve(resolveGeetorusHomeDir(), "auth.json");
}
...
export function describeLocalInstancePaths(instanceId?: string) {
  ...
  const instanceRoot = resolveGeetorusInstanceRoot(resolvedInstanceId);
  return {
    homeDir: resolveGeetorusHomeDir(),
