...
import { resolveGeetorusHomeDir } from "./config/home.js";

export const INSTALL_MANIFEST_VERSION = 1;
export const MANAGED_SHIM_MARKER = "geetorusai managed install shim v1";
export const MANAGED_STORE_MARKER = "geetorusai managed install store v1\n";
export const PATH_BLOCK_START = "# >>> geetorusai managed PATH >>>";
export const PATH_BLOCK_END = "# <<< geetorusai managed PATH <<<";
...
export function resolveInstallStorePaths(options: {
  ...
  const geetorusHome = path.resolve(options.geetorusHome ?? resolveGeetorusHomeDir());
  const homeDir = path.resolve(options.homeDir ?? process.env.HOME ?? path.dirname(geetorusHome));
  const cliRoot = path.join(geetorusHome, "cli");
  return {
    geetorusHome,
    cliRoot,
    ...
    shimPath: path.join(homeDir, ".local", "bin", "geetorusai"),
  };
}
...
export function writeManagedShim(paths = resolveInstallStorePaths()): void {
  ...
  assertManagedShimWritable(paths);
  const entrypoint = path.join(paths.currentPath, "node_modules", "geetorusai", "dist", "index.js");
