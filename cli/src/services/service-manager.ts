...
import { resolveGeetorusHomeDir, resolveGeetorusInstanceId } from "../config/home.js";
...
export function resolveServiceShimPath(homeDir = os.homedir()): string {
  return process.env.GEETORUS_SHIM_PATH?.trim() || path.join(homeDir, ".local", "bin", "geetorusai");
}

// The installed definition, not the current environment, is the truth
// about what the service executes: GEETORUS_SHIM_PATH may have changed
...
export function systemdServiceName(instanceId: string): string {
  return instanceId === "default" ? "geetorusai.service" : `geetorusai-${instanceId}.service`;
}

export function launchdServiceName(instanceId: string): string {
  return instanceId === "default" ? "ing.geetorus.geetorusai" : `ing.geetorus.geetorusai.${instanceId}`;
}

export function renderSystemdUnit(input: { instanceId: string; shimPath: string; homeDir: string }): string {
  return `[Unit]
Description=Geetorus AI (${escapeSystemd(input.instanceId)})
...
Environment="GEETORUS_SERVICE_MANAGED=1"
Environment="GEETORUS_INSTANCE_ID=${escapeSystemd(input.instanceId)}"
Environment="GEETORUS_HOME=${escapeSystemd(input.homeDir)}"
WorkingDirectory=%h
Restart=always
RestartSec=5
TimeoutStopSec=300
...
<dict>
  ...
  <dict>
    <key>GEETORUS_SERVICE_MANAGED</key><string>1</string>
    <key>GEETORUS_INSTANCE_ID</key><string>${escapeXml(input.instanceId)}</string>
    <key>GEETORUS_HOME</key><string>${escapeXml(input.homeDir)}</string>
  </dict>
...
export class SystemdServiceManager implements ServiceManager {
  ...
  constructor(readonly instanceId: string, private readonly runner: CommandRunner = defaultCommandRunner, private readonly homeDir = resolveGeetorusHomeDir(), private readonly shimPath = resolveServiceShimPath(), userHomeDir = os.homedir()) {
...
export async function detectServiceManager(input: { instanceId?: string; platform?: NodeJS.Platform; runner?: CommandRunner } = {}): Promise<ServiceManagerDetection> {
  const instanceId = resolveGeetorusInstanceId(input.instanceId);
  ...
  if (platform !== "linux") return { supported: false, reason: `Service management is not supported on ${platform}. Use geetorusai run instead.` };
  try {
    await runner("systemctl", ["--user", "show-environment"]);
    return { supported: true, manager: new SystemdServiceManager(instanceId, runner) };
  } catch {
    return { supported: false, reason: "No usable systemd user manager was detected (common in containers and WSL1). Use geetorusai run instead." };
  }
}
...
export async function assertForegroundRunAllowed(instanceId: string, force = false, detector: typeof detectServiceManager = detectServiceManager): Promise<void> {
  if (force || process.env.GEETORUS_SERVICE_MANAGED === "1") return;
  const detection = await detector({ instanceId });
  if (!detection.supported) return;
  const status = await detection.manager.status();
  if (status.active) throw new Error(`Geetorus instance '${instanceId}' is already running as ${status.serviceName}. Use 'geetorusai service status --instance ${instanceId}' or pass --force to bypass this safety check.`);
}
