...
const DATA_DIR_OPTION_HELP =
  "Geetorus data directory root (isolates state from ~/.geetorus)";

program.enablePositionalOptions();

program
  .name("geetorusai")
  .description("Geetorus CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);

program
  .command("install")
  .description("Install Geetorus into a managed per-user CLI store")
...
program
  .command("update")
  .alias("upgrade")
  .description("Check, update, or roll back the Geetorus CLI")
...
program.hook("preAction", async (_thisCommand, actionCommand) => {
  ...
  loadGeetorusEnvFile(options.config);
  if (actionCommand.name() === "test-drive") {
...
program
  .command("doctor")
  .description("Run diagnostic checks on your Geetorus setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "geetorus")
...
const run = program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Geetorus")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Geetorus server API")
