...
type ServiceCheckDependencies = {
  ...
  probe: (config: GeetorusConfig) => Promise<HealthResult>;
  shimPresent: (executablePath: string) => Promise<boolean>;
};

async function probeHealth(config: GeetorusConfig): Promise<HealthResult> {
  try {
...
export async function serviceHealthChecks(
  ...
  if (process.env.GEETORUS_SERVICE_MANAGED === "1") return [];
  ...
  results.push(
    definitionCurrent
      ...
      : {
          ...
          repairHint: "Run `geetorusai service install` to regenerate the service definition",
        },
  );
        ...
        ? {
            ...
            repairHint:
              path.resolve(serviceExecutable) === path.resolve(resolveInstallStorePaths().shimPath)
                ? "Run `geetorusai install` to restore the managed payload and shim, then `geetorusai service start`"
                : `Restore the executable at ${serviceExecutable}, or unset GEETORUS_SHIM_PATH and run \`geetorusai install\` followed by \`geetorusai service install\` to re-point the service at the managed shim`,
          }
        : health.ok
          ? {
              name: "Service runtime",
              status: "fail",
              message: `${status.serviceName} is inactive but the configured port is serving another Geetorus process`,
              repairHint: "Run `geetorusai service start`, or stop the conflicting foreground process first",
            }
          : {
              name: "Service runtime",
              status: "fail",
              message: `${status.serviceName} is ${status.detail ?? "inactive"}`,
              repairHint: "Run `geetorusai service start`; inspect `geetorusai service logs` if it does not stay up",
            },
  );
  ...
  results.push(
    !health.ok
      ? {
          ...
          repairHint: "Inspect `geetorusai service status` and `geetorusai service logs`",
        }
      : expectedVersion && health.version !== expectedVersion
        ? {
            ...
            repairHint: "Run `geetorusai service restart --expected-version " + expectedVersion + "`",
          }
        : status.active
          ? {
              name: "Service health",
  ...
  if (status.enabled && status.linger === false) {
    results.push({
      ...
      repairHint: "Re-run `geetorusai service install --enable-linger` if the service must survive logout",
    });
  }
