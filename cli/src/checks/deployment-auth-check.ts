import { inferBindModeFromHost } from "@geetorusai/shared";
import type { GeetorusConfig } from "../config/schema.js";
import type { CheckResult } from "./index.js";

export function deploymentAuthCheck(config: GeetorusConfig): CheckResult {
  ...
  if (mode === "local_trusted") {
    if (bind !== "loopback") {
      return {
        ...
        canRepair: false,
        repairHint: "Run `geetorusai configure --section server` and choose Local trusted / loopback reachability",
      };
    }
    return {
      name: "Deployment/auth mode",
  ...
  const secret =
    ...
    process.env.GEETORUS_AGENT_JWT_SECRET?.trim();
  if (!secret) {
    return {
      name: "Deployment/auth mode",
      status: "fail",
      message: "authenticated mode requires BETTER_AUTH_SECRET (or GEETORUS_AGENT_JWT_SECRET)",
      canRepair: false,
      repairHint: "Set BETTER_AUTH_SECRET before starting Geetorus",
    };
  }
  ...
  if (auth.baseUrlMode === "explicit" && !auth.publicBaseUrl) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `geetorusai configure --section server` and provide a base URL",
    };
  }
  ...
  if (exposure === "public") {
    if (auth.baseUrlMode !== "explicit" || !auth.publicBaseUrl) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `geetorusai configure --section server` and select public exposure",
      };
    }
    try {
      ...
      return {
        ...
        canRepair: false,
        repairHint: "Run `geetorusai configure --section server` and provide a valid URL",
      };
    }
  }
