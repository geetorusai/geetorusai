...
export async function databaseCheck(config: GeetorusConfig, configPath?: string): Promise<CheckResult> {
  if (config.database.mode === "postgres") {
    if (!config.database.connectionString) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `geetorusai configure --section database`",
      };
    }
  ...
  if (config.database.mode === "embedded-postgres") {
    ...
    // flag: this is what happens when GEETORUS_HOME / GEETORUS_IN_WORKTREE leak
    ...
    // ephemeral/CI instances that don't set GEETORUS_IN_WORKTREE are not flagged.)
    // Check BEFORE creating the dir so we don't bootstrap the very temp location
    // we're warning about.
    if (isInsideOsTmpDir(dataDir) && process.env.GEETORUS_IN_WORKTREE === "true") {
      return {
        name: "Database",
        status: "warn",
        message:
          `Embedded PostgreSQL data dir is inside the OS temp directory (${dataDir}) ` +
          "while running in worktree mode (GEETORUS_IN_WORKTREE=true). Data stored here is " +
          "ephemeral and will be lost on reboot or a temp cleanup. If this is your primary " +
          "instance, GEETORUS_HOME / GEETORUS_IN_WORKTREE likely leaked into its environment, " +
          "pointing it at a throwaway worktree home instead of your real data.",
        canRepair: false,
        repairHint:
          "If this is the primary instance, unset GEETORUS_HOME and GEETORUS_IN_WORKTREE " +
