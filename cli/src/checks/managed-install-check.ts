...
export function nodeRuntimeCheck(): CheckResult {
  return isSupportedNodeVersion(process.versions.node)
    ...
    : {
        ...
        repairHint: `Install Node.js ${MINIMUM_NODE_VERSION} or newer before installing or running Geetorus`,
      };
}
    ...
    return [
      {
        ...
        repairHint: "Re-run `geetorusai install` to rebuild the managed install metadata",
      },
    ];
  }
    ...
    return [
      {
        ...
        repairHint: "Re-run `geetorusai install`",
      },
    ];
  }
  ...
  results.push(
    payloadExists && currentMatches
      ...
      : {
          ...
          repairHint: "Re-run `geetorusai install` or roll back to a retained payload",
        },
  );
  ...
  results.push(
    orphaned.length === 0
      ...
      : {
          ...
          repairHint: "A successful `geetorusai update` prunes unretained payloads",
        },
  );
