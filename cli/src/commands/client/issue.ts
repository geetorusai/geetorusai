...
async function uploadAttachment(
  ...
  // This multipart upload uses a hand-rolled fetch rather than GeetorusApiClient,
  ...
  // (the client injects x-geetorus-run-id automatically for JSON requests).
  const headers: Record<string, string> = {};
  if (apiKey) headers.authorization = `Bearer ${apiKey}`;
  if (input.runId) headers["x-geetorus-run-id"] = input.runId;
