...
import { ApiRequestError, GeetorusApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", "Geetorus data directory root (isolates state from ~/.geetorus)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Geetorus API")
    .option("--api-key <token>", "Bearer token for agent-authenticated calls")
    .option("--run-id <id>", "Heartbeat run id for agent-authenticated mutations (checkout/release/interactions/in-progress update); falls back to $GEETORUS_RUN_ID")
...
export function resolveCommandContext(
  ...
  const companyId =
    ...
    process.env.GEETORUS_COMPANY_ID?.trim() ||
    profile.companyId;
  ...
  // in-progress issue) require the X-Geetorus-Run-Id header (the server returns
  // "401 Agent run id required" without it). Source it from --run-id, else the
  // GEETORUS_RUN_ID env the adapter/embodiment context already exports.
  const runId = options.runId?.trim() || process.env.GEETORUS_RUN_ID?.trim() || undefined;

  const api = new GeetorusApiClient({
    apiBase,
    apiKey,
    runId,
...
export function resolveApiBase(options: Pick<BaseClientOptions, "apiBase" | "config">, profile: ClientContextProfile = {}): string {
  return normalizeApiBase(
    ...
    process.env.GEETORUS_API_URL?.trim() ||
    profile.apiBase ||
...
function resolveApiKey(
  ...
  const envValue = process.env.GEETORUS_API_KEY?.trim();
...
export function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.GEETORUS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.GEETORUS_SERVER_PORT || "");
