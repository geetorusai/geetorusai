...
export function registerTeamCommands(program: Command): void {
  ...
  addCommonClientOptions(
    teams
      ...
      .option("--approval-issue-id <id>", "Issue ID to link to the fallback approval request; defaults to GEETORUS_TASK_ID when set")
...
function shouldRequestInstallApproval(error: unknown, opts: TeamInstallOptions): error is ApiRequestError {
  if (!(opts.requestApprovalOnForbidden || isGeetorusTaskRun())) return false;
...
function isGeetorusTaskRun(): boolean {
  return Boolean(process.env.GEETORUS_TASK_ID?.trim());
}
...
async function requestInstallApproval(
  ...
  const payload = {
    ...
    payload: {
      ...
      summary:
        `A Geetorus CLI agent-run attempted to install catalog team "${trimmedRef}" into company "${ctx.companyId}", ` +
...
function resolveApprovalIssueIds(opts: TeamInstallOptions): string[] | undefined {
  const issueId = opts.approvalIssueId?.trim() || process.env.GEETORUS_TASK_ID?.trim();
