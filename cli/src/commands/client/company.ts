...
import { ApiRequestError, type GeetorusApiClient } from "../../client/http.js";
...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isGeetorusYaml = baseName === ".geetorus.yaml" || baseName === ".geetorus.yml";
  const contentType = binaryContentTypeByExtension[path.extname(baseName).toLowerCase()];
  return isMarkdown || isGeetorusYaml || Boolean(contentType) || isBlobStorePath(filePath);
}

function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".geetorus.yaml"] !== undefined) return ".geetorus.yaml";
  if (files[".geetorus.yml"] !== undefined) return ".geetorus.yml";
...
async function promptForImportSelection(preview: CompanyPortabilityPreviewResult): Promise<string[]> {
  ...
  while (true) {
    const choice = await p.select<ImportSelectableGroup | "company" | "confirm">({
      message: "Select what Geetorus should import",
      options: [
        {
          value: "company",
...
export async function uploadCompanyImportTransfer(
  api: Pick<GeetorusApiClient, "post" | "putRaw">,
  zipBytes: Uint8Array,
...
export function registerCompanyCommands(program: Command): void {
          ...
          printOutput(
            {
              ...
              geetorusExtensionPath: exported.geetorusExtensionPath,
  ...
  addCommonClientOptions(
    company
      ...
      .option("--geetorus-url <url>", "Alias for --api-base on this command")
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          if (!opts.apiBase?.trim() && opts.geetorusUrl?.trim()) {
            opts.apiBase = opts.geetorusUrl.trim();
          }
          const ctx = resolveCommandContext(opts);
...
async function createCompanyForContext(ctx: {
  ...
  try {
    ...
    if (isBoardAccessRequiredError(error) || isInstanceAdminRequiredError(error)) {
      throw new Error(
        "Creating companies requires board/instance-admin authentication. Agent API keys are scoped to one company; use `geetorusai company list --json` or `geetorusai company current --json` to select the scoped company, or rerun create with a board token/login.",
      );
    }
    throw error;
  }
...
async function resolveCurrentCompanyId(ctx: { companyId?: string; api: { get<T>(path: string): Promise<T | null> } }): Promise<string> {
  ...
  try {
    ...
    if (error instanceof ApiRequestError && (error.status === 401 || error.status === 403)) {
      throw new Error(
        "Current company is not available. Pass --company-id, set GEETORUS_COMPANY_ID, set a context profile companyId, or authenticate with an agent API key.",
      );
    }
    throw error;
  }
