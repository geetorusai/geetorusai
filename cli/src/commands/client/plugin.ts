...
} from "../../../../packages/plugins/create-geetorus-plugin/src/index.js";
...
/** Result of probing the Geetorus instance the CLI is about to talk to. */
...
function requireCompanyId(ctx: { companyId?: string }): string {
  if (!ctx.companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set GEETORUS_COMPANY_ID, or set context profile companyId via `geetorusai context set`.",
    );
  }
...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; Geetorus watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}

/**
 * Probe `GET /api/health` on the instance the CLI is configured to talk to so a
 * developer can confirm *which* Geetorus they are about to install into. This
...
export function formatTargetDiagnostics(diag: TargetDiagnostics): string {
  const lines = [pc.dim(`Target Geetorus: ${pc.cyan(diag.apiBase)}`)];

  if (!diag.reachable) {
    lines.push(pc.yellow(`  health: unreachable${diag.error ? ` (${diag.error.split("\n")[0]})` : ""}`));
    lines.push(
      pc.dim(
        `  Verify the right instance is running, then pass ${pc.cyan("--api-base <url>")} or set ${pc.cyan("GEETORUS_API_URL")} if it lives elsewhere.`,
      ),
    );
    return lines.join("\n");
  }
...
export function buildPluginInitNextCommands(outputDir: string): string[] {
  ...
  return [
    ...
    "pnpm install",
    "pnpm dev",
    `geetorusai plugin install ${quotedOutputDir}`,
  ];
}
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local Geetorus plugin project")
      ...
      .option("--sdk-path <path>", "Local @geetorusai/plugin-sdk package path")
    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    geetorusai plugin install ./my-plugin              # local path\n" +
          "    geetorusai plugin install @acme/plugin-linear      # npm package\n" +
          "    geetorusai plugin install @acme/plugin-linear@1.2  # pinned version",
      )
      ...
      .option(
        "--no-verify-target",
        "Skip the pre-install probe that reports which Geetorus instance the plugin installs into",
      )
  ...
  addCommonClientOptions(
    plugin
      .command("target")
      .description(
        "Show which Geetorus instance plugin commands will talk to.\n" +
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`geetorusai plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
