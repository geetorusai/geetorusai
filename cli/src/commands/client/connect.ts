...
import type { Agent, Company } from "@geetorusai/shared";
import { createAgentKeySchema, createBoardApiKeySchema } from "@geetorusai/shared";
...
export function registerConnectCommand(program: Command): void {
  addCommonClientOptions(
    program
      ...
      .option("--api-key-env-var-name <name>", "Env var name to store in the profile", "GEETORUS_API_KEY")
...
async function connectWizard(opts: ConnectOptions) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error("`geetorusai connect` is interactive. For scripts, pass --api-base/--api-key or use context set/token commands.");
  }

  p.intro(pc.bgCyan(pc.black(" geetorusai connect ")));
  ...
  const boardLogin = await loginBoardCli({
    ...
    command: "geetorusai connect",
  });
  const boardApi = new GeetorusApiClient({ apiBase, apiKey: boardLogin.token });
  ...
  const apiKeyEnvVarName = opts.apiKeyEnvVarName?.trim() || "GEETORUS_API_KEY";
...
async function verifyHealth(apiBase: string): Promise<void> {
  const api = new GeetorusApiClient({ apiBase });
...
function buildExports(input: {
  ...
  return [
    `export GEETORUS_API_URL='${escaped(input.apiBase)}'`,
    input.companyId ? `export GEETORUS_COMPANY_ID='${escaped(input.companyId)}'` : null,
    input.agentId ? `export GEETORUS_AGENT_ID='${escaped(input.agentId)}'` : null,
