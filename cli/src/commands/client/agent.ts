...
function buildAgentEnvExports(input: {
  ...
  return [
    ...
    `export GEETORUS_COMPANY_ID='${escaped(input.companyId)}'`,
    `export GEETORUS_AGENT_ID='${escaped(input.agentId)}'`,
    `export GEETORUS_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Geetorus skills for Codex/Claude, and print shell exports",
      )
      .argument("<agentRef>", "Agent ID or shortname/url-key")
      .requiredOption("-C, --company-id <id>", "Company ID")
      ...
      .option(
        "--no-install-skills",
        "Skip installing Geetorus skills into ~/.codex/skills, ~/.claude/skills, and ~/.kimi-code/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveGeetorusSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Geetorus skills directory. Expected ./skills in the repo checkout.",
              );
            }
