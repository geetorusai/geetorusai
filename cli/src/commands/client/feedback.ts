...
import type { Company, FeedbackTrace, FeedbackTraceBundle } from "@geetorusai/shared";
import {
...
export async function resolveFeedbackCompanyId(
  ...
  if (!companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set GEETORUS_COMPANY_ID, or configure a CLI context default.",
    );
  }
  return companyId;
}
...
export function renderFeedbackReport(input: {
  ...
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Geetorus Feedback Report")));
...
export function renderFeedbackExportSummary(exported: FeedbackExportResult): string {
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Geetorus Feedback Export")));
