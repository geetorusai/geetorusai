...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" geetorus doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `geetorusai configure --section database` or `geetorusai onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  loadGeetorusEnvFile(input.configPath);
