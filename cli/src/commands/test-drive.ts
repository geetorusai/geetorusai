...
import type { Agent, Company, InstanceExperimentalSettings } from "@geetorusai/shared";
import { GeetorusApiClient } from "../client/http.js";
...
export type TestDriveApi = Pick<GeetorusApiClient, "get" | "post" | "patch" | "delete">;
...
const NON_GEETORUS_ISOLATED_ENV_KEYS = [
  "DATABASE_URL",
...
function requiredApiResult<T>(value: T | null, action: string): T {
  if (value === null) {
    throw new Error(`Geetorus returned no result while ${action}.`);
  }
  return value;
}
...
export function resolveTestDriveDataDir(dataDir?: string): string {
  ...
  return fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-test-drive-"));
}
 ...
 * a GEETORUS_ prefix; all other Geetorus routing/configuration is discarded.
 */
export async function prepareTestDriveEnvironment(
  ...
  for (const key of NON_GEETORUS_ISOLATED_ENV_KEYS) {
    delete process.env[key];
  }
  ...
  process.env.GEETORUS_HOME = dataDir;
  process.env.GEETORUS_INSTANCE_ID = "default";
  process.env.GEETORUS_CONFIG = resolveDefaultConfigPath("default");
  process.env.GEETORUS_CONTEXT = resolveDefaultContextPath();
  process.env.GEETORUS_IN_WORKTREE = linkedWorktree ? "true" : "false";
  process.env.GEETORUS_OPEN_ON_LISTEN = "false";
  process.env.GEETORUS_DISABLE_CWD_ENV_FILE = "true";
  process.env.GEETORUS_DEPLOYMENT_MODE = "local_trusted";
  process.env.GEETORUS_DEPLOYMENT_EXPOSURE = "private";
  process.env.GEETORUS_BIND = "loopback";
...
export function assertTestDriveDatabaseIsolation(
  ...
  readConfigFile: (path?: string) => GeetorusConfig | null = readConfig,
): void {
...
export async function reconcileTestDriveWorktreeExecution(
  ...
  if (!worktreeExecutionArmed(verified, instanceId)) {
    throw new Error(
      `Could not arm “Run tasks in this worktree” for Geetorus instance ${instanceId}. ` +
        "Check that GEETORUS_IN_WORKTREE=true and retry the command.",
    );
  }
}
...
export async function testDriveCommand(
  ...
  dependencies: TestDriveDependencies = {
    run: runCommand,
    createApi: (apiBase) => new GeetorusApiClient({ apiBase }),
    openBrowser: openUrl,
  },
): Promise<void> {
  ...
  const dataDir = path.resolve(process.env.GEETORUS_HOME ?? resolveTestDriveDataDir(options.dataDir));
  const linkedWorktree = process.env.GEETORUS_IN_WORKTREE === "true";
  const instanceId = process.env.GEETORUS_INSTANCE_ID ?? "default";
  ...
  p.log.message(pc.dim("The data directory is retained when Geetorus exits."));
  ...
  try {
    await dependencies.run({
      ...
      introLabel: "geetorusai test-drive",
      afterStart: async (server) => {
        ...
        if (options.browser === false) {
          p.log.success(`Geetorus is ready at ${pc.cyan(url)}.`);
          return;
        }
        const opened = await dependencies.openBrowser(url);
        if (opened) {
          p.log.success(`Geetorus is ready and opened at ${pc.cyan(url)}.`);
        } else {
          p.log.warn(`Geetorus is ready, but the browser could not be opened. Visit ${url}.`);
        }
      },
    });
  } catch (error) {
...
export function registerTestDriveCommand(program: Command): void {
  program
    .command("test-drive")
    .description("Start an isolated, initialized Geetorus instance for manual testing")
    .option("-d, --data-dir <path>", "Geetorus data directory to create or reuse")
