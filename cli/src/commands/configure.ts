...
function defaultConfig(): GeetorusConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" geetorus configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `geetorusai onboard` first.");
    p.outro("");
    process.exitCode = 1;
    return;
  }

  let config: GeetorusConfig;
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because GEETORUS_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
