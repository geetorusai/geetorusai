...
export const CLAUDE_MANAGED_SYSTEM_PROMPT =
  "You are a Geetorus remote agent. Follow the current user turn and use only the custom tools supplied for that session. Geetorus tool authority, completion, blocking, review, and yielding are enforced by the runner. Never request or infer a Geetorus endpoint or credential.";
...
function resourceByProfile(
  ...
  const matches = resources.filter(
    (resource) =>
      ...
      && record(resource.metadata).geetorus_profile === profileKey,
  );
  if (matches.length > 1) {
    throw new Error(
      `Multiple Anthropic ${resourceLabel} resources use Geetorus profile ${profileKey}; pass an explicit resource ID`,
    );
  }
...
export function assertSafeManagedEnvironment(environment: Record<string, unknown>): void {
  ...
  if (
    ...
    throw new Error(
      "Existing Anthropic Environment does not match Geetorus's no-network, no-package profile",
    );
  }
}
...
async function resolveEnvironment(
  ...
  const environment = await anthropicRequest(key, "POST", "/v1/environments", {
    name: `Geetorus · ${options.displayName}`,
    description: "Geetorus remote-agent environment: no network or added packages.",
    config: {
      type: "cloud",
      networking: {
        type: "limited",
    ...
    metadata: { geetorus_profile: options.profileKey },
  });
  assertSafeManagedEnvironment(environment);
  return environment;
}
...
async function resolveAgent(
  ...
  const agent = await anthropicRequest(key, "POST", "/v1/agents", {
    name: `Geetorus · ${options.displayName}`,
    description: "Versioned Geetorus remote agent; runnerd supplies session tools.",
    model: options.model,
