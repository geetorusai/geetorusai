...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@geetorusai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://geetorus:geetorus@127.0.0.1:${port}/geetorus`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" geetorus db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveGeetorusInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "geetorus";
