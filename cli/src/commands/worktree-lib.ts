...
export const DEFAULT_WORKTREE_HOME = "~/.geetorus-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".geetorus");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: GeetorusConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "geetorus",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    GEETORUS_HOME: paths.homeDir,
    GEETORUS_INSTANCE_ID: paths.instanceId,
    GEETORUS_CONFIG: paths.configPath,
    GEETORUS_CONTEXT: paths.contextPath,
    GEETORUS_IN_WORKTREE: "true",
    GEETORUS_DB_BACKUP_ENABLED: "false",
    ...(branding?.name ? { GEETORUS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { GEETORUS_WORKTREE_COLOR: branding.color } : {}),
  };
}
