...
} from "@geetorusai/shared/worktree-seed-source";
import {
  readWorktreePortRegistry,
  withWorktreePortRegistryLock,
  writeWorktreePortRegistry,
} from "@geetorusai/shared/worktree-port-registry";
import {
...
import { ensureAgentJwtSecret, ensureToolActionSigningSecret, loadGeetorusEnvFile, mergeGeetorusEnvEntries, readGeetorusEnvEntries, resolveGeetorusEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.GEETORUS_CONFIG;
...
const WORKTREE_NAME_PREFIX = "geetorus-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.GEETORUS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.GEETORUS_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromGeetorusConfig(config: GeetorusConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromGeetorusConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.geetorus${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".geetorus", "worktrees");
}
...
function collectClaimedWorktreePorts(
  ...
  for (const configPath of registeredConfigPaths) {
    ...
    if (resolvedConfigPath !== path.resolve(cwd, ".geetorus", "config.json") && existsSync(resolvedConfigPath)) {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".geetorus", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.geetorus"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readGeetorusEnvEntries(resolveGeetorusEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.GEETORUS_HOME);
  const instanceId = nonEmpty(envEntries.GEETORUS_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Geetorus instance. Expected GEETORUS_HOME and GEETORUS_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasGeetorusConfig: existsSync(path.resolve(directPath, ".geetorus", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".geetorus", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".geetorus", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".geetorus", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: GeetorusConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://geetorus:geetorus@127.0.0.1:${port}/geetorus`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: GeetorusConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.GEETORUS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.GEETORUS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.GEETORUS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.GEETORUS_SECRETS_MASTER_KEY_FILE) : null);
...
export function requiresWorktreeSeedCredentialAccount(
  deploymentMode: GeetorusConfig["server"]["deploymentMode"],
): boolean {
...
export function resolveWorktreeSeedMigrationRevision(
  ...
  if (
    ...
    throw new Error("Migration journal is not a prefix of this Geetorus checkout's migration journal.");
  }
 ...
 * migration journal and the core Geetorus tables. The physical PG_VERSION
...
export async function inspectLegacyWorktreeDatabase(
  ...
  const envEntries = readGeetorusEnvEntries(resolveGeetorusEnvFile(configPath));
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveGeetorusEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readGeetorusEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "geetorus");
    }
    ...
    const adminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${targetHandle.port}/postgres`;
    await resetPostgresDatabase(adminConnectionString, "geetorus");
    const targetConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${targetHandle.port}/geetorus`;
...
function resolveSeedInstanceId(configPath: string): string {
  const envEntries = readGeetorusEnvEntries(resolveGeetorusEnvFile(configPath));
  return nonEmpty(envEntries.GEETORUS_INSTANCE_ID)
...
export async function ensureWorktreeSeeded(
  ...
  const registeredBaseWorkspaceCwd = opts.registeredBaseWorkspaceCwd
    ?? nonEmpty(process.env.GEETORUS_WORKSPACE_BASE_CWD)
    ?? null;
  ...
  const registeredProjectWorkspaceId = opts.registeredProjectWorkspaceId
    ?? nonEmpty(process.env.GEETORUS_PROJECT_WORKSPACE_ID)
    ?? null;
  const expectedCompanyId = opts.expectedCompanyId
    ?? nonEmpty(process.env.GEETORUS_SEED_EXPECTED_COMPANY_ID)
    ?? nonEmpty(process.env.GEETORUS_COMPANY_ID)
    ?? undefined;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  if (opts.force) {
    ...
    // <repo>/.geetorus/worktrees/ holding every repo-managed worktree
  ...
  const sourceEnvEntries = readGeetorusEnvEntries(resolveGeetorusEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.GEETORUS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.GEETORUS_AGENT_JWT_SECRET);
  const existingToolActionSigningSecret =
    nonEmpty(sourceEnvEntries.GEETORUS_TOOL_ACTION_SIGNING_SECRET) ??
    nonEmpty(process.env.GEETORUS_TOOL_ACTION_SIGNING_SECRET);
  mergeGeetorusEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { GEETORUS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
      ...(existingToolActionSigningSecret ? { GEETORUS_TOOL_ACTION_SIGNING_SECRET: existingToolActionSigningSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  ensureToolActionSigningSecret(paths.configPath);
  loadGeetorusEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Geetorus inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" geetorusai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeEnsureSeededCommand(opts: WorktreeEnsureSeededOptions): Promise<void> {
  printGeetorusCliBanner();
  p.intro(pc.bgCyan(pc.black(" geetorusai worktree ensure-seeded ")));
...
export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printGeetorusCliBanner();
  p.intro(pc.bgCyan(pc.black(" geetorusai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasGeetorusConfig: existsSync(path.resolve(worktreePath, ".geetorus", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printGeetorusCliBanner();
  p.intro(pc.bgCyan(pc.black(" geetorusai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  const configPath = resolveConfigPath(opts.config);
  const envPath = resolveGeetorusEnvFile(configPath);
  const envEntries = readGeetorusEnvEntries(envPath);
  const out = {
    GEETORUS_CONFIG: configPath,
    ...(envEntries.GEETORUS_HOME ? { GEETORUS_HOME: envEntries.GEETORUS_HOME } : {}),
    ...(envEntries.GEETORUS_INSTANCE_ID ? { GEETORUS_INSTANCE_ID: envEntries.GEETORUS_INSTANCE_ID } : {}),
    ...(envEntries.GEETORUS_CONTEXT ? { GEETORUS_CONTEXT: envEntries.GEETORUS_CONTEXT } : {}),
    ...envEntries,
  };
...
export function resolveCurrentWorktreeEndpoint(): ResolvedWorktreeEndpoint {
  ...
  const localConfigPath = path.join(rootPath, ".geetorus", "config.json");
  return {
    rootPath,
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasGeetorusConfig)
      .map((choice) => path.resolve(choice.worktree, ".geetorus", "config.json")),
  ];
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Geetorus once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: GeetorusConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasGeetorusConfig ? "geetorus" : "no-geetorus-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".geetorus", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".geetorus", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .geetorus/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasGeetorusConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Geetorus worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentWorktreeEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasGeetorusConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Geetorus worktrees were found. Run `geetorusai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Geetorus configs are the same. Choose different --from/--to worktrees.");
  }
...
async function backupWorktreeReseedTarget(input: {
  ...
  try {
    const adminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "geetorus");
    const result = await runDatabaseBackup({
      connectionString: `postgres://geetorus:geetorus@127.0.0.1:${targetHandle.port}/geetorus`,
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Geetorus configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Geetorus in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated Geetorus DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
  ...
  try {
    ...
    const seeded = await runVerifiedWorktreeSeed({
      ...
      seedMode,
      preserveLiveWork: opts.preserveLiveWork,
      expectedCompanyId: nonEmpty(process.env.GEETORUS_SEED_EXPECTED_COMPANY_ID) ?? undefined,
      seedDatabase: seedWorktreeDatabase,
    });
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target Geetorus configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readGeetorusEnvEntries(resolveGeetorusEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.GEETORUS_HOME) && nonEmpty(targetEnvEntries.GEETORUS_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Geetorus in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Geetorus instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Geetorus instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with geetorus- if needed (created at ~/geetorus-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: GEETORUS_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: GEETORUS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source GEETORUS_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Geetorus instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Geetorus worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another Geetorus instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source GEETORUS_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local Geetorus instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .geetorus/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: GEETORUS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source GEETORUS_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with geetorus- if needed")
