...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://geetorus:geetorus@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "geetorus");
      const connectionString = `postgres://geetorus:geetorus@127.0.0.1:${embeddedHandle.port}/geetorus`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set GEETORUS_COMPANY_ID.");
  }
