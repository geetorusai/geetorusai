...
export async function channelsCommand(
  ...
  console.log(pc.bold("Geetorus release channels"));
  console.log("");
  for (const entry of state) {
    ...
    console.log(`  ${" ".repeat(8)} ${pc.dim(`npx geetorusai@${entry.distTag} onboard`)}`);
    console.log("");
  }
  ...
  console.log(`Docker images use the same names: ghcr.io/geetorusai/geetorus:{latest,beta,nightly,canary}`);
}
