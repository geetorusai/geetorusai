...
export function resolveEnvLabSshStatePath(instanceId?: string): string {
  const resolvedInstanceId = resolveGeetorusInstanceId(instanceId);
  return path.resolve(
    resolveGeetorusInstanceRoot(resolvedInstanceId),
    "env-lab",
    "ssh-fixture",
    "state.json",
  );
...
export async function envLabDoctorCommand(opts: { instance?: string; json?: boolean }) {
  ...
  //
  // The doctor diagnoses the instance that `resolveGeetorusInstanceId` selects
  // from `opts.instance` or the `GEETORUS_INSTANCE_ID` environment variable.
  // The hint pins that resolved instance, so a contributor who pastes the hint
  // in a shell without `GEETORUS_INSTANCE_ID` stops the diagnosed fixture, not
  // the default instance.
  const cleanupInstance = resolveGeetorusInstanceId(opts.instance);
...
export function registerEnvLabCommands(program: Command) {
  ...
  envLab
    ...
    .option("-i, --instance <id>", "Geetorus instance id (default: current/default)")
