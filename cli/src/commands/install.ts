...
import { isSupportedNodeVersion, MINIMUM_NODE_VERSION } from "@geetorusai/shared/node-version";
import {
  addManagedPathBlock,
...
const DEFAULT_GITHUB_REPO = "geetorusai/geetorus";
...
export function resolveGitInstallWorkspacePackages(checkoutPath: string): ReleasePackageEntry[] {
  ...
  const visit = (packageName: string): void => {
    ...
    for (const section of ["dependencies", "optionalDependencies", "peerDependencies"] as const) {
      ...
      for (const dependencyName of Object.keys(dependencies)) {
        if (dependencyName.startsWith("@geetorusai/")) visit(dependencyName);
      }
    }
  ...
  visit("@geetorusai/server");
  return ordered;
}

export function assertSupportedNodeVersion(): void {
  if (!isSupportedNodeVersion(process.versions.node)) {
    throw new Error(`Installing or updating Geetorus requires Node.js ${MINIMUM_NODE_VERSION} or newer (found ${process.version} at ${process.execPath}). Put a supported Node bin directory first on PATH and run 'npx geetorusai@latest install --yes' to re-pin an existing managed install.`);
  }
}
...
export async function resolvePublishedVersion(spec: string, runCommand: CommandRunner): Promise<string> {
  const result = await runCommand(
    "npm",
    ["view", `geetorusai@${spec}`, "version", "--json", `--registry=${PUBLIC_NPM_REGISTRY}`],
    { maxBuffer: 1024 * 1024 },
  );
...
async function runGitHubCurl(
  ...
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorusai-gh-"));
...
export async function resolveGitHubRef(repo: string, ref: string, runCommand: CommandRunner): Promise<string> {
  const result = await runGitHubCurl(["--fail", "--silent", "--show-error", "--location", "--header", "Accept: application/vnd.github+json", "--header", "User-Agent: geetorusai-install", `https://api.github.com/repos/${repo}/commits/${encodeURIComponent(ref)}`], runCommand, { maxBuffer: 4 * 1024 * 1024 });
  let sha: unknown;
...
function payloadEntrypoint(payloadPath: string): string {
  return path.join(payloadPath, "node_modules", "geetorusai", "dist", "index.js");
}
...
export async function installGitPayload(repo: string, sha: string, runCommand: CommandRunner, paths = resolveInstallStorePaths()): Promise<{ payloadPath: string; reused: boolean; version: string }> {
  ...
  if (fs.existsSync(payloadPath)) {
    const metadata = JSON.parse(fs.readFileSync(path.join(payloadPath, "node_modules", "geetorusai", "package.json"), "utf8")) as { version: string };
  ...
  try {
    ...
    await runCommand("corepack", ["pnpm", "-r", "--filter", "@geetorusai/server...", "--if-present", "run", "build"], { cwd: checkoutPath, env: buildEnv(), maxBuffer: 32 * 1024 * 1024 });
    ...
    for (const [index, workspacePackage] of workspacePackages.entries()) {
      ...
      if (bundledDependencies.length > 0) {
        ...
        await runCommand("corepack", ["pnpm", "--dir", workspacePackage.dir, "pack", "--pack-destination", stagingRoot], { cwd: checkoutPath, env: buildEnv({ GEETORUS_RELEASE_REUSE_UI_DIST: "1" }), maxBuffer: 32 * 1024 * 1024 });
      }
    }
    ...
    const cliTarball = tarballs.find((entry) => entry === `geetorusai-${metadata.version}.tgz`);
...
async function ensureShimOnPath(options: InstallOptions): Promise<void> {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY || !rcPath) {
    console.log(pc.yellow(`Add Geetorus to PATH for this shell:\n  ${manualInstruction}`));
    return;
  }
...
export async function installCommand(
  ...
  if (gitRequest) {
    ...
    console.log(pc.green(`${installed.reused ? "Activated cached" : "Installed"} geetorusai git payload ${sha.slice(0, 12)}.`));
    return;
  }
  const request = resolveNpmInstallRequest(options);
  console.log(`Resolving geetorusai@${request.spec} from ${PUBLIC_NPM_REGISTRY}...`);
  const version = await resolvePublishedVersion(request.spec, runCommand);
  console.log(`Installing geetorusai@${version}...`);
  ...
  console.log(pc.green(`${installed.reused ? "Activated cached" : "Installed"} geetorusai ${version} (${request.channel}).`));
  console.log(pc.dim(`Payload: ${installed.payloadPath}`));
  console.log(`Run ${pc.cyan("geetorusai --version")} to verify the managed install.`);
}
