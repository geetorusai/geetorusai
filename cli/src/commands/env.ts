...
const DEFAULT_AGENT_JWT_ISSUER = "geetorus";
const DEFAULT_AGENT_JWT_AUDIENCE = "geetorus-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveGeetorusInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveGeetorusInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" geetorus env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: GeetorusConfig | null = null;
...
function collectDeploymentEnvRows(config: GeetorusConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrlSource: EnvSource =
    process.env.GEETORUS_PUBLIC_URL
      ? "env"
      : process.env.GEETORUS_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.GEETORUS_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.GEETORUS_SECRETS_STRICT_MODE ??
  ...
  const storageProvider =
    process.env.GEETORUS_STORAGE_PROVIDER ??
  ...
  const storageS3Bucket =
    process.env.GEETORUS_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "geetorus";
  const storageS3Region =
    process.env.GEETORUS_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.GEETORUS_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.GEETORUS_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.GEETORUS_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "GEETORUS_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from GEETORUS_PUBLIC_URL when possible)",
    },
    {
      key: "GEETORUS_AGENT_JWT_TTL_SECONDS",
      value: process.env.GEETORUS_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.GEETORUS_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "GEETORUS_AGENT_JWT_ISSUER",
      value: process.env.GEETORUS_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.GEETORUS_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "GEETORUS_AGENT_JWT_AUDIENCE",
      value: process.env.GEETORUS_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.GEETORUS_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "GEETORUS_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.GEETORUS_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.GEETORUS_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "GEETORUS_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.GEETORUS_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.GEETORUS_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.GEETORUS_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.GEETORUS_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.GEETORUS_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.GEETORUS_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.GEETORUS_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "GEETORUS_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.GEETORUS_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.GEETORUS_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "GEETORUS_CONFIG",
      value: process.env.GEETORUS_CONFIG ?? configPath,
      source: process.env.GEETORUS_CONFIG ? "env" : "default",
      required: false,
