...
type OnboardDefaults = Pick<GeetorusConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or GEETORUS_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "GEETORUS_PUBLIC_URL",
  "DATABASE_URL",
  "GEETORUS_DB_BACKUP_ENABLED",
  "GEETORUS_DB_BACKUP_INTERVAL_MINUTES",
  "GEETORUS_DB_BACKUP_RETENTION_DAYS",
  "GEETORUS_DB_BACKUP_DIR",
  "GEETORUS_DEPLOYMENT_MODE",
  "GEETORUS_DEPLOYMENT_EXPOSURE",
  "GEETORUS_BIND",
  "GEETORUS_BIND_HOST",
  "GEETORUS_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "GEETORUS_ALLOWED_HOSTNAMES",
  "GEETORUS_AUTH_BASE_URL_MODE",
  "GEETORUS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "GEETORUS_STORAGE_PROVIDER",
  "GEETORUS_STORAGE_LOCAL_DIR",
  "GEETORUS_STORAGE_S3_BUCKET",
  "GEETORUS_STORAGE_S3_REGION",
  "GEETORUS_STORAGE_S3_ENDPOINT",
  "GEETORUS_STORAGE_S3_PREFIX",
  "GEETORUS_STORAGE_S3_FORCE_PATH_STYLE",
  "GEETORUS_SECRETS_PROVIDER",
  "GEETORUS_SECRETS_STRICT_MODE",
  "GEETORUS_SECRETS_MASTER_KEY_FILE",
] as const;
...
async function runOnboardedForeground(configPath: string): Promise<void> {
  const previousOpenOnListen = process.env.GEETORUS_OPEN_ON_LISTEN;
  const browserDisabled = parseBooleanFromEnv(process.env.GEETORUS_NO_BROWSER) === true;
  ...
  if (browserDisabled) {
    process.env.GEETORUS_OPEN_ON_LISTEN = "false";
  } else if (interactive && previousOpenOnListen === undefined) {
    process.env.GEETORUS_OPEN_ON_LISTEN = "true";
  }
  ...
  try {
    ...
    if (previousOpenOnListen === undefined) {
      delete process.env.GEETORUS_OPEN_ON_LISTEN;
    } else {
      process.env.GEETORUS_OPEN_ON_LISTEN = previousOpenOnListen;
    }
  }
}
...
function describeServerBinding(server: Pick<GeetorusConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.GEETORUS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.GEETORUS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.GEETORUS_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.GEETORUS_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.GEETORUS_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.GEETORUS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.GEETORUS_ALLOWED_HOSTNAMES
    ? process.env.GEETORUS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.GEETORUS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.GEETORUS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.GEETORUS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.GEETORUS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.GEETORUS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.GEETORUS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.GEETORUS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.GEETORUS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.GEETORUS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.GEETORUS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.GEETORUS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.GEETORUS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.GEETORUS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.GEETORUS_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.GEETORUS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "GEETORUS_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.GEETORUS_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "GEETORUS_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.GEETORUS_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "GEETORUS_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<GeetorusConfig, "database" | "server">): boolean {
...
function printManagedInstallHint(): void {
  ...
  p.log.info(
    `This npx run is temporary. Use ${pc.cyan("geetorusai install")} for atomic updates, rollback, and service support.`,
  );
}
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" geetorusai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveGeetorusInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: GeetorusConfig | null = null;
  ...
  if (configExists(opts.config)) {
    ...
    try {
      ...
      for (const warning of findGeetorusConfigKeyWarnings(existingConfig)) {
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Geetorus install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("geetorusai configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("GEETORUS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.GEETORUS_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("GEETORUS_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("GEETORUS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    const toolActionSigningSecret = ensureToolActionSigningSecret(configPath);
    if (toolActionSigningSecret.created) {
      p.log.success(`Created ${pc.cyan("GEETORUS_TOOL_ACTION_SIGNING_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: GEETORUS_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("geetorusai run")}`,
        `Reconfigure later: ${pc.cyan("geetorusai configure")}`,
        `Diagnose setup: ${pc.cyan("geetorusai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    p.outro("Existing Geetorus setup is ready.");
    return;
  }
  ...
  let llm: GeetorusConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@geetorusai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `geetorusai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("geetorusai run")}`,
        `Then: ${pc.cyan("geetorusai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
