...
import { resolveGeetorusInstanceId, resolveGeetorusInstanceRoot } from "../config/home.js";
...
function hasGeetorusInstanceData(): boolean {
  return Boolean(process.env.DATABASE_URL?.trim())
    || fs.existsSync(resolveConfigPath())
    || fs.existsSync(resolveGeetorusInstanceRoot());
}
...
async function runPreUpdateBackup(options: UpdateOptions, backup: () => Promise<void>, hasInstanceData = hasGeetorusInstanceData): Promise<void> {
  if (!hasInstanceData()) {
    const message = "Skipping the pre-update backup because this Geetorus instance has not been onboarded and has no data to back up.";
  ...
  try {
    ...
    if (isDatabaseUnreachableError(error)) {
      throw new Error(
        "The Geetorus database is not running or reachable, so the pre-update backup cannot be taken. Start the service with `geetorusai service start` and retry, or skip the backup with `geetorusai update --no-backup`.",
        { cause: error },
      );
    }
    throw error;
...
export function detectInstallMode(executablePath = process.argv[1] ?? "", paths = resolveInstallStorePaths()): InstallMode {
  ...
  if (normalized.includes("/node_modules/geetorusai/")) return "global-npm";
...
export async function updateCommand(options: UpdateOptions, overrides: Partial<Dependencies> = {}): Promise<void> {
  ...
  if (options.rollback) {
    ...
    emit(options, { mode, action: "rollback", version: next.version, restarted }, pc.green(`Rolled back to geetorusai ${next.version}${restarted ? " and restarted the active service" : ""}. Database migrations are not reversed; restore the pre-update backup if needed.`));
    return;
  }
  if (mode === "npx") { emit(options, { mode, action: "install" }, "This is an ephemeral npx install. Run `geetorusai install`, then use `geetorusai update` from the managed shim."); return; }
  if (mode === "source" || mode === "unknown") { emit(options, { mode, action: "manual" }, "This appears to be a source checkout. Update it with `git pull` followed by `pnpm install`; Geetorus will not mutate the repository."); return; }
  ...
  if (options.check) { emit(options, { mode, currentVersion: currentVersion ?? null, targetVersion, updateAvailable: comparison > 0, downgrade: comparison < 0, channel: request.channel }, comparison > 0 ? `Update available: ${targetVersion}` : comparison < 0 ? `Target ${targetVersion} is older than ${currentVersion}.` : `geetorusai ${targetVersion} is current.`); if (comparison > 0) process.exitCode = 10; return; }
  if (mode === "global-npm") {
    if (comparison < 0 && options.yes !== true) { const confirmed = await (overrides.confirm ?? defaultConfirm)(`Downgrade geetorusai from ${currentVersion} to ${targetVersion}?`); if (!confirmed) throw new Error("Downgrade cancelled. Re-run with --yes to confirm explicitly."); }
    const args = ["install", "-g", `geetorusai@${targetVersion}`, `--registry=${PUBLIC_NPM_REGISTRY}`, `--@geetorusai:registry=${PUBLIC_NPM_REGISTRY}`]; console.log(`Running: npm ${args.join(" ")}`);
    if (!options.dryRun) {
      const npmConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), "geetorus-npm-"));
      const npmUserConfigPath = path.join(npmConfigDir, "npmrc");
      try {
        fs.writeFileSync(npmUserConfigPath, `registry=${PUBLIC_NPM_REGISTRY}\n@geetorusai:registry=${PUBLIC_NPM_REGISTRY}\n`, { mode: 0o600 });
  ...
  if (comparison === 0) { emit(options, { mode, currentVersion, targetVersion, changed: false }, `geetorusai ${targetVersion} is already active.`); return; }
  if (comparison < 0 && options.yes !== true) { const confirmed = await (overrides.confirm ?? defaultConfirm)(`Downgrade geetorusai from ${currentVersion} to ${targetVersion}?`); if (!confirmed) throw new Error("Downgrade cancelled. Re-run with --yes to confirm explicitly."); }
  if (options.dryRun) { emit(options, { mode, currentVersion, targetVersion, action: comparison < 0 ? "downgrade" : "update", backup: options.backup !== false, dryRun: true }, `Would ${comparison < 0 ? "downgrade" : "update"} geetorusai ${currentVersion} → ${targetVersion}${options.backup === false ? " without a backup" : " after a database backup"}.`); return; }
  if (options.backup !== false) await runPreUpdateBackup(options, overrides.backup ?? (() => dbBackupCommand({})), overrides.hasInstanceData);
  const installed = await withInstallStoreLock(async () => {
    assertManagedShimWritable(paths);
  ...
  emit(options, { mode, currentVersion, targetVersion, changed: true, reused: installed.reused, restarted }, pc.green(`Updated geetorusai ${currentVersion} → ${targetVersion}${restarted ? " and restarted the active service" : ""}. Run \`geetorusai update --rollback\` for an instant payload rollback.`));
}
