...
export function resolveBoardAuthStorePath(overridePath?: string): string {
  ...
  if (process.env.GEETORUS_AUTH_STORE?.trim()) return path.resolve(process.env.GEETORUS_AUTH_STORE.trim());
...
export async function loginBoardCli(params: {
  ...
  const challenge = await requestJson<CreateChallengeResponse>(createUrl, {
    ...
    body: JSON.stringify({
      command,
      clientName: params.clientName?.trim() || "geetorusai cli",
      requestedAccess: params.requestedAccess,
      requestedCompanyId: params.requestedCompanyId?.trim() || null,
    }),
  });

  const publicBase = params.publicBaseUrl?.trim() || process.env.GEETORUS_PUBLIC_URL?.trim();
  ...
  const wantBrowser = params.openBrowser !== false && !isTruthyEnv(process.env.GEETORUS_NO_BROWSER);
