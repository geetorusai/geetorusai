...
export class GeetorusApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-geetorus-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Geetorus API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Geetorus server is not running, the configured URL is wrong, or the request is being blocked before it reaches Geetorus.",
    "",
    "Try:",
    "- Start Geetorus with `pnpm dev` (from a source checkout) or `npx geetorusai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Geetorus is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`GEETORUS_API_URL\`.`,
  );
  return lines.join("\n");
}
