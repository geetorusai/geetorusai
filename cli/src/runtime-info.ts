...
import { resolveGeetorusInstanceRoot } from "./config/home.js";

export const GEETORUS_RUNTIME_INFO_FILENAME = "runtime-info.json";

export type GeetorusRuntimeInfo = {
  schemaVersion: 1;
  instanceId: string;
  pid: number;
  host: string;
...
export function resolveRuntimeInfoPath(instanceId?: string): string {
  return path.join(resolveGeetorusInstanceRoot(instanceId), GEETORUS_RUNTIME_INFO_FILENAME);
}

function parseRuntimeInfo(value: unknown): GeetorusRuntimeInfo | null {
  ...
  return record as GeetorusRuntimeInfo;
}

export function readRuntimeInfo(instanceId?: string, filePath = resolveRuntimeInfoPath(instanceId)): GeetorusRuntimeInfo | null {
  try {
