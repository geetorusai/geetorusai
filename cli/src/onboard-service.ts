...
import { readRuntimeInfo, type GeetorusRuntimeInfo } from "./runtime-info.js";
import {
  readInstallManifest,
...
type OnboardServiceDashboardConfig = {
  auth: Pick<GeetorusConfig["auth"], "baseUrlMode" | "publicBaseUrl">;
  server: Pick<GeetorusConfig["server"], "host" | "port">;
};

type OnboardServiceDashboardDependencies = {
  isInteractive: () => boolean;
  waitUntilReady: () => Promise<GeetorusRuntimeInfo | null>;
...
function envDisablesBrowser(value = process.env.GEETORUS_NO_BROWSER): boolean {
  const normalized = value?.trim().toLowerCase();
  return normalized === "1" || normalized === "true" || normalized === "yes";
}

async function waitUntilDashboardReady(timeoutMs = 60_000): Promise<GeetorusRuntimeInfo | null> {
...
export function resolveOnboardServiceDashboardUrl(
  ...
  runtime?: Pick<GeetorusRuntimeInfo, "dashboardUrl"> | null,
): string {
...
export async function handoffToOnboardedService(
  ...
  deps.info(`Geetorus dashboard: ${pc.cyan(dashboardUrl)}`);

  if (!runtime) {
    deps.warn(
      `The background service started, but the dashboard is not ready yet. ` +
        `Open ${dashboardUrl} after checking \`geetorusai service logs\`.`,
    );
    return;
  }
  ...
  if (await deps.openDashboard(dashboardUrl)) {
    deps.success("Sent the Geetorus dashboard to your browser.");
  } else {
...
const defaultDependencies: OnboardServiceDependencies = {
  ...
  ensureServiceShim: async () => {
    ...
    if (path.resolve(shimPath) !== path.resolve(storeShimPath)) {
      return {
        ok: false,
        installedNow: false,
        reason: `no executable exists at ${shimPath} (GEETORUS_SHIM_PATH), and it is outside the managed install store`,
      };
    }
      ...
      if (manifest?.source === "git" && manifest.repo) {
        ...
        return {
          ...
          reason:
            ...
            "run `geetorusai install` (or `geetorusai install --repo <repo> --ref <ref>` for source builds) first",
        };
      }
    } catch (error) {
      return {
  ...
  confirm: async () => {
    const answer = await p.confirm({
      message: "Install Geetorus as a background service?",
      initialValue: true,
    });
  ...
  confirmLinger: async () => {
    const answer = await p.confirm({
      message: "Allow Geetorus to keep running after logout? This may request system authorization.",
      initialValue: false,
    });
    return !p.isCancel(answer) && answer === true;
  },
...
export async function handleOnboardService(
  ...
  if (!explicitlyRequested && !canPrompt) {
    deps.info(
      "Background service not installed. Use `geetorusai onboard --install-service` or `geetorusai service install` to opt in.",
    );
    return false;
  }
  ...
  if (!shim.ok) {
    deps.warn(
      `Background service not installed: ${shim.reason ?? "the managed install could not be completed"}. ` +
        "Run `geetorusai install`, then `geetorusai service install`.",
    );
    return false;
  }
  if (shim.installedNow) {
    deps.success("Installed the managed geetorusai payload and command shim for the service.");
  }
