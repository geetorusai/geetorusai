...
```sh
curl -sS -H "Authorization: Bearer $GEETORUS_API_KEY" \
  "$GEETORUS_API_URL/api/issues/$GEETORUS_TASK_ID/heartbeat-context"
```
...
If `currentExecutionWorkspace` is `null`, the issue does not currently have a realized execution workspace. For child/follow-up work, create the child with `parentId` or use `inheritExecutionWorkspaceFromIssueId` so Geetorus preserves workspace continuity.

## Control Services

Prefer Geetorus-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.
...
curl -sS -X POST \
  ...
  "$GEETORUS_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
  -H "Content-Type: application/json" \
  "$GEETORUS_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
  -H "Content-Type: application/json" \
  "$GEETORUS_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
When the Geetorus MCP tools are available, prefer these issue-scoped tools:

- `geetorusGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `geetorusControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `geetorusWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
