...
- App-shipped catalog: a curated set of company skills in `@geetorusai/skills-catalog`, browseable and installable without leaving Geetorus.
...
`geetorusai skills` CLI documented in `doc/CLI.md` — it wraps every endpoint
...
```sh
curl -sS "$GEETORUS_API_URL/api/skills/catalog?kind=bundled" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"

curl -sS "$GEETORUS_API_URL/api/skills/catalog/ref?ref=github-pr-workflow" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"

curl -sS -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/skills/install-catalog" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "catalogSkillId": "geetorusai:bundled:software-development:github-pr-workflow"
  }'
```
...
```sh
curl -sS -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/skills/import" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/skills" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```sh
curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"

curl -sS "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```sh
curl -sS -X POST "$GEETORUS_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$GEETORUS_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```sh
curl -sS -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/agents" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Geetorus runtime skills are still added automatically when required by the adapter.
