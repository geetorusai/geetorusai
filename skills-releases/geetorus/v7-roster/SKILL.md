...
There is no dedicated Geetorus tool in your harness. Every Geetorus action is an HTTP request made with `curl` through your shell (`bash`) tool. These rules override any other habit:
...
3. **JSON bodies go through a file, never inline.** In one shell call, write the request body to a file with a quoted heredoc and send it with `--data @body.json`:
   ...
   JSON
   curl -s -X POST "$GEETORUS_API_URL/api/issues/$ISSUE_ID/comments" \
     -H "Authorization: Bearer $GEETORUS_API_KEY" \
     -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
...
**Step 2 — Approval follow-up (when triggered).** If `GEETORUS_APPROVAL_ID` is set (or wake reason indicates approval resolution), the opening of the heartbeat is one **fixed four-step recipe** — no step is optional and the order never varies:
...
```bash
curl -s "$GEETORUS_API_URL/api/approvals/$GEETORUS_APPROVAL_ID" -H "Authorization: Bearer $GEETORUS_API_KEY"
curl -s "$GEETORUS_API_URL/api/approvals/$GEETORUS_APPROVAL_ID/issues" -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```bash
curl -s -X POST "$GEETORUS_API_URL/api/issues/$ISSUE_ID/checkout" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
  -H "Content-Type: application/json" \
  -d "{\"agentId\": \"$GEETORUS_AGENT_ID\", \"expectedStatuses\": [\"todo\", \"backlog\", \"blocked\", \"in_review\"]}"
```
...
If `currentParticipant` does not match you, do not try to advance the stage — Geetorus will reject other actors with `422`. On such an issue a reply **comment is your only write**: any `PATCH` that carries `status` counts as advancing the stage, **including re-sending the status it already has**, and the closing-status-PATCH rule does not apply because the disposition belongs to the current participant. Never write `executionState` through a PATCH body. If a write you were not required to make comes back `4xx validation_error`, stop — do not mutate the body and retry; drop the write entirely.
...
```bash
curl -s -X POST "$GEETORUS_API_URL/api/companies/$GEETORUS_COMPANY_ID/issues/$ISSUE_ID/attachments" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
  -F "file=@report.md"
```
...
**Registering a work product is one POST** — `POST /api/issues/{issueId}/work-products` with the `X-Geetorus-Run-Id` header — never a comment and never a status field. Pick the body by deliverable shape:
...
```bash
curl -s -X POST "$GEETORUS_API_URL/api/issues/<internal-id>/interactions" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
  -H "Content-Type: application/json" --data @body.json \
&& curl -s -X PATCH "$GEETORUS_API_URL/api/issues/<internal-id>" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" -H "X-Geetorus-Run-Id: $GEETORUS_RUN_ID" \
  -H "Content-Type: application/json" \
