...
- Fire externally: `POST /api/routine-triggers/public/{publicId}/fire`
  ...
  - HMAC: `X-Geetorus-Signature` + `X-Geetorus-Timestamp` headers
...
Use the generic API endpoint tables in `skills/geetorus/references/api-reference.md` when you need a full cross-domain reference. Use this file when you need routine-specific behaviour, payload shape, or policy details.
